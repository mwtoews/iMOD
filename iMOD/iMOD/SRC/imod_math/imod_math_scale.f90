...
MODULE MOD_MATH_SCALE
 ...
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:),PRIVATE :: DZ,TZ
...
CONTAINS
 ...
 IF(SIZE(IDFNAMES).GT.1)THEN
  ...
  CALL QKSORT(SIZE(IDFNAMES),MATH%TOP)
...
!    IF(TZ(ILAY).GT.TVALUE(4).OR.TZ(ILAY).LT.BVALUE(4))CYCLE
 ...
 DO ILAY=1,NLAY-1 !## nlay-1 cause last layer is thickness zero or extraction aquifer
  ...
  IF(DH(1).NE.0.0D0)KDX=KDX+TQ(1)/DH(1)  !## x-dir
  IF(DH(2).NE.0.0D0)KDY=KDY+TQ(2)/DH(2)  !## y-dir
 ENDDO
   ...
   DO ICOL=1,NCOL
    ...
    IF(ILAY.LT.NLAY)THEN
     IF(IB(ICOL,IROW,ILAY).NE.0.AND.IB(ICOL,IROW,ILAY+1).NE.0)THEN
      C1=0.0D0; C2=0.0D0
      IF(CC(ICOL,IROW,ILAY).NE.0.0D0)  C1=(0.5*DZ(ILAY  ))/(CC(ICOL,IROW,ILAY)  *VER_FCT)
      IF(CC(ICOL,IROW,ILAY+1).NE.0.0D0)C2=(0.5*DZ(ILAY+1))/(CC(ICOL,IROW,ILAY+1)*VER_FCT)
