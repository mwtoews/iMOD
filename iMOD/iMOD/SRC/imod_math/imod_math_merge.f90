...
CONTAINS
 ...
 REAL(KIND=DP_KIND) :: IDFVAL,TW,TD,MD,X,Y
 ...
 IF(.NOT.IDFOPEN(MATH(NIDF+1)%IU,OUTNAME,'WO',MATH(NIDF+1)%ITYPE,0,1))RETURN
 
 IF(IBATCH.EQ.1)WRITE(*,*)
 
 MATH(NIDF+1)%DMIN= 10.0D10
 MATH(NIDF+1)%DMAX=-10.0D10
 ...
 DO IROW=1,MATH(NIDF+1)%NROW
  ...
  IF(IBATCH.EQ.1)WRITE(6,'(A,F10.2)') '+Progress ',REAL(IROW)/REAL(MATH(NIDF+1)%NROW)*100.0D0
 ENDDO
