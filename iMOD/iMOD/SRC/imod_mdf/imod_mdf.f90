...
CONTAINS
 ...
 !###======================================================================
 LOGICAL FUNCTION MDF_MAIN(IPLOT)
 !###======================================================================
 IMPLICIT NONE
 ...
 ALLOCATE(MDF_ILIST(N+1))
 MDF_ILIST=0
 MDF_ILIST(MP(IPLOT)%NLIDF)=1
 CALL MDF_UPDATE(N)
   ...
   CASE (FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_MENU1)
      CALL MDF_FIELDS(N)
     CASE (IDF_CHECK1)
      CALL WDIALOGGETMENU(IDF_MENU1,MDF_ILIST)
      CALL MDF_UPDATE(N)
    END SELECT
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDOK,IDCANCEL)
      EXIT
     CASE (IDHELP)
      CALL UTL_GETHELP('3.4.4','MMO.GroupIDF')     
    END SELECT
  END SELECT
 ENDDO
 ...
 CALL WDIALOGUNLOAD()
 DEALLOCATE(MDF_ILIST)
 CALL MDFDEALLOCATE()

 IF(MESSAGE%VALUE1.EQ.IDOK) MDF_MAIN=.FALSE.

 END FUNCTION MDF_MAIN
 ...
 IF(I.EQ.0)CALL WDIALOGPUTMENU(IDF_MENU1,MDF%ALIAS,N,MDF_ILIST)
 IF(I.EQ.1)CALL WDIALOGPUTMENU(IDF_MENU1,MDF%FNAME,N,MDF_ILIST)
 ...
 CALL WDIALOGGETMENU(IDF_MENU1,MDF_ILIST)
 I=MIN(1,SUM(MDF_ILIST(1:N)))
 IF(MDF_ILIST(1).EQ.1)I=0
 CALL WDIALOGFIELDSTATE(ID_MOVEUP,I)
 I=MIN(1,SUM(MDF_ILIST(1:N)))
 IF(MDF_ILIST(N).EQ.1)I=0
 ...
 IF(TSTATUS(1:3).EQ.'OLD')THEN
  ...
  IF(.NOT.LEX)THEN
   IF(TSTATUS(4:4).NE.'1')CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot find'//CHAR(13)//TRIM(MDFNAME),'Warning')
   RETURN
  ENDIF
  TSTATUS=TSTAT(1:3)
  ...
  IF(IOS.NE.0)THEN
   ...
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot open'//CHAR(13)//TRIM(MDFNAME)//CHAR(13)// &
 ...
 ELSEIF(TSTATUS(1:7).EQ.'UNKNOWN')THEN
  ...
  IF(IOS.NE.0)THEN
   UTL_GETUNITMDF=0
   CALL OSD_IOSTAT_MSG(IOS,MESSAGE)
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot create'//CHAR(13)//TRIM(MDFNAME)//CHAR(13)// &
   TRIM(MESSAGE),'Warning')
  ENDIF
 ENDIF
