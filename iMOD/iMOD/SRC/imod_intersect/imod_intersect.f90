...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: XMIN,XMAX,YMIN,YMAX,CSX_IN,CSY_IN
...
!    CS=DELC(IMN-1)-DELC(IMN)
 ...
 DO I=N_IN+2,N
  ...
  DX  =XA(I)-XA(I-1)
  DY  =YA(I)-YA(I-1)
  LENG=DX**2.0+DY**2.0; IF(LENG.GT.0.0D0)LENG=SQRT(LENG)
 ...
 DX=X2-X1; DY=Y2-Y1
 IVER=0; IHOR=0
 IF(DX.EQ.0.0D0)IVER=1
 IF(DY.EQ.0.0D0)IHOR=1
 IF(DX.NE.0.0D0)THEN 
  A=DY/DX
  B=Y1-A*X1
 ENDIF
