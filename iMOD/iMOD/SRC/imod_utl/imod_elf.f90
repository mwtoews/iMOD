...
subroutine linregression(N,Xc,Yc,A,B,R2)
!USE lsq
IMPLICIT NONE
REAL(KIND=8),INTENT(OUT) :: A,B,R2
integer,intent(in) :: n
real(KIND=8),intent(in),dimension(n) :: xc,yc
REAL ( kind=8 ) :: x(n,1), y(n), xrow(2), yelem, beta(2), one = 1.D0,VAR,STDEV_RES
INTEGER           :: nvar, i, j, ifault
LOGICAL           :: fit_const, lindep(2)
CHARACTER (LEN=1) :: key
