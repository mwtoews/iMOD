 ...
 ELSE
  ...
  DO I=SIZE(IATTRIB),1,-1
   IF(IATTRIB(I).EQ.1)THEN
    ...
    IF(ILABELNAME.EQ.1)THEN
     IF(LEN_TRIM(ATTRIB(I)).GT.0)THEN
      CALL DBL_WGRTEXTSTRING(XC+DX,YC+(DYS/2.0D0),TRIM(ATTRIB(I))//'= '//STRING(I)(J:),IOFFSET=1)
     ELSE
      CALL DBL_WGRTEXTSTRING(XC+DX,YC+(DYS/2.0D0),STRING(I)(J:),IOFFSET=1)
     ENDIF
    ELSE
     CALL DBL_WGRTEXTSTRING(XC+DX,YC+(DYS/2.0D0),STRING(I)(J:),IOFFSET=1)
    ENDIF
  ...
  CALL DBL_IGRRECTANGLE(XC+DX,Y-(DY/2.0D0),XC+DX+DXS,Y+(DY/2.0D0),IOFFSET=1)
...
! X3=X1+X2
 ...
 IF(JECHO.EQ.1)WRITE(*,'(A,99(I8,A1))') TRIM(TXT)//'=',(IPOINTER(I),',',I=1,NPOINTER)
...
!###=========================================================================
 ...
 DXY =X**2.0+Y**2.0; IF(DXY.GT.0.0D0)DXY=SQRT(DXY)
 DXYZ=X**2.0+Y**2.0+Z**2.0; IF(DXYZ.GT.0.0D0)DXYZ=SQRT(DXYZ)
...
! !###======================================================================
! INTEGER FUNCTION DBL_IGRINSIDEPOLYGON(PX,PY,XX,YY,N,IOFFSET)
! !###======================================================================
! IMPLICIT NONE
! INTEGER,INTENT(IN) :: N
! INTEGER,INTENT(IN),OPTIONAL :: IOFFSET
! REAL(KIND=DP_KIND),INTENT(IN) :: PX,PY
! REAL(KIND=DP_KIND),DIMENSION(:),INTENT(IN) :: XX,YY
! INTEGER :: ND
! REAL(KIND=DP_KIND),DIMENSION(4) :: XXD,YYD 
! 
! DBL_IGRINSIDEPOLYGON=-1
! IF(N.EQ.2)THEN
!  ND=4
!  IOFF=0; IF(PRESENT(IOFFSET))IOFF=IOFFSET
!  XXD(1)=XX(1); YYD(1)=YY(1)
!  XXD(2)=XX(2); YYD(2)=YY(1)
!  XXD(3)=XX(2); YYD(3)=YY(2)
!  XXD(4)=XX(1); YYD(4)=YY(2)
!  IF(DBL_IGRINSIDEPOLYGON(XXD,YYD,ND,PX,PY,IOFFSET=IOFFSET))DBL_IGRINSIDEPOLYGON=1
! ELSE
!  IF(DBL_IGRINSIDEPOLYGON(XX,YY,N,PX,PY,IOFFSET=IOFFSET))DBL_IGRINSIDEPOLYGON=1
! ENDIF
! 
...
! IF(ED.EQ.SD)THEN; F1=F2-F1; F2=0.0D0; ENDIF
 ...
 J=1; I=0; DO !I=1,N
  ...
  IF(SEARCH(J:J).EQ.'*')THEN
   ...
   LEX=.TRUE.; IF(STRING(I:I).EQ.SEARCH(MIN(M,J+1):MIN(M,J+1)))J=MIN(M,J+1)
   !## increase search in string as well as all is good
   IF(J.LE.M)THEN
    DO 
     I=I+1
     IF(ICASE.EQ.0)THEN
  ...
  ENDIF
  IF(.NOT.LEX)EXIT
  IF(J.GT.M)EXIT !LEN_TRIM(SEARCH))EXIT
  IF(I.EQ.N)EXIT
 ENDDO
...
! ENDIF
 ...
 INTEGER :: J
 REAL(KIND=DP_KIND) :: B,A,D,Y
 
 !## line equation
 B=YC(J1); A=(YC(J2)-YC(J1))/(XC(J2)-XC(J1)); MD=-1.0D0; MJ=0
