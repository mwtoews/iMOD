...
! CHARACTER(LEN=52),POINTER,DIMENSION(:,:) :: VAR
 ...
 ELSEIF(ISEL.EQ.2)THEN
  CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,GXG_GENFNAME) !,VAR)
...
! LEX = .TRUE.
 ...
 IF(IOS.EQ.0) THEN
  ...
  WRITE(IU,'(A)') '  802.0      801.0D0    254        32         0    "GT VIIId"'
  WRITE(IU,'(A)') '  801.0D0      800.0D0    254        32         0    "GT VIIIo"'
  WRITE(IU,'(A)') '  800.0D0      700.0D0    254        32         0    "GT VIII"'
  WRITE(IU,'(A)') '  700.0D0      600.0D0    254       211         0    "GT VII"'
  WRITE(IU,'(A)') '  600.0D0      502.0    254       254       124    "GT VI"'
  WRITE(IU,'(A)') '  502.0      501.0D0    170       170        73    "GT Vb"'
  WRITE(IU,'(A)') '  501.0D0      500.0D0     88       127        73    "GT Va"'
  WRITE(IU,'(A)') '  500.0D0      401.0D0     78       137        63    "GT V"'
  WRITE(IU,'(A)') '  401.0D0      400.0D0    170       254        73    "GT IVc"'
  WRITE(IU,'(A)') '  400.0D0      302.0    170       254        73    "GT IVu"'
  WRITE(IU,'(A)') '  302.0      301.0D0     25       254        30    "GT IIIb"'
  WRITE(IU,'(A)') '  301.0D0      300.0D0     25       170        30    "GT IIIa"'
  WRITE(IU,'(A)') '  300.0D0      203.0     25       170        30    "GT III"'
  WRITE(IU,'(A)') '  203.0      202.0     50       254       254    "GT IIc"'
  WRITE(IU,'(A)') '  202.0      201.0D0     50       254       254    "GT IIb"'
  WRITE(IU,'(A)') '  201.0D0      200.0D0     25       170       254    "GT IIa"'
  WRITE(IU,'(A)') '  200.0D0      103.0     25       170       254    "GT II"'
  WRITE(IU,'(A)') '  103.0      101.0D0     88        76       208    "GT Ic"'
  WRITE(IU,'(A)') '  101.0D0      100.0D0     88        76       208    "GT Ia"'
  WRITE(IU,'(A)') '  100.0D0       99.0     88        76       208    "GT I"'
 ENDIF
 CLOSE(IU)
 ...
 !## Technisch Vademecum)
 REAL(KIND=DP_KIND) FUNCTION GXG1GETGT(GLG ,GHG ,NODATA)
 !###======================================================================
 IMPLICIT NONE
