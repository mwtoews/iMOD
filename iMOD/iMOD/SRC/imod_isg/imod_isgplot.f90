...
CONTAINS
 ...
 IF(ISIZE.EQ.0)THEN
  YFRAC=SQRT((XMAX-XMIN)**2.0+(YMAX-YMIN)**2.0)/250.0D0
  DX=(MAXVAL(X)-MINVAL(X))**2.0+(MAXVAL(Y)-MINVAL(Y))**2.0; IF(DX.NE.0.0D0)DX=SQRT(DX)
  IF(YFRAC.GT.DX/5.0)YFRAC=DX/5.0
 ELSE
  YFRAC=SQRT((XMAX-XMIN)**2.0+(YMAX-YMIN)**2.0)/50.0D0
 ENDIF
 ...
 IF(ISIZE.EQ.0)THEN
  YFRAC=SQRT((XMAX-XMIN)**2.0+(YMAX-YMIN)**2.0)/250.0D0
  DX=(MAXVAL(XC)-MINVAL(XC))**2.0+(MAXVAL(YC)-MINVAL(YC))**2.0; IF(DX.NE.0.0D0)DX=SQRT(DX)
  IF(YFRAC.GT.DX/5.0)YFRAC=DX/5.0
 ELSE
  YFRAC=SQRT((XMAX-XMIN)**2.0+(YMAX-YMIN)**2.0)/50.0D0
 ENDIF
...
! CALL DBL_IGRPOLYLINE(XPOS,YPOS,NVERT)
 ...
 CALL DBL_IGRPOLYLINE(XC,YC,4,IOFFSET=1)
...
!Draws a curve which passes through all of the supplied control points
 ...
 IF(ISHAPE.EQ.1)THEN
  ...
  CALL IGRFILLPATTERN(OUTLINE)
  CALL DBL_IGRRECTANGLE(XC-DX,YC-DY,XC+DX,YC+DY,IOFFSET=1)    !  |--|--|
  CALL DBL_IGRJOIN(XC-DX,YC-DY,XC+DX,YC+DY,IOFFSET=1)         !  | /|\ |
  CALL DBL_IGRJOIN(XC-DX,YC+DY,XC+DX,YC-DY,IOFFSET=1)         !  |--|--|
  CALL IGRFILLPATTERN(SOLID)
 !## triangle
 ELSEIF(ISHAPE.EQ.2)THEN
  CALL IGRCOLOURN(ICLRSC)
  CALL DBL_IGRJOIN(XC-2.0*DX,YC+DY,XC+2.0*DX,YC+DY,IOFFSET=1) !\--------/
  CALL DBL_IGRJOIN(XC-2.0*DX,YC+DY,XC-DX,YC-DY,IOFFSET=1)     ! \      /
  CALL DBL_IGRJOIN(XC-DX,YC-DY,XC+DX,YC-DY,IOFFSET=1)         !  \ *  /
  CALL DBL_IGRJOIN(XC+DX,YC-DY,XC+2.0*DX,YC+DY,IOFFSET=1)     !   \--/
 !## structure
 ELSEIF(ISHAPE.EQ.3)THEN
  CALL IGRCOLOURN(ICLRST)
  CALL DBL_IGRJOIN(XC-DX,YC-DY,XC,YC+DY,IOFFSET=1)        !    /\
  CALL DBL_IGRJOIN(XC,YC+DY,XC+DX,YC-DY,IOFFSET=1)        !   /* \
  CALL DBL_IGRJOIN(XC+DX,YC-DY,XC-DX,YC-DY,IOFFSET=1)     !  /----\
 !## qh-relationship
 ELSEIF(ISHAPE.EQ.4)THEN
  CALL IGRCOLOURN(ICLRQH)
  CALL DBL_IGRPOLYGONCOMPLEX((/XC-DX,XC-DX,XC+DX/),(/YC-DY,YC+DY,YC+DY/),3,IOFFSET=1)
 ...
 CALL DBL_IGRPOLYGONCOMPLEX(XPOL,YPOL,4,IOFFSET=1)
