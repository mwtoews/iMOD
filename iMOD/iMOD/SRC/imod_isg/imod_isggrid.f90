...
!   CALL WSORT(FIDATE,1,K,IORDER=ISORT)
   ...
   CALL QKSORT_INT(K,FIDATE,V2=ISORT) !,K,K)
...
!    enddo
 ...
 DO
  ...
  IF(GRIDISG%ISTEADY.EQ.2.AND.GRIDISG%EDATE.LT.GRIDISG%SDATE)THEN
   ...
   ENDIF
  ENDIF
  ISG2GRIDMAIN=ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,0,TOP,BOT,KHV,BND,VCW,IBATCH,MP,0,GRIDISG,SFT,.FALSE.,1) 
   ...
   IF(.NOT.LEX)THEN
    ...
    ELSE
     DO ITRAP=1,NETTRAP(1)
      ...
      TRAP(1,ITRAP)%COND_IN  =1.0D0/(TRAP(1,ITRAP)%COND_IN/(GRIDISG%CS**2.0))
      TRAP(1,ITRAP)%COND_OUT =1.0D0/(TRAP(1,ITRAP)%COND_OUT/(GRIDISG%CS**2.0))
 ...
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NROW,NCOL,NLAY,ILAY,IBATCH,JU,ISYS
 ...
 REAL(KIND=DP_KIND) :: C,INFF,DXY,RWIDTH,WETPER,ISGLEN,AORG,ATRAP,XSTW,YSTW,GSTW,ZCHK
 ...
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: XIN,YIN,XSYM,YSYM
          ...
          IF(GRIDISG%ISIMGRO.EQ.1)THEN
           ...
           DO ITRAP=1,NTRAP
            ...
            IF(NETWD.GT.0.0D0)THEN
             ...
             WRITE(IUSIMGRO,'(2F10.2,2I10,6F10.2,A30,3F10.2)') XC,YC,IROW,ICOL,LN(J),BH,BW,CT,MAX(0.01D0,COND),MAX(0.0D01,COND*ISGVALUE(1,4)), &
...
IRLOOP: DO IR=MAX(1,IROW-1),MIN(NROW,IROW+1)
 ...
 ELSEIF(GRIDISG%IEXPORT.EQ.1)THEN
  CALL ISG2GRID_EXPORTRIVER(JU,IDF,NLAY,ILAY,TOP,BOT,KHV,BND,VCW,MP,GRIDISG,SFT,LSFT,ISYS)
 ENDIF
 ...
 REAL(KIND=DP_KIND),PARAMETER :: MOFFSET=0.0D001  !#3 minimal offset for gradients less or equal zero
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: QSORT,RVAL
 ...
 !###====================================================================
 SUBROUTINE ISG2GRID_EXPORTRIVER(JU,IDF,NLAY,ILAY,TOP,BOT,KHV,BND,VCW,MP,GRIDISG,SFT,LSFT,ISYS)
 !###====================================================================
 IMPLICIT NONE
 TYPE(GRIDISGOBJ),INTENT(INOUT) :: GRIDISG
 INTEGER,INTENT(IN) :: NLAY,ILAY,JU,ISYS
 INTEGER,INTENT(INOUT),DIMENSION(:) :: MP
