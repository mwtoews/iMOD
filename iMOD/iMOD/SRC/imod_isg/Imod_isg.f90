...
USE MOD_MANAGER_UTL
USE MOD_QKSORT
!USE MOD_CREATEIPF, ONLY : UTL_PLOTPOLYGON
...
CONTAINS
 ...
 DO
  ...
  SELECT CASE (ITYPE)
   CASE (MOUSEMOVE)
    CALL WDIALOGPUTDOUBLE(IDF_REAL1,REAL(MESSAGE%GX,8))
    CALL WDIALOGPUTDOUBLE(IDF_REAL2,REAL(MESSAGE%GY,8))
   ...
   ELSE
    ...
    SELECT CASE(ITYPE)
     ...
     CASE(MOUSEMOVE)
      CALL WDIALOGPUTDOUBLE(IDF_REAL1,REAL(MESSAGE%GX,8))
      CALL WDIALOGPUTDOUBLE(IDF_REAL2,REAL(MESSAGE%GY,8))
      XC2=REAL(MESSAGE%GX,8); YC2=REAL(MESSAGE%GY,8)
...
! END SUBROUTINE ISGPROFILENAMES
 ...
 REAL(KIND=DP_KIND) :: H1,H2,DIST,TDIST
 ...
 !## sort on x
 IF(ITOP.GT.0)CALL QKSORT(NTOP,XTOP,V2=YTOP) 
...
! CALL WDIALOGPUTMENU(IDF_MENU1,MP,12,4)
 ...
 DO 
  ...
  SELECT CASE (ITYPE)
   CASE (MOUSEMOVE)
    !## highlight line
    LEX=ISGGETSEGMENT(REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8),0,JJSG,MINDIST,0)
 ...
 DO
  ...
  SELECT CASE (ITYPE)
   ...
   CASE (MOUSEMOVE)
    CALL WINDOWSELECT(0)
    CALL WINDOWOUTSTATUSBAR(1,'X:'//TRIM(ITOS(INT(MESSAGE%GX/1000.0D0)))//' km, Y:'// &
                                    TRIM(ITOS(INT(MESSAGE%GY/1000.0D0)))//' km')
...
!  CALL WDIALOGPUTMENU(IDF_MENU4,(/'Q     ','Width ','Depth ','Factor'/),4,1)
 ...
 IF(ID.EQ.ID_SAVEAS)THEN
  ...
  ELSEIF(ISCN(J).EQ.-1)THEN
   ...
   ICHK=0; IF(DATISC2(J,1)%DISTANCE.LT.0.0D0.AND.DATISC2(J,1)%BOTTOM.LT.0.0D0)ICHK=1
...
!   ENDIF
  ...
  DO I=1,TISC(J)
   CALL DBL_IGRCIRCLE(DATISC2(J,I)%DISTANCE,DATISC2(J,I)%BOTTOM,DX/2.0)
  END DO
  ...
  IF(ISIM.EQ.1)THEN
   ...
   CALL IGRLINEWIDTH(1) 
   DO I=1,NSIM
    CALL DBL_IGRCIRCLE(XSIM(I),YSIM(I),DX/2.0)
   END DO
   CALL IGRLINEWIDTH(1)
...
! YMAX=MAX(YMAX,MAXVAL(DATISQ2(J,1:TISQ(J))%F))
 ...
 DO K=1,TISQ(J)
  ...
  ENDIF
  CALL DBL_IGRCIRCLE(X1,Y1,DX/2.0)
 ENDDO
 CALL IGRCOLOURN(WRGB(255,0,0))
 DO K=1,TISQ(J)
  X1=DATISQ2(J,K)%Q
...
! CALL IDFPLOTFAST(1)
 ...
 REAL(KIND=DP_KIND) :: DX,DY,X1,X2,X3,X4,X5,Y1,Y2,Y3,Y4,Y5,DIST,TDIST,D
...
! CALL WDIALOGFIELDSTATE(IDF_CHECK8,I)
 ...
 IF(ISFR.EQ.0)THEN
  ...
  CALL WGRIDPUTDOUBLE(IDF_GRID1,2,DATISD2(J,:)%WLVL,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,3,DATISD2(J,:)%BTML,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,4,DATISD2(J,:)%RESIS,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,5,DATISD2(J,:)%INFF,TISD(J),'(F15.3)')
 ELSEIF(ISFR.EQ.1)THEN
  CALL WGRIDPUTINTEGER(IDF_GRID1,1,DATISD2(J,:)%IDATE,TISD(J))
  CALL WGRIDPUTSTRING(IDF_GRID1,2,DATISD2(J,:)%CTIME,TISD(J))
  CALL WGRIDPUTDOUBLE(IDF_GRID1,3,DATISD2(J,:)%WLVL ,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,4,DATISD2(J,:)%BTML ,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,5,DATISD2(J,:)%WIDTH,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,6,DATISD2(J,:)%THCK ,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,7,DATISD2(J,:)%HCND ,TISD(J),'(F15.3)')
  ...
  CALL WGRIDPUTDOUBLE(IDF_GRID1,12,DATISD2(J,:)%QFLW,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,13,DATISD2(J,:)%QROF,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,14,DATISD2(J,:)%PPTSW,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,15,DATISD2(J,:)%ETSW,TISD(J),'(F15.3)')
...
!  CALL WGRIDSTATE(IDF_GRID1,11,ISTATE)
 ...
 IF(ISCN(J).EQ.1)THEN
  ...
  CALL WGRIDPUTDOUBLE(IDF_GRID1,1,DATISC2(J,:)%DISTANCE,TISC(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,2,DATISC2(J,:)%BOTTOM,TISC(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,3,DATISC2(J,:)%MRC,TISC(J),'(F15.3)')
 ...
 ELSEIF(ISCN(J).EQ.-1)THEN
  ...
  ELSE
   ...
   CALL WGRIDPUTDOUBLE(IDF_GRID1,2,DATISC2(J,:)%BOTTOM,TISC(J),'(F15.3)')
   CALL WGRIDPUTDOUBLE(IDF_GRID1,3,DATISC2(J,:)%MRC,TISC(J),'(F15.3)')
   CALL WGRIDPUTDOUBLE(IDF_GRID1,4,DATISC2(J,:)%ZP,TISC(J),'(F5.1)')
  ENDIF
 ENDIF
 ...
 CALL WGRIDPUTDOUBLE(IDF_GRID1,2,DATIST2(J,:)%WLVL_UP,TIST(J),'(F15.3)')
 CALL WGRIDPUTDOUBLE(IDF_GRID1,3,DATIST2(J,:)%WLVL_DOWN,TIST(J),'(F15.3)')
 ...
 CALL WGRIDPUTDOUBLE(IDF_GRID1,1,DATISQ2(J,:)%Q,TISQ(J),'(G10.5)')
 CALL WGRIDPUTDOUBLE(IDF_GRID1,2,DATISQ2(J,:)%W,TISQ(J),'(G10.5)')
 CALL WGRIDPUTDOUBLE(IDF_GRID1,3,DATISQ2(J,:)%D,TISQ(J),'(G10.5)')
 CALL WGRIDPUTDOUBLE(IDF_GRID1,4,DATISQ2(J,:)%F,TISQ(J),'(G10.5)')
...
!                                            ,(/0.0D0/),(/0.0D0/),(/0.0D0/)) !DATIST2(J,:)%WLVL_UP,DATIST2(J,:)%WLVL_UP  ,DATIST2(J,:)%WLVL_UP)
 ...
 !call sortem(1,TISQ(J),DATISQ2(J,:)%Q,3,DATISQ2(J,:)%W,DATISQ2(J,:)%D,DATISQ2(J,:)%F,(/0.0D0/) &
!                                        ,(/0.0D0/),(/0.0D0/),(/0.0D0/))
