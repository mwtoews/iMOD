...
!  IF(ID.NE.IDF_REAL1)CALL WDIALOGPUTDOUBLE(IDF_REAL1,XMIN,'(F15.3)')
!  IF(ID.NE.IDF_REAL2)CALL WDIALOGPUTDOUBLE(IDF_REAL2,YMIN,'(F15.3)')
!  IF(ID.NE.IDF_REAL5)CALL WDIALOGPUTDOUBLE(IDF_REAL5,YMAX,'(F15.3)')
!  IF(ID.NE.IDF_REAL6)CALL WDIALOGPUTDOUBLE(IDF_REAL6,XMAX,'(F15.3)')
...
! IF(WINFODIALOGFIELD(IDF_MENU3,FIELDSTATE).EQ.1)CALL WDIALOGGETMENU(IDF_MENU3,IZ)
 ...
 !## sampling
 IF(IINT.EQ.1)THEN
  CALL IDFCOPY(NXIDF(1),XIDF(2))
 !## bivar
 ELSEIF(IINT.EQ.2)THEN
  IF(.NOT.IDFALLOCATESXY(NXIDF(1)))RETURN
 !## kriging
 ELSEIF(IINT.EQ.5.OR.IINT.EQ.6)THEN
  CALL IDFCOPY(NXIDF(1),XIDF(3))
 ENDIF
 ...
 IF(IINT.EQ.1)THEN
  ...
  DO I=1,IPF(1)%NROW
   IF(IPF(1)%XYZ(3,I).NE.NODATA)THEN
    CALL IDFIROWICOL(NXIDF(1),IROW,ICOL,IPF(1)%XYZ(1,I),IPF(1)%XYZ(2,I))
 ...
 ELSEIF(IINT.EQ.2)THEN
  ...
  CALL BIVARIATE_INT(XD,YD,ZD,ND,IERROR,NXIDF(1))
 ...
 ELSEIF(IINT.EQ.3)THEN
  ...
  DO I=1,IPF(1)%NROW
   ...
   ENDIF
  ENDDO
  CALL SOLID_PCGINT(XD,YD,ZD,INT(ND,4),IERROR,NXIDF(1),1,IDUPLICATE)
 ...
 ELSEIF(IINT.EQ.4)THEN
  ...
  DO I=1,IPF(1)%NROW
   ...
   ENDIF
  ENDDO
  CALL KRIGING_VARIOGRAM(INT(ND,4),XD,YD,ZD,I,NXIDF(1),IBATCH=0)
 ...
 ELSEIF(IINT.EQ.5.OR.IINT.EQ.6)THEN
  ...
  CALL KRIGING_MAIN(ND,XD,YD,ZD,NXIDF(1),XIDF(3),MINP,RANGE,SILL,NUGGET,KTYPE,PNTSEARCH,COINCIDENT,COINCIDENTDIST,IQUADRANT,0,0,0.0D0,0)
  IERROR=0
 ...
 IF(IERROR.EQ.0)THEN
  CALL IDFFILLCOMMENT(NXIDF(1),'Units: Unknown'//NEWLINE// &
 ...
 INTEGER :: IRAT,IRAT1,IR,IR1,IR2,IC,IC1,IC2,MX,I,J,N,IROW,ICOL,JL,IOS,IDUPLICATE,IFORCELINE,NL,NV
 ...
 IF(IINT.EQ.1)THEN
  ...
  CALL UTL_GENLABELSREAD(SHPFILE(:INDEX(SHPFILE,'.',.TRUE.)-1)//'.dat',VAR,NL,NV)

  MX=MAXVAL(SHP%POL(1:SHP%NPOL)%N)
  CALL IDFCOPY(NXIDF(1),XIDF(2)); XIDF(2)%NODATA=0.0D0
...
!     WRITE(CID,*) SHPID(SHPI)
     ...
     ENDIF
     READ(VAR(IV,JL-1),*,IOSTAT=IOS) XV
     IF(IOS.NE.0)THEN
      CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot convert '//TRIM(VAR(IV,JL))//' into a real','Error'); RETURN
     ENDIF
    ENDIF
    LEX=.FALSE.
    IF(SHP%POL(SHPI)%ITYPE.EQ.ID_LINE)LEX=.TRUE.
    IF(IFORCELINE.EQ.1)LEX=.TRUE.
        
    !## check whether it is a polygon or a line
    IF(LEX)THEN 
     DO I=1,SHP%POL(SHPI)%N-1
      X1=SHP%POL(SHPI)%X(I); X2=SHP%POL(SHPI)%X(I+1)
      Y1=SHP%POL(SHPI)%Y(I); Y2=SHP%POL(SHPI)%Y(I+1)
      !## intersect line
      ...
      DO J=1,N
       ...
       IF(IC.GE.1.AND.IR.GE.1.AND.IC.LE.XIDF(1)%NCOL.AND.IR.LE.XIDF(1)%NROW)THEN
        XIDF(1)%X(IC,IR)=XIDF(1)%X(IC,IR)+XV*LN(J)
        XIDF(2)%X(IC,IR)=XIDF(2)%X(IC,IR)+LN(J)
       ENDIF
      ENDDO
     ENDDO
     CALL UTL_WAITMESSAGE(IRAT,IRAT1,SHPI,SHP%NPOL,'Line '//TRIM(ITOS(SHPI))//', constructing IDF')
    ELSEIF(SHP%POL(SHPI)%ITYPE.EQ.ID_POLYGON)THEN
     !## proces polygon
     X1 =MINVAL(SHP%POL(SHPI)%X(1:SHP%POL(SHPI)%N)); X2 =MAXVAL(SHP%POL(SHPI)%X(1:SHP%POL(SHPI)%N))
     Y1 =MINVAL(SHP%POL(SHPI)%Y(1:SHP%POL(SHPI)%N)); Y2 =MAXVAL(SHP%POL(SHPI)%Y(1:SHP%POL(SHPI)%N))
     ...
     DO IR=MAX(1,IR1),MIN(IR2,NROW)
      DO IC=MAX(1,IC1),MIN(IC2,NCOL)
       CALL IDFGETLOC(NXIDF(1),IR,IC,XC,YC)
  ...
  DO IC=1,NCOL; DO IR=1,NROW
   IF(XIDF(2)%X(IC,IR).GE.1.0D0)THEN
  ...
  IF(IDFWRITE(NXIDF(1),IDFNAME,1))IERROR=0
...
!     WRITE(CID,*) SHPID(SHPI)
    ...
    DO I=1,SHP%POL(SHPI)%N 
     IF(I.EQ.SHP%POL(SHPI)%N.AND.SHP%POL(SHPI)%ITYPE.EQ.ID_LINE)EXIT
     IF(I.EQ.SHP%POL(SHPI)%N)THEN
      X1=SHP%POL(SHPI)%X(I); X2=SHP%POL(SHPI)%X(1); Y1=SHP%POL(SHPI)%Y(I); Y2=SHP%POL(SHPI)%Y(1)
     ELSE
      X1=SHP%POL(SHPI)%X(I); X2=SHP%POL(SHPI)%X(I+1); Y1=SHP%POL(SHPI)%Y(I); Y2=SHP%POL(SHPI)%Y(I+1)
     ENDIF
     ANGLE=ATAN2(Y2-Y1,X2-X1); MD=(Y2-Y1)**2.0+(X2-X1)**2.0; IF(MD.GT.0.0D0)MD=SQRT(MD); D=0.0D0
     DO
  ...
  SELECT CASE (IINT)
   !## bivar
   CASE(2)
    CALL BIVARIATE_INT(XD,YD,ZD,INT(N,4),IERROR,NXIDF(1))
   !## pcg
   CASE(3)
    J=0; DO I=1,N
     CALL IDFIROWICOL(NXIDF(1),IROW,ICOL,XD(I),YD(I))
     IF(IROW.NE.0.AND.ICOL.NE.0)THEN; J=J+1; XD(J)=REAL(ICOL); YD(J)=REAL(IROW); ZD(J)=ZD(I); ENDIF  
    ENDDO; N=J
    CALL SOLID_PCGINT(XD,YD,ZD,INT(N,4),IERROR,NXIDF(1),1,IDUPLICATE)
   CASE (4)
    CALL KRIGING_VARIOGRAM(INT(N,4),XD,YD,ZD,I,NXIDF(1),IBATCH=0)
   CASE (5,6)
    CALL IDFCOPY(NXIDF(1),XIDF(2)); XIDF(2)%NODATA=HUGE(1.0D0)
     
    XIDF(1)%X=XIDF(1)%NODATA
    CALL KRIGING_MAIN(N,XD,YD,ZD,NXIDF(1),XIDF(2),MINP,RANGE,SILL,NUGGET,KTYPE,PNTSEARCH,COINCIDENT,COINCIDENTDIST,IQUADRANT,0,0,0.0D0,0)
    IERROR=0   
   CASE DEFAULT
  END SELECT
  DEALLOCATE(XD,YD,ZD)
