...
REAL(KIND=DP_KIND),PARAMETER :: XPBND=0.01D0 !## boundary percentage
...
REAL(KIND=DP_KIND),DIMENSION(2) :: PEST_ITARGET
INTEGER,ALLOCATABLE,DIMENSION(:,:) :: PEST_IPERIOD
LOGICAL :: LGRAD,LLNSRCH,LSENS
REAL(KIND=DP_KIND) :: PEST_JSTOP,PEST_SENSITIVITY,PEST_PADJ,PEST_DRES
REAL(KIND=DP_KIND) :: TJOBJ,TJ,PJ    !## objective function value, plausibility value (pj)
TYPE BATCHOBJ
 CHARACTER(LEN=256) :: BATCHFILE,OUTFILE
 REAL(KIND=DP_KIND) :: FRACTION
END TYPE BATCHOBJ
...
TYPE PARAOBJ
 ...
 REAL(KIND=DP_KIND) :: MIN,MAX,INI,FADJ   !## min,max,initial value of parameter
 REAL(KIND=DP_KIND) :: TJOBJ  !## objective function value
 REAL(KIND=DP_KIND),DIMENSION(2) :: ALPHA !## alpha value
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: ALPHA_HISTORY
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: ALPHA_ERROR_VARIANCE
 REAL(KIND=DP_KIND) :: DELTA              !## delta alpha
 ...
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:,:) :: XY  !## xy location for pilot-points
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: X  !## copy of variable to be adjusted
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: F  !## fraction of variable to be adjusted
END TYPE PARAOBJ
TYPE(PARAOBJ),ALLOCATABLE,DIMENSION(:) :: PARAM
TYPE ZONEOBJ
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:,:) :: X,XY
...
REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: W,W_DUMMY
REAL(KIND=DP_KIND),POINTER,DIMENSION(:,:) :: DH,DH_DUMMY
REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: JQJ
REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: JQR,U
REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: QPP
REAL(KIND=DP_KIND) :: MARQUARDT
REAL(KIND=DP_KIND),PARAMETER :: NUGGET=0.0D0, SILL=100.0D0
...
DATA PTYPE/'KD',& ! 1 transmissivity
           ...
           'RE',& !17 recharge
           'EX',& !18 external lognormal distribution
           'GC'/  !19 general head conductance
