...
USE MOD_UTL !, ONLY : UTL_GETUNIT,ITOS,UTL_MESSAGEHANDLE,UTL_WAITMESSAGE,UTL_IDATETOJDATE,UTL_JDATETOIDATE,UTL_DIRINFO,UTL_GDATE,NEWLINE, &
...
CONTAINS
 ...
 REAL(KIND=DP_KIND) :: XC,YC,XVAL,XMIN,YMAX,XMAX,YMIN
 ...
 REAL(KIND=DP_KIND),DIMENSION(1) :: XMED
 ...
 ELSEIF(MEAN_ISEL.EQ.2)THEN
  CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,MEAN_GENFNAME)
 ...
 DO II=1,MAX(1,MEAN_NLAYER)
  ...
  ALLOCATE(MEAN_LISTNAME(NFILES),MEAN_ILIST(NFILES),MEAN_JLIST(NFILES))
  IF(MEAN_NLAYER.EQ.0)THEN
   CALL UTL_DIRINFO(TRIM(MEAN_RESDIR),TRIM(WC),MEAN_LISTNAME,NFILES,'F')
  ELSE
  ...
  IF(MEAN_NLAYER.EQ.0)THEN
   JD1=0; JD2=0
  ELSE
   JD1=UTL_IDATETOJDATE(MEAN_FYR); JD2=UTL_IDATETOJDATE(MEAN_TYR)
  ENDIF
 
  K=0
  L=0
  DO I=1,NFILES
   IF(.NOT.IDFREAD(IDF(I),TRIM(MEAN_RESDIR)//'\'//MEAN_LISTNAME(I),0))THEN
    CALL MEAN1ABORT()
    RETURN
   ENDIF
   IF(MEAN_NLAYER.EQ.0)THEN
   ...
   IF(LEX.AND.MEAN_NYEAR.GT.0)THEN
    ...
    CALL UTL_GDATE(IDF(I)%JD,IY,IM,ID)
    !## check year
   ...
   IF(LEX)THEN
    K       =K+1
    IF(MEAN_NLAYER.EQ.0)THEN; MEAN_ILIST(K)=I
    ELSE; MEAN_ILIST(K)=IDF(I)%JD; ENDIF
    MEAN_JLIST(K)=I
   ELSE
    L=L+1
  ...
  !## sort julian dates
  CALL WSORT(MEAN_ILIST,1,NFILES)
  ...
  CALL TOOLS_UTL_FILLPOINTER(MEAN_ISEL,IDFRP,IDFCP,NIP)
  ...
  SELECT CASE (TRIM(CFUNC))
   CASE ('MEAN','SUM')
    IDF(-1)%X= 0.0D0; IDF(0)%X=  0.0D0
   CASE ('MIN')
    IDF(-1)%X= 0.0D0; IDF(0)%X= 10.0D10
   CASE ('MAX')
    IDF(-1)%X= 0.0D0; IDF(0)%X=-10.0D10
   CASE ('PERC')
  ...
  IF(LEX)THEN
   DO IROW=1,IDF(0)%NROW
    ...
    DO ICOL=1,IDF(0)%NCOL
     IF(IDFCP%X(ICOL,IROW).NE.0.0D0)THEN !IDFCP%NODATA)THEN

      IF(TRIM(CFUNC).EQ.'PERC')IDF(-1)%X(ICOL,IROW)=0.0D0
       ...
       IF(XVAL.NE.IDF(IIDF)%NODATA)THEN
        SELECT CASE (TRIM(CFUNC))
         CASE ('MEAN','SUM')
          ...
          IDF(-1)%X(ICOL,IROW)=IDF(-1)%X(ICOL,IROW)+1.0D0
         CASE ('MIN')
  ...
  ELSE
   DO IROW=1,IDF(0)%NROW
    ...
    IF(IBATCH.EQ.0)THEN
     CALL WINDOWSELECT(0)
     CALL UTL_WAITMESSAGE(IRAT,IRAT1,IROW,IDF(0)%NROW,'Progress NON-Equal IDFs: ')
    ELSE
     WRITE(*,'(1X,A,F10.2,A)') 'Progress NON-Equal IDFs: ',REAL(IROW*100)/REAL(IDF(0)%NROW),'%'
    ENDIF
   ENDDO
  ENDIF
 ...
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: NP
 ...
 ALLOCATE(NP(IDF(0)%NCOL,IDF(0)%NROW)); NP=0.0D0
 ...
 DO I=1,NFILES
  ...
  IF(.NOT.IDFREADSCALE_GETX(IDF(I),IDF(-1),10,1,0.0D0))THEN; CALL MEAN1ABORT(); RETURN; ENDIF
  DO IROW=1,IDF(-1)%NROW; DO ICOL=1,IDF(-1)%NCOL
   IF(IDF(-1)%X(ICOL,IROW).NE.IDF(I)%NODATA)THEN
    SELECT CASE (IOPTION)
     ...
     CASE (3) !## mean
      IDF(0)%X(ICOL,IROW)=IDF(0)%X(ICOL,IROW)+IDF(-1)%X(ICOL,IROW)
      NP(ICOL,IROW)=NP(ICOL,IROW)+1.0D0
     CASE (4)   !## max
 ...
 ENDDO
 DO IROW=1,IDF(-1)%NROW; DO ICOL=1,IDF(-1)%NCOL
  IF(NP(ICOL,IROW).GT.0)THEN 
   IDF(0)%X(ICOL,IROW)=IDF(0)%X(ICOL,IROW)/NP(ICOL,IROW)
  ELSE
   IDF(0)%X(ICOL,IROW)=IDF(0)%NODATA
  ENDIF
 ENDDO; ENDDO
 ...
 IF(ALLOCATED(MEAN_LISTNAME))DEALLOCATE(MEAN_LISTNAME)
 IF(ALLOCATED(MEAN_ILIST))DEALLOCATE(MEAN_ILIST)
 IF(ALLOCATED(MEAN_JLIST))DEALLOCATE(MEAN_JLIST)
