...
CONTAINS
 ...
 IF(.NOT.TS_INIT_IPF(NROW,NCOL))RETURN
 ...
 DO I=1,NROW
  ...
  IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('Error occurred during read of file '//TRIM(IPFNAME1)//'.'); RETURN
  ENDIF

  !## assuming these are the x/y coordinates
  CDUM=STRING(1);READ(CDUM,*,IOSTAT=IOS) X
  IF(IOS.NE.0)THEN; CALL TS_ERROR_MESSAGE('Error reading X-coordinate in IROW '//TRIM(ITOS(I))//' in file '//TRIM(IPFNAME1)//'.'); RETURN; ENDIF
  CDUM=STRING(2); READ(CDUM,*,IOSTAT=IOS) Y
  IF(IOS.NE.0)THEN; CALL TS_ERROR_MESSAGE('Error reading Y-coordinate in IROW '//TRIM(ITOS(I))//' in file '//TRIM(IPFNAME1)//'.'); RETURN; ENDIF
  ...
  IF(IEXT.GT.0)THEN 
   ...
   IF(IOS.NE.0)THEN; CALL TS_ERROR_MESSAGE('Error reading identification in IROW '//TRIM(ITOS(I))//' in file '//TRIM(IPFNAME1)//'.'); RETURN; ENDIF
  ENDIF
  ...
  IF(IOS.NE.0)THEN; CALL TS_ERROR_MESSAGE('Error occurred during writing line '//TRIM(ITOS(I))//' to file '//TRIM(IPFNAME2)//'.'); RETURN; ENDIF
 ...
 IF(IOS.NE.0)THEN; CALL TS_ERROR_MESSAGE('Error while opening file '//TRIM(FNAME)//'. (1)'); RETURN; ENDIF

 !## header
 READ(IU(3),*,IOSTAT=IOS) NDATES
 IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('ERROR READING FILE, line 1 in '//TRIM(FNAME)//'. (2)'); RETURN; ENDIF
 READ(IU(3),*,IOSTAT=IOS) NC
 IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('ERROR READING FILE, line 2 in '//TRIM(FNAME)//'. (2)'); RETURN; ENDIF
 ...
 IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('ERROR READING FILE, line 3 in  '//TRIM(FNAME)//'. (2A)'); RETURN; ENDIF

 READ(IU(3),*,IOSTAT=IOS) CDUM,OBS%NODATA
 IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('ERROR READING FILE, line 4 in  '//TRIM(FNAME)//'. (2B)'); RETURN; ENDIF

 !## skip the rest
 DO I=1,NC-2
  READ(IU(3),*,IOSTAT=IOS)
  IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('ERROR READING FILE, line >4 <ncols in  '//TRIM(FNAME)//'. (2C)'); RETURN; ENDIF
 ENDDO
 ...
 J=0; DO I=1,NDATES
  ...
  IF(IOS.NE.0)THEN; CALL TS_ERROR_MESSAGE('Error reading file in dataline '//TRIM(ITOS(I))//' in file '//TRIM(FNAME)//'. (3)'); RETURN; ENDIF
 ...
 IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('Error while creating file '//TRIM(FNAME)//'. (1)'); RETURN; ENDIF

 !## count number of records to be written
 IF(OBS%NPER.EQ.0)THEN
  NRECS=MSR%NPER
 ELSE
  !## get unique combinations
  ALLOCATE(IDATES(MSR%NPER+OBS%NPER))
  DO I=1,MSR%NPER; IDATES(I)=MSR%IDATE(I); ENDDO
  I=I-1; DO J=1,MSR%NPER; I=I+1; IDATES(I)=OBS%IDATE(J); ENDDO
 ...
 IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('Error while writing to file '//TRIM(FNAME)//'. (2)'); RETURN; ENDIF
 ...
 DO I=1,NFIELDS
  SELECT CASE (I)
   ...
   CASE (3); LINE=TRIM(CF(I))//','//TRIM(RTOS(OBS%NODATA,'G',8))
  END SELECT
 ...
 ELSE
  DO I=1,NRECS
   LINE=TRIM(ITOS_DBL(MSR%IDATE(I)))
   DO J=1,MSR%NPER
    IF(MSR%IDATE(J).EQ.IDATES(I))LINE=TRIM(LINE)//','//TRIM(RTOS(MSR%OBS(J),'G',8)); EXIT
   ENDDO
   DO J=1,OBS%NPER
    IF(OBS%IDATE(J).EQ.IDATES(I))LINE=TRIM(LINE)//','//TRIM(RTOS(OBS%OBS(J),'G',8)); EXIT
   ENDDO
  ENDDO
 ENDIF
 
 CLOSE(IU(3)); IF(ALLOCATED(IDATES))DEALLOCATE(IDATES)
 ...
 !###====================================================================
 LOGICAL FUNCTION TS_INIT_IPF(NROW,NCOL)
 !###====================================================================
 IMPLICIT NONE
 ...
 IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('File '//TRIM(IPFNAME1)//' cannot be opened. (1)'); RETURN; ENDIF

 !## create outputfile
 IU(2) = UTL_GETUNIT()
 CALL OSD_OPEN(IU(2),FILE=IPFNAME2,ACTION='WRITE',IOSTAT=IOS,STATUS='REPLACE')
 IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('File '//TRIM(IPFNAME2)//' cannot be created.'); RETURN; ENDIF
 ...
 IF(IOS.EQ.0) THEN
  ...
  IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('Error occurred during writing to file '//TRIM(IPFNAME2)//'.'); RETURN; ENDIF
  READ(IU(1),*,IOSTAT=IOS) NCOL
  IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE('Error occurred during read of file '//TRIM(IPFNAME1)//'.'); RETURN; ENDIF
  IF(LCOL.GT.NCOL)THEN; CALL TS_ERROR_MESSAGE('LCOL should be maximal equal to NCOL of file '//TRIM(IPFNAME1)//'.'); RETURN; ENDIF
 ...
 J=0; DO I=1,SIZE(IDFNAMES)
  ...
  IF(LEX)THEN
   ...
   IF(IDATE.GE.SDATE.AND.IDATE.LE.EDATE)THEN
    J=J+1; MSR%IDATE(J)=IDATE; MSR%NODATA=IDF(I)%NODATA
   ENDIF
