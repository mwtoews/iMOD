...
USE MOD_UTL, ONLY : ITOS,UTL_DIRINFO,UTL_PLOTLOCATIONIDF,UTL_GETRELEVANTDIR,UTL_CAP,UTL_GETAXESCALES,SXVALUE,SYVALUE,NSX,NSY,  &
                    UTL_JDATETOIDATE,UTL_FILLDATES,JD,UTL_MESSAGEHANDLE,RTOS,JD,JDATETOGDATE,&
                    UTL_GETUNIT,UTL_HIDESHOWDIALOG,UTL_IDATETOJDATE,UTL_DRAWLEGENDBOX,UTL_IDFGETDATES,UTL_IDFGETDATE, &
                    UTL_PLOT1BITMAP,UTL_PLOT2BITMAP,UTL_WSELECTFILE,JDATETOFDATE,UTL_IMODVERSION,ITIMETOGDATE,UTL_JDATETOIDATE, &
                    UTL_GETHELP
USE MOD_IDFGETVALUE, ONLY : IDFGETVALUE_FIELDS
USE MOD_MANAGER_UTL, ONLY : MANAGER_UTL_UPDATE,MANAGER_UTL_CLOSE,MANAGER_UTL_ADDFILE
USE MOD_IDFTIMESERIE_UTL
USE MOD_IDFTIMESERIE_PAR
USE MOD_IPF, ONLY : IPFINIT,IPFDEALLOCATE,IPFREAD
USE MOD_IPFGETVALUE, ONLY : IPFGETVALUE_MOUSEMOVE,ISEL,JIPF,IPFGETVALUE_ADJIP,IPFGETVALUE_PLOTCURRENTPOS
USE MOD_IPFASSFILE, ONLY : IPFDIMENSIONASSFILE,IPFDRAWITOPIC1
...
REAL(KIND=DP_KIND),PRIVATE :: XMIN,XMAX,YMIN,YMAX,Y2MIN,Y2MAX,XINT,YINT,Y2INT  !## graph dimensions
...
CONTAINS
 ...
 REAL(KIND=DP_KIND) :: X,Y,MOUSEX,MOUSEY

 CALL MAIN_UTL_INACTMODULE(ID_TIMESERIES)
 ...
 DO
  ...
  MOUSEX=REAL(MESSAGE%GX,8)+OFFSETX
  MOUSEY=REAL(MESSAGE%GY,8)+OFFSETY
   ...
   CASE (MOUSEMOVE)
      ...
      IF(IDOWN.EQ.1)THEN
       ...
       IF(NIPF.GT.0)THEN
        IF(IFIX.EQ.0)THEN
         ...
         CALL IPFGETVALUE_MOUSEMOVE(MOUSEX,MOUSEY,0,ID_DTIMESERIES)
         !## set new one
       ...
       CALL IDFIROWICOL(IDF(1,1),IROW,ICOL,MOUSEX,MOUSEY)
    ...
    ELSEIF(MESSAGE%WIN.EQ.ID_DTIMESERIESTAB1)THEN
     ...
     IF(NDOWN.GT.0.AND.SUM(NFILES).GT.0)THEN
      IF(IWINDOW.EQ.MESSAGE%WIN)CALL IDFTIMESERIE_PLOTXY(MOUSEX,MOUSEY)
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%WIN)
     ...
     CASE (ID_DTIMESERIES)
      SELECT CASE (MESSAGE%VALUE1)
       CASE (IDHELP)
         CALL UTL_GETHELP('5.2.1','TMO.TT.DrawTS') 
     ...
     CASE (ID_DTIMESERIESTAB1)
      SELECT CASE (MESSAGE%VALUE1)
       ...
       CASE (ID_COPY)
        CALL WCLIPBOARDPUTBITMAP(IDFS_IBITMAP)
       CASE (ID_LEGEND)
...
!    IF(MESSAGE%WIN.EQ.0)CALL IMOD1EXPOSERESIZE()
 ...
 IF(NDOWN.GT.0)THEN
  ...
  CALL UTL_PLOTLOCATIONIDF(IDF(1,1),IROW,ICOL)
  IF(NIPF.GT.0)THEN
   CALL IPFGETVALUE_PLOTCURRENTPOS()
   !## remove previous one - if available
 ...
 DO I=1,NIDF
  DO J=1,MFILES(I)
   ...
   IF(.NOT.IDFOPEN(IDF(I,J)%IU,IDF(I,J)%FNAME,'RO',IDF(I,J)%ITYPE,0,IQUESTION=0))THEN
   ENDIF
  ENDDO
 ENDDO
    ...
    CASE(MOUSEMOVE)
     ...
     ELSEIF(IDZ.EQ.ID_ZOOMBOX)THEN
      ...
      IF(IDOWN.GT.0)THEN
       ...
       CALL IGRSELECT(DRAWBITMAP,IDFS_IBITMAP)
       CALL DBL_IGRAREA(0.0D0,0.0D0,1.0D0,1.0D0)
       CALL DBL_IGRUNITS(GRAPHUNITS(1,1),GRAPHUNITS(2,1),GRAPHUNITS(3,1),GRAPHUNITS(4,1))
       ...
       CALL WDIALOGSELECT(ID_DTIMESERIESTAB1)
       CALL IGRSELECT(DRAWFIELD,IDF_PICTURE1)
       CALL WBITMAPPUT(IDFS_IBITMAP,0,1)
...
! ENDIF
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDOK)
      ...
      EXIT
     CASE (IDHELP)
      CALL UTL_GETHELP('5.2.2','TMO.TT.Leg')  
    END SELECT
   CASE (FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_GRID1)
 ...
 CALL WBITMAPCREATE(IDFS_IBITMAP,IW,IH)
 ...
 DO I=1,NIDF
  ...
  DO J=1,NFILES(I)
   ...
   IF(ICOL.NE.0.AND.IROW.NE.0)THEN
    TS(I)%VALUE(J)=IDFGETVAL(IDF(I,J),IROW,ICOL,LTYPE(I)%UNITS)
   ELSE
    !## all nodata value
  ...
  DO I=1,NIDF
   DO J=1,NFILES(I)
    IF(IDF(I,J)%JD.NE.0)THEN
     ...
     IF(IDF(I,J)%DAYFRACTION.GT.0.0D0)TS(I)%IDATE(J)=TS(I)%IDATE(J)+IDF(I,J)%DAYFRACTION
 ...
 DO I=1,NIDF
  !call sortem(1,NFILES(I),TS(I)%VALUE,1,TS(I)%IDATE,(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/))
  CALL QKSORT(NFILES(I),TS(I)%VALUE,V2=TS(I)%IDATE)
  DO J=1,NFILES(I)
   TS(I)%IDATE(J)=100.0D0*REAL(J)/REAL(NFILES(I))
  END DO
 ENDDO

 IF(NIPF.GT.0)THEN
  DO I=1,SIZE(TSIPF)
   !call sortem(1,SIZE(TSIPF(I)%IDATE),TSIPF(I)%VALUE,1,TSIPF(I)%IDATE,(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/))
  CALL QKSORT(SIZE(TSIPF(I)%IDATE),TSIPF(I)%VALUE,V2=TSIPF(I)%IDATE)
   DO J=1,SIZE(TSIPF(I)%IDATE)
    TSIPF(I)%IDATE(J)=100.0D0*REAL(J)/REAL(SIZE(TSIPF(I)%IDATE))
   END DO
  END DO
 ENDIF
  ...
  DO I=2,SIZE(RTIME)
   IF(ABS(RTIME(I)-RTIME(I-1)).GT.0.5)THEN
    ...
    IF(II.EQ.2)THEN
     ...
     FRAC=(TSDIFF(1)%IHR(N)*24*60*60+TSDIFF(1)%IMT(N)*60+TSDIFF(1)%ISC(N))/86400.0D0
 ...
 IF(IFIXX.EQ.0)THEN
  ...
  IF(IDURATION.EQ.0)THEN
   XMIN=MAX(0.0D0,XMIN)
   XMAX=MIN(REAL(MAXDATE-MINDATE+1),XMAX)
  ENDIF
 ...
 !## check axes
 Y(1,1)=10.0D10; Y(2,1)=-10.0D10
 Y(1,2)=10.0D10; Y(2,2)=-10.0D10
 DO IAXE=1,2
  ...
  IF(IFXY(IAXE).EQ.0)THEN 
   ...
   !## flat line
   IF(ABS(Y(2,IAXE)-Y(1,IAXE)).LE.0.01D0)THEN; Y(1,IAXE)=Y(1,IAXE)-0.01D0; Y(2,IAXE)=Y(2,IAXE)+0.01D0; ENDIF
 ...
 REAL(KIND=DP_KIND) :: DX,DY,CHH,BOXX,OFFX,SX_RATIO,Y,X1,X2,Y1,Y2
 CHARACTER(LEN=100) :: EXTRAINFO
 ...
 CALL DBL_WGRTEXTSTRING(XMAX-OFFX,YMIN+0.75*DY,'[x='//TRIM(RTOS(XC,'F',3))//' ; y='//TRIM(RTOS(YC,'F',3))//']')
 ...
 DO I=1,NIDF
  ...
  CALL DBL_WGRTEXTSTRING(XMIN+OFFX+BOXX,Y-0.5*DY,' '//TRIM(LEGENDNAME(I)))
  ...
  IF(LTYPE(IROW)%IAXES.EQ.1)CALL DBL_IGRUNITS(XMIN, YMIN,XMAX, YMAX)
  IF(LTYPE(IROW)%IAXES.EQ.2)CALL DBL_IGRUNITS(XMIN,Y2MIN,XMAX,Y2MAX)

  !## time constant 
  IF(NFILES(I).EQ.1)THEN
   IF(TS(I)%VALUE(1).NE.IDF(I,1)%NODATA)THEN
    CALL DBL_IGRMOVETO(XMIN,TS(I)%VALUE(1))
    CALL DBL_IGRLINETO(XMAX,TS(I)%VALUE(1))
   ENDIF
  ELSE
 ...
 IF(NIPF.GT.0)THEN
  ...
  DO I=1,SIZE(TSIPF)
   ...
   CALL DBL_WGRTEXTSTRING(XMIN+(2.0*OFFX)+BOXX,Y-0.5*DY,TRIM(IPF(JIPF)%ALIAS)//':'//TRIM(ASSF(1)%ATTRIB(I+1))//' '//TRIM(EXTRAINFO))
   X1=XMIN+OFFX
   Y1=Y-DY+0.1*DY
   X2=XMIN+OFFX+BOXX
   Y2=Y-0.1*DY
 ...
 IF(IRESIDUAL.EQ.1)THEN
  ...
  !## plot axes-text
  CALL IGRCOLOURN(WRGB(0,0,0))
  CALL DBL_WGRTEXTSTRING(XMIN+(2.0*OFFX)+BOXX,Y-0.5*DY,'Difference')
  X1=XMIN+OFFX
  Y1=Y-DY+0.1*DY
  X2=XMIN+OFFX+BOXX
  Y2=Y-0.1*DY
 ...
 !## continuous
 IF(ISTYLE.EQ.1)THEN
  LEX=.FALSE.; X2=-HUGE(1.0D0)
  DO I=1,N
   IF(Y(I).NE.NODATA)THEN
    IF(X(I).GT.X2)THEN
     IF(.NOT.LEX)CALL DBL_IGRMOVETO(X(I),Y(I))
     IF(LEX)CALL DBL_IGRLINETO(X(I),Y(I))
     LEX=.TRUE.
    ELSE
 ...
 ELSEIF(ISTYLE.EQ.2)THEN
  ...
  DO I=1,N
   IF(Y(I).NE.NODATA)THEN
    IF(X(I).GT.X2)THEN
     IF(.NOT.LEX)CALL DBL_IGRMOVETO(X(I),Y(I))
     IF(LEX)THEN; CALL DBL_IGRLINETO(X(I),Y(I-1)); CALL DBL_IGRLINETO(X(I),Y(I)); ENDIF
     LEX=.TRUE.
    ELSE
     !## stop if times are not sequentially
     EXIT
