...
! END SUBROUTINE POLYGON_UTL_GETLABELSGEN
 ...
 INTEGER :: I,J,IOS,MAXCOL,N,STRLEN,MAXPOL,ITYPE,JCLR
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IVAR
 REAL(KIND=DP_KIND) :: FROMX,FROMY,TOX,TOY,TWIDTH,THEIGHT,X,XMID,YMID
...
!   !## no labels (yet)
!   WRITE(JU) MAXPOL,MAX(1,MAXCOL)
!   IF(MAXCOL.GT.0)THEN
!!    WRITE(JU) ICOLWIDTH
!!    WRITE(JU) LABELS (LEN=11)
!   ELSE
!    WRITE(JU) 11
!    LABEL='ID'
!    WRITE(JU) LABEL
...
! END SUBROUTINE POLYGON_UTL_CONVERTGEN
 ...
 MP(IPLOT)%TSIZE=MAX(1,MP(IPLOT)%TSIZE,1)
