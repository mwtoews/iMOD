...
CONTAINS
 ...
 !###====================================================================
 SUBROUTINE LEGPLOT_PLOT(LEG,NC,LTYPE,TSIZE,ISYMBOL)
 !###====================================================================
 IMPLICIT NONE
 ...
 REAL(KIND=DP_KIND) :: X1,X2,Y1,Y2,OFFX,RAT,YLINE,TLINE,TXTSIZE,CHW_M,CHH_M
...
! i=winfoscreen(ScreenHeight)
 ...
 ELSE
  ...
  TDY=(Y2-Y1)/REAL(NY) 
  IF(IGRY.EQ.0)THEN
   TXTSIZE=(Y2-Y1)/REAL(TINTERVAL*2,8)
   TXTSIZE=TXTSIZE
  ELSE
   TXTSIZE=TDY*(1.0D0/(Y2-Y1))
  ENDIF
  CALL UTL_SETTEXTSIZE(CHW,CHH,FCT=TXTSIZE/0.0033)
 ENDIF
...
!  Y2=Y2-TDY
 ...
 BXY =TDY
 BXX =BXY/RAT
 BXX =MAX(BXX,0.1D0*(X2-X1)/REAL(NC,8))
 ...
 DO I=1,LEG%NCLR
  ...
  SELECT CASE (LLTYPE)
   ...
   !## dots
   CASE (2)
    CALL UTL_SETTEXTSIZE(CHW_M,CHH_M,FCT=TXTSIZE/0.0033,IMARKER=1)
    CALL DBL_WGRTEXTFONT(IFAMILY=0,TWIDTH=CHW_M,THEIGHT=CHH_M,ISTYLE=0)
    CALL DBL_IGRMARKER(X,Y,iSYMBOL) !,IOFFSET=1)
    CALL DBL_WGRTEXTFONT(IFAMILY=TFONT,TWIDTH=CHW,THEIGHT=CHH,ISTYLE=0)
!    CALL DBL_IGRCIRCLE(X+0.5D0*BXX,Y-0.5D0*BXY,) !X+BXX,Y-BXY)
   !## lines
   CASE (3)
