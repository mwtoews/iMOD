...
USE MOD_UTL, ONLY : UTL_GETUNIT,DBL_IGRINSIDEPOLYGON,ITOS,UTL_CREATEDIR,UTL_CAP
USE MOD_PLINES_PAR
USE MOD_SPOINTS_PAR
...
! SP%MXL =SP%SLAY
 ...
 REAL(KIND=DP_KIND) :: XC,YC,ZC,GRAD2RAD,DIFFX,DIFFY,TL,DL,XTOP,XBOT,XREF
 ...
 IF(ALLOCATED(XCRD))THEN
  ...
  XCMIN=INT(XCMIN)-1.0D0; XCMAX=INT(XCMAX)+1.0D0 
  YCMIN=INT(YCMIN)-1.0D0; YCMAX=INT(YCMAX)+1.0D0 
 ENDIF
  ...
  CASE (ID_POLYGON)
   ...
   DO
    ...
    DO
     XC=XC+SPNT(I)%IDX
     IF(XC.GT.XCMAX)EXIT
     IF(DBL_IGRINSIDEPOLYGON(XC,YC,XCRD,YCRD,NCRD).EQ.1)CALL TRACEASSIGNSP(NPART,XC,YC,XTOP,XBOT,XREF,-1.0D0)
    ENDDO
   ENDDO
  ...
  CASE (ID_POINT,ID_CIRCLE)
   ...
   DO J=1,NCRD
    ...
    DO K=1,NOR 
     ...
     CALL TRACEASSIGNSP(NPART,XCRD(J)+DIFFX,YCRD(J)+DIFFY,XTOP,XBOT,XREF,-1.0D0)
    END DO
