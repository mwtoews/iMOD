...
USE MOD_UTL !, ONLY : JD,UTL_GETUNIT,ITOS,RTOS,UTL_MESSAGEHANDLE,UTL_GDATE,UTL_JDATETOIDATE,UTL_IDATETOJDATE,UTL_DIRINFO, &
     !UTL_IMODFILLMENU,UTL_WSELECTFILE,UTL_CLOSEUNITS,UTL_CREATEDIR,UTL_GETHELP
USE DATEVAR
USE MOD_PLINES_PAR
...
!  CALL WDIALOGSELECT(MESSAGE%WIN)
 ...
 FRAC=0.99
 IF(ISNK.EQ.2)CALL WDIALOGGETDOUBLE(IDF_REAL1,FRAC)
 FRAC=MIN(0.99,MAX(0.0D0,FRAC))
 ...
 IF(I.EQ.1)THEN
  CALL WDIALOGGETDOUBLE(IDF_REAL1,TMAX)
 ...
 IF(IDB.EQ.IDOK)THEN
  ...
  IF(TRACEMAIN(RUNFNAME,0,0,ZOOMIDF))THEN 
   DO I=1,2
    IF(IMODE(I).EQ.1)THEN
     CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(MDLDIRNAME)//'\'//TRIM(FNAME)//'.'//TRIM(EXT(I)))
     !call idfplotfast()       
    ENDIF
   ENDDO
  ENDIF
