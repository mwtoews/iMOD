...
!          Linear A-DIR UNDEF SEGMENT FALSE
  ...
  IF(DEVWELL_IMPORT_MULTIPLE_FILE2(FNAME(2)))THEN
   IF(DEVWELL_IMPORT_MULTIPLE_FILE3(FNAME(3)))THEN
    IF(DEVWELL_IMPORT_MULTIPLE_FILE4(FNAME(4)))THEN
     CALL DEVWELL_IMPORT_MULTIPLE_WRITE(LEN_TRIM(FNAME(3)),LEN_TRIM(FNAME(4)))
    ENDIF
   ENDIF
  ENDIF
 ENDIF
 ...
 !###======================================================================
 SUBROUTINE DEVWELL_IMPORT_MULTIPLE_WRITE(INT1,INT2)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: INT1,INT2
 CHARACTER(LEN=256) :: FNAME,LINE
 CHARACTER(LEN=52) :: CTYPE
 INTEGER :: II,IOS,IU,I,J,JJ,K,N,N1,N2,N3,IVERTICAL,K1,K2
   ...
   IF(IVERTICAL.EQ.0)THEN
    DO J=1,SIZE(DRC)
     IF(TRIM(DRC(J)%CID).EQ.TRIM(HEADER(I)%CID))THEN
      N=N+1; N1=N1+1; IF(II.EQ.2)THEN; IORDER(N1)=J; ZD1(N1)=DRC(J)%MD; ZDD(N)=DRC(J)%MD; ENDIF
     ENDIF
    ENDDO
   ELSE
 ...
 END SUBROUTINE DEVWELL_IMPORT_MULTIPLE_WRITE
