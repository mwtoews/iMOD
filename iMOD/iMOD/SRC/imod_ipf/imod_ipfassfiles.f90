...
CONTAINS
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: XLOC,YLOC,GXMIN,GYMIN,GXMAX,GYMAX,AXMIN,AXMAX,AYMIN,AYMAX,D,DWIDTH,YDIS
 REAL(KIND=DP_KIND) :: DX,DY,AX1,AX2,AY1,AY2,XP,YP,MINX,MAXX,MINY,MAXY,XFRAC,YFRAC,XINT,YINT
...
!## weg?
  ...
  SELECT CASE (ASSF(IASSF)%ITOPIC)
   ...
   CASE (3)
    CALL DBL_IGRUNITS(MINX,MINY,MAXX,MAXY)
    XW=0.0D0; CALL IPFDRAWITOPIC3(IASSF,PLOTSTYLE,0.0D0,XW)
    IF(IACT.NE.INT(1,1))CALL IPFDRAWLEGEND(IASSF,MINX,MAXY,ISKIP)
...
!  CALL DBL_WGRTEXTBLOCK(XLOC,YLOC,XLOC+DX,YLOC-DY,'Cannot plot associated file!',1.0D0,TBJUSTIFY+TBFONTSIZE)
 ...
 DO I=1,MIN(MAXCOLOUR,ASSF(IASSF)%NCASS-1)
  ...
  !## plot axes-text
  CALL IGRCOLOURN(WRGB(0,0,0))
!  CALL DBL_WGRTEXTSTRING(XMIN+(2.0*OFFX)+BOXX,Y-0.5*DY,ASSF(IASSF)%ATTRIB(I+1))
  CALL DBL_WGRTEXTSTRING(XMIN+(2.0*OFFX)+BOXX,Y-0.5*DY,TRIM(ASSF(IASSF)%ATTRIB(I+1))) !//'('//TRIM(ASSF(IASSF)%FNAME)//')')
 ...
 DO J=1,MIN(MAXCOLOUR,ASSF(IASSF)%NCASS-1)
  ...
  IF(IMARKDATA.EQ.1)THEN
   DO I=1,ASSF(IASSF)%NRASS
    CALL DBL_IGRMARKER(REAL(ASSF(IASSF)%IDATE(I),8),ASSF(IASSF)%MEASURE(J,I),IMARKER)
   END DO
  ENDIF
...
!   ENDIF
 ...
 CALL DBL_WGRTEXTSTRING(X1,Y1,TRIM(ASSF(1)%FNAME))
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: XLOC,WIDTH,D,YDIS
 REAL(KIND=DP_KIND),DIMENSION(2,2),INTENT(IN) :: XY
 INTEGER :: I,ICLR
...
!!      Y2=YS2*(-1.0D0*SIN(RAD))+YS2*COS(RAD) !y2'
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: XLOC,MXW
 INTEGER :: IPLOT
 ...
 CALL DBL_WGRTEXTFONT(IFAMILY=FFHELVETICA,TWIDTH=0.03D0,THEIGHT=0.015D0,ISTYLE=0)
 !## right from borecolumn
 IF(ICHECK.NE.2)CALL DBL_WGRTEXTSTRING(XLOC-(3.0D0*MXW),(ASSF(IASSF)%Z(I)+ASSF(IASSF)%Z(I+1))/2.0D0,TRIM(PERC1))
 !## left from borecolumn
 IF(ICHECK.NE.1)CALL DBL_WGRTEXTSTRING(XLOC+(3.0D0*MXW),(ASSF(IASSF)%Z(I)+ASSF(IASSF)%Z(I+1))/2.0D0,TRIM(PERC2))
 ...
 INTEGER,INTENT(IN) :: IASSF
 INTEGER :: I,ICLR
 REAL(KIND=DP_KIND) :: XW,MXW,IWIDTH
...
! DO I=1,NLITHO(ILEG); IF(TRIM(UTL_CAP(ASSF(IASSF)%L(ICOLUMN,IROW),'U')).EQ.TRIM(UTL_CAP(BH(ILEG,I)%LITHO,'U')))EXIT; END DO
 ...
 REAL(KIND=DP_KIND) :: X1,Y1,X2,Y2,XS,XSC,DXOFF,MINX,MAXX,CMINX2,CMAXX2
