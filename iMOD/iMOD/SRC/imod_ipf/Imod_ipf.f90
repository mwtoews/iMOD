...
!    write(*,*)
 ...
 CALL UTL_SETTEXTSIZE(TWIDTH,THEIGHT,FCT=REAL(MP(IPLOT)%TSIZE,8)) !*0.025D0) 
  ...
  IF(IPF(IIPF)%IPOS(I).EQ.INT(1,1))THEN 
   ...
   ELSE
    IF(MP(IPLOT)%HCOL.EQ.0)THEN
     ...
     CALL UTL_PLOTPOINT(IPF(IIPF)%XYPOS(1,I),IPF(IIPF)%XYPOS(2,I),MP(IPLOT)%SYMBOL,REAL(MP(IPLOT)%THICKNESS,8))
     CALL IGRCOLOURN(ICLR)
     CALL UTL_PLOTPOINT(IPF(IIPF)%XYPOS(1,I),IPF(IIPF)%XYPOS(2,I),MP(IPLOT)%SYMBOL,REAL(MP(IPLOT)%THICKNESS,8))
    ENDIF
   ENDIF
...
10 CONTINUE
 ...
 IF(PIATTRIB.NE.0)THEN
  ...
  ENDIF
  J=MAX(1,J)
  CALL DBL_WGRTEXTSTRING(X,Y+(1.5*DY),TRIM(IPF(IIPF)%INFO(PIATTRIB,I)(J:)),IOFFSET=1)
 ENDIF
 ...
 CALL IGRCOLOURN(WRGB(0,0,0))
 CALL UTL_PLOTPOINT(X,Y,SYMBOL,(LZ+0.25D0)*THICKNESS)
 CALL IGRCOLOURN(ICLR)
 CALL UTL_PLOTPOINT(X,Y,SYMBOL,LZ*THICKNESS)
 ...
 XNEW=X-XY(1,1)  !x1
 YNEW=Y-XY(2,1)  !y1
 DIST=0.0D0; IF(XNEW+YNEW.NE.0.0D0)DIST=SQRT(XNEW**2.0+YNEW**2.0)
 ...
 ELSEIF(XROT.LT.0.0D0)THEN 
  DIST=0.0D0; IF(XROT+YROT.NE.0.0D0)DIST=SQRT(XROT**2.0+YROT**2.0)
 ...
 ELSEIF(XROT.GT.XDIS)THEN 
  DIST=0.0D0; IF((XROT-XDIS)+YROT.NE.0.0D0)DIST=SQRT((XROT-XDIS)**2.0+YROT**2.0)
  IF(DIST.LE.YDIS)THEN
   X=XDIS+IPF_OFFSETX
   Y=Z  
   IEDGE= 1
