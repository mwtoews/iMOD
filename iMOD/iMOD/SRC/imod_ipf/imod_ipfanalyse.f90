    ...
    IF(NPOS.GT.0)THEN
     ...
     IF(IASSF.NE.0)THEN
      IF(ASSF(IASSF)%ITOPIC.EQ.1)THEN
       ...
       CALL WINDOWOUTSTATUSBAR(1,'X:'//TRIM(CDATE)//', Y:'//TRIM(RTOS(Y,'F',2))//'m')
      ELSE
       CALL WINDOWOUTSTATUSBAR(1,'X:'//TRIM(RTOS(X,'F',2))//', Y:'//TRIM(RTOS(Y,'F',2))//'m')
      ENDIF
...
! ENDIF
 ...
 IF(NPOS.GT.0)THEN
  ...
  DO IPOS=1,NPOS
   ...
   CALL IPFPLOTASSFILE(0.0D0,0.0D0,1.0D0,IPOS,ABS(INT(3,1)),PLOTSTYLE,0.0D0,1.0D0,0.0D0,1.0D0,IMARKDATA,.FALSE., & !LPROF, &
               0.0D0,1.0D0,0.0D0,1.0D0,IZOOM,(IPOS-1)*IFRAME,0.0D0,1,(/0.0D0,0.0D0,0.0D0,0.0D0/),0.0D0,0)
  ...
  IF(MESSAGE%WIN.EQ.ID_DIPFINFOSERIE)THEN 
   ...
   SELECT CASE(ITYPE)
    ...
    CASE(MOUSEMOVE)
     ...
     X=REAL(MESSAGE%GX,8); Y=REAL(MESSAGE%GY,8) 
     CALL IPFANALYSE_GETXY(X,Y,IASSF)
     MESSAGE%GX=REAL(X,4); MESSAGE%GY=REAL(Y,4)
     ...
     IF(IASSF.NE.0)THEN
      ...
      IF(IASSF.NE.0)THEN
       IF(ASSF(IASSF)%ITOPIC.EQ.1)THEN
        ...
        CDATE=TRIM(ITOS(ID))//'/'//TRIM(ITOS(IM))//'/'//TRIM(ITOS(IY))
        CALL WINDOWOUTSTATUSBAR(1,'X:'//TRIM(CDATE)//', Y:'//TRIM(RTOS(REAL(MESSAGE%GY,8),'F',2))//'m')
       ELSE
        CALL WINDOWOUTSTATUSBAR(1,'X:'//TRIM(RTOS(REAL(MESSAGE%GX,8),'F',2))//', Y:'//TRIM(RTOS(REAL(MESSAGE%GY,8),'F',2))//'m')
       ENDIF
      ELSE
       CALL WINDOWOUTSTATUSBAR(1,'X:'//TRIM(RTOS(REAL(MESSAGE%GX,8),'F',2))//', Y:'//TRIM(RTOS(REAL(MESSAGE%GY,8),'F',2))//'m')
      ENDIF
