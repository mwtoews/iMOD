  ...
  IF(IDFALLOCATEX(IDFK(1)).AND.IDFALLOCATEX(IDFK(2)))THEN
   ...
   IF(IKRIGING.EQ.1)THEN
    ...
    CALL KRIGING_MAIN(SIZE(XD),XD,YD,ZD,IDFK(1),IDFK(2),          &
                      ...
                      KSETTINGS(ILAY)%COINCIDENT,KSETTINGS(ILAY)%COINCIDENTDIST, &
                      KSETTINGS(ILAY)%IQUADRANT,IBATCH,0,0.0D0,0)   
   ...
   IF(MAXVAL(PCG(ILAY)%IB).GT.0)THEN
    ...
    IF(ALLOCATED(GENSOL))THEN
     ...
     J=0; DO I=1,SIZE(GENSOL)
      ...
      J=J+1; 
      IF(LEN_TRIM(GENSOL(I)%FNAME).GT.0)CALL ASC2IDF_HFB(SOLIDF(ILAY),SOLIDF(ILAY)%NROW,SOLIDF(ILAY)%NCOL,IPC,(/GENSOL(I)%FNAME/),(/0.0D0/),0)
     ENDDO
     IF(J.GT.0)THEN
...
!   WRITE(IU(ISPF),'(A/)') '[END OF STDV BOUNDARIES]'
   ...
   WRITE(IU(ISPF),'(A)') '    0.0D00 - X world 1 -'
   WRITE(IU(ISPF),'(A)') '    0.0D00 - Y world 1 -'
   WRITE(IU(ISPF),'(A)') '    0.0D00 - X world 2 -'
   WRITE(IU(ISPF),'(A)') '    0.0D00 - Y world 2 -'
