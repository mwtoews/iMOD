...
USE MOD_UTL, ONLY : UTL_CAP,ITOS,UTL_SETTEXTSIZE,UTL_INVERSECOLOUR,PEUCKER_SIMPLIFYLINE,DBL_IGRINSIDEPOLYGON, &
    UTL_FILLARRAY,UTL_EQUALS_REAL,UTL_DEBUGLEVEL,UTL_GETHELP,UTL_PLOTLABEL
...
!          IS=IS+1
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: MINDIST=1.0D0 !## minimum distance 10 meter
 ...
 DO IELEV=1,SIZE(SPF(ISPF)%PROF)
  ...
  SPF(ISPF)%PROF(IELEV)%PX(2)=SPF(ISPF)%PROF(IELEV)%PX(2) !N)
 ENDDO
       ...
       IF(SOLID_PROFILEZVALUE(DX,JSPF,I,Z))THEN
        ...
        IF(ALLOCATED(PROFIDF))THEN   
         IF(I.LE.SIZE(PROFIDF))THEN        
          ...
          CALL DBL_WGRTEXTSTRING(DX+DXB,Z,TRIM(SPF(JSPF)%FNAME)//';'//TRIM(ALIAS)) 
         ELSE
          CALL DBL_WGRTEXTSTRING(DX+DXB,Z,TRIM(SPF(JSPF)%FNAME)//';Elevation:'//TRIM(ITOS(I))) 
         ENDIF
        ELSE
         CALL DBL_WGRTEXTSTRING(DX+DXB,Z,TRIM(SPF(JSPF)%FNAME)//';Elevation:'//TRIM(ITOS(I))) 
        ENDIF
...
!## added
 ...
 CALL DBL_WGRTEXTFONT(IFAMILY=FFHELVETICA,ISTYLE=FSOPAQUE) 
...
! ENDDO 
 ...
 CALL UTL_SETTEXTSIZE(TW,TH,FCT=SLABEL*0.01D05) 
 ...
 DO I=1,NSPF 
  IF(SPF(I)%NXY.GT.0)THEN
   ...
   DO J=2,SPF(I)%NXY
    CALL DBL_IGRJOIN(PX(J-1),PY(J-1),PX(J),PY(J))
   ENDDO
   CALL IGRLINEWIDTH(1)
   CALL IGRCOLOURN(WRGB(0,0,0))
   DO J=2,SPF(I)%NXY
   ...
   IF(ILABEL.EQ.1)THEN
    ...
    CALL UTL_PLOTLABEL(DX,DY,(/TRIM(FNAME)/),(/1/),1,TW,TH,(/''/),.FALSE.,-1,ALIGNLEFT,GANGLE=GANGLE)
   ENDIF
