...
USE RESOURCE
USE MOD_SOBEK_PAR  
USE MOD_UTL, ONLY : UTL_WSELECTFILE,UTL_GETHELP
...
CONTAINS
 ...
 !##=====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: RAD=360.0D0/(2.0*3.1415926)
 ...
 !##=====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: NODATA=-999.99
 INTEGER,INTENT(IN) :: IB
  ...
  IF(TRIM(TP2(IB)%ID).EQ.TRIM(GR(I)%CI))THEN
   ...
   IF(NTIMES.GT.0)THEN
    ...
    DO J=1,N
     ...
     WRITE(IU(ISD2),REC=IREFSD+1) ID,SELECTEDVALUES(1,1,J),SELECTEDVALUES(2,1,J),1.0D0,0.3
    END DO
 ...
 DO I=1,NQHREL
  IF(TRIM(TP2(IB)%ID).EQ.TRIM(QH(I)%CI))THEN
   ...
   WRITE(IU(ISQ2),REC=IREFSQ+1) 20.0D0,11.0D0,10.0D0,2.3
   IREFSQ=IREFSQ+1
   WRITE(IU(ISQ2),REC=IREFSQ+1) 50.0D0,15.0,50.0D0,6.0
