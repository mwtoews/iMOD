...
!PBMAN%UNCONFINED
 ...
 ELSE
  LINE='1'
 ENDIF
 LINE=TRIM(LINE)//',0,'//TRIM(ITOS(PBMAN%IDOUBLE))//',0,0,0'
...
!         'Select one of the following:'//CHAR(13)//'NONE, THIEM, SKIN, GENERAL, SPECIFYCWC','Error'); IOS=-1; EXIT
 ...
 REAL(KIND=DP_KIND),PARAMETER :: DLEAK=0.0D001
 INTEGER,INTENT(IN) :: IBATCH,ICB,ITOPIC,IPRT
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME,CPCK
 LOGICAL,INTENT(IN) :: LEX
  ...
  IF(KPER.LE.0)THEN
   SELECT CASE (ITOPIC)
    ...
    CASE (24) 
     IF(IPER.EQ.1)THEN
      WRITE(IU,'(A)') '0,0,0'
      DO I=1,3; WRITE(IU,'(A)') 'CONSTANT 0.0D000000E-00'; ENDDO
     ELSE; WRITE(IU,'(A)') '-1,-1,-1';  ENDIF
    !## rch
    CASE (26) 
     IF(IPER.EQ.1)THEN; WRITE(IU,'(I10)') 0; WRITE(IU,'(A)') 'CONSTANT 0.0D000000E-00'
     ELSE; WRITE(IU,'(I10)') -1; ENDIF
...
!  END SELECT
 ...
 IF(ITOPIC.EQ.31)THEN
  IF(ALLOCATED(FHBFLW))THEN
   LINE=TRIM(ITOS(IFHBUN))//',1.0D0,1'; WRITE(IU,'(A)') TRIM(LINE)
   ...
   I=0; DO ILAY=1,PRJNLAY; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
    IF(BND(ILAY)%X(ICOL,IROW).EQ. 2)THEN
     I=I+1; WRITE(IU,*) ILAY,IROW,ICOL,1.0D0,(FHBFLW(I,J),J=1,NBDTIM)
    ENDIF
   ENDDO; ENDDO; ENDDO
  ENDIF
  IF(ALLOCATED(FHBHED))THEN
   ...
   I=0; DO ILAY=1,PRJNLAY; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
    IF(BND(ILAY)%X(ICOL,IROW).EQ.-2)THEN
    I=I+1; WRITE(IU,*) ILAY,IROW,ICOL,1.0D0,(FHBHED(I,J),J=1,NBDTIM)
    ENDIF
   ENDDO; ENDDO; ENDDO
  ENDIF
 ENDIF
 ...
 CALL UTL_MF2005_MAXNO(TRIM(DIRMNAME)//'.'//CPCK//'7_',(/NP_IPER(0)/))
...
!  IF(LCHD)THEN
!   LINE='SAVE BUDGET '//TRIM(ITOS(ICHDCB)); DO ILAY=1,PRJNLAY; LINE=TRIM(LINE)//' '//TRIM(ITOS(ILAY)); ENDDO; WRITE(IU,'(A)') TRIM(LINE)
!  ENDIF
...
! WRITE(IU,'(A)') TRIM(LINE)
! LINE=TRIM(RTOS(HCLOSE,'E',7))//','//TRIM(RTOS(RCLOSE,'E',7))//','//TRIM(RTOS(RELAX,'E',7))//',1,1,0,1.0D0'
! WRITE(IU,'(A)') TRIM(LINE)
...
! LINE='0,0.0D0,0,0.0D0,0'; WRITE(IU,'(A)') TRIM(LINE)
 ...
 CHARACTER(LEN=256) :: FFNAME,DIRMSP
...
!    CALL METASWAP_METEGRID1(FFNAME,trim(simwd)//'mete_grid.inp')     
   ELSEIF(INDEX(FFNAME,'PARA_SIM.INP').GT.0)THEN
    CALL PMANAGER_SAVEMF2005_MSP_PARASIM(FFNAME,PRJIDF)
   ELSE
...
! CALL PMANAGER_SAVEMF2005_MSP_METEGRID()
 ...
 TINY=MIN(1.0D0,0.01D0*(PRJIDF%SX(PRJIDF%NCOL)-PRJIDF%SX(0)))
 CALL POL1LOCATE(PRJIDF%SX,PRJIDF%NCOL+1,PRJIDF%XMIN+SUBMODEL(6)+TINY,IC1)
 CALL POL1LOCATE(PRJIDF%SX,PRJIDF%NCOL+1,PRJIDF%XMAX-SUBMODEL(6)-TINY,IC2)
 CALL POL1LOCATE(PRJIDF%SY,PRJIDF%NROW+1,PRJIDF%YMIN+SUBMODEL(6)-TINY,IR1)
 CALL POL1LOCATE(PRJIDF%SY,PRJIDF%NROW+1,PRJIDF%YMAX-SUBMODEL(6)+TINY,IR2)
 ...
 CALL IDFGETLOC(PRJIDF,IR1,IC1,X1,Y1)
! CALL IDFGETLOC(PRJIDF,IR2,IC2,X2,Y2)
 ...
 WRITE(JU,'(A)') TRIM(LINE) 
 LINE='      idf_nodata              =      '//TRIM(RTOS(HUGE(1.0D0),'G',7))
 WRITE(JU,'(A)') TRIM(LINE)
   ...
   DO ICOL=1,PRJIDF%NCOL
    ...
    IF(ARND.GT.0.0D0)THEN
     ...
     IF(ICOL.GE.IC1.AND.ICOL.LE.IC2.AND.IROW.GE.IR1.AND.IROW.LE.IR2)THEN
      IF(IACT.EQ.2)WRITE(IIDF,'(3I10)') NUND,IROW,ICOL
     ENDIF
     ...
     IF(IACT.EQ.2)THEN
      ...
      WRITE(IAREA,'(I10,F10.1,F8.3,8X,I6,8X,8X,I6,F8.3,I10,2F8.3)') NUND,ARND,SIMGRO(ICOL,IROW)%MV, & 
            ...
            SIMGRO(ICOL,IROW)%METEO,1.0D0,1.0D0
    ...
    IF(ARND.GT.0.0D0)THEN
     ...
     IF(IACT.EQ.2)THEN
      ...
      WRITE(IAREA,'(I10,F10.1,F8.3,8X,I6,16X,I6,F8.3,I10,2F8.2)') &  
        NUND,ARND,SIMGRO(ICOL,IROW)%MV+MSWPMV,SIMGRO(ICOL,IROW)%BODEM,18,0.1,SIMGRO(ICOL,IROW)%METEO,1.0D0,1.0D0
...
! END IF
 ...
 DO
  ...
  READ(S,*) (SA(I),I=1,7)
!  CALL UTL_REL_TO_ABS(ROOT%RUNFILEROOT,SA(3)) 
!  CALL UTL_REL_TO_ABS(ROOT%RUNFILEROOT,SA(4))
  DO I = 3, 7
...
! CALL IMOD_UTL_OPENASC(IU,ASCIIFNAME,'R')
 ...
 READ(IU,*) TXT,A_NROW
 READ(IU,*) TXT,A_XLLC
 TXT=UTL_CAP(TXT,'U');IX=0.0; IF(TRIM(TXT).EQ.'XLLCENTER')IX=1.0
 READ(IU,*) TXT,A_YLLC
 TXT=UTL_CAP(TXT,'U'); IY=0.0; IF(TRIM(TXT).EQ.'YLLCENTER')IY=1.0
...
!CALL MATH1SCALEPDELRC()
! CALL IMOD_UTL_SCALE1PDELRC(A_XLLC,A_YLLC,A_XLLC+(A_NCOL*A_CELLSIZE),A_YLLC+(A_NROW*A_CELLSIZE), &
!           DELC,DELR,PDELR,PDELC,PRJIDF%NROW,PRJIDF%NCOL,A_CELLSIZE,A_NROW,A_NCOL,0,0,0)
...
! CALL IMOD_UTL_OPENASC(IU,INPFNAME,'W')
 ...
 NUND=0 
 DO IROW=1,PRJIDF%NROW
!  DY=PRJIDF%DY !DELC(IROW-1)-DELC(IROW)
  DO ICOL=1,PRJIDF%NCOL
!   DX    = PRJIDF%DELR(ICOL)-DELR(ICOL-1)
!   ARND  = DX*DY
   ARND=IDFGETAREA(PRJIDF,ICOL,IROW)
   ARND  = ARND-SIMGRO(ICOL,IROW)%NOPP-SIMGRO(ICOL,IROW)%SOPP
...
!# dit klopt niet ... de vulling van een breuk gaan niet op de cell niveau maar op vlak-niveau :-(
 ...
 IF(MAXV.EQ.MINV)THEN
  ...
  IF(IINT.EQ.0)THEN
   IF(MAXV.EQ.IDF%NODATA)THEN
    WRITE(IU,'(A)') 'CONSTANT '//TRIM(RTOS(HNOFLOW,'E',7))
   ELSE
 ...
 REAL(KIND=DP_KIND) :: C,ZT,ZB,A,X1,X2,Y1,Y2,DX,DY,L,TIB,F,KD1,KD2,OT1,OT2
