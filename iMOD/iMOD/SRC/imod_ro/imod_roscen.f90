...
DO IROW=1,ROSCEN(CODE,5)%NROW
 ...
 DO ICOL=1,ROSCEN(CODE,5)%NCOL
  ...
  IREC=12+(IROW-1)*ROSCEN(CODE,5)%NCOL+ICOL
...
DO IROW=1,MATH(3)%NROW
 ...
 DO ICOL=1,MATH(3)%NCOL
  ...
  IREC=12+(IROW-1)*MATH(3)%NCOL+ICOL
...
! - LGN (LGN 5 values)
! - HLPSOIL (reclassified soilmap using database with 6110 1:50.0D00 soil \
