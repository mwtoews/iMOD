    ...
    ELSE
     ...
     IF(DBL_IGRINSIDEPOLYGON(IFF(1)%X,IFF(1)%Y,XYPOL(:,1),XYPOL(:,2),4).EQ.1.OR. &
      ...
      IF((X1+IFF_OFFSETX-X2+IFF_OFFSETX).NE.0.0D0.OR.(IFF(1)%Z-IFF(2)%Z).NE.0.0D0)THEN
       CALL DBL_IGRJOIN(X2+IFF_OFFSETX,IFF(2)%Z,X1+IFF_OFFSETX,IFF(1)%Z,IOFFSET=1)
       CALL DBL_IGRARROWJOIN(X2+IFF_OFFSETX,IFF(2)%Z,X1+IFF_OFFSETX,IFF(1)%Z,1,IOFFSET=1)
      ENDIF
     ENDIF
