...
USE MOD_IPF, ONLY : IPFINIT,IPFREAD,IPFDEALLOCATE,IPFALLOCATE

INTEGER,PARAMETER,PRIVATE :: MAXFUNC=74
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'ADJUSTVOXELIDF','MFTOIMOD','KDC','IMODTOSTOMP','XYZTOVOXEL','REGRESSION'/
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 PBMAN%OUTPUT=''; PBMAN%IDOUBLE=0; PBMAN%ICONSISTENCY=1; PBMAN%MINTHICKNESS=0.1D0
 ...
 IF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  ...
  IF(PBMAN%IFORMAT.EQ.2)THEN
   ...
   WRITE(*,'(A,G15.9)') 'MINTHICKNESS=',PBMAN%MINTHICKNESS
  ENDIF
  ...
  WRITE(*,'(A,G15.9)') 'MINKD=',PBMAN%MINKD
  IF(UTL_READINITFILE('MINC',LINE,IU,1))READ(LINE,*) PBMAN%MINC
  WRITE(*,'(A,G15.9)') 'MINC=',PBMAN%MINC
  ...
  ELSE
   IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
    ...
    WRITE(*,'(A,4G15.9)') 'WINDOW=',PBMAN%XMIN,PBMAN%YMIN,PBMAN%XMAX,PBMAN%YMAX
 ...
 !## overrule system in case of ipest
 IF(PBMAN%IPEST.EQ.1)PBMAN%SSYSTEM=1
 ...
 DX=X(2)-X(1); DY=Y(2)-Y(1); DM=0.0D0; IF(DX.NE.0.0D0.OR.DY.NE.0.0D0)DM=SQRT(DX**2.0+DY**2.0); TD=0.0D0
 ...
 !###======================================================================
 USE MOD_TS_PAR, ONLY : IPFNAME1,IPFNAME2,TSILAY,SDATE,EDATE,IASSF,TSDIR,IBATCH,LCOL
 IMPLICIT NONE
...
! WRITE(*,'(A,I2)') 'ITALUSCOL=',DATCOL(9)
 ...
 CALL ASC2IDF_HFB(IDF(3),IDF(3)%NROW,IDF(3)%NCOL,IPC,(/GENFILE/),(/0.0D0/),-1)
 ...
 N=0; DO IROW=1,IDF(3)%NROW; DO ICOL=1,IDF(3)%NCOL
   ...
   IF(ICOL.LT.IDF(3)%NCOL)THEN
    ...
    IF(LEX)THEN
     N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF(3),IDF(3)%NROW,IDF(3)%NCOL,IROW,ICOL,N, &
          0.0D0,0.0D0,0.0D0,0,1,.TRUE.,T1,B1) 
    ENDIF
   ENDIF
  ENDIF
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   ...
   IF(IROW.LT.IDF(3)%NROW)THEN
    ...
    IF(LEX)THEN
     N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF(3),IDF(3)%NROW,IDF(3)%NCOL,IROW,ICOL,N, &
          0.0D0,0.0D0,0.0D0,0,2,.TRUE.,T1,B1) 
    ENDIF
   ENDIF
  ENDIF
...
!             1    0.0D000000E+00  8.7779391E-01  2.8722062E+00  7.1803361E-02  6.3689098E-02
...
! 9014 FORMAT (62X, 2(1PE14.7, 1X))
 ...
 IMPLICIT NONE
 INTEGER :: I,N,IKD,ICW,SDATE,EDATE,ISS,IMIDF,IQCOL,STNLAY
 REAL(KIND=DP_KIND) :: HNODATA,FNODATA,MINKHT,MINKD
 
 STNLAY=-1; IMIDF=0; IKD=0; ICW=0; MINKHT=0.0D0; SDATE=0; EDATE=0; ISS=0
...
!  WRITE(*,'(A,I5)') 'ICLAY=',ICLAY 
 ...
 DO I=1,N
  ...
  IF(.NOT.ST1CREATEIPF_STEADY(IKD,ICW,MINKHT,IMIDF,SDATE,EDATE,ISS,HNODATA,FNODATA,MINKD,STNLAY))WRITE(*,'(A)') 'Error IPF '//TRIM(IPF(1)%FNAME)//' failed'
 ENDDO
    ...
    IF(IPFREADASSFILELABEL(JU,N,FNAME).AND.IPFREADASSFILE(JU,N,FNAME))THEN
     ...
     DO II=1,2
      ...
      IF(II.EQ.1)THEN
       ...
       ELSE
!        WRITE(*,'(A)') 'No overlapping datapoints for '//TRIM(FILTER1)//' and '//TRIM(FILTER2)
!        IPF(1)%INFO(IPF(1)%ACOL,I)=TRIM(IPF(1)%INFO(IPF(1)%ACOL,I))//'_regression'
!        R2=0.0D0; WRITE(KU,'(99A)') (TRIM(IPF(1)%INFO(JJ,I))//',',JJ=1,IPF(1)%NCOL),TRIM(RTOS(R2,'G',5)),','//TRIM(ITOS(NN))
        EXIT
       ENDIF
      ELSE
!      ku=utl_getunit(); open(ku,file='d:\tmp.csv',status='unknown',action='write')
!      do jj=1,nn; write(ku,*) x(jj),y(jj); enddo
!      close(ku)
...
!   CLOSE(JU)
 ...
 USE MOD_UTL  
 IMPLICIT NONE
 INTEGER :: I,J,K,N,IN_TYPE,JU,M,IDATE,SDATE,EDATE,DDATE,IOS,ICOL,IROW,JF,ID,KSUM,ILAY,NLAYVAL,NBLNFILE
 ...
 ASSF_IDEPTH=0; ASSF_TOP=0.0D0; ASSF_BOT=0.0D0; ASSF_DZ=0.0D0; ASSF_NTHRESHOLD=1; ASSF_INDICATOR=0; ELLIPS_IDF%FNAME=''; ASSF_ZPLUS=0.0D0
...
!   1    1    1     1     1    6.3357E+04  7.9484E+03   5.5409E+04  0.0D00E+00  0.0D00E+00  0.0D00E+00  3.37445E+02  3.587E-01  3.931E+00  5.850E+03  1.359E+00
...
! IY1=1980; IY2=2015; 
 ...
 DO I=1,SIZE(BAL); ALLOCATE(BAL(I)%Q(NYR)); BAL(I)%Q=0.0D0; ENDDO
 ...
 DO I=1,NFILE
  ...
  DO J=1,N
   ...
   BAL(IBAL)%Q(IPER)=BAL(IBAL)%Q(IPER)+Q1*31.0D0
   CLOSE(KU)
  ENDDO
  CLOSE(JU)
 ENDDO
 ...
 DO IPER=1,NYR
  ...
  DO J=1,SIZE(BAL); BAL(J)%Q(IPER)=-1.0D0*BAL(J)%Q(IPER)/1.0E6; ENDDO
  IY=IY+1
 ...
 IWINDOW=0; HCLOSE=0.01D0; MICNVG=5; FMIDELEV=1.0D0; IBNDCHK=0; SOL_IINT_IDF=1
...
!
 ...
 DO I=1,NBAL
  ...
  READ(LINE,*) WCTP(I)%BDGNAME; LINE='BAL'//TRIM(ITOS(I))//'='//TRIM(WCTP(I)%BDGNAME) 
  WRITE(*,'(A)') TRIM(LINE)
...
!
 ...
 REAL(KIND=DP_KIND) :: XMIN,YMIN,XMAX,YMAX,YFRACLEGEND,RAT,TSIZE
...
! ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: TSIZE,YFRACLEGEND
 ...
 CALL DBL_WGRTEXTSTRING(XP-BND,0.0D0+0.5D0*BND,'Powered by '//TRIM(UTL_IMODVERSION())//' Copyright © 2005-2018')
 ...
 DL=0.0D0; IF(N.GT.0)DL =(YFRACLEGEND/REAL(N))/100.0D0*(LT-LB)
 LB1=LT-DL
 ...
 DO I=1,MXMPLOT
  ...
  CALL LEGPLOT_PLOT(MP(I)%LEG,1,1,TSIZE=TSIZE)  !## plot legend for iplot=1, nc=2 columns
