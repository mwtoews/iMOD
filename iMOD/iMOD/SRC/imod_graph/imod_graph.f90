...
CONTAINS
 ...
 INTEGER :: ITYPE,NG,IBITMAP,I,IXDLU,IYDLU,IXPIX,IYPIX
 ...
 IF(MESSAGE%WIN.EQ.ID_DSCENTOOL_FIGURE)THEN
  ...
  IF(ASSOCIATED(GRAPHDIM%XTXT))THEN
   ...
   IF(I.GE.1.AND.I.LE.SIZE(GRAPHDIM%XTXT))THEN
    CALL WDIALOGPUTSTRING(IDF_LABEL1,'Date='//TRIM(GRAPHDIM%XTXT(I))//'; Ycrd='//TRIM(RTOS(REAL(MESSAGE%GY,8),'F',2)))
   ELSE
...
!    CALL DBL_IGRRECTANGLE(GRAPH(I,NG)%RX(J),GRAPH(I,NG)%RY(J),GRAPH(I,NG)%RX(J+1),GRAPH(I,NG)%RY(J+1))
!   END DO
  ENDIF
 ...
 DO I=1,SIZE(GRAPH,1)
  ...
  !## plot axes-text
  CALL IGRCOLOURN(WRGB(0,0,0))
  CALL DBL_WGRTEXTSTRING(XMAX+DX,(YMAX+YMIN)/2.0,' '//TRIM(GRAPH(I,NG)%LEGTXT))
 ...
 CALL DBL_WGRTEXTSTRING(AXES%XMAX-1.5*DX,AXES%YMIN+1.5*DY,TRIM(GRAPHNAMES(NG)))
...
! FCT=6.5D0*SY_RATIO 
 ...
 IF(.NOT.ASSOCIATED(AXES%YTXT))THEN
  ...
  DO
   ...
   IF(Y.GT.AXES%YMIN)THEN
    CALL DBL_IGRJOIN((AXES%XMIN+XJDCOR)-DXTIC/2.0,Y,(AXES%XMIN+XJDCOR)+DXTIC/2.0,Y)
    CALL DBL_IGRJOIN((AXES%XMAX+XJDCOR)-DXTIC/2.0,Y,(AXES%XMAX+XJDCOR)+DXTIC/2.0,Y)
   ENDIF
  ENDDO
 ...
 IF(AXES%IAXES(2).EQ.1)THEN
  ...
  IF(.NOT.ASSOCIATED(AXES%Y2TXT))THEN
   ...
   DO
    ...
    IF(Y.GE.AXES%Y2MAX)EXIT
    IF(Y.GT.AXES%Y2MIN)THEN
     CALL DBL_IGRJOIN(AXES%XMAX+DXTIC/2.0+XJDCOR,Y,AXES%XMAX-DXTIC/2.0+XJDCOR,Y)
     CALL DBL_IGRJOIN(AXES%XMAX+DXTIC/2.0+XJDCOR,Y,AXES%XMAX-DXTIC/2.0+XJDCOR,Y)
    ENDIF
   END DO
 ...
 IF(.NOT.ASSOCIATED(AXES%XTXT))THEN
  ...
  DO
   ...
   IF(X.GT.AXES%XMIN+XJDCOR)THEN
    CALL DBL_IGRJOIN(X,AXES%YMIN-DYTIC/2.0,X,AXES%YMIN+DYTIC/2.0)
    CALL DBL_IGRJOIN(X,AXES%YMAX-DYTIC/2.0,X,AXES%YMAX+DYTIC/2.0)
   ENDIF
  ENDDO
