...
SELECT CASE (IDOWN)
 !## left
 CASE (4,5)
  DOWNX=MOUSEX !MESSAGE%GX
  DOWNY=MOUSEY !MESSAGE%GY
  ...
  IF(WMENUGETSTATE(ID_ISGEDIT,2).EQ.1)THEN
   ...
   SELECT CASE (ITAB)
    CASE (ID_DISGEDITTAB1)
     CALL ISGCHECKISG(MOUSEX,MOUSEY,0) !REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8),0)
    CASE DEFAULT
     !## check polygon
     CALL POLYGON1SELECT()
   END SELECT
 ...
 !## shift-left - inzoomen
 CASE (6)
  CALL IDFZOOM(ID_ZOOMINMAP,MOUSEX,MOUSEY,0) !REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8),0)
  CALL IDFPLOTFAST(1)
...
!    CALL WMENUFLOATING(ID_MENU10,MESSAGE%X,MESSAGE%Y)   
 ...
 !## shift right
 CASE (18)
  CALL IDFZOOM(ID_ZOOMOUTMAP,MOUSEX,MOUSEY,0) !REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8),0)
  CALL IDFPLOTFAST(1)
  CALL WGRCURSORPOS(MESSAGE%GX,MESSAGE%GY)
END SELECT
...
ELSE
 ...
 IY1=IY1-20
 IY2=IY2+30
 CALL WBITMAPGET(IHANDLE,1,IX1,IY1,IX2,IY2)
 CALL WCLIPBOARDPUTBITMAP(IHANDLE)
 CALL WBITMAPDESTROY(IHANDLE)
ENDIF

END SUBROUTINE IMODCLIPBOARD
