...
CONTAINS
  ...
  CASE(PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_SAVESHAPE,ID_LOADSHAPE,ID_SAVE)
     IF(MESSAGE%VALUE1.NE.ID_LOADSHAPE)CALL WDIALOGGETMENU(IDF_MENU1,SHP%POL%IACT)
     IF(PRESENT(GENFNAME))THEN
      CALL POLYGON1SAVELOADSHAPE(MESSAGE%VALUE1,TRIM(GENFNAME))
     ELSE
      CALL POLYGON1SAVELOADSHAPE(MESSAGE%VALUE1,'')
     ENDIF
 ...
 MPW%XMIN=XMIN-((MPW%XMAX-MPW%XMIN)/25.0D0)
 MPW%XMAX=XMAX+((MPW%XMAX-MPW%XMIN)/25.0D0)
 MPW%YMIN=YMIN-((MPW%YMAX-MPW%YMIN)/25.0D0)
 MPW%YMAX=YMAX+((MPW%YMAX-MPW%YMIN)/25.0D0)
...
!    CALL WCURSORSHAPE(ID_CURSORPOLYGON)
 ...
 INTEGER :: I,J,IOLD,INEW,N,STRLEN,MAXCOL
 ...
 DO SHPI=1,SHP%NPOL
  ...
  IF(SHP%POL(SHPI)%N.GT.0)THEN
   ...
   IF(CRDITYPE.EQ.3.OR.CRDITYPE.EQ.4)THEN
    IF(SHP%POL(SHPI)%ITYPE.EQ.ID_RECTANGLE)THEN
