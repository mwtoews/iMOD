...
USE MOD_UTL, ONLY : ITOS,RTOS,UTL_GETUNIT,UTL_WSELECTFILE,UTL_CREATEDIR,UTL_GENLABELSREAD, &
          UTL_GENLABELSDEALLOCATE,UTL_GENLABELSWRITE,CCNST,UTL_GETHELP,UTL_PLOT1BITMAP,UTL_PLOT2BITMAP
...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE POLYGON_UTL_CONVERTGEN(FNAME)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(INOUT),OPTIONAL :: FNAME
 INTEGER :: IOS,IU,JU,N,M,I,J,K,MAXCOL,MAXPOL,IFORMAT
 ...
 CHARACTER(LEN=124),DIMENSION(:,:),POINTER :: VAR
 LOGICAL :: LEX
   
 GENFNAME=''
 IF(PRESENT(FNAME))THEN
  IF(LEN_TRIM(FNAME).EQ.0)THEN
   IF(.NOT.UTL_WSELECTFILE('ESRI Generate File (*.gen)|*.gen|',&
      LOADDIALOG+MUSTEXIST+PROMPTON+DIRCHANGE+APPENDEXT,GENFNAME,'Load ESRI Generate File (*.gen)'))RETURN
  ELSE
   GENFNAME=FNAME
  ENDIF
 ELSE
 ...
 IF(.NOT.UTL_WSELECTFILE('iMOD Binary Generate File (*.gen)|*.gen|',&
    SAVEDIALOG+PROMPTON+DIRCHANGE+APPENDEXT,GENFNAME,'Save iMOD Binary Generate File (*.gen)'))THEN
  CLOSE(IU); RETURN
 ENDIF
 JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=GENFNAME,STATUS='UNKNOWN',FORM='UNFORMATTED',ACTION='WRITE')
...
!   CALL WCURSORSHAPE(ID_CURSORLINE)
     ...
     CASE (ID_RENAME)
      ...
      ELSE
       ...
       IF(IWIDTH.NE.SHP%LWIDTH(ICOL))THEN
        ...
        DO J=1,SHP%NPOL
         ALLOCATE(SHP%POL(J)%LBL_BU(ICOL)%STRING(STRLEN)); SHP%POL(J)%LBL_BU(ICOL)%STRING=''
         DO K=1,MIN(STRLEN,SHP%LWIDTH(ICOL))
          SHP%POL(J)%LBL_BU(ICOL)%STRING(K)=SHP%POL(J)%LBL(ICOL)%STRING(K)
         ENDDO
        ENDDO
        DO J=1,SHP%NPOL; DEALLOCATE(SHP%POL(J)%LBL); SHP%POL(J)%LBL=>SHP%POL(J)%LBL_BU; ENDDO
 ...
 CALL WDIALOGPUTMENU(IDF_MENU1,SHP%COLNAMES,SHP%NPOL,SHP%ILBL)
 
 END SUBROUTINE POLYGON_UTL_FILLDATAGRID_GRID

 !###======================================================================
 SUBROUTINE POLYGON1SAVELOADSHAPE(CODE,GENFNAME)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: GENFNAME
 INTEGER,INTENT(IN) :: CODE
 INTEGER :: IU,I,J,IOS,STRLEN,FTYPE
 CHARACTER(LEN=256) :: FNAME
 ...
 IF(CODE.EQ.ID_LOADSHAPE)THEN
  ...
  IF(GENFNAME.EQ.'')THEN
   IF(.NOT.UTL_WSELECTFILE('GEN File (*.gen)|*.gen|GEN Files (*.gen)|*.gen|',&
        LOADDIALOG+MUSTEXIST+PROMPTON+DIRCHANGE+APPENDEXT,FNAME,'Load GEN File'))RETURN
  ELSE
   FNAME=GENFNAME
  ENDIF
  ...
  IF(MAXCOL.GT.0)THEN
   ...
   DO J=1,SHP%NPOL; ALLOCATE(SHP%COLNAMES(MAXCOL)); ENDDO
   DO J=1,SHP%NPOL
 ...
 ELSEIF(CODE.EQ.ID_SAVESHAPE)THEN 
  ...
  FTYPE=1
  IF(GENFNAME.EQ.'')THEN
   IF(.NOT.UTL_WSELECTFILE('iMOD GEN Files (*.gen)|*.gen|ESRI GEN file (*.gen)|*.gen|',     &
        SAVEDIALOG+PROMPTON+DIRCHANGE+APPENDEXT,FNAME,'Save GEN File',FTYPE=FTYPE))RETURN
  ELSE
   FNAME=GENFNAME
  ...
  IF(FTYPE.EQ.1)THEN
   ...
   ENDDO
   SHP%XMIN=MINVAL(SHP%POL(1:SHP%NPOL)%XMIN)
   SHP%XMAX=MAXVAL(SHP%POL(1:SHP%NPOL)%XMAX)
   SHP%YMIN=MINVAL(SHP%POL(1:SHP%NPOL)%YMIN)
   SHP%YMAX=MAXVAL(SHP%POL(1:SHP%NPOL)%YMAX)
   ...
   DO I=1,SHP%NPOL
    ...
    WRITE(*,*) (SHP%POL(I)%LBL(J)%STRING,J=1,MAXCOL)
  ...
  ELSE
   ...
   MAXCOL=0; IF(ASSOCIATED(SHP%COLNAMES))MAXCOL=SIZE(SHP%COLNAMES)
   DO I=1,SHP%NPOL
    WRITE(IU,'(99A)') (SHP%POL(I)%LBL(J)%STRING//' ',J=1,MAXCOL)
    DO J=1,SHP%POL(I)%N
     WRITE(IU,'(2(G15.9,A1))') SHP%POL(I)%X(J),',',SHP%POL(I)%Y(J)
    ENDDO
    WRITE(IU,'(A3)') 'END'
   ENDDO
   WRITE(IU,'(A3)') 'END'
