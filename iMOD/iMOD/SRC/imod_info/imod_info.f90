...
!      IF(MP(IPLOT)%IPLOT.EQ.6)CALL POLYGON_UTL_FILLDATAGRID()
 ...
 ELSE
  ...
  CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,1,D,'(F15.3)')
  CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,2,'Population')
  D=REAL(NPOP,8)
  CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,2,D,'(F15.3)')
  CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,3,'Mean')
  D=REAL(MEAN,8)
  CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,3,D,'(F15.3)')
  CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,4,'Standard Deviation')
  D=VAR
  CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,4,D,'(F15.3)')
  CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,5,'Sum')
  D=REAL(MEAN*REAL(NPOP),8)
  CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,5,D,'(F15.3)')
 ...
 IF(MP(IPLOT)%IPLOT.EQ.1.OR.MP(IPLOT)%IPLOT.EQ.5.OR.MP(IPLOT)%IPLOT.EQ.7)THEN
  ...
  IF(LEX)THEN
   ...
   IF(LEX)THEN
    ...
    CALL WDIALOGPUTDOUBLE(ID_ZMIN,MP(IPLOT)%IDF%DMIN,'(F15.3)')
    CALL WDIALOGPUTDOUBLE(ID_ZMAX,MP(IPLOT)%IDF%DMAX,'(F15.3)')
    CALL WDIALOGPUTDOUBLE(ID_DZ,MP(IPLOT)%IDF%DMAX-MP(IPLOT)%IDF%DMIN,'(F15.3)') 

    CALL WDIALOGPUTDOUBLE(ID_ZMIN2,MP(IPLOT)%UMIN,'(F15.3)')
    CALL WDIALOGPUTDOUBLE(ID_ZMAX2,MP(IPLOT)%UMAX,'(F15.3)')
    CALL WDIALOGPUTDOUBLE(ID_DZ2,MP(IPLOT)%UMAX-MP(IPLOT)%UMIN,'(F15.3)')
...
!  ENDIF
  ...
  IF(MP(IPLOT)%ILEG.EQ.1)THEN
   ...
   CALL WDIALOGPUTDOUBLE(ID_ZMIN,MP(IPLOT)%UMIN,'(F15.3)')
   CALL WDIALOGPUTDOUBLE(ID_ZMAX,MP(IPLOT)%UMAX,'(F15.3)')
   CALL WDIALOGPUTDOUBLE(ID_DZ,MP(IPLOT)%UMAX-MP(IPLOT)%UMIN,'(F15.3)')
  ENDIF
