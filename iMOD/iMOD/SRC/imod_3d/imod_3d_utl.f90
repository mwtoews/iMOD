...
!   IF(INDEX(MP(I)%FNAME),'TMP\3D\ARTWELL').GT.0)THEN
...
! ENDDO
 ...
 IF(IOSDIREXISTS(TRIM(PREFVAL(1))//'\TMP\3D\ARTWELL'))THEN
  IF(UTL_DEL1TREE(TRIM(PREFVAL(1))//'\TMP\3D\ARTWELL'))THEN; ENDIF 
 ENDIF

  
 ! IPFFNAME=TRIM(PREFVAL(1))//'\TMP\3D\ARTWELL\ARTIFICIAL_WELL_'//TRIM(ITOS(I))//.IPF'
 ! INQUIRE(FILE=IPFFNAME,EXIST=LEX); IF(.NOT.LEX)EXIT
