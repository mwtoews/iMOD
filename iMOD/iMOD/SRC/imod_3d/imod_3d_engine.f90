   ...
   I=I+1
    ...
    IF(DX.GT.X1.AND.DX.LT.X2)THEN
     IF(MOD(I-1,4).EQ.0)THEN
      CALL GLVERTEX3D(DX,Y1-DXY*2.0,Z1); CALL GLVERTEX3D(DX,Y1+DXY*2.0,Z1)
      CALL GLVERTEX3D(DX,Y2-DXY*2.0,Z1); CALL GLVERTEX3D(DX,Y2+DXY*2.0,Z1)
      CALL GLVERTEX3D(DX,Y2-DXY*2.0,Z2); CALL GLVERTEX3D(DX,Y2+DXY*2.0,Z2)
     ELSE
  ...
  I=0; DO 
   I=I+1
    ...
    IF(DY.GT.Y1.AND.DY.LT.Y2)THEN
     IF(MOD(I-1,4).EQ.0)THEN
      CALL GLVERTEX3D(X1-DXY*2.0,DY,Z1); CALL GLVERTEX3D(X1+DXY*2.0,DY,Z1)
      CALL GLVERTEX3D(X2-DXY*2.0,DY,Z1); CALL GLVERTEX3D(X2+DXY*2.0,DY,Z1)
      CALL GLVERTEX3D(X2-DXY*2.0,DY,Z2); CALL GLVERTEX3D(X2+DXY*2.0,DY,Z2)
     ELSE
...
! END FUNCTION IMOD3D_LEGEND_BITMAP
     ...
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (IDOK)
       IF(IMOD3D_IPF_QUERY_READ(IIPF))THEN
        ...
        CALL WDIALOGGETCHECKBOX(IDF_CHECK4,IPFPLOT(IIPF)%ISELECT(4))
 ...
 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  !## reconsruct ipf if needed ... is nquery was > 0 before entering this tool
...
!###======================================================================
 ...
 ILST=0; DO I=1,SIZE(IDFPLOT) 
  ...
  DO II=1,JJ
   ...
   CALL WINDOWOUTSTATUSBAR(2,'Processing '//TRIM(IDFPLOT(I)%FNAME)//'('//TRIM(RTOS(REAL(I*100,8)/REAL(SIZE(IDFPLOT)),'F',2))//'%)')
   !## first is template
   IF(II.EQ.1)THEN
...
!     IF(.NOT.IDFREADSCALE(IDF(5)%FNAME,IDF(1),IDFDATA(3),1,0.0D0,0))EXIT
    ELSE
...
!    IF(.NOT.IDFREADSCALE(IDF(2)%FNAME,IDF(4),IDFDATA(3),1,0.0D0,0))EXIT
...
!    IF(.NOT.IDFREADSCALE(IDF(3)%FNAME,IDF(5),IDFDATA(3),1,0.0D0,0))EXIT
...
!   IF(IDF(1)%IVF.EQ.1)CALL SOF_COMPUTE_GRAD_3D(IDF(1),ICOL,IROW,DZDX,DZDY,DZDZ)
   ...
   F=DZDX**2.0+DZDY**2.0; IF(F.NE.0.0D0)F=SQRT(F)
...
!## eigenlijk deze twee keer aanroepen voor imode=1 en imode=2
 ...
 REAL(KIND=GLDOUBLE),PARAMETER :: TS= 10.0D0 !0.025D0 !## textsize
 ...
 DO IIPF=1,NIPF
  ...
  DO I=1,IPF(IIPF)%NROW
   ...
   IF(IPF(IIPF)%IPOS(I).EQ.INT(1,1))THEN
    ...
    !## label-drawing list
    IF(ACOL.NE.0)N=N+1
    IF(IPFLISTINDEX(N,3).NE.0)CALL GLDELETELISTS(IPFLISTINDEX(N,3),1_GLSIZEI)
...
! IF(L3D)RETURN
 ...
 L3D=.TRUE.; LS3D=.FALSE.; NX=100; ALLOCATE(X(NX),Y(NX),Z(NX))
...
!     XCOR(2)=X(I+1); YCOR(2)=Y(I+1)
     CALL GLVERTEX3D(X(I)  ,Y(I),0.0_GLDOUBLE)
     CALL GLVERTEX3D(X(I+1),Y(I+1),0.0_GLDOUBLE)
!     CALL IMOD3D_LINE(XCOR,YCOR,(/0.0_GLDOUBLE,0.0_GLDOUBLE/))
    ENDIF
   ENDDO
   CALL GLEND()
...
! CALL SOLIDDEALLOCATESLD()
 ...
 REAL(KIND=DP_KIND),PARAMETER :: NODATA_Z=-999.99
 ...
 DO JPROF=1,SIZE(SPF(I)%PROF) 
  ...
  DO J=2,SPF(I)%NXY-1
   ...
   IF(DXX.NE.0.0D0.OR.DYY.NE.0.0D0)DXY=SQRT(DXX**2.0+DYY**2.0)
   TX(1)     =TX(1)+DXY
   IPOS      =IPOS+1
   XT(IPOS)  =TX(1)
  ...
  !call sortem(1,N,XT,3,ZT(:,1),ZT(:,2),ZT(:,3),(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/))
...
!  CALL GLNEWLIST(SOLLISTINDEX(NSOLLIST,3),GL_COMPILE)
  ...
  DO J=2,SPF(I)%NXY 
   ...
   DXY=DXX**2.0+DYY**2.0; IF(DXY.NE.0.0D0)DXY=SQRT(DXY)
