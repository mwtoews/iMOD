...
!almost none. You can think of specularity as shininess.
 ...
 DO I=1,NCLPLIST
  IF(CLPPLOT(I)%ISEL.EQ.1)THEN
   ...
   IF(ABS(CLPPLOT(I)%EQN(1)).EQ.1.0_GLDOUBLE)THEN
    DX=(TOP%X-BOT%X)*REAL(CLPPLOT(I)%IPOS)/100.0D0
   ...
   IF(ABS(CLPPLOT(I)%EQN(2)).EQ.1.0_GLDOUBLE)THEN
    DY=(TOP%Y-BOT%Y)*REAL(CLPPLOT(I)%IPOS)/100.0D0
   ...
   IF(ABS(CLPPLOT(I)%EQN(3)).EQ.1.0_GLDOUBLE)THEN
    DZ=(TOP%Z-BOT%Z)*REAL(CLPPLOT(I)%IPOS)/100.0D0
 ...
 IF(ABS(CLPPLOT(I)%EQN(1)).EQ.1.0_GLDOUBLE)THEN
  ...
  OX=0.0_GLDOUBLE !(TOP%X-BOT%X)/200.0D0*CLPPLOT(I)%EQN(1)
 ...
 IF(ABS(CLPPLOT(I)%EQN(2)).EQ.1.0_GLDOUBLE)THEN
  ...
  OY=0.0_GLDOUBLE !(TOP%Y-BOT%Y)/200.0D0*CLPPLOT(I)%EQN(2)
 ...
 IF(ABS(CLPPLOT(I)%EQN(3)).EQ.1.0_GLDOUBLE)THEN
  ...
  OZ=0.0_GLDOUBLE !(TOP%Z-BOT%Z)/200.0D0*CLPPLOT(I)%EQN(3)
...
! enddo  
 ...
 DO I=1,SIZE(IDFLISTINDEX) 
  ...
  IF(FS.GT.0.0D0)THEN !IDFPLOT(I)%ISTACKED.GT.0)THEN
   CALL GLPUSHMATRIX()  !## pushes all matrices in the current stack down one level, topmost is copied
