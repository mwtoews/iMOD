...
CONTAINS
 ...
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK4,IPFPLOT(IIPF)%ISELECT(4))
 CALL WDIALOGPUTDOUBLE(IDF_REAL1,IPFPLOT(IIPF)%RSELECT(1),'(F10.2)')
 CALL WDIALOGPUTDOUBLE(IDF_REAL2,IPFPLOT(IIPF)%RSELECT(2),'(F10.2)')
 CALL WDIALOGPUTDOUBLE(IDF_REAL3,IPFPLOT(IIPF)%RSELECT(3),'(F10.2)')
...
! CALL WDIALOGFIELDSTATE(IDF_OPEN,2); CALL WDIALOGFIELDSTATE(IDF_STRING1,2)
...
! CALL WGRIDPOS(MESSAGE%X,ICOL,IROW) !## from
 ...
 IF(N.GT.0)THEN
  ...
  IPFPLOT(IIPF)%QUERY(I)%IFIELD=3 !## z
 ...
 DO
  N=0; DO I=1,IPFPLOT(IIPF)%NQUERY
   ...
   IPFPLOT(IIPF)%QUERY(J)%IFIELD=3 !## z
 ...
 ELSE
  ...
  DO I=1,IPFPLOT(IIPF)%NQUERY
   ...
   IF(J.GT.SIZE(IDFS))THEN; CERROR='Cannot match FNAME1 in curent IDF list.'; EXIT; ENDIF
   ...
   IF(J.GT.SIZE(IDFS))THEN; CERROR='Cannot match FNAME2 in curent IDF list.'; EXIT; ENDIF
 ...
 WRITE(IU,*) IPFPLOT(IIPF)%NQUERY
 
 !## write all
 DO I=1,IPFPLOT(IIPF)%NQUERY
  WRITE(IU,'(1X,I2)') IPFPLOT(IIPF)%QUERY(I)%IACT
  WRITE(IU,'(1X,I2)') IPFPLOT(IIPF)%QUERY(I)%ANDOR
  J=IPFPLOT(IIPF)%QUERY(I)%IFIELD
  WRITE(IU,'(1X,A)')  TRIM(UQLABELS(J))
  WRITE(IU,'(1X,I2)') IPFPLOT(IIPF)%QUERY(I)%OPER
  WRITE(IU,'(1X,A) ') TRIM(IPFPLOT(IIPF)%QUERY(I)%CVALUE)
  WRITE(IU,'(1X,A) ') TRIM(IPFPLOT(IIPF)%QUERY(I)%FNAME1)
  WRITE(IU,'(1X,A) ') TRIM(IPFPLOT(IIPF)%QUERY(I)%FNAME2)
 ENDDO
 CLOSE(IU)
...
ILOOP: DO I=1,SIZE(IDFS)
 ...
 ALLOCATE(LEXQ(IPFPLOT(IIPF)%NQUERY))
 ...
 DO III=1,NPNT
  ...
  DO I=1,IPFPLOT(IIPF)%NQUERY
   ...
   SELECT CASE (IPFPLOT(IIPF)%QUERY(I)%OPER)
    CASE (1)
     IF(III.LT.NPNT)THEN
      IF(ZBH(III).GT.ZINT(1).AND.ZBH(III+1).LT.ZINT(1))LEXQ(I)=.TRUE.
     ENDIF
    CASE (2)
     IF(ZBH(III).GT.ZINT(1))LEXQ(I)=.TRUE.
    CASE (3)
     IF(ZBH(III).LE.ZINT(1))LEXQ(I)=.TRUE.
    CASE (4)
     IF(ZBH(III).LE.ZINT(1).AND.ZBH(III).GT.ZINT(2))LEXQ(I)=.TRUE.
    !## rest of operators work as logicals
    CASE DEFAULT
     !## selected attribute
     JJ=IPFPLOT(IIPF)%QUERY(I)%IFIELD
  ...
  IF(IALL.EQ.2)THEN
   ...
   ENDIF
  ELSE
   !## stop searching, a part of well agrees with query
   IF(L)EXIT
  ENDIF
...
! RBH(IPOS2)=RBH(IPOS2)
 ...
 ALLOCATE(XBH_DUMMY(MM),YBH_DUMMY(MM),ZBH_DUMMY(MM),CBH_DUMMY(MM),RBH_DUMMY(MM))
 ...
 DEALLOCATE(XBH,YBH,ZBH,CBH,RBH)
