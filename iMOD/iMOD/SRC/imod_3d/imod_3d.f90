...
! CALL GLGETBOOLEANV(GL_STENCIL_TEST,LEX)  !TRUE
 ...
 I=INT(100.0D0*REAL(F_L1_AMBIENT)) ; CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR2,I)
 I=INT(100.0D0*REAL(F_L1_DIFFUSE)) ; CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR3,I)
 I=INT(100.0D0*REAL(F_L1_SPECULAR)); CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR4,I)
 ...
 IF(J.GT.0)THEN
  ...
  DO I=1,SIZE(MP)
   IF(MP(I)%IACT.AND.MP(I)%ISEL.AND.MP(I)%IPLOT.EQ.6)THEN
    ...
    IF(LEX)THEN
     CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,MP(I)%IDFNAME)
     DO J=1,SHP%NPOL
...
! IF(NIPFLIST.GT.0)CALL IMOD3D_SELECTOBJECT_INIT()
 ...
 DO
  ...
  IF(.NOT.UTL_EQUALS_REAL(Z1,Z2))EXIT !REAL(BOT%Z)-DXY,REAL(BOT%Z)))EXIT
  DXY=DXY*10.0D0
 ENDDO
 BOT%Z=BOT%Z-DXY
 TOP%Z=TOP%Z+DXY
