...
USE MOD_PMANAGER_UTL, ONLY: PMANAGERSTART,PMANAGER_UTL_SHOW
USE MOD_PMANAGER, ONLY: PMANAGERRUN
    ...
    CASE (ID_DMDLTAB1)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (ID_PRJMANAGER)
       ...
       IF(PMANAGERRUN(ID_OPENRUN,FNAME,0))CALL PMANAGER_UTL_SHOW(1)
      !## copy runfile
      CASE (ID_COPY)
      ...
      CASE (ID_ZOOMFULL)
       CALL IDFZOOM(ID_DMODEL,0.0D0,0.0D0,0)
       CALL IDFPLOTFAST(1)
     END SELECT
...
! ENDIF
 ...
 !###======================================================================
 SUBROUTINE MODEL1MOUSEMOVE(MOUSEX,MOUSEY,IDOWN,DOWNX,DOWNY)
 !###======================================================================
 IMPLICIT NONE
 ...
 LINE=TRIM(ITOS(NMULT))//','//TRIM(ITOS(IDBG))//','//TRIM(ITOS(IDOUBLE))//','//TRIM(ITOS(IPOSWEL))//',' &
           //TRIM(ITOS(0))//','//TRIM(ITOS(IBUDGET))//','//TRIM(RTOS(MINKD,'F',3))//','//TRIM(RTOS(MINC,'F',3))
 WRITE(JU,'(A)') TRIM(LINE)
 ...
 LINE=TRIM(ITOS(MXITER))//','//TRIM(ITOS(NITER))//','// &
      TRIM(RTOS(HCLOSE,'F',3))//','//TRIM(RTOS(RCLOSE,'F',3))//',' &
           //TRIM(RTOS(RELAX,'F',3))
 ...
 REAL(KIND=DP_KIND) :: X1,Y1,X2,Y2,DX,BUFFER
...
! LCOPY=.FALSE.; IF(SIMBOX(1).EQ.SIMBOX(3).AND.SIMBOX(2).EQ.SIMBOX(4))LCOPY=.TRUE.
 ...
 IF(IBATCH.EQ.0.OR.SIMCSIZE.GT.0.0D0)THEN
  LCOPY=.FALSE.
  READ(IU,'(A256)') LINE
  DO
  ...
  ENDDO
 ELSE
  SIMBOX(1)=-10.0D10; SIMBOX(2)=-10.0D10 
  SIMBOX(3)= 10.0D10; SIMBOX(4)= 10.0D10
 ENDIF
...
MAINLOOP: DO
 ...
 DO
  ...
  SELECT CASE(ITYPE)
   ...
   CASE(MOUSEMOVE)
    ...
    XC2=MOUSEX !MESSAGE%GX
    YC2=MOUSEY !MESSAGE%GY
 ...
 CALL WDIALOGSELECT(ID_DMDLTAB6)
 CALL WDIALOGPUTDOUBLE(IDF_REAL1,HCLOSE,'(F15.3)')
 CALL WDIALOGPUTDOUBLE(IDF_REAL2,RCLOSE,'(F15.3)')
