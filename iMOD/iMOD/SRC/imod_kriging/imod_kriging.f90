...
!   Y1=Y0-KRIGING_DIST*SIN(A)
 ...
 ELSE
  ...
  SELECT CASE (ABS(KTYPE))
   ...
   CASE (4) !## gaussian
    H=1.0D0-EXP(-3.0*(DXY**2.0)/(RANGE**2.0))
   CASE (5) !## power
    H=DXY**0.5
   CASE DEFAULT
...
! nugget=5.0
 ...
 CALL WDIALOGPUTDOUBLE(IDF_REAL4,COINCIDENTDIST,'(F10.2)')  
 CALL WDIALOGPUTDOUBLE(IDF_REAL1,SILL,'(F10.2)')  
 CALL WDIALOGPUTDOUBLE(IDF_REAL2,RANGE,'(F10.2)') 
 CALL WDIALOGPUTDOUBLE(IDF_REAL3,NUGGET,'(F10.2)')  
