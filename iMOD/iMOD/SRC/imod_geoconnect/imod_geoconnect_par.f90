...
CONTAINS
 ...
 IF(IWINDOW.EQ.1)THEN
  ...
  CALL WDIALOGPUTDOUBLE(IDF_REAL5,100.0D0,'(F10.2)')
 ELSEIF(IWINDOW.EQ.2)THEN
  CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO2)
  CALL WDIALOGPUTDOUBLE(IDF_REAL1,IDF%XMIN,'(F10.2)'); CALL WDIALOGPUTDOUBLE(IDF_REAL2,IDF%YMIN,'(F10.2)')
  CALL WDIALOGPUTDOUBLE(IDF_REAL3,IDF%XMAX,'(F10.2)'); CALL WDIALOGPUTDOUBLE(IDF_REAL4,IDF%YMAX,'(F10.2)')
  CALL WDIALOGPUTDOUBLE(IDF_REAL5,IDF%DX,'(F10.2)')
 ENDIF

 !## read formation name from grid
 DO I=1,NLAYR
  !## write factor related to formation name from grid
  CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,3,I,IPFAC(I)%FACT,'(F10.3)')
 ENDDO
