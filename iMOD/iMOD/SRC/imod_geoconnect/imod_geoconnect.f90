...
CONTAINS
 ...
 IF(DX.GT.0.0D0)THEN
  ...
  CALL WDIALOGPUTDOUBLE(IDF_REAL1,X1,'(F10.2)'); CALL WDIALOGPUTDOUBLE(IDF_REAL2,Y1,'(F10.2)')
  CALL WDIALOGPUTDOUBLE(IDF_REAL3,X2,'(F10.2)'); CALL WDIALOGPUTDOUBLE(IDF_REAL4,Y2,'(F10.2)')     
 ENDIF
 ...
 DO
  ...
  SELECT CASE (ITYPE)
   CASE (MOUSEMOVE)
    ...
    CALL UTL_PLOTLOCATIONIDF(TOPM(1),IROW,ICOL)
    !## get fractions
    CALL GC_IDENTIFY_COMPUTE(REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8),TM,BM)
...
!    CALL WDIALOGSELECT(ID_DIDFINFO)
 ...
 N=0; DO I=1,NLAYR
  ...
  K=1; DO J=1,NLAYM
   ...
   ELSE
    CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,K,N,IPFAC(I)%FVAL(J)*100.0D0,'(F10.3)')
   ENDIF
