...
USE WINTERACTER
USE RESOURCE
USE MOD_UTL, ONLY : ITOS,RTOS,LISTNAME,UTL_IMODFILLMENU,UTL_CAP,UTL_GETUNIT,UTL_MESSAGEHANDLE,UTL_PLOT1BITMAP, &
   UTL_PLOT2BITMAP,DBL_IGRINSIDEPOLYGON
USE MOD_POLYGON_PAR
...
! CALL WINDOWOUTSTATUSBAR(1,TRIM(STRING))
 ...
 I=0
 K=0
 IF(SHP%NPOL.GT.0.AND.SUM(SHP%POL(1:SHP%NPOL)%IACT).EQ.1)I=1
 IF(SHP%NPOL.LT.MAXSHAPES)K=1

 CALL WDIALOGSELECT(ID_DIR_PMTAB1TAB1)

 !## fields on targets tab
 CALL WDIALOGFIELDSTATE(ID_ADD,I)
 IF(SHP%NPOL.LE.1)CALL WDIALOGFIELDSTATE(ID_COPY,0)
 IF(SHP%NPOL.GT.1)CALL WDIALOGFIELDSTATE(ID_COPY,1)
! CALL WDIALOGFIELDSTATE(ID_COPY,I)
 ...
 CALL WDIALOGSELECT(ID_DIR_PMTAB1TAB1)
 IF(I.EQ.0)THEN
  IF(SUM(SHP%POL(1:SHP%NPOL)%IACT).EQ.0)CALL WDIALOGPUTSTRING(IDF_STRING1,'No polygon selected. You need to create and/or select ONE '//&
  'polygon to be able to assign a target definition to it.')
  IF(SUM(SHP%POL(1:SHP%NPOL)%IACT).GT.1)CALL WDIALOGPUTSTRING(IDF_STRING1,'More than one polygon selected. You need to select ONE '//&
  'polygon to be able to assign a target definition to it.')
 ELSE
  !## fill field
 ...
 DO IPOL=1,TTREE(IFIELD)%NPOL
  IF(SHP%POL(IPOL)%IACT.EQ.1)EXIT
 ENDDO
...
! WRITE(*,*) 'TAB2',SHP%NPOL,SUM(SHP%POL(1:SHP%NPOL)%IACT)

 I=0
 J=0
 K=0
 IF(SHP%NPOL.GT.0.AND.SUM(SHP%POL(1:SHP%NPOL)%IACT).EQ.1)I=1
 IF(SHP%NPOL.GT.0.AND.SUM(SHP%POL(1:SHP%NPOL)%IACT).GT.0)J=1
 IF(SHP%NPOL.LT.MAXSHAPES)K=1
...
! INTEGER :: ITREE,IFIELD
 ...
 IF(NLIST.GT.0)THEN
  ...
  IF(LEX)THEN
   CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=IDFNAME,LEGNAME=TRIM(TARGETLEG))
   !call idfplotfast()       
   !## draw legend
...
!  CALL POLYGON1DRAWSHAPE(1,SHP%NPOL)
 ...
 IF(SUM(SHP%POL(1:SHP%NPOL)%IACT).EQ.1)THEN
  DO IPOL=1,TTREE(IFIELD)%NPOL
   IF(SHP%POL(IPOL)%IACT.EQ.1)EXIT
  ENDDO
...
!         ' (target: '//TRIM(ITOS(TTREE(IFIELD)%POL(IPOL)%EFFECT))//'%)'
 ...
 IF(IRES.LE.NRES)THEN
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(IDFRESLIST(I))//'.IDF',LEGNAME=TRIM(RES(IRES)%LEGRES))
  !call idfplotfast()       
  J=1
 ELSE
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(IDFRESLIST(I))//'.IDF') !,'')
  !call idfplotfast()       
  J=0
 ENDIF
...
! CALL IR1FIELDS_GETIPERIRES(PERRES(I),IPER,IRES)
 ...
 REAL(KIND=DP_KIND) :: X,Y,XX,YY,XRESULT,XMIN,YMIN,XMAX,YMAX
 ...
 DO IPOL=1,TTREE(ITARGET)%NPOL
  ...
  DO IDEF=1,TTREE(ITARGET)%POL(IPOL)%NDEF
   TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%MEAN      =0.0D0
      ...
      IF(TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%INEWP.EQ.IPER.AND.&
       ...
       DO IROW=IR1,IR2
        DO ICOL=IC1,IC2
         ...
         !## inside polygon?
         IF(DBL_IGRINSIDEPOLYGON(X,Y,TTREE(ITARGET)%POL(IPOL)%X,TTREE(ITARGET)%POL(IPOL)%Y, &
 ...
 DO IROW=1,IDFDIFF(2)%NROW
  DO ICOL=1,IDFDIFF(2)%NCOL
   ...
   DO IPOL=1,TTREE(ITARGET)%NPOL
    !## inside polygon?
    IF(DBL_IGRINSIDEPOLYGON(X,Y,TTREE(ITARGET)%POL(IPOL)%X,TTREE(ITARGET)%POL(IPOL)%Y,TTREE(ITARGET)%POL(IPOL)%NCRD).EQ.1)THEN
     ...
     DO IDEF=1,TTREE(ITARGET)%POL(IPOL)%NDEF
      ...
      XX=TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%MEAN
      IF(ABS(XX-TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%LOWER).LE.0.01D0.AND. &
         ABS(XX-TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%UPPER).LE.0.01D0)THEN
       XX=100.0D0/REAL(TTREE(ITARGET)%POL(IPOL)%NDEF) !## to make it 100%
      ELSE
       XX=0.0D0
      ENDIF
