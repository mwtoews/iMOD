...
USE MOD_UTL, ONLY : ITOS,RTOS,UTL_GETUNIT,UTL_INVERSECOLOUR,UTL_CAP,UTL_CLOSEUNITS,UTL_MESSAGEHANDLE,UTL_IDFSNAPTOGRID,UTL_WAITMESSAGE,&
                    UTL_DEL1TREE,UTL_CREATEDIR,UTL_READINITFILE,DBL_IGRINSIDEPOLYGON
USE MOD_POLYGON_PAR
...
CHARACTER(LEN=256),PRIVATE :: DIR
CHARACTER(LEN=256),POINTER,DIMENSION(:),PRIVATE :: QPFLISTNAME
...
!    WRITE(*,*) 'ERROR IN IR2COUNTIMP'
 ...
 ELSE
  IF(JFIELD.LE.N)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You cannot give a duplicate name','Error')
  ELSE
   IR1CHECKCNAME=.TRUE.
  ENDIF
 ENDIF
...
! CALL WDIALOGGETMENU(IDF_MENU1,SHP%POL%IACT)
 ...
 IF(IC.EQ.1)THEN
  ...
  IF(SHP%NPOL.EQ.TTREE(IFIELD)%NPOL)THEN!RETURN

! WRITE(*,*) SHP%POL(1:SHP%NPOL)%IACT
...
!WRITE(*,*) SHP%NPOL,MTREE(IFIELD)%NPOL

  IPOL2=0

  IF(SHP%NPOL.EQ.MTREE(IFIELD)%NPOL)THEN!RETURN
   ...
   DO IPOL1=1,SHP%NPOL
    ...
    ENDIF
!WRITE(*,*) IPOL1,SHP%NPOL,MTREE(IFIELD)%POL(IPOL1)%NMES
   END DO
!WRITE(*,*) 'SHP%NPOL=',SHP%NPOL
  ENDIF
...
!  WRITE(*,*) 'IPOL2=',IPOL2
  ...
  !## clean removed definition fields
  !## polygon information is adjusted in polygon1delete()
  DO IPOL1=IPOL2,MAXSHAPES!SHP%NPOL
   !## already target to assigned - clean memory
...
!  ENDIF
 ...
 CALL MAIN_UTL_INACTMODULE(ID_IRDATABASE)
   ...
   CASE (FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_STRING1,IDF_RADIO3,IDF_RADIO4,IDF_RADIO5)
      IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)CALL IR1STARTFILLMENU()
...
!       CALL UTL_GETHELP('2.1','GS.StartiMOD')
     ...
     CASE (ID_OPEN)
      ...
      I=1; IF(.NOT.IR1PRJFILES(ID_OPEN))I=0
      IF(I.EQ.0)THEN
       CALL IR1STARTINIT()
       CALL IR1STARTFILLMENU()
       CALL WDIALOGSHOW(-1,-1,0,3)
     ...
     CASE (IDOK)
      ...
      IF(I.EQ.2)THEN
       ...
       CALL WDIALOGUNLOAD()
       FNAME=TRIM(MAINRESDIR)//'\'//TRIM(IMFFILE)
       IF(.NOT.IR1PRJFILES(ID_OPEN,FNAME))THEN; ENDIF
      ELSE
       CALL WDIALOGUNLOAD()
!       IF(PLUGIN_INITMENU_FILL())THEN; ENDIF
 ...
 IF(ASSOCIATED(QPFLISTNAME))DEALLOCATE(QPFLISTNAME)

 END SUBROUTINE IR1START

 !###======================================================================
 SUBROUTINE IR1STARTINIT()
 !###======================================================================
 IMPLICIT NONE
 ...
 END SUBROUTINE IR1STARTINIT

 !###======================================================================
 SUBROUTINE IR1STARTFILLMENU()
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=52) :: WC
 INTEGER :: IRADIO
 ...
 LEX=UTL_DIRINFO_POINTER(DIR,WC,QPFLISTNAME,'F',CORDER=CORDER(IRADIO))
 IF(SIZE(QPFLISTNAME).LE.0)THEN
  IF(ASSOCIATED(QPFLISTNAME))DEALLOCATE(QPFLISTNAME)
  LEX=.FALSE.
 ENDIF
 ...
 IF(LEX)THEN
  ...
  CALL WDIALOGPUTMENU(IDF_MENU1,QPFLISTNAME,SIZE(QPFLISTNAME),1)
 ...
 END SUBROUTINE IR1STARTFILLMENU
 
 !###======================================================================
 SUBROUTINE IR1STARTFIELDS()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,K,IRADIO

 CALL WDIALOGGETRADIOBUTTON(IDF_RADIO1,I); I=I-1
 J=0; IF(ASSOCIATED(QPFLISTNAME))J=I*1
...
! TTREE(IFIELD)%POL(IPOL2)%EFFECT       =TTREE(IFIELD)%POL(IPOL1)%EFFECT
 ...
 IF(TRIM(PREFVAL(6)).EQ.'')THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot initiate IR-tool since the variable IRDBASE '//CHAR(13)// &
 ...
 IF(.NOT.LEX)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot initiate IR-tool since file: '//CHAR(13)// &
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open [ '// &
