...
CONTAINS
    ...
    ELSEIF(IP.EQ.1)THEN
     ...
     IF(IDOWN_PRF.EQ.1)THEN
      ...
      IF(LLINE)THEN
       ...
       CALL DBL_IGRJOIN(XY(1,NXY),XY(2,NXY),XCRD,YCRD)
 ...
 IMPLICIT NONE
 INTEGER :: I,IWINID
 REAL(KIND=DP_KIND) :: CRITDIST,CRITPROF,DXY,X1,Y1

 CRITDIST=SQRT((MPW%XMAX-MPW%XMIN)**2.0+(MPW%YMAX-MPW%YMIN)**2.0)/100.0D0
 CRITPROF=0.0D0
 ...
 IF(MESSAGE%WIN.EQ.ID_DSERIESTAB1)THEN
  ...
  IF(IP.EQ.0.AND..NOT.LMOVEPROF)THEN
   DXY=(SQRT((MPW%XMAX-MPW%XMIN)**2.0+(MPW%YMAX-MPW%YMIN)**2.0))/100.0D0
   !## check box
   IXY=0
   DO I=1,NXY
   ...
   IF(I.GT.NXY)THEN
    DO I=2,NXY
     IF(DBL_IGRDISTANCELINE(XY(1,I-1),XY(2,I-1),XY(1,I),XY(2,I),REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8),0).LE.CRITDIST)EXIT
    END DO
    IF(I.LE.NXY)THEN
  ...
  IF(IDOWN_PRF.EQ.1)THEN
   ...
   X1   =MESSAGE%GX
   Y1   =MESSAGE%GY
   CALL DBL_IGRJOIN(XY(1,NXY),XY(2,NXY),X1,Y1)
   CALL PROFILE_PLOTVIEWBOX(XY(1,NXY),XY(2,NXY),X1,Y1)
   CALL IGRLINEWIDTH(1)

   IF(ABS(XCRD-MESSAGE%GX).GT.CRITPROF.OR.ABS(YCRD-REAL(MESSAGE%GY,8)).GT.CRITPROF)THEN
 ...
 ELSE
  ...
  IF(I.LE.SIZE(IWINPROFILE))THEN
   ...
   DO IWINID=1,SIZE(IWINPROFILE)
    ...
    CALL WINDOWOUTSTATUSBAR(1,'Distance: '//TRIM(ITOS(INT(MESSAGE%GX)))//' m, Map Value: '//TRIM(RTOS(REAL(MESSAGE%GY,8),'F',2))) 
   ENDDO
   !## solid adjustment?
   IF(NXY.GT.0)THEN
    IF(IDOWN_SOLID.EQ.0)THEN 
     ...
     IF(ICURSOR_SOLID.EQ.0)THEN
      ...
      ELSE
       ...
       CALL PROFILE_PLOTSURVEY_MOUSEMOVE(MESSAGE%XPIX,MESSAGE%YPIX,ICURSOR_SURVEY,ICRD_SURVEY)
    ...
    ELSE
     ...
     ENDIF
     IF(ICURSOR_SURVEY.NE.0)THEN
      CALL PROFILE_PLOTSURVEY_ADJUST(MESSAGE%XPIX,MESSAGE%YPIX,ICURSOR_SURVEY) 
      CALL PROFILE_PLOT()
     ENDIF
    ENDIF
   ENDIF
   !## view on 2d-plot old position of cross-section
   CALL PROFILE_CLEAR(); XPOSPROF=REAL(MESSAGE%GX,8); CALL PROFILE_CLEAR()
 ...
 ENDDO
 XY(1,ISEG)=X; XY(2,ISEG)=Y; XYLABEL(ISEG)=TRIM(LABELNAME) !RTOS(XPOSPROF,'F',2))
...
!     CALL WDIALOGSELECT(ID_DIPFINFOSERIE); CALL WDIALOGUNLOAD() 
 ...
 DO I=2,NXY
  CALL IGRLINEWIDTH(LINEWIDTHPLOT)
  CALL IGRCOLOURN(LINECOLORPLOT)
  CALL DBL_IGRJOIN(XY(1,I-1),XY(2,I-1),XY(1,I),XY(2,I))
  CALL IGRLINEWIDTH(1)
 ...
 CALL DBL_IGRPOLYGONSIMPLE(XYPOL(:,1),XYPOL(:,2),4)
 CALL IGRFILLPATTERN(OUTLINE)
 CALL IGRCOLOURN(WRGB(255,255,255))
 CALL IGRLINEWIDTH(1)
...
!   CALL WGRIDCOLOURCELL(IDF_GRID1,1,I,-1,PROFIDF(IPOS(I))%SCOLOR)
 ...
 DO
  ...
  MOUSEX=REAL(MESSAGE%GX,8)
  MOUSEY=REAL(MESSAGE%GY,8)
   ...
   SELECT CASE (ITYPE)
    ...
    CASE(MOUSEMOVE)
     ...
     IF(NMOVE.EQ.1)THEN
      ...
      X1=REAL(MESSAGE%GX,8)
      Y1=REAL(MESSAGE%GY,8)
     ENDIF
   ...
   SELECT CASE(ITYPE)
    ...
    CASE(MOUSEMOVE)
     ...
     IF(IWINID.LE.SIZE(IWINPROFILE))THEN
      ...
      XC2=REAL(MESSAGE%GX,8)
      YC2=REAL(MESSAGE%GY,8)
      ...
      SELECT CASE (MESSAGE%VALUE1)
       CASE (1)
        ...
        ELSE
         ...
         YMAX=MAX(YC1,REAL(MESSAGE%GY,8))
         YMIN=MIN(YC1,REAL(MESSAGE%GY,8))
         EXIT
        ENDIF
       CASE (3)
...
!      CALL PROFILE_LEGENDUPDATE()
  ...
  CASE (ID_DSERIESPROP)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE(IDOK)
     ...
     CALL WDIALOGGETCHECKBOX(IDF_CHECK10,GRAPHPERCENTAGES1)
     ICOL1=26; IF(GRAPHPERCENTAGES1.EQ.1)CALL WDIALOGGETINTEGER(IDF_INTEGER1,ICOL1)
     CALL WDIALOGGETCHECKBOX(IDF_CHECK11,GRAPHPERCENTAGES2)
     ICOL2=28; IF(GRAPHPERCENTAGES2.EQ.1)CALL WDIALOGGETINTEGER(IDF_INTEGER2,ICOL2)
  ...
  IF(MP(MPLOT(IIPF))%PRFTYPE.EQ.1)THEN
   ...
   DO IROW=1,IPF(IIPF)%NROW
    ...
    IF(X.GE.MPW%XMIN.AND.X.LE.MPW%XMAX.AND.Y.GE.MPW%YMIN.AND.Y.LE.MPW%YMAX)THEN
     D=(X-MESSAGE%GX)**2.0+(Y-REAL(MESSAGE%GY,8))**2.0
 ...
 IF(CODE.EQ.0)THEN
  IF(NXY.GT.0)THEN
   ...
   DO I=1,NXY
    CALL WGRIDPUTCELLDOUBLE  (IDF_GRID1,1,I,XY(1,I))
    CALL WGRIDPUTCELLDOUBLE  (IDF_GRID1,2,I,XY(2,I)) 
  ...
  IF(MP(IPLOT)%ISEL.AND.(MP(IPLOT)%IPLOT.EQ.1.OR.MP(IPLOT)%IPLOT.EQ.5))THEN
   ...
   IF(MP(IPLOT)%IPLOT.EQ.5)THEN
    ...
    IF(READMDF(MP(IPLOT)%IDFNAME,N))THEN
     DO I=1,N
      MXNIDF=MXNIDF+1
      IF(.NOT.IDFREAD(PROFIDF(MXNIDF)%IDF,MDF(I)%FNAME,0))K=K+1 !EXIT
