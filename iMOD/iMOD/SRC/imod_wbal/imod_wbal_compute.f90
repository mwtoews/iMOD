...
CONTAINS
 ...
 INTEGER :: I,J,ILAY,JLAY,IY,IM,ID,IU,ICSV,IBAL,ITYPE, &
   JSYS,NBDG,IBDG,NFILES,IFILES,NU,IROW,ICOL,IDATE,ITRY
 LOGICAL :: LEX,LIP
 ...
 ELSEIF(WBAL_ISEL.EQ.2)THEN  
  CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,WBAL_GENFNAME)
 ...
 WCTP%UNIT='unknown'
 DO I=1,SIZE(WCTP)
  IF(INDEX(UTL_CAP(WCTP(I)%BDGNAME,'U'),'BDG').GT.0)WCTP(I)%UNIT='m3/d'
 ENDDO
...
!    IF(IBAL.EQ.3)THEN
 ...
 IF(WBAL_ISTEADY.EQ.0)THEN     !## transient
  ...
  DO I=1,NIP
   ...
   WRITE(IUTXT(I),'(A,G10.5)') 'Date,',HUGE(1.0D0)
   DO IBAL=1,SIZE(WCTP)
    DO JSYS=1,WCTP(IBAL)%NSYS
     IF(.NOT.ASSOCIATED(WCTP(IBAL)%ISYS))THEN
      WRITE(IUTXT(I),'(A,G10.5)') TRIM(WCTP(IBAL)%BDGNAME)//'_in'//TRIM(WCTP(IBAL)%UNIT),HUGE(1.0D0)
      WRITE(IUTXT(I),'(A,G10.5)') TRIM(WCTP(IBAL)%BDGNAME)//'_out'//TRIM(WCTP(IBAL)%UNIT),HUGE(1.0D0)
      WRITE(IUTXT(I),'(A,G10.5)') TRIM(WCTP(IBAL)%BDGNAME)//'_sum'//TRIM(WCTP(IBAL)%UNIT),HUGE(1.0D0)
     ELSE
      LINE=TRIM(WCTP(IBAL)%BDGNAME)//'sys'//TRIM(ITOS(WCTP(IBAL)%ISYS(JSYS)))//'_in'//TRIM(WCTP(IBAL)%UNIT)
      WRITE(IUTXT(I),'(A,G10.5)') TRIM(LINE)//',',HUGE(1.0D0)
