...
USE RESOURCE
USE MOD_DINO_PAR
USE MOD_UTL, ONLY : ITOS,RTOS,UTL_CREATEDIR,UTL_GETUNIT,UTL_CAP,UTL_CAP_BIG,DBL_IGRINSIDESHAPE
...
! CHARACTER(LEN=52),POINTER,DIMENSION(:,:) :: VAR
 ...
 IF(GENFNAME.NE.'')THEN
  ...
  DO I=1,SHP%NPOL
   MPW%XMIN=MIN(MPW%XMIN,MINVAL(SHP%POL(I)%X(1:SHP%POL(I)%N)))
   MPW%XMAX=MAX(MPW%XMAX,MAXVAL(SHP%POL(I)%X(1:SHP%POL(I)%N)))
   MPW%YMIN=MIN(MPW%YMIN,MINVAL(SHP%POL(I)%Y(1:SHP%POL(I)%N)))
   MPW%YMAX=MAX(MPW%YMAX,MAXVAL(SHP%POL(I)%Y(1:SHP%POL(I)%N)))
  ENDDO
  IWINDOW=1; N=MAXVAL(SHP%POL(1:SHP%NPOL)%N)
 ENDIF
 ...
 DO ICSV=1,SIZE(CSVFNAME)
  ...
  DO IND=1,ND(ICSV)
   ...
   IF(SHP%NPOL.GT.0)THEN
    DO I=1,SHP%NPOL
     IF(DBL_IGRINSIDESHAPE(REAL(DINO(JB)%IX,8),REAL(DINO(JB)%IY,8),SHP%POL(I)).EQ.1)EXIT
!     IF(DBL_IGRINSIDEPOLYGON(REAL(DINO(JB)%IX,8),REAL(DINO(JB)%IY,8),SHP%POL(I)%X,SHP%POL(I)%Y,SHP%POL(I)%N).EQ.1)EXIT
    ENDDO
    IF(I.GT.SHP%NPOL)DINO(JB)%OPB='OUT'
   ENDIF
   IF(TRIM(DINO(JB)%OPB).NE.'OPB')CYCLE
   ...
   DO I=2,ND(ICSV)
    ...
    ELSE
     ...
     LINE=TRIM(ITOS(DINO(I-1)%IX))//','//TRIM(ITOS(DINO(I-1)%IY))//','//TRIM(FNAME)//'\'//TRIM(DINO(I-1)%ID)//','// &
          TRIM(RTOS(DINO(I-1)%MV/1000.0D0,'F',2))//','//TRIM(RTOS(DINO(I-1)%YEND/1000.0D0,'F',2))
     ...
     DO II=I2,I1,-1
      ...
      LINE=TRIM(RTOS(DINO(II)%YTOP/1000.0D0,'F',2))//','// &
           '"'//TRIM(DINO(II)%LITH)//'",'// &
           TRIM(ITOS(IGEOTOP))//','// &
           '"'//TRIM(DINO(II)%ZMK)//'",'// &
           '"'//TRIM(BIJM)//'",'// &
     ...
     END DO
     LINE=TRIM(RTOS(DINO(I1)%YBOT/1000.0D0,'F',2))//',-,-,-,-,-'
     WRITE(KU,'(A)') TRIM(LINE)
     CLOSE(KU) 
