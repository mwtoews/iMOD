...
USE MOD_POLYGON_UTL, ONLY : POLYGON1SAVELOADSHAPE
 ...
 REAL,DIMENSION(:),ALLOCATABLE :: XPOL,YPOL
 ...
 IF(GENFNAME.NE.'')THEN
  ...
  ENDDO
  IWINDOW=1
  N=MAXVAL(SHPNCRD(1:SHPNO)); ALLOCATE(XPOL(N),YPOL(N))
 ENDIF
 ...
 DO ICSV=1,SIZE(CSVFNAME)
  ...
  IF(IU.EQ.0)THEN; WRITE(*,'(/A/)') 'Can not open file: '//TRIM(CSVFNAME(ICSV)); RETURN; ENDIF
 ...
 DO ICSV=1,SIZE(CSVFNAME)
  ...
  DO IND=1,ND(ICSV)
   ...
   IF(SHPNO.GT.0)THEN
    DO I=1,SHPNO
     IF(UTL_INSIDEPOLYGON(REAL(DINO(JB)%IX),REAL(DINO(JB)%IY),SHPXC(:,I),SHPYC(:,I),XPOL,YPOL,SHPNCRD(I)).EQ.1)EXIT
    ENDDO
