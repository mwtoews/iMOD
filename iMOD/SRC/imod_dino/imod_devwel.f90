...
USE MOD_UTL, ONLY : UTL_CREATEDIR,UTL_GENLABELSREAD,NV,NL,VAR,UTL_GENLABELSDEALLOCATE,UTL_GETUNIT, &
   MAXLEN,ITOS,UTL_CAP,UTL_ROTATE_XYZ,PI
...
!   CALL UTL_ROTATE_XYZ(X,Y,Z,AX,AY,AZ)
   
   WRITE(JU,'(3(F10.2,1X))') X1,Y1,Z1
  ...
  WRITE(IU,'(A)') TRIM(VAR(ICOLS(2),I1))//','//TRIM(VAR(ICOLS(3),I1))//',"'//TRIM(VAR(ICOLS(1),I1))//'",'//TRIM(CL1)
