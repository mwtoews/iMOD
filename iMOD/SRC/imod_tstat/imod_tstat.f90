...
USE DATEVAR
USE MOD_UTL, ONLY : UTL_FILLDATES,GDATE,UTL_MESSAGEHANDLE,OSD_OPEN,UTL_GETUNIT,ITOS,RTOS,UTL_GETMED,UTL_GETHIST, &
 UTL_WAITMESSAGE,UTL_WSELECTFILE,UTL_INSIDEPOLYGON,UTL_GETUNIQUE_INT,UTL_GETMED_INVERSE,JDATETOIDATE,UTL_STDEF,UTL_CREATEDIR
USE MOD_TSTAT_PAR
...
CONTAINS
 ...
 REAL :: M,H,W,DH,DHW,VAR,MEAN,XC,YC,MH,MM,STVDH,MDH,XCOR,YCOR,ZCOR ,DYNH,DYNM !,STVH,STVM
 ...
 IF(IOS.NE.0)THEN
  WRITE(*,'(A)') 'Error: Can not write file: ['//TRIM(OUTNAME)//']'; RETURN
 ENDIF
 ...
 DO IZ=1,NZ
  ...
  WRITE(JU,'(A52,9(A1,A15),A1,A10)') 'Name',',','MH',',','MM',',','MH-MM',',','STVDH',',','DYNH',',','DYNM',',','DYNH-DYNM',',','XCOR',',','W',',','JJ'
   ...
   ELSE
    ...
    DO I=1,IPF(1)%NROW
     ...
     IF(IPFOPENASSFILE(IU,1,FNAME).AND. &
      ...
      ENDDO
      CALL UTL_STDEF(X,JJ,-999.99,STVDH,MDH,NPOP)
      !## get dynamic measure
      JJ=0; DO J=1,ASSF(1)%NRASS
       M=ASSF(1)%MEASURE(1,J); H=ASSF(1)%MEASURE(2,J)   
       IF(H.NE.ASSF(1)%NODATA(3).AND.M.NE.ASSF(1)%NODATA(2))THEN; JJ=JJ+1; X(JJ)=H; ENDIF      
      ENDDO
      CALL UTL_GETMED(X,JJ,-999.99,(/10.0,90.0/),2,NPOP,XMED); DYNH=XMED(2)-XMED(1)
      !## get dynamic model
      JJ=0; DO J=1,ASSF(1)%NRASS
       M=ASSF(1)%MEASURE(1,J); H=ASSF(1)%MEASURE(2,J)   
       IF(H.NE.ASSF(1)%NODATA(3).AND.M.NE.ASSF(1)%NODATA(2))THEN; JJ=JJ+1; X(JJ)=M; ENDIF      
      ENDDO
      CALL UTL_GETMED(X,JJ,-999.99,(/10.0,90.0/),2,NPOP,XMED); DYNM=XMED(2)-XMED(1)
      !## get mean
      JJ=0; MH=0.0; MM=0.0
      DO J=1,ASSF(1)%NRASS
       M=ASSF(1)%MEASURE(1,J); H=ASSF(1)%MEASURE(2,J)   
      ...
      IF(JJ.GT.0)THEN
       ...
       RES(N)%M   =   STVDH !STVM
       RES(N)%H   =   DYNH !STVH
       RES(N)%DH  =   MDH !STVH-STVM)
       RES(N)%DHW =   DYNH-DYNM
       RES(N)%COR =   XCOR
       RES(N)%MM  =   MM
       RES(N)%MH  =   MH
       RES(N)%W   =   W
!       RES(N)%DHW =W*(STVH-STVM)
       RES(N)%ILAY=ILAY
       RES(N)%X   =XC
       RES(N)%Y   =YC
       RES(N)%CID ='timeseries\'//TRIM(IPF(1)%INFO(IPF(1)%ACOL,I)) !//'.'//TRIM(IPF(1)%FEXT)
...
!    WRITE(JU,'(/A)') 'Histogram'; DO I=1,SIZE(HIST)-1; WRITE(JU,'(2(A1,F10.2),F10.1)') '>',HIST(I),'<=',HIST(I+1),XHIST(I); ENDDO
  ...
  !## write ipf files
  DO IL=IL1,IL2
   FNAME='residual_ilay'//TRIM(ITOS(IL))//'izone'//TRIM(ITOS(IZONE(IZ)))//'.IPF'
...
!   WRITE(KU,*) 'X'; WRITE(KU,*) 'Y'; WRITE(KU,*) 'M'; WRITE(KU,*) 'H'; WRITE(KU,*) 'DH'; WRITE(KU,*) 'DHW'; WRITE(KU,*) 'W'; WRITE(KU,*) '0,TXT'
   J=0; DO I=1,N
    IF(RES(I)%ILAY.EQ.IL)THEN
     J=J+1
     IF(ICOLLECT.EQ.0)WRITE(KU,'(10(F10.3,A1))') RES(I)%X,',',RES(I)%Y,',',RES(I)%M,',',RES(I)%H,',',RES(I)%DHW,',',RES(I)%DH,',',RES(I)%MH,',',RES(I)%MM,',',RES(I)%COR,',',RES(I)%W !RES(I)%DHW/RES(I)%DH
     IF(ICOLLECT.EQ.1)WRITE(KU,'(10(F10.3,A))')  RES(I)%X,',',RES(I)%Y,',"'//TRIM(RES(I)%CID)//'",',RES(I)%M,',',RES(I)%H,',',RES(I)%DHW,',',RES(I)%DH,',',RES(I)%MH,',',RES(I)%MM,',',RES(I)%COR,',',RES(I)%W !RES(I)%DHW/RES(I)%DH
    ENDIF
   ENDDO
   CLOSE(KU)
  ENDDO
 ...
 SELECT CASE(ITYPE)
  ...
  CASE(PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    CASE (IDHELP)
       CALL IMODGETHELP('5.16','Compute Time-variant Statistics')
 ...
 REAL,ALLOCATABLE,DIMENSION(:) :: XPOL,YPOL,XPERC
 ...
 IF(TRIM(SURFACELEVEL).NE.'')THEN
  IF(.NOT.IDFREAD(IDF(1),SURFACELEVEL,0))THEN
   WRITE(*,'(A)') 'Can not open: '//TRIM(SURFACELEVEL); RETURN
  ENDIF
 ENDIF
 ...
 IF(IGEN.EQ.1)THEN
  ...
  IF(IOS.NE.0)THEN
   IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not translate percentiles to numeric values'//CHAR(13)// &
    TRIM(CXPERC),'Error')
   IF(IBATCH.EQ.1)WRITE(*,'(A)') 'Can not translate percentiles to numeric values:'//TRIM(CXPERC)
   RETURN
  ENDIF
 ENDIF
 ...
 ELSE
  MXPOL=1
 ENDIF
 ALLOCATE(XPOL(MXPOL),YPOL(MXPOL))
 ...
 IF(IOS.NE.0)THEN
  IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not open given result file (E)'//CHAR(13)// &
   '['//TRIM(OUTNAME)//']','Error')
  IF(IBATCH.EQ.1)WRITE(*,'(A)') 'Can not open given result file (E) ['//TRIM(OUTNAME)//']'
  RETURN
 ENDIF
 ...
 IF(TSTAT1READIPFS(IIPF,JIPF,IBATCH))THEN
  ...
  DO I=1,IPF(IIPF)%NROW
   ...
   IF(TSTAT1UTL_INSIDEPOLYGON(IPF(IIPF)%XYZ(1,I),IPF(IIPF)%XYZ(2,I),XPOL,YPOL,MXPOL,JPOL))THEN
    ! FR 20140206  if no MV given than MV is ten ontrecht 0
    ! MV=0.0; IF(IDF(1)%IU.NE.0)MV=IDFGETXYVAL(IDF(1),IPF(IIPF)%XYZ(1,I),IPF(IIPF)%XYZ(2,I))
...
!    write(*,'(a)') 'polygon: '//trim(line)
 ...
 IF(LEX.AND.EXITCODE.EQ.0)THEN
  IF(IBATCH.EQ.0)THEN
   ...
   CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfully created '//CHAR(13)//TRIM(OUTNAME)//CHAR(13)//&
   'File has been added to the iMOD-Manager','Information')
  ELSE
   WRITE(*,'(A)') 'Succesfully created '//TRIM(OUTNAME)
  ENDIF
 ENDIF
 ...
 !###======================================================================
 LOGICAL FUNCTION TSTAT1UTL_INSIDEPOLYGON(X,Y,XPOL,YPOL,MXPOL,IPOL)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: MXPOL
 REAL,DIMENSION(MXPOL),INTENT(INOUT) :: XPOL,YPOL
 REAL,INTENT(IN) :: X,Y
 ...
 IF(SHPNO.GT.0)THEN
  DO IPOL=1,SHPNO
   IF(X.GT.MINVAL(SHPXC(1:SHPNCRD(IPOL),IPOL)).AND.X.LT.MAXVAL(SHPXC(1:SHPNCRD(IPOL),IPOL)).AND. &
    ...
    IF(UTL_INSIDEPOLYGON(X,Y,SHPXC(:,IPOL),SHPYC(:,IPOL),XPOL,YPOL,SHPNCRD(IPOL)).EQ.1)EXIT
   ENDIF
  ENDDO
