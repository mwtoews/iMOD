...
CONTAINS
 ...
 !###====================================================================== 
 IMPLICIT NONE
 INTEGER :: IRUN,IEXE,IOS,I
 LOGICAL :: LEX
 
 DO IRUN=1,NRUN
  IUOUT=UTL_GETUNIT()
  I=INDEX(TRIM(RUN(IRUN)%FNAME),'\')
  OPEN(IUOUT,FILE=TRIM(OUTDIR)//'\testbank_out_'//TRIM(RUN(IRUN)%FNAME(I+1:LEN_TRIM(RUN(IRUN)%FNAME)-4))//'.txt',STATUS='UNKNOWN',ACTION='WRITE',IOSTAT=IOS)
  !IF(IOS.NE.0)CALL UTL_PRINTTEXT('OPENING OUTPUT FILE',2)
  DO IEXE=1,NEXE
