...
USE MOD_UTL, ONLY : UTL_MESSAGEHANDLE,UTL_CAP,UTL_INVERSECOLOUR,ITOS,RTOS,UTL_GETUNIT
...
USE MOD_3D_UTL, ONLY : IMOD3D_SETCOLOR,IMOD3D_GETCOLOR
...
! ISELECTED=0; 
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,IIPF,ISEL,IROW,ICOL,ICLR,MC,MAXROW
 REAL(KIND=DP_KIND) :: X,IWIDTH
 ...
 DO J=1,NIDFLIST
  ...
  IF(IROW.GE.1.AND.IROW.LE.IDF_SAMPLE(J)%NROW.AND. &
   ...
   IF(X.EQ.IDF_SAMPLE(J)%NODATA)THEN
    CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,'NoData')
   ELSE
    CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,TRIM(RTOS(X,'G',7)))
   ENDIF
  ELSE
   CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,'Outside')
  ENDIF
 END DO
 ...
 MAXROW=WINFOGRID(IDF_GRID1,GRIDROWSMAX)
 IF(ASSF(ISELECTED)%NRASS.GT.MAXROW)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can display a maximal number of '//TRIM(ITOS(MAXROW))//'row'//CHAR(13)// &
    'The current dataset contains '//TRIM(ITOS(ASSF(ISELECTED)%NRASS))//' rows'//CHAR(13)// &
    'iMOD excludes the remaning rows from the table','Warning')
 ENDIF
 ...
 DO I=1,MIN(MAXROW,ASSF(ISELECTED)%NRASS)

  IF(ASSF(ISELECTED)%ITOPIC.EQ.2)THEN
   CALL WGRIDPUTCELLDOUBLE(  IDF_GRID1,1,I,ASSF(ISELECTED)%Z(I),'(F15.3)')
  ...
  ELSEIF(ASSF(ISELECTED)%ITOPIC.EQ.4)THEN
   CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,1,I,ASSF(ISELECTED)%DX(I),'(F15.3)')
   CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,I,ASSF(ISELECTED)%DY(I),'(F15.3)')
   CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,3,I,ASSF(ISELECTED)%Z(I) ,'(F15.3)')
