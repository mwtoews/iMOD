...
CONTAINS
 ...
 DO I=1,ASSF(ISELECTED)%NRASS 
  IF(ASSF(ISELECTED)%ITOPIC.EQ.2)THEN
   DO J=1,ASSF(ISELECTED)%NCASS
    IF(IC(J).EQ.1)CALL WGRIDPUTCELLREAL(  IDF_GRID1,J,I,ASSF(ISELECTED)%Z(I),'(F10.2)')
    IF(IC(J).EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,J,I,ASSF(ISELECTED)%L(J,I))
   ENDDO
