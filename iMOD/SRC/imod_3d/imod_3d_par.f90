...
LOGICAL :: LCAP
INTEGER,SAVE :: IORTHO
INTEGER,PARAMETER :: MXROWFORFANCY=1500 !## maximum number of rows to change to non-fancy
...
TYPE IDFLISTINDEXOBJ
 INTEGER(KIND=GLUINT),POINTER,DIMENSION(:) :: INDEX=>NULL()
 INTEGER :: ISLIST   !## start of list
 INTEGER :: IELIST   !## end of list
END TYPE IDFLISTINDEXOBJ
TYPE(IDFLISTINDEXOBJ),ALLOCATABLE,DIMENSION(:) :: IDFLISTINDEX

INTEGER(KIND=GLUINT),ALLOCATABLE,DIMENSION(:) ::   GENLISTINDEX,IFFLISTINDEX,BMPLISTINDEX,STPLISTINDEX !IDFLISTINDEX
