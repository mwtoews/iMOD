...
USE MOD_3D_ENGINE
USE MOD_3D_PROCESS
USE MOD_SOLID_UTL, ONLY : SOLID_INITSPF,SOLIDDEALLOCATESLD
...
! BEGIN_LEFT%Y=WY
 ...
 ISOLID_3D=IACTSOLID
 IF(ISOLID_3D.EQ.0)CALL SOLIDDEALLOCATESLD()
 ...
 CALL WINDOWOPENCHILD(IWIN3D,FLAGS=IFLAGS,MENUID=ID_MENU9,DIALOGID=ID_D3DSETTINGS,TITLE=TITLE)
 TMPINT1=[2000,-1]; TMPINT2=[1,1]
 ...
 RAT=REAL(IWINWIDTH,8)/REAL(IWINHEIGHT,8)
...
! CALL GLGETBOOLEANV(GL_STENCIL_TEST,LEX)  !TRUE
 ...
 ALLOCATE(CLPPLOT(6))
 IF(MAXCPLANES(1).LT.6)THEN
  !## no clipping planes allowed ...
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'No clipping planes available','Warning')
 ENDIF
...
! ENDIF
 ...
 DO
  ...
  IF(.NOT.UTL_EQUALS_REAL(X1,X2))EXIT
  DXY=DXY*10.0D0
 ENDDO
 BOT%X=BOT%X-DXY
 TOP%X=TOP%X+DXY
 ...
 DO
  ...
  IF(.NOT.UTL_EQUALS_REAL(Y1,Y2))EXIT 
  DXY=DXY*10.0D0
 ENDDO
 BOT%Y=BOT%Y-DXY
 TOP%Y=TOP%Y+DXY
 ...
 CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB7)
 ALLOCATE(TMPNAME(NCLPLIST),TMPINT(NCLPLIST)); TMPNAME=CLPPLOT%FNAME; TMPINT=CLPPLOT%ISEL
