...
USE MOD_UTL, ONLY : UTL_MESSAGEHANDLE3D,UTL_MESSAGEHANDLE,IDFPLOT2BITMAP,RTOS,UTL_IDFSNAPTOGRID,UTL_LOADIMAGE
...
CONTAINS
 ...
 IDFM%YMAX=MIN(IDFM%YMAX,REAL(TOP%Y)) 
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=GLFLOAT) :: X1,X2,Y1,Y2,Z,ZF,DX,DY,XT1,XT2,YT1,YT2,Z1,Z2,Z3,Z4, &
         ZOFFSET,MPWDX,MPWDY 
 ...
 IF(IACTBITMAP.EQ.1)THEN
  ...
  IF(IMOD3D_CREATE_SXY(IDF(1),IIDF,DX,DY))THEN; ENDIF  
...
! CALL WBITMAPPUTDATA(MPW%IBITMAP,IBMPDATA)
 IF(.NOT.UTL_LOADIMAGE(TRIM(PREFVAL(1))//'\TMP\TMP.BMP',SIZE(IBMPDATA),IBMPDATA,0))THEN !,IPAL)
 ...
 IF(NIDFLIST.GT.0)THEN
  DO I=1,SIZE(IDFLISTINDEX); IF(IDFLISTINDEX(I).NE.0)CALL GLDELETELISTS(IDFLISTINDEX(I),1_GLSIZEI); ENDDO
 ENDIF
 IF(NIFFLIST.GT.0)THEN
  DO I=1,SIZE(IFFLISTINDEX); IF(IFFLISTINDEX(I).NE.0)CALL GLDELETELISTS(IFFLISTINDEX(I),1_GLSIZEI); ENDDO
 ENDIF
 ...
 IF(NGENLIST.GT.0)THEN
  DO I=1,SIZE(GENLISTINDEX); IF(GENLISTINDEX(I).NE.0)CALL GLDELETELISTS(GENLISTINDEX(I),1_GLSIZEI); ENDDO
 ENDIF
 IF(NSOLLIST.GT.0)THEN
  DO I=1,SIZE(SOLLISTINDEX,1); DO J=1,SIZE(SOLLISTINDEX,2)
   IF(SOLLISTINDEX(I,J).NE.0)CALL GLDELETELISTS(SOLLISTINDEX(I,J),1_GLSIZEI)
  ENDDO; ENDDO
 ENDIF
 IF(NCLPLIST.GT.0)THEN
  DO I=1,SIZE(CLPLISTINDEX); IF(CLPLISTINDEX(I).NE.0)CALL GLDELETELISTS(CLPLISTINDEX(I),1_GLSIZEI); ENDDO
 ENDIF
 IF(ALLOCATED(BMPLISTINDEX))THEN
  DO I=1,SIZE(BMPLISTINDEX); IF(BMPLISTINDEX(I).NE.0)CALL GLDELETELISTS(BMPLISTINDEX(I),1_GLSIZEI); ENDDO
 ENDIF
 ...
 CALL WMESSAGETIMER(60*1000,IREPEAT=1)  
