...
!  CALL UTL_IDFSNAPTOGRID(IDFM%XMIN,IDFM%XMAX,IDFM%YMIN,IDFM%YMAX,IDFM%DX,IDFM%NCOL,IDFM%NROW)
 ...
 IF(ALLOCATED(IDFPLOT))THEN
  DO I=1,SIZE(IDFPLOT); IF(ASSOCIATED(IDFPLOT(I)%TFILES))DEALLOCATE(IDFPLOT(I)%TFILES); ENDDO; DEALLOCATE(IDFPLOT)
 ENDIF
...
!   DO J=1,SIZE(IDFLISTINDEX(I)%INDEX)
   CALL GLDELETELISTS(IDFLISTINDEX(I)%INDEX(I),N) !1_GLSIZEI)
!    IF(IDFLISTINDEX(I)%INDEX(J).NE.0)THEN
!     CALL GLDELETELISTS(IDFLISTINDEX(I)%INDEX(J),1_GLSIZEI)
   CALL WINDOWOUTSTATUSBAR(2,'Clearing memory for IDF '//TRIM(ITOS(I))//' ...') !//'-'//TRIM(ITOS(J))//' ...')
!    ENDIF
!   ENDDO
  ENDDO
 ENDIF
