...
USE MOD_UTL, ONLY : UTL_MESSAGEHANDLE3D,UTL_MESSAGEHANDLE,UTL_PLOT2BITMAP,RTOS,UTL_IDFSNAPTOGRID,UTL_LOADIMAGE,ITOS,UTL_DEL1TREE
...
USE MOD_IPFGETVALUE_COLOURS, ONLY : IPFGETVALUE_GETCOLOURS,IPFGETVALUE_PLOTCOLOURS
USE MOD_PLINES_TRACE, ONLY : TRACE_3D_CLOSE
USE MOD_PLINES_PAR, ONLY : PL
USE MOD_PLUGIN_PAR
USE MOD_SOLID_UTL, ONLY : SOLIDDEALLOCATESPF
USE MOD_MANAGER_UTL
USE MOD_SOLID_PROFILE, ONLY : SOLID_PROFILECLOSE

CONTAINS
 
 !###======================================================================
 SUBROUTINE IMOD3D_SETTINGSINIT_IPF()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ILEG
 ...
 IF(NIPF.GT.0)THEN
  ...
  CALL WDIALOGPUTIMAGE(ID_EXPORTAS,ID_ICONSAVEAS,1)
  ALLOCATE(TMPNAME(NIPF),TMPINT(NIPF))
  TMPNAME=IPFPLOT%FNAME
  TMPINT=IPFPLOT%ISEL
  CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,NIPF,TMPINT)
  ...
  CALL WDIALOGSELECT(ID_D3DSETTINGS)
  CALL WDIALOGTABSTATE(IDF_TAB1,ID_D3DSETTINGS_TAB2,1)
 ELSE
  CALL WDIALOGSELECT(ID_D3DSETTINGS)
  CALL WDIALOGTABSTATE(IDF_TAB1,ID_D3DSETTINGS_TAB2,0)
 ENDIF

 END SUBROUTINE IMOD3D_SETTINGSINIT_IPF

 !###======================================================================
 SUBROUTINE IMOD3D_SETNORMALVECTOR(P11,P12,P13,P21,P22,P23,P31,P32,P33)
 !###======================================================================
 IMPLICIT NONE
! REAL(GLDOUBLE),INTENT(IN),DIMENSION(3) :: P1,P2,P3
 REAL(GLDOUBLE),INTENT(IN) :: P11,P12,P13,P21,P22,P23,P31,P32,P33
 REAL(GLDOUBLE),DIMENSION(3) :: NV
 REAL(GLDOUBLE) :: QX,QY,QZ,PX,PY,PZ

 CALL IMOD3D_ERROR('IMOD3D_SETNORMALVECTOR_BEGIN')

 QX=P21-P11; QY=P22-P12; QZ=P23-P13
 PX=P31-P11; PY=P32-P12; PZ=P33-P13
...
! PX=P3(1)-P1(1); PY=P3(2)-P1(2); PZ=P3(3)-P1(3)
 ...
 NV(2)=PZ*QX-PX*QZ
 NV(3)=PX*QY-PY*QX
 CALL GLNORMAL3D(NV(1),NV(2),NV(3))
...
!)
 ...
 REAL(KIND=DP_KIND),INTENT(OUT) :: X,Y,Z
...
! WRITE(*,*) OBJX,OBJY,OBJZ
 ...
 IF(Z.GE.BOT%Z.AND.Z.LE.TOP%Z)THEN
  !## update pointer
  CALL WINDOWOUTSTATUSBAR(2,'Current Mouse 3D Position ('//TRIM(RTOS(X,'F',3))//' , '//TRIM(RTOS(Y,'F',3))//' , '//TRIM(RTOS(Z,'F',3))//')')
  INDPOS%X=X; INDPOS%Y=Y; INDPOS%Z=Z; IVALIDPOS=1
 ELSE
  CALL WINDOWOUTSTATUSBAR(2,'Current Mouse 3D Position (unknown)')
  INDPOS%X=0.0_GLDOUBLE; INDPOS%Y=0.0_GLDOUBLE; INDPOS%Z=0.0_GLDOUBLE; IVALIDPOS=0
 ENDIF

 IF(IDRAWCROSS.EQ.1.AND.IVALIDPOS.EQ.1)THEN
 ...
 INTEGER :: IR1,IR2,IC1,IC2,NCOL,NROW,N,M
 REAL(KIND=DP_KIND) :: X1,X2,Y1,Y2
 
 IMOD3D_DRAWIDF_SIZE=.FALSE. 
  
 IDFM%XMIN=MAX(IDFC%XMIN,DBLE(BOT%X)) 
 IDFM%XMAX=MIN(IDFC%XMAX,DBLE(TOP%X)) 
 IDFM%YMIN=MAX(IDFC%YMIN,DBLE(BOT%Y)) 
 IDFM%YMAX=MIN(IDFC%YMAX,DBLE(TOP%Y)) 
 ...
 IF(IDFM%IEQ.EQ.0)THEN
  ...
  IDFM%DX=IDFC%DX*DBLE(N); IDFM%DY=IDFM%DX
  CALL UTL_IDFSNAPTOGRID_LLC(IDFM%XMIN,IDFM%XMAX,IDFM%YMIN,IDFM%YMAX,IDFM%DX,IDFM%DY,IDFM%NCOL,IDFM%NROW,LLC=.TRUE.)
 ELSE
 ...
 INTEGER :: IR,IG,IB
 REAL(KIND=GLDOUBLE) :: XR,XG,XB,ALPHA

 CALL WRGBSPLIT(IRGB,IR,IG,IB)
 XR=REAL(IR)/255.0_GLDOUBLE; XG=REAL(IG)/255.0_GLDOUBLE; XB=REAL(IB)/255.0_GLDOUBLE
 IF(PRESENT(IALPHA))THEN
  ALPHA=REAL(IALPHA)/100.0D0
  CALL GLCOLOR4D(XR,XG,XB,ALPHA)
 ELSE
 ...
 CALL WRGBSPLIT(IRGB,IR,IG,IB)
 GLCOLOR(1)=REAL(IR)/255.0_GLDOUBLE
 GLCOLOR(2)=REAL(IG)/255.0_GLDOUBLE
 GLCOLOR(3)=REAL(IB)/255.0_GLDOUBLE
 ...
 REAL(KIND=GLDOUBLE),DIMENSION(3),INTENT(IN) :: GLCOLOR
 INTEGER :: IR,IG,IB

 IR  =INT(GLCOLOR(1)*255.0_GLDOUBLE)
 IG  =INT(GLCOLOR(2)*255.0_GLDOUBLE)
 IB  =INT(GLCOLOR(3)*255.0_GLDOUBLE)
 IRGB=WRGB(IR,IG,IB)
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=GLDOUBLE) :: X1,X2,Y1,Y2,Z,ZF,XT1,XT2,YT1,YT2,Z1,Z2,Z3,Z4, &
 ...
 IF(IACTBITMAP.EQ.1)THEN
  ...
  ZOFFSET=(TOP%Z-BOT%Z)/1000_GLDOUBLE
   ...
   DO IROW=1,IDF(1)%NROW-1; DO ICOL=1,IDF(1)%NCOL-1
    IF(IDF(1)%X(ICOL  ,IROW)  .NE.IDF(1)%NODATA.AND. &
     ...
     CALL GLBEGIN(GL_QUADS)
      CALL GLTEXCOORD2D(XT1,YT1); CALL GLVERTEX3D(X1,Y1,Z1+ZOFFSET)
      CALL GLTEXCOORD2D(XT2,YT1); CALL GLVERTEX3D(X2,Y1,Z2+ZOFFSET)
      CALL GLTEXCOORD2D(XT2,YT2); CALL GLVERTEX3D(X2,Y2,Z3+ZOFFSET)
      CALL GLTEXCOORD2D(XT1,YT2); CALL GLVERTEX3D(X1,Y2,Z4+ZOFFSET)
     CALL GLEND()
  ...
  ELSE
   DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL 
    IF(IDF(1)%X(ICOL,IROW).NE.IDF(1)%NODATA)THEN
     ...
     Z =IDF(1)%X(ICOL,IROW)
     CALL GLBEGIN(GL_QUADS)
      CALL GLTEXCOORD2D(XT1,YT1); CALL GLVERTEX3D(X1,Y1,Z+ZOFFSET)
      CALL GLTEXCOORD2D(XT1,YT2); CALL GLVERTEX3D(X1,Y2,Z+ZOFFSET)
      CALL GLTEXCOORD2D(XT2,YT2); CALL GLVERTEX3D(X2,Y2,Z+ZOFFSET)
      CALL GLTEXCOORD2D(XT2,YT1); CALL GLVERTEX3D(X2,Y1,Z+ZOFFSET)
     CALL GLEND()
    ENDIF
   ENDDO; ENDDO   
  ENDIF
 ...
 ELSEIF(IACTBITMAP.EQ.2)THEN
  ...
  Y1=BOT%Y
  Y2=TOP%Y
  Z =BOT%Z+(TOP%Z-BOT%Z)*((100.0D0-ZF)/100.0D0)
  ...
  !## connect 2d texture to 3d object (in this case top of cube)
  CALL GLBEGIN(GL_QUADS)
   CALL GLTEXCOORD2D(XT1,YT1); CALL GLVERTEX3D(X1,Y1,Z)
   CALL GLTEXCOORD2D(XT1,YT2); CALL GLVERTEX3D(X1,Y2,Z)
   CALL GLTEXCOORD2D(XT2,YT2); CALL GLVERTEX3D(X2,Y2,Z)
   CALL GLTEXCOORD2D(XT2,YT1); CALL GLVERTEX3D(X2,Y1,Z)
  CALL GLEND()
 ENDIF
 ...
 IF(IMOD3D_DRAWIDF_SIZE(IDF(2),IDF(1)))THEN
  IF(.NOT.IDFREADSCALE_GETX(IDF(2),IDF(1),IDFDATA(3),1,0.0D0))RETURN   !## child,mother,blockvalue,percentile
 ELSE
 ...
 CALL WINDOWSELECT(IWIN3D); CALL WINDOWOUTSTATUSBAR(2,'Reading Bitmap ...')
 ...
 IF(ALLOCATED(IDFLISTINDEX))THEN
  DO I=1,SIZE(IDFLISTINDEX); IF(ASSOCIATED(IDFLISTINDEX(I)%INDEX))DEALLOCATE(IDFLISTINDEX(I)%INDEX); ENDDO
  DEALLOCATE(IDFLISTINDEX)
 ENDIF
 ...
 IF(ALLOCATED(IDFPLOT))THEN
  DO I=1,SIZE(IDFPLOT)
   IF(ASSOCIATED(IDFPLOT(I)%TFILES))DEALLOCATE(IDFPLOT(I)%TFILES)
   IF(ASSOCIATED(IDFPLOT(I)%CDATES))DEALLOCATE(IDFPLOT(I)%CDATES)
   IF(ASSOCIATED(IDFPLOT(I)%ICLR))  DEALLOCATE(IDFPLOT(I)%ICLR)
   IF(ASSOCIATED(IDFPLOT(I)%IACT))  DEALLOCATE(IDFPLOT(I)%IACT)
  ENDDO
  DEALLOCATE(IDFPLOT)
 ENDIF
 ...
 IF(ASSOCIATED(XYZCROSS))DEALLOCATE(XYZCROSS)
 IF(ALLOCATED(NXYZCROSS))DEALLOCATE(NXYZCROSS)
 IF(ASSOCIATED(XYZWELL)) DEALLOCATE(XYZWELL)

 IDRAWCROSS=0; IDRAWWELL=0; D4ITIMEDIR=0
 ...
 IF(NIDFLIST.GT.0)THEN
  DO I=1,SIZE(IDFLISTINDEX)
   N=(IDFLISTINDEX(I)%IELIST-IDFLISTINDEX(I)%ISLIST)+1
   CALL GLDELETELISTS(IDFLISTINDEX(I)%INDEX(1),N) 
 ...
 IF(NSOLLIST.GT.0)THEN
  IF(ISOLID_3D.NE.0)CALL SOLID_PROFILECLOSE()
 ...
 IF(IWIN3D.NE.0)CALL WINDOWCLOSECHILD(IWIN3D); IWIN3D=0
 ...
 !## remove temporary artifical wells
 IF(IOSDIREXISTS(TRIM(PREFVAL(1))//'\TMP\3D\ARTWELL'))THEN
  IF(UTL_DEL1TREE(TRIM(PREFVAL(1))//'\TMP\3D\ARTWELL'))THEN
   !## remove ipf from imod manager
   MP%ISEL=.FALSE.
   DO I=1,MXMPLOT
    IF(MP(I)%IPLOT.EQ.2)THEN
     IF(INDEX(MP(I)%IDFNAME,'TMP\3D\ARTWELL').GT.0)THEN
      MP(I)%ISEL=.TRUE.
 ...
 CALL WDIALOGSELECT(ID_D3DIDFSETTINGS); CALL WDIALOGUNLOAD()
 ...
 IF(PRESENT(LENXY))LENXY=SQRT(DX**2.0D0+DY**2.0D0) 
 HEADING=ATAN2(DY,DX)
 DXY=SQRT(DX**2.0D0+DY**2.0D0)
 TILT=ATAN2(DZ,DXY)
 IF(PRESENT(LENXZ))LENXZ=SQRT(DXY**2.0D0+DZ**2.0D0)
