...
USE MODPLOT, ONLY : MP
USE MOD_UTL, ONLY : UTL_MESSAGEHANDLE3D,UTL_MESSAGEHANDLE,RTOS,UTL_WSELECTFILE
...
!almost none. You can think of specularity as shininess.
 ...
 DO IASSF=1,NIPFLIST
  ...
  IF(IPFDLIST(5,IASSF).EQ.0)CYCLE
  !## part of selected ipf
  IIPF=IPFDLIST(1,IASSF); IF(IIPF.EQ.0)CYCLE
  !## selected in menu-field
  IF(IPFPLOT(IIPF)%ISEL.NE.1.OR.IPFLISTINDEX(IASSF,IMODE).EQ.0)CYCLE
   
  IF(IMODE.EQ.1)then
   !## line width
   IF(ISELECTED.EQ.IASSF)THEN
  ...
  CALL GLCALLLIST(IPFLISTINDEX(IASSF,IMODE))
 ...
 !## draw ipf labels
 IF(IMODE.EQ.1.AND.IPLOTLABELS.EQ.1)THEN
  !## associated file drawn
  DO IASSF=1,NIPFLIST
   !## active based upon selection
   IF(IPFDLIST(5,IASSF).EQ.0)CYCLE
   !## part of selected ipf
   IIPF=IPFDLIST(1,IASSF) !ASSF(IASSF)%IIPF
   !## selected in menu-field
   IF(IPFPLOT(IIPF)%ISEL.EQ.0.OR.IPFLISTINDEX(IASSF,3).EQ.0)CYCLE
   IF(ISELECTED.EQ.IASSF)THEN
    CALL IMOD3D_SETCOLOR(WRGB(255,0,0))
   ELSE
    CALL IMOD3D_SETCOLOR(LCOLOR)
   ENDIF
   CALL GLCALLLIST(IPFLISTINDEX(IASSF,3))
  ENDDO
 ENDIF
...
!     CALL GLMATERIALFV(GL_FRONT_AND_BACK,GL_AMBIENT_AND_DIFFUSE,AMBIENT)
 ...
 DO I=1,NIFFLIST
  ...
  CALL GLLINEWIDTH(XW)!1.0_GLFLOAT)
 ...
 !## no lightning ... difficult to draw in correct seqence/order

 CALL GLENABLE(GL_LIGHTING)
 CALL GLSHADEMODEL(GL_FLAT) !## heeft te maken met invullen kleuren

 DO I=1,NSOLLIST
  IF(SOLPLOT(I)%ISEL.NE.1.OR.SOLLISTINDEX(I).EQ.0)CYCLE
  
  IF(SOLPLOT(I)%IBLEND.EQ.1)THEN
   CALL GLBLENDFUNC(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA)  !## (1) source (2) destination
  ENDIF
  CALL GLCALLLIST(SOLLISTINDEX(I))
 END DO
 ...
 DO I=0,(NDX*NDY)-1
  CALL IMOD3D_GETCOLOR(IRGB(I+1),(/FRGB(I*3+1),FRGB(I*3+2),FRGB(I*3+3)/))
 ENDDO
 ...
 CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfuly save the current image to'//CHAR(13)//TRIM(FNAME),'Information')
