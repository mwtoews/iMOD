   ...
   SELECT CASE (ITYPE)
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      !## start new
      CASE (ID_NEW)
       PL%NPART=0; CALL WDIALOGFIELDSTATE(ID_ADD,1); CALL TRACE_3D_STARTPOINTS_SHOW(); CALL IMOD3D_DISPLAY(1)
      !## add starting point to the list
      CASE (ID_ADD)
       CALL TRACE_3D_STARTPOINTS(); CALL TRACE_3D_STARTPOINTS_SHOW(); CALL IMOD3D_DISPLAY(1)
      !## change colour
      CASE (ID_COLOUR)
       IRGB=PL%SPCOLOR; CALL WSELECTCOLOUR(IRGB); IF(WINFODIALOG(4).EQ.1)THEN; PL%SPCOLOR=IRGB; ENDIF
      CASE (ID_START)
