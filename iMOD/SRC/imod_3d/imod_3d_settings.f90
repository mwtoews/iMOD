...
USE MOD_3D_PROCESS, ONLY : IMOD3D_PROCESSRESIZE
  ...
  IF(SUM(SOLPLOT(1:NSOLLIST)%ISEL).GT.0)THEN
   DO I=1,NSOLLIST
    IF(SOLPLOT(I)%ISEL.EQ.1)THEN
     CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR1,SOLPLOT(I)%IBLEND)  !## blending percentage
 ...
 IF(NSOLLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB6)THEN
  ...
  IF(SUM(SOLPLOT(1:NSOLLIST)%ISEL).GT.0)THEN
   DO I=1,NSOLLIST; IF(SOLPLOT(I)%ISEL.EQ.1)THEN
    IF(JD.EQ.IDF_TRACKBAR1)CALL WDIALOGGETTRACKBAR(IDF_TRACKBAR1,SOLPLOT(I)%IBLEND) !## blending percentage
