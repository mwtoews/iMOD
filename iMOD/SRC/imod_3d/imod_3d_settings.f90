...
CONTAINS
 ...
 IF(IPATHLINE_3D.GT.0)THEN
  ...
  CALL WDIALOGFIELDSTATE(ID_PAUSE,0)
 ELSE
  CALL WDIALOGSELECT(ID_D3DSETTINGS)
    ...
    CASE (FIELDCHANGED)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (IDF_RADIO1,IDF_RADIO2)
       IF(MESSAGE%VALUE1.NE.MESSAGE%VALUE2)THEN
        !## switch
        PL%IREV=ABS(PL%IREV-1); CALL TRACEIREV()
!        CALL IMOD3D_SETTINGSPUT(ID_D3DSETTINGS_TAB4)
!        CALL IMOD3D_SETTINGSFIELDS(ID_D3DSETTINGS_TAB4)
!        CALL IMOD3D_DISPLAY(1)
       ENDIF
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      !## start new
      CASE (ID_NEW)
       PL%NPART=0; CALL TRACE_3D_STARTPOINTS_SHOW(); CALL IMOD3D_DISPLAY(1)
      !## add starting point to the list
      CASE (ID_ADD)
       CALL TRACE_3D_STARTPOINTS(); CALL TRACE_3D_STARTPOINTS_SHOW(); CALL IMOD3D_DISPLAY(1)
      !## change colour
      CASE (ID_COLOUR)
       IRGB=PL%SPCOLOR; CALL WSELECTCOLOUR(IRGB)
       IF(WINFODIALOG(4).EQ.1)THEN; PL%SPCOLOR=IRGB;CALL IMOD3D_DISPLAY(1); ENDIF
      CASE (ID_START)
       CALL TRACE_3D_RESET()
       CALL WDIALOGFIELDSTATE(ID_START,0); CALL WDIALOGFIELDSTATE(ID_STOP ,1); CALL WDIALOGFIELDSTATE(ID_PAUSE,1)
       IRUN=1
       DO
        IF(IRUN.EQ.1)THEN
        ...
        ENDIF
        IF(PL_ITYPE.EQ.PUSHBUTTON)THEN
         SELECT CASE (PL_MESSAGE%VALUE1)
          CASE (ID_START)
           CALL WDIALOGFIELDSTATE(ID_START,0); CALL WDIALOGFIELDSTATE(ID_STOP,1); CALL WDIALOGFIELDSTATE(ID_PAUSE,1); IRUN=1
          CASE (ID_PAUSE)
           CALL WDIALOGPUTSTRING(ID_START,'Continue'); CALL WDIALOGFIELDSTATE(ID_START,1); CALL WDIALOGFIELDSTATE(ID_PAUSE,0); IRUN=0
          CASE (ID_STOP)
           EXIT
         END SELECT
        ENDIF
        IF(IRUN.EQ.1)THEN
         ...
         IF(.NOT.TRACE_3D_COMPUTE())EXIT
         CALL IMOD3D_DISPLAY(1)
        ENDIF
       ENDDO
       CALL WDIALOGPUTSTRING(ID_START,'Start')
       CALL WDIALOGFIELDSTATE(ID_START,1); CALL WDIALOGFIELDSTATE(ID_STOP,0); CALL WDIALOGFIELDSTATE(ID_PAUSE,0)
