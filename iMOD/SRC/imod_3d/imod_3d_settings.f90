...
USE MOD_3D_PROCESS
USE MOD_IPF_PAR, ONLY : NIPF,NLITHO,BH,ASSF
...
USE MOD_LEGEND, ONLY : LEG_MAIN,LEG_COPYTHEM
...
CONTAINS
 ...
 IF(NIDFLIST.GT.0)THEN
  ...
  ALLOCATE(TMPNAME(DISP_NIDFLIST),TMPINT(DISP_NIDFLIST))
  TMPNAME=IDFPLOT%DISP_ALIAS; TMPINT=IDFPLOT%DISP_ISEL
  CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,DISP_NIDFLIST,TMPINT)
  ...
  CALL IMOD3D_SETTINGS_4DVAR_FIELDS()
 ...
 IF(NIFFLIST.GT.0)THEN
  ...
  ALLOCATE(TMPNAME(NIFFLIST),TMPINT(NIFFLIST))
  TMPNAME=IFFPLOT%FNAME; TMPINT=IFFPLOT%ISEL 
  CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,NIFFLIST,TMPINT) !IFFPLOT%ISEL)
  DEALLOCATE(TMPNAME,TMPINT)
  CALL WDIALOGPUTIMAGE(IDF_COLOUR1,ID_ICONLEGEND)
  CALL WDIALOGSELECT(ID_D3DSETTINGS)
...
!   CALL WDIALOGFIELDSTATE(IDF_CHECK2,2)
 ...
 ELSE
  ...
  TMPNAME=SPF%FNAME; TMPINT=SOLPLOT%ISEL 
  ...
  CALL WDIALOGFIELDSTATE(ID_NEWAUTO,2)
!  CALL WDIALOGSELECT(ID_D3DSETTINGS)
!  CALL WDIALOGTABSTATE(IDF_TAB1,ID_D3DSETTINGS_TAB6,1)
 ENDIF
...
!  IF(NIDFLIST+NIPF+NIFFLIST.EQ.0)CALL WDIALOGSETTAB(IDF_TAB1,ID_D3DSETTINGS_TAB6)
 ...
 IF(IPATHLINE_3D.GT.0)THEN
  ...
  IF(ALLOCATED(IDFPLOT))THEN
   ALLOCATE(TMPNAME(NSPF))
   TMPNAME=IDFPLOT%DISP_ALIAS
   CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,DISP_NIDFLIST,1)
   DEALLOCATE(TMPNAME)
  ...
  !## set spinner steps
  CALL WDIALOGSPINNERSTEP(IDF_REAL8,0.05D0,0.1D0)    !## fraction
  CALL WDIALOGSPINNERSTEP(IDF_REAL4,10.0D0,100.0D0)  !## total year
  CALL WDIALOGSPINNERSTEP(IDF_REAL5,0.1D0,1.0D0)     !## travelsteps (year)
  CALL WDIALOGSPINNERSTEP(IDF_INTEGER4,1,2)      !## tail length
  CALL WDIALOGSPINNERSTEP(IDF_REAL6,0.1D0,1.0D0)     !## repeat frequency

  CALL WDIALOGPUTDOUBLE(IDF_REAL8,FRAC)
 ...
 IF(NGENLIST.GT.0)THEN
  ...
  ALLOCATE(TMPINT(NGENLIST)); TMPINT=GENPLOT%ISEL
  ALLOCATE(TMPNAME(NGENLIST)); TMPNAME=GENPLOT%FNAME
  CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,NGENLIST,TMPINT) !GENPLOT%ISEL)
 ...
 IF(NIDFLIST.GT.0)THEN
  ALLOCATE(TMPNAME(DISP_NIDFLIST)); TMPNAME=IDFPLOT%DISP_ALIAS
  CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,DISP_NIDFLIST,1)
  DEALLOCATE(TMPNAME)
 ELSE
 ...
 ALLOCATE(TMPINT(NCLPLIST)); TMPINT=CLPPLOT%ISEL
 ALLOCATE(TMPNAME(NCLPLIST)); TMPNAME=CLPPLOT%FNAME
 CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,NCLPLIST,TMPINT)
 CALL WDIALOGPUTIMAGE(IDF_COLOUR1,ID_ICONLEGEND)
 DEALLOCATE(TMPNAME,TMPINT)
 IF(.NOT.LCAP)THEN
  CALL WDIALOGFIELDSTATE(IDF_CHECK1,0)
  CALL WDIALOGPUTSTRING(IDF_CHECK1,'Capping Disabled')
  CALL WDIALOGPUTCHECKBOX(IDF_CHECK1,0)
 ELSE
  !## by default capping off
  CALL WDIALOGPUTCHECKBOX(IDF_CHECK1,0)
 ENDIF
 
 CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB9)
 CALL WDIALOGPUTIMAGE(ID_SELECT,ID_ICONSELECTPOINT,1)
 CALL WDIALOGPUTIMAGE(ID_DISPLAY,ID_ICONINFO,1)
 ...
 REAL(KIND=DP_KIND) :: ANGLE,X,Y,Z,D
   ...
   SELECT CASE (ITYPE)
    CASE (FIELDCHANGED)
     SELECT CASE (MESSAGE%VALUE1)
      !## time variant idf files
      CASE (IDF_MENU2)
        CALL IMOD3D_SETTINGS_4DVAR_PROGRESS(MESSAGE%VALUE1)
        CALL IMOD3D_DISPLAY(1)
      ...
      CASE (IDF_MENU1)
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
        ...
        CALL IMOD3D_DISPLAY(1)
       ENDIF
      CASE (IDF_CHECK3,IDF_CHECK2,IDF_CHECK4,IDF_CHECK5)    !## shading/plotlegend/transparancy
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (ID_LEGEND)
       IF(IMOD3D_SETTINGS_CHANGELEGEND())THEN
        CALL IMOD3D_SETTINGSGET(ID_D3DSETTINGS_TAB1,MESSAGE%VALUE2)
        CALL IMOD3D_SETTINGSFIELDS(ID_D3DSETTINGS_TAB1)
        CALL UTL_MESSAGEHANDLE(0)
        !## recompute current idf
      ...
      CASE (ID_LEFT,ID_RIGHT) 
       CALL IMOD3D_SETTINGS_4DVAR_PROGRESS(MESSAGE%VALUE1)
       CALL IMOD3D_DISPLAY(1)
      CASE(ID_FASTLEFT,ID_FASTRIGHT)
       CALL WDIALOGFIELDSTATE(ID_LEFT,0)
       CALL WDIALOGFIELDSTATE(ID_RIGHT,0)
       CALL WDIALOGFIELDSTATE(ID_STOP,1)
       IF(MESSAGE%VALUE1.EQ.ID_FASTLEFT)CALL WDIALOGFIELDSTATE(ID_FASTRIGHT,0)
       IF(MESSAGE%VALUE1.EQ.ID_FASTRIGHT)CALL WDIALOGFIELDSTATE(ID_FASTLEFT,0)
       D4ITIMEDIR=MESSAGE%VALUE1
      CASE(ID_STOP)
       CALL WDIALOGFIELDSTATE(ID_LEFT,1)
       CALL WDIALOGFIELDSTATE(ID_RIGHT,1)
       CALL WDIALOGFIELDSTATE(ID_STOP,0)
       CALL WDIALOGFIELDSTATE(ID_FASTRIGHT,1)
       CALL WDIALOGFIELDSTATE(ID_FASTLEFT,1)
       D4ITIMEDIR=0
      CASE (IDF_COLOUR1)
     ...
     SELECT CASE (MESSAGE%VALUE2) !## from field - store result first, before overwrite it
      CASE (IDF_MENU1,IDF_MENU2,IDF_GRID1)
       IF(MESSAGE%VALUE1.NE.MESSAGE%VALUE2)THEN
        ...
        ENDIF    
       ENDIF
       CALL IMOD3D_SETTINGS_IPF_SELECTED()
     END SELECT
     SELECT CASE (MESSAGE%VALUE1) !## change in menu field
      ...
      CASE (IDF_CHECK5)
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
        ...
        IF(IMOD3D_IPF_SELECTION(IIPF))THEN
         CALL IMOD3D_LEGEND_MAIN()
         CALL IMOD3D_DISPLAY(1)
        ENDIF
       ENDIF
         
      CASE (IDF_CHECK1,IDF_CHECK2,IDF_CHECK4)   !## labeling/plotlegend
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
        CALL IMOD3D_SETTINGSGET(ID_D3DSETTINGS_TAB2,MESSAGE%VALUE2)
        CALL IMOD3D_SETTINGSFIELDS(ID_D3DSETTINGS_TAB2)
        CALL IMOD3D_LEGEND_MAIN()
...
!       ENDIF
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (ID_PROPERTIES)
       IF(SUM(IPFPLOT%ISEL).GT.1)THEN
        CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You need to select a single IPF to continue.','Warning')
       ELSE
        DO IIPF=1,NIPF; IF(IPFPLOT(IIPF)%ISEL.EQ.1)EXIT; ENDDO
        !## refresh if needed ...
        ...
        IF(.NOT.IMOD3D_IPF_SELECTION(IIPF))THEN; ENDIF
        CALL IMOD3D_DISPLAY(1)
       ENDIF
      ...
      CASE (ID_EXPORTAS)
       ...
       ELSE
        DO IIPF=1,NIPF; IF(IPFPLOT(IIPF)%ISEL.EQ.1)EXIT; ENDDO
        IF(.NOT.IMOD3D_IPF_EXPORTIPF(IIPF))THEN; ENDIF
       ENDIF
     ...
     SELECT CASE (MESSAGE%VALUE1)
      CASE (IDF_MENU1)
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
        ...
        CALL IMOD3D_DISPLAY(1)
       ENDIF
      CASE (IDF_CHECK1,IDF_CHECK4)   !## plotlegend
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
   ...
   SELECT CASE (ITYPE)
     ...
     SELECT CASE (MESSAGE%VALUE1)
      CASE (IDF_MENU1)
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
        ...
        CALL IMOD3D_DISPLAY(1)
       ENDIF
      CASE (IDF_CHECK2,IDF_CHECK3,IDF_CHECK4) 
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      !## add new fence-diagram
      CASE (ID_NEW)
       !## start drawing a cross-section
       IDRAWCROSS=1; NXYZCROSS=0; NXYZCROSS(1)=1; IVALIDPOS=0
      CASE (ID_NEWAUTO)
       !## initiate window
  ...
  CASE (ID_D3DSETTINGS_TAB7)
   SELECT CASE (ITYPE)
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (ID_RESET)
       CALL WMESSAGEBOX(YESNO,QUESTIONICON,COMMONNO,'Are you sure to reset all clipping planes?','Question')
       IF(WINFODIALOG(4).EQ.1)THEN
        CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR2,0)
        CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR3,0)
        CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR4,0)
        CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR1,0)
        CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR5,0)
        CALL WDIALOGPUTTRACKBAR(IDF_TRACKBAR6,0)
        CLPPLOT%IPOS=0
!        CALL IMOD3D_SETTINGSGET(ID_D3DSETTINGS_TAB7,MESSAGE%VALUE1)
        CALL IMOD3D_DISPLAY(1)
       ENDIF
     ...
     SELECT CASE (MESSAGE%VALUE1)
      CASE (IDF_MENU1)
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
        ...
        CALL IMOD3D_DISPLAY(1)
       ENDIF
      CASE (IDF_INTEGER2,IDF_TRACKBAR1,IDF_CHECK1,IDF_CHECK4)
    ...
    CASE (FIELDCHANGED)
     SELECT CASE (MESSAGE%VALUE2)
      ...
      CASE (IDF_TRACKBAR5)
       ...
       ANGLE=360.0D0/(2.0*PI_OPENGL)
       X=COS(REAL(I)/ANGLE)
       Y=SIN(REAL(I)/ANGLE)
       Z=1.0D0
       D=SQRT(X**2.0D0+Y**2.0D0+Z**2.0D0)
       X=X/D; Y=Y/D; Z=Z/D
       D=SQRT(X**2.0D0+Y**2.0D0+Z**2.0D0)
...
!       CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB7)
     ...
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (ID_PROP1,ID_PROP2,ID_PROP3,ID_PROP5,ID_PROP6)
       ...
       IF(MESSAGE%VALUE1.EQ.ID_PROP2)THEN
        CALL WDIALOGGETMENU(IDF_MENU2,I); CALL WDIALOGGETDOUBLE(IDF_REAL10,X)
        CALL WDIALOGSELECT(ID_D3DSETTINGS_SINKS)
        CALL WDIALOGPUTOPTION(IDF_MENU2,I); CALL WDIALOGPUTDOUBLE(IDF_REAL10,X)
  ...
  CASE (ID_D3DSETTINGS_TAB9)
   SELECT CASE (ITYPE)
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)    
      !## display graph of borehole
      CASE (ID_DISPLAY)
       CALL IMOD3D_SETTINGSMAIN_IPFGRAPH()
  ...
  CASE (ID_D3DSETTINGS_FENCES)
   SELECT CASE (ITYPE)
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (IDOK)
       IF(IMOD3D_SOL_ADD())CALL IMOD3D_FENCECLOSE()
      CASE (IDHELP)
 ...
 !###======================================================================
 LOGICAL FUNCTION IMOD3D_SETTINGS_CHANGELEGEND()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,NI,IPLOT
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IL
 
 IMOD3D_SETTINGS_CHANGELEGEND=.FALSE.

 CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB1)
 CALL  IMOD3D_SETTINGS_GETSELECTED()
! CALL WDIALOGGETMENU(IDF_MENU1,IDFPLOT%DISP_ISEL)
 !## nothing selected
 IF(SUM(IDFPLOT%DISP_ISEL).EQ.0)RETURN
 
 IPLOT=0; IDFPLOT%ISEL=0; DO I=1,DISP_NIDFLIST; IF(IDFPLOT(I)%DISP_ISEL.EQ.1)EXIT; ENDDO; IPLOT=IDFPLOT(I)%IPLOT
 IF(IPLOT.LE.0)RETURN
 
 !## modify legend
 IF(.NOT.LEG_MAIN(IPLOT))RETURN

 ALLOCATE(IL(DISP_NIDFLIST)); IL=0
 NI=0; DO I=1,DISP_NIDFLIST
  IF(IDFPLOT(I)%DISP_ISEL.EQ.1)THEN
   NI=NI+1; IL(NI)=IDFPLOT(I)%IPLOT
  ENDIF
 ENDDO
 
 !## change legend for all selected
 IF(NI.GT.1)CALL LEG_COPYTHEM(IPLOT,NI,IL)
 !## update legend for 3D
 DO I=1,NI; IDFPLOT(I)%LEG=MP(IL(I))%LEG; ENDDO; DEALLOCATE(IL)
 
 IMOD3D_SETTINGS_CHANGELEGEND=.TRUE.

 END FUNCTION IMOD3D_SETTINGS_CHANGELEGEND
 
 !###======================================================================
 SUBROUTINE IMOD3D_SETTINGS_4DVAR_PROGRESS(ID)
 !###======================================================================
 IMPLICIT NONE
...
! !## nothing selected
 ...
 !## change time for selected idf files
 JJ=0; DO II=1,DISP_NIDFLIST
  !## skip non-selected files
  IF(IDFPLOT(II)%DISP_ISEL.EQ.0)CYCLE
  !## skip non-time variant files
  J=IDFPLOT(II)%DISP_ILIST; IF(IDFPLOT(J)%ICUBE.LE.5)CYCLE
  SELECT CASE (ID)
   CASE (IDF_MENU2)
    CALL WDIALOGGETMENU(IDF_MENU2,IDFPLOT(J)%D4ITIME)
    IDFPLOT(J)%D4ITIME=MAX(1,MIN(IDFPLOT(J)%D4ITIME,SIZE(IDFPLOT(J)%TFILES)))
   CASE (ID_RIGHT)
    IDFPLOT(J)%D4ITIME=MIN(IDFPLOT(J)%D4ITIME+1,SIZE(IDFPLOT(J)%TFILES))
   !## repeat
   CASE (ID_FASTRIGHT)
    IDFPLOT(J)%D4ITIME=IDFPLOT(J)%D4ITIME+1
    IF(IDFPLOT(J)%D4ITIME.GT.SIZE(IDFPLOT(J)%TFILES))IDFPLOT(J)%D4ITIME=1
   CASE (ID_LEFT)
    IDFPLOT(J)%D4ITIME=MAX(IDFPLOT(J)%D4ITIME-1,1)
   !## repeat
    CASE (ID_FASTLEFT)
    IDFPLOT(J)%D4ITIME=IDFPLOT(J)%D4ITIME-1
    IF(IDFPLOT(J)%D4ITIME.LT.1)IDFPLOT(J)%D4ITIME=SIZE(IDFPLOT(J)%TFILES)   
  END SELECT
  !## only the first
  IF(JJ.EQ.0)THEN; CALL WDIALOGPUTOPTION(IDF_MENU2,IDFPLOT(J)%D4ITIME); JJ=J; ENDIF
 ENDDO
 
 END SUBROUTINE IMOD3D_SETTINGS_4DVAR_PROGRESS

 !###======================================================================
 SUBROUTINE IMOD3D_SETTINGS_GETSELECTED()
 !###======================================================================
 IMPLICIT NONE
 INTEGER,ALLOCATABLE,DIMENSION(:) :: TMPINT

! CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB1)
 ALLOCATE(TMPINT(SIZE(IDFPLOT))) !DISP_NIDFLIST))
 CALL WDIALOGGETMENU(IDF_MENU1,TMPINT) 
 IDFPLOT%DISP_ISEL=TMPINT
 DEALLOCATE(TMPINT)

 END  SUBROUTINE IMOD3D_SETTINGS_GETSELECTED
 
 !###======================================================================
 SUBROUTINE IMOD3D_SETTINGS_4DVAR_FIELDS()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,II,N
...
! CALL WDIALOGGETMENU(IDF_MENU1,TMPINT) 
 ...
 !## nothing to adjust
 I=0; IF(SUM(IDFPLOT(1:NIDFLIST)%ISEL).GT.0)THEN
  !## get first selected idf-file to fill in menu
  DO II=1,DISP_NIDFLIST; IF(IDFPLOT(II)%DISP_ISEL.EQ.1)EXIT; ENDDO; J=IDFPLOT(II)%DISP_ILIST
  IF(IDFPLOT(J)%ICUBE.GT.5)THEN
   I=1; N=SIZE(IDFPLOT(J)%TFILES)
   CALL WDIALOGPUTMENU(IDF_MENU2,IDFPLOT(J)%CDATES,N,IDFPLOT(J)%D4ITIME)
  ENDIF
 ENDIF
 
 CALL WDIALOGFIELDSTATE(ID_FASTLEFT,I);  CALL WDIALOGFIELDSTATE(ID_LEFT,I)
 CALL WDIALOGFIELDSTATE(ID_RIGHT,I);     !CALL WDIALOGFIELDSTATE(ID_STOP,I)
 CALL WDIALOGFIELDSTATE(ID_FASTRIGHT,I); CALL WDIALOGFIELDSTATE(IDF_MENU2,I)

 END SUBROUTINE IMOD3D_SETTINGS_4DVAR_FIELDS
 
 !###======================================================================
 SUBROUTINE IMOD3D_SETTINGSMAIN_IPFGRAPH()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,M,N,NXG,ICLR,WID,IEXIT
 ...
 M=ASSF(ISELECTED)%NRASS; N=1; NXG=1
 ...
 ALLOCATE(GRAPHDIM(1)%XTXT(NXG),GRAPHDIM(1)%XPOS(NXG)); GRAPHDIM(1)%XTXT=''; GRAPHDIM(1)%XPOS=0.0D0; GRAPHDIM(1)%IFIXX=1; GRAPHDIM(1)%IGROUP=1
 
 GRAPHDIM(1)%XTXT(1)='Borehole'
 GRAPHDIM(1)%XPOS(1)=0.0
 ...
 DO I=1,M
  ...
  !## fill in grid with information - vertical well
  IF(ASSF(ISELECTED)%ITOPIC.EQ.2)THEN
   GRAPH(I,1)%RY(1)=ASSF(ISELECTED)%Z(I)
  ELSEIF(ASSF(ISELECTED)%ITOPIC.EQ.4)THEN
   GRAPH(I,1)%RY(1)=ASSF(ISELECTED)%Z(I)
  ENDIF
  CALL IPFDRAWITOPIC2_ICLR(I,ISELECTED,ICLR,IWIDTH)
  GRAPH(I,1)%ICLR=ICLR 
  GRAPH(I,1)%LEGTXT=ASSF(ISELECTED)%L(J,I)
 END DO

 !## skip non-unique legends
 DO I=1,M
  DO J=I+1,M 
   !## skip cleaned ones
   IF(TRIM(GRAPH(J,1)%LEGTXT).EQ.'')CYCLE
   IF(TRIM(GRAPH(I,1)%LEGTXT).EQ.TRIM(GRAPH(J,1)%LEGTXT))GRAPH(J,1)%LEGTXT=''
  ENDDO
 ENDDO

 !## make sure last is equal to final-least
 GRAPH(1,1)%NP=1; GRAPHDIM%GRAPHNAMES=''; GRAPHDIM%XTITLE='[-]'; GRAPHDIM%YTITLE='Depth (m+MSL)'; GRAPHDIM%LDATE=.FALSE.
 CALL GRAPH_INIT(3)
 DO
  CALL WMESSAGE(ITYPE,MESSAGE)
  CALL GRAPH_MAIN(ITYPE,MESSAGE,IEXIT)
  IF(IEXIT.EQ.1)EXIT
 ENDDO
 ...
 END SUBROUTINE IMOD3D_SETTINGSMAIN_IPFGRAPH
 ...
 SELECT CASE (MESSAGE%VALUE2)
  CASE (IDF_CHECK7)
   CALL WDIALOGGETCHECKBOX(IDF_CHECK7,I)
   I=ABS(I-1)
   !## overrule when a simulation is on
   IF(ABS(PL%IRUN).EQ.1)I=0
 ...
 CALL WDIALOGGETCHECKBOX(IDF_CHECK7,I)
 IF(I.EQ.1)THEN
  CALL WDIALOGGETDOUBLE(IDF_REAL1,Z1); CALL WDIALOGGETDOUBLE(IDF_REAL2,Z2)
  IF(Z1.LE.Z2)THEN
...
!  CALL WDIALOGGETMENU(IDF_MENU1,IDFPLOT%DISP_ISEL)
  ...
  IF(SUM(IDFPLOT(1:NIDFLIST)%ISEL).GT.0)THEN
   DO I=1,NIDFLIST
    IF(IDFPLOT(I)%ISEL.EQ.1)THEN
     ...
     CALL WDIALOGPUTCHECKBOX(IDF_CHECK4,IDFPLOT(I)%ICLIP)
     ...
     CALL WDIALOGPUTCHECKBOX(IDF_CHECK5,IDFPLOT(I)%ITRANSPARANCY)
 ...
 IF(NIPF.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB2)THEN
  CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB2)
  ALLOCATE(TMPINT(SIZE(IPFPLOT)))
  CALL WDIALOGGETMENU(IDF_MENU1,TMPINT)
  IPFPLOT%ISEL=TMPINT; DEALLOCATE(TMPINT)
  DO I=1,SIZE(IPFPLOT)
   IF(IPFPLOT(I)%ISEL.EQ.1)THEN
    ...
    CALL WDIALOGPUTCHECKBOX(IDF_CHECK4,IPFPLOT(I)%ICLIP)
    ...
    CALL WDIALOGPUTCHECKBOX(IDF_CHECK5,IPFPLOT(I)%IPLOTACOL)         !## overrule acol
    EXIT
   ENDIF
  ENDDO
 ENDIF
 ...
 IF(NIFFLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB3)THEN
  CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB3)
  ALLOCATE(TMPINT(NIFFLIST))
  CALL WDIALOGGETMENU(IDF_MENU1,TMPINT)
  IFFPLOT%ISEL=TMPINT; DEALLOCATE(TMPINT)
  IF(SUM(IFFPLOT(1:NIFFLIST)%ISEL).GT.0)THEN
   DO I=1,NIFFLIST
    IF(IFFPLOT(I)%ISEL.EQ.1)THEN
     ...
     CALL WDIALOGPUTCHECKBOX(IDF_CHECK4,IFFPLOT(I)%ICLIP)
 ...
 IF(NSOLLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB6)THEN
  CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB6)
  ALLOCATE(TMPINT(NSOLLIST))
  SOLPLOT%ISEL=0; CALL WDIALOGGETMENU(IDF_MENU1,TMPINT)
  SOLPLOT(1:NSOLLIST)%ISEL=TMPINT; DEALLOCATE(TMPINT)
  IF(SUM(SOLPLOT(1:NSOLLIST)%ISEL).GT.0)THEN
   DO I=1,NSOLLIST
    IF(SOLPLOT(I)%ISEL.EQ.1)THEN
     CALL WDIALOGPUTCHECKBOX(IDF_CHECK4,SOLPLOT(I)%ICLIP)
  ...
  IF(SUM(GENPLOT(1:NGENLIST)%ISEL).GT.0)THEN
   DO I=1,NGENLIST
    IF(GENPLOT(I)%ISEL.EQ.1)THEN
     ...
     CALL WDIALOGPUTCHECKBOX(IDF_CHECK4,GENPLOT(I)%ICLIP)
     EXIT
    ENDIF
   ENDDO
  ENDIF
 ...
 IF(ID.EQ.ID_D3DSETTINGS_TAB7)THEN
  CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB7)
  ALLOCATE(TMPINT(NCLPLIST))
  CALL WDIALOGGETMENU(IDF_MENU1,TMPINT)
  CLPPLOT%ISEL=TMPINT; DEALLOCATE(TMPINT)
  IF(SUM(CLPPLOT(1:NCLPLIST)%ISEL).GT.0)THEN
   DO I=1,NCLPLIST
    IF(CLPPLOT(I)%ISEL.EQ.1)THEN
     ...
     CALL WDIALOGPUTCHECKBOX(IDF_CHECK1 ,CLPPLOT(I)%ICAP)
     EXIT
    ENDIF
   ENDDO
  ENDIF
...
!  CALL WDIALOGGETMENU(IDF_MENU1,IDFPLOT%DISP_ISEL)
  ...
  IF(SUM(IDFPLOT(1:NIDFLIST)%ISEL).GT.0)THEN
   ...
   DO I=1,NIDFLIST
    IF(IDFPLOT(I)%ISEL.EQ.1)THEN 
     ...
     IF(JD.EQ.IDF_CHECK4)CALL WDIALOGGETCHECKBOX(IDF_CHECK4,IDFPLOT(I)%ICLIP)               !## iclip
     ...
     IF(JD.EQ.IDF_CHECK5)CALL WDIALOGGETCHECKBOX(IDF_CHECK5,IDFPLOT(I)%ITRANSPARANCY)       !## transparancy value
 ...
 IF(NIPF.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB2)THEN
  ...
  DO I=1,SIZE(IPFPLOT); IF(IPFPLOT(I)%ISEL.EQ.1)THEN 
   ...
   IF(JD.EQ.IDF_CHECK4)CALL WDIALOGGETCHECKBOX(IDF_CHECK4,IPFPLOT(I)%ICLIP)       !## iclip
   ...
   IF(JD.EQ.IDF_CHECK5)CALL WDIALOGGETCHECKBOX(IDF_CHECK5,IPFPLOT(I)%IPLOTACOL)   !## overrule acol
  ENDIF; ENDDO
 ...
 IF(NIFFLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB3)THEN
  ...
  IF(SUM(IFFPLOT(1:NIFFLIST)%ISEL).GT.0)THEN
   ...
   DO I=1,NIFFLIST; IF(IFFPLOT(I)%ISEL.EQ.1)THEN
    ...
    IF(JD.EQ.IDF_CHECK4)CALL WDIALOGGETCHECKBOX(IDF_CHECK4,IFFPLOT(I)%ICLIP)       !## iclip
 ...
 IF(NSOLLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB6)THEN
  ...
  ALLOCATE(TMPINT(NSOLLIST))
  CALL WDIALOGGETMENU(IDF_MENU1,TMPINT)
  SOLPLOT%ISEL=TMPINT; DEALLOCATE(TMPINT)
  IF(SUM(SOLPLOT(1:NSOLLIST)%ISEL).GT.0)THEN
   DO I=1,NSOLLIST; IF(SOLPLOT(I)%ISEL.EQ.1)THEN
    ...
    IF(JD.EQ.IDF_CHECK4)CALL WDIALOGGETCHECKBOX(IDF_CHECK4,SOLPLOT(I)%ICLIP)               !## iclip
   ENDIF; END DO
  ENDIF
 ENDIF
 ...
 IF(ID.EQ.ID_D3DSETTINGS_TAB7)THEN
  ...
  IF(SUM(CLPPLOT(1:NCLPLIST)%ISEL).GT.0)THEN
   ...
   DO I=1,NCLPLIST; IF(CLPPLOT(I)%ISEL.EQ.1)THEN
    ...
    IF(JD.EQ.IDF_CHECK1)CALL WDIALOGGETCHECKBOX(IDF_CHECK1,CLPPLOT(I)%ICAP)
   ENDIF; ENDDO
  ENDIF
...
!  CALL WDIALOGGETMENU(IDF_MENU1,IDFPLOT%DISP_ISEL)
  ...
  CALL WDIALOGFIELDSTATE(IDF_RADIO5,J); CALL WDIALOGFIELDSTATE(IDF_TRACKBAR2,J) 
  CALL WDIALOGFIELDSTATE(IDF_LABEL6,J); CALL WDIALOGFIELDSTATE(IDF_CHECK5,J)
  CALL WDIALOGFIELDSTATE(IDF_CHECK4,J); CALL WDIALOGFIELDSTATE(IDF_CHECK2,J)
  CALL WDIALOGFIELDSTATE(IDF_LABEL7,J); CALL WDIALOGFIELDSTATE(IDF_LABEL10,J)
  
  IF(J.EQ.1)THEN
   CALL WDIALOGFIELDSTATE(ID_LEGEND,J)
   ...
   !## print selected IDF name on the graphical window
   CALL WINDOWSELECT(IWIN3D)
   IF(SUM(IDFPLOT(1:NIDFLIST)%ISEL).EQ.1)THEN
    DO I=1,NIDFLIST; IF(IDFPLOT(I)%ISEL.EQ.1)EXIT; ENDDO
    CALL WINDOWOUTSTATUSBAR(2,'Selected IDF file: '//TRIM(IDFPLOT(I)%FNAME))
   ELSE
 ...
 IF(NIPF.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB2)THEN
  ...
  I=MAX(0,MIN(1,SUM(IPFPLOT%ISEL)))
  CALL WDIALOGFIELDSTATE(ID_PROPERTIES,I)
  CALL WDIALOGFIELDSTATE(ID_SELECTION,I)
  CALL WDIALOGFIELDSTATE(ID_EXPORTAS,I)

  CALL WDIALOGFIELDSTATE(ID_OPEN,I); CALL WDIALOGFIELDSTATE(ID_SAVEAS,I); CALL WDIALOGFIELDSTATE(ID_REDRAW,I)
  CALL WDIALOGFIELDSTATE(IDF_CHECK1,I); CALL WDIALOGFIELDSTATE(IDF_INTEGER1,I); CALL WDIALOGFIELDSTATE(IDF_COLOUR1,I)
  CALL WDIALOGFIELDSTATE(IDF_RADIO3,I); CALL WDIALOGFIELDSTATE(IDF_RADIO4,I); CALL WDIALOGFIELDSTATE(IDF_RADIO5,I)
  CALL WDIALOGFIELDSTATE(IDF_CHECK4,I); CALL WDIALOGFIELDSTATE(IDF_CHECK5,I)
  CALL WDIALOGFIELDSTATE(IDF_MENU2,I); CALL WDIALOGFIELDSTATE(IDF_GRID1,I); CALL WDIALOGFIELDSTATE(IDF_CHECK2,I)
  ...
  !## print selected IPF name on the graphical window
  CALL WINDOWSELECT(IWIN3D)
  IF(SUM(IPFPLOT(1:NIPF)%ISEL).EQ.1)THEN
   DO I=1,NIPF; IF(IPFPLOT(I)%ISEL.EQ.1)EXIT; ENDDO
   CALL WINDOWOUTSTATUSBAR(2,'Selected IPF file: '//TRIM(IPFPLOT(I)%FNAME))
  ELSE
   CALL WINDOWOUTSTATUSBAR(2,'')
  ENDIF
 ...
 IF(NIFFLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB3)THEN
  CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB3)
  ALLOCATE(TMPINT(SIZE(IFFPLOT)))
  CALL WDIALOGGETMENU(IDF_MENU1,TMPINT)
  IFFPLOT%ISEL=TMPINT; DEALLOCATE(TMPINT)
  !## at least one selected
  J=0; IF(SUM(IFFPLOT(1:NIFFLIST)%ISEL).GT.0)J=1 
  CALL WDIALOGFIELDSTATE(IDF_CHECK4,J)
  CALL WDIALOGFIELDSTATE(IDF_CHECK1,J)
  ...
  !## print selected IPF name on the graphical window
  CALL WINDOWSELECT(IWIN3D)
  IF(SUM(IFFPLOT(1:NIFFLIST)%ISEL).EQ.1)THEN
   DO I=1,NIFFLIST; IF(IFFPLOT(I)%ISEL.EQ.1)EXIT; ENDDO
   CALL WINDOWOUTSTATUSBAR(2,'Selected IFF file: '//TRIM(IFFPLOT(I)%FNAME))
  ELSE
   CALL WINDOWOUTSTATUSBAR(2,'')
  ENDIF
 ...
 IF(NSOLLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB6)THEN
  CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB6)
  ALLOCATE(TMPINT(SIZE(SOLPLOT)))
  CALL WDIALOGGETMENU(IDF_MENU1,TMPINT)
  SOLPLOT%ISEL=TMPINT; DEALLOCATE(TMPINT)

  !## at least one selected
  J=0; IF(SUM(SOLPLOT(1:NSOLLIST)%ISEL).GT.0)J=1 
  CALL WDIALOGFIELDSTATE(IDF_CHECK3,J)
  CALL WDIALOGFIELDSTATE(IDF_CHECK4,J)
  
  !## print selected IPF name on the graphical window
  CALL WINDOWSELECT(IWIN3D)
  IF(SUM(SOLPLOT(1:NSOLLIST)%ISEL).EQ.1)THEN
   DO I=1,NSOLLIST; IF(SOLPLOT(I)%ISEL.EQ.1)EXIT; ENDDO
   CALL WINDOWOUTSTATUSBAR(2,'Selected SOL file: '//TRIM(SPF(I)%FNAME))
  ELSE
   CALL WINDOWOUTSTATUSBAR(2,'')
  ENDIF
 ...
 IF(NGENLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB4)THEN
  ...
  !## print selected IPF name on the graphical window
  CALL WINDOWSELECT(IWIN3D)
  IF(SUM(GENPLOT(1:NGENLIST)%ISEL).EQ.1)THEN
   DO I=1,NGENLIST; IF(GENPLOT(I)%ISEL.EQ.1)EXIT; ENDDO
   CALL WINDOWOUTSTATUSBAR(2,'Selected GEN file: '//TRIM(GENPLOT(I)%FNAME))
  ELSE
   CALL WINDOWOUTSTATUSBAR(2,'')
  ENDIF
