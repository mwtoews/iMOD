...
!       CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB7)
 ...
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:) :: CLIST
 INTEGER :: I,J,II,N
 ...
 I=0; IF(SUM(IDFPLOT(1:NIDFLIST)%ISEL).GT.0)THEN
  ...
  IF(IDFPLOT(J)%ICUBE.GT.5)THEN
   I=1; N=SIZE(IDFPLOT(J)%TFILES) !; ALLOCATE(CLIST(N))
   !DO II=1,N; CLIST(II)=IDFPLOT(J)%TFILES(II)(INDEX(IDFPLOT(J)%TFILES(II),'\',.TRUE.)+1:); ENDDO
   !CALL WDIALOGPUTMENU(IDF_MENU2,CLIST,N,IDFPLOT(J)%D4ITIME)
