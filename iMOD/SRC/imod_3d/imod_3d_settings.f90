...
USE MOD_PLINES_TRACE, ONLY : TRACEIREV,TRACE_3D_COMPUTE,TRACE_3D_STARTPOINTS,TRACE_3D_STARTPOINTS_SHOW,TRACE_3D_RESET
   ...
   SELECT CASE (ITYPE)
    CASE (FIELDCHANGED)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (IDF_RADIO1,IDF_RADIO2)
       !## switch
       IF(MESSAGE%VALUE1.NE.MESSAGE%VALUE2)PL%IREV=ABS(PL%IREV-1)
       CALL TRACEIREV()
     END SELECT
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
         ...
         CASE (PUSHBUTTON)
          SELECT CASE (PL_MESSAGE%VALUE1)
           CASE (ID_START)
            ...
            ELSEIF(IRUN.EQ.1)THEN; CALL WDIALOGPUTSTRING(ID_START,'Start'); IRUN=0; ENDIF
           CASE (ID_STOP)
            EXIT
          END SELECT
        END SELECT
