...
USE MOD_3D_PROCESS, ONLY : IMOD3D_PROCESSRESIZE,IMOD3D_PROCESSMOUSEMOVE,IMOD3D_PROCESSMOUSEBUTTON
...
USE MOD_PLINES_TRACE, ONLY : TRACEIREV,TRACE_3D_COMPUTE,TRACE_3D_STARTPOINTS,TRACE_3D_STARTPOINTS_SHOW, &
            TRACE_3D_RESET,TRACE_AL_SPR,TRACE_DEAL_SPR
...
CONTAINS
 ...
 IF(IPATHLINE_3D.GT.0)THEN
  ...
  CALL WDIALOGPUTIMAGE(ID_PROP4,ID_ICONPROPERTIES,1)
 ...
 !## show bitmap slightly on bottom
 ...
 INTEGER :: IIPF,IROW,ICOL,IRGB,I,ILEG,PL_ITYPE,IRUN
     ...
     SELECT CASE (MESSAGE%VALUE1)
      CASE (IDF_MENU1)
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
        ...
        CALL IMOD3D_DELETECLIPPLANE('UP'); CALL IMOD3D_DELETECLIPPLANE('DOWN')
   ...
   SELECT CASE (ITYPE)
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      !## location
      CASE (ID_PROP1,ID_PROP2,ID_PROP3,ID_PROP4,ID_PROP5)
       IF(MESSAGE%VALUE1.EQ.ID_PROP1)CALL WDIALOGSELECT(ID_D3DSETTINGS_LOCATION)
       IF(MESSAGE%VALUE1.EQ.ID_PROP2)CALL WDIALOGSELECT(ID_D3DSETTINGS_SINKS)
       IF(MESSAGE%VALUE1.EQ.ID_PROP3)CALL WDIALOGSELECT(ID_D3DSETTINGS_LAYER)
!       IF(MESSAGE%VALUE1.EQ.ID_PROP4)CALL WDIALOGSELECT(ID_D3DSETTINGS_LOCATION)
      ...
      !## start new
      CASE (ID_NEW)
       IF(TRACE_3D_RESET())THEN; ENDIF
       PL%NPART=0; CALL WDIALOGFIELDSTATE(ID_ADD,1); CALL TRACE_3D_STARTPOINTS_SHOW()
       CALL IMOD3D_DISPLAY(1)
      ...
      CASE (ID_START)
       IF(TRACE_3D_RESET())THEN
        !## set the startpoints
        CALL TRACE_AL_SPR()
        CALL WDIALOGGETRADIOBUTTON(IDF_RADIO3,PL%ITYPE)
        CALL WDIALOGFIELDSTATE(ID_STOP,1); CALL WDIALOGPUTSTRING(ID_START,'Pause'); IRUN=1
        DO
         IF(IRUN.EQ.1)THEN
          !## simulation busy
          CALL WMESSAGEPEEK(PL_ITYPE,PL_MESSAGE)
         ELSE
          !## wait for message
          CALL WMESSAGE(PL_ITYPE,PL_MESSAGE)
         ENDIF
         CALL WDIALOGSELECT(PL_MESSAGE%WIN)
         SELECT CASE (PL_ITYPE)
          CASE (MOUSEBUTDOWN,MOUSEBUTUP)
           CALL IMOD3D_PROCESSMOUSEBUTTON(PL_ITYPE,PL_MESSAGE%VALUE1,PL_MESSAGE%X,PL_MESSAGE%Y)
          CASE (MOUSEMOVE)
           CALL IMOD3D_PROCESSMOUSEMOVE(PL_MESSAGE%X,PL_MESSAGE%Y)
          CASE (EXPOSE)
           CALL IMOD3D_DISPLAY(1)
          CASE (RESIZE)
           CALL IMOD3D_PROCESSRESIZE(PL_MESSAGE%VALUE1,PL_MESSAGE%VALUE2)
          CASE (PUSHBUTTON)
           SELECT CASE (PL_MESSAGE%VALUE1)
            CASE (ID_START)
             IF(IRUN.EQ.0)THEN; CALL WDIALOGPUTSTRING(ID_START,'Pause'); IRUN=1
             ELSEIF(IRUN.EQ.1)THEN; CALL WDIALOGPUTSTRING(ID_START,'Continue'); IRUN=0; ENDIF
            CASE (ID_STOP)
             EXIT
           END SELECT
         END SELECT
         IF(IRUN.EQ.1)THEN
          ...
          IF(.NOT.TRACE_3D_COMPUTE())EXIT; CALL IMOD3D_DISPLAY(1)
         ENDIF
        ENDDO
        CALL WDIALOGPUTSTRING(ID_START,'Start'); CALL WDIALOGFIELDSTATE(ID_STOP,0)
        CALL TRACE_DEAL_SPR(); CALL IMOD3D_DISPLAY(1)
       ENDIF      
     END SELECT
   END SELECT
  ...
  IF(SUM(SOLPLOT(1:NSOLLIST)%ISEL).GT.0)THEN
   DO I=1,NSOLLIST
    IF(SOLPLOT(I)%ISEL.EQ.1)THEN
     ...
     CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,SOLPLOT(I)%IBITMAP)
     CALL WDIALOGPUTCHECKBOX(IDF_CHECK3,SOLPLOT(I)%IINTERFACE)
     EXIT
    ENDIF
   ENDDO
  ENDIF
 ...
 IF(NSOLLIST.GT.0.AND.ID.EQ.ID_D3DSETTINGS_TAB6)THEN
  ...
  IF(SUM(SOLPLOT(1:NSOLLIST)%ISEL).GT.0)THEN
   DO I=1,NSOLLIST; IF(SOLPLOT(I)%ISEL.EQ.1)THEN
    ...
    IF(JD.EQ.IDF_CHECK2)CALL WDIALOGGETCHECKBOX(IDF_CHECK2,SOLPLOT(I)%IBITMAP)
    IF(JD.EQ.IDF_CHECK3)CALL WDIALOGGETCHECKBOX(IDF_CHECK3,SOLPLOT(I)%IINTERFACE)
   ENDIF; END DO
  ENDIF
 ENDIF
