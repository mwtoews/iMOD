...
! BSIZE=XYZAXES(1)*(IPFPLOT(IIPF)%RADIUS/100.0) 
 BSIZE=IPFPLOT(IIPF)%RADIUS !/100.0
   ...
   ELSEIF(IPFPLOT(IIPF)%IFANCY.EQ.1)THEN
    IF(ASSF(NASSLIST)%NRASS.GE.2)THEN
     DO I=1,ASSF(NASSLIST)%NRASS-1
      ...
      ENDIF
      ZT=ASSF(NASSLIST)%Z(I); ZD=ASSF(NASSLIST)%Z(I+1)
...
!    Y=(Y-MIDPOS%Y)/VIEWDY
 ...
 IF(ALLOCATED(GENLISTINDEX))THEN
  ...
  DO I=1,K
   NINGEN=0; NGENLIST=NGENLIST+1; CALL IMOD3D_DRAWGEN(GENPLOT(I)%GENFNAME,GENPLOT(I)%L3D,NINGEN)  !VIEWDX,VIEWDY,
  ENDDO
 ...
 DO I=1,MXGEN
  ...
  IF(GEN(I)%IACT.AND.GEN(I)%ISEL.AND.ABS(GEN(I)%ITYPE).EQ.1)THEN
   ...
   IF(LEX)THEN
    ...
    CALL IMOD3D_DRAWGEN(GENPLOT(NGENLIST)%GENFNAME,GENPLOT(NGENLIST)%L3D,NINGEN) !VIEWDX,VIEWDY,
    IF(NINGEN.LE.0)THEN
