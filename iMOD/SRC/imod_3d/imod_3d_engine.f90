...
SOLLOOP: DO I=1,NSOLLIST
 ...
 NIDF_CC=0; DO I=1,SIZE(MP) 
  IF(MP(I)%IACT.AND.MP(I)%ISEL.AND.MP(I)%IPLOT.EQ.6)THEN
   ...
   IF(LEX)THEN
    ...
    IF(IMOD3D_CREATECOOKIECUTTERS_FILL(XPOL,YPOL,N,NIDF_CC))THEN; ENDIF
   ENDIF
  ENDIF
 ENDDO
 CALL POLYGON1CLOSE() 
 ...
 !###======================================================================
 LOGICAL FUNCTION IMOD3D_CREATECOOKIECUTTERS_FILL(XPOL,YPOL,N,M)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N
 ...
 DO I=1,SHPNO
  IF(SHPNCRD(I).GT.0)THEN
   ...
   DO IROW=IR1,IR2; DO ICOL=IC1,IC2
    ...
    IF(UTL_INSIDEPOLYGON(XVAL,YVAL,SHPXC(:,I),SHPYC(:,I),XPOL,YPOL,SHPNCRD(I)).EQ.1)THEN
 ...
 !## get display-list pointers
 ALLOCATE(SOLLISTINDEX(NSOLLIST),SOLPLOT(NSOLLIST))
 SOLLISTINDEX=0
 ...
 DO I=1,NSPF
  ...
  !## list index for
  SOLLISTINDEX(NSOLLIST)=GLGENLISTS(1)
  !## start new drawing list
  CALL GLNEWLIST(SOLLISTINDEX(NSOLLIST),GL_COMPILE)

  !## process each cross-section (nidf) --- two-by-two
  DO IPROF=1,SIZE(SPF(I)%PROF)-1  !,2
 ...
 !## list index for
 NCLPLIST=NCLPLIST+1
 SOLLISTINDEX(NCLPLIST)=GLGENLISTS(1)
 !## start new drawing list
