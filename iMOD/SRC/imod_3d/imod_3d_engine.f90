...
USE MOD_UTL, ONLY : UTL_INVERSECOLOUR,UTL_CAP,UTL_GETUNIT,ITOS,RTOS,UTL_FILLARRAY,UTL_IDFGETCLASS, &
       ...
       UTL_GETHELP,UTL_DIST,UTL_GETUNIQUE_DINT,YMDHMSTOITIME !,UTL_LISTOPENFILES
USE MOD_GENPLOT_PAR
...
!  IF(IPFPLOT(IIPF)%ISEL.EQ.1)NSEL(IIPF)=NSEL(IIPF)+IPFDLIST(1,I)
 ...
 !# hier moet je legenda idf selecteren
...
!###======================================================================
 ...
 INTEGER :: I,II,JJ,ILST,KLST,LLST,ICOL,IROW,IU,ITYPE
 TYPE(WIN_MESSAGE) :: MESSAGE
  ...
  ILST=0; DO I=1,SIZE(IDFPLOT) 
   ...
   DO II=1,SIZE(IDFPLOT(I)%TFILES)
    ...
    JJ=0; DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
     JJ=JJ+1; IDFPLOT(I)%ICLR(JJ)=UTL_IDFGETCLASS(IDFPLOT(I)%LEG,IDF(1)%X(ICOL,IROW))
    ENDDO; ENDDO
...
! call UTL_LISTOPENFILES()
 ...
 !## breidtr tekentn van een planes uit met de zijkanten cvia die subroutine in geval icube=6
...
! IF(ICUBE.EQ.6)THEN; CALL GLDISABLE(GL_COLOR_MATERIAL); CALL GLENDLIST(); ENDIF
