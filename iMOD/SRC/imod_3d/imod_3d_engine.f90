...
USE MOD_UTL, ONLY : INVERSECOLOUR,UTL_CAP,UTL_GETUNIT,ITOS,RTOS,UTL_FILLARRAY,UTL_IDFGETCLASS, &
       UTL_IDFSNAPTOGRID,UTL_MESSAGEHANDLE,UTL_INSIDEPOLYGON,UTL_WAITMESSAGE,NEWLINE
USE MOD_GENPLOT_PAR
...
USE MOD_3D_DISPLAY, ONLY : IMOD3D_SETCOLOR,IMOD3D_RETURNCOLOR
USE MOD_PROFILE_UTL, ONLY : PROFILE_AXES
...
!USE MOD_SOF, ONLY : SOF_COMPUTE_GRAD
...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=GLFLOAT) :: X1,X2,Y1,Y2,Z1,Z2,DX,DY,DZ,V1,V2,VI
 INTEGER :: I,J,N,M
 REAL,PARAMETER :: D= 100.0  !## 1/d=percentage of axes-lines

 !## number of labels on axes
 N=6 !10
 ...
 CALL GLNEWLIST(AXESINDEX(2),GL_COMPILE)
  ...
  V1=X1+(V1-BOT%X)*((X2-X1)/(TOP%X-BOT%X))
  V2=X1+(V2-BOT%X)*((X2-X1)/(TOP%X-BOT%X))
  VI=(V2-V1)/REAL(M)

  DY=(Y2-Y1)/D
  DO I=1,M
   DO J=0,1
    DX=V1+VI*(REAL(I-1)+(REAL(J)*0.5))
    IF(DX.GT.X1.AND.DX.LT.X2)THEN
     CALL GLVERTEX3F(DX,Y1,Z1)
     CALL GLVERTEX3F(DX,Y2,Z1)
     CALL GLVERTEX3F(DX,Y2,Z1)
     CALL GLVERTEX3F(DX,Y2,Z2)
    ENDIF
   ENDDO
  END DO
  ...
  !## check i
  DO WHILE(M.LT.N); M=MAX(1,M)*2; ENDDO
  V1=Y1+(V1-BOT%Y)*((Y2-Y1)/(TOP%Y-BOT%Y))
  V2=Y1+(V2-BOT%Y)*((Y2-Y1)/(TOP%Y-BOT%Y))
  VI=(V2-V1)/REAL(M)

  DX=(X2-X1)/D
  DO I=1,M
   DO J=0,1
    DY=V1+VI*(REAL(I-1)+(REAL(J)*0.5))
    IF(DY.GT.Y1.AND.DY.LT.Y2)THEN
     CALL GLVERTEX3F(X1,DY,Z1)
     CALL GLVERTEX3F(X2,DY,Z1)
     CALL GLVERTEX3F(X2,DY,Z1)
     CALL GLVERTEX3F(X2,DY,Z2)
    ENDIF
   END DO
  END DO
  ...
  !## check i
  DO WHILE(M.LT.N); M=MAX(1,M)*2; ENDDO
  V1=Z1+(V1-BOT%Z)*((Z2-Z1)/(TOP%Z-BOT%Z))
  V2=Z1+(V2-BOT%Z)*((Z2-Z1)/(TOP%Z-BOT%Z))
  VI=(V2-V1)/REAL(M)
  ...
  DO I=1,M
   DO J=0,1
    ...
    IF(DZ.GT.Z1.AND.DZ.LT.Z2)THEN
     CALL GLVERTEX3F(X1,Y2,DZ)
     CALL GLVERTEX3F(X2,Y2,DZ)
     CALL GLVERTEX3F(X2,Y1,DZ)
     CALL GLVERTEX3F(X2,Y2,DZ)
    ENDIF
   END DO
  END DO
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=GLFLOAT) :: X1,X2,Y1,Y2,Z1,Z2,DX,DY,DZ,V1,V2,VI,T1,TI,TT,DT!,T2
 ...
 CALL GLNEWLIST(AXESINDEX(3),GL_COMPILE)
  ...
  DO I=1,M
   ...
   IF(DX.GT.X1.AND.DX.LT.X2)THEN
    ...
    CALL WGLTEXTSTRING(' '//TRIM(RTOS(REAL(TT),'F',2)))
    CALL GLPOPMATRIX()
    !## top
  ...
  CALL GLPUSHMATRIX()
  CALL GLTRANSLATEF((V2+V1)/2.0,Y1-(DY*30.0),Z1)
  CALL GLSCALED(1.0_GLDOUBLE/XS,1.0_GLDOUBLE/YS,1.0_GLDOUBLE/ZS)
  CALL GLSCALED(TS,TS,TS)
  CALL WGLTEXTSTRING('X-axes (meters)')
  CALL GLPOPMATRIX()
  CALL GLPUSHMATRIX()
  CALL GLTRANSLATEF((V2+V1)/2.0,Y2+(DY*30.0),Z2)
  CALL GLSCALED(1.0_GLDOUBLE/XS,1.0_GLDOUBLE/YS,1.0_GLDOUBLE/ZS)
  CALL GLSCALED(TS,TS,TS)
  CALL WGLTEXTSTRING('X-axes (meters)')
  CALL GLPOPMATRIX()
  ...
  CALL WGLTEXTORIENTATION(ALIGNCENTRE)
  CALL GLPUSHMATRIX()
  CALL GLTRANSLATEF(X1-(DX*30.0),(V1+V2)/2.0,Z1)
  CALL GLSCALED(1.0_GLDOUBLE/XS,1.0_GLDOUBLE/YS,1.0_GLDOUBLE/ZS)
  CALL GLSCALED(TS,TS,TS)
  CALL GLROTATED(-90.0_GLDOUBLE,0.0_GLDOUBLE,0.0_GLDOUBLE,1.0_GLDOUBLE)
  CALL WGLTEXTSTRING('Y-axes (meters)')
  CALL GLPOPMATRIX()
  CALL GLPUSHMATRIX()
  CALL GLTRANSLATEF(X2+(DX*30.0),(V1+V2)/2.0,Z2)
  CALL GLSCALED(1.0_GLDOUBLE/XS,1.0_GLDOUBLE/YS,1.0_GLDOUBLE/ZS)
  CALL GLSCALED(TS,TS,TS)
  CALL GLROTATED(-90.0_GLDOUBLE,0.0_GLDOUBLE,0.0_GLDOUBLE,1.0_GLDOUBLE)
  CALL WGLTEXTSTRING('Y-axes (meters)')
  ...
  DO I=1,M
   ...
   IF(DZ.GT.Z1.AND.DZ.LT.Z2)THEN
    ...
    CALL WGLTEXTREAL(REAL(TT),'(F10.2)')
    CALL GLPOPMATRIX()
    !## right
    CALL WGLTEXTORIENTATION(ALIGNLEFT)
    CALL GLPUSHMATRIX()
    ...
    CALL GLSCALED(TS,TS,TS)
    CALL GLROTATED( 90.0_GLDOUBLE,1.0_GLDOUBLE,0.0_GLDOUBLE,0.0_GLDOUBLE)
    CALL WGLTEXTSTRING(TRIM(RTOS(TT,'F',2)))
    CALL GLPOPMATRIX()
   ENDIF
  ENDDO
  ...
  CALL GLROTATED(-90.0_GLDOUBLE,0.0_GLDOUBLE,0.0_GLDOUBLE,1.0_GLDOUBLE)
  CALL WGLTEXTORIENTATION(ALIGNCENTRE)
  CALL WGLTEXTSTRING('Z-axes (meters)')
  CALL GLPOPMATRIX()
  CALL GLPUSHMATRIX()
  ...
  IF(IPF(I)%ACOL.NE.0)THEN
   ...
   DO J=1,NLITHO
    ...
    CALL GLPUSHMATRIX()
     CALL GLTRANSLATEF(X+(DX*1.5_GLFLOAT),Y- DY+((XSIZE-TS)/2.0_GLFLOAT),0.0_GLFLOAT) 
     CALL GLSCALEF(1.0_GLFLOAT,1.0_GLFLOAT/RAT,1.0_GLFLOAT)
     CALL GLSCALEF(TS,TS,TS); CALL WGLTEXTSTRING(TRIM(BH(J)%LITHOTXT))
    CALL GLPOPMATRIX() 

    CALL IMOD3D_SETCOLOR(BH(J)%LITHOCLR)
    CALL GLBEGIN(GL_POLYGON)
     CALL GLVERTEX3F(X   ,Y   ,0.0_GLFLOAT); CALL GLVERTEX3F(X+DX,Y   ,0.0_GLFLOAT)
     CALL GLVERTEX3F(X+DX,Y-DY,0.0_GLFLOAT); CALL GLVERTEX3F(X   ,Y-DY,0.0_GLFLOAT)
    CALL GLEND()

    IF(J.NE.NLITHO)Y=Y-(DY*1.5_GLFLOAT)
 ...
 IF(.NOT.IMOD3D_SETTINGS_IDF())RETURN
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (IDOK,IDCANCEL)
      EXIT
     CASE (IDHELP)
      CALL IMODGETHELP('5.3.2','3D Plot Settings') 
    END SELECT
  END SELECT
 END DO
 ...
 !###======================================================================
 LOGICAL FUNCTION IMOD3D_SETTINGS_IDF()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,IS,ITYPE,ICONFIG,N,M,IC1,IC2,IR1,IR2
 TYPE(WIN_MESSAGE) :: MESSAGE
   ...
   CASE (FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_MENU4)
      ...
      DO I=IS,NIDFLIST
       IF(I.LE.SIZE(ICOLOR))THEN
        IDFPLOT(I)%ICOLOR=ICOLOR(I) 
       ELSE
        IDFPLOT(I)%ICOLOR=WRGB(255,0,0)  
       ENDIF
      ENDDO    
      ...
      SELECT CASE (ICONFIG)
       ...
       CASE (2)
        ...
        IDFPLOT%IPLOTLEGEND=1; IDFPLOT%IFILL=3; IDFPLOT(IS-1)%IFILL=2; IDFPLOT(IS-1)%ICOLOR=WRGB(255,255,255)
       ...
       CASE (3)
        ...
        IDFPLOT%IPLOTLEGEND=0; IDFPLOT%IFILL=3 
       ...
       CASE (5)
        ...
        IDFPLOT(IS-1)%IFILL=2; IDFPLOT(IS-1)%ICOLOR=WRGB(255,255,255) 
       ...
       CASE (7)
        DO I=IS,NIDFLIST-2,2
         IDFPLOT(I)%IFILL=3; IDFPLOT(I)%ICOMBINE=MIN(NIDFLIST,I+2); IDFPLOT(I)%IDFLEGEND=MIN(NIDFLIST,I+1); IDFPLOT(I+1)%ICUBE=5
        ENDDO
 ...
 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  ...
  CALL WDIALOGGETMENU(IDF_MENU1,I); IDFDATA(1)=IACC(I,1); IDFDATA(2)=IACC(I,2)  !## col,row
...
!###======================================================================
 ...
 REAL(KIND=GLFLOAT) :: DX,DY
 INTEGER,DIMENSION(3) :: ID_IDF,ND_IDF
 
 IMOD3D_REDRAWIDF=.FALSE.

 DX=(TOP%X-BOT%X)/2.0_GLFLOAT/XYZAXES(1); DY=(TOP%Y-BOT%Y)/2.0_GLFLOAT/XYZAXES(2)
 ...
 ILST=0; DO I=1,SIZE(IDFPLOT) 
  ...
  DO II=1,JJ
   ...
   CALL WINDOWOUTSTATUSBAR(1,'Processing '//TRIM(IDFPLOT(I)%FNAME)//'('//TRIM(RTOS(REAL(I*100)/REAL(SIZE(IDFPLOT)),'F',2))//'%)')
   !## first is template
   IF(II.EQ.1)THEN
  ...
  IF(II.GT.JJ.AND.SUM(ND_IDF).EQ.3)THEN
   ...
   IF(IMOD3D_CREATE_SXY(IDF(1),I,DX,DY))THEN; ENDIF  
 ...
 CALL IMOD3D_THICKNESS(ILST,LLST,LEG)!,MINT,MAXT)
 ...
 REAL :: DX,DY,DZ,F,FF,VL,MINZ,MAXZ,DZDX,DZDY,DZDZ
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IIPF,I,J,IPLOT,IU,IRAT,IRAT1
 REAL(KIND=GLFLOAT) :: DX,DY,X,Y,Z,Z2
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.2)THEN
   ...
   IF(UTL_CAP(IPF(IIPF)%FNAME,'U').EQ.UTL_CAP(MP(IPLOT)%IDFNAME,'U'))THEN
    CALL WINDOWOUTSTATUSBAR(1,'RF Memory ...'//TRIM(MP(IPLOT)%IDFNAME))
   ELSE
    !## read always from disc
    CALL WINDOWOUTSTATUSBAR(1,'RF Disc ...'//TRIM(MP(IPLOT)%IDFNAME))
 ...
 DO IIPF=1,NIPF
  ...
  CALL WINDOWOUTSTATUSBAR(1,'Get selection for '//TRIM(IPF(IIPF)%FNAME)//' ...')
 ...
 IF(ALLOCATED(IPFLISTINDEX))THEN
  CALL WINDOWOUTSTATUSBAR(1,'Cleaning memory ...')
 ...
 DO IIPF=1,NIPF
  ...
  DO I=1,IPF(IIPF)%NROW
   ...
   IF(ACOL.NE.0)CALL UTL_WAITMESSAGE(IRAT,IRAT1,I,IPF(IIPF)%NROW,'Get boreholes for '//TRIM(IPF(IIPF)%FNAME),IBOX=1)
   ...
   IF(IPF(IIPF)%IPOS(I).EQ.INT(1,1))THEN
     ...
     IF(IPFOPENASSFILE(IU,NASSLIST,FNAME))THEN
      ...
      IF(ASSF(NASSLIST)%ITOPIC.EQ.2)THEN
       IF(IPFREADASSFILELABEL(IU,NASSLIST,FNAME).AND.  &
        ...
        CALL IMOD3D_DRAWIPF(X,Y,Z,Z2,NASSLIST,NIPFLIST,1,1,ICLR,IIPF) !## actual drills
        CALL IMOD3D_DRAWIPF(X,Y,Z,Z2,NASSLIST,NIPFLIST,1,2,ICLR,IIPF) !## selection numbers drills
        LEX=.TRUE.
       ELSE
        CLOSE(IU)
    ...
    IF(ACOL.EQ.0)THEN
     ...
     CALL IMOD3D_DRAWIPF(X,Y,Z,Z2,NIPFLIST,NIPFLIST,0,1,ICLR,IIPF)  !## actual drills
     CALL IMOD3D_DRAWIPF(X,Y,Z,Z2,NIPFLIST,NIPFLIST,0,2,ICLR,IIPF)  !## selection numbers drills
    ENDIF
...
SOLLOOP: DO I=1,NSOLLIST
    ...
    DO J=2,SPF(I)%NXY
     DZ=IGRDISTANCELINE(SPF(I)%X(J-1),SPF(I)%Y(J-1),SPF(I)%X(J),SPF(I)%Y(J), &
        REAL(IPFDLIST(3,IASSF)),REAL(IPFDLIST(4,IASSF)),0)
     IF(DZ.LE.Z)THEN; IPFDLIST(5,IASSF)=1; EXIT SOLLOOP; ENDIF
    ENDDO
   ENDDO SOLLOOP
  ELSE
   IPFDLIST(5,IASSF)=1
  ...
  IF(ALLOCATED(ASSF))THEN
   IF(ASSOCIATED(ASSF(IASSF)%Z))THEN
    Z=ASSF(IASSF)%Z(1)-ASSF(IASSF)%Z(ASSF(IASSF)%NRASS)
    !## less than 
    IF(IPFPLOT(IIPF)%ISELECT(1).EQ.1.AND.Z.LT.IPFPLOT(IIPF)%RSELECT(1))IPFDLIST(5,IASSF)=0
    !## greater than 
    IF(IPFPLOT(IIPF)%ISELECT(2).EQ.1.AND.Z.GT.IPFPLOT(IIPF)%RSELECT(2))IPFDLIST(5,IASSF)=0
   ENDIF
  ENDIF
 ...
 !###======================================================================
 SUBROUTINE IMOD3D_DRAWIPF(X,Y,Z,Z2,IASSF,IPFILIST,IPLOTTYPE,IMODE,ICLR,IIPF)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IASSF,IPLOTTYPE,IMODE,ICLR,IIPF,IPFILIST
 REAL(KIND=GLFLOAT),INTENT(IN) :: X,Y,Z,Z2
 REAL(KIND=GLFLOAT) :: ZD,ZT,BSIZE,FRAC,MXW
 INTEGER :: I,JCLR
 REAL :: IWIDTH

 BSIZE=XYZAXES(1)*(IPFPLOT(IIPF)%RADIUS/100.0) !/25.0_GLFLOAT
 MXW  =MAXVAL(BH%LITHOWIDTH)
  
 !## imode=1: drawing list
 !## imode=2: selection drawing list to be drawn in background only

 IPFLISTINDEX(IPFILIST,IMODE)=GLGENLISTS(1)

 !## start new drawing list for current object
 CALL GLNEWLIST(IPFLISTINDEX(IPFILIST,IMODE),GL_COMPILE)
 ...
 IF(IPLOTTYPE.EQ.1)THEN
  ...
  IF(IMODE.EQ.1)THEN
   ...
   IF(IPFPLOT(IIPF)%IFANCY.EQ.0)THEN
    ...
    DO I=1,ASSF(IASSF)%NRASS-1
     CALL IPFDRAWITOPIC2_ICLR(I,IASSF,JCLR,IWIDTH)
   ...
   ELSEIF(IPFPLOT(IIPF)%IFANCY.EQ.1)THEN
    IF(ASSF(IASSF)%NRASS.GE.2)THEN
     DO I=2,ASSF(IASSF)%NRASS
      CALL IPFDRAWITOPIC2_ICLR(I-1,IASSF,JCLR,IWIDTH)
      FRAC=IWIDTH/MXW
      CALL IMOD3D_SETCOLOR(JCLR)
  ...
  ELSEIF(IMODE.EQ.2)THEN
   ...
   IF(IPFPLOT(IIPF)%IFANCY.EQ.0)THEN
    CALL GLBEGIN(GL_LINES)
    CALL IMOD3D_SETCOLOR(IASSF)
    CALL GLVERTEX3F(X,Y,ZD)
    ZD=ASSF(IASSF)%Z(ASSF(IASSF)%NRASS)
    CALL GLVERTEX3F(X,Y,ZD)
    CALL GLEND()
   ELSEIF(IPFPLOT(IIPF)%IFANCY.EQ.1)THEN
    IF(ASSF(IASSF)%NRASS.GE.2)THEN
     CALL IMOD3D_SETCOLOR(IASSF)
     ZT=ZD; ZD=ASSF(IASSF)%Z(ASSF(IASSF)%NRASS)
 ...
 REAL(KIND=GLDOUBLE),PARAMETER :: TS= 0.1 !## textsize
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.3)THEN
   ...
   CALL IMOD3D_DRAWIFF(DX,DY,IPLOT,NINIFF)!MP(IPLOT)%IDFNAME)
  ENDIF
 ENDDO
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.3)THEN
   ...
   CALL IMOD3D_DRAWIFF(DX,DY,IPLOT,NINIFF)
 ...
 INTEGER :: I,N,IOS,IU,ICLR
 REAL(KIND=GLFLOAT),DIMENSION(2) :: XCOR,YCOR,ZCOR
 TYPE IFFTYPE
  INTEGER :: IL,IPART
  REAL :: X,Y,Z,V,T
 END TYPE IFFTYPE
 TYPE(IFFTYPE),DIMENSION(2) :: IFF
 REAL :: XVAL

 CALL WINDOWSELECT(IWIN); CALL WINDOWOUTSTATUSBAR(1,'Reading '//TRIM(MP(IPLOT)%IDFNAME)//'...')
 ...
 IF(ALLOCATED(GENLISTINDEX))THEN
  ...
  DO I=1,K
   NINGEN=0; NGENLIST=NGENLIST+1; CALL IMOD3D_DRAWGEN(DX,DY,GENPLOT(I)%GENFNAME,GENPLOT(I)%L3D,NINGEN)
  ENDDO
 ...
 DO I=1,MXGEN
  ...
  IF(GEN(I)%IACT.AND.GEN(I)%ISEL.AND.ABS(GEN(I)%ITYPE).EQ.1)THEN
   ...
   IF(LEX)THEN
    ...
    CALL IMOD3D_DRAWGEN(DX,DY,GENPLOT(NGENLIST)%GENFNAME,GENPLOT(NGENLIST)%L3D,NINGEN)
    IF(NINGEN.LE.0)THEN
 ...
 CALL WINDOWSELECT(IWIN); CALL WINDOWOUTSTATUSBAR(1,'Reading '//TRIM(FNAME)//'...')
 ...
 DO
  ...
  ELSE
   IF((X(1).GT.BOT%X.OR.X(2).GT.BOT%X).AND.(X(1).LT.TOP%X.OR.X(2).LT.TOP%X).AND. &
    ...
    XCOR(3)=XCOR(2); YCOR(3)=YCOR(2)
    XCOR(4)=XCOR(1); YCOR(4)=YCOR(1)
    !## begin OpenGL-Quads       
    CALL GLBEGIN(GL_QUADS)
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=GLFLOAT) :: DX,DY,DXX,DYY,GX,GY,GZ,DXY
 INTEGER :: I,J,K,II,JJ,IPROF,IPOS,N,I1,I2,IICLR,ILAY
 ...
 DO I=1,NSPF
  ...
  DO IPROF=1,SIZE(SPF(I)%PROF)-1  
   ...
   ILAY=INT(IPROF/2); IF(MOD(IPROF,2).NE.0)ILAY=ILAY+1
   K=1; IF(MOD(IPROF,2).EQ.0)K=2
   IICLR=SLD(1)%TBCLR(ILAY,K)
 ...
 IMOD3D_SOL=IMOD3D_SOL_BMP(DX,DY) 
 ...
 IMPLICIT NONE
 REAL(KIND=GLFLOAT),INTENT(IN) :: DX,DY
 REAL(KIND=GLFLOAT) :: X1,X2,Y1,Y2,XT1,XT2,YT1,YT2,Z1,Z2,DXX,DYY,DXY
 INTEGER :: I,J
 ...
 DO I=1,NSPF
  ...
  J=SPF(I)%NXY
  X1=(SPF(I)%X(1)-MIDPOS%X)/DX
  X2=(SPF(I)%X(J)-MIDPOS%X)/DX
  Y1=(SPF(I)%Y(1)-MIDPOS%Y)/DY
  ...
  CALL GLBEGIN(GL_QUADS)
   CALL GLTEXCOORD2F(XT1,YT1); CALL GLVERTEX3F(X1,Y1,Z1)
   CALL GLTEXCOORD2F(XT1,YT2); CALL GLVERTEX3F(X1,Y1,Z2)
   CALL GLTEXCOORD2F(XT2,YT2); CALL GLVERTEX3F(X2,Y2,Z2)
   CALL GLTEXCOORD2F(XT2,YT1); CALL GLVERTEX3F(X2,Y2,Z1)
  CALL GLEND()

!  DO J=2,SPF(I)%NXY 
!   DXX=SPF(I)%X(J)-SPF(I)%X(J-1); DYY=SPF(I)%Y(J)-SPF(I)%Y(J-1)
!   DXY=0.0; IF(DXX+DYY.NE.0.0)DXY=SQRT(DXX**2.0+DYY**2.0)
...
!  CALL GLBEGIN(GL_QUADS)
!   CALL GLTEXCOORD2F(XT1,YT1); CALL GLVERTEX3F(X1,Y1,Z1)
!   CALL GLTEXCOORD2F(XT1,YT2); CALL GLVERTEX3F(X1,Y2,Z2)
!   CALL GLTEXCOORD2F(XT2,YT2); CALL GLVERTEX3F(X2,Y2,Z2)
!   CALL GLTEXCOORD2F(XT2,YT1); CALL GLVERTEX3F(X2,Y1,Z1)
!  CALL GLEND()
 ENDDO
 ...
 CALL IGRFILEINFO(SPF(ISPF)%PBITMAP%FNAME,INFO,3)
 ...
 ALLOCATE(IBMPDATA(IWIDTH*IHEIGHT),STAT=IOS)
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not allocate enough memory IBMPDATA() to store image'//CHAR(13)// &
 ...
 IF(IOS.NE.0)THEN
  DEALLOCATE(IBMPDATA)
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not allocate enough memory FRGB() to store image'//CHAR(13)// &
  TRIM(SPF(ISPF)%PBITMAP%FNAME),'Error'); RETURN
 ENDIF

 CALL IGRLOADIMAGEDATA(SPF(ISPF)%PBITMAP%FNAME,IBMPDATA) 
