     ...
     IF(IPFOPENASSFILE(IU,NASSLIST,FNAME))THEN
      ...
      IF(ASSF(NASSLIST)%ITOPIC.EQ.2.OR.ASSF(NASSLIST)%ITOPIC.EQ.4)THEN
       IF(IPFREADASSFILELABEL(IU,NASSLIST,FNAME).AND.  &
        ...
        CALL IMOD3D_DRAWIPF(X,Y,Z,Z2,ASSF(NASSLIST)%ITOPIC,1,ICLR,IIPF) !## actual drills
        CALL IMOD3D_DRAWIPF(X,Y,Z,Z2,ASSF(NASSLIST)%ITOPIC,2,ICLR,IIPF) !## selection numbers drills
        !## put label in centre
...
SOLLOOP: DO I=1,NSOLLIST
 ...
 !###======================================================================
 SUBROUTINE IMOD3D_DRAWIPF(X,Y,Z,ZZ,IPLOTTYPE,IMODE,ICLR,IIPF) 
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOTTYPE,IMODE,ICLR,IIPF
 REAL(KIND=GLFLOAT),INTENT(IN) :: X,Y,Z,ZZ
 REAL(KIND=GLFLOAT) :: X1,X2,Y1,Y2,Z1,Z2,BSIZE,FRAC,MXW
 INTEGER :: I,JCLR
 REAL :: IWIDTH
 LOGICAL :: LEX
 ...
 IF(IPLOTTYPE.EQ.2.OR.IPLOTTYPE.EQ.4)THEN
  ...
  Z1=ASSF(NASSLIST)%Z(1)
  
  IF(LEX)TOP%Z=MAX(TOP%Z,Z1)
...
!  ENDIF
   ...
   IF(IPFPLOT(IIPF)%IFANCY.EQ.0)THEN
    ...
    IF(IPLOTTYPE.EQ.2)THEN
     ...
     DO I=1,ASSF(NASSLIST)%NRASS-1
      ...
      CALL GLVERTEX3F(X,Y,Z1)
      Z1=ASSF(NASSLIST)%Z(I+1)
      CALL GLVERTEX3F(X,Y,Z1)
     ENDDO
     CALL GLEND()
    ...
    ELSEIF(IPLOTTYPE.EQ.4)THEN
     ...
     DO I=1,ASSF(NASSLIST)%NRASS-1
      ...
      !## from
      X1=X+ASSF(NASSLIST)%DX(I)
      Y1=Y+ASSF(NASSLIST)%DY(I)
      Z1=  ASSF(NASSLIST)%Z(I)
      CALL GLVERTEX3F(X1,Y1,Z1)
      !## to
      X2=X+ASSF(NASSLIST)%DX(I+1)
      Y2=Y+ASSF(NASSLIST)%DY(I+1)
      Z2=  ASSF(NASSLIST)%Z(I+1)
      CALL GLVERTEX3F(X2,Y2,Z2)

      IF(LEX)THEN
       TOP%X=MAX(TOP%X,X1,X2); BOT%X=MIN(BOT%X,X1,X2)
       TOP%Y=MAX(TOP%Y,Y1,Y2); BOT%Y=MIN(BOT%Y,Y1,Y2)
       TOP%Z=MAX(TOP%X,Z1,Z2); BOT%Z=MIN(BOT%Z,Z1,Z2)
      ENDIF
   ...
   ELSEIF(IPFPLOT(IIPF)%IFANCY.EQ.1)THEN
    ...
    IF(ASSF(NASSLIST)%NRASS.GE.2)THEN
     DO I=1,ASSF(NASSLIST)%NRASS-1
      ...
      CALL IMOD3D_IPF_FANCY(X,Y,(/Z1,Z2/),IPFPLOT(IIPF)%ISUB,BSIZE*FRAC,(/1,1,1,IPFPLOT(IIPF)%ISHADE/))  !## plot top/bottom/sides all
     ENDDO
    ENDIF
   ENDIF
   IF(LEX)BOT%Z=MIN(BOT%Z,Z2)
  ...
  ELSEIF(IMODE.EQ.2)THEN
   ...
   IF(IPFPLOT(IIPF)%IFANCY.EQ.0)THEN
    ...
    CALL GLVERTEX3F(X,Y,Z1)
    Z1=ASSF(NASSLIST)%Z(ASSF(NASSLIST)%NRASS)
    CALL GLVERTEX3F(X,Y,Z1)
    CALL GLEND()
   ELSEIF(IPFPLOT(IIPF)%IFANCY.EQ.1)THEN
    IF(ASSF(NASSLIST)%NRASS.GE.2)THEN
     CALL IMOD3D_SETCOLOR(NASSLIST)
     Z2=Z1; Z1=ASSF(NASSLIST)%Z(ASSF(NASSLIST)%NRASS)
     CALL IMOD3D_IPF_FANCY(X,Y,(/Z1,Z2/),IPFPLOT(IIPF)%ISUB,BSIZE,(/1,1,1,0/))   !## plot top,sides,bottom
    ENDIF
   ENDIF
...
!   ENDIF
  ...
  ELSE
   ...
   ELSEIF(IPFPLOT(IIPF)%IFANCY.EQ.1)THEN
    IF(IMODE.EQ.1)CALL IMOD3D_IPF_FANCY(X,Y,(/Z,ZZ/),IPFPLOT(IIPF)%ISUB,BSIZE,(/1,1,1,IPFPLOT(IIPF)%ISHADE/))
    IF(IMODE.EQ.2)CALL IMOD3D_IPF_FANCY(X,Y,(/Z,ZZ/),IPFPLOT(IIPF)%ISUB,BSIZE,(/1,1,1,0/))
   ENDIF
  ENDIF
