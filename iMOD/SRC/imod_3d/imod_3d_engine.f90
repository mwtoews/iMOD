...
!    ENDIF
!
!    CALL IMOD3D_IPF_FANCY((/X1,X2/),(/Y1,Y2/),(/Z1,Z2/),IPFPLOT(IIPF)%ISUB,BSIZE*FRAC,(/1,1,1,IPFPLOT(IIPF)%ISHADE/),IPLOTTYPE)
!
!    !## include mid-piece
!    if(I.LE.ASSF(NASSLIST)%NRASS-2)then
!     X3=X+ASSF(NASSLIST)%DX(I+2); Y3=Y+ASSF(NASSLIST)%DY(I+2); Z3=ASSF(NASSLIST)%Z(I+2)
!     Z3=Z3*ZSCALE_FACTOR
!     CALL IMOD3D_TUBE((/X1,X2,X3/),(/Y1,Y2,Y3/),(/Z1,Z2,Z3/),IPFPLOT(IIPF)%ISUB,BSIZE*FRAC) !,.TRUE.,.TRUE.)
!    endif
!
!    IF(LEX)THEN
!     TOP%X=MAX(TOP%X,X1,X2); BOT%X=MIN(BOT%X,X1,X2)
