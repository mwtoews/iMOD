 ...
 IF(IPLOTTYPE.EQ.2.OR.IPLOTTYPE.EQ.4)THEN
  ...
  ELSEIF(IPFPLOT(IIPF)%IFANCY.EQ.1)THEN
   ...
   DO I=1,ASSF(NASSLIST)%NRASS-1
    ...
    !## 1D borehole
    IF(IPLOTTYPE.EQ.2)THEN
     X2=X; Y2=Y; Z2=ASSF(NASSLIST)%Z(I+1)
    ELSEIF(IPLOTTYPE.EQ.4)THEN
     X2=X+ASSF(NASSLIST)%DX(I+1); Y2=Y+ASSF(NASSLIST)%DY(I+1); Z2=ASSF(NASSLIST)%Z(I+1)
    ENDIF
...
!   ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE 
 REAL(KIND=GLFLOAT),INTENT(IN),DIMENSION(2) :: XMID,YMID
 ...
 DO I=1,2
  ...
  CALL GLVERTEX3F(XMID(I),YMID(I),Z(I))
  DO J=1,NINT+1
   XPOS=XMID(I)+COS(FGRAD)*RADIUS
   YPOS=YMID(I)+SIN(FGRAD)*RADIUS 
...
!   YPOS=YMID+SIN(FGRAD)*RADIUS 
 ...
 IF(IPLT(3).EQ.1)THEN
  ...
  FGRAD=0.0_GLFLOAT
  DO J=1,NINT+1
   XPOS=XMID(1)+COS(FGRAD)*RADIUS
   YPOS=YMID(1)+SIN(FGRAD)*RADIUS
!   XPOS=XMID+COS(FGRAD)*RADIUS
!   YPOS=YMID+SIN(FGRAD)*RADIUS
   !## add shades
   ...
   XPOS=XMID(2)+COS(FGRAD)*RADIUS
   YPOS=YMID(2)+SIN(FGRAD)*RADIUS
