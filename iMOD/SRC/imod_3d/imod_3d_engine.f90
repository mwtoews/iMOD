...
! ICOL=IDF(1)%NCOL-1; DO IROW=1,IDF(1)%NROW-1  
 ...
 REAL(KIND=GLFLOAT) :: AR,AD,AX,AY,XGRAD,DXY,DX,DY,DZ,XP,YP,ZP,AX2,AY2,TL
...
SOLLOOP: DO I=1,NSOLLIST
 ...
 !## width scaling
 S=SQRT((TOP%Y-BOT%Y)**2.0+(TOP%X-BOT%X)**2.0)/500.0 !+(TOP%Z-BOT%Z)**2.0)/50.0
 ...
 IF(IPLOTTYPE.EQ.2.OR.IPLOTTYPE.EQ.4)THEN
  ...
  ELSEIF(IPFPLOT(IIPF)%IFANCY.EQ.1)THEN
   ...
   IF(IGL.EQ.1)CALL IMOD3D_TUBE2(XBH,YBH,ZBH,RBH,CBH,IPFPLOT(IIPF)%ISUB,IMODE,IPFPLOT(IIPF)%ISHADE,NASSLIST)
...
! R=0.05 
 ...
 SELECT CASE (ISTYLE)
  CASE (1); CALL GLUQUADRICDRAWSTYLE(QUADOBJ, GLU_FILL)
