...
USE MOD_UTL, ONLY : UTL_INVERSECOLOUR,UTL_CAP,UTL_GETUNIT,ITOS,RTOS,UTL_FILLARRAY,UTL_IDFGETCLASS, &
       ...
       UTL_GETHELP,UTL_DIST,UTL_GETUNIQUE_DINT,YMDHMSTOITIME,UTL_DIALOGSHOW !,UTL_LISTOPENFILES
USE MOD_GENPLOT_PAR
...
!  IF(IPFPLOT(IIPF)%ISEL.EQ.1)NSEL(IIPF)=NSEL(IIPF)+IPFDLIST(1,I)
 ...
 CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:) :: TMPNAME
 INTEGER,ALLOCATABLE,DIMENSION(:) :: TMPINT
 CHARACTER(LEN=15),DIMENSION(10) :: SCLNAMES=['BOUNDARY       ','ARITHMETIC MEAN','GEOMETRIC MEAN ', &
 ...
 IF(IOPTION.EQ.0)THEN
  ...
  IDFDATA(3)=10; CALL WDIALOGPUTMENU(IDF_MENU2,SCLNAMES,10,IDFDATA(3))
...
!   NXYZCROSS(II)=2
!   XYZCROSS(1,II)%X=347257.829
!   XYZCROSS(1,II)%Y=234694.608
!   XYZCROSS(2,II)%X=359648.665
!   XYZCROSS(2,II)%Y=233288.263
 ...
 K=0; DO I=1,SHP%NPOL
  SELECT CASE (SHP%POL(I)%ITYPE)
   ...
   CASE (ID_POLYGON)
    ...
    ENDDO
    NXYZCROSS(K)=NXYZCROSS(K)+1
    XYZCROSS(J+1,K)%X=SHP%POL(I)%X(1)
    XYZCROSS(J+1,K)%Y=SHP%POL(I)%Y(1)
   CASE (ID_RECTANGLE)
    K=K+1
    NXYZCROSS(K)=4
