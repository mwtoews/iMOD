...
USE IMODVAR, ONLY : DP_KIND,SP_KIND,IBACKSLASH,ILABELNAME,PI
USE MOD_IDF
...
USE MOD_UTL, ONLY : UTL_INVERSECOLOUR,UTL_CAP,UTL_GETUNIT,ITOS,RTOS,UTL_FILLARRAY,UTL_IDFGETCLASS, &
       ...
       UTL_GETHELP,UTL_DIST,UTL_GETUNIQUE_DINT,YMDHMSTOITIME
USE MOD_GENPLOT_PAR
...
CONTAINS
 ...
 DO I=1,NXYZWELL
  ...
  N=N-1
  !## sort data
  IF(N.GT.0)CALL WSORT(Z,1,N,IFLAGS=SORTDESCEND,IORDER=IORDER)
  WRITE(JU,'(I10)') N
  WRITE(JU,'(A)') '2,2'
  WRITE(JU,'(A)') 'Z,-999.99'
...
!  IF(IPFPLOT(IIPF)%ISEL.EQ.1)NSEL(IIPF)=NSEL(IIPF)+IPFDLIST(1,I)
 ...
 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  ...
  !## if idftype copntains a time-dependent entry - popup time-selection window
  DO I=1,NIDFLIST; SELECT CASE (IDFPLOT(I)%ICUBE); CASE (6:8); CALL IMOD3D_SETTINGS_IDF_GETTIMES(); END SELECT; ENDDO
  CALL WINDOWSELECT(IWIN3D); IMOD3D_SETTINGS_IDF=.TRUE.
 ENDIF
 
 CALL WDIALOGSELECT(ID_D3DIDFSETTINGS); CALL WDIALOGUNLOAD(); CALL WDIALOGSELECT(ID_D3DSETTINGS_TAB1)
 
 END FUNCTION IMOD3D_SETTINGS_IDF

 !###======================================================================
 SUBROUTINE IMOD3D_SETTINGS_IDF_GETTIMES()
 !###======================================================================
 IMPLICIT NONE
 ...
 N=0; DO I=1,NIDFLIST; SELECT CASE (IDFPLOT(I)%ICUBE); CASE (6:8); MP(IDFPLOT(I)%IPLOT)%ISEL=1; N=N+1; END SELECT; ENDDO
 
!   II=3; ID_IDF(II)=IDFPLOT(I)%IDFLEGEND 

 !## get number of dates
 ALLOCATE(NFILES(N)); NFILES=0
 LEX=IDFTIMESERIE_DATES(1); MP%ISEL=BUISEL; DEALLOCATE(BUISEL); IF(.NOT.LEX)RETURN
 ...
 DO I=1,NIDFLIST
  SELECT CASE (IDFPLOT(I)%ICUBE)
   CASE (6:8)
    ALLOCATE(IDFPLOT(I)%TFILES(N)); IDFPLOT(I)%TFILES=''
  END SELECT
 ENDDO
 ...
 END SUBROUTINE IMOD3D_SETTINGS_IDF_GETTIMES
...
!###======================================================================
  ...
  IF(II.GT.JJ.AND.SUM(ND_IDF).EQ.3)THEN
   ...
   IF(IDFPLOT(I)%ICUBE.EQ.0)THEN
    ...
    IF(IDFLISTINDEX(ILST)%INDEX(1).NE.0)THEN
     ...
     CALL GLDELETELISTS(IDFLISTINDEX(ILST)%INDEX(1),N) !1_GLSIZEI)
    ENDIF
   ...
   ELSEIF(IDFPLOT(I)%ICUBE.EQ.1.OR.IDFPLOT(I)%ICUBE.EQ.6)THEN
    IF(ID_IDF(1).EQ.ID_IDF(2))THEN
     ...
     CALL IMOD3D_DRAWIDF_STRIP(ILST,LLST,IDFPLOT(I)%ICUBE)
    ELSE
     !## create 3D Solid-LAYER for current idf
     CALL IMOD3D_DRAWIDF_STRIP_DUO(ILST,LLST,IDFPLOT(LLST)%LEG,IDFPLOT(I)%ICUBE)
    ENDIF
 ...
 !###======================================================================
 SUBROUTINE IMOD3D_DRAWIDF_STRIP(ILST,LLST,ICUBE)
 !###======================================================================
 IMPLICIT NONE
...
! IDFLISTINDEX(ILST)%INDEX(1)=GLGENLISTS(1); CALL GLNEWLIST(IDFLISTINDEX(ILST)%INDEX(1),GL_COMPILE)
...
!10 continue
 ...
 !###======================================================================
 SUBROUTINE IMOD3D_DRAWIDF_STRIP_DUO(ILST,LLST,LEG,ICUBE)
 !###======================================================================
 IMPLICIT NONE
...
! CALL GLENABLE(GL_COLOR_MATERIAL)
 ...
 IF(IDFPLOT(ILST)%ILEG.EQ.2)THEN
  DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
   ...
   IF(ICUBE.EQ.3)THEN; IF(ICLR.EQ.WRGB(255,255,255))IDF(1)%X(ICOL,IROW)=IDF(1)%NODATA; ENDIF
  ENDDO; ENDDO
 ENDIF
...
SOLLOOP: DO I=1,NSOLLIST
 ...
 IF(IPLOTTYPE.EQ.2.OR.IPLOTTYPE.EQ.4)THEN
  ...
  DO I=1,N
   ...
   IF(IPLOTTYPE.EQ.2)THEN
    ...
    IF(IGL.EQ.0)THEN
     ...
     ELSE
      ZBH(I)=ASSF(NASSLIST)%Z(ASSF(NASSLIST)%NRASS)
     ENDIF
    ELSE
     ZBH(I)=ASSF(NASSLIST)%Z(I)
    ENDIF
    ...
    DO J=1,IPFPLOT(IIPF)%NQUERY
     IF(ASSF(NASSLIST)%ITOPIC.EQ.2)THEN
      ...
      IF(IPFPLOT(IIPF)%QUERY(J)%IFIELD.EQ.1)THEN
       WRITE(LBH(I),*) ASSF(NASSLIST)%Z(I)
      ELSE
       K=IPFPLOT(IIPF)%QUERY(J)%IFIELD-1
       LBH(I)=UTL_CAP(ASSF(NASSLIST)%L(K,I),'U')
      ENDIF
     ELSEIF(ASSF(NASSLIST)%ITOPIC.EQ.4)THEN      
      !## read dx-coordinate
      IF(IPFPLOT(IIPF)%QUERY(J)%IFIELD.EQ.1)THEN
       WRITE(LBH(I),*) ASSF(NASSLIST)%DX(I)
      ELSEIF(IPFPLOT(IIPF)%QUERY(J)%IFIELD.EQ.2)THEN
       WRITE(LBH(I),*) ASSF(NASSLIST)%DY(I)
  ...
  IF(MAXVAL(CBH(1:N)).GE.0)THEN
   ...
   !## apply scaling
   IF(IPFPLOT(IIPF)%IFANCY.EQ.1)ZBH=ZBH*ZSCALE_FACTOR
...
!## eigenlijk deze twee keer aanroepen voor imode=1 en imode=2
     IF(IGL.EQ.1)CALL IMOD3D_TUBE(N,IPFPLOT(IIPF)%ISUB,IMM,IPFPLOT(IIPF)%ISHADE, &
                                 NASSLIST,ZTOL)
    ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE 
 REAL(KIND=GLDOUBLE),INTENT(IN),DIMENSION(2) :: XMID,YMID,ZMID
 ...
 DGRAD=2.0D0*PI_OPENGL/REAL(NINT,8) !## stepsize angle radials
 ...
 DO
  ...
  ELSE
   ...
   IF(ITYPE.EQ.ID_POLYGON)THEN
    ...
    !## draw polygon
    CALL GLBEGIN(GL_LINES) !GL_POLYGON)
     DO I=1,NX
...
! NSPF=NSOLLIST
 ...
 IF(.NOT.UTL_WSELECTFILE('Load iMOD Solid Profile File (*.spf)|*.spf|',      &
                   LOADDIALOG+MUSTEXIST+PROMPTON+DIRCHANGE+MULTIFILE,FNAME,&
                   'Load iMOD Solid Profile File (*.spf)'))RETURN
