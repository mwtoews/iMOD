...
USE MOD_UTL, ONLY : INVERSECOLOUR,UTL_CAP,UTL_GETUNIT,ITOS,RTOS,UTL_FILLARRAY,UTL_IDFGETCLASS, &
       ...
       UTL_GETCURRENTDATE,UTL_GETCURRENTTIME,PEUCKER_SIMPLIFYLINE,UTL_SUBST,UTL_WSELECTFILE
USE MOD_GENPLOT_PAR
...
!  CALL GLPOPMATRIX()
 ...
 DO I=1,NIDFLIST
  ...
  IF(IDFPLOT(I)%ILEG.EQ.1)THEN
   ...
   CALL GLBEGIN(GL_LINES)
    CALL GLVERTEX3F(X   ,Y   ,0.0_GLFLOAT); CALL GLVERTEX3F(X+DX,Y   ,0.0_GLFLOAT)
    CALL GLVERTEX3F(X+DX,Y   ,0.0_GLFLOAT); CALL GLVERTEX3F(X+DX,Y-DY,0.0_GLFLOAT)
    CALL GLVERTEX3F(X+DX,Y-DY,0.0_GLFLOAT); CALL GLVERTEX3F(X   ,Y-DY,0.0_GLFLOAT)
    CALL GLVERTEX3F(X   ,Y-DY,0.0_GLFLOAT); CALL GLVERTEX3F(X   ,Y   ,0.0_GLFLOAT)
   CALL GLEND()
  ...
  IF(IGL.EQ.1)THEN
   ...
   ELSE
    ...
    IF(IPFPLOT(IIPF)%IFANCY.EQ.0)THEN
     ...
     CALL GLBEGIN(GL_LINES); CALL GLVERTEX3F(X,Y,Z); CALL GLVERTEX3F(X,Y,ZZ); CALL GLEND()
...
!                    (/1,1,1,0/))
 ...
 REAL(KIND=GLDOUBLE),PARAMETER :: TS= 10.0 !0.025 !## textsize
...
!     XCOR(2)=X(I+1); YCOR(2)=Y(I+1)
     CALL GLVERTEX3F(X(I)  ,Y(I),0.0_GLFLOAT)
     CALL GLVERTEX3F(X(I+1),Y(I+1),0.0_GLFLOAT)
!     CALL IMOD3D_LINE(XCOR,YCOR,(/0.0_GLFLOAT,0.0_GLFLOAT/))
    ENDIF
   ENDDO
   CALL GLEND()
...
! CALL SOLIDDEALLOCATESLD()
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=GLFLOAT) :: X1,X2,Y1,Y2,XT1,XT2,YT1,YT2,BZ1,BZ2,BX1,BX2,BDX,BDY,DXX,DYY,DXY,TDX
 INTEGER :: I,J
...
!  CALL GLNEWLIST(SOLLISTINDEX(NSOLLIST,3),GL_COMPILE)
  ...
  IF(BDX.LE.0.0.OR.BDY.GE.0.0)THEN
