...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE GRAPH_INIT(LMULT,LEXPORT,DIR)  
 !###======================================================================
 IMPLICIT NONE
 ...
 INTEGER :: NG,I,IXDLU,IYDLU,IXPIX,IYPIX,NGRAPHS,IGRAPH
 ...
 IF(.NOT.LMULTIT)THEN
  ...
  ENDDO
 ELSE
  DY=1.0D0/DBLE(NGRAPHS+1)
  DO IGRAPH=1,NGRAPHS
   GRAPHAREA(1,IGRAPH)=0.0D0; GRAPHAREA(2,IGRAPH)=1.0D0-DBLE(  IGRAPH)*DY
   GRAPHAREA(3,IGRAPH)=1.0D0; GRAPHAREA(4,IGRAPH)=1.0D0-DBLE(IGRAPH-1)*DY
  ENDDO
 ENDIF
...
!  !###======================================================================
! SUBROUTINE GRAPH_PLOT(LMULT,LEXPORT,DIR)  
! !###======================================================================
! IMPLICIT NONE
! LOGICAL,INTENT(IN),OPTIONAL :: LEXPORT,LMULT
! CHARACTER(LEN=*),INTENT(IN),OPTIONAL :: DIR
...
! INTEGER :: ITYPE,NG,IGBITMAP,I,IXDLU,IYDLU,IXPIX,IYPIX,NGRAPHS,IGRAPH
...
!  ENDDO
! ELSE
!  DY=1.0/DBLE(NGRAPHS+1)
!  DO IGRAPH=1,NGRAPHS
!   GRAPHAREA(1,IGRAPH)=0.0D0; GRAPHAREA(2,IGRAPH)=1.0D0-DBLE(  IGRAPH)*DY
!   GRAPHAREA(3,IGRAPH)=1.0D0; GRAPHAREA(4,IGRAPH)=1.0D0-DBLE(1-IGRAPH)*DY
!  ENDDO
! ENDIF
! 
! !## set y-as to fit all available entries
! CALL GRAPH_DRAW(IGBITMAP,1,NGRAPHS,.TRUE.,LEXPORTIT,LMULTIT)
! 
! IF(.NOT.LEXPORTIT)THEN
!
...
!     SELECT CASE (MESSAGE%VALUE1)
!      CASE (ID_ZOOMIN,ID_ZOOMOUT,ID_ZOOMFULL,ID_ZOOMBOX,ID_MOVE)
!       CALL GRAPH_ZOOM(ID_DSCENTOOL_FIGURE,MESSAGE%VALUE1,IGBITMAP,NG,LMULTIT) 
!      CASE (ID_COPY)
!       CALL WCLIPBOARDPUTBITMAP(IGBITMAP)
!      CASE (IDHELP)
!        CALL UTL_GETHELP('5.9.2','TMO.PT.Start')              
...
!       !## redraw image at current zoom-settings
!       CALL GRAPH_DRAW(IGBITMAP,NG,NG,.FALSE.,LEXPORTIT,LMULTIT) 
!     END SELECT
!    CASE (RESIZE,EXPOSE)
!     !## refresh graph
...
! END SUBROUTINE GRAPH_PLOT
 ...
 IF(LINI)THEN
  ...
  DO IGRAPH=1,SIZE(GRAPHDIM)
   PGXMIN(IGRAPH)=AXES%XMIN; PGXMAX(IGRAPH)=AXES%XMAX
   PGYMIN(IGRAPH)=AXES%YMIN; PGYMAX(IGRAPH)=AXES%YMAX
  ENDDO
