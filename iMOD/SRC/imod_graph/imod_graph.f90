...
! !## if plot displays a "static" image with predefined axes, do not apply zoom functionality
! IF(ASSOCIATED(GRAPHDIM%XTXT))THEN
!  CALL WDIALOGFIELDSTATE(ID_ZOOMIN,3)
!  CALL WDIALOGFIELDSTATE(ID_ZOOMOUT,3)
!  CALL WDIALOGFIELDSTATE(ID_ZOOMFULL,3)
!  CALL WDIALOGFIELDSTATE(ID_ZOOMBOX,3)
!  CALL WDIALOGFIELDSTATE(ID_MOVE,3)
! ENDIF
   ...
   CASE (FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_MENU1)
      ...
      CALL GRAPH_DRAW(IBITMAP,NG,NG,XTXT,YTXT,LDATE,.FALSE.) !.TRUE.)
    END SELECT
   CASE (RESIZE,EXPOSE)
    !## refresh graph
 ...
 INTEGER :: IW,IH,I,J,NP,IDD,IDF,NG
...
!## size of array aanpassen ...
  ALLOCATE(AXES%XTXT(INT(AXES%XINT)))
  AXES%XTXT=GRAPHDIM%XTXT
  ALLOCATE(AXES%XPOS(INT(AXES%XINT)))
 ...
 ELSE
  ...
  ALLOCATE(AXES%YPOS(INT(AXES%YINT)))
 ...
 ELSE
  ...
  ALLOCATE(AXES%Y2POS(INT(AXES%Y2INT)))
...
! CALL IGRJOIN(AXES%XMIN,AXES%YMIN,AXES%XMIN,AXES%YMAX)
 CALL IGRLINEWIDTH(1)
