...
CONTAINS
 ...
 CALL GRAPH_DRAW(1,NGRAPHS,.TRUE.) !,LEXPORTIT,LMULTIT)
 ...
 ELSE
  ...
  DO NG=1,SIZE(GRAPHDIM%GRAPHNAMES)
   ...
   CALL GRAPH_DRAW(NG,NG,.FALSE.)!,LEXPORTIT,LMULTIT) 
 ...
 NG=SIZE(GRAPHDIM%GRAPHNAMES); IF(LMULTIT)NG=1
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    CASE (ID_ZOOMIN,ID_ZOOMOUT,ID_ZOOMFULL,ID_ZOOMBOX,ID_MOVE)
     IF(NG.GT.1)CALL WDIALOGGETMENU(IDF_MENU1,NG)
     CALL GRAPH_ZOOM(ID_DSCENTOOL_FIGURE,MESSAGE%VALUE1,NG) !,LMULTIT) 
    CASE (ID_COPY)
     CALL WCLIPBOARDPUTBITMAP(IGBITMAP)
    CASE (IDHELP)
      CALL UTL_GETHELP('5.9.2','TMO.PT.Start')              
    CASE (IDCANCEL)
     CALL GRAPH_CLOSE(); IF(PRESENT(IEXIT))IEXIT=1 !EXIT
   END SELECT
  CASE (FIELDCHANGED)
...
! END SUBROUTINE GRAPH_PLOT
 ...
 !###======================================================================
 SUBROUTINE GRAPH_DRAW(IG,JG,LINI)!,LEXPORTIT,LMULTIT)  
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IG,JG
 LOGICAL,INTENT(IN) :: LINI !,LEXPORTIT,LMULTIT
 ...
 !###====================================================================
 SUBROUTINE GRAPH_ZOOM(IDD,IDZ,NG) !,LMULTIT) 
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NG
 INTEGER,INTENT(IN) :: IDZ,IDD
! LOGICAL,INTENT(IN) :: LMULTIT
 ...
 ELSE  
  CALL GRAPH_DRAW(NG,NG,.TRUE.) !.FALSE.) !,LMULTIT) 
  RETURN
 ENDIF
    ...
    CASE(MOUSEMOVE)
     ...
     IF(IDZ.EQ.ID_MOVE)THEN
      IF(IDOWN.GT.0)THEN
       ...
       PGYMAX(:)=PGYMAX(:)+DY
       PGYMIN(:)=PGYMIN(:)+DY
       CALL GRAPH_DRAW(NG,NG,.FALSE.) !,.FALSE.) !,LMULTIT) 
      ENDIF
     ELSEIF(IDZ.EQ.ID_ZOOMBOX)THEN
