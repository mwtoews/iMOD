...
CONTAINS
 ...
 INTEGER :: NG,I,IXDLU,IYDLU,IXPIX,IYPIX,NGRAPHS

 LEXPORTIT=.FALSE.; IF(PRESENT(LEXPORT))LEXPORTIT=LEXPORT
 LMULTIT=  .FALSE.; IF(PRESENT(LMULT))  LMULTIT=LMULT

 IGBITMAP=0
 
 CALL WDIALOGLOAD(ID_DSCENTOOL_FIGURE,ID_DSCENTOOL_FIGURE)
 IF(LMULTIT)CALL WDIALOGPUTCHECKBOX(IDF_CHECK1,1)
 ...
 IF(SIZE(GRAPH,2).GT.1)THEN
  ...
  !## type of graph
  IF(NGRAPHS.GT.1.AND..NOT.LMULTIT)THEN 
   CALL WDIALOGPUTMENU(IDF_MENU1,GRAPHDIM%GRAPHNAMES,NGRAPHS,1)
 ...
 IF(ALLOCATED(PGXMIN))DEALLOCATE(PGXMIN)
 IF(ALLOCATED(PGXMAX))DEALLOCATE(PGXMAX)
 IF(ALLOCATED(PGYMIN))DEALLOCATE(PGYMIN)
 IF(ALLOCATED(PGYMAX))DEALLOCATE(PGYMAX)
 ALLOCATE(GRAPHUNITS(6,NGRAPHS),GRAPHAREA(4,NGRAPHS))
 ALLOCATE(PGXMIN(NGRAPHS),PGXMAX(NGRAPHS),PGYMIN(NGRAPHS),PGYMAX(NGRAPHS))
 
 CALL GRAPH_INIT_AREAS()
 ...
 IMULT=WINFODIALOGFIELD(IDF_MENU1,FIELDSTATE)
 IF(IMULT.EQ.3)THEN; IG1=1; IG2=SIZE(GRAPHDIM%GRAPHNAMES); ENDIF
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    CASE (ID_ZOOMIN,ID_ZOOMOUT,ID_ZOOMFULL,ID_ZOOMBOX,ID_MOVE)
     IF(IMULT.EQ.1)THEN; CALL WDIALOGGETMENU(IDF_MENU1,IG1); IG2=IG1; ENDIF
     CALL GRAPH_ZOOM(ID_DSCENTOOL_FIGURE,MESSAGE%VALUE1,IG1,IG2) 
    CASE (ID_COPY)
  ...
  CASE (FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
    CASE (IDF_CHECK1)
     LMULTIT=.FALSE.; CALL WDIALOGGETCHECKBOX(IDF_CHECK1,I); IF(I.EQ.1)LMULTIT=.TRUE.
     CALL WDIALOGFIELDSTATE(IDF_MENU1,ABS(I-1))
     CALL GRAPH_INIT_AREAS()  
    CASE (IDF_MENU1)
     CALL WDIALOGSELECT(ID_DSCENTOOL_FIGURE)
 ...
 IF(MESSAGE%WIN.EQ.ID_DSCENTOOL_FIGURE)THEN
  ...
  IF(ASSOCIATED(GRAPHDIM(1)%XTXT))THEN
   ...
   IF(I.GE.1.AND.I.LE.SIZE(GRAPHDIM(1)%XTXT))THEN
    CALL WDIALOGPUTSTRING(IDF_LABEL1,'Date='//TRIM(GRAPHDIM(1)%XTXT(I))//'; Ycrd='//TRIM(RTOS(DBLE(MESSAGE%GY),'F',2)))
   ELSE
    CALL WDIALOGPUTSTRING(IDF_LABEL1,'Date=[unknown]; Ycrd='//TRIM(RTOS(DBLE(MESSAGE%GY),'F',2)))
   ENDIF
  ELSE
   IF(GRAPHDIM(1)%LDATE)THEN
    CALL WDIALOGPUTSTRING(IDF_LABEL1,'Date='//TRIM(JDATETOGDATE(INT(MESSAGE%GX)))//'; Ycrd='//TRIM(RTOS(DBLE(MESSAGE%GY),'F',2)))
   ELSE
    CALL WDIALOGPUTSTRING(IDF_LABEL1,'Xcrd='//TRIM(RTOS(REAL(MESSAGE%GX,8),'F',2))//'; Ycrd='//TRIM(RTOS(DBLE(MESSAGE%GY),'F',2)))
   ENDIF
  ENDIF
 ELSE
 ...
 DO IGRAPH=IG,JG
  ...
  AXES%CHS=5.0D0; IF(LMULTIT)AXES%CHS=AXES%CHS*SIZE(GRAPHDIM)
 ...
 !###====================================================================
 SUBROUTINE GRAPH_ZOOM(IDD,IDZ,IG1,IG2)  
 !###====================================================================
 IMPLICIT NONE
     ...
     IF(IDZ.EQ.ID_ZOOMIN.OR.IDZ.EQ.ID_ZOOMOUT)THEN
      SELECT CASE (MESSAGE%VALUE1)
       CASE (1)
        XC2 = XC3
        YC2 = YC3
        DX  = PGXMAX(1)-PGXMIN(1)
        DY  = PGYMAX(1)-PGYMIN(1)
        PGXMAX(:)= XC2+0.5D0*DX*FZ
        PGXMIN(:)= XC2-0.5D0*DX*FZ
        PGYMIN(:)= YC2-0.5D0*DY*FZ
        PGYMAX(:)= YC2+0.5D0*DY*FZ
        CALL GRAPH_DRAW(IG1,IG2,.FALSE.)
       CASE (3)
        EXIT
      END SELECT
