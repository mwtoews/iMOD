...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE GRAPH_INIT(WMODE,LMULT,LEXPORT,DIR,GRAPHTXT,SUBTITLE,IBATCH)  
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: WMODE
 INTEGER,INTENT(IN),OPTIONAL :: IBATCH
 LOGICAL,INTENT(IN),OPTIONAL :: LEXPORT,LMULT
 CHARACTER(LEN=*),INTENT(IN),OPTIONAL :: DIR,GRAPHTXT,SUBTITLE
 ...
 LMESSAGE =.FALSE.; IF(PRESENT(IBATCH))THEN; IF(IBATCH.EQ.0)LMESSAGE=.TRUE.; ENDIF
...
#endif
 ENDIF
 CALL WDIALOGTITLE('Graph') ; IF(PRESENT(GRAPHTXT))CALL WDIALOGTITLE(TRIM(GRAPHTXT)) 
 ...
 CALL WDIALOGPUTSTRING(IDF_LABEL3,'') ; IF(PRESENT(SUBTITLE)) CALL WDIALOGPUTSTRING(IDF_LABEL3,TRIM(SUBTITLE))
   ...
   IF(I.NE.0)THEN 
    IF(LMESSAGE)THEN
     CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot SAVE the requested bitmap file called:'//CHAR(13)//&
      TRIM(PNGNAME),'Error')
    ELSE
     WRITE(*,'(/A/)') 'iMOD cannot SAVE the requested bitmap file called: '//TRIM(PNGNAME)
    ENDIF
   ELSE
    IF(NG.EQ.SIZE(GRAPHDIM%GRAPHNAMES))THEN
     ...
     ELSE
      WRITE(*,'(/A/)') 'iMOD SAVED (all) the requested bitmaps in the folder: '//TRIM(DIR)//'\*.PNG'
     ENDIF
    ENDIF
   ENDIF
