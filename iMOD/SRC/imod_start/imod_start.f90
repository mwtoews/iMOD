...
USE MOD_UTL, ONLY : UTL_DIRINFO,UTL_GETUNIT,UTL_DIRINFO_POINTER
...
CHARACTER(LEN=256),POINTER,DIMENSION(:),PRIVATE :: LISTNAME
...
CONTAINS
 ...
 SELECT CASE (EXT)
  CASE ('IMF','QPF')
  CASE DEFAULT
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot initiate start-dialog','Error')
   IDEXIT=0; RETURN
 END SELECT

 CALL START_INIT(EXT)
 CALL START_FILLMENU(EXT)
   ...
   CASE (FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_STRING1,IDF_RADIO3,IDF_RADIO4,IDF_RADIO5)
      IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)CALL START_FILLMENU(EXT)
...
!       CALL START_FIELDS()
 ...
 J=0; IF(ASSOCIATED(LISTNAME))J=I*1
 ...
 LEX=UTL_DIRINFO_POINTER(DIR,WC,LISTNAME,'F',CORDER=CORDER(IRADIO))
 IF(SIZE(LISTNAME).LE.0)THEN
  IF(ASSOCIATED(LISTNAME))DEALLOCATE(LISTNAME)
  LEX=.FALSE.
 ENDIF
 ...
 IF(LEX)THEN
  ...
  CALL WDIALOGPUTMENU(IDF_MENU1,LISTNAME,SIZE(LISTNAME),1)
