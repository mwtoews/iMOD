...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: LAT,LONG,XUTM,YUTM,lat1,long1
 REAL(KIND=DP_KIND) :: DEG,MIN,SEC
 INTEGER :: IZONE
 CHARACTER(LEN=1) :: LBAND
 
 DEG =40.0D0;MIN=30.0D0; SEC=0.0D0
 LAT =DEG+MIN/60.0D0+SEC/3600.0D0
 DEG =-73.0;MIN=30.0D0; SEC=00.0D0
 LONG=DEG+MIN/60.0D0+SEC/3600.0D0
 LAT1=LAT
 LONG1=LONG
 ...
 DO I=1,NUTM
  ...
  UTM(I)%ALPHA(1)=(( 1.0D0/  2.0D0)*N)      - ((2.0D0/ 3.0D0)*N**2.0D0) + (( 5.0D0/16.0D0)*N**3.0)
  UTM(I)%ALPHA(2)=((13.0D0/ 48.0D0)*N**2.0D0) - ((3.0D0/ 5.0D0)*N**3.0)
  UTM(I)%ALPHA(3)=((61.0D0/240.0D0)*N**3.0)
  UTM(I)%BETA(1) =(( 1.0D0/  2.0D0)*N)      - ((2.0D0/ 3.0D0)*N**2.0D0) + ((37.0D0/96.0D0)*N**3.0)
  UTM(I)%BETA(2) =(( 1.0D0/ 48.0D0)*N**2.0D0) + ((1.0D0/15.0D0)*N**3.0)
  UTM(I)%BETA(3) =((17.0D0/480.0D0)*N**3.0)
  UTM(I)%DELTA(1)=   2.0D0         *N       - ((2.0D0/ 3.0D0)*N**2.0D0) - (  2.0D0        *N**3.0)
  UTM(I)%DELTA(2)=(( 7.0D0/  3.0D0)*N**2.0D0) - ((8.0D0/ 5.0D0)*N**3.0)
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: LONG_DEG  
 INTEGER,INTENT(OUT) :: IZONE
 REAL(KIND=DP_KIND),INTENT(OUT) :: LONG0
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: LAT
 INTEGER :: IBAND
 ...
 INTEGER,INTENT(OUT) :: IZONE
 CHARACTER(LEN=1),INTENT(OUT) :: LBAND
 REAL(KIND=DP_KIND),INTENT(IN) :: LAT_DEG,LONG_DEG !## in degrees
 REAL(KIND=DP_KIND),INTENT(OUT) :: XUTM,YUTM
 REAL(KIND=DP_KIND) :: N,T,EPS,NU,D,LONG0,LAT,LONG,LONG0_DEG
 INTEGER :: I
 ...
 NU =ATANH(SIN(LONG-LONG0)/SQRT(1.0D0+T**2.0D0))
 ...
 CHARACTER(LEN=1),INTENT(IN) :: LBAND
 REAL(KIND=DP_KIND),INTENT(OUT) :: LAT_DEG,LONG_DEG !## in degrees
 REAL(KIND=DP_KIND),INTENT(IN) :: XUTM,YUTM
 REAL(KIND=DP_KIND) :: EPS,NU,NUQ,D,EPSQ,CHI,LAT,LONG,LONG0
 INTEGER :: I
 ...
 REAL(KIND=DP_KIND) :: XUTM,YUTM,PHIBES,LAMBES,PHIWGS,LAMWGS
...
!  LONG=PHIWGS
 ...
 !###====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: X0=1.55E5
 REAL(KIND=DP_KIND),PARAMETER :: Y0=4.63E5
 REAL(KIND=DP_KIND),PARAMETER :: K=0.9999079
 REAL(KIND=DP_KIND),PARAMETER :: BIGR=6382644.571
 REAL(KIND=DP_KIND),PARAMETER :: M=0.03773953832D0
 REAL(KIND=DP_KIND),PARAMETER :: N=1.0047585668D0
 REAL(KIND=DP_KIND),PARAMETER :: E=0.08169683122D0
 REAL(KIND=DP_KIND),INTENT(IN) :: X,Y
 REAL(KIND=DP_KIND),INTENT(OUT) :: PHIBES,LAMBES
 REAL(KIND=DP_KIND) :: D__1,D__2
 REAL(KIND=DP_KIND) :: CPSI,SPSI,PHIPRIME,B
 INTEGER :: I
 REAL(KIND=DP_KIND) :: Q,R,W,CA,CB,DL,SA,SB,LAMBDA0
 REAL(KIND=DP_KIND) :: DQ,SDL,PSI,B0

 !## convert XY to Bessel
 !## input is x,y in RD output is phi,PHIBES on the Bessel ellipsoid
! PI=ATAN(1.0D0)*4
 LAMBDA0=PI*0.0D029931327161111111
 ...
 !###====================================================================
 REAL(KIND=DP_KIND),INTENT(IN) :: PHIBES,LAMBES
 REAL(KIND=DP_KIND),INTENT(OUT) :: PHIWGS,LAMWGS
 REAL(KIND=DP_KIND) :: DLAM,DPHI,LAMCOR,PHICOR
 REAL(KIND=DP_KIND),PARAMETER :: A=52.0
 REAL(KIND=DP_KIND),PARAMETER :: B=5.0
 REAL(KIND=DP_KIND),PARAMETER :: C=-96.862
 REAL(KIND=DP_KIND),PARAMETER :: D=11.714
 REAL(KIND=DP_KIND),PARAMETER :: E=0.125
 REAL(KIND=DP_KIND),PARAMETER :: F=1.0E-5
 REAL(KIND=DP_KIND),PARAMETER :: G=0.329
 REAL(KIND=DP_KIND),PARAMETER :: H=37.902
 REAL(KIND=DP_KIND),PARAMETER :: I=14.667
