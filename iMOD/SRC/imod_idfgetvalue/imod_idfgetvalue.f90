...
USE MOD_UTL, ONLY : ITOS,UTL_PLOTLOCATIONIDF,UTL_GETRELEVANTDIR,INVERSECOLOUR,IDFPLOT1BITMAP,IDFPLOT2BITMAP
...
CONTAINS
 ...
 INTEGER :: ITYPE,I,J,IPLOT,IROW,ICOL,KSHAPE,IDOWN, &
            IR1,IR2,IC1,IC2,NPOL,N,MAXNIDF,IOPT,INODATA !,ISORT
 ...
 ELSEIF(ISHOW.EQ.2)THEN
  DO I=1,NIDFS
   CALL WGRIDCOLOURCELL(IDF_GRID1,1,I,-1,ICOLORIDF(I))!WRGB(255,255,255))
  END DO
 ENDIF
...
! CALL WDIALOGGETCHECKBOX(IDF_CHECK2,ISORT)
...
!  CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,I,TRIM(IDFTRANSFORM(IUNITS(I)+1)))
!  CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,I,TRIM(IDFTRANSFORM(IDF(I)%UNITS+1)))
 END DO
    ...
    SELECT CASE (KSHAPE)
       ...
       DO I=1,NIDFS
        ...
        DO IROW=MAX(1,IR1),MIN(IR2,IDF(I)%NROW)
         DO ICOL=MAX(1,IC1),MIN(IC2,IDF(I)%NCOL)
          ...
          IF(IGRINSIDEPOLYGON(XPOL,YPOL,NPOL+1,XCHECK,YCHECK))THEN
           IDFVAL=IDFGETVAL(IDF(I),IROW,ICOL,IUNITS(I))
           IF(IDFVAL.NE.IDF(I)%NODATA)THEN
            N     =N+1
            IDFSUM=IDFSUM+IDFVAL
     ...
     CASE (ID_POINT)
      ...
      IF(IDOWN.EQ.1)THEN
       ...
       END DO
       CALL WDIALOGSETFIELD(IDF_LABEL11)
!       IF(ISORT.EQ.2)CALL WGRIDSORT(IDF_GRID1,2,IROW1=1)
!       IF(ISORT.EQ.3)CALL WGRIDSORT(IDF_GRID1,2,IROW1=1,IFLAGS=SORTDESCEND)
      ENDIF
    END SELECT
...
!     CASE (IDF_RADIO1,IDF_RADIO2,IDF_RADIO3); CALL WDIALOGGETRADIOBUTTON(IDF_RADIO1,ISORT)
    END SELECT
