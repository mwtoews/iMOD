...
USE MOD_COLOURS
USE IMODVAR, ONLY : PLACES,DECPLACES,IFORM
...
! IF(ISHOW.EQ.2)CALL WDIALOGPUTSTRING(IDF_LABEL2,'Cells of all mentioned IDFs will be plotted.')
 ...
 !## show no box
! IF(ISHOW.EQ.0)THEN
!  CALL WGRIDCOLOURCOLUMN(IDF_GRID1,1,-1,-1) !WRGB(255,255,255))
! !## show first only
...
! IFORM=1
 ...
 CALL WDIALOGRANGEINTEGER(IDF_INTEGER2,0,7)
 ...
 DO
  ...
  SELECT CASE (ITYPE)
     ...
     CASE (IDF_INTEGER2,IDF_MENU1)
      DO I=1,NIDFS
       ...
       IF(IROW.GT.IDF(I)%NROW.OR.IROW.LE.0.OR.ICOL.GT.IDF(I)%NCOL.OR.ICOL.LE.0)THEN
        CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,I,'Outside Domain')
       ELSE
   ...
   CASE (MOUSEMOVE)
    ...
    CALL WDIALOGSELECT(ID_DIDFINFO)
    CALL WDIALOGPUTSTRING(IDF_GROUP1,'Current Location x ='//TRIM(ITOS(INT(MESSAGE%GX)))// &
     ...
     CASE (ID_POINT)
      ...
      IF(IDOWN.EQ.1)THEN
       ...
       DO I=1,NIDFS
        ...
        ENDIF
       END DO
!       CALL WDIALOGSETFIELD(IDF_GROUP1)
      ENDIF
    END SELECT
...
! CALL IDFGETVALUE_FIELDS(0)
 ...
 ELSE
  DO I=1,N 
   IF(IDF(I)%XMIN.LE.X.AND.IDF(I)%XMAX.GE.X.AND. &
    ...
    CALL IGRCOLOURN(INVERSECOLOUR(ICOLORIDF(I))) 
 ...
 REAL,INTENT(IN) :: IDFVALSP,NODATA
 INTEGER :: IOS
 ...
 ELSEIF(IFORM.EQ.2)THEN
  !## integer  
  IF(DECPLACES.EQ.0)THEN
   IDFFORMAT='(I'//TRIM(ITOS(PLACES))//')'
  ELSE 
   IDFFORMAT='(I'//TRIM(ITOS(PLACES))//'.'//TRIM(ITOS(DECPLACES))//')'
  ENDIF
  WRITE(NUMBER,TRIM(IDFFORMAT),IOSTAT=IOS) INT(IDFVALSP)
 ELSEIF(IFORM.EQ.3)THEN
  !## scientific
  IF(DECPLACES.EQ.0)THEN
   DECPLACES=1; CALL WDIALOGPUTINTEGER(IDF_INTEGER2,DECPLACES)
  ENDIF
  IDFFORMAT='(E'//TRIM(ITOS(PLACES))//'.'//TRIM(ITOS(DECPLACES))//')'
  WRITE(NUMBER,TRIM(IDFFORMAT),IOSTAT=IOS) IDFVALSP
 ENDIF
 
 IF(IDFVALSP.EQ.NODATA)NUMBER='NoDataValue'
 IF(IOS.NE.0)NUMBER='NaN'
