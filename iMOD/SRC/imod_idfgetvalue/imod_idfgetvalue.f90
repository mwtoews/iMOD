 ...
 DO I=1,NIDFS
  ...
  SELECT CASE (IOPT)
   ...
   CASE (5)
    SNAMES(I)=ALIAS(I)
  END SELECT
  IF(LEN(TRIM(SNAMES(I))).EQ.0)SNAMES(I)=' file not found '
 ENDDO
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.(MP(IPLOT)%IPLOT.EQ.1.OR.MP(IPLOT)%IPLOT.EQ.5))THEN
   ...
   IF(MP(IPLOT)%IPLOT.EQ.5)THEN
    IF(READMDF(MP(IPLOT)%IDFNAME,N))THEN
     DO I=1,N
      ...
      IF(.NOT.IDFREAD(IDF(NIDFS),MDF(I)%FNAME,0))THEN
        IDF(NIDFS)%FNAME=''
      ELSE 
   ...
   ELSE
    ...
    IF(.NOT.IDFREAD(IDF(NIDFS),MP(IPLOT)%IDFNAME,0))THEN ; IDF(NIDFS)%FNAME=''; ENDIF
    J=J+1
    IF(J.GT.MAXCOLOUR)J=1
    ICOLORIDF(NIDFS)=ICOLOR(J)
