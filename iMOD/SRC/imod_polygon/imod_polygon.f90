...
USE MOD_POLYGON_PAR
USE MOD_UTL, ONLY : ITOS,RTOS,INVERSECOLOUR,UTL_CAP,IDFPLOT1BITMAP,IDFPLOT2BITMAP,UTL_INSIDEPOLYGON,UTL_POLYGON1AREA,VAR,VAR_TMP,NL,NV
    ...
    CASE (ID_SAVESHAPE,ID_LOADSHAPE,ID_SAVE)
     IF(PRESENT(IDAT))THEN
      IF(PRESENT(GENFNAME))THEN
       CALL POLYGON1SAVELOADSHAPE(MESSAGE%VALUE1,MESSAGE%WIN,TRIM(GENFNAME),IDAT=IDAT)
      ELSE
       CALL POLYGON1SAVELOADSHAPE(MESSAGE%VALUE1,MESSAGE%WIN,'',IDAT=IDAT)
      ENDIF
     ELSE
      IF(PRESENT(GENFNAME))THEN
       CALL POLYGON1SAVELOADSHAPE(MESSAGE%VALUE1,MESSAGE%WIN,TRIM(GENFNAME))
      ELSE
       CALL POLYGON1SAVELOADSHAPE(MESSAGE%VALUE1,MESSAGE%WIN,'')
      ENDIF
     ENDIF
...
!     ENDIF
 ...
 DO I=1,SHPNO
  IF(SHPIACT(I).EQ.1)THEN
   N=SHPNCRD(I)
   XMIN=MIN(XMIN,MINVAL(SHPXC(1:N,I)))
   XMAX=MAX(XMAX,MAXVAL(SHPXC(1:N,I)))
   YMIN=MIN(YMIN,MINVAL(SHPYC(1:N,I)))
   YMAX=MAX(YMAX,MAXVAL(SHPYC(1:N,I)))
  ENDIF
 ENDDO
 ...
 IF(WMENUGETSTATE(ID_EXTRACTGEN,2).EQ.1)   ID=ID_DEXTRACT
