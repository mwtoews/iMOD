...
CONTAINS
   ...
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_SAVESHAPE,ID_LOADSHAPE,ID_SAVE)
     ...
     ELSE
      ...
      END SELECT
     ENDIF
     IF(MESSAGE%VALUE1.EQ.ID_LOADSHAPE.AND.SHP%NPOL.GT.0)THEN
 ...
 IF(SHP%POL(INEW)%ITYPE.EQ.ID_GRID)THEN
 ...
 SELECT CASE (SHP%POL(INEW)%ITYPE)
  ...
  CASE (ID_GRID)
   ...
   SHP%POL(INEW)%PNAME='GRID'//TRIM(ITOS(INEW))
...
! IF(IRWIN.GT.0)CALL IR_GETSELECTEDCELLS()
 ...
 DO I=1,SHP%NPOL
  ...
  ELSE
   N=N+1
   IF(N.NE.I)THEN
    IF(ASSOCIATED(SHP%POL(N)%X))DEALLOCATE(SHP%POL(N)%X)
    IF(ASSOCIATED(SHP%POL(N)%Y))DEALLOCATE(SHP%POL(N)%Y)
    ALLOCATE(SHP%POL(N)%X(SHP%POL(I)%N),SHP%POL(N)%Y(SHP%POL(I)%N))
