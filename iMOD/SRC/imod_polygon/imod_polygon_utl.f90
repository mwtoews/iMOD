...
!   CALL WCURSORSHAPE(ID_CURSORLINE)
 ...
 ICOL=0; CALL UTL_DIALOGSHOW(-1,-1,0,3)
 DO
  CALL WMESSAGE(ITYPE,MESSAGE)
  SELECT CASE (ITYPE)
   CASE (FIELDCHANGED)
 ...
 ELSEIF(CODE.EQ.ID_SAVESHAPE)THEN 
  ...
  ELSEIF(FTYPE.EQ.2)THEN
   ...
   DO I=1,SHP%NPOL
    ...
    IF(SHP%POL(I)%ITYPE.EQ.ID_LINE.OR.SHP%POL(I)%ITYPE.EQ.ID_POLYGON)THEN
     ...
     ENDDO
     !## write startpoint as start point again
     IF(SHP%POL(I)%ITYPE.EQ.ID_POLYGON)WRITE(IU,'(2(F15.3,A1))') SHP%POL(I)%X(1),',',SHP%POL(I)%Y(1)
     WRITE(IU,'(A3)') 'END'
  ...
  CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfully saved '//TRIM(FNAME),'Information')
