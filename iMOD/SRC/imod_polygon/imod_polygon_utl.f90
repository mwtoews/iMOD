...
! CHARACTER(LEN=MAXLEN),DIMENSION(:,:),POINTER :: VAR
 LOGICAL :: LEX
...
!   CALL WCURSORSHAPE(ID_CURSORLINE)
 ...
 INTEGER :: IU,I,J,K,N,II,JJ,IOS,STRLEN,FTYPE
 CHARACTER(LEN=256) :: FNAME
 CHARACTER(LEN=52) :: LSTR,FSTR
 ...
 SELECT CASE (CFTYPE)
  CASE ('GEN')
   FSTR='iMOD BINARY GEN Files (*.GEN)|*.GEN|ESRI ASCII GEN file (*.GEN)|*.GEN|'
   LSTR='GEN File'
  CASE ('IPF')
   FSTR='IPF Files (*.IPF)|*.IPF|'
   LSTR='IPF File'
  CASE ('GEN/IPF')
   FSTR='iMOD BINARY GEN Files (*.GEN)|*.GEN|ESRI ASCII GEN file (*.GEN)|*.GEN|IPF Files (*.IPF)|*.IPF|'
   LSTR='GEN/IPF File'
 END SELECT
 ...
 IF(CODE.EQ.ID_LOADSHAPE)THEN
  ...
  IF(GENFNAME.EQ.'')THEN
   IF(.NOT.UTL_WSELECTFILE(FSTR, & !'GEN File (*.gen)|*.gen|GEN Files (*.gen)|*.gen|IPF Files (*.ipf)|*.ipf|',&
        LOADDIALOG+MUSTEXIST+PROMPTON+DIRCHANGE+APPENDEXT,FNAME,'Load '//TRIM(LSTR)))RETURN
  ELSE
   FNAME=GENFNAME
  ENDIF
  ...
  IF(INDEX(FNAME,'.GEN').GT.0)THEN
   ...
   IF(IFORMAT.EQ.1)THEN
    CALL WMESSAGEBOX(YESNO,QUESTIONICON,COMMONNO,'You want to read an old GEN file (ASCII).'//CHAR(13)// &
 ...
 ELSEIF(CODE.EQ.ID_SAVESHAPE)THEN 
  ...
  FTYPE=1
  IF(GENFNAME.EQ.'')THEN
   IF(.NOT.UTL_WSELECTFILE(FSTR, & !'iMOD GEN Files (*.gen)|*.gen|ESRI GEN file (*.gen)|*.gen|IPF Files (*.ipf)|*.ipf|',     &
        SAVEDIALOG+PROMPTON+DIRCHANGE+APPENDEXT,FNAME,'Load '//TRIM(LSTR),FTYPE=FTYPE))RETURN !'Save GEN/IPF File',FTYPE=FTYPE))RETURN
  ELSE
   FNAME=GENFNAME
  ...
  IF(FTYPE.EQ.1)THEN
   ...
   IF(LEX)THEN
    J=0; DO I=1,SHP%NPOL; IF(SHP%POL(I)%IACT.EQ.0)J=J+1; ENDDO
    WRITE(IU) J       ,MAXCOL
   ELSE
  ...
  !## ascii gen file
   ...
   DO I=1,SHP%NPOL
    ...
    IF(SHP%POL(I)%ITYPE.EQ.ID_LINE.OR.SHP%POL(I)%ITYPE.EQ.ID_POLYGON)THEN
     WRITE(IU,'(99A)') (TRIM(STR(J)%STRING)//' ',J=1,MAXCOL)
     DO J=1,SHP%POL(I)%N
