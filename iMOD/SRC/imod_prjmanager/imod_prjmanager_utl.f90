...
CONTAINS
 ...
 INTEGER :: I
 LOGICAL :: LBAS !## is true if all BAS topcis are true
    ...
    CASE(ID_DSIMMANAGER_TAB3)
     ...
     SELECT CASE (ITYPE)
      CASE (FIELDCHANGED)
       IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
        SELECT CASE (MESSAGE%VALUE2)
         CASE (IDF_RADIO1,IDF_RADIO2,IDF_RADIO3,IDF_RADIO4,IDF_RADIO5,IDF_REAL10,IDF_REAL7,IDF_REAL8,IDF_STRING1,IDF_STRING3)
...
!         CALL PMANAGER_INITSIM_FIELDS_TAB5()
    ...
    CASE(ID_DSIMMANAGER_TAB7)
     ...
     !SELECT CASE (ITYPE)
     ! CASE (FIELDCHANGED)
!         CALL PMANAGER_INITSIM_FIELDS_TAB7()
...
! ENDIF
 ...
 SELECT CASE (IWINDOW)
  ...
  CASE (4)
   ...
   PBMAN%GENFNAME='d:\IMOD-MODELS\MF6\DBASE\BAS\TWO_RECT_100_100.GEN'
   IF(TRIM(PBMAN%GENFNAME).NE.'')THEN
    ...
    SUBMODEL(1)=SHP%XMIN; SUBMODEL(2)=SHP%YMIN
    SUBMODEL(3)=SHP%XMAX; SUBMODEL(4)=SHP%YMAX
   ENDIF
 END SELECT
...
! CALL PMANAGER_INITSIM_GETISAVE()
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_INITSIM_FIELDS_TAB7()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ID
 
 ID=WINFODIALOG(CURRENTDIALOG)
 CALL WDIALOGSELECT(ID_DSIMMANAGER_TAB7)
...
! CALL PMANAGER_INITSIM_PUTISAVE()
 ...
 END SUBROUTINE PMANAGER_INITSIM_FIELDS_TAB7
 ...
 !## misc. settings SEAWAT
 CALL WDIALOGSELECT(ID_DSIMMANAGER_TAB7)
 CALL WDIALOGPUTINTEGER(IDF_INTEGER1,PBMAN%NPRS) 
 ! Frans: to be filled in
...
! TOPICS(TSCO)%SNAME(1) ='(SCO) Starting Concentration (IDF)'
 ...
 !## for mf2005
 ALLOCATE(MC(2)%T(33),MC(2)%IACT(33))
 MC(2)%MCNAME='MODFLOW2005'; MC(2)%IACT=1
 MC(2)%T=[TBND,TTOP,TBOT,TKHV,TKVA,TKDW,TKVV,TVCW,TSHD,TSTO,TSPY,TANI,THFB,TIBS,TPWT, &
          TSFT,TCAP,TUZF,TRCH,TEVT,TDRN,TOLF,TRIV,TISG,TSFR,TLAK,TWEL,TMNW,TGHB,TCHD,TFHB,TPST,TPCG]
 !## for modflow6
