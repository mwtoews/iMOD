...
! TOPICS(TBTN)%SNAME(1) ='(BTN) Parameters Basic Transport package (-)'
 ...
 !## for mf2005
 ALLOCATE(MC(1)%T(8)); MC(1)%T=[TBND,TTOP,TBOT,TKHV,TKVV,TDRN,TOLF,TRIV]
 !## for modflow6
 ALLOCATE(MC(2)%T(7)); MC(2)%T=[TBND,TTOP,TBOT,TKHV,TKVV,TDRN,TRIV]
 !## for seawat
 ALLOCATE(MC(3)%T(4)); MC(3)%T=[TBND,TTOP,TBOT,TADV]
 !## for mt3d
 ALLOCATE(MC(4)%T(3)); MC(4)%T=[TGCG,TADV,TDSP]
