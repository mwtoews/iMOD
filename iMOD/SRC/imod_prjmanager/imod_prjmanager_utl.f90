...
CONTAINS
 ...
 IF(MAXV.EQ.MINV)THEN
  ...
  ELSEIF(IINT.EQ.1)THEN
   ...
   ENDIF
  ENDIF
  SFNAME=EXFNAME(INDEX(EXFNAME,'\',.TRUE.)+1:INDEX(EXFNAME,'.',.TRUE.)-1)
  LINE=TRIM(LINE)//'     '//TRIM(SFNAME)
...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  IF(ASSOCIATED(PEST%PARAM(I)%ICOV))         DEALLOCATE(PEST%PARAM(I)%ICOV)
 ...
 !## Only confined in RUN file option (effect on Tab 2)
 I=1; IF(IFORMAT.EQ.1)I=0  
 CALL WDIALOGSELECT(ID_DSIMMANAGER_TAB2)
 CALL WDIALOGFIELDSTATE(IDF_INTEGER1,I)
 CALL WDIALOGFIELDSTATE(ID_APPLY,I)
 CALL WDIALOGFIELDSTATE(IDF_GRID1,I)
 CALL WDIALOGFIELDSTATE(ID_APPLY2,I)
...
! CALL WDIALOGFIELDSTATE(ID_OPEN2,I)
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ID,IOS,NCOL,NROW,IWINDOW
 INTEGER :: INICE !## flag for "snapp coordinates to cell size"
 CHARACTER(LEN=256) :: DIR
 ...
 IF(SUM(SUBMODEL).NE.0.0D0)THEN
  ...
  IF(PBMAN%IGENMF6.EQ.0)THEN
   SELECT CASE (IWINDOW)
    ...
    CASE (1,2)
     IF(INICE.EQ.1)THEN
      CALL UTL_IDFSNAPTONICEGRID(SUBMODEL(1),SUBMODEL(3),SUBMODEL(2),SUBMODEL(4),SUBMODEL(5),NCOL,NROW,LMAX=.FALSE.)
     ELSE
...
! CALL PMANAGER_INITSIM_GETISAVE()
 ...
 IF(IBATCH.EQ.0)THEN
  ...
  PBMAN%NLAMBDASEARCH=PEST%NLAMBDASEARCH
   ...
   IF(ASSOCIATED(PBMAN%ISAVE(I)%ILAY))THEN
    ...
    ELSE
     DO J=1,SIZE(PBMAN%ISAVE(I)%ILAY)
      IF(PBMAN%ISAVE(I)%ILAY(J).LE.PRJMXNLAY.AND.PBMAN%ISAVE(I)%ILAY(J).GT.0)THEN
       PCKLIST(N)%ILAY(PBMAN%ISAVE(I)%ILAY(J))=1
      ENDIF
     ENDDO
    ENDIF
   ELSE
 ...
 DO IPER=1,SIZE(TOPICS(ITOPIC)%STRESS)
  PMANAGER_GETNSYS=MAX(PMANAGER_GETNSYS,SIZE(TOPICS(ITOPIC)%STRESS(IPER)%FILES,2))
 ENDDO
...
!   IF(J.GT.0)EXIT
 ...
 DO
  ...
  IF(ITOPIC.GT.0)THEN
   IF(TOPICS(ITOPIC)%IACT.EQ.1)THEN
    ...
    IF(ITOPIC.EQ.TCAP)THEN
     MSYS=MSYS-TOPICS(ITOPIC)%NSUBTOPICS+PBMAN%IARMWP
 ...
 IF(NPARAM.GT.0)THEN
  ...
  DO I=1,SIZE(PEST%PARAM)
   ...
   READ(LINE,*,IOSTAT=IOS) PEST%PARAM(I)%PACT,PEST%PARAM(I)%PPARAM,PEST%PARAM(I)%PILS,PEST%PARAM(I)%PIZONE,PEST%PARAM(I)%PINI,PEST%PARAM(I)%PDELTA, &
                           ...
                           PEST%PARAM(I)%PPRIOR,PEST%PARAM(I)%PARSTD,PEST%PARAM(I)%SDATE,PEST%PARAM(I)%EDATE
   IF(IOS.NE.0)THEN
    PEST%PARAM(I)%PARSTD=0.0D0
    PEST%PARAM(I)%SDATE=''
    PEST%PARAM(I)%EDATE=''
    READ(LINE,*,IOSTAT=IOS) PEST%PARAM(I)%PACT,PEST%PARAM(I)%PPARAM,PEST%PARAM(I)%PILS,PEST%PARAM(I)%PIZONE,PEST%PARAM(I)%PINI,PEST%PARAM(I)%PDELTA, &
 ...
 LINE='MODELERROR'//TRIM(ITOS(PEST%PE_REGULARISATION)); WRITE(IU,'(A)') TRIM(LINE)
 LINE='CHOLESKY'//TRIM(ITOS(PEST%PE_CHOLESKY)); WRITE(IU,'(A)') TRIM(LINE)
 ...
 IF(.NOT.UTL_READINITFILE('MODELERROR',LINE,IU,0))RETURN; READ(LINE,*) PEST%PE_REGULARISATION
 IF(.NOT.UTL_READINITFILE('CHOLESKY',LINE,IU,0))RETURN; READ(LINE,*)   PEST%PE_CHOLESKY
 ...
 IF(.NOT.UTL_READPOINTER_REAL(IU,N,PBMAN%LINE_SEARCH,'LINESEARCHES',0))RETURN
...
! TOPICS(TADV)%TIMDEP=.FALSE. !ADV
 ...
 TOPICS(TCAP)%SNAME(7) ='(ARL) Artificial location (IPF)' 
 TOPICS(TCAP)%SNAME(8) ='(ARL) Artificial layer (IDF)'
 TOPICS(TCAP)%SNAME(9) ='(ARQ) Artificial discharge (IDF)'
...
! TOPICS(TADV)%SNAME(1) ='(ADV) Parameters ADV package (-)'
 ...
 ENDIF
 IF(ALLOCATED(THK))THEN
  CALL IDFDEALLOCATE(THK,SIZE(THK)); DEALLOCATE(THK)
 ENDIF
 IF(ALLOCATED(ARR))THEN
  CALL IDFDEALLOCATE(ARR,SIZE(ARR)); DEALLOCATE(ARR)
 ENDIF
 IF(ALLOCATED(BCR))THEN
  CALL IDFDEALLOCATE(BCR,SIZE(BCR)); DEALLOCATE(BCR)
 ENDIF
 IF(ALLOCATED(CCA))THEN
  CALL IDFDEALLOCATE(CCA,SIZE(CCA)); DEALLOCATE(CCA)
 ENDIF
 IF(ALLOCATED(VOI))THEN
  CALL IDFDEALLOCATE(VOI,SIZE(VOI)); DEALLOCATE(VOI)
 ENDIF
 IF(ALLOCATED(SUB))THEN
  CALL IDFDEALLOCATE(SUB,SIZE(SUB)); DEALLOCATE(SUB)
 ENDIF
 IF(ALLOCATED(PCS))THEN
  CALL IDFDEALLOCATE(PCS,SIZE(PCS)); DEALLOCATE(PCS)
 ENDIF
 IF(ALLOCATED(PC0))THEN
  CALL IDFDEALLOCATE(PC0,SIZE(PC0)); DEALLOCATE(PC0)
 ENDIF
 IF(ALLOCATED(OCR))THEN
  CALL IDFDEALLOCATE(OCR,SIZE(OCR)); DEALLOCATE(OCR)
 ENDIF
 IF(ALLOCATED(GL0))THEN
  CALL IDFDEALLOCATE(GL0,SIZE(GL0)); DEALLOCATE(GL0)
 ENDIF
 IF(ALLOCATED(SGS))THEN
  CALL IDFDEALLOCATE(SGS,SIZE(SGS)); DEALLOCATE(SGS)
 ENDIF
 IF(ALLOCATED(SGM))THEN
  CALL IDFDEALLOCATE(SGM,SIZE(SGM)); DEALLOCATE(SGM)
 ENDIF
 IF(ALLOCATED(TH0))THEN
  CALL IDFDEALLOCATE(TH0,SIZE(TH0)); DEALLOCATE(TH0)
 ENDIF
