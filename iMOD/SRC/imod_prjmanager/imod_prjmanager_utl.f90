...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 INTEGER :: ID,I,J,ISTEP,ITRANSIENT,N,TMP,IFGROUND
 LOGICAL :: LEX
...
!         CALL PMANAGER_INITSIM_FIELDS_TAB5()
 ...
 IF(PBMAN%ISOLVE)IRUN=1; IF(IFGROUND.EQ.1)IRUN=-1
 ...
 ELSE
  ...
  DO I=1,SHP%NPOL
   ...
   Y2=SHP%POL(I)%YMAX
   DX=50.0 !SHP%POL(I)%
   DO X1=SHP%POL(I)%XMIN,SHP%POL(I)%XMAX,DX
    CALL DBL_IGRJOIN(X1,Y1,X1,Y2,IOFFSET=1)
   ENDDO
  ENDDO
