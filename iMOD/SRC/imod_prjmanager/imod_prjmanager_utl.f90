  ...
  DO
   ...
   SELECT CASE (MESSAGE%WIN)
       ...
       SELECT CASE (MESSAGE%VALUE1)
        CASE (IDOK) 
         ...
         IF(LEX)THEN
          ...
          I=INDEX(MODELNAME,'.',.TRUE.); IF(I.GT.0)MODELNAME=MODELNAME(:I-1) !##name without dot
          SELECT CASE (PBMAN%IFORMAT)
           ...
           CASE (5)
            ...
            PBMAN%RESULT_DIR=TRIM(PBMAN%FLOW_RESULT_DIR)//'\MT3D\'//TRIM(MODELNAME)
            FNAME=TRIM(PBMAN%RESULT_DIR)//'\'//TRIM(MODELNAME)//'.RUN'
            !## mf2005-nam,mf6-nam
           CASE (2,3)
    ...
    CASE(ID_DSIMMANAGER_TAB1)
     ...
     SELECT CASE (ITYPE)
      CASE (FIELDCHANGED)
       write(*,*) "messages ", MESSAGE%VALUE1,MESSAGE%VALUE2
!       IF(MESSAGE%VALUE1.NE.MESSAGE%VALUE2)THEN
        SELECT CASE (MESSAGE%VALUE2)
         !## change of simulator 
         CASE (IDF_RADIO1,IDF_RADIO2,IDF_RADIO3,IDF_RADIO4,IDF_RADIO5,IDF_RADIO6,IDF_MENU3)
          CALL PMANAGER_INITSIM_FIELDS_TAB1(ID1=MESSAGE%VALUE1,ID2=MESSAGE%VALUE2)
         END SELECT
!       ENDIF
...
!         CALL PMANAGER_INITSIM_FIELDS_TAB5()
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_INITSIM_FIELDS_TAB1(ID1,ID2)
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,N,ID,IFORMAT,IDTAB,IDD1,IDD2
 INTEGER,INTENT(IN),OPTIONAL :: ID1, ID2
 ...
 !## Refres the dropdown menu only if MT3D is the new or the former option 
 IDD1=IDF_RADIO5; IF(PRESENT(ID1)) IDD1=ID1
 IDD2=IDF_RADIO5; IF(PRESENT(ID2)) IDD2=ID2
 write(*,*) IDD1,"= ",  IDF_RADIO5, IDD2,"= ",  IDF_RADIO5," format = ", IFORMAT
! IF(IDD1.EQ.IDF_RADIO5.OR.IDD2.EQ.IDF_RADIO5)THEN
    SELECT CASE (IFORMAT)
...
!  CALL WDIALOGFIELDSTATE(IDF_RADIO2,0)
 ...
 IF(IBATCH.EQ.0)THEN
  CALL PMANAGER_INITSIM_FIELDS_TAB1(ID1=IDF_RADIO1,ID2=IDF_RADIO1)
  CALL PMANAGER_INITSIM_FIELDS_TAB2()
  CALL PMANAGER_INITSIM_FIELDS_TAB3()
  CALL PMANAGER_INITSIM_FIELDS_TAB4()
 ENDIF
