...
CHARACTER(LEN=MAXLENPRJ),ALLOCATABLE,DIMENSION(:),PRIVATE :: PLIST
INTEGER,ALLOCATABLE,DIMENSION(:),PRIVATE :: IPLIST,JPLIST
...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 INTEGER :: ID,IDY,IYR,IMH,I,J,IHR,IMT,ISC,ISTEADY,ISTEP,ITRANSIENT
 LOGICAL :: LEX
      ...
      CASE (FIELDCHANGED)
       SELECT CASE (MESSAGE%VALUE2)
        CASE (IDF_MENU1)
         ...
         DO I=1,SIZE(IPLIST); TOPICS(JPLIST(I))%IACT_MODEL=IPLIST(I); ENDDO
       END SELECT
       SELECT CASE (MESSAGE%VALUE1)
       END SELECT
      CASE(PUSHBUTTON)
 ...
 ELSE
  ...
  !## first idf
  IF(PBMAN%IWINDOW.EQ.0)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO1)
  !## window
  IF(PBMAN%IWINDOW.EQ.1)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO2)
  !## network idf
  IF(PBMAN%IWINDOW.EQ.2)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO3)
!  !## network igen (in case of MF6)
!  IF(PBMAN%IWINDOW.EQ.2)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO4)
...
!       CALL WGRIDGETMENU(IDF_GRID1,2,LAYCON,PRJNLAY)
 ...
 CALL WDIALOGFIELDOPTIONS(IDF_INTEGER13,EDITFIELDCHANGED,1)
 ...
 CALL WDIALOGPUTMENU(IDF_MENU2,PLIST,N,IPLIST)
 CALL WDIALOGSELECT(ID)
