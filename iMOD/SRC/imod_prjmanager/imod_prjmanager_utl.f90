...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  ...
  ALLOCATE(TMPARRAY(J))
  CALL WGRIDGETCHECKBOX(IDF_GRID2,1,TMPARRAY,J) !PCKLIST%IPLIST,J)
  PCKLIST%IPLIST=TMPARRAY
  !## interpolation options
  CALL WGRIDGETCHECKBOX(IDF_GRID2,3,TMPARRAY,J) !PCKLIST%KPLIST,J)
...
!  CALL WDIALOGFIELDSTATE(IDF_RADIO2,0)
 ...
 IMPLICIT NONE
 INTEGER :: ID,I,J,N
 INTEGER,ALLOCATABLE,DIMENSION(:) :: TMPARRAY
 CHARACTER(LEN=MAXLENPRJ),ALLOCATABLE,DIMENSION(:) :: TMPARRAYC
 ...
 ALLOCATE(TMPARRAY(N),TMPARRAYC(N))
 TMPARRAY=PCKLIST%IPLIST
 CALL WGRIDPUTCHECKBOX(IDF_GRID2,1,TMPARRAY,N)  !PCKLIST%IPLIST,N)
 TMPARRAYC=PCKLIST%PLIST
 CALL WGRIDPUTSTRING(  IDF_GRID2,2,TMPARRAYC,N) !PCKLIST%PLIST ,N)
 TMPARRAY=PCKLIST%KPLIST
 CALL WGRIDPUTCHECKBOX(IDF_GRID2,3,TMPARRAY,N)  !PCKLIST%KPLIST,N)
 DEALLOCATE(TMPARRAY)
 
 CALL WDIALOGSELECT(ID_DSIMMANAGER_TAB5)
 TMPARRAYC=PCKLIST%PLIST
 CALL WDIALOGPUTMENU(IDF_MENU1,TMPARRAYC,N,1)   !PCKLIST%PLIST,N,1)
 CALL PMANAGER_INITSIM_PUTISAVE()
