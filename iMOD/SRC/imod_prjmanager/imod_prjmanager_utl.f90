...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 DO
  ...
  IF(SNAP.EQ.1.AND.SUBMODEL(5).GT.0)CALL UTL_IDFSNAPTOGRID(MOUSEX,MOUSEY,MOUSEX,MOUSEY,SUBMODEL(5),I,J)
...
!  CALL WDIALOGFIELDSTATE(IDF_RADIO2,0)
 ...
 CALL WGRIDPUTCHECKBOX(IDF_GRID2,1,TMPINT,N)  !PCKLIST%IPLIST,N)
 TMPNAME=PCKLIST%PLIST
 CALL WGRIDPUTSTRING(  IDF_GRID2,2,TMPNAME,N) !PCKLIST%PLIST
 TMPINT=PCKLIST%KPLIST
 CALL WGRIDPUTCHECKBOX(IDF_GRID2,3,TMPINT,N)  !PCKLIST%KPLIST,N)
 
 CALL WDIALOGSELECT(ID_DSIMMANAGER_TAB5)
 CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,N,1)   !PCKLIST%PLIST,N,1)
