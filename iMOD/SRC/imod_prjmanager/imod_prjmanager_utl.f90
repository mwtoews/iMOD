...
! CALL WDIALOGFIELDSTATE(ID_ZOOMEXTENT,IACT(2))
 ...
 IF(IBATCH.EQ.0)THEN
  ...
  DO I=1,MAXTOPICS
   ALLOCATE(PBMAN%ISAVE(I)%ILAY(1)); PBMAN%ISAVE(I)%ILAY(1)=-1
  ENDDO
 ...
 N=0; DO I=1,SIZE(TOPICS)
  IF(ASSOCIATED(TOPICS(I)%STRESS))THEN
   ...
   IF(PBMAN%ISAVE(I)%ILAY(1).EQ.-1)THEN
    !## activate all 
    DO J=1,PRJMXNLAY; PCKLIST(I)%ILAY(J)=1; ENDDO
   ELSE
 ...
 CALL WDIALOGGETMENU(IDF_MENU2,PCKLIST(I)%ILAY) !PCKLIST(I)%CLAY,PRJNLAY,)
