...
CONTAINS
 ...
 !## rct
 LRCT=.FALSE.; ITOPIC=TRCT; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LRCT=.TRUE.; ENDIF 
 IF(TOPICS(ITOPIC)%IACT_MODEL.EQ.0)LRCT=.FALSE.

 !## adv n.v.t.
 
 !## vdf
 LVDF=.FALSE.; ITOPIC=TVDF; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LVDF=.TRUE.; ENDIF 
 IF(TOPICS(ITOPIC)%IACT_MODEL.EQ.0)LVDF=.FALSE.

 !## btn
 LBTN=.FALSE.; ITOPIC=TBTN; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LBTN=.TRUE.; ENDIF 
 IF(TOPICS(ITOPIC)%IACT_MODEL.EQ.0)LBTN=.FALSE.

 !## dis
 LDIS=.FALSE.
 IF(ASSOCIATED(TOPICS(TTOP)%STRESS).AND. &  !## top
    ASSOCIATED(TOPICS(TBOT)%STRESS))THEN    !## bot
  IF(ASSOCIATED(TOPICS(TTOP)%STRESS(1)%FILES).AND. & !## top
     ASSOCIATED(TOPICS(TBOT)%STRESS(1)%FILES))THEN   !## bot
   LDIS=.TRUE.
  ENDIF
 ENDIF

 IF(.NOT.LDIS)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Missing data to convert to DIS package'//CHAR(13)// &
    'You need at least TOP and BOT parameters','Warning')
  RETURN
 ENDIF

 !## dsp
 LDSP=.FALSE.; ITOPIC=TDSP; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LDSP=.TRUE.; ENDIF 
 IF(TOPICS(ITOPIC)%IACT_MODEL.EQ.0)LDSP=.FALSE.

 !## ssm
 LSSM=.FALSE.
 IF(NSPECIES.GT.0) LSSM=.FALSE.  
...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 INTEGER :: ID,IDY,IYR,IMH,I,J,IHR,IMT,ISC,ISTEP,ITRANSIENT,N,IXPOS,IYPOS
 LOGICAL :: LEX
 
 PMANAGER_INITSIM=.FALSE.; IRUN=0
 
 ID=WINFODIALOG(CURRENTDIALOG); IXPOS=WINFODIALOG(DIALOGXPOS); IYPOS=WINFODIALOG(DIALOGYPOS); CALL WDIALOGHIDE()

 !## load dialog
 CALL WDIALOGLOAD(ID_DSIMMANAGER,ID_DSIMMANAGER)

 IF(.NOT.PMANAGER_INITSIM_INITFIELDS(IBATCH,ITRANSIENT))then
       ...
       SELECT CASE (MESSAGE%VALUE1)
        CASE (IDOK,IDSIMULATE1,IDSIMULATE2)
         ...
         IF(LEX)THEN
          ...
          IF(PBMAN%IFORMAT.EQ.1.OR.PBMAN%IFORMAT.EQ.4.OR.PBMAN%IFORMAT.EQ.5)THEN
      ...
      CASE(PUSHBUTTON)
       SELECT CASE (MESSAGE%VALUE1)
        ...
        CASE (ID_OPEN2)
         ...
         IF(LEX)CALL WDIALOGPUTSTRING(IDF_STRING3,FNAME)         
       END SELECT
     END SELECT
!     CALL PMANAGER_TAB3_WINDOW()
...
!         CALL PMANAGER_INITSIM_FIELDS_TAB5()
        CASE (IDF_MENU2)
...
!         CALL PMANAGER_INITSIM_FIELDS_TAB5()
 ...
 ELSE  ! in case iMOD batch
  ...
  CALL WDIALOGPUTDOUBLE(IDF_REAL4,PBMAN%XMIN)     !## xmin
  CALL WDIALOGPUTDOUBLE(IDF_REAL5,PBMAN%YMIN)     !## ymin
  CALL WDIALOGPUTDOUBLE(IDF_REAL1,PBMAN%XMAX)     !## xmax
  CALL WDIALOGPUTDOUBLE(IDF_REAL2,PBMAN%YMAX)     !## ymax
  ...
  IF(PBMAN%IFORMAT.EQ.1)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO1) !## runfile
  IF(PBMAN%IFORMAT.EQ.2)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO2) !## modflow2005
  IF(PBMAN%IFORMAT.EQ.3)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO3) !## modflow6
  IF(PBMAN%IFORMAT.EQ.4)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO4) !## seawat
  IF(PBMAN%IFORMAT.EQ.5)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO5) !## mt3d
 ...
 IF(IBATCH.EQ.0)THEN; CALL WDIALOGSELECT(ID); CALL WDIALOGSHOW(IXPOS,IYPOS,0,2); ENDIF
 ...
 !## final check
 IF((ITRANSIENT.EQ.1.AND.PBMAN%ISTEADY.EQ.0).AND.SIZE(SIM).EQ.1)THEN
...
! CALL UTL_HIDESHOWDIALOG(ID_DMODEL,2)
 ...
 IF(IFORMAT.EQ.5)THEN ; CALL WDIALOGFIELDSTATE(IDF_MENU3,0)
   ELSE ; CALL WDIALOGFIELDSTATE(IDF_MENU3,0) ; ENDIF
 
 !## fill in dropdown menu
 IF(IFORMAT.EQ.1.OR.IFORMAT.EQ.4.OR.IFORMAT.EQ.5)THEN
  CALL UTL_IMODFILLMENU(IDF_MENU2,TRIM(PREFVAL(1))//'\RUNFILES','*.RUN','F',N,0,0) 
 ...
 IF(IFORMAT.EQ.2)THEN
  I=0; IF(LBCF)I=1; CALL WDIALOGFIELDSTATE(IDF_RADIO6,I)
  I=0; IF(LLPF)I=1; CALL WDIALOGFIELDSTATE(IDF_RADIO7,I)
 ELSE
 ...
 CALL WDIALOGFIELDSTATE(IDF_LABEL15  ,IACT(2))
 CALL WDIALOGFIELDSTATE(IDF_REAL7    ,IACT(2))
 CALL WDIALOGFIELDSTATE(IDF_REAL8    ,IACT(2))
 CALL WDIALOGFIELDSTATE(IDF_REAL9    ,IACT(2))
 ...
 !!###======================================================================
 !SUBROUTINE PMANAGER_TAB3_WINDOW()
 !!###======================================================================
 !IMPLICIT NONE
 !CHARACTER(LEN=256) :: MOD_WINDOW_STR
 !INTEGER :: IOS,I
 !
 !PBMAN%XMIN=0 ; PBMAN%YMIN=0 ; PBMAN%XMAX=0 ; PBMAN%YMAX=0
 !
 !CALL WDIALOGSELECT(ID_DSIMMANAGER_TAB3)
 !!IDF_RADIO1
 !!IDF_RADIO2
 !CALL WDIALOGGETRADIOBUTTON(IDF_RADIO2,I)
 !IF(I.GT.0)THEN
 !  CALL WDIALOGGETSTRING(IDF_STRING1,MOD_WINDOW_STR)
 !  READ(MOD_WINDOW_STR,*,IOSTAT=IOS) PBMAN%XMIN,PBMAN%YMIN,PBMAN%XMAX,PBMAN%YMAX
 !  IF(PBMAN%XMIN.GE.PBMAN%XMAX.OR.PBMAN%YMIN.GE.PBMAN%YMAX)THEN
 !     PBMAN%XMIN=MPW%XMIN ; PBMAN%YMIN=MPW%YMIN ; PBMAN%XMAX=MPW%XMAX ; PBMAN%YMAX=MPW%YMAX
 !     CALL WDIALOGPUTSTRING(IDF_STRING1,"wrong window definition")
 !  ENDIF
 !ENDIF
 !
 !!IDF_RADIO3
 ...
 !CALL WDIALOGPUTDOUBLE(IDF_REAL2,PBMAN%YMAX)     !## ymax
 !  
 !END SUBROUTINE PMANAGER_TAB3_WINDOW
 ...
 DO I=1,SIZE(DID)
  CALL WDIALOGFIELDSTATE(DID(I),J)
 ENDDO
...
! CALL PMANAGER_INITSIM_GETISAVE()
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_INITSIM_INITFIELDS(IBATCH,ITRANSIENT)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IBATCH
 INTEGER,INTENT(OUT) :: ITRANSIENT
 INTEGER :: ID,IDY,IYR,IMH,ITOPIC,IPER,I,J,K,MINJD,MAXJD,IDATE,IHR,IMT,ISC,IHMS,MINHMS,MAXHMS,ISTEADY,ISTEP
 ...
 !## initiate settings for iMOD GUI
 IF(IBATCH.EQ.0)THEN
  PBMAN%IPKS=0
  PBMAN%NSUBMODEL=1
  PBMAN%ISUBMODEL=1
 ...
 LBAS=.FALSE.
 IF(ASSOCIATED(TOPICS(TBND)%STRESS).AND. &  !## bnd
    ASSOCIATED(TOPICS(TSHD)%STRESS))THEN    !## shd
  IF(ASSOCIATED(TOPICS(TBND)%STRESS(1)%FILES).AND. & !## bnd
 ...
 IF(ASSOCIATED(TOPICS(TKDW)%STRESS))THEN
  ...
  ENDIF
 ENDIF
 I=0; IF(LBCF)I=1; CALL WDIALOGFIELDSTATE(IDF_RADIO6,I)             !## bcf
 IF(LBCF)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO6)
 ...
 J=0; IF(LLPF)J=1; CALL WDIALOGFIELDSTATE(IDF_RADIO7,J)              !## lpf
 IF(LLPF)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO7)
...
!   IF(J.GT.0)EXIT
 ...
 ! Frans: MT3D   run wordt nieuwe submap in betaande modelresultmap in ..\IMOD_USER\MODELS\<runnaam>\MT3D 
 ! Frans: SEAWAT run wordt nieuwe    map in betaande modelresultmap in ..\IMOD_USER\MODELS\<Seawat-name> 
...
!    TRIM(EXFNAME),'Error')
 ...
 TOPICS(TRCT)%TNAME='(UDF) User Difined Reaction'
 TOPICS(TDSP)%TNAME='(DSP) Dispersion '
 TOPICS(TADV)%TNAME='(ADV) Advection'
 TOPICS(TVDF)%TNAME='(VDF) Variable-Density Flow'
...
! TOPICS(TUZF)%SNAME(4) ='(RWC) Residual Water Content of Unsat. Zone (IDF)'
 ...
 TOPICS(TADV)%SNAME(1) ='(ADV) Parameters ADV (-)'
 TOPICS(TVDF)%SNAME(1) ='(VDF) Parameters VDF (-)'
 TOPICS(TBTN)%SNAME(1) ='(CBI) Concentration Boundary Indicator (IDF)'
 TOPICS(TBTN)%SNAME(2) ='(SCO) Starting concentration (IDF)'
