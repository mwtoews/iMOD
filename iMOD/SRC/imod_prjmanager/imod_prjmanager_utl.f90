...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 INTEGER :: ID,IDY,IYR,IMH,I,J,IHR,IMT,ISC,ISTEP,ITRANSIENT
 LOGICAL :: LEX
    ...
    CASE(ID_DSIMMANAGER_TAB5)
     ...
     SELECT CASE (ITYPE)
      CASE (FIELDCHANGED)
       SELECT CASE (MESSAGE%VALUE2)
        CASE (IDF_MENU1)
         IF(MESSAGE%VALUE2.EQ.MESSAGE%VALUE2)THEN
 ...
 DO I=1,SIZE(PCKLIST)
  ...
  ELSE
   IF(ASSOCIATED(PBMAN%ISAVE(PCKLIST(I)%JPLIST)%ILAY))DEALLOCATE(PBMAN%ISAVE(PCKLIST(I)%JPLIST)%ILAY)
   ALLOCATE(PBMAN%ISAVE(PCKLIST(I)%JPLIST)%ILAY(PRJNLAY))
   DO J=1,PRJNLAY
    PBMAN%ISAVE(PCKLIST(I)%JPLIST)%ILAY(J)=PCKLIST(I)%ILAY(J)
   ENDDO
  ENDIF
 ENDDO
 ...
 CALL WDIALOGGETMENU(IDF_DOUBLE1,PBMAN%ICONSISTENCY)
 ...
 !## window or bndfile
 CALL WDIALOGGETRADIOBUTTON(IDF_RADIO1,I); IACT(I)=1
...
! CALL WDIALOGFIELDSTATE(ID_ZOOMEXTENT,IACT(2))
 ...
 ENDIF
! DO I=1,MAXTOPICS
!  IF(.NOT.ASSOCIATED(PBMAN%ISAVE(I)%ILAY))THEN
!   ALLOCATE(PBMAN%ISAVE(I)%ILAY(1))
!   PBMAN%ISAVE(I)%ILAY(1)=-1
!  ENDIF
! ENDDO
 ...
 CALL WGRIDPUTSTRING(  IDF_GRID2,1,PCKLIST%PLIST ,N)
