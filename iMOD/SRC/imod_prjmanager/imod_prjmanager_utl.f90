       ...
       SELECT CASE (MESSAGE%VALUE1)
        CASE (ID_OPEN)
         ...
         IF(.NOT.IMODBATCH_RUNFILE_READ(IU,0))THEN
          !## update dialogs
          IF(.NOT.PMANAGER_INITSIM_INITFIELDS(IBATCH,ITRANSIENT))THEN; ENDIF
         ENDIF
         CLOSE(IU)
...
!         CALL PMANAGER_INITSIM_FIELDS_TAB5()
 ...
 IF(IFORMAT.EQ.1.OR.IFORMAT.EQ.4.OR.IFORMAT.EQ.5)THEN
  ...
  CALL WDIALOGPUTSTRING(IDF_LABEL25,'Enter of Select Output Name:')
 ELSE
  CALL UTL_IMODFILLMENU(IDF_MENU2,TRIM(PREFVAL(1))//'\MODELS','*','D',N,0,0) 
  CALL WDIALOGPUTSTRING(IDF_LABEL25,'Enter of Select Output Folder:')
 ENDIF
 ...
 IF(IFORMAT.EQ.5)THEN
  ...
  CALL WDIALOGFIELDSTATE(IDF_LABEL3,1)
