...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 INTEGER :: ID,IDY,IYR,IMH,I,J,IHR,IMT,ISC,ISTEP,ITRANSIENT,N,IXPOS,IYPOS,TMP
 LOGICAL :: LEX
...
!         CALL PMANAGER_INITSIM_FIELDS_TAB5()
 ...
 CALL WDIALOGGETMENU(IDF_MENU2,PBMAN%ADV%NADVFD)
 ...
 IF(IBATCH.EQ.0)THEN
  CALL WDIALOGSELECT(ID); CALL WDIALOGSHOW(IXPOS,IYPOS,0,2)
...
!  CALL WDIALOGFIELDSTATE(IDF_RADIO2,0)
 ...
 TMP=PBMAN%ADV%MIXELM+2
 CALL WDIALOGPUTOPTION(IDF_MENU1,TMP)
 CALL WDIALOGPUTOPTION(IDF_MENU2,PBMAN%ADV%NADVFD)
...
! TOPICS(TADV)%SNAME(1) ='(ADV) Parameters ADV package (-)'
 ...
 MC(5)%T=[TTOP,TTHK,TPOR,TRCH,TEVT,TRIV,TWEL,TGHB,TCHD,TOBS,TGCG,TVDF,TFDE,TCBI,TSCO,TDSP,TTVC, &
