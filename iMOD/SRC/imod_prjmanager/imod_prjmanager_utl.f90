...
JLOOP: DO K=1,SIZE(TOPICS)
 ...
 CALL WDIALOGSSELECT(ID_DSIMMANAGER_TAB1)
 CALL UTL_IMODFILLMENU(IDF_MENU1,TRIM(PREFVAL(1))//'\RUNFILES','*.INI','F',N,0,0) 
 N=MIN(1,N); CALL WDIALOGFIELDSTATE(ID_OPEN,N)
 CALL WDIALOGSSELECT(ID_DSIMMANAGER)
    ...
    CASE(ID_DSIMMANAGER_TAB1)
     ...
     SELECT CASE (ITYPE)
      ...
      CASE(PUSHBUTTON)
       SELECT CASE (MESSAGE%VALUE1)
        CASE (ID_OPEN)
         CALL WDIALOGGETMENU(IDF_MENU1,I,PBMAN%PRJFILE)
         PBMAN%PRJFILE=TRIM(PREFVAL(1))//'\RUNFILES\'//TRIM(PBMAN%PRJFILE)
         IU=UTL_GETUNIT(); OPEN(IU,FILE=PBMAN%PRJFILE,STATUS='OLD',ACTION='READ')
         !## read parameters
         IF(.NOT.IMODBATCH_RUNFILE_READ(IU,1))THEN
          !## update dialogs
         ENDIF
         CLOSE(IU)
...
! TOPICS(TUZF)%SNAME(2) ='Saturated Vertical Conductivity (IDF)'
 ...
 !## for mt3d
 ALLOCATE(MC(5)%T(23),MC(5)%IACT(23))
 MC(5)%MCNAME='MT3D'; MC(5)%IACT=1
 MC(5)%T=[TTOP,TTHK,TPOR,TRCH,TEVT,TRIV,TWEL,TGHB,TCHD,TOBS,TGCG,TVDF,TFDE,TCBI,TSCO,TADV,TDSP,TTVC, &
          TICS,TFSC,TSSC,TFOD,TFOS]
