...
JLOOP: DO K=1,SIZE(TOPICS)
  ...
  DO
   ...
   SELECT CASE (MESSAGE%WIN)
        ...
        CASE (IDOK)   ! Start modelrun 
         ...
         IF(LEX)THEN
          ...
          SELECT CASE (PBMAN%IFORMAT)
           ...
           CASE (1,4)
            PBMAN%OUTPUT=TRIM(PREFVAL(1))//'\MODELS\'//TRIM(MODELNAME)
            MODELNAME=TRIM(MODELNAME)//'.RUN'
            FNAME=TRIM(PREFVAL(1))//'\RUNFILES\'//TRIM(MODELNAME)
           !## mt3d
           CASE (5)
            ...
            MODELNAME=TRIM(MODELNAME)//'.RUN'
            FNAME=TRIM(PBMAN%RESULT_DIR)//'\'//TRIM(MODELNAME)
            !## mf2005-nam,mf6-nam
           CASE (2,3)
            FNAME=TRIM(PREFVAL(1))//'\MODELS\'//TRIM(MODELNAME)//'\'//TRIM(MODELNAME)//'.NAM'
            MODELNAME=TRIM(MODELNAME)//'.NAM'
          END SELECT
          !## create folder
    ...
    CASE(ID_DSIMMANAGER_TAB1)
     ...
     SELECT CASE (ITYPE)
      CASE (FIELDCHANGED)
       CALL PMANAGER_INITSIM_FIELDS_TAB1(MESSAGE%VALUE1,MESSAGE%VALUE2,ITRANSIENT)
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_INITSIM_FIELDS_TAB1(ID1,ID2,ITRANSIENT)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ID1,ID2,ITRANSIENT
...
!  CALL WDIALOGFIELDSTATE(IDF_RADIO2,0)
 ...
 IF(IBATCH.EQ.0)THEN
  ...
  CALL PMANAGER_INITSIM_FIELDS_TAB1(IDF_RADIO5,IDF_RADIO1,ITRANSIENT)
  CALL PMANAGER_INITSIM_FIELDS_TAB2()
  CALL PMANAGER_INITSIM_FIELDS_TAB3()
  CALL PMANAGER_INITSIM_FIELDS_TAB4()
 ENDIF
