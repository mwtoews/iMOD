...
!   IF(DBL_IGRINSIDESHAPE(X1,Y1,SHP%POL(I)).EQ.1)THEN
   IF(TMP_UTL_INSIDEPOLYGON2(X1,Y1,SHP%POL(I)%X,SHP%POL(I)%Y,SHP%POL(I)%N).EQ.1)THEN
    MF6IDF(J)%X(ICOL,IROW)=1.0D0
    !## deactivate others on this location
    DO II=I-1,1,-1 
     JJ=ISORT(II)
      IF(TMP_UTL_INSIDEPOLYGON2(X1,Y1,SHP%POL(II)%X,SHP%POL(II)%Y,SHP%POL(II)%N).EQ.1)THEN
!     IF(DBL_IGRINSIDESHAPE(X1,Y1,SHP%POL(II)).EQ.1)THEN
 ...
 !###======================================================================
 INTEGER FUNCTION TMP_UTL_INSIDEPOLYGON2(PX,PY,XX,YY,N)
 !###======================================================================
 !
