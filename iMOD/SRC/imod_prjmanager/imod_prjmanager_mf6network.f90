...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS(GENFNAME,OUTFOLDER,NSUBMODEL,IBATCH,ILAY)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IBATCH,ILAY
 CHARACTER(LEN=*),INTENT(IN) :: GENFNAME,OUTFOLDER
 INTEGER,INTENT(OUT) :: NSUBMODEL
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IPOL,IPIN
 INTEGER(KIND=1),ALLOCATABLE,DIMENSION(:,:,:) :: IPC
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: MF6IDF
 INTEGER :: I,J,IU,JU,II,JJ,N
 ...
 !## store polygon numbers currenly active in this submodel
 ...
 CALL PMANAGER_GENERATEMFNETWORKS_DIMIDF(MF6IDF,IPOL,IPIN,IBATCH)
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS_CREATEPOLYGONS(OUTFOLDER,MF6IDF)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: OUTFOLDER
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(:) :: MF6IDF
 CHARACTER(LEN=256) :: FNAME
 INTEGER :: IOS,I,J,N,II,JJ,IU,JU,KU,IROW,ICOL,JROW,JCOL,IPOL,IR,IC
 ...
 DO I=1,SIZE(MF6IDF)
  WRITE(KU,'(A)') TRIM(ITOS(I))//','//TRIM(RTOS(MF6IDF(I)%DX,'F',3))
 ENDDO
 ...
 N=0; IPOL=0; DO
  ...
  IF(I.NE.J)THEN
   ...
   CALL PMANAGER_GENERATEMFNETWORKS_PUZZLE(XC,YC,IS,JS,ID,N,JU,IPOL)
 ...
 !## read binary genfile - all arrays reloaded
 CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,TRIM(OUTFOLDER)//'\SUBMODELS_BINAIR.GEN','GEN')
 
 IF(SHP%NPOL.NE.SIZE(MF6IDF))THEN; WRiTE(*,*) 'It is not allowed to be here !!!'; PAUSE; ENDIF
 ...
 !## create active and inactive areas
 DO I=1,SHP%NPOL
  
  DO IROW=1,MF6IDF(I)%NROW; DO ICOL=1,MF6IDF(I)%NCOL
   !## skip blanked out areas
   IF(MF6IDF(I)%X(ICOL,IROW).EQ.MF6IDF(I)%NODATA)CYCLE
   CALL IDFGETLOC(MF6IDF(I),IROW,ICOL,X1,Y1)
   IF(DBL_IGRINSIDESHAPE(X1,Y1,SHP%POL(I)).EQ.1)THEN
    MF6IDF(I)%X(ICOL,IROW)=1.0D0
   ELSE
    MF6IDF(I)%X(ICOL,IROW)=MF6IDF(I)%NODATA
   ENDIF
  ENDDO; ENDDO
 ENDDO

 !## create constant head for first if needed
 I=1; DO IROW=1,MF6IDF(I)%NROW; DO ICOL=1,MF6IDF(I)%NCOL
  !## skip blanked out areas
  IF(MF6IDF(I)%X(ICOL,IROW).EQ.MF6IDF(I)%NODATA)CYCLE
IRLOOP: DO IR=MAX(IROW-1,1),MIN(IROW+1,MF6IDF(I)%NROW)
   DO IC=MAX(ICOL-1,1),MIN(ICOL+1,MF6IDF(I)%NCOL)
    IF(MF6IDF(I)%X(IC,IR).EQ.MF6IDF(I)%NODATA)THEN
     MF6IDF(I)%X(ICOL,IROW)=-1.0D0; EXIT IRLOOP
    ENDIF
   ENDDO
  ENDDO IRLOOP
 ENDDO; ENDDO 
...
!    MF6IDF(I)%X(ICOL,IROW)=1.0D0
   ...
   DO II=I-1,1,-1 
    IF(DBL_IGRINSIDESHAPE(X1,Y1,SHP%POL(II)).EQ.1)THEN
     CALL IDFIROWICOL(MF6IDF(II),JROW,JCOL,X1,Y1)
     MF6IDF(II)%X(JCOL,JROW)=MF6IDF(II)%NODATA 
    ENDIF
   ENDDO
!   ELSE
!    MF6IDF(I)%X(ICOL,IROW)=MF6IDF(I)%NODATA
!   ENDIF
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS_PUZZLE(XC,YC,IS,JS,ID,N,IU,IPOL)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(INOUT) :: N,IPOL
 INTEGER,INTENT(IN) :: IU
 INTEGER,DIMENSION(:),INTENT(OUT) :: IS,JS,ID
 REAL(KIND=DP_KIND),DIMENSION(:,:),INTENT(INOUT) :: XC,YC
 INTEGER :: NS,M,I,II,J,IPOS,ND,IDIR
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS_DIMIDF(MF6IDF,IPOL,IPIN,IBATCH)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(:) :: MF6IDF
 INTEGER,INTENT(IN),DIMENSION(:) :: IPOL,IPIN
 ...
 DO I=1,SIZE(MF6IDF)
  ...
  !## sorted polygon number
  J=IPOL(I)
  
  MF6IDF(I)%XMIN=SHP%POL(J)%XMIN; MF6IDF(I)%YMIN=SHP%POL(J)%YMIN
  MF6IDF(I)%XMAX=SHP%POL(J)%XMAX; MF6IDF(I)%YMAX=SHP%POL(J)%YMAX

  DO II=1,MAXCOL
   STR(II)%STRING=''; DO JJ=1,SHP%LWIDTH(II); STR(II)%STRING(JJ:JJ)=SHP%POL(J)%LBL(II)%STRING(JJ); ENDDO
  ENDDO

  !## read cellsize in polygon
  READ(STR(2)%STRING,*,IOSTAT=IOS) MF6IDF(I)%DX
  IF(IOS.NE.0)THEN; WRITE(*,'(/A/)') 'Cannot read cellsize for polygon #',IPOL(J); STOP; ENDIF
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS_SORT(N,IPOL,IPIN)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N
