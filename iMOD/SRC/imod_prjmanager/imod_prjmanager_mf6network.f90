...
CONTAINS
 ...
 IF(ILAY.NE.0)THEN
  SHP%POL%IACT=1
  DO I=1,SHP%NPOL
   DO J=1,SIZE(PBMAN%SM(I)%ILAY); IF(PBMAN%SM(I)%ILAY(J).EQ.ILAY)SHP%POL(I)%IACT=1; ENDDO
  ENDDO
 !## all active in this layer
 ELSE
  SHP%POL%IACT=1
 ENDIF
 
 ALLOCATE(ISORT(SHP%NPOL),IPOL(SHP%NPOL)); ISORT=0
 DO I=1,SHP%NPOL; IF(SHP%POL(I)%IACT.EQ.1)ISORT(I)=I; IPOL(I)=0; ENDDO
 CALL PMANAGER_GENERATEMFNETWORKS_SORT(SHP%NPOL,ISORT,IPOL)
 N=0; DO I=1,SHP%NPOL; IF(ISORT(I).GT.0)N=N+1; ENDDO
...
! ALLOCATE(MF6IDF(SHP%NPOL)); DO I=1,SHP%NPOL; CALL IDFNULLIFY(MF6IDF(I)); ENDDO 
 ...
 DO I=1,SHP%NPOL
  ...
  ENDIF
!  IF(ILAY.NE.0)THEN
!   MF6IDF(I)%FNAME=TRIM(OUTFOLDER)//'\SUBMODEL_'//TRIM(ITOS(I))//'_L'//TRIM(ITOS(ILAY))//'.IDF'
!  ELSE
...
!  IF(ISORT(I).EQ.1)THEN
 ...
 DO I=1,SHP%NPOL
  IF(SHP%POL(I)%N.GT.0.AND.SHP%POL(I)%IACT.EQ.1)THEN
   SELECT CASE (SHP%POL(I)%ITYPE)
    CASE (ID_RECTANGLE)
