...
CONTAINS
 ...
 DO I=1,PRJNPER-1
  ...
  !## skip steady-state
  IF(SIM(I)%DELT.EQ.0.0D0.OR.SIM(I)%CDATE.EQ.'STEADY-STATE')CYCLE
  ...
  SELECT CASE (ITYPE)
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (ID_SELECTALL)
      I=1; IF(SIM(1)%DELT.LE.0.0D0)I=2
      CALL WDIALOGPUTINTEGER(IDF_INTEGER1,I)
      CALL WDIALOGPUTINTEGER(IDF_INTEGER2,PRJNPER)
      CALL WGRIDCOLOURCOLUMN(IDF_GRID1,1,-1,-1)
      DO IROW=I,PRJNPER; CALL WGRIDCOLOURCELL(IDF_GRID1,1,IROW,-1,WRGB(255,0,0)); ENDDO
     CASE (ID_APPLY)
 ...
 CALL WDIALOGPUTINTEGER(IDF_INTEGER4,PRJNPER)
 ...
 ELSEIF(ID.EQ.ID_SAVE)THEN
  ...
  DO I=1,SIZE(SIM)
   WRITE(LINE,'(I4.4,5I2.2,A1,I2,A1,I4.4,A1,G12.5)') SIM(I)%IYR,SIM(I)%IMH,SIM(I)%IDY,SIM(I)%IHR,SIM(I)%IMT,SIM(I)%ISC, &
