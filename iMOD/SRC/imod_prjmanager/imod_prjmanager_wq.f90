...
CONTAINS
 ...
 INTEGER :: IU,KPER,N,ISS,IDSP,ISSM,IVDF,IRCT,IUDR
 LOGICAL :: LEX
 ...
 IF(.NOT.PMANAGER_SAVERUNWQ_WRTADV(IU))THEN; ENDIF
 IDSP=0; IF(PMANAGER_SAVERUNWQ_WRTDSP(IU,N))THEN; IF(N.GT.0)IDSP=1; ENDIF
 ISSM=0; IF(PMANAGER_SAVERUNWQ_WRTSSM(IU,N))THEN; IF(N.GT.0)ISSM=1; ENDIF
 IVDF=0; IF(PMANAGER_SAVERUNWQ_WRTVDF(IU,N))THEN; IF(N.GT.0)IVDF=1; ENDIF

 IRCT=0; IF(PMANAGER_SAVERUNWQ_WRTRCT(IU,N))THEN; IF(N.GT.0)IRCT=1; ENDIF
 IUDR=0; IF(PMANAGER_SAVERUNWQ_WRTUDR(IU,N))THEN; IF(N.GT.0)IRCT=1; ENDIF

 !## write Start Packages
 IF(.NOT.PMANAGER_SAVERUNWQ_WRTGEN(IU,IDSP,ISSM,IVDF,IRCT,IUDR))THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot write GEN package','Error'); RETURN
 ENDIF

!BTN, GCG, FTL, ADV, SSM, RCT
...
! WRITE(IU,'(1X,A)') '#DCHMOC = '
 ...
 N=0; DO ILAY=1,PRJNLAY
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'TRPT_L?'  ,TDSP,0,1,0,ILAY,0))N=N+1
 ...
 N=0; DO KPER=1,PRJNPER ; DO ISPECIES=1,NSPECIES
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CRCH_T?_P?',TRCH,KPER,0,0,ILAY,ISPECIES))N=N+1
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CEVT_T?_P?',TEVT,KPER,0,0,ILAY,ISPECIES))N=N+1
  DO ILAY=1,PRJNLAY
   ...
   IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CDRN_T?_P?_L?',TDRN,KPER,0,0,ILAY,ISPECIES))N=N+1
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVERUNWQ_WRTVDF(IU,N)
 !####====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IU
 INTEGER,INTENT(OUT) :: N
 ...
 TSTAMP='STEADY-STATE' !## kan pbman worden op misc tab van mt3d 
...
! WRITE(IU,'(1X,A)') 'iRCH_P = '/
 ...
 INTEGER :: NSYS,NATT,ICNST,ILAY,IP1,IP2,KPER,TPER,ISYS,IATT,IPER
 ...
 I1=1; I2=I1; DO KPER=1,PRJNPER
  ...
  IF(DT2.NE.DT1.OR.KPER.EQ.PRJNPER)THEN
   WRITE(IU,'(1X,A)') 'PERLEN_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = '//TRIM(RTOS(SIM(I1)%DELT,'G',7))
   I1=I2
  ENDIF
 ENDDO
 ...
 I1=1; I2=I1; DO KPER=1,PRJNPER
  ...
  IF(IT2.NE.IT1.OR.KPER.EQ.PRJNPER)THEN
   WRITE(IU,'(1X,A)') 'NSTP_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = '//TRIM(ITOS(SIM(I1)%NSTP))
   I1=I2
  ENDIF
 ENDDO
 ...
 IF(LEX)THEN
  ...
  I1=1; I2=I1; DO KPER=1,PRJNPER
   ...
   IF(DT2.NE.DT1.OR.KPER.EQ.PRJNPER)THEN
    WRITE(IU,'(1X,A)') 'TSMULT_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = '//TRIM(RTOS(SIM(I1)%TMULT,'G',7))
    I1=I2
   ENDIF
  ENDDO
 
  I1=1; I2=I1; DO KPER=1,PRJNPER
   I2=I2+1; DT1=SIM(I1)%DELT; IF(DT1.GT.0.0D0)DT1=1.0D0; DT2=SIM(I2)%DELT; IF(DT2.GT.0.0D0)DT2=1.0D0
   IF(DT2.NE.DT1.OR.KPER.EQ.PRJNPER)THEN
    IF(DT1.EQ.0.0D0)WRITE(IU,'(1X,A)') 'SSTR_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = SS'
    IF(DT1.GT.0.0D0)WRITE(IU,'(1X,A)') 'SSTR_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = TR'
    I1=I2
   ENDIF
  ENDDO
  ...
  I1=1; I2=I1; DO KPER=1,PRJNPER
   ...
   IF(IT2.NE.IT1.OR.KPER.EQ.PRJNPER)THEN
    WRITE(IU,'(1X,A)') 'DT0_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = '//TRIM(RTOS(SIM(I1)%DT0,'G',7))
    I1=I2
   ENDIF
  ENDDO
  ...
  I1=1; I2=I1; DO KPER=1,PRJNPER
   ...
   IF(IT2.NE.IT1.OR.KPER.EQ.PRJNPER)THEN
    WRITE(IU,'(1X,A)') 'TTSMULT_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = '//TRIM(RTOS(SIM(I1)%TTSMULT,'G',7))
    I1=I2
   ENDIF
  ENDDO
  ...
  I1=1; I2=I1; DO KPER=1,PRJNPER
   ...
   IF(IT2.NE.IT1.OR.KPER.EQ.PRJNPER)THEN
    WRITE(IU,'(1X,A)') 'TTSMAX_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = '//TRIM(RTOS(SIM(I1)%TTSMAX,'G',7))
    I1=I2
   ENDIF
  ENDDO
  ...
  I1=1; I2=I1; DO KPER=1,PRJNPER
   ...
   IF(IT2.NE.IT1.OR.KPER.EQ.PRJNPER)THEN
    WRITE(IU,'(1X,A)') 'MXSTRN_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = '//TRIM(ITOS(SIM(I1)%MXSTRN))
    I1=I2
   ENDIF
  ENDDO
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVERUNWQ_WRTGEN(IU,IDSP,ISSM,IVDF,IRCT,IUDR)
 !####====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IU,IDSP,ISSM,IVDF,IRCT,IUDR
 ...
 WRITE(IU,'(1X,A)') 'RESULT_DIR    =   '//CHAR(39)//TRIM(PBMAN%RESULT_DIR)//CHAR(39)
 ...
 LINE='BTN, GCG, FTL, ADV'
 IF(IDSP.EQ.1)LINE=TRIM(LINE)//', DSP'
 IF(ISSM.EQ.1)LINE=TRIM(LINE)//', SSM'
 IF(IRCT.EQ.1)LINE=TRIM(LINE)//', RCT'
 IF(IVDF.EQ.1)LINE=TRIM(LINE)//', VDF'
 IF(IUDR.EQ.1)LINE=TRIM(LINE)//', UDR'
 WRITE(IU,'(1X,A)') 'PACKAGES      =  '//TRIM(LINE) ! BTN, ADV, SSM, GCG, RCT, FTL' !//TRIM(LINE) 
