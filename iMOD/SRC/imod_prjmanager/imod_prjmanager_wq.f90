...
CONTAINS
 ...
 WRITE(IU,'(1X,A)') 'NPRS =   '//TRIM(ITOS(PBMAN%BTN%NPRS))   ! frans, afleiden uit isave of zo uit tab 3, aantal te bewaren tijdstappen - #TIJDTAPPEN MRET ISAVE
 WRITE(IU,'(1X,A)') 'TIMPRS = '//TRIM(PBMAN%BTN%TIMPRS) ! regel met alle NPRS tijdstappen schrijven 
...
! WRITE(IU,'(1X,A)') 'iRCH_P = '/
 ...
 I1=1; I2=I1; DO KPER=1,PRJNPER
  ...
  IF(DT2.NE.DT1.OR.KPER.EQ.PRJNPER)THEN
   IF(DT1.EQ.0.0D0)WRITE(IU,'(1X,A)') 'SSTR_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = TR'
   IF(DT1.GT.0.0D0)WRITE(IU,'(1X,A)') 'SSTR_P'//TRIM(ITOS(I1))//':'//TRIM(ITOS(I2-1))//' = SS'
   I1=I2
  ENDIF
 ENDDO
