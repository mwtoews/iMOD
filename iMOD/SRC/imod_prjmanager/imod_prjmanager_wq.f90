...
CONTAINS
 ...
 ENDIF
 !IF(TPKST.EQ.1)THEN
 !  IF(.NOT.PMANAGER_SAVERUNWQ_WRTPKST(IU))THEN
 !   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot write PKST package','Error'); CLOSE(IU); RETURN
 !  ENDIF
 !ELSE  
 ...
 WRITE(IU,'(1X,A,999A)') 'TIMPRS = 1.0' !,(TRIM(RTOS(CT(I),'F',2))//',',I=1,N-1),TRIM(RTOS(CT(I),'F',2))
 DEALLOCATE(CT)
 
 WRITE(IU,'(1X,A)') 'NPROBS = '//TRIM(ITOS(PBMAN%BTN%NPROBS))
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'OBS_L?',TOBS,1,1,0,ILAY,0))THEN; ENDIF
 ENDDO
...
! WRITE(IU,'(1X,A)') '#DCHMOC = '
 ...
 N=0; DO ILAY=1,PRJNLAY
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'AL_L?'  ,TDSP,1,1,0,ILAY,0))N=N+1
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'TRPT_L?'  ,TDSP,1,2,0,ILAY,0))N=N+1
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'TRPV_L?'  ,TDSP,1,3,0,ILAY,0))N=N+1
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'DMCOEF_L?',TDSP,1,4,0,ILAY,0))N=N+1
 ENDDO
...
! ENDDO
 ...
 DO KPER=1,PRJNPER 
  DO ISPECIES=1,NSPECIES
   ! If RCH or EVT used, concentration is obliged    
   ! IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CRCH_T?_P?',TRCH,KPER,0,0,1,ISPECIES))N=N+1
   IF(TOPICS(TRCH)%IACT_MODEL.NE.0)THEN
     IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'CRCH_T?_P?',TRCH,KPER,0,0,1,ISPECIES)) THEN
      RETURN ; ELSE ; N=N+1 ; ENDIF ; ENDIF  
   ! IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CEVT_T?_P?',TEVT,KPER,0,0,1,ISPECIES))N=N+1
   IF(TOPICS(TEVT)%IACT_MODEL.NE.0)THEN
     IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'CEVT_T?_P?',TEVT,KPER,0,0,1,ISPECIES)) THEN
      RETURN ; ELSE ; N=N+1 ; ENDIF ; ENDIF  
   DO ILAY=1,PRJNLAY
 ...
 DO ILAY=1,PRJNLAY
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'RHOB_L?'   ,THOB,1,1,0,ILAY,0))N=N+1
 ENDDO    
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'PRSITY2_L?',TPID,1,1,0,ILAY,0))N=N+1
 ENDDO    
...
! CALL PMANAGER_SAVERUNWQ_WRTIDF(IU,TICS,(/'SPECIES1','SPECIES2'/))
 ...
 IF(WQ%RCT%IGETSC.GT.0)THEN
  DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
   IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'SRCONC_T?_L?',TICS,1,1,0,ILAY,ISPECIES))N=N+1
  ENDDO; ENDDO
 ENDIF
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'SP1_T?_L?'   ,TFSC,1,1,0,ILAY,ISPECIES))N=N+1
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'SP2_T?_L?'   ,TSSC,1,1,0,ILAY,ISPECIES))N=N+1
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'RC1_T?_L?'   ,TFOD,1,1,0,ILAY,ISPECIES))N=N+1
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'RC2_T?_L?'   ,TFOS,1,1,0,ILAY,ISPECIES))N=N+1
 ENDDO; ENDDO
 ...
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'IBOUND_L?',TBND,1,1,0,ILAY,0))RETURN
 ENDDO
...
! WRITE(IU,'(1X,A)') 'IRCH_P$ = '
 ...
 IF(TOPICS(TSHD)%DEFINED)CALL PMANAGER_SAVEWQ_OC_ISAVE(IU,'SAVEHEAD_P?_L?',PBMAN%ISAVE(TSHD)%ILAY)
 !WRITE(IU,'(1X,A)') 'SAVEBUDGET_P?_L? = TRUE # default'
 IF(TOPICS(TSHD)%DEFINED)CALL PMANAGER_SAVEWQ_OC_ISAVE(IU,'SAVEBUDGET_P?_L?',PBMAN%ISAVE(TBND)%ILAY)
 !WRITE(IU,'(1X,A)') 'SAVECONCLAYER_L? = TRUE # default'
 IF(TOPICS(TSHD)%DEFINED)CALL PMANAGER_SAVEWQ_OC_ISAVE(IU,'SAVECONCLAYER_L?',PBMAN%ISAVE(TSCO)%ILAY)
 ...
 !####====================================================================
 SUBROUTINE PMANAGER_SAVEWQ_OC_ISAVE(IU,KEYNAME,ISAVE)
 !####====================================================================
 IMPLICIT NONE
 ...
 IF(ASSOCIATED(ISAVE))THEN
  DO ILAY=1,PRJNLAY
   LINE=UTL_CAP(KEYNAME,'U')
   LINE=TRIM(UTL_SUBST(LINE,'_L?','_L'//TRIM(ITOS(ILAY))))
   DO I=1,SIZE(ISAVE)
    IF(ISAVE(I).EQ.ILAY)THEN ; LINE=TRIM(LINE)//'= TRUE' ; EXIT ; ENDIF
   ENDDO
   IF(I.GT.SIZE(ISAVE)) LINE=TRIM(LINE)//'= FALSE'
   WRITE(IU,'(A)') TRIM(LINE)
  ENDDO
 ENDIF
 
 END SUBROUTINE PMANAGER_SAVEWQ_OC_ISAVE
