...
! WRITE(IU,'(A)') 'NLAY = '//TRIM(ITOS(WQFILE%NLAY)) ! PRJNLAY))
 ...
 DO ILAY=1,PRJNLAY
  ITOPIC=TKHV; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'HK_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
 ENDDO 
 DO ILAY=1,PRJNLAY
  ITOPIC=TKVA; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'VKA_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
 ENDDO 
 DO ILAY=1,PRJNLAY-1
  !## vcw is in days --- vcont is on 1/day ...??
  ITOPIC=TVCW; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'VKCB_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
 ENDDO 
 !## transient - storage coefficient
 IF(ISS.EQ.1)THEN
  DO ILAY=1,PRJNLAY
   ITOPIC=TSTO; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SS_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
  ENDDO 
  !## transient - storage coefficient
  ITOPIC=TSPY
  DO ILAY=1,PRJNLAY
   IF(LAYCON(ILAY).NE.1)THEN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SY_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
   ENDIF
  ENDDO 
 ENDIF
 ...
 IF(LBCF)THEN
  DO ILAY=1,PRJNLAY
   ITOPIC=TKDW; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'TRAN_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
  ENDDO 
 ELSE
  DO ILAY=1,PRJNLAY
   ITOPIC=TKHV; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'HY_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
  ENDDO 
 ENDIF
 DO ILAY=1,PRJNLAY-1
  !## vcw is in days --- vcont is on 1/day ...??
  ITOPIC=TVCW; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'VCONT_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
 ENDDO 
 !## transient - storage coefficient
 IF(ISS.EQ.1)THEN
  DO ILAY=1,PRJNLAY
   ITOPIC=TSTO; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SF1_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
  ENDDO 
  !## transient - storage coefficient
  ITOPIC=TSPY
  DO ILAY=1,PRJNLAY
   IF(LAYCON(ILAY).NE.1)THEN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SF2_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
   ENDIF
  ENDDO 
 ENDIF
 ...
 DO ILAY=1,PRJNLAY
  ...
  ENDIF
  ITOPIC=TBOT; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'BOTM_L?',ITOPIC,1,1,ILAY,ILAY,0))RETURN
 ENDDO
