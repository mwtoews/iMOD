...
CONTAINS
 ...
 INTEGER :: IU,KPER,N,ISS,IDSP,ISSM,IVDF,IRCT,IUDR,IOC,IRCH
 LOGICAL :: LEX
 ...
 IF(PBMAN%IFORMAT.EQ.4)THEN
  ...
  IRCH=0 ; IF(.NOT.PMANAGER_SAVERUNWQ_WRTRCH(IU))RETURN  
  ...
  IOC=0 ; IF(.NOT.PMANAGER_SAVERUNWQ_WRTOC(IU)) RETURN
 ELSE
 ...
 IF(.NOT.PMANAGER_SAVERUNWQ_WRTGEN(IU,IDSP,ISSM,IVDF,IRCT,IUDR,IOC,IRCH))THEN
...
! WRITE(IU,'(1X,A)') 'IRCH_P$ = '
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVERUNWQ_WRTGEN(IU,IDSP,ISSM,IVDF,IRCT,IUDR,IOC,IRCH)
 !####====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IU,IDSP,ISSM,IVDF,IRCT,IUDR,IOC,IRCH
 ...
 LINE='BTN, GCG, ADV'
 IF(PBMAN%IFORMAT.EQ.4) LINE=TRIM(LINE)//', BAS6, DIS, PCG'
 IF(PBMAN%IFORMAT.EQ.5) LINE=TRIM(LINE)//', FTL'
 ...
 IF(IOC.EQ.1)LINE=TRIM(LINE)//', OC'
 IF(IRCH.EQ.1)LINE=TRIM(LINE)//', RCH'
