...
CONTAINS
 ...
 IF(.NOT.PMANAGER_SAVERUNWQ_WRTDSP(IU))THEN; ENDIF
 IF(.NOT.PMANAGER_SAVERUNWQ_WRTSSM(IU))THEN; ENDIF
 IF(.NOT.PMANAGER_SAVERUNWQ_WRTVDF(IU))THEN; ENDIF

 IF(.NOT.PMANAGER_SAVERUNWQ_WRTRCT(IU))THEN; ENDIF
 IF(.NOT.PMANAGER_SAVERUNWQ_WRTUDR(IU))THEN; ENDIF

 !## write Start Packages
 IF(.NOT.PMANAGER_SAVERUNWQ_WRTGEN(IU))THEN
 ...
 WRITE(IU,'(1X,A)') 'SAVUCN = '//TRIM(LTOS(PBMAN%BTN%SAVUCN,1))//' # default'
 
 N=0; DO KPER=1,PRJNPER; IF(SIM(KPER)%ISAVE.EQ.1)N=N+1; ENDDO
 WRITE(IU,'(1X,A)') 'NPRS =   '//TRIM(ITOS(N))
 ALLOCATE(CT(0:N)) 
 CT=0.0D0; N=0; DO KPER=1,PRJNPER
  ...
  ENDIF
 ENDDO
 WRITE(IU,'(1X,A,999F10.2)') 'TIMPRS = ',(CT(I),I=1,N)
 DEALLOCATE(CT)
 ...
 ENDDO
 WRITE(IU,'(1X,A)') 'CHKMAS = '//TRIM(LTOS(PBMAN%BTN%CHKMAS,1))//' # default'
 ...
 CALL PMANAGER_SAVERUNWQ_WRT_SIMTIME(IU,LEX)
 ...
 WRITE(IU,'(1X,A)') 'PERCEL =  '//TRIM(UTL_REALTOSTRING(PBMAN%ADV%PERCEL))
 ...
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'RHOB_L?'   ,THOB,0,1,0,ILAY,0))RETURN
 ENDDO    
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'PRSITY2_L?',TPID,0,1,0,ILAY,0))RETURN
 ENDDO    

 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SRCONC_T?_L?',TICS,0,1,0,ILAY,ISPECIES))RETURN
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SP1_T?_L?'   ,TFSC,0,1,0,ILAY,ISPECIES))RETURN
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SP2_T?_L?'   ,TSSC,0,1,0,ILAY,ISPECIES))RETURN
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'RC1_T?_L?'   ,TFOD,0,1,0,ILAY,ISPECIES))RETURN
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'RC2_T?_L?'   ,TFOS,0,1,0,ILAY,ISPECIES))RETURN
 ENDDO; ENDDO
