...
! WRITE(IU,'(1X,A)') '#DCHMOC = '
 ...
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'RHOB_L?'   ,THOB,0,1,0,ILAY,0))THEN; ENDIF
 ENDDO    
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'PRSITY2_L?',TPID,0,1,0,ILAY,0))THEN; ENDIF
 ENDDO    

 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SRCONC_T?_L?',TICS,0,1,0,ILAY,ISPECIES))THEN; ENDIF
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SP1_T?_L?'   ,TFSC,0,1,0,ILAY,ISPECIES))THEN; ENDIF
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SP2_T?_L?'   ,TSSC,0,1,0,ILAY,ISPECIES))THEN; ENDIF
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'RC1_T?_L?'   ,TFOD,0,1,0,ILAY,ISPECIES))THEN; ENDIF
 ENDDO; ENDDO
 DO ISPECIES=1,NSPECIES; DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'RC2_T?_L?'   ,TFOS,0,1,0,ILAY,ISPECIES))THEN; ENDIF
 ENDDO; ENDDO
...
! WRITE(IU,'(1X,A)') 'iRCH_P = '/
 ...
 WRITE(IU,'(1X,A)') 'MODELNAME = TEST'  !//CHAR(39)//TRIM(PBMAN%RESULT_DIR)//CHAR(39)            
...
!     CASE (TBTN, TGCG, TRCT, TVDF)  !(SSM, DSP, TUDR)
         IF(TOPICS(ITOPIC)%IACT_MODEL.GT.0) LINE=TRIM(LINE)//TRIM(TOPICS(ITOPIC)%CMOD)//','   
    END SELECT
   ENDDO
 END SELECT
 LINE=(LINE(1:INDEX(LINE,',',.TRUE.)-1))  ! remove last comma
 WRITE(IU,'(1X,A)') 'PACKAGES      =   BTN, ADV, SSM, GCG, RCT, FTL' !//TRIM(LINE) 
