...
CONTAINS
 ...
 !## get modelname
 IF(IBATCH.EQ.1)THEN
   PBMAN%MODELNAME=TRIM(PBMAN%RUNFILE(INDEX(PBMAN%RUNFILE,'\',.TRUE.)+1:))
   PBMAN%MODELNAME=TRIM(PBMAN%MODELNAME(:INDEX(PBMAN%MODELNAME,'.',.TRUE.)-1))
 ENDIF   
 ...
 !IF(TPKSF.EQ.1)THEN
 !  IF(.NOT.PMANAGER_SAVERUNWQ_WRTPKSF(IU))THEN
 !   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot write PKSF package','Error'); CLOSE(IU); RETURN
 !  ENDIF
 !ELSE  
...
! ENDDO
 ...
 DO KPER=1,PRJNPER ; DO ISPECIES=1,NSPECIES
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CRCH_T?_P?',TRCH,KPER,0,0,1,ISPECIES))N=N+1
  IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CEVT_T?_P?',TEVT,KPER,0,0,1,ISPECIES))N=N+1
  DO ILAY=1,PRJNLAY
   IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CCHD_T?_P?_L?',TCHD,KPER,0,0,ILAY,ISPECIES))N=N+1
   IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CWEL_T?_P?_L?',TWEL,KPER,0,0,ILAY,ISPECIES))N=N+1
 ! IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CDRN_T?_P?_L?',TDRN,KPER,0,0,ILAY,ISPECIES))N=N+1   TOPIC Drain does not allow to add Species Concentration !
   IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CRIV_T?_P?_L?',TRIV,KPER,0,0,ILAY,ISPECIES))N=N+1
   IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CGHB_T?_P?_L?',TGHB,KPER,0,0,ILAY,ISPECIES))N=N+1
 ! IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CMAL_T?_P?_L?',TTVC,KPER,0,0,ILAY,ISPECIES))N=N+1   For now decided to have the Default
   IF(PMANAGER_SAVERUNWQ_U2DREL(IU,'CTVC_T?_P?_L?',TTVC,KPER,0,0,ILAY,ISPECIES))N=N+1
  ENDDO    
 ENDDO; ENDDO
...
! CALL PMANAGER_SAVERUNWQ_WRTIDF(IU,TICS,(/'SPECIES1','SPECIES2'/))
 ...
 WRITE(IU,'(1X,A)') 'ISOTHM = '//TRIM(ITOS(WQ%RCT%ISOTHM))
 WRITE(IU,'(1X,A)') 'IREACT = '//TRIM(ITOS(WQ%RCT%IREACT))
 WRITE(IU,'(1X,A)') 'NCRXNDATA = ...  # maximum = 7'
 WRITE(IU,'(1X,A)') 'NVRXNDATA = ...  # maximum = 4'
 WRITE(IU,'(1X,A)') 'ISOLVER = ...'
 WRITE(IU,'(1X,A)') 'IRCTOP = '//TRIM(ITOS(WQ%RCT%IRCTOP))//' # default'
 ...
 WRITE(IU,'(1X,A)') 'ATOL_T? = ...'
 WRITE(IU,'(1X,A)') 'RTOL_T? = ...'
 ! NCRXNDATA number of times
 WRITE(IU,'(1X,A)') 'RC1 = ...'
 WRITE(IU,'(1X,A)') 'RC2 = ...'
 WRITE(IU,'(1X,A)') 'RC3 = ...'
 WRITE(IU,'(1X,A)') 'RC4 = ...'
 ...
 WRITE(IU,'(1X,A)') 'RC6 = ...'
 WRITE(IU,'(1X,A)') 'RC7 = ...'
 ! NVRXNDATA number of times
 WRITE(IU,'(1X,A)') 'VRC1_L? = ...'
 WRITE(IU,'(1X,A)') 'VRC2_L? = ...'
 WRITE(IU,'(1X,A)') 'VRC3_L? = ...'
 WRITE(IU,'(1X,A)') 'VRC4_L? = ...'
 ...
 MCHDSYS=0
 DO IPER=1,PRJNPER 
  NSYS=SIZE(TOPICS(TCHD)%STRESS(IPER)%FILES,2)
  MCHDSYS=MAX(MCHDSYS,NSYS)
  DO ISYS=1,NSYS 
   DO ILAY=1,PRJNLAY 
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU, 'SHEAD_P?_S?_L?',TCHD,IPER,1,ISYS,ILAY,1))RETURN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'ESHEAD_P?_S?_L?',TCHD,IPER,2,ISYS,ILAY,1))RETURN
   ENDDO
  ENDDO 
 ENDDO

 WRITE(IU,'(1X,A,I3)') 'MCHDSYS = ',MCHDSYS
 ...
 MWELSYS=0
 DO IPER=1,PRJNPER 
  NSYS=SIZE(TOPICS(TWEL)%STRESS(IPER)%FILES,2)
  MWELSYS=MAX(MWELSYS,NSYS)
  DO ISYS=1,NSYS 
   DO ILAY=1,PRJNLAY 
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'WEL_P?_S?_L?',TWEL,IPER,1,ISYS,ILAY,1))RETURN
   ENDDO
  ENDDO 
 ENDDO
 WRITE(IU,'(1X,A,I3)') 'MWELSYS = ',MWELSYS
 ...
 MGHBSYS=0
 DO IPER=1,PRJNPER 
  NSYS=SIZE(TOPICS(TGHB)%STRESS(IPER)%FILES,2)
  MGHBSYS=MAX(MGHBSYS,NSYS)
  DO ISYS=1,NSYS 
   DO ILAY=1,PRJNLAY 
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,     'BHEAD_P?_S?_L?',TGHB,IPER,1,ISYS,ILAY,1))RETURN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,      'COND_P?_S?_L?',TGHB,IPER,2,ISYS,ILAY,1))RETURN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'GHBSSMDENS_P?_S?_L?',TGHB,IPER,3,ISYS,ILAY,1))RETURN
   ENDDO
  ENDDO 
 ENDDO
 WRITE(IU,'(1X,A,I)') 'MGHBSYS = ',MGHBSYS 
 ...
 MRIVSYS=0
 DO IPER=1,PRJNPER 
  NSYS=SIZE(TOPICS(TRIV)%STRESS(IPER)%FILES,2)
  MRIVSYS=MAX(MRIVSYS,NSYS)
  DO ISYS=1,NSYS 
   DO ILAY=1,PRJNLAY 
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,     'STAGE_P?_S?_L?',TRIV,IPER,1,ISYS,ILAY,1))RETURN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,      'COND_P?_S?_L?',TRIV,IPER,2,ISYS,ILAY,1))RETURN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,      'RBOT_P?_S?_L?',TRIV,IPER,3,ISYS,ILAY,1))RETURN
    !IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'RIVSSMDENS_P?_S?_L?',TRIV,IPER,5,ISYS,ILAY,1))RETURN  ! not available. Needs 5th subtopic?
   ENDDO
  ENDDO 
 ENDDO
 WRITE(IU,'(1X,A,I)') 'MRIVSYS = ',MRIVSYS 
...
! WRITE(IU,'(1X,A)') 'OPTION = ...'
 ...
 MDRNSYS=0
 DO IPER=1,PRJNPER 
  NSYS=SIZE(TOPICS(TDRN)%STRESS(IPER)%FILES,2)
  MDRNSYS=MAX(MDRNSYS,NSYS)
  DO ISYS=1,NSYS 
   DO ILAY=1,PRJNLAY 
  ...
  ENDDO 
 ENDDO
 WRITE(IU,'(1X,A,I)') 'MDRNSYS = ',MDRNSYS 
...
! WRITE(IU,'(1X,A)') 'IRCH_P$ = '
 ...
 DO IPER=1,PRJNPER 
  DO ILAY=1,PRJNLAY 
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,  'EVTR_P?',TEVT,IPER,1,1,ILAY,1))RETURN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,  'SURF_P?',TEVT,IPER,2,1,ILAY,1))RETURN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,  'EXDP_P?',TEVT,IPER,3,1,ILAY,1))RETURN
  ENDDO
 ENDDO 
 ...
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'HK_L?',TKHV,1,1,0,ILAY,0))RETURN
 ENDDO 
 DO ILAY=1,PRJNLAY
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'VKA_L?',TKVA,1,1,0,ILAY,0))RETURN
 ENDDO 

 IF(TOPICS(TVCW)%IACT_MODEL.EQ.1)THEN
  DO ILAY=1,PRJNLAY-1
   !## vcw is in days --- vcont is on 1/day ...??
   IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'VKCB_L?',TVCW,1,1,0,ILAY,0))RETURN
  ENDDO 
 ENDIF 
 !## transient - storage coefficient
 IF(ISS.EQ.1)THEN
  DO ILAY=1,PRJNLAY
   IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SS_L?',TSTO,1,1,0,ILAY,0))RETURN
  ENDDO 
  !## transient - storage coefficient
  DO ILAY=1,PRJNLAY
   IF(LAYCON(ILAY).NE.1)THEN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SY_L?',TSPY,1,1,0,ILAY,0))RETURN
   ENDIF
  ENDDO 
 ENDIF
 ...
 IF(LBCF)THEN
  DO ILAY=1,PRJNLAY
   IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'TRAN_L?',TKDW,1,1,0,ILAY,0))RETURN
  ENDDO 
 ELSE
  DO ILAY=1,PRJNLAY
   IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'HY_L?',TKHV,1,1,0,ILAY,0))RETURN
  ENDDO 
 ENDIF
 DO ILAY=1,PRJNLAY-1
  !## vcw is in days --- vcont is on 1/day ...??
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'VCONT_L?',TVCW,1,1,0,ILAY,0))RETURN
 ENDDO 
 !## transient - storage coefficient
 IF(ISS.EQ.1)THEN
  DO ILAY=1,PRJNLAY
   IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SF1_L?',TSTO,1,1,0,ILAY,0))RETURN
  ENDDO 
  !## transient - storage coefficient
  DO ILAY=1,PRJNLAY
   IF(LAYCON(ILAY).NE.1)THEN
    IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'SF2_L?',TSPY,1,1,0,ILAY,0))RETURN
   ENDIF
  ENDDO 
 ENDIF
 ...
 DO ILAY=1,PRJNLAY
  ITOPIC=TBND; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'IBOUND_L?',ITOPIC,1,1,0,ILAY,0))RETURN
 ENDDO

 WRITE(IU,'(A)') 'HNOFLO    = -9999.0  # default'

 DO ILAY=1,PRJNLAY
  ITOPIC=TSHD; IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'STRT_L?',ITOPIC,1,1,0,ILAY,0))RETURN
 ENDDO
 ...
 DO ILAY=1,PRJNLAY
  IF(ILAY.EQ.1)THEN
   IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'TOP',TTOP,1,1,0,ILAY,0))RETURN
  ENDIF
  IF(.NOT.PMANAGER_SAVERUNWQ_U2DREL(IU,'BOTM_L?',TBOT,1,1,0,ILAY,0))RETURN
 ENDDO
 ...
 ! * IPER=0,  always in case of parameter with no time association (e.g. TOP_LAY1)
 !
 ! Layers or Systems are both stored in the same list; TOPICS(*)%STRESS(*)%FILES(*,Lay/Sys)
 ! A Topic is Layer-based or System-based. 
 ! * ISYS=0,  only in case TOPIC is Layer-based (1 per layer) like TOP, BOT, KHV etc.
 ! * ILAY=0,  only in case TOPIC is System-based like RIV, DRN etc.
 !
 ! Sub Topics (isubt) and Species (ISPEC) are stored in the samen list; TOPICS(*)%STRESS(*)%FILES(Top+Spec,*)
 ! * ISUBT=0, only in case ISPEC > 0
 ! * ISPEC=0, always in case of parameter with no association to concentration
 ...
 !## Loop over full list to find the right system/layer
 ...
 DO JSYS=1,NSYS
  ...
  IF(ILAY.NE.0.AND.JLAY.EQ.ILAY) EXIT ! found right Layer-based parameter
  IF(ISYS.NE.0.AND.JSYS.EQ.ISYS) EXIT ! found right System-based parameter
