   ...
   CASE(FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_CHECK2)
      ...
      CALL WDIALOGFIELDSTATE(IDF_LABEL21,PBMAN%IPESTP)
      CALL WDIALOGFIELDSTATE(IDF_LABEL22,PBMAN%IPESTP)
      CALL WDIALOGFIELDSTATE(IDF_INTEGER14,PBMAN%IPESTP)
     END SELECT
    SELECT CASE (MESSAGE%VALUE1)
    END SELECT
...
! ENDIF
 ...
 DO ILAY=IL1,IL2
  ...
  SELECT CASE (ITYPE)
   ...
   CASE (3,4,6)
    IF(ILAY.LT.PRJMXNLAY)CALL MANAGER_UTL_ADDFILE(VCW(ILAY)%FNAME)
   CASE (7)
    CALL MANAGER_UTL_ADDFILE(KHV(ILAY)%FNAME)
   CASE (8)
    IF(ILAY.LT.PRJMXNLAY)CALL MANAGER_UTL_ADDFILE(KVV(ILAY)%FNAME)
  END SELECT
