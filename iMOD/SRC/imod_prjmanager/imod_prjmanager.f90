    ...
    DO J=1,NF   !## number of periods (types)
     DO K=1,MF  !## number of files (systems)
      ...
      SELECT CASE (I)
       ...
       CASE (TGCG)
        STRING=TRIM(STRING)//TRIM(RTOS(PBMAN%WQ%GCG%CCLOSE,'G',5))//';'//TRIM(ITOS(PBMAN%WQ%GCG%MXITER))
!       CASE (TRCT)
!        STRING=TRIM(STRING)//TRIM(RTOS(RCT%HCLOSE,'G',5))//';'//TRIM(ITOS(RCT%MUT))
       CASE (TADV)
        STRING=TRIM(STRING)//TRIM(RTOS(PBMAN%WQ%ADV%WD,'G',5))//';'//TRIM(ITOS(PBMAN%WQ%ADV%MIXELM))
       CASE (TVDF)
        STRING=TRIM(STRING)//TRIM(RTOS(PBMAN%WQ%VDF%DENSEMAX,'G',5))//';'//TRIM(ITOS(PBMAN%WQ%VDF%MTDNCONC))
       CASE DEFAULT
