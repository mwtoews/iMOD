...
!       CALL PMANAGER_UTL_UPDATE(1,1,1,1,0)
 ...
 SELECT CASE (ITOPIC)
  CASE (TPST,TPCG,TGCG,TRCT,TADV,TVDF)
   !## pst=settings
    ...
    DO J=1,NF   !## number of periods (types)
     DO K=1,MF  !## number of files (systems)
      ...
      SELECT CASE (I)
       ...
       CASE (TGCG)
        STRING=TRIM(STRING)//TRIM(RTOS(GCG%HCLOSE,'G',5))//';'//TRIM(ITOS(GCG%MUT))
       CASE (TRCT)
       ...
       CASE (TVDF)
        STRING=TRIM(STRING)//TRIM(RTOS(VDF%HCLOSE,'G',5))//';'//TRIM(ITOS(VDF%MUT))
       CASE DEFAULT
