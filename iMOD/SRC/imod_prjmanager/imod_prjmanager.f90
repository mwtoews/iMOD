...
!  ENDDO
 ...
 INTEGER :: IU,ITOPIC,IRUN,ILOGFILE,ISUB,I
   ...
   CASE (3)
    ...
    IF(PBMAN%IGENMF6.EQ.1)THEN
     IF(LEN_TRIM(PBMAN%OUTPUT).EQ.0)THEN
      DIR=PBMAN%RUNFILE(:INDEX(PBMAN%RUNFILE,'\',.TRUE.)-1)
     ELSE
      DIR=TRIM(PBMAN%OUTPUT)
     ENDIF
     DIR=TRIM(DIR)//'\GEN'
     !## generate them per layer
     IF(ASSOCIATED(PBMAN%SM(1)%ILAY))THEN
      DO I=1,PRJNLAY
       CALL PMANAGER_GENERATEMFNETWORKS(PBMAN%GENFNAME,DIR,PBMAN%NSUBMODEL,IBATCH,I)
      ENDDO
    ...
    DO ISUB=1,PBMAN%NSUBMODEL
     ...
     IF(ASSOCIATED(PBMAN%SM(ISUB)%ILAY))THEN
