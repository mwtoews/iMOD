...
USE MODPLOT
USE DATEVAR
USE MOD_IPEST_GLM, ONLY : IPEST_GLM_MAIN,IPEST_GLM_RESET_PARAMETER,IPEST_GLM_CREATE_RESIDUALSFILES
...
! IF(PRJ(1)%ICNST.EQ.0)CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,1)
 ...
 CALL WGRIDLABELCOLUMN(IDF_GRID1,0,'number of files: '//TRIM(ITOS(N)))
 ...
 !## depending ID input this function:
 !## - opens an existing Runfile
 !## - starts the Simulation manager Window. 
 ...
 ELSEIF(ID.EQ.ID_SAVERUN)THEN
  ...
  SELECT CASE (PBMAN%IFORMAT)
   ...
   !## mf2005
   CASE (2)
    IF(ABS(PBMAN%IEXPORTMF2005).EQ.1)THEN
    ...
    ELSE
     ...
     WRITE(*,'(/A/)') 'Reusing existing exported-files for MF2005 files:'//TRIM(PBMAN%RUNFILE)
    ENDIF
    PMANAGERRUN=.TRUE.; CALL PMANAGER_SAVEMF2005_DEALLOCATE()
   !## mf6
   CASE (3)
    IF(ABS(PBMAN%IEXPORTMF2005).EQ.1)THEN
     ...
     ENDDO
     IF(ASSOCIATED(PBMAN%SM))DEALLOCATE(PBMAN%SM)
    ELSE
     CALL IPEST_GLM_SETGROUPS(); PMANAGERRUN=.TRUE.; CALL PMANAGER_SAVEMF2005_DEALLOCATE()
     WRITE(*,'(/A/)') 'Reusing existing exported-files for MF2005 files:'//TRIM(PBMAN%RUNFILE)
    ENDIF
   !##  SeaWAT/MT3D
   CASE (4,5)
...
!  DO I=1,MAXTOPICS; IF(ASSOCIATED(PBMAN%ISAVE(I)%ILAY))DEALLOCATE(PBMAN%ISAVE(I)%ILAY); ENDDO
 ...
 CHARACTER(LEN=256) :: DIR,DIRNAME,SEXENAME,FNAME
 CHARACTER(LEN=52) :: MNAME
 INTEGER :: IU,IOS,I,II,J,K,N1,N2,IFLAGS,IEXCOD,IERROR,IMODE,I1
 ...
 IF(TOPICS(TCAP)%DEFINED.AND.PBMAN%IFORMAT.NE.3)THEN
  N1=1; N2=1; IF(PBMAN%IPESTP.EQ.1)THEN; N1=-PBMAN%NLAMBDASEARCH; N2=SIZE(PEST%PARAM); ENDIF
  DO I=N1,N2
   !## skip zero
   IF(I.EQ.0)CYCLE
   !## simulate batch-file
...
! IF(PMANAGERINI(1,"")) THEN; ENDIF
 ...
 DO I=N1,N2
  ...
  IF(PBMAN%IPESTP+PBMAN%IIES.EQ.0)THEN
   FNAME=TRIM(DIR)//'\RUN.BAT'
  ELSEIF(PBMAN%IPESTP.EQ.1)THEN
   IF(I.GT.0)THEN
    IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE
    !## mf6
    IF(PBMAN%IFORMAT.EQ.3)THEN
     FNAME=TRIM(DIR)//'\IPEST_P#'//TRIM(ITOS(I))//'\RUN_P#'//TRIM(ITOS(I))//'.BAT'
    ELSE
     FNAME=TRIM(DIR)//'\RUN_P#'//TRIM(ITOS(I))//'.BAT'
    ENDIF
   ELSE
    IF(PBMAN%IFORMAT.EQ.3)THEN
     FNAME=TRIM(DIR)//'\IPEST_L#'//TRIM(ITOS(ABS(I)))//'\RUN_L#'//TRIM(ITOS(ABS(I)))//'.BAT'
    ELSE
     FNAME=TRIM(DIR)//'\RUN_L#'//TRIM(ITOS(ABS(I)))//'.BAT'
    ENDIF
   ENDIF
  ELSEIF(PBMAN%IIES.EQ.1)THEN
   IF(PBMAN%IFORMAT.EQ.3)THEN
    FNAME=TRIM(DIR)//'\IPEST_R#'//TRIM(ITOS(I))//'\RUN_R#'//TRIM(ITOS(I))//'.BAT'
   ELSE
    FNAME=TRIM(DIR)//'\RUN_R#'//TRIM(ITOS(I))//'.BAT'
   ENDIF
  ENDIF
  CALL OSD_OPEN(IU,FILE=FNAME,STATUS='REPLACE',ACTION='WRITE,DENYREAD',IOSTAT=IOS)
  ...
  IF(IMODE.EQ.1)THEN
   ...
   ELSE
    ...
    ELSEIF(PBMAN%IPESTP.EQ.1)THEN
     IF(PBMAN%IFORMAT.EQ.3)THEN
      WRITE(IU,'(A)') 'CD '//FNAME(:INDEX(FNAME,'\',.TRUE.)-1)
      WRITE(IU,'(/A/)') '"'//TRIM(SEXENAME)
     ELSE
      IF(I.GT.0)THEN
    ...
    IF(PBMAN%IFORMAT.EQ.3)THEN
     ...
     DO J=1,PBMAN%NSUBMODEL
      ...
      IF(PRJNPER.GT.1)THEN
       ...
       ENDDO
      ENDIF
      !## write layers to be saved
      CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TSHD)%ILAY,'SAVESHD',IU)
      CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TWEL)%ILAY,'SAVEWEL',IU)
      CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TDRN)%ILAY,'SAVEDRN',IU)
      CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TRIV)%ILAY,'SAVERIV',IU)
      CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TRCH)%ILAY,'SAVERCH',IU)
      CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TEVT)%ILAY,'SAVEEVT',IU)
      CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TGHB)%ILAY,'SAVEGHB',IU)
      CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TCHD)%ILAY,'SAVECHD',IU)
      I1=0; DO II=1,SIZE(TFLX) 
       IF(ASSOCIATED(PBMAN%ISAVE(TFLX(II))%ILAY))THEN 
        SELECT CASE (TFLX(II))
         CASE (TSTO)
          CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TFLX(II))%ILAY,'SAVESTO',IU)
         CASE (TSPY)
          CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TFLX(II))%ILAY,'SAVESPY',IU)
         CASE DEFAULT
          IF(I1.EQ.0)THEN
           CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TFLX(II))%ILAY,'SAVEFLX',IU); I1=1
          ENDIF
        END SELECT
       ENDIF
      ENDDO
...
!     CALL UTL_GETHELP('7.8','TMO.ModSim.SolverSettings')
 ...
 !## fill in values
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,PEST%IPESTP)
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK3,PEST%CMDHIDE)
 PEST%NCPU=MAX(1,PEST%NCPU); CALL WDIALOGPUTINTEGER(IDF_INTEGER13,PEST%NCPU)
 PEST%NLAMBDASEARCH=MAX(1,PEST%NLAMBDASEARCH); CALL WDIALOGPUTINTEGER(IDF_INTEGER14,PEST%NLAMBDASEARCH)
 ...
 !## read values
 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  CALL WDIALOGGETCHECKBOX(IDF_CHECK2,PEST%IPESTP)
  CALL WDIALOGGETCHECKBOX(IDF_CHECK3,PEST%CMDHIDE)
  CALL WDIALOGGETINTEGER(IDF_INTEGER13,PEST%NCPU)
  CALL WDIALOGGETINTEGER(IDF_INTEGER14,PEST%NLAMBDASEARCH)
...
! ENDIF
 ...
 SELECT CASE (IOPTION)
  ...
  CASE (1)
   ...
   PRJILIST(1)=TTOP; PRJILIST(2)=TBOT
   
  !## TOP1 - KDW1 - BOT1 - TOP2 - KDW2 - BOT2 ...
  CASE (2)
   ALLOCATE(FNAMES(NLAY*3),PRJILIST(3))
   PRJILIST(1)=TTOP; PRJILIST(2)=TKDW; PRJILIST(3)=TBOT

  !## TOP1 - KDW1 - BOT1 - VCW1 - TOP2 - KDW1- BOT2 - VCW2 - TOP3 ...
  CASE (3)
   ALLOCATE(FNAMES(NLAY*4-1),PRJILIST(4))
   PRJILIST(1)=TTOP; PRJILIST(2)=TKDW; PRJILIST(3)=TBOT; PRJILIST(4)=TVCW

  !## TOP1 - BOT1 - VCW1 - TOP2 - BOT2 - VCW2 - TOP3 ...
  CASE(4)
   ALLOCATE(FNAMES(NLAY*3-1),PRJILIST(3))
   PRJILIST(1)=TTOP; PRJILIST(2)=TBOT; PRJILIST(3)=TVCW

  !## TOP1 - SHD1 - BOT1 - TOP2 - SHD2 - BOT2 ...
  CASE (5)
   ALLOCATE(FNAMES(NLAY*3),PRJILIST(3))
   PRJILIST(1)=TTOP; PRJILIST(2)=TSHD; PRJILIST(3)=TBOT

  !## TOP1 - KHV1 - BOT1 - TOP2 - KHV2 - BOT2 ...
  CASE (6)
   ALLOCATE(FNAMES(NLAY*3),PRJILIST(3))
   PRJILIST(1)=TTOP; PRJILIST(2)=TKHV; PRJILIST(3)=TBOT

  !## TOP1 - BOT1 - KVV1 - TOP2 - BOT2 - KVV2 - TOP3 ...
  CASE(7)
   ALLOCATE(FNAMES(NLAY*3-1),PRJILIST(3))
   PRJILIST(1)=TTOP; PRJILIST(2)=TBOT; PRJILIST(3)=TKVV

  !## TOP1 - KHV1 - BOT1 - KVV1 - TOP2 - KHV2 - BOT2 - KVV2 - TOP3 ...
  CASE (8)
   ALLOCATE(FNAMES(NLAY*4-1),PRJILIST(4))
   PRJILIST(1)=TTOP; PRJILIST(2)=TKHV; PRJILIST(3)=TBOT; PRJILIST(4)=TKVV
    ...
    DO J=1,NF   !## number of periods (types)
     DO K=1,MF  !## number of files (systems)
      ...
      SELECT CASE (I)
       !## pst-settings
       CASE (TPST)
        IF(ASSOCIATED(PEST%MEASURES))THEN; STRING='nmeasures='//TRIM(ITOS(SIZE(PEST%MEASURES)))
