...
!  DO I=1,MAXTOPICS; IF(ASSOCIATED(PBMAN%ISAVE(I)%ILAY))DEALLOCATE(PBMAN%ISAVE(I)%ILAY); ENDDO
 ...
 INTEGER :: IU,IOS,I,II,J,K,N1,N2,IFLAGS,IEXCOD,IERROR,IMODE,I1,I2
   ...
   IF(PBMAN%IPEST+PBMAN%IPESTP.EQ.0)THEN
    ...
    IF(PBMAN%IFORMAT.EQ.3)THEN
     ...
     DO J=1,PBMAN%NSUBMODEL
      ...
      I1=0; I2=0; DO II=1,SIZE(TFLX) 
       IF(ASSOCIATED(PBMAN%ISAVE(TFLX(II))%ILAY))THEN 
        SELECT CASE (TFLX(II))
         CASE (TSTO,TSPY)
          IF(I1.EQ.0)THEN
           CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TFLX(II))%ILAY,'SAVESTO',IU); I1=1
          ENDIF
         CASE DEFAULT
          IF(I2.EQ.0)THEN
           CALL PMANAGER_SAVEMF2005_MF6TOIDF_ISAVE(PBMAN%ISAVE(TFLX(II))%ILAY,'SAVEFLX',IU); I2=1
          ENDIF
        END SELECT
       ENDIF
      ENDDO
