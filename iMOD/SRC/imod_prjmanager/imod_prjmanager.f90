  ...
  CASE (ID_DPMANAGER)
   SELECT CASE (ITYPE)
    CASE (TABCHANGED)
     ...
     IF(MESSAGE%VALUE2.EQ.ID_DPMANAGER_TAB1)CALL PMANAGER_UTL_UPDATE(0,0,0)
    CASE (PUSHBUTTON)
...
!   IF(ITOPIC.EQ.TRCT)CALL PMANAGEROPEN_RCT()
 ...
 ENDIF
 CALL PMANAGEROPEN_NOENTRIES(TOPICS(ITOPIC)%NSUBTOPICS)
...
! IF(PRJ(1)%ICNST.EQ.0)CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,1)
 ...
 !###======================================================================
 SUBROUTINE PMANAGEROPEN_NOENTRIES(ITOPIC)
 !###======================================================================
 IMPLICIT NONE
...
! END SUBROUTINE PMANAGER_UTL_UPDATE
 ...
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IDITOPIC,IDIPER,IDISUBS
 INTEGER :: IPER,I,J,K,N,IDTOPIC,IDSUBTC,IFILES,NF,MF,JD,IMF,ITOPIC
 ...
 CAll WDialogGetMenuString(IDF_MENU1,IMF,STRING); IMF=IMF-1
 CALL WDIALOGSELECT(ID_DPMANAGER_TAB1); CALL WDIALOGCLEARFIELD(ID_TREEVIEW1)
 CALL WDIALOGTREEVIEWCHECK(0)
 CALL WDialogTitle('TreeView-'//TRIM(STRING))


 IDTOPIC=1000-1; IDSUBTC=2000-1
 I=0 
 IFILES=0; DO ITOPIC=1,SIZE(TOPICS)
  
  IF(IMF.GT.0)THEN
   DO J=1,SIZE(MC(IMF)%T)
       !write(*,'(a,i,a,i,i)') 'imf, topic name,j van jmax:', imf, TOPICS(ITOPIC)%TNAME,j,SIZE(MC(IMF)%T)

       IF(MC(IMF)%T(J).EQ.ITOPIC)then 
        !   write(*,'(a,i,i,a)') 'i, MC(IMF)%T(J),naam:', ITOPIC,MC(IMF)%T(J),TOPICS(MC(IMF)%T(J))%TNAME   
           EXIT
       ENDIF    
