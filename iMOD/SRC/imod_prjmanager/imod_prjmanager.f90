  ...
  SELECT CASE (PBMAN%IFORMAT)
     ...
     IF(PMANAGER_SAVEMF2005(FNAME,IBATCH))THEN
      IF(IBATCH.EQ.0)THEN
       ...
       IF(ISUB.EQ.PBMAN%NSUBMODEL.AND.IRUN.EQ.0)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK, &
          'Successfully written MODFLOW6 Namfile:'//CHAR(13)//TRIM(FNAME)//CHAR(13)//CHAR(13),'Information')
      ELSE
   ...
   CASE (4,5)
    IF(PMANAGER_SAVERUNWQ(FNAME,IBATCH))THEN
     IF(IBATCH.EQ.0)THEN
      IF(IRUN.EQ.0)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully written SEAWAT runfile:'//CHAR(13)//TRIM(FNAME),'Information')
     ELSE
