...
!  ENDDO
 ...
 INTEGER :: IU,ITOPIC,IRUN,ILOGFILE
 ...
 ELSEIF(ID.EQ.ID_SAVERUN)THEN
  ...
  ELSEIF(PBMAN%IFORMAT.EQ.2.OR.PBMAN%IFORMAT.EQ.3)THEN
   IF(PMANAGER_SAVEMF2005(FNAME,IBATCH))THEN
    IF(IBATCH.EQ.0)THEN
     IF(IRUN.EQ.0)THEN
      IF(PBMAN%IFORMAT.EQ.2)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully written MF2005 files:'//CHAR(13)//TRIM(FNAME),'Information')
      IF(PBMAN%IFORMAT.EQ.3)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully written MF6 files:'//CHAR(13)//TRIM(FNAME),'Information')
     ENDIF
    ELSE
     IF(PBMAN%IFORMAT.EQ.2)WRITE(*,'(/A/)') 'Successfully written MF2005 files:'//TRIM(FNAME)
     IF(PBMAN%IFORMAT.EQ.3)WRITE(*,'(/A/)') 'Successfully written MF6 files:'//TRIM(FNAME)
    ENDIF
    PMANAGERRUN=.TRUE.
   ENDIF
  ...
  IF(PBMAN%NSUBMODEL.EQ.PBMAN%ISUBMODEL)THEN
...
! J=J*II; L=L*II
 ...
 ELSEIF(IPER.GT.0.AND.ISYS.EQ.0.AND.ISUBTOPIC.GT.0)THEN
  ...
  ENDDO
 ELSE
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TOPICS(ITOPIC)%STRESS(1)%FILES(ISUBTOPIC,ISYS)%FNAME)
 ENDIF

 CALL IDFPLOTFAST(0)         !peter
...
! ALLOCATE(SHD(NLAY)); DO ILAY=1,SIZE(SHD); CALL IDFNULLIFY(SHD(ILAY)); ENDDO
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (3,8)
   ...
   ALLOCATE(VCW(IL1:IL2-1)); DO ILAY=IL1,IL2-1; CALL IDFNULLIFY(VCW(ILAY)); ENDDO
  ...
  CASE (4)
   ...
   ALLOCATE(KHV(IL1:IL2)); DO ILAY=IL1,IL2; CALL IDFNULLIFY(KHV(ILAY)); ENDDO
   ALLOCATE(KVA(IL1:IL2)); DO ILAY=IL1,IL2; CALL IDFNULLIFY(KVA(ILAY)); ENDDO
   ALLOCATE(KVV(IL1:IL2-1)); DO ILAY=IL1,IL2-1; CALL IDFNULLIFY(KVV(ILAY)); ENDDO
   ALLOCATE(VCW(IL1:IL2-1)); DO ILAY=IL1,IL2-1; CALL IDFNULLIFY(VCW(ILAY)); ENDDO
  !## thickness aquifer
  CASE (5)
   ALLOCATE(TOP(IL1:IL2)); DO ILAY=IL1,IL2; CALL IDFNULLIFY(TOP(ILAY)); ENDDO
...
! ENDIF
 ...
 IF(ALLOCATED(VCW))THEN; DO ILAY=IL1,SIZE(VCW); CALL IDFCOPY(PRJIDF,VCW(ILAY)); ENDDO; ENDIF
 IF(ALLOCATED(KHV))THEN; DO ILAY=IL1,IL2;   CALL IDFCOPY(PRJIDF,KHV(ILAY)); ENDDO; ENDIF
 IF(ALLOCATED(KVV))THEN; DO ILAY=IL1,IL2-1; CALL IDFCOPY(PRJIDF,KVV(ILAY)); ENDDO; ENDIF
 ...
 DO ILAY=IL1,IL2
  ...
  SELECT CASE (ITYPE)
   CASE (1,2,3,4,5,6,7,8)
    SCL_D=1; SCL_U=2; ITOPIC=2; LEX=.FALSE.; IINV=0
    IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
    ...
    IF(.NOT.LEX)THEN
    ENDIF
    SCL_D=1; SCL_U=2; ITOPIC=3; LEX=.FALSE.; IINV=0
    IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
     IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
      PRJILIST=ITOPIC; IF(PMANAGER_GETFNAMES(ILAY,ILAY,0,1,0).LE.0)RETURN
      CALL WINDOWOUTSTATUSBAR(4,'Reading '//TRIM(FNAMES(1)%FNAME)//' ...')
  ...
  SELECT CASE (ITYPE)
   CASE (1,2,4)
    ITOPIC=7; SCL_D=1; SCL_U=3; LEX=.FALSE.; IINV=0
    IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
     IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
      PRJILIST=ITOPIC; IF(PMANAGER_GETFNAMES(ILAY,ILAY,0,1,0).LE.0)RETURN
      CALL WINDOWOUTSTATUSBAR(4,'Reading '//TRIM(FNAMES(1)%FNAME)//' ...')
    ...
    ENDIF
   CASE (7)
    ITOPIC=6; SCL_D=1; SCL_U=3; LEX=.FALSE.; IINV=0
    IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
     IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
      PRJILIST=ITOPIC; IF(PMANAGER_GETFNAMES(ILAY,ILAY,0,1,0).LE.0)RETURN
      CALL WINDOWOUTSTATUSBAR(4,'Reading '//TRIM(FNAMES(1)%FNAME)//' ...')
  ...
  SELECT CASE (ITYPE)
   CASE (2,4)
    ITOPIC=8; SCL_D=1; SCL_U=2; LEX=.FALSE.; IINV=1
    IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
     IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
      PRJILIST=ITOPIC; IF(PMANAGER_GETFNAMES(ILAY,ILAY,0,1,0).LE.0)RETURN
      CALL WINDOWOUTSTATUSBAR(4,'Reading '//TRIM(FNAMES(1)%FNAME)//' ...')
  ...
  SELECT CASE (ITYPE)
   CASE (3,4)
    ITOPIC=10; SCL_D=1; SCL_U=3; LEX=.FALSE.; IINV=0
    IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
     IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
      PRJILIST=ITOPIC; IF(PMANAGER_GETFNAMES(ILAY,ILAY,0,1,0).LE.0)RETURN
      CALL WINDOWOUTSTATUSBAR(4,'Reading '//TRIM(FNAMES(1)%FNAME)//' ...')
    ...
    ENDIF
   CASE (8)
    ITOPIC=9; SCL_D=1; SCL_U=6; LEX=.FALSE.; IINV=0
    IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
     IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
      PRJILIST=ITOPIC; IF(PMANAGER_GETFNAMES(ILAY,ILAY,0,1,0).LE.0)RETURN
      CALL WINDOWOUTSTATUSBAR(4,'Reading '//TRIM(FNAMES(1)%FNAME)//' ...')
