   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (ID_SPECIES)
      ...
      DO ISUBTOPIC=1,MIN(SIZE(PRJ_TMP),SIZE(PRJ))
      ...
      ENDDO
      DEALLOCATE(PRJ); PRJ=>PRJ_TMP
      DEALLOCATE(MENUNAMES); ALLOCATE(MENUNAMES(TOPICS(ITOPIC)%NSUBTOPICS)) 
      DO J=1,TOPICS(ITOPIC)%NSUBTOPICS; MENUNAMES(J)=TOPICS(ITOPIC)%SNAME(J); ENDDO
      CALL WDIALOGPUTMENU(IDF_MENU1,MENUNAMES,TOPICS(ITOPIC)%NSUBTOPICS,IST)
      IF(NSPECIES.EQ.0)THEN
       CALL WDIALOGPUTSTRING(IDF_LABEL9,'No species defined')
...
!       CASE (TADV)
!        STRING=TRIM(STRING)//TRIM(RTOS(ADV%HCLOSE,'G',5))//';'//TRIM(ITOS(ADV%MUT))
       CASE (TVDF)
