...
!     CALL UTL_GETHELP('7.8','TMO.ModSim.SolverSettings')
 ...
 IF(NSPECIES.EQ.0)THEN
  CALL WGRIDCLEAR(IDF_GRID1); CALL WGRIDROWS(IDF_GRID1,0)
 ELSE
...
! END SUBROUTINE PMANAGER_UTL_UPDATE
 ...
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IDITOPIC,IDIPER,IDISUBS
 INTEGER :: IPER,I,J,K,N,IDTOPIC,IDSUBTC,IFILES,NF,MF,JD,IMF
 ...
 CALL WDIALOGSELECT(ID_DPMANAGER_TAB2); CALL WDIALOGGETMENU(IDF_MENU1,IMF); IMF=IMF-1
 ...
 IFILES=0; DO I=1,SIZE(TOPICS)
  ...
  IF(IMF.GT.0)THEN
   DO J=1,SIZE(MC(IMF)%T); IF(MC(IMF)%T(J).EQ.I)EXIT; ENDDO
   IF(I.GT.SIZE(MC(IMF)%T))CYCLE
  ENDIF
