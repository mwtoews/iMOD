  ...
  CASE (ID_DPMANAGER)
   SELECT CASE (ITYPE)
    CASE (TABCHANGED)
     ...
     ITABVIEW=MESSAGE%VALUE2; CALL PMANAGER_UTL_UPDATE(0,0,0)
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (ID_CLEAN)
       ...
       IF(WINFODIALOG(4).EQ.1)THEN
        ...
        CALL PMANAGER_UTL_UPDATE(0,0,0)
       ENDIF
      CASE (ID_DELETE)
       CALL PMANAGER_DELETE()
      CASE (ID_OPEN,ID_SAVE)  ! open/save PRJ file
       IF(PMANAGERPRJ(MESSAGE%VALUE1,'',0,1))THEN; ENDIF
      CASE (ID_OPENRUN,ID_SAVERUN) ! open/save RUN file
   ...
   SELECT CASE (ITYPE)
    CASE (FIELDCHANGED)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (IDF_MENU1)
       CALL PMANAGER_UTL_UPDATE(1,0,0) !1,1
      !## dates
      CASE (IDF_MENU2)
       CALL PMANAGER_UTL_UPDATE(1,1,0) !1,1
      !## systems
!      CASE (IDF_MENU5)
!       CALL PMANAGER_UTL_UPDATE(1,1,1,0,0)
!     !## topics
      CASE (IDF_MENU3)
!       CALL PMANAGER_UTL_UPDATE(1,1,1,1,0)
     END SELECT
   END SELECT
 END SELECT
...
!   IF(ITOPIC.EQ.TRCT)CALL PMANAGEROPEN_RCT()
   !## tadv-settings
   IF(ITOPIC.EQ.TADV)CALL PMANAGEROPEN_ADV()
   !## vdf-settings
   ...
   CALL PMANAGER_UTL_UPDATE(ITOPIC,IPER,ISYS)  
   RETURN
 END SELECT
...
! IF(PRJ(1)%ICNST.EQ.0)CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,1)
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (ID_SPECIES)
      ...
      ENDDO
      DO ISUBTOPIC=N+1,M !SIZE(PRJ)+1,N
      ...
      IF(ALLOCATED(MENUNAMES))DEALLOCATE(MENUNAMES); ALLOCATE(MENUNAMES(TOPICS(ITOPIC)%NSUBTOPICS)) 
      DO J=1,TOPICS(ITOPIC)%NSUBTOPICS; MENUNAMES(J)=TOPICS(ITOPIC)%SNAME(J); ENDDO
      CALL WDIALOGPUTMENU(IDF_MENU1,MENUNAMES,TOPICS(ITOPIC)%NSUBTOPICS,IST)
      IF(NSPECIES.EQ.0)THEN
       CALL WDIALOGPUTSTRING(IDF_LABEL9,'No species defined')
 ...
 !###======================================================================
 SUBROUTINE PMANAGEROPEN_NOENTRIES(IACT)
 !###======================================================================
 IMPLICIT NONE
 ...
 IF(ITOPIC.EQ.TCAP.OR.ITOPIC.EQ.TPST.OR.ITOPIC.EQ.TPCG)THEN
  RETURN
 ENDIF
...
!  ENDDO
 ...
 INTEGER :: IU,ITOPIC,IRUN,ILOGFILE,ISUB
 ...
 IF(ID.EQ.ID_OPENRUN)THEN
  ...
  IF(PMANAGER_GETKEYS(IU)) THEN
   IF(PMANAGER_GETFILES(IU,ITOPIC))THEN
    IF(IBATCH.EQ.0)CALL PMANAGER_UTL_UPDATE(0,0,0); PMANAGERRUN=.TRUE.
   ELSE
 ...
 ELSEIF(ID.EQ.ID_SAVERUN)THEN
  ...
  FNAME=RUNFNAME
  ! Open SIM window and get info from it, e.g. FNAME and PMAN%**
  ...
  SELECT CASE (PBMAN%IFORMAT)
   ...
   CASE (3)
    ...
    IF(PBMAN%IGENMF6.EQ.1)THEN
     DIR=PBMAN%GENFNAME(:INDEX(PBMAN%GENFNAME,'.',.TRUE.)-1)
     CALL PMANAGER_GENERATEMFNETWORKS(PBMAN%GENFNAME,DIR,PBMAN%NSUBMODEL)
     PBMAN%IWINDOW=3
    ...
    DO ISUB=1,PBMAN%NSUBMODEL
     ...
     IF(PBMAN%IWINDOW.EQ.3)PBMAN%BNDFILE=TRIM(DIR)//'\'//'SUBMODEL_'//TRIM(ITOS(ISUB))//'.IDF'
     !## set fname
     
     IF(PMANAGER_SAVEMF2005(FNAME,IBATCH))THEN
      IF(IBATCH.EQ.0)THEN
       CALL WINDOWOUTSTATUSBAR(4,'Successfully written MF6 files:'//CHAR(13)//TRIM(FNAME))
      ELSE
       WRITE(*,'(/A/)') 'Successfully written MF6 files:'//TRIM(FNAME)
      ENDIF
     ENDIF
     CALL PMANAGER_SAVEMF2005_DEALLOCATE()
    ENDDO
   !##  SeaWAT/MT3D
   CASE (4,5)
    IF(PMANAGER_SAVERUNWQ(FNAME,IBATCH))THEN
     IF(IBATCH.EQ.0)THEN
      IF(IRUN.EQ.0)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully written SEAWAT runfile:'//CHAR(13)//TRIM(FNAME)//CHAR(13)//CHAR(13),'Information')
     ELSE
      WRITE(*,'(/A/)') 'Successfully written SEAWAT runfile:'//TRIM(FNAME)
     ENDIF
     PMANAGERRUN=.TRUE.
    ENDIF
  END SELECT
  ...
  !## Start the Model
  IF(ABS(IRUN).EQ.1.AND.PMANAGERRUN)CALL PMANAGERSTART(FNAME,IRUN,IBATCH,1,ILOGFILE)
  IF(IBATCH.EQ.0)CALL UTL_MESSAGEHANDLE(1)
 ...
 DID=WINFODIALOG(CURRENTDIALOG)
 CALL WDIALOGLOAD(ID_DPMANAGER_MT3D_GCG,ID_DPMANAGER_MT3D_GCG)
...
!     CALL UTL_GETHELP('7.8','TMO.ModSim.SolverSettings')
 ...
 DID=WINFODIALOG(CURRENTDIALOG)
 CALL WDIALOGLOAD(ID_DPMANAGER_MT3D_RCT,ID_DPMANAGER_MT3D_RCT)
...
!     CALL UTL_GETHELP('7.8','TMO.ModSim.SolverSettings')
 ...
 CALL WDIALOGPUTDOUBLE(IDF_REAL10,PEST%PE_REGFACTOR,'(F15.3)')
 ...
 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  ...
  CALL WDIALOGGETDOUBLE(IDF_REAL10,PEST%PE_REGFACTOR)
 ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ITYPE,I,J,IOPTION,DID,ISYS,IPER,NPER,NSYS,NSPEC,NTOP,NORG,ISUB
 TYPE(WIN_MESSAGE) :: MESSAGE
 
 DID=WINFODIALOG(CURRENTDIALOG)
 
 CALL WDIALOGLOAD(ID_DPMANAGERSPECIES,ID_DPMANAGERSPECIES)
 CALL WDIALOGPUTIMAGE(ID_NEW,ID_ICONNEW,1)
 CALL WDIALOGPUTIMAGE(ID_DELETE,ID_ICONDELETE,1)
 CALL WDIALOGPUTIMAGE(ID_RENAME,ID_ICONRENAME,1)
 ...
 NSPEC=NSPECIES
 IF(.NOT.PMANAGERDEFINESPECIES_INIT())THEN
  CALL WDIALOGUNLOAD(); CALL WDIALOGSELECT(ID_DPMANAGEROPEN)
  RETURN
 ENDIF
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (ID_DELETE)
      CALL WDIALOGGETMENU(IDF_MENU3,IOPTION)
      CALL WMESSAGEBOX(YESNO,QUESTIONICON,COMMONNO,'Are you sure to delete selected'//CHAR(13)// &
              'period: ['//TRIM(SPECIES(IOPTION)%NAME)//']','Question')
      IF(WINFODIALOG(4).EQ.1)THEN
       DO I=IOPTION,SIZE(SPECIES)-1; SPECIES(I)=SPECIES(I+1); ENDDO; NSPECIES=MAX(NSPECIES-1,0)
      ENDIF
      IF(.NOT.PMANAGERDEFINEPERIODS_INIT())EXIT 
     CASE (ID_NEW)
      CALL WDIALOGSELECT(ID_DPMANAGERSPECIES); CALL WDIALOGGETMENU(IDF_MENU3,IOPTION)
      CALL PMANAGERDEFINESPECIES_RENAME(0)
     CASE (ID_RENAME)
      CALL PMANAGERDEFINESPECIES_RENAME(1)
     CASE (IDOK)
      CALL WDIALOGSELECT(ID_DPMANAGERSPECIES); CALL WDIALOGGETMENU(IDF_MENU3,IOPTION); EXIT
     CASE (IDCANCEL)
      EXIT
     CASE (IDHELP)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGERDEFINESPECIES_INIT()
 !###======================================================================
 IMPLICIT NONE
  
 PMANAGERDEFINESPECIES_INIT=.FALSE.
 
 IF(NSPECIES.EQ.0)THEN
  CALL PMANAGERDEFINESPECIES_RENAME(0)
  !## cannot start unless nperiod>0
  IF(NSPECIES.EQ.0)THEN
   CALL WDIALOGUNLOAD(); CALL WDIALOGSELECT(ID_DPMANAGERSPECIES); RETURN
  ENDIF
 ELSE
  !## fill in menu
  CALL WDIALOGPUTMENU(IDF_MENU3,SPECIES%NAME,NSPECIES,1)
  CALL PMANAGERDEFINESPECIES_PUT()
  IF(NSPECIES.GE.SIZE(SPECIES))CALL WDIALOGFIELDSTATE(ID_NEW,0)
 ENDIF

 PMANAGERDEFINESPECIES_INIT=.TRUE.
 
 END FUNCTION PMANAGERDEFINESPECIES_INIT

 !###======================================================================
 SUBROUTINE PMANAGERDEFINESPECIES_RENAME(ICODE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ICODE
 INTEGER :: IOPTION,ITYPE,I
 ...
 !## define new period
 IF(ICODE.EQ.0)THEN
  NSPECIES=NSPECIES+1; IOPTION=NSPECIES; SPECIES(IOPTION)%NAME=''
 !## use existing one
 ELSE
  CALL WDIALOGGETMENU(IDF_MENU3,IOPTION)
 ENDIF
 ...
 CALL WDIALOGPUTSTRING(IDF_LABEL1,'Enter a new name')
 IF(IOPTION.EQ.0)CALL WDIALOGPUTSTRING(IDF_STRING1,'...')
 IF(IOPTION.GT.0)CALL WDIALOGPUTSTRING(IDF_STRING1,TRIM(SPECIES(IOPTION)%NAME))
 DO
  CALL WMESSAGE(ITYPE,MESSAGE)
  SELECT CASE (ITYPE)
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (IDOK)
      CALL WDIALOGGETSTRING(IDF_STRING1,SPECIES(IOPTION)%NAME)
      IF(TRIM(SPECIES(IOPTION)%NAME).EQ.'')THEN
       CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You should specify a name of at least 1 character','Warning')
      ELSE
       DO I=1,NSPECIES
        IF(I.EQ.IOPTION)CYCLE
        IF(UTL_CAP(TRIM(SPECIES(I)%NAME),'U').EQ.UTL_CAP(TRIM(SPECIES(IOPTION)%NAME),'U'))THEN
         CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Given name has defined already.'//CHAR(13)// &
           'You should specify an unique name','Warning')
         EXIT
        ENDIF
       ENDDO
       IF(I.GT.NSPECIES)EXIT
      ENDIF
     CASE (IDCANCEL)
      EXIT
    END SELECT
 ...
 CALL WDIALOGUNLOAD(); CALL WDIALOGSELECT(ID_DPMANAGERSPECIES)

 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  CALL WDIALOGPUTMENU(IDF_MENU3,SPECIES%NAME,NSPECIES,IOPTION)
  CALL PMANAGERDEFINESPECIES_PUT()
 ELSEIF(MESSAGE%VALUE1.EQ.IDCANCEL)THEN
  NSPECIES=NSPECIES-1
 ENDIF
 ...
 END SUBROUTINE PMANAGERDEFINESPECIES_RENAME
 
 !###======================================================================
 SUBROUTINE PMANAGERDEFINESPECIES_PUT()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IOPTION,I
 
 CALL WDIALOGSELECT(ID_DPMANAGERSPECIES)
 CALL WDIALOGGETMENU(IDF_MENU3,IOPTION)
 CALL WDIALOGGETINTEGER(IDF_INTEGER4,I)

! !## make copy of entered data first, before overwrite it with new one
! IF(I.NE.IOPTION)CALL PMANAGERDEFINESPECIES_GET(I)

 CALL WDIALOGPUTINTEGER(IDF_INTEGER4,IOPTION)
 
 END SUBROUTINE PMANAGERDEFINESPECIES_PUT

 !!###======================================================================
 !SUBROUTINE PMANAGERDEFINESPECIES_GET(IOPTION)
 !!###======================================================================
 !IMPLICIT NONE
 !INTEGER,INTENT(IN) :: IOPTION
 !
 ...
 !ENDIF
 !
 !END SUBROUTINE PMANAGERDEFINESPECIES_GET
...
! ENDIF
 ...
 IF(ID.EQ.ID_OPEN)THEN
  ...
  ELSE
   IF(IBATCH.EQ.0.AND.IDIALOG.EQ.1)CALL PMANAGER_UTL_UPDATE(0,0,0); PMANAGERPRJ=.TRUE.
  ENDIF
...
!   !## adv module another exception
 ...
 WRITE(IU,'(/A)') 'Species'
 DO I=1,NSPECIES
  WRITE(IU,'(A)') '"'//TRIM(SPECIES(I)%NAME)//'"'
 ENDDO
...
!    CYCLE
 ...
 DO
  ...
  I=I+1; READ(LINE,*,IOSTAT=IOS) SPECIES(I)%NAME
 ...
 ELSEIF(IPER.NE.0.AND.ISYS.NE.0.AND.ISUBTOPIC.NE.0)THEN
  ...
  CALL PMANAGER_UTL_UPDATE(ITOPIC,IPER,ISUBTOPIC)
 ...
 ELSEIF(IPER.NE.0.AND.ISYS.EQ.0.AND.ISUBTOPIC.EQ.0)THEN
  ...
  CALL PMANAGER_UTL_UPDATE(ITOPIC,0,0)
...
! ENDIF
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_UTL_UPDATE(IDITOPIC,IDIPER,IDISUBS)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IDITOPIC,IDIPER,IDISUBS
 
 IF(ITABVIEW.EQ.ID_DPMANAGER_TAB1)THEN
  CALL PMANAGER_UTL_UPDATE_TREEVIEW(IDITOPIC,IDIPER,IDISUBS)
 ELSEIF(ITABVIEW.EQ.ID_DPMANAGER_TAB2)THEN
  CALL PMANAGER_UTL_UPDATE_MENUS(IDITOPIC,IDIPER,IDISUBS)
 ENDIF
 
 END SUBROUTINE PMANAGER_UTL_UPDATE
 
 !###======================================================================
 SUBROUTINE PMANAGER_UTL_UPDATE_MENUS(IDITOPIC,IDIPER,IDISUBS)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IDITOPIC,IDIPER,IDISUBS
 INTEGER :: DID,I,N,IPER
 CHARACTER(LEN=256) :: STRING
 CHARACTER(LEN=52),DIMENSION(:),ALLOCATABLE :: TDATES
 
 DID=WINFODIALOG(CURRENTDIALOG); CALL WDIALOGSELECT(ID_DPMANAGER_TAB2)

 !## fill in main topics
 IF(IDITOPIC.EQ.0)CALL WDIALOGPUTMENU(IDF_MENU1,TOPICS%TNAME,MAXTOPICS,1)
 
 !## fill in dates
 IF(IDIPER.EQ.0)THEN
  CALL WDIALOGGETMENU(IDF_MENU1,I)
  IF(.NOT.TOPICS(I)%TIMDEP)THEN
  ...
  ELSE
   ...
   IF(N.GT.0)THEN
    ...
    DO IPER=1,SIZE(TOPICS(I)%STRESS)
     ...
     ENDIF
     TDATES(IPER)=TRIM(STRING)
    ENDDO
    CALL WDIALOGFIELDSTATE(IDF_MENU2,1); CALL WDIALOGPUTMENU(IDF_MENU2,TDATES,N,1)
   ELSE
    !## clear all
    CALL WDIALOGCLEARFIELD(IDF_MENU2); CALL WDIALOGFIELDSTATE(IDF_MENU2,0)
   ENDIF
 ...
 END SUBROUTINE PMANAGER_UTL_UPDATE_MENUS
    ...
    DO J=1,NF   !## number of periods (types)
     DO K=1,MF  !## number of files (systems)
      ...
      SELECT CASE (I)
       ...
       CASE (TGCG)
        STRING=TRIM(STRING)//TRIM(RTOS(WQ%GCG%CCLOSE,'G',5))//';'//TRIM(ITOS(WQ%GCG%MXITER))
!       CASE (TRCT)
!        STRING=TRIM(STRING)//TRIM(RTOS(RCT%HCLOSE,'G',5))//';'//TRIM(ITOS(RCT%MUT))
       CASE (TADV)
        STRING=TRIM(STRING)//TRIM(RTOS(WQ%ADV%WD,'G',5))//';'//TRIM(ITOS(WQ%ADV%MIXELM))
       CASE (TVDF)
       ...
       CASE DEFAULT
        ...
        IF(TOPICS(I)%STRESS(IPER)%FILES(J,K)%ICNST.EQ.1)THEN
         ...
         STRING=TRIM(STRING)//';cnst='//TRIM(XSTRING) !TRIM(RTOS(TOPICS(I)%STRESS(IPER)%FILES(J,K)%CNST,'*',3))
        ...
        STRING=TRIM(STRING)//';fct='//TRIM(XSTRING) !TRIM(RTOS(TOPICS(I)%STRESS(IPER)%FILES(J,K)%FCT,'*',3))
        WRITE(XSTRING,UTL_GETFORMAT(TOPICS(I)%STRESS(IPER)%FILES(J,K)%IMP)) TOPICS(I)%STRESS(IPER)%FILES(J,K)%IMP
        STRING=TRIM(STRING)//';imp='//TRIM(XSTRING) !TRIM(RTOS(TOPICS(I)%STRESS(IPER)%FILES(J,K)%IMP,'*',3))
      END SELECT
