...
!  WRITE(IU,'(A)') ' PRINT_FLOWS'
  SELECT CASE (ITOPIC)
   CASE (TUZF); IF(ASSOCIATED(PBMAN%SAVEUZF))WRITE(IU,'(1X,A)') 'SAVE_FLOWS'
   CASE (TDRN); IF(ASSOCIATED(PBMAN%SAVEDRN))WRITE(IU,'(1X,A)') 'SAVE_FLOWS'
   CASE (TRIV); IF(ASSOCIATED(PBMAN%SAVERIV))WRITE(IU,'(1X,A)') 'SAVE_FLOWS'
   CASE (TEVT); IF(ASSOCIATED(PBMAN%SAVEEVT))WRITE(IU,'(1X,A)') 'SAVE_FLOWS'
   CASE (TGHB); IF(ASSOCIATED(PBMAN%SAVEGHB))WRITE(IU,'(1X,A)') 'SAVE_FLOWS'
   CASE (TRCH); IF(ASSOCIATED(PBMAN%SAVERCH))WRITE(IU,'(1X,A)') 'SAVE_FLOWS'
!   CASE (TOLF); IF(ASSOCIATED(PBMAN%SAVEDRN))WRITE(IU,'(1X,A)') 'SAVE_FLOWS'
   CASE (TCHD); IF(ASSOCIATED(PBMAN%SAVEFLX))WRITE(IU,'(1X,A)') 'SAVE_FLOWS'
  END SELECT
!  WRITE(IU,'(A)') ' TS6 FILEIN <ts6_filename>'
!  WRITE(IU,'(A)') ' OBS6 FILEIN <obs6_filename>'
!  WRITE(IU,'(A)') ' MOVER'
...
!NUZTOP=1 !## recharge specified to top cell
  CASE (TUZF); NUZGAG=PBMAN%NLOGLOC; IRUNFLG=0; NUZTOP=1
