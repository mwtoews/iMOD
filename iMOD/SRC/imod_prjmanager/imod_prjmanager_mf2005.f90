...
CONTAINS
 ...
 IF(IOPTION.EQ.2)THEN
  ...
  IF(PRJIDF%IEQ.EQ.0)THEN
   WRITE(IU,*) PRJIDF%DX
  ELSE
   WRITE(IU,*) (PRJIDF%SX(ICOL),ICOL=1,PRJIDF%NCOL)
   WRITE(IU,*) (PRJIDF%SY(IROW),IROW=1,PRJIDF%NROW)
  ENDIF
 ENDIF
 ...
 ENDIF
 WRITE(IU,'(12(I10,1X))') PRJNLAY,PRJMXNLAY,PRJNPER,0,NSCL,0,PBMAN%ICONCHK,N,0,PBMAN%IFVDL,IARMWP
 ...
 ELSE
  ...
  IF(ISUBMODEL.EQ.0)THEN
   WRITE(IU,'(6(G15.7,A1))') IDF(1)%XMIN,',',IDF(1)%YMIN,',',IDF(1)%XMAX,',',IDF(1)%YMAX,',',IDF(1)%DX,',',0.0D0
  ELSE
 ...
 INTEGER :: IU,JU,I,J,K,IM,N,M,IOS,II,ILAY,MAXNLAY,IROW,ICOL,IMDL1,IMDL2,JROW,JCOL
 ...
 CHARACTER(LEN=5),DIMENSION(3) :: PCK
 DATA PCK/'RIV6','RCH6','WEL6'/
...
! WRITE(IU,'(A)') '[OBS6 FILEIN <obs6_filename>]' !## observation
 ...
 DO II=1,2
  ...
  ENDDO
  IF(II.EQ.1)THEN
   ALLOCATE(TOP(2,1),BOT(2,MAXNLAY),BND(2,MAXNLAY))
 ...
 IMDL1=1; IMDL2=2; LSUBMODEL=.FALSE.; TLAYMODEL=''
 !## check size first
 IF(BND(2,1)%XMIN.GT.BND(1,1)%XMIN.AND. &
  ...
  ENDIF
  IMDL2=1; IMDL1=2; LSUBMODEL=.TRUE.
 !## check size second
 ELSEIF(BND(1,1)%XMIN.GT.BND(2,1)%XMIN.AND. &
  ...
  IF(BND(1,1)%DX.GT.BND(2,1)%DX)THEN
   WRITE(*,'(/A/)') 'A submodel need to have at least a cellsize which is equal or smaller than the overlapping model'; STOP 
  ENDIF
  IMDL2=2; IMDL1=1; LSUBMODEL=.TRUE.
 ...
 DO I=1,SIZE(BND,2)
  IU=UTL_GETUNIT()
  CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL1))//'\MODELINPUT\DIS6\IBOUND_L'//TRIM(ITOS(I))//'.ARR',STATUS='UNKNOWN', &
 ...
 DO I=1,SIZE(BND,2)
  IU=UTL_GETUNIT()
  CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL2))//'\MODELINPUT\DIS6\IBOUND_L'//TRIM(ITOS(I))//'.ARR',STATUS='UNKNOWN', &
      ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
 ...
 DO I=1,SIZE(PCK)
  ...
  JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL2))//'\MODELINPUT\'//TRIM(MDLNAME)//'.'//TRIM(PCK(I))//'_', &
    STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(JU.EQ.0)RETURN
  ...
  DO J=1,2
   DO
    ...
    IF(INDEX(LINE,'MAXBOUND').GT.0)THEN

     IF(J.EQ.2)WRITE(JU,'(A)') 'MAXBOUND '//TRIM(ITOS(MAX(1,M)))

    ELSEIF(INDEX(LINE,'BEGIN PERIOD').GT.0)THEN

     IF(J.EQ.2)WRITE(JU,'(A)') TRIM(LINE)
     N=0
     DO
      READ(IU,'(A256)',IOSTAT=IOS) LINE
      IF(IOS.NE.0)EXIT
      IF(INDEX(LINE,'END PERIOD').GT.0)EXIT
      READ(LINE,*) ILAY,IROW,ICOL
      !## skip inactive cells
      IF(BND(IMDL2,ILAY)%X(ICOL,IROW).EQ.0.0D0)CYCLE
      N=N+1; IF(J.EQ.2)WRITE(JU,'(A)') TRIM(LINE)
     ENDDO
     IF(J.EQ.2)WRITE(JU,'(A)') TRIM(LINE)
  ...
  CALL IOSRENAMEFILE(TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL2))//'\MODELINPUT\'//TRIM(MDLNAME)//'.'//TRIM(PCK(I))//'_', &
                     TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL2))//'\MODELINPUT\'//TRIM(MDLNAME)//'.'//TRIM(PCK(I)))
 ENDDO
...
!   BOT(ILAY)%X(ICOL,IROW)=MIN(TOP(ILAY)%X(ICOL,IROW)-0.01  ,BOT(ILAY)%X(ICOL,IROW))
 ...
 ELSEIF(PBMAN%ICONSISTENCY.EQ.2)THEN
  ...
  ALLOCATE(TP(PRJNLAY)   ,BT(PRJNLAY)   ,HK(PRJNLAY)   ,VK(PRJNLAY-1)   ,VA(PRJNLAY)   ,IB(PRJNLAY),TH(PRJNLAY), &
...
![MEMORY_PRINT_OPTION <memory_print_option>]
 ...
 DIR=MAINDIR; IF(PBMAN%NSUBMODEL.GT.1)DIR=TRIM(MAINDIR)//'\GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))
 ...
 IF(PBMAN%IFORMAT.EQ.3)THEN
  ...
  IF(PBMAN%NSUBMODEL.GT.1)DIRMNAME='GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\'//TRIM(DIRMNAME)
...
!  IF()WRITE(IU,'(A)') 'NEWTON [UNDER_RELAXATION]'
 ...
 IF(ISUBMODEL.EQ.1)THEN
  ...
  ELSE
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_COARSEGRID(PRJIDF,SUBMODEL(1)+SUBMODEL(6), &
...
! WRITE(IU,'(A)') ' ANGLE3 LAYERED'
 ...
 DO ILAY=1,PRJNLAY; ANF(ILAY)%X=1.0D0; ANA(ILAY)%X=0.0D0; ENDDO
...
!  CALL UTL_MF2005_MAXNO(TRIM(DIRMNAME)//'.'//CPCK//'7_',(/MP/))
...
!  END SELECT
 ...
 IF(ITOPIC.EQ.31)THEN
  IF(ALLOCATED(FHBFLW))THEN
   ...
   I=0; DO ILAY=1,PRJNLAY; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
    IF(BND(ILAY)%X(ICOL,IROW).EQ. 2)THEN
     I=I+1; WRITE(IU,'(3(I10,1X),F10.1,99(1X,G15.9))') ILAY,IROW,ICOL,1.0,(FHBFLW(I,J),J=1,NBDTIM)
    ENDIF
   ENDDO; ENDDO; ENDDO
  ENDIF
  IF(ALLOCATED(FHBHED))THEN
   ...
   I=0; DO ILAY=1,PRJNLAY; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
    IF(BND(ILAY)%X(ICOL,IROW).EQ.-2)THEN
    I=I+1; WRITE(IU,'(3(I10,1X),F10.1,99(1X,G15.9))') ILAY,IROW,ICOL,1.0,(FHBHED(I,J),J=1,NBDTIM)
    ENDIF
   ENDDO; ENDDO; ENDDO
  ENDIF
 ENDIF
...
!  ITMP1=1; IF(KPER.EQ.0)ITMP1=0; IF(KPER.LT.0)ITMP1=-1
 ...
 LINE='COORD_XLL '//TRIM(RTOS(PRJIDF%XMIN,'F',2))   ; WRITE(IU,'(A)') TRIM(LINE)
 LINE='COORD_YLL '//TRIM(RTOS(PRJIDF%YMIN,'F',2))   ; WRITE(IU,'(A)') TRIM(LINE)
 LINE='COORD_XLL_NB '//TRIM(RTOS(PRJIDF%XMIN,'F',2)); WRITE(IU,'(A)') TRIM(LINE)
 LINE='COORD_YLL_NB '//TRIM(RTOS(PRJIDF%YMIN,'F',2)); WRITE(IU,'(A)') TRIM(LINE)
 LINE='COORD_XUR_NB '//TRIM(RTOS(PRJIDF%XMAX,'F',2)); WRITE(IU,'(A)') TRIM(LINE)
 LINE='COORD_YUR_NB '//TRIM(RTOS(PRJIDF%YMAX,'F',2)); WRITE(IU,'(A)') TRIM(LINE)
 !## look for first
 DO KPER=1,PRJNPER; IF(SIM(KPER)%DELT.GT.0.0D0)EXIT; ENDDO
 IF(KPER.LE.PRJNPER)THEN
...
! LINE=TRIM(RTOS(SIP%RELAX,'E',7))//','//TRIM(RTOS(SIP%HCLOSE,'E',7))//',1,0.0D0,1'; WRITE(IU,'(A)') TRIM(LINE)
!
! CLOSE(IU)
!
! PMANAGER_SAVEMF2005_SIP=.TRUE.
...
!! LINE='0,1.0D0,0.0D0,0.5,1.0D0'; WRITE(IU,'(A)') TRIM(LINE)
!! LINE='0,0.0D0,0,0.0D0,0'; WRITE(IU,'(A)') TRIM(LINE)
!
! CLOSE(IU)
!
! PMANAGER_SAVEMF2005_PCGN=.TRUE.
...
!
 ...
 INTEGER :: J, LUNCB, ICOL, IROW, ILAY, ID
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 DO ISYS=1,SIZE(TOPICS(ITOPIC)%STRESS(1)%FILES,2)
  ...
  IF(LEN_TRIM(PRJIDF%FNAME).GT.0)THEN
   !## rasterize genfile
   CALL ASC2IDF_HFB(IDF,IDF%NROW,IDF%NCOL,IPC,(/IDF%FNAME/),(/0.0D0/),ILAY,TIDF,BIDF)
 ...
 !###====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: THICKNESS=0.5
 LOGICAL,INTENT(IN) :: LTB
...
!## dit klopt niet ... de vulling van een breuk gaan niet op de cell niveau maar op vlak-niveau :-(
 ...
 REAL(KIND=DP_KIND) :: C,ZT,ZB,X1,X2,Y1,Y2,DX,DY,L,TIB,F,KD1,KD2,OT1,OT2
...
!tib = 2
 ...
 IF(NN+NS+NW+NE.GT.0)THEN
  WRITE(*,'(/A)') 'Modified boundary layer '//TRIM(ITOS(ILAY))//' due to submodelling:'
  WRITE(*,'(A)') ' - North Boundary '//TRIM(ITOS(NN))
  WRITE(*,'(A)') ' - South Boundary '//TRIM(ITOS(NS))
  WRITE(*,'(A)') ' - West  Boundary '//TRIM(ITOS(NW))
  WRITE(*,'(A/)') ' - East  Boundary '//TRIM(ITOS(NE))
 ENDIF
