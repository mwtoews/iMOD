...
![MEMORY_PRINT_OPTION <memory_print_option>]
 ...
 IF(PBMAN%IFORMAT.EQ.3)THEN
  ...
  DO I=N1,N2
   ...
   IU=UTL_GETUNIT()
   IF(PBMAN%IPESTP+PBMAN%IIES.EQ.0)THEN
    NAME=TRIM(DIR)//'\'//TRIM(MNAME)//'.NAM' !FNAME 
   ELSEIF(PBMAN%IPESTP.EQ.1)THEN
    IF(I.GT.0)THEN
     IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE
...
! WRITE(IU,'(A)') ' ANGLE3 LAYERED'
 ...
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 INTEGER,INTENT(IN) :: IBATCH,ISS
 INTEGER :: IU,ILAY,ISY,KPER,JLAY,IROW,ICOL
 LOGICAL :: LEX
 ...
 JLAY=0; DO ILAY=1,SIZE(PBMAN%ILAY) 
  IF(PBMAN%ILAY(ILAY).EQ.0)CYCLE
  JLAY=JLAY+1
  !## set sto to zero for inactief layers
!  DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
!   IF(BND(ILAY)%X(ICOL,IROW).EQ.0.0D0)STO(ILAY)%X(ICOL,IROW)=0.0D0
!  ENDDO; ENDDO
 ...
 IF(ISY.EQ.1)THEN
  ...
  JLAY=0; DO ILAY=1,SIZE(PBMAN%ILAY) 
   IF(PBMAN%ILAY(ILAY).EQ.0)CYCLE
   JLAY=JLAY+1
   !## set spy to zero for inactief layers
!   DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
!    IF(BND(ILAY)%X(ICOL,IROW).EQ.0.0D0)SPY(ILAY)%X(ICOL,IROW)=0.0D0
!   ENDDO; ENDDO
   !## spy
...
!     IF(SUM(TLP).GT.0.0D0)TLP=(1.0D0/SUM(TLP))*TLP
 ...
 CHARACTER(LEN=5) :: EXT
 CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:) :: STRING
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,J,NROWIPF,NCOLIPF,IEXT,IOS,N,NP,MP,ICNST,ISYS,NSYS,IPER,KPER,IP, &
