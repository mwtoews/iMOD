...
CONTAINS
 ...
 DO I=1,SHP%NPOL
  CALL ASC2IDF_INT_NULLIFY(); ALLOCATE(XP(100),YP(100),ZP(100),WP(100),FP(100)); ALLOCATE(IPC(IDF(I)%NCOL,IDF(I)%NROW,2)); IPC=INT(0,1)
  
  !## need to write polygon as line to a new GEN file ...
  
  !## intersect line and determine ipc()
  CALL ASC2IDF_HFB(IDF(I),IDF(I)%NROW,IDF(I)%NCOL,IPC,(/GENFNAME/),-1,IPOL=I)
  !## write genfiles
  CALL PMANAGER_GENERATEMFNETWORKS_WRITEGEN(IDF(I),IPC,IU)
  CALL ASC2IDF_INT_DEALLOCATE(); DEALLOCATE(IPC)
 ENDDO
 WRITE(IU,'(A)') 'END'; CLOSE(IU)

 !## sortgen files to be a polygon
 
 !## mark inside/outside idf files
 
 !## blank out idf files statr from smallest to biggest
 ...
 DO I=1,SHP%NPOL
  ...
  IF(.NOT.IDFALLOCATEX(IDF(I)))THEN; WRITE(*,'(/A/)') 'Cannot allocate memory for idf #',I; STOP; ENDIF
 ENDDO
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS_WRITEGEN(IDF,IPC,IU)
 !###======================================================================
 IMPLICIT NONE
 ...
 N=0; DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
  ...
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.IDF%NCOL)THEN
    N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(IU,0,IPC,IDF,IDF%NROW,IDF%NCOL,IROW,ICOL,N, &
         0.0D0,0.0D0,0.0D0,0,1,.TRUE.,0.0D0,0.0D0) 
   ENDIF
  ENDIF
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   ...
   IF(IROW.LT.IDF%NROW)THEN
    N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(IU,0,IPC,IDF,IDF%NROW,IDF%NCOL,IROW,ICOL,N, &
          0.0D0,0.0D0,0.0D0,0,2,.TRUE.,0.0D0,0.0D0) 
   ENDIF
  ENDIF
