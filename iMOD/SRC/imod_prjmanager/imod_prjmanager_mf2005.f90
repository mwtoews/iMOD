 ...
 IF(TOPICS(TANI)%IACT_MODEL.NE.0)THEN
  ...
  DO ILAY=1,SIZE(PBMAN%ILAY) 
   ...
   DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
    ...
    ENDIF
   ENDDO; ENDDO
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\NPF\K22_L'//TRIM(ITOS(ILAY))//'.ARR', &
      PRJIDF,0,IU,ILAY,IFBND))RETURN
  ENDDO
  ...
  DO ILAY=1,SIZE(PBMAN%ILAY) 
   ...
   DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
    ...
    ENDIF
   ENDDO; ENDDO
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\NPF\ANGLE1_L'//TRIM(ITOS(ILAY))//'.ARR', &
      PRJIDF,0,IU,ILAY,IFBND))RETURN
  ENDDO
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 INTEGER :: IROW,ICOL,ILAY,IOS,JLAY,IC1,IC2,IR1,IR2,ISYS,NC,NR
   ...
   IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
    IF(ICOL.LT.IDF%NCOL)THEN
     ...
     ELSE
      ...
      !## fraction 
      F=(T1+T2)/(C2+T1+T2)
      WRITE(IU,'(6(I10,1X),G15.7,1X,I10)') ILAY,IROW,ICOL,ILAY,IROW,ICOL+1,     -F ,ISYS !## y-direction
   ...
   IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
    IF(IROW.LT.IDF%NROW)THEN
     ...
     ELSE
      ...
      !## fraction 
      F=(T1+T2)/(C2+T1+T2)
      WRITE(IU,'(6(I10,1X),G15.7,1X,I10)') ILAY,IROW,ICOL,ILAY,IROW+1,ICOL,      -F,ISYS !## x-direction
     ENDIF
...
!tib = 2
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IROW,ICOL,NN,NE,NS,NW,ILAY,I,ISUB
 ...
 DO ILAY=1,PRJNLAY
  ...
  IF(PBMAN%NSUBMODEL.LE.1.OR.PBMAN%IFORMAT.NE.3)THEN  !RETURN
  ...
  DO IROW=1,BND(ILAY)%NROW
   DO ICOL=1,BND(ILAY)%NCOL
    ...
    IF(BND(ILAY)%X(ICOL,IROW).LT.0)THEN
     ...
     I=0
     IF(ILAY.GT.1)THEN;              IF(BND(ILAY-1)%X(ICOL  ,IROW  ).GT.0)I=-1; ENDIF
     IF(ILAY.LT.PRJNLAY)THEN;        IF(BND(ILAY+1)%X(ICOL  ,IROW  ).GT.0)I=-1; ENDIF
     IF(ICOL.GT.1)THEN;              IF(BND(ILAY  )%X(ICOL-1,IROW  ).GT.0)I=-1; ENDIF
     IF(ICOL.LT.BND(ILAY)%NCOL)THEN; IF(BND(ILAY  )%X(ICOL+1,IROW  ).GT.0)I=-1; ENDIF
     IF(IROW.GT.1)THEN;              IF(BND(ILAY  )%X(ICOL  ,IROW-1).GT.0)I=-1; ENDIF
     IF(IROW.LT.BND(ILAY)%NROW)THEN; IF(BND(ILAY  )%X(ICOL  ,IROW+1).GT.0)I=-1; ENDIF
     BND(ILAY)%X(ICOL,IROW)=I
    ENDIF
   ENDDO
  ENDDO
