...
!  ENDIF
 ...
 IF(PBMAN%IFORMAT.EQ.3.AND.PBMAN%ISUBMODEL.EQ.PBMAN%NSUBMODEL)THEN
  ...
  IF(.NOT.ASSOCIATED(PBMAN%SM))THEN
   ...
   ENDDO
  ENDIF
  DO I=1,PBMAN%NSUBMODEL; ALLOCATE(PBMAN%SM(I)%CON(3)); ENDDO
  DO I=1,PBMAN%NSUBMODEL; DO J=1,PBMAN%NSUBMODEL
   IF(I.EQ.J)CYCLE; N=NEX(J,I)
   CALL PMANAGER_SAVEMF6_EXG(MAINDIR,DIRMNAME,I,J,N); NEX(I,J)=N
  ENDDO; ENDDO
  DEALLOCATE(NEX)
  DO I=1,PBMAN%NSUBMODEL
 ...
 IF(.NOT.PMANAGER_SAVEMF2005_IES_READWRITE(DIR,DIRMNAME,IBATCH))RETURN
...
ILAYLOOP2: DO ILAY=MNLAY(1),1,-1 !1,MNLAY(1)
     !## skip inactive cells
     IF(BND(1,ILAY)%X(ICOL,IROW).EQ.BND(1,ILAY)%NODATA)CYCLE
     !## if active in other model - probably vertical connection to the top/bottom
     CALL IDFGETLOC(BND(1,1),IROW,ICOL,XP,YP); CALL IDFIROWICOL(BND(2,1),JROW,JCOL,XP,YP)
...
! CALL IDFIROWICOL(BND(2,1   ),JROW,JCOL,XP1,YP1)
...
!  IF()WRITE(IU,'(A)') 'NEWTON [UNDER_RELAXATION]'
 ...
 ELSE 
  ...
  ELSEIF(PBMAN%IPESTP.EQ.1)THEN
   N1=-PBMAN%NLAMBDA; N2=PEST%NREALS
  ENDIF
  DO I=N1,N2
   !## skip zero
   IF(I.EQ.0)CYCLE
   ...
   IF(LLPF)THEN
    ...
    ELSE
     WRITE(IU,'(A)') 'LPF  14 '//CHAR(39)//TRIM(DIRMNAME)//'_#'//TRIM(ITOS(I))//'.LPF7'//CHAR(39)
    ENDIF
   ENDIF
...
IF(TOPICS(TSFT)%DEFINED)THEN
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_IES_READWRITE(DIR,DIRMNAME,IBATCH)
 !####====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 INTEGER,INTENT(IN) :: IBATCH
...
!  DO J=1,PEST%NREALS
!   IF(.NOT.UTL_READINITFILE('REALS_R'//TRIM(ITOS(J))//'_P'//TRIM(ITOS(I)),LINE,IU,0))RETURN; READ(LINE,*)  
 
 !## change lpf (copy) for now INTO #.lpf
 DO I=1,PEST%NREALS
  IU=UTL_GETUNIT()
   ...
   DO J=1,SIZE(PEST%PARAM)
    DO K=1,SIZE(PEST%PARAM(J)%ILS)
     ...
     SELECT CASE (PEST%PARAM(J)%PPARAM)
      CASE ('KH')
       LINE=UTL_SUBST(LINE,'HK_L'//TRIM(ITOS(ILAY))//'.ARR','HK_L'//TRIM(ITOS(ILAY))//'_S#'//TRIM(ITOS(I))//'.ARR')
     END SELECT
    ENDDO
   ENDDO
   WRITE(JU,'(A)') TRIM(LINE)
...
!below. This type of cell is referred to as a “vertical pass through” cell.
 ...
 INTEGER :: ILAY,SCL_D,SCL_U,IINV,ITOPIC,ICL
  
 ICL=0; IF(TOPICS(TCON)%DEFINED)ICL=1
 ...
 !## add chloride package
 IF(ICL.EQ.1)LINE=TRIM(LINE)//',CHLORIDE'
 WRITE(IU,'(A)') TRIM(LINE)
 !## ltype code
 ...
 DO ILAY=1,PRJNLAY
  ...
  !## concentration
  IF(ICL.EQ.1)THEN
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\LPF7\CON_L'//TRIM(ITOS(ILAY))//'.ARR', &
...
!  ENDIF
 ...
 REAL(KIND=DP_KIND) :: T,B,K,T1,T2,T3
...
!  IF(BND(ILAY)%X(ICOL,IROW).NE.0)THEN
 ...
 INTEGER :: IU,ILAY,IFBND,IHDWET,IWETIT,IROW,ICOL,ICL
...
!  WRITE(IU,'(A)') ' PRINT_INPUT'
 ...
 LOGICAL :: LFLX,LSHD,LBDG !## if true is saves fluxes for modflow2005 (replaces bdgflx as keyword - khv,kva,kvv,sto,kdw,vcw all tricker this logical)
   ...
   SELECT CASE (I)
    CASE (TCHD,TSHD)
     IF(ASSOCIATED(PBMAN%ISAVE(I)%ILAY))THEN
      WRITE(IU,'(1X,A)') 'HEAD FILEOUT .\GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT\HEAD\HEAD.HED'; LSHD=.TRUE.; EXIT
     ENDIF
    CASE DEFAULT
     IF(ASSOCIATED(PBMAN%ISAVE(I)%ILAY))THEN
      WRITE(IU,'(1X,A)') 'BUDGET FILEOUT .\GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT\BUDGET\BUDGET.CBC'; LBDG=.TRUE.; EXIT
     ENDIF
   END SELECT
  ENDDO
...
!   WRITE(IU,'(1X,A)') 'HEAD FILEOUT .\GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT\HEAD\HEAD.HED'
  IF(LSHD)CALL UTL_CREATEDIR(TRIM(MAINDIR)//'\GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT\BUDGET')
  IF(LBDG)CALL UTL_CREATEDIR(TRIM(MAINDIR)//'\GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT\HEAD')
