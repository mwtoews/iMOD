...
!  WRITE(IU,'(A)') ' PRINT_FLOWS'
  SELECT CASE (ITOPIC)
   CASE (18); IF(ASSOCIATED(PBMAN%SAVEUZF))WRITE(IU,'(A)') ' SAVE_FLOWS'
   CASE (22); IF(ASSOCIATED(PBMAN%SAVEDRN))WRITE(IU,'(A)') ' SAVE_FLOWS'
   CASE (23); IF(ASSOCIATED(PBMAN%SAVERIV))WRITE(IU,'(A)') ' SAVE_FLOWS'
   CASE (24); IF(ASSOCIATED(PBMAN%SAVEEVT))WRITE(IU,'(A)') ' SAVE_FLOWS'
   CASE (25); IF(ASSOCIATED(PBMAN%SAVEGHB))WRITE(IU,'(A)') ' SAVE_FLOWS'
   CASE (26); IF(ASSOCIATED(PBMAN%SAVERCH))WRITE(IU,'(A)') ' SAVE_FLOWS'
   CASE (27); IF(ASSOCIATED(PBMAN%SAVEDRN))WRITE(IU,'(A)') ' SAVE_FLOWS'
   CASE (28); IF(ASSOCIATED(PBMAN%SAVEFLX))WRITE(IU,'(A)') ' SAVE_FLOWS'
  END SELECT
!  WRITE(IU,'(A)') ' TS6 FILEIN <ts6_filename>'
!  WRITE(IU,'(A)') ' OBS6 FILEIN <obs6_filename>'
!  WRITE(IU,'(A)') ' MOVER'
