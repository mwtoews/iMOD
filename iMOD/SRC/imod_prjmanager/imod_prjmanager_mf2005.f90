...
CONTAINS
 ...
 IF(PBMAN%IFORMAT.EQ.3.AND.PBMAN%ISUBMODEL.EQ.PBMAN%NSUBMODEL)THEN
  ...
  DO I=1,PBMAN%NSUBMODEL; DO J=1,PBMAN%NSUBMODEL
   IF(I.EQ.J)CYCLE
   CALL PMANAGER_SAVEMF6_EXG(MAINDIR,DIRMNAME,I,J) 
  ENDDO; ENDDO
!  CALL PMANAGER_SAVEMF6_EXG(MAINDIR,DIRMNAME,1,2)
!  CALL PMANAGER_SAVEMF6_EXG(MAINDIR,DIRMNAME,2,3)
!  CALL PMANAGER_SAVEMF6_EXG(MAINDIR,DIRMNAME,3,4)
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_SAVEMF6_EXG(DIR,MNAME,M1,M2)
 !###======================================================================
 IMPLICIT NONE
...
! WRITE(IU,'(A)') '[OBS6 FILEIN <obs6_filename>]' !## observation
 ...
 IF(BND(2,1)%DX.LT.BND(1,1)%DX)THEN
  ...
  DEALLOCATE(BND); RETURN
 ENDIF

 ALLOCATE(TOP(2,MAXNLAY),BOT(2,MAXNLAY))
...
! !## who is smallest in cellsize and/or dimension - this model determines the number of connections
! IMDL1=1; IMDL2=2; IMDL(IMDL1)=M1; IMDL(IMDL2)=M2
...
! ENDIF
 ...
 DO I=1,2; DO J=1,SIZE(BND,2); CALL IDFCOPY(BND(2,1),TOP(I,J)); ENDDO; ENDDO
 ...
 DO IM=1,2
  ...
  IF(IM.EQ.1)THEN
   !## read bnd as is
   DO I=1,1;         IF(.NOT.IDFREADSCALE(TRIM(DIR)//'\GWF_'//TRIM(ITOS(M1))//'\MODELINPUT\DIS6\TOP_L'//TRIM(ITOS(I  ))//'.IDF',TOP(1,I),2,1,0.0D0,0))RETURN; ENDDO
   DO I=2,MNLAY(IM); IF(.NOT.IDFREADSCALE(TRIM(DIR)//'\GWF_'//TRIM(ITOS(M1))//'\MODELINPUT\DIS6\BOT_L'//TRIM(ITOS(I-1))//'.IDF',TOP(1,I),2,1,0.0D0,0))RETURN; ENDDO
   DO I=1,MNLAY(IM); IF(.NOT.IDFREADSCALE(TRIM(DIR)//'\GWF_'//TRIM(ITOS(M1))//'\MODELINPUT\DIS6\BOT_L'//TRIM(ITOS(I  ))//'.IDF',BOT(1,I),2,1,0.0D0,0))RETURN; ENDDO
  ELSE
   DO I=1,1;         IF(.NOT.IDFREADSCALE(TRIM(DIR)//'\GWF_'//TRIM(ITOS(M2))//'\MODELINPUT\DIS6\TOP_L'//TRIM(ITOS(I  ))//'.IDF',TOP(2,I),2,1,0.0D0,0))RETURN; ENDDO
   DO I=2,MNLAY(IM); IF(.NOT.IDFREADSCALE(TRIM(DIR)//'\GWF_'//TRIM(ITOS(M2))//'\MODELINPUT\DIS6\BOT_L'//TRIM(ITOS(I-1))//'.IDF',TOP(2,I),2,1,0.0D0,0))RETURN; ENDDO
   DO I=1,MNLAY(IM); IF(.NOT.IDFREADSCALE(TRIM(DIR)//'\GWF_'//TRIM(ITOS(M2))//'\MODELINPUT\DIS6\BOT_L'//TRIM(ITOS(I  ))//'.IDF',BOT(2,I),2,1,0.0D0,0))RETURN; ENDDO
  ENDIF
 ENDDO
...
! ENDIF
 ...
 DO I=1,2
  ...
  DO ILAY=1,MNLAY(1)
   ...
   DO IROW=1,BND(1,ILAY)%NROW; DO ICOL=1,BND(1,ILAY)%NCOL
    ...
    IF(TOP(2,ILAY)%X(ICOL,IROW).NE.TOP(2,ILAY)%NODATA)THEN
     Z1=TOP(ILAY)%X(ICOL,IROW); Z2=TOP(ILAY)%X(ICOL,IROW)
     DO ID=5,6
      SELECT CASE (CID(ID))
       !## try top connection
       CASE ('T')
        DO JLAY=MNLAY(2),1,-1
         ...
         T=TOP(JLAY)%X(ICOL,IROW); B=TOP(JLAY)%X(ICOL,IROW)
         IF(T-B.GT.0.0D0)THEN
          IF(B.GE.Z1)THEN; LEX=.TRUE.; EXIT; ENDIF
         ENDIF
        ENDDO
       !## try bot connection
       CASE ('B')
        DO JLAY=1,MNLAY(2)
         ...
         T=TOP(JLAY)%X(ICOL,IROW); B=TOP(JLAY)%X(ICOL,IROW)
         IF(T-B.GT.0.0D0)THEN
          IF(T.LE.Z2)THEN; LEX=.TRUE.; EXIT; ENDIF
         ENDIF
        ENDDO
      END SELECT
      IF(.NOT.LEX)CYCLE
      IF(PMANAGER_SAVEMF6_EXG_CONNECTIONS(CID(I),IU,ILAY,IROW,ICOL,1,2,MNLAY(1),MNLAY(2), &
         BND(1,:),BND(2,:),TOP(1,:),BOT(1,:),TOP(2,:),BOT(2,:),I))THEN
       N=N+1
       EXIT
      ENDIF
     ENDDO
    ...
    ELSE
     ...
     DO ID=1,4
      ...
      IF(.NOT.LEX)CYCLE
      IF(PMANAGER_SAVEMF6_EXG_CONNECTIONS(CID(I),IU,ILAY,IROW,ICOL,1,2,MNLAY(1),MNLAY(2), &
         BND(1,:),BND(2,:),TOP(1,:),BOT(1,:),TOP(2,:),BOT(2,:),I))N=N+1
     ENDDO
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF6_EXG_CONNECTIONS(CDIR,IU,ILAY,IROW,ICOL,IMDL1,IMDL2,NLAY1,NLAY2, &
    BND1,BND2,TOP1,BOT1,TOP2,BOT2,IIU)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: CDIR
 INTEGER,INTENT(IN) :: ILAY,IROW,ICOL,IMDL1,IMDL2,IU,IIU,NLAY1,NLAY2
 TYPE(IDFOBJ),INTENT(IN),DIMENSION(NLAY1) :: BND1,TOP1,BOT1
 TYPE(IDFOBJ),INTENT(IN),DIMENSION(NLAY2) :: BND2,TOP2,BOT2
 ...
 !## active cell?
 IF(BND1(ILAY)%X(ICOL,IROW).EQ.BND1(ILAY)%NODATA)RETURN

 !## current centre location of fine   model
 CALL IDFGETLOC( BND1(ILAY),IROW,ICOL,XP1,YP1)
 CALL IDFGETEDGE(BND1(ILAY),IROW,ICOL,X1 ,Y1 ,X2 ,Y2)
 !## get vertical position of node
 Z2=TOP1(ILAY)%X(ICOL,IROW); Z1=BOT1(ILAY)%X(ICOL,IROW)
 DZ1=Z2-Z1; ZP1=Z1+0.5D0*DZ1
 !## get cellsize of fine model
 CALL IDFGETDXDY(BND1(ILAY),IROW,ICOL,DX1,DY1)

 !## get location of nearest course model
 SELECT CASE (CDIR)
  CASE ('N'); CALL IDFGETLOC( BND1(ILAY),IROW-1,ICOL,XP,YP); IHC=1; JLAY=ILAY
  CASE ('S'); CALL IDFGETLOC( BND1(ILAY),IROW+1,ICOL,XP,YP); IHC=1; JLAY=ILAY
  CASE ('W'); CALL IDFGETLOC( BND1(ILAY),IROW,ICOL-1,XP,YP); IHC=1; JLAY=ILAY
  CASE ('E'); CALL IDFGETLOC( BND1(ILAY),IROW,ICOL+1,XP,YP); IHC=1; JLAY=ILAY
  CASE ('T'); CALL IDFGETLOC( BND1(ILAY),IROW,ICOL  ,XP,YP); IHC=0; JLAY=NLAY2
  CASE ('B'); CALL IDFGETLOC( BND1(ILAY),IROW,ICOL  ,XP,YP); IHC=0; JLAY=1
 END SELECT
 CALL IDFIROWICOL(BND2(ILAY),JROW,JCOL,XP,YP)
 !## outside parent model
 IF(JROW.LE.0.OR.JCOL.LE.0)RETURN
 
 !## active cell?
 IF(BND2(ILAY)%X(JCOL,JROW).EQ.BND2(ILAY)%NODATA)RETURN
 
 !## get location of cell outside submodel
 CALL IDFGETLOC(BND2(ILAY),JROW,JCOL,XP2,YP2)
 !## get vertical position of node
 DZ2=TOP2(ILAY)%X(JCOL,JROW)-BOT2(ILAY)%X(JCOL,JROW)
 ZP2=BOT2(ILAY)%X(JCOL,JROW)+0.5D0*DZ2
 !## get cellsize of course model
 CALL IDFGETDXDY(BND2(ILAY),JROW,JCOL,DX2,DY2)
...
!    DO ILAY=1,PRJNLAY
!     IF(HK(ILAY).GT.1000.0)THEN
!      DO I=1,PRJNLAY
!       WRITE(*,*) HK(I),VA(I),VK(I)
!      ENDDO
!     ENDIF
!    ENDDO
...
!  IF()WRITE(IU,'(A)') 'NEWTON [UNDER_RELAXATION]'
 ...
 ELSE 
  ...
  DO I=N1,N2
   ...
   IF(TOPICS(TPCG)%IACT_MODEL) WRITE(IU,'(A)') 'PCG  15 '//CHAR(39)//TRIM(DIRMNAME)//'.PCG7'//CHAR(39)
   IF(LPKS) WRITE(IU,'(A)') 'PKS  15 '//CHAR(39)//TRIM(DIRMNAME)//'.PKS'//CHAR(39)
   WRITE(IU,'(A)') 'OC   16 '//CHAR(39)//TRIM(DIRMNAME)//'.OC'//CHAR(39)
   IF(TOPICS(TRCH)%IACT_MODEL) WRITE(IU,'(A)') 'RCH  17 '//CHAR(39)//TRIM(DIRMNAME)//'.RCH7'//CHAR(39)
   IF(TOPICS(TEVT)%IACT_MODEL) WRITE(IU,'(A)') 'EVT  18 '//CHAR(39)//TRIM(DIRMNAME)//'.EVT7'//CHAR(39)
   IF(TOPICS(TDRN)%IACT_MODEL.OR.TOPICS(TOLF)%IACT_MODEL) WRITE(IU,'(A)') 'DRN  19 '//CHAR(39)//TRIM(DIRMNAME)//'.DRN7'//CHAR(39)
   IF(TOPICS(TRIV)%IACT_MODEL.OR.TOPICS(TISG)%IACT_MODEL) WRITE(IU,'(A)') 'RIV  20 '//CHAR(39)//TRIM(DIRMNAME)//'.RIV7'//CHAR(39)
   IF(TOPICS(TGHB)%IACT_MODEL) WRITE(IU,'(A)') 'GHB  21 '//CHAR(39)//TRIM(DIRMNAME)//'.GHB7'//CHAR(39)
   IF(TOPICS(TCHD)%IACT_MODEL) WRITE(IU,'(A)') 'CHD  22 '//CHAR(39)//TRIM(DIRMNAME)//'.CHD7'//CHAR(39)
   IF(TOPICS(TWEL)%IACT_MODEL) WRITE(IU,'(A)') 'WEL  23 '//CHAR(39)//TRIM(DIRMNAME)//'.WEL7'//CHAR(39)
   IF(TOPICS(THFB)%IACT_MODEL) WRITE(IU,'(A)') 'HFB6 24 '//CHAR(39)//TRIM(DIRMNAME)//'.HFB7'//CHAR(39)
   IF(TOPICS(TSFR)%IACT_MODEL) WRITE(IU,'(A)') 'SFR  25 '//CHAR(39)//TRIM(DIRMNAME)//'.SFR7'//CHAR(39)
   IF(TOPICS(TFHB)%IACT_MODEL)THEN; WRITE(IU,'(A)') 'FHB  26 '//CHAR(39)//TRIM(DIRMNAME)//'.FHB7'//CHAR(39); IFHBUN=26; ENDIF
   IF(TOPICS(TLAK)%IACT_MODEL) WRITE(IU,'(A)') 'LAK  27 '//CHAR(39)//TRIM(DIRMNAME)//'.LAK7'//CHAR(39) 
   IF(TOPICS(TUZF)%IACT_MODEL) WRITE(IU,'(A)') 'UZF  28 '//CHAR(39)//TRIM(DIRMNAME)//'.UZF7'//CHAR(39)
   IF(TOPICS(TMNW)%IACT_MODEL) WRITE(IU,'(A)') 'MNW2 29 '//CHAR(39)//TRIM(DIRMNAME)//'.MNW7'//CHAR(39)
   IF(TOPICS(TANI)%IACT_MODEL) WRITE(IU,'(A)') 'ANI  30 '//CHAR(39)//TRIM(DIRMNAME)//'.ANI1'//CHAR(39)
   IF(TOPICS(TCAP)%IACT_MODEL) WRITE(IU,'(A)') 'DXC  31 '//CHAR(39)//TRIM(DIRMNAME)//'.DXC'//CHAR(39) 
   ...
   IF(TOPICS(TRCH)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IRCHCB,' '//CHAR(39)//'BDGRCH'//CHAR(39) 
   IF(TOPICS(TEVT)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IEVTCB,' '//CHAR(39)//'BDGEVT'//CHAR(39) 
   IF(TOPICS(TDRN)%IACT_MODEL.OR.TOPICS(TOLF)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IDRNCB,' '//CHAR(39)//'BDGDRN'//CHAR(39) 
   IF(TOPICS(TRIV)%IACT_MODEL.OR.TOPICS(TISG)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IRIVCB,' '//CHAR(39)//'BDGRIV'//CHAR(39) 
   IF(TOPICS(TGHB)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IGHBCB,' '//CHAR(39)//'BDGGHB'//CHAR(39) 
   IF(TOPICS(TCHD)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',ICHDCB,' '//CHAR(39)//'BDGCHD'//CHAR(39) 
   IF(TOPICS(TWEL)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IWELCB,' '//CHAR(39)//'BDGWEL'//CHAR(39) 
   IF(TOPICS(TSFR)%IACT_MODEL)THEN
   ...
   ENDIF
   IF(TOPICS(TFHB)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IFHBCB ,' '//CHAR(39)//'BDGFHB'//CHAR(39) 
   IF(TOPICS(TLAK)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',ILAKCB ,' '//CHAR(39)//'BDGLAK'//CHAR(39) 
   IF(TOPICS(TUZF)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IUZFCB1,' '//CHAR(39)//'UZFINF BDGGRC BDGGET UZFRUN UZFET UZFSFR'//CHAR(39) 
   IF(TOPICS(TMNW)%IACT_MODEL)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IWL2CB ,' '//CHAR(39)//'BDGMNW'//CHAR(39)
!   IF(TOPICS(TUZF)%IACT_MODEL)THEN
...
!below. This type of cell is referred to as a “vertical pass through” cell.
 ...
 DO ILAY=1,PRJNLAY-1; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
  IF(BND(ILAY)%X(ICOL,IROW).NE.0.AND.BND(ILAY+1)%X(ICOL,IROW).NE.0)THEN
   ...
   T1=0.0D0; IF(T.GT.0.0D0)T1=T/(KHV(ILAY)%X(ICOL,IROW)/KVA(ILAY)%X(ICOL,IROW))
   ...
   IF(T.GT.0.0D0.AND.KHV(ILAY+1)%X(ICOL,IROW).LE.0.0D0)THEN
    WRITE(*,*) T,KHV(ILAY+1)%X(ICOL,IROW),KVA(ILAY+1)%X(ICOL,IROW),ICOL,IROW
   ENDIF
...
!  DO ILAY=1,SIZE(PBMAN%ILAY) 
   ...
   DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
    IF(BND(ILAY)%X(ICOL,IROW).NE.0)THEN
     ...
     PRJIDF%X(ICOL,IROW)=ROT !ANA(ILAY)%X(ICOL,IROW)
    ENDIF
   ENDDO; ENDDO
...
!  WRITE(IU,'(A)') ' PRINT_INPUT'
 ...
 INTEGER,DIMENSION(7) :: TFLX=[TKHV,TKVA,TKVV,TKDW,TVCW,TSTO,TSPY]
 LOGICAL :: LFLX !## if true is saves fluxes for modflow2005 (replaces bdgflx as keyword - khv,kva,kvv,sto,kdw,vcw all tricker this logical)
...
!  WRITE(IU,'(A)') '  HEAD PRINT_FORMAT COLUMNS <columns> WIDTH <width> DIGITS <digits> <format>]'
 ...
 LFLX=.FALSE.; DO I=1,SIZE(TFLX)
  IF(ASSOCIATED(PBMAN%ISAVE(I)%ILAY))THEN; LFLX=.TRUE.; EXIT; ENDIF
 ENDDO
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 REAL(KIND=DP_KIND) :: C,C1,C2,Z,ZZ,TPV,BTV,TFV,BFV,T1,T2,DX,DY,F,T
 INTEGER(KIND=1),ALLOCATABLE,DIMENSION(:,:,:) :: IPC
   ...
   IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
    IF(ICOL.LT.IDF%NCOL)THEN
     ...
     ELSE
      ...
      T=(0.50D0*(TOP(ILAY)%X(       ICOL   ,IROW)-BOT(ILAY)%X(       ICOL   ,IROW)))+ &
        (0.50D0*(TOP(ILAY)%X(MIN(NC,ICOL+1),IROW)-BOT(ILAY)%X(MIN(NC,ICOL+1),IROW)))
   ...
   IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
    IF(IROW.LT.IDF%NROW)THEN
     ...
     ELSE
      ...
      CALL IDFGETDXDY(BND(ILAY),ICOL,IROW,DX,DY)
      T=(0.50D0*(TOP(ILAY)%X(ICOL,       IROW   )-BOT(ILAY)%X(ICOL,        IROW)))+ &
        (0.50D0*(TOP(ILAY)%X(ICOL,MIN(NR,IROW+1))-BOT(ILAY)%X(ICOL,MIN(NR,IROW+1))))
...
!tib = 2
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_BND()
 !###======================================================================
 IMPLICIT NONE
...
! DO ILAY=1,PRJNLAY
 ILAY=0; DO I=1,SIZE(PBMAN%ILAY)
  ...
  !IF(PBMAN%NSUBMODEL.GT.1.AND.PBMAN%IFORMAT.EQ.3)THEN  !RETURN
  
  IF(PBMAN%NSUBMODEL.LE.1.OR.PBMAN%IFORMAT.NE.3)THEN 
