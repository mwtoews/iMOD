...
IF(TOPICS(TSFT)%DEFINED)THEN
 ...
 ELSE
  ...
  !## quasi-3d scheme add top aquifer modellayer
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\DIS6\TOPM_L'//TRIM(ITOS(1))//'.ARR', &
     TOP(I),0,IU,1,ITOPIC))RETURN  
  
  !## write idf for connection-purposes
  IF(.NOT.IDFWRITE(TOP(I),TRIM(DIR)//'\DIS6\TOPM_L'//TRIM(ITOS(1))//'.IDF',1))RETURN
  
  WRITE(IU,'(A)') 'BOTM LAYERED'
  ITOPIC=TBOT
  JLAY=1; DO ILAY=I+1,SIZE(PBMAN%ILAY)
   IF(PBMAN%ILAY(ILAY).EQ.0)CYCLE
   JLAY=JLAY+1
...
!       BOT(ILAY),0,IU,ILAY,ITOPIC))RETURN
   ...
   IF(JLAY.EQ.N)THEN !ILAY.EQ.SIZE(PBMAN%ILAY))THEN
    IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\DIS6\BOTM_L'//TRIM(ITOS(JLAY))//'.ARR', &
        BOT(ILAY),0,IU,ILAY,ITOPIC))RETURN
   ENDIF
   !## write idf for connection-purposes
