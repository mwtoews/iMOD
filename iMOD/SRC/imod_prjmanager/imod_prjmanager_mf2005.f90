 ...
 IF(PBMAN%IFORMAT.EQ.3.AND.PBMAN%ISUBMODEL.EQ.1)THEN
  ...
  DO I=N1,N2
   ...
   ELSEIF(PBMAN%IPESTP.EQ.1)THEN
    IF(I.GT.0)THEN
     IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE
     NAME=TRIM(MAINDIR)//'\IPEST_P#'//TRIM(ITOS(I)); CALL UTL_CREATEDIR(NAME)
    ELSE
     NAME=TRIM(MAINDIR)//'\IPEST_L#'//TRIM(ITOS(ABS(I))); CALL UTL_CREATEDIR(NAME)
    ENDIF
   ELSEIF(PBMAN%IIES.EQ.1)THEN
    NAME=TRIM(MAINDIR)//'\IPEST_R#'//TRIM(ITOS(I)); CALL UTL_CREATEDIR(NAME)
   ENDIF
...
!     IF(SUM(TLP).GT.0.0D0)TLP=(1.0D0/SUM(TLP))*TLP
 ...
 DO ISYS=1,NSYS
  ...
  NP=0; DO I=1,NROWIPF
   ...
   WRITE(JU,'(A)') TRIM(OBSNAME)//','//TRIM(RTOS(X,'F',3))//','//TRIM(RTOS(Y,'F',3))
   WRITE(JU,'(A)') 'TIME,HEAD,WEIGTH,ILAY'
   ...
   DO IPER=1,PRJNPER
    ...
    DO ILAY=1,SIZE(PBMAN%ILAY)
     IF(PBMAN%ILAY(ILAY).EQ.0)CYCLE
     IF(TLP(ILAY).GT.0.0D0)THEN
      WRITE(JU,'(A)') TRIM(ITOS_DBL(ITIME))//','//TRIM(RTOS(H,'F',3))//','//TRIM(RTOS(W,'F',3))//','//TRIM(ITOS(ILAY))
     ENDIF
    ENDDO
   ENDDO
