...
USE MOD_ASC2IDF_UTL
USE MOD_OSD
!USE MOD_QKSORT, ONLY : QKSORT_INT4REAL8
USE MOD_IPEST_GLM, ONLY : IPEST_GLM_SETGROUPS

CONTAINS

 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS(GENFNAME,OUTFOLDER)
 !###======================================================================
 IMPLICIT NONE
 ...
 ALLOCATE(ISORT(SHP%NPOL)); DO I=1,SHP%NPOL; ISORT=I; ENDDO; CALL PMANAGER_GENERATEMFNETWORKS_SORT(SHP%NPOL,ISORT)
 ALLOCATE(IDF(SHP%NPOL)); DO I=1,SHP%NPOL; CALL IDFNULLIFY(IDF(I)); ENDDO 
 
 !## get dimensions of the idf files
 CALL PMANAGER_GENERATEMFNETWORKS_DIMIDF(IDF)
 ...
 DO I=1,SHP%NPOL
  !## project on previous submodel
  J=I; IF(I.GT.1)J=I-1; 
  
  CALL ASC2IDF_INT_NULLIFY(); ALLOCATE(XP(100),YP(100),ZP(100),WP(100),FP(100)); ALLOCATE(IPC(IDF(J)%NCOL,IDF(J)%NROW,2)); IPC=INT(0,1)
  
  !## intersect line and determine ipc()
  CALL ASC2IDF_HFB(IDF(J),IDF(J)%NROW,IDF(J)%NCOL,IPC,(/GENFNAME/),-1,IPOL=I)
  !## write genfiles
  CALL PMANAGER_GENERATEMFNETWORKS_WRITEGEN(IDF(J),IPC,IU,JU,I)
 ...
 INTEGER,DIMENSION(:),INTENT(IN) :: ISORT
 ...
 DO J=SHP%NPOL,1,-1
  ...
  DO IROW=1,IDF(I)%NROW; DO ICOL=1,IDF(I)%NCOL
   ...
   CALL IDFGETLOC(IDF(I),IROW,ICOL,X1,Y1)
   IF(DBL_IGRINSIDESHAPE(X1,Y1,SHP%POL(I)).EQ.1)THEN
    IDF(I)%X(ICOL,IROW)=1.0D0
    !## deactivate others on this location
    DO JJ=J-1,1,-1 
     II=ISORT(JJ)
     IF(DBL_IGRINSIDESHAPE(X1,Y1,SHP%POL(II)).EQ.1)THEN
      CALL IDFIROWICOL(IDF(II),JROW,JCOL,X1,Y1)
      IDF(II)%X(JCOL,JROW)=IDF(II)%NODATA 
     ENDIF
    ENDDO
   ELSE
    IDF(I)%X(ICOL,IROW)=IDF(I)%NODATA !0.0D0
   ENDIF
  ENDDO; ENDDO
 ENDDO
 ...
 DO I=1,N; JS(I)=I; ENDDO; JS(1)=0; IS(1)=1; NS=1
 
 DO
  DO I=1,N
   !## already used   
   IF(JS(I).EQ.0)CYCLE
   IF(PMANAGER_GENERATEMFNETWORKS_PUZZLEFIT(XC,YC,IS(NS),JS(I)))THEN
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_GENERATEMFNETWORKS_PUZZLEFIT(XC,YC,IS,JS)
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),DIMENSION(:,:),INTENT(INOUT) :: XC,YC
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS_DIMIDF(IDF)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(:) :: IDF
 ...
 DO I=1,SHP%NPOL
  ...
  IDF(I)%XMIN=SHP%POL(I)%XMIN
  IDF(I)%YMIN=SHP%POL(I)%YMIN
  IDF(I)%XMAX=SHP%POL(I)%XMAX
  IDF(I)%YMAX=SHP%POL(I)%YMAX
  ...
  IDF(I)%DY=IDF(I)%DX; CS=IDF(I)%DX; IF(I.GT.1)CS=IDF(I-1)%DX
 ...
 IF(IT.EQ.1)THEN
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   ...
   WRITE(IU,'(A)') 'END'
   WRITE(JU,'(I10,4(A1,F15.3))') N,',',IDF%SX(ICOL),',',IDF%SY(IROW-1),',',IDF%SX(ICOL),',',IDF%SY(IROW)
  ENDIF
 ENDIF
 ...
 IF(IT.EQ.2)THEN
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   ...
   WRITE(IU,'(2(F15.3,A1))') IDF%SX(ICOL  ),',',IDF%SY(IROW)
   WRITE(IU,'(A)') 'END'
   WRITE(JU,'(I10,4(A1,F15.3))') N,',',IDF%SX(ICOL-1),',',IDF%SY(IROW),',',IDF%SX(ICOL  ),',',IDF%SY(IROW)
  ENDIF
 ENDIF
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GENERATEMFNETWORKS_SORT(N,ISORT)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N
 INTEGER,INTENT(OUT),DIMENSION(N) :: ISORT
 INTEGER :: SHPI
 ...
 DO SHPI=1,SHP%NPOL
  IF(SHP%POL(SHPI)%N.GT.0)THEN
   SELECT CASE (SHP%POL(SHPI)%ITYPE)
    CASE (ID_RECTANGLE)
     POLAREA(SHPI)=ABS(SHP%POL(SHPI)%X(1)-SHP%POL(SHPI)%X(2))*ABS(SHP%POL(SHPI)%Y(1)-SHP%POL(SHPI)%Y(2))
    CASE (ID_POLYGON)
     POLAREA(SHPI)=ABS(UTL_POLYGON1AREA(SHP%POL(SHPI)%X,SHP%POL(SHPI)%Y,SHP%POL(SHPI)%N))
    CASE (ID_CIRCLE)
     POLAREA(SHPI)=2.0D0*UTL_DIST(SHP%POL(SHPI)%X(1),SHP%POL(SHPI)%Y(1),SHP%POL(SHPI)%X(2),SHP%POL(SHPI)%Y(2))
     POLAREA(SHPI)=PI*POLAREA(SHPI)**2.0D0
    CASE DEFAULT
     POLAREA(SHPI)=0.0D0
   END SELECT
  ENDIF
 ...
 CALL WSORT(POLAREA,1,SHP%NPOL,IFLAGS=SORTDESCEND,IORDER=ISORT) !CALL QKSORT_INT4REAL8(SHP%NPOL,ISORT,V2=POLAREA)
 ...
 !## save sfr package
 IF(.NOT.PMANAGER_SAVEMF2005_ISG(DIR,DIRMNAME,IBATCH,LSFR,30,IRIVCB,'SFR',IPRT))RETURN
 !## save fhb package
...
!  ENDDO
 ...
 !CHARACTER(LEN=4),DIMENSION(5) :: PCK
 !LOGICAL :: LEX
 !DATA PCK/'CHD6','WEL6','DRN6','RCH6','RIV6'/
 !
 ...
 INTEGER :: IU,JU,KU,I,J,K,IM,N,IOS,II,ILAY,MAXNLAY,IROW,ICOL,IMDL1,IMDL2,JROW,JCOL
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:,:) :: TOP,BOT,BND
 INTEGER,DIMENSION(2) :: MNLAY,IMDL
 CHARACTER(LEN=256) :: FNAME,LINE
 CHARACTER(LEN=52) :: TXT,MDLNAME
 CHARACTER(LEN=1) :: TLAYMODEL
 CHARACTER(LEN=5),DIMENSION(4) :: PCK
 DATA PCK/'RIV6','RCH6','WEL6','DRN6'/
...
! WRITE(IU,'(A)') '[OBS6 FILEIN <obs6_filename>]' !## observation
 ...
 DO II=1,2
  ...
  ENDDO
  IF(II.EQ.1)THEN
   ALLOCATE(TOP(2,MAXNLAY),BOT(2,MAXNLAY),BND(2,MAXNLAY))
 ...
 ELSEIF(BND(2,1)%XMIN.EQ.BND(1,1)%XMIN.AND. &
  ...
  ENDDO; ENDDO
  IF(TLAYMODEL.EQ.'')THEN
   WRITE(*,'(/1X,A/)') 'Cannot position model vertically'; STOP
  ENDIF
 ENDIF
 ...
 DO I=1,2
  ...
  DO ILAY=1,MNLAY(IMDL1)
   ...
   IF(LSUBMODEL)THEN
    ...
    IROW=BND(IMDL1,ILAY)%NROW; DO ICOL=1,BND(IMDL1,ILAY)%NCOL
     IF(PMANAGER_SAVEMF6_EXG_CONNECTIONS('S',IU,ILAY,IROW,ICOL,IMDL1,IMDL2,MNLAY(IMDL1),MNLAY(IMDL2), &
        BND(IMDL1,ILAY),BND(IMDL2,ILAY),TOP(IMDL1,ILAY),BOT(IMDL1,ILAY),TOP(IMDL2,ILAY),BOT(IMDL2,ILAY),I))N=N+1
    ENDDO

    !## west connection
    IF(I.EQ.2)WRITE(IU,'(/A)') '# West Connection'
    ICOL=1; DO IROW=1,BND(IMDL1,ILAY)%NROW
     IF(PMANAGER_SAVEMF6_EXG_CONNECTIONS('W',IU,ILAY,IROW,ICOL,IMDL1,IMDL2,MNLAY(IMDL1),MNLAY(IMDL2), &
        BND(IMDL1,ILAY),BND(IMDL2,ILAY),TOP(IMDL1,ILAY),BOT(IMDL1,ILAY),TOP(IMDL2,ILAY),BOT(IMDL2,ILAY),I))N=N+1
    ENDDO

    !## east connection
    IF(I.EQ.2)WRITE(IU,'(/A)') '# East Connection'
    ICOL=BND(IMDL1,ILAY)%NCOL; DO IROW=1,BND(IMDL1,ILAY)%NROW
     IF(PMANAGER_SAVEMF6_EXG_CONNECTIONS('E',IU,ILAY,IROW,ICOL,IMDL1,IMDL2,MNLAY(IMDL1),MNLAY(IMDL2), &
        BND(IMDL1,ILAY),BND(IMDL2,ILAY),TOP(IMDL1,ILAY),BOT(IMDL1,ILAY),TOP(IMDL2,ILAY),BOT(IMDL2,ILAY),I))N=N+1
    ENDDO
   
    !## clean cells in course model
    DO IROW=1,BND(IMDL1,ILAY)%NROW; DO ICOL=1,BND(IMDL1,ILAY)%NCOL
     CALL IDFGETLOC(BND(IMDL1,ILAY),IROW,ICOL,XP,YP)
     CALL IDFIROWICOL(BND(IMDL2,ILAY),JROW,JCOL,XP,YP)
     BND(IMDL2,ILAY)%X(JCOL,JROW)=0.0D0
    ENDDO; ENDDO
 ...
 DO I=1,SIZE(BND,2)
  IU=UTL_GETUNIT()
  CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL(IMDL1)))//'\MODELINPUT\DIS6\IBOUND_L'//TRIM(ITOS(I))//'.ARR',STATUS='UNKNOWN', &
      ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  CALL IDFWRITEFREE(IU,BND(IMDL1,I),1,'B','*') 
  !## rewrite idf files
  IF(.NOT.IDFWRITE(BND(IMDL1,I),BND(IMDL1,I)%FNAME,1))RETURN
  CLOSE(IU)
 ENDDO
 DO I=1,SIZE(BND,2)
  IU=UTL_GETUNIT()
  CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL(IMDL2)))//'\MODELINPUT\DIS6\IBOUND_L'//TRIM(ITOS(I))//'.ARR',STATUS='UNKNOWN', &
      ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  CALL IDFWRITEFREE(IU,BND(IMDL2,I),1,'B','*') 
  !## rewrite idf files
  IF(.NOT.IDFWRITE(BND(IMDL2,I),BND(IMDL2,I)%FNAME,1))RETURN
  CLOSE(IU)
 ENDDO
 
 !## clean packages - need to remove them (or put them on an active location and deactivate them)
 DO I=1,SIZE(PCK)
  
  INQUIRE(FILE=TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL2))//'\MODELINPUT\'//TRIM(MDLNAME)//'.'//TRIM(PCK(I)),EXIST=LEX)
  IF(.NOT.LEX)EXIT
  
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'\GWF_'//TRIM(ITOS(IMDL2))//'\MODELINPUT\'//TRIM(MDLNAME)//'.'//TRIM(PCK(I)), &
    STATUS='OLD',ACTION='READ',FORM='FORMATTED'); IF(IU.EQ.0)RETURN

  DO
   READ(IU,'(A256)',IOSTAT=IOS) LINE
   IF(IOS.NE.0)EXIT

   IF(INDEX(LINE,'BEGIN PERIOD').GT.0)THEN

    READ(IU,'(A256)',IOSTAT=IOS) LINE
    FNAME=LINE(INDEX(LINE,'\')+1:INDEX(LINE,'ARR',.TRUE.)+3)
    ...
    IF(LEX)THEN
     JU=UTL_GETUNIT()
     CALL OSD_OPEN(JU,FILE=FNAME,STATUS='OLD',ACTION='READ',FORM='FORMATTED'); IF(JU.EQ.0)RETURN
     KU=UTL_GETUNIT()
     CALL OSD_OPEN(KU,FILE=TRIM(FNAME)//'_',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(KU.EQ.0)RETURN
     ...
     DO
      ...
      !## skip inactive cells
      IF(BND(IMDL2,ILAY)%X(ICOL,IROW).EQ.0.0D0)CYCLE
      N=N+1; WRITE(KU,'(A)') TRIM(LINE)
     ENDDO

     CLOSE(JU,STATUS='DELETE'); CLOSE(KU)
     CALL IOSRENAMEFILE(TRIM(FNAME)//'_',TRIM(FNAME))
 ...
 ELSEIF(PBMAN%ICONSISTENCY.EQ.2)THEN
  ...
  ALLOCATE(TP(PRJNLAY)   ,BT(PRJNLAY)   ,HK(PRJNLAY)   ,VK(PRJNLAY-1)   ,VA(PRJNLAY)   ,IB(PRJNLAY),TH(PRJNLAY,2), &
           TP_BU(PRJNLAY),BT_BU(PRJNLAY),HK_BU(PRJNLAY),VK_BU(PRJNLAY-1),VA_BU(PRJNLAY))
  DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL
   ...
   DO ILAY=1,PRJNLAY-1; VK(ILAY)=KVV(ILAY)%X(ICOL,IROW); ENDDO
   IF(ICOL.EQ.904.AND.IROW.EQ.571)THEN
    WRITE(*,*) 
   ENDIF

   CALL UTL_MINTHICKNESS(TP,BT,HK,VK,VA,TP_BU,BT_BU,HK_BU,VK_BU,VA_BU,IB,TH,PBMAN%MINTHICKNESS)
   DO ILAY=1,PRJNLAY  ; IB(ILAY)=BND(ILAY)%X(ICOL,IROW); ENDDO
...
! ENDIF
 ...
 !## shd settings
 ITOPIC=5; SCL_D=1; SCL_U=2; PRJILIST=ITOPIC; IF(PMANAGER_GETFNAMES(1,PRJNLAY,0,1,0).LE.0)RETURN
 ...
 ELSE
  ...
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIRMNAME)//'.IC',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# IC File Generated by '//TRIM(UTL_IMODVERSION())
  WRITE(IU,'(/A/)') '#General Options'
  WRITE(IU,'(A)') 'BEGIN OPTIONS'
  WRITE(IU,'(A)') 'END OPTIONS'
  ...
  JLAY=0; DO ILAY=1,SIZE(PBMAN%ILAY) !PRJNLAY
...
!below. This type of cell is referred to as a “vertical pass through” cell.
 ...
 !####====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: WETDRYTHRESS=0.1 !1.0D0 <- converges
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 INTEGER,INTENT(IN) :: IBATCH,ISS
 REAL(KIND=DP_KIND) :: WETFCT,T
...
!associated with dry cells. 
 ...
 !####====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: WETDRYTHRESS=0.1D0 !1.0D0 <- converges
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 INTEGER,INTENT(IN) :: IBATCH
REAL(KIND=DP_KIND) :: WETFCT,T,TMP_KH,KHMIN,TMP_TOP,TMP_BOT,THICK
...
! WRITE(IU,'(A)') ' [SAVE_SPECIFIC_DISCHARGE]'
 ...
 !## mf6 nees minimal k for layers with thickness of zero
 KHMIN=1.0D0
 DO ILAY=1,SIZE(PBMAN%ILAY)
  DO IROW=1,KHV(ILAY)%NROW
   DO ICOL=1,KHV(ILAY)%NCOL
    TMP_KH =KHV(ILAY)%X(ICOL,IROW)
    TMP_TOP=TOP(ILAY)%X(ICOL,IROW)
    TMP_BOT=BOT(ILAY)%X(ICOL,IROW)
    THICK  =TMP_TOP-TMP_BOT
    IF(THICK.LE.1.0D-4.AND.TMP_KH.LE.1.0D-2)KHV(ILAY)%X(ICOL,IROW)=KHMIN
   ENDDO
  ENDDO
 ENDDO
...
!## IFVDL SFT RCNC
 ...
 IF(IPER.GT.NPER)THEN
  IF(ITOPIC.EQ.29)THEN
   CALL UTL_MF2005_MAXNO(TRIM(DIRMNAME)//'.'//CPCK//'7_',(/NP(2)/))
  ELSE
   CALL UTL_MF2005_MAXNO(TRIM(DIRMNAME)//'.'//CPCK//'7_',NP)
  ENDIF
...
!  ITMP1=1; IF(KPER.EQ.0)ITMP1=0; IF(KPER.LT.0)ITMP1=-1
 ...
 CALL UTL_MF2005_MAXNO(TRIM(DIRMNAME)//'.HFB7_',(/SUM(NHFBNP)/)) 
...
!  WRITE(IU,'(A)') '  HEAD PRINT_FORMAT COLUMNS <columns> WIDTH <width> DIGITS <digits> <format>]'
 ...
 DO IPER=1,PRJNPER
  ...
  IF(PBMAN%IFORMAT.EQ.2)THEN
   ...
   IF(LDRN)CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%SAVEDRN,IDRNCB,IU)
   IF(LRIV)CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%SAVERIV,IRIVCB,IU)
...
!end
 ...
 INTEGER :: NUND,MDND,MDND2,IROW,ICOL,LYBE,TYBE,BEREGENID,JROW,JCOL,N,M,I,J,JU,IC1,IC2,IR1,IR2
   ...
   DO ICOL=1,PRJIDF%NCOL
    ...
    IF(ARND.GT.0.0D0)THEN
     ...
     ELSE
      ...
      ENDIF
     ENDIF    
     MDND2=      (JROW-1)*PRJIDF%NCOL+JCOL
     MDND2=MDND2+(LYBE-1)*PRJIDF%NCOL*PRJIDF%NROW
     ...
     IF(JROW.NE.0.AND.JCOL.NE.0)THEN
      ...
      IF(FLBE.GT.0.0D0)THEN
       IF(TYBE.EQ.1)THEN
        IF(IACT.EQ.2)WRITE(ISCAP,'(I10,F8.2,24X,I10,I6)') NUND,QBER,NUND,LYBE
       ELSEIF(TYBE.EQ.2)THEN
...
!tib = 2
 ...
 DO IROW=1,BND(ILAY)%NROW
  DO ICOL=1,BND(ILAY)%NCOL
   IF(BND(ILAY)%X(ICOL,IROW).EQ.HNOFLOW)BND(ILAY)%X(ICOL,IROW)=0
  ENDDO
 ENDDO
