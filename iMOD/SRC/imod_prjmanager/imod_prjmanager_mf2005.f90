...
CONTAINS
 ...
 WRITE(6,'(A)') '+Reading Zones and writing PARAM_DUMP_IPEST.DAT file'
 ...
 IF(ASSOCIATED(PEST%IDFFILES))THEN
  ...
  DO I=1,SIZE(PEST%IDFFILES)
   ...
   WRITE(6,'(A)') '+Reading Zones and writing PARAM_DUMP_IPEST.DAT file ('//TRIM(RTOS(REAL(100*I,8)/REAL(SIZE(PEST%IDFFILES),8),'F',2))//'%)         '     
   LINE=TRIM(PEST%IDFFILES(I))
 ...
 ENDIF
 DO I=1,SIZE(PEST%PARAM)
  WRITE(JU,*) PEST%PARAM(I)%NODES,PEST%PARAM(I)%ZTYPE
  IF(PEST%PARAM(I)%ZTYPE.EQ.0)THEN
   DO J=1,PEST%PARAM(I)%NODES
    WRITE(JU,*) PEST%PARAM(I)%IROW(J),PEST%PARAM(I)%ICOL(J),PEST%PARAM(I)%F(J)
   ENDDO
  ELSE
   DO J=1,PEST%PARAM(I)%NODES
    WRITE(JU,*) PEST%PARAM(I)%XY(J,1),PEST%PARAM(I)%XY(J,2)
   ENDDO
  ENDIF
 ENDDO
 CLOSE(JU)  
...
!  ENDIF
 ...
 IF(PBMAN%IFORMAT.EQ.3)THEN
  IF(.NOT.PMANAGER_SAVEMF2005_OBS(MAINDIR,DIRMNAME,IBATCH,TOPICS(TOBS)%IACT_MODEL,TOBS,'OBS',1))RETURN
 ENDIF
    ...
    DO
     ...
     DO J=1,SIZE(PCK)
      IF(INDEX(LINE,PCK(J)).GT.0)THEN
       ...
       KU=UTL_GETUNIT(); CALL OSD_OPEN(KU,FILE=PCKFNAME,STATUS='OLD',ACTION='READ',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
       LEX=.TRUE.
       DO
        READ(KU,'(A256)') STRING
 ...
 IF(PBMAN%IFORMAT.EQ.3.AND.PBMAN%ISUBMODEL.EQ.1)THEN
  ...
  DO I=N1,N2
   ...
   ELSEIF(PBMAN%IPESTP.EQ.1)THEN
    IF(I.GT.0)THEN
     IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE
     NAME=TRIM(MAINDIR)//'\P#'//TRIM(ITOS(I)); CALL UTL_CREATEDIR(NAME)
    ELSE
     NAME=TRIM(MAINDIR)//'\L#'//TRIM(ITOS(ABS(I))); CALL UTL_CREATEDIR(NAME)
    ENDIF
   ELSEIF(PBMAN%IIES.EQ.1)THEN
    NAME=TRIM(MAINDIR)//'\R#'//TRIM(ITOS(I)); CALL UTL_CREATEDIR(NAME)
   ENDIF
...
![MEMORY_PRINT_OPTION <memory_print_option>]
   ...
   DO K=1,PBMAN%NSUBMODEL
    IF(PBMAN%IPESTP+PBMAN%IIES.EQ.0)THEN
     NAME=MNAME !TRIM(MAINDIR) !CALL OSD_OPEN(IU,FILE=TRIM(FNAME),STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
    ELSEIF(PBMAN%IPESTP.EQ.1)THEN
     IF(I.GT.0)THEN
      IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE
      NAME='P#'//TRIM(ITOS(I))
     ELSE
      NAME='L#'//TRIM(ITOS(ABS(I)))
     ENDIF
    ELSEIF(PBMAN%IIES.EQ.1)THEN
     NAME='R#'//TRIM(ITOS(I))
    ENDIF
    WRITE(IU,'(A)') ' GWF6 '//TRIM(CRELDIR)//'GWF_'//TRIM(ITOS(K))//'\'//TRIM(MNAME)//'_'//TRIM(NAME)//'.NAM GWF_'//TRIM(ITOS(K))
!    WRITE(IU,'(A)') ' GWF6 '//TRIM(CRELDIR)//'GWF_'//TRIM(ITOS(K))//'\'//TRIM(MNAME)//'.NAM GWF_'//TRIM(ITOS(K))
   ENDDO
 ...
 IF(PBMAN%IFORMAT.EQ.3)THEN
  ...
  DO I=N1,N2
   ...
   ELSEIF(PBMAN%IPESTP.EQ.1)THEN
    IF(I.GT.0)THEN
     IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE
     NAME=TRIM(DIR)//'\'//TRIM(MNAME)//'_P#'//TRIM(ITOS(I))//'.NAM'
    ELSE
     NAME=TRIM(DIR)//'\'//TRIM(MNAME)//'_L#'//TRIM(ITOS(ABS(I)))//'.NAM'
    ENDIF
   ELSEIF(PBMAN%IIES.EQ.1)THEN
    NAME=TRIM(DIR)//'\'//TRIM(MNAME)//'_R#'//TRIM(ITOS(I))//'.NAM'
   ENDIF
   CALL OSD_OPEN(IU,FILE=TRIM(NAME),STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
   IF(IU.EQ.0)RETURN
   ...
   ELSEIF(PBMAN%IPESTP.EQ.1)THEN
    IF(I.GT.0)THEN
     WRITE(IU,'(A)') ' LIST '//TRIM(CRELDIR)//'GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\'//TRIM(MNAME)//'_P#'//TRIM(ITOS(I))//'.LST'
    ELSE
     WRITE(IU,'(A)') ' LIST '//TRIM(CRELDIR)//'GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\'//TRIM(MNAME)//'_L#'//TRIM(ITOS(ABS(I)))//'.LST'
    ENDIF
   ELSEIF(PBMAN%IIES.EQ.1)THEN
    WRITE(IU,'(A)') ' LIST '//TRIM(CRELDIR)//'GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\'//TRIM(MNAME)//'_R#'//TRIM(ITOS(I))//'.LST'
   ENDIF
   WRITE(IU,'(A)') 'END OPTIONS'
   ...
   ELSEIF(PBMAN%IPESTP.EQ.1)THEN
    IF(I.GT.0)THEN
     WRITE(IU,'(A)') ' NPF6 '//TRIM(DIRMNAME)//'_P#'//TRIM(ITOS(I))//'.NPF6'
     WRITE(IU,'(A)') ' OC6  '//TRIM(DIRMNAME)//'_P#'//TRIM(ITOS(I))//'.OC6'
    ELSE
     WRITE(IU,'(A)') ' NPF6 '//TRIM(DIRMNAME)//'_L#'//TRIM(ITOS(ABS(I)))//'.NPF6'
     WRITE(IU,'(A)') ' OC6  '//TRIM(DIRMNAME)//'_L#'//TRIM(ITOS(ABS(I)))//'.OC6'
    ENDIF
   ELSEIF(PBMAN%IIES.EQ.1)THEN
    WRITE(IU,'(A)') ' NPF6 '//TRIM(DIRMNAME)//'_R#'//TRIM(ITOS(I))//'.NPF6'
    WRITE(IU,'(A)') ' OC6  '//TRIM(DIRMNAME)//'_R#'//TRIM(ITOS(I))//'.OC6'    
   ENDIF
   ...
   IF(TOPICS(TPST)%DEFINED.OR.TOPICS(TIES)%DEFINED)THEN
    IF(PBMAN%IPESTP.EQ.1)THEN
     IF(I.GT.0)THEN
      WRITE(IU,'(A)') ' OBS6 '//TRIM(DIRMNAME)//'_P#'//TRIM(ITOS(I))//'.OBS6'
     ELSE
      WRITE(IU,'(A)') ' OBS6 '//TRIM(DIRMNAME)//'_L#'//TRIM(ITOS(ABS(I)))//'.OBS6'
     ENDIF
    ELSEIF(PBMAN%IIES.EQ.1)THEN
     WRITE(IU,'(A)') ' OBS6 '//TRIM(DIRMNAME)//'_R#'//TRIM(ITOS(I))//'.OBS6'
    ENDIF
   ELSE
...
! ENDIF
 ...
 IF(PBMAN%IFORMAT.EQ.3)THEN
  ...
  IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'\PARAM_DUMP_IPEST.DAT'//'...')
  IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'\PARAM_DUMP_IPEST.DAT'//'...'
 ...
 INTEGER,INTENT(IN) :: IBATCH
 INTEGER :: I,II,J,N,IU,JU,IOS,ILAY,N1,N2,ISUB
 ...
 DO I=N1,N2
  ...
  !## change npf (copy) for now into an #.npf
  DO J=1,SIZE(PEST%PARAM)

   !## original model
   FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELINPUT\'//TRIM(MDLNAME)//'.NPF6'
   IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=FNAME,STATUS='OLD',ACTION='READ' ,FORM='FORMATTED')
 
   IF(PBMAN%IPESTP.EQ.1)THEN
    IF(I.GT.0)THEN
     IF(PEST%PARAM(J)%PACT.EQ.0.OR.PEST%PARAM(J)%PIGROUP.LT.0)CYCLE
     FTYPE='P#'//TRIM(ITOS(I))
    ELSE
   ...
   ELSEIF(PBMAN%IIES.EQ.1)THEN
    FTYPE='R#'//TRIM(ITOS(I))
   ENDIF
   FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELINPUT\'//TRIM(MDLNAME)//'_'//TRIM(FTYPE)//'.NPF6'
  
   JU=UTL_GETUNIT()
   CALL OSD_OPEN(JU,FILE=TRIM(FNAME),STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
   DO
    READ(IU,'(A256)',IOSTAT=IOS) LINE; IF(IOS.NE.0)EXIT
    DO II=1,SIZE(PEST%PARAM)
     ...
     SELECT CASE (PEST%PARAM(II)%PPARAM)
      CASE ('KH')
       LINE=UTL_SUBST(LINE,'K_L'//TRIM(ITOS(ILAY))//'.ARR','K_L'//TRIM(ITOS(ILAY))//'_'//TRIM(FTYPE)//'.ARR')
     END SELECT
    ENDDO
    WRITE(JU,'(A)') TRIM(LINE)
   ENDDO
 ...
 DO I=N1,N2
  ...
  DO J=1,SIZE(PEST%PARAM)
   ...
   DO
    READ(IU,'(A256)',IOSTAT=IOS) LINE; IF(IOS.NE.0)EXIT
    LINE=UTL_SUBST(LINE,'OUTPUT_OBS.TXT','OUTPUT_OBS_'//TRIM(FTYPE)//'.TXT')
    WRITE(JU,'(A)') TRIM(LINE)
   ENDDO
   CLOSE(IU); CLOSE(JU)
  ENDDO
...
!     IF(SUM(TLP).GT.0.0D0)TLP=(1.0D0/SUM(TLP))*TLP
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_OBS(MAINDIR,DIRMNAME,IBATCH,IACT,ITOPIC,CPCK,IOPTION)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IBATCH,ITOPIC,IACT,IOPTION
 CHARACTER(LEN=*),INTENT(IN) :: MAINDIR,DIRMNAME,CPCK
 REAL(KIND=DP_KIND) :: X,Y,Z1,Z2,FCT,IMP,CNST,H,NCOUNT,W
 CHARACTER(LEN=256) :: SFNAME,EXFNAME,OBSNAME,CID,CDIR
 CHARACTER(LEN=5) :: EXT
 CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:) :: STRING
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,J,NROWIPF,NCOLIPF,IEXT,IOS,N,NP,MP,ICNST,ISYS,NSYS,JLAY,IPER,KPER,IP, &
...
!         TRIM(ITOS(PEST%MEASURES(I)%IPFTYPE))//','// &
  ...
  ELSEIF(IOPTION.EQ.2)THEN
   N=MAX(IXCOL,IYCOL,ILCOL,IMCOL,IVCOL)
  ENDIF
  ...
  NP=0; DO I=1,NROWIPF
   ...
   ELSEIF(IOPTION.EQ.2)THEN
    ...
    IF(IVCOL.LT.0)THEN; IF(W.LE.0.0D0)THEN; W=0.0D0; ELSE; W=1.0D0/W**2.0D0; ENDIF; ENDIF
   ENDIF
   ...
   WRITE(JU,'(A)') TRIM(OBSNAME)
   WRITE(JU,'(A14,2A15)') 'TIME','H','W'
   ...
   DO IPER=1,PRJNPER
    ...
    IF(IEXT.GT.0)THEN
     IF(.NOT.UTL_PCK_READTXT(2,ITIME,JTIME,H,TRIM(CDIR)//'\'//TRIM(CID)//'.'//TRIM(EXT),0,'',ISS,NCOUNT))THEN
      IOS=-1; EXIT
     ENDIF
     IF(NCOUNT.LE.0.0D0)H=0.0D0
    ENDIF
         
    !## use factor/impulse
    H=H*FCT; H=H+IMP  
    WRITE(JU,'(I14,2F15.7)') ITIME,H,W
   ENDDO
 ...
 !## write *.ocd file(s)
  ...
  DO I=N1,N2
   ...
   ELSEIF(PBMAN%IPESTP.EQ.1)THEN
    IF(I.GT.0)THEN
     IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE
     NAME='GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT'//'\P#'//TRIM(ITOS(I))
    ELSE
     NAME='GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT'//'\L#'//TRIM(ITOS(ABS(I)))
    ENDIF
   ELSEIF(PBMAN%IIES.EQ.1)THEN
    NAME='GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT'//'\R#'//TRIM(ITOS(ABS(I)))
   ENDIF
    ...
    SELECT CASE (K)
     CASE (TCHD,TSHD)
      IF(ASSOCIATED(PBMAN%ISAVE(K)%ILAY).AND..NOT.LSHD)THEN
       LSHD=.TRUE.; WRITE(IU,'(1X,A)') 'HEAD FILEOUT .\'//TRIM(NAME)//'\HEAD\HEAD.HED'
      ENDIF
     CASE DEFAULT
      IF(ASSOCIATED(PBMAN%ISAVE(K)%ILAY).AND..NOT.LBDG)THEN
       LBDG=.TRUE.; WRITE(IU,'(1X,A)') 'BUDGET FILEOUT .\'//TRIM(NAME)//'\BUDGET\BUDGET.CBC'
      ENDIF
    END SELECT
   ENDDO
