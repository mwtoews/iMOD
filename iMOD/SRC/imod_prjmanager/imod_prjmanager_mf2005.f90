...
! CALL IDFCOPY(BND(1,1),ICONNECTION) 
 ...
 ELSE
  LLAT=.TRUE.
 ENDIF
! DO I=1,SIZE(PBMAN%SM(I)%ILAY)
!  DO J=1,SIZE(PBMAN%SM(J)%ILAY) 
!  ENDDO
! ENDDO
...
!     IF(ICONNECTION%X(ICOL,IROW).NE.0.0D0)CYCLE
 ...
 !## write connections
 DO I=1,MNLAY(1)
  ICON1(I)%FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(M1))//'\GWF_'//TRIM(ITOS(M1))//'_L'//TRIM(ITOS(I))//'_GWF_'//TRIM(ITOS(M2))//'.IDF'
  IF(MAXVAL(ICON1(I)%X).EQ.0.0D0)CYCLE
  IF(.NOT.IDFWRITE(ICON1(I),ICON1(I)%FNAME,0))THEN; ENDIF
 ENDDO
 DO I=1,MNLAY(2)
  ICON2(I)%FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(M2))//'\GWF_'//TRIM(ITOS(M2))//'_L'//TRIM(ITOS(I))//'_GWF_'//TRIM(ITOS(M1))//'.IDF'
  IF(MAXVAL(ICON2(I)%X).EQ.0.0D0)CYCLE
  IF(.NOT.IDFWRITE(ICON2(I),ICON2(I)%FNAME,0))THEN; ENDIF
 ENDDO
