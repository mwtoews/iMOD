...
! ENDIF
 ...
 IF(PBMAN%IFORMAT.EQ.3)THEN
  ...
  !## save obs package
  IF(TOPICS(TPST)%IACT)TOPICS(TOBS)%IACT_MODEL=1
...
!     IF(SUM(TLP).GT.0.0D0)TLP=(1.0D0/SUM(TLP))*TLP
 ...
 EXFNAME=TRIM(MAINDIR)//'\GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MODELOUTPUT\OUTPUT_OBS.TXT'
 ...
 DO ISYS=1,NSYS
  ...
  !## pst-package
  ELSEIF(IOPTION.EQ.2)THEN
   SFNAME=PEST%MEASURES(I)%IPFNAME
!         TRIM(ITOS(PEST%MEASURES(I)%IPFTYPE))//','// &
   IXCOL=PEST%MEASURES(I)%IXCOL
   IYCOL=PEST%MEASURES(I)%IYCOL
   ILCOL=PEST%MEASURES(I)%ILCOL
   IMCOL=PEST%MEASURES(I)%IMCOL
   IVCOL=PEST%MEASURES(I)%IVCOL
   FCT=1.0D0
   IMP=0.0D0
  ENDIF
