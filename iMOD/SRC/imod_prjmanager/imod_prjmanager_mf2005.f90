...
! ENDIF
 ...
 !## shd settings
 ITOPIC=TSHD; SCL_D=PBMAN%INTSHD; SCL_U=2; PRJILIST=ITOPIC; IF(PMANAGER_GETFNAMES(1,PRJNLAY,0,1,0).LE.0)RETURN
...
!below. This type of cell is referred to as a “vertical pass through” cell.
 ...
 DO ILAY=1,PRJNLAY
  ...
  IF(ISS.EQ.1)THEN
   ...
   !## sf1
   ITOPIC=TSTO; SCL_D=PBMAN%INTSF1; SCL_U=2; IINV=0
  ...
  !## kdw
  ITOPIC=TKDW; SCL_D=PBMAN%INTKDW; SCL_U=3; IINV=0
  ...
  IF(ILAY.NE.PRJNLAY)THEN
   ...
   !## vcont
   ITOPIC=TVCW; SCL_D=PBMAN%INTVCW; SCL_U=6; IINV=1
 ...
 DO ILAY=1,PRJNLAY
  ...
  !## hkv
  ITOPIC=TKHV; SCL_D=PBMAN%INTKHV; SCL_U=3; IINV=0
  ...
  CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,KHV(ILAY),0,ITOPIC)
   ...
   !## vka
   ITOPIC=TKVA; SCL_D=PBMAN%INTKVA; SCL_U=2; IINV=1
  ...
  IF(ISS.EQ.1)THEN
   ...
   IF(LAYCON(ILAY).NE.1)THEN
    ...
    ITOPIC=TSPY; SCL_D=PBMAN%INTSF2; SCL_U=2; IINV=0
  ...
  IF(LQBD.AND.ILAY.NE.PRJNLAY)THEN
   ...
   !## kvv
   ITOPIC=TKVV; SCL_D=PBMAN%INTKVV; SCL_U=3; IINV=0
