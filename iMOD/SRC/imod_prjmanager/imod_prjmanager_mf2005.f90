...
!  ENDIF
 ...
 IF(PBMAN%DMMFILE.EQ.1.OR.(TOPICS(TOLF)%DEFINED.AND.TOPICS(TDRN)%DEFINED))THEN
...
! WRITE(IU,'(A)') ' [REORDERING_METHOD <reordering_method>]'
 ...
 IF(PBMAN%DMMFILE.EQ.1)THEN
  ...
  FFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\DFM2DWATLEVTOMSW_H.DMM'; IDFM_MSWP=UTL_GETUNIT(); CALL OSD_OPEN(IDFM_MSWP,FILE=FFNAME,STATUS='UNKNOWN',ACTION='WRITE')
  !## OPEN DFM2DTOMSW_WL.DMM
  FFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\MSWPONDINGTODFM2D_DV.DMM'; IMSWP_PDFM=UTL_GETUNIT(); CALL OSD_OPEN(IMSWP_PDFM,FILE=FFNAME,STATUS='UNKNOWN',ACTION='WRITE')
  !## OPEN DFM2DTOMSW_WL.DMM
  FFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\MSWSPRINKTODFM1D_Q.DMM'; IMSWP_SDFM=UTL_GETUNIT(); CALL OSD_OPEN(IMSWP_SDFM,FILE=FFNAME,STATUS='UNKNOWN',ACTION='WRITE')
  !## OPEN DFM2DTOMSW_WL.DMM
  FFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\MSWRUNOFFTODFM1D_Q.DMM'; IMSWP_RDFM=UTL_GETUNIT(); CALL OSD_OPEN(IMSWP_RDFM,FILE=FFNAME,STATUS='UNKNOWN',ACTION='WRITE')
 ENDIF
...
! ENDIF
 ...
 !###====================================================================
 INTEGER FUNCTION PMANAGER_SAVEMF2005_DMM_GETXY(XC,YC,LZONE)
 !###====================================================================
 IMPLICIT NONE
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 INTEGER :: I,J,IPER,IROW,ICOL,ILAY,INEAREST
 ...
 IF(TRIM(PCK(1)).EQ.'ISG')NDFLOWFM=0
...
! IF(MINVAL(IU(1:3)).EQ.0)RETURN
 ...
 DO IPER=1,PRJNPER
  ...
  IF(TRIM(PCK(1)).EQ.'ISG'.AND.PBMAN%DMMFILE.EQ.1)THEN
  ...
  IF(JU(1).GT.0)THEN
   DO I=1,NO(1)
    ...
    IF(IPER.EQ.1.AND.PBMAN%DMMFILE.EQ.1.AND.TRIM(PCK(2)).EQ.'DRN')THEN
     ...
     WRITE(IU(4),'(2F15.3,I15)') DFFM(INEAREST)%X,DFFM(INEAREST)%Y,I
    ENDIF
   ENDDO
   CLOSE(JU(1))
  ENDIF
  ...
  IF(JU(2).GT.0)THEN
   DO I=1,NO(2)
    ...
    IF(IPER.EQ.1.AND.PBMAN%DMMFILE.EQ.1)THEN
     ...
     INEAREST=PMANAGER_SAVEMF2005_DMM_GETXY(XC,YC,.TRUE.)
     !## write nearest coupling location
     WRITE(IU(4),'(2F15.3,I15)') DFFM(INEAREST)%X,DFFM(INEAREST)%Y,NO(1)+I
    ENDIF
   ENDDO
   CLOSE(JU(2))
  ENDIF
 ...
 IF(IU(3).GT.0)THEN
  IF(IU(1).GT.0)CLOSE(IU(1),STATUS='DELETE')
  IF(IU(2).GT.0)CLOSE(IU(2),STATUS='DELETE')
  CLOSE(IU(3))
  !## rename file
