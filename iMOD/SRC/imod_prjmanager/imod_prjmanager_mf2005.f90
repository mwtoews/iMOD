...
!     IF(SUM(TLP).GT.0.0D0)TLP=(1.0D0/SUM(TLP))*TLP
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z1,Z2,FCT,IMP,CNST
 CHARACTER(LEN=256) :: SFNAME,EXFNAME,OBSNAME
 CHARACTER(LEN=5) :: EXT
 CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:) :: STRING
 INTEGER :: IU,KU,ILAY,IROW,ICOL,I,J,NROWIPF,NCOLIPF,IEXT,IOS,N,NP,MP,ICNST,ISYS,NSYS,JLAY
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: TLP,KH,TP,BT
 CHARACTER(LEN=1) :: VTXT
 ...
 IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIRMNAME)//'.'//CPCK//VTXT//'...'
 IU=UTL_GETUNIT()
 CALL OSD_OPEN(IU,FILE=TRIM(DIRMNAME)//'.'//CPCK//VTXT,STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
 IF(IU.EQ.0)RETURN
...
! WRITE(IU,'(/A)') 'BEGIN CONTINUOUS FILEOUT '//TRIM(EXFNAME)
 WRITE(IU,'(/A)') 'BEGIN SINGLE FILEOUT '//TRIM(EXFNAME)
 ...
 DO ISYS=1,NSYS
  ...
  N=MAX(3,IEXT); IF(ILAY.EQ.0)N=MAX(4,IEXT)
  IF(N.GT.NCOLIPF)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD need at least '//TRIM(ITOS(N))//' columns however it reads only '//TRIM(ITOS(NCOLIPF))//' from:'//CHAR(13)// &
      TRIM(SFNAME),'Error'); EXIT
  ENDIF
  ...
  NP=0; DO I=1,NROWIPF
   ...
   JLAY=0; DO ILAY=1,SIZE(PBMAN%ILAY)
    ...
    IF(TLP(ILAY).GT.0.0D0)THEN
     ...
     WRITE(IU,'(A40,A6,A)') TRIM(OBSNAME),',HEAD,',TRIM(ITOS(ILAY))//','//TRIM(ITOS(IROW))//','//TRIM(ITOS(ICOL))
     NP=NP+1
    ENDIF
   ENDDO
  ENDDO
 ...
 WRITE(IU,'(A)') 'END SINGLE' !CONTINUOUS'
