...
!  CALL PMANAGER_SAVEMF6_EXG(MAINDIR,DIRMNAME,3,4)
 ...
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:,:) :: BND,TOP,BOT !,ANA
...
!        DO I=1,MNLAY(IM); IF(.NOT.IDFREAD(ANA(1,I),TRIM(DIR)//'\GWF_'//TRIM(ITOS(M1))//'\MODELINPUT\NPF\ANGLE1_L'//TRIM(ITOS(I))//'.IDF',1))RETURN; ENDDO
!       ENDIF
      ELSE
...
!       IF(TOPICS(TANI)%IACT_MODEL.EQ.1)THEN
!        DO I=1,MNLAY(IM); IF(.NOT.IDFREAD(ANA(2,I),TRIM(DIR)//'\GWF_'//TRIM(ITOS(M2))//'\MODELINPUT\NPF\ANGLE1_L'//TRIM(ITOS(I))//'.IDF',1))RETURN; ENDDO
!       ENDIF
      ENDIF
      EXIT
     ENDIF
  ...
  ENDDO
  IF(II.EQ.1)THEN
   ALLOCATE(BND(2,MAXNLAY),TOP(2,MAXNLAY),BOT(2,MAXNLAY)) !,ANA(2,MAXNLAY))
...
!   DO I=1,SIZE(ANA,1); DO J=1,SIZE(ANA,2); CALL IDFNULLIFY(ANA(I,J)); ENDDO; ENDDO
  ENDIF
 ENDDO
 ...
 !## determine whether submodel is on top or bottom - compute average values for top/bot of submodels
 MT=-10.0D10; MB= 10.0D10; LTOP=.FALSE.
 DO IROW=1,BND(IMDL1,1)%NROW; DO ICOL=1,BND(IMDL1,1)%NCOL
  IF(BND(IMDL1,1)%X(ICOL,IROW).EQ.1)THEN
   T=TOP(IMDL1,1)%X(ICOL,IROW); MT(IMDL1)=MAX(MT(IMDL1),T)
   B=BOT(IMDL1,1)%X(ICOL,IROW); MB(IMDL1)=MIN(MB(IMDL1),B)
   !## get z from other model
   CALL IDFGETLOC(BND(IMDL1,1),IROW,ICOL,XP,YP)
   CALL IDFIROWICOL(BND(IMDL2,1),JROW,JCOL,XP,YP); IF(JROW.LE.0.OR.JCOL.LE.0)CYCLE
!   IF(BND(IMDL2,1)%X(JCOL,JROW).NE.1)CYCLE
   LTOP=.TRUE.
   T=TOP(IMDL2,1)%X(JCOL,JROW); MT(IMDL2)=MAX(MT(IMDL2),T)
   B=BOT(IMDL2,1)%X(JCOL,JROW); MB(IMDL2)=MIN(MB(IMDL2),B)
  ENDIF
 ENDDO; ENDDO
 
 write(*,*) mt(imdl1),mt(imdl2)
 write(*,*) mb(imdl1),mb(imdl2)

 !## something on top/bottom
 IF(LTOP)THEN
  IF(MT(IMDL1).GT.MT(IMDL2).AND. &
     MB(IMDL1).LE.MT(IMDL2))THEN
   TLAYMODEL='T'
  ELSEIF(MT(IMDL1).LT.MT(IMDL2).AND. &
         MB(IMDL2).LE.MT(IMDL1))THEN
   TLAYMODEL='B'
  ENDIF
 ENDIF
 ...
 IF(TLAYMODEL.EQ.'')THEN
  WRITE(*,'(/A)') 'Submodel '//TRIM(ITOS(IMDL(IMDL1)))//' embedded in '//TRIM(ITOS(IMDL(IMDL2)))
 ELSE
  IF(TLAYMODEL.EQ.'T')WRITE(*,'(/A)') 'Submodel '//TRIM(ITOS(IMDL(IMDL1)))//' on top of '//TRIM(ITOS(IMDL(IMDL2)))
  IF(TLAYMODEL.EQ.'B')WRITE(*,'(/A)') 'Submodel '//TRIM(ITOS(IMDL(IMDL1)))//' below '//TRIM(ITOS(IMDL(IMDL2)))
 ENDIF
 ...
 DO I=1,2
    ...
    DO IROW=1,BND(IMDL1,ILAY)%NROW; DO ICOL=1,BND(IMDL1,ILAY)%NCOL
     ...
     IF(LEX)THEN
      IF(PMANAGER_SAVEMF6_EXG_CONNECTIONS('N',IU,ILAY,IROW,ICOL,IMDL1,IMDL2,MNLAY(IMDL1),MNLAY(IMDL2), &
         BND(IMDL1,:),BND(IMDL2,:),TOP(IMDL1,:),BOT(IMDL1,:),TOP(IMDL2,:),BOT(IMDL2,:),I))N=N+1
     ENDIF
    ENDDO; ENDDO
  ...
  IF(TRIM(TLAYMODEL).NE.'')THEN
   IF(TLAYMODEL.EQ.'T'.AND.I.EQ.2)WRITE(IU,'(/A)') '# Top Cell Connections'
   IF(TLAYMODEL.EQ.'B'.AND.I.EQ.2)WRITE(IU,'(/A)') '# Bottom Cell Connections'
   DO IROW=1,BND(IMDL1,1)%NROW; DO ICOL=1,BND(IMDL1,1)%NCOL
...
! DO I=1,SIZE(ANA,1); DO J=1,SIZE(ANA,2); CALL IDFDEALLOCATEX(ANA(I,J)); ENDDO; ENDDO
 DEALLOCATE(BND,TOP,BOT) !,ANA)
