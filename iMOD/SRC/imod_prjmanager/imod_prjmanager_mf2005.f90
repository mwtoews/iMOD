...
! WRITE(IU,'(A)') ' ANGLE3 LAYERED'
 ...
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 INTEGER,INTENT(IN) :: IBATCH
 INTEGER :: IU,ILAY,JLAY,IFBND,IROW,ICOL,NSYSTEM,NOBSSUB
...
! WRITE(IU,'(A)') 'CONSTANT 1.8 SGS'
 ...
 JLAY=0; DO ILAY=1,SIZE(PBMAN%ILAY) 
  ...
  ENDIF
  IF(PBMAN%SCR_ISTPCS.EQ.1.OR.PBMAN%SCR_ISTPCS.EQ.3)THEN
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\SCR1\PC0_L'//TRIM(ITOS(JLAY))//'.ARR', &
      PC0(ILAY),0,IU,ILAY,IFBND))RETURN
  ENDIF
  IF(PBMAN%SCR_ISTPCS.EQ.2.OR.PBMAN%SCR_ISTPCS.EQ.3)THEN
      ...
      IF(TYBE.EQ.1.AND.LYBE.GT.1)THEN
       ...
       IF(IACT.EQ.2)THEN
        ...
        IF(PBMAN%IFORMAT.EQ.3)THEN
         ...
         IF(BND(LYBE)%X(JROW,JCOL).EQ.0)THEN
          !## skip permeability < 0.1
          DO LYBE=1,PRJNLAY
           IF(BND(LYBE)%X(JROW,JCOL).NE.0.0D0.AND.KHV(LYBE)%X(JROW,JCOL).GT.0.1D0)EXIT
          ENDDO
         ENDIF
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 IF(PCK(1).EQ.'ISG')THEN
  ...
  IF(PBMAN%DMMFILE.EQ.1)THEN
   !## read afwatidf 
   IF(.NOT.IDFREADSCALE(PBMAN%AFWATIDF%FNAME,AFWATIDF,7,0,0.0D0,0))THEN
