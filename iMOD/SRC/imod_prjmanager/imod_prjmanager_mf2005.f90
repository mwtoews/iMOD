...
CONTAINS
 ...
 IF(LREUSEDAT)THEN
  IF(IBATCH.EQ.1)THEN
   ...
   IF(IPEST_GLM_READ_ZONES_OPENFILE(DIR,JU))THEN
 ...
 DO I=1,SIZE(PEST%PARAM)
  WRITE(JU,'(/2A10)') 'NODES','PAR.-TYPE'
  WRITE(JU,'(2I10)') PEST%PARAM(I)%NODES,PEST%PARAM(I)%ZTYPE
  IF(PEST%PARAM(I)%ZTYPE.EQ.0)THEN
  ...
  DO
   ...
   DO II=1,SIZE(PEST%PARAM)
    ...
    SELECT CASE (PEST%PARAM(II)%PPARAM)
     ...
     CASE ('VA')
      LINE=UTL_SUBST(LINE,'K33_L'//TRIM(ITOS(ILAY))//'.ARR','K33_L'//TRIM(ITOS(ILAY))//'_'//TRIM(FTYPE)//'.ARR')
    END SELECT
   ENDDO
   WRITE(JU,'(A)') TRIM(LINE)
  ENDDO
