...
! WRITE(IU,'(A)') ' ANGLE3 LAYERED'
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_CON_READ(ISS,IPRT)
 !####====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ISS,IPRT
 INTEGER :: ILAY,SCL_D,SCL_U,IINV,ITOPIC
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_CON_SAVE(DIR,DIRMNAME,IBATCH,ISS)
 !####====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: WETDRYTHRESS=0.1D0 
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 ...
 IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIRMNAME)//'.CON1'//'...')
 IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIRMNAME)//'.CON1'//'...'

 !## construct con1-file
 IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIRMNAME)//'.CON1',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
 WRITE(IU,'(A)') '# CON1 File Generated by '//TRIM(UTL_IMODVERSION())
! !## dry cells negative for restart
! LINE=TRIM(ITOS(IBCFCB))//','//TRIM(RTOS(HNOFLOW,'G',7))//',0,STORAGECOEFFICIENT,THICKSTRT,CONSTANTCV,NOCVCORRECTION'
! IF(PBMAN%MINKD.NE.0.0D0)LINE=TRIM(LINE)//',MINKD '//TRIM(RTOS(PBMAN%MINKD,'G',5))
! IF(PBMAN%MINC .NE.0.0D0)LINE=TRIM(LINE)//',MINC ' //TRIM(RTOS(PBMAN%MINC ,'G',5))
...
! WRITE(IU,'(A)') TRIM(LINE)
 ...
 DO ILAY=1,PRJNLAY
  ...
  !## con
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\CON1\CON_L'//TRIM(ITOS(ILAY))//'.ARR', &
...
!## AUX IC ICHONCHK IC
  ...
  CASE (TRIV)
   LINE='NaN1#,'//TRIM(ITOS(ICB))//' AUX RFCT AUX ISUB RFACT RFCT RSUBSYS ISUB NOPRINT'
   IF(TOPICS(TCON)%IACT_MODEL.EQ.1)LINE=TRIM(LINE)//' RIVDEN'
   IF(PBMAN%IFORMAT.EQ.2)WRITE(IU,'(A)') TRIM(LINE)
!## IFVDL SFT RCNC
  !## evt
...
!## NEVTOP moet twee worden voor optie laag = -1
  ...
  CASE (TGHB)
   ...
   IF(TOPICS(TCON)%IACT_MODEL)LINE=TRIM(LINE)//' GHBDEN'
   IF(PBMAN%IFORMAT.EQ.2)WRITE(IU,'(A)') TRIM(LINE)
  !## rch
...
!## NaN1 moet 3 worden voor optie laag = -1
  ...
  CASE (TCHD)
   ...
   IF(TOPICS(TCON)%IACT_MODEL)LINE=TRIM(LINE)//' CHDDEN'
   IF(PBMAN%IFORMAT.EQ.2)WRITE(IU,'(A)') TRIM(LINE)
  !## fhb package
  CASE(TFHB)
...
!  WRITE(IU,'(A)') '  HEAD PRINT_FORMAT COLUMNS <columns> WIDTH <width> DIGITS <digits> <format>]'
 ...
 DO IPER=1,PRJNPER
  ...
  IF(PBMAN%IFORMAT.EQ.2)THEN
   ...
   !## write output fluxes
   IF(IFLX.GT.0)THEN
    IF(ASSOCIATED(PBMAN%ISAVE(IFLX)%ILAY))THEN
     CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%ISAVE(IFLX)%ILAY,IBCFCB,IU)
    ELSEIF(ASSOCIATED(PBMAN%ISAVE(IFLX)%ILAY))THEN
     CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%ISAVE(IFLX)%ILAY,IBCFCB,IU)
    ENDIF
   ENDIF
