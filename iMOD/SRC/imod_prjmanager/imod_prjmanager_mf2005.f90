...
![MEMORY_PRINT_OPTION <memory_print_option>]
 ...
 IF(PBMAN%IFORMAT.EQ.3)THEN
  ...
  DO I=N1,N2
   ...
   IF(TOPICS(TCAP)%IACT_MODEL.EQ.1)THEN
    WRITE(IU,'(A)') ' WEL6 '//TRIM(CRELDIR)//'GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MSWAPINPUT\MSW.WEL6 WELLS_MSW'
    WRITE(IU,'(A)') ' RCH6 '//TRIM(CRELDIR)//'GWF_'//TRIM(ITOS(PBMAN%ISUBMODEL))//'\MSWAPINPUT\MSW.RCH6 RCH_MSW'
   ENDIF
...
!  IF(PMANAGER_GETFNAMES(1,1,1,0,KPER).LE.0)RETURN
 ...
 WRITE(IU,'(6I10)') ISCRCB,0,PRJNLAY,0,PBMAN%IMETHOD,PBMAN%ISTPCS
 WRITE(IU,'(99I10)') (I,I=1,PRJNLAY)
 ...
 IF(PBMAN%ICTPCS.EQ.0)THEN! PCSOFF PER LAAG
!IF(ICTPCS.EQ.0)THEN
! CALL RF2SEAWATGETKEYVAL('SCR','PCS',WFMTSTR2D(1,1),WIVAL2D(1,1),WRVAL2D(1,1),WCVAL2D(1,1),WLVAL2D(1,1),WTYP2D(1,1),WFACT2D(1,1),WNVAL2D(1),.TRUE.,PRINTDEF,0)
! IF(WRVAL2D(1,1).EQ.RNODATA) CALL RF2SEAWATERROR('MISSING INPUT FOR PCS, WHICH IS MANDATORY IF ICTPCS = 0. CHECK [SCR] INPUT') 
 ELSEIF(PBMAN%ICTPCS.EQ.1)THEN
! CALL RF2SEAWATGETKEYVAL('SCR','PCSOFF',WFMTSTR2D(1,1),WIVAL2D(1,1),WRVAL2D(1,1),WCVAL2D(1,1),WLVAL2D(1,1),WTYP2D(1,1),WFACT2D(1,1),WNVAL2D(1),.TRUE.,PRINTDEF,0)
! IF(WRVAL2D(1,1).EQ.RNODATA) CALL RF2SEAWATERROR('MISSING INPUT FOR PCSOFF, WHICH IS MANDATORY IF ICTPCS = 1. CHECK [SCR] INPUT') 
 ELSEIF(PBMAN%ICTPCS.EQ.2)THEN
! CALL RF2SEAWATGETKEYVAL('SCR','OCR',WFMTSTR2D(1,1),WIVAL2D(1,1),WRVAL2D(1,1),WCVAL2D(1,1),WLVAL2D(1,1),WTYP2D(1,1),WFACT2D(1,1),WNVAL2D(1),.TRUE.,PRINTDEF,0)
! IF(WRVAL2D(1,1).EQ.RNODATA) CALL RF2SEAWATERROR('MISSING INPUT FOR OCR, WHICH IS MANDATORY IF ICTPCS = 2. CHECK [SCR] INPUT') 
 ELSEIF(PBMAN%ICTPCS.EQ.3)THEN
 ENDIF
...
! IF(PMANAGER_GETFNAMES(1,1,1,0,KPER).LE.0)RETURN
 ...
 DO ISYS=1,NSYS
  ...
  !PRJILIST=ITOPIC; 
  IF(PMANAGER_GETFNAMES(ISYS,ISYS,0,1,0).LE.0)RETURN
...
! IF(IU.EQ.0)RETURN
 ...
 DO IPER=1,PRJNPER
  ...
  !## not for sfr
  IF(PBMAN%IFORMAT.EQ.2.AND.ITOPIC.EQ.TISG)CALL IDFWRITEFREE_HEADER(JU,BND(1))
 ...
 IF(PBMAN%IFORMAT.EQ.2)THEN
  ...
  DO IPER=1,PRJNPER
   ...
   IF(TOPICS(TDRN)%DEFINED)THEN
    CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%ISAVE(TDRN)%ILAY,IDRNCB,IU)
   ELSE
    IF(TOPICS(TOLF)%DEFINED)CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%ISAVE(TOLF)%ILAY,IDRNCB,IU)
   ENDIF
   IF(TOPICS(TRIV)%DEFINED)THEN
    CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%ISAVE(TRIV)%ILAY,IRIVCB,IU)
   ELSE
    IF(TOPICS(TISG)%DEFINED)CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%ISAVE(TISG)%ILAY,IRIVCB,IU)
   ENDIF
   ...
   IF(TOPICS(TSCR)%DEFINED)CALL PMANAGER_SAVEMF2005_OCD_ISAVE(PBMAN%ISAVE(TSCR)%ILAY,ISCRCB,IU) 
...
! WRITE(IU,'(A)') ' [REORDERING_METHOD <reordering_method>]'
 ...
 INDSB=0; IAREA=0; ISELSVAT=0; IGWMP=0; IMODSIM=0; ISCAP=0; IINFI=0; IIDF=0; IDFM_MSWP=0; IMSWP_DFM=0; IUNOD=0; 
 WINDEX_MSWP=0; RINDEX_MSWP=0; WMF6_MSWP=0; RMF6_MSWP=0
 ...
 IF(PBMAN%IFORMAT.EQ.3)THEN
  CALL UTL_MF2005_MAXNO(TRIM(DIRMSP)//'\MSW.WEL6_',(/NWEL/))
  CALL UTL_MF2005_MAXNO(TRIM(DIRMSP)//'\MSW.RCH6_',(/NRCH/))
 ENDIF
