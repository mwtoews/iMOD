...
CONTAINS
 ...
 DO I=1,SIZE(IDF)
  ...
  !## find correct shape for current idf
  DO J=1,SIZE(ISORT); IF(ISORT(J).EQ.I)EXIT; ENDDO
  !## use cellsize of previous sized idf
  II=I; IF(I.GT.1)II=I-1
  
  CALL ASC2IDF_INT_NULLIFY(); ALLOCATE(XP(100),YP(100),ZP(100),WP(100),FP(100)); ALLOCATE(IPC(IDF(II)%NCOL,IDF(II)%NROW,2)); IPC=INT(0,1)
  
  !## intersect line and determine ipc()
  CALL ASC2IDF_HFB(IDF(II),IDF(II)%NROW,IDF(II)%NCOL,IPC,(/GENFNAME/),-1,IPOL=J)
  !## write genfiles
  CALL PMANAGER_GENERATEMFNETWORKS_WRITEGEN(IDF(II),IPC,IU,JU,I)
...
!     II=ISORT(JJ)
    ...
    ENDDO
   ELSE
    IDF(I)%X(ICOL,IROW)=IDF(I)%NODATA
   ENDIF
  ENDDO; ENDDO
 ENDDO
 ...
 DO I=1,SHP%NPOL
  ...
  IDF(I)%XMIN=SHP%POL(J)%XMIN; IDF(I)%YMIN=SHP%POL(J)%YMIN
  IDF(I)%XMAX=SHP%POL(J)%XMAX; IDF(I)%YMAX=SHP%POL(J)%YMAX
  ...
  IF(IOS.NE.0)THEN; WRITE(*,'(/A/)') 'Cannot read cellsize for polygon #',J; STOP; ENDIF
  IDF(I)%DY=IDF(I)%DX; CS=IDF(I)%DX; IF(I.GT.1)CS=IDF(ISORT(I-1))%DX
  ...
  IF(.NOT.IDFALLOCATEX(IDF(I)))THEN; WRITE(*,'(/A/)') 'Cannot allocate memory idf%x() #',J;               STOP; ENDIF
  IF(.NOT.IDFFILLSXSY(IDF(I)))THEN;  WRITE(*,'(/A/)') 'Cannot allocate memory for idf%sx()/idf%sy() #',J; STOP; ENDIF
 ENDDO
...
! WRITE(IU,'(A)') '[OBS6 FILEIN <obs6_filename>]' !## observation
 ...
 !     !## skip inactive cells
 !     IF(BND(IMDL2,ILAY)%X(ICOL,IROW).EQ.BND(IMDL2,ILAY)%NODATA)CYCLE
