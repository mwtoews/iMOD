...
USE MOD_UTL, ONLY : UTL_HIDESHOWDIALOG,UTL_CREATEDIR,UTL_MESSAGEHANDLE,UTL_GETUNIT,ITOS,RTOS,JDATETOIDATE,UTL_DEL1TREE, &
                    IDATETOJDATE,UTL_FILLDATESDIALOG,UTL_DIRINFO,UTL_IDFGETDATE
...
CONTAINS
 ...
 CALL UTL_FILLDATESDIALOG(ID_DSCENTOOL_RESULTS,IDF_INTEGER2,IDF_MENU3,IDF_INTEGER3,JDATETOIDATE(MINDATE)) !## mindate
 CALL UTL_FILLDATESDIALOG(ID_DSCENTOOL_RESULTS,IDF_INTEGER4,IDF_MENU4,IDF_INTEGER5,JDATETOIDATE(MAXDATE)) !## maxdate
...
!   IF(.NOT.)EXIT
   ...
   !## rewrite runfile
   IF(.NOT.MODEL1WRITERUNFILE(CONF(I)%RUNF,DIR,1,SIMNPER,ITOS(JDATETOIDATE(SIMJDATE(0)))))EXIT
 ...
 DO ILAY=1,NLAY
  !## open
  IF(.NOT.IDFREAD(DDNIDF(1),TRIM(DIR)//'\head\head_'//TRIM(ITOS(JDATETOIDATE(SIMJDATE(0))))// &
           '_l'//TRIM(ITOS(ILAY))//'.idf',1))RETURN
  DO IPER=1,SIMNPER
   IF(.NOT.IDFREAD(DDNIDF(2),TRIM(DIR)//'\head\head_'//TRIM(ITOS(JDATETOIDATE(SIMJDATE(IPER))))// &
            '_l'//TRIM(ITOS(ILAY))//'.idf',1))RETURN
   DDNIDF(2)%X=DDNIDF(2)%X-DDNIDF(1)%X
   IF(.NOT.IDFWRITE(DDNIDF(2),TRIM(DIR)//'\drawdown\drawdown_'//TRIM(ITOS(JDATETOIDATE(SIMJDATE(IPER))))// &
 ...
 MINDATE=IDATETOJDATE(21000101)
 MAXDATE=IDATETOJDATE(18000101)
 ...
 DO IPER=1,SIMNPER
  ...
  LINE=TRIM(ITOS(IPER+1))//','//TRIM(RTOS(SIMDELT(IPER),'F',1))//','//&
       TRIM(ITOS(JDATETOIDATE(SIMJDATE(IPER))))//',-1'  !## simulation delt
