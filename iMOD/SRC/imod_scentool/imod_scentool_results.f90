...
USE MOD_MODEL, ONLY : MODEL1READRUNFILE,MODEL1WRITERUNFILE,MODEL1DEALLOCATE,MODEL1CHECKRUNFILE  !,MODEL1START
USE MOD_PMANAGER, ONLY : PMANAGERSTART
...
!   IF(.NOT.)EXIT
   ...
   !## start simulation
!   CALL PMANAGERSTART(CONF(I)%RUNF,DIR,-1)
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open/add to '//CHAR(13)// &
    ...
    CASE (15,16,17,18,19,20,21,22,23)
     ...
     IF(IOS.EQ.0)THEN  
      ...
      CALL OSD_OPEN(KU,FILE=TRIM(RUN1)//'\org_'//TRIM(MDLKEYWS(K))//'.RUN',STATUS='OLD', &
         ACTION='READ,DENYWRITE',IOSTAT=IOS)
      IF(IOS.NE.0)THEN
       CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot read '//TRIM(RUN1)//'\org_'// &
