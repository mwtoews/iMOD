...
USE DATEVAR
USE MOD_GRAPH, ONLY : GRAPH,GRAPH_PLOT,GRAPH_DEALLOCATE,GRAPH_ALLOCATE,GRAPHDIM
...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,NG,ILAY,NO,NR,IL,IROW,ICOL,IOBS,NLAY
 ...
 DO NG=1,NOBS
  ...
  GRAPHDIM(NG)%GRAPHNAMES='' !TRIM(ADJUSTL(OBS(NG)%CNAME))
 ...
 GRAPHDIM(1)%IFIXX=0; GRAPHDIM(1)%IFIXY=0; GRAPHDIM(1)%XTITLE='Date'; GRAPHDIM(1)%YTITLE='Heads (m+MSL)'; GRAPHDIM(1)%LDATE=.TRUE.
 !## go to the plot-mode
 IF(NG.GT.NOBS)CALL GRAPH_PLOT()
