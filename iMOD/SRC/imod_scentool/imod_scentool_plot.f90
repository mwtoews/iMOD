  ...
  IF(I.EQ.IOBS)THEN
   ...
   DO IROW=1,NROWL
    ...
    IF(WINFOGRIDCELL(IDF_GRID1,4,IROW,GRIDCELLDEFINED).EQ.1.AND. &
     ...
     CALL UTL_PLOTLABEL(X,Y,CVALUE,IOPTION,5,TWIDTH,THEIGTH,CLABEL,.FALSE.,-1,ALIGNLEFT,CFORMAT='(F10.2)')
    ENDIF
   ENDDO
  ELSE
   DO IROW=1,OBS(I)%NLOC
    IF(IALL.EQ.1.OR.(IALL.EQ.0.AND.JOBS.EQ.I))THEN
     ...
     CALL UTL_PLOTLABEL(OBS(I)%LOC(IROW)%X,OBS(I)%LOC(IROW)%Y,CVALUE,IOPTION,5,TWIDTH,THEIGTH,CLABEL,.FALSE.,-1,ALIGNLEFT,CFORMAT='(F10.2)')
    ENDIF
  ...
  ELSE
   DO IROW=1,WEL(I)%NLOC
    IF(IALL.EQ.1.OR.(IALL.EQ.0.AND.JWEL.EQ.I))THEN
     ...
     CALL UTL_PLOTLABEL(WEL(I)%LOC(IROW)%X,WEL(I)%LOC(IROW)%Y,CVALUE,IOPTION,5,TWIDTH,THEIGTH,CLABEL,.FALSE.,-1,ALIGNLEFT,CFORMAT='(F10.2)')
    ENDIF
