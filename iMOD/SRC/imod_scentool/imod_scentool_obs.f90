...
USE MOD_UTL, ONLY : UTL_HIDESHOWDIALOG,JDATETOGDATE,ITOS,RTOS,GDATETOJDATE,UTL_GETUNIT,JDATETOIDATE, &
                    UTL_CREATEDIR,UTL_WSELECTFILE,IDATETOJDATE,IDATETOJDATE
...
!      CALL STOBS1FIELDS()
 ...
 IF(ICODE.EQ.ID_SAVEAS.OR.ICODE.EQ.ID_SAVE)THEN
  ...
  DO I=1,NOBS
   ...
   DO J=1,OBS(I)%NZ
    LINE=TRIM(ITOS(JDATETOIDATE(OBS(I)%Z(J)%IDATE)))//','//TRIM(RTOS(OBS(I)%Z(J)%MEASURE,'F',2))  !## m3/day -> m3/hr
...
!   OBS(I)%NZ=OBS(I)%NZ+1
   ...
   DO J=1,OBS(I)%NZ
    ...
    OBS(I)%Z(J)%IDATE=IDATETOJDATE(IDATE)
   END DO
