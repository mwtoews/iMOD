...
USE MOD_UTL, ONLY : UTL_HIDESHOWDIALOG,JDATETOGDATE,ITOS,RTOS,GDATETOJDATE,UTL_GETUNIT,UTL_JDATETOIDATE, &
                    UTL_CREATEDIR,UTL_IDATETOJDATE,UTL_GETMED,UTL_PCK_GETTLP,UTL_PCK_READTXT
USE MOD_IDF, ONLY : IDFIROWICOL,IDFGETVAL,IDFREAD,IDFDEALLOCATE
USE MOD_OSD, ONLY : OSD_OPEN
USE MOD_IPF, ONLY : IPFPLOTLABEL,IPFREAD2,IPF,IPFWRITE
USE MOD_PMANAGER, ONLY : UTL_PCK_GETTLP,UTL_PCK_READTXT
 ...
 DO I=1,IPF(1)%NROW
  ...
  IF(FMID.NE.FNODATA)THEN
   IF(IPF(1)%ACOL.GT.0)THEN
    CALL UTL_PCK_GETTLP(STNLAY,TLP,KH,TOP,BOT,IPF(1)%XYZ(3,I),IPF(1)%XYZ(4,I))
   ELSE
    CALL UTL_PCK_GETTLP(STNLAY,TLP,KH,TOP,BOT,IPF(1)%XYZ(3,I),IPF(1)%XYZ(4,I)) 
   ENDIF   
   IF(ISS.GT.0.AND.IPF(1)%ACOL.GT.0)THEN
    ...
    STIME=UTL_JDATETOIDATE(SDATE)*1000000
    ETIME=UTL_JDATETOIDATE(EDATE)*1000000
