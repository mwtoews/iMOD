...
!      CALL ST1_PROPFIELDS()()
 ...
 DO ILAY=1,STNLAY
  ...
  IF(.NOT.IDFREAD(KDIDF(ILAY),KDIDF(ILAY)%FNAME,0))THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot read '//TRIM(KDIDF(ILAY)%FNAME),'Error')
   GOTO 10
  ENDIF
 END DO
...
10 CONTINUE
 ...
 !###======================================================================
 LOGICAL FUNCTION ST1CREATEIPF_STEADY(IKD,ICW,MINKHT,IMIDF,SDATE,EDATE,ISS,ICLAY,HNODATA,FNODATA,MINKD)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IKD,ICW,ISS,IMIDF,ICLAY
 INTEGER,INTENT(IN) :: SDATE,EDATE
 REAL,INTENT(IN) :: MINKHT,HNODATA,FNODATA,MINKD
 ...
 IF(STNLAY.GT.0)THEN
  ...
  DO ILAY=1,STNLAY
   ...
   IF(IKD.EQ.1)THEN
    IF(.NOT.IDFREAD(KDIDF(ILAY),KDIDF(ILAY)%FNAME,0))THEN
     CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot read '//TRIM(KDIDF(ILAY)%FNAME),'Error'); GOTO 10
    ENDIF
   ENDIF
  ...
  IF(STNLAY.GT.0)THEN
   ...
   IF(FMID.NE.FNODATA)THEN
    IF(IPF(1)%ACOL.GT.0)THEN
     CALL UTL_PCK_GETTLP(STNLAY,TLP,KH,TOP,BOT,IPF(1)%XYZ(3,I),IPF(1)%XYZ(4,I),MINKHT,ICLAY)
    ELSE
     CALL UTL_PCK_GETTLP(STNLAY,TLP,KH,TOP,BOT,IPF(1)%XYZ(3,I),IPF(1)%XYZ(4,I),MINKHT,ICLAY) 
    ENDIF
   ELSE
...
!  ENDIF
  ...
  IF(ISS.GT.0.AND.IPF(1)%ACOL.GT.0)THEN
   ...
   IF(.NOT.UTL_PCK_READTXT(2,STIME,ETIME,IPF(1)%XYZ(5,I),FNAME,0,'',2))THEN
...
10 CONTINUE
 ...
 DEALLOCATE(IU,QT); IF(STNLAY.GT.0)DEALLOCATE(TOP,BOT,KD,Q,C,KH)
