...
USE MODPLOT, ONLY : MP
USE MOD_UTL, ONLY : UTL_FILLARRAY,UTL_READARRAY,RTOS
USE MOD_IPF_PAR
...
CONTAINS
 ...
 CALL WDIALOGSPINNERSTEP(IDF_REAL1,0.1,0.5)
 ...
 IF(ALLOCATED(IPFPLOT))THEN  
  ...
  CALL WDIALOGPUTREAL(IDF_REAL2,IPFPLOT(IIPF)%SIMPLIFY,'(F8.2)')
    ...
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDOK)
      ...
      IF(ALLOCATED(IPFPLOT))THEN
       ...
       CALL WDIALOGGETREAL(IDF_REAL2,IPFPLOT(IIPF)%SIMPLIFY)
      ENDIF
 ...
 INTEGER :: I
 REAL :: RM
! REAL(KIND=GLFLOAT),DIMENSION(1) :: PARAMS
...
!  RM=PARAMS(1); CALL WDIALOGRANGEREAL(IDF_REAL1,0.0,RM)
  IF(ALLOCATED(IPFPLOT))IPFPLOT(IIPF)%RADIUS=MAX(IPFPLOT(IIPF)%RADIUS,0.1)
  CALL WDIALOGPUTSTRING(IDF_LABEL3,'Size symbol (>0.1)')
 ELSE
  RM=10.0
  IF(ALLOCATED(IPFPLOT))IPFPLOT(IIPF)%RADIUS=MAX(MIN(IPFPLOT(IIPF)%RADIUS,RM),0.0)
