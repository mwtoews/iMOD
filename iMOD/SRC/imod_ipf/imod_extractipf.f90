...
CONTAINS
  ...
  CASE (ID_DEXTRACT)
   SELECT CASE(ITYPE)
    ...
    CASE(PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (IDHELP)
       CALL IMODGETHELP('4.2.4','MMO.IPO.IPFExt') ! ID 1028
      CASE (IDCANCEL)
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot create '//TRIM(PREFVAL(1))//'\TMP\COPY.BAT','Error')
  RETURN
 ENDIF
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.2)THEN
   ...
   IF(.NOT.IPFREAD(IPLOT,1))THEN
    CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot read '//TRIM(MP(IPLOT)%IDFNAME),'Error')
    RETURN
   ENDIF
   EXIT
  ENDIF
