...
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,UTL_WAITMESSAGE,UTL_CREATEDIR,UTL_WSELECTFILE,DBL_IGRINSIDEPOLYGON,UTL_GETHELP
...
CONTAINS
  ...
  CASE (ID_DEXTRACT)
   SELECT CASE(ITYPE)
    ...
    CASE(PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (IDHELP)
       CALL UTL_GETHELP('4.2.4','MMO.IPO.IPFExt') ! ID 1028
      CASE (IDCANCEL)
 ...
 IF(IOS.EQ.0)THEN
  ...
  IPF(1)%IP=0
  NSEL=0 
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=IPFNAME)
 ...
 NCRD=0
 DO J=1,SHP%NPOL
  IF(SHP%POL(J)%IACT.EQ.1)NCRD=MAX(NCRD,SHP%POL(J)%N)
 ENDDO
 ...
 DO I=1,IPF(1)%NROW
  IF(IPF(1)%IP(I).EQ.INT(1,1))THEN
   ...
   IF(NCRD.GT.0)THEN
    DO J=1,SHP%NPOL
     IF(SHP%POL(J)%N.GT.0)THEN
      !## remove selection outside polygon(s)
      IF(DBL_IGRINSIDESHAPE(X,Y,SHP%POL(J)).EQ.1)THEN
!      IF(DBL_IGRINSIDEPOLYGON(X,Y,SHP%POL(J)%X,SHP%POL(J)%Y,SHP%POL(J)%N).EQ.1)THEN
       IPF(1)%IP(I)=INT(6,1)
      ENDIF
     ENDIF
    ENDDO
 ...
 CALL MAIN_UTL_INACTMODULE(ID_EXTRACTIPF)
