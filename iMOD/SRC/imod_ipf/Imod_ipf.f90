...
10 CONTINUE
 ...
 CHARACTER(LEN=256) :: DIR,FNAME
 CHARACTER(LEN=52) :: CTXT,TXTF,TXT
 INTEGER :: I,J,IU,JU,KU,LU,ILAY,KLAY,IEXT,NCOL,NROW,MROW,MCOL,N,JLAY,FLAG,IX,IY,IM,IL,IH
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: NODATA,DH,ADH
 REAL(KIND=DP_KIND) :: H1,H2,X,Y,MDH
 REAL(KIND=DP_KIND),DIMENSION(NLAY) :: XMED,XAMED
 ...
 READ(IU,*) NROW; READ(IU,*) NCOL
 DO I=1,NCOL; READ(IU,*) CTXT; ENDDO; READ(IU,*) IEXT,TXT
 ...
 WRITE(JU,'(A)') 'DH_OPT'
 WRITE(JU,'(A)') 'DH_DIF'
 DO ILAY=1,NLAY; WRITE(JU,'(A)') 'LAYER_'//TRIM(ITOS(ILAY)); ENDDO
 WRITE(JU,'(A)') TRIM(ITOS(IEXT))//','//TRIM(TXT)
 ...
 DO I=1,NROW
  ...
  XMED=0.0D0; XAMED=0.0D0
  DO ILAY=1,NLAY
   !## read from TXT-files
   IF(IEXT.GT.0.AND.ICOLS(IM).EQ.IEXT)THEN  
    FNAME=TRIM(DIR)//'\'//TRIM(ITOS(ILAY))//'\'//TRIM(TXTF)//'.'//TRIM(TXT)
    ...
    ALLOCATE(DH(MROW),ADH(MROW)); DH=-999.0D0; ADH=-999.0D0
    DO J=1,MROW
     READ(KU,*) CTXT,H1,H2
     IF(H1.NE.NODATA(2).AND.H2.NE.NODATA(3))THEN
      DH(J)=H1-H2; ADH(J)=ABS(DH(J))
     ENDIF
    ENDDO
    CALL UTL_GETMED(ADH,MROW,-999.0D0,(/50.0D0/),1,N,XAMED(ILAY))
    CALL UTL_GETMED(DH ,MROW,-999.0D0,(/50.0D0/),1,N,XMED(ILAY))
    DEALLOCATE(ADH,DH,NODATA); CLOSE(JU)
   ELSE
    READ(STRING(ICOLS(IM)),*) H1
    READ(STRING(ICOLS(IH-1+ILAY)),*) XMED(ILAY)
    XMED(ILAY) =H1-XMED(ILAY)
    XAMED(ILAY)=ABS(XMED(ILAY))
   ENDIF
  ...
  KLAY=0; MDH=10.0D10
  DO ILAY=1,NLAY
   IF(XAMED(ILAY).NE.-999.0.AND.XAMED(ILAY).LE.MDH)THEN
    MDH= XAMED(ILAY)
    KLAY=ILAY
   ENDIF
  ENDDO
  ...
  IF((XAMED(KLAY).LT.1.0D0).AND.(ABS(XAMED(KLAY)-XAMED(JLAY)).GT.1.0D0))THEN
   FLAG=1
  !## probably wrong measurement
  ELSEIF(XAMED(KLAY).GT.5.0D0)THEN
   FLAG=2
  !## leave it as it is
  ELSE
   FLAG=0
  ...
  !## use optimal layer
  SELECT CASE (FLAG)
   CASE (0); WRITE(JU,'(2F10.2,I10,A,F10.2,I10,2F10.2)') X,Y,JLAY,',"'//TRIM(ITOS(JLAY))//'\'//TRIM(TXTF)//'",',1.0,FLAG,XAMED(JLAY),XMED(JLAY)
   CASE (1); WRITE(JU,'(2F10.2,I10,A,F10.2,I10,2F10.2)') X,Y,KLAY,',"'//TRIM(ITOS(KLAY))//'\'//TRIM(TXTF)//'",',1.0,FLAG,XAMED(KLAY),XMED(KLAY)
   CASE (2); WRITE(JU,'(2F10.2,I10,A,F10.2,I10,2F10.2)') X,Y,JLAY,',"'//TRIM(ITOS(JLAY))//'\'//TRIM(TXTF)//'",',0.5,FLAG,XAMED(JLAY),XMED(JLAY)
  END SELECT
    
!  WRITE(KU,'(2F10.2,A,4I5,99F10.3)') X,Y,',"'//TRIM(IMOD_UTL_ITOS(KLAY))//'\'//TRIM(TXTF)//'",',FLAG, &
!     JLAY,KLAY,JLAY-KLAY,XMED(JLAY),XMED(KLAY),XMED(KLAY)-XMED(JLAY),(XMED(ILAY),ILAY=1,NLAY)
  FLUSH(JU)
 ENDDO  
 CLOSE(IU)
