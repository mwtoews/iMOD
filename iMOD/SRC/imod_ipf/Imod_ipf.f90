...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE IPFSAMPLE(IPFNAME1,IPFNAME2,IDFNAME,IXCOL,IYCOL,IACOL,NODATA,NFILES)
 !###======================================================================
 IMPLICIT NONE
...
!    ENDDO
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.2)THEN
   ...
   XYTMP=0.0D0
   CALL IPFPLOT(IIPF,MPW%XMIN,MPW%YMIN,MPW%XMAX,MPW%YMAX,IPLOT,XYTMP,0.0D0,.TRUE.)
 ...
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(:,:) :: XY
 CHARACTER(LEN=256) :: DIR,FNAME
 INTEGER :: I,II,J,JJ,IRAT1,ICLR,PLOTSTYLE,IMARKDATA,NPOS,IPROF,IOS,IERROR,IEDGE,PIATTRIB
...
!  IF(ITYPE.EQ.KEYDOWN.AND.MESSAGE%VALUE1.EQ.KEYESCAPE)EXIT
  ...
  IF(IPF(IIPF)%IPOS(I).EQ.INT(1,1))THEN 
   ...
   IF(MP(IPLOT)%HCOL.GT.0)CALL IPFHIGHLIGHT(IPF(IIPF)%XYPOS(1,I),IPF(IIPF)%XYPOS(2,I),IPF(IIPF)%INFO(MP(IPLOT)%HCOL,I), &
                    MP(IPLOT)%SYMBOL,ICLR,MNHV,MXHV,MP(IPLOT)%HCOL_METHOD,LPROF)
   ...
   ELSE
    IF(MP(IPLOT)%HCOL.EQ.0)THEN
     CALL IGRFILLPATTERN(SOLID); CALL IGRCOLOURN(ICLR)
     CALL UTL_PLOTPOINT(IPF(IIPF)%XYPOS(1,I),IPF(IIPF)%XYPOS(2,I),MP(IPLOT)%SYMBOL,DBLE(MP(IPLOT)%THICKNESS),LPROF)
    ENDIF
   ENDIF
   ...
   IF(ABS(MP(IPLOT)%IEQ).GT.0)CALL UTL_PLOTLABEL(IPF(IIPF)%XYPOS(1,I),IPF(IIPF)%XYPOS(2,I),&
                 ...
                 MP(IPLOT)%IEQ,ALIGNLEFT,CFORMAT=MP(IPLOT)%TFORMAT) !'(F10.2)')
...
!    IF(ASSOCIATED(ASSF(NPOS)%Z))IPF(IIPF)%XYPOS(2,I)=ASSF(NPOS)%Z(1)   
    ...
    ELSEIF(ASSF(NPOS)%ITOPIC.EQ.2)THEN
     ...
     DO JJ=1,ASSF(NPOS)%NRASS
      IF(ASSF(NPOS)%Z(JJ).NE.ASSF(NPOS)%NODATA(1))EXIT
     ENDDO
     IPF(IIPF)%XYPOS(2,I)=ASSF(NPOS)%Z(JJ) !1)
!    IF(ASSOCIATED(ASSF(NPOS)%Z))IPF(IIPF)%XYPOS(2,I)=ASSF(NPOS)%Z(1)   
    !## sonderingen
...
10 CONTINUE
 ...
 IF(PIATTRIB.NE.0)THEN
  ...
  ENDIF
  J=MAX(1,J)
  CALL DBL_WGRTEXTSTRING(X,Y+(1.5D0*DY),TRIM(IPF(IIPF)%INFO(PIATTRIB,I)(J:)),IOFFSET=ABS(IPROF-1))
 ENDIF
 
 END SUBROUTINE IPFPLOTSELECTED

 !###===============================================================================
 SUBROUTINE IPFHIGHLIGHT(X,Y,STRING,SYMBOL,ICLR,MNHV,MXHV,IMETHOD,LPROF)
 !###===============================================================================
 IMPLICIT NONE
 ...
 IF(LZ.NE.0.0D0)THEN
  CALL IGRCOLOURN(WRGB(0,0,0))
  CALL UTL_PLOTPOINT(X,Y,SYMBOL,(LZ+0.5D0),LPROF) 
  CALL IGRCOLOURN(ICLR)
  CALL UTL_PLOTPOINT(X,Y,SYMBOL,LZ,LPROF) 
 ENDIF
 ...
 !###======================================================================
 SUBROUTINE IPFEVALUATE(INIPF,OUTIPF,NLAY,ICOLS) 
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: ND=-999.99D0
 CHARACTER(LEN=*),INTENT(IN) :: INIPF,OUTIPF
 INTEGER,INTENT(IN) :: NLAY
 INTEGER,INTENT(IN),DIMENSION(:) :: ICOLS
 CHARACTER(LEN=256) :: DIR,FNAME
 CHARACTER(LEN=52) :: TXTF,TXT
 CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:) :: CTXT
 INTEGER,ALLOCATABLE,DIMENSION(:) :: LLAY
 INTEGER :: I,J,JJ,K,IU,JU,KU,ILAY,KLAY,IEXT,NCOL,NROW,MROW,MCOL,N,JLAY,FLAG,IX,IY,IM,IL,IH
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: NODATA,OBS
 REAL(KIND=DP_KIND) :: H1,X,Y,MDH,H0
 REAL(KIND=DP_KIND),DIMENSION(NLAY) :: HDIFF
 REAL(KIND=DP_KIND),DIMENSION(1) :: TMP
 CHARACTER(LEN=256),DIMENSION(:),ALLOCATABLE :: STRING
 ...
 IU=UTL_GETUNIT(); OPEN(IU,FILE=INIPF,STATUS='OLD',ACTION='READ')
 DIR=INIPF(:INDEX(INIPF,'\',.TRUE.)-1)
 CALL UTL_CREATEDIR(OUTIPF(:INDEX(OUTIPF,'\',.TRUE.)))
 JU=UTL_GETUNIT(); OPEN(JU,FILE=OUTIPF,STATUS='UNKNOWN',ACTION='WRITE')
 
 READ(IU,*) NROW; READ(IU,*) NCOL; ALLOCATE(CTXT(NCOL),LLAY(NLAY))
 DO I=1,NCOL; READ(IU,*) CTXT(I); CTXT(I)=UTL_CAP(CTXT(I),'U'); ENDDO; READ(IU,*) IEXT,TXT
 !## look for layer numbers
 K=ICOLS(IH)-1; DO I=1,NLAY
  K=K+1; J=INDEX(CTXT(K),'_L'); JJ=INDEX(CTXT(K),'.')  
  IF(J.EQ.0.OR.JJ.EQ.0)THEN; WRITE(*,'(/A/)') 'CANNOT FIND LAYER NUMBER FOR ATTRIBUTE '//TRIM(CTXT(K)); STOP; ENDIF
  J=J+2; READ(CTXT(K)(J:JJ),*) ILAY
  LLAY(I)=ILAY
 ENDDO
 DEALLOCATE(CTXT)
 
 WRITE(JU,'(A)') TRIM(ITOS(NROW))
 WRITE(JU,'(A)') TRIM(ITOS(10+NLAY))
 WRITE(JU,'(A)') 'X'
 WRITE(JU,'(A)') 'Y'
 WRITE(JU,'(A)') '(MEDIAN)OBSERVATION'
 WRITE(JU,'(A)') 'ILAY_ORG'
 WRITE(JU,'(A)') 'ILAY_OPT'
 WRITE(JU,'(A)') 'ILAY_DIF'
 WRITE(JU,'(A)') 'FLAG'
 ...
 WRITE(JU,'(A)') 'DH_OPT'
 WRITE(JU,'(A)') 'DH_DIF'
 DO ILAY=1,NLAY; WRITE(JU,'(A)') 'HEAD_LAYER_'//TRIM(ITOS(ILAY)); ENDDO
 WRITE(JU,'(A)') '0,'//TRIM(TXT)
 
 ALLOCATE(STRING(NCOL)); STRING=''
 DO I=1,NROW

  READ(IU,*) (STRING(J),J=1,NCOL)
  READ(STRING(ICOLS(IX)),*) X
  READ(STRING(ICOLS(IY)),*) Y
  
  !## read from TXT-files
  IF(IEXT.GT.0.AND.ICOLS(IM).EQ.IEXT)THEN  
   READ(STRING(ICOLS(IM)),*) TXTF
   FNAME=TRIM(DIR)//'\'//TRIM(TXTF)//'.'//TRIM(TXT)
   KU=UTL_GETUNIT(); OPEN(KU,FILE=FNAME,STATUS='OLD',ACTION='READ')
   READ(KU,*) MROW; READ(KU,*) MCOL
   ALLOCATE(NODATA(MCOL)); DO J=1,MCOL; READ(KU,*) CTXT,NODATA(J); ENDDO
   ALLOCATE(OBS(MROW)); OBS=ND
   DO J=1,MROW
    READ(KU,*) CTXT,H1
    !## observed head
    IF(H1.NE.NODATA(2))OBS(J)=H1
   ENDDO
   !## get median values for observation and computed values
   CALL UTL_GETMED(OBS,MROW,ND,(/50.0D0/),1,N,TMP); H0=TMP(1)
   DEALLOCATE(OBS,NODATA); CLOSE(KU)
  ELSE
   READ(STRING(ICOLS(IM)),*) H0
  ENDIF

  !## compute differences per layer
  HDIFF=ND; DO JJ=1,NLAY
   READ(STRING(ICOLS(IH)-1+JJ),*) H1
   ILAY=LLAY(JJ); IF(H1.NE.ND)HDIFF(ILAY)=H1-H0
  ENDDO
   
  !## read layer prior layer number
  READ(STRING(ICOLS(IL)),*) JLAY
  
  !## get optimal layer
  KLAY=0; MDH=10.0D10
  DO ILAY=1,NLAY
   IF(HDIFF(ILAY).NE.ND.AND.ABS(HDIFF(ILAY)).LE.MDH)THEN
    MDH=ABS(HDIFF(ILAY)); KLAY=ILAY
   ENDIF
  ENDDO
    
  !## determine flag, 0 is do nothing, 1 is another layer fits better, 3 is very off: double check
  IF((ABS(HDIFF(KLAY)).LT.1.0D0).AND.(ABS(ABS(HDIFF(KLAY))-ABS(HDIFF(JLAY))).GT.1.0D0))THEN
   FLAG=1
  !## probably wrong measurement
  ELSEIF(ABS(HDIFF(KLAY)).GT.5.0D0)THEN
   FLAG=2
  !## leave it as it is
  ELSE
   FLAG=0
  ...
  WRITE(JU,'(3F15.7,4I4,3F15.7,99F10.3)') X,Y,H0,JLAY,KLAY,KLAY-JLAY,FLAG,HDIFF(JLAY),HDIFF(KLAY), &
     ABS(ABS(HDIFF(KLAY))-ABS(HDIFF(JLAY))),(HDIFF(ILAY),ILAY=1,NLAY)
 ENDDO  
 DEALLOCATE(LLAY)
 CLOSE(JU)
 
 END SUBROUTINE IPFEVALUATE
