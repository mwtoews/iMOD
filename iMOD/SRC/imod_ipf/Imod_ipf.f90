...
REAL,PRIVATE,PARAMETER :: XSIZE=100.0     !## size of cross for selected ipf's
...
!  IF(.NOT.IDFREADSCALE(TBIDF(1)%FNAME,TBIDF(1),2,1,0.0,0))RETURN
  IF(.NOT.IDFREADSCALE(TBIDF(1)%FNAME,TBIDF(1),10,0,0.0,0))RETURN
  WRITE(*,'(A)') '[BOT] - '//TRIM(TBIDF(2)%FNAME)
!  IF(.NOT.IDFREADSCALE(TBIDF(2)%FNAME,TBIDF(2),2,1,0.0,0))RETURN
  IF(.NOT.IDFREADSCALE(TBIDF(2)%FNAME,TBIDF(2),10,0,0.0,0))RETURN
  ...
  ELSE
   DO J=1,SIZE(FMDL); FMDL(J)%X=0.0; ENDDO 
...
!    ENDDO
 ...
 INTEGER :: I,II,J,IRAT1,ICLR,PLOTSTYLE,IMARKDATA,NPOS,IPROF,IOS,IERROR,IEDGE,PIATTRIB,ILEG
 ...
 IF(WMENUGETSTATE(ID_PROFILE,2).EQ.1)   CALL IPFGETVALUE_GETCOLOURS(ID_DSERIESPROPTAB6,ILEG)
 IF(WMENUGETSTATE(ID_ANALYSEIPF,2).EQ.1)CALL IPFGETVALUE_GETCOLOURS(ID_DIPFINFO_TAB3,ILEG)
  ...
  IF(IPF(IIPF)%IP(I).EQ.INT(1,1).OR.IPF(IIPF)%IP(I).EQ.INT(2,1).OR. &
   ...
   IF(IPF(IIPF)%ACOL.NE.0)THEN
    ...
    CALL IPFPLOTASSFILE(X,Y,D,NPOS,IP,PLOTSTYLE,PXMIN,PYMIN,PXMAX,PYMAX, &
                       IMARKDATA,LPROF,AXMIN,AXMAX,AYMIN,AYMAX,0,0,IPF(IIPF)%XYPOS(4,I),0,IIPF)
...
10 CONTINUE
 ...
 ELSEIF(IPROF.EQ.1)THEN
  ...
  DX=(GRAPHUNITS(3,1)-GRAPHUNITS(1,1))/(XSIZE/7.5)
  DY=(GRAPHUNITS(4,1)-GRAPHUNITS(2,1))/(XSIZE/7.5)
 ENDIF
 ...
 IF(PIATTRIB.NE.0)THEN
  ...
  CALL WGRTEXTSTRING(X,Y+(1.5*DY),TRIM(IPF(IIPF)%INFO(PIATTRIB,I)))
 ENDIF
 ...
 IF(SUM(IOS).NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can not allocate enough memory to read the IPF','ERROR')
  RETURN
 ENDIF
