   ...
   DO J=1,IPF(1)%NROW
    ...
    IF(IPF(1)%XYZ(5,J).GT.0.0)THEN
     ...
     IF(T-B.GT.0.0)THEN
      ...
      T =MIN(TF,T)
      B =MAX(BF,B)
      IF(T-B.GT.0.0.AND.TF-BF.GT.0.0)THEN
       X =MAX(0.0,(T-B)/(TF-BF))
       IF(X.GT.0.0)THEN
        NTYPE(I)=NTYPE(I)+1
       ENDIF
  ...
  IF(IPF(IIPF)%IPOS(I).EQ.INT(1,1))THEN 
   ...
   ELSE
    IF(MP(IPLOT)%HCOL.EQ.0)THEN
     ...
     CALL IPFPLOTPOINT(IPF(IIPF)%XYPOS(1,I),IPF(IIPF)%XYPOS(2,I),MP(IPLOT)%SYMBOL,1.25*REAL(MP(IPLOT)%THICKNESS))
     CALL IGRCOLOURN(ICLR)
