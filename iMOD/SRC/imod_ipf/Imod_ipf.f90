...
!    ENDDO
 ...
 IF(MP(IPLOT)%HCOL.GT.0)THEN
  ...
  DO I=1,IPF(IIPF)%NROW
   ...
   !## absolute values/direct entries
   IF(MP(IPLOT)%HCOL_METHOD.EQ.2.OR. &
  ...
  IF(MP(IPLOT)%HCOL_METHOD.EQ.3)THEN
...
!  IF(ITYPE.EQ.KEYDOWN.AND.MESSAGE%VALUE1.EQ.KEYESCAPE)EXIT
  ...
  IF(IPF(IIPF)%IPOS(I).EQ.INT(1,1))THEN 
   ...
   IF(MP(IPLOT)%HCOL.GT.0)CALL IPFHIGHLIGHT(IPF(IIPF)%XYPOS(1,I),IPF(IIPF)%XYPOS(2,I),IPF(IIPF)%INFO(MP(IPLOT)%HCOL,I), &
                    MP(IPLOT)%SYMBOL,MP(IPLOT)%THICKNESS,ICLR,MNHV,MXHV,MP(IPLOT)%HCOL_METHOD)
   ...
   ELSE
    IF(MP(IPLOT)%HCOL.EQ.0)THEN
     ...
     CALL IPFPLOTPOINT(IPF(IIPF)%XYPOS(1,I),IPF(IIPF)%XYPOS(2,I),MP(IPLOT)%SYMBOL,0.25*REAL(MP(IPLOT)%THICKNESS))
     CALL IGRCOLOURN(ICLR)
  ...
  IF(IPF(IIPF)%IP(I).EQ.INT(1,1).OR.IPF(IIPF)%IP(I).EQ.INT(2,1).OR. &
   ...
   IF(IPF(IIPF)%ACOL.NE.0)THEN
    ...
    CALL IPFPLOTASSFILE(X,Y,D,NPOS,IP,PLOTSTYLE,PXMIN,PYMIN,PXMAX,PYMAX,IMARKDATA,LPROF, &
            AXMIN,AXMAX,AYMIN,AYMAX,0,0,IPF(IIPF)%XYPOS(4,I),0,XY,YDIS,MP(IPLOT)%FADEOUT)
...
!  CALL WGRTEXTORIENTATION(ALIGNLEFT,ANGLE=90.0)
 ...
 !###===============================================================================
 SUBROUTINE IPFHIGHLIGHT(X,Y,STRING,SYMBOL,THICKNESS,ICLR,MNHV,MXHV,IMETHOD)
 !###===============================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: SYMBOL,THICKNESS,ICLR,IMETHOD
 ...
 ELSE
  ...
  IF(IMETHOD.EQ.2)Z=ABS(Z)
  LZ=4.0*((Z-MNHV)/(MXHV-MNHV))
  LZ=LZ+1.0
 ENDIF
 
 CALL IGRCOLOURN(WRGB(0,0,0))
 CALL IPFPLOTPOINT(X,Y,SYMBOL,(LZ+0.25)*THICKNESS)
 CALL IGRCOLOURN(ICLR)
