...
!  CALL DBL_WGRTEXTBLOCK(XLOC,YLOC,XLOC+DX,YLOC-DY,'Cannot plot associated file!',1.0D0,TBJUSTIFY+TBFONTSIZE)
 ...
 DY =CHH*DBLE(MIN(MAXCOLOUR,ASSF(IASSF)%NCASS-1))
 ...
 DY=DY/DBLE(MIN(MAXCOLOUR,ASSF(IASSF)%NCASS-1))
 Y =YMAX+0.5D0*DY
...
!   IF(IACT.NE.INT(1,1))CALL IPFDRAWLEGEND(IASSF,MINX,MAXY,ISKIP)
   ...
   !## draw boreholes
  CASE (2)   
   MXW=WIDTH*0.5D0; CALL IGRLINEWIDTH(1)
   
   DFRAC=(ABS(D)/YDIS)*DBLE(IFADEOUT)

   J=0; DO I=1,ASSF(IASSF)%NRASS-1 
 
    IF(ASSF(IASSF)%Z(I).EQ.ASSF(IASSF)%NODATA(1))CYCLE 
    IF(J.EQ.0)THEN
     Z1=ASSF(IASSF)%Z(I); J=I !1
    ELSE
     Z2=ASSF(IASSF)%Z(I)
    ...
    IF(Z1.EQ.Z2)CYCLE
     ...
     CALL IPFDRAWITOPIC2_ICLR(J,IASSF,ICLR,IWIDTH); CALL UTL_FADEOUTCOLOUR(ICLR,DFRAC)
     XW=MXW*IWIDTH
  ...
  CASE (4)
   ...
   DO I=1,ASSF(IASSF)%NRASS-1 
    ...
    DFRAC=(ABS(TDIS)/YDIS)*DBLE(IFADEOUT)
 ...
 MXW=0.5D0; CALL IGRLINEWIDTH(1)
 J=0; DO I=1,ASSF(IASSF)%NRASS-1
  IF(ASSF(IASSF)%Z(I).EQ.ASSF(IASSF)%NODATA(1))CYCLE
  IF(J.EQ.0)THEN
   Z1=ASSF(IASSF)%Z(I); J=I
  ELSE
   Z2=ASSF(IASSF)%Z(I)
   CALL IGRFILLPATTERN(SOLID)

   CALL IPFDRAWITOPIC2_ICLR(J,IASSF,ICLR,IWIDTH)
   CALL IGRCOLOURN(ICLR)
   XW=MXW*IWIDTH
