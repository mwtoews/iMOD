...
!  CALL IGRAREA(XP-DX,
  CALL IPFDRAW_DRILLPROF(ILEG,IASSF,XLOC,DX,PLOTSTYLE,DFRAC) !#liduin
  RETURN
 ENDIF
 ...
 IF(ASSF(IASSF)%ITOPIC.EQ.3)CALL IPFDRAWITOPIC3_SCALE(IASSF,MINX,MAXX) !##LIDUIN
...
!   IF(IACT.NE.INT(1,1))CALL IPFDRAWLEGEND(IASSF,MINX,MAXY,ISKIP)
  ...
  CASE (2)
   ...
   DO I=1,ASSF(IASSF)%NRASS-1 
    ...
    CALL IGRFILLPATTERN(HATCHED,Dense3,DiagUp) !#liduin 
    CALL IGRCOLOURN(WRGB(0,0,0))
    CALL IGRRECTANGLE(XLOC-XW,ASSF(IASSF)%Z(I),XLOC+XW,ASSF(IASSF)%Z(I+1))
    ...
    CALL WGRTEXTFONT(FFHelvetica,WIDTH=0.003,HEIGHT=0.01)
    CALL WGRTEXTSTRING(XLOC+(6.0*XW),(ASSF(IASSF)%Z(I)+ASSF(IASSF)%Z(I+1))/2.0,'4%') !#grindpercentage
    CALL WGRTEXTFONT(FFHelvetica,WIDTH=0.003,HEIGHT=0.01)
    CALL WGRTEXTSTRING(XLOC-(6.0*XW),(ASSF(IASSF)%Z(I)+ASSF(IASSF)%Z(I+1))/2.0,'70%') !#slibpercentage
