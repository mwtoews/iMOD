...
!   IF(IACT.NE.INT(1,1))CALL IPFDRAWLEGEND(IASSF,MINX,MAXY,ISKIP)
 ...
 INTEGER :: I,J,ICOLUMN,ILEG

 !## unknown/initial colour ... white/grey
 ICLR  =WRGB(250,250,250)
 IWIDTH=1
 DO J=1,ASSF(IASSF)%NCASS
  IF(UTL_CAP(TRIM(ASSF(IASSF)%ATTRIB(J)),'U').EQ.UTL_CAP(TRIM('[],LITHOLOGY'),'U'))THEN
   ICOLUMN=J-1
   EXIT
  ELSE
 ...
 DO I=1,NLITHO(ILEG)
  IF(TRIM(UTL_CAP(ASSF(IASSF)%L(ICOLUMN,IROW),'U')).EQ.TRIM(UTL_CAP(BH(ILEG,I)%LITHO,'U')))EXIT
 END DO

 IF(INDEX(TRIM(UTL_CAP(ASSF(IASSF)%L(ICOLUMN,IROW),'U')),'Z').GT.0)THEN
  I=1
 ELSEIF(INDEX(TRIM(UTL_CAP(ASSF(IASSF)%L(ICOLUMN,IROW),'U')),'G').GT.0)THEN
  I=3
 ELSEIF(INDEX(TRIM(UTL_CAP(ASSF(IASSF)%L(ICOLUMN,IROW),'U')),'L').GT.0)THEN
  I=6
 ELSEIF(INDEX(TRIM(UTL_CAP(ASSF(IASSF)%L(ICOLUMN,IROW),'U')),'K').GT.0)THEN
  I=4
 ELSEIF(INDEX(TRIM(UTL_CAP(ASSF(IASSF)%L(ICOLUMN,IROW),'U')),'V').GT.0)THEN
  I=5
 ENDIF
