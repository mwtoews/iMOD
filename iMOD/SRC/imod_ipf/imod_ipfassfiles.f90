...
USE MOD_IPF_PAR
USE MOD_IPFANALYSE, ONLY : IPFANALYSE_FILLGRID_UPDATE
...
CONTAINS
 ...
 IF(IPFOPENASSFILE(IU,IASSF,FNAME).AND. &
    ...
    DO I=2,ASSF(IASSF)%NCASS
     ...
     DO J=1,ASSF(IASSF)%NRASS
      IF(ASSF(IASSF)%MEASURE(I,J).NE.ASSF(IASSF)%NODATA(I))THEN
       IF(GRAPHLINESNEGATIVE(I).EQ.1)THEN
        ASSF(IASSF)%MEASURE(I,J)=-1.0*ASSF(IASSF)%MEASURE(I,J) !##liduin
       ENDIF
  ...
  !## update grid
  CALL IPFANALYSE_FILLGRID_UPDATE(IASSF)
...
!   IF(IACT.NE.INT(1,1))CALL IPFDRAWLEGEND(IASSF,MINX,MAXY,ISKIP)
 ...
 INTEGER :: I,J
 REAL :: X1,Y1,X2,Y2,MINX,MAXX,MINY,MAXY,XS,XSC,DXOFF
 ...
 IF(.NOT.ALLOCATED(GRAPHLINESNEGATIVE))ALLOCATE(GRAPHLINESNEGATIVE(ASSF(IASSF)%NCASS))
  ...
  DO J=2,MIN(MAXCOLOUR,ASSF(IASSF)%NCASS)
   DO I=1,ASSF(IASSF)%NRASS
    IF(ASSF(IASSF)%MEASURE(J,I).NE.ASSF(IASSF)%NODATA(J))THEN
     !## when option "plot negative x-value" is checked
     IF(GRAPHLINESNEGATIVE(J).EQ.1)THEN
   ...
   IF(GRAPHLINESSCALE.EQ.1)THEN
    !## save max and min value per column (1)
    !## calculate column width per data column (2)
    !## calculate scaling factor=> dx/column_width (3)
    !## reset initial min and max value for next column (4)
    CMAXX(J)=MAXX; CMINX(J)=MINX !##1
    XSC=XSCALE/(ASSF(IASSF)%NCASS-1) !##2
    XSCF(J)=0.5*XSC/(CMAXX(J)-CMINX(J)) !##3
    MAXX=-10.0E10; MINX=10.0E10 !##4
   ENDIF
  ENDDO
  IF(GRAPHLINESXAXES.EQ.1)THEN
   MINX=GRAPHLINESXMIN; MAXX=GRAPHLINESXMAX
  ...
  IF(PLOTSTYLE.EQ.1)THEN
   DO I=2,ASSF(IASSF)%NRASS
    IF(ASSF(IASSF)%MEASURE(J,I-1).NE.ASSF(IASSF)%NODATA(J).AND.ASSF(IASSF)%MEASURE(J,I).NE.ASSF(IASSF)%NODATA(J))THEN
     ...
     !## in case "Scale each data column seperately on plot area" is checked
     IF(GRAPHLINESSCALE.EQ.1)THEN
      !# adds column scaling width to the general xoffset to fit every line in a seperate plot column
      XSC=XSCALE/(ASSF(IASSF)%NCASS-1)
      DXOFF=(XSC/2)+((J-2)*XSC) !## offset relative to 0.0 scaling
      X1=X1*XSCF(J)*SCALED(J); X2=X2*XSCF(J)*SCALED(J) !##extra scalingsfactor, default=1.0
      X1=X1+DXOFF; X2=X2+DXOFF
      X1=X1+XOFFSET+(XSC*(J-1)); X2=X2+XOFFSET+(XSC*(J-1))
     ELSE
