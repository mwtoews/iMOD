...
!   IF(IACT.NE.INT(1,1))CALL IPFDRAWLEGEND(IASSF,MINX,MAXY,ISKIP)
  ...
  CASE (2)
   ...
   DO I=1,ASSF(IASSF)%NRASS-1
    CALL IPFDRAWITOPIC2_ICLR(ILEG,I,IASSF,ICLR,IWIDTH); CALL IFFFADEOUT(ICLR,DFRAC)
...
!  XW=0.5*(IWIDTH/MXW) 
 ...
 !###===============================================================================
 SUBROUTINE IPFDRAWITOPIC2_ICLR(ILEG,IROW,IASSF,ICLR,IWIDTH)
 !###===============================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IROW,IASSF,ILEG
...
!  RETURN
 ...
 DO I=1,SIZE(BH,1)
  ...
  IF(NLITHO(I).GT.0)CYCLE

  IF(I.EQ.1)THEN
   !## try default legend ... prefval(1)\legend\drill.dlf
   CALL IPFGETVALUE_OPENSAVECOLOURS(TRIM(PREFVAL(1))//'\SETTINGS\DRILL.DLF',ID_OPEN,0,I) !## no dialog necessary for opening
   IF(NLITHO(I).GT.0)CYCLE
   !## try default legend ... prefval(1)\legend\drill1.dlf
   CALL IPFGETVALUE_OPENSAVECOLOURS(TRIM(PREFVAL(1))//'\SETTINGS\DRILL'//TRIM(ITOS(I))//'.DLF',ID_OPEN,0,I) !## no dialog necessary for opening
   IF(NLITHO(I).GT.0)CYCLE
  ELSE
  ...
  DO J=1,SIZE(BH,2)
   BH(I,J)%LITHO=''
   BH(I,J)%LITHOCLR =WRGB(255,255,255)
   BH(I,J)%LITHOWIDTH =1.0 
  ENDDO
  ...
  BH(I,1)%LITHOCLR =WRGB(255,255,0)
  BH(I,2)%LITHOCLR =WRGB(255,255,128)
  BH(I,3)%LITHOCLR =WRGB(218,165,32)
  BH(I,4)%LITHOCLR =WRGB(0,128,0)
  BH(I,5)%LITHOCLR =WRGB(255,0,255)
  BH(I,6)%LITHOCLR =WRGB(238,130,238)
  BH(I,7)%LITHOCLR =WRGB(0,255,255)
  BH(I,8)%LITHOCLR =WRGB(192,192,192)
 
  BH(I,1)%LITHOTXT ='Sand'
  BH(I,2)%LITHOTXT ='Silty Sand'
  BH(I,3)%LITHOTXT ='Gravel'
  BH(I,4)%LITHOTXT ='Clay'
  BH(I,5)%LITHOTXT ='Peat'
  BH(I,6)%LITHOTXT ='Loam'
  BH(I,7)%LITHOTXT='Sandstone'
  BH(I,8)%LITHOTXT='Limestone'

  BH(I,1)%LITHOWIDTH =0.75 
  BH(I,2)%LITHOWIDTH =0.6
  BH(I,3)%LITHOWIDTH =1.0 
  BH(I,4)%LITHOWIDTH =0.3
  BH(I,5)%LITHOWIDTH =0.5
  BH(I,6)%LITHOWIDTH =0.4
  BH(I,7)%LITHOWIDTH =0.2
  BH(I,8)%LITHOWIDTH =0.1
