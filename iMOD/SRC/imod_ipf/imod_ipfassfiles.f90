...
!   IF(IACT.NE.INT(1,1))CALL IPFDRAWLEGEND(IASSF,MINX,MAXY,ISKIP)
  ...
  CASE (2)   
   ...
   DO I=1,ASSF(IASSF)%NRASS-1 
    ...
    IF(GRAPHPERCENTAGES.EQ.1)THEN !##if checkbox is checked
     CALL IPFDRAWPERCENTAGES(IASSF,MXW,XLOC,I)
    ENDIF
 ...
 !###===============================================================================
 SUBROUTINE IPFDRAWPERCENTAGES(IASSF,MXW,XLOC,I) 
 !###===============================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IASSF,I
 REAL,INTENT(IN) :: XLOC,MXW
 INTEGER :: IPLOT
 CHARACTER(LEN=4) :: GRAPER,MUDPER 
 
 DO IPLOT=1,MXMPLOT
  
  IF(ICGRA.GT.0.AND.ICGRA.LT.ASSF(IASSF)%NCASS)THEN
   IF(ICMUD.GT.0.AND.ICMUD.LT.ASSF(IASSF)%NCASS)THEN
    GRAPER=TRIM(ASSF(IASSF)%L(ICGRA,I))
    MUDPER=TRIM(ASSF(IASSF)%L(ICMUD,I))
    IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.2)THEN
     MP(IPLOT)%GPERC = GRAPHPERCENTAGES
     MP(IPLOT)%ICPERC(1) = ICGRA
     MP(IPLOT)%ICPERC(2) = ICMUD
    ENDIF
   ENDIF
  ENDIF
 ...
 CALL WGRTEXTFONT(FFHelvetica,WIDTH=0.003,HEIGHT=0.02)
 CALL WGRTEXTSTRING(XLOC+(6.0*MXW),(ASSF(IASSF)%Z(I)+ASSF(IASSF)%Z(I+1))/2.0,TRIM(GRAPER)//'%') !#grindpercentage (=right from borecolumn)
 CALL WGRTEXTFONT(FFHelvetica,WIDTH=0.003,HEIGHT=0.02)
 CALL WGRTEXTSTRING(XLOC-(6.0*MXW),(ASSF(IASSF)%Z(I)+ASSF(IASSF)%Z(I+1))/2.0,TRIM(MUDPER)//'%') !#slibpercentage (=left from borecolumn)
