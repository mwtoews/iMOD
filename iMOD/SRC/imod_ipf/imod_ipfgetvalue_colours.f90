...
CONTAINS
 ...
 DO I=1,MIN(MAXNROW,NLITHO(ILEG))
  ...
  CALL WGRIDPUTCELLDOUBLE(   IDF_GRID1,4,I,BH(ILEG,I)%LITHOWIDTH,'(F7.2)')
 ENDDO
 ...
 DO I=1,MAXNROW 
  ...
  CALL WGRIDGETCELLDOUBLE(IDF_GRID1,4,I,BH(ILEG,I)%LITHOWIDTH)
  IF(BH(ILEG,I)%LITHO.NE.''.AND.BH(ILEG,I)%LITHOCLR.GE.0)THEN
   ...
   BH(ILEG,J)%LITHOWIDTH=MAX(0.0D0,BH(ILEG,I)%LITHOWIDTH)
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open iMOD Borehole Legend file, no permission!','ERROR')
  RETURN
 ENDIF
  ...
  IF(NLITHO(ILEG).GT.0)THEN
   ...
   DO
    ...
    IF(IOS.NE.0)THEN
     ...
     ENDIF
    ENDIF
    BH(ILEG,I)%LITHOWIDTH=MAX(0.0D0,BH(ILEG,I)%LITHOWIDTH)
