 ...
 SELECT CASE(ITYPE)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
     CASE (IDCANCEL)
      ICLOSE=1
      CALL WDIALOGSELECT(ID_DIPFINFOSERIEGRID) !#LIDUIN
      CALL WDIALOGGETMENU(IDF_MENU1,I)
      CALL WDIALOGGETREAL(IDF_REAL1,SCALED(I))
      IF(SCALED(I).GT.99.9)THEN
       CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,"Multiplying by a factor > 99.9 is a little too much don't you think?",'Error')
       ICLOSE=0
      ENDIF
     CASE (ID_REDRAW)
