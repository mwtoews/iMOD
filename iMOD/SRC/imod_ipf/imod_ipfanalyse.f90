    ...
    IF(NPOS.GT.0)THEN
     ...
     IF(IASSF.NE.0)THEN
      IF(ASSF(IASSF)%ITOPIC.EQ.1)THEN
       CALL UTL_GDATE(INT(MESSAGE%GX),IY,IM,ID)
...
! ENDIF
 ...
 ENDIF
 IF(.NOT.ALLOCATED(GRAPHLINESNEGATIVE))THEN
  ALLOCATE(GRAPHLINESNEGATIVE(GRAPHMAXLINES)); GRAPHLINESNEGATIVE=0
 ENDIF
 ...
 IF(.NOT.ALLOCATED(SCALED))THEN
  ALLOCATE(SCALED(GRAPHMAXLINES)); SCALED=1.0
 ENDIF
...
! IF(ALLOCATED(GRAPHLINESNEGATIVE))DEALLOCATE(GRAPHLINESNEGATIVE)
 ...
 INTEGER :: I,J,IIPF,IX,IY,K,IFRAME,INEG

 CALL WDIALOGSELECT(ID_DIPFINFOSERIE)
 CALL WDIALOGGETMENU(IDF_MENU1,ILIST)
 CALL WDIALOGGETCHECKBOX(IDF_CHECK1,IFRAME)
 !CALL WDIALOGGETCHECKBOX(IDF_CHECK2,INEG) !##LIDUIN
 ...
 IF(NPOS.GT.0)THEN
  ...
  DO IPOS=1,NPOS
   ...
   CALL IPFPLOTASSFILE(0.0,0.0,1.0,IPOS,ABS(INT(3,1)),PLOTSTYLE,0.0,1.0,0.0,1.0, &
                       IMARKDATA,LPROF,0.0,1.0,0.0,1.0,IZOOM,(IPOS-1)*IFRAME,0.0,1,0)
 ...
 DO I=2,ASSF(IASSF)%NCASS
  DO J=1,ASSF(IASSF)%NRASS
   ...
   WRITE(TXT,'(F10.3)') ASSF(IASSF)%MEASURE(I,J)
   TXT=ADJUSTL(TXT)
 ...
 CALL WDIALOGGETCHECKBOX(IDF_CHECK2,GRAPHLINESNEGATIVE(I))
 ...
 CALL WDIALOGSELECT(ID_DIPFINFOSERIEGRID)
 CALL WDIALOGGETMENU(IDF_MENU1,I)
 CALL WDIALOGGETREAL(IDF_REAL1,SCALED(I))
