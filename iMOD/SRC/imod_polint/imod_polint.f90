...
CONTAINS
 ...
 REAL(KIND=DP_KIND),DIMENSION(NPC),INTENT(IN) :: XCRD       !fixed point x-coordinates
 REAL(KIND=DP_KIND),DIMENSION(NPR),INTENT(IN) :: YCRD       !fixed point y-coordinates
 REAL(KIND=DP_KIND),DIMENSION(NPC,NPR),INTENT(IN) :: ZCRD   !fixed point values
 REAL(KIND=DP_KIND),DIMENSION(0:NCOL),INTENT(IN) :: DELR
 REAL(KIND=DP_KIND),DIMENSION(0:NROW),INTENT(IN) :: DELC
 REAL(KIND=DP_KIND),DIMENSION(NCOL,NROW),INTENT(INOUT) :: X
 REAL(KIND=DP_KIND) :: Y,DY,X1,X2
 INTEGER :: NMAX,IROW,ICOL 
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: C,D,YMTMP,YNTMP
 ...
 DO IROW=1,NROW
  DO ICOL=1,NCOL
   X1=(DELR(ICOL-1)+DELR(ICOL))/2.0D0
   X2=(DELC(IROW-1)+DELC(IROW))/2.0D0
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: NODATA
 REAL(KIND=DP_KIND),DIMENSION(NPC),INTENT(IN) :: XCRD
 REAL(KIND=DP_KIND),DIMENSION(NPR),INTENT(IN) :: YCRD
 REAL(KIND=DP_KIND),DIMENSION(NPC,NPR) :: ZCRD
 REAL(KIND=DP_KIND),INTENT(IN) :: XINT,YINT
 REAL(KIND=DP_KIND),INTENT(OUT) :: DY,Y
 REAL(KIND=DP_KIND),DIMENSION(NPR),INTENT(INOUT) :: YNTMP
 REAL(KIND=DP_KIND),DIMENSION(NPC),INTENT(INOUT) :: YMTMP
 REAL(KIND=DP_KIND),DIMENSION(NMAX),INTENT(INOUT) :: C,D
 ...
 REAL(KIND=DP_KIND),DIMENSION(NPR),INTENT(IN) :: XA,YA
 REAL(KIND=DP_KIND),DIMENSION(NMAX),INTENT(INOUT) :: C,D
 REAL(KIND=DP_KIND),INTENT(IN) :: X
 REAL(KIND=DP_KIND),INTENT(OUT) :: DY,Y
 INTEGER :: I,M,NS
 REAL(KIND=DP_KIND) :: DEN,DIF,DIFT,HO,HP,W
 ...
 DO M=1,NPR-1
  DO I=1,NPR-M
   ...
   W  =C(I+1)-D(I)
   DEN=HO-HP
   IF(DEN.EQ.0.0D0)PAUSE 'FAILURE IN POLINT' !## occurs whenever two xa(i) are almost the same
   DEN=W/DEN
   D(I)=HP*DEN
   C(I)=HO*DEN
  END DO
 ...
 INTEGER,INTENT(IN) :: N
 REAL(KIND=DP_KIND),INTENT(IN) :: X
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(N) :: XX
 INTEGER :: JL,JM,JU
 ...
 REAL(KIND=DP_KIND),DIMENSION(N),INTENT(IN) :: XB,YB
 REAL(KIND=DP_KIND),DIMENSION(:),POINTER,INTENT(OUT) :: XI,ZI
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: B,C,D
 REAL(KIND=DP_KIND) :: X
 INTEGER :: I
 ...
 REAL(KIND=DP_KIND),INTENT(OUT),DIMENSION(N) :: B,C,D
 INTEGER :: I,J,GAP
 REAL(KIND=DP_KIND) :: H
...
!
 ...
 !###==================================================================
 REAL(KIND=DP_KIND) FUNCTION ISPLINE(U,X,Y,B,C,D,N)
 !###==================================================================
 ...
 !###=========================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: U
 INTEGER,INTENT(IN) :: N
 REAL(KIND=DP_KIND),DIMENSION(N),INTENT(IN) :: X(N),Y(N),B(N),C(N),D(N)
 INTEGER :: I,J,K
 ...
 REAL(KIND=DP_KIND),DIMENSION(N),INTENT(IN) :: XB,YB
 REAL(KIND=DP_KIND),DIMENSION(:),POINTER,INTENT(OUT) :: XI,ZI
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: XM,Z,X,Y
 INTEGER :: NN,I,IERROR
 REAL(KIND=DP_KIND) :: DX,XX,SX1,SX2
 ...
 ALLOCATE(XM(0:NN+3),Z(0:NN),X(0:NN),Y(0:NN)); X=0.0D0; Y=0.0D0
 ...
 DO I=1,N+4
  IF(X(I).LE.X(I-1))THEN
   SX1=0.0D0; IF(I.GT.1)SX1=MIN(DX/2.0,X(I-1)-X(I-2))
   SX2=0.0D0; IF(I.LT.N+4)SX2=MIN(DX/2.0,X(I+1)-X(I))
   X(I-1)=X(I-1)-SX1
   X(I)  =X(I)  +SX2
  ENDIF
 ENDDO
...
write(*,*)
 ...
 REAL(KIND=DP_KIND),INTENT(OUT) :: YY
 REAL(KIND=DP_KIND),DIMENSION(0:IV),INTENT(INOUT) :: X,Y 
 REAL(KIND=DP_KIND),DIMENSION(0:IV+3),INTENT(OUT) :: XM ! 
 REAL(KIND=DP_KIND),DIMENSION(0:IV),INTENT(OUT) :: Z 
