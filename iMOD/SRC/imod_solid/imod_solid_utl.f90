...
CONTAINS
 ...
 IF(RW.EQ.'R'.OR.RW.EQ.'r')THEN
  ...
  DO I=1,SIZE(KSETTINGS)
   ...
   IF(KSETTINGS(I)%NUGGET.LE.0.0)THEN
    CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'It is not allowed to have a NUGGET of less than zero.'//CHAR(13)// &
     'Your value is '//TRIM(RTOS(KSETTINGS(I)%NUGGET,'F',2))//CHAR(13)//'iMOD changed the value into 0.1','Information')
    KSETTINGS(I)%NUGGET=0.1
   ENDIF
