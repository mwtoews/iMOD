...
CONTAINS
 ...
 !###====================================================================
 LOGICAL FUNCTION SOLIDOPENSPF(ISPF,RW,DIR,ICOMBINE)
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: RW,DIR
 INTEGER,INTENT(IN) :: ISPF
 INTEGER,INTENT(INOUT),DIMENSION(:,:),POINTER,OPTIONAL :: ICOMBINE
 INTEGER :: IU,IOS,I,IL,ICOMB
 CHARACTER(LEN=256) :: LINE
 ...
 LINE=UTL_CAP(SPF(ISPF)%FNAME,'U'); I=INDEX(LINE,'.SPF',.TRUE.)
 IF(I.EQ.0)LINE=TRIM(LINE)//'.SPF'
 ...
 IF(RW.EQ.'R'.OR.RW.EQ.'r')THEN
  ...
  READ(IU,'(A256)') LINE
  READ(LINE,*,IOSTAT=IOS) SPF(ISPF)%NXY,NTBSOL,ICOMB
  IF(IOS.NE.0)THEN
   ICOMB=0; READ(LINE,*,IOSTAT=IOS) SPF(ISPF)%NXY,NTBSOL
  ENDIF
  IF(ICOMB.EQ.1)THEN
   IF(PRESENT(ICOMBINE))THEN
    IF(ASSOCIATED(ICOMBINE))DEALLOCATE(ICOMBINE)
    ALLOCATE(ICOMBINE(NTBSOL,3))
    DO I=1,NTBSOL; READ(IU,*) ICOMBINE(I,1),ICOMBINE(I,2),ICOMBINE(I,3); ENDDO
   ELSE
    DO I=1,NTBSOL; READ(IU,*); ENDDO
   ENDIF
  !## if not available make up our own
  ELSE
   IF(PRESENT(ICOMBINE))THEN
    IF(ASSOCIATED(ICOMBINE))DEALLOCATE(ICOMBINE)
    ALLOCATE(ICOMBINE(NTBSOL,3))
    DO I=1,NTBSOL; ICOMBINE(I,1)=I; ICOMBINE(I,2)=I; ICOMBINE(I,3)=0; ENDDO
   ENDIF
  ENDIF
  IF(IOS.EQ.0)THEN
  ...
  DO IL=1,NTBSOL
   ...
   IF(ALLOCATED(SLD))THEN
    IF(SIZE(SLD(1)%INTNAME).GE.IL)THEN
   ...
   ENDIF
   IF(IOS.NE.0)RETURN
   NULLIFY(SPF(ISPF)%PROF(IL)%PX,SPF(ISPF)%PROF(IL)%PZ,SPF(ISPF)%PROF(IL)%IT)
   ALLOCATE(SPF(ISPF)%PROF(IL)%PX(MXPX),SPF(ISPF)%PROF(IL)%PZ(MXPX),SPF(ISPF)%PROF(IL)%IT(MXPX))
   !## read all dist/zvalue on the segments
   DO I=1,SPF(ISPF)%PROF(IL)%NPOS
    READ(IU,'(A256)',IOSTAT=IOS) LINE; IF(IOS.NE.0)RETURN
    READ(LINE,*,IOSTAT=IOS) SPF(ISPF)%PROF(IL)%PX(I),SPF(ISPF)%PROF(IL)%PZ(I),SPF(ISPF)%PROF(IL)%IT(I)
    IF(IOS.NE.0)THEN
     IF(I.EQ.1)THEN
      SPF(ISPF)%PROF(IL)%IT(I)=-1
     ELSEIF(I.EQ.SPF(ISPF)%PROF(IL)%NPOS)THEN
      SPF(ISPF)%PROF(IL)%IT(I)= 1
     ELSE
      SPF(ISPF)%PROF(IL)%IT(I)=0
     ENDIF
     READ(LINE,*,IOSTAT=IOS) SPF(ISPF)%PROF(IL)%PX(I),SPF(ISPF)%PROF(IL)%PZ(I); IF(IOS.NE.0)RETURN
    ENDIF
   ENDDO
  ENDDO
  ...
  SPF(ISPF)%PBITMAP%FNAME=UTL_SUBST(SPF(ISPF)%PBITMAP%FNAME,TRIM(REPLACESTRING),PREFVAL(5))
  IF(IOS.EQ.0)THEN
 ...
 ELSEIF(RW.EQ.'W'.OR.RW.EQ.'w')THEN
  ...
  WRITE(IU,'(A)',IOSTAT=IOS) TRIM(LINE)
  IF(PRESENT(ICOMBINE))THEN
   DO I=1,SIZE(SPF(ISPF)%PROF) !NTBSOL
    LINE=TRIM(ITOS(ICOMBINE(I,1)))//','//TRIM(ITOS(ICOMBINE(I,2)))//','//TRIM(ITOS(ICOMBINE(I,3)))
    WRITE(IU,'(A)',IOSTAT=IOS) TRIM(LINE)
   ENDDO
  ENDIF
  !## write coordinates
  ...
  DO IL=1,SIZE(SPF(ISPF)%PROF) !NTBSOL
   ...
   !## read all dist/zvalue on the segments
   DO I=1,SPF(ISPF)%PROF(IL)%NPOS
    WRITE(IU,'(F15.3,A1,F15.3,A1,I10)',IOSTAT=IOS) SPF(ISPF)%PROF(IL)%PX(I),',', &
                                                   SPF(ISPF)%PROF(IL)%PZ(I),',', &
                                                   SPF(ISPF)%PROF(IL)%IT(I)
   ENDDO
  ENDDO
