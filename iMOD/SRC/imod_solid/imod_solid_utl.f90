...
USE RESOURCE
USE MOD_COLOURS
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_READINITFILE,UTL_CAP,UTL_CLOSEUNITS,UTL_MESSAGEHANDLE, &
 UTL_IMODFILLMENU,UTL_DEBUGLEVEL
...
USE MOD_POLYGON_PAR
USE MOD_PROFILE_PAR, ONLY : PBITMAP
USE MOD_SOLID_PAR
...
CONTAINS
 ...
 IF(RW.EQ.'R'.OR.RW.EQ.'r')THEN
  ...
  IF(UTL_READINITFILE('GENFILE',LINE,IU,1))THEN
   ...
   CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,0,TRIM(DIR)//'\'//TRIM(FNAME),VAR)
  ENDIF
  ...
  DO I=1,SIZE(KSETTINGS)
   ...
   IF(KSETTINGS(I)%NUGGET.LT.0.0)THEN
    CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'It is not allowed to have a NUGGET of less than zero.'//CHAR(13)// &
     'Your value is '//TRIM(RTOS(KSETTINGS(I)%NUGGET,'F',2))//CHAR(13)//'iMOD changed the value into 0.0','Information')
    KSETTINGS(I)%NUGGET=0.0
   ENDIF
 ...
 ELSEIF(RW.EQ.'W'.OR.RW.EQ.'w')THEN
  ...
  IF(SHPNO.GT.0)THEN
   ...
   CALL POLYGON1SAVELOADSHAPE(ID_SAVESHAPE,0,TRIM(DIR)//'\SHAPES\SHAPE.GEN',VAR)
  ENDIF
 ...
 IF(RW.EQ.'R'.OR.RW.EQ.'r')THEN
  ...
  READ(IU,'(A256)') LINE
  READ(LINE,*,IOSTAT=IOS) SPF(ISPF)%NXY,NTBSOL ! //','//TRIM(ITOS(SIZE(SPF(ISPF)%PROF)))
  IF(IOS.EQ.0)THEN
   READ(LINE,*,IOSTAT=IOS) SPF(ISPF)%NXY !,NTBSOL ! //','//TRIM(ITOS(SIZE(SPF(ISPF)%PROF)))
  ENDIF
