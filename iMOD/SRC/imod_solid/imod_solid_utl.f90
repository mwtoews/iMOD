...
CONTAINS
 ...
 DO I=1,NSOLID
  ...
  ICLR=0
  DO J=1,SLD(I)%NLAY
   IF(.NOT.UTL_READINITFILE(UTL_CAP('INT_S'//TRIM(ITOS(I))//'_L'//TRIM(ITOS(J)),'U'),LINE,IU,0))RETURN
   READ(LINE,*,IOSTAT=IOS) SLD(I)%INTNAME(J),IR,IG,IB
   IF(IOS.NE.0)THEN
...
!   DO K=1,2
   DO J=1,SLD(I)%NLAY
    IF(.NOT.UTL_READINITFILE('INT_S'//TRIM(ITOS(I))//'_L'//TRIM(ITOS(J)),LINE,IU,0))RETURN
!     IF(K.EQ.1)THEN
...
!     ENDIF
 ...
 DO J=1,SLD(IS)%NLAY
  ...
  IF(JSEL_SOLID(IS).EQ.-1)THEN
   ...
   LINE='INT_S'//TRIM(ITOS(JS))//'_L'//TRIM(ITOS(J))//'="'//TRIM(LINE)//'"'
  ...
  ELSE
   LINE='INT_S'//TRIM(ITOS(JS))//'_L'//TRIM(ITOS(J))//'="'//TRIM(SLD(IS)%INTNAME(J))//'"'
   CALL WRGBSPLIT(SLD(IS)%INTCLR(J),IR,IG,IB)
   LINE=TRIM(LINE)//','//TRIM(ITOS(IR))//','//TRIM(ITOS(IG))//','//TRIM(ITOS(IB))
  ENDIF
  WRITE(IU,'(A)',IOSTAT=IOS) TRIM(LINE)
