...
!  ENDDO
 ...
 DO IROW=1,TOPIDF(1)%NROW; DO ICOL=1,TOPIDF(1)%NCOL; DO ILAY=1,NLAY
  ...
  IF(TR-BR.GT.0.0D0)THEN
   IF(KDHIDF(ILAY)%X(ICOL,IROW).EQ.KDHIDF(ILAY)%NODATA.OR.KDHIDF(ILAY)%X(ICOL,IROW).EQ.0.0D0)THEN
    !## inactivate all below
    DO JLAY=ILAY,NLAY
     KDHIDF(JLAY)%X(ICOL,IROW)=KDHIDF(JLAY)%NODATA
     IF(JLAY.LT.NLAY)CIDF(JLAY)%X(ICOL,IROW)=CIDF(JLAY)%NODATA
     TOPIDF(JLAY)%X(ICOL,IROW)=TOPIDF(JLAY)%NODATA
     BOTIDF(JLAY)%X(ICOL,IROW)=BOTIDF(JLAY)%NODATA
    ENDDO
   ENDIF
