...
USE MOD_UTL, ONLY : UTL_IMODFILLMENU,ITOS,UTL_CREATEDIR,UTL_GETUNIT,UTL_MESSAGEHANDLE,NEWLINE, &
                    ...
                    UTL_INSIDEPOLYGON,UTL_CLOSEUNITS,UTL_IDFSNAPTOGRID,UTL_DIRINFO_POINTER,UTL_SUBST
...
USE MOD_SOLID_PROFILE, ONLY : SOLID_PROFILEINIT,SOLID_PROFILECLOSE
...
CONTAINS
     ...
     SELECT CASE (MESSAGE%VALUE2)
      ...
      CASE(ID_DSOLIDTAB1)
       ...
       IF(SOLIDOPENSOL('R',GETSOLNAME()))THEN
        ...
        IF(NGEN.GT.0)THEN
         CALL WGRIDPUTINTEGER(IDF_GRID1,1,GEN%ILAY,NGEN)
         CALL WGRIDPUTSTRING(IDF_GRID1,2,GEN%FNAME,NGEN)
        ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,K,IROW,ICOL,ILAY,JLAY,IL,IU,IOS
 REAL :: C1,C2,C3,TP,BT,X
...
ILLOOP: DO
 ...
 DO I=1,IL
  LINE=',1.0,0.0,'//TRIM(RESULTFOLDER)//'\BND\BND_L'//TRIM(ITOS(I))//'.IDF'
  WRITE(IU,'(I10,A)') I,TRIM(LINE)
 ENDDO
 WRITE(IU,'(I10,A)') IL,',(SHD)'
 DO I=1,IL
  LINE=',1.0,0.0,'//TRIM(RESULTFOLDER)//'\SHD\SHD_L'//TRIM(ITOS(I))//'.IDF'
  WRITE(IU,'(I10,A)') I,TRIM(LINE)
 ENDDO
 WRITE(IU,'(I10,A)') IL,',(TOP)'
 DO I=1,IL
  LINE=',1.0,0.0,'//TRIM(RESULTFOLDER)//'\TOP\TOP_L'//TRIM(ITOS(I))//'.IDF'
  WRITE(IU,'(I10,A)') I,TRIM(LINE)
 ENDDO
 WRITE(IU,'(I10,A)') IL,',(BOT)'
 DO I=1,IL
  LINE=',1.0,0.0,'//TRIM(RESULTFOLDER)//'\BOT\BOT_L'//TRIM(ITOS(I))//'.IDF'
  WRITE(IU,'(I10,A)') I,TRIM(LINE)
 ENDDO
 WRITE(IU,'(I10,A)') IL,',(KDW)'
 DO I=1,IL
  LINE=',1.0,0.0,'//TRIM(RESULTFOLDER)//'\KDW\KDW_L'//TRIM(ITOS(I))//'.IDF'
  WRITE(IU,'(I10,A)') I,TRIM(LINE)
 ENDDO
 WRITE(IU,'(I10,A)') IL-1,',(VCW)'
 DO I=1,IL-1
  LINE=',1.0,0.0,'//TRIM(RESULTFOLDER)//'\VCW\VCW_L'//TRIM(ITOS(I))//'.IDF'
  WRITE(IU,'(I10,A)') I,TRIM(LINE)
 ENDDO
 CLOSE(IU)
...
!  CALL SOLID_PROFILECLOSE()
 ...
 IF(ITOPBOT.EQ.1)THEN
  ...
  ENDIF
 ELSE
  ILIST(1)=1 !## top idf/value
  ILIST(2)=1 !## bot idf/value
 ENDIF
 ...
 CALL WDIALOGGETREAL(IDF_REAL5,CS)
 ...
 CALL WDIALOGPUTSTRING(IDF_LABEL3,'Number of interfaces')
 ...
 IF(ICLIP.EQ.1)THEN
  CALL WDIALOGGETREAL(IDF_REAL1,SOLIDF(1)%XMIN)
  CALL WDIALOGGETREAL(IDF_REAL2,SOLIDF(1)%YMIN)
  CALL WDIALOGGETREAL(IDF_REAL3,SOLIDF(1)%XMAX)
  CALL WDIALOGGETREAL(IDF_REAL4,SOLIDF(1)%YMAX)
  CALL WDIALOGGETREAL(IDF_REAL5,SOLIDF(1)%DX)
...
!  CALL IDFCOPY(TB(1),SOLIDF(1))
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,K,ILAY,IROW,ICOL,IKH,IKV,IU,IOS,NLAY
 REAL :: TR,BR,Z1,Z2,F,KVAL,XTOP,XBOT,X
 ...
 DO I=1,SIZE(REGISFILES)
  ...
  ELSE
   ...
   !## try top
   WRITE(* ,'(A)') '-'//TRIM(REGISFILES_TOP(I)); WRITE(IU,'(A)') '-'//TRIM(REGISFILES_TOP(I))
   IF(.NOT.IDFREADSCALE(REGISFILES_TOP(I),TB(1),2,1,0.0,0))RETURN !## scale mean
   TB(1)%FNAME=TRIM(OUTPUTFOLDER)//'\REGIS\'//TRIM(REGISFILES_TOP(I)(INDEX(REGISFILES_TOP(I),'\',.TRUE.):))

   !## try bot
   WRITE(* ,'(A)') '-'//TRIM(REGISFILES_BOT(I)); WRITE(IU,'(A)') '-'//TRIM(REGISFILES_BOT(I))
   IF(.NOT.IDFREADSCALE(REGISFILES_BOT(I),TB(2),2,1,0.0,0))RETURN !## scale mean
   ...
   IF(IOS.EQ.0)THEN
    IKH=1; KH(1)%X=X
   ELSE
    IKH=1; IF(.NOT.IDFREADSCALE(REGISFILES_KHV(I),KH(1),3,1,0.0,1))IKH=0 !## scale mean
    KH(1)%FNAME=TRIM(OUTPUTFOLDER)//'\REGIS\'//TRIM(REGISFILES_KHV(I)(INDEX(REGISFILES_KHV(I),'\',.TRUE.):))
   ENDIF
   IF(IKH.EQ.1)WRITE(* ,'(A)') '-'//TRIM(REGISFILES_KHV(I)); WRITE(IU,'(A)') '-'//TRIM(REGISFILES_KHV(I))
   ...
   ELSE
    !## try kv
    IKV=1; IF(.NOT.IDFREADSCALE(REGISFILES_KVV(I),KV(1),3,1,0.0,1))IKV=0 !## scale mean
    KV(1)%FNAME=TRIM(OUTPUTFOLDER)//'\REGIS\'//TRIM(REGISFILES_KVV(I)(INDEX(REGISFILES_KVV(I),'\',.TRUE.):))
   ENDIF
   IF(IKV.EQ.1)WRITE(* ,'(A)') '-'//TRIM(REGISFILES_KVV(I)); WRITE(IU,'(A)') '-'//TRIM(REGISFILES_KVV(I))
  ...
  DO ILAY=1,SIZE(CFRAC)
   CFRAC(ILAY)%FNAME=TRIM(OUTPUTFOLDER)//'\CFRAC\'//TRIM(CTYPE)//'_l'//TRIM(ITOS(ILAY))//'.idf'; CFRAC(ILAY)%X=0.0; CFRAC(ILAY)%NODATA=0.0
  ENDDO
  ...
  DO ILAY=1,NLAY
   IF(SUM(FFRAC(ILAY)%X).GT.0.0)THEN
...
!    IF(.NOT.IDFWRITE(FFRAC(ILAY),FFRAC(ILAY)%FNAME,1))RETURN
  ...
  ENDDO
  DO ILAY=1,NLAY-1
   IF(SUM(CFRAC(ILAY)%X).GT.0.0)THEN
