...
! DO I=1,SIZE(MASK); CALL IDFDEALLOCATEX(MASK(I)%IDF); CALL IDFDEALLOCATESX(MASK(I)%IDF); ENDDO; DEALLOCATE(MASK)
 ...
 IF(SOLIDOPENSOL('R',GETSOLNAME()))THEN
  ...
  !## save solid as something might has been changed
...
!  CALL IDFCOPY(TB(1),SOLIDF(1))
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: MINTHICKNESS=0.0D0
 INTEGER,INTENT(IN) :: IHYPO
 INTEGER :: I,J,K,ILAY,JLAY,IROW,ICOL,IU,IOS,NLAY,N 
 REAL(KIND=DP_KIND) :: TR,BR,Z1,Z2,F,KVAL,XTOP,XBOT,D
  ...
  DO IROW=1,TOPIDF(1)%NROW; DO ICOL=1,TOPIDF(1)%NCOL
   ...
   DO ILAY=1,NLAY-1
    ...
    IF(Z1.GT.Z2)THEN
     ...
     KVAL=MAX(0.0D0,KV(1)%X(ICOL,IROW)) !KDVIDF(ILAY)%X(ICOL,IROW)) !KV(1)%X(ICOL,IROW))
...
!  ENDDO
 ...
 !## check and remove small thicknesses for aquifers/(aquitards)
 DO IROW=1,TOPIDF(1)%NROW; DO ICOL=1,TOPIDF(1)%NCOL; DO ILAY=1,NLAY
  ...
  IF(TR-BR.LT.MINTHICKNESS)THEN  !T-BR=0.1 MINTHICKNESS=2
   D=(TR-BR)   !2-0.1=1.9
   KDHIDF(ILAY)%X(ICOL,IROW)=0.0D0
   KDVIDF(ILAY)%X(ICOL,IROW)=0.0D0
   !## shift all below
   DO JLAY=ILAY,NLAY
    BOTIDF(JLAY)%X(ICOL,IROW)=BOTIDF(JLAY)%X(ICOL,IROW)+D
    IF(JLAY+1.LT.NLAY)TOPIDF(JLAY+1)%X(ICOL,IROW)=TOPIDF(JLAY+1)%X(ICOL,IROW)+D
   ENDDO
  ENDIF
 ENDDO; ENDDO; ENDDO
 ...
 !## overwrite top/bot for small modifications
 DO I=1,SIZE(TOPIDF);    IF(.NOT.IDFWRITE(TOPIDF(I),TOPIDF(I)%FNAME,1))RETURN; ENDDO
 DO I=1,SIZE(BOTIDF);    IF(.NOT.IDFWRITE(BOTIDF(I),BOTIDF(I)%FNAME,1))RETURN; ENDDO

 !## make boundary
 DO ILAY=1,NLAY; DO IROW=1,TOPIDF(1)%NROW; DO ICOL=1,TOPIDF(1)%NCOL
  IF(TOPIDF(ILAY)%X(ICOL,IROW).EQ.TOPIDF(ILAY)%NODATA)THEN
   TOPIDF(ILAY)%X(ICOL,IROW)=0.0D0
  ELSE
   TOPIDF(ILAY)%X(ICOL,IROW)=1.0D0
  ENDIF
 ENDDO; ENDDO; TOPIDF(ILAY)%FNAME=TRIM(OUTPUTFOLDER)//'\mdl_ib_l'//TRIM(ITOS(ILAY))//'.idf'; ENDDO
 DO I=1,SIZE(TOPIDF); IF(.NOT.IDFWRITE(TOPIDF(I),TOPIDF(I)%FNAME,1))RETURN; ENDDO
