...
!  CALL SOLIDPROFILECLOSE()
 ...
 REAL :: X,Y,TOP,BOT,DZZ,XMIN,YMIN,XMAX,YMAX,CS
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: ITYPE
 ...
 ELSE
  ...
  !## existing filename
  IF(I.EQ.0)THEN; CTOP=MP(I)%IDFNAME; IOS=1
  ELSE; READ(CTOP,*,IOSTAT=IOS) X; ENDIF
  IF(IOS.NE.0)THEN
  ...
  !## existing filename
  IF(I.EQ.0)THEN; CBOT=MP(I)%IDFNAME; IOS=1
  ELSE; READ(CBOT,*,IOSTAT=IOS) X; ENDIF
  IF(IOS.NE.0)THEN
...
! ENDIF
 ...
 NLAY=SLD(1)%NINT; ALLOCATE(TOPIDF(NLAY),BOTIDF(NLAY)) 
 DO I=1,SLD(1)%NINT; DO J=1,2
