 ...
 DO ILAY=1,NLAY
  ...
  IF(IDFALLOCATEX(IDFK(1)).AND.IDFALLOCATEX(IDFK(2)))THEN
   ...
   IF(IKRIGING.EQ.1)THEN
    ...
    CALL KRIGING_MAIN(SIZE(XD),XD,YD,ZD,PD,WD,IDFK(1),IDFK(2),IBATCH,0.0D0)
  ...
  IF(ISEL_IDF(ILAY).EQ.1)THEN
   ...
   IF(MAXVAL(PCG(ILAY)%IB).GT.0)THEN
    ...
    IF(ALLOCATED(GENSOL))THEN
     ...
     CALL ASC2IDF_INT_NULLIFY(); ALLOCATE(XP(100),YP(100),ZP(100),PP(100),FP(100),WP(100))
...
!   PCG(ILAY)%IB(:,:)=-3 !## whole modellayer inactive (constant head)
 ...
 DO I=1,2
  ...
  IF(N.EQ.0)EXIT
  IF(I.EQ.1)THEN
   M=SIZE(XD); ALLOCATE(XDDUMMY(M+N),YDDUMMY(M+N),ZDDUMMY(M+N),PDDUMMY(M+N))
   DO J=1,M; XDDUMMY(J)=XD(J); YDDUMMY(J)=YD(J); ZDDUMMY(J)=ZD(J); PDDUMMY(J)=PD(J); ENDDO
   DEALLOCATE(XD,YD,ZD,PD); XD=>XDDUMMY; YD=>YDDUMMY; ZD=>ZDDUMMY; PD=>PDDUMMY; N=M
  ENDIF
 ENDDO
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: FCT_CC_GIVEN=1.0d0 !0.01D0
    ...
    DO I=1,SHP%NPOL
     ...
     DO IROW=MAX(1,IR1),MIN(IR2,SOLIDF(ILAY)%NROW)
      DO ICOL=MAX(1,IC1),MIN(IC2,SOLIDF(ILAY)%NCOL)
       ...
       IF(DBL_IGRINSIDESHAPE(X1,Y1,SHP%POL(I)).EQ.1)PCG(ILAY)%IB(ICOL,IROW)=1
...
! SOLID_CALC_FILL=.TRUE.
 ...
 DO J=1,2
  ...
  IF(J.EQ.1)THEN
   ...
   ALLOCATE(XD(N),YD(N),ZD(N),PD(N),WD(N))
  ENDIF
 ENDDO
  ...
  DO J=1,2
   DO I=1,NIPFP
    ...
    DO II=1,N
     ...
     !## add point to list
     IF(LEX)THEN; NP=NP+1; IF(J.EQ.2)THEN; XD(NP)=X; YD(NP)=Y; ZD(NP)=0.0D0; PD(NP)=Z; WD(NP)=1.0D0; ENDIF; ENDIF
   ...
   IF(J.EQ.1)THEN
    M=SIZE(XD); ALLOCATE(XDDUMMY(M+NP),YDDUMMY(M+NP),ZDDUMMY(M+NP),PDDUMMY(M+NP),WDDUMMY(M+NP))
    DO I=1,M; XDDUMMY(I)=XD(I); YDDUMMY(I)=YD(I); ZDDUMMY(I)=ZD(I); PDDUMMY(I)=PD(I); WDDUMMY(I)=WD(I); ENDDO
    DEALLOCATE(XD,YD,ZD,PD,WD); XD=>XDDUMMY; YD=>YDDUMMY; ZD=>ZDDUMMY; PD=>PDDUMMY; WD=>WDDUMMY; NP=M
   ENDIF
 ...
 IF(IEXPORTMODE.EQ.1)THEN
  ...
  ROT=0.0D0; RAN=1.0D0
  CALL KRIGING_INIT(SIZE(XD),XD,YD,ZD,PD,WD,ND,IDF,COINCIDENT,COINCIDENTDIST,0,0.0D0,0,ROT,RAN) 
  ...
  IF(IU(1).LE.0)RETURN
  WRITE(IU(1),'(A)') 'X,Y,Z,P'
  DO I=1,ND; WRITE(IU(1),'(3(F15.7,A1))') XD(I),',',YD(I),',',ZD(I),',',PD(I); ENDDO
