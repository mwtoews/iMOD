   ...
   ELSEIF(IKRIGING.EQ.2.OR.IKRIGING.EQ.3)THEN
    ...
    IF(IDFALLOCATEX(IDFK(1)).AND.IDFALLOCATEX(IDFK(2)))THEN
     ...
     ELSE
      ...
      DO IROW=1,NROW; DO ICOL=1,NCOL
       IF(IB(ICOL,IROW,ILAY).GT.0)HOLD(ICOL,IROW,ILAY)=IDFK(1)%X(ICOL,IROW)
      ENDDO; ENDDO
     ENDIF      
     ICNVG(ILAY)=1
...
!      ENDDO
 ...
 IF(IBATCH.EQ.0)THEN
  CALL UTL_MESSAGEHANDLE(1)
  IF(IKRIGING.EQ.4)LEX=.TRUE.
  IF(LEX)THEN
   IF(IKRIGING.LE.3)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfully written new solid.','Information')
   IF(IKRIGING.EQ.4)THEN
    IF(IEXPORTMODE.EQ.1)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfully exported solid as IPF/IDF files.','Information')
    IF(IEXPORTMODE.EQ.2)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfully exported solid as GEO file(s).','Information')
   ENDIF
  ENDIF
 ENDIF
 ...
 DO I=1,NTBSOL
  ...
  CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,I+1,SLD(1)%INTNAME(I)(INDEX(SLD(1)%INTNAME(I),'\',.TRUE.)+1:))
 ...
 DO ITER1=1,MXITER1 
  ...
  CALL PCG2AP(NROW*NCOL,NROW,NCOL,1,IB(:,:,1),CR,CC,CV,HCOF,RHS(:,:,1),V,SS,P, &
              CD,HNEW(:,:,1),MXITER1,MXITER2,ITER1,ITER2,ICNVG,HCLOSE,RCLOSE,IECHO,NICNVG,RELAX,HCHG) 
  !## convergence achieved
  IF(ICNVG.EQ.1)EXIT
  IF(MICNVG.GT.0.AND.NICNVG.GT.MICNVG)EXIT
