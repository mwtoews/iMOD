...
USE MOD_SOLID_PAR
USE MOD_UTL, ONLY : ITOS,UTL_INSIDEPOLYGON,NEWLINE,UTL_CREATEDIR,UTL_IDFSNAPTOGRID,UTL_SUBST,UTL_IMODVERSION,UTL_PCK_READTXT
...
USE MOD_SOLID_UTL
USE MOD_KRIGING, ONLY : KRIGINGSETTINGS,KRIGING_VARIOGRAM,KRIGING_MAIN,KRIGING_INIT
  ...
  IF(IDFALLOCATEX(IDFK(1)).AND.IDFALLOCATEX(IDFK(2)))THEN
   ...
   IF(IKRIGING.EQ.1)THEN
    ...
    CALL KRIGING_MAIN(SIZE(XD),XD,YD,ZD,IDFK(1),IDFK(2),          &
                      ...
                      0,0.0,KSETTINGS(ILAY)%COINCIDENT,KSETTINGS(ILAY)%COINCIDENTDIST, &
                      KSETTINGS(ILAY)%IQUADRANT,IBATCH,GENFNAME)   
   ...
   ELSEIF(IKRIGING.EQ.3)THEN
    CALL SOLID_EXPORT(ILAY,IEXPORTMODE,IDFK(1),KSETTINGS(ILAY)%KTYPE,KSETTINGS(ILAY)%COINCIDENT,KSETTINGS(ILAY)%COINCIDENTDIST)
   ENDIF
   ...
   IF(MAXVAL(PCG(ILAY)%IB).GT.0)THEN
    ...
    IF(ALLOCATED(GEN))THEN
     ...
     J=0; DO I=1,SIZE(GEN)
      ...
      J=J+1; 
      IF(LEN_TRIM(GEN(I)%FNAME).GT.0)CALL ASC2IDF_HFB(SOLIDF(ILAY),SOLIDF(ILAY)%NROW,SOLIDF(ILAY)%NCOL,IPC,GEN(I)%FNAME,0)
     ENDDO
     IF(J.GT.0)THEN
...
!    ENDIF    
 ...
 LINE=TRIM(RTOS(MPW%XMIN,'F',2))//','//TRIM(RTOS(MPW%YMIN,'F',2))//','// &
...
!      CALL WDIALOGGETRADIOBUTTON(IDF_RADIO3,IKRIGING)
      ...
      CALL KRIGINGSETTINGS(KSETTINGS(I)%MINP,KSETTINGS(I)%KTYPE,KSETTINGS(I)%RANGE,KSETTINGS(I)%SILL, &
                           ...
                           KSETTINGS(I)%COINCIDENTDIST,KSETTINGS(I)%IQUADRANT)
      CALL WDIALOGSELECT(ID_DSOLID_CALC)
      CALL WDIALOGGETINTEGER(IDF_INTEGER2,I)
...
!    IF(.NOT.IDFREADSCALE(MASK(ILAY)%FNAME,MASK(ILAY)%IDF,7,1,0.0,0))RETURN
...
! SOLID_CALC_FILL=.TRUE.
 ...
 DO J=1,2
  DO I=1,NIPFP
   ...
   DO II=1,N
    ...
    LEX=UTL_PCK_READTXT(1,INT(ILAY,8),INT(ILAY,8),Z,TXTFNAME)
 ...
 !###====================================================================
 SUBROUTINE SOLID_EXPORT(ILAY,IEXPORTMODE,IDF,KTYPE,COINCIDENT,COINCIDENTDIST)
 !###====================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 INTEGER,INTENT(IN) :: ILAY,IEXPORTMODE,COINCIDENT,KTYPE
 ...
 IF(IEXPORTMODE.EQ.1)THEN
  ...
  CALL KRIGING_INIT(SIZE(XD),XD,YD,ZD,ND,IDF,KTYPE,COINCIDENT,COINCIDENTDIST) 
