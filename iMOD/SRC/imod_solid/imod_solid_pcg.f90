...
USE MOD_SOLID_UTL
USE MOD_KRIGING, ONLY : KRIGINGSETTINGS,KRIGING_VARIOGRAM,KRIGING_MAIN,KRIGING_INIT,KRIGING_READGEN
...
CONTAINS
 ...
 DO ILAY=1,NLAY
  ...
  IF(IDFALLOCATEX(IDFK(1)).AND.IDFALLOCATEX(IDFK(2)))THEN
   ...
   CALL KRIGING_READGEN(ILAY,NGENSOL,GENSOL%FNAME,GENSOL%ILAY) 
   ...
   IF(IKRIGING.EQ.1)THEN
    ...
    CALL KRIGING_MAIN(SIZE(XD),XD,YD,ZD,IDFK(1),IDFK(2),          &
                      ...
                      KSETTINGS(ILAY)%IQUADRANT,IBATCH,0,0.0,0)   
...
! SOLID_CALC_FILL=.TRUE.
  ...
  DO J=1,2
   DO I=1,NIPFP
    ...
    DO II=1,N
     ...
     LEX=UTL_PCK_READTXT(1,INT(ILAY,8),INT(ILAY,8),Z,TXTFNAME,0,'',0)
 ...
 IF(IEXPORTMODE.EQ.1)THEN
  ...
  CALL KRIGING_INIT(SIZE(XD),XD,YD,ZD,ND,IDF,KTYPE,COINCIDENT,COINCIDENTDIST,0,0.0,0) 
