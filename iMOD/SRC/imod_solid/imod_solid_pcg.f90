...
USE MOD_SOLID_PAR
USE MOD_UTL, ONLY : ITOS,UTL_INSIDEPOLYGON,NEWLINE,UTL_CREATEDIR,UTL_IDFSNAPTOGRID,UTL_SUBST
...
USE MOD_INTERSECT, ONLY : INTERSECT_EQUI,INTERSECT_NONEQUI,INTERSECT_DEALLOCATE  !,INTERSECT_NONEQUI
...
CONTAINS
 ...
 INTEGER :: I,J,K,ITER1,ITER2,ILAY,IROW,ICOL,ITYPE,IL,IECHO,IVERSION,NICNVG, &
     JKRIGING,ICOMP,NCOMP,NPR,NPC,JROW,JCOL,IR,IC
...
!    PCG(1)%HCOF(ICOL,IROW)=PCG(1)%HCOF(ICOL,IROW)-(FTIGHT*COND/10000.0) 
!    PCG(1)%RHS(ICOL,IROW) =PCG(1)%RHS(ICOL,IROW) -(FTIGHT*COND/10000.0)*PCG(JLAY)%HOLD(ICOL,IROW) 
   ENDIF
  ENDDO; ENDDO
 ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ICOL,IROW,ILAY,MX,ISPF,I,J,N,IPOS,NLOC,IR1,IR2,IC1,IC2,M,ISTEP
 REAL :: DX,DZZ,G,X1,X2,Y1,Y2,COND
...
!     CALL SPLINE_MAIN(SPF(ISPF)%PROF(ILAY)%PX,SPF(ISPF)%PROF(ILAY)%PZ,SPF(ISPF)%PROF(ILAY)%NPOS,XI,ZI,M)
    ...
    IF(N.GT.2)THEN
     ...
     N=0; DO IROW=SOLIDF(ILAY)%NROW,1,-1
      ...
      IF(ZI(N).NE.SOLIDF(ILAY)%NODATA)THEN
       IF(PCG(ILAY)%IB(ICOL,IROW).NE.-2)THEN
        PCG(ILAY)%IB(ICOL,IROW)=-2; PCG(ILAY)%HOLD(ICOL,IROW)=ZI(N); PCG(ILAY)%COND(ICOL,IROW)=COND/10.0
       ENDIF
      ENDIF
     ENDDO
    ENDIF
 ...
 DO I=1,SIZE(PCG)
  ...
  ALLOCATE(PCG(I)%COND(NC,NR),STAT=IOS(4))
 ENDDO
 ...
 IF(ILAY.EQ.0)THEN
  ...
  ELSEIF(IEXPORTMODE.EQ.3)THEN
   ...
   DO ISPF=1,NSPF
    ...
    IF(INDEX(UTL_CAP(BVERSION,'U'),'V').GT.0)THEN
     CVERSION=BVERSION(2:)
     !## remove "_" for "."
     DO
      IF(INDEX(CVERSION,'_').LE.0)EXIT
      CVERSION=UTL_SUBST(CVERSION,'_','.')
     ENDDO
     WRITE(IU(ISPF),'(A)') 'CREATED BY : iMOD version '//TRIM(CVERSION)
    ELSE
     WRITE(IU(ISPF),'(A)') 'CREATED BY : iMOD version '//TRIM(BVERSION)
    ENDIF
    WRITE(IU(ISPF),'(78A1)') ('=',J=1,78)
