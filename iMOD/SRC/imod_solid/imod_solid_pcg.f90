...
INTEGER,PRIVATE :: MINP=10,KTYPE=2,PNTSEARCH=0,IEXPVARIOGRAM=0,IADJRANGE=0,IQUADRANT=0
REAL,PRIVATE :: RANGE=5000.0,SILL=30.0,NUGGET=0.0
INTEGER :: IKRIGING=0
INTEGER :: IEXPORTMODE
LOGICAL :: LSEMIVARIOGRAM,LSPLINE,LSMOOTH
...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,ITER1,ITER2,ILAY,IROW,ICOL,ITYPE,IECHO,IVERSION,NICNVG, &
     JROW,JCOL,IR,IC
 INTEGER,ALLOCATABLE,DIMENSION(:) :: ICNVG
 REAL,ALLOCATABLE,DIMENSION(:) :: ARRAY_HCHG
 REAL,ALLOCATABLE,DIMENSION(:,:) :: DUMMY
 REAL :: TOP,C,HCHG,RCHG,HCHGOLD,S,X,Y,H1,H2
 LOGICAL :: LEX
 CHARACTER(LEN=256) :: FNAME,LINE,GENFNAME
 ...
 IF(IKRIGING.EQ.3)THEN 
  ...
  CALL SOLID_EXPORT(  TRIM(OUTPUTFOLDER)//'\EXPORT',0)
...
!    ENDIF
   ...
   ELSEIF(IKRIGING.EQ.2)THEN
    ...
    IF(IDFALLOCATEX(IDFK(1)).AND.IDFALLOCATEX(IDFK(2)))THEN
     ...
     CALL KRIGING_MAIN(SIZE(XD),XD,YD,ZD,IDFK(1),IDFK(2),MINP,RANGE,SILL,NUGGET,KTYPE, &
         PNTSEARCH,IEXPVARIOGRAM,0,0.0,IADJRANGE,IQUADRANT,IBATCH,GENFNAME)   
...
!   PCG(ILAY)%IB(:,:)=-3 !## whole modellayer inactive (constant head)
 ...
 IF(LEX)THEN 
  ...
  IF(LSMOOTH)THEN
   DO ILAY=1,NLAY
    IF(ISEL_IDF(ILAY).EQ.1)THEN
     ALLOCATE(DUMMY(SOLIDF(ILAY)%NCOL,SOLIDF(ILAY)%NROW)); DUMMY=0.0
     DO IROW=1,SOLIDF(ILAY)%NROW; DO ICOL=1,SOLIDF(ILAY)%NCOL
      !## smooth only active nodes
      IF(PCG(ILAY)%IB(ICOL,IROW).GT.0)THEN
       H1=0.0; H2=0.0
       DO IR=MAX(1,IROW-1),MIN(SOLIDF(ILAY)%NROW,IROW+1)
        DO IC=MAX(1,ICOL-1),MIN(SOLIDF(ILAY)%NCOL,ICOL+1)
         IF(PCG(ILAY)%IB(IC,IR).EQ.0)CYCLE
         H1=H1+PCG(ILAY)%HOLD(IC,IR)
         H2=H2+1.0        
        ENDDO
       ENDDO
       IF(H2.GT.0.0)THEN
        DUMMY(ICOL,IROW)=H1/H2
       ENDIF
      ENDIF
     ENDDO; ENDDO
     DO IROW=1,SOLIDF(ILAY)%NROW; DO ICOL=1,SOLIDF(ILAY)%NCOL
      IF(PCG(ILAY)%IB(ICOL,IROW).GT.0)PCG(ILAY)%HOLD(ICOL,IROW)=DUMMY(ICOL,IROW)
     ENDDO; ENDDO
     DEALLOCATE(DUMMY)
    ENDIF
   ENDDO
 ...
 IF(IBATCH.EQ.0)THEN
  ...
  IF(LEX)THEN
   IF(IKRIGING.LE.2)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully written new solid.','Information')
   IF(IKRIGING.EQ.3)THEN
    ...
    IF(IEXPORTMODE.EQ.2)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully exported solid as *.IDF files.','Information')
    IF(IEXPORTMODE.EQ.3)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully exported solid as *.GEO files.','Information')
   ENDIF
  ENDIF
 ENDIF
...
JROWLOOP: DO JROW=MAX(1,IROW-1),MIN(SELIDF(ILAY)%NROW,IROW+1)
      DO JCOL=MAX(1,ICOL-1),MIN(SELIDF(ILAY)%NCOL,ICOL+1)
       IF(PCG(ILAY)%IB(ICOL,IROW).GT.0)THEN
        N=N+1
        IF(I.EQ.2)THEN
...
!   IF(.NOT.IDFREAD(SOLIDF(I),SLD(1)%INTNAME(I),1))RETURN !## scale use mean algorithm
 ...
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: ITYPE,I,N,ICOL,IROW,IMIDELEV
 TYPE(IDFOBJ) :: XIDF
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (ID_PROPERTIES_KRIGING)
      ...
      CALL KRIGINGSETTINGS(MINP,KTYPE,RANGE,SILL,NUGGET,PNTSEARCH,IADJRANGE,IQUADRANT)
      CALL WDIALOGSELECT(ID_DSOLID_CALC)     
     ...
     CASE (IDOK,IDF_BUTTON6,IDCANCEL)
      ...
      LSMOOTH=.FALSE.; IF(I.EQ.1)LSMOOTH=.TRUE.
      IMIDELEV=0; FMIDELEV=REAL(IMIDELEV)
      CALL WDIALOGGETRADIOBUTTON(IDF_RADIO1,I)
 ...
 !## overrule spline settings in case of export mode
 IF(IKRIGING.EQ.3)LSPLINE=.FALSE.
...
!     CALL SPLINE_MAIN(SPF(ISPF)%PROF(ILAY)%PX,SPF(ISPF)%PROF(ILAY)%PZ,SPF(ISPF)%PROF(ILAY)%NPOS,XI,ZI,M)
 ...
 IF(ITIGHT.EQ.2)THEN
  ...
  DO ILAY=1,NLAY
   ...
   solidf(ilay)%x=pcg(ilay)%cond
   if(idfwrite(solidf(ilay),'d:\tmp_cond_l'//trim(itos(ilay))//'.idf',1))then; endif
   solidf(ilay)%x=pcg(ilay)%HOLD
   if(idfwrite(solidf(ilay),'d:\tmp_hold_l'//trim(itos(ilay))//'.idf',1))then; endif
