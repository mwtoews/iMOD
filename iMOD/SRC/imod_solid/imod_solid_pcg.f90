...
! NCOL=MDLIDF%NCOL; NROW=MDLIDF%NROW
 ...
 DO ITER1=1,MXITER1 
  ...
  IF(ITIGHT.EQ.2)THEN
   C=0.1
   DO IROW=1,NROW; DO ICOL=1,NCOL
    IF(PCG(1)%IB(ICOL,IROW).EQ.3)THEN
 ...
 DO I=1,SIZE(PCG)
  NC=SOLIDF(I)%NCOL; NR=SOLIDF(I)%NROW
...
!   WRITE(IU(ISPF),'(A/)') '[END OF LAYERLOADS]'
 ...
 DO IROW=1,NROW; DO ICOL=1,NCOL
  IF(Y(ICOL,IROW).EQ.0.AND.PCG(ILAY)%IB(ICOL,IROW).GT.0)THEN
   IP=1
   CALL SOLID_TRACE(IROW,ICOL,PCG(ILAY)%IB,PCG(ILAY)%CC,PCG(ILAY)%CR,Y,THREAD,ISPEC,YSEL,N,NROW,NCOL,IP,LCNST,MAXTHREAD,MAXN)
