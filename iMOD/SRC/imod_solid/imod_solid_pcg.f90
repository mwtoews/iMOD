...
!    CALL SPLINE_MAIN(SPF(ISPF)%PROF(ILAY)%PX,SPF(ISPF)%PROF(ILAY)%PZ,SPF(ISPF)%PROF(ILAY)%NPOS,XI,ZI,M)
 ...
 IF(IEXPORTMODE.EQ.1)THEN
  ...
  ALLOCATE(IU(1))
  IU(1)=UTL_GETUNIT()
  CALL OSD_OPEN(IU(1),FILE=TRIM(OUTPUTFOLDER)//'\EXPORT_INTERFACE_'//TRIM(ITOS(ILAY))//'.IPF',STATUS='UNKNOWN',FORM='FORMATTED',ACTION='WRITE')
 ...
 ELSEIF(IEXPORTMODE.EQ.2)THEN
  ...
  DO ISPF=1,NSPF
   IU(ISPF)=UTL_GETUNIT()
   CALL OSD_OPEN(IU(ISPF),FILE=TRIM(OUTPUTFOLDER)//'\SOLIDTOOL_CROSSECTION'//TRIM(ITOS(ISPF))//'.GEO',STATUS='UNKNOWN',FORM='FORMATTED',ACTION='WRITE')
