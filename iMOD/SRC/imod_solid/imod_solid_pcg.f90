...
INTEGER,PRIVATE :: IEXPVARIOGRAM=0
INTEGER :: IKRIGING=0
INTEGER :: IEXPORTMODE
LOGICAL :: LSEMIVARIOGRAM,LSPLINE 
...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,ILAY,IROW,ICOL,IVERSION,IKRIGING
 LOGICAL :: LEX
 ...
 LEX=SOLID_CALC_INIT(IVERSION,IKRIGING)
  ...
  IF(IDFALLOCATEX(IDFK(1)).AND.IDFALLOCATEX(IDFK(2)))THEN
   ...
   IF(IKRIGING.EQ.1)THEN
    ...
    CALL KRIGING_MAIN(SIZE(XD),XD,YD,ZD,IDFK(1),IDFK(2),          &
                      ...
                      IEXPVARIOGRAM,0,0.0, &
                      KSETTINGS(ILAY)%IADJRANGE,KSETTINGS(ILAY)%IQUADRANT,IBATCH,GENFNAME)   
    ...
    CALL IDFFILLCOMMENT(SOLIDF(ILAY),'Created by SolidTool'//NEWLINE//'Units: m2'//NEWLINE//TRIM(TXT)//' '//TRIM(ITOS(ILAY)))
...
JROWLOOP: DO JROW=MAX(1,IROW-1),MIN(SOLIDF(ILAY)%NROW,IROW+1)
      DO JCOL=MAX(1,ICOL-1),MIN(SOLIDF(ILAY)%NCOL,ICOL+1)
       IF(PCG(ILAY)%IB(ICOL,IROW).GT.0)THEN
        N=N+1
        IF(I.EQ.2)THEN
  ...
  IF(I.EQ.1)THEN
   ...
   DO J=1,M; XDDUMMY(J)=XD(J); YDDUMMY(J)=YD(J); YDDUMMY(J)=YD(J); ENDDO
   DEALLOCATE(XD,YD,ZD); XD=>XDDUMMY; YD=>YDDUMMY; ZD=>ZDDUMMY; N=M
  ENDIF
 ENDDO

 END SUBROUTINE SOLID_ADDBND
 ...
 !###======================================================================
 LOGICAL FUNCTION SOLID_CALC_INIT(IVERSION,IKRIGING)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(OUT) :: IVERSION,IKRIGING
 TYPE(WIN_MESSAGE) :: MESSAGE
...
! ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ICOL,IROW,ILAY,MX,ISPF,I,J,N,IPOS,NLOC,IR1,IR2,IC1,IC2,M,ISTEP,NN
 REAL :: DX,DZZ,G,X1,X2,Y1,Y2,COND,FX
...
!     CALL SPLINE_MAIN(SPF(ISPF)%PROF(ILAY)%PX,SPF(ISPF)%PROF(ILAY)%PZ,SPF(ISPF)%PROF(ILAY)%NPOS,XI,ZI,M)
 ...
 !###====================================================================
 SUBROUTINE SOLID_EXPORT(FNAME,ILAY)
 !###====================================================================
 IMPLICIT NONE
