  ...
  IF(ISEL_IDF(ILAY).EQ.1)THEN
   ...
   IF(IKRIGING.EQ.1)THEN
    ...
    IF(MAXVAL(PCG(ILAY)%IB).GT.0)THEN
     ...
     !## reset isotropic
     PCG(1)%CR=(MDLIDF%DX*MDLIDF%DY)/C; PCG(1)%CC=(MDLIDF%DX*MDLIDF%DY)/C
     !## include horizontal flow barriers
     CALL HFB_CALC(MDLIDF,PCG(1)%CR,PCG(1)%CC,ILAY)
...
!     CALL SOLID_CHECKIBND(ILAY)
     ...
     DO ITER1=1,MXITER1
      ...
      CALL PCG2AP(NROW*NCOL,NROW,NCOL,1,PCG(ILAY)%IB,PCG(1)%CR,PCG(1)%CC,PCG(1)%CV,PCG(1)%HCOF,PCG(1)%RHS,PCG(1)%V, &
                  PCG(1)%SS,PCG(1)%P,PCG(1)%CD,PCG(1)%HNEW,MXITER1,MXITER2,ITER1,ITER2,ICNVG(ILAY), &
                  HCLOSE,RCLOSE,IECHO,NICNVG,RELAX,HCHG)
    ...
    IF(IDFALLOCATEX(IDFK(1)).AND.IDFALLOCATEX(IDFK(2)))THEN
     ...
     IF(IKRIGING.EQ.3)THEN
      ...
      NPR=0; DO IROW=1,NROW,IOFFSET; NPR=NPR+1; YD(NPR)=MDLIDF%YMAX-MDLIDF%DY*REAL(IROW-0.5)
       ...
       XPINT(NPC,NPR)=PCG(ILAY)%HOLD(ICOL,IROW)
      ENDDO; ENDDO
      ...
      CALL POL1INTMAIN(NCOL,NROW,NPC,NPR,XD,YD,XPINT,DELR,DELC,PCG(ILAY)%HOLD,4,IDFK(1)%NODATA)
...
! CALL WDIALOGFIELDOPTIONS(IDF_GRID1,EDITFIELDCHANGED,0)
   ...
   DO I=1,SHPNO
    ...
    DO IROW=MAX(1,IR1),MIN(IR2,MDLIDF%NROW)
     DO ICOL=MAX(1,IC1),MIN(IC2,MDLIDF%NCOL)
      ...
      IF(UTL_INSIDEPOLYGON(X1,Y1,SHPXC(:,I),SHPYC(:,I),SHPNCRD(I)).EQ.1)PCG(1)%IB(ICOL,IROW)=1
     ENDDO
    ENDDO
   ENDDO
   !## copy shape to all layers
   DO ILAY=2,NLAY; PCG(ILAY)%IB=PCG(1)%IB; ENDDO
  ENDIF
  ...
  IF(NMASK.GT.0)THEN
   DO I=1,NMASK
    ...
    DO IROW=1,MDLIDF%NROW; DO ICOL=1,MDLIDF%NCOL
     ...
     PCG(I)%IB(ICOL,IROW)=MAX(MIN(2,INT(MASK(I)%IDF%X(ICOL,IROW))),-1)
    ENDDO; ENDDO
 ...
 DO ILAY=1,NLAY
  ...
  DO IROW=1,NROW; DO ICOL=1,NCOL
   ...
   PCG(ILAY)%HOLD(ICOL,IROW)=MDLIDF%X(ICOL,IROW) 
    ...
    DO IPOS=2,SPF(ISPF)%PROF(IL)%NPOS
     ...
     DO I=1,NLOC
      ...
      IF(TL(I).GE.PX(IPOS-1).AND. &
       ...
       IF(PCG(IL)%IB(IC(I),IR(I)).NE.0.AND. &
          PCG(IL)%IB(IC(I),IR(I)).NE.-1)PCG(IL)%IB(IC(I),IR(I))=-2
       PCG(IL)%RHS(IC(I),IR(I)) = PCG(IL)%RHS(IC(I),IR(I))+1.0

       IF(PCG(IL)%RHS(IC(I),IR(I)).EQ.1)THEN
        !## overwrite HOLD with new value
        PCG(IL)%HOLD(IC(I),IR(I))=PZ(IPOS-1)+(TL(I)-PX(IPOS-1))*G
       ELSE
        !## add new value to HOLD
        PCG(IL)%HOLD(IC(I),IR(I))=PCG(IL)%HOLD(IC(I),IR(I)) + (PZ(IPOS-1)+(TL(I)-PX(IPOS-1))*G)
       ENDIF
 ...
 IF(IKRIGING.LE.1)THEN
  ALLOCATE(PCG(1)%P(NCOL,NROW)     ,STAT=IOS(1))
  ALLOCATE(PCG(1)%V(NCOL,NROW)     ,STAT=IOS(2))
  ALLOCATE(PCG(1)%SS(NCOL,NROW)    ,STAT=IOS(3))
  ALLOCATE(PCG(1)%CD(NCOL,NROW)    ,STAT=IOS(4))
  ALLOCATE(PCG(1)%CC(NCOL,NROW)    ,STAT=IOS(7))
  ALLOCATE(PCG(1)%CR(NCOL,NROW)    ,STAT=IOS(8))
  ALLOCATE(PCG(1)%HNEW(NCOL,NROW)  ,STAT=IOS(9))
  ALLOCATE(PCG(1)%HCOF(NCOL,NROW)  ,STAT=IOS(10))
  ALLOCATE(PCG(1)%CV(1,1)          ,STAT=IOS(11))
 ENDIF

 DO I=1,SIZE(PCG)
  ALLOCATE(PCG(I)%RHS(NCOL,NROW)   ,STAT=IOS(5))
  ALLOCATE(PCG(I)%IB(NCOL,NROW)    ,STAT=IOS(6))
  ALLOCATE(PCG(I)%HOLD(NCOL,NROW  ),STAT=IOS(12))
 ENDDO
