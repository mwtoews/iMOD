...
!   CALL IDFCOPY(MDLIDF,SOLIDF(I))
!   IF(.NOT.IDFREADSCALE(SLD(1)%INTNAME(I),SOLIDF(I),2,1,0.0,0))RETURN !## scale use mean algorithm
   IF(.NOT.IDFREAD(SOLIDF(I),SLD(1)%INTNAME(I),1))RETURN !## scale use mean algorithm
  ENDDO
 ENDIF
 ...
 !###======================================================================
 SUBROUTINE SOLID_PCGINT(XD,YD,ZD,ND,IERROR,IDF,IECHO,IDUPLICATE,HNOFLOW,CD,LCR) 
 !###======================================================================
 IMPLICIT NONE
...
! ENDIF
 ...
 DO ITER1=1,MXITER1 
  ...
  CALL PCG2AP(NROW*NCOL,NROW,NCOL,1,PCG(1)%IB,PCG(1)%CR,PCG(1)%CC,PCG(1)%CV,PCG(1)%HCOF,PCG(1)%RHS,PCG(1)%V, &
              PCG(1)%SS,PCG(1)%P,PCG(1)%CD,PCG(1)%HNEW,MXITER1,MXITER2,ITER1,ITER2,ICNVG,HCLOSE,RCLOSE,IECHO,NICNVG,RELAX,HCHG,RCHG) 
  !## convergence achieved
  IF(ICNVG.EQ.1)EXIT
  IF(MICNVG.GT.0.AND.NICNVG.GT.MICNVG)EXIT
...
!          PCG(ILAY)%IB(IC(I),IR(I)).NE.-1)THEN
