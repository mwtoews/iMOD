...
USE MOD_UTL, ONLY : UTL_CAP,ITOS,UTL_SETTEXTSIZE,INVERSECOLOUR,PEUCKER_SIMPLIFYLINE,UTL_INSIDEPOLYGON,UTL_FILLARRAY
USE MOD_SOLID_UTL, ONLY : SOLIDOPENSOL,GETSOLNAME
USE MOD_PROF_PAR, ONLY : NXY,XY,ISOLID,IBITMAP,IWINPROFILE,MXNIDF,PROFIDF,SERIE,XMIN,YMIN,XMAX,YMAX
USE MOD_PROFILE_UTL, ONLY : GRAPHUNITS,GRAPHAREA,PROFILE_EXTENT_GRAPH,PROFILE_GETLOCATION
USE MOD_IPF_PAR, ONLY : NIPF,IPF
USE MOD_IPFASSFILE, ONLY : IPFINITASSFILE,IPFCLOSEASSFILE,IPFDIMENSIONASSFILE,ASSF,IPFASSFILEALLOCATE  !IPFPLOTASSFILE
...
CONTAINS
 ...
 CALL WDIALOGSELECT(ID_DSOLID_CROSS)
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (IDOK)
      ...
      IF(LEX)THEN
!       IF(INEW.EQ.0)THEN !## adjust, remove previous one ...
!        DO IWINID=1,SIZE(IWINPROFILE); CALL SOLIDPROFILEDRAW(1,SIZE(SPF(ISPF)%PROF),0,0,IWINID); ENDDO
!       ENDIF
...
!        CALL SOLIDPROFILEDRAW(1,SIZE(SPF(ISPF)%PROF),0,0,IWINID)
...
!       CALL SOLIDPROFILEPUTINTERSECTION(DX,Z,I)
 ...
 IF(X.GT.SPF(ISPF)%PROF(IL)%PX(N))THEN
! IF(X-SPF(ISPF)%PROF(IL)%PX(N).GT.MINDIST)THEN
  N=N+1
  !## reformulate begin
...
!  IF(SPF(JSPF)%PROF(IL)%PX(I).GE.X)THEN
 ...
 DO I=J,NSPF-1 
  ...
  DO K=1,SIZE(SPF(I)%PROF)
   ...
   ALLOCATE(SPF(I)%PROF(K)%PX(MXPX)) !SPF(I)%PROF(K)%NPOS))
   ALLOCATE(SPF(I)%PROF(K)%PZ(MXPX)) !SPF(I)%PROF(K)%NPOS))
 ...
 DZ=(GRAPHUNITS(4,1)-GRAPHUNITS(2,1))/REAL(NTBSOL)
 DO I=1,SIZE(SPF(NSPF)%PROF) !NTBSOL
  ...
  IF(IEXIST(I+1).EQ.1)THEN
   ...
   !## mean vertical depth as first guess!
   SPF(NSPF)%PROF(I)%PZ(1)=GRAPHUNITS(4,1)-DZ*REAL(I-1) !(GRAPHUNITS(4,1)+GRAPHUNITS(2,1))/2.0
  ...
  IF(MOD(I,2).NE.0)SPF(NSPF)%PROF(I)%IWIDTH=2
...
! ENDDO 
 ...
 REAL,DIMENSION(:),ALLOCATABLE :: X,XTMP,YTMP
 ...
 ALLOCATE(XTMP(MAXVAL(SHPNCRD(1:SHPNO))),YTMP(MAXVAL(SHPNCRD(1:SHPNO))))
 !## default outside unless otherwise
 JON=0
 DO J=1,SHPNO
  !## check FIRST point inside/outside polygon j
  ION=UTL_INSIDEPOLYGON(XY(1,1),XY(2,1),SHPXC(:,J),SHPYC(:,J),XTMP,YTMP,SHPNCRD(J))
  !## inside/on edge
  IF(ION.NE.-1)THEN
   JON=1
   EXIT
...
!  IF(ION.EQ.-1)ION=0
 ENDDO
! ION=JON
! !## check LAST  point inside/outside polygon
! I2=UTL_INSIDEPOLYGON(XY(1,NXY),XY(2,NXY),SHPXC(:,J),SHPYC(:,J),XTMP,YTMP,SHPNCRD(J))
...
! CALL IGRCOLOURN(WRGB(150,150,150))
 ...
 ICRD   =0
 DX     =(GRAPHUNITS(3,IWINID)-GRAPHUNITS(1,IWINID))/250.0
 DY     =(GRAPHUNITS(4,IWINID)-GRAPHUNITS(2,IWINID))/250.0
 ...
 SELECT CASE (ICURSOR)
  ...
  CASE (1)
   ...
   !## current point
   X2=GX !SPF(ISPF)%PROF(IELEV)%PX(ICRD)
   Y2=GY !SPF(ISPF)%PROF(IELEV)%PZ(ICRD)
...
ILLOOP: DO IL=MAX(1,IELEV-1),MIN(NTBSOL,IELEV+1)
   ...
   ENDIF
   IF(.NOT.LSNP.AND..NOT.LINT)THEN !ELSE
    SPF(ISPF)%PROF(IELEV)%PX(ICRD)=GX   
 ...
 !###======================================================================
 SUBROUTINE SOLIDPROFILEDELNODE(ICRD,IELEV)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ICRD,IELEV
 INTEGER :: I
 ...
 CALL WDIALOGLOAD(ID_DSOLID_CROSS,ID_DSOLID_CROSS)
 ...
 ENDIF
 CALL WDIALOGPUTIMAGE(IDF_CHECK1,ID_ICONLOCK,1)
 ILOCK=1; CALL WDIALOGPUTCHECKBOX(IDF_CHECK1,ILOCK)
 ...
 ISPF=0
 IF(NSPF.GT.0)THEN
  CALL WDIALOGPUTMENU(IDF_MENU1,SPF%FNAME,NSPF,1)
  ISPF=1
 ENDIF
 CALL SOLIDPROFILEFIELDS()
...
! ALLOCATE(IELEVATION(NTBSOL)) !## store elevation for simular z-values
! ALLOCATE(CELEVATION(NTBSOL)) !## store string elevation
! ALLOCATE(NELEVATION(NTBSOL)) !## store string elevation
...
! DEALLOCATE(IELEVATION,CELEVATION,NELEVATION)
