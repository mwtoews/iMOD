...
! ENDDO 
 ...
 ALLOCATE(CMENU(SIZE(SPF(ISPF)%PROF))); CMENU=''
 ...
 CALL WDIALOGPUTMENU(IDF_MENU1,CMENU,SIZE(SPF(ISPF)%PROF),1)
 ...
 SELECT CASE (ISEL)
  ...
  CASE (3) !## merge with level j
   CALL SOLIDPROFILEMERGELINE(SP(:,2),SP(:,1),J)
 END SELECT
...
ILLOOP: DO IL=MAX(1,IELEV-1),MIN(NTBSOL,IELEV+1)
 ...
 !###======================================================================
 SUBROUTINE SOLIDPROFILEMERGELINE(VCRD,VIELEV,IELEV)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN),DIMENSION(:)  :: VCRD,VIELEV
 INTEGER,INTENT(IN) :: IELEV
 INTEGER :: I,J,ICRD,JCRD,JELEV
 REAL :: X1,Z1,X2,Z2
