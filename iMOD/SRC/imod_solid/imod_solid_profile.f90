...
CONTAINS
 ...
 !## 1 meter
 DTOL=1.0D0; XEXCLUDE=-999.99D0; IEXIST=1
 DO I=2,MIN(N,MXNIDF+1); XEXCLUDE(I)=PROFIDF(I-1)%IDF%NODATA; ENDDO
 ...
 DO I=1,SIZE(SPF(ISPF)%PROF) 
  ...
  DO J=1,SPF(ISPF)%PROF(I)%NPOS
   IF(IFIXX.EQ.0)THEN
    ...
    XMAX=MAX(XMAX,PX(J))
   ENDIF
   IF(IFIXY.EQ.0)THEN !.AND.PZ(J).NE.SPF(ISPF)%NODATA)THEN
    YMIN=MIN(YMIN,PZ(J))
    YMAX=MAX(YMAX,PZ(J))
   ENDIF
  ENDDO
...
ILLOOP: DO IL=MAX(1,IELEV-1),MIN(NTBSOL,IELEV+1)
 ...
 ISPF=0
 IF(NSPF.GT.0)THEN
  ALLOCATE(TMPNAME(NSPF)); TMPNAME=SPF%FNAME
  ISPF=1; CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,NSPF,ISPF) !SPF%FNAME
  DEALLOCATE(TMPNAME)
