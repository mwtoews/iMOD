...
CONTAINS
 ...
 !###======================================================================
 INTEGER FUNCTION IFFGETUNIT(IFFNAME,TSTAT)
 !###======================================================================
 IMPLICIT NONE
 ...
 IF(LOPEN)THEN
  INQUIRE(FILE=IFFNAME,NUMBER=IFFGETUNIT)
  CLOSE(IFFGETUNIT)
 ENDIF
 IF(TSTATUS(1:3).EQ.'OLD')THEN
  INQUIRE(FILE=IFFNAME,EXIST=LEX)
  IF(.NOT.LEX)THEN
   IF(TSTATUS(4:4).NE.'1')CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot find'//CHAR(13)//TRIM(IFFNAME),'Warning')
   RETURN
  ENDIF
  INQUIRE(FILE=IFFNAME,DIRECT=YESNO)
  IF(UTL_CAP(YESNO,'U').EQ.'YES')CALL IFFOLD2IFFNEW(IFFNAME)
  TSTATUS=TSTAT(1:3)
  IFFGETUNIT=UTL_GETUNIT()
  CALL OSD_OPEN(IFFGETUNIT,FILE=IFFNAME,STATUS=TSTATUS,FORM='FORMATTED',ACTION='READ,DENYWRITE',IOSTAT=IOS)
  IF(IOS.NE.0)THEN
   IFFGETUNIT=0
   CALL OSD_IOSTAT_MSG(IOS,MESSAGE)
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot open'//CHAR(13)//TRIM(IFFNAME)//CHAR(13)// &
   TRIM(MESSAGE),'Warning')
  ENDIF
 ENDIF

 END FUNCTION IFFGETUNIT
 ...
 REAL(KIND=DP_KIND),DIMENSION(2,2) :: XTMP
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.3)THEN
   ...
   IF(IU.GT.0)THEN
    ...
    CALL IFFPLOT(IU,MPW%XMIN,MPW%XMAX,MPW%YMIN,MPW%YMAX,IPLOT,XTMP,0.0D0)
 ...
 !###===============================================================================
 SUBROUTINE IFFPLOT(IU,XMIN,XMAX,YMIN,YMAX,IPLOT,XY,IFF_OFFSETX)
 !###===============================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IU,IPLOT
 REAL(KIND=DP_KIND),DIMENSION(2,2),INTENT(IN) :: XY
 REAL(KIND=DP_KIND),INTENT(IN) :: XMIN,XMAX,YMIN,YMAX,IFF_OFFSETX 
 INTEGER :: IOS,ICLR,I,J,II,NC,N
 REAL(KIND=DP_KIND) :: X1,X2,XVAL,DX,DY,YDIS,TDIS,RAD,XS1,XS2,YS1,YS2,XMN,YMN,XMX,YMX
 ...
 DO
  ...
  IF(IFF(1)%IPART.EQ.IFF(2)%IPART)THEN
   ...
   IF(IFF(1)%X.LT.XMAX.AND.IFF(1)%X.GE.XMIN.AND. &
    ...
    !## normal 2d plot
    IF(SUM(XY).EQ.0.0D0)THEN

     !## length line is zero
     IF((IFF(1)%X-IFF(2)%X).NE.0.0D0.OR.(IFF(1)%Y-IFF(2)%Y).NE.0.0D0)THEN
      !## correct for sight-depth
      CALL IGRCOLOURN(ICLR)   
      CALL DBL_IGRJOIN(IFF(1)%X,IFF(1)%Y,IFF(2)%X,IFF(2)%Y,IOFFSET=1)
     ENDIF

    !## perform coordinate transformation for serie-plotting!
    ELSE
     
     IF(DBL_IGRINSIDEPOLYGON(IFF(1)%X,IFF(1)%Y,XYPOL(:,1),XYPOL(:,2),4).EQ.1.OR. &
        DBL_IGRINSIDEPOLYGON(IFF(2)%X,IFF(2)%Y,XYPOL(:,1),XYPOL(:,2),4).EQ.1)THEN
      ...
      IF(MP(IPLOT)%FADEOUT.EQ.1)THEN
       ...
       YDIS=DBL_IGRDISTANCELINE(XY(1,1),XY(1,2),XY(2,1),XY(2,2),XS1,YS1,1) 
       CALL UTL_FADEOUTCOLOUR(ICLR,TDIS/YDIS/2.0) 
      ENDIF
      CALL IGRCOLOURN(ICLR)

      !## draw line X-Z only when length line is zero!
      IF((X1+IFF_OFFSETX-X2+IFF_OFFSETX).NE.0.0D0.OR.(IFF(1)%Z-IFF(2)%Z).NE.0.0D0)THEN
       CALL DBL_IGRJOIN(X2+IFF_OFFSETX,IFF(2)%Z,X1+IFF_OFFSETX,IFF(1)%Z)
       IF(MP(IPLOT)%IARROW.EQ.1)CALL DBL_IGRARROWJOIN(X2+IFF_OFFSETX,IFF(2)%Z,X1+IFF_OFFSETX,IFF(1)%Z,1)
 ...
 CHARACTER(LEN=*),INTENT(IN) :: IFFNAME
 REAL(KIND=DP_KIND),DIMENSION(3) :: XYZT
 REAL(KIND=DP_KIND) :: CT,DT,T
 ...
 TYPE IFFOBJ
  REAL(KIND=DP_KIND) :: X1,X2,Y1,Y2,Z1,Z2,T1,T2,V
  INTEGER :: IPART
  INTEGER(KIND=2) :: IL
 END TYPE IFFOBJ
