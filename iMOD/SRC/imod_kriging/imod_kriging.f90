...
CONTAINS
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z,MP,KEST,KVAR,R
  ...
  DO ICOL=1,IDF%NCOL
   ...
   IF(ASSOCIATED(ELLIPS_IDF(1,1)%X))THEN
    ...
    AGL=0.0D0; DO I=1,SIZE(ELLIPS_IDF,1); DO J=1,SIZE(ELLIPS_IDF,2)
     CALL IDFIROWICOL(IDF,JROW,JCOL,X,Y)
...
! KRIGING_GETGAMMA=(C1-C0)*H+C0
 KRIGING_GETGAMMA=C0+C1*H
 
 END FUNCTION KRIGING_GETGAMMA

 !###======================================================================
 SUBROUTINE KRIGING_INIT(MD,XD,YD,ZD,PD,WD,ND,IDF,COINCIDENT,COINCIDENTDIST,IBLANKOUT,BO_VALUE,IBLNTYPE,ROT,RAN)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
