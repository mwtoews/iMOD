...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE KRIGING_MAIN(MD,XD,YD,ZD,IDF,IDFV,IBATCH,BO_VALUE,ELLIPS)     
! SUBROUTINE KRIGING_MAIN(MD,XD,YD,ZD,IDF,IDFV,MINP,RANGE,SILL,NUGGET,KTYPE, &
!               PNTSEARCH,COINCIDENT,COINCIDENTDIST, &
...
! USE OMP_LIB
 ...
 INTEGER,INTENT(IN) :: MD !,KTYPE,COINCIDENT,IQUADRANT
! INTEGER,INTENT(INOUT) :: PNTSEARCH
! INTEGER,INTENT(INOUT) :: MINP
 REAL(KIND=DP_KIND),INTENT(IN) :: BO_VALUE  !COINCIDENTDIST
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(MD) :: XD,YD,ZD
! REAL(KIND=DP_KIND),INTENT(INOUT) :: RANGE,SILL,NUGGET
...
! ENDIF
 ...
 CALL KRIGING_INIT(MD,XD,YD,ZD,ND,IDF,COINCIDENT,COINCIDENTDIST,IBLANKOUT,BO_VALUE,IBLNTYPE) 
...
!if(idfwrite(idf,'d:\tmp.idf',1))then; endif
 ...
 DO IROW=1,IDF%NROW
  DO ICOL=1,IDF%NCOL
   ...
   ELSE
    R=RANGE
   ENDIF
   CALL KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MINP,X,Y,R,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,PNTSEARCH, &
  ...
  ELSEIF(IFORMAT.EQ.1)THEN
   ...
   DO
    ...
    IF(N.GT.MAXN)THEN
     ...
     DEALLOCATE(IXY); IXY=>IXYDUMMY
     ALLOCATE(FFCTDUMMY(MAXN+10)); DO I=1,MAXN; FFCTDUMMY(I)=FFCT(I); ENDDO
     DEALLOCATE(FFCT); FFCT=>FFCTDUMMY; MAXN=MAXN+10
    ENDIF
 ...
 SUBROUTINE KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET, &
          KTYPE,MZ,KEST,KVAR,PNTSEARCH,IQUADRANT,IDF,IBLANKOUT,BO_VALUE,RAT,ANI,IBLNTYPE)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 INTEGER,INTENT(IN) :: ND,MINP,KTYPE,PNTSEARCH,IQUADRANT,IBLANKOUT,IBLNTYPE
 REAL(KIND=DP_KIND),INTENT(OUT) :: KEST,KVAR
 REAL(KIND=DP_KIND),INTENT(IN) :: X,Y,SILL,NUGGET,MZ,RANGE,BO_VALUE,RAT,ANI
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(ND) :: XD,YD,ZD
 ...
 IF(PNTSEARCH.EQ.1)THEN
  ...
  DO ID=1,ND
   ...
   DXY=KRIGING_DIST(XCD(ID),YCD(ID),X,Y,IDF,IBLANKOUT,BO_VALUE,USERANGE,RAT,ANI,IBLNTYPE)
...
! WRITE(IU,'(A)') 'X,Y,Z,id'; WRITE(IU,*) X,Y,0.0D0,0
! DO I=1,NP
!  ID=SELID(I)
...
! CLOSE(IU)
 ...
 !###======================================================================
 REAL(KIND=DP_KIND) FUNCTION KRIGING_DIST(X1,Y1,X0,Y0,IDF,IBLANKOUT,BO_VALUE,MAXDIST,RAT,ANI,IBLNTYPE)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: X0,Y0,BO_VALUE,MAXDIST,RAT,ANI
...
!   Y1=Y0-KRIGING_DIST*SIN(A)
 ...
 ELSE
  ...
  SELECT CASE (ABS(KTYPE))
   ...
   CASE (3) !## exponential
    H=1.0D0-EXP(-3.0*(DXY/RANGE))
   CASE (4) !## gaussian
    H=1.0D0-EXP(-3.0*(DXY**2.0D0)/(RANGE**2.0D0))
   CASE (5) !## power
    H=DXY**0.5
   CASE DEFAULT
 ...
 !###======================================================================
 SUBROUTINE KRIGING_INIT(MD,XD,YD,ZD,ND,IDF,COINCIDENT,COINCIDENTDIST,IBLANKOUT,BO_VALUE,IBLNTYPE)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 ...
 INTEGER,INTENT(OUT) :: ND
 INTEGER :: I,J,N
 REAL(KIND=DP_KIND) :: XYCRIT,XP,YP,ZP
 ...
 DO I=1,MD
  ...
  DO J=I+1,MD
   ...
   IF(KRIGING_DIST(XD(I),YD(I),XD(J),YD(J),IDF,IBLANKOUT,BO_VALUE,XYCRIT,1.0D0,0.0D0,IBLNTYPE).LE.XYCRIT)THEN
    N=N+1
 ...
 REAL(KIND=DP_KIND),DIMENSION(MD) :: XD,YD,ZD,XCD,YCD
 DATA XD/100.0D0, 50.0D0,250.0D0/
 DATA YD/100.0D0,250.0D0,200.0D0/
 DATA ZD/5.0,20.0D0,32.0/
...
! MINP=MD; RANGE=4141.0D0; NUGGET=0.01D0; SILL=0.78; KTYPE=2; PNTSEARCH=0; NR=0; NC=0
! LAGINTERVAL=20; LAGDISTANCE=RANGE/REAL(LAGINTERVAL); COINCIDENT=0; IQUADRANT=0

 MINP=MD; RANGE=400.0D0; NUGGET=4.0; SILL=9.0; KTYPE=-2; PNTSEARCH=0; NR=0; NC=0
...
! nugget=5.0
 ...
 CALL KRIGING_INIT(MD,XD,YD,ZD,ND,IDF,COINCIDENT,COINCIDENTDIST,0,0.0D0,0)
 ...
 X=150.0D0; Y=150.0D0
 CALL KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,PNTSEARCH,IQUADRANT,IDF,0,0.0D0,1.0D0,0.0D0,0)
 ...
 !###====================================================================
 SUBROUTINE KRIGINGSETTINGS(MINP,KTYPE,RANGE,SILL,NUGGET,PNTSEARCH,COINCIDENT,COINCIDENTDIST,IQUADRANT,NGEN,IBREAK)
 !###====================================================================
 IMPLICIT NONE
