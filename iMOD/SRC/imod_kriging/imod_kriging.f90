...
! write(*,*) omp_get_num_threads()
 ...
 IF(KTYPE.GT.0)THEN
  MP=0.0D0; DO I=1,ND; MP=MP+PD(I); ENDDO; IF(ND.GT.0.0D0)MP=MP/REAL(ND)
...
!   if(icol.eq.20.and.irow.eq.61)then
!    write(*,*) 
!   endif
...
!     LT=.TRUE. !; LB=.TRUE. !; ISS=2
    ...
    CASE (1)
     ...
     Z1=REAL(ASSF_TOP+ASSF_ZPLUS,8)*100.0D0
     Z2=REAL(ASSF_BOT-ASSF_ZPLUS,8)*100.0D0
    !## spatial thickness
    CASE (2)
     !## depth in centimeters
    ...
    AGL=0.0D0; DO I=1,SIZE(ELLIPS_IDF,1); DO J=1,SIZE(ELLIPS_IDF,2)
     ...
     IF(JROW.GT.0.AND.JROW.LE.ELLIPS_IDF(I,J)%NROW.AND. &
      ...
      IF(ELLIPS_IDF(I,J)%X(JCOL,JROW).NE.ELLIPS_IDF(I,J)%NODATA)THEN
       IF(J.EQ.1)THEN
        AGL(I)=ELLIPS_IDF(I,J)%X(JCOL,JROW) !-90.0D0
        !## convert to radians
   ...
   ELSE
    R=RANGE; ROT=0.0D0; DO I=1,3; ROT(I,I)=1.0D0; ENDDO; RAN=0.0D0
   ENDIF
...
! c=0.0
 ...
 SUBROUTINE KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ZCD,PD,WD,ND,MAXPNT,X,Y,Z,RANGE,SILL,NUGGET, &
          KTYPE,MP,KEST,KVAR,PNTSEARCH,IQUADRANT,IDF,IBLANKOUT,BO_VALUE,ROT,RAN,IBLNTYPE,NACTP)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 ...
 !## save points
 IU=UTL_GETUNIT()
 CALL OSD_OPEN(IU,FILE='d:\IMOD-TEST\IMODBATCH_XYZ2IDF\SYNTESTS\pospunt.ipf',STATUS='UNKNOWN',ACTION='WRITE')
 ...
 ENDDO
 CLOSE(IU)
 CALL OSD_OPEN(IU,FILE='d:\IMOD-TEST\IMODBATCH_XYZ2IDF\SYNTESTS\orgpunt.ipf',STATUS='UNKNOWN',ACTION='WRITE')
...
!   Z1=Z0+XYZE(3)
 ...
 SUBROUTINE KRIGING_INIT(MD,XD,YD,ZD,PD,WD,ND,IDF,COINCIDENT,COINCIDENTDIST, &
    IBLANKOUT,BO_VALUE,IBLNTYPE,ROT,RAN)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 ...
 REAL(KIND=DP_KIND),DIMENSION(3,3) :: ROT
 REAL(KIND=DP_KIND),DIMENSION(3) :: RAN
...
! nugget=5.0
 ...
 CALL KRIGING_INIT(MD,XD,YD,ZD,PD,WD,ND,IDF,COINCIDENT,COINCIDENTDIST,0,0.0D0,0,ROT,RAN)
 ...
 CALL KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ZCD,PD,WD,ND,MAXPNT,X,Y,Z,RANGE,SILL,NUGGET,KTYPE,MP,&
    KEST,KVAR,PNTSEARCH,IQUADRANT,IDF,0,0.0D0,ROT,RAN,0,NACTP)
 ...
 DO IROW=1,IDF%NROW
  DO ICOL=1,IDF%NCOL
   ...
   CALL KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ZCD,PD,WD,ND,MAXPNT,X,Y,Z,RANGE,SILL,NUGGET,KTYPE,MP,KEST, &
     KVAR,PNTSEARCH,IQUADRANT,IDF,0,0.0D0,ROT,RAN,0,NACTP)
   IDF%X(ICOL,IROW)=KEST
