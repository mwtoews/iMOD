...
!if(idfwrite(idf,'d:\tmp.idf',1))then; endif
 ...
 DO IROW=1,IDF%NROW
  DO ICOL=1,IDF%NCOL
   ...
   CALL KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MAXPNT,X,Y,R,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,PNTSEARCH, &
  ...
  IF(IBATCH.EQ.0)THEN
   IF(KTYPE.GT.0)THEN
    CALL UTL_WAITMESSAGE(IRAT,IRAT1,IROW,IDF%NROW,'Progress Simple Kriging ('//TRIM(ITOS(NACTP))//') points')
   ELSE
 ...
 !###======================================================================
 SUBROUTINE KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MAXPNT,X,Y,RANGE,SILL,NUGGET, &
...
! nugget=5.0
 ...
 X=150.0D0; Y=150.0D0
 CALL KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MAXPNT,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,PNTSEARCH,IQUADRANT,IDF,0,0.0D0,1.0D0,0.0D0,0,NACTP)
