...
CONTAINS
 ...
 REAL(KIND=DP_KIND),DIMENSION(N,N),INTENT(OUT) :: ICOV,SQRTCOV,ISQRTCOV
 REAL(KIND=DP_KIND),DIMENSION(:,:),ALLOCATABLE :: A,IE
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: V,E,VP,VV
 INTEGER :: I,J,II
 REAL(KIND=DP_KIND) :: ET,TV
 
 
 ALLOCATE(A(N,N),IE(N,N),E(N),V(N),VP(N),VV(N))
 !## copy covariance matrix
 A=COV
 
! !## compute inverse
! CALL LUDCMP_CALC(N,N,A,AI=IE) !,B)
!PAUSE
 ...
 DO II=1,3
  ...
  DO I=1,N
   IF(V(I).GT.0.0D0)THEN
    IF(II.EQ.1)VV(I)=1.0D0/V(I)        !## inverse()
    IF(II.EQ.2)VV(I)=SQRT(V(I))        !## squareroot()
    IF(II.EQ.3)VV(I)=1.0D0/SQRT(V(I))  !## inverse-squareroot()
   ELSE
    VV(I)=0.0D0
   ENDIF
  ENDDO
  ...
  IF(II.EQ.1)CALL UTL_MATMUL(IE,A, ICOV   )
  IF(II.EQ.2)CALL UTL_MATMUL(IE,A, SQRTCOV)
  IF(II.EQ.3)CALL UTL_MATMUL(IE,A,ISQRTCOV)
 ENDDO 
 
 DEALLOCATE(A,E,IE,V,VV)
...
1  DO M=L,N-1
  ...
  M=N
2  IF(M.NE.L)THEN
   IF(ITER.EQ.1000)WRITE(*,'(A)') 'VERY MANY ITERATIONS IN TQLI - HOWEVER IT IS STILL CONTINUING ...'
...
!end]
 ...
 !###====================================================================
 SUBROUTINE LUDCMP_CALC(N,M,A,AI,B)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N,M
 REAL(KIND=DP_KIND),DIMENSION(M,M),INTENT(IN) :: A
 REAL(KIND=DP_KIND),DIMENSION(N,N),INTENT(OUT),OPTIONAL :: AI
...
!
!end SUBROUTINE UTL_MAIN_INVERSE
!
!  subroutine UTL_inverse(a,c,n)
!!============================================================
!! Inverse matrix
...
!end subroutine UTL_inverse
 ...
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(TIMEDIM,SPACEDIM) :: V
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(MAXDIM) :: W
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(SPACEDIM,SPACEDIM) :: U
 INTEGER :: MINDIM,I,J,PASS
 REAL(KIND=DP_KIND) :: TEMP1
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: TEMP2 
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: TEMP3
...
!###  Null space excluded
 ...
 ALLOCATE(TEMP2(TIMEDIM),TEMP3(SPACEDIM)); TEMP2=0.0D0; TEMP3=0.0D0
...
!  return
!end
REAL(KIND=DP_KIND) function r8poly_value ( m, c, x )
