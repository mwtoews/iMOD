...
USE MOD_IDFPLOT
USE MOD_DBL
USE MOD_UTL, ONLY : UTL_IMODFILLMENU,LISTNAME,UTL_DIRINFO,ITOS,UTL_IDFGETDATE,UTL_GETUNIQUE_CHAR,UTL_IDFGETLAYERS,UTL_GETHELP
...
CONTAINS
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    CASE (IDOK)
     ...
     DO I=1,SIZE(ILAY)
      IF(ILAY(I).EQ.1)THEN
       CALL MANAGER_UTL_ADDFILE(TRIM(ROOT)   //'\'// &
                    TRIM(VARIANT)//'\'// &
                    TRIM(CTYPE)  //'\'// &
                    TRIM(CTYPE)//'_'//TRIM(CPERIOD)//'_L'//TRIM(CLAY(I))//'.IDF')
       IF(I.EQ.1)CALL IDFPLOTFAST(1)
     ENDIF
     END DO
     CALL WDIALOGSELECT(ID_DQUICKOPEN)
     ...
     ENDIF
    CASE (IDHELP)
      CALL UTL_GETHELP('3.4.2','MMO.QuickOpen')
    CASE (IDCANCEL)
     CALL IDFQUICKOPEN_CLOSE()
   END SELECT
  CASE (FIELDCHANGED)
 ...
 IF(ALLOCATED(ILAY))DEALLOCATE(ILAY); ALLOCATE(ILAY(LDIM))
