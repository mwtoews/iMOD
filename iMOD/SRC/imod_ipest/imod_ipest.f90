...
!USE MOD_ISIMGRO
USE MOD_IPEST_PAR
!USE MODPLOT_UTL, ONLY : IMOD_UTL_GETMED
!USE MOD_UTL, ONLY : PRINTTEXT,UTL_CAPF,ITOS,GETREAL(KIND=DP_KIND),UTL_FILENAME,RTOS,UTL_STRING,OPENASC,UTL_SWAPSLASH,GETUNIT,CREATEDIR,GETRCL, &
...
REAL(KIND=DP_KIND),PRIVATE :: DAMPINGFACTOR=1.5
...
!   IF(IOS.NE.0)THEN
!    PEST_PADJ=0.0D0 !## optional parameter adjustment
...
!  IF(PEST_ITARGET(I).LT.0.0D0)CALL PRINTTEXT('Error PEST_ITARGET('//TRIM(ITOS(I))//') < 0.0D0',2)
! ENDDO
! 
! TF=SUM(PEST_ITARGET)
! 
...
!   IF(PEST_IBATCH(I)%FRACTION.LT.0.0D0)CALL PRINTTEXT('Error PEST_IBATCH('//TRIM(ITOS(I))//')%FRACTION < 0.0D0',2)
...
!   CALL PRINTTEXT('Maximal Adjustment Factor should be at least greater than 1.0D0',0)
...
! REAL(KIND=DP_KIND),DIMENSION(NODES),INTENT(OUT) :: HCOF
! INTEGER :: ICOL,IROW,ILAY,I,J,K,IP,INODE,IPP,NXYZ,IP1,IP2,IS
! REAL(KIND=DP_KIND) :: FCT,PPART,NODATA,RANGE
! REAL(KIND=DP_KIND),DIMENSION(:,:),ALLOCATABLE :: XYZ,XPP
...
!     !## areas outside range do get a value of 0.0D0, convert to 1.0D0
!     IF(XPP(ICOL,IROW).EQ.0.0D0)XPP(ICOL,IROW)=1.0D0
!    ENDIF
!   ENDDO; ENDDO
!   
...
! 
! !###====================================================================
! REAL(KIND=DP_KIND) FUNCTION PEST_GETRANGE()
! !###====================================================================
! IMPLICIT NONE
!    
...
! REAL(KIND=DP_KIND) :: FCT,THETA,PRZ,COND,PWT
! TYPE SCLOBJ1
!  INTEGER :: NUND,IROW,ICOL
! END TYPE SCLOBJ1
! TYPE SCLOBJ2
!  REAL(KIND=DP_KIND) :: THETA,COND,PRZ,PWT
! END TYPE SCLOBJ2
...
! ALLOCATE(SCL2(NCOL,NROW)); SCL2%THETA=0.0D0; SCL2%COND=0.0D0; SCL2%PRZ=0.0D0
!
! CALL OPENASC(IUSCL,'uscl_svat.inp','R')
...
!  SCL1(I)%ICOL =ICOL
!  !## initialize again
!  SCL2(ICOL,IROW)%THETA=THETA !1.0D0 !THETA
!  SCL2(ICOL,IROW)%COND =COND  !1.0D0 !COND
!  SCL2(ICOL,IROW)%PRZ  =PRZ   !1.0D0 !PRZ
!  SCL2(ICOL,IROW)%PWT  =PWT   !1.0D0 !PWT
! ENDDO; CLOSE(IUSCL)
! 
...
! DO J=1,I
!  METEO(J)%N=RRECHSTRUCNIV*1000.0D0
!  IF(METEO(J)%D.EQ.1.0D0)METEO(J)%N=METEO(J)%N+(RCH_IMPULSE_SIZE*1000.0D0)  !## impulse in meters
...
! !#####=================================================================
! IMPLICIT NONE
! REAL(KIND=DP_KIND) :: IMPROVEMENT,F
! INTEGER :: I
! 
...
!   IF(TJ.LE.0.0D0)THEN
!    PESTNEXT=.TRUE.
!    CALL PRINTTEXT('',-1); CALL PRINTTEXT('Objective Function <= 0.0D0 ('//TRIM(RTOS(REAL(TJ),'G',7))//')',-1)
!   ENDIF    
!    
...
!    CALL PRINTTEXT('',-1); CALL PRINTTEXT('Pest iteration terminated decrease objective function ('//TRIM(RTOS(100.0D0*IMPROVEMENT,'G',7))// &
!     '%) > PEST_JSTOP ('//TRIM(RTOS(100.0D0*PEST_JSTOP,'G',7))//'%)',-1)
!   ENDIF
!
!   TJOBJ=TJ 
...
!   Z1=TINY(1.0D0)
!   IF(PARAM(I)%ALPHA(2)-ZW.LT.LOG(HUGE(1.0D0)))THEN
...
!!   IF(Z1.NE.Z1)Z1=TINY(1.0D0)
!!   IF(Z2.NE.Z2)Z2=HUGE(1.0D0)
!  ELSE
!   Z= PARAM(I)%ALPHA(2)
...
! REAL(KIND=DP_KIND) :: TS,DF1,EIGWTHRESHOLD
...
! REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: S,N,RU 
! REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: EIGW
...
! NP=SIZE(PARAM)
! !## sensitivity
! IF(.NOT.ALLOCATED(S)) ALLOCATE(S(NP)); S =0.0D0
!
! DO IP1=1,NP
...
!    IF(PARAM(IP1)%IBND.EQ.-1.AND.-1.0D0*JQR(I).LT.0.0D0)PARAM(IP1)%IACT=-1 !## still outside parameter domain(min)
!    IF(PARAM(IP1)%IBND.EQ. 1.AND.-1.0D0*JQR(I).GT.0.0D0)PARAM(IP1)%IACT=-1 !## still outside parameter domain(max)
!   ENDIF
!  ENDDO
!    
...
!   C=0.0D0; DO I=1,NP; C(I,I)=1.0D0/SQRT(JQJ(I,I)); ENDDO
!   
...
!  IF(LSVD)THEN
!  
!   EIGWTHRESHOLD=0.0D0 !% explained variance
...
!   PT=0.0D0; DO I=1,NE; DO J=1,NP
!    DO K=1,NP
!     PT(I,J)=PT(I,J)+P(K,I)*JQJ(K,J)
!    ENDDO
!   ENDDO; ENDDO
!   !## project jqj on eigen-space
!   M=0.0D0; DO I=1,NE; DO J=1,NE
!    DO K=1,NP
...
!  IF(PESTUPGRADEVECTOR(1.0D0,.TRUE.))EXIT 
...
! REAL(KIND=DP_KIND),DIMENSION(NP,NP),INTENT(INOUT) :: JQJ,EIGV,COV
! REAL(KIND=DP_KIND),DIMENSION(NP),INTENT(INOUT) :: EIGW
! INTEGER :: I,J,IP1,IP2,II,N
! REAL(KIND=DP_KIND) :: DF1,DF2,DJ1,DJ2,B1,TV,TEV,CB,DET,KAPPA
...
!  DO I=1,NP; IF(EIGW(I).LE.0.0D0)EIGW(I)=0.0D0; ENDDO; TEV=SUM(EIGW)
!  TV=0.0D0
!  DO I=1,NP
!   TV=TV+(EIGW(I)*100.0D0/TEV)
!   IF(EIGW(I).GT.0.0D0)THEN
!    WRITE(IUPESTOUT,'(I10,4F15.7)') I,EIGW(I),SQRT(EIGW(I)),EIGW(I)*100.0D0/TEV,TV
!   ELSE
!    WRITE(IUPESTOUT,'(I10,4F15.7)') I,EIGW(I),     EIGW(I) ,EIGW(I)*100.0D0/TEV,TV
!   ENDIF
!  ENDDO
!  EIGV= B  
...
!!   ENDDO
!!  ENDDO
!!  R1=0.0D0; DO I=1,NP; R1=R1+COR(I,1)**2.0; ENDDO
!!  R2=0.0D0; DO I=1,PEST_NOBS; R2=R2+DH(0,I)**2.0; ENDDO
...
! REAL(KIND=DP_KIND) :: F,G,MINP,MAXP,MINAP,MAXAP
...
!   ENDIF 
!  
!   IF(F.LT.1.0D0/PARAM(IP1)%FADJ.OR. &
...
!
!   IF(PARAM(IP1)%LOG)THEN
!    MINAP=1.0D0/PARAM(IP1)%FADJ*EXP(PARAM(IP1)%ALPHA(2))
!    MAXAP=PARAM(IP1)%FADJ    *EXP(PARAM(IP1)%ALPHA(2))
!    MINAP=LOG(MINAP)
!    MAXAP=LOG(MAXAP)    
!   ELSE
!    MINAP=1.0D0/PARAM(IP1)%FADJ*PARAM(IP1)%ALPHA(2)
...
!     IOS,NAJ,NP
! REAL(KIND=DP_KIND) :: X,Y,Z,H,WW,MC,MM,DHH,XCOR,YCOR,ZCOR,XCROSS,RFIT !,FR1,FR2
! CHARACTER(LEN=52) :: ID
! REAL(KIND=DP_KIND) :: DHW
! REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: TSNODATA,M,C,GF_H,GF_O
...
! IF(PEST_IGRAD.EQ.0)DH(PEST_IGRAD,:)=0.0D0
...
!    ALLOCATE(M(NROWIPFTXT),C(NROWIPFTXT),IDATE(NROWIPFTXT)); IDATE=0; C=0.0D0; M=0.0D0
...
!     !## percentiles
!     CALL IMOD_UTL_GETMED(M,KK,-999.99,(/10.0D0,90.0D0/),2,NAJ,PM)
!     CALL IMOD_UTL_GETMED(C,KK,-999.99,(/10.0D0,90.0D0/),2,NAJ,PC)
...
! 
!! !###====================================================================
!! REAL(KIND=DP_KIND) FUNCTION PEST_GOODNESS_OF_FIT(GF_H,GF_O,PEST_NOBS)
!! !###====================================================================
!! IMPLICIT NONE
!! INTEGER,INTENT(IN) :: PEST_NOBS
!! REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(:) :: GF_H,GF_O
!! REAL(KIND=DP_KIND) :: MU_H,MU_O,X1,X2
!! INTEGER :: I
!! 
...
!
! !###====================================================================
! REAL(KIND=DP_KIND) FUNCTION PEST_GOODNESS_OF_FIT(X,Y,N)
! !###====================================================================
! IMPLICIT NONE
! INTEGER,INTENT(IN) :: N
...
! ENDDO
!
! IF(X2.NE.0.0D0.AND.X3.NE.0.0D0)PEST_GOODNESS_OF_FIT=X1/(SQRT(X2)*SQRT(X3))
!  
! END FUNCTION PEST_GOODNESS_OF_FIT
! 
! !###====================================================================
...
! INTEGER :: I,J,II,JJ
! REAL(KIND=DP_KIND) :: ALPHA,GAMMA,X1,X2,Y1,Y2,RANGE
! REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: AQPP
! 
! RANGE=PEST_GETRANGE()
! 
! IF(ALLOCATED(QPP))DEALLOCATE(QPP)
! ALLOCATE(QPP(NP,NP)); QPP =0.0D0
! !## fill array zone and set appropriate pointers in type 
...
!    GAMMA=SILL-GAMMA
!    QPP(II,JJ)=1.0D0/(2.0*SQRT(GAMMA))
!   ENDDO
!  ENDIF
! ENDDO
!
...
!  IF(PARAM(IP)%DELTA.EQ.1.0D0)CALL PRINTTEXT('You can not specify delta alpha eq 1.0D0 for log-transformed parameters',2)
!  IF(PARAM(IP)%MIN  .EQ.0.0D0)CALL PRINTTEXT('You can not specify minimal value eq 0.0D0 for log-transformed parameters',2)
...
!
! !###========================================================================
! REAL(KIND=DP_KIND) FUNCTION PEST_FIND_DET(JQJ,N)
! !###========================================================================
! IMPLICIT NONE
! INTEGER,INTENT(IN) :: N
! REAL(KIND=DP_KIND),DIMENSION(N,N) :: JQJ
! REAL(KIND=DP_KIND),DIMENSION(:,:),ALLOCATABLE :: MATRIX
! REAL(KIND=DP_KIND) :: M, TEMP
! INTEGER :: I, J, K, L
...
! REAL(KIND=DP_KIND),DIMENSION(NP),INTENT(INOUT) :: D
! REAL(KIND=DP_KIND),DIMENSION(NP,NP),INTENT(INOUT) :: A
! INTEGER :: I,J,K
! REAL(KIND=DP_KIND) :: P
!
! DO I=1,N-1
...
! INTEGER,INTENT(IN) :: N,NP
! REAL(KIND=DP_KIND),DIMENSION(NP,NP),INTENT(INOUT) :: A
! REAL(KIND=DP_KIND),DIMENSION(NP),INTENT(INOUT) :: D,E
! INTEGER :: I,J,K,L
! REAL(KIND=DP_KIND) :: F,G,H,HH,SCALE
!
! DO I=N,2,-1
!  L=I-1
!  H=0.
...
!
! !###========================================================================
! REAL(KIND=DP_KIND) FUNCTION PYTHAG(A,B)
! !###========================================================================
! IMPLICIT NONE
! REAL(KIND=DP_KIND),INTENT(IN) :: A,B
! REAL(KIND=DP_KIND) :: ABSA,ABSB
!
! ABSA=ABS(A)
! ABSB=ABS(B)
...
!
! !###========================================================================
! REAL(KIND=DP_KIND) FUNCTION PYTHAG_DBL(A,B)
! !###========================================================================
! IMPLICIT NONE
! REAL(KIND=DP_KIND),INTENT(IN) :: A,B
! REAL(KIND=DP_KIND) :: ABSA,ABSB
...
! REAL(KIND=DP_KIND) :: B,C,DD,F,G,P,R,S
!
! DO I=2,N
!  E(I-1)=E(I)
! ENDDO
