...
CONTAINS
 ...
 TYPE(IDFOBJ) :: IDF
 REAL(KIND=DP_KIND) :: XSUM,RN,WW,X,VAR,A,L,TL,GAMMA
...
! CALL UTL_MATMUL(TMP1,TMP2,TMP3)
 ...
 IF(ITER.GT.1)THEN
  ...
  ELSE
   LAMBDA=LAMBDA*GAMMA
   !## reset ensembles...or continue from here
  ENDIF
 ENDIF
 ...
 TL=0.0D0; DO I=1,SIZE(PEST%PARAM)
  ...
  IF(ITER.EQ.1)THEN; ALLOCATE(MPR(TIMEDIM,SPACEDIM)); MPR=REALS; ENDIF
  ...
  !## processing the inverse of the prior parameter covariance matrix
  CALL IPEST_IES_COMPUTE_PARTIAL_M(ITER,I,TIMEDIM,SPACEDIM,OBSDIM,DM,A,AM,MPR,REALS,L) 
  TL=TL+L
  ...
  DO J=1,SIZE(PEST%PARAM)
   DO K=1,TIMEDIM
    II=0; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL; II=II+1
     PRJIDF%X(ICOL,IROW)=REALS(K,II)
    ENDDO; ENDDO
    FNAME=PEST%PARAM(J)%REALSFNAME(K); FNAME=FNAME(:INDEX(FNAME,'.',.TRUE.)-1)//'_ITER'//TRIM(ITOS(ITER))//'.IDF'
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_PARTIAL_M(ITER,IPARAM,TIMEDIM,SPACEDIM,OBSDIM,DM,A,AM,MPR,REALS,L)
 !#####=================================================================
 IMPLICIT NONE
...
!  PM2=MATMUL(X1,X7) 
 ...
 ALLOCATE(X1(TIMEDIM,SPACEDIM))
 X1=REALS+PM1; IF(ITER.GT.1)X1=X1+PM2
...
!  AM=0.0D0
  DO I=1,SPACEDIM; DO J=1,SPACEDIM
...
! IF(ALLOCATED(S))DEALLOCATE(S)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_GETREALS(IPARAM,TIMEDIM,SPACEDIM,REALS) 
 !#####=================================================================
 IMPLICIT NONE
 ...
 REALS=0.0D0
 DO J=1,TIMEDIM
  IF(.NOT.IDFREADSCALE(PEST%PARAM(IPARAM)%REALSFNAME(J),PRJIDF,10,0,1.0D0,0))RETURN
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_DM(IPARAM,TIMEDIM,SPACEDIM,REALS,DM) 
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPARAM,TIMEDIM,SPACEDIM
 REAL(KIND=DP_KIND),DIMENSION(TIMEDIM,SPACEDIM) :: DM,REALS
 INTEGER :: N,I,J,K,IROW,ICOL
 REAL(KIND=DP_KIND) :: RN,XSUM
 
 !## read realisatons
 CALL IPEST_IES_COMPUTE_GETREALS(IPARAM,TIMEDIM,SPACEDIM,REALS) 
...
! DM=MATMUL(PEST%PARAM(IPARAM)%ISQRTCOV,REALS)
 ...
 !## check
 IF(NCA.NE.NRB)THEN; WRITE(*,*) 'COLUMN DIMENSIONS ARRAY A NEED TO BE EQUAL TO ROWS   ARRAY B'; STOP; ENDIF
 IF(NRA.NE.NRC)THEN; WRITE(*,*) 'ROWS   DIMENSIONS ARRAY A NEED TO BE EQUAL TO ROWS   ARRAY C'; STOP; ENDIF
 IF(NCB.NE.NCC)THEN; WRITE(*,*) 'COLUMN DIMENSIONS ARRAY B NEED TO BE EQUAL TO COLUMN ARRAY C'; STOP; ENDIF
