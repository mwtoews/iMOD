...
CONTAINS
     ...
     N=0; DO JGRAD=1,SIZE(RNG)
      ...
      IF(ISTATUS(JGRAD).EQ.0)THEN
       ...
       IF(ISTATUS(JGRAD).EQ.0)THEN
        ...
        IF(.NOT.IPEST_GLM_GETJ(DIR,JGRAD,GPARAM(JGRAD),'R',IBATCH))RETURN 
 ...
 ENDDO
 WRITE(IUPESTOUT,'(A10,999(1X,F15.7))') 'TOTAL',(SUM(JE(IGRAD,0:PEST%PE_MXITER)),IGRAD=1,PEST%NREALS)
...
!  PEST%PARAM(I)%ICREATEENSEMBLES
 ...
 INTEGER :: I,II,J,JJ,K,N,SPACEDIM,TIMEDIM,OBSDIM,IROW,ICOL,IX,ILOG
 ...
 DO J=1,TIMEDIM
  ...
  CALL IPEST_IES_GETPDF(TMP1,PDF(:,J),RCLASS) !,'N')
 ENDDO
 ...
 IF(ITER.GT.0)THEN
  ...
  WRITE(IUPESTOUT,'(I10,5(1X,F15.3))') ITER,MSR%TJ,MSR%PJ,MSR%J,100.0D0*(1.0D0-MSR%TJ/MSR%PJ),LAMBDA
  WRITE(IUPESTPROGRESS,'(I10,99(1X,F15.3))') ITER,MSR%TJ,MSR%PJ,MSR%J,100.0D0*(1.0D0-MSR%TJ/MSR%PJ),LAMBDA, &
     (PEST%PARAM(J)%MEAN(TIMEDIM+1),PEST%PARAM(J)%STD(TIMEDIM+1),J=1,SIZE(PEST%PARAM))
  WRITE(IUPESTOUT,'(/2A10,4A15)') 'ITER','ENSEMBLE','MEAN','STDEV','EXP_MEAN','EXP_STDEV'
  DO J=1,SIZE(PEST%PARAM)
   ...
   ENDDO
   I=TIMEDIM+1
   WRITE(IUPESTOUT,'(/2I10,4F15.7/)') I,0,PEST%PARAM(J)%MEAN(I),PEST%PARAM(J)%STD(I), &
     10.0D0**PEST%PARAM(J)%MEAN(I),10.0D0**PEST%PARAM(J)%STD(I)
  ENDDO
 ...
 DO
  ...
  DO I=1,SIZE(PEST%PARAM)
   ...
   CALL IPEST_IES_COMPUTE_DM(DIR,I,TIMEDIM,SPACEDIM,REALS,DM,ITER,ILOG) 
...
!   CALL IPEST_IES_COMPUTE_GETREALS(DIR,I,TIMEDIM,SPACEDIM,REALS,ITER)
   ...
   DO K=1,TIMEDIM
    II=0; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL; II=II+1
     TMP1(II)=REALS(K,II)
     IF(ILOG.EQ.1)THEN
      PRJIDF%X(ICOL,IROW)=10.0D0**TMP1(II) 
     ELSE
    ...
    CALL IPEST_IES_GETPDF(TMP1,PDF(:,K),PCLASS) !,'N')
 ...
 INQUIRE(FILE=TRIM(DIR)//'\ENSEMBLES\ISQRTCOV.IDF',EXIST=L1)
 INQUIRE(FILE=TRIM(DIR)//'\ENSEMBLES\ICOV.IDF',EXIST=L2)
 IF(L1.AND.L2)THEN
  WRITE(*,'(/A$)') 'Found existing ISQRTCOV.IDF and ICOV.IDF, do you want to use them ?'
  READ(*,'(A1)') YN
  IF(YN.EQ.'Y'.OR.YN.EQ.'y')THEN
   IF(.NOT.IDFREAD(IDF,TRIM(DIR)//'\ENSEMBLES\ISQRTCOV.IDF',1))STOP
   PEST%PARAM(IPARAM)%INVCOV=IDF%X
   IF(.NOT.IDFREAD(IDF,TRIM(DIR)//'\ENSEMBLES\ICOV.IDF',1))STOP
 ...
 IF(.NOT.L1.OR..NOT.L2)THEN
  ...
  IDF%FNAME=TRIM(DIR)//'\ENSEMBLES\ICOV.IDF'; IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
 ENDIF
 CALL IDFDEALLOCATEX(IDF)
...
! CLOSE(IU)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_GETREALS(DIR,IPARAM,TIMEDIM,SPACEDIM,REALS,ITER,ILOG) 
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER,INTENT(IN) :: IPARAM,TIMEDIM,SPACEDIM,ITER,ILOG
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_DM(DIR,IPARAM,TIMEDIM,SPACEDIM,REALS,DM,ITER,ILOG) 
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER,INTENT(INOUT) :: ITER
 INTEGER,INTENT(IN) :: IPARAM,TIMEDIM,SPACEDIM,ILOG
 REAL(KIND=DP_KIND),DIMENSION(TIMEDIM,SPACEDIM) :: DM,REALS
 INTEGER :: J,K
 REAL(KIND=DP_KIND) :: RN,XSUM
 
 !## read realisatons
 CALL IPEST_IES_COMPUTE_GETREALS(DIR,IPARAM,TIMEDIM,SPACEDIM,REALS,ITER,ILOG) 
