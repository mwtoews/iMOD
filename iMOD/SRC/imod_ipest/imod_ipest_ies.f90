...
!  PEST%PARAM(I)%ICREATEENSEMBLES
 ...
 REAL(KIND=DP_KIND) :: X,VAR,A,L,TL,MAXTL,GAMMA,F1,F2,F3,J0
 ...
 IF(ITER.GT.0)THEN
  ...
  WRITE(IUPESTPROGRESS,'(I10,99(1X,F15.3))') ITER,MSR%TJ,F3,MSR%J,F1,F2,LAMBDA, &
     (PEST%PARAM(J)%MEAN(TIMEDIM+1),PEST%PARAM(J)%STD(TIMEDIM+1),J=1,SIZE(PEST%PARAM))

  WRITE(IUPESTOUT,'(/60X,A30)') '--------IMPROVEMENT (%)-------'
  WRITE(IUPESTOUT,'(A10,6(1X,A15))') 'ITER','CURRENT_TJ','PREVIOUS_PJ','AVG-RESIDUAL','RELATIVE','ABSOLUTE','LAMBDA'
  WRITE(IUPESTOUT,'(I10,6(1X,F15.3))') ITER,MSR%TJ,F3,MSR%J,F1,F2,LAMBDA
...
! ENDDO
 ...
 WRITE(IUPESTPROGRESS,'(/60X,A30)') '--------IMPROVEMENT (%)-------'
 WRITE(IUPESTPROGRESS,'(A10,6(1X,A15))') 'ITER','CURRENT_TJ','PREVIOUS_PJ','AVG-RESIDUAL','RELATIVE','ABSOLUTE','LAMBDA'
