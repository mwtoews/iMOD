...
CONTAINS
 ...
 INTEGER :: ITER,IGRAD,ILOG,ILAMBDA,JLAMBDA,ILINE,KLINE,JLINE,SPACEDIM,TIMEDIM,OBSDIM
 REAL(KIND=DP_KIND) :: MINJ,L,L0,L1,M0,M1,MINL,F
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: MO
 ...
 DO WHILE (ITER.LE.PEST%PE_MXITER)
  ...
  IF(ITER.EQ.0)THEN
   ...
   JLAMBDA=1; JLINE=1; CALL IPEST_IES_SAVEREALS(DIR,ITER,JLAMBDA,JLINE,MNAME)
   ...
   !## add model-error (=0.0 here) to it
   MO=0.0D0; JE(:,ITER)=JE(:,ITER)+MO; CALL IPEST_IES_PROGRESS_OUT(ITER)
   WRITE(IUPESTEFFICIENCY,'(I5,A5,1X,F15.3,6(16X),1X,F15.3)') ITER,'-',SUM(JE(:,ITER)),LAMBDA
   !## dump dhg as backstepping might be possible
   CALL IPEST_IES_WRITE_DHG(DIR,ITER,JLAMBDA,JLINE,MO)    
  !## carry out the lambda tests
  ELSE
   !## simulate ensembles of different lambdas
   MINJ=10.0D20; JLAMBDA=0
   DO ILAMBDA=1,PBMAN%NLAMBDASEARCH
    MINL=10.0D20; KLINE=0
    DO ILINE=1,PBMAN%NLINESEARCH
     F=PBMAN%LINE_SEARCH(ILINE)
     !## read from idf file and save the realisations in lpf package - apply the correct IDF files and export them for the realizations
     CALL IPEST_IES_SAVEREALS(DIR,ITER,ILAMBDA,ILINE,MNAME)
     L=LAMBDA*PBMAN%LAMBDA_TEST(ILAMBDA)
     !## run and process all realizations-testing
     WRITE(IUPESTPROGRESS,'(/A)') 'Lambda Testing '//TRIM(ITOS(ILAMBDA))//' ('//TRIM(RTOS(L,'G',7))//'); Line-Search '//TRIM(ITOS(ILINE))//' (Factor '//TRIM(RTOS(F,'G',7))//')'
     IF(.NOT.IPEST_GLM_RUNMODELS(IBATCH,RNG,GPARAM,'R',DIR,MNAME,ITER,L,ILAMBDA))EXIT
     !## compute model error
     IF(PEST%PE_REGULARISATION.EQ.1)CALL IPEST_IES_COMPUTE_MODEL_ERROR(DIR,MO,SPACEDIM,TIMEDIM,OBSDIM,ILOG,ITER,ILAMBDA,ILINE)
     !## add model-error to it
     JE(:,ITER)=JE(:,ITER)+MO; CALL IPEST_IES_PROGRESS_OUT(ITER)
     WRITE(IUPESTEFFICIENCY,'(2I5,1X,F15.3,6(16X),2(1X,F15.3))') ITER,ILINE,SUM(JE(:,ITER)),L,F
     !## dump dhg as backstepping might be possible
     CALL IPEST_IES_WRITE_DHG(DIR,ITER,ILAMBDA,ILINE,MO)
     !## evaluate which lambda is best
     IF(SUM(JE(:,ITER)).LT.MINL)THEN
      MINL=SUM(JE(:,ITER)); KLINE=ILINE
     ENDIF
    ENDDO
    !## check whether this line-search is minimal compared to previous values, if so store it
    IF(MINL.LT.MINJ)THEN
     MINJ=MINL; JLAMBDA=ILAMBDA; JLINE=KLINE
    ENDIF
   ENDDO
   IF(JLAMBDA.EQ.0)THEN; STOP 'SOMETHING WENT WRONG: JLAMBDA=0'; ENDIF
   !## echo selected lambda
   L=LAMBDA*PBMAN%LAMBDA_TEST(JLAMBDA)
   F=PBMAN%LINE_SEARCH(JLINE)
   WRITE(IUPESTPROGRESS,'(/A/)') 'Selected lambda = '//TRIM(ITOS(JLAMBDA))//' ('//TRIM(RTOS(L,'F',3))// &
                                ') Selected linesearch = '//TRIM(ITOS(JLINE))//' ('//TRIM(RTOS(F,'F',3))// &
           ') has minimal Objective Function Value ('//TRIM(RTOS(MINJ,'F',3))//')'
   WRITE(IUPESTEFFICIENCY,'(/A/)') 'Selected lambda = '//TRIM(ITOS(JLAMBDA))//' ('//TRIM(RTOS(L,'F',3))// &
                                ') Selected linesearch = '//TRIM(ITOS(JLINE))//' ('//TRIM(RTOS(F,'F',3))// &
           ') has minimal Objective Function Value ('//TRIM(RTOS(MINJ,'F',3))//')'
   !## read residuals of all ensembles for selected jlambda and linesearch
  ...
  OBSDIM=MSR%NOBS
  IF(.NOT.IPEST_IES_UPDATE_ENSEMBLES(DIR,ITER,JLAMBDA,JLINE,IBATCH,MNAME,SPACEDIM,TIMEDIM,OBSDIM))EXIT  
  !## save mean/stdev heads
  CALL IPEST_IES_SAVE_HEAD_STATS(DIR,ITER,JLAMBDA)
  !## save mean/stdev parameters
  CALL IPEST_IES_SAVE_PARAMETERS_STATS(DIR,ITER,JLAMBDA,JLINE)
 ...
 ENDDO
 WRITE(IUPESTOUT,'(/A10,999(1X,A15))') 'ITER','TOTAL',('VARIANCE_SIM'//TRIM(ITOS(IGRAD)),IGRAD=1,PEST%NREALS)
 DO ITER=0,PEST%PE_MXITER
  WRITE(IUPESTOUT,'(I10,999(1X,F15.7))') ITER,SUM(SIGMA(1:PEST%NREALS,ITER))/REAL(PEST%NREALS),(SIGMA(IGRAD,ITER),IGRAD=1,PEST%NREALS)
 ENDDO
 WRITE(IUPESTOUT,'(/A10,999(1X,A15))') 'ITER','TOTAL',('IMPROVEM._SIM'//TRIM(ITOS(IGRAD)),IGRAD=1,PEST%NREALS)
 DO ITER=0,PEST%PE_MXITER
  L0=SUM(SIGMA(1:PEST%NREALS,0))/REAL(PEST%NREALS)
  M0=SUM(MU(1:PEST%NREALS,0))/REAL(PEST%NREALS)
  L1=SUM(SIGMA(1:PEST%NREALS,ITER))/REAL(PEST%NREALS)
  M1=SUM(MU(1:PEST%NREALS,ITER))/REAL(PEST%NREALS)
  L =100.0D0*(1.0D0-(M0+L0)/(M1+L1))
  WRITE(IUPESTOUT,'(I10,999(1X,F15.7))') ITER,L, &
      (100.0D0*(1.0D0-((MU(IGRAD,ITER)+SIGMA(IGRAD,ITER))/(MU(IGRAD,0)+SIGMA(IGRAD,0)))),IGRAD=1,PEST%NREALS)
 ENDDO
 WRITE(IUPESTOUT,'(/A10,999(1X,A15))') 'ITER','TOTAL',('AVERAGE_SIM'//TRIM(ITOS(IGRAD)),IGRAD=1,PEST%NREALS)
 DO ITER=0,PEST%PE_MXITER
  WRITE(IUPESTOUT,'(I10,999(1X,F15.7))') ITER,SUM(MU(1:PEST%NREALS,ITER))/REAL(PEST%NREALS),(MU(IGRAD,ITER),IGRAD=1,PEST%NREALS)
 ENDDO
 ...
 !#####=================================================================
 LOGICAL FUNCTION IPEST_IES_CREATEENSEMBLES(DIR,ILOG)
 !#####=================================================================
 IMPLICIT NONE
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_SAVE_HEAD_STATS(DIR,ITER,JLAMBDA)
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 ...
 END SUBROUTINE IPEST_IES_SAVE_HEAD_STATS
 
 !#####=================================================================
 SUBROUTINE IPEST_IES_SAVE_PARAMETERS_STATS(DIR,ITER,JLAMBDA,JLINE)
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER,INTENT(IN) :: ITER,JLAMBDA,JLINE
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: TMP1
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: PAR
 TYPE(IDFOBJ) :: MPR,VPR
 REAL(KIND=DP_KIND) :: AVG,STD
 INTEGER :: I,J,IROW,ICOL,N
 
 !## compute mean and variance heads
 ALLOCATE(PAR(PEST%NREALS)); DO I=1,PEST%NREALS; CALL IDFNULLIFY(PAR(I)); ENDDO; CALL IDFNULLIFY(MPR); CALL IDFNULLIFY(VPR)
 DO I=1,SIZE(PEST%PARAM)
  DO J=1,PEST%NREALS
   IF(ITER.EQ.0)THEN
    PAR(J)%FNAME=TRIM(DIR)//'\ENSEMBLES\PARAMETER'//TRIM(ITOS(I))//'_R'//TRIM(ITOS(J))//'.IDF'
   ELSE
    PAR(J)%FNAME=TRIM(DIR)//'\ENSEMBLES\PARAMETER'//TRIM(ITOS(I))//'_R'//TRIM(ITOS(J))//'_ITER'//TRIM(ITOS(ITER))// &
                 '_ILAMBDA'//TRIM(ITOS(JLAMBDA))//'_ILINE'//TRIM(ITOS(JLINE))//'.IDF'
   ENDIF
   IF(.NOT.IDFREAD(PAR(J),PAR(J)%FNAME,1))STOP
  ENDDO
  IF(I.EQ.1)THEN; CALL IDFCOPY(PAR(1),MPR); CALL IDFCOPY(PAR(1),VPR); ENDIF
  ALLOCATE(TMP1(PEST%NREALS)); DO IROW=1,MPR%NROW; DO ICOL=1,MPR%NCOL
   TMP1(1)=0.0D0; DO J=1,PEST%NREALS; TMP1(J)=PAR(J)%X(ICOL,IROW); ENDDO
   CALL UTL_STDEF(TMP1,PEST%NREALS,PAR(1)%NODATA,STD,AVG,N)
   MPR%X(ICOL,IROW)=AVG; VPR%X(ICOL,IROW)=STD
  ENDDO; ENDDO
  DEALLOCATE(TMP1)
  CALL UTL_CREATEDIR(TRIM(DIR)//'\STATPARS')
  MPR%FNAME=TRIM(DIR)//'\STATPARS\MEAN_PARAMETER'//TRIM(ITOS(I))//'_ITER_'//TRIM(ITOS(ITER))//'.IDF'
  IF(.NOT.IDFWRITE(MPR,MPR%FNAME,1))STOP
  VPR%FNAME=TRIM(DIR)//'\STATPARS\STDEV_PARAMETER'//TRIM(ITOS(I))//'_ITER_'//TRIM(ITOS(ITER))//'.IDF'
  IF(.NOT.IDFWRITE(VPR,VPR%FNAME,1))STOP
 ENDDO
 
 END SUBROUTINE IPEST_IES_SAVE_PARAMETERS_STATS
 
 !#####=================================================================
 LOGICAL FUNCTION IPEST_IES_UPDATE_ENSEMBLES(DIR,ITER,JLAMBDA,JLINE,IBATCH,MNAME,SPACEDIM,TIMEDIM,OBSDIM) 
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,MNAME
 INTEGER,INTENT(IN) :: IBATCH,SPACEDIM,TIMEDIM,OBSDIM
 INTEGER,INTENT(INOUT) :: ITER
 INTEGER,INTENT(INOUT) :: JLAMBDA,JLINE
 INTEGER :: I,II,JJ,J,K,N,IROW,ICOL,IX,ILOG,ILAMBDA,IPARAM,NPOP,ILINE
 REAL(KIND=DP_KIND) :: X,VAR,A,L,TL,MAXTL,GAMMA,F1,F2,F3,J0,MLT
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: DM,X1,X2,X3
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: SO,MO,O,TMP1,PCLASS,RCLASS
 ...
 LMODELERROR=.FALSE.; IF(PEST%PE_REGULARISATION.EQ.1)LMODELERROR=.TRUE.
 ...
 DO J=1,TIMEDIM
  ...
  DO K=1,OBSDIM
   TMP1(K)=MSR%DHG(J,K) 
   SO(J)=SO(J)+MSR%DHG(J,K)**2.0D0*MSR%W(K)
   O(J)=O(J)+MSR%DHG(J,K)
  ENDDO
  CALL UTL_STDEF(TMP1,OBSDIM,99999.0D0,SIGMA(J,ITER),MU(J,ITER),NPOP)
  !## get pdf of error distribution
  CALL IPEST_IES_GETPDF(TMP1,PDF(:,J),RCLASS) 
 ENDDO

 !## compute model-error per ensemble
 ALLOCATE(MO(TIMEDIM)); MO=0.0D0
 IF(LMODELERROR.AND.ITER.GT.0)CALL IPEST_IES_COMPUTE_MODEL_ERROR(DIR,MO,SPACEDIM,TIMEDIM,OBSDIM,ILOG,ITER,JLAMBDA,JLINE)

 !## get current sum of weighted objective function values
 MSR%TJ=SUM(SO)+SUM(MO)
 MSR%J =SUM(O)/DBLE(TIMEDIM)/DBLE(OBSDIM)
 ...
 IF(ITER.GT.0)THEN
  ...
  WRITE(IUPESTEFFICIENCY,'(I5,A5,99(1X,F15.3))') ITER,'-',MSR%TJ,F3,SUM(SO),SUM(MO),MSR%J,F1,F2,LAMBDA, &
  ...
  IF(MSR%TJ.LE.F3)THEN 
   ...
   ELSE
    ...
    !## get update for lambda
    CALL IPEST_IES_UPDATE_LAMBDA(DIR,ITER,GAMMA,MO)
  ...
  ELSE
   ...
   WRITE(IUPESTEFFICIENCY,'(/A,F15.3)') 'OLD LAMBDA',LAMBDA

   LAMBDA=LAMBDA*PBMAN%LAMBDA_TEST(JLAMBDA)
   N=PBMAN%NLAMBDASEARCH/2; IF(MOD(PBMAN%NLAMBDASEARCH,2).NE.0)N=N+1
   IF(JLAMBDA.LT.N)THEN
    LAMBDA=1.0D0/PBMAN%LAMBDA_TEST(PBMAN%NLAMBDASEARCH)*LAMBDA
   ELSEIF(JLAMBDA.GT.N)THEN
    LAMBDA=1.0D0/PBMAN%LAMBDA_TEST(1)*LAMBDA
   ENDIF

!   IF(PBMAN%LAMBDA_TEST(PBMAN%NLAMBDASEARCH).NE.1.0D0)THEN
!    LAMBDA=LAMBDA*PBMAN%LAMBDA_TEST(PBMAN%NLAMBDASEARCH)
!   ELSE
!    LAMBDA=LAMBDA*GAMMA
!   ENDIF
 ...
 WRITE(IUPESTOUT,'(/A)') 'Process Lambdas ('//TRIM(ITOS(PBMAN%NLAMBDASEARCH))//')'
 DO ILAMBDA=1,PBMAN%NLAMBDASEARCH
  ...
  DO
   ...
   DO I=1,SIZE(PEST%PARAM)
    ...
    IF(ILAMBDA.EQ.1)THEN
     ...
     CALL IPEST_IES_COMPUTE_DM(DIR,I,TIMEDIM,SPACEDIM,REALS,DM,ITER,ILOG,JLAMBDA,JLINE) 
    ...
    DO ILINE=1,PBMAN%NLINESEARCH
     ...
     CALL IPEST_IES_COMPUTE_GETREALS(DIR,I,TIMEDIM,SPACEDIM,REALS,ITER,ILOG,JLAMBDA,JLINE) 

     !## processing the inverse of the prior parameter covariance matrix
     CALL IPEST_IES_COMPUTE_PARTIAL_M(ITER,I,TIMEDIM,SPACEDIM,OBSDIM,DM,A,REALS,L,LMODELERROR,ILINE); TL=TL+L
     ...
     DO K=1,TIMEDIM
      ...
      CALL IPEST_IES_GETENSEMBLENAME(ITER+1,DIR,PEST%PARAM(I)%REALSFNAME(K),FNAME,ILAMBDA,ILINE,'Write')
...
! IF(TL.LT.PEST%PE_PADJ)IPEST_IES_UPDATE_ENSEMBLES=.FALSE.
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_MODEL_ERROR(DIR,MO,SPACEDIM,TIMEDIM,OBSDIM,ILOG,ITER,JLAMBDA,JLINE)
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(OUT) :: MO
 INTEGER,INTENT(IN) :: SPACEDIM,TIMEDIM,OBSDIM,ILOG,ITER,JLAMBDA,JLINE
 INTEGER :: IPARAM,I,J
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: X1,X2,X3
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: REALS
 
 ALLOCATE(X1(SPACEDIM,TIMEDIM),X2(SPACEDIM,SPACEDIM),X3(SPACEDIM,TIMEDIM))
 IF(ALLOCATED(REALS))DEALLOCATE(REALS); ALLOCATE(REALS(TIMEDIM,SPACEDIM))
 
 DO IPARAM=1,SIZE(PEST%PARAM)
  !## read realisatons for model-error
  WRITE(IUPESTOUT,'(/A)') 'Read previous set of parameter to compute the MODEL-ERROR'
  CALL IPEST_IES_COMPUTE_GETREALS(DIR,IPARAM,TIMEDIM,SPACEDIM,REALS,ITER,ILOG,JLAMBDA,JLINE) 
  X2=PEST%PARAM(IPARAM)%ICOV
  DO I=1,TIMEDIM
   DO J=1,SPACEDIM; X1(J,I)=REALS(I,J)-PEST%PARAM(IPARAM)%MPR(I,J); ENDDO
  ENDDO
  CALL UTL_MATMUL(X1,X2,X3); DEALLOCATE(X1); ALLOCATE(X1(TIMEDIM,SPACEDIM))
  X1=TRANSPOSE(X3)
  DEALLOCATE(X2); ALLOCATE(X2(TIMEDIM,TIMEDIM))
  CALL UTL_MATMUL(X3,X1,X2)
  DO I=1,TIMEDIM; MO(I)=X2(I,I); ENDDO    
  DEALLOCATE(X1,X2,X3)
 ENDDO

 MO=MO/(REAL(SPACEDIM,8)/REAL(OBSDIM,8))

 END SUBROUTINE IPEST_IES_COMPUTE_MODEL_ERROR
 
 !#####=================================================================
 SUBROUTINE IPEST_IES_UPDATE_LAMBDA(DIR,ITER,GAMMA,MO)
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(OUT) :: MO
 ...
 INTEGER :: I,J
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: XLAMBDA,YLAMBDA,CPARABOL
 REAL(KIND=DP_KIND) :: X,MINJ,JJ

 WRITE(IUPESTEFFICIENCY,'(/A,F15.3)') 'OLD LAMBDA ',LAMBDA

 IF(PBMAN%NLAMBDASEARCH.GT.1)THEN 

  I=PBMAN%NLAMBDASEARCH
  ALLOCATE(XLAMBDA(I),YLAMBDA(I),CPARABOL(3))
  DO I=1,PBMAN%NLAMBDASEARCH
   MINJ=1.0D21; DO J=1,PBMAN%NLINESEARCH
    CALL IPEST_IES_READ_DHG(DIR,ITER,I,J,MO)
    JJ=SUM(JE(:,ITER))+SUM(MO)
    IF(JJ.LT.MINJ)MINJ=JJ
   ENDDO
   YLAMBDA(I)=MINJ 
   XLAMBDA(I)=PBMAN%LAMBDA_TEST(I)*LAMBDA
  ENDDO
  
  WRITE(IUPESTEFFICIENCY,'(/2A10,1X,2A15)') 'ITER','ILAMBDA','LAMBDA','J'
  DO I=1,PBMAN%NLAMBDASEARCH
   WRITE(IUPESTEFFICIENCY,'(2I10,1X,2F15.3)') ITER,I,YLAMBDA(I),XLAMBDA(I)
  ENDDO
  ...
  IF(CPARABOL(3).GT.0.0D0)THEN
   ...
   WRITE(IUPESTEFFICIENCY,'(/A,F15.3))') 'BOT LAMBDA ', X 
  ELSE
   LAMBDA=LAMBDA/GAMMA
   WRITE(IUPESTEFFICIENCY,'(/A,F15.3))') 'TOP LAMBDA ', X 
  ENDIF
  WRITE(IUPESTEFFICIENCY,'(A,F15.3/)') 'NEW LAMBDA ',LAMBDA
  DEALLOCATE(XLAMBDA,YLAMBDA,CPARABOL)
 ELSE
  X=LAMBDA; LAMBDA=LAMBDA/GAMMA
  WRITE(IUPESTEFFICIENCY,'(/A,F15.3/)') 'NEW LAMBDA ',LAMBDA
 ENDIF
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_WRITE_DHG(DIR,ITER,ILAMBDA,ILINE,MO)
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(IN) :: MO
 INTEGER,INTENT(IN) :: ITER,ILAMBDA,ILINE
 INTEGER :: IU,I,J
 REAL(KIND=DP_KIND) :: W
 
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(DIR)//'\IIES\DHG_ITER'//TRIM(ITOS(ITER))//'_ILAMBDA'//TRIM(ITOS(ILAMBDA))// &
              '_ILINE'//TRIM(ITOS(ILINE))//'.TXT',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(IU,'(/A/)') 'TOTAL-ERROR'
 WRITE(IU,'(2A10)') 'REAL.','ERROR'
 DO I=1,SIZE(MSR%DHG,1); WRITE(IU,'(I10,F15.3)') I,JE(I,ITER); ENDDO
 WRITE(IU,'(20A1)') ('-',I=1,20)
 WRITE(IU,'(A10,F15.3)') 'TOTAL',SUM(JE(:,ITER))
 WRITE(IU,'(/A/)') 'DATA-ERROR'
 ...
 ENDDO
 WRITE(IU,'(/A/)') 'MODEL-ERROR'
 WRITE(IU,'(2A10)') 'REAL.','ERROR'
 DO I=1,SIZE(MO)
  WRITE(IU,'(I10,F15.3)') I,MO(I)
 ENDDO
 CLOSE(IU)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_READ_DHG(DIR,ITER,ILAMBDA,ILINE,MO)
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER,INTENT(IN) :: ITER,ILAMBDA,ILINE
 ...
 IU=UTL_GETUNIT()
 IF(ITER.EQ.0)THEN
  OPEN(IU,FILE=TRIM(DIR)//'\IIES\DHG_ITER'//TRIM(ITOS(ITER))//'_ILAMBDA'// &
       TRIM(ITOS(1))//'_ILINE'//TRIM(ITOS(1))//'.TXT',STATUS='OLD',ACTION='READ')
 ELSE
  OPEN(IU,FILE=TRIM(DIR)//'\IIES\DHG_ITER'//TRIM(ITOS(ITER))//'_ILAMBDA'// &
       TRIM(ITOS(ILAMBDA))//'_ILINE'//TRIM(ITOS(ILINE))//'.TXT',STATUS='OLD',ACTION='READ')
 ENDIF
 DO I=1,4; READ(IU,*); ENDDO
 DO I=1,SIZE(MSR%DHG,1)
  READ(IU,'(10X,F15.3)') JE(I,ITER)
 ENDDO
 DO I=1,6; READ(IU,*); ENDDO
 DO J=1,MSR%NOBS
...
!  ENDDO
 ...
 LOGICAL :: L1,L2,L3
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: XT1,XT2,XT3,XT4
 
 !## carried out only once
 IF(ASSOCIATED(PEST%PARAM(IPARAM)%ICOV))RETURN
 ...
 ELSE
  ...
  N=SPACEDIM
 ENDIF
 !## save as IDF (for fun) - use double precision
 CALL IDFNULLIFY(IDF); IDF%ITYPE=8
 ...
 ALLOCATE(PEST%PARAM(IPARAM)%ISQRTCOV(N,N),PEST%PARAM(IPARAM)%SQRTCOV(N,N),PEST%PARAM(IPARAM)%ICOV(N,N))
 IF(.NOT.IDFALLOCATEX(IDF))RETURN
 
 INQUIRE(FILE=TRIM(DIR)//'\ENSEMBLES\ICOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF'    ,EXIST=L1)
 INQUIRE(FILE=TRIM(DIR)//'\ENSEMBLES\SQRTCOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF' ,EXIST=L2)
 INQUIRE(FILE=TRIM(DIR)//'\ENSEMBLES\ISQRTCOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF',EXIST=L3)
 IF(L1.AND.L2.AND.L3)THEN
  WRITE(*,'(/A)') 'Found existing:'
  WRITE(*,'(A)') '- ICOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF'
  WRITE(*,'(A)') '- SQRTCOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF'
  WRITE(*,'(A)') '- ISQRTCOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF'
  WRITE(*,'(/A$)') 'Do you want to use them (Y/y) ? '
  READ(*,'(A1)') YN
  IF(YN.EQ.'Y'.OR.YN.EQ.'y')THEN
   WRITE(*,'(A)') 'Reading '//TRIM(DIR)//'\ENSEMBLES\ICOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF ...'
   IF(.NOT.IDFREAD(IDF,TRIM(DIR)//'\ENSEMBLES\ICOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF',1))STOP
   PEST%PARAM(IPARAM)%ICOV=IDF%X
   WRITE(*,'(A)') 'Reading '//TRIM(DIR)//'\ENSEMBLES\SQRTCOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF ...'
   IF(.NOT.IDFREAD(IDF,TRIM(DIR)//'\ENSEMBLES\SQRTCOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF',1))STOP
   PEST%PARAM(IPARAM)%SQRTCOV=IDF%X
   WRITE(*,'(A)') 'Reading '//TRIM(DIR)//'\ENSEMBLES\ISQRTCOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF ...'
 ...
 IF(.NOT.L1.OR..NOT.L2)THEN
  ALLOCATE(XT1(N,N),XT2(N,N),XT3(N,N),XT4(N,N))
  XT1=PEST%PARAM(IPARAM)%COV
  XT2=0.0D0; XT3=0.0D0; XT4=0.0D0
  call IPEST_IES_DUMP(Xt1,'cov',0)
  CALL LUDCMP_CALC_SQRTROOTINVERSE(N,XT1,XT2,XT3,XT4)
  !## use variances only (not the covariates)
!  XT2=0.0D0; DO J=1,N; XT2(J,J)=1.0D0/PEST%PARAM(IPARAM)%COV(J,J); ENDDO
  PEST%PARAM(IPARAM)%ICOV    =XT2
  PEST%PARAM(IPARAM)%SQRTCOV =XT3
  PEST%PARAM(IPARAM)%ISQRTCOV=XT4
!## checkt as it need to be identity matrix
!  xt3=PEST%PARAM(IPARAM)%COV
!  CALL UTL_MATMUL(xt3,xt2,xt4)
  DEALLOCATE(XT1,XT2,XT3,XT4)
  DO J=1,N; DO K=1,N; IDF%X(J,K)=PEST%PARAM(IPARAM)%ICOV(J,K); ENDDO; ENDDO
  IDF%FNAME=TRIM(DIR)//'\ENSEMBLES\ICOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF'; IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
  DO J=1,N; DO K=1,N; IDF%X(J,K)=PEST%PARAM(IPARAM)%SQRTCOV(J,K); ENDDO; ENDDO
  IDF%FNAME=TRIM(DIR)//'\ENSEMBLES\SQRTCOV_IPARAM'//TRIM(ITOS(IPARAM))//'.IDF'; IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_GETPDF(X,PDF,XCLASS) 
 !#####=================================================================
 IMPLICIT NONE
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_PARTIAL_M(ITER,IPARAM,TIMEDIM,SPACEDIM,OBSDIM,DM,A,REALS,L,LMODELERROR,ILINE)
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ITER,IPARAM,TIMEDIM,SPACEDIM,OBSDIM,ILINE
 LOGICAL,INTENT(IN) :: LMODELERROR
 ...
 REAL(KIND=DP_KIND) :: RN,XSUM,VAR,F
...
! STOP
 ...
 ALLOCATE(X2(SPACEDIM,SPACEDIM)); X2=-PEST%PARAM(IPARAM)%SQRTCOV
 CALL UTL_MATMUL(X2,DM,X1)
 DEALLOCATE(X2)
 CALL IPEST_IES_DUMP(PEST%PARAM(IPARAM)%SQRTCOV,'X17',ITER)
 ...
 IF(ITER.GT.0.AND.LMODELERROR)CALL IPEST_IES_DUMP(PM2,'PM2',ITER)
 
 F=PBMAN%LINE_SEARCH(ILINE)
 X1=REALS+F*PM1; IF(ITER.GT.0)X1=X1+F*PM2
! X1=REALS+PM1; IF(ITER.GT.0)X1=X1+PM2
 ...
 CALL UTL_CREATEDIR('D:\IMOD-MODELS\IES\MODELS\TMP')
...
!   IF(W(J).GT.0.0D0)AM(J,I)=U(J,I)*1.0D0/W(J)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_GETREALS(DIR,IPARAM,TIMEDIM,SPACEDIM,REALS,ITER,ILOG,ILAMBDA,ILINE) 
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER,INTENT(IN) :: IPARAM,TIMEDIM,SPACEDIM,ITER,ILOG,ILAMBDA,ILINE
 ...
 REALS=0.0D0
 DO J=1,TIMEDIM
  CALL IPEST_IES_GETENSEMBLENAME(ITER,DIR,PEST%PARAM(IPARAM)%REALSFNAME(J),FNAME,ILAMBDA,ILINE,'Read')
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_DM(DIR,IPARAM,TIMEDIM,SPACEDIM,REALS,DM,ITER,ILOG,ILAMBDA,ILINE) 
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER,INTENT(INOUT) :: ITER
 INTEGER,INTENT(IN) :: IPARAM,TIMEDIM,SPACEDIM,ILOG,ILAMBDA,ILINE
 REAL(KIND=DP_KIND),DIMENSION(TIMEDIM,SPACEDIM) :: DM,REALS
 INTEGER :: J,K
 REAL(KIND=DP_KIND) :: RN,XSUM
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: X1
 
 !## read realisatons
 WRITE(IUPESTOUT,'(/A)') 'Read previous set of parameter to compute DM'
 CALL IPEST_IES_COMPUTE_GETREALS(DIR,IPARAM,TIMEDIM,SPACEDIM,REALS,ITER,ILOG,ILAMBDA,ILINE) 
 ...
 DO K=1,SPACEDIM
  ...
  ENDDO
 ENDDO
 ALLOCATE(X1(SPACEDIM,SPACEDIM)); X1=PEST%PARAM(IPARAM)%ISQRTCOV
 CALL UTL_MATMUL(X1,REALS,DM)
 DEALLOCATE(X1)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_SAVEREALS(DIR,ITER,ILAMBDA,ILINE,MNAME) 
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ITER
 INTEGER,INTENT(IN) :: ILAMBDA,ILINE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,MNAME
 ...
 DO I=1,PEST%NREALS
  ...
  DO J=1,SIZE(PEST%PARAM)
   DO K=1,SIZE(PEST%PARAM(J)%ILS)
    ...
    CALL IPEST_IES_GETENSEMBLENAME(ITER,DIR,PEST%PARAM(J)%REALSFNAME(I),FNAME,ILAMBDA,ILINE,'Read')
...
!SAVEDOUBLE 0
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_GETENSEMBLENAME(ITER,DIR,FNAME_IN,FNAME_OUT,ILAMBDA,ILINE,TXT)
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ITER,ILAMBDA,ILINE
 ...
 ELSE
  ...
  FNAME_OUT=FNAME_OUT(:INDEX(FNAME_OUT,'.',.TRUE.)-1)//'_ITER'//TRIM(ITOS(ITER))// &
         '_ILAMBDA'//TRIM(ITOS(ILAMBDA))//'_ILINE'//TRIM(ITOS(ILINE))//'.IDF'
 ...
 ALLOCATE(SIGMA(N,0:PEST%PE_MXITER)); SIGMA=0.0D0
 ALLOCATE(MU   (N,0:PEST%PE_MXITER)); MU   =0.0D0
 ...
 WRITE(IUPESTEFFICIENCY,'(2A5,10(1X,A15))') 'ITER','LTEST','TOTAL-ERROR_NEW','TOTAL-ERROR_OLD','DATA-ERROR','MODEL-ERROR','AVG-RESIDUAL','RELATIVE','ABSOLUTE','LAMBDA','MEAN','STDEV'
 WRITE(IUPESTEFFICIENCY,'(10X,10(1X,A15))') ('[-]',I=1,4),'[M]','[%]','[%]','[-]','[-]','[-]'
