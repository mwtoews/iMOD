...
CONTAINS
 ...
 WRITE(IUPESTPROGRESS,'(A10,99(1X,A15))') 'ITER',('OBJF_SIM'//TRIM(ITOS(IGRAD)),IGRAD=1,PEST%NREALS)
 DO ITER=0,PEST%PE_MXITER
  WRITE(IUPESTPROGRESS,'(I10,99(1X,F15.7))') ITER,(JE(IGRAD,ITER),IGRAD=1,PEST%NREALS)
 ENDDO
 ...
 DO I=1,PEST%NREALS
  HDS(I)%FNAME=TRIM(DIR)//'\IIES_R#'//TRIM(ITOS(I))//'\HEAD\HEAD_STEADY-STATE_L1.IDF'
  IF(.NOT.IDFREAD(HDS(I),HDS(I)%FNAME,1))STOP
 ENDDO
 CALL IDFCOPY(HDS(1),MHD); CALL IDFCOPY(HDS(1),VHD)
 ...
 MHD%FNAME=TRIM(DIR)//'\STATHEADS\MEAN_HEAD_STEADY-STATE_L1_ITER_'//TRIM(ITOS(ITER))//'.IDF'
 IF(.NOT.IDFWRITE(MHD,MHD%FNAME,1))STOP
 VHD%FNAME=TRIM(DIR)//'\STATHEADS\STDEV_HEAD_STEADY-STATE_L1_ITER_'//TRIM(ITOS(ITER))//'.IDF'
 ...
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: SO,O,TMP1,PCLASS
...
! CALL UTL_MATMUL(TMP1,TMP2,TMP3)
 ...
 IF(ALLOCATED(PDF))DEALLOCATE(PDF); ALLOCATE(PDF(N,TIMEDIM))
 ...
 ENDDO
 DO JJ=1,SIZE(PDF,1)
  WRITE(IUPESTPROGRESS,'(F15.7,99(1X,I10))') PCLASS(JJ),(PDF(JJ,II),II=1,TIMEDIM) 
 ENDDO
 DEALLOCATE(TMP1)
 
 !## get current sum of weighted objective function values
 MSR%TJ=SUM(SO)
 MSR%J= SUM(O)/DBLE(TIMEDIM)
 ...
 IF(ITER.GT.0)THEN
  ...
  WRITE(IUPESTPROGRESS,'(A10,5(1X,A15))') 'ITER','CURRENT_TJ','PREVIOUS_PJ','UNWEIGHTED_J','IMPROVEMENT(%)','LAMBDA'
  WRITE(IUPESTPROGRESS,'(I10,5(1X,F15.7))') ITER,MSR%TJ,MSR%PJ,MSR%J,100.0D0*(1.0D0-MSR%TJ/MSR%PJ),LAMBDA
  ...
  IF(MSR%TJ.LE.MSR%PJ)THEN
   ...
   ELSE
    ...
    LAMBDA=LAMBDA/GAMMA
    WRITE(IUPESTPROGRESS,'(/A/)') 'Awesome, significant reduction of the objective function reduction: proceed'
  ...
  ELSE
   ...
   ITER=ITER-1
   WRITE(IUPESTPROGRESS,'(/A/)') 'No objective function reduction: reset to previous iteration'
  ENDIF
 ENDIF
...
! DO J=1,TIMEDIM
!  XSUM=0.0D0; DO I=1,OBSDIM; XSUM=XSUM+MSR%DHG(J,I); ENDDO; XSUM=XSUM/DBLE(OBSDIM)
!  DO I=1,OBSDIM; X1(J,I)=(MSR%DHG(J,I)-XSUM)/RN; ENDDO
! ENDDO

 DO I=1,OBSDIM
  !## compute average observational error
  XSUM=0.0D0; DO J=1,TIMEDIM; XSUM=XSUM+MSR%DHG(J,I); ENDDO; XSUM=XSUM/DBLE(TIMEDIM)
!  XSUM=SUM(MSR%DHG(I,:))/DBLE(TIMEDIM)
...
! DO I=1,DD_TRUNCATED; DO J=1,OBSDIM; X1(J,I)=V(I,J); ENDDO; ENDDO 
...
! DO I=1,DD_TRUNCATED; DO J=1,TIMEDIM; X4(I,J)=U(I,J); ENDDO; ENDDO
...
!  DO I=1,DD_TRUNCATED; DO J=1,TIMEDIM; X3(I,J)=U(I,J); ENDDO; ENDDO
  CALL UTL_MATMUL(X3,X1,X2)
  DEALLOCATE(X1,X3)

  ALLOCATE(X1(TIMEDIM,DD_TRUNCATED),X3(TIMEDIM,TIMEDIM))
  DO I=1,TIMEDIM; DO J=1,DD_TRUNCATED; X1(I,J)=V(J,I); ENDDO; ENDDO
!  DO I=1,TIMEDIM; DO J=1,DD_TRUNCATED; X1(I,J)=U(J,I); ENDDO; ENDDO
  CALL UTL_MATMUL(X2,X1,X3)
  DEALLOCATE(X2,X1)

!  ALLOCATE(X1(TIMEDIM,TIMEDIM),X2(TIMEDIM,DD_TRUNCATED))
!  DO I=1,TIMEDIM; DO J=1,DD_TRUNCATED; X2(I,J)=U(I,J); ENDDO; ENDDO
!  CALL UTL_MATMUL(X3,X2,X1) !U,X1)
!  DEALLOCATE(X3,X2)
...
! DO K=1,TIMEDIM
!  XSUM=0.0D0; DO J=1,SPACEDIM; XSUM=XSUM+REALS(K,J); ENDDO; XSUM=XSUM/DBLE(SPACEDIM)
!  DO J=1,SPACEDIM
!   REALS(K,J)=(REALS(K,J)-XSUM)/RN
!  ENDDO
! ENDDO
