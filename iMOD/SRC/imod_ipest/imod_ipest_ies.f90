...
!  PEST%PARAM(I)%ICREATEENSEMBLES
 ...
 ALLOCATE(SO(TIMEDIM),O(TIMEDIM),MO(TIMEDIM)); SO=0.0D0; MO=0.0D0; O=0.0D0
 ...
 IF(.FALSE.)THEN
  ALLOCATE(X1(1,SPACEDIM),X2(SPACEDIM,SPACEDIM),X3(1,SPACEDIM))
  DO I=1,TIMEDIM
   DO IPARAM=1,SIZE(PEST%PARAM)
    DO J=1,SPACEDIM; X1(1,J)=REALS(I,J)-PEST%PARAM(IPARAM)%MPR(I,J); ENDDO
    X2=PEST%PARAM(IPARAM)%ICOV
    CALL UTL_MATMUL(X1,X2,X3)
    DO J=1,SPACEDIM; MO(I)=MO(I)+X3(1,J)**2.0D0; ENDDO
   ENDDO
  ENDDO
 ENDIF
...
!  CALL UTL_MATMUL(AM,X4,X5)
  ...
  ALLOCATE(X1(TIMEDIM,SPACEDIM))
  ALLOCATE(X2(SPACEDIM,SPACEDIM)); X2=-PEST%PARAM(IPARAM)%SQRTCOV
!  ALLOCATE(X2(SPACEDIM,SPACEDIM)); X2=-PEST%PARAM(IPARAM)%INVCOV
