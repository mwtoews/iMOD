...
CONTAINS
 ...
 INTEGER :: I,J,K,IU,N,SPACEDIM,TIMEDIM,OBSDIM,IROW,ICOL,IX
 TYPE(IDFOBJ) :: IDF
 REAL(KIND=DP_KIND) :: XSUM,RN,WW,X,LAMBDA,VAR,A
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: U,V,AM,DM,TMP1,TMP2,TMP3,MPR
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: W,SO
...
!## used for testing
  ...
  !## compute dm-matrix
  ALLOCATE(REALS(SPACEDIM,TIMEDIM),DM(TIMEDIM,SPACEDIM))
  CALL IPEST_IES_COMPUTE_DM(I,TIMEDIM,SPACEDIM,REALS,DM) 

  !## store prior-estimated parameters
  IF(ITER.EQ.1)THEN; ALLOCATE(MPR(SPACEDIM,TIMEDIM)); MPR=REALS; ENDIF
  
  !## compute am-matrix initially through svd
  ALLOCATE(AM(SPACEDIM,SPACEDIM))
  CALL IPEST_IES_COMPUTE_AM(TIMEDIM,SPACEDIM,DM,AM) 
 ...
 IF(ITER.EQ.1)THEN
  X=SUM(SO)/DBLE(2.0D0*OBSDIM); X=LOG10(X); IX=FLOOR(X); LAMBDA=10.0**IX
 ENDIF
 ...
 !## processing the inverse of the prior parameter covariance matrix
 DO I=1,SIZE(PEST%PARAM)
  CALL IPEST_IES_COMPUTE_PARTIAL_M1(ITER,I,TIMEDIM,SPACEDIM,OBSDIM,DM,A,AM,MPR,REALS) 
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_PARTIAL_M1(ITER,IPARAM,TIMEDIM,SPACEDIM,OBSDIM,DM,A,AM,MPR,REALS)
 !#####=================================================================
 IMPLICIT NONE
 ...
 REAL(KIND=DP_KIND),DIMENSION(SPACEDIM,TIMEDIM),INTENT(IN) :: MPR
 REAL(KIND=DP_KIND),DIMENSION(SPACEDIM,SPACEDIM),INTENT(IN) :: AM
 ...
 ALLOCATE(X1(SPACEDIM,TIMEDIM))
 X1=MATMUL(-PEST%PARAM(IPARAM)%SQRTCOV,DM)
 ALLOCATE(PM1(SPACEDIM,TIMEDIM))
  
 PM1=MATMUL(X1,X3); DEALLOCATE(X1,X3)

 IF(ITER.GT.1)THEN
  ALLOCATE(X1(SPACEDIM,SPACEDIM))
  DO I=1,SPACEDIM; DO J=1,SPACEDIM; X1(I,J)=AM(J,I); ENDDO; ENDDO
 ...
 REALS=REALS+PM1; IF(ITER.GT.1)REALS=REALS+PM2
 DEALLOCATE(PM1,PM2)

 !# save the updates and done
! DEALLOCATE(X1,X2,X3)
 
 END SUBROUTINE IPEST_IES_COMPUTE_PARTIAL_M1
 
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_AM(TIMEDIM,SPACEDIM,DM,AM) 
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: TIMEDIM,SPACEDIM
 REAL(KIND=DP_KIND),DIMENSION(TIMEDIM,SPACEDIM),INTENT(IN) :: DM
 REAL(KIND=DP_KIND),DIMENSION(SPACEDIM,SPACEDIM),INTENT(OUT) :: AM
 INTEGER :: N,i,J,K,IROW,ICOL,NT,NS
 REAL(KIND=DP_KIND) :: RN,XSUM
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: W
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: V,U,S
 LOGICAL :: LTEST
...
! IF(ALLOCATED(U))DEALLOCATE(U)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_DM(I,TIMEDIM,SPACEDIM,REALS,DM) 
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: I,TIMEDIM,SPACEDIM
 REAL(KIND=DP_KIND),DIMENSION(SPACEDIM,TIMEDIM) :: REALS
 REAL(KIND=DP_KIND),DIMENSION(TIMEDIM,SPACEDIM) :: DM
 INTEGER :: N,J,K,IROW,ICOL
 REAL(KIND=DP_KIND) :: RN,XSUM
 ...
 DO J=1,TIMEDIM
  ...
  K=0; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL; K=K+1; REALS(K,J)=PRJIDF%X(ICOL,IROW); ENDDO; ENDDO
  CALL IDFDEALLOCATEX(PRJIDF)
 ENDDO
 RN=SQRT(DBLE(TIMEDIM)-1.0D0)
 DO J=1,TIMEDIM; XSUM=SUM(REALS(:,J))/DBLE(TIMEDIM)
  DO K=1,TIMEDIM; REALS(K,J)=(REALS(K,J)-XSUM)/RN; ENDDO
 ENDDO
 IF(.NOT.IDFALLOCATEX(PRJIDF))STOP
 DO J=1,TIMEDIM
  K=0; DO IROW=1,PRJIDF%NROW; DO ICOL=1,PRJIDF%NCOL; K=K+1; PRJIDF%X(ICOL,IROW)=REALS(K,J); ENDDO; ENDDO
  PRJIDF%FNAME='D:\IMOD-MODELS\IES\CREATEENSEMBLES\VAR_ENSEMBLE'//TRIM(ITOS(J))//'.IDF'; IF(.NOT.IDFWRITE(PRJIDF,PRJIDF%FNAME,1))STOP
 ENDDO

 DM=MATMUL(PEST%PARAM(I)%ISQRTCOV,REALS)
