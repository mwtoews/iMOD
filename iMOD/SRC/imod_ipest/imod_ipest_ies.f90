...
CONTAINS
 ...
 IF(PEST%ICREATEENSEMBLES.EQ.1)THEN; IF(.NOT.IPEST_IES_CREATEENSEMBLES(DIR,ILOG))RETURN; ENDIF
     ...
     N=0; DO JGRAD=1,SIZE(RNG)
      ...
      IF(ISTATUS(JGRAD).EQ.0)THEN
       ...
       IF(ISTATUS(JGRAD).EQ.0)THEN
        ...
        IF(.NOT.IPEST_GLM_GETJ(DIR,JGRAD,GPARAM(JGRAD),'R',IBATCH))RETURN !,TNSC))RETURN
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  WRITE(*,*)  'MIN,MAX,MEAN,STDEV',PEST%PARAM(I)%PMIN,PEST%PARAM(I)%PMAX,PEST%PARAM(I)%PARMEAN,PEST%PARAM(I)%PARSTD
