  ...
  ELSE
   ...
   DO ILAMBDA=1,PBMAN%NLAMBDASEARCH
    ...
    DO ILINE=1,PBMAN%NLINESEARCH
     ...
     ENDIF
     !## compute model error
     CALL IPEST_IES_COMPUTE_MODEL_ERROR(DIR,MO,SPACEDIM,TIMEDIM,ILOG,ITER,JLAMBDA,JLINE)
     CALL IPEST_IES_PROGRESS_OUT(ITER)
   ...
   WRITE(IUPESTPROGRESS,'(/A/)') 'Selected lambda = '//TRIM(ITOS(JLAMBDA))//' ('//TRIM(RTOS(L,'G',7))// &
                                ') Selected linesearch = '//TRIM(ITOS(JLINE))//' ('//TRIM(RTOS(F,'G',7))// &
           ') has minimal Objective Function Value ('//TRIM(RTOS(MINJ,'G',7))//')'
   WRITE(IUPESTEFFICIENCY,'(/A/)') 'Selected lambda = '//TRIM(ITOS(JLAMBDA))//' ('//TRIM(RTOS(L,'G',7))// &
                                ') Selected linesearch = '//TRIM(ITOS(JLINE))//' ('//TRIM(RTOS(F,'G',7))// &
           ') has minimal Objective Function Value ('//TRIM(RTOS(MINJ,'G',7))//')'
...
! OBSDIM=MSR%NOBS
 ...
 ALLOCATE(MO(TIMEDIM)); MO=0.0D0
 IF(LMODELERROR.AND.ITER.GT.0)CALL IPEST_IES_COMPUTE_MODEL_ERROR(DIR,MO,SPACEDIM,TIMEDIM,ILOG,ITER,JLAMBDA,JLINE)
 
 !## half of the objective function determined by measurement-error if model-error is included
 IF(LMODELERROR)THEN
  F1=(REAL(SPACEDIM,8)/REAL(OBSDIM,8))
  MO=MO*1.0D0/F1
 ENDIF
...
! IF(TL.LT.PEST%PE_PADJ)IPEST_IES_UPDATE_ENSEMBLES=.FALSE.
 ...
 !#####=================================================================
 SUBROUTINE IPEST_IES_COMPUTE_MODEL_ERROR(DIR,MO,SPACEDIM,TIMEDIM,ILOG,ITER,JLAMBDA,JLINE)
 !#####=================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(OUT) :: MO
 INTEGER,INTENT(IN) :: SPACEDIM,TIMEDIM,ILOG,ITER,JLAMBDA,JLINE
 INTEGER :: IPARAM,I,J
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: DM,X1,X2,X3
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: REALS
