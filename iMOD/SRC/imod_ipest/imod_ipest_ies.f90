...
USE MOD_IPEST_GLM, ONLY : IPEST_GLM_GETJ,IPEST_GLM_ALLOCATEMSR,IPEST_GLM_RUNMODELS
...
CONTAINS
 ...
 DO WHILE (ITER.LE.PEST%PE_MXITER)
  ...
  IUPESTRESIDUAL=0; CALL IPEST_GLM_RUNMODELS(IBATCH,RNG,GPARAM,'R',DIR,MNAME,ITER,LAMBDA)
...
!  PEST%PARAM(I)%ICREATEENSEMBLES
 ...
 WRITE(IUPESTPROGRESS,'(95A1)') ('-',I=1,95)
...
! SUBROUTINE IPEST_IES_COMPUTE_PARTIAL_M(ITER,IPARAM,TIMEDIM,SPACEDIM,OBSDIM,DM,A,MPR,REALS,L)  
 ...
 CALL IPEST_IES_DUMP(DD,'DD',ITER) !EQUAL
 ...
 CALL IPEST_IES_DUMP(U,'U',ITER)  !SMALL DIFF
 CALL IPEST_IES_DUMP(VV,'VV',ITER)!SMALL DIFF
...
!  CALL UTL_MATMUL(AM,X4,X5)
 ...
 IU=UTL_GETUNIT()
 OPEN(IU,FILE='D:\IMOD-MODELS\IES\MODELS\TMP\DEBUG_'//  TRIM(TXT)//'_ITER'//TRIM(ITOS(ITER))//'.TXT',STATUS='UNKNOWN',ACTION='WRITE')
! OPEN(IU,FILE='D:\IMOD-MODELS\IES\MODELS\TMP\RELEASE_'//TRIM(TXT)//'_ITER'//TRIM(ITOS(ITER))//'.TXT',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(IU,*) 'COL,ROW,VALUE'
 DO I=1,SIZE(X,1); DO J=1,SIZE(X,2); WRITE(IU,*) I,J,X(I,J); ENDDO; ENDDO
 CLOSE(IU)
