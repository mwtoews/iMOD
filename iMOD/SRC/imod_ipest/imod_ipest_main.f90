...
!   CASE (4);
 ...
 IF(IPARAM.EQ.1)THEN
  XMIN=MINVAL(IPEST(IPOS)%MEASURE%OBS); XMAX=MAXVAL(IPEST(IPOS)%MEASURE%OBS)
  YMIN=MINVAL(IPEST(IPOS)%MEASURE%COM); YMAX=MAXVAL(IPEST(IPOS)%MEASURE%COM)
  XMIN=MIN(XMIN,YMIN); XMAX=MIN(XMAX,YMAX)
  YMIN=XMIN; YMAX=XMAX
 ELSE
...
!  XMIN=-MAX(ABS(XMIN),ABS(XMAX))
  ...
  DEALLOCATE(X)
  NDIM=NSX
  ALLOCATE(HCLASSES(NDIM),XCLASSES(NDIM))
 ...
 DO J=1,IPEST(IPOS)%NMEASURE
  D=ABS(IPEST(IPOS)%MEASURE(J)%OBS-IPEST(IPOS)%MEASURE(J)%COM)
  IF(D.LT.HCLASSES(2))THEN
   XCLASSES(1)=XCLASSES(1)+1; N=N+1
  ELSEIF(D.GT.HCLASSES(NDIM-1))THEN
   XCLASSES(NDIM)=XCLASSES(NDIM)+1; N=N+1
  ELSE
 ...
 !## get percentages
 DO I=1,SIZE(HCLASSES); XCLASSES(I)=100.0D0*(XCLASSES(I)/DBLE(N)); ENDDO
 ...
 INTEGER :: IPOS,I,IPARAM,N,NPOP
 REAL(KIND=DP_KIND) :: X1,X2,GOF,X,Y,AVG,VAR,XT,MINX,MAXX
 ...
 ELSE
  ...
  CALL DBL_WGRTEXTSTRING(X,Y,TRIM(LINE)) !CALL DBL_WGRTEXTBLOCK(0.20D0,0.5D0,0.50D0,0.8D0,TRIM(LINE)) 
!  CALL DBL_WGRTEXTSTRING(X,Y,'Goodness of Fit '//TRIM(RTOS(GOF*100.0D0,'G',7))//'%') 
