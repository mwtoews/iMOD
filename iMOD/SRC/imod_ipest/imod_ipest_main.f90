...
!     CALL IPEST_ANALYSE_READLOG('d:\IMOD-MODELS\SWISS\CALIBRATION\pest')
  ...
  CASE (FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
    CASE (IDF_MENU1,IDF_RADIO1,IDF_RADIO2,IDF_RADIO3,IDF_RADIO4,IDF_RADIO5,IDF_RADIO7,IDF_RADIO8,IDF_RADIO9,IDF_RADIO10,IDF_CHECK1, &
          IDF_CHECK2,IDF_CHECK3,IDF_MENU2,IDF_MENU3,IDF_RADIO11,IDF_RADIO12,IDF_CHECK4)
 ...
 DO
  ...
  IF(INDEX(LINE,'Parameter Correlation Matrix').GT.0)THEN
   ...
   DO I=1,SIZE(PARAM)
    READ(IU,'(A256)') LINE
    READ(LINE,'(15X,999F15.0)',IOSTAT=IOS) (IPEST(NPER)%CORR(I,J),J=1,SIZE(PARAM))
   ENDDO
  ENDIF
  ...
  IF(ID.NE.0)THEN
   SELECT CASE (ID)
    ...
    CASE (IDF_RADIO3,IDF_RADIO4,IDF_CHECK1,IDF_CHECK3)
     IF(I.NE.2)CYCLE
    CASE (IDF_RADIO11,IDF_RADIO12,IDF_CHECK4)
     IF(I.NE.4)CYCLE
...
!  YMIN= HUGE(1.0D0); YMAX=-HUGE(1.0D0)
!  DO I=1,SIZE(IPEST)
!   YMIN=MIN(YMIN,MINVAL(IPEST(I)%EIGV))
!   YMAX=MAX(YMAX,MAXVAL(IPEST(I)%EIGV))
!  ENDDO
 ENDIF
...
! ENDIF
 ...
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: YMIN,YMAX
 INTEGER :: I,J,IPARAM,IPOS,IRED,IGRN,ICLR,ILABEL
 REAL(KIND=DP_KIND) :: X1,Y1,X2,Y2,X,Y
 ...
 IF(IPARAM.EQ.1)THEN
  ...
  DO I=1,SIZE(PARAM); DO J=1,SIZE(PARAM)
   ...
   ELSE
    ...
    IF(IPEST(IPOS)%CORR(I,J).GT.0.0D0)THEN
     IRED=INT(255.0D0*IPEST(IPOS)%CORR(I,J),4)
     ICLR=WRGB(255,IRED,IRED)
    !## negative correlated
    ELSEIF(IPEST(IPOS)%CORR(I,J).LT.0.0D0)THEN
     IGRN=INT(255.0D0*IPEST(IPOS)%CORR(I,J),4)
...
!  CALL DBL_IGRMOVETO(XMIN,XMIN+BW); CALL DBL_IGRLINETO(XMAX,XMAX+BW)
!  CALL DBL_IGRMOVETO(XMIN,XMIN-BW); CALL DBL_IGRLINETO(XMAX,XMAX-BW)
