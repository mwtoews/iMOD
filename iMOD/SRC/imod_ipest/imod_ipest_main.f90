...
!     CALL IPEST_ANALYSE_READLOG('d:\IMOD-MODELS\SWISS\CALIBRATION\pest')
 ...
 INTEGER :: I,J,IU,NI,NP,NU,IOS,N,M,IACT,ISS,NPER,NDIM
 ...
 N=0; DO I=1,SIZE(IPEST)
  DO J=1,IPEST(I)%NMEASURE
   N=N+1; X(N)=ABS(IPEST(I)%MEASURE(J)%OBS-IPEST(I)%MEASURE(J)%COM)
  ENDDO
 ENDDO
 ...
 IF(ID.EQ.IDF_MENU1.OR.ID.EQ.IDF_MENU2.OR.ID.EQ.IDF_MENU3)THEN
  CALL WDIALOGGETMENU(IDF_MENU1,IPOS)
  DO I=1,IPEST(IPOS)%NMEASURE
   IPEST(IPOS)%MEASURE(I)%ISEL=IPEST_ANALYSE_QUERY(IPEST(IPOS)%MEASURE(I)%ILAY,IPEST(IPOS)%MEASURE(I)%IDATE)
  ENDDO
 ENDIF
 ...
 DO I=1,SIZE(GID)
  ...
  SELECT CASE (I)
   ...
   !## xy plot
   CASE (6); 
    XMIN=X1G6; YMIN=Y1G6; XMAX=X2G6; YMAX=Y2G6; YTITLE='Y-coordinate (m)'; XTITLE='X-coordinate (m)'
   CASE DEFAULT; CYCLE
  END SELECT
  ...
  !## set axes
  CALL IPEST_ANALYSE_PLOT_AXES(XMIN,YMIN,XMAX,YMAX,.FALSE.,XTITLE,YTITLE,I)
...
!!! PEST BOOK 5.3.2.1 Goodness of Fit
 ...
 !###======================================================================
 INTEGER FUNCTION IPEST_ANALYSE_QUERY(JLAY,JDATE)
 !###======================================================================
 IMPLICIT NONE
