...
!     CALL IPEST_ANALYSE_READLOG('d:\IMOD-MODELS\SWISS\CALIBRATION\pest')
 ...
 INTEGER,DIMENSION(:),ALLOCATABLE :: IGROUP,SGROUP,IPS
 ...
 DO I=1,2
  ...
  DO
   ...
   ENDIF
  ENDDO
  IF(I.EQ.1)ALLOCATE(IGROUP(NP),ACRONYM(NP),SGROUP(NP))
  REWIND(IU)
 ENDDO
 ...
 DO
  ...
  IF(INDEX(LINE,'Upgrade Vector Parameter History:').GT.0)THEN
   READ(IU,*); READ(IU,'(A256)',IOSTAT=IOS) LINE; IF(IOS.NE.0)EXIT
 ...
 DO
  ...
  IF(INDEX(LINE,'Parameter Correlation Matrix').GT.0)THEN
   ...
   DO I=1,N 
    READ(IU,'(A256)') LINE
    READ(LINE,'(15X,999F15.0)',IOSTAT=IOS) (IPEST(NPER)%CORR(IPS(I),IPS(J)),J=1,N) 
   ENDDO
  ENDIF
 ...
 DO 
  READ(IU,'(I10,999F15.0)',IOSTAT=IOS) I,(IPEST(I+1)%SENSI(J),J=1,NP)
  IF(IOS.NE.0)EXIT
  IF(I+1.EQ.SIZE(IPEST))EXIT
 ENDDO
 CLOSE(IU)
