...
!  PROCCMDPROC
 ...
 WRITE(IUPESTOUT,'(I2,1X,A5,2(1X,I5),5(1X,F15.7),I10,I10,A10,A15)') I,PEST%PARAM(IP)%PPARAM,PEST%PARAM(IP)%PILS, &
    ...
    ABS(PEST%PARAM(IP)%PIGROUP),PEST%PARAM(IP)%PLOG,'Unknown',PEST%PARAM(IP)%ACRONYM
...
!  ENDIF
 ...
 DO
  ...
  ELSE
   ...
   IF(PBMAN%PREGULARISATION.EQ.1)THEN
    DO IP1=1,SIZE(PEST%PARAM)  !## row
     IF(PEST%PARAM(IP1)%PACT.NE.1)CYCLE
     DP=PEST%PARAM(IP1)%ALPHA(1)-PEST%PARAM(IP1)%PINI
     JQR(I)=JQR(I)+DP
    ENDDO
   ENDIF
...
2  IF(M.NE.L)THEN
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z,H,WW,MC,MM,DHH,XCOR,YCOR,ZCOR,XCROSS,DHW,RFIT,DP
...
!
 ...
 IF(PBMAN%PREGULARISATION.EQ.1)THEN
  DO I=1,SIZE(PEST%PARAM)  !## row
   ...
   DP=(PEST%PARAM(I)%ALPHA(1)-PEST%PARAM(I)%PINI)**2.0D0
   MSR%TJ=MSR%TJ+DP
  ENDDO
 ENDIF
