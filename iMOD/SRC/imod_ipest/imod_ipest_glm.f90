...
CONTAINS
 ...
 REAL(KIND=DP_KIND) :: RFIT,LAMBDA
 INTEGER :: I,J,ITER,N,M,IFLAGS,IGRAD,NCPU,IU,NDONE,IX,ILIN
 ...
 DO 
  ...
  CALL IPEST_GLM_PROCESSES(IBATCH,RNL,LPARAM,'L',DIR,MNAME,ITER)
  ...
  DO 
   ...
   DO
    ...
    CALL IPEST_GLM_NEXTGRAD(GPARAM(IGRAD),'P',IGRAD)
   ...
   NDONE=IPEST_GLM_EVALUATE(IBATCH,NCPU,DIR,ITER,RNG,'P',GPARAM)
 ...
 !## start processes
  DO
   ...
   !## save residuals for lambda-testing
   IF(RT.EQ.'L')THEN
    ...
    IUPESTRESIDUAL=UTL_GETUNIT(); OPEN(IUPESTRESIDUAL,FILE=TRIM(DIR)//'\IPEST\LOG_PEST_RESIDUAL_'// &
      TRIM(ITOS(ITER))//'_'//TRIM(ITOS(IGRAD))//'.TXT',STATUS='UNKNOWN',ACTION='WRITE')
   ENDIF
   ...
   IF(PBMAN%NSWAIT.GT.0)CALL IOSWAIT(PBMAN%NSWAIT)
   !## clear error
   I=WINFOERROR(1); IDPROC=0; CALL IOSCOMMAND(TRIM(RNL(IGRAD)),IFLAGS=IFLAGS,IDPROC=IDPROC); IPROC(:,IGRAD)=IDPROC
   IF(WINFOERROR(1).EQ.ERROSCOMMAND)THEN
  ...
  IF(ITYPE.EQ.TIMEREXPIRED)THEN
   N=0; DO JGRAD=1,SIZE(RN) !G)
    ...
    IF(ISTATUS(JGRAD).EQ.0)THEN
     ...
     IF(ISTATUS(JGRAD).EQ.0)THEN
      IF(.NOT.IPEST_GLM_GETJ(DIR,JGRAD,RPARAM(JGRAD),RT,IBATCH,TNSC))RETURN
      !## write echo
      CALL IPEST_GLM_PROGRESS(ITER,JGRAD,ILIN,RT); FLUSH(IUPESTPROGRESS)
     ENDIF
...
!  ENDIF
 ...
 IF(RTYPE.EQ.'L')THEN
  ...
  PEST%PARAM%ALPHA(1)=PEST%PARAM%LALPHA(ISIM)
