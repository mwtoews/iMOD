 ...
 DO 
  ...
  IF(PEST%PE_REGULARISATION.EQ.1)THEN
   ...
   DO IP1=1,SIZE(PEST%PARAM)  !## row
    ...
    DP=(PEST%PARAM(IP1)%ALPHA(2)-PEST%PARAM(IP1)%PPRIOR)*WW !*PEST%PE_REGFACTOR
    JQR(I)=JQR(I)+DP
...
!  IF(PEST%PARAM(I)%PLOG.EQ.1)THEN
!   F=10.0D0**(PEST%PARAM(I)%ALPHA(1))/10.0D0**(PEST%PARAM(I)%ALPHA(2))
!  ELSE
...
! ENDIF
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z,H,WW,MC,MM,DHH,XCOR,YCOR,ZCOR,DHW,DTH,DTD,NSC,SIGMA,F
...
!
 ...
 IF(PEST%PE_REGULARISATION.EQ.1)THEN
  ...
  DO I=1,SIZE(PEST%PARAM)  !## row
   ...
   MSR%RJ=MSR%RJ+F*WW !PEST%PE_REGFACTOR
  ENDDO
 ENDIF
