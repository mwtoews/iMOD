...
CONTAINS
 ...
 INTEGER :: I,ITER,N,IFLAGS,ISTATUS,IEXCOD,ITYPE,IGRAD,ILIN,JGRAD,ILNSRCH,NCPU
 ...
 DO I=1,PBMAN%NLINESEARCH
  !## parameter
  RNL(I)=TRIM(DIR)//'\RUN_L#'//TRIM(ITOS(0))//'.BAT'; LPARAM(I)=I
 ENDDO

 !## set gradient-runbatch-files
 ...
 !## open files and get total number of observations - allocate memory for dh()
 MX=0; DO I=1,SIZE(PEST%MEASURES)
 ...
 DO 
  ...
  IF(PBMAN%CMDHIDE.EQ.1)IFLAGS=IFLAGS+PROCSILENT
  ...
  DO ILNSRCH=1,MXLNSRCH
   ...
   CALL IPEST_GLM_PROGRESS(ITER,ILIN,'L')
  ...
  !## update alphas
  CALL IPEST_GLM_NEXT(ITER)

  !## next cycle
  ITER=ITER+1
  !## stop maximum iterations reached
  IF(ITER.GT.PEST%PE_MXITER)EXIT
  ...
  NCPU=0; IGRAD=0; IPROC=0; ILNSRCH=0
  DO 
    ...
    IF(ITYPE.EQ.TIMEREXPIRED)THEN
     ...
     N=0; DO JGRAD=1,SIZE(RNG)
      ...
      IF(ISTATUS.EQ.0)THEN
       ...
       !## write echo
       CALL IPEST_GLM_PROGRESS(ITER,JGRAD,'P')
...
!  ENDIF
 ...
 IPARAM=0
 DO I=1,SIZE(PEST%PARAM)
  IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LE.0)CYCLE
  DF1=PEST%PARAM(I)%PDELTA; IPARAM=IPARAM+1
  DO J=1,MSR%NOBS
 ...
 TS=SUM(ABS(S)); IPARAM=0; DO I=1,SIZE(PEST%PARAM)
  IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LE.0)CYCLE
  IPARAM=IPARAM+1; IF(TS.NE.0.0D0)S(IPARAM)=S(IPARAM)/TS
 ENDDO
 S=ABS(S)*100.0D0
 ...
 IPARAM=0; DO I=1,SIZE(PEST%PARAM)
  IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LE.0)CYCLE
  IPARAM=IPARAM+1; WRITE(SLINE,'(E15.7)') S(IPARAM); BLINE=TRIM(BLINE)//TRIM(SLINE)
 ENDDO
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  CALL IPEST_GLM_GETBOUNDARY(I,IBND,P1,P2,PMIN,PMAX); PEST%PARAM(I)%IBND=IBND
 ENDDO

 !## "freeze"-insensitive parameters
 IPARAM=0; DO I=1,SIZE(PEST%PARAM)
  IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LE.0)CYCLE
  IPARAM=IPARAM+1; IF(S(IPARAM).LT.PEST%PE_SENS)PEST%PARAM(I)%PACT=-1
 ENDDO
...
! IF(LSENS)CALL PESTWRITESTATISTICS_FOSM(NP,COV)
 ...
 DO IP1=1,SIZE(PEST%PARAM); IF(PEST%PARAM(IP1)%PACT.NE.0)PEST%PARAM(IP1)%PACT=1; ENDDO
 ...
 IF(LCHECK)THEN
  ...
  DO IP1=1,SIZE(PEST%PARAM)
   ...
   !## inactive parameter
   IF(PEST%PARAM(IP1)%PACT.NE.1)CYCLE !.OR.PEST%PARAM(IP1)%PIGROUP.LE.0)CYCLE
...
! IF(PEST_IREGULARISATION.EQ.1)CALL PEST_GETQPP(NP,.TRUE.,idf)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_GLM_PROGRESS(ITER,IGRAD,CTYPE)
 !#####=================================================================
 IMPLICIT NONE
 ...
 DO I=1,SIZE(PEST%PARAM)
  IF(PEST%PARAM(I)%PLOG)THEN
   IF(TRIM(CTYPE).EQ.'P')THEN
    X(I)=EXP(PEST%PARAM(I)%GALPHA(IGRAD))
   ELSEIF(TRIM(CTYPE).EQ.'L')THEN
    X(I)=EXP(PEST%PARAM(I)%LALPHA(IGRAD))
   ENDIF
  ELSE
   IF(TRIM(CTYPE).EQ.'P')THEN
    X(I)=PEST%PARAM(I)%GALPHA(IGRAD)
   IF(TRIM(CTYPE).EQ.'L')THEN
    X(I)=PEST%PARAM(I)%LALPHA(IGRAD)
   ENDIF
  ENDIF
 ENDDO

 IF(IGRAD.EQ.0.AND.ILNSRCH.EQ.1)THEN
 ...
 IF(IGRAD.EQ.0)ALLOCATE(MSR%DH(0:SIZE(RNG),M))
 ALLOCATE(MSR%W (M)) 
 ALLOCATE(MSR%X (M)) 
 ALLOCATE(MSR%Y (M)) 
 ALLOCATE(MSR%L (M)) 
