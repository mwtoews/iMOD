...
CONTAINS
 ...
 ITER=0; DO 
  ...
  !## run and process all lambda-testing
  WRITE(IUPESTPROGRESS,'(/A5,4A15)') 'GD','PARAMETER','FACTOR','TOT_J','RED_J'
 ...
 !## start processes
  ...
  NDONE=IPEST_GLM_EVALUATE(IBATCH,ITER,NCPU,DIR,RN,RT,RPARAM,LAMBDA)
 ...
 !###====================================================================
 INTEGER FUNCTION IPEST_GLM_EVALUATE(IBATCH,ITER,NCPU,DIR,RN,RT,RPARAM,LAMBDA)
 !###====================================================================
 IMPLICIT NONE
   ...
   N=0; DO JGRAD=1,SIZE(RN) !G)
    ...
    IF(ISTATUS(JGRAD).EQ.0)THEN
     ...
     IF(ISTATUS(JGRAD).EQ.0)THEN
      ...
      ENDIF
      !## write echo
      CALL IPEST_GLM_PROGRESS(ITER,JGRAD,RT,LAMBDA); FLUSH(IUPESTPROGRESS)
     ENDIF
 ...
 WRITE(IUPESTPROGRESS,'(/A)') 'New Lambda '//TRIM(RTOS(LAMBDA,'F',7))
...
! FLUSH(IUPESTOUT)
 ...
 !###====================================================================
 SUBROUTINE IPEST_GLM_EIGDECOM(IBATCH,JQJ,EIGW,EIGV,NP,LPRINT,MARQUARDT) 
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NP,IBATCH
 ...
 IF(LPRINT)THEN
  WRITE(IUPESTOUT,'(A,E15.7)') 'Determinant JQJ+'//TRIM(RTOS(MARQUARDT,'F',7))//'*DIAG(JQJ) = ',DET
  IF(DET.LT.0.0D0)STOP 'DET.LT.0'
 ENDIF
...
!
 ...
 IF(CTYPE.EQ.'P')THEN
  ...
  X=PEST%PARAM(IGRAD)%GALPHA(IGRAD)
  IF(PEST%PARAM(IGRAD)%PLOG)X=EXP(X)
 ELSEIF(CTYPE.EQ.'L')THEN
  !## present lambda
  X=LAMBDA*PBMAN%LAMBDA_TEST(IGRAD) 
 ENDIF

 IF(CTYPE.EQ.'P')THEN
  WRITE(IUPESTPROGRESS,'(I5,A15,3F15.7)') IGRAD,PEST%PARAM(IGRAD)%ACRONYM  ,X,MSR%TJ,MSR%TJ-MSR%PJ
 ELSEIF(CTYPE.EQ.'L')THEN
  IF(ITER.EQ.0)THEN
   WRITE(IUPESTPROGRESS,'(I5,30X,F15.7)') 0,MSR%TJ
  ELSE
   WRITE(IUPESTPROGRESS,'(I5,A15,3F15.7)') IGRAD,'LAMBDA'//TRIM(ITOS(IGRAD)),X,MSR%TJ,MSR%TJ-MSR%PJ
  ENDIF
 ENDIF
