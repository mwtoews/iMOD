...
CONTAINS
 ...
 ITER=0; DO 
  ...
  WRITE(IUPESTPROGRESS,'(/A5,4A15,15X,A15)') 'LS','PARAMETER','LAMBDA','TOT_J','RED_J','SIM_TIME(SEC)' 
  ...
  ELSE
   IUPESTRESIDUAL=0
  ENDIF
  CALL IPEST_GLM_RUNMODELS(IBATCH,RNL,LPARAM,'L',DIR,MNAME,ITER,LAMBDA)
  ...
  !## run and process all lambda-testing
  WRITE(IUPESTPROGRESS,'(/A5,6A15)') 'GD','PARAMETER','FACTOR','TOT_J','RED_J','CUR_PARAMETER','SIM_TIME(SEC)' 
 ...
 ELSEIF(RT.EQ.'R')THEN
  WRITE(IUPESTOUT,'(/A/)') ' *** Realization Cycle ***'; WRITE(*,'(/A/)') ' *** Realization Cycle ***'
 ENDIF
 ...
 !## start processes
  ...
  NDONE=IPEST_GLM_EVALUATE(IBATCH,ITER,NCPU,DIR,RN,RT,RPARAM,LAMBDA,LRESTART)
 ...
 !###====================================================================
 INTEGER FUNCTION IPEST_GLM_EVALUATE(IBATCH,ITER,NCPU,DIR,RN,RT,RPARAM,LAMBDA,LRESTART)
 !###====================================================================
 IMPLICIT NONE
  ...
  IF(ITYPE.EQ.TIMEREXPIRED)THEN
   N=0; DO JGRAD=1,SIZE(RN) 
    ...
    IF(ISTATUS(JGRAD).EQ.0)THEN
     ...
     IF(ISTATUS(JGRAD).EQ.0)THEN
      IF(.NOT.IPEST_GLM_GETJ(DIR,JGRAD,RPARAM(JGRAD),RT,IBATCH))RETURN 
 ...
 IF(.NOT.IPEST_GLM_GETJ(DIR,IJ,LPARAM(IJ),'L',IBATCH))RETURN
 ...
 REAL(KIND=DP_KIND) :: DJ1,DJ2,DP,WW,TS,DF1,EIGWTHRESHOLD,W,DH1,DH2,MARQUARDT,P1,P2,PMIN,PMAX
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  IPARAM=IPARAM+1
  DO J=1,MSR%NOBS
   W=MSR%W(J); DH1=MSR%DHG(IPARAM,J); DH2=MSR%DHL(0,J) !; JAC(IPARAM,J)=W*((DH1-DH2)/DF1);
   S(IPARAM)=S(IPARAM)+(W*((DH1-DH2)/DF1))
  ENDDO
 ENDDO
 DO I=1,NP; S(I)=S(I)/DBLE(MSR%NOBS); ENDDO
 
! !## check linear dependency of dhg() - is removed by eigenvalue decomposition
...
! IF(ABS(P1-PMIN).LE.XPBND)IBND=-1; IF(ABS(PMAX-P1).LE.XPBND)IBND= 1
 ...
 IF(ITER.EQ.0)THEN
  WRITE(IUPESTEFFICIENCY,'(4(F15.7,1X))') MSR%TJ,MSR%TJ-MSR%RJ,MSR%RJ,REAL(SQRT(MSR%TJ))/REAL(MSR%NOBS,8)
 ELSE
  WRITE(IUPESTEFFICIENCY,'(8(F15.7,1X))') MSR%TJ,MSR%TJ-MSR%RJ,MSR%RJ,REAL(SQRT(MSR%TJ))/REAL(MSR%NOBS,8),C1,C2,C3
 ENDIF
 ...
 IF(.NOT.LCOV)THEN
  IF(.TRUE.)THEN !PEST%PE_REGULARISATION.EQ.0)THEN
   !## levenberg
   DO I=1,NP
...
! ENDIF
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z,H,WW,MC,MM,DHH,XCOR,YCOR,ZCOR,DHW,DTH,DTD,NSC,SIGMA,F,DRES,D
...
!    WRITE(*,'(I10,5F15.7)') II,H,Z,MSR%E(IERROR,II),MSR%W(II),MSR%TJ
...
!
 ...
 !## sensitivity
 IF(CTYPE.EQ.'P')THEN
  WRITE(IUPESTPROGRESS,'(I5,A15,F15.3,2F15.3,2F15.7)') IGRAD,PEST%PARAM(IGRAD)%ACRONYM  ,X1,MSR%TJ,MSR%TJ-MSR%PJ,X2,ET
 !## lambda testing
 ELSEIF(CTYPE.EQ.'L')THEN
  IF(ITER.EQ.0)THEN
   WRITE(IUPESTPROGRESS,'(I5,30X,F15.3,30X,F15.7)') 0,MSR%TJ,ET
  ELSE
   WRITE(IUPESTPROGRESS,'(I5,A15,F15.3,2F15.3,15X,F15.7)') IGRAD,'LAMBDA'//TRIM(ITOS(JGRAD)),X1,MSR%TJ,MSR%TJ-MSR%PJ,ET
  ENDIF
 !## realization
 ELSEIF(CTYPE.EQ.'R')THEN
  IF(ITER.EQ.0)THEN
   WRITE(IUPESTPROGRESS,'(I5,A15,2F15.3,30X,F15.7)') IGRAD,'REALS'//TRIM(ITOS(JGRAD)),X1,MSR%TJ,ET
  ELSE
   WRITE(IUPESTPROGRESS,'(I5,A15,2F15.3,F15.3,15X,F15.7)') IGRAD,'REALS'//TRIM(ITOS(JGRAD)),X1,MSR%TJ,MSR%TJ-JE(IGRAD,ITER-1),ET
  ENDIF
