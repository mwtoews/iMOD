...
CONTAINS
 ...
 IF(PBMAN%PTEST.EQ.1)THEN; CALL IPEST_GLM_TEST(DIR); STOP; ENDIF
 ...
 WRITE(IUPESTOUT,'(99A15)') ('P('//TRIM(ITOS(I))//'),',I=1,SIZE(RNG)),'MEASURE_J','PARAMETER_J','TOTAL_J','MEAN_J','GOODNESS_OF_FIT'
 IUPESTRESIDUAL=0
...
MAINLOOP: DO
  ...
  WRITE(IUPESTOUT,'(99F15.7)') (PVAL(I),I=1,SIZE(IP)),(XSTAT(I),I=1,5)
 ...
 !## create idf
  ...
  DO II=1,3
   ...
   DO I=IDF%NROW,1,-1
    DO J=1,IDF%NCOL
     READ(IUPESTOUT,'(99F15.7)') (PVAL(K),K=1,SIZE(IP)),(XSTAT(K),K=1,5)
     IDF%X(J,I)=XSTAT(II)
    ENDDO
   ENDDO
