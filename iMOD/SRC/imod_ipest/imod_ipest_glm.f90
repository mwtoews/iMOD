...
! ENDIF
 ...
 IF(PBMAN%IFORMAT.EQ.3.AND.PBMAN%IPESTP.EQ.1)THEN
  CALL IPEST_GLM_CREATE_RESIDUALSFILES(DIR,IGRAD,IPARAM,CTYPE,IBATCH,IGRAD,MNAME) 
...
!
 ...
 !###====================================================================
 SUBROUTINE IPEST_GLM_CREATE_RESIDUALSFILES(DIR,IGRAD,IPARAM,CTYPE,IBATCH,ILAMBDA,MNAME) 
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,CTYPE,MNAME
 INTEGER,INTENT(IN) :: IGRAD,IPARAM,IBATCH,ILAMBDA
 INTEGER :: N1,N2,I,IU,JU,KU,ISUB,NPER,NOBS,IOBS,IPER,N
 ...
 N=0; DO ISUB=1,PBMAN%NSUBMODEL
  ...
  IF(NOBS.GT.0)THEN
   ...
   !## read measurements
   DO IOBS=1,NOBS
    READ(KU,*) ONAME(IOBS),X(IOBS),Y(IOBS)
    READ(KU,*)
    DO IPER=1,NPER
   ...
   DO IOBS=1,NOBS
    ...
    ELSE
     FNAME=ONAME(IOBS)
     WRITE(IU,'(A)') TRIM(RTOS(X(IOBS),'F',3))//','//TRIM(RTOS(Y(IOBS),'F',3))//','//TRIM(FNAME)//','//TRIM(RTOS(H(IOBS,IPER)%W,'F',3))
