...
CONTAINS
 ...
 DO 
  ...
  DO ILNSRCH=1,MXLNSRCH
   ...
   CALL IPEST_GLM_PROGRESS(ITER,IGRAD,ILNSRCH,'L')
  ...
  DO 
   ...
   DO
    ...
    !## clear error
    I=WINFOERROR(1); CALL IOSCOMMAND(TRIM(RNG(IGRAD)),IFLAGS=IFLAGS,IDPROC=IPROC(:,IGRAD))
    IF(WINFOERROR(1).EQ.ERROSCOMMAND)THEN
     WRITE(*,'(/A/)') 'FAILED TO START MODEL P#'//TRIM(ITOS(GPARAM(IGRAD)))
     STOP
    ENDIF
    !## all started
...
!  ENDIF
 ...
 INTEGER :: I,II,J,K,NP,MP,IP1,NE,ISING,ITRIES,IBND,IPARAM
...
! IF(PEST_IREGULARISATION.EQ.1)CALL PEST_GETQPP(NP,.TRUE.,idf)
 ...
 IF(ITER.EQ.0)THEN
  WRITE(BLINE,'(3I5, F15.2)') ITER,IGRAD,ILNSRCH,MSR%TJ
 ELSE
  WRITE(BLINE,'(3I5,2F15.2)') ITER,IGRAD,ILNSRCH,MSR%TJ,MSR%TJ-MSR%PJ
 ENDIF
 DO I=1,SIZE(PEST%PARAM)
