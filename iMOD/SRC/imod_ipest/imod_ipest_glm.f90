...
CONTAINS
 ...
 ITER=0; DO 
  ...
  IUPESTRESIDUAL=0; CALL IPEST_GLM_RUNMODELS(IBATCH,RNL,LPARAM,'L',DIR,MNAME,ITER)
  
  !## initial lambda
  IF(ITER.GT.0)THEN
   !## determine best lambda and continue, otherwise determine another set of lambda's
   IF(.NOT.IPEST_GLM_NEXT(IBATCH,ITER,DIR))THEN
    LAMBDA=LAMBDA*GAMMA; CYCLE
   ENDIF
   LAMBDA=LAMBDA/GAMMA
  ELSE
  ...
  !## run and process all lambda-testing
  IUPESTRESIDUAL=0; CALL IPEST_GLM_RUNMODELS(IBATCH,RNG,GPARAM,'P',DIR,MNAME,ITER)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_GLM_RUNMODELS(IBATCH,RN,RPARAM,RT,DIR,MNAME,ITER)
 !#####=================================================================
 IMPLICIT NONE
 ...
 !## start processes
  DO
   ...
   !## clear error
   I=WINFOERROR(1); IDPROC=0; CALL IOSCOMMAND(TRIM(RN(IGRAD)),IFLAGS=IFLAGS,IDPROC=IDPROC); IPROC(:,IGRAD)=IDPROC
  ...
  NDONE=IPEST_GLM_EVALUATE(IBATCH,NCPU,DIR,ITER,RN,RT,RPARAM)
 ...
 !###====================================================================
 INTEGER FUNCTION IPEST_GLM_EVALUATE(IBATCH,NCPU,DIR,ITER,RN,RT,RPARAM)
 !###====================================================================
 IMPLICIT NONE
  ...
  IF(ITYPE.EQ.TIMEREXPIRED)THEN
   N=0; DO JGRAD=1,SIZE(RN) !G)
    ...
    IF(ISTATUS(JGRAD).EQ.0)THEN
     ...
     IF(ISTATUS(JGRAD).EQ.0)THEN
      IF(.NOT.IPEST_GLM_GETJ(DIR,JGRAD,RPARAM(JGRAD),RT,IBATCH))RETURN !,TNSC))RETURN
      IF(RT.EQ.'L')THEN
      ...
      ENDIF
      !## write echo
      CALL IPEST_GLM_PROGRESS(ITER,JGRAD,RT); FLUSH(IUPESTPROGRESS)
     ENDIF
 ...
 M=MAX(M,N); ALLOCATE(ISTATUS(M),IPROC(2,M))
 ...
 !#####=================================================================
 LOGICAL FUNCTION IPEST_GLM_NEXT(IBATCH,ITER,DIR)
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ITER,IBATCH
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 REAL(KIND=DP_KIND) :: IMPROVEMENT,F,GUPDATE,MJ
 INTEGER :: I,J,ILOG,IJ
 ...
 IF(.NOT.IPEST_GLM_GETJ(DIR,IJ,LPARAM(IJ),'L',IBATCH))RETURN !,TNSC))RETURN
...
! ENDIF
 ...
 IF(IMPROVEMENT.LE.PEST%PE_STOP)THEN
  CALL IPEST_GLM_ERROR(IBATCH,'Pest iteration terminated decrease objective function ('//TRIM(RTOS(100.0D0*IMPROVEMENT,'G',7))// &
       '%) > PEST_JSTOP ('//TRIM(RTOS(100.0D0*PEST%PE_STOP,'G',7))//'%)'); RETURN
 ENDIF
...
! !## copy jqj to jqjb for covariance - furtherup
! IF(LCOV)THEN
...
!
 ...
 IF(IGRAD.EQ.1)THEN
  IF(CTYPE.EQ.'L')WRITE(BLINE,'(2A5,2A15)') 'IT','LS','TOT_J','RED_J'
  IF(CTYPE.EQ.'P')WRITE(BLINE,'(2A5,2A15)') 'IT','GD','TOT_J','RED_J'
  N=0; DO J=1,SIZE(PEST%PARAM)
  ...
  N=0; DO J=1,SIZE(PEST%PARAM)
   ...
   ENDIF
  ENDDO
  WRITE(IUPESTPROGRESS,'(40X,99999A15)') (CTMP(I),I=1,N) 
  N=0; DO J=1,SIZE(PEST%PARAM)
   IF(ABS(PEST%PARAM(J)%PACT).EQ.1)THEN
 ...
 IF(ITER.EQ.0)THEN
  WRITE(BLINE,'(2I5, F15.2,14X,A1)') ITER,IGRAD,MSR%TJ,'-'
 ELSE
  WRITE(BLINE,'(2I5,2F15.2)') ITER,IGRAD,MSR%TJ,MSR%TJ-MSR%PJ
 ENDIF
 N=0; DO I=1,SIZE(PEST%PARAM)
 ...
 WRITE(IUPESTPROGRESS,'(A40,99999A15)') TRIM(BLINE),(CTMP(I),I=1,N)
