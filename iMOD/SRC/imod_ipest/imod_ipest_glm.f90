...
CONTAINS
 ...
 IF(ABS(PEST%PE_KTYPE).EQ.1)THEN
  WRITE(*,'(/1X/)') 'It is not adviced to use Simple Kriging for PilotPoint interpolation, rather use Ordinary Kriging'
  WRITE(*,'(1X,A$)') 'Do you want to continue ?'
...
!   ENDIF
 ...
 INTEGER :: I,J,K,IP1,IP2,II,N,M,ISING,IIU,IPARAM,JPARAM,JUPESTOUT
 ...
 IF(LSENS)THEN
  ...
  DO I=1,MSR%NOBS
   ...
   ENDDO
   WRITE(IUJACOBIAN,'(2(F10.2,A1),I10,A1,F10.2,A,A32,999(G15.7,A1))') MSR%X(I),',',MSR%Y(I),',',MSR%L(I),',', &
           MSR%W(I),',',TRIM(MSR%CLABEL(I))//',',(JQJ(J,1),',',J=1,N)
  ENDDO
  IF(M.EQ.1)THEN; WRITE(IUJACOBIAN,'(/44X,A32,999(G15.7,A1))') 'TOTAL,',(ABS(JQJ(J,1)),',',J=1,N)
  ELSE; WRITE(IUJACOBIAN,'(/44X,A32,999(G15.7,A1))') 'TOTAL,',(JQJ(J,2),',',J=1,N); ENDIF
  STOP
 ENDIF
...
2  IF(M.NE.L)THEN
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z,H,WW,MC,MM,DHH,XCOR,YCOR,ZCOR,XCROSS,DHW,DP,F
