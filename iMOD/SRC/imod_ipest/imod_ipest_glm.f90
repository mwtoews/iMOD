...
CONTAINS
 ...
 INTEGER :: I,ITER,N,M,IFLAGS,ISTATUS,IEXCOD,ITYPE,IGRAD,ILIN,JGRAD,ILNSRCH,NCPU,IU
 ...
 N=0 ; DO I=1,SIZE(PEST%PARAM); IF(PEST%PARAM(I)%PACT.EQ.1)N=N+1; ENDDO; CALL IPEST_GLM_ALLOCATE(N)
 ...
 !## allocate memory
 IF(.NOT.IPEST_GLM_ALLOCATEMSR(N))STOP
 ...
 DO 
  ...
  IFLAGS=PROCCMDPROC+PROCBLOCKED; IF(PBMAN%CMDHIDE.EQ.1)IFLAGS=IFLAGS+PROCSILENT
  ...
  DO ILNSRCH=1,MXLNSRCH
   ...
   IF(.NOT.IPEST_GLM_PST(DIR,ILIN,LPARAM(ILIN),'L'))THEN; WRITE(*,'(/A/)') 'ERROR CREATED PST1 FILE FOR L#'//TRIM(ITOS(LPARAM(ILIN))); STOP; ENDIF
   !## run model
   CALL IOSCOMMAND(TRIM(RNL(ILIN)),IFLAGS=IFLAGS)
  ...
  !## select correct linesearch result
  ILIN=-1
  !## update alphas
  CALL IPEST_GLM_NEXT(ITER,ILIN)

  !## next cycle
  ITER=ITER+1; IF(ITER.GT.PEST%PE_MXITER)EXIT

  !## executes on commandtool such that commands alike 'dir' etc. works
  IFLAGS=PROCCMDPROC; IF(PBMAN%CMDHIDE.EQ.1)IFLAGS=IFLAGS+PROCSILENT

  !## start finite difference approximation for parameters
  CALL WMESSAGETIMER(NCSECS,IREPEAT=1); ILNSRCH=0; IGRAD=0; NCPU=0; IPROC=0
  ...
  DO 
   ...
   DO
    ...
    !## define update in pst file
    IF(.NOT.IPEST_GLM_PST(DIR,IGRAD,GPARAM(IGRAD),'P'))THEN; WRITE(*,'(/A/)') 'ERROR CREATED PST1 FILE FOR P#'//TRIM(ITOS(GPARAM(IGRAD))); STOP; ENDIF
    CALL IOSCOMMAND(TRIM(RNG(IGRAD)),IFLAGS=IFLAGS,IDPROC=IPROC(:,IGRAD))
    IF(WINFOERROR(1).EQ.ERROSCOMMAND)THEN; WRITE(*,'(/A/)') 'FAILED TO START MODEL P#'//TRIM(ITOS(GPARAM(IGRAD))); STOP; ENDIF
    !## all started
    ...
    IF(ITYPE.EQ.TIMEREXPIRED)THEN
     ...
     N=0; DO JGRAD=1,SIZE(RNG)
      ...
      IF(ISTATUS.EQ.0)THEN
       !## error occured 
       IF(IEXCOD.NE.0)THEN; WRITE(*,'(/A/)') 'ERROR OCCURED RUNNING MODEL P#'//TRIM(ITOS(GPARAM(JGRAD))); STOP; ENDIF
       !## set part of objective function
       CALL IPEST_GLM_GETJ(DIR,JGRAD,GPARAM(JGRAD),'P')
       !## write echo
       CALL IPEST_GLM_PROGRESS(ITER,JGRAD,ILNSRCH,'P')
 ...
 ALLOCATE(PEST%PARAM(IP)%GALPHA(SIZE(RNG))); PEST%PARAM(IP)%GALPHA=0.0D0
 ALLOCATE(PEST%PARAM(IP)%LALPHA(SIZE(RNL))); PEST%PARAM(IP)%LALPHA=0.0D0

 PEST%PARAM(IP)%ACRONYM=ADJUSTR(PEST%PARAM(IP)%ACRONYM)
 
 !## inactive for gradient computation
 IF(PEST%PARAM(IP)%PIGROUP.LT.0)PEST%PARAM(IP)%PACT=2
 ...
 !#####=================================================================
 LOGICAL FUNCTION IPEST_GLM_PST(DIR,IGRAD,IPARAM,CTYPE)
 !#####=================================================================
 IMPLICIT NONE
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  IF(PEST%PARAM(I)%PLOG.EQ.0)THEN
   LINE=TRIM(ITOS(MIN(1,PEST%PARAM(I)%PACT)))    //','// &
 ...
 CALL IOSRENAMEFILE(TRIM(DIR)//'\MODELINPUT\'//TRIM(MNAME)//'_'//TRIM(CTYPE)//'#'//TRIM(ITOS(IPARAM))//'.PST1_', &
                    TRIM(DIR)//'\MODELINPUT\'//TRIM(MNAME)//'_'//TRIM(CTYPE)//'#'//TRIM(ITOS(IPARAM))//'.PST1')
 ...
 !#####=================================================================
 SUBROUTINE IPEST_GLM_NEXT(ITER,ILINESEARCH)
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ITER,ILINESEARCH
 REAL(KIND=DP_KIND) :: IMPROVEMENT,F,GUPDATE
 INTEGER :: I,J,ILOG

 !## set current dh on correct position
 DO I=1,MSR%NOBS; MSR%DH(0,I)=MSR%DH(ILINESEARCH,I); ENDDO

 !## not necessary for first cycle
 IF(ITER.EQ.0)THEN; MSR%PJ=MSR%TJ; RETURN; ENDIF
...
!  ENDIF
 ...
 INTEGER :: I,II,J,K,NP,MP,IP1,NE,ISING,ITRIES,IBND,IPARAM,JPARAM
 ...
 !## sensitivity
 NP=SIZE(RNG); IF(.NOT.ALLOCATED(S)) ALLOCATE(S(NP)); S=0.0

 IPARAM=0
 DO I=1,SIZE(PEST%PARAM)
  IF(PEST%PARAM(I)%PACT.NE.1)CYCLE; DF1=PEST%PARAM(I)%PDELTA; IPARAM=IPARAM+1
  DO J=1,MSR%NOBS
   W=MSR%W(J); DH1=MSR%DH(IPARAM,J); DH2=MSR%DH(0,J); S(IPARAM)=S(IPARAM)+W*((DH1-DH2)/DF1)
  ENDDO
 ENDDO
 ...
 DO
  ...
  NP=0; DO I=1,SIZE(PEST%PARAM); IF(PEST%PARAM(I)%PACT.EQ.1)NP=NP+1; ENDDO
  IF(NP.EQ.0)THEN; WRITE(*,'(/A/)') 'NO PARAMETERS LEFT THAT ARE SENSITIVE, PROCESS STOPPED!'; STOP; ENDIF
  ...
  DO IP1=1,SIZE(PEST%PARAM)  !## row
   ...
   I=I+1; IPARAM=IPARAM+1
   DO J=1,MSR%NOBS
    DH1=MSR%DH(IPARAM,J); DH2=MSR%DH(0,J)
  ...
  ELSE
   ...
   DO I=1,MSR%NOBS    !## row
    ...
    DO IP1=1,SIZE(PEST%PARAM)
     IF(PEST%PARAM(IP1)%PACT.NE.1)CYCLE
     DF1=PEST%PARAM(IP1)%PDELTA
     J=J+1; IPARAM=IPARAM+1
     DH1=MSR%DH(IPARAM,I); DH2=MSR%DH(0,I)
     DJ1=(DH1-DH2)/DF1
...
! IF(LSENS)CALL PESTWRITESTATISTICS_FOSM(NP,COV)
 ...
 DO IP1=1,SIZE(PEST%PARAM); IF(PEST%PARAM(IP1)%PACT.EQ.-1)PEST%PARAM(IP1)%PACT=1; ENDDO
...
!     ENDDO
    ...
    ELSE
     ...
     F=MIN(AF,F); WRITE(IUPESTOUT,'(A,2F10.3)') 'AF,F',AF,F
...
!   ENDIF
 ...
 INTEGER :: I,J,K,IP1,IP2,II,N,ISING,IIU,IPARAM,JPARAM,JUPESTOUT
 ...
 DO IP1=1,SIZE(PEST%PARAM)                !## row
  IF(PEST%PARAM(IP1)%PACT.NE.1)CYCLE
  DF1=PEST%PARAM(IP1)%PDELTA
  I=I+1; II=0; IPARAM=IPARAM+1; JPARAM=0
  DO IP2=1,SIZE(PEST%PARAM)  !## column
   IF(PEST%PARAM(IP2)%PACT.NE.1)CYCLE
   DF2=PEST%PARAM(IP2)%PDELTA
   II=II+1; JPARAM=JPARAM+1
   DO J=1,MSR%NOBS
    DH1=MSR%DH(IPARAM,J); DH2=MSR%DH(0,J)
    DJ1=(DH1-DH2)/DF1; DH1=MSR%DH(JPARAM,J)
...
2  IF(M.NE.L)THEN
 ...
 !###====================================================================
 SUBROUTINE IPEST_GLM_GETJ(DIR,IGRAD,IPARAM,CTYPE)
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,CTYPE
 INTEGER,INTENT(IN) :: IGRAD,IPARAM
 ...
 DO I=1,SIZE(PEST%MEASURES)
  ...
  IF(TRIM(CTYPE).EQ.'L'.AND.IGRAD.EQ.1)THEN
  ...
  IF(NR.GT.0)THEN
   IF(IEXT.GT.0)WRITE(IUPESTOUT,'(A)') 'MEAN Cross-Correlation: '// &
          TRIM(RTOS(REAL(XCROSS)/DBLE(NR),'F',7))//' (n='//TRIM(ITOS(NR))//')'
  ENDIF
...
! IF(PEST_IREGULARISATION.EQ.1)CALL PEST_GETQPP(NP,.TRUE.,idf)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_GLM_PROGRESS(ITER,IGRAD,ILNSRCH,CTYPE)
 !#####=================================================================
 IMPLICIT NONE
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  ELSEIF(TRIM(CTYPE).EQ.'L')THEN
   X(I)=PEST%PARAM(I)%LALPHA(ILNSRCH)
  ENDIF
  IF(PEST%PARAM(I)%PLOG)X(I)=EXP(X(I))
 ENDDO

 IF(ILNSRCH.EQ.1.AND.TRIM(CTYPE).EQ.'L')THEN
  ...
  DO J=1,SIZE(PEST%PARAM)
   ...
   ENDIF
  ENDDO
  WRITE(IUPESTPROGRESS,'(45X,A)') TRIM(BLINE) 
  BLINE=''
  DO J=1,SIZE(PEST%PARAM)
   IF(PEST%PARAM(J)%PACT.EQ.1.AND.PEST%PARAM(J)%PIGROUP.GT.0)THEN
    WRITE(SLINE,'(A15)') PEST%PARAM(J)%ACRONYM
 ...
 ELSE
  WRITE(BLINE,'(3I5,2E15.7)') ITER,IGRAD,ILNSRCH,MSR%TJ,MSR%TJ-MSR%PJ
 ENDIF
 DO I=1,SIZE(PEST%PARAM)
 ...
 N1=-1*SIZE(RNL); N2=SIZE(RNG)
 ALLOCATE(MSR%DH(N1:N2,M),STAT=IOS); IF(IOS.NE.0)THEN; WRITE(*,'(/A/)') 'CANNOT ALLOCATE MEMORY FOR MSR%DH';     RETURN; ENDIF
