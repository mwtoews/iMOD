...
!
 ...
 !## sensitivity
 IF(CTYPE.EQ.'P')THEN
  WRITE(IUPESTPROGRESS,'(I5,A15,F15.3,2F15.3,30X,2F15.7)') IGRAD,PEST%PARAM(IGRAD)%ACRONYM,X1,MSR%TJ,MSR%TJ-MSR%PJ,X2,ET
 !## lambda testing
 ELSEIF(CTYPE.EQ.'L')THEN
  IF(ITER.EQ.0)THEN
   WRITE(IUPESTPROGRESS,'(I5,30X,F15.3,30X,3F15.7)') 0,MSR%TJ,MSR%GOF(JGRAD),MSR%NSC(JGRAD),ET
  ELSE
   WRITE(IUPESTPROGRESS,'(I5,A15,F15.3,2F15.3,15X,3F15.7)') IGRAD,'LAMBDA'//TRIM(ITOS(JGRAD)),X1,MSR%TJ,MSR%TJ-MSR%PJ, &
                                        MSR%GOF(JGRAD),MSR%NSC(JGRAD),ET
  ENDIF
 !## realization
 ELSEIF(CTYPE.EQ.'R')THEN
  IF(ITER.EQ.0)THEN
   WRITE(IUPESTPROGRESS,'(I5,A15,2F15.3,30X,3F15.7)')       IGRAD,'REALS'//TRIM(ITOS(JGRAD)),X1,MSR%TJ,MSR%GOF(JGRAD),MSR%NSC(JGRAD),ET
  ELSE
   WRITE(IUPESTPROGRESS,'(I5,A15,2F15.3,F15.3,15X,3F15.7)') IGRAD,'REALS'//TRIM(ITOS(JGRAD)),X1,MSR%TJ,MSR%TJ-JE(IGRAD,ITER-1),&
                                        MSR%GOF(JGRAD),MSR%NSC(JGRAD),ET
  ENDIF
