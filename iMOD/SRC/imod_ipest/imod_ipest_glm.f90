...
!  X1=
 ...
 !## sensitivity
 IF(CTYPE.EQ.'P')THEN
  WRITE(IUPESTPROGRESS,'(I5,A15,F15.7,2F15.3,2F15.7)') IGRAD,PEST%PARAM(IGRAD)%ACRONYM  ,X1,MSR%TJ,MSR%TJ-MSR%PJ,X2,ET
 !## lambda testing
 ELSEIF(CTYPE.EQ.'L')THEN
  IF(ITER.EQ.0)THEN
   WRITE(IUPESTPROGRESS,'(I5,30X,F15.3,30X,F15.7)') 0,MSR%TJ,ET
  ELSE
   WRITE(IUPESTPROGRESS,'(I5,A15,F15.7,2F15.3,15X,F15.7)') IGRAD,'LAMBDA'//TRIM(ITOS(JGRAD)),X1,MSR%TJ,MSR%TJ-MSR%PJ,ET
  ENDIF
 !## realization
 ELSEIF(CTYPE.EQ.'R')THEN
  IF(ITER.EQ.0)THEN
  ...
  ELSE
   WRITE(IUPESTPROGRESS,'(I5,A15,15X,F15.3,F15.7,15X,F15.7)') IGRAD,'REALS'//TRIM(ITOS(JGRAD)),MSR%TJ,MSR%TJ-JE(IGRAD,ITER-1),ET
  ENDIF
