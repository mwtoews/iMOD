...
MAINLOOP: DO 
 ...
 INTEGER :: I,J,K,ILAY,ISUB,IROW,ICOL,NC,NCOL,NROW,IPER
 REAL(KIND=DP_KIND) :: FCT,F
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:,:) :: XORG,XADJ,FRAC
 INTEGER,POINTER,DIMENSION(:,:) :: IDX
 CHARACTER(LEN=2),DIMENSION(7) :: IPTYPE
 CHARACTER(LEN=3),DIMENSION(7) :: FPTYPE
 CHARACTER(LEN=4),DIMENSION(7) :: SVFILE
 INTEGER,DIMENSION(7) :: NCLIST
 CHARACTER(LEN=256) :: FNAME
 LOGICAL :: LEX
 
 IPTYPE=['KH ' ,'VA'  ,'SC'  ,'SY'  ,'DC'  ,'RC'  ,'GC'  ]
 FPTYPE=['K'   ,'K33' ,'SS'  ,'SY'  ,'DRN' ,'RIV' ,'GHB' ]
 SVFILE=['NPF6','NPF6','STO6','STO6','DRN6','RIV6','GHB6']
 NCLIST=[     0,     0,     0,     0,    2,     4,     2 ]
   ...
   SELECT CASE (IPTYPE(I))
    ...
    CASE ('DC','RC','GC')
     ...
     DO IPER=1,PRJNPER
        ...
        DO K=1,SIZE(IDX,2)
         ...
         IF(F.GT.0.0D0)THEN
          IF(IDX(5,K).EQ.0)THEN
           XADJ(2,K)=XORG(2,K)*F*FCT
          ELSE
           !## modify current package for this zone
           XADJ(2,K)=XADJ(2,K)+XORG(2,K)*F*FCT
          ENDIF
          IDX(5,K)=IDX(5,K)+1
         ENDIF
        ENDDO      
      ...
      FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELINPUT\'//SVFILE(I)//'\'//TRIM(FPTYPE(I))//'_T'//TRIM(ITOS(IPER))//'_'//RT//'#'//TRIM(ITOS(IPARAM))//'.ARR'
  ...
  IF(NOBS.GT.0)THEN
   ...
   DO IOBS=1,NOBS
    ...
    ELSE
     ...
     WRITE(JU,'(A)') 'MEASURED,-9999'
     WRITE(JU,'(A)') 'COMPUTED,-9999'
     DO IPER=1,NPER
      IF(H(IOBS,IPER)%ITIME.EQ.0)CYCLE
      WRITE(JU,'(A)') TRIM(ITOS_DBL(H(IOBS,IPER)%ITIME))//','//TRIM(RTOS(H(IOBS,IPER)%H,'F',3))//','// &
                      TRIM(RTOS(H(IOBS,IPER)%M,'F',3))
     ENDDO
     CLOSE(JU)
    ENDIF
   ENDDO
