...
MAINLOOP: DO 
 ...
 ELSE
  ...
  N=PBMAN%NLINESEARCH/2; IF(MOD(PBMAN%NLINESEARCH,2).NE.0)N=N+1
  IF(IJ.LT.N)THEN
   LAMBDA=1.0D0/PBMAN%LAMBDA_TEST(PBMAN%NLINESEARCH)*LAMBDA
  ELSEIF(IJ.GT.N)THEN
...
! LAMBDA=1.0D0/PBMAN%LAMBDA_TEST(PBMAN%NLINESEARCH)*LAMBDA
 ...
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:,:) :: XORG,XADJ,FRAC
 ...
 NODATA=HUGE(1.0) !3.402823466385289E+038
 DO IROW=1,NROW
