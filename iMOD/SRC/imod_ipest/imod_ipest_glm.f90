...
CONTAINS
 ...
 INTEGER :: I,ITER,N,M,IFLAGS,ISTATUS,IEXCOD,ITYPE,IGRAD,ILIN,JGRAD,NCPU,IU
 ...
 BLINE=''; DO I=1,SIZE(PEST%PARAM)
  IF(ABS(PEST%PARAM(I)%PACT).EQ.1)THEN
   WRITE(SLINE,'(3X,A2,2I5.5)') PEST%PARAM(I)%PPARAM,PEST%PARAM(I)%PILS,PEST%PARAM(I)%PIZONE
   BLINE=TRIM(BLINE)//TRIM(SLINE)
  ENDIF
 ENDDO
 WRITE(IUPESTSENSITIVITY,'(10X,A)') TRIM(BLINE) 
...
!  PROCCMDPROC+
  ...
  !## executes on commandtool such that commands alike 'dir' etc. works
  IFLAGS=0; IF(PBMAN%CMDHIDE.EQ.1)IFLAGS=IFLAGS+PROCSILENT
!  PROCCMDPROC
   ...
   DO
    ...
    IF(ITYPE.EQ.TIMEREXPIRED)THEN
     ...
     N=0; DO JGRAD=1,SIZE(RNG)
      ...
      IDPROC=IPROC(:,JGRAD); CALL IOSCOMMANDCHECK(IDPROC,ISTATUS,IEXCOD=IEXCOD)
      !## stopped running
 ...
 !###====================================================================
 SUBROUTINE IPEST_GLM_TEST(DIR)
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER :: I,J,IFLAGS,ILIN,NINT,IPARAM
 REAL(KIND=DP_KIND),DIMENSION(5) :: XSTAT
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: IP
 
 WRITE(IUPESTOUT,'(A)') 'MEASURE_J,PARAMETER_J,TOTAL_J,MEAN_J,GOODNESS_OF_FIT'
 IUPESTRESIDUAL=0
 ...
 ALLOCATE(IP(SIZE(RNG))); IP=1.0D0; IP(1)=0.0D0
 ...
 DO
  ...
  I=1; DO
   ...
   IF(IP(I).LE.NINT)THEN
    EXIT
   ELSE
    IP(I)=1.0D0; I=I+1; IF(I.GT.SIZE(IP))STOP
   ENDIF
  ENDDO
  WRITE(*,*) IP
  ...
  !## get objective function value
  CALL IPEST_GLM_GETJ(DIR,ILIN,LPARAM(ILIN),'L')
  XSTAT(1)=MSR%TJ-MSR%RJ; XSTAT(2)=MSR%RJ; XSTAT(3)=MSR%TJ; XSTAT(4)=MSR%TJ/REAL(MSR%NOBS,8); XSTAT(5)=IPEST_GOODNESS_OF_FIT(GF_H,GF_O,MSR%NOBS)
  WRITE(IUPESTOUT,'(5F15.7)') (IP(I),I=,1,SIZE(IP)),(XSTAT(I),I=1,5)
 ENDDO
 ...
 !#####=================================================================
 SUBROUTINE IPEST_GLM_NEXT(ITER,ILIN)
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ITER,ILIN
...
!  ENDIF
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  DO J=1,MSR%NOBS
   W=MSR%W(J); DH1=MSR%DHG(IPARAM,J); DH2=MSR%DHL(0,J); S(IPARAM)=S(IPARAM)+W*((DH1-DH2)/DF1)
  ENDDO
 ENDDO
...
!   ENDIF
 ...
 !   DH1=REAL(MSR%DHG(IP1,I),8)
 !   DH2=REAL(MSR%DHL(0,I),8)
 !   DJ1=(DH1-DH2)/DF1
 !   N=N+1
 !   JQJ(N,1)=DJ1
...
2  IF(M.NE.L)THEN
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z,H,WW,MC,MM,DHH,XCOR,YCOR,ZCOR,XCROSS,DHW,DP
...
!
 ...
 IF(PEST%PE_REGULARISATION.EQ.1)THEN
  DO I=1,SIZE(PEST%PARAM)  !## row
   ...
   DP=(PEST%PARAM(I)%ALPHA(1)-PEST%PARAM(I)%PPRIOR)**2.0D0
   DP=DP*PEST%PE_REGFACTOR
   MSR%RJ=MSR%RJ+DP
  ENDDO
 ENDIF
