...
! ENDIF
 ...
 IF(PBMAN%PTEST.EQ.0)THEN
  ...
  BLINE=''; DO J=1,SIZE(PEST%PARAM)
   ...
   ENDIF
  ENDDO
  WRITE(IUPESTSENSITIVITY,'(/A)') TRIM(BLINE) 
  BLINE=''; DO J=1,SIZE(PEST%PARAM)
   IF(ABS(PEST%PARAM(J)%PACT).EQ.1)THEN
   ...
   ENDIF
  ENDDO
  WRITE(IUPESTSENSITIVITY,'(45X,A)') TRIM(BLINE) 
  BLINE=''; DO J=1,SIZE(PEST%PARAM)
   IF(ABS(PEST%PARAM(J)%PACT).EQ.1)THEN
 ...
 DO 
  ...
  DO ILIN=1,PBMAN%NLINESEARCH
   ...
   IF(IEXCOD.NE.0)THEN; WRITE(*,'(/A/)') 'ERROR OCCURED RUNNING MODEL L#'//TRIM(ITOS(LPARAM(ILIN))); STOP; ENDIF
...
MAINLOOP: DO
 ...
 !## copy parameters
 READ(IU,*,IOSTAT=IOS) N; IF(IOS.NE.0)RETURN; WRITE(JU,*) N
 READ(IU,'(A256)',IOSTAT=IOS) I,LINE; IF(IOS.NE.0)RETURN; WRITE(JU,'(A)') TRIM(LINE)
