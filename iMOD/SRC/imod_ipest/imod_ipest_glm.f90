...
CONTAINS
 ...
 DO 
  ...
  IFLAGS=PROCSILENT+PROCCMDPROC+PROCBLOCKED
  ...
  DO ILNSRCH=1,MXLNSRCH
   ...
   CALL IPEST_GLM_GETJ(DIR,IGRAD,ILNSRCH)
...
!!   NDAMPING=1.0 !## do it onces only
    ...
    IF(ITYPE.EQ.TIMEREXPIRED)THEN
     ...
     N=0; DO JGRAD=1,SIZE(RN)-1
      ...
      IF(ISTATUS.EQ.0)THEN
       ...
       CALL IPEST_GLM_GETJ(DIR,JGRAD,ILNSRCH)
       !## write echo
 ...
 ALLOCATE(PEST%PARAM(IP)%GALPHA(0:SIZE(RN)-1)); PEST%PARAM(IP)%GALPHA=0.0D0
...
!  ENDIF
 ...
 DO IP1=1,NP
  IF(PEST%PARAM(IP1)%PACT.EQ.0.OR.PEST%PARAM(IP1)%PIGROUP.LE.0)CYCLE
...
2  IF(M.NE.L)THEN
 ...
 !###====================================================================
 SUBROUTINE IPEST_GLM_GETJ(DIR,IGRAD,ILNSRCH)
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER,INTENT(IN) :: IGRAD,ILNSRCH
...
! IF(IGRAD.EQ.0)ALLOCATE(MSR%DH(0:SIZE(PEST%PARAM),M))
 IF(IGRAD.EQ.0)ALLOCATE(MSR%DH(0:SIZE(RN),M))
 ALLOCATE(MSR%W (M)) 
 ALLOCATE(MSR%X (M)) 
 ALLOCATE(MSR%Y (M)) 
 ALLOCATE(MSR%L (M)) 
