...
USE MOD_PMANAGER_PAR, ONLY : PEST,SIM,PARAM,PRJNLAY,PBMAN,PRJNPER,BND,HNOFLOW,PCG
...
CONTAINS
 ...
 WRITE(IUPESTOUT,'(A2,1X,A5,2(1X,A5),5(1X,A15),3A10,5A15)') 'AC','PTYPE','ILS','IZN','INITIAL','DELTA','MINIMUM','MAXIMUM','FADJ','IGROUP','LTRANS', &
     'NODES','ACRONYM','PPRIOR','STDEV','SDATE','EDATE'
...
MAINLOOP: DO 
  ...
  ELSE
   IUPESTRESIDUAL=0
  ENDIF 
  IF(.NOT.IPEST_GLM_RUNMODELS(IBATCH,RNL,LPARAM,'L',DIR,MNAME,ITER,LAMBDA,0))EXIT
  
  !## initial lambda
  IF(ITER.GT.0)THEN
   !## determine best lambda and continue, otherwise determine another set of lambda's
   IF(.NOT.IPEST_GLM_NEXT(IBATCH,ITER,DIR,LAMBDA,GAMMA,MNAME))THEN
  ...
  ELSE
   ...
   !## save initial heads with these (only for modflow2005)
   IF(PBMAN%IFORMAT.EQ.2)CALL IPEST_GLM_UPDATEHEADS(DIR,1)
  ENDIF
  ...
  !## run and process all lambda-testing
  IUPESTRESIDUAL=0; IF(.NOT.IPEST_GLM_RUNMODELS(IBATCH,RNG,GPARAM,'P',DIR,MNAME,ITER,LAMBDA,0))EXIT
 ...
 !#####=================================================================
 LOGICAL FUNCTION IPEST_GLM_RUNMODELS(IBATCH,RN,RPARAM,RT,DIR,MNAME,ITER,LAMBDA,ILAMBDA)
 !#####=================================================================
 IMPLICIT NONE
 ...
 !## start processes
   ...
   SELECT CASE (RT)
    CASE ('L','P')
     ...
     IF(PBMAN%IFORMAT.EQ.3)THEN
      ...
      CALL IPEST_GLM_SAVE_PARAMETERS(DIR,RPARAM(IGRAD),RT,IGRAD) 
     ELSE
  ...
  NDONE=IPEST_GLM_EVALUATE(IBATCH,ITER,NCPU,DIR,RN,RT,RPARAM,LAMBDA,LRESTART,ILAMBDA,MNAME)
 ...
 IPEST_GLM_RUNMODELS=.TRUE.
 
 END FUNCTION IPEST_GLM_RUNMODELS
 
 !###====================================================================
 INTEGER FUNCTION IPEST_GLM_EVALUATE(IBATCH,ITER,NCPU,DIR,RN,RT,RPARAM,LAMBDA,LRESTART,ILAMBDA,MNAME)
 !###====================================================================
 IMPLICIT NONE
 ...
 CHARACTER(LEN=*),INTENT(IN),DIMENSION(:) :: RN
 CHARACTER(LEN=*),INTENT(IN) :: DIR,RT,MNAME
  ...
  IF(ITYPE.EQ.TIMEREXPIRED)THEN
   N=0; DO JGRAD=1,SIZE(RN) 
    ...
    IF(ISTATUS(JGRAD).EQ.0)THEN
     ...
     IF(ISTATUS(JGRAD).EQ.0)THEN
      IF(.NOT.IPEST_GLM_GETJ(DIR,JGRAD,RPARAM(JGRAD),RT,IBATCH,ILAMBDA,MNAME))RETURN 
 ...
 !###====================================================================
 LOGICAL FUNCTION IPEST_GLM_READ_ZONES_OPENFILE(DIR,JU,NCOL,NROW)
 !###====================================================================
 IMPLICIT NONE
 ...
 IPEST_GLM_READ_ZONES_OPENFILE=.FALSE.

 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(DIR)//'\PARAM_DUMP_IPEST.DAT',STATUS='OLD',ACTION='READ',FORM='FORMATTED')
 READ(JU,*)
 READ(JU,*)
 READ(JU,'(22X,I10)',IOSTAT=IOS) NCOL
 READ(JU,'(22X,I10)',IOSTAT=IOS) NROW
 READ(JU,'(22X,I10)',IOSTAT=IOS) N
 IF(IOS.NE.0)THEN
  CLOSE(JU); WRITE(*,'(/A)') 'OOPS, AN OLD VERSION OF A PARAM_DUMP_IPEST FOUND, NEED TO BE RECREATED'; RETURN
 ENDIF
 IF(N.NE.SIZE(PEST%PARAM))THEN
  CLOSE(JU); WRITE(*,'(/A)') 'NUMBER OF ZONES IN THE PARAM_DUMP_IPEST DOES NOT COINCIDE WITH THE ACTUAL NUMBER OF PARAMETERS'; RETURN
 ENDIF

 IPEST_GLM_READ_ZONES_OPENFILE=.TRUE.
 
 END FUNCTION  IPEST_GLM_READ_ZONES_OPENFILE

 !###====================================================================
 SUBROUTINE IPEST_GLM_READ_ZONES(DIR,NCOL,NROW)
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 ...
 IF(.NOT.IPEST_GLM_READ_ZONES_OPENFILE(DIR,JU,NCOL,NROW))STOP
 
 DO I=1,SIZE(PEST%PARAM)
  READ(JU,*)
  READ(JU,*)
  READ(JU,'(2I10)') PEST%PARAM(I)%NODES,PEST%PARAM(I)%ZTYPE
  IF(PEST%PARAM(I)%ZTYPE.EQ.0)THEN
   ...
   DO J=1,PEST%PARAM(I)%NODES
    READ(JU,'(2I10,F10.4)') PEST%PARAM(I)%IROW(J),PEST%PARAM(I)%ICOL(J),PEST%PARAM(I)%F(J)
   ENDDO
  ELSE
 ...
 IF(PEST%PARAM(IP)%PLOG.EQ.0)THEN
  WRITE(IUPESTOUT,'(I2,1X,A5,2(1X,I5),5(1X,F15.7),I10,I10,A10,A15,2F15.7,2A15)') I,PEST%PARAM(IP)%PPARAM,PEST%PARAM(IP)%PILS, &
     ...
     PEST%PARAM(IP)%SDATE,PEST%PARAM(IP)%EDATE
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  DO J=1,I-1 
   ...
   IF(PEST%PARAM(J)%PIGROUP.EQ.PEST%PARAM(I)%PIGROUP)THEN
    ...
    ENDIF
    PEST%PARAM(I)%PIGROUP=-1*ABS(PEST%PARAM(I)%PIGROUP); EXIT
   ENDIF
  ENDDO
 ENDDO
 ...
 IU=UTL_GETUNIT(); OPEN(IU,FILE=DIRNAME,STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  WRITE(*,'(/A)') 'Cannot read the appropriate PST1-file for parameter '//TRIM(ITOS(IGRAD))
  WRITE(*,'(A/)') TRIM(DIRNAME)
  STOP
 ENDIF
 DIRNAME=TRIM(DIR)//'\MODELINPUT\'//TRIM(MNAME)//'_'//TRIM(CTYPE)//'#'//TRIM(ITOS(IPARAM))//'.PST1_'
 JU=UTL_GETUNIT(); OPEN(JU,FILE=DIRNAME,STATUS='UNKNOWN',ACTION='WRITE',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  WRITE(*,'(/A)') 'Cannot update the appropriate PST-file for parameter '//TRIM(ITOS(IGRAD))
  WRITE(*,'(A/)') TRIM(DIRNAME)
  STOP
 ENDIF
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  IF(PEST%PARAM(I)%PLOG.EQ.0)THEN
   LINE=TRIM(ITOS(MIN(1,ABS(PEST%PARAM(I)%PACT))))//','// &
        ...
        TRIM(RTOS(PEST%PARAM(I)%PARSTD,'G',7))//','// &        
        TRIM(PEST%PARAM(I)%SDATE)//','// &        
 ...
 DO I=1,PBMAN%NLAMBDASEARCH
  !## modflow6
  IF(PBMAN%IFORMAT.EQ.3)THEN
   RNL(I)=TRIM(DIR)//'\IPEST_L#'//TRIM(ITOS(I))//'\RUN_L#'//TRIM(ITOS(I))//'.BAT'
  ELSE
   RNL(I)=TRIM(DIR)//'\RUN_L#'//TRIM(ITOS(I))//'.BAT'
  ENDIF
  LPARAM(I)=I
  INQUIRE(FILE=RNL(I),EXIST=LEX)
  IF(.NOT.LEX)THEN
   WRITE(*,'(/A)') 'Cannot read the appropriate BAT-file for parameter '//TRIM(ITOS(I))
   WRITE(*,'(A/)') TRIM(RNL(I))
   STOP
  ENDIF  
 ENDDO
 ...
 N=0; DO I=1,SIZE(PEST%PARAM)
  !## parameter
  IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE; N=N+1
  IF(PBMAN%IFORMAT.EQ.3)THEN
   RNG(N)=TRIM(DIR)//'\IPEST_P#'//TRIM(ITOS(I))//'\RUN_P#'//TRIM(ITOS(I))//'.BAT'
  ELSE
   RNG(N)=TRIM(DIR)//'\RUN_P#'//TRIM(ITOS(I))//'.BAT'
  ENDIF
  INQUIRE(FILE=RNG(N),EXIST=LEX)
  IF(.NOT.LEX)THEN
   WRITE(*,'(/A)') 'Cannot read the appropriate BAT-file for parameter '//TRIM(ITOS(I))
   WRITE(*,'(A/)') TRIM(RNG(N))
   STOP
 ...
 !#####=================================================================
 LOGICAL FUNCTION IPEST_GLM_NEXT(IBATCH,ITER,DIR,LAMBDA,GAMMA,MNAME)
 !#####=================================================================
 IMPLICIT NONE
 ...
 IJ=0; MJ=HUGE(1.0D0); DO I=1,PBMAN%NLAMBDASEARCH
  XLAMBDA(I)=PBMAN%LAMBDA_TEST(I)*LAMBDA
  YLAMBDA(I)=MSR%DHL_J(I)
 ...
 IF(CPARABOL(3).GT.0.0D0)THEN
  ...
  WRITE(IUPESTPROGRESS,'(2(A,F15.3))') 'BOT PARABOLA @ LAMBDA= ', X,' NEW LAMBDA= ',LAMBDA
  WRITE(IUPESTPROGRESS,'(1(A,F15.3))') 'ESTIMATED OBJECTIVE FUNCTION VALUE (NEXT TIME) = ', CPARABOL(1)+CPARABOL(2)*X+CPARABOL(3)*X**2.0D0
 ELSE
  LAMBDA=LAMBDA*PBMAN%LAMBDA_TEST(IJ)
  N=PBMAN%NLAMBDASEARCH/2; IF(MOD(PBMAN%NLAMBDASEARCH,2).NE.0)N=N+1
  IF(IJ.LT.N)THEN
 ...
 IF(.NOT.IPEST_GLM_GETJ(DIR,IJ,LPARAM(IJ),'L',IBATCH,0,MNAME))RETURN
 ...
 IF(PBMAN%IFORMAT.EQ.2)CALL IPEST_GLM_UPDATEHEADS(DIR,IJ)
 ...
 !#####=================================================================
 SUBROUTINE IPEST_GLM_SAVE_PARAMETERS(DIR,IPARAM,RT,IGRAD) 
 !#####=================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IGRAD,IPARAM
 CHARACTER(LEN=*),INTENT(IN) :: DIR,RT
 INTEGER :: I,J,K,ILAY,ISUB,IROW,ICOL,NCOL,NROW,IPER
 REAL(KIND=DP_KIND) :: FCT,F
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:,:) :: XORG,XADJ,FRAC,XCNT
 INTEGER,POINTER,DIMENSION(:,:) :: IDX
 CHARACTER(LEN=2),DIMENSION(8) :: IPTYPE
 CHARACTER(LEN=3),DIMENSION(8) :: FPTYPE
 CHARACTER(LEN=4),DIMENSION(8) :: SVFILE
 INTEGER,DIMENSION(8) :: NCLIST,ACLIST
 CHARACTER(LEN=256) :: FNAME
 LOGICAL :: LEX
 
 IPTYPE=['KH ' ,'VA'  ,'SC'  ,'SY'  ,'DC'  ,'RC'  ,'GC',  'RE'   ]
 FPTYPE=['K'   ,'K33' ,'SS'  ,'SY'  ,'DRN' ,'RIV' ,'GHB' ,'RCH' ]
 SVFILE=['NPF6','NPF6','STO6','STO6','DRN6','RIV6','GHB6','RCH6']
 NCLIST=[     0,     0,     0,     0,    2,     4,     2 ,    1 ]
 ACLIST=[     0,     0,     0,     0,    2,     2,     2 ,    1 ]
 
 !## modify parameters per submodel
 DO ISUB=1,PBMAN%NSUBMODEL
 
  !## read the zones per submodel
  CALL IPEST_GLM_READ_ZONES(TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELINPUT',NCOL,NROW)
  ALLOCATE(FRAC(NCOL,NROW))
   ...
   SELECT CASE (IPTYPE(I))
    ...
    CASE ('DC','RC','GC','RE')
     ...
     DO J=1,SIZE(PEST%PARAM)
      ...
      IF(PEST%PARAM(J)%PPARAM.EQ.IPTYPE(I))EXIT
     ...
     DO IPER=1,PRJNPER
      ...
      FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELINPUT\'//SVFILE(I)//'\'//TRIM(FPTYPE(I))//'_T'//TRIM(ITOS(IPER))//'.ARR'
      INQUIRE(FILE=FNAME,EXIST=LEX)
      !## skip it for this period
      IF(.NOT.LEX)CYCLE
      IF(.NOT.IPEST_GLM_READ_LSTFILE(FNAME,NCLIST(I),XORG,IDX))RETURN
      IF(.NOT.ASSOCIATED(XADJ))ALLOCATE(XADJ(SIZE(XORG,1),SIZE(XORG,2)))
      XADJ=XORG
      !## process permeability-values
      DO J=1,SIZE(PEST%PARAM); IF(PEST%PARAM(J)%PPARAM.NE.IPTYPE(I))CYCLE
       ...
       IF(PEST%PARAM(J)%ZTYPE.EQ.0)THEN
        !## fill array with location of a zone
        DO K=1,PEST%PARAM(J)%NODES
         IROW=PEST%PARAM(J)%IROW(K)
         ICOL=PEST%PARAM(J)%ICOL(K)
         FRAC(ICOL,IROW)=PEST%PARAM(J)%F(K)
        ENDDO
        DO K=1,SIZE(IDX,2)
         IROW=IDX(2,K)
         ICOL=IDX(3,K)
         !## skip wrong system number
         IF(IDX(4,K).NE.PEST%PARAM(J)%PILS)CYCLE
         F=FRAC(ICOL,IROW)
         IF(F.GT.0.0D0)THEN
          IF(IDX(5,K).EQ.0)THEN
           XADJ(ACLIST(I),K)=XORG(ACLIST(I),K)*F*FCT
          ELSE
           !## modify current package for this zone
           XADJ(ACLIST(I),K)=XADJ(ACLIST(I),K)+XORG(ACLIST(I),K)*F*FCT
          ENDIF
          IDX(5,K)=IDX(5,K)+1
         ENDIF
        ENDDO      
      ...
      FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELINPUT\'//SVFILE(I)//'\'//RT//'#'//TRIM(ITOS(IPARAM))//'\'// &
            TRIM(FPTYPE(I))//'_T'//TRIM(ITOS(IPER))//'_'//RT//'#'//TRIM(ITOS(IPARAM))//'.ARR'
      IF(.NOT.IPEST_GLM_WRITELSTFILE(FNAME,XADJ,IDX))RETURN

      !## deallocate memory
      DEALLOCATE(XORG,XADJ,IDX)
     ENDDO
    ...
    CASE ('KH','VA','SC','SY')
     ...
     DO ILAY=1,PRJNLAY
      ...
      IF(.NOT.ASSOCIATED(XADJ))ALLOCATE(XADJ(SIZE(XORG,1),SIZE(XORG,2)))
      IF(.NOT.ASSOCIATED(XCNT))ALLOCATE(XCNT(SIZE(XORG,1),SIZE(XORG,2)))
      XADJ=XORG; XCNT=0.0D0
      !## process permeability-values
      DO J=1,SIZE(PEST%PARAM)
       IF(PEST%PARAM(J)%PPARAM.NE.IPTYPE(I))CYCLE
       IF(PEST%PARAM(J)%PILS.NE.ILAY)CYCLE
       !## found correct parameter - apply factor
       FCT=PEST%PARAM(J)%ALPHA(1); IF(PEST%PARAM(J)%PLOG.EQ.1)FCT=10.0D0**FCT
       IF(PEST%PARAM(J)%ZTYPE.EQ.0)THEN
        DO K=1,PEST%PARAM(J)%NODES
         ...
         ICOL=PEST%PARAM(J)%ICOL(K)
         F   =PEST%PARAM(J)%F(K)
         IF(XCNT(ICOL,IROW).EQ.0.0D0)THEN
          XADJ(ICOL,IROW)=XORG(ICOL,IROW)*F*FCT
         ELSE
      ...
      !## save adjusted permeability-values
      FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELINPUT\'//SVFILE(I)//'\'//RT//'#'//TRIM(ITOS(IPARAM))//'\'// &
            TRIM(FPTYPE(I))//'_L'//TRIM(ITOS(ILAY))//'_'//RT//'#'//TRIM(ITOS(IPARAM))//'.ARR'
      IF(.NOT.IPEST_GLM_WRITE_ARRFILE(FNAME,XADJ))RETURN
      
      !## deallocate memory
      DEALLOCATE(XORG,XADJ,XCNT)
 ...
 !###====================================================================
 LOGICAL FUNCTION IPEST_GLM_READ_LSTFILE(FNAME,NC,X,IDX)
 !###====================================================================
 IMPLICIT NONE
 ...
 INTEGER,POINTER,DIMENSION(:,:) :: IDX
 INTEGER :: IU,IOS,I,J,N
 CHARACTER(LEN=256) :: LINE
 
 IPEST_GLM_READ_LSTFILE=.FALSE.
 ...
 DO I=1,2
  N=0; DO
   ...
   IF(I.EQ.2)THEN
    READ(LINE,*) (IDX(J,N),J=1,3),(X(J,N),J=1,NC),IDX(4,N)
   ENDIF
  ENDDO
  IF(I.EQ.1)THEN
   ALLOCATE(IDX(5,N),X(NC,N)); IDX=0; X=0.0D0
  ENDIF
  REWIND(IU)
 ENDDO
 ...
 IPEST_GLM_READ_LSTFILE=.TRUE.
 
 END FUNCTION IPEST_GLM_READ_LSTFILE
 
 !###====================================================================
 LOGICAL FUNCTION IPEST_GLM_WRITELSTFILE(FNAME,X,IDX)
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: FNAME
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:,:) :: X
 ...
 WRITE(FRM,'(A10,I2.2,A14)') '(3(I5,1X),',SIZE(X,1),'(G15.7,1X),I5)'

 DO I=1,SIZE(IDX,2)
  WRITE(IU,FRM) (IDX(J,I),J=1,3),(X(J,I),J=1,SIZE(X,1)),IDX(4,I)
 ENDDO
 
 CLOSE(IU)
 
 IPEST_GLM_WRITELSTFILE=.TRUE.
 
 END FUNCTION IPEST_GLM_WRITELSTFILE
 ...
 REAL(KIND=DP_KIND),INTENT(INOUT) :: LAMBDA
 REAL(KIND=DP_KIND),INTENT(IN) :: GAMMA
 REAL(KIND=DP_KIND) :: DJ1,DJ2,DP,WW,TS,DF1,EIGWTHRESHOLD,W,DH1,DH2,MARQUARDT,P1,P2,PMIN,PMAX,WF !,DH
 ...
 IPARAM=0
 DO I=1,SIZE(PEST%PARAM)
  IF(ABS(PEST%PARAM(I)%PACT).NE.1)CYCLE
!     IF(PEST%PARAM(I)%PACT.NE.1)CYCLE
  DF1=PEST%PARAM(I)%PDELTA
  IPARAM=IPARAM+1
  DO J=1,MSR%NOBS
   W=MSR%W(J); DH1=MSR%DHG(IPARAM,J); DH2=MSR%DHL(0,J) 
   !## truncate for noise (do not do this)
!   DH=DH1-DH2; IF(ABS(DH).LE.PCG%HCLOSE)DH=0.0D0
!   S(IPARAM)=S(IPARAM)+(W*(DH/DF1))
 ...
 TS=SUM(ABS(S)); IPARAM=0; DO I=1,SIZE(PEST%PARAM)
  !## skip inactive parameters
  IF(ABS(PEST%PARAM(I)%PACT).NE.1)CYCLE; IPARAM=IPARAM+1; IF(TS.NE.0.0D0)S(IPARAM)=S(IPARAM)/TS
 ENDDO; S=ABS(S)*100.0D0
 WRITE(IUPESTSENSITIVITY,'(I10,99999F15.7)') ITER,(S(I),I=1,NP)

 IPARAM=0; DO I=1,SIZE(PEST%PARAM)
  IF(ABS(PEST%PARAM(I)%PACT).NE.1)CYCLE; IPARAM=IPARAM+1
 ...
 K=0; IPARAM=0; DO I=1,SIZE(PEST%PARAM)
  IF(ABS(PEST%PARAM(I)%PACT).NE.1)CYCLE; IPARAM=IPARAM+1
  IF(S(IPARAM).LE.PEST%PE_SENS)THEN
   PEST%PARAM(I)%PACT=-1; K=K+1
   IF(K.EQ.1)WRITE(IUPESTOUT,'(/A/)') 'List of Insensitive Parameter (Sensitivity <= '//TRIM(RTOS(PEST%PE_SENS,'F',7))//' %)'
 ...
 DO 
  ...
  ILAMBDA=ILAMBDA+1
  !## finished
  IF(ILAMBDA.GT.PBMAN%NLAMBDASEARCH)EXIT
 ...
 WRITE(IUPESTOUT,'(15X,99A15)') ('LAMBDA'//TRIM(ITOS(J)),J=1,PBMAN%NLAMBDASEARCH)
 WRITE(IUPESTOUT,'(A15,99G15.7)') 'Parameters',(LAMBDA*PBMAN%LAMBDA_TEST(J),J=1,PBMAN%NLAMBDASEARCH)
 WRITE(IUPESTOUT,'(999A1)') ('-',I=1,15*(PBMAN%NLAMBDASEARCH+1))
 DO I=1,SIZE(PEST%PARAM)
  IF(PEST%PARAM(I)%PACT.EQ.0.OR.PEST%PARAM(I)%PIGROUP.LT.0)CYCLE
  IF(PEST%PARAM(I)%PLOG.EQ.0)THEN
   WRITE(IUPESTOUT,'(A15,99F15.7)') PEST%PARAM(I)%ACRONYM,(PEST%PARAM(I)%LALPHA(J),J=1,PBMAN%NLAMBDASEARCH)
  ELSE
   WRITE(IUPESTOUT,'(A15,99F15.7)') PEST%PARAM(I)%ACRONYM,(10.0D0**(PEST%PARAM(I)%LALPHA(J)),J=1,PBMAN%NLAMBDASEARCH)
  ENDIF
 ENDDO
...
! IF(ABS(P1-PMIN).LE.XPBND)IBND=-1; IF(ABS(PMAX-P1).LE.XPBND)IBND= 1
 ...
 DO I=1,SIZE(PEST%PARAM)
  ...
  IF(PEST%PARAM(I)%PLOG.EQ.1)THEN
   WRITE(IUPESTRUNFILE,'(I2,1X,A,1X,2(I5,1X),5(F10.3,1X),I5,1X,I2,1X,A,2F10.3,2A15)') MIN(1,ABS(PEST%PARAM(I)%PACT)), &  !## iact
 ...
 INTEGER :: I,J,IP1,IP2,II,JJ,N,IPARAM,JPARAM,ISING,IERROR,IOS
 ...
 IF(ISING.EQ.0)THEN
  ...
  DO IP1=1,SIZE(PEST%PARAM)
   IF(PEST%PARAM(IP1)%PACT.EQ.1)THEN
    I=I+1
    IF(NP.LT.PRINTLIMIT)THEN
     WRITE(IUPESTOUT,'(A15,99999G15.7)') PEST%PARAM(IP1)%ACRONYM,(B(I,J),J=1,NP)
    ENDIF
  ...
  DO IP1=1,SIZE(PEST%PARAM)
   IF(PEST%PARAM(IP1)%PACT.EQ.1)THEN
    ...
    ENDDO
    IF(NP.LT.PRINTLIMIT)THEN
     WRITE(IUPESTOUT,'(A15,99999F15.7)',IOSTAT=IOS) PEST%PARAM(IP1)%ACRONYM,(COR(I,J),J=1,NP)
    ENDIF
   ENDIF
  ENDDO
...
! ENDIF
 ...
 !###====================================================================
 LOGICAL FUNCTION IPEST_GLM_GETJ(DIR,IGRAD,IPARAM,CTYPE,IBATCH,ILAMBDA,MNAME) 
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,CTYPE,MNAME
 ...
 IF(PBMAN%IFORMAT.EQ.3.AND.PBMAN%IPESTP.EQ.1)THEN
  CALL IPEST_GLM_CREATE_RESIDUALSFILES(DIR,IPARAM,CTYPE,IBATCH,MNAME) 
  IF(ASSOCIATED(PEST%MEASURES))DEALLOCATE(PEST%MEASURES)
  ALLOCATE(PEST%MEASURES(1))
  PEST%MEASURES(1)%IPFNAME=TRIM(DIR)//'\OUTPUT_OBS.IPF'
  PEST%MEASURES(1)%IXCOL=1
  PEST%MEASURES(1)%IYCOL=2
  PEST%MEASURES(1)%ILCOL=3
  PEST%MEASURES(1)%IMCOL=4
  PEST%MEASURES(1)%IVCOL=5
  PEST%MEASURES(1)%IDCOL=0
 ENDIF 
 ...
 DO I=1,SIZE(PEST%MEASURES)
  ...
  IF(IEXT.EQ.0)THEN
   ...
   DO J=1,NR
    ...
    !## entries in stdev - weight is 1/stdev
...
!    STOP
  ...
  ELSE
   ...
   DO J=1,NR
    ...
    ENDIF
    !## input is a stdev - convert it to a weight as w=1/stdev
    IF(PEST%MEASURES(I)%IVCOL.GT.0)THEN
...
!
 ...
 !###====================================================================
 SUBROUTINE IPEST_GLM_CREATE_RESIDUALSFILES(DIR,IGRAD,CTYPE,IBATCH,MNAME) 
 !###====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,CTYPE,MNAME
 INTEGER,INTENT(IN) :: IGRAD,IBATCH
 INTEGER :: IU,JU,KU,ISUB,NPER,NOBS,IOBS,IPER,N,MPER
 REAL(KIND=DP_KIND) :: CUMTT
 CHARACTER(LEN=52) :: TXT
 CHARACTER(LEN=256) :: FNAME
 TYPE HTYPE
  INTEGER(KIND=DP_KIND) :: ITIME
  INTEGER :: ILAY
  REAL(KIND=DP_KIND) :: H,STDEV,M
 END TYPE HTYPE
 TYPE(HTYPE),ALLOCATABLE,DIMENSION(:,:) :: H
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: X,Y
 CHARACTER(LEN=40),ALLOCATABLE,DIMENSION(:) :: ONAME
 
 IF(TRIM(CTYPE).EQ.'')THEN 
  CALL UTL_CREATEDIR(TRIM(DIR)//'\TIMESERIES')
  FNAME=TRIM(DIR)//'\TIMESERIES\OUTPUT_OBS.IPF_' 
 ELSE
  CALL UTL_CREATEDIR(TRIM(DIR)//'\IPEST_'//CTYPE//'#'//TRIM(ITOS(IGRAD))//'\TIMESERIES')
  FNAME=TRIM(DIR)//'\IPEST_'//CTYPE//'#'//TRIM(ITOS(IGRAD))//'\TIMESERIES\OUTPUT_OBS.IPF_' 
 ENDIF
 ...
 N=0; DO ISUB=1,PBMAN%NSUBMODEL

  !## skip zero
  IF(TRIM(CTYPE).EQ.'')THEN
   FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELOUTPUT\OUTPUT_OBS.TXT'
  ELSE
   FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELOUTPUT\IPEST_'//TRIM(CTYPE)//'#'// &
          TRIM(ITOS(IGRAD))//'\OUTPUT_OBS_'//TRIM(CTYPE)//'#'//TRIM(ITOS(IGRAD))//'.TXT'
  ENDIF
  JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=FNAME,STATUS='OLD',ACTION='READ',FORM='FORMATTED'); IF(JU.EQ.0)RETURN
  FNAME=TRIM(DIR)//'\GWF_'//TRIM(ITOS(ISUB))//'\MODELINPUT\'//TRIM(MNAME)//'.MES6'
  ...
  IF(NOBS.GT.0)THEN
   ALLOCATE(H(NOBS,NPER),ONAME(NOBS),X(NOBS),Y(NOBS))
   
   READ(KU,*)
   !## read measurements
   DO IOBS=1,NOBS
    READ(KU,*) ONAME(IOBS),IPER,X(IOBS),Y(IOBS)
    IF(IPER.NE.NPER)THEN
     WRITE(*,'(/A/)') 'Something goes wrong (number of stressperiods in OBS6 and MET6 not equal) in the postprocessing of the measurements into IPF-files'; STOP
    ENDIF
    READ(KU,*)
    DO IPER=1,NPER
     READ(KU,*) H(IOBS,IPER)%ITIME,H(IOBS,IPER)%H,H(IOBS,IPER)%STDEV,H(IOBS,IPER)%ILAY
    ENDDO
   ENDDO
   ...
   READ(JU,*)
   DO IPER=1,NPER
    READ(JU,*) CUMTT,(H(IOBS,IPER)%M,IOBS=1,NOBS)
   ENDDO
   
   !## close files
   CLOSE(JU); CLOSE(KU)

   !## store them in aggregated IPF-file
   DO IOBS=1,NOBS
    N=N+1
    IF(PRJNPER.EQ.1)THEN
     IPER=1
     WRITE(IU,'(A)') TRIM(RTOS(X(IOBS),'F',3))//','//TRIM(RTOS(Y(IOBS),'F',3))//','//TRIM(ITOS(H(IOBS,IPER)%ILAY))//','// &
                     TRIM(RTOS(H(IOBS,IPER)%H,'F',3))//','//TRIM(RTOS(H(IOBS,IPER)%STDEV,'F',3))//','//TRIM(RTOS(H(IOBS,IPER)%M,'F',3))
    ELSE
     FNAME=ONAME(IOBS)
     WRITE(IU,'(A)') TRIM(RTOS(X(IOBS),'F',3))//','//TRIM(RTOS(Y(IOBS),'F',3))//','//TRIM(ITOS(H(IOBS,1)%ILAY))//','// &
                      TRIM(FNAME)//','//TRIM(RTOS(H(IOBS,1)%STDEV,'F',3))
     IF(TRIM(CTYPE).EQ.'')THEN
      FNAME=TRIM(DIR)//'\TIMESERIES\'//TRIM(FNAME)//'.TXT'
     ELSE
      FNAME=TRIM(DIR)//'\IPEST_'//CTYPE//'#'//TRIM(ITOS(IGRAD))//'\TIMESERIES\'//TRIM(FNAME)//'.TXT'
     ENDIF
     JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=FNAME,STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(JU.EQ.0)RETURN
     MPER=0; DO IPER=1,NPER
      IF(H(IOBS,IPER)%ITIME.EQ.0)CYCLE; MPER=MPER+1
     ENDDO
     WRITE(JU,'(A)') TRIM(ITOS(MPER))
     WRITE(JU,'(A)') '3,1'
     WRITE(JU,'(A)') 'IDATE,-9999'
     WRITE(JU,'(A)') 'MEASURED,'//TRIM(RTOS(HNOFLOW,'G',7))
     WRITE(JU,'(A)') 'COMPUTED,'//TRIM(RTOS(HNOFLOW,'G',7))
     DO IPER=1,NPER
      IF(H(IOBS,IPER)%ITIME.EQ.0)CYCLE
      WRITE(JU,'(A)') TRIM(ITOS_DBL(H(IOBS,IPER)%ITIME))//','//TRIM(RTOS(H(IOBS,IPER)%H,'G',7))//','// &
                      TRIM(RTOS(H(IOBS,IPER)%M,'G',7))
     ENDDO
     CLOSE(JU)
    ENDIF
   ENDDO
   DEALLOCATE(H,ONAME,X,Y)
 ...
 !## update number of records in ipf file
 IF(TRIM(CTYPE).EQ.'')THEN
  CALL UTL_MF2005_MAXNO(TRIM(DIR)//'\TIMESERIES\OUTPUT_OBS.IPF_',(/N/))
 ELSE
  CALL UTL_MF2005_MAXNO(TRIM(DIR)//'\IPEST_'//CTYPE//'#'//TRIM(ITOS(IGRAD))//'\TIMESERIES\OUTPUT_OBS.IPF_',(/N/))
 ENDIF
 
 END SUBROUTINE IPEST_GLM_CREATE_RESIDUALSFILES
 ...
 IF(CTYPE.EQ.'P')THEN
  ...
  IF(PEST%PARAM(IGRAD)%PLOG.EQ.1)X1=10.0D0**X1
  X2=PEST%PARAM(IGRAD)%ALPHA(2)
  IF(PEST%PARAM(IGRAD)%PLOG.EQ.1)X2=10.0D0**X2
