...
CONTAINS
 ...
 ITER=0; DO 
  ...
  !## run and process all lambda-testing
  WRITE(IUPESTPROGRESS,'(/A5,5A15)') 'GD','PARAMETER','FACTOR','TOT_J','RED_J','CUR_PARAMETER'
 ...
 ELSE
  WRITE(IUPESTOUT,'(I2,1X,A5,2(1X,I5),5(1X,F15.7),I10,I10,A10,A15,F15.7)') I,PEST%PARAM(IP)%PPARAM,PEST%PARAM(IP)%PILS, &
     ...
     ABS(PEST%PARAM(IP)%PIGROUP),PEST%PARAM(IP)%PLOG,'Unknown',PEST%PARAM(IP)%ACRONYM,10.0D0**(PEST%PARAM(IP)%PPRIOR)
...
!    ENDIF
 ...
 IF(PEST%PE_REGULARISATION.EQ.1)THEN
  ...
  DO IP1=1,SIZE(PEST%PARAM)  !## row
   ...
   I=I+1
   DP=(PEST%PARAM(IP1)%ALPHA(2)-PEST%PARAM(IP1)%PPRIOR)*PEST%PE_REGFACTOR
   JQR(I)=JQR(I)+DP
...
!
 ...
 IF(PEST%PE_REGULARISATION.EQ.1)THEN
  DO I=1,SIZE(PEST%PARAM)  !## row
   ...
   F=(PEST%PARAM(I)%ALPHA(1)-PEST%PARAM(I)%PPRIOR)**2.0D0
   MSR%RJ=MSR%RJ+(F*PEST%PE_REGFACTOR)
  ENDDO
 ENDIF
