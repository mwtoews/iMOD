...
! CALL PLINES1READFILES(ID)
 ...
 RUNFNAME=MDLDIRNAME(INDEX(MDLDIRNAME,'\',.TRUE.)+1:)
 RUNFNAME=TRIM(PREFVAL(1))//'RUNFILES\IMODPATH_'//TRIM(FNAME)//'.RUN'

!TRIM(PREFVAL(1))//'\TMP\imodpath.run'
 ...
 IF(IDB.EQ.IDOK)THEN
  ...
  IF(TRACEMAIN(RUNFNAME,0,0))THEN !TRIM(PREFVAL(1))//'\TMP\IMODPATH.RUN',0,0))THEN
   DO I=1,2
    IF(IMODE(I).EQ.1)THEN
