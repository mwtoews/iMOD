...
USE MODPLOT
USE MOD_UTL, ONLY : JD,UTL_GETUNIT,ITOS,RTOS,UTL_MESSAGEHANDLE,GDATE,JDATETOIDATE,IDATETOJDATE,UTL_DIRINFO, &
     UTL_IMODFILLMENU,UTL_WSELECTFILE,CLOSEUNITS
USE DATEVAR
USE MOD_PLINES_PAR
USE MOD_PLINES_TRACE, ONLY : TRACEMAIN
USE IMODVAR
USE IMOD
 ...
 SELECT CASE(ITYPE)
  ...
  CASE (FIELDCHANGED)
   SELECT CASE (MESSAGE%WIN)
    ...
    CASE (ID_DPATHTAB2)
     SELECT CASE (MESSAGE%VALUE2)
      CASE (IDF_INTEGER4,IDF_MENU3,IDF_INTEGER3, &
       ...
       CALL PLINES1FIELDS(IDF_INTEGER4,IDF_MENU3,IDF_INTEGER3)
       CALL PLINES1FIELDS(IDF_INTEGER6,IDF_MENU4,IDF_INTEGER5)
    ...
    CASE (ID_DPATHTAB4)
     SELECT CASE (MESSAGE%VALUE2)
      CASE (IDF_RADIO7,IDF_RADIO8,IDF_CHECK1,IDF_CHECK2) !forw./backw./both
       CALL PLINES4FIELDS()
     END SELECT
    CASE (ID_DPATHTAB5)
     SELECT CASE (MESSAGE%VALUE2)
    ...
    CASE (ID_DPATHTAB1)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (ID_OPEN)
       ...
       CALL WSELECTDIR(DIRCHANGE,IDFNAME,'Select Model Result Directory')
...
!      CASE (ID_OPENISD)
!       IDFNAME=TRIM(PREFVAL(1))//'\*.isd'
!       IF(UTL_WSELECTFILE('iMOD StartPoint Definition File (*.isd)|*.isd|',&
!                    LOADDIALOG+MUSTEXIST+PROMPTON+DIRCHANGE+APPENDEXT+MULTIFILE,IDFNAME,&
!                    'Load StartPoint Definition File (*.isd)'))THEN
!        CALL WDIALOGSELECT(ID_DPATHTAB3)
!        CALL WDIALOGPUTSTRING(IDF_STRING1,IDFNAME)
!       ENDIF
 ...
 SELECT CASE (ID)
  ...
  CASE (ID_PROPERTIES2)
   ...
   CALL WGRIDROWS(IDF_GRID1,NLAY*2)
   J=0
   DO I=1,NLAY
    J=J+1
    CALL WGRIDLABELROW(IDF_GRID1,J,'Top of Layer '//TRIM(ITOS(I)))
    J=J+1
    CALL WGRIDLABELROW(IDF_GRID1,J,'Bottom of Layer '//TRIM(ITOS(I)))
   END DO
  CASE (ID_PROPERTIES3)
   ...
   DO I=1,NLAY
    J=J+1
    CALL WGRIDLABELROW(IDF_GRID1,J,'Porosity of Aquifer '//TRIM(ITOS(I)))
    J=J+1
    CALL WGRIDLABELROW(IDF_GRID1,J,'Porosity of Aquitard '//TRIM(ITOS(I)))
   END DO
 END SELECT
...
! CALL PLINES1READFILES(ID)
    ...
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDOK)
      SELECT CASE (ID)
       CASE (ID_PROPERTIES1)
        DO I=1,NLAY
         CALL WGRIDGETCELLSTRING(IDF_GRID1,1,I,LISTFNAME(I))!IBIDF(I))
       ...
       CASE (ID_PROPERTIES2)
        ...
        DO I=1,NLAY
         J=J+1
         CALL WGRIDGETCELLSTRING(IDF_GRID1,1,J,LISTFNAME(J))!TBIDF(J))
       ...
       CASE (ID_PROPERTIES3)
        ...
        DO I=1,NLAY
         J=J+1
         CALL WGRIDGETCELLSTRING(IDF_GRID1,1,J,LISTFNAME(J))!PORIDF(J))
         IF(.NOT.PLINES1CHECKFILES(LISTFNAME(J)))EXIT
         J=J+1
         CALL WGRIDGETCELLSTRING(IDF_GRID1,1,J,LISTFNAME(J))!PORIDF(J))
         IF(.NOT.PLINES1CHECKFILES(LISTFNAME(J)))EXIT
 ...
 CHARACTER(LEN=8) :: CDATE1,CDATE2
 INTEGER :: IM1,IM2,ID1,ID2,IY1,IY2,IMXT,ISTOP,IWK,IU,IOS,TOS,ILAY
 REAL :: MXT,FWK
 LOGICAL :: LEX
 ...
 ELSEIF(CODE.EQ.ID_OPEN)THEN
  ...
  IF(ILAY.NE.NLAY)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not load ips file'//CHAR(13)// &
    'since the current number of modellayers ('//TRIM(ITOS(NLAY))//')'//CHAR(13)// &
    'does not math the number of modellayers inside the ips file ('//TRIM(ITOS(ILAY))//')','Error')
   CLOSE(IU)
   RETURN
  ENDIF
 ...
 !###======================================================================
 SUBROUTINE PLINES1START()
 !###======================================================================
 !### Routine assembles imodpath.run file
 
 IMPLICIT NONE
 CHARACTER(LEN=256) :: IDFNAME,LINE,FNAME !SPFNAME,IFFFNAME,
 CHARACTER(LEN=3),DIMENSION(2) :: EXT
 LOGICAL :: LEX
 INTEGER :: ILAY,IPER,I,J,ISS,IREV,ISNK,ID,IY,IM,JD1,JD2,ISTOPCRIT,IU
 ...
 IU=UTL_GETUNIT()
 CALL OSD_OPEN(IU,FILE=TRIM(PREFVAL(1))//'\TMP\imodpath.run',STATUS='UNKNOWN')
 ...
 IF(SUM(IMODE).EQ.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You should specify at least one output variable!','Error')
  RETURN
 ENDIF
 CALL WDIALOGGETMENU(IDF_MENU1,I,FNAME)
 IF(.NOT.PLINESCHECKFNAME(IU,FNAME,'No Result File given'))RETURN
 CALL IUPPERCASE(FNAME)
 I=INDEXNOCASE(FNAME,'.',.TRUE.)
 IF(I.GT.0)FNAME=FNAME(:I-1)
 DO I=1,2
  IF(IMODE(I).EQ.1)THEN
   INQUIRE(FILE=TRIM(MDLDIRNAME)//'\'//TRIM(FNAME)//'.'//TRIM(EXT(I)),EXIST=LEX)
   IF(LEX)THEN
    CALL WMESSAGEBOX(YESNO,QUESTIONICON,COMMONNO,'Are you sure to overwrite '//CHAR(13)//&
       TRIM(TRIM(MDLDIRNAME)//'\'//TRIM(FNAME)//'.'//TRIM(EXT(I)))//' ?','Question')
 ...
 !FNAME=TRIM(MDLDIRNAME)//'\'//TRIM(FNAME)
 WRITE(IU,'(A)') '"'//TRIM(MDLDIRNAME)//'\'//TRIM(FNAME)//'", !## result file'
 ...
 ISNK=ISNK-1
 LINE=TRIM(ITOS(ISNK))//', !## isnk'
 WRITE(IU,'(A)') TRIM(LINE)
 !0 = PARTICLES PASS THROUGH CELLS WITH WEAK SINKS'
 !1 = PARTICLES ARE STOPPED WHEN THEY ENTER CELLS WITH INTERNAL SINKS'
 !2 = PARTICLES ARE STOPPED WHEN THEY ENTER CELLS WHERE DISCHARGE TO SINKS IS LARGER THAN A SPECIFIED FRACTION OF THE TOTAL INFLOW TO THE CELL'
 FRAC=0.99
 IF(ISNK.EQ.2)CALL WDIALOGGETREAL(IDF_REAL1,FRAC)
 ...
 IF(I.EQ.1)THEN
  CALL WDIALOGGETREAL(IDF_REAL1,TMAX)
  !## transform from year into days
  TMAX=TMAX*365.25 !days
 ELSE
  TMAX=1.0E+30     !days
 ENDIF
 ...
 LINE=TRIM(ITOS(JDATETOIDATE(JD1)))//', !## startdate';   WRITE(IU,'(A)') TRIM(LINE)
 LINE=TRIM(ITOS(JDATETOIDATE(JD1)))//', !## startwindow'; WRITE(IU,'(A)') TRIM(LINE)
 LINE=TRIM(ITOS(JDATETOIDATE(JD2)))//', !## endwindow';   WRITE(IU,'(A)') TRIM(LINE)
 ...
 IF(TRACEMAIN(TRIM(PREFVAL(1))//'\TMP\IMODPATH.RUN',0))THEN
  DO I=1,2
   IF(IMODE(I).EQ.1)THEN
    CALL IDFINIT(IDFNAMEGIVEN=TRIM(MDLDIRNAME)//'\'//TRIM(FNAME)//'.'//TRIM(EXT(I)))
   ENDIF
  ENDDO
 ENDIF
 ...
 IF(LEN_TRIM(FNAME).EQ.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'No '//TRIM(TXT)//' given.','Error')
  CLOSE(IU)
  RETURN
 ENDIF
 ...
 !###======================================================================
 SUBROUTINE PLINES1FIELDS(IDY,IDM,IDD)
 !###======================================================================
 IMPLICIT NONE
 ...
 CALL WDIALOGFIELDSTATE(IDF_LABEL4,K)
 K=ABS(K-1)
 ...
 IF(CODE.EQ.-1.OR.CODE.EQ.0)THEN
  IF(I.EQ.1)THEN
   IF(J.EQ.1)PLDIRNAME=TRIM(PREFVAL(1))//'\MODELS'
   IF(J.EQ.2)PLDIRNAME=TRIM(PREFVAL(1))//'\SCENARIOS'
   CALL UTL_IMODFILLMENU(IDF_MENU1,TRIM(PLDIRNAME),'*','D',PLNDIR,0,1)
  ELSE
   CALL WDIALOGGETSTRING(IDF_STRING1,PLBROWSENAME)
   MDLDIRNAME=PLBROWSENAME
   PLNDIR=1
!   CALL UTL_IMODFILLMENU(IDF_MENU1,TRIM(MDLDIRNAME),'*','D',PLNDIR,0,1)
  ENDIF
 ENDIF
 ...
 CALL WDIALOGFIELDSTATE(ID_SAVE,I)

 END SUBROUTINE PLINES2FIELDS
...
! CALL PLINESRESNAMEEXT(EXT)
 
 CALL WDIALOGSELECT(ID_DPATHTAB4)
 CALL UTL_IMODFILLMENU(IDF_MENU1,MDLDIRNAME,'*.i*f','F',N,0,0)  !## results for ipf and iff files
 CALL WDIALOGFIELDSTATE(IDF_MENU1,1)
 CALL WDIALOGPUTSTRING(IDF_LABEL1,'Give ResultName in: '//TRIM(MDLDIRNAME))
 ...
 CALL WDIALOGSELECT(ID_DPATHTAB1)
 CALL WDIALOGPUTSTRING(IDF_CHECK2,'Budget Flow Right Face [x] ('//TRIM(ITOS(NL(1)))//' layers)')
 CALL WDIALOGFIELDSTATE(IDF_CHECK2,MIN(1,NL(1)))
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,MIN(1,NL(1)))
 CALL WDIALOGPUTSTRING(IDF_CHECK3,'Budget Flow Front Face [y] ('//TRIM(ITOS(NL(2)))//' layers)')
 CALL WDIALOGFIELDSTATE(IDF_CHECK3,MIN(1,NL(2)))
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK3,MIN(1,NL(2)))
 CALL WDIALOGPUTSTRING(IDF_CHECK4,'Budget Flow Lower Face [z] ('//TRIM(ITOS(NL(3)))//' layers)')
 CALL WDIALOGFIELDSTATE(IDF_CHECK4,MIN(1,NL(3)))
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK4,MIN(1,NL(3)))

 !## no transient simulation available
 CALL WDIALOGSELECT(ID_DPATHTAB2)
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK1,MIN(1,NL(3)))
 ...
 INTEGER :: I,J,K,IT,IOS,ILAY,MAXNLAY,MN
 ...
 DO IT=1,MAXTYPE
  IF(N(IT).GT.0)THEN
   ...
   DO I=1,N(IT)
    CALL IUPPERCASE(LISTNAME(I,IT))
   ENDDO

   !## find maximal number of layers
   MAXNLAY=0
   DO I=1,N(IT)
    J=INDEXNOCASE(LISTNAME(I,IT),'_L',.TRUE.)
    IF(J.GT.0)THEN
     K=INDEXNOCASE(LISTNAME(I,IT),'.IDF',.TRUE.)
     IF(K.GT.0)THEN
      READ(LISTNAME(I,IT)(J+2:K-1),*,IOSTAT=IOS) ILAY
      IF(IOS.EQ.0)MAXNLAY=MAX(MAXNLAY,ILAY)
     ENDIF
    ENDIF
   END DO
   ...
   DO I=1,N(IT)
    !## determine max. number of layers continued
    J=INDEXNOCASE(LISTNAME(I,IT),'_L',.TRUE.)
    IF(J.GT.0)THEN
     K=INDEXNOCASE(LISTNAME(I,IT),'.IDF',.TRUE.)
     IF(K.GT.0)THEN
      READ(LISTNAME(I,IT)(J+2:K-1),*,IOSTAT=IOS) ILAY
      IF(IOS.EQ.0)IL(ILAY)=1
     ENDIF
    ENDIF
    !## determine period
    ...
    IF(J.EQ.0)THEN
     J=INDEXNOCASE(LISTNAME(I,IT),'_L',.TRUE.)
     K=INDEXNOCASE(LISTNAME(I,IT),TRIM(CTYPE(IT))//'_',.FALSE.)
     IF(J.GT.0.AND.K.GT.0)THEN
      K=K+LEN_TRIM(CTYPE(IT))+1
      J=J-1
      READ(LISTNAME(I,IT)(K:J),*) PLIPER(I,IT)
      !## generate julian-date
      PLIPER(I,IT)=IDATETOJDATE(PLIPER(I,IT))
      NT=NT+1
     ENDIF
    ELSE
     NS=NS+1
 ...
 IF(N.EQ.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'It is compulsory to have at least one *.isd file in the folder:'//CHAR(13)// &
   TRIM(PREFVAL(1))//'\STARTPOINTS','Error')
  CALL PLINES1CLOSE()
  RETURN
 ENDIF
 ...
 CALL WDIALOGSELECT(ID_DPATHTAB5)
 CALL WDIALOGRANGEREAL(IDF_REAL1,0.0,1.0)
 CALL WDIALOGSPINNERSTEP(IDF_REAL1,0.01,0.05)
