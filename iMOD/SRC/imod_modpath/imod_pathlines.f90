...
USE MOD_UTL, ONLY : JD,UTL_GETUNIT,ITOS,RTOS,UTL_MESSAGEHANDLE,UTL_GDATE,UTL_JDATETOIDATE,UTL_IDATETOJDATE,UTL_DIRINFO, &
     UTL_IMODFILLMENU,UTL_WSELECTFILE,UTL_CLOSEUNITS,UTL_CREATEDIR
USE DATEVAR
USE MOD_PLINES_PAR
...
! CALL PLINES1READFILES(ID)
 ...
 IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(PREFVAL(1))//'\TMP\imodpath.run',STATUS='UNKNOWN')
 IF(IU.LE.0)RETURN
 ...
 IF(IDB.EQ.IDOK)THEN
  ...
  CALL UTL_CLOSEUNITS()
 ELSE
  IF(TRACE_3D_INIT(TRIM(PREFVAL(1))//'\TMP\imodpath.run'))CALL IMOD3D_INIT(0,1)
 ENDIF
...
! CALL PLINESRESNAMEEXT(EXT)
 ...
 IF(N.EQ.0)THEN
  ...
  CALL WDIALOGSELECT(ID_DPATHTAB3)
  CALL WDIALOGFIELDSTATE(IDF_GROUP5,3)
