...
! SP%MXL =SP%SLAY
 ...
 ELSE
  ...
  I=1; DO
   ...
   ENDIF 
   SPNT(I)%TOPIDF=UTL_SUBST(SPNT(I)%TOPIDF,TRIM(REPLACESTRING),PREFVAL(5))
   ...
   SELECT CASE (SPNT(I)%ISHAPE)
    ...
    CASE (ID_LINE,ID_POINT,ID_POLYGON,ID_RECTANGLE,ID_CIRCLE)
     ...
     ENDIF
     SPNT(I)%BOTIDF=UTL_SUBST(SPNT(I)%BOTIDF,TRIM(REPLACESTRING),PREFVAL(5))
     READ(IU,'(A256)') LINE
     ...
     IF(SPNT(I)%IREF.EQ.1)THEN
      ...
      ENDIF
      SPNT(I)%REFIDF=UTL_SUBST(SPNT(I)%REFIDF,TRIM(REPLACESTRING),PREFVAL(5))
     ENDIF
   ...
   DO
    ...
    DO
     ...
     ELSE
      XCC=XC; YCC=YC
     ENDIF
     IF(XCC.GT.MIN(XCRD(1),XCRD(2)).AND.XCC.LT.MAX(XCRD(1),XCRD(2)).AND. &
        YCC.GT.MIN(YCRD(1),YCRD(2)).AND.YCC.LT.MAX(YCRD(1),YCRD(2)))THEN
