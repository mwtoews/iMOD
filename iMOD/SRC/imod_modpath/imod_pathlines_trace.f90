...
USE MOD_QKSORT, ONLY : UTL_WSORT
USE MOD_ASC2IDF_PAR, ONLY : IDFFILE,IGRIDFUNC,CS,NODATA,XYZFNAMES,IXCOL,IYCOL,IZCOL,TRIMDEPTH_IDF,ELLIPS_IDF
USE MOD_PREF_PAR, ONLY : PREFVAL
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,RTOS,JD,UTL_IDATETOJDATE,UTL_WAITMESSAGE,UTL_IDFGETDATE,UTL_CAP, &
           UTL_GETUNIQUE_CHAR,UTL_CREATEDIR,UTL_IDFSNAPTOGRID,UTL_WSELECTFILE,UTL_SUBST,UTL_DIALOGSHOW
USE MOD_IDF, ONLY : IDFREAD,IDFDEALLOCATEX,IDFALLOCATEX,IDFWRITE,IDFIROWICOL,IDFREADSCALE,IDFGETLOC,IDFCOPY,IDFDEALLOCATESX
USE MOD_PLINES_PAR
...
USE MOD_MANAGER_UTL, ONLY : MANAGER_UTL_DELETE,MANAGER_UTL_ADDFILE
USE MOD_DEMO_PAR
USE MOD_3D_PAR, ONLY : PLLISTINDEX,PLLISTCLR,PLLISTAGE,STPLISTINDEX,MIDPOS,TOP,BOT,IDFPLOT,IPATHLINE_3D
USE MOD_IPF, ONLY : IPFALLOCATE,IPFDEALLOCATE,IPFREAD2,NIPF,IPFFILE=>IPF

REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE,PRIVATE :: XSP,YSP,ZSP
...
CONTAINS
 ...
 IF(IDEF.EQ.1)THEN
  ...
  DO ILAY=1,NLAY
   !## top
   IF(INDEX(UTL_CAP(ITBFNAME(2,ILAY),'U'),'.IDF').GT.0)CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=ITBFNAME(2,ILAY),LDEACTIVATE=.FALSE.)
   !## bot
   IF(INDEX(UTL_CAP(ITBFNAME(3,ILAY),'U'),'.IDF').GT.0)CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=ITBFNAME(3,ILAY),LDEACTIVATE=.FALSE.)
  ENDDO
 ...
 !## age
 ALLOCATE(PLLISTAGE(N)); PLLISTAGE=0.0D0
 
 PL%TCUR=0.0D0
 PL%NPER=0
 PL%NTIME=0
 
 CALL WDIALOGPUTDOUBLE(IDF_REAL7,PL%TCUR) 
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (ID_SAVEAS)
      ...
      CALL WGRIDGETDOUBLE  (IDF_GRID1,5,SP%SPWIDTH,NSPG)
      CALL WGRIDGETDOUBLE  (IDF_GRID1,6,SP%PWIDTH,NSPG)
 ...
 IF(NSPG.GT.0)THEN
  ...
  CALL WGRIDPUTDOUBLE(IDF_GRID1,5,SP%SPWIDTH,NSPG)
  !## size of the particles
  CALL WGRIDPUTDOUBLE(IDF_GRID1,6,SP%PWIDTH,NSPG)
 ...
 IF(ID.EQ.ID_OPEN)THEN
  ...
  IF(.NOT.UTL_WSELECTFILE('iMOD Particle File (*.ptf)|*.ptf|',&
           LOADDIALOG+MUSTEXIST+PROMPTON+DIRCHANGE+APPENDEXT,FNAME,'Load iMOD Particle File (*.ptf)'))RETURN
 ...
 ELSE
  ...
  IF(.NOT.UTL_WSELECTFILE('iMOD Map (*.ipf)|*.ipf|iMOD Particle File (*.ptf)|*.ptf|',&
           SAVEDIALOG+DIRCHANGE+APPENDEXT,FNAME,'Save iMOD Particle File (*.ipf,*.ptf)'))RETURN
...
! write(*,*) nspg
 ...
 REAL(KIND=DP_KIND) :: R,G2R,Q,OR,QERROR,XC,YC,DZ,X,Y,Z,OFR
 LOGICAL,DIMENSION(2) :: LEX

 CALL WDIALOGGETDOUBLE(IDF_REAL10,Q)
 ...
 CALL WDIALOGGETDOUBLE(IDF_REAL14,R)             !## radius
 ...
 G2R=360.0D0/(2.0*PI); OR=360.0D0/REAL(NC); OR=OR/G2R
   ...
   DO ILAY=IL1,IL2; DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
    ...
    IF(ISTRONG.EQ.1)THEN
     ...
     IF(PL%IREV.EQ.0)THEN
      IF(QX(ICOL,IROW,ILAY).LT.0.0D0.OR.QX(ICOL+1,IROW,ILAY).GT.0.0D0.OR. &
         QY(ICOL,IROW,ILAY).GT.0.0D0.OR.QY(ICOL,IROW+1,ILAY).LT.0.0D0.OR. &
         QZ(ICOL,IROW,ILAY).GT.0.0D0.OR.QZ(ICOL,IROW,ILAY+1).LT.0.0D0)LEX(1)=.FALSE.
     !## backward simulation
     ELSE
      IF(QX(ICOL,IROW,ILAY).GT.0.0D0.OR.QX(ICOL+1,IROW,ILAY).LT.0.0D0.OR. &
         QY(ICOL,IROW,ILAY).LT.0.0D0.OR.QY(ICOL,IROW+1,ILAY).GT.0.0D0.OR. &
         QZ(ICOL,IROW,ILAY).LT.0.0D0.OR.QZ(ICOL,IROW,ILAY+1).GT.0.0D0)LEX(1)=.FALSE.
     ENDIF
    ENDIF
    !## no a strong sink
    IF(.NOT.LEX(1))CYCLE
    ...
    DO II=1,NC  !## number on circle      
     ...
     DO JJ=1,NZ !## number vertically
      ...
      IF(IRANDOM.EQ.0)THEN
       Z=Z-DZ
      ELSE
       CALL RANDOM_NUMBER(X); OFR=(X*360.0D0)/G2R; X=XC+R*COS(OFR); Y=YC+R*SIN(OFR)
 ...
 REAL(KIND=DP_KIND) :: DZ,XC,YC,Z,X
   ...
   CASE (5) !## all
    DO I=IC1,IC2,DC; DO J=IR1,IR2,DR
     IF(IRANDOM.EQ.0)THEN
      ICOL=I; IROW=J
     ELSE
      CALL RANDOM_NUMBER(X); ICOL=MAX(1.0D0,IDF%NCOL*X)
      CALL RANDOM_NUMBER(X); IROW=MAX(1.0D0,IDF%NROW*X)
     ENDIF
     IF(IBOUND(ICOL,IROW,ILAY).EQ.0)CYCLE
     DZ=ZTOP(ICOL,IROW,IL1)-ZBOT(ICOL,IROW,IL2)
     IF(IRANDOM.EQ.0)DZ=DZ/REAL(NZ)
 ...
 LOGICAL :: LIDF,LIPF
 LOGICAL,DIMENSION(2) :: LEX
 INTEGER :: I,J,II,IROW,ICOL,IIDF,INCLD,NX,JLAY,IRANDOM,N,M
 REAL(KIND=DP_KIND) :: DZ,X
 TYPE(IDFOBJ) :: PIDF
 INTEGER,ALLOCATABLE,DIMENSION(:) :: ISC,ISR
 ...
 CALL WDIALOGGETCHECKBOX(IDF_CHECK1,IRANDOM)
 CALL WDIALOGGETDOUBLE(IDF_REAL1,DZ)
 ...
 IF(LIDF)THEN
  ...
  CALL WDIALOGGETSTRING(IDF_STRING2,PIDF%FNAME)
  IF(ISP.EQ.2)THEN
   IF(.NOT.IDFREADSCALE(IDFPLOT(IIDF)%FNAME,PIDF,2,0,0.0D0,0))RETURN
  ELSEIF(ISP.EQ.5)THEN
   IF(.NOT.IDFREADSCALE(PIDF%FNAME,PIDF,2,0,0.0D0,0))RETURN
  ENDIF

  !## store unique number of col/row
  DO II=1,2
   M=0
   DO I=1,PIDF%NROW,NX; DO J=1,PIDF%NCOL,NX
    ...
    IF(II.EQ.2)THEN
     IF(IRANDOM.EQ.0)THEN
      ICOL=J; IROW=I
     ELSE
      CALL RANDOM_NUMBER(X); ICOL=MAX(1.0D0,PIDF%NCOL*X)
      CALL RANDOM_NUMBER(X); IROW=MAX(1.0D0,PIDF%NROW*X)
     ENDIF
     ISC(M)=ICOL; ISR(M)=IROW
    ENDIF
   ENDDO; ENDDO
   IF(II.EQ.1)ALLOCATE(ISC(M),ISR(M))
  ENDDO
  
  DO II=1,2
   N=0; M=0
   DO I=1,PIDF%NROW,NX; DO J=1,PIDF%NCOL,NX
    M=M+1; ICOL=ISC(M); IROW=ISR(M)
    IF(PIDF%X(ICOL,IROW).NE.PIDF%NODATA)THEN
     ...
     SELECT CASE (INCLD)
      CASE (1) !## both (-/+)
       LEX(1)=.TRUE.
      CASE (2) !## down (-)
       IF(X.GT.0.0D0)LEX(1)=.TRUE.
      CASE (3) !## up (+)
       IF(X.LT.0.0D0)LEX(1)=.TRUE.
     END SELECT
     IF(LEX(1))THEN
      N=N+1
      IF(II.EQ.2)THEN
  ...
  ENDDO
  CALL IDFDEALLOCATEX(PIDF); DEALLOCATE(ISC,ISR)
 ENDIF
 ...
 IF(LIPF)THEN
  ...
  DO I=1,N
   XSP(I)=IPFFILE(1)%XYZ(1,I); YSP(I)=IPFFILE(1)%XYZ(2,I); ZSP(I)=IPFFILE(1)%XYZ(3,I) 
  ENDDO
  CALL IPFDEALLOCATE()
 ENDIF
 ...
 INTEGER,INTENT(OUT) :: NG
 INTEGER,DIMENSION(:),INTENT(INOUT) :: NSGRP
 REAL(KIND=DP_KIND) :: XC,YC,ZC,X,Y,Z,DX,DY,DZ
 INTEGER :: NX,NZ,I,J,K,N
 
 NSGRP=0
 
 CALL WDIALOGSELECT(ID_D3DSETTINGS_LOCATION)
 CALL WDIALOGGETDOUBLE(IDF_REAL1,XC)
 CALL WDIALOGGETDOUBLE(IDF_REAL2,YC)
 CALL WDIALOGGETDOUBLE(IDF_REAL3,ZC)
 CALL WDIALOGGETDOUBLE(IDF_REAL11,X)
 CALL WDIALOGGETDOUBLE(IDF_REAL12,Y)
 CALL WDIALOGGETDOUBLE(IDF_REAL13,Z)
 ...
 REAL(KIND=DP_KIND) :: XC,YC,ZC,ZL,DZ
 ...
 DO I=IG1,IG2 
  ...
  DO ILAY=1,NLAY
   ...
   IF(ZC.LE.ZTOP(ICOL,IROW,ILAY).AND.ZC.GE.ZBOT(ICOL,IROW,ILAY))THEN
    ...
    DZ=ZTOP(ICOL,IROW,ILAY)-ZBOT(ICOL,IROW,ILAY)
    IF(DZ.NE.0.0D0)ZL=(ZC-ZBOT(ICOL,IROW,ILAY))/DZ
    IZL=1
   ENDIF
   IF(ZL.EQ.0.0D0.AND.ILAY.LT.NLAY)THEN
    ...
    IF(ZC.LT.ZBOT(ICOL,IROW,ILAY).AND.ZC.GT.ZTOP(ICOL,IROW,ILAY+1))THEN
     ...
     IF(DZ.NE.0.0D0)ZL=(ZTOP(ICOL,IROW,ILAY+1)-ZC)/DZ
     IZL=1
    ENDIF
   ENDIF
...
! VIEWDX=(TOP%X-BOT%X)/2.0_GLDOUBLE/XYZAXES(1)
! VIEWDY=(TOP%Y-BOT%Y)/2.0_GLDOUBLE/XYZAXES(2)
...
!   X=(X-MIDPOS%X)/VIEWDX; Y=(Y-MIDPOS%Y)/VIEWDY
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: DYEAR=365.25
 INTEGER :: I,IPART,IDSCH,NPACT,IG,ITRAPPED,NTAIL,IFREQ,ICAPT
 REAL(KIND=DP_KIND) :: TIME,TTMAX,MAXVELOCITY,XTREP
 ...
 CALL WDIALOGGETDOUBLE(IDF_REAL4,PL%TMAX); PL%TMAX=PL%TMAX*DYEAR
 CALL WDIALOGGETDOUBLE(IDF_REAL5,PL%TDEL); PL%TDEL=PL%TDEL*DYEAR
 CALL WDIALOGGETDOUBLE(IDF_REAL6,XTREP)
 CALL WDIALOGGETDOUBLE(IDF_REAL7,PL%TCUR); PL%TCUR=PL%TCUR*DYEAR
 ...
 CALL WDIALOGGETDOUBLE(IDF_REAL8,FRAC)
 
 PL%NTREP=INT((XTREP+1.0D0)*REAL(NTAIL))
 ...
 !## current time/years
 PL%TCUR=TTMAX/DYEAR; CALL WDIALOGPUTDOUBLE(IDF_REAL7,PL%TCUR) 
...
!   ENDIF
   ...
   IF(SPR(IG)%KLC(IPART).GT.0)THEN
    ...
    CALL FLOLIN(IPART,IMODE(1),TIME,TTMAX,IDSCH, &
                ...
                MAXVELOCITY,DELX,DELY,SPR(IG)%MXL(IPART),IVISIT,LVISIT,NVISIT) 
...
!    ENDIF
 ...
 CALL WDIALOGPUTDOUBLE(IDF_REAL7,0.0D0,'(F15.3)')
...
! IF(ALLOCATED(LVISIT))DEALLOCATE(LVISIT)
 ...
 !###======================================================================
 LOGICAL FUNCTION TRACEMAIN(RUNFILE,IBATCH,ICONVERTGEN,ZOOMIDF)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: ZOOMIDF
 ...
 IF(TRACEREADRUNFILE(RUNFILE,IBATCH))THEN
  ...
  !## trace particles
  IF(TRACE_CALC(IBATCH,IULOG,ICONVERTGEN,ZOOMIDF))TRACEMAIN=.TRUE.
 ...
 !###======================================================================
 SUBROUTINE TRACE_BRING_IN_DATA(MODELIDF,N,TOP,BOT)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N
 TYPE(IDFOBJ),INTENT(INOUT) :: MODELIDF
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(N) :: TOP,BOT
 INTEGER :: ILAY,IROW,ICOL,NROW,NCOL
 
 NROW=TOP(1)%NROW
 NCOL=TOP(1)%NCOL
 NLAY=N
 
 CALL IDFCOPY(MODELIDF,IDF)
 ALLOCATE(ZTOP(NCOL,NROW,NLAY),ZBOT(NCOL,NROW,NLAY),IBOUND(NCOL,NROW,NLAY)); IBOUND=0
 DO ILAY=1,NLAY
  DO IROW=1,NROW; DO ICOL=1,NCOL
   ZTOP(ICOL,IROW,ILAY)=TOP(ILAY)%X(ICOL,IROW)
   ZBOT(ICOL,IROW,ILAY)=BOT(ILAY)%X(ICOL,IROW)
   IF(ZTOP(ICOL,IROW,ILAY).NE.TOP(ILAY)%NODATA.AND. &
      ZBOT(ICOL,IROW,ILAY).NE.BOT(ILAY)%NODATA)IBOUND(ICOL,IROW,ILAY)=1
  ENDDO; ENDDO
 ENDDO
 
 END SUBROUTINE TRACE_BRING_IN_DATA
 
 !###======================================================================
 LOGICAL FUNCTION TRACE_CALC(IBATCH,IULOG,ICONVERTGEN,ZOOMIDF)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: ZOOMIDF
 ...
 !## define zoom area for particle tracking - overrule area
 IF(ZOOMIDF%XMAX-ZOOMIDF%XMIN.GT.0.0D0)THEN
  IDF%XMIN=ZOOMIDF%XMIN; IDF%XMAX=ZOOMIDF%XMAX
  IDF%YMIN=ZOOMIDF%YMIN; IDF%YMAX=ZOOMIDF%YMAX
  CALL UTL_IDFSNAPTOGRID(IDF%XMIN,IDF%XMAX,IDF%YMIN,IDF%YMAX,IDF%DX,IDF%NCOL,IDF%NROW)
 ...
 DO ISPFNAME=1,NSPFNAME
  ...
  IF(.NOT.TRACEPREPARESP(SP(ISPFNAME)%NPART,IBATCH,0))RETURN 
 ...
 DO
  ...
  IF(LEX)THEN
   ...
   IF(ISS.EQ.1)WRITE(IULOG,'(1X,A,I10,A,2(F10.2,A))') 'Using following files for current stressperiod',IPER,' time upto: ', &
                             TTMAX,' days (timestep',DT,' days)'
   DO ILAY=1,NLAY
...
!      ') startpoints from '//TRIM(SPFNAME(ISPFNAME))//' ...'
    ...
    DO IPART=1,SP(ISPFNAME)%NPART 
     ...
     IF(SPR(ISPFNAME)%KLC(IPART).NE.0)THEN
      ...
      CALL FLOLIN(IPART,IMODE(1),TIME,TTMAX,IDSCH, &
                  ...
                  MAXVELOCITY,DELX,DELY,SPR(ISPFNAME)%MXL(IPART),IVISIT,LVISIT,NVISIT)
      !## clean visited places that have been visited before
      DO I=1,NVISIT; IVISIT(LVISIT(I))=INT(0,1); ENDDO
     ...
     IF(IBATCH.EQ.0)CALL UTL_WAITMESSAGE(IRAT,IRAT1,IPART,SP(ISPFNAME)%NPART,'Busy tracking particles ('//TRIM(ITOS(SP(ISPFNAME)%NPART))// &
      ') startpoints from '//TRIM(SPFNAME(ISPFNAME))//' ...')
 ...
 !###======================================================================
 SUBROUTINE TRACEPOSTPROCESSING(IFFFLOW,IPFFLOW,IDFFLOW,IPFFNAME,IPFICOL,ICONVERTGEN,TOPFNAME,BOTFNAME,IEXTRACT) 
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: IFFFLOW,IPFFLOW,IDFFLOW,IPFFNAME,TOPFNAME,BOTFNAME
 INTEGER,INTENT(IN),DIMENSION(4) :: IPFICOL
 INTEGER,INTENT(IN) :: ICONVERTGEN,IEXTRACT
 CHARACTER(LEN=256) :: LINE
 TYPE IFFOBJ
  INTEGER :: ID,ILAY,IROW,ICOL
  REAL(KIND=DP_KIND) :: X,Y,Z,T,V
 END TYPE IFFOBJ
 TYPE(IFFOBJ),DIMENSION(:),POINTER :: IFF,IFF_BU
 TYPE(IDFOBJ) :: TOPIDF,BOTIDF
 INTEGER :: IU,I,J,N,IOS,NROW,NCOL,EPIROW,EPICOL,EPILAY,EPCMTT,EPX,EPY,EPZ,J1,J2
 REAL(KIND=DP_KIND) :: EPS,Z1,Z2
 ...
 IF(TRIM(IPFFNAME).NE.'')THEN
  IF(.NOT.TRACEPOSTPROCESSING_INIT(IPFFNAME,IPFICOL))RETURN
  IF(TRIM(IDFFLOW).NE.'')THEN 
   IF(.NOT.IDFREAD(IDF,IDFFLOW,0))RETURN; EPS=IDF%DX/10.0D0
  ENDIF
  !## get ipf-information - equal to imodflow to assign well to locations
  DO I=1,SIZE(IPF); CALL IDFIROWICOL(IDF,IPF(I)%IROW,IPF(I)%ICOL,IPF(I)%X,IPF(I)%Y); ENDDO
 ELSE
  IF(TRIM(TOPFNAME).NE.'')THEN; IF(.NOT.IDFREAD(TOPIDF,TOPFNAME,1))RETURN; ENDIF
  IF(TRIM(BOTFNAME).NE.'')THEN; IF(.NOT.IDFREAD(BOTIDF,BOTFNAME,1))RETURN; ENDIF
  NUNQ=1; ALLOCATE(IPF(NUNQ)); IPF(1)%UNQLABEL='3D_iextract'//TRIM(ITOS(IEXTRACT)); IPF(1)%INQ=1
 ENDIF 
 ...
 DO I=1,NUNQ
  IF(IMODE(1).EQ.1)THEN
   JU(I,1)=UTL_GETUNIT(); J=INDEX(IFFFLOW,'.',.TRUE.)-1; IF(J.LE.0)J=LEN_TRIM(IFFFLOW)
  ...
  IF(IMODE(2).EQ.1)THEN
   JU(I,2)=UTL_GETUNIT(); J=INDEX(IPFFLOW,'.',.TRUE.)-1; IF(J.LE.0)J=LEN_TRIM(IPFFLOW)
 ...
 IF(IMODE(1).EQ.1)THEN
  ...
  ALLOCATE(IFF(100))
  READ(IMODE(1),*,IOSTAT=IOS) IFF(1)%ID,IFF(1)%ILAY,IFF(1)%X,IFF(1)%Y,IFF(1)%Z,IFF(1)%T,IFF(1)%V,IFF(1)%IROW,IFF(1)%ICOL
  
  !## process entire particle
  N=1; DO
   N=N+1
   IF(N.GT.SIZE(IFF))THEN
    IF(N.GT.10000)THEN
     ...
     ALLOCATE(IFF(100))
     DO
      READ(IMODE(1),*) IFF(2)%ID,IFF(2)%ILAY,IFF(2)%X,IFF(2)%Y,IFF(2)%Z,IFF(2)%T,IFF(2)%V
      IF(IFF(2)%ID.NE.IFF(1)%ID)THEN; IFF(1)=IFF(2); N=2; EXIT; ENDIF
     ENDDO
    ELSE
     ALLOCATE(IFF_BU(N+100),STAT=IOS)
     DO I=1,SIZE(IFF); IFF_BU(I)=IFF(I); ENDDO; DEALLOCATE(IFF); IFF=>IFF_BU
    ENDIF
   ENDIF
   READ(IMODE(1),*,IOSTAT=IOS) IFF(N)%ID,IFF(N)%ILAY,IFF(N)%X,IFF(N)%Y,IFF(N)%Z,IFF(N)%T,IFF(N)%V,IFF(N)%IROW,IFF(N)%ICOL
   
   !## new particle starts - evaluate particle
...
IFFLOOP: DO I=1,SIZE(IPF)
     ...
     IF(TRIM(IPFFNAME).NE.'')THEN
      ...
      LEX=IFF(N-1)%ILAY.EQ.IPF(I)%ILAY.AND. &
          IFF(N-1)%IROW.EQ.IPF(I)%IROW.AND. &
          IFF(N-1)%ICOL.EQ.IPF(I)%ICOL
      J1=1; J2=N-1

     ELSE

      LEX=.FALSE.
      SELECT CASE (IEXTRACT)
       !## test if any of the fragments pass the 3d zone
       CASE (1,2,3)
        J1=0; J2=0; DO J=1,N-1
         IF(TRACEPOSTPROCESSING_Z1Z2(IFF(J)%X,IFF(J)%Y,TOPIDF,BOTIDF,Z1,Z2))THEN
          IF(IFF(J)%Z.LE.Z1.AND.IFF(J)%Z.GE.Z2)THEN
           !## start of the (first) zone
           IF(J1.EQ.0)J1=J
          ELSE
           !## end of the (first) zone
           IF(J1.NE.0.AND.J2.EQ.0)J2=J
          ENDIF
         ENDIF
        ENDDO
        !## okay somewhere crosses the 3d zone - determine what part to write
        IF(J1.NE.0)THEN
         LEX=.TRUE.
         IF(IEXTRACT.EQ.1)THEN
          J1=1; J2=N-1
         ELSEIF(IEXTRACT.EQ.2)THEN
          J2=J1; J1=1
         ELSEIF(IEXTRACT.EQ.3)THEN
          IF(J2.GT.0)THEN
           J1=J2; J2=N-1; IF(J1.EQ.0)J1=1
          ELSE
           LEX=.FALSE.
          ENDIF
         ENDIF
        ENDIF
       !## test end of pathline
       CASE (4)
        J=N-1
        IF(TRACEPOSTPROCESSING_Z1Z2(IFF(J)%X,IFF(J)%Y,TOPIDF,BOTIDF,Z1,Z2))THEN
         IF(IFF(J)%Z.LE.Z1.AND.IFF(J)%Z.GE.Z2)LEX=.TRUE.; J1=1; J2=J
        ENDIF
      END SELECT
     ...
     IF(LEX)THEN
      ...
      DO J=J1,J2
       WRITE(JU(IPF(I)%INQ,1),'(2(I10,1X),5(E15.7,1X),2(I10,1X))') IFF(J)%ID,IFF(J)%ILAY,IFF(J)%X,IFF(J)%Y,IFF(J)%Z,IFF(J)%T,IFF(J)%V,IFF(J)%IROW,IFF(J)%ICOL
      ENDDO
      EXIT IFFLOOP
     ENDIF   
 ...
 IF(IMODE(2).EQ.1)THEN
  ...
  DO I=1,NCOL;
   ...
   IF(TRIM(CATTRIB).EQ.'EP_XCRD.')   EPX   =I
   IF(TRIM(CATTRIB).EQ.'END_XCRD.')  EPX   =I
   IF(TRIM(CATTRIB).EQ.'EP_YCRD.')   EPY   =I
   IF(TRIM(CATTRIB).EQ.'END_YCRD.')  EPY   =I
   IF(TRIM(CATTRIB).EQ.'EP_ZCRD.')   EPZ   =I
   IF(TRIM(CATTRIB).EQ.'END_ZCRD.')  EPZ   =I
  ENDDO
  READ(IMODE(2),*)
...
IPFLOOP: DO I=1,SIZE(IPF)
   ...
   ELSE
    ...
    IF(TRACEPOSTPROCESSING_Z1Z2(XYZ(EPX),XYZ(EPY),TOPIDF,BOTIDF,Z1,Z2))THEN
     IF(XYZ(EPZ).LE.Z1.AND.XYZ(EPZ).GE.Z2)THEN  
      NP(IPF(1)%INQ,2)=NP(IPF(1)%INQ,2)+1
      WRITE(JU(IPF(1)%INQ,2),'(A)') TRIM(LINE)
     ENDIF
    ENDIF
 ...
 WRITE(*,'(/3A10)') 'UNIQUE','N_IFF','N_IPF'
 !## construct header for ipf files
 DO I=1,NUNQ
  WRITE(*,'(3I10)') I,NP(I,1),NP(I,2)
  IF(IMODE(2).EQ.1)THEN
   IF(NP(I,2).GT.0)THEN
    JU(I,2)=UTL_GETUNIT()
 ...
 !###======================================================================
 LOGICAL FUNCTION TRACEPOSTPROCESSING_Z1Z2(X,Y,TOPIDF,BOTIDF,Z1,Z2)
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: X,Y
 REAL(KIND=DP_KIND),INTENT(OUT) :: Z1,Z2
 TYPE(IDFOBJ),INTENT(IN) :: TOPIDF,BOTIDF
 INTEGER :: IROW,ICOL
 
 TRACEPOSTPROCESSING_Z1Z2=.FALSE.
 
 Z1=HUGE(1.0D0)
 IF(ASSOCIATED(TOPIDF%X))THEN
  CALL IDFIROWICOL(TOPIDF,IROW,ICOL,X,Y)
  IF(ICOL.GT.0.AND.ICOL.LE.TOPIDF%NCOL.AND. &
     IROW.GT.0.AND.IROW.LT.TOPIDF%NROW)THEN
   Z1=TOPIDF%X(ICOL,IROW); IF(Z1.EQ.TOPIDF%NODATA)RETURN
  ELSE
   !## outside idf extent
   RETURN
  ENDIF
 ...
 Z2=-HUGE(1.0D0)
 IF(ASSOCIATED(BOTIDF%X))THEN
  CALL IDFIROWICOL(BOTIDF,IROW,ICOL,X,Y)
  IF(ICOL.GT.0.AND.ICOL.LE.BOTIDF%NCOL.AND. &
     IROW.GT.0.AND.IROW.LT.BOTIDF%NROW)THEN
   Z2=BOTIDF%X(ICOL,IROW); IF(Z2.EQ.BOTIDF%NODATA)RETURN
  ELSE
   !## outside idf extent
   RETURN
  ENDIF
 ...
 TRACEPOSTPROCESSING_Z1Z2=.TRUE.
 
 END FUNCTION TRACEPOSTPROCESSING_Z1Z2
...
!###======================================================================
 ...
 IF(IBATCH.EQ.1)WRITE(*,'(A10,I4)') 'NLAY: ',NLAY
 READ(IU,'(A256)',IOSTAT=IOS) LINE; IF(.NOT.TRACECHECKRUN(IOS,'NPER',IU))RETURN
 READ(LINE,*,IOSTAT=IOS) NPER,ISTO
 IF(IOS.NE.0)THEN
 ...
 ELSE
  ALLOCATE(HFFNAME(4,NLAY,NPER))  !## bdgfrf,bdgfff,bdgflf,bdgsto
 ENDIF
 ...
 IF(IOS.EQ.0)THEN
  ...
  DO I=1,NSPFNAME
   ...
   SPFNAME(I)=UTL_SUBST(SPFNAME(I),TRIM(REPLACESTRING),PREFVAL(5))
   ...
   IFFFNAME(I)=UTL_SUBST(IFFFNAME(I),TRIM(REPLACESTRING),PREFVAL(5))
 ...
 ELSE
  ...
  SPFNAME(1)=UTL_SUBST(SPFNAME(1),TRIM(REPLACESTRING),PREFVAL(5))
  IF(IBATCH.EQ.1)WRITE(*,'(A)') 'SPFNAME: '//TRIM(SPFNAME(1))
  READ(IU,*,IOSTAT=IOS) IFFFNAME(1); IF(.NOT.TRACECHECKRUN(IOS,'IFFFNAME',IU))RETURN
  IFFFNAME(1)=UTL_SUBST(IFFFNAME(1),TRIM(REPLACESTRING),PREFVAL(5))
 ...
 IF(ISNK.NE.2)FRAC=1.0D0; FRAC=MIN(1.0D0,MAX(0.0D0,FRAC))
 ...
 DO ILAY=1,NLAY
  DO J=1,5
   IF(J.NE.5.OR.ILAY.NE.NLAY)THEN
    ...
    ITBFNAME(J,ILAY)=UTL_CAP(ITBFNAME(J,ILAY),'U')
    ITBFNAME(J,ILAY)=UTL_SUBST(ITBFNAME(J,ILAY),TRIM(REPLACESTRING),PREFVAL(5))
  ...
  DO ILAY=1,NLAY
   DO J=1,NJ
    IF(J.NE.3.OR.ILAY.NE.NLAY)THEN
     ...
     HFFNAME(J,ILAY,IPER)=UTL_CAP(HFFNAME(J,ILAY,IPER),'U')
     HFFNAME(J,ILAY,IPER)=UTL_SUBST(HFFNAME(J,ILAY,IPER),TRIM(REPLACESTRING),PREFVAL(5))
     ...
     ELSEIF(J.EQ.4)THEN
      ...
      IF(.NOT.TRACECHECKRUN(IOS,'BDGSTO FOR LAYER '//TRIM(ITOS(ILAY))//' IPER '//TRIM(ITOS(IPER)),IU))RETURN
      LINE='BDGSTO FOR LAYER'//TRIM(ITOS(ILAY))//'='//TRIM(HFFNAME(J,ILAY,IPER))
      IF(IBATCH.EQ.1)WRITE(*,'(A)') LINE
      IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'BDGSTO FOR LAYER'//TRIM(ITOS(ILAY))//'='//TRIM(HFFNAME(J,ILAY,IPER)))
 ...
 PLIPER(NPER+1,1)=PLIPER(NPER,1)+(JD2-PLIPER(NPER,1))
 ...
 IF(ALLOCATED(QZ))      DEALLOCATE(QZ);     IF(ALLOCATED(QS))   DEALLOCATE(QS)
 ...
 DIST=SQRT((SP(IG)%XLC(IPART)-SPR(IG)%XLC(IPART))**2.0D0+ &
 ...
 !###======================================================================
 REAL(KIND=DP_KIND) FUNCTION TRACEGETV(XYZT,APOR)
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: APOR
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(4,3) :: XYZT
 REAL(KIND=DP_KIND) :: DX,DY,DZ,DT,D

 DX=XYZT(1,2)-XYZT(1,1)
 DY=XYZT(2,2)-XYZT(2,1)
 DZ=XYZT(3,2)-XYZT(3,1)
 DT=XYZT(4,2)-XYZT(4,1)
 D =SQRT(DX**2.0D0+DY**2.0D0+DZ**2.0D0)
 !#m/year
 TRACEGETV=D/DT*APOR
 ...
 IF(ISTO.EQ.1)ALLOCATE(QS(IDF%NCOL,IDF%NROW,NLAY))
 ...
 !## particle groups
 ALLOCATE(SP(NG)); SP%NPART=0; SP%ICLR=0; SP%IACT=0; SP%IREV=0; SP%SPWIDTH=0.0D0; SP%PWIDTH=0.0D0
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: QCRIT=0.01D0  !## smaller value might be noise
 INTEGER,INTENT(IN) :: IBATCH,IPER
 CHARACTER(LEN=256) :: STRING
 INTEGER :: ILAY,IROW,ICOL,ITYPE
 REAL(KIND=DP_KIND) :: QERROR
 TYPE(WIN_MESSAGE) :: MESSAGE
 TYPE(IDFOBJ) :: IDFTMP
...
!  IF(IBATCH.EQ.1)WRITE(*,'(A)') TRIM(STRING)
  ...
  IF(.NOT.IDFREADSCALE(HFFNAME(1,ILAY,IPER),IDFTMP,10,1,0.0D0,0))RETURN !## block-value
  DO ICOL=1,IDF%NCOL+1; DO IROW=1,IDF%NROW
   IF(IDFTMP%X(ICOL,IROW).EQ.IDFTMP%NODATA)THEN
    QX(ICOL,IROW,ILAY)=0.0D0
   ELSE
...
!  IF(IBATCH.EQ.1)WRITE(*,'(A)') TRIM(STRING)
  ...
  IF(.NOT.IDFREADSCALE(HFFNAME(2,ILAY,IPER),IDFTMP,10,1,0.0D0,0))RETURN !## block-value
  DO ICOL=1,IDF%NCOL; DO IROW=1,IDF%NROW+1
   IF(IDFTMP%X(ICOL,IROW).EQ.IDFTMP%NODATA)THEN
    QY(ICOL,IROW,ILAY)=0.0D0
   ELSE
...
!   IF(IBATCH.EQ.1)WRITE(*,'(A)') TRIM(STRING)
   ...
   IF(.NOT.IDFREADSCALE(HFFNAME(3,ILAY,IPER),IDF,10,1,0.0D0,0))RETURN !## block-value
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
    IF(IDF%X(ICOL,IROW).EQ.IDF%NODATA)THEN
     QZ(ICOL,IROW,ILAY)=0.0D0
    ELSE
...
!   IF(IBATCH.EQ.1)WRITE(*,'(A)') TRIM(STRING)
   ...
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
    IF(IDF%X(ICOL,IROW).EQ.IDF%NODATA)THEN
     QS(ICOL,IROW,ILAY)=0.0D0
    ELSE
     QS(ICOL,IROW,ILAY)=IDF%X(ICOL,IROW) 
    ENDIF
   ENDDO; ENDDO
  ENDDO
 ENDIF
 ...
 DO ILAY=1,NLAY
  DO IROW=1,IDF%NROW
   DO ICOL=1,IDF%NCOL
    ...
    !## add storage as a known source
    IF(ISTO.EQ.1)QERROR=QERROR+QS(ICOL,IROW,ILAY)

    QSS(ICOL,IROW,ILAY)=-QERROR
    !## original code flags all buff().lt.0 - potential escape location
...
! if(idfwrite(idf,idf%fname,1))then; endif
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: MINTHICKNESS=0.01D0 !## OTHERWISE TRACING IS PROBLEM, DIVIDING BY ZERO AND NO PARTICLE BEHAVIOR IN Z-DIRECTION
 INTEGER,INTENT(IN) :: IBATCH
 INTEGER,INTENT(OUT) :: NCONS
 CHARACTER(LEN=256) :: STRING
 REAL(KIND=DP_KIND) :: APOR,XZTOP,XZBOT
 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   ...
   ENDIF
   IF(LEX)THEN
    IF(.NOT.IDFREADSCALE(ITBFNAME(1,ILAY),IDF,1,1,0.0D0,0))RETURN !## scale boundary
 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   ...
   IF(.NOT.IDFREADSCALE(ITBFNAME(2,ILAY),IDF,2,1,0.0D0,0))RETURN !## scale mean
   ZTOP(:,:,ILAY)=IDF%X
 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   ...
   IF(.NOT.IDFREADSCALE(ITBFNAME(3,ILAY),IDF,2,1,0.0D0,0))RETURN !## scale mean
   ZBOT(:,:,ILAY)=IDF%X   
   !## adjust boundary whenever top
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   ...
   IF(.NOT.IDFREADSCALE(ITBFNAME(4,ILAY),IDF,2,1,0.0D0,0))RETURN !## scale mean
   POR(:,:,ILAY)=IDF%X   
   ...
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
    ...
    IF(IBOUND(ICOL,IROW,ILAY).NE.0)THEN
     IF(POR(ICOL,IROW,ILAY).GE.1.0D0.OR.POR(ICOL,IROW,ILAY).LE.0.0D0)THEN
      IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You cannot specify a porosity of '//TRIM(RTOS(POR(ICOL,IROW,ILAY),'F',3))//CHAR(13)// &
         'It need to be always 0.0D0 > porosity < 1.0D0','Error')
      IF(IBATCH.EQ.1)THEN; WRITE(*,'(A)') 'You cannot specify a porosity of '//TRIM(RTOS(POR(ICOL,IROW,ILAY),'F',3))
                     WRITE(*,'(A)') 'It need to be always 0.0D0 > porosity < 1.0D0'; ENDIF
      RETURN
     ENDIF
    ENDIF
   ENDDO; ENDDO
 ...
 DO ILAY=1,NLAY-1
  ...
  ELSE
   ...
   IF(.NOT.IDFREADSCALE(ITBFNAME(5,ILAY),IDF,2,1,0.0D0,0))RETURN !## scale mean
   POR(:,:,NLAY+ILAY)=IDF%X   
   !## adjust boundary whenever por
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
