...
USE MOD_DEMO_PAR
USE MOD_3D_PAR, ONLY : PLLISTINDEX,PLLISTCLR,PLLISTAGE,MIDPOS,TOP,BOT,XYZAXES,IDFPLOT
...
!                  RUNFILE,'Select iMODPATH runfile (*.run)'))RETURN
 ...
 !## drawinglist index
 ALLOCATE(PLLISTINDEX(N)); PLLISTINDEX=0
 !## colour fraction
 ...
 CALL WDIALOGPUTREAL(IDF_REAL7,PL%TCUR) !; CALL WDIALOGPUTINTEGER(IDF_INTEGER1,PL_NPER)
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,K,NX,NZ,N,ISP,IROW,ICOL,ILAY,IOPT,ISTRONG
 REAL :: X,Y,Z,DX,DZ,XC,YC,ZC,Q,QERROR
    ...
    DO ILAY=1,NLAY; DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
     ...
     IF(LEX)THEN
      ...
      IF(I.EQ.2)THEN
       ...
       ZSP(N)=0.5*(ZTOP(ICOL,IROW,ILAY)-ZBOT(ICOL,IROW,ILAY))
      ENDIF
     ENDIF
...
! MAXILAY =SLAY
 ...
 INTEGER :: I,IPART,IDSCH,NPACT,NPER
 ...
 !## restart in stack
 IF(PL%NPER.GT.SIZE(PLLISTINDEX))PL%NPER=1
 ...
 !## open drawing list per timestep/call
 IF(PLLISTINDEX(PL%NPER).NE.0)CALL GLDELETELISTS(PLLISTINDEX(PL%NPER),1_GLSIZEI)
 !## list index for,  !## start new drawing list
 PLLISTINDEX(PL%NPER)=GLGENLISTS(1); CALL GLNEWLIST(PLLISTINDEX(PL%NPER),GL_COMPILE)
 ...
 IF(NPACT.LE.0)THEN
  CALL GLDELETELISTS(PLLISTINDEX(PL%NPER),1_GLSIZEI)
  PLLISTINDEX(PL%NPER)=0
 ENDIF
