...
USE MOD_DEMO_PAR
USE MOD_3D_PAR, ONLY : PLLISTINDEX,PLLISTCLR,PLLISTAGE,STPLISTINDEX,MIDPOS,TOP,BOT,XYZAXES,IDFPLOT,SPGCLR,IPATHLINE_3D
 ...
 DO IG=1,NSPG
  ...
  DO IPART=1,SP(IG)%NPART 
   ...
   IF(SPR(IG)%KLC(IPART).GT.0)THEN
    ...
    IF(IDSCH.NE.0.OR.TIME.GE.PL%TMAX)SPR(IG)%KLC(IPART)=0
