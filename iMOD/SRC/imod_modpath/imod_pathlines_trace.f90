...
USE MOD_DEMO_PAR
USE MOD_3D_PAR, ONLY : PLLISTINDEX,PLLISTCLR,PLLISTAGE,STPLISTINDEX,MIDPOS,TOP,BOT,XYZAXES,IDFPLOT,IPATHLINE_3D
...
!    CALL IMOD3D_DISPLAY(1)
 ...
 IF(ID.EQ.ID_OPEN)THEN
  ...
  READ(IU,'(I10)') NSPG
  DO I=1,NSPG
   READ(IU,'(4I10)') SP(I)%NPART,SP(I)%IACT,SP(I)%ICLR,SP(I)%IREV
  ...
  ENDDO
  CLOSE(IU)
  CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfully read particles from'//CHAR(13)//TRIM(FNAME),'Information')
 ...
 ELSE
  ...
  ELSE
   WRITE(IU,'(I10)') NSPG
   DO I=1,NSPG
    WRITE(IU,'(4I10)') SP(I)%NPART,SP(I)%IACT,SP(I)%ICLR,SP(I)%IREV
    DO J=1,SP(I)%NPART
     WRITE(IU,'(3F10.2)') IDF%XMIN+SP(I)%XLC(J),IDF%YMIN+SP(I)%YLC(J),SP(I)%ZLC(J)
    ENDDO
   ENDDO
  ENDIF
  CLOSE(IU)
 ...
 INTEGER :: I,J,II,JJ,K,NX,NZ,N,ISP,IROW,ICOL,ILAY,ISTRONG,IC1,IC2, &
    IR1,IR2,ISHAPE,NC,IRANDOM,DR,DC,IIDF,INCLD
 ...
 SELECT CASE (ISP)
  ...
  CASE (2,5)
   ...
   IF(LIDF)THEN  
    ...
    DO I=1,2
     N=0
     DO IROW=1,PIDF%NROW,NX; DO ICOL=1,PIDF%NCOL,NX
      IF(PIDF%X(ICOL,IROW).NE.PIDF%NODATA)THEN
       ...
       SELECT CASE (J)
        CASE (1) !## both (-/+)
         LEX(1)=.TRUE.
        CASE (2) !## down (-)
         IF(PIDF%X(ICOL,IROW).GT.0.0)LEX(1)=.TRUE.
        CASE (3) !## up (+)
         IF(PIDF%X(ICOL,IROW).LT.0.0)LEX(1)=.TRUE.
       END SELECT
       IF(LEX(1))THEN
        N=N+1
        IF(I.EQ.2)THEN
  ...
  CASE (4)
   ...
   ELSE
    N=NZ*MAX(IC2,IR2)
   ENDIF
   N=SUM(ILAYERS)*N; ALLOCATE(XSP(N),YSP(N),ZSP(N))

   N=0
   DO ILAY=1,NLAY
    IF(ILAYERS(ILAY).EQ.0)CYCLE
    SELECT CASE (IOPT(1))
     CASE (1,4,5) !## west/east/all
...
! ENDIF
 ...
 CALL WDIALOGGETCHECKBOX(IDF_CHECK3,IFREQ); IF(IFREQ.EQ.1)ITRAPPED=0
 ...
 DO IG=1,NSPG
  ...
  DO IPART=1,SPR(IG)%NPART 
   ...
   IF(SPR(IG)%KLC(IPART).GT.0)THEN
    ...
    IF(IDSCH.NE.0)SPR(IG)%KLC(IPART)=-IDSCH !0
   ...
   ELSE
    ...
    IF(ITRAPPED.EQ.1)THEN
     ...
     IF(ICAPT.GT.0.AND.SP(IG)%KLC(IPART).LT.0)THEN
      IF(ICAPT.NE.ABS(SP(IG)%KLC(IPART)))CYCLE
     ENDIF
