...
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,RTOS,JD,IDATETOJDATE,UTL_WAITMESSAGE,UTL_IDFGETDATE,UTL_CAP,UTL_GETUNIQUE_CHAR,UTL_CREATEDIR
  ...
  DO J=1,5
   IF(J.NE.5.OR.ILAY.NE.NLAY)THEN
    ...
    IF(J.EQ.1)THEN
     ...
     WRITE(*,'(A10,A)') 'IBOUND'//TRIM(ITOS(ILAY))//'=',TRIM(ITBFNAME(J,ILAY))
    ELSEIF(J.EQ.2)THEN
     IF(.NOT.TRACECHECKRUN(IOS,'TOP FOR LAYER '//TRIM(ITOS(ILAY))))RETURN
     WRITE(*,'(A10,A)') 'TOP'//TRIM(ITOS(ILAY))//'=',TRIM(ITBFNAME(J,ILAY))
    ELSEIF(J.EQ.3)THEN
     IF(.NOT.TRACECHECKRUN(IOS,'BOT FOR LAYER '//TRIM(ITOS(ILAY))))RETURN
     WRITE(*,'(A10,A)') 'BOT'//TRIM(ITOS(ILAY))//'=',TRIM(ITBFNAME(J,ILAY))
    ELSEIF(J.EQ.4)THEN
     IF(.NOT.TRACECHECKRUN(IOS,'POR AQUIFER FOR LAYER '//TRIM(ITOS(ILAY))))RETURN
     WRITE(*,'(A10,A)') 'POR AQUIFER'//TRIM(ITOS(ILAY))//'=',TRIM(ITBFNAME(J,ILAY))
    ELSEIF(J.EQ.5)THEN
     IF(.NOT.TRACECHECKRUN(IOS,'POR AQUITARD FOR LAYER '//TRIM(ITOS(ILAY))))RETURN
     WRITE(*,'(A10,A)') 'POR AQUITARD'//TRIM(ITOS(ILAY))//'=',TRIM(ITBFNAME(J,ILAY))
    ENDIF
