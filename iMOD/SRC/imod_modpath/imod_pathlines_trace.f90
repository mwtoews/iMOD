...
USE MOD_DEMO_PAR
USE MOD_3D_PAR, ONLY : PLLISTINDEX,PLLISTCLR,PLLISTAGE,STPLISTINDEX,MIDPOS,TOP,BOT,IDFPLOT,IPATHLINE_3D !,VIEWDX,VIEWDY,XYZAXES
...
! IF(ALLOCATED(LVISIT))DEALLOCATE(LVISIT)
 ...
 !## define zoom area for particle tracking - overrule area
 IF(ZOOMIDF%XMAX-ZOOMIDF%XMIN.GT.0.0)THEN
 ...
 IF(TRIM(IPFFNAME).NE.'')THEN
  ...
  IF(TRIM(IDFFLOW).NE.'')THEN 
   IF(.NOT.IDFREAD(IDF,IDFFLOW,0))RETURN; EPS=IDF%DX/10.0
  ENDIF
...
! !## generate a grid if iddfile is given
...
!###======================================================================
 ...
 IF(ISTO.EQ.1)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(IBATCH.EQ.1)WRITE(*,'(A)') TRIM(STRING)
   IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,STRING)
   IF(.NOT.IDFREADSCALE(HFFNAME(4,ILAY,IPER),IDF,10,1,0.0,0))RETURN !## block-value
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
    IF(IDF%X(ICOL,IROW).EQ.IDF%NODATA)THEN
     QS(ICOL,IROW,ILAY)=0.0
    ELSE
