...
!                  IFONT=4,ISIZE=10)
 ...
 !## skip header
 DO J=1,8; READ(IU(1),*); ENDDO
 ID=0
 DO
  READ(IU(1),*,IOSTAT=IOS) IFF(1)%ID,IFF(1)%IL,IFF(1)%X,IFF(1)%Y,IFF(1)%Z,IFF(1)%T,IFF(1)%V 
  IF(IOS.NE.0)EXIT
  IF(IFF(1)%ID.EQ.IFF(2)%ID)THEN
   ID=ID+1
   WRITE(IU(2),'(I10)') ID 
   WRITE(IU(2),'(2(F15.7,1X))') IFF(1)%X,IFF(1)%Y
   WRITE(IU(2),'(2(F15.7,1X))') IFF(2)%X,IFF(2)%Y
   WRITE(IU(2),'(A)') 'END'
   LINE=TRIM(ITOS(ID))//','//TRIM(ITOS(IFF(2)%IL))//','//TRIM(RTOS(IFF(2)%T,'F',2))
   WRITE(IU(3),'(A)') TRIM(LINE)
  ENDIF
  IFF(2)=IFF(1)
 ENDDO
 DO J=1,SIZE(IU); CLOSE(IU(I)); ENDDO
