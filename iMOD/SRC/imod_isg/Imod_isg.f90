...
! END SUBROUTINE ISGPROFILENAMES
 ...
 CALL DBL_IGRPOLYGONCOMPLEX(XBOT(1:NBOT),YBOT(1:NBOT),NBOT) 
   
 !## drawing second parameter
 IF(NTOP.GT.0)THEN
  CALL IGRCOLOURN(BCLR) 
  CALL DBL_IGRPOLYGONCOMPLEX(XTOP(1:NTOP),YTOP(1:NTOP),NTOP) 
 ENDIF
...
!     ENDIF
 ...
 IF(ISFR.EQ.0) THEN
  IF(ISGDOUBLE.EQ.4)N=SIZE(DMIN)-1
  IF(ISGDOUBLE.EQ.8)N=SIZE(DMIN)-2
 ELSE
  N=SIZE(DMIN)-2
 ENDIF
 DO I=1,N 
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_LEGEND)
     CALL WDIALOGGETMENU(IDF_MENU4,I)
     MP(IISGPLOT)%LEG=ISGLEG(I)
     IF(LEG_MAIN(IISGPLOT))THEN
      CALL IDFPLOTFAST(1); ISGLEG(I)=MP(IISGPLOT)%LEG; CALL ISGLEGENDSHOW(I)
     ENDIF
   END SELECT
  CASE (FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
...
! CALL ISGPROFILE_GETDATES()
 ...
 IF(GRIDISG%CS.LE.0.0D0)THEN   ! CellSize
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You should give a cellsize greater than 0.0.','Error'); RETURN
 ENDIF

 IF(GRIDISG%ISTEADY.EQ.2)THEN  ! Period
...
! CALL WDIALOGPUTMENU(IDF_MENU1,MP,12,4)
 ...
 IF(GRIDISG%ISTEADY.EQ.1)CALL WDIALOGPUTSTRING(IDF_LABEL22,'Attribute values will be computed as MEAN value over available data points within the ISG')
 IF(GRIDISG%ISTEADY.EQ.2) THEN
   IF(GRIDISG%IAVERAGE.EQ.1) CALL WDIALOGPUTSTRING(IDF_LABEL22,'Attribute values will be computed as MEAN values between the period '// &
                             TRIM(ITOS(GRIDISG%SDATE))//' and '//TRIM(ITOS(GRIDISG%EDATE)))
   IF(GRIDISG%IAVERAGE.EQ.2) CALL WDIALOGPUTSTRING(IDF_LABEL22,'Attribute values will be computed as MEDIAN values between the period '// &
                             TRIM(ITOS(GRIDISG%SDATE))//' and '//TRIM(ITOS(GRIDISG%EDATE)))
 ENDIF     
...
!      CALL UTL_IDFSNAPTOGRID(GRIDISG%XMIN,GRIDISG%XMAX,GRIDISG%YMIN,GRIDISG%YMAX,GRIDISG%CS,NCOL,NROW)
 ...
 IF(I.EQ.1)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO6)
 ...
 !###====================================================================
 SUBROUTINE ISGSELECTROTATE()
 !###====================================================================
 IMPLICIT NONE
 INTEGER :: I
 
 DO I=1,NISG
  IF(ISG(I)%ILIST.EQ.0)CYCLE; CALL ISGATTRIBUTESROTATEISP(I)
 ENDDO
 CALL IDFPLOTFAST(1)
 
 END SUBROUTINE ISGSELECTROTATE
...
!  IF(I.GE.2)TD=TD+SQRT((ISP2(I-1)%X-ISP2(I)%X)**2.0D0+(ISP2(I-1)%Y-ISP2(I)%Y)**2.0D0)
 ...
 !###====================================================================
 SUBROUTINE ISGATTRIBUTESROTATEISP(IISG)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IISG
 INTEGER :: I,J,K,JJSG,N
 ...
 ELSE
  ...
  !## rotate
  I=ISG(IISG)%ISEG-1; K=I+ISG(IISG)%NSEG
  DO J=1,ISG(IISG)%NSEG/2
   X         =ISP(I+J)%X; Y         =ISP(I+J)%Y
   ISP(I+J)%X=ISP(K)%X;   ISP(I+J)%Y=ISP(K)%Y
   ISP(K)%X  =X;          ISP(K)%Y  =Y
   K=K-1
  ENDDO
 
  !## get total distance of current line
  TDIST=0.0D0; DO I=2,ISG(IISG)%NSEG
   DIST=(ISP(I)%X-ISP(I-1)%X)**2.0D0+(ISP(I)%Y-ISP(I-1)%Y)**2.0D0
   IF(DIST.GT.0.0D0)DIST=SQRT(DIST); TDIST=TDIST+DIST
  END DO
  JJSG=IISG
 ...
 I=ISG(JJSG)%ICLC-1
 DO J=1,ISG(JJSG)%NCLC
  ISD(I+J)%DIST=MAX(0.0D0,TDIST-ISD(I+J)%DIST)
 ENDDO
 I=ISG(JJSG)%ICLC; N=ISG(JJSG)%NCLC; CALL ISGSORTSEGMENTENTRIES(ISD(I:)%DIST,ISD(I:)%IREF,ISD(I:)%N,N)
 
 !## adjust distance for weirs
 I=ISG(JJSG)%ISTW-1
 DO J=1,ISG(JJSG)%NSTW
  IST(I+J)%DIST=MAX(0.0D0,TDIST-IST(I+J)%DIST)
 ENDDO
 I=ISG(JJSG)%ISTW; N=ISG(JJSG)%NSTW; CALL ISGSORTSEGMENTENTRIES(IST(I:)%DIST,IST(I:)%IREF,IST(I:)%N,N)

 !## adjust distance for cross-sections
 I=ISG(JJSG)%ICRS-1
 DO J=1,ISG(JJSG)%NCRS
  ISC(I+J)%DIST=MAX(0.0D0,TDIST-ISC(I+J)%DIST)
 ENDDO
 I=ISG(JJSG)%ICRS; N=ISG(JJSG)%NCRS; CALL ISGSORTSEGMENTENTRIES(ISC(I:)%DIST,ISC(I:)%IREF,ISC(I:)%N,N)
 
 !## adjust distance for qh-relationships
 I=ISG(JJSG)%IQHR-1
 DO J=1,ISG(JJSG)%NQHR
  ISQ(I+J)%DIST=MAX(0.0D0,TDIST-ISQ(I+J)%DIST)
 ENDDO
 I=ISG(JJSG)%IQHR; N=ISG(JJSG)%NQHR; CALL ISGSORTSEGMENTENTRIES(ISQ(I:)%DIST,ISQ(I:)%IREF,ISQ(I:)%N,N)
 ...
 !###====================================================================
 IMPLICIT NONE
 INTEGER :: I,J,IDBG,IROW,IHR,IMT,ISC
! CHARACTER(LEN=14),ALLOCATABLE,DIMENSION(:) :: CLIST
 INTEGER(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: IORDER,ILIST
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ALLOCATE(ILIST(TISD(J)),IORDER(TISD(J)))
  DO I=1,TISD(J)
   READ(DATISD2(J,I)%CTIME(1:2),*) IHR
   READ(DATISD2(J,I)%CTIME(4:5),*) IMT
   READ(DATISD2(J,I)%CTIME(7:8),*) ISC
   ILIST(I)=DATISD2(J,I)%IDATE*1000000+IHR*10000+IMT*100+ISC
  ENDDO
  CALL QKSORT_INT8(TISD(J),V1=ILIST,V2=IORDER) 
  DO I=1,TISD(J)
   IROW=IORDER(I)
 ...
 CALL WDIALOGSELECT(ID_DIDFEDITTABLE); CALL UTL_DIALOGSHOW(0,0,0,3)
