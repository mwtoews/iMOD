...
CONTAINS
 ...
 CALL WDIALOGPUTMENU(IDF_MENU1,ISG%SNAME,NISG,ISG%ILIST) 
 CALL WDIALOGPUTMENU(IDF_MENU3,ISDLABELS,SIZE(ISDLABELS),1)
 CALL WDIALOGPUTMENU(IDF_MENU4,ISDLABELS,SIZE(ISDLABELS),2)
 ...
 CALL WDIALOGFIELDSTATE(IDF_COLOUR2,1)
...
! END SUBROUTINE ISGPROFILENAMES
 ...
 !###====================================================================
 IMPLICIT NONE
 INTEGER :: IISG,I,K,IDATE,IBOT,ITOP
 ...
 NTOP=0; NSTW=0; NCRS=0; NQHR=0; NBOT=0; TDIST=0.0
 DO IISG=1,NISG
  IF(ISG(IISG)%ILIST.EQ.0)CYCLE
  ...
  DO I=ISG(IISG)%ICLC,ISG(IISG)%ICLC+ISG(IISG)%NCLC-1
   ...
   IF(K.LE.ISD(I)%IREF+ISD(I)%N-1)THEN
    ...
    IF(ITOP.GT.0)THEN
     ...
     IF(I.EQ.ISG(IISG)%ICLC+ISG(IISG)%NCLC-1)THEN; H2=YTOP(NTOP); DIST=ISD(I)%DIST; ENDIF

    ENDIF

    NBOT=NBOT+1
    XBOT(NBOT)=ISD(I)%DIST+TDIST
    YTOP(NBOT)=ISGPROFILE_GETVARIABLE(IBOT,K)

!    YTOP(NTOP)=DATISD(K)%WLVL
...
! CALL ISGPROFILENAMES(XTOP(1),YTOP(1),1,ICHECK)
