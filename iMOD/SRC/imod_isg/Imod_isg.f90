...
!     CALL WDIALOGPUTOPTION(IDF_MENU2,)
  ...
  CASE (FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
    CASE (IDF_INTEGER1)
     CALL ISGLEGENDSHOW()
    CASE (IDF_MENU2,IDF_MENU4,IDF_MENU5,IDF_MENU6,IDF_RADIO3,IDF_RADIO5)
     CALL IDFPLOT(1)
    CASE (IDF_TRACKBAR1)
...
! CALL ISGPROFILEGETDATES()
 ...
 INTEGER :: I,J,K,NROW,NCOL,ITYPE,NLAY,ISTEADY 
 ...
 CALL WDIALOGSELECT(ID_DISGEDITGRID)
 CALL WDIALOGGETRADIOBUTTON(IDF_RADIO1,ISTEADY)
 ...
 IF(ISTEADY.EQ.2)THEN
  ...
  IF(SDATE.GT.EDATE)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You should specify the startdate to be earlier than/equal to the enddate',&
    'Error'); RETURN
  ENDIF
  STIME=INT(SDATE,8)*1000000; ETIME=INT(EDATE,8)*1000000; DTIME=INT(0,8)
 ENDIF
 ...
 ENDIF
 IF(ISTEADY.EQ.1)CALL WDIALOGPUTSTRING(IDF_LABEL22,'Attribute values will be computed as MEAN value over all periods '// &
                                   'that EXIST with data within given date.')
 IF(ISTEADY.EQ.2)CALL WDIALOGPUTSTRING(IDF_LABEL22,'Attribute values will be computed as MEDIAN values between the periods '// &
...
!   DX=DX+DY; IF(DX.NE.0.0)DX=SQRT(DX)
 ...
 IF(WMENUGETSTATE(ID_ISGEDIT,2).EQ.1)THEN
  ...
  CALL WDIALOGSELECT(ID_DISGEDITTAB1)
  IF(WINFODIALOGFIELD(ID_LEGEND,FIELDSTATE).EQ.1)THEN
