...
USE MOD_UTL, ONLY : ITOS,INVERSECOLOUR,UTL_MESSAGEHANDLE,UTL_CAP,EQUALNAMES,UTL_FILLDATES,RTOS,UTL_IDFSNAPTOGRID,UTL_GETUNIT,         &
             ...
             JDATETOFDATE,UTL_DIST,UTL_DEBUGLEVEL,UTL_MEASURE
...
REAL,ALLOCATABLE,DIMENSION(:),PRIVATE :: XSTG,XSTW,XBOT,XCRS,XQHR
REAL,ALLOCATABLE,DIMENSION(:),PRIVATE :: YSTG,YBOT
INTEGER,PRIVATE :: NSTG,NBOT,NCRS,NQHR,NSTW
...
!    CALL ISGPROFILENAMES(MESSAGE%GX,MESSAGE%GY,0,(/0,0,0,0/))
   CASE (FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_CHECK1,IDF_CHECK2,IDF_CHECK3,IDF_CHECK4)
      IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)CALL ISGPROFILE_PLOT() !CALL ISGPROFILE_GETDATA()
     CASE (IDF_MENU1)
      CALL WDIALOGGETMENU(IDF_MENU1,ISG%ILIST)
      CALL ISGPROFILE_GETDATES(IDF_MENU2) 
      CALL ISGPROFILE_GETDATA()
   ...
   CASE (PUSHBUTTON)
    ...
    END SELECT
   CASE (EXPOSE,RESIZE)
     CALL ISGPROFILE_PLOT()!    CALL ISGPROFILE_GETDATA()
  END SELECT
 ENDDO
 ...
 ELSEIF(IDZ.EQ.ID_ZOOMFULL)THEN
  ...
  PROFXMIN=XSTG(1)
  PROFXMAX=XSTG(NSTG-1)
  PROFYMAX=-10.0E10
  PROFYMIN= 10.0E10
  PROFYMIN=MIN(PROFYMIN,MINVAL(YSTG(1:NSTG))) 
  PROFYMAX=MAX(PROFYMAX,MAXVAL(YSTG(1:NSTG))) 
...
!       write(*,*) 1,dx,dy,profxmin,profxmax,profymin,profymax
       CALL ISGPROFILE_PLOT()
!       write(*,*) 2,dx,dy,profxmin,profxmax,profymin,profymax
!       XC1=XC2; YC1=YC2
      ENDIF
...
! END SUBROUTINE ISGPROFILENAMES
 ...
 NSTG=0; NBOT=0; NCRS=0; NQHR=0; NSTW=0
 DO IISG=1,NISG
  IF(ISG(IISG)%ILIST.EQ.0)CYCLE
  ...
  NSTG=NSTG+ISG(IISG)%NCLC+2*ISG(IISG)%NSTW+2
 ...
 ALLOCATE(XSTG(NSTG),YSTG(NSTG),XBOT(NBOT),YBOT(NBOT))
 ALLOCATE(XSTW(NSTW),XCRS(NCRS),XQHR(NQHR))

 NSTG=0; NSTW=0; NCRS=0; NQHR=0; NBOT=0; TDIST=0.0
 DO IISG=1,NISG
  IF(ISG(IISG)%ILIST.EQ.0)CYCLE
  ...
  DO I=ISG(IISG)%ICLC,ISG(IISG)%ICLC+ISG(IISG)%NCLC-1
   ...
   IF(K.LE.ISD(I)%IREF+ISD(I)%N-1)THEN
    NSTG=NSTG+1
    NBOT=NBOT+1
    XSTG(NSTG)=ISD(I)%DIST+TDIST
    XBOT(NBOT)=ISD(I)%DIST+TDIST
    YSTG(NSTG)=DATISD(K)%WLVL
    YBOT(NBOT)=DATISD(K)%BTML
    IF(I.EQ.ISG(IISG)%ICLC)H1=YSTG(NSTG)
    IF(I.EQ.ISG(IISG)%ICLC+ISG(IISG)%NCLC-1)THEN; H2=YSTG(NSTG); DIST=ISD(I)%DIST; ENDIF
   ENDIF
  END DO
  ...
  DO I=ISG(IISG)%ISTW,ISG(IISG)%ISTW+ISG(IISG)%NSTW-1
   ...
   IF(K.LE.IST(I)%IREF+IST(I)%N-1)THEN
    NSTG=NSTG+1
    NSTW=NSTW+1
    XSTG(NSTG)=IST(I)%DIST+TDIST
    XSTW(NSTW)=IST(I)%DIST+TDIST
    YSTG(NSTG)=DATIST(K)%WLVL_UP
    NSTG=NSTG+1
    !## depends on flow-direction how to put the weir
    IF(H1.LT.H2)THEN
     XSTG(NSTG)=IST(I)%DIST-0.01+TDIST
    ELSE
     XSTG(NSTG)=IST(I)%DIST+0.01+TDIST
    ENDIF
    YSTG(NSTG)=DATIST(K)%WLVL_DOWN
   ENDIF
  END DO
 ...
 !## sort on x
 CALL UTL_QKSORT2(XSTG,YSTG,SIZE(XSTG),NSTG) 
 ...
 CALL IGRPOLYGONCOMPLEX(XSTG,YSTG,NSTG) 
 !## drawing bottomlevel
 CALL IGRCOLOURN(WRGB(160,136,121))
 CALL IGRPOLYGONCOMPLEX(XBOT,YBOT,NBOT) 
   
! !## start plotting waterlevels incl. structures
! CALL ISGPROFILE_PLOTFEATURE(XSTG,YSTG,I1,1,WRGB(119,219,214),ICLRSD,ICHECK(1),YMIN)  !## waterlevel - filled+vert.lines
...
! !###====================================================================
! SUBROUTINE ISGPROFILE_PLOTFEATURE(X,Y,NDIM,ITOPIC,ICLR,JCLR,ICHECK,YMIN)
! !###====================================================================
! IMPLICIT NONE
...
! CALL IGRAREA(0.0,0.0,1.0,1.0); CALL IGRAREACLEAR()
!
! CALL ISGATTRIBUTESPLOT_AXES(XMIN,YMIN,XMAX,YMAX,.FALSE.,'Distance (m)','Altitude (m)')
!
! RETURN
! 
! CALL IGRFILLPATTERN(SOLID)
...
! END SUBROUTINE ISGPROFILEEXTENT
 ...
 IF(ALLOCATED(XSTG))DEALLOCATE(XSTG); IF(ALLOCATED(YSTG))DEALLOCATE(YSTG)
 ...
 !## get available dates in the isg file
 CALL ISGPROFILE_GETDATES(IDF_MENU2) !1,NISG,IDF_MENU2)
 ...
 IF(ISFR.EQ.0)THEN
  CALL WDIALOGPUTMENU(IDF_MENU4,TATTRIB1(2:),SIZE(TATTRIB1)-1,1)
 ELSEIF(ISFR.EQ.1)THEN
  CALL WDIALOGPUTMENU(IDF_MENU4,TATTRIB2(3:),SIZE(TATTRIB2)-2,1)
 ENDIF
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_LEGEND)
     CALL LEG_MAIN(IISGPLOT); ISGLEG=MP(IISGPLOT)%LEG; CALL ISGLEGENDSHOW()
   END SELECT
  CASE (FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
    CASE (IDF_INTEGER1)
    ...
    CASE (IDF_MENU4,IDF_MENU5,IDF_MENU6,IDF_RADIO3,IDF_RADIO5)
     IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)CALL IDFPLOT(1)
    CASE (IDF_TRACKBAR1)
     IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
...
! MAXCRD=50; ALLOCATE(XCRD(MAXCRD),YCRD(MAXCRD))
 ...
 ELSEIF(ID.EQ.ID_OPEN)THEN
  ...
  IF(ISFR.EQ.1)NK=13 !## sfr package
...
!###====================================================================
 ...
 !###====================================================================
 IMPLICIT NONE
 INTEGER :: I,J,ITRAP,ISYM,NDIM,NSYM,NTRAP
 REAL :: XMIN,YMIN,XMAX,YMAX,DX,DY,AORG,ATRAP,X1,X2,Y1,Y2
...
!  DATISQ2(ISEL,1:J-I+1)=DATISQ(I:J)
 ...
 !###======================================================================
 SUBROUTINE ISGISP_MENUFIELDS()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I
 
 CALL WINDOWSELECT(0)
 I=WMENUGETSTATE(ID_ISGISPSTART,1)
 I=ABS(I-1)
 
 CALL WMENUSETSTATE(ID_ISGISPSTART,1,I)
 CALL WMENUSETSTATE(ID_ISGISPRESET,1,I)
 CALL WMENUSETSTATE(ID_ISGISPSAVE,1,I)
 CALL WMENUSETSTATE(ID_ISGISPSTOP,1,I)
...
! CALL WDIALOGFIELDSTATE(IDF_CHECK8,I)
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  CALL WGRIDSTATE(IDF_GRID1,7 ,ISTATE)
...
! INTEGER,PARAMETER :: INODATA=0
 ...
 !## riv-approach isg
 IF(ISFR.EQ.0)THEN 
  CALL WGRIDCOLUMNS(IDF_GRID1,SIZE(TATTRIB1),CTATTRIB1) 
  DO I=1,SIZE(TATTRIB1); CALL WGRIDLABELCOLUMN(IDF_GRID1,I,TATTRIB1(I)); ENDDO
 !## sfr-approach isg
 ELSEIF(ISFR.EQ.1)THEN
  CALL WGRIDCOLUMNS(IDF_GRID1,SIZE(TATTRIB2),CTATTRIB2) 
  DO I=1,SIZE(TATTRIB2); CALL WGRIDLABELCOLUMN(IDF_GRID1,I,TATTRIB2(I)); ENDDO
  ALLOCATE(IOPTIONS(ISDMAXROW)); IOPTIONS=1
  CALL WGRIDPUTMENU(IDF_GRID1,10,(/'0', '1', '2', '3', '4'/),5,IOPTIONS,ISDMAXROW)
  CALL WGRIDPUTMENU(IDF_GRID1,11,(/'0','-1','-2','-3'/),4,IOPTIONS,ISDMAXROW)
  DEALLOCATE(IOPTIONS)
  !## fill menu fields
 ENDIF
...
!       CDATE=JDATETOFDATE(MESSAGE%GX,0) !MINDATE)

       CALL WDIALOGPUTSTRING(IDF_STRING2,UTL_WRITENUMBER(Y))
!       I=UTL_JDATETOIDATE(IX)
!       WRITE(CDATE,'(I8)',IOSTAT=IOS) I
!       IF(IOS.EQ.0)
       CALL WDIALOGPUTSTRING(IDF_STRING1,TRIM(JDATETOFDATE(MESSAGE%GX,0))) !CDATE(1:4)//'-'//CDATE(5:6)//'-'//CDATE(7:8))
