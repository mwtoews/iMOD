...
! END SUBROUTINE ISGATTRIBUTESSORT
 ...
 ELSEIF(ISCN(J).EQ.-1)THEN
  ...
  IF(IEEE_IS_NAN(DATISC2(J,1)%KM))THEN
   CALL WGRIDPUTREAL(IDF_GRID1,1,DATISC2(J,:)%DISTANCE,TISC(J),'(F10.2)')
   CALL WGRIDPUTREAL(IDF_GRID1,2,DATISC2(J,:)%BOTTOM,TISC(J),'(F10.2)')
   CALL WGRIDPUTREAL(IDF_GRID1,3,DATISC2(J,:)%KM,TISC(J),'(F10.2)')
  !## reference height and pointers used
  ELSE
   CALL WGRIDPUTCELLREAL(IDF_GRID1,1,1,DATISC2(J,I)%DISTANCE,'(F10.2)')
   CALL WGRIDPUTCELLREAL(IDF_GRID1,1,1,DATISC2(J,I)%BOTTOM,'(F10.2)')
   CALL WGRIDPUTCELLREAL(IDF_GRID1,1,1,DATISC2(J,I)%KM,'(F10.2)')
   DO I=2,TISC(J)
    Z=DATISC2(J,I)%ZM+REAL(DATISC2(J,I)%ZC)/10.0
    CALL WGRIDPUTCELLREAL(IDF_GRID1,1,I,Z,'(F10.2)')
   ENDDO
   CALL WGRIDPUTREAL(IDF_GRID1,3,REAL(DATISC2(J,:)%ZP),TISC(J),'(F10.0)')
  ENDIF
 ENDIF
