...
USE MOD_ISG_PLOT, ONLY : ISGPLOTSHAPE,ISGPLOT_FDIRECTION
USE MOD_ISG_UTL
 ...
 SELECT CASE (ITYPE)
  ...
  CASE(FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
    ...
    CASE (IDF_CHECK7)
     CALL IDFPLOTFAST(1)
    CASE (IDF_CHECK1,IDF_CHECK2,IDF_CHECK3,IDF_CHECK4,IDF_CHECK5,IDF_CHECK6,IDF_CHECK9)
    ...
    CASE (IDF_STRING1,IDF_STRING2,IDF_STRING3,IDF_STRING4,IDF_STRING5,IDF_STRING6,IDF_STRING7)
     CALL WDIALOGSELECT(ID_DISGEDIT)
...
! CALL WDIALOGCOLOUR(IDF_LABEL9,INVERSECOLOUR(ICLRSF),ICLRSF)
  ...
  DO I=1,TISD(J)
   ...
   ELSE
    LINE=TRIM(ITOS(DATISD2(J,I)%IDATE))//','//          DATISD2(J,I)%CTIME       //','//TRIM(RTOS(DATISD2(J,I)%WLVL,'F',3))//  &
                                         ','//TRIM(RTOS(DATISD2(J,I)%BTML,'F',3))//  &
                                         ','//TRIM(RTOS(DATISD2(J,I)%THCK,'F',2)) //','//TRIM(RTOS(DATISD2(J,I)%HCND,'F',2))//  &
!                                         ','//TRIM(RTOS(DATISD2(J,I)%WIDTH,'F',2))//','//TRIM(RTOS(DATISD2(J,I)%DEPTH,'F',2))// &
                                         ','//TRIM(ITOS(DATISD2(J,I)%DWNS))       //','//TRIM(ITOS(DATISD2(J,I)%UPSG))// &
                                         ','//TRIM(ITOS(DATISD2(J,I)%ICLC))       //','//TRIM(ITOS(DATISD2(J,I)%IPRI))// &
                                         ','//TRIM(RTOS(DATISD2(J,I)%QFLW,'F',2)) //','//TRIM(RTOS(DATISD2(J,I)%QROF,'F',2))
   ENDIF
 ...
 ELSEIF(ID.EQ.ID_OPEN)THEN
  ...
  IF(ISFR.EQ.1)NK=14 !## sfr package
...
!      IF(K.EQ.7) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%WIDTH
!      IF(K.EQ.8) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%DEPTH
      IF(K.EQ.7) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%DWNS
      IF(K.EQ.8)READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%UPSG
      IF(K.EQ.9)READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%ICLC
      IF(K.EQ.10)READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%IPRI
 ...
 ELSEIF(ID.EQ.ID_OPEN)THEN
  ...
  ELSEIF(ISCN(J).EQ.-1)THEN
   IF(.NOT.UTL_DATA_CSV((/'X-crd.     ','Y-crd.     ','Z-value    ','Pointer    '/)))RETURN
  ENDIF
...
!  DATISQ2(ISEL,1:J-I+1)=DATISQ(I:J)
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  CALL WGRIDPUTINTEGER(IDF_GRID1,7,DATISD2(J,:)%DWNS,TISD(J))
...
! ISQ(I)%DIST 
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  CALL WGRIDGETINTEGER(IDF_GRID1,7,DATISD2(J,:)%DWNS,ISDMAXROW)
...
!   IF(DATISD2(J,I)%UPSG .EQ.INODATA)EXIT
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  DO I=1,TISD(J)
   ...
   CALL WGRIDGETCELLINTEGER(IDF_GRID1,7,IROW,DATISD2(J,I)%DWNS)
