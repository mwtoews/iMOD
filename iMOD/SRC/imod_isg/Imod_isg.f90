...
USE MOD_UTL, ONLY : ITOS,INVERSECOLOUR,UTL_MESSAGEHANDLE,UTL_CAP,EQUALNAMES,UTL_FILLDATES,RTOS,UTL_IDFSNAPTOGRID,UTL_GETUNIT,&
             JDATETOIDATE,UTL_WSELECTFILE,IDFPLOT1BITMAP,IDFPLOT2BITMAP,IDATETOJDATE,UTL_GENLABELSDEALLOCATE,UTL_DATA_CSV,NL,&
    ...
    CASE (ID_DISGEDIT)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (ID_UPDATE)
       CALL IDFPLOTFAST(1)
      CASE(IDHELP)
       CALL IMODGETHELP('4.4.3','ISG Edit')      
      CASE(IDCANCEL)
       CALL ISGEDITCLOSE(1)
     END SELECT
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDOK,IDCANCEL)
      EXIT
     CASE (IDHELP)
       CALL IMODGETHELP('4.4.3.4','ISG Profile')     
    END SELECT
   CASE (EXPOSE,RESIZE)
    CALL ISGPROFILEDATA()
  END SELECT
...
! READ(CDATES(IDATE),'(I8)') IDATE
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDOK)
      ...
      EXIT
     CASE (IDHELP)
       CALL IMODGETHELP('4.4.3.3','ISG Search')     
    END SELECT
  END SELECT
 ENDDO
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDCANCEL)
      EXIT
     CASE (IDHELP)
       CALL IMODGETHELP('4.4.3.5','ISG Rasterize')     
    END SELECT
 ...
 INTEGER :: I,J,K,NROW,NCOL,ITYPE,IU 
 CHARACTER(LEN=256) :: LINE
! INTEGER,DIMENSION(MAXITEMS) :: ID
 ...
 ISAVE=1; CALL ISG2GRIDMAIN(0,LOKAY) 
...
! END SUBROUTINE ISGATTRIBUTESSORT
 ...
 SELECT CASE (I)
  CASE (1)
   DO K=1,TISD(J)-1
    X1=IDATETOJDATE(DATISD2(J,K)%IDATE)
    X2=IDATETOJDATE(DATISD2(J,K+1)%IDATE)
    Y1=DATISD2(J,K)%WLVL
 ...
 XMIN=IDATETOJDATE(XMIN)
 XMAX=IDATETOJDATE(XMAX)
 ...
 DO I=1,2
  IF(I.EQ.1)THEN
   ...
   DO K=1,TIST(J)-1
    X1=IDATETOJDATE(DATIST2(J,K)%IDATE)
    X2=IDATETOJDATE(DATIST2(J,K+1)%IDATE)
  ...
  IF(ITYPE.EQ.PUSHBUTTON)THEN
   SELECT CASE(MESSAGE%VALUE1)
    CASE(IDOK,IDCANCEL)
     EXIT
    CASE(IDHELP)
     CALL IMODGETHELP('4.4.3.2','ISG Attributes')
   END SELECT
  ENDIF
 ENDDO
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDCANCEL)
      EXIT
     CASE (IDHELP)
       CALL IMODGETHELP('4.1.4.3','IDF Edit Calculate')  
    END SELECT
  END SELECT
