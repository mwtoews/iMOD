...
USE MOD_UTL, ONLY : ITOS,INVERSECOLOUR,UTL_MESSAGEHANDLE,UTL_CAP,EQUALNAMES,UTL_FILLDATES,RTOS,UTL_IDFSNAPTOGRID,UTL_GETUNIT,         &
             ...
             UTL_IDFGETCLASS,UTL_IDFCRDCOR,UTL_GETCURRENTDATE,UTL_GETCURRENTTIME,UTL_WRITENUMBER,         &
             JDATETOFDATE,UTL_DIST,UTL_DEBUGLEVEL,UTL_MEASURE,UTL_INSIDEPOLYGON,MAXLENVAR
...
CHARACTER(LEN=MAXLENVAR),POINTER,DIMENSION(:,:),PRIVATE :: VAR
CHARACTER(LEN=MAXLENVAR),POINTER,DIMENSION(:),PRIVATE :: CCNST
INTEGER,ALLOCATABLE,DIMENSION(:),PRIVATE :: IVAR,ICOL_VAR,IACT_VAR
...
! CALL ISGPROFILE_GETDATES()
 ...
 INTEGER,POINTER,DIMENSION(:) :: IPLIST,SGLIST
...
!  IF(IISG.EQ.JJSG)EXIT
 ...
 !###====================================================================
 IMPLICIT NONE
 INTEGER :: ICLC,NCLC,ICRS,NCRS,ISEG,NSEG,I,J,K,ISTW,NSTW,IQHR,NQHR,IDWNS,IUPSG,IREF,N
...
!  ENDDO
 ...
 ELSEIF(ID.EQ.ID_OPEN)THEN
  ...
  IF(.NOT.UTL_DATA_CSV(ISDLABELS,VAR,IVAR,ICOL_VAR,IACT_VAR,CCNST))RETURN
  ...
  DO I=1,MIN(ISDMAXROW,TISD(J))
   ...
   ENDDO
  ENDDO
  CALL UTL_GENLABELSDEALLOCATE(VAR,IVAR,ICOL_VAR,IACT_VAR,CCNST)
 ...
 ELSEIF(ID.EQ.ID_OPEN)THEN
  ...
  IF(ISCN(J).EQ.1)THEN
   IF(.NOT.UTL_DATA_CSV((/'Distance   ','BottomLevel','MRC        '/),VAR,IVAR,ICOL_VAR,IACT_VAR,CCNST))RETURN; N=3
  ELSEIF(ISCN(J).EQ.-1)THEN
   IF(.NOT.UTL_DATA_CSV((/'X-crd.     ','Y-crd.     ','Z-value    ','Pointer    '/),VAR,IVAR,ICOL_VAR,IACT_VAR,CCNST))RETURN; N=4
  ENDIF
...
! CHARACTER(LEN=52),POINTER,DIMENSION(:,:) :: VAR
 ...
 ELSEIF(ID.EQ.ID_OPEN)THEN
  ...
  IF(.NOT.UTL_DATA_CSV((/'Date          ','UpWaterLevel  ','DownWaterLevel'/),VAR,IVAR,ICOL_VAR,IACT_VAR,CCNST))RETURN
...
! CHARACTER(LEN=52),POINTER,DIMENSION(:,:) :: VAR
 ...
 ELSEIF(ID.EQ.ID_OPEN)THEN
  ...
  IF(.NOT.UTL_DATA_CSV((/'Q     ','Width ','Depth ','Factor'/),VAR,IVAR,ICOL_VAR,IACT_VAR,CCNST))RETURN
...
!###====================================================================
 ...
 REAL :: XMIN,YMIN,XMAX,YMAX,DX,DY,AORG,ATRAP,ASIM,X1,X2,Y1,Y2
...
! YMAX=MAX(YMAX,MAXVAL(DATISQ2(J,1:TISQ(J))%F))
 ...
 CALL ISGATTRIBUTESPLOT_AXES(REAL(XMIN),YMIN,REAL(XMAX),YMAX,.FALSE.,'Q (m3/s)','Width (m) and Depth (m)')
...
!  CALL WGRIDSTATE(IDF_GRID1,11,ISTATE)
 ...
 CALL WGRIDPUTREAL(IDF_GRID1,1,DATISQ2(J,:)%Q,TISQ(J),'(G10.5)')
 CALL WGRIDPUTREAL(IDF_GRID1,2,DATISQ2(J,:)%W,TISQ(J),'(G10.5)')
 CALL WGRIDPUTREAL(IDF_GRID1,3,DATISQ2(J,:)%D,TISQ(J),'(G10.5)')
 CALL WGRIDPUTREAL(IDF_GRID1,4,DATISQ2(J,:)%F,TISQ(J),'(G10.5)')
