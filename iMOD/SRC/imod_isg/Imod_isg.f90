...
!  IF(I.GE.2)TD=TD+SQRT((ISP2(I-1)%X-ISP2(I)%X)**2.0D0+(ISP2(I-1)%Y-ISP2(I)%Y)**2.0D0)
 ...
 ENDDO
 I=ISG(JJSG)%ICLC; CALL ISGSORTSEGMENTENTRIES(ISD(I:)%DIST,ISD(I:)%IREF,ISD(I:)%N,ISG(JJSG)%NCLC)
 ...
 ENDDO
 I=ISG(JJSG)%ISTW; CALL ISGSORTSEGMENTENTRIES(IST(I:)%DIST,IST(I:)%IREF,IST(I:)%N,ISG(JJSG)%NSTW)
 ...
 ENDDO
 I=ISG(JJSG)%ICRS; CALL ISGSORTSEGMENTENTRIES(ISC(I:)%DIST,ISC(I:)%IREF,ISC(I:)%N,ISG(JJSG)%NCRS)
 ...
 ENDDO
 I=ISG(JJSG)%IQHR; CALL ISGSORTSEGMENTENTRIES(ISQ(I:)%DIST,ISQ(I:)%IREF,ISQ(I:)%N,ISG(JJSG)%NQHR)
