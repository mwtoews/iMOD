...
USE MOD_ISG_PLOT, ONLY : ISGPLOTSHAPE,ISGPLOT_FDIRECTION,ISGPLOT_FCONNECTION,ISGPLOT_LEGENDLINES
USE MOD_ISG_UTL
...
CONTAINS
 ...
 CALL ISGPROFILEGETDATES(I,I,IDF_MENU2)
 CALL ISGPROFILEDATA()
 ...
 !###====================================================================
 SUBROUTINE ISGPROFILEGETDATES(ISG1,ISG2,ID)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ISG1,ISG2,ID
 INTEGER :: I,J,N,IISG

! CALL WDIALOGGETMENU(IDF_MENU1,IISG)
 CALL UTL_MESSAGEHANDLE(0)
 !## get max. number of idates()
 N=0; DO IISG=ISG1,ISG2
...
! READ(CDATES(IDATE),'(I8)') IDATE
 ...
 CALL ISGPROFILEGETDATES(1,NISG,IDF_MENU2)
...
! CALL ISGPROFILEGETDATES()
 ...
 INTEGER :: I,J,K,NROW,NCOL,ITYPE,NLAY,ISTEADY,SDATE,EDATE
...
! MAXCRD=50; ALLOCATE(XCRD(MAXCRD),YCRD(MAXCRD))
 ...
 INTEGER :: ITYPE 
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: I,J,N,ICLC,NCLC,IDOWN,ISEG,IISG,JJSG,IREF
 REAL :: DOWNX,DOWNY
 LOGICAL :: LEX
 ...
 DO 
  ...
  SELECT CASE (ITYPE)
   CASE (MOUSEMOVE)
    ...
    !## remove previous one
    IF(IISG.NE.0)CALL ISGGETSEGMENT_PLOT(IISG)
    IISG=0; IF(LEX)THEN; IISG=JJSG; CALL ISGGETSEGMENT_PLOT(IISG); ENDIF
 ...
 !###====================================================================
 SUBROUTINE ISGGETSEGMENT_PLOT(IISG)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IISG
 INTEGER :: IISP,NISP,I
  
 IISP=ISG(IISG)%ISEG-1; NISP=ISG(IISG)%NSEG
 DO I=1,NISP
  IISP=IISP+1
  IF(I.EQ.1)THEN
   CALL IGRMOVETO(ISP(IISP)%X,ISP(IISP)%Y)
  ELSE
   CALL IGRLINETO(ISP(IISP)%X,ISP(IISP)%Y)
  ENDIF
 ENDDO

 END SUBROUTINE ISGGETSEGMENT_PLOT
  ...
  DO I=1,TISD(J)
   ...
   ELSE
    LINE=TRIM(ITOS(DATISD2(J,I)%IDATE))//','//          DATISD2(J,I)%CTIME       //','//TRIM(RTOS(DATISD2(J,I)%WLVL,'F',3))//  &
                                         ','//TRIM(RTOS(DATISD2(J,I)%BTML,'F',3))//','//TRIM(RTOS(DATISD2(J,I)%THCK,'F',2))//  &
 ...
 ELSEIF(ID.EQ.ID_OPEN)THEN
  ...
  IF(ISFR.EQ.1)NK=12 !## sfr package
   ...
   DO K=1,NK
    IF(IACT_VAR(K).EQ.1)THEN
     ...
     ELSEIF(ISFR.EQ.1)THEN
      ...
      IF(K.EQ.5) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%THCK
      IF(K.EQ.6) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%HCND
      IF(K.EQ.7) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%UPSG
      IF(K.EQ.8) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%DWNS
      IF(K.EQ.9)THEN
      ...
      IF(K.EQ.10)THEN
       ...
       ENDIF
      ENDIF
      IF(K.EQ.11)READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%QFLW
      IF(K.EQ.12)READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%QROF
     ENDIF
     IF(IOS.NE.0)THEN
      TISD(J)=I-1
...
!   DX=(X-ISP(ISEG)%X)**2.0; DY=(Y-ISP(ISEG)%Y)**2.0
!   DX=DX+DY; IF(DX.NE.0.0)DX=SQRT(DX)
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  CALL WGRIDPUTREAL(IDF_GRID1,5,DATISD2(J,:)%THCK ,TISD(J),'(F10.2)')
  CALL WGRIDPUTREAL(IDF_GRID1,6,DATISD2(J,:)%HCND ,TISD(J),'(F10.2)')
  CALL WGRIDPUTINTEGER(IDF_GRID1,7,DATISD2(J,:)%UPSG,TISD(J))
  CALL WGRIDPUTINTEGER(IDF_GRID1,8,DATISD2(J,:)%DWNS,TISD(J))
  CALL WGRIDPUTOPTION(IDF_GRID1,9,DATISD2(J,:)%ICLC,TISD(J))
  CALL WGRIDPUTOPTION(IDF_GRID1,10,DATISD2(J,:)%IPRI,TISD(J))
  CALL WGRIDPUTREAL(IDF_GRID1,11,DATISD2(J,:)%QFLW,TISD(J),'(F10.2)')
  CALL WGRIDPUTREAL(IDF_GRID1,12,DATISD2(J,:)%QROF,TISD(J),'(F10.2)')
...
! ISQ(I)%DIST 
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  CALL WGRIDGETREAL(IDF_GRID1,5,DATISD2(J,:)%THCK,ISDMAXROW)
  CALL WGRIDGETREAL(IDF_GRID1,6,DATISD2(J,:)%HCND,ISDMAXROW)
  CALL WGRIDGETINTEGER(IDF_GRID1,7,DATISD2(J,:)%UPSG,ISDMAXROW)
  CALL WGRIDGETINTEGER(IDF_GRID1,8,DATISD2(J,:)%DWNS,ISDMAXROW)
  CALL WGRIDGETMENU(IDF_GRID1,9,DATISD2(J,:)%ICLC,ISDMAXROW)
  CALL WGRIDGETMENU(IDF_GRID1,10,DATISD2(J,:)%IPRI,ISDMAXROW)
  CALL WGRIDGETREAL(IDF_GRID1,11,DATISD2(J,:)%QFLW,ISDMAXROW)
  CALL WGRIDGETREAL(IDF_GRID1,12,DATISD2(J,:)%QROF,ISDMAXROW)
 ENDIF
...
!   IF(DATISD2(J,I)%UPSG .EQ.INODATA)EXIT
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  DO I=1,TISD(J)
   ...
   CALL WGRIDGETCELLREAL(IDF_GRID1,5,IROW,DATISD2(J,I)%THCK)
   CALL WGRIDGETCELLREAL(IDF_GRID1,6,IROW,DATISD2(J,I)%HCND)
   CALL WGRIDGETCELLINTEGER(IDF_GRID1,7,IROW,DATISD2(J,I)%UPSG)
   CALL WGRIDGETCELLINTEGER(IDF_GRID1,8,IROW,DATISD2(J,I)%DWNS)
   CALL WGRIDGETCELLMENU(IDF_GRID1,9,IROW,DATISD2(J,I)%ICLC)
   CALL WGRIDGETCELLMENU(IDF_GRID1,10,IROW,DATISD2(J,I)%IPRI)
   CALL WGRIDGETCELLREAL(IDF_GRID1,11,IROW,DATISD2(J,I)%QFLW)
   CALL WGRIDGETCELLREAL(IDF_GRID1,12,IROW,DATISD2(J,I)%QROF)
  ENDDO
...
! INTEGER,PARAMETER :: INODATA=0
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  CALL WGRIDPUTMENU(IDF_GRID1, 9,(/'0', '1', '2', '3', '4'/),5,IOPTIONS,ISDMAXROW)
  CALL WGRIDPUTMENU(IDF_GRID1,10,(/'0','-1','-2','-3'/),4,IOPTIONS,ISDMAXROW)
  DEALLOCATE(IOPTIONS)
  !## fill menu fields
 ENDIF
