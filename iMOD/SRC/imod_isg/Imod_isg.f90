 ...
 DO
  ...
  SELECT CASE (ITYPE)
   ...
   CASE (MOUSEMOVE)
    ...
    XC1=DBLE(MESSAGE%GX); YC1=DBLE(MESSAGE%GY)
    IF(NPOL.GT.1)THEN
     CALL UTL_PLOT1BITMAP()
...
! CALL WDIALOGFIELDSTATE(IDF_CHECK8,I)
 ...
 IF(ISFR.EQ.0)THEN
  ...
  K=0; IF(ISGDOUBLE.EQ.8)THEN
   ...
   DO I=1,TISD(J); IF(INDEX(DATISD2(J,I)%CTIME,':').EQ.0)DATISD2(J,I)%CTIME='00:00:00'; ENDDO
   K=1; CALL WGRIDPUTSTRING(IDF_GRID1,2,DATISD2(J,:)%CTIME,TISD(J))
  ENDIF
  CALL WGRIDPUTDOUBLE(IDF_GRID1,2+K,DATISD2(J,:)%WLVL,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,3+K,DATISD2(J,:)%BTML,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,4+K,DATISD2(J,:)%RESIS,TISD(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,5+K,DATISD2(J,:)%INFF,TISD(J),'(F15.3)')
 ...
 ELSE
  CALL WGRIDPUTSTRING(IDF_GRID1,2,DATIST2(J,:)%CTIME,TIST(J))
  CALL WGRIDPUTDOUBLE(IDF_GRID1,3,DATIST2(J,:)%WLVL_UP,TIST(J),'(F15.3)')
  CALL WGRIDPUTDOUBLE(IDF_GRID1,4,DATIST2(J,:)%WLVL_DOWN,TIST(J),'(F15.3)')
 ENDIF
