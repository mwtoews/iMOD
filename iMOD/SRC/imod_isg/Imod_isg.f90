...
! READ(CDATES(IDATE),'(I8)') IDATE
 ...
 IF(ID.EQ.ID_SAVEAS)THEN
  ...
  DO I=1,TISD(J)
   ...
   ELSE
    LINE=TRIM(ITOS(DATISD2(J,I)%IDATE))//','//TRIM(ITOS(DATISD2(J,I)%ITIME))      //','//TRIM(RTOS(DATISD2(J,I)%BTML,'F',3))//  &
...
!              'DivOpt','QInFlow','QRunoff'/
  ...
  IF(ISFR.EQ.0)THEN
   IF(.NOT.UTL_DATA_CSV(TATTRIB1))RETURN !(/'Date              ','WaterLevel        ','BottomLevel       ', &
!                          'Resistance        ','InfiltrationFactor'/)))RETURN
  ELSEIF(ISFR.EQ.1)THEN
   IF(.NOT.UTL_DATA_CSV(TATTRIB2))RETURN !(/'Date              ','WaterLevel        ','BottomLevel       ', &
   ...
   DO K=1,NK
    IF(IACT_VAR(K).EQ.1)THEN
     ...
     ELSEIF(ISFR.EQ.1)THEN
      ...
      IF(K.EQ.3) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%BTML
      IF(K.EQ.4) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%THCK
      IF(K.EQ.5) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%HCND
      IF(K.EQ.6) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%WIDTH
      IF(K.EQ.7) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%DEPTH
      IF(K.EQ.8) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%DWNS
      IF(K.EQ.9) READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%UPSG
      IF(K.EQ.10)READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%ICLC
      IF(K.EQ.11)READ(LINE,*,IOSTAT=IOS) DATISD2(J,I)%IPRI
...
! CALL IGRUNITS(X1,Y1,X2,Y2)
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  CALL WGRIDPUTREAL(IDF_GRID1,3,DATISD2(J,:)%BTML ,TISD(J),'(F10.2)')
  CALL WGRIDPUTREAL(IDF_GRID1,4,DATISD2(J,:)%THCK ,TISD(J),'(F10.2)')
  CALL WGRIDPUTREAL(IDF_GRID1,5,DATISD2(J,:)%HCND ,TISD(J),'(F10.2)')
  CALL WGRIDPUTREAL(IDF_GRID1,6,DATISD2(J,:)%WIDTH,TISD(J),'(F10.2)')
  CALL WGRIDPUTREAL(IDF_GRID1,7,DATISD2(J,:)%DEPTH,TISD(J),'(F10.2)')
  CALL WGRIDPUTINTEGER(IDF_GRID1,8,DATISD2(J,:)%DWNS,TISD(J))
  CALL WGRIDPUTINTEGER(IDF_GRID1,9,DATISD2(J,:)%UPSG,TISD(J))
...
! ISQ(I)%DIST 
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  CALL WGRIDGETREAL(IDF_GRID1,3,DATISD2(J,:)%BTML,ISDMAXROW)
  CALL WGRIDGETREAL(IDF_GRID1,4,DATISD2(J,:)%THCK,ISDMAXROW)
  CALL WGRIDGETREAL(IDF_GRID1,5,DATISD2(J,:)%HCND,ISDMAXROW)
  CALL WGRIDGETREAL(IDF_GRID1,6,DATISD2(J,:)%WIDTH,ISDMAXROW)
  CALL WGRIDGETREAL(IDF_GRID1,7,DATISD2(J,:)%DEPTH,ISDMAXROW)
  CALL WGRIDGETINTEGER(IDF_GRID1,8,DATISD2(J,:)%DWNS,ISDMAXROW)
  CALL WGRIDGETINTEGER(IDF_GRID1,9,DATISD2(J,:)%UPSG,ISDMAXROW)
 ...
 ELSEIF(ISFR.EQ.1)THEN
  ...
  DO I=1,TISD(J)
   ...
   CALL WGRIDGETCELLREAL(IDF_GRID1,3,IROW,DATISD2(J,I)%BTML)
   CALL WGRIDGETCELLREAL(IDF_GRID1,4,IROW,DATISD2(J,I)%THCK)
   CALL WGRIDGETCELLREAL(IDF_GRID1,5,IROW,DATISD2(J,I)%HCND)
   CALL WGRIDGETCELLREAL(IDF_GRID1,6,IROW,DATISD2(J,I)%WIDTH)
   CALL WGRIDGETCELLREAL(IDF_GRID1,7,IROW,DATISD2(J,I)%DEPTH)
   CALL WGRIDGETCELLINTEGER(IDF_GRID1,8,IROW,DATISD2(J,I)%DWNS)
   CALL WGRIDGETCELLINTEGER(IDF_GRID1,9,IROW,DATISD2(J,I)%UPSG)
