 ...
 SELECT CASE (ITYPE)
  ...
  CASE(FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
    ...
    CASE (IDF_MENU1)
     CALL ISGCHECKISG(0.0,0.0,1)
     !## reason to colour selected line
     CALL IDFPLOTFAST(1)
    !## show selected
    CASE (IDF_CHECK7)
     CALL IDFPLOTFAST(1)
...
!----  
  ...
  IF(ISELISG.NE.0)THEN
   ...
   IF(GKEYPRESSED.EQ.0)CALL ISGISPSTOP(0)
  ENDIF
 ...
 ELSEIF(CODE.EQ.1)THEN
  ...
  CALL WDIALOGGETMENU(IDF_MENU1,ISG(1:NISG)%ILIST)
  !## check whether current isg is already selected
  IF(SUM(ISG(1:NISG)%ILIST).EQ.1)THEN
...
! !## multiply segment selected
! ELSE

!  ISELISG=0 !; SHPI=1
!  !## remove current line
!  CALL POLYGON1DRAWSHAPE(SHPNO,SHPNO)
!  SHPNO=ISGSHAPES; IF(ALLOCATED(SHPIACT))SHPIACT=0

! ENDIF

! !## mouse selected - restore dialog menu field
! IF(CODE.EQ.0)THEN
!  CALL WDIALOGSELECT(ID_DISGEDITTAB1)
!!  ISG%ILIST=0; IF(ISELISG.GT.0)ISG(ISELISG)%ILIST=1
!  CALL WDIALOGPUTOPTION(IDF_MENU1,ISG(1:NISG)%ILIST)
! ENDIF

 CALL ISGDRAWSELECTEDSEGMENTS()
 ...
 !###======================================================================
 SUBROUTINE ISGDRAWSELECTEDSEGMENTS()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,IREC
 ...
 END SUBROUTINE ISGDRAWSELECTEDSEGMENTS
