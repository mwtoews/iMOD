...
! READ(CDATES(IDATE),'(I8)') IDATE
 ...
 IF(ISFR.EQ.0)THEN
  CALL WDIALOGPUTMENU(IDF_MENU4,TATTRIB1,SIZE(TATTRIB1),1)
 ELSEIF(ISFR.EQ.1)THEN
  CALL WDIALOGPUTMENU(IDF_MENU4,TATTRIB2,SIZE(TATTRIB2),1)
 ENDIF
...
!     CALL WDIALOGPUTOPTION(IDF_MENU2,)
 ...
 !## aanroepen vanuit isgplot ... als window actief is ...
  ...
  CASE (FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE1)
    CASE (IDF_MENU2,IDF_MENU4,IDF_MENU5,IDF_MENU6)
    CASE (IDF_TRACKBAR1)
   END SELECT 
 END SELECT
...
!   DX=DX+DY; IF(DX.NE.0.0)DX=SQRT(DX)
 ...
 IF(NISGFILES.GT.1)THEN
  ...
  DO IPLOT=1,MXMPLOT
   IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.4)THEN
    IISGPLOT=IISGPLOT+1
    ISGFILE(IISGPLOT)=TRIM(MP(IPLOT)%IDFNAME)
   ENDIF
  END DO
 ELSE
