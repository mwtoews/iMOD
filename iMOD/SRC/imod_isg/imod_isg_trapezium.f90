...
CONTAINS
 ...
 !###====================================================================
 SUBROUTINE ISGCOMPUTEEIGHTPOINTS(XIN,YIN,N,XSIMPLE,YSIMPLE,NSIMPLE,AORG,ASIMPLE)
 !###====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: ZTOLSTART=0.01D0
 REAL(KIND=DP_KIND) :: ZTOLERANCE
 INTEGER,INTENT(IN) :: N
 INTEGER,INTENT(INOUT) :: NSIMPLE
 REAL(KIND=DP_KIND),INTENT(OUT),OPTIONAL :: AORG,ASIMPLE
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(N) :: XIN,YIN
 REAL(KIND=DP_KIND),INTENT(OUT),DIMENSION(MAX(N,NSIMPLE)) :: XSIMPLE,YSIMPLE
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: GCODE,X,Y,XTMP,YTMP
 INTEGER :: I,J,M,NTARGET
 REAL(KIND=DP_KIND) :: Z
 
 ZTOLERANCE=ZTOLSTART; NTARGET=NSIMPLE

 ALLOCATE(X(N),Y(N))
 DO I=1,N; X(I)=XIN(I); Y(I)=YIN(I); ENDDO

 IF(PRESENT(AORG))THEN
  M=N+2; ALLOCATE(XTMP(M),YTMP(M))
  DO I=1,N; XTMP(I+1)=XIN(I); YTMP(I+1)=YIN(I); ENDDO
  XTMP(1)=XIN(1); YTMP(1)=MAXVAL(YIN)
  XTMP(M)=XIN(N); YTMP(M)=MAXVAL(YIN)
  AORG=UTL_POLYGON1AREA(XTMP,YTMP,M)
 ...
 IF(N.GT.NTARGET)THEN
  ...
  DO
   ...
   DO I=1,M
    ...
    ENDIF
   ENDDO
   IF(NSIMPLE.LE.8)EXIT
   ZTOLERANCE=ZTOLERANCE+ZTOLSTART
  ENDDO
 ...
 ELSE
  ...
  NSIMPLE=N; DO I=1,N; XSIMPLE(I)=X(I); YSIMPLE(I)=Y(I); ENDDO 

 ENDIF

 !## extent if necessary
 IF(NSIMPLE.LT.NTARGET)THEN
  Z=MAXVAL(YSIMPLE(1:NSIMPLE))
  DO J=NSIMPLE+1,NTARGET; XSIMPLE(J)=XSIMPLE(J-1)+1.0D0; YSIMPLE(J)=Z; ENDDO
 ENDIF
 NSIMPLE=NTARGET

 IF(PRESENT(ASIMPLE))THEN
  M=NSIMPLE+2; ALLOCATE(XTMP(M),YTMP(M))
  DO I=1,NSIMPLE; XTMP(I+1)=XSIMPLE(I); YTMP(I+1)=YSIMPLE(I); ENDDO
  XTMP(1)=XSIMPLE(1);       YTMP(1)=MAXVAL(YSIMPLE(1:NSIMPLE))
  XTMP(M)=XSIMPLE(NSIMPLE); YTMP(M)=YTMP(1)
  ASIMPLE=UTL_POLYGON1AREA(XTMP,YTMP,M)
  DEALLOCATE(XTMP,YTMP)
 ENDIF
 ...
 REAL(KIND=DP_KIND),INTENT(OUT) :: AORG,ATRAP
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(NDIM) :: X,Y
 REAL(KIND=DP_KIND),INTENT(OUT),DIMENSION(4,NDIM) :: XTRAP,YTRAP
 REAL(KIND=DP_KIND),INTENT(OUT),DIMENSION(NDIM) :: XSYM,YSYM
 INTEGER :: I,J,K,I1,I2,NN,N1,N2,N3,IMID,ITRAP
 REAL(KIND=DP_KIND) :: YMIN,XMIN,RC,RC1,DX,DY,XC,A1,A2,A3,YV,XT
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: XN,YN
...
! CALL UTL_QKSORT2(XN(:,3),YN(:,3),NDIM,N3)
 ...
 !#extend
  ...
  !## if net area > 0.0D0 than add trapezium, otherwise skip it!
  IF(A2.GT.0.0D0.AND.DX.GT.0.0D0)THEN

   !## new crosssection
   XTRAP(1,I)= 0.0D0
   XTRAP(2,I)= DX
   XTRAP(3,I)= 0.0D0 !to be fitted
   XTRAP(4,I)= 0.0D0
 ...
 !###====================================================================
 REAL(KIND=DP_KIND) FUNCTION ISGGETX(X1,Y1,X2,Y2,Y3)
 !###====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: X1,Y1,X2,Y2,Y3
 REAL(KIND=DP_KIND) :: DX,DY,RC,B
 ...
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(NDIM,3) :: XN,YN
 INTEGER :: J,I,N
 ...
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(NDIM) :: XN,YN,XA
 REAL(KIND=DP_KIND),INTENT(OUT),DIMENSION(NDIM) :: XTRAP,YTRAP
 REAL(KIND=DP_KIND) :: AREA
 INTEGER :: K
 ...
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N
 REAL(KIND=DP_KIND),INTENT(IN) :: AREA
 REAL(KIND=DP_KIND),DIMENSION(N),INTENT(INOUT) :: X,Y
 REAL(KIND=DP_KIND) :: A,B
