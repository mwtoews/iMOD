...
CONTAINS
 ...
 REAL,INTENT(OUT),OPTIONAL :: AORG,ASIMPLE
 REAL,INTENT(IN),DIMENSION(N) :: XIN,YIN
 REAL,INTENT(OUT),DIMENSION(MAX(N,NSIMPLE)) :: XSIMPLE,YSIMPLE
 REAL,ALLOCATABLE,DIMENSION(:) :: GCODE,X,Y,XTMP,YTMP
 ...
 IF(NSIMPLE.LT.NTARGET)THEN
  ...
  DO J=NSIMPLE+1,NTARGET; XSIMPLE(J)=XSIMPLE(J-1)+1.0; YSIMPLE(J)=Z; ENDDO
 ENDIF
 NSIMPLE=NTARGET
