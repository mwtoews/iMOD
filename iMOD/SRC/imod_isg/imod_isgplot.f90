 ...
 IF(LOPEN)THEN
  ...
  DO I=1,NISG
   ...
   IF(NCLC.GT.DIMCLC)THEN
    ...
    DIMCLC=NCLC; ALLOCATE(ISD(DIMCLC))
   ENDIF
   IF(NCRS.GT.DIMCRS)THEN
    IF(ALLOCATED(ISC))DEALLOCATE(ISC)
    DIMCRS=NCRS; ALLOCATE(ISC(DIMCRS))
   ENDIF
   IF(NSTW.GT.DIMSTW)THEN
    IF(ALLOCATED(IST))DEALLOCATE(IST)
    DIMSTW=NSTW; ALLOCATE(IST(DIMSTW))
   ENDIF
   IF(NQHR.GT.DIMQHR)THEN
    IF(ALLOCATED(ISQ))DEALLOCATE(ISQ)
    DIMQHR=NQHR; ALLOCATE(ISQ(DIMQHR))
   ENDIF

   !## read calculation point information from *.isd1,*.isd2
   IREC=ID(2)-1
   DO J=1,NCLC
    READ(ISGIU(3,1),REC=IREC+J+ICF) ISD(J)%N,ISD(J)%IREF,ISD(J)%DIST,ISD(J)%CNAME
   END DO
   !## read cross-section information from *.isc1,*.isc2
   IREC=ID(3)-1
   DO J=1,NCRS
    READ(ISGIU(5,1),REC=IREC+J+ICF) ISC(J)%N,ISC(J)%IREF,ISC(J)%DIST,ISC(J)%CNAME
   END DO
   !## read structure information from *.ist1,*.ist2
   IREC=ID(4)-1
   DO J=1,NSTW
    READ(ISGIU(7,1),REC=IREC+J+ICF) IST(J)%N,IST(J)%IREF,IST(J)%DIST,IST(J)%CNAME
   END DO
    ...
    IF(LEX)THEN
     ...
     IF(J.GT.1)THEN
      IF(MIN(X1,X).LT.XMAX.AND.MAX(X1,X).GT.XMIN.AND. &
       ...
       IF(NCLC.GT.0.AND.ICLC.EQ.1)CALL ISGPLOTLOCCRS(X1,X,Y1,Y,DX,TDIST(1),DIMCLC,ISD%DIST,ISD%CNAME,ISD%N,NCLC,1,IISGLABEL)
       !## plot cross-sections on segment
       IF(NCRS.GT.0.AND.ICRS.EQ.1)CALL ISGPLOTLOCCRS(X1,X,Y1,Y,DX,TDIST(2),DIMCRS,ISC%DIST,ISC%CNAME,ISC%N,NCRS,2,IISGLABEL)
       !## plot weirs on segment
       IF(NSTW.GT.0.AND.ISTW.EQ.1)CALL ISGPLOTLOCCRS(X1,X,Y1,Y,DX,TDIST(3),DIMSTW,IST%DIST,IST%CNAME,IST%N,NSTW,3,IISGLABEL)
       !## plot weirs on segment
       IF(NQHR.GT.0.AND.IQHR.EQ.1)CALL ISGPLOTLOCCRS(X1,X,Y1,Y,DX,TDIST(4),DIMQHR,ISQ%DIST,ISQ%CNAME,ISQ%N,NQHR,4,IISGLABEL)
      ELSE
       DIST =SQRT((X1-X)**2.0+(Y1-Y)**2.0); TDIST=TDIST+DIST
      ENDIF
     ENDIF
    ENDIF
  ...
  IF(ALLOCATED(ISD))DEALLOCATE(ISD);   IF(ALLOCATED(ISC))DEALLOCATE(ISC)
  IF(ALLOCATED(IST))DEALLOCATE(IST);   IF(ALLOCATED(ISQ))DEALLOCATE(ISQ)
