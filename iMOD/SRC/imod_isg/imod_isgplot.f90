...
CONTAINS
 ...
 IF(LOPEN)THEN
  ...
  DO I=1,NISG
   ...
   IF(IFDR.EQ.1)THEN
    ...
    CALL ISGPLOT_FDIRECTION(XSEG,YSEG,NSEG,XMIN,XMAX,YMIN,YMAX,1.0)
   ENDIF
 ...
 ELSE
  ...
  DO I=1,NISG
   ...
   DO J=1,NSEG
    ...
    LEX=.TRUE.
    IF(IISG.EQ.1)THEN
     LEX=.FALSE.; IF(ISG(I)%ILIST.NE.1)LEX=.FALSE. !EQ.1)LEX=.TRUE.
    ENDIF
   ...
   IF(IFDR.EQ.1)THEN
    ...
    IF(LEX)THEN
     CALL IGRCOLOURN(ICLRSF); CALL IGRFILLPATTERN(SOLID)
     !## get correct record-number for coordinates
     IREC=ID(1)-1; CALL ISGPLOT_FDIRECTION(ISP(IREC+1:IREC+NSEG)%X,ISP(IREC+1:IREC+NSEG)%Y,NSEG,XMIN,XMAX,YMIN,YMAX,1.0)
    ENDIF
   ENDIF
 ...
 !###======================================================================
 SUBROUTINE ISGPLOT_FDIRECTION(X,Y,N,XMIN,XMAX,YMIN,YMAX,FMULT)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N
