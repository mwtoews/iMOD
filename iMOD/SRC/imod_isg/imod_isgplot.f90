...
!Draws a curve which passes through all of the supplied control points
 ...
 DO ILOC=1,NLOC
  ...
  IF(LEX)THEN
   ...
   CALL ISGPLOTSHAPE(ISHAPE,XC,YC,D,D)
 ...
 !###======================================================================
 SUBROUTINE ISGPLOTSHAPE(ISHAPE,XC,YC,DX,DY)
 !###======================================================================
 IMPLICIT NONE
 ...
 IF(ISHAPE.EQ.1)THEN
  ...
  CALL IGRFILLPATTERN(OUTLINE)
  CALL IGRRECTANGLE(XC-DX,YC-DY,XC+DX,YC+DY)    !  |--|--|
  CALL IGRJOIN(XC-DX,YC-DY,XC+DX,YC+DY)         !  | /|\ |
  CALL IGRJOIN(XC-DX,YC+DY,XC+DX,YC-DY)         !  |--|--|
  CALL IGRFILLPATTERN(SOLID)
 !## triangle
 ELSEIF(ISHAPE.EQ.2)THEN
  CALL IGRCOLOURN(ICLRSC)
  CALL IGRJOIN(XC-2.0*DX,YC+DY,XC+2.0*DX,YC+DY) !\--------/
  CALL IGRJOIN(XC-2.0*DX,YC+DY,XC-DX,YC-DY)     ! \      /
  CALL IGRJOIN(XC-DX,YC-DY,XC+DX,YC-DY)         !  \ *  /
  CALL IGRJOIN(XC+DX,YC-DY,XC+2.0*DX,YC+DY)     !   \--/
 !## structure
 ELSEIF(ISHAPE.EQ.3)THEN
  CALL IGRCOLOURN(ICLRST)
  CALL IGRJOIN(XC-DX,YC-DY,XC,YC+DY)        !    /\
  CALL IGRJOIN(XC,YC+DY,XC+DX,YC-DY)        !   /* \
  CALL IGRJOIN(XC+DX,YC-DY,XC-DX,YC-DY)     !  /----\
 !## qh-relationship
 ELSEIF(ISHAPE.EQ.4)THEN
  CALL IGRCOLOURN(ICLRQH)
  CALL IGRPOLYGONCOMPLEX((/XC-DX,XC-DX,XC+DX/),(/YC-DY,YC+DY,YC+DY/),3)
