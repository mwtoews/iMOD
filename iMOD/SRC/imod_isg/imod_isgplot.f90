...
CONTAINS
 ...
 IF(.NOT.LEX)THEN
  READ(ISGIU(1,1),*) N
  DO I=1,N
   READ(ISGIU(1,1),*) CID,II,NN
   !## read segments
   II=II-1
   DO J=1,NN
    IF(ISGDOUBLE.EQ.4)THEN
     READ(ISGIU(2,1),REC=II+J+ICF) XS,YS
     X=REAL(XS,8); Y=REAL(YS,8)
    ELSE
     READ(ISGIU(2,1),REC=II+J+ICF) X,Y
    ENDIF
 ...
 DO ILOC=1,NLOC
  ...
  IF(LEX)THEN
   ...
   IF(IISGLABEL.EQ.1)THEN
    CALL ISGTEXTANGLE(X2-X1,Y2-Y1,-90.0D0,2.0D0*D,ALIGNLEFT,XPOS,YPOS)
