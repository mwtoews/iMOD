...
USE MOD_ISG_PAR, ONLY : ISGIU,NISG,ISG,ISP,IST,ISC,ISD,ISQ,DATISD,ICLRSP,ICLRND,ICLRSF,ICLRSD,ICLRSC, &
                        ICLRST,ICLRQH,ICLRCO,ISFR,NISP,ISGFNAME,ISGLEG
...
CONTAINS
 ...
 IF(XMIN.EQ.XMAX.AND.YMIN.EQ.YMAX)THEN
  XMIN=10.0E10; YMIN=10.0E10; XMAX= 0.0; YMAX= 0.0
 ENDIF
 ...
 INTEGER,INTENT(IN) :: IPLOT
 REAL,INTENT(INOUT) :: XMIN,YMIN,XMAX,YMAX
 INTEGER :: IRAT,IRAT1,I,J,NSEG,NCRS,IREC,IQHR,IFDR,IFCO,ISTW,NSTW,IISGLABEL,DWNSEG,  &
 ...
 IF(LOPEN)THEN
  ...
  DO I=1,N
   ...
   CALL UTL_WAITMESSAGE(IRAT,IRAT1,I,N,'Drawing ISG ...') 
...
!       ENDIF
 ...
 INTEGER :: I,J,K,IREC,JREC,MAXNSEG,TTIME,NSEG,NDIM,ICLR, &
            IVAR,ILINEW,ILINET,ISTEADY,IISG
 LOGICAL :: LEX
 REAL :: X1,Y1,X2,Y2
 ...
 !## aanroepen vanuit isgplot ... als window actief is ...
...
! ENDIF
 ...
 CALL IGRLINETYPE(ILINET); CALL IGRLINEWIDTH(ILINEW); IF(ILINEW.GT.1)CALL IGRLINECAP(ROUNDCAP)
 ...
 DO I=1,NISG
  ...
  CALL ISGGRIDGETSTREAMDATA(X,Y,DIST,IPOS,RVAL,ISG(I)%ICLC,ISG(I)%NCLC,ISG(I)%ISTW, &
                            ISG(I)%NSTW,NSEG,MAXNSEG,QSORT,XNR,NDATA,TTIME,NDIM,ISTEADY)
  !## interpolate all segments in between and give proper values!
  CALL ISGGRIDINTSTREAMDATA(DIST,IPOS,RVAL,NSEG,MAXNSEG,NDIM)
  
  CALL IGRMOVETO(X(1),Y(1))

  DO J=2,NSEG

   ICLR=UTL_IDFGETCLASS(ISGLEG(1),RVAL(IVAR,J))
 ...
 CALL IGRLINETYPE(SOLIDLINE); CALL IGRLINEWIDTH(1); CALL IGRLINECAP(BUTTCAP)
