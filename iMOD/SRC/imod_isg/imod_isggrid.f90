...
USE MODPLOT
USE MOD_ISG_PAR
USE MOD_UTL, ONLY : ITOS,RTOS,IDATETOJDATE,JDATETOIDATE,JDATETOGDATE,UTL_GETUNIT,UTL_WAITMESSAGE,UTL_IDFSNAPTOGRID,UTL_GETMED, &
...
!      CALL ISGMEMORYISC(1,ISELISG,IPOS)
 ...
 !###====================================================================
 SUBROUTINE ISG2GRIDMAIN(IBATCH,LOKAY) 
 !###====================================================================
 IMPLICIT NONE
 LOGICAL,INTENT(OUT),OPTIONAL :: LOKAY
 INTEGER,INTENT(IN) :: IBATCH
 INTEGER :: NROW,NCOL,IU,SIMDATE1,SIMDATE2,SIMDATE3 
 ...
 SIMDATE1=IDATETOJDATE(SDATE) !## begin juliandate
 SIMDATE3=IDATETOJDATE(EDATE) !## eind  juliandate
 DO
  ...
  SDATE=JDATETOIDATE(SIMDATE1)
  EDATE=JDATETOIDATE(SIMDATE2)
  ...
  IF(PRESENT(LOKAY))LOKAY=LEX
  !## no multiply griddings
  IF(DDATE.EQ.0)EXIT
  SIMDATE1=SIMDATE2+1
 END DO

 END SUBROUTINE ISG2GRIDMAIN
 ...
 CALL IDFNULLIFY(THIESSEN); IF(.NOT.IDFREAD(THIESSEN,THIESSENFNAME,1))THEN; ENDIF
...
!      XC=XMIN+(TRAP(1,ITRAP)%ICOL*CSIZE)-(CSIZE/2.0)
!      YC=YMAX-(TRAP(1,ITRAP)%IROW*CSIZE)+(CSIZE/2.0)

      !## determine surface level
      MV=IDFGETXYVAL(AHN,XC,YC)
!      IDFGETXYVAL(JU,XC,YC,NCOLIDF,NROWIDF,XMINIDF,YMAXIDF,CSIDF,NODATAIDF)
!      IF(IUNITAHN.EQ.2)MV=MV/100.0
 ...
 !###====================================================================
 SUBROUTINE ISG2GRID(LOKAY,PPOSTFIX,NROW,NCOL,IBATCH)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NROW,NCOL,IBATCH 
 ...
 REAL :: X1,X2,Y1,Y2,D,FCT,CT,BH,BW,NETWD,COND,H1,H2,Z,WL,TD,TC
 LOGICAL :: LEX,LNODAT
 ...
 NITEMS=MAXITEMS; IF(ICDIST.EQ.0)NITEMS=9 !8
 ...
 ALLOCATE(QSORT(TTIME,NITEMS),XNR(NITEMS))
  ...
  IF(LEX)THEN
   ...
   DO
    ...
    IF(IPOS(J).GT.0)THEN
     ...
     CALL ISG2GRIDGETDATA(SDATE,EDATE,TTIME,QSORT,XNR,NITEMS,RVAL(1,J),ISD(IREF)%N,ISD(IREF)%IREF,ISS,1,NODATA) !,NTOC)
    ENDIF
    IF(IPOS(J).LT.0)THEN
     IREF=ISG(I)%ISTW+ABS(IPOS(J))-1
     CALL ISG2GRIDGETDATA(SDATE,EDATE,TTIME,QSORT,XNR,NITEMS,RVAL(1,J),IST(IREF)%N,IST(IREF)%IREF,ISS,-1,NODATA) !,NTOC)
     J=J+1
   ...
   IF(LNODAT)THEN !H1.NE.NODATA.AND.H2.NE.NODATA)THEN
    H1=RVAL(1,1)
    H2=RVAL(1,NSEG)
...
!            VALUE(1,1)= VALUE(1,1)+COND   !total conductance(m2/dag)
 ...
 DO IROW=1,NROW; DO ICOL=1,NCOL
  ...
  IF(IDF(1)%X(ICOL,IROW).EQ.IDF(1)%NODATA)THEN
   IDF(2)%X(ICOL,IROW)=NODATA
   IDF(3)%X(ICOL,IROW)=NODATA
   IF(ICDIST.EQ.1)IDF(10)%X(ICOL,IROW)=NODATA
  ENDIF
 ENDDO; ENDDO
 !## replace nodata value for waterlevels [idf(1)], bottomlevels [idf(2)] to -999.99
 IDF(2)%NODATA=NODATA; IDF(3)%NODATA=NODATA
 IF(ICDIST.EQ.1)IDF(10)%NODATA=NODATA
 ...
 DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
  IF(IDF(1)%X(ICOL,IROW).GT.0.0)THEN
   ...
   IF(NLAY.EQ.0)THEN
    LINE='1,'//TRIM(ITOS(IROW))//','//TRIM(ITOS(ICOL))//','//TRIM(RTOS(IDF(2)%X(ICOL,IROW),'F',3))//','// &
               TRIM(RTOS(IDF(1)%X(ICOL,IROW),'F',2))//','//TRIM(RTOS(IDF(3)%X(ICOL,IROW),'F',3))//','// &
               TRIM(RTOS(IDF(4)%X(ICOL,IROW),'F',2))//',1'
   ...
   ELSE
    ...
    DO I=1,NLAY
     ...
     IF(WL.GT.B.AND.BL.LT.T)THEN
      ...
      LINE='1,'//TRIM(ITOS(IROW))//','//TRIM(ITOS(ICOL))//','//TRIM(RTOS(IDF(2)%X(ICOL,IROW),'F',3))//','// &
                 TRIM(RTOS(CD*F,'F',2))//','//TRIM(RTOS(IDF(3)%X(ICOL,IROW),'F',3))//','// &
                 TRIM(RTOS(IDF(4)%X(ICOL,IROW),'F',2))//',1'
!      WRITE(LINE,*) I,IROW,ICOL,IDF(2)%X(ICOL,IROW),CD*F,IDF(3)%X(ICOL,IROW),IDF(4)%X(ICOL,IROW),1
      WRITE(IU,'(A)') TRIM(LINE)
!      WRITE(IU,'(3I10,F10.2,G10.4,2F10.2)') I,IROW,ICOL,IDF(2)%X(ICOL,IROW),CD*F,IDF(3)%X(ICOL,IROW),IDF(4)%X(ICOL,IROW)
     ENDIF
    ENDDO
   ENDIF
  ENDIF
...
!ENDIF    
 ...
 INTEGER(KIND=2),DIMENSION(2,MAXTHREAD) :: YSEL !(2,MAXTHREAD)
 REAL :: NOR,OR,X,Y,DXY
 INTEGER :: I,IDX,IDY,ICOL,IROW

! CALL IDFIROWICOL(IDF,IROW,ICOL,XC,YC)
 DXY=0.5*(IDF%DX+IDF%DY) !SQRT(IDF%DX**2.0+IDF%DY**2.0)

 DXY=1.0*(IDF%DX+IDF%DY) !SQRT(IDF%DX**2.0+IDF%DY**2.0)
  
 !## block correct side of structure
 OR=90.0
 DO
!  OR=OR+5.0; IF(OR.GT.180.0)EXIT
  OR=OR+5.0; IF(OR.GT.270.0)EXIT
  X=XC-COS((ORIENT-OR)/GRAD)*DXY !TAN(ORIENT-OR)*DXY
  Y=YC-SIN((ORIENT-OR)/GRAD)*DXY !TAN(ORIENT-OR)*DXY
 ...
 SUBROUTINE ISG2GRIDGETDATA(SDATE,EDATE,TTIME,QSORT,XNR,NITEMS,RVAL, &
                            NR,IREF,ISS,ITYPE,NODATA) !,NTOC)
 !###====================================================================
 IMPLICIT NONE
 !INTEGER,INTENT(INOUT) :: NTOC
 ...
 INTEGER :: IR,N,I,I1,I2,IDATE,NAJ,IREC,NDATE !,ITIC,ITOC
 ...
 IF(ISS.EQ.2)QSORT=NODATA !-999.99
 I1   = 1
...
!   IF(NITEMS.GT.4)RVAL(5)=DATISD(IREC)%WL_STW
