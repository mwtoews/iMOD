...
!    enddo
 ...
 DO
  ...
  IF(GRIDISG%ISTEADY.EQ.2.AND.GRIDISG%EDATE.LT.GRIDISG%SDATE)THEN
   ...
   ENDIF
  ENDIF
  ISG2GRIDMAIN=ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,0,TOP,BOT,KHV,BND,IBATCH,MP,0,GRIDISG) 
 ...
 !###====================================================================
 LOGICAL FUNCTION ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,ILAY,TOP,BOT,KHV,BND,IBATCH, &
                           MP,JU,GRIDISG) 
 !###====================================================================
 IMPLICIT NONE
...
! IF(.NOT.IDFWRITE(IDF(9),TRIM(GRIDISG%ROOT)//'\'//TRIM(FNAME(9))//TRIM(PPOSTFIX)//'.IDF',1))THEN; endif
 ...
 ELSEIF(GRIDISG%IEXPORT.EQ.1)THEN
  CALL ISG2GRID_EXPORTRIVER(JU,IDF,NLAY,ILAY,TOP,BOT,KHV,BND,MP,GRIDISG)
 ENDIF
 ...
 !###====================================================================
 SUBROUTINE ISG2GRID_EXPORTRIVER(JU,IDF,NLAY,ILAY,TOP,BOT,KHV,BND,MP,GRIDISG)
 !###====================================================================
 IMPLICIT NONE
 TYPE(GRIDISGOBJ),INTENT(INOUT) :: GRIDISG
  ...
  DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
   ...
   ELSE
    ...
    DO I=1,NLAY
     ...
     ENDIF
    ENDDO
!    IF(ICOL.EQ.46.AND.IROW.EQ.76)THEN
!  WRITE(*,*)
!    ENDIF
    CALL UTL_PCK_GETTLP(NLAY,TLP,KH,TP,BT,WL,BL,MINKHT,ICLAY) 
    DO I=1,NLAY
     IF(TLP(I).EQ.0.0)CYCLE
     N=N+1; IF(J.EQ.2)WRITE(IU,FRM) I,IROW,ICOL,WL,CD*TLP(I),BL,F,1
    ENDDO
   ENDIF
  ENDDO; ENDDO
