...
!    DATISD(IREF)%WLVL=DATISD(IREF+NTXT)%WLVL
 ...
 !##=====================================================================
 SUBROUTINE ISG_ADDCROSSSECTION(FNAME,WIDTHFNAME,MAXDIST,CROSS_PNTR,CROSS_BATH,CROSS_ZCHK,CELL_SIZE)
 !##=====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: FNAME,WIDTHFNAME,CROSS_PNTR,CROSS_BATH,CROSS_ZCHK
 REAL,INTENT(IN) :: MAXDIST,CELL_SIZE
 INTEGER :: IU,IOS,N,I,J,IISG,IPOS,ISEG,NSEG,ICRS,NCRS,IPNT,NPNT,IROW,ICOL,IP,ZCHK
 ...
 ELSE
  ...
  IF(.NOT.IDF_EXTENT(2+ZCHK,ICROSS,ICROSS(4),2))RETURN
  
  IF(CELL_SIZE.NE.0.0)THEN
   ICROSS(4)%DX=CELL_SIZE; ICROSS(4)%DY=ICROSS(4)%DX
   CALL UTL_IDFSNAPTOGRID(ICROSS(4)%XMIN,ICROSS(4)%XMAX,ICROSS(4)%YMIN,ICROSS(4)%YMAX,ICROSS(4)%DX,ICROSS(4)%NCOL,ICROSS(4)%NROW)
  ENDIF
  
  !## read pointer
  CALL IDFCOPY(ICROSS(4),ICROSS(1))
  IF(.NOT.IDFREADSCALE(CROSS_PNTR,ICROSS(1),7,0,0.0,0))RETURN !## most frequent occurence
  !## read zval at pointer scale
  CALL IDFCOPY(ICROSS(4),ICROSS(2))
  IF(.NOT.IDFREADSCALE(CROSS_BATH,ICROSS(2),2,1,0.0,0))RETURN !## average value
  IF(ZCHK.EQ.1)THEN
   !## read zchk values
   CALL IDFCOPY(ICROSS(4),ICROSS(3))
