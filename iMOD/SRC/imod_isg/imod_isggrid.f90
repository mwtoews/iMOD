...
!      CALL ISGMEMORYISC(1,ISELISG,IPOS)
 ...
 !###====================================================================
 LOGICAL FUNCTION ISG2GRIDMAIN(ISGFILE,IBATCH,NLAY,TOP,BOT,ISTEADY)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IBATCH,NLAY,ISTEADY
 CHARACTER(LEN=*),INTENT(IN) :: ISGFILE
 ...
 DO
  ...
  ENDIF
  IF(SIMDATE2.GT.SIMDATE3.AND.ISTEADY.EQ.2)EXIT
  ...
  IF(ISTEADY.EQ.2.AND.EDATE.LT.SDATE)THEN
   IF(IBATCH.EQ.0)THEN
   ...
   ENDIF
  ENDIF
  ISG2GRIDMAIN=ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,0,TOP,BOT,IBATCH,MP,0,ISTEADY)
...
!      CALL IDFGETLOC(THIESSEN,TRAP(1,ITRAP)%IROW,TRAP(1,ITRAP)%ICOL,XC,YC)
 ...
 !###====================================================================
 LOGICAL FUNCTION ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,ILAY,TOP,BOT,IBATCH,MP,JU,ISTEADY)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NROW,NCOL,NLAY,ILAY,IBATCH,JU,ISTEADY
 ...
 ISGLOOP: DO I=1,NISG
  ...
  IF(LEX)THEN
   ...
   CALL ISGGRIDGETSTREAMDATA(X,Y,DIST,IPOS,RVAL,ISG(I)%ICLC,ISG(I)%NCLC,ISG(I)%ISTW,ISG(I)%NSTW,NSEG,MAXNSEG, &
                             QSORT,XNR,NDATA,TTIME,MDIM,ISTEADY)
...
!   CALL ISG2GRIDINTSEGMENT(X,Y,DIST,NSEG,MAXNSEG)
   ...
   IF(LNODAT)THEN
    ...
    CALL ISGGRIDINTSTREAMDATA(DIST,IPOS,RVAL,NSEG,MAXNSEG,MDIM)
...
!                                            TRIM(ROOT)//'\'//TRIM(FNAME(12))//TRIM(PPOSTFIX)//'.IDF')    !## next_id
 ...
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: MAXNSEG,ICLC,ISTW,NCLC,NSTW,TTIME,NDIM,ISTEADY
 ...
 DO
  ...
  IF(IPOS(I).GT.0)THEN
   IREF=ICLC+IPOS(I)-1
   CALL ISG2GRIDGETDATA(SDATE,EDATE,TTIME,QSORT,XNR,NDIM,RVAL(1,I),ISD(IREF)%N,ISD(IREF)%IREF,ISTEADY, 1,NDATA)
  ENDIF
  IF(IPOS(I).LT.0)THEN
   IREF=ISTW+ABS(IPOS(I))-1
   CALL ISG2GRIDGETDATA(SDATE,EDATE,TTIME,QSORT,XNR,NDIM,RVAL(1,I),IST(IREF)%N,IST(IREF)%IREF,ISTEADY,-1,NDATA)
   I=I+1
   !## replace waterlevel_down to waterlevel structure
   RVAL(1,I)=RVAL(2,I-1)
  ENDIF
 ...
 !###====================================================================
 SUBROUTINE ISGGRIDINTSTREAMDATA(DIST,IPOS,RVAL,NSEG,MAXNSEG,NDIM)
 !###====================================================================
 IMPLICIT NONE
 ...
 !###====================================================================
 LOGICAL FUNCTION ISG2SFR(NROW,NCOL,NLAY,ILAY,TOP,BOT,IPER,MP,JU,ISTEADY)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NROW,NCOL,NLAY,ILAY,JU,IPER,ISTEADY
 INTEGER,INTENT(INOUT),DIMENSION(:) :: MP
 TYPE(IDFOBJ),DIMENSION(NLAY),INTENT(INOUT) :: TOP,BOT
 ...
 NSTREAM=0; DO I=1,NISG
  ...
  IREF=ISG(I)%ICLC
  CALL ISG2GRIDGETDATA(SDATE,EDATE,TTIME,QSORT,XNR,SIZE(XNR),RVAL(1,1),ISD(IREF)%N,ISD(IREF)%IREF,ISTEADY,1,NDATA)
  !## read data for end-point
  IREF=ISG(I)%ICLC+1
  CALL ISG2GRIDGETDATA(SDATE,EDATE,TTIME,QSORT,XNR,SIZE(XNR),RVAL(1,2),ISD(IREF)%N,ISD(IREF)%IREF,ISTEADY,1,NDATA)
