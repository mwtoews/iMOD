...
CONTAINS
 ...
 !##=====================================================================
 SUBROUTINE ISG_IMPORT(ISGFILE,EXPORTFNAME,IBATCH)
 !##=====================================================================
 IMPLICIT NONE
 ...
 READ(IU(2),*); DO NISP=1,SIZE(ISP); READ(IU(2),'(10X,2(1X,F15.3))') ISP(NISP)%X,ISP(NISP)%Y; ENDDO; CLOSE(IU(2));

 !## read isd1/isd2
 READ(IU(3),*); DIMDATISD=0
 DO NISD=1,SIZE(ISD)
  READ(IU(3),'(10X,2(1X,I10),1X,F15.3,1X,A32)') ISD(NISD)%N,ISD(NISD)%IREF,ISD(NISD)%DIST,ISD(NISD)%CNAME
 ...
 IF(ISFR.EQ.0)THEN
  DO I=1,SIZE(DATISD)
   READ(IU(4),'(10X,1X,I10,4(1X,F15.3))') DATISD(I)%IDATE,DATISD(I)%WLVL,DATISD(I)%BTML,DATISD(I)%RESIS,DATISD(I)%INFF
  ENDDO
 ELSE
  DO I=1,SIZE(DATISD)
   READ(IU(4),'(10X,1X,I10,1X,A10,5(1X,F15.3),4(1X,I10),4(1X,F15.3))') DATISD(I)%IDATE,DATISD(I)%CTIME,DATISD(I)%WLVL, &   
 ...
 DO NISC=1,SIZE(ISC)
  READ(IU(5),'(10X,2(1X,I10),1X,F15.3,1X,A32)') ISC(NISC)%N,ISC(NISC)%IREF,ISC(NISC)%DIST,ISC(NISC)%CNAME
  DIMDATISC=DIMDATISC+ISC(NISC)%N
 ENDDO
 CLOSE(IU(5)); ALLOCATE(DATISC(DIMDATISC))
 READ(IU(6),*)
 DO I=1,SIZE(DATISC)
  READ(IU(6),'(10X,4(1X,F15.3))') DATISC(I)%DISTANCE,DATISC(I)%BOTTOM,DATISC(I)%MRC,DATISC(I)%ZP
 ENDDO
 CLOSE(IU(6))
 ...
 DO NIST=1,SIZE(IST)
  READ(IU(7),'(10X,2(1X,I10),1X,F15.3,1X,A32)') IST(NIST)%N,IST(NIST)%IREF,IST(NIST)%DIST,IST(NIST)%CNAME
  DIMDATIST=DIMDATIST+IST(NIST)%N
 ENDDO
 CLOSE(IU(7)); ALLOCATE(DATIST(DIMDATIST))
 READ(IU(8),*)
 DO I=1,SIZE(DATIST)
  READ(IU(8),'(10X,1X,I10,2(1X,F15.3))') DATIST(I)%IDATE,DATIST(I)%WLVL_UP,DATIST(I)%WLVL_DOWN
 ENDDO
 CLOSE(IU(8))
 ...
 DO NISQ=1,SIZE(ISQ)
  READ(IU(9),'(10X,2(1X,I10),1X,F15.3,1X,A32)') ISQ(NISQ)%N,ISQ(NISQ)%IREF,ISQ(NISQ)%DIST,ISQ(NISQ)%CNAME
  DIMDATISQ=DIMDATISQ+ISQ(NISQ)%N
 ENDDO
 CLOSE(IU(9)); ALLOCATE(DATISQ(DIMDATISQ))
 READ(IU(10),*)
 DO I=1,SIZE(DATISQ)
  READ(IU(10),'(10X,4(1X,F15.3))') DATISQ(I)%Q,DATISQ(I)%W,DATISQ(I)%D,DATISQ(I)%F
 ENDDO
 CLOSE(IU(10))
 ...
 IF(IEXPORT.EQ.0)THEN
  ...
  DO I=1,SIZE(ISD); WRITE(IU,'(I10,2(1X,I10),1X,F15.3,1X,A32)') I,ISD(I)%N,ISD(I)%IREF,ISD(I)%DIST,ISD(I)%CNAME; ENDDO
  CLOSE(IU)
  ...
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=FNAME,STATUS='UNKNOWN',FORM='FORMATTED',ACTION='WRITE,DENYREAD',ACCESS='SEQUENTIAL')
  WRITE(IU,'(A10,2(1X,A10),1X,A15,1X,A32)') 'RECORD','N','IREF','DIST','CNAME'
  DO I=1,SIZE(ISC); WRITE(IU,'(I10,2(1X,I10),1X,F15.3,1X,A32)') I,ISC(I)%N,ISC(I)%IREF,ISC(I)%DIST,ISC(I)%CNAME; ENDDO
  CLOSE(IU)
  ...
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=FNAME,STATUS='UNKNOWN',FORM='FORMATTED',ACTION='WRITE,DENYREAD',ACCESS='SEQUENTIAL')
  WRITE(IU,'(A10,2(1X,A10),1X,A15,1X,A32)') 'RECORD','N','IREF','DIST','CNAME'
  DO I=1,SIZE(IST); WRITE(IU,'(I10,2(1X,I10),1X,F15.3,1X,A32)') I,IST(I)%N,IST(I)%IREF,IST(I)%DIST,IST(I)%CNAME; ENDDO
  CLOSE(IU)
  ...
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=FNAME,STATUS='UNKNOWN',FORM='FORMATTED',ACTION='WRITE,DENYREAD',ACCESS='SEQUENTIAL')
  WRITE(IU,'(A10,2(1X,A10),1X,A15,1X,A32)') 'RECORD','N','IREF','DIST','CNAME'
  DO I=1,SIZE(ISQ); WRITE(IU,'(I10,2(1X,I10),1X,F15.3,1X,A32)') I,ISQ(I)%N,ISQ(I)%IREF,ISQ(I)%DIST,ISQ(I)%CNAME; ENDDO
  CLOSE(IU)
