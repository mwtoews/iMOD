...
!      CALL ISGMEMORYISC(1,ISELISG,IPOS)
 ...
 DO
  ...
  IF(ISS.EQ.2.AND.EDATE.LT.SDATE)THEN
   ...
   ENDIF
  ENDIF
  ISG2GRIDMAIN=ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,TOP,BOT,IBATCH,MP,0)
...
!      CALL IDFGETLOC(THIESSEN,TRAP(1,ITRAP)%IROW,TRAP(1,ITRAP)%ICOL,XC,YC)
 ...
 !###====================================================================
 LOGICAL FUNCTION ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,TOP,BOT,IBATCH,MP,JU)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NROW,NCOL,NLAY,IBATCH,JU
...
!          ENDDO
 ...
 ELSEIF(IEXPORT.EQ.1)THEN
  CALL ISG2GRID_EXPORTRIVER(JU,IDF,NLAY,TOP,BOT,MP)
 ENDIF
 ...
 !###====================================================================
 SUBROUTINE ISG2GRID_EXPORTRIVER(JU,IDF,NLAY,TOP,BOT,MP)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NLAY,JU
 INTEGER,INTENT(INOUT) :: MP
 ...
 DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
  IF(IDF(1)%X(ICOL,IROW).GT.0.0)THEN
   ...
   IF(NLAY.EQ.0)THEN
    ...
    WRITE(IU,FRM) 1,IROW,ICOL,IDF(2)%X(ICOL,IROW),IDF(1)%X(ICOL,IROW),IDF(3)%X(ICOL,IROW),IDF(4)%X(ICOL,IROW),1
