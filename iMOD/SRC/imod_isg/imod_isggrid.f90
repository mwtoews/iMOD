...
USE MOD_ISG_UTL, ONLY : UTL_GETUNITSISG,ISGREAD,ISGDEAL,ISGDELISD,ISGSTUWEN_INTERSECT,ISGGETPOSID,ISGMEMORYISC,ISGMEMORYDATISC,ISGDELISC,ISGGETXY, &
             ISGATTRIBUTESREADISCVALUE,ISGATTRIBUTES_2DCROSS_READ
...
!  NCLC=10; DO K=1,NCLC; XDIST(K)=REAL(K); CALL RANDOM_NUMBER(ZDIST(K)); ENDDO
!  ZDIST(2)=NODATA
...
! IDF(9)%X=0.0
 ...
 DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
  DO I=1,4
   IF(IDF(9)%X(ICOL,IROW).NE.0.0)IDF(I)%X(ICOL,IROW)=IDF(I)%X(ICOL,IROW)/IDF(9)%X(ICOL,IROW)
 ...
 DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
  IF(IDF(9)%X(ICOL,IROW).GT.1.0)IDF(9)%X(ICOL,IROW)=1.0
