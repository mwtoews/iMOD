...
USE MOD_UTL, ONLY : ITOS,RTOS,UTL_IDATETOJDATE,UTL_JDATETOIDATE,JDATETOGDATE,UTL_GETUNIT,UTL_WAITMESSAGE,UTL_IDFSNAPTOGRID,UTL_GETMED, &
               PEUCKER_SIMPLIFYLINE,UTL_DIST,UTL_GETCURRENTDATE,UTL_IDFSNAPTONICEGRID,UTL_PCK_GETTLP
...
USE MOD_ISG_UTL, ONLY : UTL_GETUNITSISG,ISGREAD,ISGDEAL,ISGDELISD,ISGSTUWEN_INTERSECT,ISGGETPOSID,ISGMEMORYISC,ISGMEMORYDATISC,ISGDELISC,ISGGETXY, &
             ISGATTRIBUTESREADISCVALUE,ISGATTRIBUTES_2DCROSS_READ,ISGMEMORYDATISD,ISGSAVEHEADERS,ISGOPENFILES,ISG_UTL_LEAKAGE
...
CONTAINS
 ...
 DO ISELISG=1,NISG
  ...
  DO J=1,NCLC
   ...
   IF(STAGETYPE.EQ.1)THEN; DO I=1,K; IF(FTXT%ITYPE(I).EQ.0)FTXT%STAGE(I)=FTXT%STAGE(I)-BTML; ENDDO; ENDIF
   ...
   DO I=1,K
    ...
    IF(STAGETYPE.EQ.0)DATISD(IREF)%WLVL =FTXT%STAGE(ISORT(I))
    IF(STAGETYPE.EQ.1)DATISD(IREF)%WLVL =BTML+FTXT%STAGE(ISORT(I))
...
!    enddo
 ...
 !###====================================================================
 LOGICAL FUNCTION ISG2GRIDMAIN(ISGFILE,IBATCH,NLAY,TOP,BOT,KHV,BND,GRIDISG)
 !###====================================================================
 IMPLICIT NONE
 ...
 DO
  ...
  IF(GRIDISG%ISTEADY.EQ.2.AND.GRIDISG%EDATE.LT.GRIDISG%SDATE)THEN
   ...
   ENDIF
  ENDIF
  ISG2GRIDMAIN=ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,0,TOP,BOT,KHV,BND,VCW,IBATCH,MP,0,GRIDISG,SFT,.FALSE.) 
 ...
 !###====================================================================
 LOGICAL FUNCTION ISG2GRID(PPOSTFIX,NROW,NCOL,NLAY,ILAY,TOP,BOT,KHV,BND,VCW,IBATCH, &
                           MP,JU,GRIDISG,SFT,LSFT) 
 !###====================================================================
 IMPLICIT NONE
 ...
 REAL :: X1,X2,Y1,Y2,CT,BH,BW,NETWD,COND,H1,H2,Z,WL,TD,TC,XC,YC
 LOGICAL :: LEX,LNODAT
...
!  IF(IROW.EQ.47.AND.ICOL.EQ.51)THEN
!   WRITE(*,*)
!  ENDIF
...
! IF(.NOT.IDFWRITE(IDF(9),TRIM(GRIDISG%ROOT)//'\'//TRIM(FNAME(9))//TRIM(PPOSTFIX)//'.IDF',1))THEN; endif
 ...
 ELSEIF(GRIDISG%IEXPORT.EQ.1)THEN
  CALL ISG2GRID_EXPORTRIVER(JU,IDF,NLAY,ILAY,TOP,BOT,KHV,BND,VCW,MP,GRIDISG,SFT,LSFT)
 ENDIF
...
! TYPE(IDFOBJ),DIMENSION(NLAY),INTENT(INOUT) :: TOP,BOT
 ...
 NSTREAM=0; DO I=1,NISG
  ...
  ELSEIF(ICALC.EQ.4)THEN
   ...
   J=ISQ(IQHR)%IREF-1 ; DO K=1,NSTRPTS
    J=J+1; QCRS(K)=DATISQ(J)%Q*86400.0; WCRS(K)=DATISQ(J)%W; DCRS(K)=DATISQ(J)%D
   ENDDO
 ...
 !###====================================================================
 SUBROUTINE ISG2GRID_EXPORTRIVER(JU,IDF,NLAY,ILAY,TOP,BOT,KHV,BND,CV,MP,GRIDISG,SFT,LSFT)
 !###====================================================================
 IMPLICIT NONE
 TYPE(GRIDISGOBJ),INTENT(INOUT) :: GRIDISG
 INTEGER,INTENT(IN) :: NLAY,ILAY,JU
 INTEGER,INTENT(INOUT),DIMENSION(:) :: MP
 TYPE(IDFOBJ),DIMENSION(:),INTENT(INOUT) :: IDF,SFT,CV
 TYPE(IDFOBJ),DIMENSION(NLAY),INTENT(INOUT) :: TOP,BOT,KHV,BND
 LOGICAL,INTENT(IN) :: LSFT
 INTEGER :: IROW,ICOL,N,IU,I,J
 REAL :: T,WL,BL,CD,F,TL,WP,LC,C1,C0,DXY,D
 ...
 DO J=1,2
  ...
  DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
   ...
   IF(IDF(1)%X(ICOL,IROW).LE.0.0)CYCLE
   ...
   IF(ILAY.EQ.0)THEN
    DO I=1,NLAY
     ...
     IF(BND(I)%X(ICOL,IROW).GT.0.AND.T.GT.0.0)THEN
      IF(KH(I)*T.LT.PBMAN%MINKD)KH(I)=PBMAN%MINKD/T
     ELSE
      KH(I)=0.0
     ENDIF
    ENDDO
    CALL UTL_PCK_GETTLP(NLAY,TLP,KH,TP,BT,WL,BL,MINKHT,ICLAY) 
   ELSE
    TLP(ILAY)=1.0
   ENDIF
...
!   IF(ILAY.NE.0)THEN
!    N=N+1
!    IF(J.EQ.2)WRITE(IU,FRM) ILAY,IROW,ICOL,IDF(2)%X(ICOL,IROW),IDF(1)%X(ICOL,IROW),IDF(3)%X(ICOL,IROW),IDF(4)%X(ICOL,IROW),1
!   ELSE

   DO I=1,NLAY
    IF(TLP(I).EQ.0.0)CYCLE
    N=N+1
    IF(J.EQ.2)THEN
     LC=1.0
     IF(PBMAN%IFVDL.EQ.1)THEN
      DXY=IDFGETAREA(IDF(1),ICOL,IROW)
      C1=0.0; IF(CV(ILAY)%X(ICOL,IROW).GT.0.0)C1 =DXY/CV(ILAY)%X(ICOL,IROW)
      !## if sft package is active use that parameter
      IF(LSFT)THEN
       D =SFT(1)%X(ICOL,IROW)
       KH(I)=SFT(2)%X(ICOL,IROW)
      ELSE
       D =TOP(ILAY)%X(ICOL,IROW)-BOT(ILAY)%X(ICOL,IROW)
      ENDIF
      LC=ISG_UTL_LEAKAGE(CD,DXY,F,TL,WP,C0,C1,D,KH(I))
     ENDIF
     WRITE(IU,FRM) I,IROW,ICOL,WL,LC*CD*TLP(I),BL,F,1
    ENDIF
   ENDDO
 ...
 ELSE
  ...
  DO IROW=IR1,IR2; DO ICOL=IC1,IC2
   ...
   IF(JROW.NE.0.AND.JCOL.NE.0)THEN
    ...
    !## manipulate resistance for ...
    CR=C; IF(PCROSS%X(ICOL,IROW).NE.0.0)CR=CR*ABS(PCROSS%X(JCOL,JROW))
...
!  IDF(1)%X(ICOL,IROW)=IDF(1)%X(ICOL,IROW)*IDF(9)%X(ICOL,IROW)
 ...
 CALL UTL_IDFSNAPTONICEGRID(XMIN,XMAX,YMIN,YMAX,CS,NCOL,NROW)
