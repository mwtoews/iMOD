...
USE MOD_UTL, ONLY : ITOS,RTOS,IDATETOJDATE,JDATETOIDATE,JDATETOGDATE,UTL_GETUNIT,UTL_WAITMESSAGE,UTL_IDFSNAPTOGRID,UTL_GETMED, &
               UTL_CREATEDIR,PEUCKER_SIMPLIFYLINE
...
!      CALL ISGMEMORYISC(1,ISELISG,IPOS)
 ...
 INTEGER :: JCRS,MAXNSEG,NSVATS,IRAT,IRAT1
 ...
 INTEGER :: ISEG,JSEG,NSEG,IREF,MAXDIM,NDIM,NSYM,NTRAP,ITRAP,JQHR,NITEMS,LU,IOS,ICRS,NCRS !,ITIC,ITOC,NTOC
 REAL :: X1,X2,Y1,Y2,D,FCT,CT,BH,BW,NETWD,COND,H1,H2,Z,WL,TD
 LOGICAL :: LEX,LNODAT
       ...
       IF(LN(J).GT.0.0)THEN
        ...
        IF(ICOL.GE.1.AND.IROW.GE.1.AND. &
         ...
         IF(ISIMGRO.GT.0)THEN
          ...
          ENDDO
          NETTRAP=ITRAP-1; WRITE(ISIMGRO,'(I10)') NETTRAP
 ...
 IF(ISIMGRO.GT.0)THEN
  DEALLOCATE(XIN,YIN,XSYM,YSYM,XTRAP,YTRAP)
  EDATE=NSVATS
 ENDIF
