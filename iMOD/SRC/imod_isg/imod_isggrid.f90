...
!    enddo
 ...
 INTEGER :: I,J,K,II,JJ,TTIME,IROW,ICOL,NETTRAP,ITYPE,N,ISTW,IR,IC,MDIM,I1,I2,IZONE
 INTEGER :: JCRS,MAXNSEG,IRAT,IRAT1
 INTEGER,DIMENSION(4) :: IUDMM
 ...
 INTEGER :: ISEG,JSEG,NSEG,IREF,MAXDIM,NDIM,NSYM,NTRAP,ITRAP,JQHR,NITEMS,LU,IOS,ICRS,NCRS,IUSIMGRO,NDMM,INODE
 ...
 IF(PBMAN%DMMFILE.EQ.1)THEN
  !## make sure dmm-files are in riv7 folders
  TMPFNAME=GRIDISG%ROOT
  DO I=1,2
   TMPFNAME=TMPFNAME(:INDEX(TMPFNAME,'\',.TRUE.)-1)
  ENDDO
  IUDMM(1)=UTL_GETUNIT()
  CALL OSD_OPEN(IUDMM(1),FILE=TRIM(TMPFNAME)//'\MFRIVTODFM1D_Q.DMM',STATUS='UNKNOWN', &
                FORM='FORMATTED',ACTION='WRITE,DENYREAD',ACCESS='SEQUENTIAL')
  WRITE(IUDMM(1),'(3A15)') 'FM-X','FM-Y','RIV'
  IUDMM(2)=UTL_GETUNIT(); NDMM=0
  CALL OSD_OPEN(IUDMM(2),FILE=TRIM(TMPFNAME)//'\DFM1DWATLEVTOMFRIV_H.DMM',STATUS='UNKNOWN', &
                FORM='FORMATTED',ACTION='WRITE,DENYREAD',ACCESS='SEQUENTIAL')
  WRITE(IUDMM(2),'(4A15)') 'RIV','FM-X','FM-Y','WEIGHT'
  IUDMM(3)=UTL_GETUNIT(); NDMM=0
  CALL OSD_OPEN(IUDMM(3),FILE=TRIM(TMPFNAME)//'\DFLOWFM_POINTS.DAT',STATUS='UNKNOWN', &
                FORM='FORMATTED',ACTION='WRITE,DENYREAD',ACCESS='SEQUENTIAL')
  WRITE(IUDMM(3),'(5A15)') 'ISEG','INODE','IZONE','FM-X','FM-Y'
  !## read afwatidf 
  ...
  IF(JU.EQ.0)THEN
   IUDMM(4)=UTL_GETUNIT()
   IF(GRIDISG%DDATE.EQ.0)THEN
    CALL OSD_OPEN(IUDMM(4),FILE=TRIM(GRIDISG%ROOT)//'\modflow.riv_',STATUS='UNKNOWN',ACTION='WRITE')
   ELSE
    CALL OSD_OPEN(IUDMM(4),FILE=TRIM(GRIDISG%ROOT)//'\modflow_'//TRIM(JDATETOGDATE(GRIDISG%SDATE,2))//'.riv_', &
                  STATUS='UNKNOWN',ACTION='WRITE')
   ENDIF
   WRITE(IUDMM(4),'(A)') 'NaN1#'
  ELSE
   IUDMM(4)=JU
  ENDIF
 ...
 ISGLOOP: DO I=1,NISG
  ...
  !## write nodes for coupling with dflowfmm
  IF(PBMAN%DMMFILE.EQ.1)THEN
   IZONE=0; DO INODE=1,ISG(I)%NCLC
    ISEG=ISG(I)%ISEG
    NSEG=ISG(I)%NSEG
    D=ISD(ISG(I)%ICLC+INODE-1)%DIST
    CALL ISGGETXY(ISP(ISEG:)%X,ISP(ISEG:)%Y,NSEG,D,XCRD,YCRD)
    !## get zone  number for dfflow-fm node
    IF(PBMAN%AFWATIDF%FNAME.NE.'')THEN
     CALL IDFIROWICOL(PBMAN%AFWATIDF,IROW,ICOL,XCRD,YCRD)
     IZONE=PBMAN%AFWATIDF%X(ICOL,IROW)
    ENDIF
    WRITE(IUDMM(3),'(3I15,2F15.3)') I,INODE,IZONE,XCRD,YCRD
   ENDDO
   CALL IDFDEALLOCATEX(PBMAN%AFWATIDF)
  ENDIF
   ...
   IF(LNODAT)THEN
    ...
    DO ISEG=2,NSEG
     ...
     IF(DXY.GT.0.0D0)THEN
      ...
      IF(ISEG.GT.2)THEN
       IF(IPSPOS(1,ISEG-1).NE.IPSPOS(1,ISEG-2))CDXY=0.0D0 
      ENDIF
         ...
         IF(C.GT.0.0D0)THEN
          ...
          IF(PBMAN%DMMFILE.EQ.1)THEN
           ...
           IF(ABS(CUMDIST(I1)-CUMDIST(ISEG)).LE.ABS(CUMDIST(I2)-CUMDIST(ISEG)))THEN
            WRITE(IUDMM(1),'(2F15.3,I15)') X(I1),Y(I1),MP(1)
           ELSE
            WRITE(IUDMM(1),'(2F15.3,I15)') X(I2),Y(I2),MP(1)
           ENDIF
           ...
           !## ignore small parts
           IF(W.GT.0.99D0)THEN
            WRITE(IUDMM(2),'(I15,3F15.3)') MP(1),X(I2),Y(I2),1.0D0  ; NDMM=NDMM+1
           ELSEIF(W.LT.0.01D0)THEN
            WRITE(IUDMM(2),'(I15,3F15.3)') MP(1),X(I1),Y(I1),1.0D0  ; NDMM=NDMM+1
           ELSE
            WRITE(IUDMM(2),'(I15,3F15.3)') MP(1),X(I1),Y(I1),1.0D0-W; NDMM=NDMM+1
            WRITE(IUDMM(2),'(I15,3F15.3)') MP(1),X(I2),Y(I2),W      ; NDMM=NDMM+1
           ENDIF           

           !## write conventional river package
           WRITE(IUDMM(4),'(3I10,4F10.3,I10)') 1,IROW,ICOL,ISGVALUE(1,2),ISGVALUE(1,1),ISGVALUE(1,3),ISGVALUE(1,4),ISYS
...
! INTEGER,PARAMETER :: ICLAY=1 !## shift to nearest aquifer
 ...
 !## not for sfr
 IF(PBMAN%IFORMAT.EQ.2)CALL IDFWRITEFREE_HEADER(IU,BND(1))
 ...
 DO IROW=1,ICROSS%NROW; DO ICOL=1,ICROSS%NCOL
  ...
  CR=C
  !## skip location with c=0.0
  IF(CR.EQ.0.0D0)CYCLE

  IF(PCROSS%X(ICOL,IROW).NE.0.0D0)CR=CR*ABS(PCROSS%X(ICOL,IROW))
