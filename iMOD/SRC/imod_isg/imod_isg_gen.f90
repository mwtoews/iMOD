...
INTEGER :: SAMPLE,NPROF,ISTART,ISTOP,IBOT,ICDY,IINF,ISUMMER_BACKUP,IWINTER_BACKUP,NP,NBRCH,ICORDIR
...
END SUBROUTINE ISGGEN_CREATEISG
 ...
 DO I=IP(IB-1),IP(IB)-1
  ...
  IF(I.NE.IP(IB-1))THEN
   IF(PNTX(I).EQ.PNTX(I-1).AND.PNTY(I).EQ.PNTY(I-1))LEX=.FALSE.
  ENDIF
...
IF(IMODE.EQ.2)THEN
 ...
 LCRDFLIP=.FALSE.; IF(ICORDIR.EQ.1.AND.MXWL_F.LT.MXWL_T)LCRDFLIP=.TRUE.
