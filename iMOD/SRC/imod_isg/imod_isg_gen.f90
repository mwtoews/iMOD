...
CONTAINS
 ...
 LDAT=.FALSE.; ALLOCATE(PROF(4,IPF(1)%NROW))
 !## fill in cross-sections - not really neccesarry - maybe average cross-section dimensions
 DO I=1,IPF(1)%NROW
  READ(IPF(1)%INFO(DATCOL(5),I),*) XW
!  READ(IPF(1)%INFO(DATCOL(6),I),*) XD
  PROF(1,I)%DISTANCE=-0.5*XW
  PROF(1,I)%BOTTOM  = 10.0 
  PROF(2,I)%DISTANCE=-0.5*XW
  PROF(2,I)%BOTTOM  = 10.0 
  PROF(3,I)%DISTANCE= 0.5*XW
  PROF(3,I)%BOTTOM  = 10.0 
  PROF(4,I)%DISTANCE= 0.5*XW
  PROF(4,I)%BOTTOM  = XD
 END DO

 !## allocate pntx/pnty/ip/ibid
 DO I=1,2
  NBRCH=1; NP=1; CS1=IPF(1)%INFO(DATCOL(4),1); CS2=UTL_CAP(CS1,'U')
  IF(I.EQ.2)THEN
   PNTX(NP)=IPF(1)%XYZ(1,1); PNTY(NP)=IPF(1)%XYZ(2,1); IP(NBRCH-1)=1
  ENDIF
  DO J=2,IPF(1)%NROW
   CS1=IPF(1)%INFO(DATCOL(4),J)
   NP=NP+1
   IF(I.EQ.2)THEN
    PNTX(NP)=IPF(1)%XYZ(1,J); PNTY(NP)=IPF(1)%XYZ(2,J); IP(NBRCH)=NP+1
   ENDIF
   !## duplicate coordinates
   IF(TRIM(UTL_CAP(CS1,'U')).EQ.CS2)THEN
    NP=NP+1 !; NBRCH=NBRCH+1
    IF(I.EQ.2)THEN
     PNTX(NP)=IPF(1)%XYZ(1,J); PNTY(NP)=IPF(1)%XYZ(2,J) !; IP(NBRCH)=NP
    ENDIF
   ENDIF
   NBRCH=NBRCH+1
   CS2=UTL_CAP(CS1,'U')
  ENDDO
  IF(I.EQ.1)THEN
   ALLOCATE(PNTX(NP),PNTY(NP),IP(0:NBRCH),IBID(NBRCH)) 
  ENDIF
 ENDDO
!  READ(IPF(1)%INFO(DATCOL(6),I),*) XD
...
DO IB=1,NBRCH
 ...
 IF(ISFR.EQ.0)CALL ISGGEN_CREATEISD1(ICLC,NCLC,IREFSD,DIST,IB)
 IF(ISFR.EQ.1)CALL ISGGEN_CREATEISD1_SFR(ICLC,NCLC,IREFSD,DIST,IB)
 CALL ISGGEN_CREATEISC1(ICRS,NCRS,IREFSC,DIST,IB)
...
INTEGER :: IDATE,ITIME,UPSG,DWNS,ICALC,IPRI,N
!CHARACTER(LEN=8) :: CDATE
!DOUBLE PRECISION :: XC,YC
CHARACTER(LEN=32) :: CNAME
CHARACTER(LEN=52) :: CID
 ...
 WRITE(CID,*) IBID(IB); CID=ADJUSTL(CID)
...
! READ(VAR(DATCOL(3),IL),*,IOSTAT=IOS) WB
 ...
 WRITE(IU(ISD2),REC=IREFSD+ICF) IDATE,ITIME,WLVL,BTML,WD,THCK,HCND,UPSG,DWNS,ICALC,IPRI,QFLW,QROF
...
!##=====================================================================
SUBROUTINE ISGGEN_CREATEISC1(ICRS,NCRS,IREFSC,DIST,IB)
!##=====================================================================
IMPLICIT NONE
...
ELSE
 ...
 DO I=1,NPROF
  IREFSC=IREFSC+1
  WRITE(IU(ISC2),REC=IREFSC+ICF) PROF(I,1)%DISTANCE,PROF(I,1)%BOTTOM,25.0
 END DO
