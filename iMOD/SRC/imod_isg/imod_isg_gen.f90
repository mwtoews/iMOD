...
CONTAINS
 ...
 IF(INDEX(IPFFILE,'*').GT.0)THEN
  ...
  DO I=1,NIPF
   ...
   IPF(I)%XCOL =DATCOL(1)  !## x
   IPF(I)%YCOL =DATCOL(2)  !## y
   IPF(I)%ZCOL =DATCOL(1)  !## z1 - not used
   IPF(I)%Z2COL=DATCOL(1)  !## z2 - not used
   IPF(I)%QCOL =DATCOL(1)  !## q  - not used
   !## read entire ipf
  ...
  DO IIPF=1,SIZE(IPF)
   DO J=1,IPF(IIPF)%NROW-1
    ...
    IF(TRIM(CS1).EQ.TRIM(CS2))THEN
     ...
     IF(UTL_DIST(IPF(IIPF)%XYZ(1,J),IPF(IIPF)%XYZ(2,J),IPF(IIPF)%XYZ(1,J+1),IPF(IIPF)%XYZ(2,J+1)).GT.0.0)THEN
      IF(I.EQ.2)THEN
       PNTX(NP+1)=IPF(IIPF)%XYZ(1,J);   PNTY(NP+1)=IPF(IIPF)%XYZ(2,J) 
       PNTX(NP+2)=IPF(IIPF)%XYZ(1,J+1); PNTY(NP+2)=IPF(IIPF)%XYZ(2,J+1)
     
       IP(NBRCH+1)=NP+3
       CBID(NBRCH+1)=TRIM(IPF(IIPF)%INFO(DATCOL(3),I))//'_REACH'//TRIM(ITOS(NBRCH+1))
       IDNSEG(NBRCH+1)=NBRCH+2
     
       !## left side
       READ(IPF(IIPF)%INFO(DATCOL(5),J),*)   XW1
       READ(IPF(IIPF)%INFO(DATCOL(6),J),*)   XD1
       !## right side
       READ(IPF(IIPF)%INFO(DATCOL(5),J+1),*) XW2
       READ(IPF(IIPF)%INFO(DATCOL(6),J+1),*) XD2
       !## average width
       XW=(XW1+XW2)/2.0/2.0
