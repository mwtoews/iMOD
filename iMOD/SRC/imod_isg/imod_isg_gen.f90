...
CONTAINS
 ...
 REAL :: XW1,XD1,XW2,XD2,XW
 INTEGER :: I,J
 
 !## read ipf
 NIPF=1; CALL IPFALLOCATE()
 IPF(1)%FNAME=IPFFILE
 IPF(1)%XCOL =DATCOL(1)  !## x
 IPF(1)%YCOL =DATCOL(2)  !## y
 IPF(1)%ZCOL =DATCOL(1)  !## z1 - not used
 IPF(1)%Z2COL=DATCOL(1)  !## z2 - not used
 IPF(1)%QCOL =DATCOL(1)  !## q  - not used
 !## read entire ipf
 ...
 DO I=1,2
  ...
  DO J=1,IPF(1)%NROW-1
   CS1=UTL_CAP(IPF(1)%INFO(DATCOL(4),J  ),'U')
   CS2=UTL_CAP(IPF(1)%INFO(DATCOL(4),J+1),'U')
   !## duplicate coordinates, start new branch
   IF(TRIM(CS1).EQ.TRIM(CS2))THEN
    !## skip distance is zero
    IF(UTL_DIST(IPF(1)%XYZ(1,J),IPF(1)%XYZ(2,J),IPF(1)%XYZ(1,J+1),IPF(1)%XYZ(2,J+1)).GT.0.0)THEN
     IF(I.EQ.2)THEN
      PNTX(NP+1)=IPF(1)%XYZ(1,J);   PNTY(NP+1)=IPF(1)%XYZ(2,J) 
      PNTX(NP+2)=IPF(1)%XYZ(1,J+1); PNTY(NP+2)=IPF(1)%XYZ(2,J+1)
     
      IP(NBRCH+1)=NP+3
      CBID(NBRCH+1)=TRIM(IPF(1)%INFO(DATCOL(3),I))//'_REACH'//TRIM(ITOS(NBRCH+1))
      IDNSEG(NBRCH+1)=NBRCH+2
     
      !## left side
      READ(IPF(1)%INFO(DATCOL(5),J),*)   XW1
      READ(IPF(1)%INFO(DATCOL(6),J),*)   XD1
      !## right side
      READ(IPF(1)%INFO(DATCOL(5),J+1),*) XW2
      READ(IPF(1)%INFO(DATCOL(6),J+1),*) XD2
      !## average width
      XW=(XW1+XW2)/2.0/2.0
      
      PROF(1,NBRCH+1)%DISTANCE=-XW1
      PROF(1,NBRCH+1)%BOTTOM  = XD1
      PROF(2,NBRCH+1)%DISTANCE=-2.0*XW
      PROF(2,NBRCH+1)%BOTTOM  = 10.0
      PROF(3,NBRCH+1)%DISTANCE=-XW
      PROF(3,NBRCH+1)%BOTTOM  = 10.0
      PROF(4,NBRCH+1)%DISTANCE=-XW
      PROF(4,NBRCH+1)%BOTTOM  = 0.0
      PROF(5,NBRCH+1)%DISTANCE= XW
      PROF(5,NBRCH+1)%BOTTOM  = 0.0
      PROF(6,NBRCH+1)%DISTANCE= XW
      PROF(6,NBRCH+1)%BOTTOM  = 10.0
      PROF(7,NBRCH+1)%DISTANCE= 2.0*XW
      PROF(7,NBRCH+1)%BOTTOM  = 10.0
      PROF(8,NBRCH+1)%DISTANCE= XW2
      PROF(8,NBRCH+1)%BOTTOM  = XD2
     ENDIF
     NP=NP+2; NBRCH=NBRCH+1
    ENDIF
   ENDIF

!1  350757.268933,140214.242603,2,1,2.0,1.763
!2  350796.08653,140242.719053,2,1,2.0,1.643
!3  350796.08653,140242.719053,2,1,2.0,1.643
!4  351219.223766,140545.476983,2,1,2.5,3.55
!5  351219.223766,140545.476983,2,1,2.5,3.55
!6  351251.810107,140553.328728,2,1,10.2,4.0
!7  351286.372092,140594.306477,2,2,3.0,4.535
!8  352230.415068,141272.166989,2,2,2.0,3.35
!9  352230.415068,141272.166989,2,2,2.0,3.35
!10 352451.527228,141430.142205,2,2,0.7,2.125
!11 352451.527228,141430.142205,2,2,0.7,2.125
!12 352469.471382,141442.205243,2,2,1.5,2.28

  ENDDO
  IF(I.EQ.1)THEN
   ALLOCATE(PNTX(NP),PNTY(NP),IP(0:NBRCH),CBID(NBRCH),IDNSEG(NBRCH)); ALLOCATE(PROF(8,NBRCH))
  ENDIF
 ENDDO
...
DO IB=1,NBRCH
 ...
 LINE='"SEGMENT'//TRIM(CBID(IB))//'",'//TRIM(ITOS(ISEG-NSEG+1))//','//TRIM(ITOS(NSEG))//','// &
...
END SUBROUTINE ISGGEN_CREATEISD1
 ...
 BTML=PROF(1,IB)%BOTTOM
 WLVL=BTML+1.0
 WD=-1.0*PROF(1,IB)%DISTANCE
 THCK=0.10
 HCND=1.00
 ...
 BTML=PROF(8,IB)%BOTTOM
 WLVL=BTML+1.0
 WD=PROF(8,IB)%DISTANCE
 THCK=0.10
 HCND=1.00
...
ELSE
 ...
 IF(ISFR.EQ.1)THEN
  PROF(1,IPROF)%DISTANCE=PROF(2,IPROF)%DISTANCE
  PROF(1,IPROF)%BOTTOM=10.5
  PROF(8,IPROF)%BOTTOM=10.5
  PROF(8,IPROF)%DISTANCE=PROF(7,IPROF)%DISTANCE
 ENDIF
