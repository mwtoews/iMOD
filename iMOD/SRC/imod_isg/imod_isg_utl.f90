  ...
  DO I=1,ISG(IS)%NCRS
   ...
   IF(DATISC(JREC)%DISTANCE.LT.0.0.AND.DATISC(JREC)%BOTTOM.LT.0.0)THEN
    DO J=2,ABS(ISC(IREC)%N)
     JREC=JREC+1
     ZM=INT(DATISC(JREC)%KM,2)
     ZC=INT(100.0*(DATISC(JREC)%KM-REAL(ZM)),1)
