...
!
 ...
 DO ISGFILES=1,NISGFILES
  DO J=1,NISGF(ISGFILES)
   ...
   I=ISG(IS)%ICLC; CALL ISGSORTSEGMENTENTRIES(ISD(I:)%DIST,ISD(I:)%IREF,ISG(IS)%NCLC)
   I=ISG(IS)%ISTW; CALL ISGSORTSEGMENTENTRIES(IST(I:)%DIST,IST(I:)%IREF,ISG(IS)%NSTW)
   I=ISG(IS)%ICRS; CALL ISGSORTSEGMENTENTRIES(ISC(I:)%DIST,ISC(I:)%IREF,ISG(IS)%NCRS)
   I=ISG(IS)%IQHR; CALL ISGSORTSEGMENTENTRIES(ISQ(I:)%DIST,ISQ(I:)%IREF,ISG(IS)%NQHR)
 ...
 !###====================================================================
 SUBROUTINE ISGSORTSEGMENTENTRIES(DI,IREF,N)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(N) :: DI
 INTEGER,INTENT(INOUT),DIMENSION(N) :: IREF
 INTEGER,ALLOCATABLE,DIMENSION(:) :: ISORT,JREF
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: D
 INTEGER :: I
 
 ALLOCATE(D(N),ISORT(N),JREF(N)); D=DI; JREF=IREF; DO I=1,N; ISORT(I)=I; ENDDO

 !## sort based on distance
 CALL WSORT(D,1,N,IORDER=ISORT)

 DO I=1,N; DI(I)=D(I); IREF(ISORT(I))=JREF(I); ENDDO
 
 DEALLOCATE(D,ISORT,JREF)
