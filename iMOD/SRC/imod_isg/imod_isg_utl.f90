...
!
 ...
 DO ISGFILES=1,NISGFILES
  DO J=1,NISGF(ISGFILES)
   ...
   I=ISG(IS)%ICLC; CALL ISGSORTSEGMENTENTRIES(ISD(I:)%DIST,ISD(I:)%IREF,ISD(I:)%N,ISG(IS)%NCLC)
   I=ISG(IS)%ISTW; CALL ISGSORTSEGMENTENTRIES(IST(I:)%DIST,IST(I:)%IREF,IST(I:)%N,ISG(IS)%NSTW)
   I=ISG(IS)%ICRS; CALL ISGSORTSEGMENTENTRIES(ISC(I:)%DIST,ISC(I:)%IREF,ISC(I:)%N,ISG(IS)%NCRS)
   I=ISG(IS)%IQHR; CALL ISGSORTSEGMENTENTRIES(ISQ(I:)%DIST,ISQ(I:)%IREF,ISQ(I:)%N,ISG(IS)%NQHR)
 ...
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(NP) :: DI
 INTEGER,INTENT(INOUT),DIMENSION(NP) :: IREF,N
 ...
 CALL WSORT(D,1,NP,IORDER=ISORT)

 DO I=1,NP; DI(I)=D(I); IREF(ISORT(I))=JREF(I); N(ISORT(I))=NN(I); ENDDO
