...
! CALL WCURSORSHAPE(CURARROW)
 ...
 DO IR=1,NR
  ...
  ELSEIF(ITYPE.EQ.-1)THEN
   ...
   READ(DATIST(IREC)%CTIME,'(3(I2,1X))') IH,IM,IS
 ...
 IDF%XMIN=MINVAL(DATISC2(J,2:TISC(J))%DISTANCE)-0.5D0*IDF%DX 
 IDF%XMAX=MAXVAL(DATISC2(J,2:TISC(J))%DISTANCE)+0.5D0*IDF%DX 
 IDF%YMIN=MINVAL(DATISC2(J,2:TISC(J))%BOTTOM)  -0.5D0*IDF%DY
 IDF%YMAX=MAXVAL(DATISC2(J,2:TISC(J))%BOTTOM)  +0.5D0*IDF%DY
 ...
 DO I=1,NISG
  ...
  DO J=1,ISG(I)%NSEG-1
   ...
   IF(ISTATUS.EQ.3.OR.ISTATUS.EQ.5)THEN
    !## compute distance
    D=SQRT((X-XC)**2.0D0+(Y-YC)**2.0D0)
    !## first time to put results, or replace it whenever new point is closer
    IF(D.LT.MD)THEN
     IF(ISMALL.EQ.1)MD=D
     DIST=TD+SQRT((ISP(K)%X-X)**2.0D0+(ISP(K)%Y-Y)**2.0D0)
     ISGX=X; ISGY=Y 
   ...
   ELSE
    ...
    D=SQRT((XC-ISP(K)%X)**2.0D0+(YC-ISP(K)%Y)**2.0D0)
    IF(D.LT.MD)THEN
     IF(ISMALL.EQ.1)MD=D
     DIST=0.0D0; IF(J.GT.1)DIST=TD+SQRT(DX**2.0D0+DY**2.0D0)
    ...
    IF(J.EQ.ISG(I)%NSEG-1)THEN
     D=SQRT((XC-ISP(K+1)%X)**2.0D0+(YC-ISP(K+1)%Y)**2.0D0)
     IF(D.LT.MD)THEN
      IF(ISMALL.EQ.1)MD=D
      DIST=TD+SQRT(DX**2.0D0+DY**2.0D0)
 ...
 TSTATUS=TSTAT; TSTATUS=UTL_CAP(TSTATUS,'U')
...
!
 ...
 INTEGER :: IS,I,J,ISGFILES,IREC,NISGFILES,NV,N
 ...
 DO ISGFILES=1,NISGFILES
  DO J=1,NISGF(ISGFILES)
   ...
   !## sort all for distances
   I=ISG(IS)%ICLC; N=ISG(IS)%NCLC; CALL ISGSORTSEGMENTENTRIES(ISD(I:)%DIST,ISD(I:)%IREF,ISD(I:)%N,N)
   I=ISG(IS)%ISTW; N=ISG(IS)%NSTW; CALL ISGSORTSEGMENTENTRIES(IST(I:)%DIST,IST(I:)%IREF,IST(I:)%N,N)
   I=ISG(IS)%ICRS; N=ISG(IS)%NCRS; CALL ISGSORTSEGMENTENTRIES(ISC(I:)%DIST,ISC(I:)%IREF,ISC(I:)%N,N)
   I=ISG(IS)%IQHR; N=ISG(IS)%NQHR; CALL ISGSORTSEGMENTENTRIES(ISQ(I:)%DIST,ISQ(I:)%IREF,ISQ(I:)%N,N)
 ...
 DO IREC=ISG(IS)%ISEG,ISG(IS)%ISEG+ISG(IS)%NSEG-1
  ...
  IF(ISGDOUBLE.EQ.4)THEN
   ...
   ISP(JREC)%X=DBLE(X_SP(1))
   ISP(JREC)%Y=DBLE(X_SP(2))
  ELSE
  ...
  DO J=1,ISD(JREC)%N
   ...
   IF(ISFR.EQ.0)THEN
    IF(ISGDOUBLE.EQ.4)THEN
     ...
     DATISD(KREC)%WLVL= DBLE(X_SP(1))
     DATISD(KREC)%BTML= DBLE(X_SP(2))
     DATISD(KREC)%RESIS=DBLE(X_SP(3))
     DATISD(KREC)%INFF= DBLE(X_SP(4))
     DATISD(KREC)%CTIME='00:00:00'
    ELSE
     READ(ISGIU(4,ISGFILES),REC=IREC+ICF) DATISD(KREC)%IDATE,DATISD(KREC)%CTIME,DATISD(KREC)%WLVL,DATISD(KREC)%BTML, &
   ...
   ELSEIF(ISFR.EQ.1)THEN
    IF(ISGDOUBLE.EQ.4)THEN
     ...
     DATISD(KREC)%WLVL =DBLE(X_SP(1)) 
     DATISD(KREC)%BTML= DBLE(X_SP(2))
     DATISD(KREC)%WIDTH=DBLE(X_SP(3))
     DATISD(KREC)%THCK= DBLE(X_SP(4))
     DATISD(KREC)%HCND= DBLE(X_SP(5))
     DATISD(KREC)%QFLW= DBLE(X_SP(6))
     DATISD(KREC)%QROF= DBLE(X_SP(7))
     DATISD(KREC)%PPTSW=DBLE(X_SP(8))
     DATISD(KREC)%ETSW =DBLE(X_SP(9))                       
    ELSE
  ...
  DO J=1,ABS(ISC(JREC)%N)
   ...
   IF(J.EQ.1.OR.ICSTYPE.EQ.0)THEN
    IF(ISGDOUBLE.EQ.4)THEN
     ...
     DATISC(KREC)%DISTANCE=DBLE(X_SP(1))
     DATISC(KREC)%BOTTOM  =DBLE(X_SP(2))
     DATISC(KREC)%MRC     =DBLE(X_SP(3))
    ELSE
 ...
 DO I=1,ISG(IS)%NSTW
  ...
  DO J=1,IST(JREC)%N
   ...
   KREC =IREC+IOFF
   IF(ISGDOUBLE.EQ.4)THEN
    READ(ISGIU(8,ISGFILES),REC=IREC+ICF) DATIST(KREC)%IDATE,(X_SP(K),K=1,2)
    DATIST(KREC)%WLVL_UP  =DBLE(X_SP(1))
    DATIST(KREC)%WLVL_DOWN=DBLE(X_SP(2))
    DATIST(KREC)%CTIME='00:00:00'
   ELSE
    READ(ISGIU(8,ISGFILES),REC=IREC+ICF) DATIST(KREC)%IDATE,DATIST(KREC)%CTIME,DATIST(KREC)%WLVL_UP,DATIST(KREC)%WLVL_DOWN
   ENDIF
  END DO
 ...
 DO I=1,ISG(IS)%NQHR
  ...
  DO J=1,ISQ(JREC)%N
   ...
   IF(ISGDOUBLE.EQ.4)THEN
    ...
    DATISQ(KREC)%Q=DBLE(X_SP(1))
    DATISQ(KREC)%W=DBLE(X_SP(2))
    DATISQ(KREC)%D=DBLE(X_SP(3))
    DATISQ(KREC)%F=DBLE(X_SP(4))
   ELSE
 ...
 !###====================================================================
 SUBROUTINE ISGSORTSEGMENTENTRIES(DI,IREF,N,NP)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NP
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(:) :: DI
 INTEGER,INTENT(INOUT),DIMENSION(:) :: IREF,N
 INTEGER,ALLOCATABLE,DIMENSION(:) :: ISORT,JREF,NN
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: D
 INTEGER :: I
 
 ALLOCATE(D(NP),ISORT(NP),JREF(NP),NN(NP)); D=DI; JREF=IREF; NN=N; DO I=1,NP; ISORT(I)=I; ENDDO

 !## sort based on distance
 CALL UTL_WSORT(D,1,NP,IORDER=ISORT)

 DO I=1,NP; DI(I)=D(I); IREF(I)=JREF(ISORT(I)); N(I)=NN(ISORT(I)); ENDDO
 
 DEALLOCATE(D,ISORT,JREF,NN)
 
 END SUBROUTINE ISGSORTSEGMENTENTRIES
 ...
 DO I=2,NSEG
  IF(ISEG+1.EQ.IDIST)THEN
   ISEG=ISEG+1
   DIST=(XINTER-ISP(ISEG-1)%X)**2.0D0+(YINTER-ISP(ISEG-1)%Y)**2.0D0
   IF(DIST.GT.0.0D0)DIST=SQRT(DIST)
   TDIST=TDIST+DIST
   EXIT
  ENDIF
  ISEG=ISEG+1
  DIST=(ISP(ISEG)%X-ISP(ISEG-1)%X)**2.0D0+(ISP(ISEG)%Y-ISP(ISEG-1)%Y)**2.0D0
  IF(DIST.GT.0.0D0)DIST=SQRT(DIST)
  TDIST=TDIST+DIST
 END DO
 ...
 IF(ISFR.EQ.0)THEN
  IF(ISGDOUBLE.EQ.4)LEX=SIZE(ISDLABELS).EQ.SIZE(CTATTRIB1)
  IF(ISGDOUBLE.EQ.8)LEX=SIZE(ISDLABELS).EQ.SIZE(CTATTRIB3)
 ELSEIF(ISFR.EQ.1)THEN
  LEX=SIZE(ISDLABELS).EQ.SIZE(CTATTRIB2)
 ENDIF
 !## reset labels
 IF(.NOT.LEX)THEN
  DEALLOCATE(ISDLABELS)
  IF(ISFR.EQ.0)THEN
   IF(ISGDOUBLE.EQ.4)THEN
    ALLOCATE(ISDLABELS(SIZE(TATTRIB1))); DO I=1,SIZE(ISDLABELS); ISDLABELS(I)=TATTRIB1(I); ENDDO
   ELSEIF(ISGDOUBLE.EQ.8)THEN
 ...
 END SUBROUTINE ISG_CHECKLABELS

 !###===============================================================================
 SUBROUTINE ISGSAVEIT() !- saving ONLY *.ISG, *.isp, *.isd, *.isc
 !###===============================================================================
 IMPLICIT NONE
 INTEGER :: IS,I,J,K,IREC,JREC,N
 INTEGER(KIND=2) :: ZM
 INTEGER(KIND=1) :: ZC,ZP
 CHARACTER(LEN=256) :: LINE
 ...
 DO IS=1,NISG
  ...
  DO I=1,ISG(IS)%NSTW
   ...
   DO J=1,IST(IREC)%N
    ...
    ELSE
     WRITE(ISGIU(8,1),REC=JREC+ICF) DATIST(JREC)%IDATE,DATIST(JREC)%CTIME,DATIST(JREC)%WLVL_UP,DATIST(JREC)%WLVL_DOWN
    ENDIF
   ENDDO
  ENDDO
 ...
 IREF=ISG(K)%ICLC
 DO I=1,NISG
  IF(ISG(I)%ICLC.GT.IREF)ISG(I)%ICLC=ISG(I)%ICLC+DN
 END DO
