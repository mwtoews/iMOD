...
USE RESOURCE
USE MOD_ISG_PAR 
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,UTL_WSELECTFILE,UTL_MESSAGEHANDLE,UTL_CREATEDIR,UTL_CREATEREAL
...
CONTAINS
 ...
 IDF%DX=DATISC2(J,1)%DISTANCE 
 IDF%DY=DATISC2(J,1)%BOTTOM   
 ...
 DO I=1,ISG(IS)%NCRS
  ...
  DO J=1,ABS(ISC(JREC)%N)
   ...
   ELSE
    READ(ISGIU(6,ISGFILES),REC=IREC+ICF) DATISC(KREC)%DISTANCE,DATISC(KREC)%BOTTOM,ZM,ZC,ZP !DATISC(KREC)%ZM,DATISC(KREC)%ZC,DATISC(KREC)%ZP
    DATISC(KREC)%KM=UTL_CREATEREAL(ZM,ZC)
    DATISC(KREC)%ZP=INT(ZP,4)
   ENDIF
  END DO
