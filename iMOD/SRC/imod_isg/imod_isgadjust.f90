...
CONTAINS
 ...
 CALL WDIALOGSELECT(ID_DISGEDIT)
 CALL WDIALOGFIELDSTATE(ID_SHOW,MIN(1,SUM(IOPT)))
 ...
 INTEGER :: IU,IOS,K,I,J,N,ITYPE,IACT,ISAVE
 CHARACTER(LEN=256) :: LINE
 CHARACTER(LEN=50) :: WC !,TEXT
 ...
 IF(ID.EQ.ID_SAVEAS.OR.ID.EQ.IDOK)THEN
  ...
  IF(IOS.NE.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can not create file:'//CHAR(13)//TRIM(SESFNAME),'Error')
   RETURN
  ENDIF

  WRITE(IU,'(A)') '"'//TRIM(ISGFNAME)//'" :ISG'
 ...
 ELSEIF(ID.EQ.ID_LOAD)THEN
  ...
  IF(IOS.NE.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can not open file:'//CHAR(13)//TRIM(SESFNAME),'Error')
   RETURN
  ENDIF

  READ(IU,*) LINE
  IF(TRIM(LINE).NE.TRIM(ISGFNAME))THEN
   CLOSE(IU)
   RETURN
  ENDIF
 ...
 IF(ID.EQ.IDOK)THEN
  ...
  IF(WINFODIALOG(4).EQ.1)THEN
   IF(ISGADJUSTAPPLY(SESFNAME,TRIM(PREFVAL(1))//'\TMP\log_ses.txt',0))THEN
    ISAVE=1
    CALL ISGSAVE(ISAVE,0) !- saving ONLY *.ISG, *.isp, *.isd
   ENDIF
   CALL IDFPLOTFAST(1)
  ENDIF
 ENDIF
 ...
 DO I=1,4
 !calc(1),struc(2),cross(3),qh(4) - active?
  ...
  IF(ISGADJ(I)%IACT.EQ.1.AND.ISELADJ.EQ.2)THEN
   ...
   IF(ISGADJ(I)%ISOURCE.EQ.1)THEN
    ...
    IF(.NOT.LEX)THEN
     IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not open File: '//TRIM(ISGADJ(I)%FNAME),'Error')
     IF(IBATCH.EQ.1)WRITE(*,*) 'Can not open File: '//TRIM(ISGADJ(I)%FNAME)
     CLOSE(JU)
     CALL ISGADJUSTABORT()
     RETURN
    ENDIF
   ...
   ELSEIF(ISGADJ(I)%ISOURCE.EQ.2)THEN
    DO J=1,NATTRIB(I)
     ...
     IF(ISGADJ(I)%IATTRIB(J).EQ.1)THEN
      ...
      ELSE
       !## open idf file
 !      ISGADJ(I)%IU(J)=UTL_GETUNITIDF(ISGADJ(I)%IDFNAME(J),'OLD')
       IF(.NOT.IDFREAD(ISGADJ(I)%IDF(J),ISGADJ(I)%IDFNAME(J),0))THEN
 !      IF(ISGADJ(I)%IU(J).LE.0)THEN
        IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not open IDF '//TRIM(ISGADJ(I)%IDFNAME(J)),'Error')
        IF(IBATCH.EQ.1)WRITE(*,*) 'Can not open IDF '//TRIM(ISGADJ(I)%IDFNAME(J))
        CLOSE(JU)
        CALL ISGADJUSTABORT()
        RETURN
       ENDIF
 !      CALL IDFREADDIM(ISGADJ(I)%IU(J),ISGADJ(I)%NCOL(J),ISGADJ(I)%NROW(J))
 !      CALL IDFREADDX(ISGADJ(I)%IU(J),ISGADJ(I)%CS(J))
 !      CALL IDFREADULC(ISGADJ(I)%IU(J),ISGADJ(I)%XMIN(J),ISGADJ(I)%YMAX(J))
 !      CALL IDFREADNODAT(ISGADJ(I)%IU(J),ISGADJ(I)%NODATA(J))
      ENDIF
     ENDIF
    END DO
   ENDIF
 ...
 ELSEIF(IPOLSEG.EQ.2)THEN
  ...
  ALLOCATE(XPOL(MXXY),YPOL(MXXY),XP(MXXY),YP(MXXY))
 ...
 ELSE
  WRITE(*,*)
  WRITE(*,*) 'Succesfully completed ISG editing, results written in:'
  WRITE(*,*) TRIM(LOGFNAME)
 ENDIF
 ...
 DO I=1,NPOS
  ...
  IF(IPOLSEG.EQ.2)THEN
   IF(UTL_INSIDEPOLYGON(ISGX,ISGY,XPOL,YPOL,XP,YP,NXY).NE.1)LEX=.FALSE.
  ENDIF
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (3)
   ...
   DO I=1,NRECORDS(ITYPE)
    READ(IU,*) ISGEDITISC(I)%DISTANCE,ISGEDITISC(I)%BOTTOM,ISGEDITISC(I)%KM
    WRITE(IULOG,'(3F10.3)') ISGEDITISC(I)%DISTANCE,ISGEDITISC(I)%BOTTOM,ISGEDITISC(I)%KM
   END DO
   WRITE(IULOG,'(A)')
  !## adjust current qh relationships on segment
  CASE (4)
   WRITE(IULOG,'(A)') 'Read Qh-relatioship Data from: '//TRIM(ISGADJ(ITYPE)%FNAME)
   WRITE(IULOG,'(A)')
   WRITE(IULOG,'(4A10)') 'Q-SUMMER','H-SUMMER','Q-WINTER','H-WINTER'
   ALLOCATE(ISGEDITISQ(NRECORDS(ITYPE)))
   DO I=1,NRECORDS(ITYPE)
    READ(IU,*) ISGEDITISQ(I)%QZ,ISGEDITISQ(I)%HZ,ISGEDITISQ(I)%QW,ISGEDITISQ(I)%HW
    WRITE(IULOG,'(4F10.3)') ISGEDITISQ(I)%QZ,ISGEDITISQ(I)%HZ,ISGEDITISQ(I)%QW,ISGEDITISQ(I)%HW
   END DO
   WRITE(IULOG,'(A)')
 END SELECT
...
!   ISG(ISELISG)%ICRS
   ...
   DO I=1,NRECORDS(ITYPE)
    ...
    DATISC(IPOS+I-1)%KM      =ISGEDITISC(I)%KM
   END DO
  !## adjust current qh relationships on segment
  CASE (4)
   ...
   DO I=1,NRECORDS(ITYPE)
    DATISQ(IPOS+I-1)%QZ=ISGEDITISQ(I)%QZ
    DATISQ(IPOS+I-1)%HZ=ISGEDITISQ(I)%HZ
    DATISQ(IPOS+I-1)%QW=ISGEDITISQ(I)%QW
    DATISQ(IPOS+I-1)%HW=ISGEDITISQ(I)%HW
   END DO
 END SELECT
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (3)
   ...
   ELSEIF(J.EQ.3)THEN
    WRITE(IULOG,'(A)') ' KM:'
    DO K=1,N
     X1=DATISC(IREF+K-1)%KM
     X2=X1
     CALL ISGADJUSTCURRENTATTRIBUTES(X2,ITYPE,J)
     DATISC(IREF+K-1)%KM=X2
     WRITE(IULOG,'(A,2(F15.7,A))') '  old/new value:',X1,',',X2
    END DO
   ENDIF
  !## adjust current qh relationships on segment
  CASE (4)
   IF(J.EQ.1)THEN
    WRITE(IULOG,'(A)') ' Discharge Summer:'
    DO K=1,N
     X1=DATISQ(IREF+K-1)%QZ
     X2=X1
     CALL ISGADJUSTCURRENTATTRIBUTES(X2,ITYPE,J)
     DATISQ(IREF+K-1)%QZ=X2
     WRITE(IULOG,'(A,2(F15.7,A))') '  old/new value:',X1,',',X2
    END DO
   ELSEIF(J.EQ.2)THEN
    WRITE(IULOG,'(A)') ' Waterlevel Summer:'
    DO K=1,N
     X1=DATISQ(IREF+K-1)%HZ
     X2=X1
     CALL ISGADJUSTCURRENTATTRIBUTES(X2,ITYPE,J)
     DATISQ(IREF+K-1)%HZ=X2
     WRITE(IULOG,'(A,2(F15.7,A))') '  old/new value:',X1,',',X2
    END DO
   ELSEIF(J.EQ.3)THEN
    WRITE(IULOG,'(A)') ' Discharge Winter:'
    DO K=1,N
     X1=DATISQ(IREF+K-1)%QW
     X2=X1
     CALL ISGADJUSTCURRENTATTRIBUTES(X2,ITYPE,J)
     DATISQ(IREF+K-1)%QW=X2
     WRITE(IULOG,'(A,2(F15.7,A))') '  old/new value:',X1,',',X2
    END DO
   ELSEIF(J.EQ.4)THEN
    WRITE(IULOG,'(A)') ' Waterlevel Winter:'
    DO K=1,N
     X1=DATISQ(IREF+K-1)%HW
     X2=X1
     CALL ISGADJUSTCURRENTATTRIBUTES(X2,ITYPE,J)
     DATISQ(IREF+K-1)%HW=X2
     WRITE(IULOG,'(A,2(F15.7,A))') '  old/new value:',X1,',',X2
    END DO
   ENDIF
 END SELECT
