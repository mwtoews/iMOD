...
CONTAINS
 ...
 ELSEIF(I.EQ.1)THEN
  CALL WDIALOGFIELDSTATE(ID_LOAD2,I)
  ...
  IF(LEX)THEN
   ...
   ELSEIF(ISELADJ.EQ.2)THEN
    ...
    IF(ISGADJ(ITYPE)%ISOURCE.EQ.1.OR.ISGADJ(ITYPE)%ISOURCE.EQ.3)THEN
     CALL ISGADJUSTFROMFILE(ITYPE,IPOS+K-1,ISGADJ(ITYPE)%ISOURCE,ISEG,IPOS) 
  ...
  CASE (1)
   ...
   I=0; DO J=1,NRECORDS(ITYPE)
    ...
    IF(ISOURCE.EQ.3)THEN
     IF(ISGEDITISD_SEG(I)%ISEG.NE.ISEG.OR. &
        ISGEDITISD_SEG(I)%ICLC.NE.ICLC)CYCLE
    ENDIF
