...
REAL(KIND=DP_KIND),PARAMETER,PRIVATE :: R2G=360.0D0/(2.0*3.1415)  !1 rad = 57.15 degrees
REAL(KIND=DP_KIND),PARAMETER,PRIVATE :: HNODATA=-999.99
INTEGER,PRIVATE :: IUIPF,NCOLIPF,NROWIPF,IOS,IASS,NY,NIP,JDS,IDMD,IMMD,IYMD
INTEGER,ALLOCATABLE,DIMENSION(:,:),PRIVATE :: IP
REAL(KIND=DP_KIND),PRIVATE :: XC,YC,WP,ZP,OR,ANGL,DIST,DORTHO
CHARACTER(LEN=MAXLENISG),PRIVATE :: CI
CHARACTER(LEN=3),PRIVATE :: TXT
CHARACTER(LEN=MAXLEN),PRIVATE,ALLOCATABLE,DIMENSION(:) :: STRING
...
! ENDIF
 ...
 DO J=1,NROWIPF
  ...
  IF(GETSTRINGVALUE(STRING(IX),XC).AND. &
     ...
     GETSTRINGVALUE(STRING(IWISG),WP).AND. &
...
!     IF(IISG.NE.0)CALL ISGSTUWEN_CORANGLE(IISG)
 ...
 IUIPF=UTL_GETUNIT()
 CALL OSD_OPEN(IUIPF,FILE=STRUCIPFFNAME,STATUS='OLD',ACTION='READ,DENYWRITE',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open file:'//CHAR(13)//TRIM(STRUCIPFFNAME),'Error')
  IF(IBATCH.EQ.1)WRITE(*,*) 'iMOD cannot open file: '//TRIM(STRUCIPFFNAME)
  RETURN
 ENDIF
 READ(IUIPF,*) NROWIPF
 READ(IUIPF,*) NCOLIPF
 ...
 IF(MAX(IX,IY,ID,IO,IS,IWISG).GT.NCOLIPF)THEN
  IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'There are not enough columns within:' &
                  //CHAR(13)//TRIM(STRUCIPFFNAME),'Error')
  IF(IBATCH.EQ.1)WRITE(*,*) 'There are nog enough columns within: '//TRIM(STRUCIPFFNAME)
  RETURN
 ENDIF
...
! REAL(KIND=DP_KIND) :: DX,DY,TD,D,MD,X,Y
!
! !## initialize
! ISGX  = XC
! ISGY  = YC
...
!    MD    =D
!    DIST=0.0D0
!    IF(J.GT.1)DIST=TD+SQRT(DX**2.0D0+DY**2.0D0)
!    DORTHO=D
!    ISGX  =ISP(K)%X
!    ISGY  =ISP(K)%Y
!    IISG  =I
...
! END SUBROUTINE ISGSTUWEN_INTERSECT
 ...
 !## turn around push-direction
 IF(H1.NE.HNODATA.AND.H2.NE.HNODATA.AND.H2.GT.H1)ANGL=ANGL+180.0D0
 ANGL=ORIENT(ANGL)
 ...
 !###===============================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(OUT) :: H
 INTEGER,INTENT(IN) :: I
 ...
 REAL(KIND=DP_KIND) :: H1,H2,D
 INTEGER,DIMENSION(2) :: ID
 REAL(KIND=DP_KIND),DIMENSION(2) :: DI
...
! IIST=0
 ...
 !###===============================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: ORIENT
 REAL(KIND=DP_KIND),INTENT(IN) :: OR

 ORIENT=OR
 DO
  IF(ORIENT.LT.0.0D0)ORIENT=OR+360.0D0
  IF(ORIENT.GT.360)ORIENT=OR-360.0D0
  IF(ORIENT.GE.0.0D0.AND.ORIENT.LE.360.0D0)EXIT
 ENDDO
