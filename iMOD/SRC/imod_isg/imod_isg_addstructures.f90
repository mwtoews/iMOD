...
USE WINTERACTER
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,JD,JDATETOIDATE,IDATETOJDATE
USE MOD_ISG_PAR 
...
!   ENDIF
   ...
   IF(IISG.GT.0.AND.IISG.LE.NISG)THEN
    ...
    DO I=1,NIP
     ...
     !## start of structure
     ISTW=ISTW+1
     DATIST(ISTW)%IDATE    =JDATETOIDATE(JDS+IP(I,1))
     ...
     !## end of structure
     ISTW=ISTW+1
     DATIST(ISTW)%IDATE    =JDATETOIDATE(JDS+IP(I,2))
 ...
 ELSE
  ...
  DO K=J,J+N-1
   JULD(1)=IDATETOJDATE(DATISD(K)%IDATE)
   DD     =JULD(1)-JULD(2)
