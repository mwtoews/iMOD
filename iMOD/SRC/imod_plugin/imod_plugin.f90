...
!## plugin not available for winteracter 8
 ...
 IF(NFILE.GT.0)THEN
  DO I=1,NFILE
   ...
   J=LEN_TRIM(RESULTFILE)
   IF(INDEX(RESULTFILE,':',.TRUE.).GT.1)THEN
    K=INDEX(RESULTFILE,'\',.TRUE.)
    RESULTFILE=RESULTFILE(K+1:J)
   ENDIF
   IF(IOS.EQ.0)THEN
    INQUIRE(FILE=TRIM(DIRNAME)//TRIM(RESULTFILE),EXIST=LEX)
    IF(.NOT.LEX)THEN
     CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Error, cannot find '//TRIM(DIRNAME)//TRIM(RESULTFILE)//'.'//CHAR(13)// &
