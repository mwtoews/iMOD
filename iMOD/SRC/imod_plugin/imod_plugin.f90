...
!###======================================================================
 ...
 CHARACTER(LEN=256) :: EXE,BACK,DIRNAME,DIRNAME_PL
 LOGICAL :: LEX
 ...
 IF(.NOT.PLUGIN_EXE_READ_INI(IPI,IDP,WNW,EXE,BACTION,BACK))RETURN
 ...
 CALL PLUGIN_EXE_READ_BACK(BACK,EXE,IFLAG)
 ...
 !###======================================================================
 LOGICAL FUNCTION PLUGIN_EXE_READ_INI(IPI,IDP,WNW,EXE,BACTION,BACK)
 !###======================================================================
 !## Function to read exe-specifications from plugin ini-file variables/commands
 IMPLICIT NONE
 INTEGER,PARAMETER :: STRLEN=256 
 CHARACTER(LEN=*),INTENT(OUT) :: WNW,EXE,BACK
 ...
 !###======================================================================
 SUBROUTINE PLUGIN_EXE_READ_BACK(BACK,EXE,IFLAG)
 !###======================================================================
 !## Subroutine to read variables from PLUG-IN.out
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IFLAG
 CHARACTER(LEN=*),INTENT(IN) :: EXE,BACK
 INTEGER :: IU,LEX
 CHARACTER(LEN=256) :: DIRNAME,LINE,WINDOW,RESULTFILE,STOPCOM,MESSINFO,MESSERR,MESSPROG
 
 DIRNAME=EXE(:INDEX(EXE,'\',.TRUE.)-1)
 CALL IOSDIRCHANGE(DIRNAME) !#Kan dit ook niet in de controle loop hierboven worden geinitialiseerd??
 INQUIRE(FILE=TRIM(DIRNAME)//'\'//BACK,EXIST=LEX)
 IF(.NOT.LEX)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Error, cannot find '//TRIM(DIRNAME)//'\'//BACK,'Error')
  RETURN
 ENDIF
 ...
 IF(IFLAG.EQ.2)THEN !# if "wait"-command given then Back-file will be read
  CALL OSD_OPEN(IU,FILE=TRIM(DIRNAME)//'\'//BACK,STATUS='OLD',ACTION='READ')
  IF(IU.EQ.0)RETURN
 ENDIF 
 
 WINDOW='';IF(UTL_READINITFILE('WINDOW',LINE,IU,1)) READ(LINE,*) WINDOW
 RESULTFILE='';IF(UTL_READINITFILE('FILE',LINE,IU,1)) READ(LINE,*) RESULTFILE
 STOPCOM='';IF(UTL_READINITFILE('STOP',LINE,IU,1)) READ(LINE,*) STOPCOM
 MESSINFO='';IF(UTL_READINITFILE('MESSAGE_INFO',LINE,IU,1)) READ(LINE,*) MESSINFO
