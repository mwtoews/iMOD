...
CONTAINS
 ...
 REAL :: X1,X2,Y1,Y2,AX1,AX2,AY1,AY2,OFFX,RAT,RATBOX
 ...
 IF(LEN_TRIM(LEG%HEDTXT).GT.0)THEN
  CALL IGRCOLOURN(WRGB(0,0,0))
  CALL WGRTEXTSTRING(X1,Y2-0.5*TDY,TRIM(LEG%HEDTXT)) 
  Y2=Y2-TDY
 ELSE
  Y2=Y2-DY
 ENDIF 
...
!  CALL IGRFILLPATTERN(OUTLINE)
  ...
  IF(IGRY.EQ.0)THEN
   ...
   ELSEIF(MOD(J,NJ).EQ.0)THEN
    IF(NY-J.GT.TINTERVAL)RSTRING=UTL_REALTOSTRING(LEG%CLASS(I))
   ENDIF
  !## max. 50 classes
  ELSE
  ...
  IF(LEN_TRIM(RSTRING).NE.0)THEN
   CALL IGRCOLOURN(WRGB(0,0,0))
   CALL WGRTEXTSTRING(X+BXX+OFFX,Y-(BXY/2.0),TRIM(RSTRING))
   CALL IGRCOLOURN(WRGB(150,150,150)) 
   CALL IGRJOIN(X,Y-BXY,X+BXX+(0.5*OFFX),Y-BXY)
  ENDIF
