...
CONTAINS
 ...
 IF(LEN_TRIM(LEG%HEDTXT).GT.0)THEN
  CALL WGRTEXTSTRING(X1+0.25*BOXX,Y2-(TDY/2.25),TRIM(LEG%HEDTXT)) 
 ...
 DO I=1,LEG%NCLR
  ...
  CALL IGRRECTANGLE(X,Y,X+BOXX,Y-TDY) !BOXY)
  IF(IGRY.EQ.1)THEN
   CALL IGRFILLPATTERN(OUTLINE)
   CALL IGRLINEWIDTH(1)
   CALL IGRCOLOURN(WRGB(150,150,150)) 
   CALL IGRRECTANGLE(X,Y,X+BOXX,Y-TDY)
!   CALL IGRRECTANGLE(X,Y,X+BOXX,Y-BOXY)
   CALL IGRJOIN(X+BOXX,Y-TDY,X+BOXX+(0.1*BOXX),Y-TDY)
...
!    CALL WGRTEXTSTRING(X+BOXX+OFFX,Y-BOXY,TRIM(LEG%LEGTXT(I)))
  ...
  IF(LEN_TRIM(LEG%LEGTXT(I)).NE.0)THEN
   TDX=MAX(TDX,WGRTEXTLENGTH(''//TRIM(LEG%LEGTXT(I))//'')*WINFOGRREAL(GRAPHICSCHWIDTH))
  ENDIF
