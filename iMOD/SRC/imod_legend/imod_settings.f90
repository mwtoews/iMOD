...
USE MOD_IPF_LABEL
 ...
 IF(LIPF)THEN
  ...
  MP(IPLOT)%HCOL_METHOD=MIN(MAX(1,MP(IPLOT)%HCOL_METHOD),5)
  ...
  CASE(PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (IDOK)
     ...
     CALL WDIALOGGETSTRING(IDF_STRING1,MP(IPLOT)%TFORMAT)
...
END SUBROUTINE SETTINGS_IPFSYMBOLS
 ...
 IF(TRIM(GEN(IGEN)%TFORMAT).EQ.'')GEN(IGEN)%TFORMAT='F10.2'
 CALL WDIALOGPUTSTRING(IDF_STRING1,GEN(IGEN)%TFORMAT)
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDOK)
      ...
      CALL WDIALOGGETSTRING(IDF_STRING1,GEN(IGEN)%TFORMAT)
      EXIT
     CASE (IDCANCEL)
      EXIT
    END SELECT
