...
! IF(ASSOCIATED(LEG%CLASS))DEALLOCATE(LEG%CLASS)
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot WRITE legend file, no permission!','ERROR')
  RETURN
 ENDIF
...
!  CALL UTL_DELSPACE(L1,LINE) 
 ...
 IF(IOS.NE.0)THEN
  ...
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot READ legend file:'//CHAR(13)// &
