...
! !## computes clrgiven
! CALL LEG_CREATE_COLORSGIVEN(IPLOT)
...
! CLRGIVEN(1)=1; CLRGIVEN(MXCGRAD)=MXCLR
 ...
 ELSE
  ...
  DO ICLR=1,MXCGRAD
   ...
   ENDIF
  ENDDO
  !## create colour from stretched legend
  CALL LEG_CREATE_COLORS(IPLOT)
  !## plot legend
  CALL LEG_PLOTSTRETCHEDLEGEND(IPLOT)
 ...
 DO I=1,MXCGRAD
  ...
  IF(MP(IPLOT)%LEG%CGRAD(I).NE.0)CYCLE
...
! !###================================================================================
! SUBROUTINE LEG_CREATE_COLORSGIVEN(IPLOT)
! !###================================================================================
! IMPLICIT NONE
! INTEGER,INTENT(IN) :: IPLOT
...
! END DO
!
! END SUBROUTINE LEG_CREATE_COLORSGIVEN
...
!
! DO I=1,MXCGRAD
!  MP(IPLOT)%LEG%ICLRGRAD(I)=MP(IPLOT)%LEG%RGB(CLRGIVEN(I))
! ENDDO
!  
!! K=MXCLR/MXCGRAD
!! 
!! J=K; DO I=2,MXCGRAD-1
!!  MP(IPLOT)%LEG%ICLRGRAD(I)=MP(IPLOT)%LEG%RGB(J)
!!  J=J+K
!! ENDDO
!! 
!! MP(IPLOT)%LEG%ICLRGRAD(1)      =MP(IPLOT)%LEG%RGB(1)
!! K=SIZE(MP(IPLOT)%LEG%RGB)
!! MP(IPLOT)%LEG%ICLRGRAD(MXCGRAD)=MP(IPLOT)%LEG%RGB(K)
! 
...
! CALL LEG_CREATE_COLORSGIVEN(IPLOT)
 ...
 I1=1
 DO I=2,MXCGRAD
  IF(MP(IPLOT)%LEG%ICLRGRAD(I).EQ.0)CYCLE
