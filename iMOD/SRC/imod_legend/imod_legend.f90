...
USE MOD_LEGEND_UTL, ONLY : LEGREAD,LEGWRITE,LEGALLOCATE
...
CONTAINS
 ...
 DO I=1,MXMPLOT
  MP(I)%LEG%NCLR=0
  CALL LEGALLOCATE(MP(I)%LEG)
 END DO

 END SUBROUTINE LEGINIT

 !###====================================================================
 SUBROUTINE LEGDEFAULT()
 !###====================================================================
 IMPLICIT NONE
 INTEGER :: I,J
 ...
 INTEGER :: ITYPE,IPLOT,IROW,ICOL,IRGB,IOS,N,IOPTION,NLEG
     ...
     CASE(TABCHANGED)
      ...
      SELECT CASE (MESSAGE%VALUE2)
       CASE (ID_DLEGTAB1)
        ...
        CALL LEGGETNOCLASSES(IPLOT,IOPTION)
        IF(MP(IPLOT)%LEG%NCLR.GT.0)THEN
         CALL LEGGETCLASS(IPLOT)
         CALL LEGSAMPLECLASS(IPLOT,IOPTION,NLEG)
 !       MP(IPLOT)%NCLR=MXCLASS
         !## generate legend text
         CALL LEGPUT(IPLOT,1) !0)
        ELSE
      ...
      SELECT CASE (MESSAGE%VALUE1)
       ...
       CASE (ID_SAVE)
        IF(LEGCHECK(IPLOT))THEN
         ...
         IF(UTL_WSELECTFILE('iMOD Legend File (*.leg)|*.leg|', &
          ...
          CALL LEGGET(IPLOT)
          CALL LEGWRITE(MP(IPLOT)%LEG,IDFNAME)
         ENDIF
        ENDIF
       ...
       CASE (ID_OPEN)
        ...
        IF(UTL_WSELECTFILE('iMOD Legend File (*.leg)|*.leg|',&
         ...
         CALL LEGREAD(MP(IPLOT)%LEG,IDFNAME,IOS)
         IF(IOS.EQ.0)THEN
          CALL LEGINITCLRGIVEN(IPLOT)
          CALL LEGPUTCLASS(IPLOT)
          CALL LEGPUT(IPLOT,0)
       ...
       CASE (ID_PRELEGEND)
        IF(LEGPREDEFINED())THEN
         CALL LEGREAD(MP(IPLOT)%LEG,TRIM(PREFVAL(1))//'\tmp\tmp.leg',IOS)
         IF(IOS.EQ.0)THEN
          CALL LEGINITCLRGIVEN(IPLOT)
          CALL LEGPUTCLASS(IPLOT)
          CALL LEGPUT(IPLOT,0)
    ...
    SELECT CASE (ITYPE)
     ...
     CASE(PUSHBUTTON)
      SELECT CASE (MESSAGE%VALUE1)
       ...
       CASE (ID_PLUS,ID_MIN)
        CALL LEGLEVELS(MESSAGE%VALUE1)
       CASE (ID_FLIP)
        CALL LEGENDFLIP(ID_DLEGTAB1,IPLOT)
       CASE (ID_HISTOGRAM)
        CALL LEGENDHISTOGRAM(IPLOT,1)
       CASE (IDF_BUTTON5)
        CALL LEGSORT(1)
      END SELECT
    END SELECT
     ...
     CASE(PUSHBUTTON)
      SELECT CASE (MESSAGE%VALUE1)
       ...
       CASE (ID_DEFAULT)
        ...
        CALL LEGPUT(IPLOT,0)
       CASE (ID_FLIP)
        CALL LEGENDFLIP(ID_DLEGTAB2,IPLOT)
       CASE (ID_HISTOGRAM)
        CALL LEGENDHISTOGRAM(IPLOT,0)
      END SELECT
    END SELECT
 ...
 DATA CLEGEND/'GWS_NAP (-6:35)','GWS_SURFLEVEL (0:10)','FLUX_MMD (-1.5:0.05)','RESIDUAL (-2:2)','TRANSMISSIVITY (0:10.000)', &
     'GT (100-802)'/ !,'K_HORIZONTAL','K_VERTICAL','TRANSMISSIVITY','RESISTANCE'/ 
 ...
 DO I=1,MP(IPLOT)%LEG%NCLR+1
  GRAPH(1,1)%RX(I)= MP(IPLOT)%LEG%CLASS(I-1) !+MP(IPLOT)%LEG%CLASS(I))/2.0
...
! CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Values for histogram updated and'//CHAR(13)// &
!   'written in fifth column of table.','Info')
 ENDIF
 
 END SUBROUTINE

 !###====================================================================
 SUBROUTINE LEGENDFLIP(ITAB,IPLOT)
 !###====================================================================
 IMPLICIT NONE
 ...
 ELSEIF(ITAB.EQ.ID_DLEGTAB2)THEN
  ...
  DO I=1,MXCGRAD
   ...
   IF(J.EQ.1)CALL WDIALOGGETINTEGER(ID3(I),ICLR(I))
  END DO
  DO I=1,MXCGRAD
   IF(ICLR(MXCGRAD-I+1).NE.0)THEN
    CALL WDIALOGPUTCHECKBOX(ID2(I),1)
    CALL WDIALOGPUTINTEGER(ID3(I),ICLR(MXCGRAD-I+1))
    CALL WDIALOGCOLOUR(ID3(I),ICLR(MXCGRAD-I+1),ICLR(MXCGRAD-I+1))
   ELSE
 ...
 CALL LEGPUT(IPLOT,0) !## do not create autom. legend-text

 END SUBROUTINE

 !###====================================================================
 SUBROUTINE LEGGETNOCLASSES(IPLOT,IOPTION)
 !###====================================================================
 IMPLICIT NONE
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (IDOK)
      ...
      CALL WDIALOGGETCHECKBOX(IDF_CHECK1,IOPTION)
      EXIT
     CASE (IDCANCEL)
      MP(IPLOT)%LEG%NCLR=0
      EXIT
 ...
 END SUBROUTINE LEGGETNOCLASSES

 !###====================================================================
 SUBROUTINE LEGMINMAX()
 !###====================================================================
 IMPLICIT NONE
 INTEGER :: ID(7)
 INTEGER :: I,J
 ...
 END SUBROUTINE LEGMINMAX

 !###====================================================================
 LOGICAL FUNCTION LEGCHECK(IPLOT)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT
 INTEGER            :: I,ITAB
 ...
 END FUNCTION LEGCHECK

 !###====================================================================
 SUBROUTINE LEGCOPYFROM()
 !###====================================================================
 IMPLICIT NONE
 TYPE(WIN_MESSAGE)                           :: MESSAGE
...
!     MP(IL(I))%SCOLOR=MP(IPLOT)%SCOLOR
!     MP(IL(I))%PRFTYPE=MP(IPLOT)%PRFTYPE
 ...
 END SUBROUTINE LEGCOPYFROM

 !###====================================================================
 SUBROUTINE LEGCOPY(IPLOT,CODE)
 !###====================================================================
 IMPLICIT NONE
 ...
 END SUBROUTINE LEGCOPY

 !###====================================================================
 SUBROUTINE LEGPUTCLASS(IPLOT)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN)                     :: IPLOT
 INTEGER                                :: I,J!ICGRAD,
 ...
 ! ELSEIF(I.EQ.MXCGRAD)THEN
 !  CALL WDIALOGPUTREAL(ID1(I),MP(IPLOT)%CLASS(MXCLR))
 !  CALL WDIALOGPUTINTEGER(ID3(I),MP(IPLOT)%RGB(MXCLR))
 !  CALL WDIALOGCOLOUR(ID3(I),MP(IPLOT)%RGB(MXCLR),MP(IPLOT)%RGB(MXCLR))
  ELSE

   CALL WDIALOGPUTREAL(ID1(I),MP(IPLOT)%LEG%CLASS(J))
   CALL WDIALOGPUTINTEGER(ID3(I),MP(IPLOT)%LEG%RGB(J))!-1))
   CALL WDIALOGCOLOUR(ID3(I),MP(IPLOT)%LEG%RGB(J),MP(IPLOT)%LEG%RGB(J))!-1))
  ENDIF
 ! J=J+CLRGIVEN(I)
  J=MIN(MXCLR,J+CLRGIVEN(I))
 END DO

 END SUBROUTINE LEGPUTCLASS

 !###====================================================================
 SUBROUTINE LEGPUT(IPLOT,CODE)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT,CODE
 INTEGER :: I,ICLR
...
!   IF(CODE.EQ.1)MP(IPLOT)%LEG%LEGTXT(I)='>='//TRIM(RTOS(MP(IPLOT)%LEG%CLASS(I),'G',4))//' - <'// &
!      TRIM(RTOS(MP(IPLOT)%LEG%CLASS(I-1),'G',4))
 ...
 END SUBROUTINE LEGPUT

 !###====================================================================
 SUBROUTINE LEGUPDATE(IPLOT)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT
 INTEGER            :: I
 ...
 END SUBROUTINE LEGUPDATE

 !###====================================================================
 SUBROUTINE LEGGET(IPLOT)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT
 INTEGER            :: I,ITAB
 ...
 IF(ITAB.EQ.ID_DLEGTAB1)THEN
  ...
  END DO
  CALL WGRIDGETCELLREAL(ID_GRIDLEVELS,2,MP(IPLOT)%LEG%NCLR,MP(IPLOT)%LEG%CLASS(MP(IPLOT)%LEG%NCLR))
 ELSE
  CALL LEGGETCLASS(IPLOT)
 ENDIF

 END SUBROUTINE LEGGET

 !###====================================================================
 SUBROUTINE LEGGETCLASS(IPLOT)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT
 INTEGER :: I,J,II,JJ
...
! CALL WDIALOGGETCHECKBOX(IDF_CHECK8,I)
 ...
 END SUBROUTINE LEGGETCLASS

 !###====================================================================
 SUBROUTINE LEGSAMPLECLASS(IPLOT,IOPTION,NLEG)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT,IOPTION,NLEG
 INTEGER :: I,J,M,N
 REAL :: DI,V1,V2
 ...
 IF(IOPTION.EQ.0)THEN
  ...
  M =MP(IPLOT)%LEG%NCLR
  N =M
  V1=MP(IPLOT)%LEG%CLASS(NLEG) !MXCLR) !## min
  ...
  ELSE
   M=1
  ENDIF
! DO WHILE(M.LT.MP(IPLOT)%LEG%NCLR); M=MAX(1,M)*2; ENDDO
! DO
!  IF(V1.GT.MP(IPLOT)%LEG%CLASS(MXCLR))V1=V1-DI
! ENDDO
...
!   V2=V2-DI   
   ...
   J=J+NINT(REAL(NLEG)/REAL(M)); J=MIN(J,NLEG) !MXCLR)
!   J=J+NINT(REAL(MXCLR)/REAL(M)); J=MIN(J,MXCLR)
   MP(IPLOT)%LEG%RGB(I)=MP(IPLOT)%LEG%RGB(J)
  END DO
!  MP(IPLOT)%LEG%CLASS(MP(IPLOT)%LEG%NCLR)=MP(IPLOT)%LEG%CLASS(MXCLR)
!  MP(IPLOT)%LEG%RGB(MP(IPLOT)%LEG%NCLR)  =MP(IPLOT)%LEG%RGB(MXCLR)
 
 !## take over exactly
 ELSEIF(IOPTION.EQ.1)THEN

  DI=0.0
  DO I=1,MP(IPLOT)%LEG%NCLR !MXCLASS
   DI=DI+REAL(NLEG)/REAL(MP(IPLOT)%LEG%NCLR)
!   DI=DI+REAL(MXCLR)/REAL(MP(IPLOT)%LEG%NCLR)
   J=INT(DI)
   J=MIN(J,NLEG) !MXCLR)
...
!  MP(IPLOT)%LEG%RGB(MP(IPLOT)%LEG%NCLR)  =MP(IPLOT)%LEG%RGB(MXCLR)
 ...
 END SUBROUTINE LEGSAMPLECLASS

 !###====================================================================
 SUBROUTINE LEGLEVELS(ID)
 !###====================================================================
 IMPLICIT NONE
 ...
 END SUBROUTINE LEGLEVELS

 !###====================================================================
 SUBROUTINE LEGSORT(CODE)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: CODE
 INTEGER :: NCLR,I,IRGB
...
!   IF(I.EQ.1)CALL WGRIDPUTCELLSTRING(ID_GRIDLEVELS,4,I,'>='//TRIM(ADJUSTL(TXT1))//' - <= '//TRIM(ADJUSTL(TXT2)) )
!   IF(I.NE.1)CALL WGRIDPUTCELLSTRING(ID_GRIDLEVELS,4,I,'>='//TRIM(ADJUSTL(TXT1))) !//' - <  '//TRIM(ADJUSTL(TXT2)) )
  ENDIF
  CALL WGRIDLABELROW(ID_GRIDLEVELS,I,ITOS(I))
 END DO
 ...
 END SUBROUTINE LEGSORT

 !###======================================================================
 SUBROUTINE LEGCREATEINIT(ID)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN)            :: ID
 ...
 DO IPLOT=1,MXMPLOT
  IF(ACTLIST(IPLOT).EQ.1)THEN
   ...
   SELECT CASE (ID)
    CASE (ID_CDLNL)
     CALL LEGCREATE(IPLOT,'NON','ALC')
    CASE (ID_CDLL)
     CALL LEGCREATE(IPLOT,'LIN','ALC')
    CASE (ID_CDUV)
     CALL LEGCREATE(IPLOT,'UQV','ALC')
    CASE (ID_TDLNL)
     CALL LEGCREATE(IPLOT,'NON','ALE')
    CASE (ID_TDLL)
     CALL LEGCREATE(IPLOT,'LIN','ALE')
    CASE (ID_TDUV)
     CALL LEGCREATE(IPLOT,'UQV','ALE')
   END SELECT
  ENDIF
 ENDDO
 ...
 END SUBROUTINE LEGCREATEINIT

 !###======================================================================
 SUBROUTINE LEGCREATE(IPLOT,LEGOPTION,LEGDOMAIN)
 !###======================================================================
 IMPLICIT NONE
...
!       MP(IPLOT)%LEG%LEGTXT(I+1)=TRIM(RTOS(IDFVAL(I+1),'F',2))
!       IF(INDEX(MP(IPLOT)%LEG%LEGTXT(I+1),'.00').GT.0)MP(IPLOT)%LEG%LEGTXT(I+1)=TRIM(ITOS(INT(IDFVAL(I+1))))
       WRITE(LEGTXT,'('//PROFILE_GETFORMAT(IDFVAL(I+2))//')') IDFVAL(I+2)
!       IF(INDEX(LEGTXT,'.00').GT.0)LEGTXT=TRIM(ITOS(INT(IDFVAL(I+2))))
...
!  IF(I.EQ.1)MP(IPLOT)%LEG%LEGTXT(I)='>='//TRIM(ADJUSTL(TXT1))//' - <= '//TRIM(ADJUSTL(TXT2))
!  IF(I.NE.1)MP(IPLOT)%LEG%LEGTXT(I)='>='//TRIM(ADJUSTL(TXT1))) !//' - < '//TRIM(ADJUSTL(TXT2))
 ENDDO
 
 END SUBROUTINE LEGCREATE

 !==============================================================================
 SUBROUTINE LEGINITCLRGIVEN(IPLOT)
 !==============================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT
 ...
 END SUBROUTINE LEGINITCLRGIVEN

 !==============================================================================
 SUBROUTINE LEGINITCLR(IPLOT)
 !==============================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT
 ...
 DO I=1,MXCGRAD
  ...
  IF(J.EQ.1)THEN
   NCLRDEF=NCLRDEF+1
   II     =II+1
   CALL WDIALOGGETINTEGER(ID3(I),IRGB)
   CALL WRGBSPLIT(IRGB,CLR(NCLRDEF,1),CLR(NCLRDEF,2),CLR(NCLRDEF,3))
  ELSE
   CLRGIVEN(II)=CLRGIVEN(II)+CLRGIVEN(I)
  ENDIF
 END DO

 !WRITE(*,'(1x,a1,20i4)') '2',CLRGIVEN
 ...
 DO I=1,NCLRDEF-1
  DO J=1,3
   INTCLR(J)=REAL(CLR(I+1,J)-CLR(I,J))/REAL(CLRGIVEN(I)-1.0)
 !  INTCLR(J)=REAL(CLR(I+1,J)-CLR(I,J))/REAL(CLRGIVEN(I))
  END DO
  DO J=0,CLRGIVEN(I)-1
   K=K+1
   K=MIN(K,MXCLR)
   IRED=  CLR(I,1)+INT(INTCLR(1)*J)
   IGREEN=CLR(I,2)+INT(INTCLR(2)*J)
   IBLUE= CLR(I,3)+INT(INTCLR(3)*J)
   MP(IPLOT)%LEG%RGB(K)=WRGB(IRED,IGREEN,IBLUE)
  END DO
 END DO
 !WRITE(*,*) 'k=',k

 ! MP(IPLOT)%LEGTXT(I)='>='//TRIM(RTOS(MP(IPLOT)%CLASS(I),'G',4))//' - <'//TRIM(RTOS(MP(IPLOT)%CLASS(I-1),'G',4))

 END SUBROUTINE LEGINITCLR
