...
CONTAINS
 ...
 !###====================================================================
 LOGICAL FUNCTION LEG_MAIN(CODE)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: CODE
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: ITYPE,IPLOT,IROW,ICOL,IRGB,IOS,N,IOPTION,NLEG,LTYPE,I,DID
 REAL(KIND=DP_KIND) :: XINT,DX,X1,X2,X3,X4
 ...
 IF(CODE.EQ.-1)THEN
  ...
  IF(SUM(ACTLIST).GT.1)THEN; LEG_MAIN=LEG_COPYFROM(); RETURN; ENDIF
    ...
    SELECT CASE (ITYPE)
     CASE(TABCHANGED)
      ...
      SELECT CASE (MESSAGE%VALUE2)
       ...
       CASE (ID_DLEGTAB1)
        ...
        CALL LEG_GETNOCLASSES(IPLOT,LTYPE,IOPTION,XINT,NLEG)
     ...
     CASE(PUSHBUTTON)
      SELECT CASE (MESSAGE%VALUE1)
       ...
       CASE (IDHELP)
        CALL UTL_GETHELP('3.4.5','MMO.AdjustLeg')
      ...
      SELECT CASE (MESSAGE%VALUE2)
       CASE (ID_GRIDLEVELS)
        ...
        ELSEIF(ICOL.EQ.1)THEN
         IF(IROW.GT.1)THEN
          CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,IROW,XINT)
          CALL WGRIDPUTCELLDOUBLE(ID_GRIDLEVELS,2,IROW-1,XINT)
         ENDIF
        ENDIF
...
!        CALL LEG_MINMAX(IPLOT)
 ...
 DATA CLEGEND/'GWS_NAP (-6:35)','GWS_SURFLEVEL (0:10)','FLUX_MMD (-1.5:0.05)','RESIDUAL (-2:2)','TRANSMISSIVITY (0:10.00)', &
 ...
 SELECT CASE (ILEG)
  CASE (1) !## gws_nap
   ...
   WRITE(IU,'(A)') '200.000,35.00000,113,0,0,         "> 35.0m"    '
   WRITE(IU,'(A)') '35.00000,30.0000,160,0,0,         "30.0 - 35.0m"'  
   WRITE(IU,'(A)') '30.0000,25.00000,203,0,0,         "25.0 - 30.0m" ' 
   WRITE(IU,'(A)') '25.00000,20.0000,250,0,0,         "20.0 - 25.0m"  '
   WRITE(IU,'(A)') '20.0000,15.00000,255,27,0,        "15.0 - 20.0m"  '
   WRITE(IU,'(A)') '15.00000,10.0000,255,57,0,        "10.0 - 15.0m"   '
   WRITE(IU,'(A)') '10.0000,7.500000,255,91,0,        "7.5 - 10.0m"   '
   ...
   WRITE(IU,'(A)') '2.000000,1.00000,255,248,0,       "1.0 - 2.0m"    '
   WRITE(IU,'(A)') '1.00000,0.0000000,206,255,0,  "0.0 - 1.0m"    '
   WRITE(IU,'(A)') '0.0000000,-1.00000,144,255,0, "-1.0 - 0.0m"   '
   WRITE(IU,'(A)') '-1.00000,-2.000000,76,255,0,      "-2.0 - -1.0m"   '
   ...
   WRITE(IU,'(A)') '-6.000000,-200.000,0,77,169,      "<-6.0m"      '
  CASE (2) !## gws_surface
   WRITE(IU,'(A)') '     24    1    1    1    1    1    1    1'
   WRITE(IU,'(A)') ' UPPER BND LOWER BND      IRED    IGREEN     IBLUE     DOMAIN'
   WRITE(IU,'(A)') ' 200.0      10.0             75          0          0 "> 10.0 m"'
   WRITE(IU,'(A)') ' 10.0      6.000            115          0          0 "6.0-10.0 m"'
   ...
   WRITE(IU,'(A)') ' 1.200      1.00            209        255        115 "1.0-1.2 m"'
   WRITE(IU,'(A)') ' 1.00     0.8000            163        255        115 "0.8-1.0 m"'
   ...
   WRITE(IU,'(A)') '0.2000      0.00             38        115          0 "0.0-0.2 m"'
   WRITE(IU,'(A)') '0.00     -200.0              0         77        168 "<0.0 m"'
  CASE (3) !## flux mm/d
   ...
   WRITE(IU,'(A)') '-0.5000000     -1.000000     255     131     128     "-1.0 - -0.50 mm/d"'
   WRITE(IU,'(A)') '-1.000000     -1.500000     255     0     0     "-1.5 - -1.0 mm/d"'
   WRITE(IU,'(A)') '-1.500000     -1E31     217     0     0     "<-1.5mm/d"  '
  CASE (4) !## residual
   WRITE(IU,'(A)') '   11    1    0    0    0    0    0    1'
   WRITE(IU,'(A)') ' UPPER BND LOWER BND      IRED    IGREEN     IBLUE    DOMAIN'
   WRITE(IU,'(A)') '    1E31     2.000          0         0       115    ">2 m"'
   WRITE(IU,'(A)') '   2.000     1.00         0         0       153    "2 - 1 m"'
   WRITE(IU,'(A)') '  1.00     0.500         0       102       255    "1 - 0.5 m"'
   ...
   WRITE(IU,'(A)') '    -0.500    -1.00       255       102         0    "-0.5 - -1 m"'
   WRITE(IU,'(A)') '    -1.00    -2.00        202         0         0    "-1 - -2 m"'
   WRITE(IU,'(A)') '    -2.000    -1E31        115         0         0    "< -2 m"'
 CASE (5) !## c/kd
   WRITE(IU,'(A)') '31    1    1    1    1    1    1    1'
   WRITE(IU,'(A)') 'UPPERBND    LOWERBND    IRED    IGREEN    IBLUE    DOMAIN'
   WRITE(IU,'(A)') '100000.0    10000.0    98    0    0    "> 10000"'
   WRITE(IU,'(A)') '10000.0    8000.00    147    0    0    "8000"'
   WRITE(IU,'(A)') '8000.00    6000.00    166    4    0    "6000"'
   WRITE(IU,'(A)') '6000.00    4000.00    221    58    11    "4000"'
   WRITE(IU,'(A)') '4000.00    2500.00    234    117    0    "2500"'
   WRITE(IU,'(A)') '2500.00    2000.00    234    165    10    "2000"'
   WRITE(IU,'(A)') '2000.00    1500.00    255    199    6    "1500"'
   WRITE(IU,'(A)') '1500.00    1000.00    245    222    5    "1000"'
   WRITE(IU,'(A)') '1000.00    950.000    252    240    20    "950"'
   WRITE(IU,'(A)') '950.000    900.000    255    255    128    "900"'
   WRITE(IU,'(A)') '900.000    850.000    225    255    90    "850"'
   WRITE(IU,'(A)') '850.000    800.000    195    255    45    "800"'
   WRITE(IU,'(A)') '800.000    750.000    166    254    3    "750"'
   WRITE(IU,'(A)') '750.000    700.000    80    254    50    "700"'
   WRITE(IU,'(A)') '700.000    650.000    4    253    104    "650"'
   WRITE(IU,'(A)') '650.000    600.000    3    243    135    "600"'
   WRITE(IU,'(A)') '600.000    550.000    5    243    172    "550"'
   WRITE(IU,'(A)') '550.000    500.000    57    232    215    "500"'
   WRITE(IU,'(A)') '500.000    450.000    96    187    242    "450"'
   WRITE(IU,'(A)') '450.000    400.000    110    160    250    "400"'
   WRITE(IU,'(A)') '400.000    300.000    128    128    255    "300"'
   WRITE(IU,'(A)') '300.000    200.000    128    70    255    "200"'
   WRITE(IU,'(A)') '200.000    100.000    128    0    255    "100"'
   WRITE(IU,'(A)') '100.000    80.0000    139    1    214    "80"'
   WRITE(IU,'(A)') '80.0000    60.0000    160    1    209    "60"'
   WRITE(IU,'(A)') '60.0000    40.0000    194    1    216    "40"'
   WRITE(IU,'(A)') '40.0000    30.0000    235    53    255    "30"'
   WRITE(IU,'(A)') '30.0000    20.0000    255    132    255    "20"'
   WRITE(IU,'(A)') '20.0000    10.0000    255    174    255    "10"'
   WRITE(IU,'(A)') '10.0000    5.000000    255    204    255    "5"'
   WRITE(IU,'(A)') '5.000000    0.000000E+00    255    225    255    "0"'
  CASE (6) !## gt
  ...
  CASE (7) !##fluxdiff
   ...
   WRITE(IU,'(A)') '2.000000,1.00000,255,128,128,"2 Negative Decrease"'
   WRITE(IU,'(A)') '1.00000,0.000000E+00,255,0,0,"1 Negative Increase"'
   WRITE(IU,'(A)') '0.000000E+00,-1.00000,192,192,192,"0 Equal/Too small"'
 END SELECT
 ...
 INTEGER :: IROW,ICOL,I,IRAT,IRAT1,N,ITYPE,IEXIT
 REAL(KIND=DP_KIND) :: GRD,NHIST
 LOGICAL :: LEX
 TYPE(WIN_MESSAGE) :: MESSAGE
  ...
  DO ICOL=1,MP(IPLOT)%IDF%NCOL
   ...
   IF(GRD.NE.MP(IPLOT)%IDF%NODATA)THEN
    ...
    IF(I.GT.0.AND.I.LE.MP(IPLOT)%LEG%NCLR)THEN
     GRAPH(1,1)%RY(I)=GRAPH(1,1)%RY(I)+1.0D0
     NHIST=NHIST+1.0D0
    ENDIF
   ENDIF
  ENDDO
 ...
 DO I=1,MP(IPLOT)%LEG%NCLR+1
  ...
  IF(I.LE.MP(IPLOT)%LEG%NCLR)THEN
   GRAPH(1,1)%RY(I)=(GRAPH(1,1)%RY(I)*100.0D0)/NHIST
   IF(IGRID.EQ.1)CALL WGRIDPUTCELLDOUBLE(ID_GRIDLEVELS,5,I,GRAPH(1,1)%RY(I)) 
  ENDIF
 END DO
 ...
 GRAPHDIM%GRAPHNAMES=MP(IPLOT)%IDFNAME(INDEX(MP(IPLOT)%IDFNAME,'\',.TRUE.)+1:)
 GRAPH(1,1)%LEGTXT='Classes'; GRAPH(1,1)%ICLR=WRGB(56,180,176)
 GRAPHDIM(1)%IFIXX=0; GRAPHDIM(1)%IFIXY=0; GRAPHDIM(1)%XTITLE='Class'; GRAPHDIM(1)%YTITLE='Frequency (%)'; GRAPHDIM(1)%LDATE=.FALSE.
 ...
 !###====================================================================
 SUBROUTINE LEG_GETNOCLASSES(IPLOT,LTYPE,IOPTION,XINT,NLEG)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT,NLEG
 INTEGER,INTENT(OUT) :: IOPTION,LTYPE
 REAL(KIND=DP_KIND),INTENT(OUT) :: XINT
 REAL(KIND=DP_KIND) :: XMIN,XMAX
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: ITYPE

 !## get minimal and maximal values
 CALL WDIALOGSELECT(ID_DLEGTAB2)
 CALL WDIALOGGETDOUBLE(ID1(1),XMAX)
 CALL WDIALOGGETDOUBLE(ID1(SIZE(ID1)),XMIN)
   
 CALL WDIALOGLOAD(ID_DLEGENDNOCLASSES,ID_DLEGENDNOCLASSES)
 CALL WDIALOGPUTDOUBLE(IDF_REAL2,XMIN)
 CALL WDIALOGPUTDOUBLE(IDF_REAL3,XMAX)
 CALL WDIALOGPUTDOUBLE(IDF_REAL1,(XMAX-XMIN)/50.0D0)
 ...
 INTEGER,INTENT(IN) :: IPLOT
 INTEGER :: I,ITAB
 REAL(KIND=DP_KIND),DIMENSION(3) :: CLASS
 ...
 IF(ITAB.EQ.ID_DLEGTAB1)THEN
  ...
  DO I=1,MP(IPLOT)%LEG%NCLR
   CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,I,CLASS(1))
   CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,2,I,CLASS(2))
   IF(CLASS(1).LE.CLASS(2))EXIT
   IF(I.GT.1)THEN
    CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,2,I-1,CLASS(3))
    IF(CLASS(3).LT.CLASS(1))EXIT
   ENDIF
   IF(I.LT.MP(IPLOT)%LEG%NCLR)THEN
    CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,I+1,CLASS(3))
 ...
 DO 
  ...
  IF(ITYPE.EQ.PUSHBUTTON.AND.MESSAGE%VALUE1.EQ.IDOK)THEN
   ...
   IPLOT=IL(J); CALL LEG_COPYTHEM(IPLOT,NI,IL)
   LEG_COPYFROM=.TRUE.; EXIT
  ENDIF
 END DO
 CALL WDIALOGUNLOAD()
 ...
 END FUNCTION LEG_COPYFROM

 !###====================================================================
 SUBROUTINE LEG_COPYTHEM(IPLOT,NI,IL)
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT,NI
 INTEGER,INTENT(IN),DIMENSION(NI) :: IL
 INTEGER :: I
 ...
 DO I=1,NI
  IF(IL(I).NE.IPLOT)THEN
   ...
   MP(IL(I))%HCOL_METHOD =MP(IPLOT)%HCOL_METHOD
   ...
   MP(IL(I))%TFORMAT=MP(IPLOT)%TFORMAT
...
!  ENDDO
 ...
 DO I=1,MP(IPLOT)%LEG%NCLR
  ...
  CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,I,X2)
  IF(I.LT.MP(IPLOT)%LEG%NCLR)THEN
   CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,I+1,X1)
   CALL WGRIDPUTCELLDOUBLE(ID_GRIDLEVELS,2,I,X1) 
   CALL WGRIDSTATECELL(ID_GRIDLEVELS,2,I,2)
  ELSE
   CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,2,I,X1)
 ...
 DX=1.0D0/MP(IPLOT)%LEG%NCLR
 X =0.0D0
 DO I=1,MP(IPLOT)%LEG%NCLR
  CALL IGRCOLOURN(MP(IPLOT)%LEG%RGB(I))
  CALL DBL_IGRRECTANGLE(0.0D0,1.0D0-X,1.0D0,1.0D0-X+DX)
  X=X+DX
 ENDDO
 ...
 IF(MP(IPLOT)%LEG%NCLR.LE.MXCLASS)THEN
  ...
  DO I=1,MP(IPLOT)%LEG%NCLR
   CALL WGRIDLABELROW(ID_GRIDLEVELS,I,ITOS(I))
   CALL WGRIDPUTCELLDOUBLE(ID_GRIDLEVELS,1,I,MP(IPLOT)%LEG%CLASS(I-1))
   CALL WGRIDPUTCELLDOUBLE(ID_GRIDLEVELS,2,I,MP(IPLOT)%LEG%CLASS(I))  
 ...
 ELSE
  ...
  DO I=1,MXCGRAD
   ...
   IF(I.EQ.1)THEN
    ...
    CALL WDIALOGPUTDOUBLE(ID1(I),MP(IPLOT)%LEG%CLASS(0))
   ...
   ELSE
    ...
    CALL WDIALOGPUTDOUBLE(ID1(I),MP(IPLOT)%LEG%CLASS(J))
    CALL WDIALOGPUTINTEGER(ID3(I),MP(IPLOT)%LEG%RGB(J))
    CALL WDIALOGCOLOUR(ID3(I),MP(IPLOT)%LEG%RGB(J),MP(IPLOT)%LEG%RGB(J))
    MP(IPLOT)%LEG%ICLRGRAD(I)=MP(IPLOT)%LEG%RGB(J)  
 ...
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPLOT
 INTEGER :: I,I1,II,ITAB,DR
 REAL(KIND=DP_KIND) :: DC
 ...
 IF(ITAB.EQ.ID_DLEGTAB1)THEN
  ...
  DO I=1,MP(IPLOT)%LEG%NCLR
   CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,I,MP(IPLOT)%LEG%CLASS(I-1))
  ...
  END DO
  IF(MP(IPLOT)%LEG%NCLR.GT.0)CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,2,MP(IPLOT)%LEG%NCLR,MP(IPLOT)%LEG%CLASS(MP(IPLOT)%LEG%NCLR))
 ...
 ELSE
  ...
  DO I=1,MXCGRAD
   ...
   IF(I.EQ.1)THEN
    CALL WDIALOGGETDOUBLE(ID1(I),MP(IPLOT)%LEG%CLASS(0))
   ELSE
    CALL WDIALOGGETDOUBLE(ID1(I),MP(IPLOT)%LEG%CLASS(CLRGIVEN(I)))
 ...
 DO I=1,MXCGRAD
  ...
  CALL WDIALOGPUTDOUBLE(ID1(I),MP(IPLOT)%LEG%CLASS(IPOS(I)-1))
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: XINT
 INTEGER :: I,J,K,M,N
 REAL(KIND=DP_KIND) :: DI,V1,V2,O1,O2,DC,DSI,DV
 ...
 IF(IOPTION.EQ.0)THEN
  ...
  IF(LTYPE.EQ.1)THEN
   ...
   IF(V1-V2.NE.0.0D0)THEN
    CALL UTL_GETAXESCALES(V1,0.0D0,V2,1.0D0)
    !## number of classes
    ...
    ELSEIF(M.LT.N.AND.M.GT.0)THEN
     DO
      DV=V2-V1; M=DV/DI; IF(MOD(DV,DI).NE.0)M=M+1
      IF(M.GE.N)EXIT; DSI=DSI/2.0D0; DI=DSI
     ENDDO
     IF(M.NE.N)DI=DI*2.0D0
    !## error occured, probably step too large
    ENDIF
   ELSE
    V2=V1+0.5D0; V1=V1-0.5D0
    !## interval
    M=1; DI=(V2-V1)/REAL(M)
   ENDIF
  ...
  DO K=1,2
   IF(K.EQ.2)DC=DBLE(NLEG)/DBLE(MAX(1,I-1))
 ...
 ELSEIF(IOPTION.EQ.1)THEN
  ...
  DI=0.0D0
  DO I=1,MP(IPLOT)%LEG%NCLR 
   DI=DI+DBLE(NLEG)/DBLE(MP(IPLOT)%LEG%NCLR)
   J=INT(DI)
   J=MIN(J,NLEG) 
 ...
 IF(ID.EQ.ID_PLUS)THEN
  ...
  CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,IROW,X1)
  X2=X1-1.0D0
  IF(IROW-1.GE.1)CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,IROW-1,X2)
  X1=(X1+X2)/2.0D0
  ...
  CALL WGRIDPUTCELLDOUBLE(ID_GRIDLEVELS,1,IROW,X1) 
  IF(IROW.LT.MP(IPLOT)%LEG%NCLR)THEN
   CALL WGRIDGETCELLDOUBLE(ID_GRIDLEVELS,1,IROW+1,X2)
   CALL WGRIDPUTCELLDOUBLE(ID_GRIDLEVELS,2,IROW,X2) 
  ENDIF
 ...
 INTEGER :: N,I,J,ICOL,IROW,NODES,NUNIQUE,SAMPLE,LNCOL,LNROW,NC1,NC2,NR1,NR2,IRAT,IRAT1,IPLOT,IP,JP,I1,I2
 REAL(KIND=DP_KIND) :: XMIN,XMAX,YMIN,YMAX,IDFVALUE,DMIN,DMAX,DR,X,DH,H,DX
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: IDFVAL=>NULL()
! REAL(KIND=SP_KIND) :: XS
 LOGICAL :: LTOOMUCH,LEX
 CHARACTER(LEN=50) :: TXT1  !LEGTXT
 CHARACTER(LEN=256) :: FNAME
 ...
 DMIN=HUGE(1.0D0); DMAX=-HUGE(1.0D0); NODES=0
 ...
 DO IPLOT=IP,JP 
  ...
  SELECT CASE (LEGOPTION)
    ...
    IF(LNCOL.GT.0.AND.LNROW.GT.0)THEN
     ...
     DO IROW=(NR1+SAMPLE-1),NR2,SAMPLE
      DO ICOL=(NC1+SAMPLE-1),NC2,SAMPLE
       ...
       IF(IDFVALUE.NE.MP(IPLOT)%IDF%NODATA.AND. &  !## NodataValue
          IDFVALUE.EQ.IDFVALUE.AND.             &  !## NaN
          ABS(IDFVALUE).LT.HUGE(1.0D0))THEN        !## Inf
   ...
   CASE ('LIN')
    ...
    IF(LEGDOMAIN.EQ.'ALE')THEN
     IF(MP(IPLOT)%IDF%DMIN.EQ.MP(IPLOT)%IDF%DMIN.AND. &
        MP(IPLOT)%IDF%DMAX.EQ.MP(IPLOT)%IDF%DMAX)THEN
      DMIN=MIN(DMIN,MP(IPLOT)%IDF%DMIN); DMAX=MAX(DMAX,MP(IPLOT)%IDF%DMAX)
     ELSE
      DMIN=-1.0D0; DMAX=1.0D0
     ENDIF
    ELSEIF(LEGDOMAIN.EQ.'ALC')THEN
     IF(MP(IPLOT)%IDF%DMIN.EQ.MP(IPLOT)%IDF%DMIN.AND. &
        MP(IPLOT)%IDF%DMAX.EQ.MP(IPLOT)%IDF%DMAX)THEN
  ...
  CASE ('UQV')
   ...
   IF(NODES.GT.0)THEN
    ...
    DO IPLOT=IP,JP 
     ...
     !## resort big to small
     CALL UTL_WSORT(IDFVAL,1,NUNIQUE,IFLAGS=SORTDESCEND)
...
!        XS=REAL(IDFVAL(I+1),4); WRITE(TXT1,*) XS; READ(TXT1,*) X 
!       !## double precision
!       ELSE
!        X=IDFVAL(I+1)
!       ENDIF
       ...
       IF(LTOOMUCH)THEN
!        WRITE(MP(IPLOT)%LEG%LEGTXT(I+1),UTL_GETFORMAT(X)) X
...
!        WRITE(MP(IPLOT)%LEG%LEGTXT(I+1),UTL_GETFORMAT(X)) X
      ...
      ELSE
       MP(IPLOT)%LEG%CLASS(I)=IDFVAL(I)-1.0D0
      ENDIF
     ENDDO
    ENDDO
 ...
 INTEGER :: M,I
 REAL(KIND=DP_KIND),DIMENSION(:),POINTER,INTENT(INOUT) :: IDFVAL
 REAL(KIND=DP_KIND),DIMENSION(:),POINTER :: IDFVAL_BU=>NULL()
 ...
 REAL(KIND=DP_KIND),DIMENSION(3) :: INTCLR
 INTEGER,DIMENSION(MXCGRAD,3) :: PCLR
 
 !## read current color settings on legend-dialog
 PCLR=0.0D0
 DO I=1,MXCGRAD
  IF(MP(IPLOT)%LEG%CGRAD(I).EQ.1)CALL WRGBSPLIT(MP(IPLOT)%LEG%ICLRGRAD(I),PCLR(I,1),PCLR(I,2),PCLR(I,3))
!  write(*,*) i
 END DO
