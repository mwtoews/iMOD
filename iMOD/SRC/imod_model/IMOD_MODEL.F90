...
! ENDIF
 ...
 IF(I.EQ.1)I=-1
 !## start runfile
 CALL PMANAGERSTART(TRIM(RUN2)//'\IMODFLOW.RUN',I,0,NCORES,.FALSE.) 
 ...
 LINE=TRIM(ITOS(MXITER))//','//TRIM(ITOS(NITER))//','// &
      TRIM(RTOS(HCLOSE,'E',3))//','//TRIM(RTOS(RCLOSE,'E',3))//',' &
           //TRIM(RTOS(RELAX,'F',2))
 
 IF(NCORES.GT.1)THEN
  !##PKS options
  LINE=TRIM(LINE)//','//TRIM(ITOS(PARTOPT-1))//','//TRIM(ITOS(IMERGE))
 ELSE
  !## PCG option
  LINE=TRIM(LINE)//','//TRIM(ITOS(NPCOND))//','//TRIM(RTOS(MAXWBAL,'F',3))
 ENDIF
 
 WRITE(JU,'(A)') TRIM(LINE)
 
 IF(PARTOPT.EQ.2.AND.TRIM(MRGFNAME).EQ.'')THEN
