...
USE PMANAGER_MOD, ONLY: PMANAGERRUN,PMANAGERSHOW
...
CONTAINS
  ...
  CASE (4)
   ...
   IF(IEDGE.GT.0)THEN
    ...
    IF(MESSAGE%GX.GE.MODBOX(1).AND.MESSAGE%GX.LE.MODBOX(3).AND. &
     ...
     SELECT CASE (IEDGE)
      CASE (1)
       SIMBOX(IEDGE)=MIN(SIMBOX(3),SIMBOX(IEDGE)+DX) !MAX(MODBOX(1),
      CASE (3)
       SIMBOX(IEDGE)=MAX(SIMBOX(1),SIMBOX(IEDGE)+DX) !MIN(MODBOX(3),
      CASE (2)
       SIMBOX(IEDGE)=MIN(SIMBOX(4),SIMBOX(IEDGE)+DY) !MAX(MODBOX(2),
      CASE (4)
       SIMBOX(IEDGE)=MAX(SIMBOX(2),SIMBOX(IEDGE)+DY) !MIN(MODBOX(4),
      CASE (5)
 ...
 INTEGER :: IU,IOS,I,IFLAGS,IEXCOD
 LOGICAL :: LEX
...
! IF(IRUNMODE.EQ.0)THEN
 ...
 WRITE(IU,*) 'START "Runfile:'//TRIM(RUN1(INDEX(RUN1,'\',.TRUE.):))//'" /B '//TRIM(PREFVAL(8)(I:))//' '//'IMODFLOW.RUN'
 CLOSE(IU)
! ENDIF
 ...
 IF(NSCL_ORG.GT.2)THEN !.OR.IIPF.EQ.1)THEN
 ...
 LINE=TRIM(ITOS(NLAY))//','//TRIM(ITOS(MXNLAY))//','//TRIM(ITOS(NPER+SIMNPER))//','//TRIM(ITOS(ISS))//','//TRIM(ITOS(NSCL))//','// &
      TRIM(ITOS(IFTEST))//','//TRIM(ITOS(ICONCHK))//','//TRIM(ITOS(IIPF))
...
! CALL IDFPLOT(1)
 ...
 READ(IU,*,IOSTAT=IOS) NLAY,MXNLAY,NPER,ISS,NSCL,IFTEST,ICONCHK,IIPF
