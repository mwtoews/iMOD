...
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_SUBST,ITOS,RTOS,UTL_CREATEDIR,UTL_IMODFILLMENU,IDFPLOT1BITMAP,IDFPLOT2BITMAP,&
     INVERSECOLOUR,UTL_CAP,UTL_WSELECTFILE,UTL_HIDESHOWDIALOG,UTL_MESSAGEHANDLE,UTL_SYSCOREINFO
    ...
    CASE (ID_DMDLTAB1)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (ID_PRJMANAGER)
       ...
       IF(PMANAGERRUN(ID_OPENRUN,FNAME,0))CALL PMANAGERSHOW(1)
      !## copy runfile
      CASE (ID_COPY)
...
!## scenarios turned off IDF_CHECK1
...
! ENDIF
 ...
 CALL WDIALOGGETMENU(IDF_MENU2,PARTOPT)
 ...
 IF(I.EQ.1)I=-1
 !## start runfile
 CALL PMANAGERSTART(TRIM(RUN2)//'\IMODFLOW.RUN',I,0) 
 ...
 READ(IU,*)
 WRITE(JU,'(A)') CHAR(39)//TRIM(RUN2)//CHAR(39)
 ...
 IF(PARTOPT.GT.1)THEN
  LINE=TRIM(LINE)//','//TRIM(ITOS(PARTOPT-2))//','//TRIM(ITOS(IMERGE))
 ENDIF
 
 WRITE(JU,'(A)') TRIM(LINE)
 
 IF(PARTOPT.EQ.3.AND.TRIM(MRGFNAME).EQ.'')THEN
 ...
 IF(ISCEN.EQ.1)WRITE(JU,*) CHAR(39)//TRIM(SCENFNAME)//CHAR(39)
...
! LCOPY=.FALSE.; IF(SIMBOX(1).EQ.SIMBOX(3).AND.SIMBOX(2).EQ.SIMBOX(4))LCOPY=.TRUE.
 ...
 !## get the cellsize if not yet specified
 IF(IBATCH.EQ.0.OR.SIMCSIZE.GT.0.0)THEN
  LCOPY=.FALSE.
  READ(IU,'(A256)') LINE
  DO
   ...
   IF(IOS.NE.0)THEN
    IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot find [ACTIVE MODULES]','Error')
    IF(IBATCH.EQ.1)WRITE(*,'(A)') 'Cannot find [ACTIVE MODULES]'
    RETURN
   ENDIF
   IF(TRIM(UTL_CAP(STRING,'U')).EQ.'ACTIVE MODULES')THEN
    READ(LINE,*,IOSTAT=IOS) I,X1,Y1,X2,Y2,DX,BUFFER
    IF(IOS.EQ.0)THEN
     !## submodel concerned
     IF(ABS(I).EQ.1.OR.I.EQ.0)READ(LINE,*,IOSTAT=IOS) X1,Y1,X2,Y2,DX,BUFFER
    ELSE
     !## no submodel concerned
     READ(LINE,*,IOSTAT=IOS) X1,Y1,X2,Y2,DX,BUFFER
    ENDIF
  ...
  ENDDO
 ELSE
  SIMBOX(1)=-10.0E10; SIMBOX(2)=-10.0E10 
  SIMBOX(3)= 10.0E10; SIMBOX(4)= 10.0E10
 ENDIF
    ...
    IF(J.GT.0)THEN
     ...
     IF(.NOT.LEX)THEN
      ...
      ELSE
       ...
       WRITE(*,'(A)') 'Copying ISG file '//TRIM(FNAME)//' ...'
...
! LSIM=LEX
 ...
 REAL :: DX
 CHARACTER(LEN=256) :: FNAME
 INTEGER,ALLOCATABLE,DIMENSION(:) :: COPTS
...
! CALL WDIALOGPUTMENU(IDF_MENU3,CSIZES,MAXCSIZES,16)
 ...
 ! COPTS(I)=
 !ENDDO 
 !CALL WDIALOGPUTMENU(IDF_MENU3,COPTS,NMAXCORES,0)
...
! CALL IDFPLOT(1)
 ...
 ENDIF
 IF(I.NE.0)THEN
  IF(.NOT.MODEL1READRUNFILEERROR(1,IU,'You cannot use a runfile that includes a scenario file, ISCEN.EQ.1'))RETURN
 ENDIF

 READ(IU,'(A256)',IOSTAT=IOS) LINE
 READ(LINE,*,IOSTAT=IOS) MXITER,NITER,HCLOSE,RCLOSE,RELAX,NPCOND,MAXWBAL,MAXICVNG,DELTCNVG,IDAMPING,PARTOPT
 IF(IOS.NE.0)THEN
  PARTOPT=-99
