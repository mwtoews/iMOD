...
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_SUBST,ITOS,RTOS,UTL_CAP,UTL_WSELECTFILE,UTL_MESSAGEHANDLE, &
    UTL_INVERSECOLOUR,UTL_PLOTLOCATIONIDF,UTL_GETUNIQUE_INT,UTL_IDFSNAPTONICEGRID
...
CONTAINS
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_REFRESH)
     ...
     IF(WINFODIALOG(4).EQ.1)THEN
      !## reset factors
      IPFAC%FACT=1.0D0; IF(.NOT.GC_REGISFILES_PUT(ID_DGEOCONNECT_TAB2))RETURN
     ENDIF
   END SELECT
  CASE (FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: X1,X2,Y1,Y2,DX
 INTEGER :: N,M

 X1=MPW%XMIN; Y1=MPW%YMIN; X2=MPW%XMAX; Y2=MPW%YMAX; CALL WDIALOGGETDOUBLE(IDF_REAL5,DX)
 IF(DX.GT.0.0D0)THEN
  CALL UTL_IDFSNAPTONICEGRID(X1,X2,Y1,Y2,DX,N,M)
  CALL WDIALOGPUTDOUBLE(IDF_REAL1,X1,'(F15.3)'); CALL WDIALOGPUTDOUBLE(IDF_REAL2,Y1,'(F15.3)')
  CALL WDIALOGPUTDOUBLE(IDF_REAL3,X2,'(F15.3)'); CALL WDIALOGPUTDOUBLE(IDF_REAL4,Y2,'(F15.3)')     
 ENDIF
...
!      CALL WDIALOGPUTDOUBLE(IDF_REAL1,MPW%XMIN,'(F10.2)'); CALL WDIALOGPUTDOUBLE(IDF_REAL2,MPW%YMIN,'(F10.2)')
!      CALL WDIALOGPUTDOUBLE(IDF_REAL3,MPW%XMAX,'(F10.2)'); CALL WDIALOGPUTDOUBLE(IDF_REAL4,MPW%YMAX,'(F10.2)')     
!     ENDIF
   END SELECT
 END SELECT
 ...
 CALL MAIN_UTL_INACTMODULE(ID_GEOCONNECT)
 ...
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: TM,BM
 ...
 DO
  ...
  SELECT CASE (ITYPE)
   CASE (MOUSEMOVE)
    ...
    XC=DBLE(MESSAGE%GX)+OFFSETX; YC=DBLE(MESSAGE%GY)+OFFSETY
    ...
    CALL UTL_PLOTLOCATIONIDF(TOPM(1),IROW,ICOL)
    !## get fractions
    CALL GC_IDENTIFY_COMPUTE(DBLE(MESSAGE%GX)+OFFSETX,DBLE(MESSAGE%GY)+OFFSETY,TM,BM)
...
!    CALL WDIALOGSELECT(ID_DIDFINFO)
 ...
 CALL WDIALOGSELECT(ID_DGEOCONNECT_TAB3)
 CALL WDIALOGGETDOUBLE(IDF_REAL5,IDF%DX); IDF%DY=IDF%DX
 IF(IDF%DX.LE.0.0D0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You should specify a CellSize of greater than 0.0D0.','Error')
  RETURN
 ENDIF
 ...
 DO I=1,SIZE(IPFAC)
  DO J=1,SIZE(IPFAC(I)%FVAL)
   IF(IPFAC(I)%FVAL(J).GT.0.0D0)IGRP(I)=1
  ENDDO
 ENDDO
 ...
 N=0; DO I=1,NLAYR
  ...
  IF(SUM(IPFAC(I)%FVAL).LE.0.0D0)CYCLE
  !## current formation contains results, fractions gt 0.0D0
  ...
  K=1; DO J=1,NLAYM
   ...
   IF(IACTM(J).EQ.0)CYCLE
   K=K+1
   IF(IDENTIFY_PIPET.OR.IPFAC(I)%FVAL(J).LE.0.0D0)THEN
    CALL WGRIDCLEARCELL(IDF_GRID1,K,N)
   ELSE
    CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,K,N,IPFAC(I)%FVAL(J)*100.0D0,'(F15.3)')
   ENDIF
   IF(IPFAC(I)%FVAL(J).LE.0.0D0)THEN
    ICLR=WRGB(255,255,255)
   ELSE
    ICLR=100*(1.0D0-IPFAC(I)%FVAL(J)); ICLR=WRGB(0,150+ICLR,0)
   ENDIF
 ...
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(OUT) :: TM,BM
 INTEGER :: I,J,IKHR,IKVR
 REAL(KIND=DP_KIND) :: TR,BR,Z1,Z2
 CHARACTER(LEN=52) :: FTYPE
 REAL(KIND=DP_KIND),PARAMETER :: MINFRAC=0.01D0
 ...
 !## initiate fractions
 DO I=1,NLAYR; IPFAC(I)%FVAL=0.0D0; ENDDO
 ...
 DO I=1,NLAYR
  ...
  !## process data
  LINE='Process Formation: '//TRIM(FTYPE)//' '//TRIM(RTOS(REAL(I*100,8)/REAL(NLAYR,8),'F',2))//'%'
 ...
 TYPE(WIN_MESSAGE) :: MESSAGE
 REAL(KIND=DP_KIND) :: TR,BR,Z1,Z2,F,KVAL,XTOP,XBOT,KV
 CHARACTER(LEN=52) :: FTYPE
 ...
 DO I=1,NLAYM
  KDHIDF(I)%X=0.0D0; KDVIDF(I)%X=0.0D0
  KVAIDF(I)%X=0.0D0; KHVIDF(I)%X=0.0D0
 ENDDO
 DO I=1,NLAYM-1
  CIDF(I)%X=0.0D0; KVVIDF(I)%X=0.0D0
 ENDDO  
  ...
  DO IROW=1,TOPM(1)%NROW; DO ICOL=1,TOPM(1)%NCOL
   ...
   DO ILAY=1,NLAYM
    ...
    IF(Z1.GT.Z2.AND.XTOP.GT.XBOT)THEN
     ...
     !## and select the correct formation factor per Regislayer/modellayer, if IPFAC is filled, else IPFAC(J)%FACT=1.0D0
     ...
     !## if IPFAC is filled, else IPFAC(J)%FACT=1.0D0
     DO J=1,SIZE(IPFAC%FORM)
      !## stored in IPFAC by reading in iPEST-files
      IF(TRIM(FTYPE).EQ.TRIM(IPFAC(J)%FORM))THEN 
 ...
 DO IROW=1,TOPM(1)%NROW; DO ICOL=1,TOPM(1)%NCOL; DO ILAY=1,NLAYM
  ...
  IF(TR-BR.GT.0.0D0)THEN
   !## KH-value modellayer
   KHVIDF(ILAY)%X(ICOL,IROW)=KDHIDF(ILAY)%X(ICOL,IROW)/ABS(TR-BR) 
   IF(KHVIDF(ILAY)%X(ICOL,IROW).NE.0.0D0)THEN 
    !## KV-value modellayer
...
!  REWIND(IU)
 ...
 DO ILAY=1,NLAYM
  ...
  TOPM(ILAY)%FNAME=TRIM(TOPFOLDER)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
!  TOPM(ILAY)%FNAME=TRIM(TOPFOLDER)//'\TOP_L'//TRIM(ITOS(ILAY))//'.IDF'
  ...
  ELSE
   ...
   IF(.NOT.IDFREADSCALE(TOPM(ILAY)%FNAME,TOPM(ILAY),2,0,0.0D0,0))RETURN 
  ENDIF

  !## get BOTTOM filename and add layer number
  BOTM(ILAY)%FNAME=TRIM(BOTFOLDER)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
!  BOTM(ILAY)%FNAME=TRIM(BOTFOLDER)//'\BOT_L'//TRIM(ITOS(ILAY))//'.IDF'
  ...
  ELSE
   ...
   IF(.NOT.IDFREADSCALE(BOTM(ILAY)%FNAME,BOTM(ILAY),2,0,0.0D0,0))RETURN 
  ENDIF
...
! ALLOCATE(RESM(NLAYM)); DO I=1,SIZE(RESM); CALL IDFNULLIFY(RESM(I)); ENDDO
 ...
 DO ILAY=1,NLAYM
  ...
  IF(.NOT.IDFREADSCALE(RESM(ILAY)%FNAME,RESM(ILAY),10,0,0.0D0,0))RETURN 
 ...
 !## try top
 TOPR%FNAME=TRIM(REGISFOLDER)//'\'//TRIM(FTYPE)//'-T-C.IDF'
 IF(IREAD.EQ.0)THEN
  !## open file only
  IF(.NOT.IDFREAD(TOPR,TOPR%FNAME,IREAD))RETURN 
 ELSE
  !## scale mean
  IF(.NOT.IDFREADSCALE(TOPR%FNAME,TOPR,2,0,0.0D0,0))RETURN 
 ENDIF
 
 !## try bot
 BOTR%FNAME=TRIM(REGISFOLDER)//'\'//TRIM(FTYPE)//'-B-C.IDF'
 IF(IREAD.EQ.0)THEN
  !## open file only
  IF(.NOT.IDFREAD(BOTR,BOTR%FNAME,IREAD))RETURN 
 ELSE
  !## scale mean
  IF(.NOT.IDFREADSCALE(BOTR%FNAME,BOTR,2,0,0.0D0,0))RETURN 
 ENDIF

 !## try kh - if requested ikhr.eq.1
 IF(IKHR.EQ.1)THEN
  KHR%FNAME=TRIM(REGISFOLDER)//'\'//TRIM(FTYPE)//'-KH-S.IDF'
  IKHR=1
  IF(IREAD.EQ.0)THEN
   IF(.NOT.IDFREAD(KHR,KHR%FNAME,0,IQ=1))IKHR=0
  ELSE
   IF(.NOT.IDFREADSCALE(KHR%FNAME,KHR,3,0,0.0D0,IOPTIONAL=1))IKHR=0
  ENDIF
 ENDIF
 
 !## try kv - if requested ikvr.eq.1
 IF(IKVR.EQ.1)THEN
  KVR%FNAME=TRIM(REGISFOLDER)//'\'//TRIM(FTYPE)//'-KV-S.IDF'
  IKVR=1
  IF(IREAD.EQ.0)THEN
   IF(.NOT.IDFREAD(KVR,KVR%FNAME,0,IQ=1))IKVR=0
  ELSE
   IF(.NOT.IDFREADSCALE(KVR%FNAME,KVR,3,0,0.0D0,IOPTIONAL=1))IKVR=0
  ENDIF
 ENDIF
 ...
 REAL(KIND=DP_KIND) :: TR,BR,TM,BM,FM,FP,Z1,Z2,KVAL
 CHARACTER(LEN=52) :: FTYPE
  
 !## initiate %x
 IDF%X=  0.0D0;    IDF%NODATA=-999.99
 TOP%X=-10.0D10; TOP%NODATA=-999.99
 BOT%X= 10.0D10; BOT%NODATA=-999.99
 THK%X=  0.0D0;    THK%NODATA=-999.99
 
 !## find minimal values
 IF(IAGGR_TYPE.EQ.1)IDF%X= 10.0D10
 !## find maximal values
 IF(IAGGR_TYPE.EQ.2)IDF%X=-10.0D10
 ...
 DO I=1,NLAYR
  ...
  !## process data
  LINE='Process Formation: '//TRIM(FTYPE)//' ('//TRIM(RTOS(IPFAC(I)%FACT,'F',3))//') '//TRIM(RTOS(REAL(I*100,8)/REAL(NLAYR,8),'F',2))//'%'
  IF(IMODBATCH.EQ.1)WRITE(*,'(A)') TRIM(LINE)
  IF(IMODBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,TRIM(LINE))
         ...
         CASE (2) !## modelinput
          SELECT CASE (INPUTTYPE)
           CASE (2,4) !## vcw/kvv
            SELECT CASE (IAGGR_TYPE)
             CASE (1) !## min
              IDF%X(ICOL,IROW)=MIN(IDF%X(ICOL,IROW),(1.0D0/KVAL)*FP)
             CASE (2) !## max
              IDF%X(ICOL,IROW)=MAX(IDF%X(ICOL,IROW),(1.0D0/KVAL)*FP)
             CASE (3,4) !## mean/sum
              IDF%X(ICOL,IROW)=IDF%X(ICOL,IROW)+(((Z1-Z2)/KVAL)*FP)
            END SELECT
          END SELECT 
 ...
 DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
  IF(TOP%X(ICOL,IROW)-BOT%X(ICOL,IROW).GT.0.0D0)THEN
