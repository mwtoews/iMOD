...
USE RESOURCE
USE MODPLOT
USE MOD_UTL, ONLY : UTL_READINITFILE,UTL_GETUNIT,ITOS,RTOS,UTL_IDFSNAPTOGRID,UTL_CAP,UTL_DIRINFO_POINTER
USE MOD_IDF_PAR, ONLY : IDFOBJ
USE MOD_IDF, ONLY : IDFNULLIFY,IDFCOPY,IDFDEALLOCATE,IDFDEALLOCATEX
USE MOD_OSD, ONLY : OSD_OPEN
!## IDF-types to read IDF-files from given folders
TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:),SAVE :: TOPM,BOTM,CIDF,KDHIDF,KDVIDF,KHVIDF,KVAIDF,KVVIDF,RESM 
!## IDF-types for REGIS-files
TYPE(IDFOBJ),SAVE :: TOPR,BOTR,KHR,KVR,IDF,TOP,BOT,THK
...
INTEGER,SAVE :: NLAYM, &     !## NLAYM (model), 
                ...
                IAGGR_TYPE, & !## expression to aggregate (1,2,3,4)
                IOPTW, &
...
TYPE FRMOBJ
 ...
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: FVAL => NULL()
END TYPE
...
CONTAINS
 ...
 IF(ALLOCATED(IACTM))DEALLOCATE(IACTM)
 ...
 IF(ALLOCATED(RESM))THEN;   CALL IDFDEALLOCATE(RESM,SIZE(RESM));     DEALLOCATE(RESM);   ENDIF
 ...
 CALL IDFDEALLOCATEX(KHR);  CALL IDFDEALLOCATEX(KVR)
 CALL IDFDEALLOCATEX(TOPR); CALL IDFDEALLOCATEX(BOTR)
 CALL IDFDEALLOCATEX(IDF);  CALL IDFDEALLOCATEX(TOP)
 CALL IDFDEALLOCATEX(BOT);  CALL IDFDEALLOCATEX(THK)
 ...
 IF(IMODBATCH.EQ.0)CALL GC_INIT_PUT()

 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  IWINDOW=2; READ(LINE,*) IDF%XMIN,IDF%YMIN,IDF%XMAX,IDF%YMAX
  IF(IMODBATCH.EQ.1)WRITE(*,'(A,4(F10.3,1X))') 'WINDOW=',IDF%XMIN,IDF%YMIN,IDF%XMAX,IDF%YMAX
  IF(.NOT.UTL_READINITFILE('CELLSIZE',LINE,IU,0))RETURN
  READ(LINE,*) IDF%DX; IDF%DY=IDF%DX
  IF(IMODBATCH.EQ.1)WRITE(*,'(A,F10.3)') 'CELLSIZE=',IDF%DX
 ...
 DO I=1,NLAYR
  ...
  IF(IMODBATCH.EQ.0)THEN
   FORM=UTL_CAP(FORM,'U')
   DO J=1,NLAYR
    IF(TRIM(FORM).EQ.TRIM(UTL_CAP(IPFAC(J)%FORM,'U')))THEN; IPFAC(J)%FACT=FACT; EXIT; ENDIF
   ENDDO
   IF(J.GT.NLAYR)THEN
 ...
 IF(.NOT.UTL_READINITFILE('ISAVEK',LINE,IU,0))RETURN
 READ(LINE,*) ISAVEK; IF(IMODBATCH.EQ.1)WRITE(*,'(A7,I1)') 'ISAVEK=',ISAVEK
 IF(.NOT.UTL_READINITFILE('ISAVEC',LINE,IU,0))RETURN
 READ(LINE,*) ISAVEC; IF(IMODBATCH.EQ.1)WRITE(*,'(A7,I1)') 'ISAVEC=',ISAVEC

 IF(.NOT.UTL_READINITFILE('OUTPUTFOLDER',LINE,IU,0))RETURN
 READ(LINE,*) OUTPUTFOLDER; IF(IMODBATCH.EQ.1)WRITE(*,'(A)') 'OUTPUTFOLDER='//TRIM(OUTPUTFOLDER)
 ...
 IF(IWINDOW.EQ.1)THEN
  ...
  CALL WDIALOGPUTDOUBLE(IDF_REAL1,MPW%XMIN,'(F15.3)'); CALL WDIALOGPUTDOUBLE(IDF_REAL2,MPW%YMIN,'(F15.3)')
  CALL WDIALOGPUTDOUBLE(IDF_REAL3,MPW%XMAX,'(F15.3)'); CALL WDIALOGPUTDOUBLE(IDF_REAL4,MPW%YMAX,'(F15.3)')
  CALL WDIALOGPUTDOUBLE(IDF_REAL5,100.0D0,'(F15.3)')
 ELSEIF(IWINDOW.EQ.2)THEN
  CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO2)
  CALL WDIALOGPUTDOUBLE(IDF_REAL1,IDF%XMIN,'(F15.3)'); CALL WDIALOGPUTDOUBLE(IDF_REAL2,IDF%YMIN,'(F15.3)')
  CALL WDIALOGPUTDOUBLE(IDF_REAL3,IDF%XMAX,'(F15.3)'); CALL WDIALOGPUTDOUBLE(IDF_REAL4,IDF%YMAX,'(F15.3)')
  CALL WDIALOGPUTDOUBLE(IDF_REAL5,IDF%DX,'(F15.3)')
 ENDIF

 !## read formation name from grid
 DO I=1,NLAYR
  !## write factor related to formation name from grid
  CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,3,I,IPFAC(I)%FACT,'(F15.3)')
 ENDDO
 ...
 IF(IWINDOW.EQ.1)THEN
  IDF%NCOL=0; IDF%NROW=0
 ELSE
  CALL WDIALOGGETDOUBLE(IDF_REAL1,IDF%XMIN); CALL WDIALOGGETDOUBLE(IDF_REAL2,IDF%YMIN)
  CALL WDIALOGGETDOUBLE(IDF_REAL3,IDF%XMAX); CALL WDIALOGGETDOUBLE(IDF_REAL4,IDF%YMAX)
...
!  CALL WGRIDGETCELLSTRING(IDF_GRID1,1,I,IPFAC(I)%FORM)
  ...
  CALL WGRIDGETCELLDOUBLE(IDF_GRID1,3,I,IPFAC(I)%FACT)
 ENDDO
 ...
 WRITE(IU,'(A,I10)') 'IDUPLICATES=',IAGGR_TYPE
 ...
 READ(LINE,*) DBASEFOLDER; IF(IMODBATCH.EQ.1)WRITE(*,'(A)') 'DBASEFOLDER='//TRIM(DBASEFOLDER)
...
!  ENDIF
 ...
 READ(LINE,*) IAGGR; IF(IMODBATCH.EQ.1)WRITE(*,'(A,I10)') 'IAGGR=',IAGGR

 SELECT CASE (IAGGR)
  !## model results
  CASE (1)
   IF(.NOT.UTL_READINITFILE('MODELFOLDER',LINE,IU,0))RETURN
   READ(LINE,*) MODELFOLDER; IF(IMODBATCH.EQ.1)WRITE(*,'(A)') 'MODELFOLDER='//TRIM(MODELFOLDER)
   IF(.NOT.UTL_READINITFILE('MODELTYPE',LINE,IU,0))RETURN
   READ(LINE,*) MODELTYPE; IF(IMODBATCH.EQ.1)WRITE(*,'(A,I10)') 'MODELTYPE=',MODELTYPE
  !## model input
  CASE (2)
   IF(.NOT.UTL_READINITFILE('INPUTTYPE',LINE,IU,0))RETURN
   READ(LINE,*) INPUTTYPE; IF(IMODBATCH.EQ.1)WRITE(*,'(A,I10)') 'INPUTTYPE=',INPUTTYPE
  !## ipf-file
  CASE (3)
   IF(.NOT.UTL_READINITFILE('IPFFILE',LINE,IU,0))RETURN
   READ(LINE,*) IPFFILE; IF(IMODBATCH.EQ.1)WRITE(*,'(A)') 'IPFFILE='//TRIM(IPFFILE)
 END SELECT

 IF(.NOT.UTL_READINITFILE('IDUPLICATES',LINE,IU,0))RETURN
 READ(LINE,*) IAGGR_TYPE; IF(IMODBATCH.EQ.1)WRITE(*,'(A,I10)') 'IDUPLICATES=',IAGGR_TYPE
 IF(.NOT.UTL_READINITFILE('ISAVETB',LINE,IU,0))RETURN
 READ(LINE,*) ISAVETB; IF(IMODBATCH.EQ.1)WRITE(*,'(A,I10)') 'ISAVETB=',ISAVETB
 ...
 IF(IAGGR_TYPE.EQ.1)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO6)
 IF(IAGGR_TYPE.EQ.2)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO7)
 IF(IAGGR_TYPE.EQ.3)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO8)
 IF(IAGGR_TYPE.EQ.4)CALL WDIALOGPUTRADIOBUTTON(IDF_RADIO9)
 ...
 !## get aggregate option (1=model; 2=input; 3=ipf)
 CALL WDIALOGGETRADIOBUTTON(IDF_RADIO6,IAGGR_TYPE)
 ...
 IF(.NOT.UTL_DIRINFO_POINTER(REGISFOLDER,'*-T-C.IDF',RFILES,'F'))RETURN
 ...
 N=0; DO I=1,SIZE(IFORM)
  IF(IFORM(I).EQ.1)THEN; N=N+1; REGISFILES(N)=RFORM(I)//'-T-C.IDF'; ENDIF
 ENDDO
 ...
 DO I=1,NLAYR
  ...
  IPFAC(I)%FVAL=0.0D0; IPFAC(I)%FACT=1.0D0; IPFAC(I)%IGRP=I
  ...
  SELECT CASE (IPFAC(I)%LITHO)
   ...
   CASE ('C') !## complex (grey)
    IPFAC(I)%LITHOCLR=WRGB(200,200,200)
   CASE ('Q') !## limestone (orange)
    IPFAC(I)%LITHOCLR=WRGB(255,128,64)
   CASE ('B') !## browncoal (cyan)
    IPFAC(I)%LITHOCLR=WRGB(128,255,255)
   CASE DEFAULT
    IPFAC(I)%LITHOCLR=WRGB(250,250,250)
  END SELECT
