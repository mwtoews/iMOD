...
USE RESOURCE
USE MOD_SOBEK_PAR  
USE MOD_UTL, ONLY : UTL_WSELECTFILE,UTL_GETHELP,UTL_PUTRECORDLENGTH,ITIMETOGTIME
...
INTEGER,ALLOCATABLE,DIMENSION(:) :: IYMD,IHMS
...
CONTAINS
 ...
 INTEGER :: IERR,IONCID,NMESH,IM,NETWORKINDEX,NCID,ID_S1,NT,ID_TIMEDIM,I,ID_TIME,TITLELENGTH,IYR,IMH,IDY,IHR,IMT,ISC,NDAY
! TYPE(T_UG_NETWORK) :: NETID 
 ...
 CHARACTER(LEN=256) :: NETWORK1DNAME, MESH1DNAME,REFDAT_MAP
! REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: MAPTIMES
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IMAPTIMES
! INTEGER :: ID_NETNODEZ
  ...
  IF(MESHGEOM%DIM.EQ.1)THEN
   ...
   IF(IERR.EQ.NF90_NOERR)THEN
    ...
    IF(IERR.EQ.NF90_NOERR)THEN
     ...
     IERR = NF90_INQUIRE_DIMENSION(NCID, ID_TIMEDIM, LEN=NT)
     IERR = NF90_INQUIRE_VARIABLE(NCID, ID_TIMEDIM, XTYPE=I)
     SELECT CASE(I)
      CASE(NF90_DOUBLE)
       WRITE(*,*) 'DOUBLE'
      CASE (NF90_FLOAT)
      ...
      CASE (NF90_INT)
       WRITE(*,*) 'INT'
       ALLOCATE(iMAPTIMES(NT))
       IERR = NF90_GET_VAR(NCID, ID_TIME, iMAPTIMES)
     END SELECT
     IERR = NF90_INQUIRE_ATTRIBUTE(NCID, ID_TIME, 'units', LEN = TITLELENGTH)
     IERR = NF90_GET_ATT(NCID, ID_TIME, "units", REFDAT_MAP)
     READ(REFDAT_MAP(15:18),*) IYR
     READ(REFDAT_MAP(20:21),*) IMH
     READ(REFDAT_MAP(23:24),*) IDY
     READ(REFDAT_MAP(26:27),*) IHR
     READ(REFDAT_MAP(29:30),*) IMT
     READ(REFDAT_MAP(32:33),*) ISC
   
     ALLOCATE(IYMD(NT),IHMS(NT))
     DO I=1,NT
      ISC=ISC+IMAPTIMES(I)

      DO 
       IF(ISC.LT.60)EXIT
       IMT=IMT+1; ISC=ISC-60
      ENDDO
      DO 
       IF(IMT.LT.60)EXIT
       IHR=IHR+1; IMT=IMT-60
      ENDDO
      DO 
       IF(IHR.LT.24)EXIT
       IDY=IDY+1; IHR=IHR-24
      ENDDO
      DO 
       NDAY=WDATEDAYSINMONTH(IYR,IMH)
       IF(IDY.LE.NDAY)EXIT
       IMH=IMH+1; IDY=IDY-NDAY
      ENDDO
      DO 
       IF(IMH.LE.12)EXIT
       IMH=IMH-12; IYR=IYR+1
      ENDDO 
    
      IYMD(I)=IYR*10000+IMH*100+IDY
      IHMS(I)=IHR*3600+IMT*60 +ISC
     ENDDO
...
!De variable mesh1d_s1 bevat de waterstanden op de 1D-gridpunten, in dezelfde volgorde als mesh1d_node_x / mesh1d_node_y /
 ...
 IF(ALLOCATED(IYMD))DEALLOCATE(IYMD)
 IF(ALLOCATED(IHMS))DEALLOCATE(IHMS)
 ...
 DO IB=1,NBRCH
  CALL DFLOWFM_CREATEIPS(IB,ISEG,NSEG,DIST)
  CALL DFLOWFM_CREATEISD(IB,ICLC,NCLC,IREFSD,DIST)
  CALL DFLOWFM_CREATEISC(IB,ICRS,NCRS,IREFSC,DIST)
  NSTW=0
  NQHR=0
 ...
 !###======================================================================
 SUBROUTINE DFLOWFM_CREATEIPS(IB,ISEG,NSEG,DIST)
 !###======================================================================
 IMPLICIT NONE
...
! MESHGEOM%NODEOFFSETS(1) = 0.000000000000000D+000 _ CALC. POINTS
 ...
 !###======================================================================
 SUBROUTINE DFLOWFM_CREATEISD(IB,ICLC,NCLC,IREFSD,TDIST)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IB
 ...
 INTEGER :: I,J,JJ,II,III,N,ID1,ID2,IH,IM,IS
 REAL(KIND=DP_KIND) :: DIST,WP,BH,MIND1,MIND2,D,F1,F2
 CHARACTER(LEN=MAXLEN) :: CROSNAME
 CHARACTER(LEN=8) :: CTIME
 INTEGER,DIMENSION(2) :: IFT
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IDS
 
 WP=0.0D0
 BH=0.0D0
 
 ALLOCATE(IDS(MESHGEOM%NUMNODE)); IDS=0
 
 NCLC=0; DO JJ=1,MESHGEOM%NUMEDGE
  IFT(1)=MESHGEOM%EDGE_NODES(1,JJ)!+1
  IFT(2)=MESHGEOM%EDGE_NODES(2,JJ)!+1

  !## is on current current branch
  IF(MESHGEOM%EDGEBRANCHIDX(JJ).NE.IB)CYCLE

  DO III=1,2
   I=IFT(III)
   !## already processed
   IF(IDS(I).EQ.1)CYCLE; IDS(I)=1
! !## add calculation nodes for current branch
! NCLC=0; DO I=1,MESHGEOM%NUMNODE
   !## not on current branch - set distance to zero
   IF(MESHGEOM%NODEBRANCHIDX(I).NE.IB)THEN
    DIST=0.0D0 !CYCLE
   ELSE
   ...
   IF(ALLOCATED(S1))THEN
    IF(ISFINAL.EQ.0)THEN
     DO J=1,SIZE(S1,2)
      CALL ITIMETOGTIME(IHMS(J),IH,IM,IS)
      WP=S1(I,J)
      WP=MAX(WP,BH)
      IREFSD=IREFSD+1
      WRITE(CTIME,'(3(I2.2,A1))') IH,':',IM,':',IS
      WRITE(IU(ISD2),REC=IREFSD+1) IYMD(J),CTIME,WP,BH,1.0D0,0.3D0
     ENDDO
    ELSE
     J=SIZE(S1,2)
     CALL ITIMETOGTIME(IHMS(J),IH,IM,IS)
   ...
   ELSE
    IREFSD=IREFSD+1
    WRITE(IU(ISD2),REC=IREFSD+1) 20200101,'00:00:00',BH+1.0D0,BH,1.0D0,0.3D0
   ENDIF
  ENDDO
 ENDDO
 ...
 !###======================================================================
 SUBROUTINE DFLOWFM_CREATEISC(IB,ICRS,NCRS,IREFSC,TDIST)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IB
...
! TYPE(T_CROSSSECTION),POINTER :: CRSDEF
 ...
 NCRS=0; DO I=1,NETWORK%CRS%COUNT
  ...
  !## get correct cross-section definition
  DO II=1,NETWORK%CSDEFINITIONS%COUNT
   PCSDEF=>NETWORK%CSDEFINITIONS%CS(II)
   IF(NETWORK%CRS%CROSS(I)%TABDEF%ID.EQ.PCSDEF%ID)THEN
    CROSNAME=NETWORK%CRS%CROSS(I)%TABDEF%ID
    EXIT
   ENDIF
  ENDDO  
