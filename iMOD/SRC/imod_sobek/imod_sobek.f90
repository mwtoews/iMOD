...
!  WP=BH+1.0D0 
 ...
 NCRS=0; DO I=1,NETWORK%CRS%COUNT
  ...
  SELECT CASE(PCSDEF%CROSSTYPE)
   !## yz table, z is rising but y can decrease which cannot in iMOD
   CASE (CS_TABULATED)

    N=PCSDEF%LEVELSCOUNT
    ALLOCATE(XPROF(N*2),YPROF(N*2),XP(N),YP(N))
    ...
    YP(1)=YPROF(1)
    J=1; DO II=2,N
     !## skip as cross-section goes the opposite direction
     IF(XPROF(II).GE.XPROF(J))THEN
      J=J+1
      XP(J)=XPROF(II)
      YP(J)=YPROF(II)
!     ELSE
!      WRITE(*,*) 
    ...
    II=0; DO J=1,N
     II=II+1; XPROF(II)=-XP(N-J+1); YPROF(II)=YP(N-J+1)
    ENDDO
    DO J=1,N
     II=II+1; XPROF(II)= XP(J); YPROF(II)=YP(J)
    ENDDO
  ...
  !## shift such that centre is mid
  XMID=(XPROF(1)+XPROF(N))/2.0D0
  !## set new x-values
  XPROF=XPROF-XMID
