...
MODULE MOD_SOBEK_PAR
 ...
 TYPE GROBJ
  DOUBLE PRECISION ::  LC !gr gr = grid table, with the distance of every grid point from the beginning of the branch.
 ...
 TYPE STOBJ
  DOUBLE PRECISION ::  LC !st st = stuwen table, with the distance of every weir from the beginning of the branch.
 ...
 TYPE PDEFOBJ
  INTEGER :: PTYPE,NNXY
  REAL,POINTER,DIMENSION(:) :: XPROF,YPROF
  CHARACTER(LEN=MAXLEN) :: ID
 END TYPE PDEFOBJ
 ...
 DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE :: JULIANTIMES
 REAL,ALLOCATABLE,DIMENSION(:,:,:) :: SELECTEDVALUES
