   ...
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
    ...
    ELSE  
     DO J=1,IAGGR(I)
      ...
      ENDIF
     ENDDO
     KHV(JLAY,1)%X(ICOL,IROW)=  EXP(KHV(KLAY,1)%X(ICOL,IROW)/REAL(IAGGR(I)))
     KHV(JLAY,2)%X(ICOL,IROW)=  EXP(KHV(KLAY,2)%X(ICOL,IROW)/REAL(IAGGR(I)))
     KHV(JLAY,3)%X(ICOL,IROW)= 1.0/(KHV(KLAY,3)%X(ICOL,IROW)/REAL(IAGGR(I)))    
    ENDIF
   ENDDO; ENDDO
