...
!   ENDDO; ENDDO
 ...
 IF(ASC_TYPE.EQ.1)THEN
  REWIND(IU)
  IF(.NOT.ASC2IDF_IMPORTASC_TYPE1(IU,IDF(1),IBATCH,ISAVEIDF))IERROR=0 
 !## petrel
 ELSEIF(ASC_TYPE.EQ.2)THEN
  IF(.NOT.ASC2IDF_IMPORTASC_TYPE2(IU,IDF(1),IX,IY,IZ,ISAVEIDF))IERROR=0
 !## zims
 ELSEIF(ASC_TYPE.EQ.3)THEN
  IF(.NOT.ASC2IDF_IMPORTASC_TYPE3(IU,IDF(1),ISAVEIDF))IERROR=0
 ELSEIF(ASC_TYPE.EQ.4)THEN
  READ(LINE,*) (TXT(I),I=1,5),IDF(1)%NODATA
  IF(.NOT.ASC2IDF_IMPORTASC_TYPE4(IU,IDF(1),ISAVEIDF))IERROR=0
 ENDIF

 IF(PRESENT(IDFIMPORT))THEN
...
! ALLOCATE(IDF); CALL IDFNULLIFY(IDF)
 ...
 IF(ISAVEIDF.EQ.1)THEN
  ...
  ENDIF 
  IF(.NOT.IDFWRITE(IDF,IDF%FNAME//'.IDF',1))THEN; ENDIF
 ENDIF
