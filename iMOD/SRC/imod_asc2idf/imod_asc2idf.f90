...
!0.1E+31 0.1E+31 0.1E+31 0.1E+31 0.1E+31  
 ...
 WRITE(*,'(A)') 'Fill grid ...'
 IF(.NOT.ASC2IDF_INT_GETVALUES(XYZFNAMES(IFILE),IIPF,IXCOL,IYCOL,IZCOL,IWCOL,IWFACTOR))RETURN
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  CASE (-6,6)
   ...
   IF(ALLOCATED(BLNFILE))THEN
    ...
    DEALLOCATE(BLNILAY)
   ENDIF
   CALL KRIGING_MAIN(SIZE(XP),XP,YP,ZP,WP,IDF(1),IDF(4),1,BO_VALUE)
 ...
 !###======================================================================
 LOGICAL FUNCTION ASC2IDF_INT_GETVALUES(FNAME,ITYPE,IXCOL,IYCOL,IZCOL,IWCOL,IWFACTOR)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ITYPE,IXCOL,IYCOL,IZCOL,IWCOL,IWFACTOR
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z,W,X1,X2,Y1,Y2,ZV,NCOUNT,WW
 LOGICAL :: LEX,LT,LB
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (2)
   ...
   NCOL=MAX(1,IXCOL,IYCOL,IZCOL,IWCOL,IEXT); ALLOCATE(STRING(NCOL))
   NP=0; DO I=1,N
    ...
    SELECT CASE (IWFACTOR)
     CASE (1); WW=LOG10(W); WW=MIN(3.0D0,WW); W=1.0D0+(3.0D0-WW)
    END SELECT
    ...
    IF(LEX)THEN
     NP=NP+1; CALL ASC2IDF_INT_RESIZEVECTORS(NP,100)
     XP(NP)=X; YP(NP)=Y; ZP(NP)=Z; WP(NP)=W
    ENDIF
