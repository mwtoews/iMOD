...
!0.1E+31 0.1E+31 0.1E+31 0.1E+31 0.1E+31  
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  CASE (7)
   N=SIZE(XP)
   IF(LAGINTERVAL.EQ.0.OR.LAGDISTANCE.EQ.0.0)THEN
    CALL KRIGING_VARIOGRAM(INT(N,4),XP,YP,PP,J,IDF(1),IBATCH=IBATCH,SNAME=XYZFNAMES(IFILE),IFILE=IFILE)
   ELSE
    CALL KRIGING_VARIOGRAM(INT(N,4),XP,YP,PP,J,IDF(1),LAGINTERVAL=LAGINTERVAL,LAGDISTANCE=LAGDISTANCE, &
        IBATCH=IBATCH,SNAME=XYZFNAMES(IFILE),IFILE=IFILE)
   ENDIF
 ...
 IF(IGRIDFUNC.NE.7)THEN
  ...
  IF(ILOG.EQ.1)THEN
   DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
    IF(IDF(1)%X(ICOL,IROW).NE.IDF(1)%NODATA)IDF(1)%X(ICOL,IROW)=EXP(IDF(1)%X(ICOL,IROW))
   ENDDO; ENDDO
  ENDIF
