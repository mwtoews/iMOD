...
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_WAITMESSAGE,UTL_CAP,UTL_DIRINFO,UTL_IDFSNAPTOGRID,&
    UTL_GETMED,UTL_DIRINFO_POINTER,UTL_IDATETOJDATE,UTL_GETMED,UTL_JDATETOIDATE
USE MODPLOT
...
!USE MOD_INTERSECT, ONLY : INTERSECT_DEALLOCATE
...
! REAL,POINTER,DIMENSION(:) :: XP,YP,ZP
 REAL,ALLOCATABLE,DIMENSION(:) :: ZV
 INTEGER :: IU,ID,IROW,ICOL,J,K,M,MM,II,JJ,MX,IIPF,IERROR,NCOL,IEXT,NP
 ...
 IF(.NOT.ASC2IDF_GETEXTENT(XYZFNAMES(IFILE),IIPF))RETURN
...
!  CASE (5,-6,6,7,8)
!   ALLOCATE(XP(N),STAT=IOS(1)); ALLOCATE(YP(N),STAT=IOS(2)); ALLOCATE(ZP(N),STAT=IOS(3))
!   IF(SUM(IOS).NE.0)THEN
!    WRITE(*,'(A,I10,A1,I10,A)') 'ERROR, can not allocate memory for XP,YP,ZP(',N,')-arrays.'; RETURN
!   ENDIF
...
! END SELECT
!
 !## collect x,z,w values
 WRITE(*,'(A)') 'Fill grid ...'
 IF(.NOT.ASC2IDF_GETVALUES(XYZFNAMES(IFILE),IIPF,IXCOL,IYCOL,IZCOL))RETURN
...
!## idf(3) is mask!
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  !## bivariate
  CASE (5)
   CALL BIVARIATE_INT(XP,YP,ZP,INT(N,4),IERROR,IDF(1))
  
  !## kriging,variogram,pcg
  CASE (-6,6,7,8)
   DO I=1,SIZE(XP)
    CALL IDFIROWICOL(IDF(1),IROW,ICOL,XP(I),YP(I))
    XP(N)=REAL(ICOL); YP(N)=REAL(IROW)
   ENDDO
 !  !## create irow/icol indices for pcg interpolation
!   IF(IGRIDFUNC.EQ.8)THEN
!    J=0; DO I=1,N
!      CALL IDFIROWICOL(IDF(1),IROW,ICOL,XP(I),YP(I))
!      IF(IROW.NE.0.AND.ICOL.NE.0)THEN; J=J+1; XP(J)=REAL(ICOL); YP(J)=REAL(IROW); ZP(J)=ZP(I); ENDIF 
!     ENDDO
!     N=J
!    ENDIF
!   ENDIF
...
!    IF(IGRIDFUNC.EQ. 6)CALL KRIGING_MAIN(INT(N,4),XP,YP,ZP,IDF(1),IDF(4),MINP,MAXP,RANGE,SILL,NUGGET, KTYPE,SEARCHDISTANCE,NOSEARCH, &
...
!   ENDIF
!   IF(IGRIDFUNC.EQ.7)CALL KRIGING_VARIOGRAM(INT(N,4),XP,YP,ZP,J,IDF(1),LAGINTERVAL,LAGDISTANCE,IBATCH=1,SNAME=XYZFNAMES(IFILE))
!   MICNVG=25; 
!   IF(IGRIDFUNC.EQ.8)CALL SOLID_PCGINT(XP,YP,ZP,INT(N,4),IERROR,IDF(1),-1,GENFILE=GENFILE)
!   
...
! IF(ALLOCATED(NZ))DEALLOCATE(NZ)
 ...
 CALL ASC2IDF_DEALLOCATE()

 END FUNCTION ASC2IDF_TYPE3

 !###======================================================================
 LOGICAL FUNCTION ASC2IDF_GETVALUES(FNAME,ITYPE,IXCOL,IYCOL,IZCOL)
 !###======================================================================
 IMPLICIT NONE
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (1)
   ...
   DO
    ...
    IF(IOS.NE.0)EXIT
    NP=NP+1; CALL ASC2IDF_RESIZEVECTORS(NP,100)
    XP(NP)=X; YP(NP)=Y; ZP(NP)=Z 
   ENDDO
  ...
  CASE (2)
   ...
   NP=0; DO I=1,N
    ...
    ELSE
     ...
     LEX=PMANAGER_SAVEMF2005_PCK_READTXT(ASSF_COLUMN,STIME,ETIME,ASSF_MTYPE,Z,TXTFNAME) 
    ENDIF
 ...
 CALL ASC2IDF_RESIZEVECTORS(NP,0)
 ...
 ASC2IDF_GETVALUES=.TRUE.
 
 END FUNCTION ASC2IDF_GETVALUES
 
 !###======================================================================
 LOGICAL FUNCTION ASC2IDF_GETEXTENT(FNAME,ITYPE)
 !###======================================================================
 IMPLICIT NONE
 ...
 ASC2IDF_GETEXTENT=.TRUE.
 
 END FUNCTION ASC2IDF_GETEXTENT
 ...
 !###======================================================================
 SUBROUTINE ASC2IDF_NULLIFY()
 !###======================================================================
 IMPLICIT NONE
 
 NULLIFY(XP,YP,ZP)
  
 END SUBROUTINE ASC2IDF_NULLIFY

 !###======================================================================
 SUBROUTINE ASC2IDF_DEALLOCATE()
 !###======================================================================
 IMPLICIT NONE
 ...
 END SUBROUTINE ASC2IDF_DEALLOCATE
 
 !###======================================================================
 SUBROUTINE ASC2IDF_RESIZEVECTORS(N,DN)
 !###======================================================================
 IMPLICIT NONE
 ...
 ALLOCATE(XP_DUMMY(MM),YP_DUMMY(MM),ZP_DUMMY(MM))
 ...
 END SUBROUTINE ASC2IDF_RESIZEVECTORS
