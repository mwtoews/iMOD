...
CONTAINS
 ...
 REAL :: X,Y,Z,P,X1,X2,Y1,Y2,Z1,Z2,P1,P2,DZ,ZCOR
 ...
 CALL OSD_OPEN(IU,FILE='d:\IMOD-MODELS\ALBERTA\MEDICIN_HAT\DBASE\PROPERTIES\POROSITY\VERSION_1\POINTS.IPF',STATUS='UNKNOWN',FORM='FORMATTED',ACTION='WRITE')
...
! IF(.NOT.IDFREAD(top,TOP%FNAME,1))RETURN
! IF(.NOT.IDFREAD(bot,BOT%FNAME,1))RETURN
! CALL IDFCOPY(TOP,IDF)
 
 IF(.NOT.IDFREADSCALE(TOP%FNAME,TOP,2,1,0.0,1))RETURN
 IF(.NOT.IDFREADSCALE(BOT%FNAME,BOT,2,1,0.0,1))RETURN
 
 !## voxel model
 IDF%NODATA=HUGE(1.0); IF(.NOT.IDFALLOCATEX(IDF))RETURN
...
!    Z=Z+ZCOR
    ...
    !## current element in current layer
    IF(Z.LT.Z1.AND.Z.GE.Z2)THEN
     IDF%X(ICOL,IROW) = IDF%X(ICOL,IROW)+P
     IDFC%X(ICOL,IROW)=IDFC%X(ICOL,IROW)+1.0
     TP%X(ICOL,IROW)  =Z1
     BT%X(ICOL,IROW)  =Z2
     M=M+1
    ENDIF
   ...
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
    IF(IDFC%X(ICOL,IROW).GT.0.0)THEN
   ...
   IL=IL+1
  ENDDO
  IF(ILOOP.EQ.1)THEN; JL=IL+1; NN=IL*2; ENDIF
 ENDDO
   ...
   NP=0; DO I=1,N
    ...
    ELSE     
     ...
     IF(LT.AND.LB.AND.(STIME.NE.ETIME))THEN
      ...
      LEX=UTL_PCK_READTXT(ASSF_COLUMN,STIME,ETIME,Z,TXTFNAME,ASSF_INDICATOR,ASSF_THRESHOLD(MAX(1,ASSF_INDICATOR)),0)
     ELSE
     LEX=.FALSE.; Z=0.0
     ENDIF
    ENDIF
