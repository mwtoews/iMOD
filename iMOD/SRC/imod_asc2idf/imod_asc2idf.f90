...
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_WAITMESSAGE,UTL_CAP,UTL_DIRINFO,UTL_IDFSNAPTOGRID,&
    UTL_GETMED,UTL_DIRINFO_POINTER,UTL_IDATETOJDATE,UTL_GETMED,UTL_JDATETOIDATE,UTL_GENLABELSGET,VAR
USE MODPLOT
...
CONTAINS
 ...
 IF(ASC_TYPE.EQ.0)THEN
  IF(.NOT.ASC2IDF_IMPORTASC_TYPE1(IU,IDFNAME,TOP,BOT))IERROR=0
 ...
 !###======================================================================
 LOGICAL FUNCTION ASC2IDF_IMPORTASC_TYPE1(IU,IDFNAME,TOP,BOT)
 !###======================================================================
 IMPLICIT NONE
...
! IF(INDEX(UTL_CAP(XYZFNAMES(IFILE),'U'),'.IFF',.TRUE.).GT.0)IIPF=5
 ...
 CALL ASC2IDF_INT_GETFACES(IDF(1),IDF(1)%NROW,IDF(1)%NCOL,IPC,BLNFILE)
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  CASE (8)
   ...
   CALL SOLID_PCGINT(XP,YP,ZP,SIZE(XP),IERROR,IDF(1),-1,CD=WP,IPC=IPC)
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (3)
   ...
   NP=0; DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
    NP=NP+1; CALL IDFGETLOC(IDF(1),IROW,ICOL,XP(NP),YP(NP)); ZP(NP)=IDF(1)%X(ICOL,IROW); WP(NP)=1.0
   ENDDO; ENDDO
  ...
  CASE (4)
   ...
   DO
    ...
    !## get value for line
    CALL UTL_GENLABELSGET(ID,J)
    IF(J.LE.0)THEN; WRITE(*,'(A)') 'Cannot get the label for current line '//TRIM(ITOS(ID)); RETURN; ENDIF
 ...
 !###====================================================================
 SUBROUTINE ASC2IDF_INT_GETFACES(IDF,NROW,NCOL,IPC,FNAME)
 !###====================================================================
 IMPLICIT NONE
 ...
 DO I=2,SIZE(XP)
  ...
  CALL ASC2IDF_INT_GETFACES_HFB(IC1,IC2,IR1,IR2,IP1,IP2,IPC,NROW,NCOL)

 ENDDO

 END SUBROUTINE ASC2IDF_INT_GETFACES

!###====================================================================
SUBROUTINE ASC2IDF_INT_GETFACES_HFB(IC1,IC2,IR1,IR2,IP1,IP2,IPC,NROW,NCOL)
!###====================================================================
IMPLICIT NONE
...
END SUBROUTINE ASC2IDF_INT_GETFACES_HFB
 ...
 IF(DN.EQ.0)THEN
  MM=N; NN=N; IF(NN.EQ.SIZE(XP))RETURN
 ELSE 
