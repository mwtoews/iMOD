 ...
 IF(IGRIDFUNC.NE.7)THEN
  ...
  IF(ILOG.EQ.1)THEN
   DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
    IF(IDF(1)%X(ICOL,IROW).GT.0.0)IDF(1)%X(ICOL,IROW)=EXP(IDF(1)%X(ICOL,IROW))
   ENDDO; ENDDO
  ENDIF
   ...
   NP=0; DO I=1,N
    ...
    ELSE     
     ...
     IF(LT.AND.LB.AND.(STIME.NE.ETIME))THEN
      ...
      LEX=UTL_PCK_READTXT(ASSF_COLUMN,STIME,ETIME,Z,TXTFNAME,ASSF_INDICATOR,ASSF_THRESHOLD(MAX(1,ASSF_INDICATOR)),ISS) !0)
     ELSE
     LEX=.FALSE.; Z=0.0
     ENDIF
    ENDIF
