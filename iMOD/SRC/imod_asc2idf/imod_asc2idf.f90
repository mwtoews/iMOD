...
!   ENDDO; ENDDO
 ...
 IDF(1)%ITYPE=UTL_DETERMINEIDFTYPE(XMIN,YMIN,XMAX,YMAX,CSIZE,NCOL,NROW)
 ...
 IF(LBIG)THEN
  !## open idf
  J=INDEXNOCASE(IDFNAME,'.',.TRUE.)-1
  IF(IDFOPEN(IDF(1)%IU,IDFNAME(:J)//'.IDF','WO',IDF(1)%ITYPE,1,IQUESTION=1).AND. &
...
!0.1E+31 0.1E+31 0.1E+31 0.1E+31 0.1E+31  
 ...
 CHARACTER(LEN=5),ALLOCATABLE,DIMENSION(:) :: TI
 CHARACTER(LEN=256) :: LINE
 ...
 !###======================================================================
 LOGICAL FUNCTION ASC2IDF_INT_MAIN(IFILE,XMIN,YMIN,XMAX,YMAX,IN_TYPE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IFILE,IN_TYPE
 REAL(KIND=DP_KIND),INTENT(IN) :: XMIN,YMIN,XMAX,YMAX
 REAL(KIND=DP_KIND) :: BO_VALUE
 REAL(KIND=DP_KIND),DIMENSION(1) :: ZPERC
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: RC,RR,ZV
...
! IF(INDEX(UTL_CAP(XYZFNAMES(IFILE),'U'),'.IFF',.TRUE.).GT.0)IIPF=5
 ...
 IF(SIZE(BLNFILE).GT.0.AND. &
    IBLNTYPE.EQ.0.AND.          &
    IGRIDFUNC.EQ.8)CALL ASC2IDF_HFB(IDF(1),IDF(1)%NROW,IDF(1)%NCOL,IPC,BLNFILE,FCTBLNFILE,0)
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  CASE (4)
   ...
   M=INT(N); !call sortem(1,M,RR,2,RC,ZV,(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/))
   CALL QKSORT(M,RR,V2=RC,V3=ZV)
   ...
   DO
    ...
    IF(M.GT.0)THEN
     ...
     !call sortem(1,M,RC(I:),2,RR(I:),ZV(I:),(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/),(/0.0D0/))
     CALL QKSORT(M,RC(I:),V2=RR(I:),V3=ZV(I:))
  ...
  CASE (-6,6)
   ...
   IF(ELLIPS_IDF(1)%FNAME.EQ.'')THEN
    CALL KRIGING_MAIN(SIZE(XP),XP,YP,ZP,IDF(1),IDF(4),1,BO_VALUE)
   ELSE
    CALL KRIGING_MAIN(SIZE(XP),XP,YP,ZP,IDF(1),IDF(4),1,BO_VALUE,ELLIPS_IDF)
   ENDIF
 ...
 IF(IGRIDFUNC.NE.7)THEN
  ...
  IF(ILOG.EQ.1)THEN
   DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
    IF(IDF(1)%X(ICOL,IROW).GT.0.0D0)IDF(1)%X(ICOL,IROW)=EXP(IDF(1)%X(ICOL,IROW))
   ENDDO; ENDDO
  ENDIF
 ...
 REAL(KIND=DP_KIND) :: X,Y,Z,X1,X2,Y1,Y2,ZV,NCOUNT
 LOGICAL :: LEX,LT,LB
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (1)
   ...
   DO
    ...
    XP(NP)=X; YP(NP)=Y; ZP(NP)=Z; WP(NP)=1.0D0
   ENDDO
  ...
  CASE (3)
   ...
   NP=0; DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
    IF(IDF(2)%X(ICOL,IROW).NE.IDF(2)%NODATA)THEN 
     NP=NP+1; CALL IDFGETLOC(IDF(2),IROW,ICOL,XP(NP),YP(NP)); ZP(NP)=IDF(2)%X(ICOL,IROW); WP(NP)=1.0D0
    ENDIF
   ENDDO; ENDDO
