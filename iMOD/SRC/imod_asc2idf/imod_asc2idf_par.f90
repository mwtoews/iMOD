...
MODULE MOD_ASC2IDF_PAR
 ...
 INTEGER :: IGRIDFUNC,IXCOL,IYCOL,IZCOL,IPCOL,IWCOL,ASSF_INDICATOR,ASSF_NTHRESHOLD,IWFACTOR,  &
       ASSF_COLUMN,ASSF_STARTDATE,ASSF_ENDDATE,ASSF_DDATE,ASSF_IDEPTH,IINT_IDF,TRIMCONF_IDF
 ...
 REAL(KIND=DP_KIND) :: CS,NODATA,PERCENTILE,ASSF_TOP,ASSF_BOT,ASSF_DZ,ASSF_ZPLUS
 CHARACTER(LEN=256),ALLOCATABLE,DIMENSION(:) :: XYZFNAMES
 TYPE(IDFOBJ) :: ZONE_IDF
 TYPE(IDFOBJ),DIMENSION(2) :: TRIMDEPTH_IDF
 TYPE(IDFOBJ),DIMENSION(3,2) :: ELLIPS_IDF  !## three axis with range and angle,
 TYPE(IDFOBJ),DIMENSION(:),ALLOCATABLE :: INT_IDF
 
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: XP,XP_DUMMY,YP,YP_DUMMY,ZP,ZP_DUMMY,PP,PP_DUMMY,WP,WP_DUMMY,FP,FP_DUMMY
