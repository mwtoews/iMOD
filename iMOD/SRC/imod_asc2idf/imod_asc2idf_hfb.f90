...
CONTAINS
 ...
 !###====================================================================
 SUBROUTINE ASC2IDF_HFB(IDF,NROW,NCOL,IPC,FNAME,FCTFNAME,ITB,TOP,BOT)
 !###====================================================================
 IMPLICIT NONE
 ...
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(IN) :: FCTFNAME
 REAL(KIND=DP_KIND) :: X1,Y1,X2,Y2,Z1,Z2,TL,DZ,NODATA,ZL,ZF
 INTEGER :: IU,II,I,J,IOS,N,IROW,ICOL,ILINE,NP,IC1,IC2,IR1,IR2,IP1,IP2,IL1,IL2
 ...
 DO II=1,SIZE(FNAME)
  ...
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=FNAME(II),ACTION='READ',STATUS='OLD',FORM='FORMATTED')
  IF(IU.EQ.0)THEN; WRITE(*,'(A)') 'Error opening '//TRIM(FNAME(II)); RETURN; ENDIF
  ...
  IF(ITB.EQ.0)THEN
   ...
   DO IROW=IR1,IR2; DO ICOL=IC1,IC2
    IF(TOP%X(ICOL,IROW).EQ.NODATA)THEN
     TOP%X(ICOL,IROW)=ZF/100.0D0
     BOT%X(ICOL,IROW)=ZF/100.0D0
    ELSE
