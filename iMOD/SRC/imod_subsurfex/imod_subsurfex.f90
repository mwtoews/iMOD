...
CONTAINS
  ...
  SELECT CASE (ITYPE) 
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDHELP)
      CALL UTL_GETHELP('ssexplorer','VMO.SubsurfExpl')
     CASE (IDZOOMIN)
    ...
    SELECT CASE(MESSAGE%VALUE1)
     CASE(1)
      IF(MOVE .EQ. .TRUE.)THEN
       OLDX=DBLE(MESSAGE%GX); OLDY=DBLE(MESSAGE%GY)
      ...
      ELSEIF(DRAW .EQ. .TRUE.)THEN
       ...
       ELSEIF(CLICKS .EQ. 2)THEN
        NEWX=DBLE(MESSAGE%GX); NEWY=DBLE(MESSAGE%GY)
       ...
       END IF  
      ELSE
       CALL SUBSURFEXSELECTCELL(REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8))      
      END IF
     CASE(3)
   ...
   CASE (MOUSEBUTUP)
    IF(MOVE .EQ. .TRUE.)THEN
     MOVE=.FALSE.; DRAW=.FALSE.
     CALL SUBSURFEXMOVE(REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8))
     MOVE=.TRUE.    
     CALL WCURSORSHAPE(ID_CURSORHAND)
    END IF
   CASE (MOUSEMOVE)
     CALL SUBSURFEXPOSITION(REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8))
  END SELECT
 ...
 CALL DBL_IGRUNITS(XMIN,YMIN,XMAX,YMAX) 
 CALL IGRAREACLEAR()
 ...
 !###====================================================================== 
 IMPLICIT NONE
 INTEGER ::  A,I,J,K,N,M,IU,JU,NLINES,NCOL,EXISTS,IOS,COUNTER,INDX,LEGEND
 INTEGER,ALLOCATABLE,DIMENSION(:) :: PREVLOADED
 ...
 CHARACTER(LEN=256),ALLOCATABLE,DIMENSION(:) :: OUTPUT,IDFNAMES_TEMP,ALLIDFNAMES
 CHARACTER(LEN=256),DIMENSION(:),POINTER :: IDFNAMES_POINTER,DEL_IDFNAMES
 LOGICAL :: EX,EX2
 ...
 DO I=1,NPARAM
  ...
  INQUIRE(FILE=TRIM(PREFVAL(1))//'\SUBSURFACE_EXPLORER\'//TRIM(PROJECT)//'\'&
   ...
   CALL MANAGER_UTL_ADDFILE(TRIM(PREFVAL(1))//'\SUBSURFACE_EXPLORER\'//TRIM(PROJECT)//'\'//TRIM(PARAM(I))//'\'&
                //TRIM(PARAM(I))//'.IPF',LDEACTIVATE=.FALSE.)            
  ...
  END IF 
   ...
   !## import legend if it is missing
    ...
    CALL MANAGER_UTL_ADDFILE(TRIM(PREFVAL(1))//'\SUBSURFACE_EXPLORER\'//TRIM(PROJECT)//'\'//TRIM(PARAM(I))//'\'&
                 ...
                 //TRIM(PARAM(I))//'\'//TRIM(PARAM(I))//'.LEG',LDEACTIVATE=.FALSE.)                                    
   
   CASE(0)
    CALL MANAGER_UTL_ADDFILE(TRIM(PREFVAL(1))//'\SUBSURFACE_EXPLORER\'//TRIM(PROJECT)//'\'//TRIM(PARAM(I))//'\'&
                 //TRIM(PARAM(I))//'.MDF',LDEACTIVATE=.FALSE.)   
   END SELECT
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IU,LINEWIDTH,FINDCELLS,IOS
 CHARACTER(LEN=256) :: GENFILE
 CHARACTER(LEN=32) :: XMINSTRING,XMAXSTRING,YMINSTRING,YMAXSTRING
...
! CALL UTL_IDFCRDCOR(XMIN,YMIN,XMAX,YMAX,WIDTH,HEIGTH)
 ...
 IMPLICIT NONE 
 INTEGER :: I,A,B
 REAL(KIND=DP_KIND) :: FROMX,FROMY,TOX,TOY
 ...
 DO I=1,A+1
  CALL DBL_IGRJOIN(FROMX,FROMY,TOX,TOY)
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: X1,X2,Y1,Y2,DX,DY 
 ...
 DO I=Y1,Y2,INT(GRIDSIZE)
  DO J=X1,X2,INT(GRIDSIZE)
   ...
   IF(X .GT. J .AND. X .LT. J+INT(GRIDSIZE) .AND. Y .GT. I.AND. Y .LT. I+INT(GRIDSIZE)) THEN
    ...
    CALL IGRLINEWIDTH(3)
    CALL DBL_IGRRECTANGLE(J*1.0D0,I*1.0D0,J*1.0D0+GRIDSIZE,I*1.0D0+GRIDSIZE)
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IU,JU,KU,IOS,I,J,K,N,NPOINTS,LINEWIDTH,FINDCELLS,COUNTER,OLDNROFCELLS,LOCALNROFCELLS,X1,X2,Y1,Y2,EXISTS,NROW,NCOL
 INTEGER,ALLOCATABLE,DIMENSION(:) :: CELL,OLDCELLS
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: POLYGONX,POLYGONY,POINTSX,POINTSY
 REAL(KIND=DP_KIND) :: FROMX,FROMY,TOX,TOY,X,Y,MAXVALUE
 ...
 DO
  ...
  IF(FINDCELLS .EQ. 1) THEN !## find which cells the polygon contains
   ...
   X=INIXMIN+0.5D0*(GRIDSIZE/10)
   Y=INIYMIN+0.5D0*(GRIDSIZE/10)
   DO I=1,NROW*10
    DO J=1,NCOL*10
     COUNTER=COUNTER+1
   ...
   N=SIZE(POLYGONX)
   DO I=1,SIZE(POINTSX)
     IF(DBL_IGRINSIDEPOLYGON(POINTSX(I),POINTSY(I),POLYGONX,POLYGONY,N).EQ.1)THEN
     
     ELSE
      POINTSX(I)=0.0D0; POINTSY(I)=0.0D0
     END IF
   END DO
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: NEWX,NEWY,XDIFF,YDIFF,X1,X2,Y1,Y2
...
!###======================================================================
 ...
 INQUIRE(FILE=GENFILE,EXIST=EX) 
  ...
  IF(EX .EQ. .FALSE.)THEN
   ...
   OPEN(UNIT=IU,FILE=GENFILE,ACTION='WRITE',FORM='FORMATTED')
   WRITE(IU,*) '1,SHAPE1'
   WRITE(IU,'(F15.3,1X,F15.3)') FROMX,FROMY
   WRITE(IU,'(F15.3,1X,F15.3)') TOX,TOY
   CLOSE(IU)
  ELSE
   IU=UTL_GETUNIT()
   OPEN(UNIT=IU,FILE=GENFILE,ACTION='READ',FORM='FORMATTED')
   ...
   CLOSE(IU,STATUS='DELETE') 
   IF(BUTTON .EQ. 1)THEN
    WRITE(JU,'(F15.3,1X,F15.3)') TOX,TOY  
   ELSEIF(BUTTON .EQ. 3)THEN
    WRITE(JU,'(F15.3,1X,F15.3)') FIRSTX,FIRSTY
    WRITE(JU,*) 'END'
    WRITE(JU,*) 'END'   
   END IF
   CLOSE(JU)
 ...
 !## plot gen file and find corresponding cells    
  IF(BUTTON .EQ. 3)THEN
   CALL DBL_IGRJOIN(FIRSTX,FIRSTY,TOX,TOY)  
 ...
 DO
  ...
  END DO
  N=N+1
  VERTICESX(N)=99999.00D0
  VERTICESY(N)=99999.00D0  
 END DO
 CLOSE(IU)
 END IF
 ...
 DO I=1,SIZE(VERTICESX)-1
  IF(VERTICESX(I) .EQ. 99999.00D0 .OR. VERTICESX(I+1) .EQ. 99999.00D0) CYCLE
  CALL DBL_IGRJOIN(VERTICESX(I),VERTICESY(I),VERTICESX(I+1),VERTICESY(I+1))
 END DO
