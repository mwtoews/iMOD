...
REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: GEF
CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:,:) :: GEF2,GEFEX
CHARACTER(LEN=256) :: GEFDIR,IPFFNAME,GENFNAME
CHARACTER(LEN=50) :: CID
REAL(KIND=DP_KIND) :: X,Y,ZEND,Z
INTEGER :: N,NCOL,NROW,IU,JU
REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: FMULT
INTEGER,DIMENSION(:),ALLOCATABLE :: NCOLLINE
...
CONTAINS
 ...
 N=MAX(NCOLIPF,NCOL)
 ALLOCATE(ATTRIB1(N),ATTRIB2(N),IATTRIB(NCOLIPF),NODATA(N),FMULT(NCOLIPF))
 ...
 ALLOCATE(NCOLLINE(NROW)); NCOLLINE=0
 ...
 DO IROW=1,NROW
  ...
  IF(IOS.NE.0)THEN
   ...
   CLOSE(IU); RETURN
  ENDIF
  !## GEF2=dummy array for storing whole row into an array and extract only the character part later)  
  ALLOCATE(GEF2(NCOLLINE(IROW),NROW)) 
 ...
 IF(ALLOCATED(NCOLLINE))DEALLOCATE(NCOLLINE)
