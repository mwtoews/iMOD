   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (IDOK)
      ...
      IF(I.EQ.1)CALL GEF2IPF_GEFCPT(IBATCH)
      IF(I.EQ.2)CALL GEF2IPF_GEFBORE(IBATCH)
      EXIT
     CASE (IDHELP)
    END SELECT
  END SELECT
 ...
 DO I=1,SIZE(GEFNAMES)
  ...
  IF(LREADGEF_BORE(I))THEN
   ...
   DO ICOL=1,MAXVAL(NCOLLINE)
    ...
    IF(ICOL.GE.NCOL+1)THEN
     IF(ICOL.EQ.NCOL+1)THEN
      IF(INDEX(UTL_CAP(TRIM(ATTRIB2(ICOL)),'U'),'LITOGRAPHY').GT.0)IATTRIB(10)=ICOL
     ELSE
      IF(INDEX(UTL_CAP(TRIM(ATTRIB2(ICOL)),'U'),'EXTRAVARIABLE').GT.0)IATTRIB(ICOL)=ICOL
      WRITE(*,*) ICOL
     ENDIF
    ENDIF
