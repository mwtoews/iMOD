...
CONTAINS
 ...
 I=INDEX(IPFFNAME,'.',.TRUE.)-1; IPFDIR=IPFFNAME(:I); CALL IOSDIRMAKE(TRIM(IPFDIR))

 I=INDEX(IPFFNAME,'\',.TRUE.)+1
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(IPFDIR)//'\'//IPFFNAME(I:),STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')

 PREFVAL(1)='.'
 WC=GEFDIR(INDEX(GEFDIR,'\',.TRUE.)+1:)
 CALL IOSDIRMAKE(TRIM(PREFVAL(1))//'\TMP'); CALL IOSDIRENTRYTYPE('F'); CALL IOSDIRCOUNT(GEFDIR(:INDEX(GEFDIR,'\',.TRUE.)-1),WC,N)

 IF(N.EQ.0)STOP 'No gef files found!'
 ...
 CALL UTL_DIRINFO(GEFDIR(:INDEX(GEFDIR,'\',.TRUE.)-1),WC,GEFNAMES,N,'F')
 ...
 DO I=1,N
  ...
  IF(LREADGEF(I))THEN
   ...
   IF(IATTRIB(1).EQ.0)THEN !.OR.(IATTRIB(2).EQ.0.AND.IATTRIB(3).EQ.0))THEN
   ...
   ELSE
    ...
    DO ICOL=1,NCOLIPF 
!     IF(IATTRIB(ICOL).EQ.0)THEN
     IF(ICOL.EQ.1)LINE='"m,NAP",' !Sondeerlengte",'
     ...
     ENDIF
!     ELSE
!      LINE='"'//TRIM(ATTRIB1(IATTRIB(ICOL)))//','//TRIM(ATTRIB2(IATTRIB(ICOL)))//'",'//TRIM(RTOS(NODATA(IATTRIB(ICOL)),'F',3))
!     ENDIF
     WRITE(KU,*) TRIM(LINE)
    END DO
