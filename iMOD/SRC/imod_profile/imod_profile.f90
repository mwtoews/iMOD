...
CONTAINS
 ...
 DO I=1,NSCREEN
  IF(I.EQ.1)THEN
   ...
   CALL WINDOWOPENCHILD(IWINPROFILE(I),FLAGS=SYSMENUON+MINBUTTON+MAXBUTTON+INSIDEPARENT+MAXWINDOW+STATUSBAR, &
                        TOOLID=TMPINT3,TITLE='iMOD Cross-Section CHILD window')
  ELSE
   !## create new window, floated
   CALL WINDOWOPENCHILD(IWINPROFILE(I),FLAGS=SYSMENUON+MINBUTTON+MAXBUTTON+STATUSBAR, & !+OWNEDBYPARENT, &
                        TOOLID=TMPINT3,TITLE='iMOD Cross-Section FLOATING window', &
                        WIDTH=INT(MPW%DIX/1.5),HEIGHT=INT(MPW%DIY/1.5))
  ENDIF
  TMPINT1=[3000,-1]; TMPINT2=[1,1]
  CALL WINDOWSTATUSBARPARTS(2,TMPINT1,TMPINT2)
 ...
 IF(LEX)THEN
  ...
  DO
   ...
   CALL PROFILE_MAIN_MESSAGE(IEXIT) 
   IF(IEXIT.EQ.1)EXIT
  ...
  CASE(PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_DELETE)
     ...
     IF(SOLID_PROFILEDELETE(ID_DSERIESTAB2,NSPF,ISEL))THEN
      CALL SOLID_PROFILEUPDATECROSS(1,0)    
     ENDIF
     DEALLOCATE(ISEL)
...
!   CALL WGRIDCOLOURCELL(IDF_GRID1,1,I,-1,PROFIDF(IPOS(I))%SCOLOR)
 ...
 CALL IGRLINETYPE(SOLIDLINE); CALL IGRFILLPATTERN(SOLID); CALL IGRLINEWIDTH(LINETHICKNESS)
       ...
       IF(SERIE(IIDF-1)%Y(I).NE.SERIE(IIDF+1)%Y(I))THEN
        ...
        IF(I.GT.1)THEN
         IF(SERIE(IIDF-1)%Y(I-1).NE.PROFIDF(IIDF-1)%IDF%NODATA)THEN
          X1=(SERIE(IIDF-1)%X(I-1)+SERIE(IIDF-1)%X(I))/2.0D0
         ENDIF
        ENDIF
        !## end rectangle
        X2=SERIE(IIDF-1)%X(I)
        IF(I.LT.SERIE(IIDF)%N)THEN   
         IF(SERIE(IIDF-1)%Y(I+1).NE.PROFIDF(IIDF-1)%IDF%NODATA)THEN
          X2=(SERIE(IIDF-1)%X(I)+SERIE(IIDF-1)%X(I+1))/2.0D0
         ENDIF
        ENDIF     
    ...
    DO I=1,SERIE(IIDF)%N 
     ...
     IF(SERIE(IIDF)%Y(I).NE.PROFIDF(IIDF)%IDF%NODATA)THEN
      ...
      IF(LEX)THEN
       CALL DBL_IGRMOVETO(SERIE(IIDF)%X(I),SERIE(IIDF)%Y(I)); LEX=.FALSE.
      ELSE
       CALL DBL_IGRLINETO((SERIE(IIDF)%X(I)+SERIE(IIDF)%X(I-1))/2.0D0,SERIE(IIDF)%Y(I-1))
       CALL DBL_IGRLINETO((SERIE(IIDF)%X(I)+SERIE(IIDF)%X(I-1))/2.0D0,SERIE(IIDF)%Y(I))
      ENDIF
     ELSE
   ...
   IF(IBLOCKFILLS.EQ.0)THEN
    ...
    DO I=1,SERIE(IIDF)%N 
     ...
     IF(SERIE(IIDF)%Y(I).NE.PROFIDF(IIDF)%IDF%NODATA)THEN
      J=J+1
      IF(J.GE.2)THEN
       XPOL(1)=SERIE(IIDF)%X(I-1)
       YPOL(1)=SERIE(IIDF)%Y(I-1)
       XPOL(2)=SERIE(IIDF)%X(I)
       YPOL(2)=SERIE(IIDF)%Y(I)
       XPOL(3)=SERIE(IIDF)%X(I)
       XPOL(4)=SERIE(IIDF)%X(I-1)
       CALL DBL_IGRPOLYGONCOMPLEX(XPOL,YPOL,4)
      ENDIF
     ENDIF
    ENDDO
    ...
    DO I=1,SERIE(IIDF)%N 
     IF(SERIE(IIDF)%Y(I).NE.PROFIDF(IIDF)%IDF%NODATA)THEN
      ...
      IF(I.EQ.1)THEN
       X1= SERIE(IIDF)%X(I)
       X2=(SERIE(IIDF)%X(I)+SERIE(IIDF)%X(I+1))/2.0D0
       IF(X1.NE.X2)CALL DBL_IGRRECTANGLE(X1,Y1,X2,SERIE(IIDF)%Y(I))   
      ELSEIF(I.EQ.SERIE(IIDF)%N)THEN 
       X1=(SERIE(IIDF)%X(I)+SERIE(IIDF)%X(I-1))/2.0D0
       X2= SERIE(IIDF)%X(I)
       IF(X1.NE.X2)CALL DBL_IGRRECTANGLE(X1,Y1,X2,SERIE(IIDF)%Y(I))   
  ...
  IF(KU(I).GT.0)THEN
   IF(MP(KPLOT(I))%PRFTYPE.GT.0)THEN
    ...
    DO J=1,NXY-1
     ...
     !## compute offset
     IF(J.GT.1)THEN
      DX=DX+UTL_DIST(XY(1,J-1),XY(2,J-1),XY(1,J),XY(2,J))
...
!      IF(DXY.GT.0.0D0)DX=DX+SQRT(DXY)
     ...
     XTMP(1,2)=XY(1,J+1)
     XTMP(2,2)=XY(2,J+1)
     CALL IFFPLOT(KU(I),XMN,XMX,YMN,YMX,KPLOT(I),XTMP,DX)
!     CALL IFFPLOT(KU(I),XMN,XMX,YMN,YMX,KPLOT(I),XY(:,J:J+1),DX)
 ...
 REAL(KIND=DP_KIND),DIMENSION(4,2) :: XYPOL
 REAL(KIND=DP_KIND),DIMENSION(2,2) :: XYTMP
 ...
 DO I=1,NXY-1
  ...
  DO IIPF=1,NIPF
   IF(MP(MPLOT(IIPF))%PRFTYPE.EQ.1.AND. &
    ...
    XYTMP=XY(:,I:I+1)
    CALL IPFPLOT(IIPF,XMN,YMN,XMX,YMX,MPLOT(IIPF),XYTMP,DX,.TRUE.)
   ENDIF
  ENDDO
 ...
 DO IIPF=1,NIPF
  IF(MP(MPLOT(IIPF))%PRFTYPE.EQ.1.AND. &
   ...
   XYTMP=1.0D0
   CALL IPFPLOT(IIPF,0.0D0,0.0D0,0.0D0,0.0D0,MPLOT(IIPF),XYTMP,0.0D0,.FALSE.)
  ENDIF
 ...
 IF(IDZ.EQ.ID_ZOOMIN.OR.IDZ.EQ.ID_ZOOMOUT)THEN
  ...
  XMAX=XC2+0.5D0*DX*FZ; XMIN=XC2-0.5D0*DX*FZ
  YMIN=YC2-0.5D0*DY*FZ; YMAX=YC2+0.5D0*DY*FZ
 ...
 IF(NIPF.GT.0)THEN
  ...
  END DO
  ALLOCATE(TMPNAME(NIPF))
  TMPNAME=MP(MPLOT)%IDFNAME(J:)
  CALL WDIALOGPUTMENU(IDF_MENU1,TMPNAME,NIPF,1)
   ...
   SELECT CASE (MESSAGE%VALUE2)
    ...
    CASE (IDF_GRID1)
     ...
     IF(ICOL.EQ.4.AND.MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
      ...
      IF(IRGB.LE.0)IRGB=0; CALL WSELECTCOLOUR(IRGB)
      IF(WINFODIALOG(4).EQ.1)THEN
     ...
     SELECT CASE (ICONFIG)
      ...
      CASE(2,3,4)
       ...
       DO I=1,MXNIDF
        ...
        IF(I.EQ.MXNIDF)ICLR=WRGB(255,255,255) !195,195,195)
        CALL WGRIDPUTCELLINTEGER( IDF_GRID1,4 ,I+1,ICLR) !## colour
        CALL WGRIDCOLOURCELL(     IDF_GRID1,4 ,I+1,ICLR,ICLR)
        CALL WGRIDPUTCELLCHECKBOX(IDF_GRID1,1 ,I+1,1)    !## iact
