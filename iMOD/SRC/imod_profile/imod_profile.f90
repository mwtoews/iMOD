...
!INTEGER,PRIVATE :: LINEWIDTHPLOT=3
!INTEGER,PRIVATE :: LINECOLORPLOT=WRGB(255,0,0)
!INTEGER,PRIVATE :: ICLRRASTER=WRGB(191,191,191)
!INTEGER,PRIVATE :: ICLRKNIKCP=WRGB(255,0,0)
!INTEGER,PRIVATE :: ICLRVIEWAR=INVERSECOLOUR(WRGB(255,0,0))
...
CONTAINS
 ...
 !## allready loaded from solid
 IF(ISOLID.EQ.0)CALL WDIALOGLOAD(ID_DSERIES,ID_DSERIES)
 ...
 CALL IPFGETVALUE_PLOTCOLOURS(ID_DSERIESPROPTAB6)
...
!   CALL IGRCOLOURN(WRGB(0,0,0))
 ...
 SELECT CASE (MESSAGE%VALUE2)
  ...
  CASE (IDF_CHECK1)
   IF(MESSAGE%WIN.EQ.ID_DSERIESPROPTAB3)THEN
   ...
   ELSEIF(MESSAGE%WIN.EQ.ID_DSERIESPROPTAB5)THEN
    CALL WDIALOGGETMENU(IDF_MENU1,J)
    CALL WDIALOGGETCHECKBOX(IDF_CHECK1,I)
    CALL WDIALOGFIELDSTATE(IDF_MENU2,I)
    IF(I.EQ.1)THEN
  ...
  CASE (IDF_GRID1)
   CALL WDIALOGSELECT(ID_DSERIESPROP)
   CALL WDIALOGGETTAB(ID_DSERIESPROPTAB,ITAB)
   IF(ITAB.EQ.ID_DSERIESPROPTAB1)THEN
    CALL WDIALOGSELECT(ID_DSERIESPROPTAB1)
   ...
   ELSEIF(ITAB.EQ.ID_DSERIESPROPTAB5)THEN

   ELSEIF(ITAB.EQ.ID_DSERIESPROPTAB6)THEN
    CALL WDIALOGSELECT(ID_DSERIESPROPTAB6)
    CALL WGRIDPOS(MESSAGE%Y,ICOL,IROW)
    IF(ICOL.EQ.2)THEN
     ...
     IF(WINFODIALOG(4).EQ.1)THEN
      ...
      CALL IPFGETVALUE_GETCOLOURS(ID_DSERIESPROPTAB6)
      CALL IPFGETVALUE_PLOTCOLOURS(ID_DSERIESPROPTAB6)
      !## force another cell after colourselection
  ...
  CASE (ID_DSERIESPROPTAB6)
   SELECT CASE (MESSAGE%VALUE1)
    CASE (ID_OPEN,ID_SAVEAS)
     CALL IPFGETVALUE_OPENSAVECOLOURS('',MESSAGE%VALUE1,ID_DSERIESPROPTAB6)
   END SELECT
