...
USE MOD_IPF_PAR, ONLY : IPF,NIPF,NLITHO,BH,ICGRA,ICMUD,GRAPHPERCENTAGES
USE MOD_PROFILE_UTL
...
! CALL WDIALOGLOAD(ID_DIPFINFOSERIE,ID_DIPFINFOSERIE)
 ...
 CALL WDIALOGGETINTEGER(IDF_INTEGER1,ICGRA)
 CALL WDIALOGGETINTEGER(IDF_INTEGER2,ICMUD)
 ...
 SELECT CASE (MESSAGE%WIN)
  ...
  CASE (ID_DSERIESPROPTAB3)
   SELECT CASE (MESSAGE%VALUE2)
    ...
    CASE (IDF_CHECK10)
     CALL WDIALOGGETCHECKBOX(IDF_CHECK10,GRAPHPERCENTAGES)
     CALL WDIALOGFIELDSTATE(IDF_INTEGER1,GRAPHPERCENTAGES)
     CALL WDIALOGFIELDSTATE(IDF_INTEGER2,GRAPHPERCENTAGES)
   END SELECT
  ...
  CASE (ID_DSERIESPROP)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE(IDOK)
     ...
     CALL WDIALOGGETCHECKBOX(IDF_CHECK10,GRAPHPERCENTAGES)
     ICGRA=26; IF(GRAPHPERCENTAGES.EQ.1)CALL WDIALOGGETINTEGER(IDF_INTEGER1,ICGRA)
     ICMUD=28; IF(GRAPHPERCENTAGES.EQ.1)CALL WDIALOGGETINTEGER(IDF_INTEGER2,ICMUD)
     
     IEXIT=1
     IF(ICGRA.LE.0)THEN
      CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'The gravel column number you gave is smaller/equal to 0:'//CHAR(13)//'that cannot be correct. Make me larger please!','Error')
      IEXIT=0
     ELSEIF(ICMUD.LE.0)THEN
      CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'The mud column number you gave is smaller/equal to 0:'//CHAR(13)//'that cannot be correct. Make me larger please!','Error')
      IEXIT=0
     ELSEIF(ICGRA.GT.34)THEN
      CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Oops, the gravel column number you gave does not exist (>34).'//CHAR(13)//'Make me smaller please!','Error') 
      IEXIT=0
     ELSEIF(ICMUD.GT.34)THEN
      CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Oops, the mud column number you gave does not exist (>34).'//CHAR(13)//'Make me smaller please!','Error')              
      IEXIT=0
     ENDIF
