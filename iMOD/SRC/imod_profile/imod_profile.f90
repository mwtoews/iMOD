...
USE MOD_SOLID_PROFILE, ONLY : SOLIDPROFILEDRAW,SOLIDPROFILEMOUSE,SOLIDPROFILEADJUST,SOLIDPROFILEDELETE,SOLIDPROFILEFITDRILL, &
     ...
     SOLIDPROFILEDRAW_INTERSECTIONS,SOLIDPROFILESELECTNODES
  ...
  CASE(FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
    CASE (IDF_CHECK1)
     ...
     IF(ILOCK.EQ.1)THEN
      ILOCK=0; CALL WDIALOGPUTIMAGE(IDF_CHECK1,ID_ICONLOCK_DESELECT,1)
     ELSE
      ILOCK=1; CALL WDIALOGPUTIMAGE(IDF_CHECK1,ID_ICONLOCK_SELECT,1)
     ENDIF
    CASE (IDF_CHECK2)
     CALL WDIALOGSELECT(ID_DSERIESTAB2)
    ...
    CASE (IDF_CHECK3)
     ...
     IF(ISSNAP.EQ.1)THEN
      ISSNAP=0; CALL WDIALOGPUTIMAGE(IDF_CHECK3,ID_ICONSNAPPEN_DESELECT,1)
     ELSE
      ISSNAP=1; CALL WDIALOGPUTIMAGE(IDF_CHECK3,ID_ICONSNAPPEN_SELECT,1)
     ENDIF     
    CASE (IDF_CHECK4)
     CALL WDIALOGSELECT(ID_DSERIESTAB2)
...
!  IF(ISOLID.EQ.1)CALL WGRIDSTATECELL(IDF_GRID1,2,I+1,0)
 ...
 IF(NIPF.EQ.0)  CALL WDIALOGFIELDSTATE(IDF_CHECK6,0)
 IF(MXNIFF.EQ.0)CALL WDIALOGFIELDSTATE(IDF_CHECK7,0)
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,K,ITAB,IROW,ICOL,IRGB,IFIX,ICONFIG,ICLR,ILEG,IIPF
 LOGICAL :: LEX
...
!   IF(ISOLID.EQ.0)CALL WGRIDSTATECELL(IDF_GRID1,2,IROW,1)
 ...
 DO IPLOT=1,MXMPLOT
  ...
  ELSEIF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.2)THEN
   IIPF=IIPF+1
   MP(MPLOT(IIPF))%ILEGDLF=IPF(IIPF)%ILEGDLF  
   MP(MPLOT(IIPF))%ASSCOL1=IPF(IIPF)%ASSCOL1  
   MP(MPLOT(IIPF))%ASSCOL2=IPF(IIPF)%ASSCOL2  
  ENDIF
 ENDDO
