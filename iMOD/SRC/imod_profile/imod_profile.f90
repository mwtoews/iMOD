...
USE WINTERACTER
USE RESOURCE
USE MODPLOT, ONLY : MP,MPW,DRWLIST,MXMPLOT,MXCLR,MXCGRAD
USE MOD_PREF_PAR, ONLY : PREFVAL
USE IMODHELP
USE IMODVAR, ONLY : IDIAGERROR
USE MOD_UTL, ONLY : ITOS,RTOS,UTL_GETUNIT,CLOSEUNITS,UTL_HIDESHOWDIALOG,INVERSECOLOUR,UTL_DRAWLEGENDBOX,UTL_DIST,EQUALS_REAL,NEWLINE, &
...
USE MOD_LEGEND, ONLY : LEGMAIN,LEGCREATEINIT
USE MOD_LEGEND_UTL, ONLY : LEGREAD,LEGDEALLOCATE,LEGALLOCATE
...
USE MOD_SOLID_PROFILE, ONLY : SOLIDPROFILEDRAW,SOLIDPROFILEMOUSE,SOLIDPROFILEADJUST, &
     SOLIDPROFILEDELETE,SOLIDPROFILEFITDRILL, &
     SOLIDPROFILEDELNODE,SOLIDPROFILELINECOLOR,SOLIDPROFILELINETHICKNESS,SOLIDPROFILEMINMAX, &
     SOLIDPLOTLOCATIONCROSSSECTIONS,ILOCK,ISSNAP,IFIND,SOLIDPROFILEFIT,SOLIDPROFILEDRAW_POLYGON,SOLIDPROFILEDRAW_MASK, &
     SOLIDPROFILEDRAW_INTERSECTIONS 
...
CONTAINS
 ...
 !!## include cross-sections in bitmap, only whenever solids are active
 !IF(WMENUGETSTATE(ID_SOLIDS,2).EQ.1)CALL IDFPLOT(1)

 !CALL WDIALOGSELECT(ID_DSERIESTAB1)
 !DO I=1,10; CALL WGRIDSTATE(IDF_GRID1,I,1); END DO

 CALL WDIALOGSELECT(ID_DSERIESTAB3)
 ...
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,IBLOCKFILLS) !## blockfilles
 ...
 IF(ALLOCATED(IBITMAP))THEN
  ...
  DEALLOCATE(IBITMAP)
 ENDIF
 IF(PBITMAP%IACT)CALL WBITMAPDESTROY(PBITMAP%IBITMAP); PBITMAP%IACT=0
 ...
 DO
  ...
  IF(IQUICK.EQ.1)THEN
   ...
   IF(IEXIT.EQ.1)THEN
    ...
    IQUICK=0; ITYPE=0
   ENDIF
   !## message allready processed by quickview
  ...
  SELECT CASE (ITYPE)
   ...
   CASE (FIELDCHANGED)
    ...
    SELECT CASE (MESSAGE%WIN)
     CASE (ID_DSOLID_CROSS)
      CALL SOLIDPROFILEMAIN(ITYPE,MESSAGE,IEXIT)
     CASE DEFAULT
 ...
 !###======================================================================
 SUBROUTINE SOLIDPROFILEMAIN(ITYPE,MESSAGE,IEXIT)
 !###======================================================================
 IMPLICIT NONE
  ...
  CASE(FIELDCHANGED)
   SELECT CASE (MESSAGE%VALUE2)
    ...
    CASE (IDF_CHECK4)
     ...
     ISOLID=1
     CALL PROFILE_PLOT()
!     CALL SOLIDPROFILEINTERSECT()
!     CALL SOLIDPROFILEUPDATECROSS()
    CASE (IDF_MENU1)
     ISOLID=1
     CALL SOLIDPROFILEUPDATECROSS()    
   END SELECT
...
!     CALL PROFILE_STARTPROFILE(1) 
!     CALL SOLIDPROFILEUPDATECROSS()
     ISOLID=1
     CALL SOLIDPROFILEFIT(1)
     CALL SOLIDPROFILEUPDATECROSS()
    CASE (ID_DELETE)
     CALL SOLIDPROFILEDELETE()
     CALL SOLIDPROFILEUPDATECROSS()    
    !## fit cross-section to current cross-sectional lines
    CASE (ID_FIT)
     ISOLID=1
     CALL SOLIDPROFILEFIT(0)
     CALL SOLIDPROFILEUPDATECROSS()
    CASE (ID_FITDRILL)
     ISOLID=1
 ...
 END SUBROUTINE SOLIDPROFILEMAIN

 !###======================================================================
 SUBROUTINE SOLIDPROFILEUPDATECROSS()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I
 ...
 END SUBROUTINE SOLIDPROFILEUPDATECROSS
 ...
 SELECT CASE (MESSAGE%VALUE1)
  ...
  CASE (1)
   IF(LMOVEPROF)THEN
    ...
    CALL WCURSORSHAPE(CURARROW)
   ENDIF
   IF(MESSAGE%WIN.EQ.ID_DSERIES)THEN
...
!    ICURSOR_SOLID=0 !## clean cursor
!WRITE(*,*) 'butup:',IDOWN_SOLID
   ENDIF
 END SELECT
 ...
 SELECT CASE (MESSAGE%VALUE1)
  ...
  CASE (1)
   ...
   IF(MESSAGE%WIN.EQ.ID_DSERIES)THEN
    ...
    !## draw profile
    IF(IP.EQ.1)THEN !.AND.MESSAGE%WIN.EQ.ID_DSERIES)THEN
     IDOWN    =1
     NXY      =NXY+1
...
!      SERIE%N(1)=SERIE%N
    ...
    IF(NXY.GT.0.AND.IP.EQ.0)THEN !.AND.MESSAGE%WIN.EQ.ID_DSERIES)THEN
...
!      CALL PROFILE_PLOT()
      CALL PROFILE_ZOOM(ID_ZOOMFULL) !<--- verwijdert zodat fixx en fixy actief blijven!
...
!     CALL SOLIDPROFILEUPDATECROSS()
 ...
 IMPLICIT NONE
 INTEGER :: I,IWINID
 REAL :: CRITDIST,CRITPROF,DXY,X1,Y1,X2,Y2
 ...
 IF(MESSAGE%WIN.EQ.ID_DSERIES)THEN
  ...
  IF(IDOWN.EQ.1)THEN
   ...
   IF(LLINE)THEN
    ...
    CALL PROFILE_PLOTVIEWBOX(XY(1,NXY),XY(2,NXY),XCRD,YCRD)
    !## draw edges
    !CALL IGRRECTANGLE(XCRD-DXY,XCRD-DXY,XCRD+DXY,XCRD+DXY)
   ENDIF
...
!    X2=XCRD
!    Y2=YCRD
!    CALL PROFILE_PROFSPOTLINE(X1,X2,Y1,Y2)
  ...
  IF(I.LE.SIZE(IWINPROFILE))THEN
   ...
   IF(NXY.GT.0)THEN
    IF(IDOWN_SOLID.EQ.0)THEN 
     CALL SOLIDPROFILEMOUSE(MESSAGE%GX,MESSAGE%GY,ICURSOR_SOLID,ICRD_SOLID,IELEV_SOLID,I) 
    ...
    ELSE
     IF(IELEV_SOLID.NE.0)THEN
      ...
      DO IWINID=1,SIZE(IWINPROFILE)
       CALL SOLIDPROFILEDRAW(IELEV_SOLID,IELEV_SOLID,ICRD_SOLID,ICURSOR_SOLID,IWINID)
      ENDDO
      CALL SOLIDPROFILEADJUST(MESSAGE%GX,MESSAGE%GY,ICURSOR_SOLID,ICRD_SOLID,IELEV_SOLID,I) 
     ...
     IF(ICURSOR_BITMAP.NE.0)THEN
      CALL PROFILE_BACKGROUND_BITMAP_ADJUST(MESSAGE%XPIX,MESSAGE%YPIX,ICURSOR_BITMAP) !,ICRD_BITMAP)
      CALL PROFILE_PLOT()
     ENDIF
    ENDIF
   ENDIF
 ...
 CALL WDIALOGPUTSTRING(IDF_LABEL1,'Give a Label Name')
 ...
 !MESSAGE%GX,MESSAGE%GY
 CALL PROFILE_GETLOCATION(X,Y,XPOS,ISEG) !MESSAGE%GX,ISEG)

 !## check double points, can happen, change label and return
 DO I=1,NXY
  IF(EQUALS_REAL(XY(1,I),X).AND.EQUALS_REAL(XY(2,I),Y))THEN
 ...
 SELECT CASE (MESSAGE%WIN)
  ...
  CASE (ID_DSERIES)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    !## zoom
    CASE (ID_ZOOMINMAP,ID_ZOOMOUTMAP,ID_ZOOMFULLMAP)!,ID_ZOOMRECTANGLEMAP)
     CALL IDFZOOM(MESSAGE%VALUE1,(MPW%XMAX+MPW%XMIN)/2.0,(MPW%YMAX+MPW%YMIN)/2.0,ID_DSERIES) !IWINPROFILE(1))
     CALL PROFILE_REPLOT2D()
...
!     CALL IPFGETVALUE_QUICKVIEW(IWINPROFILE(1),1)
!
!     CALL UTL_HIDESHOWDIALOG(ID_DSERIES,2)
!     CALL WDIALOGLOAD(ID_DIPFINFOSERIE,ID_DIPFINFOSERIE)     
!     !## redraw cross-sections
...
!     GY        =0.0
 ...
 SELECT CASE (MESSAGE%VALUE2)
   ...
   CASE (ID_ZOOMRECTANGLEMAP)
    ...
    CALL IDFZOOM(MESSAGE%VALUE2,(MPW%XMAX+MPW%XMIN)/2.0,(MPW%YMAX+MPW%YMIN)/2.0,ID_DSERIES) !IWINPROFILE(1))
    CALL PROFILE_DIALOGFIELDS(MESSAGE%VALUE2,0,1)
    CALL PROFILE_REPLOT2D()
   !## move
   CASE(ID_MOVEMAP)
    CALL PROFILE_DIALOGFIELDS(MESSAGE%VALUE2,1,2)
    CALL IDFMOVE(ID_DSERIES)
    CALL PROFILE_DIALOGFIELDS(MESSAGE%VALUE2,0,1)
    CALL PROFILE_REPLOT2D()
  CASE(ID_MOVIE)
   CALL WDIALOGSELECT(ID_DSERIES)
...
!   CALL WINDOWSELECT(0)
  ...
  CASE (ID_COPY)
   DO I=1,SIZE(IWINPROFILE)
!    IF(MESSAGE%WIN.EQ.IWINPROFILE(I))CALL WBITMAPCLIPBOARD(IBITMAP(I))
    IF(MESSAGE%WIN.EQ.IWINPROFILE(I))CALL WCLIPBOARDPUTBITMAP(IBITMAP(I))
   ENDDO
  CASE (ID_DELNODE)
   CALL SOLIDPROFILEDELNODE(ICRD_SOLID,IELEV_SOLID)
   CALL PROFILE_PLOT()
  CASE (ID_LINECOLOR)
   CALL SOLIDPROFILELINECOLOR(IELEV_SOLID)
   CALL PROFILE_PLOT()
  CASE (ID_LTHICKNESS1,ID_LTHICKNESS2,ID_LTHICKNESS3)
   CALL SOLIDPROFILELINETHICKNESS(MESSAGE%VALUE1,IELEV_SOLID)
   CALL PROFILE_PLOT()
  CASE (ID_BITMAP)
   CALL PROFILE_BACKGROUND_BITMAP()
  CASE (ID_FLOATLEGEND)
   CALL PROFILE_LEGEND()
 ...
 CALL PROFILE_COMPVIEWBOX(X1,X2,Y1,Y2,XYPOL)
 CALL IGRLINEWIDTH(1)
 ...
 CALL IGRFILLPATTERN(OUTLINE)
 CALL IGRCOLOURN(WRGB(255,255,255))
 CALL IGRLINEWIDTH(1)!3)

 END SUBROUTINE PROFILE_PLOTVIEWBOX

!###======================================================================
 SUBROUTINE PROFILE_COMPVIEWBOX(X1,X2,Y1,Y2,XYPOL)
 !###======================================================================
 IMPLICIT NONE
 REAL,PARAMETER :: RAD=360.0/(2*3.1415)
 REAL,INTENT(IN) :: X1,X2,Y1,Y2
 ...
 END SUBROUTINE PROFILE_COMPVIEWBOX
...
!  CALL WMENUSETSTATE(ID_2DPLOT,1,I)
 
  IF(ID.EQ.0.AND.ABS(I-1).EQ.0)THEN !RETURN
!     ID.EQ.ID_2DPLOT.AND.ABS(I-1).EQ.0)THEN !RETURN
  ELSE
 ...
 !###======================================================================
 SUBROUTINE PROFILE_BACKGROUND_BITMAP()
 !###======================================================================
 IMPLICIT NONE
 
 !## window are synchronized, does not matter what menu-item to take
 CALL WINDOWSELECT(IWINPROFILE(1))
 IF(PBITMAP%IACT.EQ.0)THEN
  CALL WMENUSETSTATE(ID_BITMAP,2,1)
  PBITMAP%IACT=1
  CALL PROFILE_BACKGROUND_OPENBITMAP()
 ELSE
  CALL WBITMAPDESTROY(PBITMAP%IBITMAP)
  CALL WMENUSETSTATE(ID_BITMAP,2,0)
  PBITMAP%IACT=0
 ENDIF
 CALL PROFILE_PLOT()
 
 END SUBROUTINE PROFILE_BACKGROUND_BITMAP
...
!###======================================================================
 ...
 PBITMAP%NCOL =INFO(2)!INFO(2) Image width in pixels.
 PBITMAP%NROW =INFO(3)!INFO(3) Image height in pixels.
 PBITMAP%NCLR =INFO(4)!INFO(4) Number of colours.
 PBITMAP%COMPR=INFO(5)!INFO(5) Is file compressed ? 0 = no , 1 = yes.
 PBITMAP%CDEPT=INFO(6)!INFO(6) Colour depth in bits-per-pixel (1-32)
...
! RATIO=REAL(PBITMAP%NCOL)/REAL(PBITMAP%NROW)
! !## wider
...
! ENDIF  
 ...
 DX=10.0 !## pixels SQRT((MPW%XMAX-MPW%XMIN)**2.0+(MPW%YMAX-MPW%YMIN)**2.0)/500.0
 ...
 !###======================================================================
 SUBROUTINE PROFILE_BACKGROUND_BITMAP_ADJUST(IPX,IPY,CRDITYPE) !,ICRD)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPX,IPY
 INTEGER,INTENT(IN) :: CRDITYPE !,ICRD
 INTEGER :: IDX,IDY
 ...
 !## window are synchronized, does not matter what menu-item to take
 CALL WINDOWSELECT(IWINPROFILE(1))
! IF(ID.EQ.ID_2DPLOT.AND.WMENUGETSTATE(ID,2).EQ.1)THEN
!  CALL WMENUSETSTATE(ID,2,0)
!  IF(ALLOCATED(IPIPET))DEALLOCATE(IPIPET); NPIPET=0
!  CALL PROFILE_RESIZE()
!  RETURN
! ENDIF
 ...
 IF(ID.EQ.ID_SELECT)THEN !## plot all result in id_select mode
  ...
  ENDDO
! ELSEIF(ID.EQ.ID_2DPLOT)THEN !## plot only spotted ones in 2dplot mode
!  CALL WGRIDROWS(IDF_GRID1,NSCREEN) 
!  DO I=1,MXNIDF; IPOS(I)=I; ENDDO
!  N=MXNIDF
 ENDIF
 DEALLOCATE(ILIST)
...
!  IF(JD.EQ.ID_2DPLOT)CALL WMENUSETSTATE(JD,2,0)
...
!  CALL PROFILE_RESIZE()
 ...
 INTEGER :: I,J,ICOL,IROW,IZ,ISCREEN,ICLR,IZZ
 ...
 DO ISCREEN=NSCREEN,1,-1
  IF(NND(ISCREEN).GT.0)THEN
   ...
   END DO
   !## aborted due to inconsistency in top/bottoms or equivalents
!   IF(J.LE.N)IZ=0
   
   IF(JD.EQ.ID_SELECT)THEN
!    DO J=1,N; CALL WGRIDPUTCELLREAL(IDF_GRID1,2,J,ZIDF(J),'(G15.7)'); ENDDO
!   ELSEIF(JD.EQ.ID_2DPLOT)THEN
!    
!    IF(IZ.NE.0)THEN
!     ICLR=PROFIDF(IZ)%SCOLOR
!
!     !## check whether "filled in" option is active
!     IF(MINZ.GT.0.0)THEN    !## above
!      IZZ=MAX(1,IZ-1)
!     ELSE                   !## beneath
!      IZZ=MIN(N,IZ+1)
!     ENDIF
!
!     !## same graph-number
!     IF(PROFIDF(IZZ)%ISCREEN.EQ.PROFIDF(IZ)%ISCREEN)THEN
!      CALL UTL_FILLARRAY(IPRF,7,PROFIDF(IZZ)%PRFTYPE)
!      !## filled-in: take this idf to be plotted instead
!      IF(IPRF(5).EQ.1)THEN
!       IZ  =IZZ
!       ICLR=WRGB(200,0,0) !UTL_IDFGETCLASS(PROFIDF(IZ)%LEG,ZIDF(IZ))
!      ENDIF
!     ENDIF
!   
!     IPIPET(ISCREEN)=IZ
!     CALL PROFILE_PIPET_SEARCH_CLEAR(JD,ICLRIROW,ISCREEN)
!     CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,ISCREEN,PROFIDF(IPIPET(ISCREEN))%ALIAS)
!     CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,ISCREEN,TRIM(IDFTRANSFORM(PROFIDF(IPIPET(ISCREEN))%UNITS+1)))
!     CALL WGRIDPUTCELLREAL(IDF_GRID1,2,ISCREEN,ZIDF(IPIPET(ISCREEN)),'(G15.7)')
!    ELSE
!     CALL WDIALOGPUTSTRING(IDF_LABEL2,'')
!    ENDIF
   ELSE
 ...
 IF(JD.EQ.ID_SELECT)THEN
  ...
  ICLRIROW=0
! ELSEIF(JD.EQ.ID_2DPLOT)THEN
!  CALL WGRIDCLEARCELL(IDF_GRID1,2,JGRAPH)
 ENDIF
...
! AXES%IFIXY=0!IFIXY
 ...
 DO IWINID=1,SIZE(IWINPROFILE)
  ...
  IF(PBITMAP%IACT.EQ.1)THEN
   CALL WBITMAPPUT(PBITMAP%IBITMAP,1,1,PBITMAP%IX1,PBITMAP%IY1,PBITMAP%IX2,PBITMAP%IY2)
!   CALL IGRAREA(PBITMAP%XMIN,PBITMAP%YMIN,PBITMAP%XMAX,PBITMAP%YMAX)
!   CALL WBITMAPPUT(PBITMAP%IBITMAP,2,1)
!   CALL IGRAREA(GRAPHAREA(1,IWINID),GRAPHAREA(2,IWINID),GRAPHAREA(3,IWINID),GRAPHAREA(4,IWINID))
...
!   CALL PROFILE_PLOTAXES_VIEW(AXES,IWINID)
  ...
  IF(ISOLID.NE.0)THEN
   IF(ALLOCATED(SPF).AND.ISPF.GT.0)THEN
    CALL SOLIDPROFILEDRAW(1,SIZE(SPF(ISPF)%PROF),0,0,IWINID) !## 0 means all
   ENDIF
!  ENDIF

!  IF(ISOLID.NE.0)THEN
   !## plot intersections by others
   CALL SOLIDPROFILEDRAW_INTERSECTIONS(IWINID)
  ENDIF
 ...
 IF(ISOLID.NE.0)THEN
  ...
  CALL SOLIDPROFILEDRAW_MASK()  
  !## plot extend of polygon(s)
  CALL SOLIDPROFILEDRAW_POLYGON()  
 ENDIF
...
! IF(WMENUGETSTATE(ID_2DPLOT,2).EQ.0)RETURN
 !## plot selected idf's
 IF(NPIPET.EQ.0)RETURN !.NOT.ALLOCATED(IPIPET))RETURN
 ...
 CALL UTL_GETUNIQUE(XTMP,MXNIDF,NSCREEN,NODATA=-1.0)
 
 !## recompute igraphs
 DO I=1,MXNIDF
  DO J=1,NSCREEN
   IF(PROFIDF(I)%ISCREEN.EQ.INT(XTMP(J)))THEN
    PROFIDF(I)%ISCREEN=J 
    EXIT
   ENDIF
  END DO
...
! DY=DY/REAL(MXNIDF)
 ...
 DO I=1,MXNIDF
  ...
  CALL UTL_FILLARRAY(IPRF,7,PROFIDF(I)%PRFTYPE)
!  IF(IPRF(1).EQ.1.AND.IPRF(7).EQ.1.AND.SUM(IPRF(2:5)).GT.0.AND.PROFIDF(I)%ISCREEN.EQ.IWINID)THEN
  !## activated and plot legend switch on
  IF(IPRF(1).EQ.1.AND.IPRF(7).EQ.1.AND.PROFIDF(I)%ISCREEN.EQ.IWINID)THEN
...
!    ENDIF
 ...
 CALL WDIALOGSELECT(ID_DSERIESTAB5)
 DO I=1,NIPF
  CALL WGRIDGETCELLCHECKBOX(IDF_GRID1,5,I,J)
  IF(J.EQ.0)CYCLE
  ...
  DO J=1,NLITHO
   ...
   CALL UTL_DRAWLEGENDBOX(X1,Y1,X2,Y2,BH(J)%LITHOCLR,1,0,0) !## SOLID
   CALL IGRCOLOURN(WRGB(0,0,0)); CALL WGRTEXTSTRING(XT,YT,TRIM(BH(J)%LITHOTXT))  
 ...
 DO IIDF=1,MXNIDF
  ...
  IF(IPRF(1).EQ.0.OR.IWINID.NE.PROFIDF(IIDF)%ISCREEN)CYCLE
  ...
  ELSEIF(PROFIDF(IIDF)%IDF%ITB.EQ.1)THEN
   ...
   !## obsolute
!   DO I=1,SERIE(IIDF)%N-1
!    IF(SERIE(IIDF)%Y(I).NE.PROFIDF(IIDF)%IDF%NODATA)THEN
!     ICLR=UTL_IDFGETCLASS(PROFIDF(IIDF)%LEG,SERIE(IIDF)%Y(I))
!     CALL IGRCOLOURN(ICLR)
!     CALL IGRRECTANGLE(SERIE(IIDF)%X(I),PROFIDF(IIDF)%IDF%BOT,SERIE(IIDF)%X(I+1),PROFIDF(IIDF)%IDF%TOP)
!    ENDIF
!   ENDDO
  ...
  IF(KU(I).GT.0)THEN
   IF(MP(KPLOT(I))%PRFTYPE.GT.0)THEN
    ...
    DO J=1,NXY-1
     ...
     CALL PROFILE_GETVIEWBOX(XY(1,J),XY(2,J),XY(1,J+1),XY(2,J+1),XMN,YMN,XMX,YMX)
 ...
 !###======================================================================
 SUBROUTINE PROFILE_BITMAPIPF()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,NB,IB,ITX,IIPF
 REAL :: DX,DXY,X1,X2,XMN,YMN,XMX,YMX,DXB,MXB,X0,DXX
 ...
 DO IIPF=1,NIPF
  ...
  IF(MP(MPLOT(IIPF))%PRFTYPE.EQ.1.AND.MP(MPLOT(IIPF))%UNITS.EQ.1)IPF(IIPF)%IP=INT(4,1)!1,1)
 ENDDO
 ...
 DO I=1,NXY-1
  ...
  CALL PROFILE_GETVIEWBOX(XY(1,I),XY(2,I),XY(1,I+1),XY(2,I+1),XMN,YMN,XMX,YMX)
  ...
  !## determine for each line-segment what to draw!
  DO IIPF=1,NIPF
   IF(MP(MPLOT(IIPF))%PRFTYPE.EQ.1)CALL IPFPLOT(IIPF,XMN,YMN,XMX,YMX,MPLOT(IIPF),XY(:,I:I+1),DX,.TRUE.)
  ENDDO
 ...
 INTEGER :: JDOWN,IDCURSOR,I,IWINID,IDX,IDY,IIDX,IIDY
 REAL :: FZ,XC1,YC1,XC2,YC2,XC3,YC3,DX,DY,DXG,DYG
      ...
      IF(IDZ.EQ.ID_MOVE)THEN
       IF(JDOWN.EQ.1)THEN
        ...
        IF(PBITMAP%IACT.EQ.1)THEN
         ...
         CALL PROFILE_PLOT()
        ENDIF
!        CALL IGRRECTANGLE(BMPX1,BMPY1,BMPX2,BMPY2)
       ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IEXIT,IFIXX,IFIXY,IIPF,I,J
 ...
 IF(I.NE.IFIXX.OR.J.NE.IFIXY)CALL PROFILE_IDFMINMAX()
 CALL PROFILE_CLEAR()
 ...
 DO I=1,MXNIDF
  ...
  !## if solid tool used ... grey out screen option
 ...
 !## iff's
 CALL WDIALOGSELECT(ID_DSERIESTAB4)
 CALL WDIALOGCLEARFIELD(IDF_GRID1)
 ...
 DO I=1,MXNIFF
  ...
  IF(MP(KPLOT(I))%SCOLOR.EQ.0)THEN
   CALL WGRIDPUTCELLCHECKBOX(IDF_GRID1,2,I,0)
  ELSE
   CALL WGRIDPUTCELLCHECKBOX(IDF_GRID1,2,I,1)
  ENDIF
  J=INDEXNOCASE(MP(KPLOT(I))%IDFNAME,'\',.TRUE.)+1
  CALL WGRIDPUTCELLSTRING(IDF_GRID1,4,I,MP(KPLOT(I))%IDFNAME(J:))
 END DO
 IF(MXNIFF.GT.0)THEN
  ...
  END DO                                    !  12345678
  CALL WGRIDPUTMENU(IDF_GRID1,3,(/'IPART   ','ILAY    ','X-COORD.','Y-COORD.','Z-COORD.','TIME    ','VELOCITY'/),7,ILIST,MXNIFF)
  DEALLOCATE(ILIST)
 ENDIF
 ...
 IF(NIPF.GT.0)THEN
  ...
  DO IIPF=1,NIPF
   ...
   !MP(MPLOT(IIPF))%UNITS=1
   CALL WGRIDPUTCELLCHECKBOX(IDF_GRID1,3,IIPF,MP(MPLOT(IIPF))%UNITS)
   J=INDEXNOCASE(MP(MPLOT(IIPF))%IDFNAME,'\',.TRUE.)+1
   !## ipf-name
   CALL WGRIDPUTCELLSTRING(IDF_GRID1,4,IIPF,MP(MPLOT(IIPF))%IDFNAME(J:))
   I=IPF(IIPF)%ZCOL
   CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,IIPF,TRIM(IPF(IIPF)%ATTRIB(I)))
   CALL WGRIDPUTCELLCHECKBOX(IDF_GRID1,5,IIPF,1)  !MP(MPLOT(IIPF))%UNITS)
  END DO
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,K,ITAB,IROW,ICOL,IRGB,IFIX,ICONFIG,ICLR
 LOGICAL :: LEX
 ...
 SELECT CASE (MESSAGE%VALUE2)
  ...
  CASE (IDF_CHECK1)
   IF(MESSAGE%WIN.EQ.ID_DSERIESTAB3)THEN
...
!   CALL WDIALOGFIELDSTATE(IDF_REAL2,I)
  CASE (IDF_GRID1)
   CALL WDIALOGSELECT(ID_DSERIESPROP)
   CALL WDIALOGGETTAB(ID_DSERIESTAB,ITAB)
   IF(ITAB.EQ.ID_DSERIESTAB1)THEN
    CALL WDIALOGSELECT(ID_DSERIESTAB1)
...
!     !## copy settings from first row to all others!
!     SELECT CASE(ICOL)
!      CASE (2) !## igraph number/screen number
!       CALL WGRIDGETCELLINTEGER(IDF_GRID1,ICOL,1,J)
!       DO I=1,MXNIDF; CALL WGRIDPUTCELLINTEGER(IDF_GRID1,ICOL,I+1,J); ENDDO
!     END SELECT
   ...
   ELSEIF(ITAB.EQ.ID_DSERIESTAB5)THEN

   ELSEIF(ITAB.EQ.ID_DSERIESTAB6)THEN
    CALL WDIALOGSELECT(ID_DSERIESTAB6)
    CALL WGRIDPOS(MESSAGE%Y,ICOL,IROW)
    IF(ICOL.EQ.2)THEN
     ...
     IF(WINFODIALOG(4).EQ.1)THEN
      ...
      CALL IPFGETVALUE_GETCOLOURS(ID_DSERIESTAB6)
      CALL IPFGETVALUE_PLOTCOLOURS(ID_DSERIESTAB6)
      !## force another cell after colourselection
...
!###======================================================================
 ...
 !IF(MXNIDF.LE.2)THEN
! CALL WGRIDSTATECELL(IDF_GRID1,8,1+1,2)
! CALL WGRIDSTATECELL(IDF_GRID1,8,MXNIDF+1,2)
 ...
 IMPLICIT NONE
 INTEGER,INTENT(OUT) :: IEXIT
 INTEGER :: IU,IOS,I,J,IACT,IFIXX,IFIXY,IPN,IFL,ILN,ICL,IIPF,I1T,ILG,ICLR
 REAL :: XINT,YINT
  ...
  CASE (ID_DSERIESTAB6)
   SELECT CASE (MESSAGE%VALUE1)
    CASE (ID_OPEN,ID_SAVEAS)
     CALL IPFGETVALUE_OPENSAVECOLOURS('',MESSAGE%VALUE1,ID_DSERIESTAB6)
   END SELECT
...
!       !## puzzle (if neccessary), to become one single line, or at least one
!       GENFNAME=FNAME; CALL PUZZLEMAIN()
    ...
    CASE(IDOK)
     ...
     IF(MXNIFF.GT.0)THEN
      ...
      DO I=1,MXNIFF
       ...
       CALL WGRIDGETCELLCHECKBOX(IDF_GRID1,2,I,MP(KPLOT(I))%SCOLOR)
     ...
     DO IIPF=1,NIPF
      ...
      CALL WGRIDGETCELLCHECKBOX(IDF_GRID1,3,IIPF,MP(MPLOT(IIPF))%UNITS)   !## yes/no
...
!   IF(IEXIT.EQ.1)CALL PROFILE_CLEAR()
 ...
 !###======================================================================
 SUBROUTINE PROFILE_GETVIEWBOX(X1,Y1,X2,Y2,XMN,YMN,XMX,YMX)
 !###======================================================================
 IMPLICIT NONE
 REAL,INTENT(IN) :: X1,X2,Y1,Y2
 ...
 END SUBROUTINE PROFILE_GETVIEWBOX
 ...
 IF(PROFIDF(IIDF)%IDF%IEQ.EQ.0)THEN
  ...
  CALL INTERSECT_EQUI(PROFIDF(IIDF)%IDF%XMIN,PROFIDF(IIDF)%IDF%XMAX,PROFIDF(IIDF)%IDF%YMIN, &
                      PROFIDF(IIDF)%IDF%YMAX,PROFIDF(IIDF)%IDF%DX,X1ISG,X2ISG,Y1ISG,Y2ISG,N)
 ELSE
  ...
  CALL INTERSECT_NONEQUI(PROFIDF(IIDF)%IDF%SX,PROFIDF(IIDF)%IDF%SY,PROFIDF(IIDF)%IDF%NROW, &
                      PROFIDF(IIDF)%IDF%NCOL,X1ISG,X2ISG,Y1ISG,Y2ISG,N)
 ENDIF
 ...
 REAL :: Z,DXY,TLEN
 INTEGER :: IROW,ICOL,MX,I,J,ILEN,II,ISTEP
 ...
 IF(CODE.EQ.0)THEN
  IF(NXY.GT.0)THEN
   ...
   END DO
!   CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,IPLOTCOORDWITHLABELS) !## plotcoordinatesonlywithlabels
  ELSE
   CALL WDIALOGCLEARFIELD(IDF_GRID1)
  ENDIF
 ELSEIF(CODE.EQ.1)THEN
  ...
  END DO
!  XY=XY*1000.0
!  CALL WDIALOGGETCHECKBOX(IDF_CHECK2,IPLOTCOORDWITHLABELS) !## plotcoordinatesonlywithlabels
 ENDIF
 ...
 DO IIDF=1,MXNIDF
  ...
  IF(IPRF(1).EQ.1.AND.IPRF(5).EQ.0)THEN
   ...
   ELSEIF(PROFIDF(IIDF)%IDF%ITB.EQ.1)THEN
    IF(IFIXY.EQ.0)THEN
!     WRITE(*,*) YMIN,YMAX,PROFIDF(IIDF)%IDF%TOP,PROFIDF(IIDF)%IDF%BOT
...
!     WRITE(*,*) YMIN,YMAX,PROFIDF(IIDF)%IDF%TOP,PROFIDF(IIDF)%IDF%BOT
 ...
 ENDIF
 CALL SOLIDPROFILEMINMAX(IFIXX,IFIXY)
...
!      MDF(I)%LEG%NCLR  =PROFIDF(MXNIDF)%LEG%NCLR
!      MDF(I)%LEG%CGRAD =PROFIDF(MXNIDF)%LEG%CGRAD
!      MDF(I)%LEG%CLASS =PROFIDF(MXNIDF)%LEG%CLASS
!      MDF(I)%LEG%LEGTXT=PROFIDF(MXNIDF)%LEG%LEGTXT
!      MDF(I)%LEG%RGB   =PROFIDF(MXNIDF)%LEG%RGB
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IU,IIDF,I,J,IOS,IDELIM,IP,IB,JP,JB
 REAL :: XC,YC !,OLDXPOS
 CHARACTER(LEN=256) :: FNAME
 CHARACTER(LEN=3) :: EXT
 
 FNAME=TRIM(PREFVAL(1))//'\shapes\*.csv'
 IF(.NOT.UTL_WSELECTFILE('iMOD Comma-Separated File (*.csv)|*.csv|Postscript File (*.ps)|*.ps|Encapsulated Postscript File (*.eps)|*.eps|'// &
...
!    CALL IGRSELECT(DRAWWIN,IBITMAP(I))
!    IB=WINFOBITMAP(IBITMAP(I),BITMAPWIDTH)
!    JB=WINFOBITMAP(IBITMAP(I),BITMAPHEIGHT)
...
!    !## adjust image width
!    CALL IGRPRINTIMAGEOPTIONS(1,IP) !IB) 
!    JB=IP*REAL(JB)/REAL(IB)
!    !## adjust image height
!    CALL IGRPRINTIMAGEOPTIONS(2,JB) !JP*(IP/IB)) 
!!    IF(EXT.EQ.'EPS')CALL IGRPRINTIMAGEOPTIONS(22,2)
!    !## landscape plotting
!    CALL IGRPRINTIMAGEOPTIONS(5,INTLANDSCAPE) !WH)
    
!    CALL IGRPRINTIMAGEOPTIONS(3,0) !## horizontal adjustment
!    CALL IGRPRINTIMAGEOPTIONS(4,0) !## vertical adjustment      

!    IF(I.EQ.1)THEN; CALL WPRINTIMAGEOPTIONS(4,FNAME); IF(WINFODIALOG(4).NE.1)RETURN; ENDIF
!    IF(SIZE(IWINPROFILE).GT.1)FNAME=FNAME(:INDEX(FNAME,'.',.TRUE.)-1)//'_'//TRIM(ITOS(I))//'.PS'
...
!   ENDDO
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL :: X1,X2,Y1,Y2,DX,DXS,DY,YY1,YY2 
 INTEGER :: I,J
...
! !## get current textsizes
! DY=(Y2-Y1)*AXES%CHH*MXNIDF

 !## plot axes-text
 CALL IGRCOLOURN(WRGB(0,0,0))

 DX=0.0
! YY1=Y1+(AXES%CHH*(Y2-Y1))
! YY2=YY1+0.75*DY 
...
!  IF(I.EQ.1)THEN
!   CALL WGRTEXTORIENTATION(ALIGNLEFT,ANGLE=0.0)
!  ELSEIF(I.EQ.NXY)THEN
  IF(I.GT.1)THEN
!   CALL WGRTEXTORIENTATION(ALIGNRIGHT,ANGLE=0.0)
   DX=DX+SQRT((XY(1,I)-XY(1,I-1))**2.0+(XY(2,I)-XY(2,I-1))**2.0)
!  ELSE
...
!  CALL WGRTEXTSTRING(DX,(YY1+YY2)/2.0,' '//TRIM(STRING)//' ') 
!  CALL WGRTEXTFONT(AXES%TFONT,WIDTH=AXES%CHW,HEIGHT=AXES%CHH,ISTYLE=0)
!  CALL IGRCOLOURN(WRGB(0,0,0))
!  CALL WGRTEXTSTRING(DX,(YY1+YY2)/2.0,' '//TRIM(STRING)//' ') 
 ...
 IF(MXNIDF.LE.0)THEN
  I=0
  CALL WDIALOGTABSTATE(ID_DSERIESTAB,ID_DSERIESTAB1,I)
 ELSE
  I=1
  CALL WDIALOGTABSTATE(ID_DSERIESTAB,ID_DSERIESTAB1,I)
  CALL WDIALOGSETTAB(ID_DSERIESTAB,ID_DSERIESTAB1)
 ENDIF
 CALL WDIALOGSELECT(ID_DSERIESTAB3)

 CALL WDIALOGSELECT(ID_DSERIESPROP)
 IF(MXNIFF.LE.0)THEN
  CALL WDIALOGTABSTATE(ID_DSERIESTAB,ID_DSERIESTAB4,0)
 ELSE
  CALL WDIALOGTABSTATE(ID_DSERIESTAB,ID_DSERIESTAB4,1)
  IF(MXNIDF.LE.0)CALL WDIALOGSETTAB(ID_DSERIESTAB,ID_DSERIESTAB4)
 ENDIF

 CALL WDIALOGSELECT(ID_DSERIESPROP)
 IF(NIPF.LE.0)THEN
  CALL WDIALOGTABSTATE(ID_DSERIESTAB,ID_DSERIESTAB5,0)
  CALL WDIALOGTABSTATE(ID_DSERIESTAB,ID_DSERIESTAB6,0)
 ELSE
  CALL WDIALOGTABSTATE(ID_DSERIESTAB,ID_DSERIESTAB5,1)
  CALL WDIALOGTABSTATE(ID_DSERIESTAB,ID_DSERIESTAB6,1)
  IF(MXNIDF.LE.0.AND.MXNIFF.LE.0)CALL WDIALOGSETTAB(ID_DSERIESTAB,ID_DSERIESTAB5)
 ENDIF
   ...
   IF(MP(IPLOT)%IPLOT.EQ.5)THEN
    ...
    IF(READMDF(MP(IPLOT)%IDFNAME,N))THEN
     DO I=1,N
      ...
      CALL LEGALLOCATE(PROFIDF(MXNIDF)%LEG)
 ...
 IF(ALLOCATED(PROFIDF))THEN
  DO I=1,SIZE(PROFIDF)
   ...
   CALL LEGDEALLOCATE(PROFIDF(I)%LEG)
  ENDDO
  DEALLOCATE(PROFIDF)
 ENDIF
