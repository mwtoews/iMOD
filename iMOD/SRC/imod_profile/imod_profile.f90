...
USE MOD_UTL, ONLY : ITOS,RTOS,UTL_GETUNIT,UTL_CLOSEUNITS,UTL_HIDESHOWDIALOG,INVERSECOLOUR,UTL_DRAWLEGENDBOX,UTL_DIST,UTL_EQUALS_REAL,NEWLINE, &
                    ...
                    UTL_PROFILE_COMPVIEWBOX,UTL_PROFILE_GETVIEWBOX,UTL_CAP_BIG,UTL_DEBUGLEVEL
...
USE MOD_IPFGETVALUE, ONLY : IPFGETVALUE_QUICKVIEW,IPFGETVALUE_QUICKVIEW_INIT,GXMIN,GYMIN,GXMAX,GYMAX
...
!   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMNOK,'The maximum of IDF files for the ProfileTool is '//TRIM(ITOS(NR))//CHAR(13)// &
...
!  CALL WMENUSETSTATE(ID_BITMAP,1,I)
 ...
 IF(J.LE.0)THEN
  ...
  DO I=1,3; CALL WGRIDCLEARCELL(IDF_GRID1,I,1); ENDDO
#endif
 ...
 ELSE
  ...
  IF(ZIDF(I).NE.PROFIDF(IPOS(I))%IDF%NODATA)THEN
   ...
   CALL WGRIDPUTCELLREAL(IDF_GRID1,2,J,ZIDF(I)) 
  ENDIF; ENDDO
 ENDIF
    ...
    DO I=1,SERIE(IIDF)%N 
     IF(SERIE(IIDF)%Y(I).NE.PROFIDF(IIDF)%IDF%NODATA)THEN
      ...
      IF(I.EQ.1)THEN
       ...
       IF(X1.NE.X2)CALL IGRRECTANGLE(X1,PROFIDF(IIDF)%IDF%BOT,X2,PROFIDF(IIDF)%IDF%TOP)   
    ...
    DO I=1,SERIE(IIDF)%N 
     IF(SERIE(IIDF)%Y(I).NE.PROFIDF(IIDF)%IDF%NODATA)THEN
      IF(I.EQ.1)THEN
       X1= SERIE(IIDF)%X(I)
       X2=(SERIE(IIDF)%X(I)+SERIE(IIDF)%X(I+1))/2.0
       IF(X1.NE.X2)CALL IGRRECTANGLE(X1,YMIN,X2,SERIE(IIDF)%Y(I))   
      ELSEIF(I.EQ.SERIE(IIDF)%N)THEN 
       X1=(SERIE(IIDF)%X(I)+SERIE(IIDF)%X(I-1))/2.0
       X2= SERIE(IIDF)%X(I)
       IF(X1.NE.X2)CALL IGRRECTANGLE(X1,YMIN,X2,SERIE(IIDF)%Y(I))   
...
!      F=(XT(J)-X1)/(X2-X1)
 ...
 DO I=0,N+1
  ...
  CLEN=0.0; IF(I.GT.0.AND.I.LE.N)CLEN=LN(I)/2.0
 ...
 CALL IGRCOLOURN(WRGB(0,0,0))
 CALL WGRTEXTFONT(AXES%TFONT,WIDTH=AXES%CHW,HEIGHT=AXES%CHH)
