...
USE WINTERACTER
USE RESOURCE
USE MOD_UTL, ONLY : RTOS,ITOS,UTL_GDATE,UTL_SETTEXTSIZE,UTL_DRAWLEGENDBOX,JDATETOGDATE,JDATETOFDATE
...
! CALL IGRLINEWIDTH(3)
 ...
 DO
  ...
  DO
   ...
   IF(Y.GT.AXES%YMIN)THEN
    DY=DY+WGRTEXTLENGTH(TRIM(PROFILE_GETFORMAT(Y*AXES%YFACTOR)))*WINFOGRREAL(GRAPHICSCHHEIGHT) !WIDTH)
   ENDIF
  END DO
  ...
  YASINT=YASINT*2.0 !MAX(1,NINT(DY/(AXES%YMAX-AXES%YMIN)))
 ENDDO
 ...
 DO
  ...
  IF(X.GT.AXES%XMIN+XJDCOR)THEN
   IF(AXES%LDATE)THEN
!    CALL UTL_GDATE(INT(X+XJDCOR),IY,IM,ID)
