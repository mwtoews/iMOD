...
USE MOD_IDF,ONLY : IDFGETVAL,IDFDEALLOCATEX,IDFGETAREA
USE MOD_UTL
USE MOD_PROFILE_PAR
USE MODPLOT
...
CONTAINS
 ...
 DXY=SQRT((X2-X1)**2.0D0+(Y2-Y1)**2.0D0); IF(DXY.EQ.0.0D0)RETURN
 ...
 IF(PROFIDF(IIDF)%IDF%IEQ.EQ.0)THEN
  ...
  CALL INTERSECT_EQUI(PROFIDF(IIDF)%IDF%XMIN,PROFIDF(IIDF)%IDF%XMAX,PROFIDF(IIDF)%IDF%YMIN, &
                      ...
                      X1ISG,X2ISG,Y1ISG,Y2ISG,N,.FALSE.) 
 ELSE
 ...
 IF(N.EQ.-2)THEN
  ...
  ILEN=ILEN+1
  SERIE(IIDF)%X(ILEN)=TLEN+SQRT((X1-X2)**2.0D0+(Y1-Y2)**2.0D0)
  SERIE(IIDF)%Y(ILEN)=PROFIDF(IIDF)%IDF%NODATA
 ENDIF
 ...
 DO I=0,N+1
  ...
  CLEN=0.0D0; IF(I.GT.0.AND.I.LE.N)CLEN=LN(I)/2.0D0
  IF(I.GT.1)CLEN=CLEN+LN(I-1)/2.0D0
  ...
  IF(ISKIPSHORTS.EQ.1.AND.CLEN.LT.(0.75D0*SQRT(IDFGETAREA(PROFIDF(IIDF)%IDF,ICOL,IROW))))CYCLE
 ...
 !###======================================================================
 SUBROUTINE PROFILE_PLOTSURVEY_MOUSEMOVE(X,Y,CRDITYPE,ICRD)
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: X,Y
 INTEGER,INTENT(OUT) :: CRDITYPE,ICRD
 INTEGER :: ISURVEY,IWINID
 REAL(KIND=DP_KIND) :: DX,XC,YC,WD,HD !WC,HC,WA,HA
 ...
 DO IWINID=1,SIZE(IWINPROFILE)
  CALL IGRSELECT(DRAWWIN,IWINPROFILE(IWINID))
  WD=REAL(WINFODRAWABLE(DRAWABLEWIDTH),8)
  HD=REAL(WINFODRAWABLE(DRAWABLEHEIGHT),8)
  EXIT
 ENDDO
 ...
 IF(UTL_DIST(XCRD(2),YCRD(2),XC,YC).LE.DX)ICRD=6 !## urc
 IF(UTL_DIST(XCRD(2),YCRD(1),XC,YC).LE.DX)ICRD=7 !## lrc
 IF(UTL_DIST(XCRD(1),YCRD(1),XC,YC).LE.DX)ICRD=8 !## llc
 IF(UTL_DIST(XCRD(1),YCRD(2),XC,YC).LE.DX)ICRD=9 !## ulc 
 ...
 !###======================================================================
 SUBROUTINE PROFILE_PLOTSURVEY_ADJUST(XC,YC,CRDITYPE) 
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: XC,YC
 INTEGER,INTENT(IN) :: CRDITYPE
 INTEGER :: IWINID
 REAL(KIND=DP_KIND) :: DX,DY,WD,HD
 ...
 SELECT CASE (CRDITYPE)
  CASE (1) !## bottom  
   YSURVEY(2)=YSURVEY(2)+DY
  CASE (2) !## right
   XSURVEY(2)=XSURVEY(2)+DX
  CASE (3) !## top
   YSURVEY(1)=YSURVEY(1)+DY
  CASE (4) !## left
   XSURVEY(1)=XSURVEY(1)+DX
  CASE (5) !## move
   XSURVEY(1)=XSURVEY(1)+DX
   XSURVEY(2)=XSURVEY(2)+DX
   YSURVEY(1)=YSURVEY(1)+DY
 ...
 IF(PROFILE_GETLOCATION(X,Y,XPOSPROF))THEN
  ...
  CALL DBL_IGRCIRCLE(X,Y,RADIUS,IOFFSET=1)
  CALL IGRLINEWIDTH(1)
 ENDIF
