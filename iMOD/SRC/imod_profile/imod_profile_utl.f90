...
USE WINTERACTER
USE RESOURCE
USE MOD_UTL, ONLY : RTOS,ITOS,UTL_GDATE,UTL_SETTEXTSIZE,UTL_DRAWLEGENDBOX,JDATETOGDATE,JDATETOFDATE,UTL_GETFORMAT
...
! CALL IGRLINEWIDTH(3)
 ...
 CALL IPGUNITS(XASMIN,YASMIN,XASMAX,YASMAX)
 CALL IPGXSCALE('N')
 ...
 IF(AXES%IFIXX.EQ.0)THEN
  ...
  CALL IPGXGETSCALE(SVALUE,NVALUE)
...
!  AXES%XINT=XASINT
  XASINT=SVALUE(2)-SVALUE(1) !; XASINT=AXES%XINT
!  AXES%XMIN=SVALUE(1); 
  XASMIN=SVALUE(1)
 ENDIF
 ...
 IF(AXES%IFIXY.EQ.0)THEN
  ...
  CALL IPGYGETSCALE(SVALUE,NVALUE)
...
!  AXES%YINT=YASINT
  YASINT=SVALUE(2)-SVALUE(1) !; YASINT=AXES%YINT 
!  AXES%YMIN=SVALUE(1); 
  YASMIN=SVALUE(1)
 ...
 IF(AXES%IAXES(2).EQ.1)THEN
  IF(AXES%IFIXY2.EQ.0)THEN
   ...
   CALL IPGUNITS(XASMIN,Y2ASMIN,XASMAX,Y2ASMAX)
   CALL IPGYGETSCALE(SVALUE,NVALUE)
...
!   Y2ASINT=(Y2ASMAX-Y2ASMIN)/REAL(I)
   Y2ASINT=SVALUE(2)-SVALUE(1) !; Y2ASINT=AXES%Y2INT
!   AXES%Y2MIN=SVALUE(1); 
   Y2ASMIN=SVALUE(1)
!   AXES%Y2INT=Y2ASINT
!   AXES%Y2MIN=Y2ASMIN
 ...
 DO
  ...
  DO
   ...
   IF(Y.GT.AXES%YMIN)THEN
    DY=DY+WGRTEXTLENGTH(TRIM(UTL_GETFORMAT(Y*AXES%YFACTOR)))*WINFOGRREAL(GRAPHICSCHHEIGHT) !WIDTH)
   ENDIF
  END DO
