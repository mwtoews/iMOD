...
! CALL IGRLINEWIDTH(3)
 ...
 CALL UTL_GETAXESCALES(XASMIN,YASMIN,XASMAX,YASMAX,SXVALUE,NSX,SYVALUE,NSY)
...
!  AXES%XINT=XASINT
  XASINT=SXVALUE(2)-SXVALUE(1) !; XASINT=AXES%XINT
!  AXES%XMIN=SVALUE(1); 
  XASMIN=SXVALUE(1)
 ENDIF
...
!  AXES%YINT=YASINT
  YASINT=SYVALUE(2)-SYVALUE(1) !; YASINT=AXES%YINT 
!  AXES%YMIN=SVALUE(1); 
  YASMIN=SYVALUE(1)
 ...
 IF(AXES%IAXES(2).EQ.1)THEN
  IF(AXES%IFIXY2.EQ.0)THEN
   ...
   CALL UTL_GETAXESCALES(XASMIN,Y2ASMIN,XASMAX,Y2ASMAX,SXVALUE,NSX,SYVALUE,NSY)
...
!   Y2ASINT=(Y2ASMAX-Y2ASMIN)/REAL(I)
   Y2ASINT=SYVALUE(2)-SYVALUE(1) !; Y2ASINT=AXES%Y2INT
!   AXES%Y2MIN=SVALUE(1); 
   Y2ASMIN=SYVALUE(1)
!   AXES%Y2INT=Y2ASINT
!   AXES%Y2MIN=Y2ASMIN
 ...
 DO
  ...
  DO
   ...
   IF(Y.GT.AXES%YMIN)THEN
    DY=DY+WGRTEXTLENGTH(TRIM(UTL_GETFORMAT(Y*AXES%YFACTOR)))*WINFOGRREAL(GRAPHICSCHHEIGHT)
   ENDIF
  END DO
