...
USE WINTERACTER
USE RESOURCE
USE MOD_UTL, ONLY : RTOS,ITOS,GDATE,UTL_SETTEXTSIZE,UTL_DRAWLEGENDBOX,JDATETOGDATE
USE MOD_PROF_PAR, ONLY : XY,NXY,XPOS,IWINPROFILE,IBITMAP,AREA  !,NGRAPH
...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE PROFILE_PLOTSURVEY()
 !###======================================================================
 IMPLICIT NONE
 REAL :: XP1,XP2,YP1,YP2,SX_RATIO,SXYRATIO,BITMAPRATIO
 REAL :: XMARGE,DXMARGE,YMARGE,DX,DY
 INTEGER :: IWD,IHD,IWS,ISURVEY,IWINID
...
!      CALL WBITMAPCLIPBOARD(IBITMAP)
 ...
 REAL :: DX,DY,XMIN,YMIN,XMAX,YMAX
 TYPE(AXESOBJ) :: AXES
  ...
  IF(GRAPH(I,NG)%GTYPE.EQ.1)THEN
   ...
   DO J=1,GRAPH(I,NG)%NP-1
    ...
    IF(MIN(0.0,GRAPH(I,NG)%RY(J)).NE.MAX(0.0,GRAPH(I,NG)%RY(J)))THEN
     ...
     CALL IGRRECTANGLE(GRAPH(I,NG)%RX(J),MIN(0.0,GRAPH(I,NG)%RY(J)),GRAPH(I,NG)%RX(J+1),MAX(0.0,GRAPH(I,NG)%RY(J)))
     CALL IGRFILLPATTERN(OUTLINE)
     CALL IGRCOLOURN(WRGB(0,0,0))
     CALL IGRRECTANGLE(GRAPH(I,NG)%RX(J),MIN(0.0,GRAPH(I,NG)%RY(J)),GRAPH(I,NG)%RX(J+1),MAX(0.0,GRAPH(I,NG)%RY(J)))
    ELSE
     CALL IGRJOIN(GRAPH(I,NG)%RX(J),MIN(0.0,GRAPH(I,NG)%RY(J)),GRAPH(I,NG)%RX(J+1),MAX(0.0,GRAPH(I,NG)%RY(J)))
    ENDIF
   END DO
...
!    CALL IGRRECTANGLE(GRAPH(I,NG)%RX(J),MIN(0.0,GRAPH(I,NG)%RY(J)),GRAPH(I,NG)%RX(J+1),MAX(0.0,GRAPH(I,NG)%RY(J)))
 ...
 DO I=1,SIZE(GRAPH,1)
  ...
  SELECT CASE (GRAPH(I,NG)%GTYPE)
   ...
   CASE (1,3) !## filled                                                     ,IWIDTH,lineTYPE,IPATTERN,
...
! CALL IGRUNITS(GRAPHUNITS(1,IWINID),GRAPHUNITS(2,IWINID),GRAPHUNITS(3,IWINID),GRAPHUNITS(4,IWINID))
 ...
 !###====================================================================
 SUBROUTINE PROFILE_PLOTLOCATION()
 !###====================================================================
 IMPLICIT NONE
 REAL :: X,Y,RADIUS
 ...
 !###======================================================================
 CHARACTER(LEN=10) FUNCTION PROFILE_GETFORMAT(X)
 !###======================================================================
 IMPLICIT NONE
 REAL,INTENT(IN) :: X
 CHARACTER(LEN=20) :: XC
 ...
 CALL IUPPERCASE(XC)
 J=INDEX(XC,'E+00')
 IF(J.GT.0)XC=XC(:J-1) !LEN_TRIM(XC)-4)
 I=INDEX(XC,'E')
 IF(I.GT.0)THEN
  PROFILE_GETFORMAT='(E10.4)'
 ELSE
  I=INDEX(XC,'.')
  IF(I.EQ.0)THEN
   PROFILE_GETFORMAT='(F10.0)'
  ELSE
   J=LEN_TRIM(XC)
   DO K=J,I+1,-1
    IF(XC(K:K).NE.'0')EXIT
   ...
   IF(NDEC.EQ.0)THEN
    PROFILE_GETFORMAT='(F10.0)'
   ELSE
    WRITE(PROFILE_GETFORMAT,'(A5,I2.2,A)') '(F10.',NDEC,')'
   ENDIF
  ENDIF
 ENDIF

 END FUNCTION PROFILE_GETFORMAT
 ...
 INTEGER :: I,IY,ID,IM,IWD,IHD,IWS,IHS,NL
 REAL :: DX,DY,DXTIC,DYTIC,X,Y,DMX1, &
 ...
 IF(AXES%IFIXX.EQ.0)THEN
  ...
  I=10    !## first quess interval
  CALL PROFILE_AXES(XASMIN,XASMAX,0.0,I)
  !## check i
  DO WHILE(I.LT.10); I=MAX(1,I)*2; ENDDO
  XASINT=(XASMAX-XASMIN)/REAL(I)
  AXES%XINT=XASINT
 ...
 IF(AXES%IFIXY.EQ.0)THEN
  ...
  I=10    !## first quess interval
  CALL PROFILE_AXES(YASMIN,YASMAX,0.0,I)
  !## check i
  DO WHILE(I.LT.10); I=MAX(1,I)*2; ENDDO
  YASINT=(YASMAX-YASMIN)/REAL(I)
  AXES%YINT=YASINT
 ...
 IF(AXES%IAXES(2).EQ.1)THEN
  IF(AXES%IFIXY2.EQ.0)THEN
   I=10    !## first quess interval
   CALL PROFILE_AXES(Y2ASMIN,Y2ASMAX,0.0,I)
   !## check i
   DO WHILE(I.LT.10); I=MAX(1,I)*2; ENDDO
   Y2ASINT=(Y2ASMAX-Y2ASMIN)/REAL(I)
   AXES%Y2INT=Y2ASINT
   AXES%Y2MIN=Y2ASMIN
   AXES%Y2MAX=Y2ASMAX
  ENDIF
 ...
 IF(AXES%IAXES(2).EQ.1)THEN
  ...
  DMY1=D2Y*(SY_RATIO*(1.0/AXES%DYAXESB))! DYAXESB=20.0))
  DMY2=D2Y*(SY_RATIO*(1.0/AXES%DYAXEST))! DYAXEST=75.0))
 ...
 DMX1=DX*(SX_RATIO*(1.0/AXES%DXAXESL))! DXAXESL=40.0))
 DMX2=DX*(SX_RATIO*(1.0/AXES%DXAXESR))! DXAXESR=150.0))
 DMY1=DY*(SY_RATIO*(1.0/AXES%DYAXESB))! DYAXESB=20.0))
 DMY2=DY*(SY_RATIO*(1.0/AXES%DYAXEST))! DYAXEST=75.0))

! IF(.NOT.ALLOCATED(GRAPHUNITS))THEN
!  ALLOCATE(GRAPHUNITS(4,IWINID))
! ENDIF
 ...
 DO
  ...
  IF(Y.GT.AXES%YMIN)THEN
   DY=DY+WGRTEXTLENGTH(TRIM(PROFILE_GETFORMAT(Y*AXES%YFACTOR)))*WINFOGRREAL(GRAPHICSCHWIDTH)
  ENDIF
 END DO

 DY=DY*(IWD)/(IHD)
 !## increase interval in case axes is to big
 IF(DY.GT.(AXES%XMAX-AXES%XMIN))YASINT=YASINT*MAX(1,NINT(DY/(AXES%XMAX-AXES%XMIN)))
 ...
 DO
  ...
  IF(Y.GT.AXES%YMIN)THEN
   ...
   !## plot axes-text
   CALL WGRTEXTREAL((AXES%XMIN+XJDCOR)-DX,Y,Y*AXES%YFACTOR,TRIM(PROFILE_GETFORMAT(Y*AXES%YFACTOR)))
  ENDIF
 END DO
 ...
 IF(AXES%IAXES(2).EQ.1)THEN
  ...
  DO
   ...
   IF(Y.GT.AXES%Y2MIN)THEN
    ...
    !## plot axes-text
    CALL WGRTEXTREAL(AXES%XMAX+DX+XJDCOR,Y,Y,TRIM(PROFILE_GETFORMAT(Y)))
   ENDIF
  END DO
 ...
 DO
  ...
  IF(X.GT.AXES%XMIN+XJDCOR)THEN
   IF(AXES%LDATE)THEN
    CALL GDATE(INT(X+XJDCOR),IY,IM,ID)
    CDATE=TRIM(ITOS(ID))//'/'//TRIM(ITOS(IM))//'/'//TRIM(ITOS(IY))
    DX=DX+1.75*WGRTEXTLENGTH(TRIM(CDATE))*WINFOGRREAL(GRAPHICSCHWIDTH)
   ELSE
    DX=DX+WGRTEXTLENGTH(TRIM(PROFILE_GETFORMAT((X+XJDCOR)*AXES%XFACTOR)))*WINFOGRREAL(GRAPHICSCHWIDTH)
   ENDIF
  ENDIF
 END DO
 ...
 DO 
  ...
  IF(X.GT.AXES%XMIN+XJDCOR)THEN
   ...
   IF(AXES%LDATE)THEN
    CALL GDATE(INT(X-XJDCOR),IY,IM,ID)
    CDATE=TRIM(ITOS(ID))//'/'//TRIM(ITOS(IM))//'/'//TRIM(ITOS(IY))
    CALL WGRTEXTSTRING(X,AXES%YMIN-DY,CDATE)
   ELSE
    CALL WGRTEXTREAL(X,AXES%YMIN-DY,X*AXES%XFACTOR,TRIM(PROFILE_GETFORMAT((X+XJDCOR)*AXES%XFACTOR)))
   ENDIF
  ENDIF
 END DO
...
! DY = GRAPHUNITS(4,1)-GRAPHUNITS(2,1)
! !## correct units for marge
! X1V=(AXES%XMIN-GRAPHUNITS(1,1))/DX
! X2V=(AXES%XMAX-GRAPHUNITS(1,1))/DX
! Y1V=(AXES%YMIN-GRAPHUNITS(2,1))/DY
! Y2V=(AXES%YMAX-GRAPHUNITS(2,1))/DY
