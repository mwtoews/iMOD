...
CHARACTER(LEN=30),PARAMETER :: RVERSION='V3_2'    !## release message - only with single subnummers
CHARACTER(LEN=30),PARAMETER :: BVERSION='V3_2_00' !## banner message - no characters !!!
...
DATA TP%ALIAS/'GROUNDWATERHEAD','CONSTANT_HEAD','FLUX_LOWER_FACE','FLUX_RIGHT_FACE','FLUX_FRONT_FACE','STORAGE',   &
              ...
              'SEGMENTS','CAPSIM','DECREASE_WATER_ST.ROOTZ.','MEASURED_PRECIPITATION','SPRINKLING_PRECIPITATION', &
              'NET_EVAPORATION_WATER','RUNOFF','PURGE_WATER_TABLE_HEAD','GROUNDWATERLEVEL',&
              'TOTAL_ACTUAL_TRANSPIRATION',&
              'SPRINKLING_PRECIPITATION,_FROM_GROUNDWATER',&
              'POTENTIAL_EVAPORATION_BARE_SOIL',&
              'EVAPORATION_INTERCEPTION_WATER', &
              'EVAPORATION_PONDING WATER',&
              'EVAPORATION_SPRINKLING WATER',&
              'POTENTIAL_TRANSPIRATION_VEGETATION',&
              'GROUNDWATER_EXTRACTION_FOR_SPRINKLING',&
              'EVAPORATION_BARE_SOIL',&
              'UPWARD_SEEPAGE_OF_MODFLOW CELL',&
              'FLOW_THROUGH_BOTTOM_OF_BOX1,_ROOT_ZONE', &
              'DECREASE_STORAGE'/
