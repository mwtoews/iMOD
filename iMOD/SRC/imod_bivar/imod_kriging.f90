...
CONTAINS
 ...
 SUBROUTINE KRIGING_MAIN(MD,XD,YD,ZD,IDF,IDFV,MINP,RANGE,SILL,NUGGET,KTYPE, &
               PNTSEARCH,IEXPVARIOGRAM,LAGINTERVAL,LAGDISTANCE,IBATCH,GENFNAME)     
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF,IDFV
 INTEGER,INTENT(IN) :: IBATCH
 INTEGER,INTENT(IN) :: MD,KTYPE,PNTSEARCH,IEXPVARIOGRAM,LAGINTERVAL
 ...
 !## use experimental variogram
 IF(IEXPVARIOGRAM.EQ.1)THEN
  !## make copy of data points
  ALLOCATE(XDC(MD),YDC(MD),ZDC(MD)); XDC=XD; YDC=YD; ZDC=ZD
  CALL KRIGING_VARIOGRAM(MD,XD,YD,ZD,ND,IDF,LAGINTERVAL,LAGDISTANCE,IBATCH=IBATCH,SNAME='') 
  XD=XDC; YD=YDC; ZD=ZDC; DEALLOCATE(XDC,YDC,ZDC)
  WRITE(*,'(2A15)') 'LAG_DISTANCE','VARIOGRAM'
  DO I=0,SIZE(XLAG)-1; WRITE(*,'(2F15.7)') XLAG(I),ZLAG(I); ENDDO; WRITE(*,*)
  J=0
  DO I=1,SIZE(XLAG)-1
   IF(ZLAG(I).LE.ZLAG(J))THEN
    XLAG(I)=0.0
   ELSE
    J=I
   ENDIF
  ENDDO
  J=0; DO I=1,SIZE(XLAG)-1
   IF(XLAG(I).NE.0.0)THEN
    J=J+1; XLAG(J)=XLAG(I); ZLAG(J)=ZLAG(I)
   ENDIF
  ENDDO
  ALLOCATE(XDC(0:J),ZDC(0:J))
  DO I=0,J; XDC(I)=XLAG(I); ZDC(I)=ZLAG(I); ENDDO
  DEALLOCATE(XLAG,ZLAG); ALLOCATE(XLAG(0:J),ZLAG(0:J))
  XLAG=XDC; ZLAG=ZDC; DEALLOCATE(XDC,ZDC)
  DO I=0,SIZE(XLAG)-1; WRITE(*,'(2F15.7)') XLAG(I),ZLAG(I); ENDDO; WRITE(*,*)
  MZ=MAXVAL(ZLAG); ZLAG=ZLAG/MZ; RANGE=MAXVAL(XLAG); SILL=1.0; NUGGET=0.0
  DO I=0,SIZE(XLAG)-1; WRITE(*,'(2F15.7)') XLAG(I),ZLAG(I); ENDDO; WRITE(*,*)
 ENDIF
 ...
 DO IROW=1,IDF%NROW
  ...
  DO ICOL=1,IDF%NCOL
   ...
   CALL KRIGING_FILLSYSTEM(XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,IDF%NODATA)
 ...
 SUBROUTINE KRIGING_FILLSYSTEM(XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET, &
          KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,NODATA)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ND,MINP,KTYPE,MX,PNTSEARCH
...
!  IF(ALLOCATED(SELID))THEN
!   IF(N.GT.SIZE(SELID))DEALLOCATE(SELID,SELD)
!  ENDIF
!  IF(.NOT.ALLOCATED(SELID))ALLOCATE(SELID(MINP),SELD(MINP))
...
!## range is afhanjelijk van wat er aan minp punten inzit ...
   ...
   DXY=KRIGING_DIST(XD(ID),YD(ID),X,Y,RANGE)
   !## outside range, take next
   IF(DXY.GT.RANGE)CYCLE
   !## not in current list
   ...
   SELID(I)=ID; SELD(I)=DXY
  ENDDO
  NP=N !; DEALLOCATE(SELD)
  
 ELSE

  NP=ND !; ALLOCATE(SELID(NP))

!  IF(ALLOCATED(SELID))THEN
!   IF(NP.GT.SIZE(SELID))DEALLOCATE(SELID)
!  ENDIF
!  IF(.NOT.ALLOCATED(SELID))ALLOCATE(SELID(NP))
  
  J=0
  DO ID=1,NP
   IF(KRIGING_DIST(XD(ID),YD(ID),X,Y,RANGE).LE.RANGE)THEN
    J=J+1; SELID(J)=ID
   ENDIF
  ENDDO
  NP=J
 ...
 IF(N.GT.SIZE(B))THEN; DEALLOCATE(A,B); ALLOCATE(A(N,N),B(N)); ENDIF
! IF(N.GT.SIZE(B))THEN; DEALLOCATE(A,B,L,U); ALLOCATE(A(N,N),L(N,N),U(N,N),B(N)); ENDIF
 ...
 DO I=1,NP 
  ID=SELID(I)
  !## semivariance
  GAMMA=KRIGING_GETGAMMA(XD(ID),YD(ID),XD(ID),YD(ID),RANGE,SILL,NUGGET,KTYPE)
  !## variance
  A(I,I)=SILL-GAMMA
  DO J=I+1,NP 
   JD=SELID(J)
   GAMMA=KRIGING_GETGAMMA(XD(ID),YD(ID),XD(JD),YD(JD),RANGE,SILL,NUGGET,KTYPE)
   A(I,J)=SILL-GAMMA; A(J,I)=A(I,J)
  ENDDO

  ID=SELID(I)
  GAMMA=KRIGING_GETGAMMA(XD(ID),YD(ID),X,Y,RANGE,SILL,NUGGET,KTYPE)
  B(I)=SILL-GAMMA
...
! CALL LUDCMP(A(1:N,1:N),B(1:N),L(1:N,1:N),U(1:N,1:N),N) 
...
! DEALLOCATE(A,B)
 ...
 !###======================================================================
 REAL FUNCTION KRIGING_DIST(X1,Y1,X2,Y2,RANGE)
 !###======================================================================
 IMPLICIT NONE
 REAL,INTENT(IN) :: X1,Y1,X2,Y2,RANGE
 ...
 DO J=1,NXY
  ...
  DO I=I1+1,I2
   ...
   IF(ISTATUS.EQ.5)KRIGING_DIST=RANGE+1.0 !KRIGING_DIST*100.0  
  ENDDO
 ENDDO
 ...
 INTEGER :: MINP,KTYPE,IROW,ICOL,I,PNTSEARCH,ND,NR,NC,MX,IEXPVARIOGRAM,LAGINTERVAL
 ...
 IEXPVARIOGRAM=1; LAGINTERVAL=20; LAGDISTANCE=RANGE/REAL(LAGINTERVAL)
 MX=RANGE/IDF%DX
  
 !## use experimental variogram
 IF(IEXPVARIOGRAM.EQ.1)THEN
  CALL KRIGING_VARIOGRAM(INT(MD,4),XD,YD,ZD,ND,IDF,LAGINTERVAL,LAGDISTANCE,IBATCH=1,SNAME='') 
  MZ=MAXVAL(ZLAG)
  ZLAG=ZLAG/MZ
  RANGE=MAXVAL(XLAG)
 ENDIF
...
! CLOSE(10)
 ...
 !###====================================================================
 SUBROUTINE KRIGINGSETTINGS(MINP,KTYPE,RANGE,SILL,NUGGET,PNTSEARCH)
 !###====================================================================
 IMPLICIT NONE
 REAL,INTENT(INOUT) :: RANGE,SILL,NUGGET
 INTEGER,INTENT(INOUT) :: MINP,KTYPE,PNTSEARCH
 TYPE(WIN_MESSAGE) :: MESSAGE
