...
REAL,POINTER,DIMENSION(:,:),PRIVATE :: XY,XYDUMMY
INTEGER,POINTER,DIMENSION(:),PRIVATE :: NXY,NXYDUMMY
...
CONTAINS
 ...
 IF(ASSOCIATED(XY))DEALLOCATE(XY);    IF(ASSOCIATED(NXY))DEALLOCATE(NXY)
 ...
 NULLIFY(XY,XYDUMMY,NXY,NXYDUMMY)
 MAXP=100; MAXN=10; ALLOCATE(XY(MAXP,2),NXY(MAXN))
 ...
 N=0; M=0; DO
  ...
  !## increase memory
  IF(N.GT.MAXN)THEN
   ALLOCATE(NXYDUMMY(MAXN+10)); DO I=1,MAXN; NXYDUMMY(I)=NXY(I); ENDDO
   DEALLOCATE(NXY); NXY=>NXYDUMMY; MAXN=MAXN+10
  ENDIF
 ...
 REAL,ALLOCATABLE,DIMENSION(:) :: SELD 
 ...
 IF(PNTSEARCH.EQ.1)THEN
  ...
  N=MIN(MINP,ND); ALLOCATE(SELID(MINP),SELD(MINP))
 ...
 ELSE
  ...
  DO ID=1,NP
   IF(KRIGING_DIST(XD(ID),YD(ID),X,Y).LE.RANGE)THEN
    J=J+1; SELID(J)=ID
   ENDIF
  ENDDO
  NP=J
