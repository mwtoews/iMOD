...
CONTAINS
 ...
 REAL :: DX,DY,DXY,GAMMA,H,MZZ,F,USERANGE,MAXRANGE
 ...
 IF(PNTSEARCH.EQ.1)THEN
  ...
  DO ID=1,ND
   ...
   IF(IADJRANGE.EQ.0.AND.DXY.GE.USERANGE)CYCLE
  ...
  SELID=0; NP=0; MAXRANGE=0.0
  DO IQ=1,1+(IQUADRANT*3)
   DO J=1,MINP
    IF(SELQID(IQ,J).GT.0)THEN
     NP=NP+1; SELID(NP)=SELQID(IQ,J); MAXRANGE=MAX(MAXRANGE,SELD(IQ,J))
    ENDIF
   ENDDO 
  ENDDO 
 ...
 DO I=1,NP 
  ...
  DO J=I+1,NP 
   JD=SELID(J)
   GAMMA=KRIGING_GETGAMMA(XD(ID),YD(ID),XD(JD),YD(JD),USERANGE,SILL,NUGGET,KTYPE,1)
   A(I,J)=GAMMA; A(J,I)=A(I,J)
  ENDDO

  ID=SELID(I)
  GAMMA=KRIGING_GETGAMMA(XD(ID),YD(ID),X,Y,USERANGE,SILL,NUGGET,KTYPE,1)
  B(I)=GAMMA
...
! ENDDO
 ...
 DO I=1,NP
  ID=SELID(I)
  GAMMA=KRIGING_GETGAMMA(XD(ID),YD(ID),X,Y,USERANGE,SILL,NUGGET,KTYPE,0)
  KVAR=KVAR+B(I)*GAMMA
 ENDDO
! IF(KTYPE.LT.0)KVAR=KVAR-B(N)
 ...
 !###======================================================================
 REAL FUNCTION KRIGING_GETGAMMA(X1,Y1,X2,Y2,RANGE,SILL,NUGGET,KTYPE,INVERSE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: KTYPE,INVERSE
 ...
 IF(INVERSE.EQ.1)H=1.0-H
 
 !## correct - sill = nugget + sill
 KRIGING_GETGAMMA=(SILL-NUGGET)*H
! KRIGING_GETGAMMA=SILL-((SILL-NUGGET)*H)
...
! CLOSE(10)
 ...
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK2,IADJRANGE)
