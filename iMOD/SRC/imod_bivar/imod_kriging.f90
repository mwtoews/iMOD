...
CONTAINS
 ...
 DO IROW=1,IDF%NROW
  ...
  DO ICOL=1,IDF%NCOL
   ...
   CALL KRIGING_FILLSYSTEM(XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,COINCIDENT,IQUADRANT,IDF%NODATA)
 ...
 SUBROUTINE KRIGING_FILLSYSTEM(XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET, &
          KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,COINCIDENT,IQUADRANT,NODATA)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ND,MINP,KTYPE,MX,PNTSEARCH,COINCIDENT,IQUADRANT
 ...
 REAL :: DX,DY,DXY,GAMMA,H,MZZ,F,USERANGE,MAXRANGE,C0,C1
 ...
 IF(PNTSEARCH.EQ.1)THEN
  ...
  DO ID=1,ND
   ...
   IF(COINCIDENT.EQ.0.AND.DXY.GE.USERANGE)CYCLE
  ...
  IF(NP.GT.0.AND.COINCIDENT.EQ.1)USERANGE=MAXRANGE+0.05*MAXRANGE
...
! LAGINTERVAL=20; LAGDISTANCE=RANGE/REAL(LAGINTERVAL); COINCIDENT=0; IQUADRANT=0
 ...
 MINP=MD; RANGE=10.0; NUGGET=1.0; SILL=10.0; KTYPE=-2; PNTSEARCH=0; NR=0; NC=0
 LAGINTERVAL=20; LAGDISTANCE=RANGE/REAL(LAGINTERVAL); COINCIDENT=0; IQUADRANT=0
 MX=RANGE/IDF%DX
...
! IF(KTYPE.GT.3)KTYPE=1; IF(KTYPE.LT.-3)KTYPE=-3; IF(KTYPE.EQ.0)KTYPE=1
 ...
 J=I; IF(I.EQ.1)CALL WDIALOGGETCHECKBOX(IDF_CHECK2,J)
