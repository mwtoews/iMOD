...
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_IDFSNAPTOGRID,UTL_STDEF,UTL_MESSAGEHANDLE,UTL_WAITMESSAGE,UTL_DIST,ITOS,RTOS, &
      UTL_CAP,UTL_INSIDEELLIPSE,UTL_POINTELLIPSE
...
CONTAINS
 ...
 SUBROUTINE KRIGING_MAIN(MD,XD,YD,ZD,IDF,IDFV,MINP,RANGE,SILL,NUGGET,KTYPE, &
               PNTSEARCH,LAGINTERVAL,LAGDISTANCE,COINCIDENT,COINCIDENTDIST,IQUADRANT,IBATCH,IBLANKOUT,BO_VALUE,ELLIPS)     
 !###======================================================================
! USE OMP_LIB
 IMPLICIT NONE
 ...
 REAL,ALLOCATABLE,DIMENSION(:) :: XCD,YCD  !XDC,YDC,ZDC,
 REAL :: X,Y,MZ,KEST,KVAR,ANI,RAT
...
!if(idfwrite(idf,'d:\tmp.idf',1))then; endif
 ...
 DO IROW=1,IDF%NROW
  DO ICOL=1,IDF%NCOL
   ...
   ENDIF
   CALL KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH, &
        IQUADRANT,IDF,IBLANKOUT,BO_VALUE,RAT,ANI)
 ...
 SUBROUTINE KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET, &
          KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,IQUADRANT,IDF,IBLANKOUT,BO_VALUE,RAT,ANI)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
...
! CLOSE(IU)
 ...
 !###======================================================================
 REAL FUNCTION KRIGING_DIST(X1,Y1,X2,Y2,IDATA,IDF,IBLANKOUT,BO_VALUE,MAXDIST,RAT,ANI)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 INTEGER,INTENT(IN) :: IDATA  !=0 FOR INSERTION IN KRIGING MATRIX  !=1 FOR AVERAGE POINT VALUES
 INTEGER,INTENT(IN) :: IBLANKOUT
 REAL,INTENT(IN) :: X1,Y1,X2,Y2,BO_VALUE,MAXDIST,RAT,ANI
 ...
 IF(RAT.NE.1.0)THEN
  IF(.NOT.UTL_INSIDEELLIPSE(X2,Y2,MAXDIST,RAT*MAXDIST,ANI,X1,Y1))THEN
...
!   CALL UTL_POINTELLIPSE(X2,Y2,A,RAT,MAXDIST,ANI,X3,Y3)
!   D=UTL_DIST(X2,Y2,X3,Y3)
!   F=MAXDIST/D
...
! nugget=5.0
 ...
 X=150.0; Y=150.0
 CALL KRIGING_FILLSYSTEM(XD,XCD,YD,YCD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,IQUADRANT,IDF,0,0.0,1.0,0.0)
