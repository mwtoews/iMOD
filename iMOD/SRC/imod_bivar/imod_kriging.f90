...
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_IDFSNAPTOGRID,UTL_STDEF,UTL_MESSAGEHANDLE,UTL_WAITMESSAGE,UTL_DIST
...
!TYPE(IDFOBJ),PRIVATE :: IDFBN !,IDFB
!TYPE LPOBJ
! INTEGER,POINTER,DIMENSION(:) :: IP
!END TYPE LPOBJ
!TYPE(LPOBJ),ALLOCATABLE,DIMENSION(:,:) :: LP
...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE KRIGING_MAIN(MD,XD,YD,ZD,IDF,IDFV,MINP,MAXP,RANGE,SILL,NUGGET,KTYPE, &
               IOFFSET,SEARCHDISTANCE,NOSEARCH,IEXPVARIOGRAM,LAGINTERVAL,LAGDISTANCE,IBATCH)     
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF,IDFV
 INTEGER,INTENT(IN) :: IBATCH
 INTEGER,INTENT(IN) :: MD,KTYPE,IOFFSET,NOSEARCH,IEXPVARIOGRAM,LAGINTERVAL
 INTEGER,INTENT(INOUT) :: MINP,MAXP
 REAL,INTENT(IN) :: SEARCHDISTANCE,LAGDISTANCE
...
!  DO I=0,SIZE(XLAG)-1; WRITE(*,'(2F15.7)') XLAG(I),ZLAG(I); ENDDO; WRITE(*,*)
 ...
 CALL KRIGING_INIT(MD,XD,YD,ZD,ND,IDF,KTYPE,MZ,NOSEARCH) 
 
 MAXP=MIN(MAXP,ND)

! !## make sure you find enough per quadrant
! MX=RANGE/IDFBN%DX
! NR=MX+1; NC=MX+1
 
 IRAT=0; IRAT1=IRAT
 DO IROW=1,IDF%NROW,IOFFSET
  CALL WMESSAGEPEEK(ITYPE,MESSAGE)
  DO ICOL=1,IDF%NCOL,IOFFSET
   IF(IDF%X(ICOL,IROW).NE.IDF%NODATA)CYCLE
   CALL IDFGETLOC(IDF,IROW,ICOL,X,Y)
   CALL KRIGING_FILLSYSTEM(MAXP,XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,NR,NC,MX,NOSEARCH,IDF%NODATA,SEARCHDISTANCE)
...
!###======================================================================
 ...
 INTEGER :: I,J,K,N,IU,NPOP,LAGINT,IRAT,IRAT1,IS,IOS
 ...
 DO
  ...
  !## n.pairs is n(n-1)/2
  N=(ND*(ND-1))/2; ALLOCATE(DVG(N),STAT=IOS)
 ...
 DEALLOCATE(DVG,ZVG) !,XLAG,ZLAG)
 
 END SUBROUTINE KRIGING_VARIOGRAM

 !###======================================================================
 SUBROUTINE KRIGING_FILLSYSTEM(MAXP,XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET, &
          KTYPE,MZ,KEST,KVAR,NR,NC,MX,NOSEARCH,NODATA,SEARCHDISTANCE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ND,MINP,MAXP,KTYPE,MX,NOSEARCH
 REAL,INTENT(OUT) :: KEST,KVAR
 REAL,INTENT(IN) :: X,Y,RANGE,SILL,NUGGET,MZ,NODATA,SEARCHDISTANCE
 ...
 IF(NOSEARCH.EQ.0)THEN
  ...
  DO ID=1,NP
   IF(UTL_DIST(XD(ID),YD(ID),X,Y).LE.SEARCHDISTANCE)THEN
    J=J+1; SELID(J)=ID
   ENDIF
  ENDDO
  NP=J
 ...
 !###======================================================================
 SUBROUTINE KRIGING_INIT(MD,XD,YD,ZD,ND,IDF,KTYPE,MZ,NOSEARCH)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 INTEGER,INTENT(IN) :: MD,KTYPE,NOSEARCH
 REAL,INTENT(INOUT),DIMENSION(MD) :: XD,YD,ZD 
 REAL,INTENT(OUT) :: MZ
 INTEGER,INTENT(OUT) :: ND
 ...
 REAL :: EX,EY,RANGE,NUGGET,SILL,MZ,X,Y,KEST,KVAR,SEARCHDISTANCE,LAGDISTANCE
 INTEGER :: MINP,MAXP,KTYPE,IROW,ICOL,I,NOSEARCH,ND,NR,NC,MX,IEXPVARIOGRAM,LAGINTERVAL
 ...
 MINP=MD; MAXP=MD; RANGE=4141.0; NUGGET=0.0; SILL=0.78; KTYPE=2; NOSEARCH=1; SEARCHDISTANCE=RANGE; NR=0; NC=0
 IEXPVARIOGRAM=1; LAGINTERVAL=20; LAGDISTANCE=RANGE/REAL(LAGINTERVAL)
 MX=SEARCHDISTANCE/IDF%DX
...
! CALL KRIGING_MAIN(MD,XD,YD,ZD,IDF,IDFV,MINP,MAXP,RANGE,SILL,NUGGET,KTYPE) 
! CALL IDFIROWICOL(IDF,IROW,ICOL,EX,EY)
! WRITE(*,*) IDF%X(ICOL,IROW),ICOL,IROW
...
! CLOSE(10)
 ...
 !###====================================================================
 SUBROUTINE KRIGINGSETTINGS(MINP,MAXP,KTYPE,RANGE,SILL,NUGGET,IOFFSET,SEARCHDISTANCE,NOSEARCH)
 !###====================================================================
 IMPLICIT NONE
 REAL,INTENT(INOUT) :: RANGE,SILL,NUGGET,SEARCHDISTANCE
 INTEGER,INTENT(INOUT) :: MINP,MAXP,KTYPE,IOFFSET,NOSEARCH
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: ITYPE,I
 ...
 NOSEARCH=ABS(NOSEARCH-1)
 CALL WDIALOGPUTCHECKBOX(IDF_CHECK1,NOSEARCH)
 CALL WDIALOGPUTINTEGER(IDF_INTEGER1,MINP)
 CALL WDIALOGPUTINTEGER(IDF_INTEGER2,MAXP)
 CALL WDIALOGPUTINTEGER(IDF_INTEGER3,IOFFSET)
 ...
 CALL WDIALOGPUTREAL(IDF_REAL4,SEARCHDISTANCE,'(F10.2)')  
...
! CALL WDIALOGFIELDSTATE(IDF_MENU2,2)
  ...
  SELECT CASE (ITYPE)
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (IDOK)
      CALL WDIALOGGETCHECKBOX(IDF_CHECK1,NOSEARCH)
      NOSEARCH=ABS(NOSEARCH-1)
      CALL WDIALOGGETINTEGER(IDF_INTEGER1,MINP)
      CALL WDIALOGGETINTEGER(IDF_INTEGER2,MAXP)
      CALL WDIALOGGETINTEGER(IDF_INTEGER3,IOFFSET)
      CALL WDIALOGGETREAL(IDF_REAL1,SILL) 
      CALL WDIALOGGETREAL(IDF_REAL2,RANGE)  
      CALL WDIALOGGETREAL(IDF_REAL3,NUGGET)  
      CALL WDIALOGGETREAL(IDF_REAL4,SEARCHDISTANCE)  
 ...
 !###====================================================================
 SUBROUTINE KRIGINGSETTINGSFIELS()
 !###====================================================================
 IMPLICIT NONE
 INTEGER :: I
 ...
 END SUBROUTINE KRIGINGSETTINGSFIELS
