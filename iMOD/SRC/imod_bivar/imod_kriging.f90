...
!INTEGER,PRIVATE :: NCORESV

CONTAINS

 !###======================================================================
 SUBROUTINE KRIGING_MAIN(MD,XD,YD,ZD,IDF,IDFV,MINP,RANGE,SILL,NUGGET,KTYPE, &
               PNTSEARCH,LAGINTERVAL,LAGDISTANCE,COINCIDENT,COINCIDENTDIST,IQUADRANT,IBATCH,IBLANKOUT,BO_VALUE)     
 !###======================================================================
! USE OMP_LIB
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF,IDFV
 INTEGER,INTENT(IN) :: IBATCH,IBLANKOUT
...
! write(*,*) omp_get_num_threads()
 ...
 IF(IBLANKOUT.EQ.1)PNTSEARCH=1
 
 !## clean points for duplicates and get mean for simple kriging
 CALL KRIGING_INIT(MD,XD,YD,ZD,ND,IDF,KTYPE,COINCIDENT,COINCIDENTDIST,IBLANKOUT,BO_VALUE) 
...
!IF(ICOL.EQ.586.AND.IROW.EQ.337)THEN
!WRITE(*,*)
!ENDIF
   CALL IDFGETLOC(IDF,IROW,ICOL,X,Y)
   CALL KRIGING_FILLSYSTEM(XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,IQUADRANT,IDF,IBLANKOUT,BO_VALUE)
   IDF%X(ICOL,IROW) =KEST
   IDFV%X(ICOL,IROW)=IDFV%NODATA; IF(KVAR.GT.0.0)IDFV%X(ICOL,IROW)=SQRT(KVAR) !## standard deviation
  ENDDO
  IF(IBATCH.EQ.0)THEN
   IF(KTYPE.GT.0)THEN
 ...
 IF(IBATCH.EQ.0)THEN
  ...
  CALL GRAPH_PLOT('Distance','Semivariance (m2)',.FALSE.)
  IF(ALLOCATED(GRAPH))CALL GRAPH_DEALLOCATE()
 ELSE
 ...
 SUBROUTINE KRIGING_FILLSYSTEM(XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET, &
          KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,IQUADRANT,IDF,IBLANKOUT,BO_VALUE)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 INTEGER,INTENT(IN) :: ND,MINP,KTYPE,MX,PNTSEARCH,IQUADRANT,IBLANKOUT
 REAL,INTENT(OUT) :: KEST,KVAR
 REAL,INTENT(IN) :: X,Y,SILL,NUGGET,MZ,RANGE,BO_VALUE
 ...
 IF(PNTSEARCH.EQ.1)THEN
  ...
  DO ID=1,ND
   ...
   DXY=KRIGING_DIST(XD(ID),YD(ID),X,Y,0,IDF,IBLANKOUT,BO_VALUE,USERANGE)
...
! CLOSE(IU)
 ...
 !###======================================================================
 REAL FUNCTION KRIGING_DIST(X1,Y1,X2,Y2,IDATA,IDF,IBLANKOUT,BO_VALUE,MAXDIST)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 INTEGER,INTENT(IN) :: IDATA  !=0 FOR INSERTION IN KRIGING MATRIX  !=1 FOR AVERAGE POINT VALUES
 INTEGER,INTENT(IN) :: IBLANKOUT
 REAL,INTENT(IN) :: X1,Y1,X2,Y2,BO_VALUE,MAXDIST
 REAL :: X3,Y3,X4,Y4,XINTER,YINTER,A,SSX,DX,DY,D,CA,SA
 INTEGER :: I,J,I1,I2,ISTATUS,N,IROW,ICOL,IR,IC
 
 KRIGING_DIST=UTL_DIST(X1,Y1,X2,Y2)

 !## to far away
 IF(KRIGING_DIST.GT.MAXDIST)THEN
  RETURN
...
!  RETURN
 ...
 IF(IBLANKOUT.EQ.1)THEN
  !## aspect
  DY=(Y2-Y1); DX=(X2-X1); A=ATAN2(DY,DX)
  SSX=IDF%DX
  D=KRIGING_DIST
...
!     !## remove point if passed by nodata
!     IF(IDATA.EQ.0)THEN
!      KRIGING_DIST=-999.0 !USERANGE !-999.0
!      EXIT
!     !## increase distance to exclude from dataset for average computation
!     ELSEIF(IDATA.EQ.1)THEN
     KRIGING_DIST=MAXDIST !-999.0
!      KRIGING_DIST=KRIGING_DIST*10000.0 !## default
     EXIT
!     ENDIF
    ENDIF
...
!    ENDIF
 ...
 !###======================================================================
 SUBROUTINE KRIGING_INIT(MD,XD,YD,ZD,ND,IDF,KTYPE,COINCIDENT,COINCIDENTDIST,IBLANKOUT,BO_VALUE)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 INTEGER,INTENT(IN) :: MD,KTYPE,COINCIDENT,IBLANKOUT
 REAL,INTENT(INOUT),DIMENSION(MD) :: XD,YD,ZD 
 ...
 DO I=1,MD
  ...
  DO J=I+1,MD
   ...
   IF(KRIGING_DIST(XD(I),YD(I),XD(J),YD(J),1,IDF,IBLANKOUT,BO_VALUE,XYCRIT).LE.XYCRIT)THEN
    N=N+1
...
! nugget=5.0
 ...
 CALL KRIGING_INIT(MD,XD,YD,ZD,ND,IDF,KTYPE,COINCIDENT,COINCIDENTDIST,0,0.0)
 ...
 X=150.0; Y=150.0
 CALL KRIGING_FILLSYSTEM(XD,YD,ZD,ND,MINP,X,Y,RANGE,SILL,NUGGET,KTYPE,MZ,KEST,KVAR,NR,NC,MX,PNTSEARCH,IQUADRANT,IDF,0,0.0)
