...
!(14) NodataValues
 ...
 DO SHPI=1,MAX(1,SHP%NPOL)
  ...
  IF(IUSE(1).EQ.1)THEN
   ...
   ENDIF
  ELSE
   IR1 =1; IR2 =SELIDF(1)%NROW
   IC1 =1; IC2 =SELIDF(1)%NCOL
  ENDIF
    ...
    DO ICOL=IC1,IC2
     ...
     IF(LEX)THEN
      ...
      IF(JCOL.NE.0.AND.JROW.NE.0)THEN
       ...
       LX(1)=IDFEDITGETLOGICAL(ILOGICAL(1),X(1),IDFVAL(1),ICHECK(1),MP(IPLOT(1))%IDF%NODATA,MP(IPLOT(1))%IDF, &
                               JROW,JCOL,MP(IPLOT(1))%IDF%ITYPE)
       ...
       IF(IUSE(2).EQ.1)THEN
        ...
        IF(JCOL.NE.0.AND.JROW.NE.0)THEN
         ...
         LX(2)=IDFEDITGETLOGICAL(ILOGICAL(2),X(2),IDFVAL(2),ICHECK(2),MP(IPLOT(2))%IDF%NODATA,MP(IPLOT(2))%IDF, &
                                 JROW,JCOL,MP(IPLOT(1))%IDF%ITYPE)
 ...
 !###======================================================================
 LOGICAL FUNCTION IDFEDITGETLOGICAL(ILOGICAL,X,IDFVAL,ICHECK,NODATA,IDF,JROW,JCOL,IDFTYPE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ILOGICAL,ICHECK
 REAL(KIND=DP_KIND),INTENT(IN) :: IDFVAL,X,NODATA
 INTEGER,INTENT(IN) :: JROW,JCOL,IDFTYPE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 INTEGER,DIMENSION(4) :: DC,DR
 INTEGER :: IROW,ICOL,I,J
 DATA DC/ 0, 1, 0,-1/
 DATA DR/-1, 0, 1, 0/
 REAL(KIND=SP_KIND) :: XS,IS
 
 IDFEDITGETLOGICAL=.FALSE.
 IF(ICHECK.EQ.1.AND.IDFVAL.EQ.NODATA)RETURN
 
 !## in case single precision, convert for accurate comparison
 IF(IDFTYPE.EQ.4)THEN
  ...
  SELECT CASE (ILOGICAL)
   CASE (1)
    IF(IS.EQ.XS)IDFEDITGETLOGICAL=.TRUE.
   CASE (2)
    IF(IS.NE.XS)IDFEDITGETLOGICAL=.TRUE.
   CASE (3)
    IF(IS.LT.XS)IDFEDITGETLOGICAL=.TRUE.
   CASE (4)
    IF(IS.LE.XS)IDFEDITGETLOGICAL=.TRUE.
   CASE (5)
    IF(IS.GT.XS)IDFEDITGETLOGICAL=.TRUE.
   CASE (6)
    IF(IS.GE.XS)IDFEDITGETLOGICAL=.TRUE.
    ...
    IF(IS.EQ.XS)THEN
     J=0; DO I=1,SIZE(DC)
      ...
      IF(IROW.GT.IDF%NROW.OR.IROW.LT.1.OR. &
       ...
       J=1
      ELSE
       IS=REAL(IDFGETVAL(IDF,IROW,ICOL),4)
       IF(IS.NE.XS)J=1
      ENDIF
     ENDDO
   ...
   CASE (8,9,10) !## absolute spikes
    J=0; DO I=1,SIZE(DC)
     ...
     IF(ABS(REAL(IDFGETVAL(IDF,IROW,ICOL),4)-IS).GT.XS)J=J+1
    ENDDO  
   ...
   CASE (12) !## NaN - not a number
    IF(IS.NE.IS)IDFEDITGETLOGICAL=.TRUE.
   CASE (13) !## Inf
    IF(IS.GT. HUGE(1.0))IDFEDITGETLOGICAL=.TRUE.
    IF(IS.LT.-HUGE(1.0))IDFEDITGETLOGICAL=.TRUE.
   CASE (14) !## NodataValues
    IF(IS.EQ.REAL(NODATA,4))IDFEDITGETLOGICAL=.TRUE.
  END SELECT
...
!                                      TRIM(RTOS(MESSAGE%GY/100.0D0,'F',2))//'km)')
 ...
 IF(ICHECK.EQ.5)THEN
  ...
  ELSEIF(IINTOPT.EQ.3)THEN
   CALL KRIGINGSETTINGS(MAXPNT,KTYPE,RANGE,SILL,NUGGET,PNTSEARCH,COINCIDENT,COINCIDENTDIST,IQUADRANT,0,IBREAK)
   CALL WDIALOGSELECT(ID_DIDFEDITCALC); IF(IBREAK.EQ.1)RETURN
  ENDIF
 ENDIF
 ...
 !## new-value/nodata value
  ...
  CASE (5)
   ...
   IDF%XMIN=IDF%XMIN-(IDF%DX/2.0D0); IDF%YMIN=IDF%YMIN-(IDF%DY/2.0D0)
   IDF%XMAX=IDF%XMAX+(IDF%DX/2.0D0); IDF%YMAX=IDF%YMAX+(IDF%DY/2.0D0)
   ...
   DO IROW=1,NR; DO ICOL=1,NC
    IF(IDF%X(ICOL,IROW).EQ.1.0D0)THEN
     DO IR1=MAX(1,IROW-1),MIN(IROW+1,NR); DO IC1=MAX(1,ICOL-1),MIN(ICOL+1,NC)
      IF(IDF%X(IC1,IR1).EQ.0.0D0)IDF%X(IC1,IR1)=2.0D0
     ENDDO; ENDDO
    ENDIF
   ENDDO; ENDDO
   ND=0; DO IROW=1,NR; DO ICOL=1,NC; IF(IDF%X(ICOL,IROW).EQ.2.0)ND=ND+1; ENDDO; ENDDO 
   !## number data points
   ALLOCATE(XD(ND),YD(ND),ZD(ND),PD(ND),WD(ND))
   ...
   ND=0; DO IROW=1,NR; DO ICOL=1,NC
    IF(IDF%X(ICOL,IROW).EQ.2.0)THEN
     ...
     IF(JCOL.NE.0.AND.JROW.NE.0)THEN
      ...
      ENDIF
      ZD(ND)=0.0D0
      PD(ND)=IDFGETVAL(MP_CALC,JROW,JCOL)     
      !## remove point whenever equal to nodata
      IF(PD(ND).EQ.MP_CALC%NODATA)ND=ND-1
     ENDIF
    ENDIF
   ENDDO; ENDDO 
   ...
   SELECT CASE (IINTOPT)
    !## bivariate
    CASE (1)
     CALL BIVARIATE_INT(XD,YD,PD,ND,IOS,IDF)
    !## pcg solver
    CASE (2)
     CALL SOLID_PCGINT(XD,YD,PD,ND,IOS,IDF,1)
    !## kriging
    CASE (3)
     ...
     ENDDO; ENDDO
     CALL KRIGING_MAIN(SIZE(XD),XD,YD,ZD,PD,WD,IDF,STDEVIDF,0,0.0D0)
   END SELECT
...
!  ALLOCATE(SELIDF(1)%YSEL(2,ITHRD(SELIDF(1)%NCOL*SELIDF(1)%NROW))
 ...
 CALL WDIALOGPUTMENU(IDF_MENU2,TMPNAME,MPW%NACT,I)
 DEALLOCATE(TMPNAME)
...
! CALL WDIALOGPUTIMAGE(ID_SAVEAS,ID_ICONSAVEAS,1)
 ...
 CALL WDIALOGPUTMENU(IDF_MENU1,SCLNAMES_UP,SIZE(SCLNAMES_UP),2)
 CALL WDIALOGPUTMENU(IDF_MENU3,SCLNAMES_DOWN,SIZE(SCLNAMES_DOWN),1)
 ...
 CALL WDIALOGSELECT(ID_DIDFEDIT); CALL UTL_DIALOGSHOW(-0,65,0,2)
