...
USE MODPLOT
USE MOD_IDF, ONLY : IDFREAD,IDFGETVAL,IDFOPEN,IDFALLOCATEX,IDFDEALLOCATE,IDFGETLOC,IDFIROWICOL,IDFFILLSXSY, &
...
REAL,ALLOCATABLE,DIMENSION(:),PRIVATE :: XGRIDSIZE  !## gridsize inside the polygon
...
!     CALL IDFEDITGETGRIDSIZE()
 ...
 INTEGER :: I,II,J,IPLOT,ISCALE,JSCALE,NC,NR,N,IPP,I1,I2,NSX,NSY,MSX,MSY
 REAL :: DX,DY,X1,X2,Y1,Y2
 TYPE(IDFOBJ) :: IDFRESIZE
 REAL,POINTER,DIMENSION(:) :: SX,SY,SX_BU,SY_BU,SY_TMP
...
! SY=SY(NSY:1:-1) !## put array in reverse order
 ...
 DEALLOCATE(SY); ALLOCATE(SY(NSY)); SY=>SY_TMP
 DEALLOCATE(SY_TMP)
 
 DO II=1,SHPNO

  !## get position of shape j
  X1=MINVAL(SHPXC(1:SHPNCRD(II),II)); Y1=MINVAL(SHPYC(1:SHPNCRD(II),II));
 ...
 !## SET SX(NCOL+1)/SY OP IDFRESIZE%SX(0:NCOL)/SY
...
! SY=SY(NSY:1:-1)
 ...
 DEALLOCATE(SY); ALLOCATE(SY(NSY)); SY=>SY_TMP; DEALLOCATE(SY_TMP)

 DO I=1,NSY; IDFRESIZE%SY(I-1)=SY(I); ENDDO 
 IDFRESIZE%NCOL=NSX-1; IDFRESIZE%NROW=NSY-1; IDFRESIZE%IEQ=1
 
 IF(IDFREADSCALE(MP(IPLOT)%IDFNAME,IDFRESIZE,ISCALE,1,0.0,0))THEN
 ...
 DIFF_SXY=0.0
 DO I=2,NSXY
  DIFF_SXY=SXY(I)-SXY(I-1)
  IF(DIFF_SXY.LT.0.0)THEN
   WRITE(*,*) TRIM(ITOS(I)),TRIM(RTOS(DIFF_SXY,'F',1))
  ENDIF
 ENDDO
