...
USE MOD_OSD, ONLY : OSD_OPEN,ICF,OSD_GETENV,OSD_IOSTAT_MSG
...
!(13) Inf
  ...
  IF(LPOL)THEN
   ...
   IRLOOP: DO IROW=IR1,IR2
    ...
    DO ICOL=IC1,IC2
     ...
     LEX=.TRUE.
     IF(IUSE(1).EQ.1)THEN
      IF(UTL_INSIDEPOLYGON(XC,YC,SHPXC(:,SHPI),SHPYC(:,SHPI),SHPNCRD(SHPI)).NE.1)LEX=.FALSE.
     ENDIF
     IF(LEX)THEN
 ...
 SELECT CASE (ILOGICAL)
  ...
  CASE (13) !## Inf
   IF(IDFVAL.GT.HUGE(1.0))IDFEDITGETLOGICAL=.TRUE.
 END SELECT
...
!  ALLOCATE(SELIDF(1)%YSEL(2,ITHRD(SELIDF(1)%NCOL*SELIDF(1)%NROW))
 ...
 !###======================================================================
 SUBROUTINE IDFEDITNODATA(IDF_OLDND,IPLOT)
 !###======================================================================
 IMPLICIT NONE
 REAL,INTENT(IN) :: IDF_OLDND
 INTEGER,INTENT(IN) :: IPLOT
 INTEGER :: ICOL,IROW,JCOL,JROW,IU,IOS
 LOGICAL :: LEX
 CHARACTER(LEN=50) :: ERRTXT
 REAL :: XC,YC,DMIN,DMAX,IDFVAL
 
 DMIN=10.0E10; DMAX=-10.0E10
 
 !## Check if idf-file is already opened
 INQUIRE(FILE=MP(IPLOT)%IDFNAME,OPENED=LEX)
 IF(LEX)THEN
  INQUIRE(FILE=MP(IPLOT)%IDFNAME,NUMBER=IU)
  CLOSE(IU)
 ENDIF
 !## open idf-file that's need to be changed
 MP(IPLOT)%IDF%IU=UTL_GETUNIT() 
 IF(ICF.EQ.0)THEN
  CALL OSD_OPEN(MP(IPLOT)%IDF%IU,FILE=MP(IPLOT)%IDFNAME,STATUS='OLD',ACCESS='DIRECT',RECL=4, &
       ACTION='READWRITE',FORM='UNFORMATTED',IOSTAT=IOS)
 ELSEIF(ICF.EQ.1)THEN
  CALL OSD_OPEN(MP(IPLOT)%IDF%IU,FILE=MP(IPLOT)%IDFNAME,STATUS='OLD',ACCESS='DIRECT',RECL=1, &
       ACTION='READWRITE',FORM='UNFORMATTED',IOSTAT=IOS)
 ENDIF
 ...
 IF(IOS.NE.0)THEN
  ...
  RETURN
 ELSE 
  !## change nodata value based upon adjusted nodata variable
  DO ICOL=1,MP(IPLOT)%IDF%NCOL
   DO IROW=1,MP(IPLOT)%IDF%NROW
    IF(ICOL.NE.0.AND.IROW.NE.0)THEN   
     IDFVAL=IDFGETVAL(MP(IPLOT)%IDF,IROW,ICOL)
     IF(IDFVAL.EQ.IDF_OLDND)THEN
      CALL IDFPUTVAL(MP(IPLOT)%IDF,IROW,ICOL,MP(IPLOT)%IDF%NODATA)
     ELSE
      DMIN=MIN(DMIN,IDFVAL)
      DMAX=MAX(DMAX,IDFVAL)
     ENDIF
