...
!                                      TRIM(RTOS(MESSAGE%GY/100.0D0,'F',2))//'km)')
 ...
 !## new-value/nodata value
  ...
  CASE (5)
   ...
   IDF%XMIN=IDF%XMIN-(IDF%DX/2.0); IDF%YMIN=IDF%YMIN-(IDF%DY/2.0D0)
   IDF%XMAX=IDF%XMAX+(IDF%DX/2.0); IDF%YMAX=IDF%YMAX+(IDF%DY/2.0D0)
   ...
   !## number data points
   ALLOCATE(XD(ND),YD(ND),ZD(ND),WD(ND))
   ...
   SELECT CASE (IINTOPT)
    ...
    CASE (3)
     ...
     ENDDO; ENDDO
     CALL KRIGING_MAIN(SIZE(XD),XD,YD,ZD,WD,IDF,STDEVIDF,0,0.0D0)
   END SELECT
