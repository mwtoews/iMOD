...
USE RESOURCE
USE IMODHELP
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_MESSAGEHANDLE,UTL_WAITMESSAGE,UTL_IDFSNAPTOGRID,UTL_HIDESHOWDIALOG,&
                    IDFPLOT1BITMAP,IDFPLOT2BITMAP,UTL_INSIDEPOLYGON,UTL_IDFGETCLASS,UTL_WSELECTFILE
USE MOD_PREF_PAR, ONLY : PREFVAL
USE MODPLOT
USE MOD_IDF, ONLY : IDFREAD,IDFGETVAL,IDFOPEN,IDFALLOCATEX,IDFDEALLOCATE,IDFGETLOC,IDFIROWICOL, &
                    IDFDEALLOCATEX,IDFALLOCATESXY,IDFPUTVAL,IDFCOPY,IDFREADDIM,IDFNULLIFY,IDFREADSCALE,IDFWRITE
USE MOD_IDF_PAR, ONLY : IDFOBJ
USE MOD_POLYGON_PAR
USE MOD_POLYGON_UTL, ONLY : POLYGON1INIT,POLYGON1CLOSE,POLYGON1DEALLOCATE_SELIDF,POLYGON1FIELDS,POLYGON1IMAGES
...
USE MOD_OSD, ONLY : OSD_OPEN,ICF,OSD_GETENV
USE MOD_BIVARIATE, ONLY : BIVARIATE_INT
USE MOD_SOLID_PCG, ONLY : SOLID_PCGINT,MXITER1,MXITER2,HCLOSE,RCLOSE,ITIGHT,MICNVG,RELAX,IDAMPING
USE MOD_PCG, ONLY : PCGSETTINGS
USE MOD_IDFEDIT_TABLE, ONLY : UTL_EDITTABLE

TYPE(IDFOBJ),PRIVATE :: IDFSEL
INTEGER(KIND=1),ALLOCATABLE,DIMENSION(:,:),PRIVATE :: ITHRD
INTEGER :: IACTIONISEL  !## number of selection-action
REAL,ALLOCATABLE,DIMENSION(:) :: XGRIDSIZE  !## gridsize inside the polygon
...
CONTAINS
 ...
 CALL WDIALOGSELECT(MESSAGE%WIN) !ID_DIDFEDIT)
 ...
 IF(SUM(SHPIACT).NE.1)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You should select at least one polygon to asign a gridsize to it','Error')
  RETURN
 ENDIF
 
 DO IPOL=1,SIZE(SHPIACT); IF(SHPIACT(IPOL).EQ.1)EXIT; ENDDO; X=XGRIDSIZE(IPOL)
 
 CALL WDIALOGLOAD(ID_DGIVEREAL,ID_DGIVEREAL)
 CALL WDIALOGPUTSTRING(IDF_LABEL1,'Enter gridsize (meter) for the selected Polygon ['//TRIM(SHPNAME(IPOL))//']; current value is '//TRIM(RTOS(X,'F',2)))
 ...
 SHPIACT=0; SHPTYPE=ID_POLYGON; SHPWIDTH=2; SHPNO=0
 IF(MP(IPLOT)%IDF%IPG.EQ.0)THEN
  SHPNO=1; SHPIACT(1)=1; SHPCOLOR(1)=WRGB(255,0,0)
  SHPNCRD(1)=5
  SHPXC(1,1)=MP(IPLOT)%IDF%XMIN; SHPYC(1,1)=MP(IPLOT)%IDF%YMIN
  SHPXC(2,1)=MP(IPLOT)%IDF%XMAX; SHPYC(2,1)=MP(IPLOT)%IDF%YMIN
  SHPXC(3,1)=MP(IPLOT)%IDF%XMAX; SHPYC(3,1)=MP(IPLOT)%IDF%YMAX
  SHPXC(4,1)=MP(IPLOT)%IDF%XMIN; SHPYC(4,1)=MP(IPLOT)%IDF%YMAX    
  SHPXC(5,1)=MP(IPLOT)%IDF%XMIN; SHPYC(5,1)=MP(IPLOT)%IDF%YMIN
  XGRIDSIZE(1)=MP(IPLOT)%IDF%DX
  SHPNAME(1)='GridSize'//TRIM(ITOS(1))//':'//TRIM(RTOS(XGRIDSIZE(1),'F',2))//'m'
 ELSE
 ...
 CALL POLYGON1FIELDS(ID_DIDFEDIT)
 CALL IDFPLOTFAST(1)

 END SUBROUTINE IDFEDITGETGRIDSIZE
 ...
 INTEGER :: I,J,IPLOT,ISCALE,JSCALE,NC,NR,N,IPP
 REAL :: DX,DY
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: IDFRESIZE
 REAL,ALLOCATABLE,DIMENSION(:) :: X,Y,IPX,ITX,IPY,ITY
  
 !## use all polygons ... doesnot matter if they are selected
 IF(SHPNO.LE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You should create polygons first that determine the gridsizes'//CHAR(13)// &
    'Use the option [Get Current GridSizes] to start with the current IDF definition','Error')
  RETURN
 ENDIF
 DO I=1,SHPNO
  IF(XGRIDSIZE(I).LE.0.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You should give each polygon an appropriate cellsize (>0.0).'//CHAR(13)// &
     'Polygon '//TRIM(SHPNAME(I))//' does have a gridsize of '//TRIM(RTOS(XGRIDSIZE(I),'F',2))//' meter','Error')
   RETURN
  ENDIF
 ENDDO
 
 CALL WDIALOGSELECT(ID_DIDFEDITTAB2); CALL WDIALOGGETMENU(IDF_MENU2,IPLOT)  
 CALL WDIALOGGETMENU(IDF_MENU1,ISCALE); CALL WDIALOGGETMENU(IDF_MENU3,JSCALE)

 IF(ID.EQ.ID_SAVEAS)THEN
  IDFNAME=MP(IPLOT)%IDFNAME(:INDEX(MP(IPLOT)%IDFNAME,'\',.TRUE.))//'*.IDF'
 ...
 ALLOCATE(IDFRESIZE(1)); CALL IDFNULLIFY(IDFRESIZE(1))
 CALL IDFCOPY(MP(IPLOT)%IDF,IDFRESIZE(1))
  
 !## get sx(.) and sy(.)
 IF(SHPNO.EQ.1)THEN
  IDFRESIZE(1)%IEQ=0; IDFRESIZE(1)%DX=XGRIDSIZE(1); IDFRESIZE(1)%DY=XGRIDSIZE(1)
 ELSE
  IDFRESIZE(1)%IEQ=1 
  N=SHPNO*2
  ALLOCATE(X(N),Y(N),IPX(N),ITX(N),IPY(N),ITY(N))
 
  !## get zones of different gridsizes X-DIRECTION
  I=0; DO J=1,SHPNO
  I=I+1
  X(I)=MINVAL(SHPXC(1:SHPNCRD(J),J)); IPX(I)=J; ITX(I)=1
  Y(I)=MINVAL(SHPYC(1:SHPNCRD(J),J)); IPY(I)=J; ITY(I)=1
  I=I+1
  X(I)=MAXVAL(SHPXC(1:SHPNCRD(J),J)); IPX(I)=J; ITX(I)=2
  Y(I)=MAXVAL(SHPYC(1:SHPNCRD(J),J)); IPY(I)=J; ITY(I)=2
  ENDDO
  DO I=1,SHPNO*2
   X(I)=MAX(X(I),IDFRESIZE(1)%XMIN); X(I)=MIN(X(I),IDFRESIZE(1)%XMAX)
   Y(I)=MAX(Y(I),IDFRESIZE(1)%YMIN); Y(I)=MIN(Y(I),IDFRESIZE(1)%YMAX)
  ENDDO
  !## sort boundaries x/y direction
  CALL SORTEM(1,SHPNO*2,X,2,IPX,ITX,X,X,X,X,X)
  CALL SORTEM(1,SHPNO*2,Y,2,IPY,ITY,Y,Y,Y,Y,Y)
  
  CALL IDFEDITGRIDSIZE_SXY(IDFRESIZE(1)%NCOL,X,ITX,IPX,IDFRESIZE(1)%XMAX)
  CALL IDFEDITGRIDSIZE_SXY(IDFRESIZE(1)%NROW,Y,ITY,IPY,IDFRESIZE(1)%YMAX)
  
  ALLOCATE(IDFRESIZE(1)%SX(0:IDFRESIZE(1)%NCOL),IDFRESIZE(1)%SY(0:IDFRESIZE(1)%NROW))
  
  CALL IDFEDITGRIDSIZE_FILLSXY(IDFRESIZE(1)%NCOL,X,IPX,IDFRESIZE(1)%SX)
  CALL IDFEDITGRIDSIZE_FILLSXY(IDFRESIZE(1)%NROW,Y,IPY,IDFRESIZE(1)%SY)
  !## swap y coordinates
  DO I=0,IDFRESIZE(1)%NROW/2
   DY=IDFRESIZE(1)%SY(IDFRESIZE(1)%NROW-I)
   IDFRESIZE(1)%SY(IDFRESIZE(1)%NROW-I)=IDFRESIZE(1)%SY(I)
   IDFRESIZE(1)%SY(I)=DY
  ENDDO
  DEALLOCATE(X,Y,IPX,ITX,IPY,ITY)

 ENDIF
 
 IF(IDFREADSCALE(MP(IPLOT)%IDFNAME,IDFRESIZE(1),ISCALE,1,0.0,0))THEN
  IF(.NOT.IDFWRITE(IDFRESIZE(1),IDFNAME,1))THEN; ENDIF
 ENDIF

 CALL IDFDEALLOCATE(IDFRESIZE,1); DEALLOCATE(IDFRESIZE)

 END SUBROUTINE IDFEDITSAVEGRIDSIZE

 !###======================================================================
 SUBROUTINE IDFEDITGRIDSIZE_SXY(N,X,IT,IP,XMAX)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(OUT) :: N
 REAL,INTENT(IN),DIMENSION(:) :: IT
 REAL,INTENT(IN) :: XMAX
 REAL,INTENT(INOUT),DIMENSION(:) :: X,IP
 INTEGER :: IPP,I,NN,ISIZE
 REAL :: D,XSIZE,YSIZE

 !## get x values
 N=0; IPP=0; DO I=1,(SHPNO*2)-1
  IF(IT(I).EQ.1)IPP=IP(I)
  IF(IT(I).EQ.2)THEN
   IF(IT(I+1).EQ.1)IPP=IP(I-2)
   IF(IT(I+1).EQ.2)IPP=IP(I+1)
  ENDIF
  IP(I)=IPP
  D=X(I+1)-X(I); NN=D/XGRIDSIZE(IPP) !; IF(MOD(DX,CS).EQ.0.0)NC=NC-1  
  X(I+1)=X(I)+NN*XGRIDSIZE(IPP)
  N=N+NN
 ENDDO
 
 !## continue to distribute rest of model
 DO
  IF(X(SHPNO*2).EQ.XMAX)EXIT

  D=XMAX-X(SHPNO*2)
  !## find largest to try to fit rest product
  YSIZE=10.0E10
  DO
   XSIZE=0.0; ISIZE=0
   DO I=1,SIZE(IP)-1
    IF(XGRIDSIZE(IP(I)).GT.XSIZE.AND.XGRIDSIZE(IP(I)).LT.YSIZE)THEN
     XSIZE=XGRIDSIZE(IP(I))
     ISIZE=IP(I)
    ENDIF
   ENDDO
   IF(ISIZE.EQ.0)THEN
    CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can not ensure that the total gridsize remains identical!'//CHAR(13)// &
       'After inserting the refinements/coarsenings, there remains a rest of '//TRIM(RTOS(D,'F',2))//'m','Error')
    EXIT
   ENDIF
   YSIZE=XGRIDSIZE(ISIZE)
   !## try to put rest in 
   NN=D/XGRIDSIZE(ISIZE)
   !## okay can put in some cells, do it
   IF(NN.GT.0)THEN
    N=N+NN
    DO I=ISIZE+1,SIZE(X)
     X(I)=X(I)+NN*XGRIDSIZE(ISIZE)
    ENDDO
    EXIT
   ENDIF
 ...
 END SUBROUTINE IDFEDITGRIDSIZE_SXY

 !###======================================================================
 SUBROUTINE IDFEDITGRIDSIZE_FILLSXY(N,X,IP,SXY)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: N
 REAL,INTENT(OUT),DIMENSION(0:N) :: SXY
 REAL,INTENT(IN),DIMENSION(:) :: X,IP
 INTEGER :: IPP,I,J,K,NN
 REAL :: D 

 SXY(0)=X(1)

 !## get x values
 K=0; DO I=1,(SHPNO*2)-1
  IPP=IP(I); D=X(I+1)-X(I); NN=D/XGRIDSIZE(IPP) 
  DO J=1,NN; K=K+1; SXY(K)=SXY(K-1)+XGRIDSIZE(IPP); ENDDO
 ENDDO
 
 END SUBROUTINE IDFEDITGRIDSIZE_FILLSXY
...
! FNAME=TRIM(PREFVAL(1))//'\TMP\TMPSELECTED'//TRIM(ITOS(IACTIONISEL))//'.DAT'
 ...
 CALL OSD_OPEN(IU,FILE=FNAME,STATUS='OLD',ACTION='READ',FORM='UNFORMATTED',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can not READ current file'//CHAR(13)// &
    TRIM(FNAME),'Error')
  RETURN
 ENDIF
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can not WRITE current file'//CHAR(13)// &
    TRIM(FNAME),'Error')
  RETURN
 ENDIF
 ...
 IMPLICIT NONE
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: ITYPE,ICOL,IROW,IBUTDOWN,I,J,IMODE,CRDITYPE,JCOL,JROW !,IPLOT
 REAL :: IDFVALUE
 LOGICAL :: LEX
 ...
 DO 
  ...
  SELECT CASE (ITYPE)
   ...
   CASE (MENUSELECT)
    CALL IMOD1MENUSELECT(MESSAGE) !,CRDITYPE)
...
!      CALL IDFEDITALLOCATE()
 ...
 !     CALL WMENUFLOATING(ID_MENU4,MESSAGE%X,MESSAGE%Y)
   ...
   CASE (MOUSEMOVE)
    ...
    CALL IDFIROWICOL(SELIDF(1),IROW,ICOL,MESSAGE%GX,MESSAGE%GY)
    !## position with current selidf
    IF(ICOL.NE.0.AND.IROW.NE.0)THEN

     CALL WINDOWSELECT(0)
     CALL WINDOWOUTSTATUSBAR(1,'(x:'//TRIM(RTOS(MESSAGE%GX,'F',2))//' m ,y:'// &
...
! CALL WDIALOGFIELDSTATE(ID_SHOW,I)
 ...
 ENDIF
 CALL WDIALOGPUTSTRING(IDF_CHECK3,'Skip NoDataValue ('//TRIM(RTOS(MP(IPLOT(1))%IDF%NODATA,'F',3))//')')
 CALL WDIALOGPUTSTRING(IDF_CHECK4,'Skip NoDataValue ('//TRIM(RTOS(MP(IPLOT(2))%IDF%NODATA,'F',3))//')')
 ...
 INTEGER :: ITYPE,IROW,ICOL,IR1,IR2,IC1,IC2,IRAT1,IRAT,JROW,JCOL,IADDSEL
 REAL :: XMIN,YMIN,XMAX,YMAX,XC,YC
 REAL,DIMENSION(2) :: X,IDFVAL
...
!  CALL IDFPLOTFAST(0)
 ...
 DO SHPI=1,MAX(1,SHPNO)

  LPOL=.TRUE.
  IF(IUSE(1).EQ.1)THEN
   LPOL=.FALSE.
   IF(SHPIACT(SHPI).EQ.1.AND.SHPTYPE(SHPI).EQ.ID_POLYGON)THEN
    XMIN=MINVAL(SHPXC(1:SHPNCRD(SHPI),SHPI))
    XMAX=MAXVAL(SHPXC(1:SHPNCRD(SHPI),SHPI))
    YMIN=MINVAL(SHPYC(1:SHPNCRD(SHPI),SHPI))
    YMAX=MAXVAL(SHPYC(1:SHPNCRD(SHPI),SHPI))
  ...
  IF(LPOL)THEN
   ...
   IRLOOP: DO IROW=IR1,IR2
    ...
    DO ICOL=IC1,IC2
     ...
     LEX=.TRUE.
     IF(IUSE(1).EQ.1)THEN
      IF(UTL_INSIDEPOLYGON(XC,YC,SHPXC(:,SHPI),SHPYC(:,SHPI),XPOL,YPOL,SHPNCRD(SHPI)).NE.1)LEX=.FALSE.
     ENDIF
     IF(LEX)THEN
      ...
      IF(JCOL.NE.0.AND.JROW.NE.0)THEN
       ...
       IF(LX(1))THEN
        ...
        END SELECT
!        IF(IADDSEL.EQ.1.OR.IADDSEL.EQ.2)THEN
!         !## new selection
!         ITHRD(ICOL,IROW)=INT(1,1)
!        ELSE
!         !## delete from
...
!        ENDIF
 ...
 REAL,INTENT(IN) :: IDFVAL,X,NODATA
...
!      CALL POLYGON1DRAWYSEL()
    ...
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (1)
      ...
      IF(IDRAW.EQ.1)THEN
       ...
       CALL WDIALOGGETREAL(IDF_REAL1,XSTOP)
       
       INQUIRE(FILE=MP(IPLOT)%IDFNAME,OPENED=LEX)
       IF(.NOT.LEX)THEN
        IF(.NOT.IDFOPEN(MP(IPLOT)%IDF%IU,MP(IPLOT)%IDFNAME,'R',0,0))EXIT !## stop pipet action
       ELSE
       ...
       ELSE
        CALL IDFEDITTRACE(MP(IPLOT)%IDF,SELIDF(1),THREAD,YSEL,ISPEC,DTERM,IMENU,MAXTHREAD,MAXN,IDFVALUE,NTHREAD,1,I,STOPVALUE=XSTOP)
       ENDIF
   ...
   CASE (MOUSEMOVE)
    ...
    IF(MP(IPLOT)%IDF%IU.LE.0)THEN
     IF(.NOT.IDFOPEN(MP(IPLOT)%IDF%IU,MP(IPLOT)%IDFNAME,'R',0,0))THEN; ENDIF
    ENDIF
    CALL IDFIROWICOL(MP(IPLOT)%IDF,IROW,ICOL,MESSAGE%GX,MESSAGE%GY)
    IF(IROW.NE.0.AND.ICOL.NE.0)THEN
     IDFVALUE=IDFGETVAL(MP(IPLOT)%IDF,IROW,ICOL)
     CALL WINDOWOUTSTATUSBAR(1,'(x:'//TRIM(RTOS(MESSAGE%GX/100.0,'F',2))//'km ,y:'// &
                                      TRIM(RTOS(MESSAGE%GY/100.0,'F',2))//'km)')
...
! CALL WDIALOGSELECT(ID_DIDFEDIT)
 ...
 IF(IACTION.NE.0)THEN
  CALL WMESSAGEBOX(YESNO,QUESTIONICON,COMMONOK,'Changes can not be restored after you left this editing mode'//CHAR(13)//CHAR(13)//&
 ...
 INTEGER :: IOS,IROW,ICOL,JROW,JCOL,IACTION,ICHECK,NSMOOTH,I,J,IR,IC,IPLOT, &
     ICALC,N,ISAVE,JPLOT,IC1,IC2,IR1,IR2,NR,NC,IDIST,KCOL,KROW,NBUFFER,IRAT,&
     IRAT1,IINTOPT,ITABLE,JCALC, IBREAK
 INTEGER :: IU,JU,ND
 REAL :: IDFVAL,XT,NT,DMIN,DMAX
 REAL :: X,D,TD 
 LOGICAL :: LEX
 REAL,POINTER,DIMENSION(:) :: XD,YD,ZD
 TYPE(IDFOBJ) :: IDF,MP_CALC
 ...
 IF(ICHECK.EQ.5.AND.IINTOPT.EQ.2)THEN
  CALL PCGSETTINGS(MXITER1,MXITER2,HCLOSE,RCLOSE,ITIGHT,MICNVG,RELAX,IDAMPING,IBREAK)
  CALL WDIALOGSELECT(ID_DIDFEDITCALC)
 ...
 IF(ICHECK.EQ.3)THEN
  ...
  IF(IPLOT.EQ.JPLOT)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You can not copy FROM and TO an identical IDF','Error'); RETURN 
  ENDIF
 ENDIF
 ...
 IF(IOS.NE.0)THEN
  ...
  IF(IU.GT.0)CLOSE(IU) ! ,STATUS='DELETE')
  RETURN
 ENDIF
 IF(.NOT.IDFREADDIM(0,MP_CALC))THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can not read header information for file'//CHAR(13)// &
 ...
 !## new-value/nodata value
  ...
  CASE (3)
   IF(IDFOPEN(MP(JPLOT)%IDF%IU,MP(JPLOT)%IDFNAME,'R',0,0))THEN
  ...
  CASE (5)
   ...
   CALL IDFNULLIFY(IDF); IDF%IEQ=0; IDF%ITB=0
   IDF%DX=SELIDF(1)%DX; IDF%DY=SELIDF(1)%DY; IDF%NODATA=0.0
   ...
   IF(.NOT.IDFALLOCATEX(IDF))THEN; ENDIF; IDF%X=0.0
   !## mark fixed points
   DO J=1,SELIDF(1)%NTHREAD
    ICOL=INT(SELIDF(1)%YSEL(1,J)); IROW=INT(SELIDF(1)%YSEL(2,J))
    CALL IDFEDITGETCURRENTNODE(ICOL,IROW,IPLOT,JCOL,JROW)
   ...
   ND=0; DO IROW=1,NR; DO ICOL=1,NC
    IF(IDF%X(ICOL,IROW).EQ.2.0)THEN
     ...
     IF(JCOL.NE.0.AND.JROW.NE.0)THEN
      ND=ND+1
      !## bivariate (x,y,z)
      IF(IINTOPT.EQ.1)CALL IDFGETLOC(IDF,IROW,ICOL,XD(ND),YD(ND))
      !## pcg solver (icol,irow,z)
      IF(IINTOPT.EQ.2)THEN; XD(ND)=REAL(ICOL); YD(ND)=REAL(IROW); ENDIF
...
!     CALL KRIGING_MAIN(ND,XD,YD,ZD,IDF,MINP,MAXP,RANGE,SILL,NUGGET)
   ...
   !## succesfully completed interpolation
   IF(IOS.EQ.0)THEN   
    IC1=MINVAL(SELIDF(1)%YSEL(1,1:N))-1; IC2=MAXVAL(SELIDF(1)%YSEL(1,1:N))+1
    IR1=MINVAL(SELIDF(1)%YSEL(2,1:N))-1; IR2=MAXVAL(SELIDF(1)%YSEL(2,1:N))+1
    IC1=MAX(1,IC1); IC2=MIN(SELIDF(1)%NCOL,IC2)
 ...
 CASE (6)
  ...
  IF(IOS.NE.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD can not read the spreadsheet from the temporary file:'//CHAR(13)// &
 ...
 WRITE(MP_CALC%IU,REC=7+ICF) MP_CALC%DMIN
 WRITE(MP_CALC%IU,REC=8+ICF) MP_CALC%DMAX
 CLOSE(MP_CALC%IU)
 CALL IDFDEALLOCATEX(MP_CALC)
 ...
 IF(LEX)THEN
  ...
  IF(.NOT.IDFREADDIM(0,IDF))THEN
   ...
   RETURN
  ENDIF
!  IF(.NOT.IDFREAD(IDF,FNAME,-1,0))THEN
!  ENDIF
  DO
...
!  ALLOCATE(SELIDF(1)%YSEL(2,ITHRD(SELIDF(1)%NCOL*SELIDF(1)%NROW))
 ...
 CALL MAIN1INACTMODULE(ID_IDFEDIT)
 ...
 IF(ALLOCATED(XGRIDSIZE))DEALLOCATE(XGRIDSIZE); ALLOCATE(XGRIDSIZE(MAXSHAPES)); XGRIDSIZE=0.0
