...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),PARAMETER :: TRATIO=1.0D0
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: XCRIT
 REAL(KIND=DP_KIND),INTENT(IN),OPTIONAL :: X2CRIT,STOPVALUE
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(:),OPTIONAL :: DZ
 ...
 INTEGER :: N,IR,IC,JR,JC,IDIR,JROW,JCOL,I,II,JJ,N1,N2,N3,M1,M2,MAXDIR
 INTEGER(KIND=1),POINTER,DIMENSION(:) :: ISPEC,ISPEC_BU 
 INTEGER(KIND=2),POINTER,DIMENSION(:,:) :: THREAD,YSEL,THREAD_BU,YSEL_BU
 REAL(KIND=DP_KIND) :: IDFV,ZCRIT,ZMAX,XTHRESHOLD
 ...
 LTHRESHOLD=.FALSE.; IF(PRESENT(THRESHOLD))LTHRESHOLD=.TRUE.; XTHRESHOLD=0.0D0
 ...
 NLOOP: DO WHILE(N.GT.0)
  ...
  IDIR    =ISPEC(N)+1
  CALL IDFEDITGETDIR(JCOL,JROW,IR,IC,IDIR,DTERM,YSEL,NTHREAD,IPZ,IDF=IDF,IDFP=IDFP,DZ=DZ) 
  ISPEC(N)=INT(IDIR,1)
    ...
    IF(LGETY(YSEL,IDFP,NTHREAD,MAXTHREAD,IC,IR,DTERM))THEN
     ...
     IF(LCORLOC(IDFV,ZCRIT,IDF%NODATA,ABS(IMENU),XTHRESHOLD,X2CRIT))THEN
      ...
      IF(DTERM.EQ.2.OR.DTERM.EQ.3)THEN
       ...
       IF(LPTRACE)THEN
        IF(PTRACE%X(IC,IR).GT.0.0D0)THEN
 ...
 !###====================================================
 SUBROUTINE IDFEDITGETDIR(JCOL,JROW,IR,IC,IDIR,DTERM,YSEL,NTHREAD,IPZ,IDF,IDFP,DZ) 
 !###====================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN),OPTIONAL :: IDF,IDFP 
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(OUT),OPTIONAL :: DZ
 INTEGER,INTENT(IN) :: JCOL,JROW,DTERM,IPZ
 INTEGER,INTENT(INOUT) :: NTHREAD
 INTEGER,INTENT(OUT) :: IC,IR
 INTEGER,INTENT(INOUT) :: IDIR
 INTEGER(KIND=2),POINTER :: YSEL(:,:) 
 INTEGER :: I,IZ,JC,JR,IDZ,ISPILL
 REAL(KIND=DP_KIND) :: MINZ,Z,MZ
 ...
 ELSEIF(DTERM.EQ.2.OR.DTERM.EQ.3)THEN
  ...
  CALL IDFEDIT_GETDZ(IDF%X(IC,IR),IDF,IDFP,IC,IR,DZ,IDZ,IPZ,DTERM)
...
!RETURN
  ...
  DO I=NTHREAD,1,-1
   ...
   CALL IDFEDIT_GETDZ(IDF%X(JC,JR),IDF,IDFP,JC,JR,DZ,IDZ,IPZ,DTERM)
   MZ=MINVAL(DZ)

   !## ispill=1: spill level; ispill=0: dropdown
   ISPILL=1; IF(MZ.LT.0.0D0)ISPILL=0
...
!   ENDIF
 ...
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(:) :: DZ
 INTEGER :: IDIR
 REAL(KIND=DP_KIND) :: MINZ,MAXZ
...
!  IF(MAXZ.LT.0.0D0)RETURN
  IF(MAXZ.LE.0.0D0)RETURN
 ...
 !###====================================================
 SUBROUTINE IDFEDIT_GETGRAD(Z,IDF,IDFP,JCOL,JROW,DZ,IDZ,IPZ,DTERM)
 !###====================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: Z
 TYPE(IDFOBJ),INTENT(IN) :: IDF,IDFP
 INTEGER,INTENT(IN) :: JCOL,JROW,IPZ,DTERM
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(OUT) :: DZ
 ...
 IF(DTERM.EQ.3)THEN
  ...
  DXY=SQRT(IDF%DX**2.0D0+IDF%DY**2.0D0)
  DZ(5)=DZ(5)/DXY
  DZ(6)=DZ(6)/DXY
  DZ(7)=DZ(7)/DXY
  DZ(8)=DZ(8)/DXY
 ...
 !###====================================================
 SUBROUTINE IDFEDIT_GETDZ(Z,IDF,IDFP,JCOL,JROW,DZ,IDZ,IPZ,DTERM)
 !###====================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(IN) :: Z
 TYPE(IDFOBJ),INTENT(IN) :: IDF,IDFP
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: ZCRIT,IDFVAL,NODATA,XTHRESHOLD
 REAL(KIND=DP_KIND),INTENT(IN),OPTIONAL :: Z2CRIT
