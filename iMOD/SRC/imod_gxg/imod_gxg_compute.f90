...
CONTAINS
 ...
 REAL :: RS,SUMLG,SUMHG,SUMLGX,SUMHGX,NGXG,MV,XC,YC,NODATA,XMIN,YMIN,XMAX,YMAX
...
!       MVIROW.GE.1.AND.MVIROW.LE.MVIDF%NROW)THEN
      ...
      DO JAAR=1,NYR
       ...
       IF(GXG_HGLG3.EQ.1)THEN
        SUMLGX=LG(1,JAAR)+LG(2,JAAR)+LG(3,JAAR)
        SUMHGX=HG(1,JAAR)+HG(2,JAAR)+HG(3,JAAR)
        GXGLH(1,JAAR)%X(ICOL,IROW)=SUMLGX/REAL(3)
        GXGLH(2,JAAR)%X(ICOL,IROW)=SUMHGX/REAL(3)
        IF(IUSEMV.EQ.1)THEN
         GXGLH(1,JAAR)%X(ICOL,IROW)=MV-(SUMLGX/REAL(3))
         GXGLH(2,JAAR)%X(ICOL,IROW)=MV-(SUMHGX/REAL(3))
        ENDIF
       ENDIF
      ENDDO
