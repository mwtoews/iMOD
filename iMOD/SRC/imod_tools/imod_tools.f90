...
CONTAINS
    ...
    CASE(PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      CASE (IDHELP)
       SELECT CASE (TOOLSID)
        CASE (ID_WBAL)
          CALL IMODGETHELP('5.12','Compute Waterbalance')     
        CASE (ID_GXG)
          CALL IMODGETHELP('5.13','Compute Mean Groundwaterfluctuations (GxG)')
        CASE (ID_MEAN)
          CALL IMODGETHELP('5.14','Compute Mean Values')
        CASE (ID_TS)
          CALL IMODGETHELP('5.15','Compute Timeseries')
       END SELECT
      CASE (IDCANCEL)
       CALL TOOLS1CLOSE()
      CASE (IDOK)
 ...
 !###======================================================================
 USE MOD_WBAL_PAR
 USE MOD_PROFILE_UTL, ONLY : GRAPH,PROFILE_PLOTGRAPH,PROFILE_DEALLGRAPH,PROFILE_ALLGRAPH !,PROFILE_GETFORMAT
 IMPLICIT NONE
 INTEGER :: I,J,K,IY,IM,ID,ICSV,JPERIOD,IERROR
...
! CALL TSBATCH(TRIM(PREFVAL(1))//'\TMP\BATCH.TXT',LOKAY)
 ...
 IF(LOKAY)THEN
  ...
  CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfully added timeserie information to'//CHAR(13)// &
 ...
 INTEGER,ALLOCATABLE,DIMENSION(:) :: TLAY !,IYEAR
 ...
 IF(SUM(TP%IACT).NE.0)THEN
  ...
  DO I=1,SIZE(ILIST)
   IF(ILIST(I).EQ.1)THEN
    ...
    IF(N.GT.0)THEN
     IF(ALLOCATED(LISTIDF))DEALLOCATE(LISTIDF)
     ALLOCATE(LISTIDF(N))
...
! CALL WDIALOGFIELDSTATE(IDF_STRING1,2)
 ...
 CALL WDIALOGPUTSTRING(IDF_CHECK2,'Use system files [*_sys{i}_*.idf], max systems='//TRIM(ITOS(MXSYS)))

 SELECT CASE (TOOLSID)
  CASE (ID_WBAL)
   CALL WDIALOGFIELDSTATE(IDF_GROUP7,1)
   CALL WDIALOGFIELDSTATE(IDF_CHECK2,1)
  CASE (ID_GXG,ID_MEAN,ID_TS)
   CALL WDIALOGFIELDSTATE(IDF_GROUP7,0)
