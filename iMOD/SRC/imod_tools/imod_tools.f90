...
! CALL TSBATCH(TRIM(PREFVAL(1))//'\TMP\BATCH.TXT',LOKAY)
 ...
 IF(SUM(TP%IACT).NE.0)THEN
  ...
  CALL WDIALOGRANGEPROGRESSBAR(IDF_PROGRESS1,1,SIZE(ILIST))

  DO I=1,SIZE(ILIST)
   
   CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,2)
   ...
   IF(N.GT.0)THEN
    ...
    CALL WDIALOGRANGEPROGRESSBAR(IDF_PROGRESS2,1,N)
    ...
    !## get mindate/maxdate
    CALL UTL_IDFGETDATES(LISTIDF,SIZE(LISTIDF),IS,IT,DATE1,DATE2,IDF_PROGRESS2)
...
! CALL WDIALOGFIELDSTATE(IDF_STRING1,2)
 ...
 TOOLSNDIR=0; CALL TOOLS1FIELDS1(0)

 CALL POLYGON1INIT(); CALL POLYGON1FIELDS(ID_TOOLSTAB3); CALL TOOLS1FIELDS3()
