...
CONTAINS
 ...
 IF(WBAL_ISEL.EQ.2)CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,TRIM(PREFVAL(1))//'\TMP\tmp.gen')
...
! CALL TSBATCH(TRIM(PREFVAL(1))//'\TMP\BATCH.TXT',LOKAY)
 ...
 ELSEIF(ISEL.EQ.2)THEN
  ...
  CALL POLYGON1SAVELOADSHAPE(ID_SAVESHAPE,GENFNAME,ISAVESEL=1)
 ...
 !## load all polygons
 SHP%NPOL=0
 CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,TRIM(PREFVAL(1))//'\TMP\TMP.GEN')
