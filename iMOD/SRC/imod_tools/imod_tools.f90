...
USE IMODVAR, ONLY : DP_KIND,SP_KIND,IDIAGERROR,MXTP,TP !,MXSYS
USE MODPLOT
USE DATEVAR
USE MOD_COLOURS
...
USE MOD_GXG_CLC, ONLY : FGLG,FGHG,FGVG,FGT,FNLEG,GXG1_COMPUTEGXG,GXG1_ABORT
...
CONTAINS
      ...
      CASE (IDHELP)
       SELECT CASE (TOOLSID)
        CASE (ID_WBAL_GENERATE)
          CALL UTL_GETHELP('5.12','TMO.CompWatBal')     
        CASE (ID_GXG)
          CALL UTL_GETHELP('5.13','TMO.GxG')
        CASE (ID_MEAN)
          CALL UTL_GETHELP('5.14','TMO.CompMeanVal')
        CASE (ID_TS)
          CALL UTL_GETHELP('5.15','TMO.CompTimeSer')
       END SELECT
      CASE (IDCANCEL)
       CALL TOOLS_CLOSE()
 ...
 IF(TOOLSID.EQ.ID_TS)THEN
  ...
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=FNAME)!,'')
 ENDIF
 ...
 INTEGER :: I,J,K,IY,IM,ID,ICSV,JPERIOD,IERROR,IU,IWIN,ICLOSE,IHR,IMT,ISC,IBAL,NSYS,KLAY,JLAY,NU,II
 CHARACTER(LEN=500) :: STRING
 CHARACTER(LEN=256) :: ROOT
 LOGICAL :: LEX
 INTEGER(KIND=DP_KIND),POINTER,DIMENSION(:) :: ISYS,ISYS_BU
 INTEGER(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: LSYS
 ...
 CALL WDIALOGSELECT(ID_TOOLSTAB1)
 CALL WDIALOGGETMENU(IDF_MENU2,ILIST)
 ALLOCATE(WCTP(SUM(ILIST)))
 J=0; DO I=1,SIZE(ILIST); IF(ILIST(I).EQ.0)CYCLE; J=J+1; WCTP(J)%BDGNAME=LISTNAME(I); ENDDO
 ...
 IF(WBAL_ISTEADY.EQ.0)THEN
  ...
  WBAL_FYR=YMDHMSTOITIME(IY,IM,ID,IHR,IMT,ISC)
  CALL WDIALOGGETINTEGER(IDF_INTEGER5 ,ID)
  CALL WDIALOGGETINTEGER(IDF_INTEGER6 ,IY)
  CALL WDIALOGGETINTEGER(IDF_INTEGER14,IHR)
  CALL WDIALOGGETINTEGER(IDF_INTEGER13,IMT)
  CALL WDIALOGGETINTEGER(IDF_INTEGER12,ISC)
  CALL WDIALOGGETMENU(IDF_MENU4,IM)
  WBAL_TYR=YMDHMSTOITIME(IY,IM,ID,IHR,IMT,ISC)
 ...
 ALLOCATE(ISYS(1000)); ISYS=INT(0,8)
  
 !## get all unique files
 DO IBAL=1,SIZE(WCTP)
  NSYS=0
  ROOT=TRIM(WBAL_RESDIR)//'\'//TRIM(WCTP(IBAL)%BDGNAME); NSYS=0
  DO JLAY=1,WBAL_NLAYER
   KLAY=WBAL_ILAYER(JLAY)
   !## construct filename
   FNAME=TRIM(WCTP(IBAL)%BDGNAME)//'_SYS*_*_L'//TRIM(ITOS(KLAY))//'.IDF'
   IF(UTL_DIRINFO_POINTER(ROOT,FNAME,IDFNAMES,'F',CORDER='N'))THEN; ENDIF
   DO I=1,SIZE(IDFNAMES)
    IDFNAMES(I)=UTL_CAP(IDFNAMES(I),'U')
           J=INDEX(IDFNAMES(I),'_SYS',.TRUE.);   IF(J.LE.0)CYCLE
    J=J+4; K=INDEX(IDFNAMES(I)(J:),'_',.FALSE.); IF(K.LE.0)CYCLE
    K=K-1; K=J+K-1
    NSYS=NSYS+1
    IF(NSYS.GT.SIZE(ISYS))THEN
     ALLOCATE(ISYS_BU(SIZE(ISYS)+1000))
     DO II=1,SIZE(ISYS); ISYS_BU(II)=ISYS(II); ENDDO
     DEALLOCATE(ISYS); ISYS=>ISYS_BU
    ENDIF
    READ(IDFNAMES(I)(J:K),*) ISYS(NSYS)
   ENDDO
   DEALLOCATE(IDFNAMES)
  ENDDO
  IF(NSYS.GT.0)THEN
   ALLOCATE(LSYS(NSYS)); DO I=1,NSYS; LSYS(I)=ISYS(I); ENDDO
   !## get number unique systems
   CALL UTL_GETUNIQUE_DINT(LSYS,NSYS,NU,0); NSYS=NU
   ALLOCATE(WCTP(IBAL)%ISYS(NSYS)); DO I=1,NSYS; WCTP(IBAL)%ISYS(I)=LSYS(I); ENDDO
   IF(ALLOCATED(LSYS))DEALLOCATE(LSYS)
  ENDIF
  WCTP(IBAL)%NSYS=MAX(NSYS,1)
 ENDDO
 DEALLOCATE(ISYS)
 ...
 SHP%NPOL=0; LEX=WBALCOMPUTE()

 CALL UTL_MESSAGEHANDLE(1)
 
 !## reread polygons in dialog
 IF(WBAL_ISEL.EQ.2)CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,TRIM(PREFVAL(1))//'\TMP\tmp.gen','GEN')
 ...
 IF(LEX)THEN
  ...
  INQUIRE(FILE=WBAL_OUTFNAME(:INDEX(WBAL_OUTFNAME,'.',.TRUE.)-1)//'_ZONES.IDF',EXIST=LEX)
  IF(LEX)CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=WBAL_OUTFNAME(:INDEX(WBAL_OUTFNAME,'.',.TRUE.)-1)//'_ZONES.IDF')

  !## plot IPF file
  IF(ICSV.EQ.3)THEN

   CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=WBAL_OUTFNAME)
   CALL IDFPLOTFAST(1)       
  ...
  ELSEIF(ICSV.EQ.1)THEN
   ...
   ELSE
    CLOSE(IU)
    CALL WINDOWOPENCHILD(IWIN,FLAGS=SYSMENUON+MINBUTTON+MAXBUTTON,WIDTH=1000,HEIGHT=500)
 ...
 IF(GXG1_COMPUTEGXG())THEN
  INQUIRE(FILE=TRIM(PREFVAL(1))//'\TMP\POINTER.IDF',EXIST=LEX)
  IF(LEX)CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(PREFVAL(1))//'\TMP\POINTER.IDF')
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=FGLG)
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=FGHG)
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=FGVG)
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=FGT,LEGNAME=FNLEG)
  CALL IDFPLOTFAST(1)       
 ENDIF
...
!## determine periods
 ...
 IF(MEAN1COMPUTE())THEN
  DO I=1,MEAN_NLAYER
   CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=MEAN_FTOTAL(I))
   CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=MEAN_FMEAN(I))
   CALL IDFPLOTFAST(1)       
  ENDDO
 ENDIF
...
! CALL TSBATCH(TRIM(PREFVAL(1))//'\TMP\BATCH.TXT',LOKAY)
 ...
 IPF(1)%FNAME=''
 IF(LOKAY)THEN
  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=ADIR)
  CALL IDFPLOTFAST(1)       
 ...
 ELSEIF(ISEL.EQ.2)THEN
  ...
  ALLOCATE(COPYSHPIACT(MAXSHAPES))
  COPYSHPIACT=SHP%POL%IACT
  !## save all of them
  IF(IALL.EQ.0)SHP%POL(1:SHP%NPOL)%IACT=1
  CALL POLYGON1SAVELOADSHAPE(ID_SAVESHAPE,GENFNAME,'GEN',ISAVESEL=1)
 ...
 !## load all polygons
 SHP%NPOL=0
 CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,TRIM(PREFVAL(1))//'\TMP\TMP.GEN','GEN')
 SHP%POL%IACT=COPYSHPIACT

 CALL WDIALOGSELECT(ID_TOOLSTAB3)
 CALL WDIALOGPUTMENU(IDF_MENU1,SHP%POL%PNAME,SHP%NPOL,SHP%POL%IACT)
! CALL WDIALOGPUTOPTION(IDF_MENU1,SHP%POL%IACT)
 ...
 INTEGER :: IS,IT,TIS,TIT,NYEAR,IY,IM,ID,IHR,IMT,ISC
 CHARACTER(LEN=100) :: TXT
 INTEGER(KIND=DP_KIND) :: DDATE1,DDATE2,MINDATE,MAXDATE
...
! IF(SUM(TP%IACT).NE.0)THEN
 ...
 MINDATE= 21001231000000
 MAXDATE=-18000101000000
 TIS    =0
 TIT    =0

 N=0; DO I=1,SIZE(ILIST); IF(ILIST(I).EQ.1)N=N+1; ENDDO
 ...
 DO I=1,SIZE(ILIST)
  ...
  IF(N.GT.0)THEN
   ...
   TIT=TIT+IT
   IF(IT.GT.0)THEN
    MINDATE=MIN(MINDATE,DDATE1)
    MAXDATE=MAX(MAXDATE,DDATE2)
   ENDIF
  ENDIF
 ...
 IF(SUM(TLAY).GT.0.AND. &      !## layers found
  ...
  IF(TIT.GT.0)THEN
   !## put begin date
   CALL ITIMETOGDATE(MINDATE,IY,IM,ID,IHR,IMT,ISC)
   DATE1=IY*10000+IM*100+ID
   CALL UTL_FILLDATESDIALOG(ID_TOOLSTAB2,IDF_INTEGER3,IDF_MENU3,IDF_INTEGER4,DATE1) 
   CALL WDIALOGPUTINTEGER(IDF_INTEGER9,IHR)
   CALL WDIALOGPUTINTEGER(IDF_INTEGER10,IMT)
   CALL WDIALOGPUTINTEGER(IDF_INTEGER11,ISC)
   !## put end date
   CALL ITIMETOGDATE(MAXDATE,IY,IM,ID,IHR,IMT,ISC)
   DATE2=IY*10000+IM*100+ID
   CALL UTL_FILLDATESDIALOG(ID_TOOLSTAB2,IDF_INTEGER5,IDF_MENU4,IDF_INTEGER6,DATE2) 
   CALL WDIALOGPUTINTEGER(IDF_INTEGER14,IHR)
   CALL WDIALOGPUTINTEGER(IDF_INTEGER13,IMT)
   CALL WDIALOGPUTINTEGER(IDF_INTEGER12,ISC)
  ENDIF

  IF(ALLOCATED(CLAY))DEALLOCATE(CLAY); ALLOCATE(CLAY(NLAY))
...
! END SUBROUTINE TOOLS_FIELDS1_GETTOPIC
 ...
 ELSEIF(I.EQ.2)THEN
  ...
  IF(SHP%NPOL.GT.0)THEN
   ...
   IF(SUM(SHP%POL(1:SHP%NPOL)%IACT).GT.0)J=1
  ENDIF
 ELSEIF(I.EQ.3)THEN
  K=1
  CALL WDIALOGGETSTRING(IDF_STRING1,FNAME)
...
! CALL WDIALOGSELECT(ID_TOOLSTAB4)
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER,PARAMETER :: MAXCRD=1000.0D0
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: ITYPE,IU,I
 INTEGER :: NIPF
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: XCRD,YCRD
 REAL(KIND=DP_KIND) :: DX,DY

 ALLOCATE(XCRD(MAXCRD),YCRD(MAXCRD))
 CALL WCURSORSHAPE(ID_CURSORPIPET)

 DX=(MPW%XMAX-MPW%XMIN)/500.0D0
 DY=(MPW%YMAX-MPW%YMIN)/500.0D0
   ...
   CASE (MOUSEBUTDOWN)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (1)
      ...
      CALL DBL_IGRJOIN(XCRD(NIPF)-DX,YCRD(NIPF),XCRD(NIPF)+DX,YCRD(NIPF))
      CALL DBL_IGRJOIN(XCRD(NIPF),YCRD(NIPF)-DY,XCRD(NIPF),YCRD(NIPF)+DY)
...
! CALL WDIALOGFIELDSTATE(IDF_STRING1,2)
 ...
 SELECT CASE (TOOLSID)
  CASE (ID_WBAL_GENERATE)
   ...
   CALL WDIALOGPUTSTRING(IDF_GROUP1,'Save Water Balance As:')
   CALL WDIALOGPUTSTRING(IDOK,'Create CSV ...')
  CASE (ID_GXG)
   CALL WDIALOGTITLE('Compute GxG'//CHAR(39)//'s')
   CALL WDIALOGPUTSTRING(IDF_GROUP1,'Save GxGs As:')
   CALL WDIALOGPUTSTRING(IDOK,'Create IDF ...')
  CASE (ID_TS)
   CALL WDIALOGTITLE('Compute Timeseries')
   CALL WDIALOGPUTSTRING(IDF_GROUP1,'Save Timeseries As:')
   CALL WDIALOGPUTSTRING(IDOK,'Create IPF ...')
  CASE (ID_MEAN)
   CALL WDIALOGTITLE('Compute Mean Values')
   CALL WDIALOGPUTSTRING(IDF_GROUP1,'Save Mean Values As:')
   CALL WDIALOGPUTSTRING(IDOK,'Create IDF ...')
 END SELECT
...
!   CALL WDIALOGTITLE('Result Folder')
   CALL WDIALOGFIELDSTATE(IDF_MODFLOW,3)
   CALL WDIALOGFIELDSTATE(IDF_METASWAP,3)
   CALL WDIALOGPUTSTRING(IDF_LABEL4,'Select Term:')
  CASE (ID_WBAL_GENERATE)
   CALL WDIALOGPUTSTRING(IDF_LABEL4,'Select Budget Term:')
 END SELECT
 ...
 SELECT CASE (TOOLSID)
  ...
  CASE (ID_WBAL_GENERATE)
   CALL WDIALOGPUTSTRING(IDF_RADIO2,'Transient (Names contain "_yyyymmdd_" or "_yyyymmddhhmmss_")')
 END SELECT
 ...
 CALL WDIALOGSELECT(ID_TOOLS); CALL UTL_DIALOGSHOW(-1,-1,0,2)
