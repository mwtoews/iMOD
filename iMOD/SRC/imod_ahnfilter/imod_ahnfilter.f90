...
CONTAINS
 ...
 REAL(KIND=DP_KIND) :: XC,TY,NY,MAXD
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: XA,YA,ZA
    ...
    IF(Y(ICOL,IROW).NE.INT(ISURF,1))THEN
     ...
     DO IR=MAX(1,IROW-DX),MIN(IROW+DX,IDFY%NROW)
      DO IC=MAX(1,ICOL-DX),MIN(ICOL+DX,IDFY%NCOL)
       IF(IDFY%X(IC,IR).NE.AHN_NODATA)THEN
        TY=TY+IDFY%X(IC,IR)
        NY=NY+1.0D0
       ENDIF
      END DO
 ...
 DO I=1,SIZE(IDFAHN)
  ...
  DO IR=IR1,IR2
   ...
   DO IC=IC1,IC2
    ICOL=ICOL+1
    IF(IDFAHN(I)%X(ICOL,IROW).EQ.IDFAHN(I)%NODATA)IDFAHN(I)%X(ICOL,IROW)=AHN_NODATA
 ...
 DO IROW=1,IDFX%NROW
  DO ICOL=1,IDFX%NCOL
   IF(IDFX%X(ICOL,IROW).NE.AHN_NODATA)THEN
   ...
   ENDIF
  ENDDO
  WRITE(*,*) 'Tracing ',REAL(IROW)/REAL(IDFX%NROW)*100.0D0,'%'
 ENDDO
 ...
 LINE=IDFY%FNAME(:INDEX(IDFY%FNAME,'\',.TRUE.))//'y'//TRIM(ITOS(IFILTER))//'_'//TRIM(ITOS(1))//'.idf'
 IF(.NOT.IDFWRITE(IDFY,TRIM(LINE),1))THEN
 ENDIF
 ...
 IDFY%X=REAL(Y)
 LINE=IDFY%FNAME(:INDEX(IDFY%FNAME,'\',.TRUE.))//'y'//TRIM(ITOS(IFILTER))//'_'//TRIM(ITOS(2))//'.idf'
 IF(.NOT.IDFWRITE(IDFY,TRIM(LINE),1))THEN
 ENDIF
 ...
 DO WHILE(NTHREAD.GT.0)
  ...
  ISPEC(NTHREAD)=IDIR
  CALL IDFEDITGETDIR(JCOL,JROW,IR,IC,IDIR,DTERM,THREAD,NTHREAD,0) 
  ...
  IF(IDIR.LE.4+(4*DTERM))THEN
   ...
   IF(IR.GE.1.AND.IR.LE.IDFX%NROW.AND.IC.GE.1.AND.IC.LE.IDFX%NCOL)THEN
    ...
    IF(Y(IC,IR)     .EQ.INT(IINI,1).AND. &    !## not yet been there
       IDFX%X(IC,IR).NE.AHN_NODATA)THEN           !## correct location
...
!  CALL IDFEDITGETDIR(JCOL,JROW,IR,IC,IDIR,DTERM)
 ...
 DO J=MAX(1,IR-DPW),MIN(IR+DPW,IDFX%NROW)
  DO I=MAX(1,IC-DPW),MIN(IC+DPW,IDFX%NCOL)
   NP    =NP+1
   AHN_XP(NP)=IDFX%X(I,J)
  END DO
 END DO

 IF(MAXVAL(AHN_XP(1:NP))-MINVAL(AHN_XP(1:NP)).GT.LOCCRIT)RETURN

 !## percentile
 CALL UTL_GETMED(AHN_XP,NP,AHN_NODATA,(/DP1,DP2/),2,N,XMED)
 ...
 IF(BUFFER.GT.IDFX%DX)THEN
  IF(MOD(BUFFER,IDFX%DX).NE.0.0D0)BUFFER=IDFX%DX*INT(BUFFER/IDFX%DX)
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot allocate enough memory.'//CHAR(13)// &
 ...
 IF(ALLOCATED(AHN_XP))DEALLOCATE(AHN_XP)
