...
! IF(PRESENT(ITIME))PAUSE
 ...
 !###======================================================================
 SUBROUTINE OSD_OPEN(IU,FILE,STATUS,FORM,ACTION,ACCESS,POSITION,RECL,IOSTAT,IQUESTION)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(INOUT) :: IU
 CHARACTER(LEN=*),INTENT(IN),OPTIONAL :: FILE
 CHARACTER(LEN=*),INTENT(IN),OPTIONAL :: STATUS,FORM,ACTION,ACCESS,POSITION
 INTEGER,INTENT(IN),OPTIONAL :: RECL,IQUESTION
 INTEGER,INTENT(OUT),OPTIONAL :: IOSTAT
 CHARACTER(LEN=25) :: TSTAT,TFORM,TACTION,TACCESS,TPOS
 INTEGER :: TRECL,IOS,IQ

 IQ=1; IF(PRESENT(IQUESTION))IQ=IQUESTION
 
 !## get valid unitnumber
 IF(IU.LE.0)THEN
  IU=0; IF(IQ.EQ.1)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'No valid unit number given (IU.LE.0)!','Error')
  RETURN
 ENDIF
 ...
 IF(IU.LE.0)THEN
  IF(IQ.EQ.1)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open file:'//CHAR(13)//TRIM(FILE),'Error')
 ENDIF
