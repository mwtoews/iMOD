...
CONTAINS
 ...
 REAL,DIMENSION(:,:),INTENT(INOUT) :: CC,CR  !IDF%NCOL,IDF%NROW
 INTEGER,PARAMETER :: INEIGHBOURS=2
 INTEGER :: IROW,ICOL,J,IHFB,ILINE,JLINE
 ...
 DO IHFB=1,NHFBFILES
  ...
  DO J=HFBIP(IHFB-1)+1,HFBIP(IHFB)
   ...
   IROW =HFBPOS(J,2)
   ILINE=HFBPOS(J,3)
   IF(ILINE.EQ.JLINE)IPC(ICOL,IROW,0)=INT(1,1)
   !## process line
   IF(ILINE.NE.JLINE.OR.J.EQ.HFBIP(IHFB))THEN
    ...
    DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
     IF(IPC(ICOL,IROW,0)  .EQ.INT(1,1).AND. &
        IPC(ICOL-1,IROW,0).EQ.INT(0,1))THEN
      IF(IPC(ICOL+1,IROW,0).EQ.INT(1,1).AND. &
         IPC(ICOL+2,IROW,0).EQ.INT(0,1))IPC(ICOL,IROW,1)=INT(1,1)   !## sure
      IF(IPC(ICOL+1,IROW,0).EQ.INT(0,1).AND. &
         IPC(ICOL,IROW,3).GE.INEIGHBOURS)IPC(ICOL-1,IROW,1)=INT(1,1)!## estimated
     ENDIF
    ENDDO; ENDDO
    ...
    DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
     IF(IPC(ICOL,IROW,0)  .EQ.INT(1,1).AND. &
        IPC(ICOL,IROW-1,0).EQ.INT(0,1))THEN
      IF(IPC(ICOL,IROW+1,0).EQ.INT(1,1).AND. &
         IPC(ICOL,IROW+2,0).EQ.INT(0,1))IPC(ICOL,IROW,2)=INT(1,1)    !## sure
      IF(IPC(ICOL,IROW+1,0).EQ.INT(0,1).AND. &
         IPC(ICOL,IROW,3).GE.INEIGHBOURS)IPC(ICOL,IROW-1,2)=INT(1,1) !## estimated
     ENDIF
    ENDDO; ENDDO
    ...
    DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
     IF(IPC(ICOL,IROW  ,0).EQ.INT(1,1))THEN
      IF(IPC(ICOL,IROW-1,2).EQ.INT(1,1).AND. &
         IPC(ICOL+1,IROW,2).EQ.INT(1,1))IPC(ICOL,IROW,1)=INT(1,1) !## close "hole"
      IF(IPC(ICOL+1,IROW-1,2).EQ.INT(1,1).AND. &
         IPC(ICOL  ,IROW,2).EQ.INT(1,1))IPC(ICOL,IROW,1)=INT(1,1) !## close "hole"
     ENDIF
    ENDDO; ENDDO
    ...
    DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
     IF(IPC(ICOL,IROW  ,0).EQ.INT(1,1))THEN
      IF(IPC(ICOL-1,IROW,1).EQ.INT(1,1).AND. &
         IPC(ICOL,IROW+1,1).EQ.INT(1,1))IPC(ICOL,IROW,2)=INT(1,1)   !## close "hole"
      IF(IPC(ICOL,IROW ,1).EQ.INT(1,1).AND. &
         IPC(ICOL-1,IROW+1,1).EQ.INT(1,1))IPC(ICOL,IROW,2)=INT(1,1) !## close "hole"
     ENDIF
    ENDDO; ENDDO
    ...
    DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
     IF(IPC(ICOL,IROW,3).EQ.INT(1,1))THEN
      IF(IPC(ICOL-1,IROW,1).EQ.INT(1,1).AND. &
         IPC(ICOL-1,IROW,2).EQ.INT(1,1))IPC(ICOL,IROW-1,2)=INT(1,1)   !## close top
      IF(IPC(ICOL-1,IROW,1)  .EQ.INT(0,1).AND. &
         IPC(ICOL-1,IROW-1,2).EQ.INT(1,1))IPC(ICOL,IROW-1,2)=INT(1,1) !## close top
      IF(IPC(ICOL-1,IROW,1)  .EQ.INT(1,1).AND. &
         IPC(ICOL-1,IROW-1,2).EQ.INT(1,1))IPC(ICOL,IROW,2)=INT(1,1)   !## close bottom
      IF(IPC(ICOL-1,IROW+1,1).EQ.INT(1,1).AND. &
         IPC(ICOL  ,IROW  ,2).EQ.INT(1,1))IPC(ICOL,IROW,1)=INT(1,1)   !## close right
      IF(IPC(ICOL-1,IROW+1,1).EQ.INT(1,1).AND. &
         IPC(ICOL,IROW ,2).EQ.INT(0,1))IPC(ICOL-1,IROW,1)=INT(1,1)    !## close right
      IF(IPC(ICOL-1,IROW-1,1).EQ.INT(1,1).AND. &
         IPC(ICOL,  IROW-1,2).EQ.INT(0,1))IPC(ICOL-1,IROW,1)=INT(1,1) !## close right
     ENDIF
    ENDDO; ENDDO
    ...
    IPC=INT(0,1)
    ICOL =HFBPOS(J,1); IROW =HFBPOS(J,2); IPC(ICOL,IROW,0)=INT(1,1)
    JLINE=ILINE
 ...
 HFB_GETCOND=0.0 !(DX*DY)/MAX(CC,C)
