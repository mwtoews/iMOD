...
USE MOD_POLYGON_PAR, ONLY : SHPNO,IACTSHAPES,SHPXC,SHPYC,CSHPXC,CSHPYC,SHPNCRD
...
! ENDIF
 ...
 INTEGER :: NCRD,I,J,K,II,ISTATUS,NINT
 ...
 DO II=1,2
  ...
  DO I=1,SHPNO
   DO J=1,SHPNCRD(I)-1
    ...
    !## copy as line segment is not intersected
    IF(.NOT.LINT)THEN
     CSHPXC(,NINT)=SHPXC(J,I)
     CSHPYC(,NINT)=SHPYC(J,I)
    ENDIF   
   
   ENDDO
  ENDDO
  IF(II.EQ.1)THEN
   NINT=NINT+SHPNO; ALLOCATE(CSHPXC(NINT),CSHPYC(NINT))
  ENDIF 
 ENDDO
