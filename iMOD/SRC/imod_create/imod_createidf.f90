...
INTEGER,PRIVATE :: MINP=10,KTYPE=2,PNTSEARCH=0,IADJRANGE=0,IQUADRANT=0
...
!  READ(IU,*,IOSTAT=IOS) IFF(1)%IPART,IFF(1)%IL,IFF(1)%X,IFF(1)%Y,IFF(1)%Z,IFF(1)%T,IFF(1)%V
 ...
 ELSEIF(IINT.EQ.5.OR.IINT.EQ.6)THEN
  ...
  X(1)%X=X(1)%NODATA
  CALL KRIGING_MAIN(ND,XD,YD,ZD,X(1),X(3),MINP,RANGE,SILL,NUGGET,KTYPE,PNTSEARCH,0,0.0,IADJRANGE,IQUADRANT,0,'')
  IERROR=0
 ...
 ELSE
  ...
  SELECT CASE (IINT)
   ...
   CASE (5,6)
    ...
    X(1)%X=X(1)%NODATA
    CALL KRIGING_MAIN(N,XD,YD,ZD,X(1),X(2),MINP,RANGE,SILL,NUGGET,KTYPE,PNTSEARCH,0,0.0,IADJRANGE,IQUADRANT,0,'')
    IERROR=0   
   CASE DEFAULT
  END SELECT
  DEALLOCATE(XD,YD,ZD)
