...
USE MOD_IPF_PAR
USE MOD_IFF, ONLY : UTL_GETUNITIFF,IFF
...
INTEGER,PRIVATE :: MINP=10,MAXP=50,KTYPE=2,IOFFSET=1,NOSEARCH=1,IEXPVARIOGRAM=0
REAL,PRIVATE :: RANGE=3000.0,SILL=30.0,NUGGET=0,SEARCHDISTANCE=4500.0
...
! IF(WINFODIALOGFIELD(IDF_MENU3,FIELDSTATE).EQ.1)CALL WDIALOGGETMENU(IDF_MENU3,IZ)
 ...
 INTEGER :: I,L,IU,N,IROW,ICOL,ITYPE,IOS
 LOGICAL :: LEX
 ...
 DO
  ...
  IF(IFF(1)%IPART.EQ.IFF(2)%IPART)THEN
   ...
   IF(IFF(1)%X.LT.XMAX.AND.IFF(1)%X.GE.XMIN.AND. &
    ...
    !## get plot value
    CALL CREATEIDF1GETIFFVAL(IX,XVAL)

    !## apply selection
    LEX=.TRUE.
    IF(IWHILE.EQ.1)THEN
     !## get selection value
     CALL CREATEIDF1GETIFFVAL(IY,YVAL)
 ...
 IF(ALLOCATED(IFF))DEALLOCATE(IFF)
 ...
 !###======================================================================
 SUBROUTINE CREATEIDF1GETIFFVAL(IPOS,X)
 !###======================================================================
 IMPLICIT NONE
 ...
 SELECT CASE (IPOS)
  ...
  CASE (6)
   X=(IFF(1)%T+IFF(2)%T)/2.0
  CASE (7)
   X=(IFF(1)%V+IFF(2)%V)/2.0
 END SELECT

 END SUBROUTINE CREATEIDF1GETIFFVAL
 ...
 ELSEIF(IINT.EQ.5.OR.IINT.EQ.6)THEN
  ...
  X(1)%X=X(1)%NODATA
  CALL KRIGING_MAIN(ND,XD,YD,ZD,X(1),X(3),MINP,MAXP,RANGE,SILL,NUGGET,KTYPE,1,SEARCHDISTANCE,NOSEARCH,IEXPVARIOGRAM,0,0.0,0)
  IERROR=0
 ...
 IF(IINT.EQ.1)THEN
  ...
  MX=MAXVAL(SHPNCRD(1:SHPNO)); ALLOCATE(XPOL(MX),YPOL(MX))
   ...
   IF(SHPNCRD(SHPI).GT.0)THEN
    ...
    DO I=1,SHPNCRD(SHPI) !-1
     ...
     DO
      ...
      ENDIF
      N=N+1; XD(N)=X1+D*COS(ANGLE); YD(N)=Y1+D*SIN(ANGLE); ZD(N)=XV !10.0
...
!  MINP=100    !## minimal number of points
!  MAXP=100+1 !## maximal number of points, +1 for lambda
!  SILL=2.0
!  RANGE=12000.0
!  NUGGET=0.0
!  KTYPE=2 !## spherical
    ...
    X(1)%X=X(1)%NODATA
    CALL KRIGING_MAIN(N,XD,YD,ZD,X(1),X(2),MINP,MAXP,RANGE,SILL,NUGGET,KTYPE,1,SEARCHDISTANCE,NOSEARCH,IEXPVARIOGRAM,0,0.0,0)
    IERROR=0   
   CASE DEFAULT
  END SELECT
  DEALLOCATE(XD,YD,ZD)
