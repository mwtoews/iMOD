     ...
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (IDOK)
       IF(CREATEIDF1APPLY())THEN
        CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Succesfully created an IDF which is addd to the iMOD Manager.','Information')
       ENDIF
...
!       CALL CREATEIDF1GETCRD(0) !## automatically
  ...
  IF(IFF(1)%IPART.EQ.IFF(2)%IPART)THEN
   ...
   IF(IFF(1)%X.LT.XMAX.AND.IFF(1)%X.GE.XMIN.AND. &
    ...
    IF(IWHILE.EQ.1)THEN
     ...
     LEX=IDFEDITGETLOGICAL(IZ,IZVAL,YVAL,0,0.0D0,XIDF(1),0,0,0)
    ENDIF
 ...
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: XD,YD,ZD,PD,WD
 ...
 ELSEIF(IINT.EQ.2)THEN
  ...
  !## number data points
  ND=IPF(1)%NROW; ALLOCATE(XD(ND),YD(ND),ZD(ND),PD(ND))
  ...
  DO I=1,IPF(1)%NROW
   IF(IPF(1)%XYZ(3,I).NE.NODATA)THEN
    ND=ND+1
    XD(ND)=IPF(1)%XYZ(1,I); YD(ND)=IPF(1)%XYZ(2,I); ZD(ND)=0.0D0; PD(ND)=IPF(1)%XYZ(3,I)
   ENDIF
  ENDDO
 
  CALL BIVARIATE_INT(XD,YD,PD,ND,IERROR,XIDF(1))
  ...
  DO I=1,IPF(1)%NROW
   IF(IPF(1)%XYZ(3,I).NE.NODATA)THEN
    ...
    IF(IROW.NE.0.AND.ICOL.NE.0)THEN
     ND=ND+1; XD(ND)=REAL(ICOL); YD(ND)=REAL(IROW); ZD(ND)=0.0D0; PD(ND)=IPF(1)%XYZ(3,I)
    ENDIF
   ENDIF
  ENDDO
  CALL SOLID_PCGINT(XD,YD,PD,INT(ND,4),IERROR,XIDF(1),1,IDUPLICATE)

 !## variogram
 ELSEIF(IINT.EQ.4)THEN

  ND=IPF(1)%NROW; ALLOCATE(XD(ND),YD(ND),PD(ND))
  !## fill coordinates --- ne nodatavalue
  ND=0
  DO I=1,IPF(1)%NROW
   IF(IPF(1)%XYZ(3,I).NE.NODATA)THEN
    ND=ND+1; XD(ND)=IPF(1)%XYZ(1,I); YD(ND)=IPF(1)%XYZ(2,I); PD(ND)=IPF(1)%XYZ(3,I)
   ENDIF
  ENDDO
  CALL KRIGING_VARIOGRAM(INT(ND,4),XD,YD,PD,I,XIDF(1),IBATCH=0)

 !## kriging (simple/ordinary)
 ELSEIF(IINT.EQ.5.OR.IINT.EQ.6)THEN

  ND=IPF(1)%NROW; ALLOCATE(XD(ND),YD(ND),ZD(ND),PD(ND),WD(ND))
  !## fill coordinates --- ne nodatavalue
  ND=0
  DO I=1,IPF(1)%NROW
   IF(IPF(1)%XYZ(3,I).NE.NODATA)THEN
    ND=ND+1; XD(ND)=IPF(1)%XYZ(1,I); YD(ND)=IPF(1)%XYZ(2,I); ZD(ND)=0.0D0; PD(ND)=IPF(1)%XYZ(3,I); WD(ND)=1.0D0
   ENDIF
  ENDDO
  
  XIDF(1)%X=XIDF(1)%NODATA
  CALL KRIGING_MAIN(ND,XD,YD,ZD,PD,WD,XIDF(1),XIDF(3),0,0.0D0)
  IERROR=0
 ...
 REAL(KIND=DP_KIND) :: X1,X2,Y1,Y2,XC,YC,ANGLE,D,MD
 REAL(KIND=DP_KIND) :: XV ! variable: value from GEN record to use in the IDF
 REAL(KIND=DP_KIND),DIMENSION(:),POINTER :: XD,YD,ZD,PD,WD,XD_DUMMY,YD_DUMMY,ZD_DUMMY,PD_DUMMY,WD_DUMMY
 LOGICAL :: LEX
  ...
  DO I=1,SHP%NPOL
   ...
   IF(SHP%POL(I)%N.GT.0)THEN
    ...
    ELSE  
      ...
      IF(IOS.NE.0)THEN
       CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot convert "'//TRIM(STRING)//'" into a real.'//CHAR(13) &
                       //'(Column '//TRIM(ITOS(SHP%ILBL))//' and record '//TRIM(ITOS(I))//').'//CHAR(13) &
                       //'Uncheck the option "use following column for gridding/interpolation" to use the GEN order number for gridding','error')
       DEALLOCATE(STRING) ; RETURN
      ENDIF
    ENDIF
    LEX=.FALSE.
 ...
 ELSE
  ...
  N=1000; ALLOCATE(XD(N),YD(N),ZD(N),PD(N),WD(N))
  N=0; DO I=1,SHP%NPOL
   !## selected polygon
   IF(SHP%POL(I)%N.GT.0)THEN
    ...
    IF(IOS.NE.0)THEN
     ...
     DEALLOCATE(XD,YD,ZD,PD,WD); RETURN
    ENDIF
    ...
    DO J=1,SHP%POL(I)%N 
     ...
     DO
      IF(N+1.GT.SIZE(XD))THEN
       ALLOCATE(XD_DUMMY(N*2),YD_DUMMY(N*2),ZD_DUMMY(N*2),PD_DUMMY(N*2),WD_DUMMY(N*2))
       XD_DUMMY(1:N)=XD(1:N); YD_DUMMY(1:N)=YD(1:N); ZD_DUMMY(1:N)=ZD(1:N)
       PD_DUMMY(1:N)=PD(1:N); WD_DUMMY(1:N)=WD(1:N)
       DEALLOCATE(XD,YD,ZD,PD,WD); XD=>XD_DUMMY; YD=>YD_DUMMY; ZD=>ZD_DUMMY; PD=>PD_DUMMY; WD=>WD_DUMMY
      ENDIF
      N=N+1; XD(N)=X1+D*COS(ANGLE); YD(N)=Y1+D*SIN(ANGLE); ZD(N)=0.0D0; PD(N)=XV; WD(N)=1.0D0
  ...
  SELECT CASE (IINT)
   !## bivar
   CASE(2)
    CALL BIVARIATE_INT(XD,YD,PD,INT(N,4),IERROR,XIDF(1))
   !## pcg
   CASE(3)
    J=0; DO I=1,N
     CALL IDFIROWICOL(XIDF(1),IROW,ICOL,XD(I),YD(I))
     IF(IROW.NE.0.AND.ICOL.NE.0)THEN
      J=J+1; XD(J)=REAL(ICOL); YD(J)=REAL(IROW); ZD(J)=ZD(I); PD(J)=PD(I); WD(J)=WD(I)
     ENDIF
    ENDDO; N=J
    CALL SOLID_PCGINT(XD,YD,PD,INT(N,4),IERROR,XIDF(1),1,IDUPLICATE)
   CASE (4)
    CALL KRIGING_VARIOGRAM(INT(N,4),XD,YD,PD,I,XIDF(1),IBATCH=0)
   CASE (5,6)
    CALL IDFCOPY(XIDF(1),XIDF(2)); XIDF(2)%NODATA=HUGE(1.0D0)
    XIDF(1)%X=XIDF(1)%NODATA
    CALL KRIGING_MAIN(N,XD,YD,ZD,PD,WD,XIDF(1),XIDF(2),0,0.0D0)
    IERROR=0   
   CASE DEFAULT
  END SELECT
  DEALLOCATE(XD,YD,ZD,PD,WD)
 ...
 CALL WDIALOGSELECT(ID_DCREATEIDF)
 CALL WDIALOGGETCHECKBOX(IDF_CHECK2,IDOUBLE)
 IDOUBLE=IDOUBLE+1 !## if single precision - modify if needed
 IF(IDOUBLE.EQ.1)THEN
  XS=REAL(XMIN,4); WRITE(TMPXS,*) XS; READ(TMPXS,*) XMIN
  XS=REAL(XMAX,4); WRITE(TMPXS,*) XS; READ(TMPXS,*) XMAX
  XS=REAL(YMIN,4); WRITE(TMPXS,*) XS; READ(TMPXS,*) YMIN
  XS=REAL(YMAX,4); WRITE(TMPXS,*) XS; READ(TMPXS,*) YMAX
 ...
 INTEGER :: I
 REAL(KIND=SP_KIND) :: SNODATA
 CHARACTER(LEN=20) :: TMPNODATA
...
!   CALL WDIALOGFIELDSTATE(IDF_LABEL10,3)
 ...
 !## initial values
 SNODATA=HUGE(1.0); GENFNAME=''; MXITER1=1000; MXITER2=500
 SILL=60.0D0; RANGE=1000.0D0; NUGGET=0.0D0; KTYPE=-2; PNTSEARCH=0
 COINCIDENT=0.0D0; IQUADRANT=0; MAXPNT=10

 !## make a nice single precision nodata
 WRITE(TMPNODATA,*) SNODATA; READ(TMPNODATA,*) NODATA
 CALL WDIALOGPUTDOUBLE(IDF_REAL4,NODATA,'(G15.7)')
