...
! Y1=MIN(YMIN,YMAX); Y2=MAX(YMIN,YMAX); YMIN=Y1; YMAX=Y2
 ...
 IF(ID.NE.IDF_REAL1)CALL WDIALOGPUTREAL(IDF_REAL1,XMIN,'(F10.2)')
 IF(ID.NE.IDF_REAL2)CALL WDIALOGPUTREAL(IDF_REAL2,YMIN,'(F10.2)')
 IF(ID.NE.IDF_REAL5)CALL WDIALOGPUTREAL(IDF_REAL5,YMAX,'(F10.2)')
 IF(ID.NE.IDF_REAL6)CALL WDIALOGPUTREAL(IDF_REAL6,XMAX,'(F10.2)')
...
! ENDIF
 ...
 SELECT CASE (IINT)
  ...
  CASE (4)   !## variogram
  CASE (5,6) !## kriging
   CALL KRIGINGSETTINGS(MINP,KTYPE,RANGE,SILL,NUGGET,PNTSEARCH,COINCIDENT,COINCIDENTDIST,IQUADRANT)
 END SELECT
...
!   CALL WDIALOGFIELDSTATE(IDF_LABEL10,3)
 ...
 !## initial value
 NODATA=0.0; GENFNAME=''; MXITER1=1000; MXITER2=500
