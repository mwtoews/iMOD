...
! IF(WINFODIALOGFIELD(IDF_MENU3,FIELDSTATE).EQ.1)CALL WDIALOGGETMENU(IDF_MENU3,IZ)
 ...
 ELSEIF(IINT.EQ.5.OR.IINT.EQ.6)THEN
  ...
  X(1)%X=X(1)%NODATA
  CALL KRIGING_MAIN(ND,XD,YD,ZD,X(1),X(3),MINP,MAXP,RANGE,SILL,NUGGET,KTYPE,SEARCHDISTANCE,NOSEARCH,IEXPVARIOGRAM,0,0.0,0)
  IERROR=0
 ...
 ELSE
  ...
  SELECT CASE (IINT)
   ...
   CASE (5,6)
    ...
    X(1)%X=X(1)%NODATA
    CALL KRIGING_MAIN(N,XD,YD,ZD,X(1),X(2),MINP,MAXP,RANGE,SILL,NUGGET,KTYPE,SEARCHDISTANCE,NOSEARCH,IEXPVARIOGRAM,0,0.0,0)
    IERROR=0   
   CASE DEFAULT
  END SELECT
  DEALLOCATE(XD,YD,ZD)
