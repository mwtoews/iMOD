...
!  READ(IU,*,IOSTAT=IOS) IFF(1)%IPART,IFF(1)%IL,IFF(1)%X,IFF(1)%Y,IFF(1)%Z,IFF(1)%T,IFF(1)%V
 ...
 ELSEIF(IINT.EQ.5.OR.IINT.EQ.6)THEN
  ...
  X(1)%X=X(1)%NODATA
  CALL KRIGING_MAIN(ND,XD,YD,ZD,X(1),X(3),MINP,RANGE,SILL,NUGGET,KTYPE,PNTSEARCH,0,0.0,COINCIDENT,COINCIDENTDIST,IQUADRANT,0,0,0.0,0)
  IERROR=0
...
!    CALL UTL_GENLABELSREAD(SHPFILE(:INDEX(SHPFILE,'.',.TRUE.)-1)//'.dat',VAR,NL,NV)
    ...
    IF(LEX)THEN 
     DO I=1,SHPNCRD(SHPI)-1
      ...
      !## intersect line
      N=0; CALL INTERSECT_EQUI(X(1)%XMIN,X(1)%XMAX,X(1)%YMIN,X(1)%YMAX,X(1)%DX,X(1)%DY,X1,X2,Y1,Y2,N,.FALSE.)
      !## fill result array
      DO J=1,N
       IC=CA(J); IR=RA(J)
 ...
 ELSE
  ...
  SELECT CASE (IINT)
   ...
   CASE (5,6)
    CALL IDFCOPY(X(1),X(2)); X(2)%NODATA=HUGE(1.0)
     
    X(1)%X=X(1)%NODATA
    CALL KRIGING_MAIN(N,XD,YD,ZD,X(1),X(2),MINP,RANGE,SILL,NUGGET,KTYPE,PNTSEARCH,0,0.0,COINCIDENT,COINCIDENTDIST,IQUADRANT,0,0,0.0,0)
    IERROR=0   
   CASE DEFAULT
  END SELECT
  DEALLOCATE(XD,YD,ZD)
