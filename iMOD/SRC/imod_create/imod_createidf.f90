...
!      ICOL=INT(XA(L)); IROW=INT(YA(L))
 ...
 ELSEIF(IINT.EQ.5.OR.IINT.EQ.6)THEN
  ...
  X(1)%X=X(1)%NODATA
  CALL KRIGING_MAIN(ND,XD,YD,ZD,X(1),X(3),MINP,RANGE,SILL,NUGGET,KTYPE,PNTSEARCH,0,0.0,COINCIDENT,COINCIDENTDIST,IQUADRANT,0)
  IERROR=0
 ...
 ELSE
  ...
  SELECT CASE (IINT)
   ...
   CASE (5,6)
    ...
    X(1)%X=X(1)%NODATA
    CALL KRIGING_MAIN(N,XD,YD,ZD,X(1),X(2),MINP,RANGE,SILL,NUGGET,KTYPE,PNTSEARCH,0,0.0,COINCIDENT,COINCIDENTDIST,IQUADRANT,0)
    IERROR=0   
   CASE DEFAULT
  END SELECT
  DEALLOCATE(XD,YD,ZD)
