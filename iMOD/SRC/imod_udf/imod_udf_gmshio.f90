...
CONTAINS
 ...
 LUN=UTL_GETUNIT(); OPEN(LUN,FILE=FNAME,IOSTAT=IOS) 
 IF(IOS.NE.0)THEN
...
!  READ(S,*) J, X1, Y1
 ...
 IF(IREAD.EQ.0)THEN; CLOSE(LUN); READGMSH=.TRUE.; RETURN; ENDIF
 
 ALLOCATE(MESH%ELEM(MESH%NELEM)); ELEM=>MESH%ELEM
 
 DO I=1,MESH%NELEM
  READ(LUN,FORM) S !'(A)') S  
  READ(S,*) J, ELEM(J)%TYP, ELEM(J)%NTAG
  N=MAX(MESH%NTAG,2);   ALLOCATE(ELEM(J)%TAG(N))
  N=MAX(MESH%MAXTAG,1); ALLOCATE(ELEM(J)%ITAG(N))
  ELEM(J)%ITAG = 0
...
!    USE KDTREE2_MODULE
 ...
 REAL, DIMENSION(2,NXY), INTENT(INOUT) :: XY
