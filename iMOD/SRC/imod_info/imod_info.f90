...
CONTAINS
 ...
 CALL WDIALOGPUTREAL(ID_XMIN2,MPW%XMIN,'(F10.2)')
 CALL WDIALOGPUTREAL(ID_XMAX2,MPW%XMAX,'(F10.2)')
 CALL WDIALOGPUTREAL(ID_DX2,(MPW%XMAX-MPW%XMIN),'(F10.2)')
 CALL WDIALOGPUTREAL(ID_YMIN2,MPW%YMIN,'(F10.2)')
 CALL WDIALOGPUTREAL(ID_YMAX2,MPW%YMAX,'(F10.2)')
 CALL WDIALOGPUTREAL(ID_DY2,(MPW%YMAX-MPW%YMIN),'(F10.2)')
   ...
   SELECT CASE (ITYPE)
    ...
    CASE (PUSHBUTTON)
     ...
     CASE (IDF_NOSAVE)
      ...
      CALL WDIALOGPUTREAL(ID_NODATA,MP(IPLOT)%IDF%NODATA)
...
!   WRITE(IU,'(A)') 'Percentile('//TRIM(LINE) 
 ...
 IF(MP(IPLOT)%IPLOT.EQ.1.OR.MP(IPLOT)%IPLOT.EQ.5)THEN
  ...
  IF(LEX)THEN
   ...
   CALL WDIALOGPUTREAL(ID_XMIN,MP(IPLOT)%IDF%XMIN,'(F10.2)')
   CALL WDIALOGPUTREAL(ID_XMAX,MP(IPLOT)%IDF%XMAX,'(F10.2)')
   CALL WDIALOGPUTREAL(ID_DX,MP(IPLOT)%IDF%XMAX-MP(IPLOT)%IDF%XMIN,'(F10.2)')
   CALL WDIALOGPUTREAL(ID_YMIN,MP(IPLOT)%IDF%YMIN,'(F10.2)')
   CALL WDIALOGPUTREAL(ID_YMAX,MP(IPLOT)%IDF%YMAX,'(F10.2)')
   CALL WDIALOGPUTREAL(ID_DY,MP(IPLOT)%IDF%YMAX-MP(IPLOT)%IDF%YMIN,'(F10.2)')

   CALL WDIALOGPUTOPTION(IDF_MENU2,MP(IPLOT)%UNITS+1)
   CALL WDIALOGPUTREAL(ID_NODATA,MP(IPLOT)%IDF%NODATA)

   CALL WDIALOGPUTREAL(ID_ZMIN,MP(IPLOT)%IDF%DMIN)
   CALL WDIALOGPUTREAL(ID_ZMAX,MP(IPLOT)%IDF%DMAX)
   CALL WDIALOGPUTREAL(ID_DZ,MP(IPLOT)%IDF%DMAX-MP(IPLOT)%IDF%DMIN)

   CALL WDIALOGPUTREAL(ID_ZMIN2,MP(IPLOT)%UMIN)
   CALL WDIALOGPUTREAL(ID_ZMAX2,MP(IPLOT)%UMAX)
   CALL WDIALOGPUTREAL(ID_DZ2,MP(IPLOT)%UMAX-MP(IPLOT)%UMIN)

   IF(MP(IPLOT)%IDF%IEQ.EQ.0)THEN
    CALL WDIALOGPUTREAL(ID_DXMIN,MP(IPLOT)%IDF%DX,'(F10.2)')
    CALL WDIALOGPUTREAL(ID_DYMIN,MP(IPLOT)%IDF%DY,'(F10.2)')
   ...
   ELSE
    ...
    END DO
    CALL WDIALOGPUTREAL(ID_DXMIN,MINCS,'(F10.2)')
    CALL WDIALOGPUTREAL(ID_DXMAX,MAXCS,'(F10.2)')
    ...
    CALL WDIALOGPUTREAL(ID_DYMIN,MINCS,'(F10.2)')
    CALL WDIALOGPUTREAL(ID_DYMAX,MAXCS,'(F10.2)')
   ENDIF
   ...
   IF(MP(IPLOT)%IDF%ITB.EQ.0)THEN
    ...
    CALL WDIALOGFIELDSTATE(IDF_LABEL25,2)!3)
    CALL WDIALOGFIELDSTATE(IDF_LABEL26,2)!3)
    CALL WDIALOGFIELDSTATE(IDF_LABEL27,2)!3)
    CALL WDIALOGFIELDSTATE(IDF_LABEL28,2)!3)
   ELSE
    CALL WDIALOGFIELDSTATE(IDF_REAL1,2)
    CALL WDIALOGFIELDSTATE(IDF_REAL2,2)
    CALL WDIALOGPUTREAL(IDF_REAL1,MP(IPLOT)%IDF%TOP,'(F10.2)')
    CALL WDIALOGPUTREAL(IDF_REAL2,MP(IPLOT)%IDF%BOT,'(F10.2)')
 ...
 ELSE
  ...
  CALL WDIALOGPUTREAL(ID_XMIN,MP(IPLOT)%XMIN,'(F10.2)')
  CALL WDIALOGPUTREAL(ID_XMAX,MP(IPLOT)%XMAX,'(F10.2)')
  CALL WDIALOGPUTREAL(ID_DX,MP(IPLOT)%XMAX-MP(IPLOT)%XMIN,'(F10.2)')
  CALL WDIALOGPUTREAL(ID_YMIN,MP(IPLOT)%YMIN,'(F10.2)')
  CALL WDIALOGPUTREAL(ID_YMAX,MP(IPLOT)%YMAX,'(F10.2)')
  CALL WDIALOGPUTREAL(ID_DY,MP(IPLOT)%YMAX-MP(IPLOT)%YMIN,'(F10.2)')
...
!   CALL WDIALOGFIELDSTATE(ID_ZMIN2,1)
!   CALL WDIALOGFIELDSTATE(ID_ZMAX2,1)
!   CALL WDIALOGFIELDSTATE(ID_DZ2  ,1)
   CALL WDIALOGPUTREAL(ID_ZMIN,MP(IPLOT)%UMIN)
   CALL WDIALOGPUTREAL(ID_ZMAX,MP(IPLOT)%UMAX)
   CALL WDIALOGPUTREAL(ID_DZ,MP(IPLOT)%UMAX-MP(IPLOT)%UMIN)
!   CALL WDIALOGPUTREAL(ID_ZMIN2,MP(IPLOT)%UMIN)
!   CALL WDIALOGPUTREAL(ID_ZMAX2,MP(IPLOT)%UMAX)
!   CALL WDIALOGPUTREAL(ID_DZ2,MP(IPLOT)%UMAX-MP(IPLOT)%UMIN)
  ENDIF
