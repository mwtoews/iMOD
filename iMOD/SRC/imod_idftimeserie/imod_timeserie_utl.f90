...
USE MOD_UTL, ONLY : JD,UTL_JDATETOIDATE,UTL_FILLDATES,IDATETOGDATE,FTIMETOCTIME,CTIMETOFTIME,ITOS,UTL_IDATETOJDATE, &
    UTL_DIRINFO_POINTER,UTL_IDFGETDATES,UTL_MESSAGEHANDLE,UTL_IDFGETDATE
...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,ID1,ID2,IPLOT,O,M,IM,IY,ID,IDFDATE,N
...
!     DIR,WC,LISTNAME,N,FT
!     CALL UTL_DIRINFO(MP(IPLOT)%IDFNAME(:I-1),TRIM(ROOT)//'????????'//TRIM(MP(IPLOT)%IDFNAME(J+8:)),IDFNAMES,'F')
    ELSE
     IF(UTL_DIRINFO_POINTER(MP(IPLOT)%IDFNAME(:I-1),TRIM(ROOT)//'??????????????'//TRIM(MP(IPLOT)%IDFNAME(J+14:)),IDFNAMES,'F'))THEN; ENDIF
!     CALL UTL_DIRINFO(MP(IPLOT)%IDFNAME(:I-1),TRIM(ROOT)//'??????????????'//TRIM(MP(IPLOT)%IDFNAME(J+14:)),IDFNAMES,NFILES(N),'F')
    ENDIF
 ...
 !## get list of files ... IN two dimensions
! DEALLOCATE(IDFNAMES)
! ALLOCATE(,)
 CALL UTL_MESSAGEHANDLE(0)
