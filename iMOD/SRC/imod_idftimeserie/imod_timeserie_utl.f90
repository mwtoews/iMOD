 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.1)THEN
   ...
   IF(IDFDATE.NE.0)THEN
    ...
    !## get them all
    IF(UTL_DIRINFO_POINTER(MP(IPLOT)%IDFNAME(:K-1),TRIM(ROOT)//'_*_'//TRIM(MP(IPLOT)%IDFNAME(I+1:)),IDFNAMES,'F'))THEN; ENDIF

    NFILES(N)=SIZE(IDFNAMES)
   
   !UTL_IDFGETDATE(IDFNAME,DAYFRACTION,IYR,IMH,IDY,IHR,IMT,ISC)
