...
USE MOD_UTL, ONLY : ITOS,UTL_DIRINFO,UTL_PLOTLOCATIONIDF,UTL_GETRELEVANTDIR,UTL_CAP,  &
                    ...
                    IDFPLOT1BITMAP,IDFPLOT2BITMAP,UTL_WSELECTFILE,JDATETOFDATE,UTL_IMODVERSION,ITIMETOGDATE
...
! IPFPLUS(NPLUS)%ID=NPLUS
 ...
 INTEGER(KIND=8) :: PCKTIME_TS,PCKTIME_TSDIFF
 REAL :: FRAC
 ...
 !## get all j-dates
! ITIMES=UTL_IDATETOJDATE(21000101)
! JTIMES=UTL_IDATETOJDATE(19000101)
! DO I=1,NIDF
...
! N=JTIMES-ITIMES+1
 ...
 DO I=2,SIZE(TSDIFF(1)%IDATE)
  TSDIFF(1)%IDATE(N)=TSDIFF(1)%IDATE(I)-TSDIFF(1)%IDATE(I-1)
 ENDDO
...
! ENDIF
 ...
 DO I=1,N
  ...
  DO K=1,NIDF
   J=J+1
   !## package time
   PCKTIME_TS=TS(K)%IYR(J)*10000000000+TS(K)%IMH(J)*100000000+TS(K)%IDY(J)*1000000+TS(K)%IHR(J)*10000+TS(K)%IMT(J)*100+TS(K)%ISC(J)   
