...
USE MOD_UTL, ONLY : ITOS,UTL_DIRINFO,UTL_PLOTLOCATIONIDF,UTL_GETRELEVANTDIR,UTL_CAP,  &
                    ...
                    IDFPLOT1BITMAP,IDFPLOT2BITMAP,UTL_WSELECTFILE
...
INTEGER,PRIVATE :: ICLRRASTER
INTEGER,PRIVATE,PARAMETER :: IDELIM=44

TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:,:) :: IDF
INTEGER,PRIVATE :: NIDF,IBITMAP
TYPE LINETYPE
 INTEGER :: ICLR
 INTEGER :: IWIDTH
 INTEGER :: ITYPE
 ...
 INTEGER :: IAXES
END TYPE LINETYPE
TYPE(LINETYPE),ALLOCATABLE,DIMENSION(:),PRIVATE :: LTYPE

TYPE IPFPLUSTYPE
 INTEGER :: ID
 REAL :: X,Y
END TYPE IPFPLUSTYPE
TYPE(IPFPLUSTYPE),POINTER,DIMENSION(:),PRIVATE :: IPFPLUS,IPFDUM

INTEGER,ALLOCATABLE,DIMENSION(:),PRIVATE :: NFILES,MFILES
...
INTEGER,PRIVATE :: MINDATE,MAXDATE,IDURATION,IRESIDUAL,NPLUS,IPFIU,DSKIP
   ...
   CASE (TABCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (ID_DTIMESERIESTAB1)
      CALL IDFTIMESERIE_PLOT(X,Y) !MESSAGE%GX,MESSAGE%GY) !X,Y)
     END SELECT
...
!        CALL WDIALOGFIELDSTATE(ID_PLUSSAVE,1)
 ...
 IF(IDURATION.EQ.0)THEN
  IF(I.EQ.0)THEN !RESIDUAL.EQ.0)THEN
   CALL WDIALOGPUTSTRING(IDF_LABEL2,'Current Value: x-axes: '//TRIM(JDATETOGDATE(INT(X)))// &
...
! AXES%XTITLE='Cummalitive Distribution Function [CDF] (%)'
 AXES%LDATE=.FALSE.
 ...
 CHARACTER(LEN=1000) :: LINE
 INTEGER :: I,J,K,N,NT,ITIMES,JTIMES
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IT,JT
 LOGICAL :: LEX
 ...
 IF(IFIXX.EQ.0)THEN
  ...
  !## trim to the given mindate/maxdate
  IF(IDURATION.EQ.0)THEN
   XMIN=MAX(REAL(MINDATE),XMIN)
   XMAX=MIN(REAL(MAXDATE),XMAX)
  ENDIF
 ...
 IF(XMAX.LE.XMIN)THEN
  IF(IDURATION.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'From date ('// &
     TRIM(ITOS(JDATETOIDATE(INT(XMIN))))//') is beyond End date ('// &
 ...
 IF(IFIXY.EQ.1)THEN
  ...
  ENDIF
 ENDIF
 IF(IFIXY2.EQ.1)THEN !IRESIDUAL.EQ.1)THEN
...
! IF(WMENUGETSTATE(ID_GOOGLEEARTH,1).NE.I)CALL WMENUSETSTATE(ID_GOOGLEEARTH,1,I)
 ...
 ELSE
  ...
  IF(I.LE.NIPF)THEN
   ...
   RETURN
  ENDIF
  CALL IPFASSFILEALLOCATE(1) !  ALLOCATE(ASSF(1))
 ENDIF
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.1)THEN
   ...
   IF(IDFDATE.NE.0)THEN
    ...
    N=SIZE(IDFNAMES)
    IF(DAYFRACTION.LT.0.0)THEN
     CALL UTL_DIRINFO(MP(IPLOT)%IDFNAME(:I-1),TRIM(ROOT)//'????????'//TRIM(MP(IPLOT)%IDFNAME(J+8:)),IDFNAMES,N,'F') !NFILES(NIDF)
    ELSE
     CALL UTL_DIRINFO(MP(IPLOT)%IDFNAME(:I-1),TRIM(ROOT)//'??????????????'//TRIM(MP(IPLOT)%IDFNAME(J+14:)),IDFNAMES,N,'F') !NFILES(NIDF)
    ENDIF
    !## read each file
    K=0
    DO J=1,N,DSKIP
     IDFDATE=UTL_IDFGETDATE(IDFNAMES(J)) !,DAYFRACTION)
...
! AXES%LDATE=.TRUE.
 ...
 CALL WDIALOGFIELDSTATE(ID_PLUSSAVE,0)
! CALL WDIALOGFIELDSTATE(ID_PLUSLOAD,0)
...
! CALL WDIALOGFIELDSTATE(IDF_LABELING,0)
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL.AND.MP(IPLOT)%IPLOT.EQ.1)THEN
   ...
   IF(IDFDATE.NE.0)THEN
    ...
    ENDIF
!    CALL UTL_DIRINFO(MP(IPLOT)%IDFNAME(:I-1),TRIM(ROOT)//'*.IDF',IDFNAMES,NFILES(NIDF),'F') !????????'//TRIM(MP(IPLOT)%IDFNAME(J+8:)),IDFNAMES,NFILES(NIDF),'F')
