...
USE MOD_IPFASSFILE, ONLY : IPFDIMENSIONASSFILE
...
CONTAINS
 ...
 !## open Date select window and open IDF files
   ...
   CASE (MOUSEMOVE)
    ...
    ELSEIF(MESSAGE%WIN.EQ.ID_DTIMESERIESTAB1)THEN
     ...
     IF(NDOWN.GT.0.AND.SUM(NFILES).GT.0)THEN
      IF(IWINDOW.EQ.MESSAGE%WIN)CALL IDFTIMESERIE_PLOTXY(REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8))
