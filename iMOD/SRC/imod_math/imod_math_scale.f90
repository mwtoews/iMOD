...
MODULE MOD_MATH_SCALE
 ...
 USE WINTERACTER
 USE RESOURCE
 USE MOD_LUDCMP, ONLY : LUDCMP,LUBKSB
 ...
 INTEGER,ALLOCATABLE,DIMENSION(:),PRIVATE :: INDX
...
CONTAINS
 ...
 IF(ALLOCATED(B))   DEALLOCATE(B);    IF(ALLOCATED(INDX))DEALLOCATE(INDX)
 ...
 IF(SCLTYPE_UP.GE.7)THEN
  ...
  SELECT CASE (SCLTYPE_UP)
   CASE (11,12,13)
    MXNROW=MXNROW+2
    MXNCOL=MXNCOL+2
    ALLOCATE(A(MXNROW*MXNCOL,MXNROW*MXNCOL),B(MXNROW*MXNCOL),INDX(MXNROW*MXNCOL), &
...
! MATH1GETK_CUBE=1.0/C 
 ...
 REAL :: CVT,CT,DXY,NCV,HCHG,HCHGOLD,S
 INTEGER :: MICNVG=25
 ...
 DO ILAY=1,NLAY-1
  ...
  DO IROW=1,NROW
   DO ICOL=1,NCOL
    !## skip nodata
    IF(CV(ICOL,IROW,ILAY).GT.0.0)THEN
     CVT=CVT+CV(ICOL,IROW,ILAY)
     NCV=NCV+1.0
    ENDIF
   ENDDO
  ENDDO
...
! MEM(12)='DZ'
 ...
 DO I=1,2
  ...
  CALL LUDCMP(A(1:NROW*NCOL,1:NROW*NCOL),INDX(1:),NROW*NCOL)
  CALL LUBKSB(A(1:NROW*NCOL,1:NROW*NCOL),INDX(1:),B(1:),NROW*NCOL)
