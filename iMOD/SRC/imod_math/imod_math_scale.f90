...
MODULE MOD_MATH_SCALE
 ...
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:),PRIVATE :: DELR,DELC
 INTEGER,ALLOCATABLE,DIMENSION(:,:),PRIVATE :: PDELR,PDELC
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:),PRIVATE :: FREQ
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:),PRIVATE :: A !,L,U
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:),PRIVATE :: B,DX,DY
...
! REAL(KIND=DP_KIND),PARAMETER,PRIVATE :: HCLOSE=0.01D0   !## m
! REAL(KIND=DP_KIND),PARAMETER,PRIVATE :: RCLOSE=0.01D0   !## m3/dag
 REAL(KIND=DP_KIND),PRIVATE :: HCLOSE
 REAL(KIND=DP_KIND),PRIVATE :: RCLOSE
 REAL(KIND=DP_KIND),PARAMETER,PRIVATE :: HNOFLOW=-999.99D0
 REAL(KIND=DP_KIND),PRIVATE :: RELAX=1.0D0
 INTEGER,PARAMETER,PRIVATE :: IDAMPING=1

 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:,:),PRIVATE :: RHS,CC,CR,CV,P,V,SS,CD,HCOF
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:),PRIVATE :: DZ,TZ,BZ
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:,:),PRIVATE :: HNEW
 INTEGER,ALLOCATABLE,DIMENSION(:,:,:),PRIVATE :: IB

CONTAINS

 !###======================================================================
 LOGICAL FUNCTION MATH1SCALE(IBATCH,HC,RC)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IBATCH
 REAL(KIND=DP_KIND) :: HC,RC
 INTEGER :: I,J,N,NROW,NCOL
 LOGICAL :: LEX,LSCALE
 CHARACTER(LEN=256) :: ROOT,SCLTXT
 CHARACTER(LEN=52) :: WC
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: IX
 ...
 IF(SIZE(IDFNAMES).GT.1)THEN
  ...
  CALL QKSORT(SIZE(IDFNAMES),MATH%TOP,V2=IX)
 ...
 CALL UTL_IDFSNAPTOGRID_LLC(OUTF(1)%XMIN,OUTF(1)%XMAX,OUTF(1)%YMIN,OUTF(1)%YMAX,OUTF(1)%DX,OUTF(1)%DY,OUTF(1)%NCOL,OUTF(1)%NROW)
 ...
 IF((MPW%XMIN.LT.OUTF(1)%XMAX.AND.MPW%XMAX.GT.OUTF(1)%XMIN.AND.  &
  ...
  ENDDO
   ...
   SELECT CASE (SCLTYPE_UP)
    CASE (11:14)
     IF(RCLOSE.LT.0.0D0)RCLOSE=(MATH(1)%DX**2.0D0)/100.0D0
     WRITE(*,'(A,G15.7)') 'HCLOSE=',HCLOSE
     WRITE(*,'(A,G15.7)') 'RCLOSE=',RCLOSE
 ...
 IF(ALLOCATED(DZ))  DEALLOCATE(DZ);   IF(ALLOCATED(TZ)) DEALLOCATE(TZ)
 IF(ALLOCATED(BZ))  DEALLOCATE(BZ);   IF(ALLOCATED(IOS))DEALLOCATE(IOS)
 ...
 INTEGER,INTENT(IN) :: IBATCH
 REAL(KIND=DP_KIND) :: RATIO
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: BVALUE,TVALUE
 ...
 IF(SCLTYPE_UP.GE.7)THEN
  ...
  SELECT CASE (SCLTYPE_UP)
   ...
   CASE (14)
    ...
    !## additional row/columns
    MXNCOL=MXNCOL+2
    MXNROW=MXNROW+2
    !## allocate memory for 3d (pcg)-model
    IF(.NOT.MATH1_SIM_AL(MXNCOL,MXNROW,SIZE(IDFNAMES)+2))RETURN  !## nlay=idfnames+2 (top/bottom)
    !## compute thicknesses
    DZ=0.0D0; TZ=0.0D0; BZ=0.0D0
    DO ILAY=1,SIZE(IDFNAMES)
     TZ(ILAY+1)=MATH(ILAY)%TOP
     BZ(ILAY+1)=MATH(ILAY)%BOT
    ENDDO
    TZ(1)=TZ(2); BZ(1)=TZ(1)
    TZ(SIZE(IDFNAMES)+2)=BZ(SIZE(IDFNAMES)+1)
    BZ(SIZE(IDFNAMES)+2)=TZ(SIZE(IDFNAMES)+2)
    DO ILAY=1,SIZE(IDFNAMES)+2; DZ(ILAY)=TZ(ILAY)-BZ(ILAY); ENDDO    
  END SELECT 
 ENDIF
 ...
 DO IROW=1,OUTF(1)%NROW
  ...
  DO ICOL=1,OUTF(1)%NCOL
   ...
   IF((SCLTYPE_UP.EQ.5.OR.SCLTYPE_UP.EQ.6).AND.OUTF(1)%DX.LT.MATH(1)%DX)THEN
    RATIO=(OUTF(1)%DX**2.0D0)/(MATH(1)%DX**2.0D0)
  ...
  IF(IBATCH.EQ.1)WRITE(*,'(A,F10.2)') 'Progress ',REAL(IROW)/REAL(OUTF(1)%NROW)*100.0D0
 ...
 IF(MATH(IGRID)%IEQ.EQ.0)THEN
  ...
  DO I=1,OUTF(1)%NCOL
   ...
   PDELR(1,I)=INT(DX)+1  !from
   DX=(DELR(I)-MATH(IGRID)%XMIN)/MATH(IGRID)%DX
   PDELR(2,I)=INT(DX)+1  !to
   DX=DELR(I)-MATH(IGRID)%XMIN
   IF(MOD(DX,MATH(IGRID)%DX).EQ.0.0D0)PDELR(2,I)=PDELR(2,I)-1
  ENDDO
  DO I=1,OUTF(1)%NROW
   ...
   IF(MOD(DY,MATH(IGRID)%DY).EQ.0.0D0)PDELC(2,I)=PDELC(2,I)-1
  ENDDO
 ...
 ELSEIF(MATH(IGRID)%IEQ.EQ.1)THEN
  ...
  !## start/end column direction
  DO I=1,OUTF(1)%NCOL
   CALL POL1LOCATE(MATH(IGRID)%SX,MATH(IGRID)%NCOL+1,DELR(I-1),PDELR(1,I))
   PDELR(1,I)=PDELR(1,I)+1
   CALL POL1LOCATE(MATH(IGRID)%SX,MATH(IGRID)%NCOL+1,DELR(I),PDELR(2,I))
  ENDDO
  DO I=1,OUTF(1)%NROW
   CALL POL1LOCATE(MATH(IGRID)%SY,MATH(IGRID)%NROW+1,DELC(I-1),PDELC(1,I))
   CALL POL1LOCATE(MATH(IGRID)%SY,MATH(IGRID)%NROW+1,DELC(I),PDELC(2,I))
 ...
 REAL(KIND=DP_KIND),DIMENSION(:),INTENT(OUT) :: BVALUE,TVALUE
 INTEGER :: I,IROW,ICOL,IR,IC,IOFFSET,NROW,NCOL,NLAY,ITOP,IBOT
 REAL(KIND=DP_KIND) :: IDFVAL,NVALUE,XTOP,XBOT,X,Y,ZMID
 
 BVALUE =0.0D0 !## bot value
 TVALUE =0.0D0 !## top value
 NVALUE =0.0D0
...
!   BVALUE(1)=MATH1GLOBALLOCAL()
 ...
 !###====================================================================
 LOGICAL FUNCTION MATH1GETK_CUBE(IM)
 !###====================================================================
 IMPLICIT NONE
 ...
 DO IROW=1,SIZE(IB,2)
  DO ICOL=1,SIZE(IB,1)
   ...
   DO I=MAX(1,IM-ILGROUP),MIN(IM+ILGROUP,SIZE(IB,3))
    IF(IB(ICOL,IROW,I).EQ.1)THEN 
     C=C+LOG(CC(ICOL,IROW,I))
     N=N+1
    ENDIF
   ENDDO
 ...
 IF(N.GT.0)THEN; IF(EXP(C/DBLE(N)).LT.KMIN)MATH1GETK_CUBE=.TRUE.; ENDIF
...
!   RHS(ICOL,IROW,NLAY)=QRATE
 ...
 CT=0.0D0; NCV=0.0D0
 !## skip first and last as they don't have an area
 DO IROW=2,NROW-1
  DO ICOL=2,NCOL-1
   CVT=0.0D0
   DO ILAY=TL+1,BL-1 !2,NLAY-1
    ...
    !## sum resistances
    CVT=CVT+(MATH(1)%DX*MATH(1)%DY)/CV(ICOL,IROW,ILAY)
   ENDDO
   !## add as conductances
   IF(CVT.GT.0.0D0)THEN; CT=CT+1.0D0/CVT; NCV=NCV+1.0D0; ENDIF
  ENDDO
 ENDDO
 CT=1.0D0/(CT/NCV)
 ...
 DO ILAY=TL+1,BL-1 !2,NLAY-1
  ...
  DO IROW=1,NROW
   IF(IB(ICOL,IROW,ILAY).LT.0)THEN
    D=HNEW(ICOL-1,IROW,ILAY)-HNEW(ICOL,IROW,ILAY)
    TQ(1)=TQ(1)+(CR(ICOL-1,IROW,ILAY)*D)
   ENDIF
  ENDDO
  IROW=NROW
  DO ICOL=1,NCOL
   IF(IB(ICOL,IROW,ILAY).LT.0)THEN
    D=HNEW(ICOL,IROW-1,ILAY)-HNEW(ICOL,IROW,ILAY)
    TQ(2)=TQ(2)+(CC(ICOL,IROW-1,ILAY)*D)
   ENDIF
  ENDDO
  !## mean conductances (horizontal)
  IF(DHX.NE.0.0D0)KDX=KDX+TQ(1)/DHX   !## x-dir
  IF(DHY.NE.0.0D0)KDY=KDY+TQ(2)/DHY   !## y-dir
 ENDDO
 ...
 DO IROW=2,NROW-1
  DO ICOL=2,NCOL-1
   IF(IB(ICOL,IROW,ILAY-1).NE.0.AND.IB(ICOL,IROW,ILAY).NE.0)THEN
    D=HNEW(ICOL,IROW,ILAY)-HNEW(ICOL,IROW,ILAY-1)
 ...
 IF(QRATE.EQ.0.0D0)THEN

  !## volume of flow underneath
  SQ=TQ(3)
  !## average head below - equal to drop in head
  DP=DHZ
 ...
 !## translate m3/d (volume) to m/d (velocity)
 SQ=ABS(SQ/A)
 
 KV=SQ*(SZ/DP)
 
 !## resistance over determined top/bottom of volume
 TVALUE(3)=SZ/KV
...
!  HNEW(:,:,NLAY)=-DHZ
 ...
 IF(DHX.NE.0.0D0)THEN
  DO IROW=1,NROW; DO ILAY=1,NLAY
   IF(IB(2,IROW,ILAY).NE.2)IB(1,IROW,ILAY)=-1
  ENDDO; ENDDO
  DO IROW=1,NROW; DO ILAY=1,NLAY
   IF(IB(NCOL-1,IROW,ILAY).NE.2)IB(NCOL,IROW,ILAY)=-1
  ENDDO; ENDDO
  HNEW(NCOL,:,:)=-DHX
 ENDIF
 IF(DHY.NE.0.0D0)THEN
  DO ICOL=1,NCOL; DO ILAY=1,NLAY
   IF(IB(ICOL,2,ILAY).NE.2)IB(ICOL,1,ILAY)=-1
  ENDDO; ENDDO
  DO ICOL=1,NCOL; DO ILAY=1,NLAY
   IF(IB(ICOL,NROW-1,ILAY).NE.2)IB(ICOL,NROW,ILAY)=-1
  ENDDO; ENDDO
  HNEW(:,NROW,:)=-DHY
 ENDIF
...
!  IB(:,:,NLAY)=1             !## active again
 ...
 DO ILAY=1,NLAY
  DO IROW=1,NROW
   DY=MATH(1)%DY; IF(IROW.EQ.1.OR.IROW.EQ.NROW)DY=0.0D0
   DO ICOL=1,NCOL
    DX=MATH(1)%DX; IF(ICOL.EQ.1.OR.ICOL.EQ.NCOL)DX=0.0D0

    !## compute vertical conductance
    IF(ILAY.LT.NLAY)THEN
     IF(IB(ICOL,IROW,ILAY).NE.0.AND.IB(ICOL,IROW,ILAY+1).NE.0)THEN
      C1=0.0D0; C2=0.0D0
      IF(CC(ICOL,IROW,ILAY).NE.0.0D0)  C1=(0.5D0*DZ(ILAY  ))/(CC(ICOL,IROW,ILAY)  *VER_FCT)
      IF(CC(ICOL,IROW,ILAY+1).NE.0.0D0)C2=(0.5D0*DZ(ILAY+1))/(CC(ICOL,IROW,ILAY+1)*VER_FCT)
      IF(C1+C2.NE.0.0D0)CV(ICOL,IROW,ILAY)=(1.0D0/(C1+C2))*(DX*DY)
     ENDIF
    ENDIF
  ...
  CR(NCOL,1:NROW,ILAY)=0.0D0 !## conductance over the east   of the model
  CC(1:NCOL,NROW,ILAY)=0.0D0 !## conductance over the bottom of the model
 ...
 !###====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(OUT) :: BVALUE,TVALUE
 INTEGER,INTENT(IN) :: NCOL,NROW
 INTEGER :: I,K,N
 REAL(KIND=DP_KIND),DIMENSION(2) :: XV,YV
 ...
 DO I=1,2
  ...
  IF(SCLTYPE_UP.EQ.9)THEN
   !## fill right terms
   IF(I.EQ.1)CALL MATH1FILLB(NCOL,NROW,N=(/1.0D0,1.0D0/),S=(/0.0D0,0.0D0/))  !## gradient in x-direction
   IF(I.EQ.2)CALL MATH1FILLB(NCOL,NROW,W=(/1.0D0,1.0D0/),E=(/0.0D0,0.0D0/))  !## gradient in y-direction
  ELSEIF(SCLTYPE_UP.EQ.10)THEN
   CALL MATH1PERIODIC(I,NCOL,NROW)
  ENDIF

  N=NROW*NCOL
  CALL LUDCMP(A(1:N,1:N),B(1:N),N) 
 ...
 !###====================================================================
 REAL(KIND=DP_KIND) FUNCTION MATH1GLOBALLOCAL()
 !###====================================================================
 IMPLICIT NONE
 ...
 !###====================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND),INTENT(INOUT) :: XV,YV
 INTEGER,INTENT(IN) :: NCOL,NROW
 INTEGER :: IROW,ICOL,I
 REAL(KIND=DP_KIND) :: DX,DY,QX,QY,TX,TY
 ...
 IF(TX.NE.0.0D0)XV=QX/TX
 IF(TY.NE.0.0D0)YV=QY/TY
 ...
 DO IROW=1,NROW-1
  DO ICOL=1,NCOL-1
   IF(A(ICOL,IROW).EQ.NODATA)  CR(ICOL,IROW,1)=0.0D0
   IF(A(ICOL+1,IROW).EQ.NODATA)CR(ICOL,IROW,1)=0.0D0
   IF(A(ICOL,IROW).EQ.NODATA)  CC(ICOL,IROW,1)=0.0D0
   IF(A(ICOL,IROW+1).EQ.NODATA)CC(ICOL,IROW,1)=0.0D0
  ENDDO
 ENDDO

 CR(NCOL,1:NROW,1)=0.0D0 !## conductance over the  east of the model
 CC(NCOL,1:NROW,1)=0.0D0 !## conductance along the east edge
 CC(1   ,1:NROW,1)=0.0D0 !## conductance along the west edge

 CC(1:NCOL,NROW,1)=0.0D0 !## conductance over the  bottom of the model
 CR(1:NCOL,NROW,1)=0.0D0 !## conductance along the bottom edge
 CR(1:NCOL,   1,1)=0.0D0 !## conductance along the top    edge
 ...
 A(1,1)                            =1.0D0
 A(NCOL,NCOL)                      =1.0D0
 A(NCOL*NROW,NCOL*NROW)            =1.0D0
 A((NROW-1)*NCOL+1,(NROW-1)*NCOL+1)=1.0D0
 ...
 !## initialise identity
 IF(MAXVAL(ABS(A(I,:))).EQ.0.0D0)A(I,I)=1.0D0
 ...
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NCOL,NROW
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(2),OPTIONAL :: N,S,W,E
 INTEGER :: I,ICOL,IROW
 REAL(KIND=DP_KIND) :: TX,TY,G
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: XMIN,XMAX,YMIN,XMINIDF,YMAX,YMAXIDF,DX,DXIDF,NODATA
 REAL(KIND=DP_KIND),DIMENSION(NCOL,NROW) :: X
 REAL(KIND=DP_KIND),DIMENSION(0:NCOL) :: DELR
 REAL(KIND=DP_KIND),DIMENSION(0:NROW) :: DELC
 INTEGER :: IC,IR,I,IROW,ICOL,NPC,NPR
 REAL(KIND=DP_KIND) :: XC,XX,YY,YC,XCIDF,YCIDF
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: X1A,X2A
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: Y2A
 ...
 X=0.0D0; IF(I.GT.0.AND.J.GE.I)READ(STRING(I:J),*) X
 MATH1GETISCALE=0
 IF(X.LT.1.0D0)MATH1GETISCALE=-1 !## downscaling
 IF(X.GT.1.0D0)MATH1GETISCALE= 1 !## upscaling
 IF(X.EQ.0.0D0)MATH1GETISCALE= 1 !## upscaling (ieq=1)
