...
MODULE MOD_MATH_CALC
 ...
 USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_CHECKNAME,UTL_CREATEDIR,UTL_IDFSNAPTOGRID,UTL_MESSAGEHANDLE,UTL_WAITMESSAGE,ITOS,RTOS, &
        NEWLINE,UTL_INSIDEPOLYGON,UTL_IDFSNAPTOGRID_LLC,UTL_DIRINFO_POINTER,IDATETOJDATE,UTL_IDFGETDATE,UTL_CAP,UTL_SUBST
 USE MOD_POLINT, ONLY : POL1LOCATE !,POL1INTMAIN
 USE MOD_POLYGON_UTL, ONLY : POLYGON1CLOSE,POLYGON1INIT,POLYGON1SAVELOADSHAPE
 USE MOD_POLYGON_PAR
 USE MOD_OSD, ONLY : OSD_OPEN !,OSD_DATE_AND_TIME,OSD_GETENV
 USE MODPLOT
 USE MOD_MATH_PAR
 USE IMODVAR, ONLY : RVERSION
 USE MOD_PREF_PAR, ONLY : PREFVAL
 USE MOD_GXG_CLC, ONLY : GXG1GETGT,GXG1GTLEGWRITE
...
!       IDFRESULT=EXP(IDFRESULT)
 ...
 IF(IEFUNC.EQ.4)CALL GXG1GTLEGWRITE(TRIM(PREFVAL(1))//'\tmp\gt.leg')
