...
USE MOD_UTL, ONLY : RTOS,ITOS,UTL_GETUNIT,UTL_CHECKNAME,UTL_WSELECTFILE
...
CALL WDIALOGSELECT(ID_DMATHTAB1)
CALL WDIALOGPUTSTRING(IDF_LABEL5,'Examples: C=abs(A-B); C=log(4*A); C=5*A-0.5+B; C=A<B')
CALL WDIALOGPUTSTRING(IDF_LABEL4,'Operators:'//CHAR(9)//'+ - / * ? < >'//CHAR(13)// &
                                 'Functions:'//CHAR(9)//'abs(),log(),exp(),gtp()')
   ...
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (IDOK)
     ...
     IF(ITAB.EQ.ID_DMATHTAB3)THEN
      ...
      IF(I.GT.2)CALL MATH1SUMMAIN(LEX,I)
     ENDIF
     IF(LEX)EXIT
     CALL WDIALOGSELECT(ID_DMATH)
    ...
    CASE (IDCANCEL)
     EXIT
    CASE (IDHELP) 
     CALL IMODGETHELP('4.1.3','IDF Calculator')
