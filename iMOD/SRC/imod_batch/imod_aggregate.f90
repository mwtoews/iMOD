...
!   IF(TRIM(FNAME).EQ.'-')EXIT
 ...
 N=SIZE(FLIST); ALLOCATE(TP(N+1),KH(N),KV(N))
 DO I=1,N+1; CALL IDFNULLIFY(TP(I));                         ENDDO
 DO I=1,N;   CALL IDFNULLIFY(KH(I)); CALL IDFNULLIFY(KV(I)); ENDDO
 ...
 DO I=1,N
  ...
  IF(.NOT.IDFREAD(TP(I),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF 
  FNAME=FLIST(I)(INDEX(FLIST(I),'\',.TRUE.)+1:); FNAME=TRIM(CONDIR)//'\'//TRIM(FNAME)//'-KH_L'//TRIM(ITOS(I))//'.IDF'
  IF(.NOT.IDFREAD(KH(I),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF  
  FNAME=FLIST(I)(INDEX(FLIST(I),'\',.TRUE.)+1:); FNAME=TRIM(CONDIR)//'\'//TRIM(FNAME)//'-KV_L'//TRIM(ITOS(I))//'.IDF'
  IF(.NOT.IDFREAD(KV(I),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
  IF(I.LT.N)THEN
   FNAME=FLIST(I+1)(INDEX(FLIST(I+1),'\',.TRUE.)+1:); FNAME=TRIM(CONDIR)//'\'//TRIM(FNAME)//'-T_L'//TRIM(ITOS(I+1))//'.IDF'
  ELSE
  ...
  DO J=1,2
   M=0; DO IROW=1,TP(I)%NROW; DO ICOL=1,TP(I)%NCOL
    T=TP(I  )%X(ICOL,IROW)
    B=TP(I+1)%X(ICOL,IROW)
    IF(T-B.GT.0.0D0)THEN
     ...
     IF(J.EQ.2)THEN
      ...
      FM(I)%SF(1)%AT(M)%TP  =T
      FM(I)%SF(1)%AT(M)%BT  =B
      FM(I)%SF(1)%AT(M)%KH  =KH(I)%X(ICOL,IROW)
      FM(I)%SF(1)%AT(M)%KV  =KV(I)%X(ICOL,IROW)
     ENDIF
    ENDIF
   ENDDO; ENDDO
  ...
  P=P+M 
  CALL IDFDEALLOCATEX(TP(I)); CALL IDFDEALLOCATEX(KH(I)); CALL IDFDEALLOCATEX(KV(I)); CALL IDFDEALLOCATEX(TP(I+1))
 ...
 IF(.NOT.IDFALLOCATEX(TP(1)))STOP; IF(.NOT.IDFALLOCATEX(TP(2)))STOP
 IF(.NOT.IDFALLOCATEX(KH(1)))STOP; IF(.NOT.IDFALLOCATEX(KV(1)))STOP
 ...
 IF(.NOT.IDFREAD(TP(1),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF 
 TP(2)%X=TP(1)%X
 
 DO I=1,N
  DO J=1,SIZE(FM(I)%SF)
 
   KH(1)%X=KH(1)%NODATA; KV(1)%X=KV(1)%NODATA
   DO K=1,SIZE(FM(I)%SF)
    ...
    IF(M.GT.0)THEN
     DO L=1,M
      ...
      TP(1)%X(ICOL,IROW)=FM(I)%SF(K)%AT(L)%TP
      TP(2)%X(ICOL,IROW)=FM(I)%SF(K)%AT(L)%BT  
      KH(1)%X(ICOL,IROW)=FM(I)%SF(K)%AT(L)%KH
      KV(1)%X(ICOL,IROW)=FM(I)%SF(K)%AT(L)%KV
     ENDDO
    
     FNAME=FLIST(I)(INDEX(FLIST(I),'\',.TRUE.)+1:); FNAME=TRIM(MODELDIR)//'\'//TRIM(FNAME)//'-T_L'//TRIM(ITOS(I))//'.IDF'
     IF(.NOT.IDFWRITE(TP(1),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR WRITING '//TRIM(FNAME); STOP; ENDIF 
     FNAME=FLIST(I)(INDEX(FLIST(I),'\',.TRUE.)+1:); FNAME=TRIM(MODELDIR)//'\'//TRIM(FNAME)//'-KH_L'//TRIM(ITOS(I))//'.IDF'
     IF(.NOT.IDFWRITE(KH(1),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR WRITING '//TRIM(FNAME); STOP; ENDIF  
     FNAME=FLIST(I)(INDEX(FLIST(I),'\',.TRUE.)+1:); FNAME=TRIM(MODELDIR)//'\'//TRIM(FNAME)//'-KV_L'//TRIM(ITOS(I))//'.IDF'
     IF(.NOT.IDFWRITE(KV(1),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR WRITING '//TRIM(FNAME); STOP; ENDIF
     IF(I.LT.N)THEN
     ...
     ENDIF
     IF(.NOT.IDFWRITE(TP(2),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR WRITING '//TRIM(FNAME); STOP; ENDIF
    ENDIF
