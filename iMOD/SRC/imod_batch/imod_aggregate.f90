MODULE MOD_AGGREGATE
 ...
 !###===========================
 IMPLICIT NONE
 INTEGER :: I,J,L,N,M,O,IROW,ICOL,IU
 REAL(KIND=DP_KIND) :: F
 INTEGER,DIMENSION(:),ALLOCATABLE :: NL
 TYPE(IDFOBJ) :: MDL,IDF
 
 CALL IDFNULLIFY(IDF); CALL IDFNULLIFY(MDL)
 ...
 N=SIZE(FLIST); DO I=1,N
  ...
  !## read data
  CALL IDFCOPY(MDL,IDF)
  IF(.NOT.LHM_ADDIWHB_READFLIST(IDF,FM(I)%IDF(1),FM(I)%IDF(2),FM(I)%IDF(3),FM(I)%IDF(4),FM(I)%IDF(5),SIZE(FLIST(I)%FILE),FLIST(I)%FILE,INDIR))STOP
  CALL LHM_ADDIWHB_ADDDATA(I); FM(I)%IORDER=I

  DO J=1,SIZE(FM(I)%IDF); CALL IDFDEALLOCATEX(FM(I)%IDF(J)); ENDDO
 ...
 N=SIZE(FLIST)+SIZE(ALIST); J=0; DO I=SIZE(FLIST)+1,N
  ...
  !## read data
  J=J+1
  IF(.NOT.LHM_ADDIWHB_READFLIST(IDF,FM(I)%IDF(1),FM(I)%IDF(2),FM(I)%IDF(3),FM(I)%IDF(4),FM(I)%IDF(5),SIZE(ALIST(J)%FILE),ALIST(J)%FILE,IWHBDIR))STOP
  ...
  CALL LHM_ADDIWHB_GETMAXLAYER(I,SIZE(NL),NL)
  !## nothing to add
  IF(SUM(NL).EQ.0)CYCLE
  FM(I)%IORDER=I
 ENDDO
 ...
 IF(.NOT.LHM_ADDIWHB_READFLIST(MDL,FM(1)%IDF(1),FM(1)%IDF(2),FM(1)%IDF(3),FM(1)%IDF(4),FM(1)%IDF(5),SIZE(FLIST(1)%FILE),FLIST(1)%FILE,INDIR))STOP
 IF(FM(1)%IDF(5)%NCOL.LE.0)CALL IDFCOPY(FM(1)%IDF(1),FM(1)%IDF(5))
 FM(1)%IDF(2)%X=FM(1)%IDF(1)%X
 
!## number of formations
 N=SIZE(FM); DO I=1,N
  !## number of subformations
  M=SIZE(FM(I)%SF)
  DO J=1,M !SIZE(FM(I)%SF)
 
   O=SIZE(FM(I)%SF(J)%AT); IF(O.EQ.0)EXIT

   FM(1)%IDF(3)%X=FM(1)%IDF(3)%NODATA; FM(1)%IDF(4)%X=FM(1)%IDF(4)%NODATA; FM(1)%IDF(5)%X=FM(1)%IDF(5)%NODATA
    
   DO L=1,O
    IROW=INT(FM(I)%SF(J)%AT(L)%IROW,4)
    ICOL=INT(FM(I)%SF(J)%AT(L)%ICOL,4)
    FM(1)%IDF(1)%X(ICOL,IROW)=FM(I)%SF(J)%AT(L)%TP
    FM(1)%IDF(2)%X(ICOL,IROW)=FM(I)%SF(J)%AT(L)%BT  
    FM(1)%IDF(3)%X(ICOL,IROW)=FM(I)%SF(J)%AT(L)%KH
    FM(1)%IDF(4)%X(ICOL,IROW)=FM(I)%SF(J)%AT(L)%KV
    FM(1)%IDF(5)%X(ICOL,IROW)=FM(1)%IDF(4)%X(ICOL,IROW)/FM(1)%IDF(3)%X(ICOL,IROW)
   ENDDO
    
   !## save idf files
   CALL LHM_CONVERTREGIS_OUTPUT(I,FM(1)%IDF(1),FM(1)%IDF(2),FM(1)%IDF(3),FM(1)%IDF(4),FM(1)%IDF(5))
   WRITE(IU,'(5A32)') (TRIM(FLIST(I)%FILE(L)),L=1,5)

   DO L=1,O
    IROW=INT(FM(I)%SF(J)%AT(L)%IROW,4)
    ICOL=INT(FM(I)%SF(J)%AT(L)%ICOL,4)
    FM(1)%IDF(1)%X(ICOL,IROW)=FM(I)%SF(J)%AT(L)%BT
   ENDDO

  ENDDO

  F=100.0D0*DBLE(I)/DBLE(N); WRITE(6,'(A)') '+WRITING '//TRIM(FLIST(I)%FILE(1))//'('//TRIM(RTOS(F,'F',2))//'%)'
 ...
 DO I=1,SIZE(FLIST); DO J=1,SIZE(FM(I)%IDF); CALL IDFDEALLOCATEX(FM(I)%IDF(J)); ENDDO; ENDDO

 END SUBROUTINE LHM_ADDIWHB
 
 !###===========================
 SUBROUTINE LHM_ADDIWHB_ADDDATA(I)
 !###===========================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: I
 INTEGER :: J,M,IROW,ICOL
 REAL(KIND=DP_KIND) :: T,B
 
 DO J=1,2
  M=0; DO IROW=1,FM(I)%IDF(1)%NROW; DO ICOL=1,FM(I)%IDF(1)%NCOL
   T=FM(I)%IDF(1)%X(ICOL,IROW); B=FM(I)%IDF(2)%X(ICOL,IROW)
   IF(T-B.GT.0.0D0)THEN
    ...
    IF(J.EQ.2)THEN
     ...
     FM(I)%SF(1)%AT(M)%TP  =T
     FM(I)%SF(1)%AT(M)%BT  =B
     FM(I)%SF(1)%AT(M)%KH  =FM(I)%IDF(3)%X(ICOL,IROW)
     FM(I)%SF(1)%AT(M)%KV  =FM(I)%IDF(4)%X(ICOL,IROW)
    ENDIF
   ENDIF
  ENDDO; ENDDO
...
ILOOP: DO I=1,SIZE(FM)
  ...
  DO J=1,SIZE(FM(I)%SF)
   ...
   DO K=1,M
    ...
    IF(T.GT.BF.AND.T.LT.TF)NL(I)=NL(I)+1
    IF(B.GT.BF.AND.B.LT.TF)NL(I)=NL(I)+1
...
RLOOP: DO IROW=1,TPIDF%NROW; DO ICOL=1,TPIDF%NCOL
 ...
 !###===========================
 SUBROUTINE LHM_CONVERTREGIS_OUTPUT(I,TPIDF,BTIDF,KHIDF,KVIDF,VAIDF) 
 !###===========================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: I
