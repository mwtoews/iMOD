MODULE MOD_AGGREGATE
 ...
 !###===========================
 SUBROUTINE LHM_ADDIWHB_CORDATA(IFM)
 !###===========================
 IMPLICIT NONE
  ...
  DO J=1,SIZE(FM(I)%SF)
   ...
   DO K=1,N
    ...
    IF(T-B.GT.0.0D0)THEN
     ...
     !## correct if possible
     IF(DZ.GT.0.0D0)THEN
      FM(I)%SF(J)%AT(K)%KH=TR/DZ
      FM(I)%SF(J)%AT(K)%KV=DZ/VC
     ENDIF
 ...
 END SUBROUTINE LHM_ADDIWHB_CORDATA
...
RLOOP: DO IROW=1,TPIDF%NROW; DO ICOL=1,TPIDF%NCOL
 ...
 IF(.NOT.LEX)THEN
  IF(INDEX(VAIDF%FNAME,'.IDF').LE.0)THEN
   J=INDEX(TPIDF%FNAME,'-'); FTXT=TPIDF%FNAME(:J-1); VAIDF%FNAME=TRIM(FTXT)//'-VA-C.IDF' 
  ENDIF
