...
!   IF(TRIM(FNAME).EQ.'-')EXIT
 ...
 DO I=1,N
  ...
  !## read data
  IF(.NOT.LHM_ADDIWHB_READFLIST(IDF,TP,BT,KH,KV,I,N))STOP
 ...
 !## modifying the solid
 DO
!  IF(.NOT.LHM_ADDIWHB_READFLIST(I,ALIST(I)))STOP
!  FNAME=ALIST(I)(INDEX(ALIST(I),'\',.TRUE.)+1:); FNAME=TRIM(ADIR)//'\'//TRIM(FNAME)//'-T_L'//TRIM(ITOS(I))//'.IDF'
 ENDDO
 ...
 !###===========================
 LOGICAL FUNCTION LHM_ADDIWHB_READFLIST(IDF,TP,BT,KH,KV,I,N)  
 !###===========================
 IMPLICIT NONE
 ...
 FN=TRIM(FLIST(I))//'-T_L'//TRIM(ITOS(I))//'.IDF'
 IF(IWINDOW.EQ.1)THEN
 ...
 FN=TRIM(FLIST(I))//'-KH_L'//TRIM(ITOS(I))//'.IDF'
 IF(IWINDOW.EQ.1)THEN
 ...
 FN=TRIM(FLIST(I))//'-KV_L'//TRIM(ITOS(I))//'.IDF'
 IF(IWINDOW.EQ.1)THEN
 ...
 IF(I.LT.N)THEN
  FN=FLIST(I+1); FN=TRIM(FN)//'-T_L'//TRIM(ITOS(I+1))//'.IDF'
 ELSE
  FN=FLIST(I); FN=TRIM(FN)//'-B_L'//TRIM(ITOS(I))//'.IDF'
 ENDIF
 IF(IWINDOW.EQ.1)THEN
