MODULE MOD_AGGREGATE
 ...
 !###===========================
 SUBROUTINE LHM_CONVERTREGIS_INIT()
 !###===========================
 IMPLICIT NONE
 INTEGER :: I,N,IU,IOS
 ...
 !###===========================
 SUBROUTINE LHM_CONVERTREGIS_TRIED()
 !###===========================
 IMPLICIT NONE
 INTEGER :: I,J,K,N,ICOL,IROW,BSIZE,INODE,NODES
 REAL(KIND=DP_KIND) :: T,B,K1,K2
 REAL(KIND=DP_KIND),DIMENSION(:,:),ALLOCATABLE :: TOP,BOT,KHV,KVV
 ...
 DO I=1,N
  WRITE(6,*) 'Reading '//TRIM(FLIST(I))//' ...'
  FNAME=TRIM(FLIST(I))//'-T-C.IDF';  IF(.NOT.IDFREAD(TP(I),FNAME,-1))THEN; WRITE(6,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
  FNAME=TRIM(FLIST(I))//'-B-C.IDF';  IF(.NOT.IDFREAD(BT(I),FNAME,-1))THEN; WRITE(6,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
  FNAME=TRIM(FLIST(I))//'-KH-S.IDF'; IF(.NOT.IDFREAD(KH(I),FNAME,-1,IQ=1))THEN; ENDIF
  FNAME=TRIM(FLIST(I))//'-KV-S.IDF'; IF(.NOT.IDFREAD(KV(I),FNAME,-1,IQ=1))THEN; ENDIF
 ENDDO
 ...
 !## allocate blocksize memory
 ALLOCATE(TOP(N,BLOCKSIZE),BOT(N,BLOCKSIZE),KHV(N,BLOCKSIZE),KVV(N,BLOCKSIZE),X(BLOCKSIZE))
 ...
 DO I=1,N
  ...
  DO J=I,N
   FNAME=TRIM(FLIST(J))//'-T-C.IDF';  IF(.NOT.IDFREAD(TP(J),FNAME,-1))THEN; WRITE(6,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
   FNAME=TRIM(FLIST(J))//'-B-C.IDF';  IF(.NOT.IDFREAD(BT(J),FNAME,-1))THEN; WRITE(6,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
   FNAME=TRIM(FLIST(J))//'-KH-S.IDF'; IF(.NOT.IDFREAD(KH(J),FNAME,-1,IQ=1))THEN; ENDIF
   FNAME=TRIM(FLIST(J))//'-KV-S.IDF'; IF(.NOT.IDFREAD(KV(J),FNAME,-1,IQ=1))THEN; ENDIF
  ENDDO

  NODES=TP(I)%NROW*TP(I)%NCOL; INODE=0
  ICOL=0; IROW=1
  DO 
   ...
   DO J=I,N
    IF(TP(J)%ITYPE.EQ.8)THEN
     READ(TP(J)%IU) (TOP(J,K),K=1,BSIZE)
    ELSE
     READ(TP(J)%IU) (X(K),K=1,BSIZE)
     DO K=1,BSIZE; TOP(J,K)=X(K); ENDDO
    ENDIF
    IF(BT(J)%ITYPE.EQ.8)THEN
     READ(BT(J)%IU) (BOT(J,K),K=1,BSIZE)
    ELSE
     READ(BT(J)%IU) (X(K),K=1,BSIZE)
     DO K=1,BSIZE; BOT(J,K)=X(K); ENDDO
    ENDIF
    IF(KH(J)%IU.GT.0)THEN
     IF(KH(J)%ITYPE.EQ.8)THEN
      READ(KH(J)%IU) (KHV(J,K),K=1,BSIZE)
     ELSE
      READ(KH(J)%IU) (X(K),K=1,BSIZE)
      DO K=1,BSIZE; KHV(J,K)=X(K); ENDDO
     ENDIF
    ENDIF
    IF(KV(J)%IU.GT.0)THEN
     IF(KV(J)%ITYPE.EQ.8)THEN
      READ(KV(J)%IU) (KVV(J,K),K=1,BSIZE)
     ELSE
      READ(KV(J)%IU) (X(K),K=1,BSIZE)
      DO K=1,BSIZE; KVV(J,K)=X(K); ENDDO
     ENDIF
    ENDIF
   ENDDO
   
   DO K=1,BSIZE
    ICOL=ICOL+1; IF(ICOL.GT.TP(I)%NCOL)THEN; ICOL=1; IROW=IROW+1; ENDIF
    IBIDF%X(ICOL,IROW)=0
    !## nodata remains nodata 
    IF(I.GT.1.AND.TPIDF%X(ICOL,IROW).EQ.TPIDF%NODATA)CYCLE
    T=TOP(I,K); B=BOT(I,K)
    ...
    IF(T.EQ.TP(I)%NODATA)THEN
     ...
     K1=TP(1)%NODATA; K2=TP(1)%NODATA
    ELSE
     K1=EPS; IF(KH(I)%IU.GT.0)K1=KHV(I,K); IF(K1.EQ.KH(I)%NODATA)K1=EPS
     K2=EPS; IF(KV(I)%IU.GT.0)K2=KVV(I,K); IF(K2.EQ.KV(I)%NODATA)K2=EPS
     IF(K1.EQ.0.0)K1=K2; IF(K2.EQ.0.0)K2=K1
     IF(K1.EQ.0.0.AND.K2.EQ.0.0)THEN
      K1=TP(1)%NODATA; K2=K1; T=BTIDF%X(ICOL,IROW); B=BTIDF%X(ICOL,IROW)
     ELSE
  ...
  DO J=1,N
   IF(TP(J)%IU.GT.0)CLOSE(TP(J)%IU); IF(BT(J)%IU.GT.0)CLOSE(BT(J)%IU)
   IF(KH(J)%IU.GT.0)CLOSE(KH(J)%IU); IF(KV(J)%IU.GT.0)CLOSE(KV(J)%IU)
  ENDDO
 ...
 DEALLOCATE(TOP,BOT,KHV,KVV,X)

 END SUBROUTINE LHM_CONVERTREGIS_TRIED
 ...
 TYPE(IDFOBJ),DIMENSION(:,:),ALLOCATABLE :: TP,KH,VA
  
 WRITE(OUTPUTDIR,'(A,I5.5)') TRIM(MODELDIR)//'\CMIN_',INT(CMIN)
 CALL UTL_CREATEDIR(OUTPUTDIR)
 
 N=SIZE(FLIST); ALLOCATE(TP(N+1,2),KH(N,2),VA(N,2))
 DO I=1,N+1; DO J=1,2; CALL IDFNULLIFY(TP(I,J)); ENDDO; ENDDO
 DO I=1,N;   DO J=1,2; CALL IDFNULLIFY(KH(I,J)); CALL IDFNULLIFY(VA(I,J)); ENDDO; ENDDO
 ...
 DO I=1,N
  ...
  IF(.NOT.IDFWRITE(TP(I,2),FNAME,0,-1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
  ...
  IF(.NOT.IDFWRITE(KH(I,2),FNAME,0,-1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF

  FNAME=FLIST(I)(INDEX(FLIST(I),'\',.TRUE.)+1:); FNAME=TRIM(CONDIR)//'\'//TRIM(FNAME)//'-VA_L'//TRIM(ITOS(I))//'.IDF'
  IF(.NOT.IDFREAD(VA(I,1),FNAME,-1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
  FNAME=FLIST(I)(INDEX(FLIST(I),'\',.TRUE.)+1:); FNAME=TRIM(OUTPUTDIR)//'\'//TRIM(FNAME)//'-VA_L'//TRIM(ITOS(I))//'.IDF'
  CALL IDFCOPY(VA(I,1),VA(I,2))
  IF(.NOT.IDFWRITE(VA(I,2),FNAME,0,-1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
  ...
  IF(I.EQ.N)THEN
   ...
   IF(.NOT.IDFWRITE(TP(I+1,2),FNAME,0,-1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(FNAME); STOP; ENDIF
  ENDIF
 
 ENDDO
 
 ALLOCATE(TOP(N+1,BLOCKSIZE),KHV(N,BLOCKSIZE),KVV(N,BLOCKSIZE),KVA(N,BLOCKSIZE),VCV(N),KDW(N))
 ...
 DO 
  ...
  DO I=1,N
   ...
   READ(VA(I,1)%IU) (KVA(I,J),J=1,BSIZE)
   DO J=1,BSIZE; KVV(I,J)=KHV(I,J)*KVA(I,J); ENDDO
  ENDDO
  ...
  DO I=1,N
   WRITE(TP(I,2)%IU) (TOP(I,J),J=1,BSIZE)
   WRITE(KH(I,2)%IU) (KHV(I,J),J=1,BSIZE)
   DO J=1,BSIZE; KVA(I,J)=KVV(I,J)/KHV(I,J); ENDDO
   WRITE(VA(I,2)%IU) (KVA(I,J),J=1,BSIZE)
  ENDDO
  WRITE(TP(N+1,2)%IU) (TOP(N+1,J),J=1,BSIZE)
 ...
 DEALLOCATE(TOP,KHV,KVV,VCV,KDW)
