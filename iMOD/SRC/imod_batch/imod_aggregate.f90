MODULE MOD_AGGREGATE
 ...
 SELECT CASE (IMODE)
  ...
  CASE (2)
   ...
   N=SIZE(FLIST); IF(ASSOCIATED(ALIST))N=N+SIZE(ALIST); ALLOCATE(TLIST(N))
   DO I=1,SIZE(FLIST); TLIST(I)=FLIST(I); ENDDO; I=SIZE(FLIST)
   IF(ASSOCIATED(ALIST))THEN;
    DO J=1,SIZE(ALIST); I=I+1; TLIST(I)=ALIST(J); ENDDO
   ENDIF
   CALL LHM_CONVERTREGIS_SORTFILES_DUPLICATES(TLIST)
   DO I=1,SIZE(FLIST); FLIST(I)=TLIST(I); ENDDO; I=SIZE(FLIST)
   IF(ASSOCIATED(ALIST))THEN;
    DO J=1,SIZE(ALIST); I=I+1; ALIST(J)=TLIST(I); ENDDO
   ENDIF
   DEALLOCATE(TLIST)
   CALL LHM_ADDIWHB_RASTER(OBSICOL,OBSIROW) !CALL LHM_ADDIWHB()
  !## aggregate dataset
  CASE (3)
   CALL LHM_CONVERTREGIS_SORTFILES_DUPLICATES(FLIST)
 ...
 WRITE(*,'(/A,I10/)') 'NUMBER OF FILES FOUND IS ',SIZE(SLIST)
 ...
 !###=====================================================
 SUBROUTINE LHM_CONVERTREGIS_SORTFILES_DUPLICATES(SLIST)
 !###=====================================================
 IMPLICIT NONE
 TYPE(FOBJ),INTENT(INOUT),POINTER,DIMENSION(:) :: SLIST
 INTEGER :: I,J,II,JJ
 ...
 DO I=1,SIZE(SLIST); DO J=1,SIZE(SLIST(I)%FILE)
  ...
  DO II=SIZE(SLIST),1,-1; DO JJ=SIZE(SLIST(II)%FILE),1,-1
   ...
   IF(TRIM(SLIST(I)%FILE(J)).EQ.TRIM(SLIST(II)%FILE(JJ)))THEN
    IF(II.LT.I)THEN
     SLIST(I)%DUPL_F(J)=II; SLIST(I)%DUPL_T(J)=JJ
 ...
 END SUBROUTINE LHM_CONVERTREGIS_SORTFILES_DUPLICATES
 ...
 DO I=1,SIZE(FLIST)
  CALL LHM_ADDIWHB_RASTER_OPENFILES_READ(FLIST(I)%FILE,FLIST(I)%DUPL_F,FLIST(I)%DUPL_T,INDIR,IDF,I,MDL) 
 ENDDO; I=I-1
 !## open all iwhb files for reading
 DO J=1,NA; I=I+1
  CALL LHM_ADDIWHB_RASTER_OPENFILES_READ(ALIST(J)%FILE,ALIST(J)%DUPL_F,ALIST(J)%DUPL_T,IWHBDIR,IDF,I,MDL) 
 ENDDO
 !## check all files
 DO I=1,SIZE(IDF,1); DO J=1,SIZE(IDF,2)
  IF(.NOT.IDFEQUAL(IDF(I,J),MDL,0))THEN
   WRITE(*,'(/1A/)') 'IDF '//TRIM(IDF(I,J)%FNAME)//' IS NOT EQUAL TO MODELED AREA'; STOP
  ENDIF
 ENDDO; ENDDO
...
!      ALIST(J)%ILAYER=
 ...
 DO I=1,SIZE(FLIST)
  ...
  IF(TP.GT.BT)THEN
   ...
   IF(KH.LE.0.0D0.OR.KV.LE.0.0D0)THEN
    DO J=1,SIZE(FLIST)
 ...
 IF(ICHECK.GT.0)THEN
  ...
  IF(ASSOCIATED(ALIST))THEN
   WRITE(*,'(/A/)') 'ALIST'
   J=SIZE(FLIST); DO I=1,SIZE(ALIST)
    J=J+1; WRITE(*,'(3I5,5F15.7)') K,I,J,IDF(1,J)%V(K),IDF(2,J)%V(K),IDF(3,J)%V(K),IDF(4,J)%V(K),IDF(5,J)%V(K)
   ENDDO
  ENDIF
 ENDIF
 ...
 !###=====================================================
 SUBROUTINE LHM_ADDIWHB_RASTER_OPENFILES_READ(LIST,DUPL_F,DUPL_T,DIR,IDF,IM,MDL)
 !###=====================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: MDL
 ...
 DO I=1,SIZE(IDF,1)
  ...
  IF(IOS.NE.0)THEN
   IF(TRIM(LIST(I)).NE.'')THEN
    IDF(I,IM)%FNAME=TRIM(DIR)//'\'//TRIM(LIST(I))
    !## open, otherwise already opened
    IF(DUPL_F(I).EQ.0.AND.DUPL_T(I).EQ.0)THEN
     IF(.NOT.IDFREAD(IDF(I,IM),TRIM(IDF(I,IM)%FNAME),-1))THEN; WRITE(*,'(/A/)') 'ERROR READING '//TRIM(IDF(I,IM)%FNAME); STOP; ENDIF 
    ELSE
     CALL IDFCOPY(IDF(DUPL_T(I),DUPL_F(I)),IDF(I,IM))
    ENDIF
   ELSE
    CALL IDFCOPY(MDL,IDF(I,IM))
   ENDIF
  ELSE
   CALL IDFCOPY(MDL,IDF(I,IM)); ALLOCATE(IDF(I,IM)%X(1,1)); IDF(I,IM)%X(1,1)=X
  ENDIF
 ENDDO
 ...
 IF(TRIM(LIST(5)).EQ.'')THEN
  ...
  IDF(5,IM)%V(1:BSIZE)=IDF(4,IM)%V(1:BSIZE)/IDF(3,IM)%V(1:BSIZE)
 ELSEIF(TRIM(LIST(4)).EQ.'')THEN
  IDF(4,IM)%V(1:BSIZE)=IDF(3,IM)%V(1:BSIZE)*IDF(5,IM)%V(1:BSIZE)
 ELSEIF(TRIM(LIST(3)).EQ.'')THEN
  IDF(3,IM)%V(1:BSIZE)=IDF(4,IM)%V(1:BSIZE)/IDF(5,IM)%V(1:BSIZE)
 ENDIF

 !## check k-values
 DO I=1,BSIZE
  TP=IDF(1,IM)%V(I); BT=IDF(2,IM)%V(I); KH=IDF(3,IM)%V(I); KV=IDF(4,IM)%V(I); VA=IDF(5,IM)%V(I)
  IF(TP-BT.GT.0.0D0)THEN
   IF(VA.NE.IDF(5,IM)%NODATA)THEN
    IF(KH.EQ.IDF(3,IM)%NODATA.AND.KV.NE.IDF(4,IM)%NODATA)THEN
     IDF(3,IM)%V(I)=KV/VA
    ELSEIF(KV.EQ.IDF(4,IM)%NODATA.AND.KH.NE.IDF(3,IM)%NODATA)THEN
     IDF(4,IM)%V(I)=KH*VA
    ENDIF
   ENDIF
  ENDIF
...
ILOOP: DO I=1,IFM-1
 ...
 WRITE(OUTPUTDIR,'(A,I5.5)') TRIM(OUTDIR); CALL UTL_CREATEDIR(OUTPUTDIR)
...
! ENDIF
 ...
 N=SIZE(FLIST); ALLOCATE(IDF(5,N),ODF(5,N)) 
 DO I=1,SIZE(IDF,1); DO J=1,SIZE(IDF,2); CALL IDFNULLIFY(IDF(I,J)); ENDDO; ENDDO
 DO I=1,SIZE(ODF,1); DO J=1,SIZE(ODF,2); CALL IDFNULLIFY(ODF(I,J)); ENDDO; ENDDO
 ...
 !## check all files
 DO I=1,SIZE(IDF,1); DO J=1,SIZE(IDF,2)
  IF(.NOT.IDFEQUAL(IDF(I,J),MDL,0))THEN; WRITE(*,'(/1A/)') 'IDF '//TRIM(IDF(I,J)%FNAME)//' IS NOT EQUAL TO MODELED AREA'; STOP; ENDIF
 ENDDO; ENDDO
 
 NODES=MDL%NROW*MDL%NCOL; BLOCKSIZE=1000; BLOCKSIZE=MIN(NODES,BLOCKSIZE); ALLOCATE(TMP(BLOCKSIZE))
 ...
 DO 
  ...
  DO J=1,BSIZE
   ...
   DO I=1,SIZE(IDF,2)
    ...
    KHV(I)=ODF(3,I)%V(J); KVV(I)=ODF(4,I)%V(J)
    VCV(I)=(TOP(I)-BOT(I))/KVV(I)
    KDW(I)=(TOP(I)-BOT(I))*KHV(I)
    BND(I)=1.0
   ENDDO
  ...
  !## write all data for current block
  DO I=1,SIZE(ODF,2); CALL LHM_ADDIWHB_RASTER_WRITE_DATA(ODF(:,I),BSIZE); ENDDO
 ...
 IF(IPRJ.EQ.1)CALL LHM_CONVERTREGIS_WRITEPRJ(ODF)

 DEALLOCATE(TMP,TOP,KHV,KVV,VCV,KDW,BOT)
 
 END SUBROUTINE LHM_CONVERTREGIS_AGGREGATE

 !###======================================================
 SUBROUTINE LHM_CONVERTREGIS_WRITEPRJ(ODF)
 !###======================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(:,:) :: ODF
 INTEGER :: I,IU,N
 ...
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(OUTPUTDIR)//'\MF6_AGGREGATE.PRJ',STATUS='UNKNOWN',ACTION='WRITE')

 WRITE(IU,'(/A)') '0001,(BND),1, BOUNDARY CONDITION,[BND]'
 WRITE(IU,'(A,I10)') '001,',N
 DO I=1,N
  WRITE(IU,'(A,I3,A)') '1,2,',I,',1.0,0.0,-999.99,'//TRIM(ODF(4,I)%FNAME)
 ENDDO
 
 WRITE(IU,'(/A)') '0001,(TOP),1, TOP ELEVATION,[TOP]'
 WRITE(IU,'(A,I10)') '001,',N
 DO I = 1,N
  WRITE(IU,'(A,I3,A)') '1,2,',I,',1.0,0.0,-999.99,'//TRIM(ODF(1,I)%FNAME)
 ENDDO
 
 WRITE(IU,'(/A)') '0001,(BOT),1, BOTTOM ELEVATION,[BOT]'
 WRITE(IU,'(A,I10)') '001,',N
 DO I = 1,N
  WRITE(IU,'(A,I3,A)') '1,2,',I,',1.0,0.0,-999.99,'//TRIM(ODF(2,I)%FNAME)
 ENDDO

 WRITE(IU,'(/A)') '0001,(KHV),1, HORIZONTAL PERMEABILITY,[KHV]'
 WRITE(IU,'(A,I10)') '001,',N
 DO I=1,N
  WRITE(IU,'(A,I3,A)') '1,2,',I,',1.0,0.0,-999.99,'//TRIM(ODF(3,I)%FNAME)
 ENDDO

 WRITE(IU,'(/A)') '0001,(KVA),1, VERTICAL ANISOTROPY,[KVA]'
 WRITE(IU,'(A,I10)') '001,',N 
 DO I=1,N
  WRITE(IU,'(A,I3,A)') '1,2,',I,',1.0,0.0,-999.99,'//TRIM(ODF(5,I)%FNAME)
 ENDDO
 ...
 WRITE(IU,'(A,I10)') '001,',N
 DO I=1,N
  WRITE(IU,'(A,I3,A)') '1,2,',I,',1.0,0.0,-999.99,'//TRIM(ODF(1,1)%FNAME)
 ENDDO
