MODULE MOD_AGGREGATE
 ...
 DO I=1,2; M=1; N=1; DO
   ...
   ELSE
    READ(IU,*,IOSTAT=IOS) (SLIST(M)%FILE(J),J=1,SIZE(SLIST(M)%FILE)),SLIST(M)%ILAYER,SLIST(M)%METH; IF(IOS.NE.0)EXIT
   ENDIF
   N=N+1; IF(I.EQ.2)M=N
  ENDDO
  IF(I.EQ.1)THEN
...
RLOOP: DO IROW=1,TPIDF%NROW; DO ICOL=1,TPIDF%NCOL
 ...
 IF(INDEX(KHIDF%FNAME,'.IDF').LE.0)THEN
  ...
  FTXT=TPIDF%FNAME(:JJ-1); KHIDF%FNAME=TRIM(FTXT)//'-KH-C.IDF' 
 ENDIF
 IF(INDEX(KVIDF%FNAME,'.IDF').LE.0)THEN
  J= INDEX(TPIDF%FNAME,'_',.TRUE.)
  JJ=INDEX(TPIDF%FNAME(J+1:),'-'); JJ=JJ+J
  FTXT=TPIDF%FNAME(:JJ-1); KVIDF%FNAME=TRIM(FTXT)//'-KV-C.IDF' 
 ENDIF
