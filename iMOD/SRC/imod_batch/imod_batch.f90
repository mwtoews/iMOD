...
! ENDIF
 ...
 IMPLICIT NONE
 INTEGER :: NPAR,I
 CHARACTER(LEN=12),DIMENSION(:),ALLOCATABLE :: GRN
 CHARACTER(LEN=2),DIMENSION(:),ALLOCATABLE :: FCL
 INTEGER,DIMENSION(:),ALLOCATABLE :: FCN
 ...
 DO I=1,NPAR
  ...
  READ(LINE,*) GRN(I)
  LINE='GRAIN'//TRIM(ITOS(I))//'='//TRIM(GRN(I)); WRITE(*,'(A)') TRIM(LINE)
  IF(.NOT.UTL_READINITFILE('FACIESL'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) FCL(I)
  LINE='FACIESL'//TRIM(ITOS(I))//'='//TRIM(FCL(I)); WRITE(*,'(A2)') TRIM(LINE)
  IF(.NOT.UTL_READINITFILE('FACIESN'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) FCN(I)
  LINE='FACIESN'//TRIM(ITOS(I))//'='//TRIM(ITOS(FCN(I))); WRITE(*,'(I2)') TRIM(LINE)    
 ENDDO

 CALL FLUMY_MAIN()
 
 DEALLOCATE(GRN); DEALLOCATE(FCL); DEALLOCATE(FCN)
