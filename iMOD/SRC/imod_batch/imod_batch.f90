...
!  TNG=TNG-0.5*PI
 ...
 IMPLICIT NONE
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: IDF
 INTEGER :: I,II,J,IROW,ICOL,JROW,JCOL,IPZ,JPZ,MINT
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 USE MOD_ASC2IDF_PAR, ONLY : IDFFILE,SOURCEDIR,TARGETDIR,IGRIDFUNC,CS,NODATA,LAGDISTANCE, &
   ...
   ASSF_COLUMN,ASSF_STARTDATE,ASSF_ENDDATE,ASSF_DDATE,ASSF_CDDATE,ILOG,TRIMDEPTH_IDF, &
   GENFILE,BLNFILE
 USE MOD_SOLID, ONLY : MXITER2,HCLOSE,RCLOSE
 USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_JDATETOIDATE
 IMPLICIT NONE
 INTEGER :: I,J,K,N,IN_TYPE,JU,M,IDATE,SDATE,EDATE,DDATE,IOS,IDEPTH
 ...
 IDEPTH=0; IXCOL=1; IYCOL=2; IZCOL=3; ASSF_COLUMN=0; ASSF_STARTDATE=0; ASSF_ENDDATE=0; ASSF_DDATE=0; ASSF_CDDATE=''; TRIMDEPTH_IDF%FNAME=''
 IF(IN_TYPE.EQ.2)THEN
  ...
  IF(M.GT.0.AND.M.EQ.IZCOL)THEN
   ...
   ENDIF
   IF(ASSF_STARTDATE.GT.19000000)THEN
 ...
 IF(N.EQ.0)THEN
  ...
  DO   
   ...
   ELSEIF(IDEPTH.EQ.1)THEN
    ASSF_STARTDATE= IDATE
    ASSF_ENDDATE  = MAX(IDATE+DDATE,EDATE)
   ENDIF
   !## startdate
   IF(ASSF_COLUMN.NE.0)THEN
    IF(IDEPTH.EQ.0)IDFFILE=TRIM(FNAME)//'_'//TRIM(ITOS(UTL_JDATETOIDATE(IDATE)))//'.IDF'
    IF(IDEPTH.EQ.1)IDFFILE=TRIM(FNAME)//'_'//TRIM(ITOS(IDATE))//'.IDF'
   ENDIF
   IF(.NOT.ASC2IDF_INT_MAIN(1,XMIN,YMIN,XMAX,YMAX,IDEPTH))THEN; ENDIF
   IDATE=IDATE+DDATE
   IF(IDEPTH.EQ.0)THEN
    IF(IDATE.GT.EDATE)EXIT
   ELSEIF(IDEPTH.EQ.1)THEN
    IF(IDATE.LT.EDATE)EXIT
   ENDIF
  ENDDO
 ...
 ELSE
  ...
  DO 
   ...
   DO I=1,N
    ...
    IF(.NOT.ASC2IDF_INT_MAIN(I,XMIN,YMIN,XMAX,YMAX,IDEPTH))THEN
    ENDIF
   ENDDO
