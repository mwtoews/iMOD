...
!## Write GEN and RUN file  ! FR 20131016
 ...
 IF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  IMODE=1
  READ(LINE,*) PRJFILE; WRITE(*,'(A,A)')  'PRJFILE_IN=',TRIM(PRJFILE)

  IF(UTL_READINITFILE('RUNFILE_OUT',LINE,IU,1))THEN
   READ(LINE,*) RUNFILE; WRITE(*,'(A,A)') 'RUNFILE_OUT=',TRIM(RUNFILE)
   PBMAN%IFORMAT=1
  ENDIF
  IF(UTL_READINITFILE('NAMFILE_OUT',LINE,IU,1))THEN
   READ(LINE,*) RUNFILE; WRITE(*,'(A,A)') 'NAMFILE_OUT=',TRIM(RUNFILE)
   PBMAN%IFORMAT=2
  ENDIF
  ...
  IF(PBMAN%IFORMAT.EQ.2)THEN
   ...
   WRITE(*,'(A,I10)') 'UNCONFINED=',PBMAN%UNCONFINED
