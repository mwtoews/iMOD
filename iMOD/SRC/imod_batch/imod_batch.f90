...
!## Write GEN and RUN file  ! FR 20131016
 ...
 INTEGER :: I,IFLOW,IPNTR,IWINDOW,NFTHREAD,MXTHREAD,IWRITE
 REAL :: XMIN,YMIN,XMAX,YMAX,CELLSIZE,FRACTION
 ...
 IF(IFLOW.EQ.0)THEN
  ...
  CALL SOF_MAIN(IDF,IFLOW,IPNTR,IWINDOW,XMIN,YMIN,XMAX,YMAX,CELLSIZE) 
 
 ELSEIF(IFLOW.EQ.1)THEN
 
  FRACTION=0.25
  IF(.NOT.UTL_READINITFILE('FRACTION',LINE,IU,0))RETURN
  READ(LINE,*) FRACTION; WRITE(*,'(A,F10.2)') 'FRACTION=',FRACTION
  NFTHREAD=1000
  IF(UTL_READINITFILE('NFTHREAD',LINE,IU,1))READ(LINE,*) NFTHREAD
  WRITE(*,'(A,I10)') 'NFTHREAD=',NFTHREAD
  MXTHREAD=500
  IF(UTL_READINITFILE('MXTHREAD',LINE,IU,1))READ(LINE,*) MXTHREAD
  WRITE(*,'(A,I10)') 'MXTHREAD=',MXTHREAD
  ...
  IF(UTL_READINITFILE('DISZONEIPF',LINE,IU,1))THEN
   READ(LINE,*) IDF(6)%FNAME; WRITE(*,'(A)') 'DISZONEIPF='//TRIM(IDF(6)%FNAME)
  ENDIF

  CALL SOF_TRACE(IDF,SIZE(IDF),IWRITE,FRACTION,NFTHREAD,MXTHREAD)
