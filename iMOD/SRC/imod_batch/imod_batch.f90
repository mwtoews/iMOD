...
!## Write GEN and RUN file  ! FR 20131016
 ...
 PBMAN%NLINESEARCH=10; PBMAN%NCPU=1; PBMAN%CMDHIDE=1; PBMAN%NLOGLOC=0
 ...
 ELSEIF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  ...
  IF(UTL_READINITFILE('LOGLOC',LINE,IU,1))READ(LINE,*) PBMAN%NLOGLOC
...
! ENDIF
 ...
 IF(UTL_READINITFILE('SOURCEDIR',LINE,IU,1))THEN
  ...
  READ(LINE,'(A)') TARGETDIR
  WRITE(*,'(A)') 'TARGETDIR='//TRIM(TARGETDIR)
 ELSE
  ALLOCATE(XYZFNAMES(2)); XYZFNAMES=''
  IF(UTL_READINITFILE('XYZFILE',LINE,IU,1))THEN
   READ(LINE,'(A)') XYZFNAMES(1); WRITE(*,'(A)') 'XYZFILE='//TRIM(XYZFNAMES(1)); IN_TYPE=1
  ENDIF
  IF(UTL_READINITFILE('IPFFILE',LINE,IU,1))THEN
   READ(LINE,'(A)') XYZFNAMES(1); WRITE(*,'(A)') 'IPFFILE='//TRIM(XYZFNAMES(1)); IN_TYPE=2
  ENDIF
