...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: TOP,BOT,KHV,KVV,KVA,BND,KDV,VCV
 TYPE(IDFOBJ) :: IDF
 ...
 IF(.NOT.UTL_READINITFILE('NLAY',LINE,IU,0))RETURN
 READ(LINE,*) NLAY; LINE='NLAY='//TRIM(ITOS(NLAY)); WRITE(*,'(A)') TRIM(LINE)
 ALLOCATE(TOP(NLAY),BOT(NLAY),KHV(NLAY),KVA(NLAY),KVV(NLAY-1),VCV(NLAY-1),BND(NLAY),KDV(NLAY))
 DO I=1,NLAY; CALL IDFNULLIFY(TOP(I)); CALL IDFNULLIFY(BOT(I)); CALL IDFNULLIFY(KHV(I))
  CALL IDFNULLIFY(KDV(NLAY)); CALL IDFNULLIFY(KVA(I)); CALL IDFNULLIFY(BND(I))
 ENDDO
 DO I=1,NLAY-1; CALL IDFNULLIFY(KVV(I)); CALL IDFNULLIFY(VCV(I)); ENDDO
 ...
 DO I=1,NLAY
  ...
  CALL IDFCOPY(BND(I),TOP(I))
  IF(.NOT.IDFREAD(TOP(I),TOP(I)%FNAME,1))RETURN
  CALL IDFCOPY(BND(I),BOT(I))
  IF(.NOT.IDFREAD(BOT(I),BOT(I)%FNAME,1))RETURN
  CALL IDFCOPY(BND(I),KHV(I))
  IF(.NOT.IDFREAD(KHV(I),KHV(I)%FNAME,1))RETURN
  CALL IDFCOPY(BND(I),KVA(I))
  IF(.NOT.IDFREAD(KVA(I),KVA(I)%FNAME,1))RETURN
  IF(I.LT.NLAY)THEN
   CALL IDFCOPY(BND(I),KVV(I))
   IF(.NOT.IDFREAD(KVV(I),KVV(I)%FNAME,1))RETURN; CALL IDFCOPY(BND(I),VCV(I))
  ENDIF
  CALL IDFCOPY(BND(I),KDV(I)) 
 ENDDO
 ...
 DO ILAY=1,NLAY
  ...
  DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
   ...
   IF(D.GT.0.AND.(KHV(ILAY)%X(ICOL,IROW).LE.0.0.OR.KHV(ILAY)%X(ICOL,IROW).EQ.KHV(ILAY)%NODATA))THEN
    IDF%X(ICOL,IROW)=1.0 
   ENDIF
  ENDDO; ENDDO
  ...
  DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
   IF(IDF%X(ICOL,IROW).EQ.1.0)THEN
    DO IR1=MAX(1,IROW-1),MIN(IROW+1,BND(1)%NROW); DO IC1=MAX(1,ICOL-1),MIN(ICOL+1,BND(1)%NCOL)
     IF(IDF%X(IC1,IR1).EQ.0.0)IDF%X(IC1,IR1)=2.0
    ENDDO; ENDDO
   ENDIF
  ENDDO; ENDDO
  ND=0; DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL; IF(IDF%X(ICOL,IROW).EQ.2.0)ND=ND+1; ENDDO; ENDDO 
  !## number data points
  ALLOCATE(XD(ND),YD(ND),ZD(ND))

  ND=0; DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
   IF(IDF%X(ICOL,IROW).EQ.2.0)THEN
    ND=ND+1; XD(ND)=REAL(ICOL); YD(ND)=REAL(IROW)
    ZD(ND)=KHV(ILAY)%X(ICOL,IROW)
   ENDIF
  ENDDO; ENDDO 
 ...
 DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
  DO ILAY=1,NLAY
   KDV(ILAY)%X(ICOL,IROW)=KDV(ILAY)%NODATA
   IF(ILAY.LT.NLAY)VCV(ILAY)%X(ICOL,IROW)=VCV(ILAY)%NODATA
   IF(BND(ILAY)%X(ICOL,IROW).EQ.0)CYCLE
   D=TOP(ILAY)%X(ICOL,IROW)-BOT(ILAY)%X(ICOL,IROW)
   T=D*KHV(ILAY)%X(ICOL,IROW)
   KDV(ILAY)%X(ICOL,IROW)=T
   IF(ILAY.LT.NLAY)THEN
    C1=0.5*D/(KHV(ILAY)%X(ICOL,IROW)*KVA(ILAY)%X(ICOL,IROW))
    C2=0.0
    D=BOT(ILAY)%X(ICOL,IROW)-TOP(ILAY+1)%X(ICOL,IROW)
    IF(D.GT.0.0)C2=D/KVV(ILAY)%X(ICOL,IROW)
    D=TOP(ILAY+1)%X(ICOL,IROW)-BOT(ILAY+1)%X(ICOL,IROW)
    C3=0.5*D/(KHV(ILAY+1)%X(ICOL,IROW)*KVA(ILAY+1)%X(ICOL,IROW))
 ...
 DO ILAY=1,NLAY
  ...
  IF(.NOT.IDFWRITE(KDV(ILAY),KDV(ILAY)%FNAME,0))RETURN
  IF(ILAY.LT.NLAY)THEN
   VCV(ILAY)%FNAME=TRIM(OUTPUTMAP)//'\VCV_L'//TRIM(ITOS(ILAY))//'.IDF'
   IF(.NOT.IDFWRITE(VCV(ILAY),VCV(ILAY)%FNAME,0))RETURN
  ENDIF
 ENDDO
