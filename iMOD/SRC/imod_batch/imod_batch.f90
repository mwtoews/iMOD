...
USE IMODVAR, ONLY : IBACKSLASH,ILABELNAME,EXENAME,LBETA
...
USE MOD_IDF, ONLY : IDFREAD,IDFNULLIFY,IDFWRITE,IDFDEALLOCATE,IDFREADSCALE,IDFCOPY,IDFGETXYVAL,IDFIROWICOL, &
  IDFGETLOC,IDFGETVAL,IDFEQUAL,IDFCREATEIVF,IDF_EXTENT,IDFALLOCATEX,IDFDEALLOCATEX,IDFFILLSXSY
...
USE MOD_ISG_GRID, ONLY : ISG_SIMPLIFYMAIN,ISG_ADDCROSSSECTION,ISG2GRIDMAIN,ISG_ADDSTAGES,ISG_EXPORT
...
USE MOD_PMANAGER, ONLY : PMANAGERPRJ,PMANAGERRUN,PMANAGERINIT,PMANAGER_SAVEMF2005_HFB_GENFILES
...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=73
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'ADJUSTVOXELIDF','MFTOIMOD','KDC','IMODTOSTOMP','XYZTOVOXEL'/
...
!     CALL CREATEIMODBATCHKILL()
    ...
    CASE (IDHELP)
     CALL IMODGETHELP('3.2.6','EMO.iMODBatch')
    CASE (IDCANCEL)
...
!   CALL IMODBATCH_CREATEVOXEL_MAIN()
 ...
 REAL,ALLOCATABLE,DIMENSION(:) :: XTOP
 REAL,ALLOCATABLE,DIMENSION(:) :: XOFFSET 
...
! READ(LINE,*) XOFFSET; LINE='XOFFSET='//TRIM(RTOS(XOFFSET,'E',7)); WRITE(*,'(A)') TRIM(LINE)
 ...
 ALLOCATE(XTOP(SIZE(LISTNAME)),ITOP(SIZE(LISTNAME))); XTOP=10.0E10; ITOP=0
 ...
 WRITE(JU,'(A15)') 'COUNT'
 WRITE(JU,'(2A15,99F15.7)') 'ipocket','size',(XOFFSET(I)*86400.0,I=1,NXOFFSET)
 ...
 DO I=1,SIZE(LISTNAME)
  ...
  XSUM=0.0; DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
   IF(IDF%X(ICOL,IROW).NE.IDF%NODATA)THEN
    J=1; IF(IDF%X(ICOL,IROW).LT.0.0)J=2
 ...
 REAL :: DZ,MINZ,MAXZ,Z1,Z2
 REAL,ALLOCATABLE,DIMENSION(:) :: ZT,ZB
 ...
 DZ=1.0; IF(UTL_READINITFILE('DZ',LINE,IU,1))READ(LINE,*) DZ
 WRITE(*,'(A,F15.7)') 'DZ=',DZ
 ...
 DO I=1,NZ
  ...
  Z2=Z1-DZ; IDF(4)%X=0.0; IDF(5)%X=0.0
  ...
  DO J=1,SIZE(LISTNAME)
   ...
   IF(ZT(J).LE.Z1.AND.ZB(IORDER(J)).GE.Z2)THEN
    ...
    IF(.NOT.IDFREADSCALE(LISTNAME(IORDER(J)),IDF(3),ISCALE,1,0.0,0))STOP 'Cannot read data for IDF(3)'

    DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
     IF(IDF(3)%X(ICOL,IROW).NE.IDF(3)%NODATA)THEN
      IDF(4)%X(ICOL,IROW)=IDF(4)%X(ICOL,IROW)+1.0
  ...
  DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
   IF(Z1.LE.IDF(1)%X(ICOL,IROW).AND.Z2.GT.IDF(2)%X(ICOL,IROW))THEN
    IF(IDF(4)%X(ICOL,IROW).GT.0.0)THEN
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 IMODE=0; PBMAN%IFORMAT=0; PBMAN%TIMFNAME=''; PBMAN%ISOLVE=0; PBMAN%SSYSTEM=0
 PBMAN%NSTEP=1; PBMAN%NMULT=1.0; PBMAN%ISTEADY=0; PBMAN%MINKD=0.0; PBMAN%MINC=0.0
 PBMAN%ICONCHK=0; PBMAN%UNCONFINED=0; PBMAN%IPEST=0; PBMAN%IWINDOW=0; PBMAN%ISAVEENDDATE=0
 PBMAN%ISS=0; PBMAN%IFVDL=0; PBMAN%BNDFILE=''; PBMAN%IPKS=0
 
 IF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  IMODE=1
  READ(LINE,*) PRJFILE; WRITE(*,'(A12,A)')  'PRJFILE_IN=',TRIM(PRJFILE)

  IF(UTL_READINITFILE('RUNFILE_OUT',LINE,IU,1))THEN
   READ(LINE,*) RUNFILE; WRITE(*,'(A12,A)') 'RUNFILE_OUT=',TRIM(RUNFILE)
   PBMAN%IFORMAT=1
  ENDIF
  IF(UTL_READINITFILE('NAMFILE_OUT',LINE,IU,1))THEN
   READ(LINE,*) RUNFILE; WRITE(*,'(A12,A)') 'NAMFILE_OUT=',TRIM(RUNFILE)
   PBMAN%IFORMAT=2
  ENDIF
  
  !## more specific options to create a runfile
  IF(UTL_READINITFILE('ISS',LINE,IU,1))READ(LINE,*) PBMAN%ISS
  WRITE(*,'(A12,I1)') 'ISS=',PBMAN%ISS
  !## transient                              
  IF(PBMAN%ISS.EQ.1)THEN
   !## tim-file?
   IF(UTL_READINITFILE('TIMFNAME',LINE,IU,1))THEN
    READ(LINE,*) PBMAN%TIMFNAME; WRITE(*,'(A12,A)') 'TIMFNAME=',TRIM(PBMAN%TIMFNAME)
   ELSE
    !## sdate
    IF(.NOT.UTL_READINITFILE('SDATE',LINE,IU,0))RETURN
    READ(LINE,*) PBMAN%SDATE; WRITE(*,'(A12,I14)') 'SDATE=',PBMAN%SDATE
    !## edate
    IF(.NOT.UTL_READINITFILE('EDATE',LINE,IU,0))RETURN
    READ(LINE,*) PBMAN%EDATE; WRITE(*,'(A12,I14)') 'EDATE=',PBMAN%EDATE
    !## type of interval
  ...
  WRITE(*,'(A12,I1)') 'SSYSTEM=',PBMAN%SSYSTEM
  ...
  WRITE(*,'(A12,I10)') 'ICONCHK=',PBMAN%ICONCHK

  !## specify unconfined/parameter estimation
  IF(PBMAN%IFORMAT.EQ.2)THEN
   IF(UTL_READINITFILE('UNCONFINED',LINE,IU,1))READ(LINE,*) PBMAN%UNCONFINED
   WRITE(*,'(A12,I10)') 'UNCONFINED=',PBMAN%UNCONFINED
   IF(UTL_READINITFILE('ISAVEENDDATE',LINE,IU,1))READ(LINE,*) PBMAN%ISAVEENDDATE
   WRITE(*,'(A12,I1)') 'ISAVEENDDATE=',PBMAN%ISAVEENDDATE
   IF(UTL_READINITFILE('IPKS',LINE,IU,1))READ(LINE,*) PBMAN%IPKS
   WRITE(*,'(A12,I1)') 'IPKS=',PBMAN%IPKS
!   IF(UTL_READINITFILE('MODELIDF',LINE,IU,1))READ(LINE,*) PBMAN%SUBMDL%FNAME
!   WRITE(*,'(A12,A)') 'MODELIDF=',TRIM(PBMAN%SUBMDL%FNAME)
  ENDIF
  IF(UTL_READINITFILE('IPEST',LINE,IU,1))READ(LINE,*) PBMAN%IPEST
  WRITE(*,'(A12,I10)') 'IPEST=',PBMAN%IPEST
  
  IF(UTL_READINITFILE('MINKD',LINE,IU,1))READ(LINE,*) PBMAN%MINKD
  WRITE(*,'(A12,G10.4)') 'MINKD=',PBMAN%MINKD
  IF(UTL_READINITFILE('MINC',LINE,IU,1))READ(LINE,*) PBMAN%MINC
  WRITE(*,'(A12,G10.4)') 'MINC=',PBMAN%MINC
  IF(UTL_READINITFILE('IFVDL',LINE,IU,1))READ(LINE,*) PBMAN%IFVDL
  WRITE(*,'(A12,I1)') 'IFVDL=',PBMAN%IFVDL

  IF(UTL_READINITFILE('NETWORKIDF',LINE,IU,1))THEN
   PBMAN%IWINDOW=2
   READ(LINE,*) PBMAN%BNDFILE
   WRITE(*,'(A12,A)') 'NETWORKIDF='//TRIM(PBMAN%BNDFILE)
  ELSE
   IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
    PBMAN%IWINDOW=1
    READ(LINE,*) PBMAN%XMIN,PBMAN%YMIN,PBMAN%XMAX,PBMAN%YMAX
    WRITE(*,'(A12,4G15.7)') 'WINDOW=',PBMAN%XMIN,PBMAN%YMIN,PBMAN%XMAX,PBMAN%YMAX
    IF(.NOT.UTL_READINITFILE('CELLSIZE',LINE,IU,0))RETURN
    READ(LINE,*) PBMAN%CELLSIZE; WRITE(*,'(A12,G15.7)') 'CELLSIZE=',PBMAN%CELLSIZE
    PBMAN%BUFFER=0.0; IF(UTL_READINITFILE('BUFFER',LINE,IU,1))READ(LINE,*) PBMAN%BUFFER
    WRITE(*,'(A12,G15.7)') 'BUFFER=',PBMAN%BUFFER
    IF(PBMAN%BUFFER.GT.0.0)THEN
     PBMAN%BUFFERCS=0.0; IF(UTL_READINITFILE('BUFFERCS',LINE,IU,1))READ(LINE,*) PBMAN%BUFFERCS
     WRITE(*,'(A12,G15.7)') 'BUFFERCS=',PBMAN%BUFFERCS
    ENDIF
   ELSE
    IF(UTL_READINITFILE('CELLSIZE',LINE,IU,1))THEN
     WRITE(*,'(/A/)') 'The usage of CELLSIZE is only valid in combination with the keyword WINDOW'; RETURN
    ENDIF
    IF(UTL_READINITFILE('BUFFER',LINE,IU,1))THEN
     WRITE(*,'(/A/)') 'The usage of BUFFER is only valid in combination with the keyword WINDOW'; RETURN
    ENDIF
   ENDIF
  ENDIF
 ...
 IF(UTL_READINITFILE('RUNFILE_IN',LINE,IU,1))THEN
  IMODE=2
  READ(LINE,*) RUNFILE; WRITE(*,'(A12,A)') 'RUNFILE_IN=',TRIM(RUNFILE)
  IF(.NOT.UTL_READINITFILE('PRJFILE_OUT',LINE,IU,0))RETURN
  READ(LINE,*) PRJFILE; WRITE(*,'(A12,A)') 'PRJFILE_OUT=',TRIM(PRJFILE)
 ENDIF

 IF(IMODE.EQ.0)THEN; WRITE(*,'(/A/)') 'Stop not appropriate conversion mode found'; STOP; ENDIF
 ...
 INTEGER :: I,ID,IOS
 REAL :: X1,Y1,X2,Y2,XN1,XN2,YN1,YN2,Z1,Z2,DIPANGLE,DZ,DX,DY,TNG,DIPDIRECTION
 INTEGER,DIMENSION(2) :: JU
 ...
 MAXDIST=0.0; ICLEAN=1; CROSS_PNTR=''; CROSS_BATH=''; WIDTHFNAME=''
 ...
 INTEGER :: I,J,IFLOW,IPNTR,IWINDOW,IWRITE,IGRAD,ITQP,NTQP,TTQP,IFORMAT,PITTSIZE,MINFRICTION,NQDW
 REAL,DIMENSION(:),ALLOCATABLE :: PTQP
 REAL,DIMENSION(:,:),POINTER :: QDW
 CHARACTER(LEN=256) :: RESULTIDF,OUTPUTFOLDER,FNAME
 REAL :: XMIN,YMIN,XMAX,YMAX,CELLSIZE,MINQ,RAIN
 ...
 IF(IFLOW.EQ.0)THEN
  ...
  !## size of the pitt to become outlet
  PITTSIZE=0; IF(UTL_READINITFILE('PITTSIZE',LINE,IU,1))THEN
   READ(LINE,*) PITTSIZE; WRITE(*,'(A,I4)') 'PITTSIZE=',PITTSIZE
  ENDIF
  
  !## compute sof-area with spill levels
  CALL SOF_MAIN(IDF,IPNTR,IWINDOW,XMIN,YMIN,XMAX,YMAX,CELLSIZE,IGRAD,PITTSIZE) 
 ...
 ELSEIF(IFLOW.EQ.3)THEN
  ...
  RAIN=1.0; IF(UTL_READINITFILE('RAIN',LINE,IU,1))READ(LINE,*) RAIN
 ...
 DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
  ...
  CALL IDFEDITTRACE(IDF(1),IDF(2),THREAD,YSEL,ISPEC,DTERM,IMENU,MAXTHREAD,MAXN,0.0,NTHREAD,IPZ)
...
! WRITE(*,'(A,I2)') 'ITALUSCOL=',DATCOL(9)
 ...
 CALL ISGGEN_IPFTOISG(IPFFILE,ISGFILE,DATCOL,SDATE)
 ...
 IDF(3)%XMIN=0.0; IDF(3)%YMIN=0.0; IDF(3)%XMAX=0.0; IDF(3)%YMAX=0.0
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
 ...
 N=0; DO IROW=1,IDF(3)%NROW; DO ICOL=1,IDF(3)%NCOL
  ...
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.IDF(3)%NCOL)THEN
    ...
    ENDIF
    IF(LEX)THEN
     N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF(3),IDF(3)%NROW,IDF(3)%NCOL,IROW,ICOL,N, &
          0.0,0.0,0.0,0,1,.TRUE.,T1,B1) 
    ENDIF
   ENDIF
  ENDIF
...
! 9014 FORMAT (62X, 2(1PE14.7, 1X))
 ...
 IMPLICIT NONE
 INTEGER :: I,N,IKD,ICW,SDATE,EDATE,ISS,IMIDF,ICLAY,IQCOL
 REAL :: HNODATA,FNODATA,MINKHT,MINKD
 
 STNLAY=-1; IMIDF=0; IKD=0; ICW=0; MINKHT=0.0; IMIDF=0; SDATE=0; EDATE=0; ISS=0; ICLAY=1
 HNODATA=0.0; FNODATA=-99999.0; MINKD=0.0
 ...
 DO I=1,N
  ...
  IF(.NOT.ST1CREATEIPF_STEADY(IKD,ICW,MINKHT,IMIDF,SDATE,EDATE,ISS,ICLAY,HNODATA,FNODATA,MINKD))WRITE(*,'(A)') 'Error IPF '//TRIM(IPF(1)%FNAME)//' failed'
 ENDDO
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IWINDOW,NLAY
 TYPE(IDFOBJ) :: IDF
 
 IWINDOW=0; IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  IWINDOW=1
 ...
 IF(STOMP_READ(IU,IDF,IWINDOW))THEN
  IF(STOMP_SAVEINPUT())THEN; ENDIF
 ENDIF
 ...
 REAL :: D,T,B,C1,C2,C3,K,MINTHICKNESS
 ...
 DO I=1,NLAY
  ...
  ENDIF
  CALL IDFCOPY(BND(I),TOP(I)); IF(.NOT.IDFREADSCALE(TOP(I)%FNAME,TOP(I),2,1,0.0,0))RETURN
  CALL IDFCOPY(BND(I),BOT(I)); IF(.NOT.IDFREADSCALE(BOT(I)%FNAME,BOT(I),2,1,0.0,0))RETURN
  CALL IDFCOPY(BND(I),KHV(I)); IF(.NOT.IDFREADSCALE(KHV(I)%FNAME,KHV(I),3,1,0.0,0))RETURN
  CALL IDFCOPY(BND(I),KVA(I)); IF(.NOT.IDFREADSCALE(KVA(I)%FNAME,KVA(I),2,1,0.0,0))RETURN
  CALL IDFCOPY(BND(I),KDV(I)); CALL IDFCOPY(BND(I),ERH(I))
  IF(I.LT.NLAY)THEN
   CALL IDFCOPY(BND(I),KVV(I)); IF(.NOT.IDFREADSCALE(KVV(I)%FNAME,KVV(I),3,1,0.0,0))RETURN
 ...
 DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
  DO ILAY=1,NLAY
   ...
   IF(D.GT.0.0)THEN
    IF(KHV(ILAY)%X(ICOL,IROW).EQ.KHV(ILAY)%NODATA)KHV(ILAY)%X(ICOL,IROW)=1.0
    IF(KVA(ILAY)%X(ICOL,IROW).EQ.KVA(ILAY)%NODATA)KVA(ILAY)%X(ICOL,IROW)=1.0
    IF(ILAY.LT.NLAY)THEN
     IF(KVV(ILAY)%X(ICOL,IROW).EQ.KVV(ILAY)%NODATA)KVV(ILAY)%X(ICOL,IROW)=1.0
    ENDIF
   ENDIF
  ENDDO
 ENDDO; ENDDO
...
!   endif
 ...
 KDV%NODATA=HUGE(1.0); VCV%NODATA=HUGE(1.0)
 ERH%NODATA=HUGE(1.0); ERV%NODATA=HUGE(1.0)
 ...
 DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
  DO ILAY=1,NLAY
   ...
   IF(ILAY.LT.NLAY)THEN
    ...
    D=TOP(ILAY)%X(ICOL,IROW)-BOT(ILAY)%X(ICOL,IROW); C1=0.0
    IF(D.GT.0.0)THEN
     IF(KHV(ILAY)%X(ICOL,IROW).LE.0.0.OR.KHV(ILAY)%X(ICOL,IROW).EQ.KHV(ILAY)%NODATA)ERH(ILAY)%X(ICOL,IROW)=1.0
     IF(KVA(ILAY)%X(ICOL,IROW).LE.0.0.OR.KVA(ILAY)%X(ICOL,IROW).EQ.KVA(ILAY)%NODATA)ERH(ILAY)%X(ICOL,IROW)=1.0
    ...
    ELSEIF(D.LT.0.0)THEN
     IF(ERH(ILAY)%X(ICOL,IROW).EQ.ERH(ILAY)%NODATA)ERH(ILAY)%X(ICOL,IROW)=1.0
     ERH(ILAY)%X(ICOL,IROW)=-1.0*ABS(ERH(ILAY)%X(ICOL,IROW))
    ENDIF

    IF(BND(ILAY+1)%X(ICOL,IROW).EQ.0)CYCLE

    D=BOT(ILAY)%X(ICOL,IROW)-TOP(ILAY+1)%X(ICOL,IROW); C2=0.0
    IF(D.GT.0.0)THEN
     IF(KVV(ILAY)%X(ICOL,IROW).LE.0.0.OR.KVV(ILAY)%X(ICOL,IROW).EQ.KVV(ILAY)%NODATA)ERV(ILAY)%X(ICOL,IROW)=1.0
     K =KVV(ILAY)%X(ICOL,IROW)
     IF(ERV(ILAY)%X(ICOL,IROW).EQ.ERV(ILAY)%NODATA.AND.K.GE.1.0)ERV(ILAY)%X(ICOL,IROW)=2.0
     C2=D/K
    ELSEIF(D.LT.0.0)THEN
     IF(ERV(ILAY)%X(ICOL,IROW).EQ.ERH(ILAY)%NODATA)ERV(ILAY)%X(ICOL,IROW)=1.0
     ERV(ILAY)%X(ICOL,IROW)=-1.0*ABS(ERV(ILAY)%X(ICOL,IROW))
    ENDIF

    D=TOP(ILAY+1)%X(ICOL,IROW)-BOT(ILAY+1)%X(ICOL,IROW); C3=0.0
    IF(D.GT.0.0)THEN
     IF(KHV(ILAY+1)%X(ICOL,IROW).LE.0.0.OR.KHV(ILAY+1)%X(ICOL,IROW).EQ.KHV(ILAY+1)%NODATA)ERH(ILAY+1)%X(ICOL,IROW)=1.0
     IF(KVA(ILAY+1)%X(ICOL,IROW).LE.0.0.OR.KVA(ILAY+1)%X(ICOL,IROW).EQ.KVA(ILAY+1)%NODATA)ERH(ILAY+1)%X(ICOL,IROW)=1.0
     ...
     C3=0.5*D/K
    ELSEIF(D.LT.0.0)THEN
     IF(ERH(ILAY+1)%X(ICOL,IROW).EQ.ERH(ILAY)%NODATA)ERH(ILAY+1)%X(ICOL,IROW)=1.0
     ERH(ILAY+1)%X(ICOL,IROW)=-1.0*ABS(ERH(ILAY+1)%X(ICOL,IROW))
    ENDIF
 ...
 DO IROW=1,BND%NROW; DO ICOL=1,BND%NCOL
  ...
  IF(D.GT.0.AND.(KV%X(ICOL,IROW).LE.0.0.OR.KV%X(ICOL,IROW).EQ.KV%NODATA))THEN
   IDF%X(ICOL,IROW)=1.0 
  ENDIF
 ENDDO; ENDDO
...
! ENDIF
 ...
 INTEGER :: IWINDOW
 REAL :: CELLSIZE,X1,Y1,X2,Y2,DZV,MULT
 TYPE(IDFOBJ) :: TOP,BOT
 ...
 ELSE
  CELLSIZE=0.0; IF(UTL_READINITFILE('CELLSIZE',LINE,IU,1))THEN
   READ(LINE,*) CELLSIZE; WRITE(*,'(A,F10.2)') 'CELLSIZE=',CELLSIZE
  ENDIF
 ENDIF
 DZV=0.0; IF(UTL_READINITFILE('DZV',LINE,IU,1))THEN
 ...
 !###======================================================================
 USE MOD_ASC2IDF_PAR, ONLY : IDFFILE,SOURCEDIR,TARGETDIR,IGRIDFUNC,CS,NODATA,LAGDISTANCE, &
   PERCENTILE,MINP,XYZFNAMES,RANGE,SILL,NUGGET,LAGINTERVAL,KTYPE,IXCOL,IYCOL,IZCOL,STDEVIDF,PNTSEARCH, &
   ASSF_COLUMN,ASSF_STARTDATE,ASSF_ENDDATE,ASSF_DDATE,ASSF_CDDATE,ILOG,TRIMDEPTH_IDF,ASSF_NTHRESHOLD, &
   GENFILE,BLNFILE,ASSF_TOP,ASSF_BOT,ASSF_IDEPTH,ASSF_DZ,ASSF_INDICATOR,ASSF_THRESHOLD,ASSF_K_THRESHOLD, &
   IQUADRANT,IINT_IDF,INT_IDF,ELLIPS_IDF,IBLNTYPE
 USE MOD_SOLID, ONLY : MXITER2,HCLOSE,RCLOSE
 USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_JDATETOIDATE
 IMPLICIT NONE
 INTEGER :: I,J,K,N,IN_TYPE,JU,M,IDATE,SDATE,EDATE,DDATE,IOS,ICOL,IROW,JF,ID,KSUM,ILAY,NLAY
 CHARACTER(LEN=52) :: GRIDFUNC,WC,ROOT
 CHARACTER(LEN=256) :: FNAME,SNAME
 REAL :: XMIN,YMIN,XMAX,YMAX,Z1,Z2,MF,KH,VC
 ...
 ASSF_IDEPTH=0; ASSF_TOP=0.0; ASSF_BOT=0.0; ASSF_DZ=0.0; ASSF_NTHRESHOLD=1; ASSF_INDICATOR=0; ELLIPS_IDF%FNAME=''; ASSF_ZPLUS=0.0
 IF(IN_TYPE.EQ.2)THEN
  ...
  IF(M.GT.0.AND.M.EQ.IZCOL)THEN
   ...
   IF(ASSF_IDEPTH.EQ.0)THEN
    ...
    ALLOCATE(ASSF_K_THRESHOLD(1)); ASSF_K_THRESHOLD=0.0
   ...
   ELSE
    ...
    IF(ASSF_IDEPTH.EQ.1)THEN
     ...
     IF(.NOT.UTL_READPOINTER_REAL(IU,I,DZ,'ASSF_DZ',0,EXCLVALUE=0.0))RETURN
     DO I=1,SIZE(DZ)
      IF(DZ(I).LE.0.0)THEN
       LINE='DZ('//TRIM(ITOS(I))//')='//TRIM(RTOS(DZ(I),'F',3))
    ...
    ELSEIF(ASSF_IDEPTH.EQ.2)THEN
     ...
     IF(.NOT.UTL_READINITFILE('NLAY',LINE,IU,0))RETURN
     READ(LINE,*) NLAY; WRITE(*,'(A,I3)') 'NLAY=',NLAY
     ALLOCATE(INT_IDF(NLAY))
     DO ILAY=1,NLAY
    ...
    IF(UTL_READINITFILE('INDICATOR',LINE,IU,1))THEN
     ...
     IF(ALLOCATED(ASSF_K_THRESHOLD))DEALLOCATE(ASSF_K_THRESHOLD)
     ALLOCATE(ASSF_THRESHOLD(ASSF_NTHRESHOLD)); ASSF_THRESHOLD=''
     ALLOCATE(ASSF_K_THRESHOLD(ASSF_NTHRESHOLD)); ASSF_K_THRESHOLD=0.0
     ...
     DO I=1,ASSF_NTHRESHOLD
      ...
      IF(.NOT.UTL_READINITFILE('K_THRESHOLD'//TRIM(ITOS(I)),LINE,IU,0))RETURN
      READ(LINE,*) ASSF_K_THRESHOLD(I); WRITE(*,*) 'K_THRESHOLD'//TRIM(ITOS(I))//'=',ASSF_K_THRESHOLD(I)
     ENDDO
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  CASE (-6,6)
   ...
   IF(PNTSEARCH.EQ.1)THEN
    IF(UTL_READINITFILE('MINP',LINE,IU,1))READ(LINE,*) MINP
 ...
 IF(UTL_READINITFILE('BLNFILE',LINE,IU,1))THEN
  READ(LINE,*) BLNFILE; WRITE(*,'(A)') 'BLNFILE='//TRIM(BLNFILE)
 ENDIF
 IF(UTL_READINITFILE('IBLNTYPE',LINE,IU,1))READ(LINE,*) IBLNTYPE
 WRITE(*,'(A)') 'IBLNTYPE='//TRIM(ITOS(IBLNTYPE))
 
 DDATE=0
 FNAME=IDFFILE(:INDEX(IDFFILE,'.',.TRUE.)-1)
 ...
 IF(N.EQ.0)THEN
   ...
   DO I=1,ASSF_NTHRESHOLD
    ...
    DO   
     ...
     IF(ASSF_COLUMN.NE.0)THEN
      IF(ASSF_IDEPTH.EQ.0)THEN
       IDFFILE =TRIM(FNAME)//'_'//TRIM(ITOS(UTL_JDATETOIDATE(IDATE)))//'.IDF'
       STDEVIDF=TRIM(SNAME)//'_'//TRIM(ITOS(UTL_JDATETOIDATE(IDATE)))//'.IDF'
      ELSEIF(ASSF_IDEPTH.EQ.1)THEN
       IF(ASSF_INDICATOR.GT.0)THEN
        IDFFILE =TRIM(FNAME)//'_'//TRIM(ASSF_THRESHOLD(I))//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'.IDF'
        STDEVIDF=TRIM(SNAME)//'_'//TRIM(ASSF_THRESHOLD(I))//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'.IDF'
       ELSE
        IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'.IDF'
        STDEVIDF=TRIM(SNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'.IDF'
       ENDIF
      ELSEIF(ASSF_IDEPTH.EQ.2)THEN
       IF(ASSF_INDICATOR.GT.0)THEN
        IDFFILE =TRIM(FNAME)//'_'//TRIM(ASSF_THRESHOLD(I))//'_INT'//TRIM(ITOS(IINT_IDF))//'.IDF'
        STDEVIDF=TRIM(SNAME)//'_'//TRIM(ASSF_THRESHOLD(I))//'_INT'//TRIM(ITOS(IINT_IDF))//'.IDF'
       ELSE
        IDFFILE =TRIM(FNAME)//'_INT'//TRIM(ITOS(IINT_IDF))//'.IDF'
        STDEVIDF=TRIM(SNAME)//'_INT'//TRIM(ITOS(IINT_IDF))//'.IDF'
       ENDIF
      ENDIF
     ENDIF

     IF(.NOT.ASC2IDF_INT_MAIN(1,XMIN,YMIN,XMAX,YMAX))THEN; ENDIF
  ...
  IF(ABS(ASSF_INDICATOR).GT.0.AND.ASSF_NTHRESHOLD.GT.1)THEN
   ...
   DO
     ...
     DO I=1,ASSF_NTHRESHOLD
      IF(VOXEL(I)%X(ICOL,IROW).NE.VOXEL(I)%NODATA)THEN
       ...
       IF(KSUM.EQ.1)THEN
        ...
        KH= VOXEL(I)%X(ICOL,IROW)*ASSF_K_THRESHOLD(I)
        VOXEL(ASSF_NTHRESHOLD+2)%X(ICOL,IROW)=VOXEL(ASSF_NTHRESHOLD+2)%X(ICOL,IROW)+KH
        !## vertical resistance per meter
        VC=VOXEL(I)%X(ICOL,IROW)/ASSF_K_THRESHOLD(I)
        VOXEL(ASSF_NTHRESHOLD+3)%X(ICOL,IROW)=VOXEL(ASSF_NTHRESHOLD+3)%X(ICOL,IROW)+VC
       ENDIF
      ENDIF
     ENDDO
     IF(JF.GT.0)THEN
      ...
      IF(KSUM.EQ.0)THEN
       VOXEL(ASSF_NTHRESHOLD+2)%X(ICOL,IROW)=ASSF_K_THRESHOLD(JF)
       VOXEL(ASSF_NTHRESHOLD+3)%X(ICOL,IROW)=ASSF_K_THRESHOLD(JF)
      ENDIF
      !## vertical anisotropy
    ...
    ELSEIF(ASSF_IDEPTH.EQ.2)THEN
     DO I=1,4; VOXEL(ASSF_NTHRESHOLD+I)%ITB=INT(0,1); VOXEL(ASSF_NTHRESHOLD+I)%TOP=0.0; VOXEL(ASSF_NTHRESHOLD+I)%BOT=0.0; ENDDO
    ENDIF
   
    !## main lithology
    IF(ASSF_IDEPTH.EQ.1)IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_L.IDF'
    IF(ASSF_IDEPTH.EQ.2)IDFFILE =TRIM(FNAME)//'_INT'//TRIM(ITOS(IINT_IDF))//'_L.IDF'
    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+1),IDFFILE,1))STOP
    
    !## permeability
    IF(ASSF_IDEPTH.EQ.1)IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_K.IDF'
    IF(ASSF_IDEPTH.EQ.2)IDFFILE =TRIM(FNAME)//'_INT'//TRIM(ITOS(IINT_IDF))//'_K.IDF'
    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+2),IDFFILE,1))STOP

    !## vertical anisotropy
    IF(ASSF_IDEPTH.EQ.1)IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_A.IDF'
    IF(ASSF_IDEPTH.EQ.2)IDFFILE =TRIM(FNAME)//'_INT'//TRIM(ITOS(IINT_IDF))//'_A.IDF'
    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+3),IDFFILE,1))STOP

    !## sum of fractions
    IF(ASSF_IDEPTH.EQ.1)IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_F.IDF'
    IF(ASSF_IDEPTH.EQ.2)IDFFILE =TRIM(FNAME)//'_INT'//TRIM(ITOS(IINT_IDF))//'_F.IDF'
 ...
 ELSE
  ...
  DO 
   ...
   DO I=1,N
    ...
    IF(.NOT.ASC2IDF_INT_MAIN(I,XMIN,YMIN,XMAX,YMAX))THEN
    ENDIF
   ENDDO
 ...
 USE MOD_TSTAT_PAR, ONLY : IPFFILE,OUTNAME,IHCOL,IMCOL,IWCOL,ILCOL,W_TYPE, &
    POINTERIDF,NZONE,IZONE,HNODATA,ICOLLECT,SOBSDATE,EOBSDATE
 IMPLICIT NONE
 INTEGER :: I,NIPF
 ...
 GRIDISG%MINDEPTH=0.10     !'mindepth          : minimal waterdepth for computing conductances (m)'
 GRIDISG%WDEPTH=1.0        ! waterdepth        : for simgro trapezia
 ...
 GRIDISG%IDIM=2; GRIDISG%XMIN=0.0; GRIDISG%YMIN=0.0; GRIDISG%XMAX=0.0; GRIDISG%YMAX=0.0
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  READ(LINE,*) GRIDISG%XMIN,GRIDISG%YMIN,GRIDISG%XMAX,GRIDISG%YMAX
  WRITE(*,'(A,4F10.2)') 'WINDOW=',GRIDISG%XMIN,GRIDISG%YMIN,GRIDISG%XMAX,GRIDISG%YMAX
  MPW%XMIN=GRIDISG%XMIN; MPW%YMIN=GRIDISG%YMIN; MPW%XMAX=GRIDISG%XMAX; MPW%YMAX=GRIDISG%YMAX
  GRIDISG%IDIM=1
 ENDIF
 IF(.NOT.UTL_READINITFILE('CELL_SIZE',LINE,IU,0))RETURN
 READ(LINE,*) GRIDISG%CS; WRITE(*,'(A,F10.2)') 'CELL_SIZE=',GRIDISG%CS
 IF(.NOT.UTL_READINITFILE('NODATA',LINE,IU,0))RETURN
 READ(LINE,*) GRIDISG%NODATA; WRITE(*,'(A,F10.2)') 'NODATA=',GRIDISG%NODATA
 ...
 IF(GRIDISG%ISIMGRO.EQ.1)THEN
  ...
  WRITE(*,'(A,F10.2)') 'WDEPTH=',GRIDISG%WDEPTH
 ...
 WRITE(*,'(A,F10.2)') 'MAXWIDTH=',GRIDISG%MAXWIDTH
 ...
 IF(GRIDISG%IEXPORT.EQ.1)THEN
  ...
  IF(NLAY.GT.0)THEN
   ...
   DO I=1,NLAY
    CALL IDFNULLIFY(BOT(I)); CALL IDFNULLIFY(TOP(I)); CALL IDFNULLIFY(KHV(I))
    IF(.NOT.UTL_READINITFILE('TOP_L'//TRIM(ITOS(I)),LINE,IU,0))RETURN
 ...
 ELSE
  ...
  DO I=1,NLAY; CALL IDFNULLIFY(BOT(I)); CALL IDFNULLIFY(TOP(I)); ENDDO
 ENDIF
 
 IF(.NOT.UTL_READINITFILE('ISGFILE_IN',LINE,IU,0))RETURN
 READ(LINE,*) ISGFILE; WRITE(*,'(A)') 'ISGFILE_IN='//TRIM(ISGFILE)

 IF(.NOT.ISG2GRIDMAIN(ISGFILE,1,NLAY,TOP,BOT,KHV,BND,GRIDISG))THEN; WRITE(*,'(/A/)') 'Error occured in ISG Grid routine'; ENDIF
 CALL ISGDEAL(1) 
 DO I=1,NLAY
...
!   !## extern flow components
!   QEXT= -R(2)+R(4)+R(5)-R(6)
...
! LAYER ROW COL. STREAM  RCH.  FLOW INTO     FLOW TO   FLOW OUT OF   OVRLND.     DIRECT     STREAM       STREAM     STREAM     STREAM   STREAMBED  STREAMBED
!                SEG.NO.  NO.  STRM. RCH.    AQUIFER    STRM. RCH.    RUNOFF     PRECIP       ET          HEAD       DEPTH      WIDTH   CONDCTNC.   GRADIENT
!   1    1    1     1     1    6.3357E+04  7.9484E+03   5.5409E+04  0.000E+00  0.000E+00  0.000E+00  3.37445E+02  3.587E-01  3.931E+00  5.850E+03  1.359E+00
   ...
   DO J=1,ISG(I)%NCLC
    ...
    DATISD(IREC)%INFF =QSTR/86400.0 !## m3/day - m3/sec
   ENDDO
  ENDDO
 ENDDO
 CLOSE(JU)
 ...
 INTEGER :: IPOSTP,IRUN,ICONVERTGEN,IEXPORT
 INTEGER,DIMENSION(4) :: IPFICOL
 CHARACTER(LEN=256) :: TOPFNAME,BOTFNAME
 LOGICAL :: LEX
   
 TOPFNAME=''; BOTFNAME=''; IEXPORT=4; IPFFNAME=''; IFFFLOW=''; IPFFLOW=''
 ...
 IF(IPOSTP.EQ.1)THEN
  ...
  ELSE
   ...
   IF(IEXPORT.LT.1.OR.IEXPORT.GT.4)THEN
    WRITE(*,'(A)') 'IEXPORT needs to be 1,2,3 or 4'; STOP
   ENDIF
  
  ENDIF
 ENDIF 
 
 LEX=.TRUE.; IF(IRUN.EQ.1)LEX=TRACEMAIN(RUNFILE,1,ICONVERTGEN)
 IF(LEX.AND.IPOSTP.EQ.1)CALL TRACEPOSTPROCESSING(IFFFLOW,IPFFLOW,IDFFLOW,IPFFNAME,IPFICOL,ICONVERTGEN,TOPFNAME,BOTFNAME,IEXPORT) 
 ...
 REAL :: X,MINTHICK,TD,CD,MT,TT1,TT2
 REAL,ALLOCATABLE,DIMENSION(:) :: THICK1,THICK2

 ICLEAN=0; MINTHICK=0.0; MINTHICK_OPT=0
 ...
 DO J=1,SIZE(IDF)
  ...
  ELSE
   IF(.NOT.IDFREADSCALE(IDF(J)%FNAME,IDF(J),2,1,0.0,0))RETURN
  ENDIF
 ENDDO
 ...
 DO IROW=1,MOTHER%NROW; DO ICOL=1,MOTHER%NCOL
  ...
  DO
   DO IL=IL1,NLAY-1
    !## found aquitard
    IF(THICK2(IL).GT.0.0)EXIT
   ENDDO
   IL2=IL
   TD=SUM(THICK1(IL1:IL2))
   N=((IL2-IL1)+1)
   MT=MIN(MINTHICK,TD/REAL(N))
   !## correct also layers with tickness == 0.0
   IF(MINTHICK_OPT.EQ.1.AND.MT.EQ.0.0)MT=MINTHICK
   CD=0.0 !## total correction term
   DO IL=IL1,IL2
 ...
 !LTB=.TRUE.
 !
 !!## number of layers in the model to be corrected
 !IF(.NOT.UTL_READINITFILE('NLAY',LINE,IU,0))RETURN
 !READ(LINE,*) NLAY; WRITE(*,'(A,I10)') 'NLAY=',NLAY
 !!## result folder
 !IF(.NOT.UTL_READINITFILE('OUTPUTFOLDER',LINE,IU,0))RETURN
 ...
 ! J=J+1
 ! IF(.NOT.UTL_READINITFILE('TOP_L'//TRIM(ITOS(I)),LINE,IU,0))RETURN
 ! READ(LINE,*) TOP(J)%FNAME
 ! LINE='TOP_L'//TRIM(ITOS(I))//'='//TRIM(TOP(J)%FNAME); WRITE(*,'(A)') TRIM(LINE)
 ! J=J+1
 ! IF(.NOT.UTL_READINITFILE('BOT_L'//TRIM(ITOS(I)),LINE,IU,0))RETURN
 ! READ(LINE,*) BOT(J)%FNAME; LINE='BOT_L'//TRIM(ITOS(I))//'='//TRIM(BOT(J)%FNAME); WRITE(*,'(A)') TRIM(LINE)
 !ENDDO
 !
 !!MOTHER%XMIN=0.0; MOTHER%XMAX=0.0; MOTHER%YMIN=0.0; MOTHER%YMAX=0.0
 !!IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
 ...
 !!  DO J=I-1,1,-1
 !!   IF(IDF(J)%X(ICOL,IROW).NE.IDF(J)%NODATA)THEN
 !!    IDF(I)%X(ICOL,IROW)=MIN(IDF(I)%X(ICOL,IROW),IDF(J)%X(ICOL,IROW))
 !!    EXIT
 !!   ENDIF
 !!  ENDDO
 !! ENDDO
 ...
 !ENDDO
 !
 !END SUBROUTINE IMODBATCH_IDFMINTHICKNESS
 ...
 FDISTANCES=''; ICPOINTERS=1; NCLIP=0; IEXPZONE=0; MIN_THICKNESS=0.0
 ...
 INTEGER,DIMENSION(:),ALLOCATABLE :: ILAYFILE
 CHARACTER(LEN=256) :: OUTPUTFILE,LAYFILE,FNAME
 CHARACTER(LEN=5) :: EXT
 CHARACTER(LEN=52) :: CID
 INTEGER :: I,J,K,KK,KKK,N,M,NN,JU,KU,IEXT,IDATE,JDATE,JD1,JD2,IY,IM,ID,IY1,IY2,IBAL,IROW,ICOL,IPER,DD,NYR
 REAL :: Q1,Q2,X,Y
 TYPE BALOBJ
  CHARACTER(LEN=52) :: LABEL
  INTEGER :: NPOL,NLAY
  REAL,POINTER,DIMENSION(:) :: Q=>NULL()
  INTEGER,DIMENSION(10) :: ILAY,IPOL
 END TYPE BALOBJ
 TYPE(BALOBJ),DIMENSION(:),ALLOCATABLE :: BAL
  
 IF(.NOT.UTL_READINITFILE('SYEAR',LINE,IU,0))RETURN
 READ(LINE,*) IY1; WRITE(*,'(A,I10)') 'SYEAR=',IY1
 IF(.NOT.UTL_READINITFILE('EYEAR',LINE,IU,0))RETURN
 READ(LINE,*) IY2; WRITE(*,'(A,I10)') 'EYEAR=',IY2
...
! IY1=1980; IY2=2015; 
 ...
 ALLOCATE(IPFFILE(NFILE),ILAYFILE(NFILE))
 
 DO I=1,NFILE

  IF(.NOT.UTL_READINITFILE('IPFFILE'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) ILAYFILE(I),IPFFILE(I); LINE='IPFFILE'//TRIM(ITOS(I))//'='//TRIM(ITOS(ILAYFILE(I)))//','//TRIM(IPFFILE(I))
  WRITE(*,'(A)') TRIM(LINE)
 
 ENDDO
 
 IF(.NOT.UTL_READINITFILE('LAYFILE',LINE,IU,0))RETURN
 READ(LINE,*) LAYFILE; WRITE(*,'(A)') 'LAYFILE='//TRIM(LAYFILE)
 IF(.NOT.UTL_READINITFILE('ZONEIDF',LINE,IU,0))RETURN
 READ(LINE,*) ZONEIDF%FNAME; WRITE(*,'(A)') 'ZONEIDF='//TRIM(ZONEIDF%FNAME)

 !## read zones
 IF(.NOT.IDFREAD(ZONEIDF,ZONEIDF%FNAME,1))STOP
 !## get number of zones
 N=13; ALLOCATE(BAL(N)); BAL%NPOL=0; BAL%NLAY=0
 DO I=1,SIZE(BAL); ALLOCATE(BAL(I)%Q(NYR)); BAL(I)%Q=0.0; ENDDO
 
 WRITE(*,'(/1X,A,I10,A)') 'Found ',N,' zones'
  
 JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=LAYFILE,ACTION='READ',FORM='FORMATTED',STATUS='OLD')
 READ(JU,*)
 DO I=1,SIZE(BAL)
  READ(JU,*) BAL(I)%LABEL,BAL(I)%NPOL,(BAL(I)%IPOL(J),J=1,BAL(I)%NPOL),BAL(I)%NLAY,(BAL(I)%ILAY(J),J=1,BAL(I)%NLAY)
 ENDDO
 CLOSE(JU)
 ...
 DO I=1,NFILE
  ...
  DO J=1,N
   ...
   !## skip outside zones
   IF(ZONEIDF%X(ICOL,IROW).LT.0.OR.ZONEIDF%X(ICOL,IROW).GT.SIZE(BAL))CYCLE
   
   !## what group has this zone number on the given layer
   IBAL=0
   DO K=1,SIZE(BAL)
    DO KK=1,BAL(K)%NPOL
     !## current group has current zone
     IF(BAL(K)%IPOL(KK).EQ.ZONEIDF%X(ICOL,IROW))THEN
      !## check whether is has the same layer
      DO KKK=1,BAL(K)%NLAY
       !## ihit is group number
       IF(BAL(K)%ILAY(KKK).EQ.ILAYFILE(I))IBAL=K
      ENDDO
     ENDIF
    ENDDO
   ENDDO
   ...
   IF(IBAL.EQ.0)THEN
    WRITE(*,*) 'Cannot allocate current location'
    PAUSE
   ENDIF
   ...
   READ(KU,*) IDATE,Q1
   JD1=UTL_IDATETOJDATE(IDATE)
   DO K=1,NN-1
    READ(KU,*) JDATE,Q2
    JD2=UTL_IDATETOJDATE(JDATE)
    CALL IDATETOGDATE(IDATE,IY,IM,ID)
    IF(IY.GE.IY1.AND.IY.LE.IY2)THEN
     IPER=(IY-IY1)+1
     DD= JD2-JD1
     BAL(IBAL)%Q(IPER)=BAL(IBAL)%Q(IPER)+Q1*REAL(DD)
    ENDIF
 ...
 WRITE(JU,'(14A)') 'Location Names',(','//TRIM(BAL(J)%LABEL),J=1,SIZE(BAL))
 WRITE(JU,'(14A)') 'Location Ids',(',Zone '//TRIM(ITOS(J)),J=1,SIZE(BAL))
 WRITE(JU,'(14A)') 'Time',(',Q_m3yr.wel',J=1,SIZE(BAL))

 IY=IY1-1
 DO IPER=1,NYR

  DO J=1,SIZE(BAL); BAL(J)%Q(IPER)=-1.0*BAL(J)%Q(IPER)/1.0E6; ENDDO
  IY=IY+1
  WRITE(LINE,'(I4.4,A)') IY,'-01-01 00:00:00'
  WRITE(JU,'(A,13(A1,E10.4))') TRIM(LINE),(',',BAL(J)%Q(IPER),J=1,SIZE(BAL))

 ENDDO

 CLOSE(JU)
 DO I=1,SIZE(BAL); DEALLOCATE(BAL(I)%Q); ENDDO
 DEALLOCATE(IPFFILE,ILAYFILE,BAL)
 ...
 USE MOD_SOLID_PAR, ONLY : IBATCH,IWINDOW,SOLFILE,OUTPUTFOLDER,FMIDELEV,    &
   MDLIDF,REGISTOP,REGISBOT,REGISKHV,REGISKVV,SLD,DZ,HCLOSE,MICNVG,IBNDCHK,IINT_IDF,GENSOL, &
   REGISFILES_TOP,REGISFILES_BOT,REGISFILES_KHV,REGISFILES_KVV,NGENSOL
 IMPLICIT NONE
 INTEGER :: IMASK,IHYPO,ICKDC,I,J,FNLAY
 REAL :: ZOFFSET
 
 IWINDOW=0; HCLOSE=0.001; MICNVG=5; FMIDELEV=1.0; IBNDCHK=0; IINT_IDF=1
...
! ENDDO
 ...
 !###======================================================================
 USE MOD_MDL_PAR, ONLY : RUNFILE,RESDIR,SIMBOX,CLIPDIR,SIMCSIZE
 IMPLICIT NONE
 ...
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  ...
  IF(SIMCSIZE.LE.0.0)STOP 'SIMCSIZE NEED TO BE LARGER THAN ZERO !'
 ENDIF
 CLIPDIR=''
 ...
 IF(.NOT.UTL_READPOINTER(IU,MEAN_NLAYER,MEAN_ILAYER,'ILAYER',0))RETURN
 ...
 SELECT CASE (TRIM(CFUNC))
  ...
  CASE('PERC')
   ...
   READ(LINE,*) PERCVALUE; WRITE(*,'(A,I8)') 'PERCVALUE=',PERCVALUE
   PERCVALUE=PERCVALUE/100.0
  CASE DEFAULT
...
!  STOP
 ...
 DO I=1,NBAL
  ...
  IF(J.GT.MXTP)THEN
   WRITE(*,'(/A)') 'Cannot identify keyword '//TRIM(CBAL(I))
  ...
  WRITE(*,'(A)') TRIM(LINE)
  IF(.NOT.UTL_READPOINTER(IU,TP(J)%NSYS,TP(I)%ISYS,'BAL'//TRIM(ITOS(I))//'ISYS',1))RETURN
  IF(TP(J)%ISYS(1).EQ.0)THEN
   TP(J)%NSYS=1
   LINE='- all systems'; WRITE(*,'(A)') TRIM(LINE)
  ELSE
   LINE='- systems'
   DO K=1,TP(J)%NSYS; LINE=TRIM(LINE)//', '//TRIM(ITOS(TP(J)%ISYS(K))); ENDDO; WRITE(*,'(A)') TRIM(LINE)
  ENDIF
 ENDDO
...
! I=INDEX(FUNC,'('); I=MAX(I,1)
 ...
 TRIM_VALUE=0.0; IF(UTL_READINITFILE('TRIM_VALUE',LINE,IU,1))READ(LINE,*) TRIM_VALUE
 ...
 IF(SCLTYPE_UP.GT.0)THEN
  SELECT CASE (SCLTYPE_UP)
   ...
   CASE (14)
    ...
    KMIN=1.0 !## k-value to become clay
 ...
 TOPWC=''; BOTEL=0.0; MULT =1.0; ADD=0.0
 ...
 CALL ASC2IDF_EXPORTASC_MAIN(DIRNAME,IBATCH)
 ...
 !## read legname (optional)
 TSIZE=0.05; IF(UTL_READINITFILE('LEGTSIZE',LINE,IU,1))THEN
...
! ENDIF
 ...
 DO II=II1,II2
  ...
  IF(LEN_TRIM(IPFNAME).NE.0)THEN
   IF(NLABELS.EQ.0)THEN
    CALL IDFINIT(IDFNAMEGIVEN=IPFNAME,LEGNAME=IPFLEGNAME,LPLOT=.FALSE.,ISTYLE=IPFSTYLE,LDEACTIVATE=.FALSE., &
  ...
  DO I=1,NGEN; CALL TOPOGENINIT(GENNAME=GENNAME(I),LPLOT=.FALSE.,LDEACTIVATE=.FALSE.,GENCOLOUR=GENCOLOUR(I),GENTHICKNESS=GENTHICKNESS(I)); ENDDO
  ...
  IBITMAP=0
  IF(NFIG.EQ.4)THEN
   CALL IMODBATH_PLOTFIG(TSIZE,YFRACLEGEND,(/IDFLEGTXT,IPFLEGTXT,IFFLEGTXT/),IBITMAP)
  ELSE
   IBITMAP=MPW%IBITMAP
  ENDIF
 ...
 !###======================================================================
 SUBROUTINE IMODBATH_PLOTFIG(TSIZE,YFRACLEGEND,LEGTXT,IBITMAP)
 !###======================================================================
 IMPLICIT NONE
 ...
 CALL WGRTEXTBLOCK(GR+BND,PB,XP-TBND,AB-2.5,'Project: '//CHAR(13)//TRIM(FIG(4)),1.0) 
 !## figure
 CALL WGRTEXTSTRING(GR+BND,FB,'Figure/report: '//TRIM(FIG(3)))
 !## date
 ...
 CALL WGRTEXTSTRING(GR+BND,DB,'Creation Date/time: '//TRIM(STRING))

 CALL WGRTEXTORIENTATION(IALIGN=ALIGNRIGHT,ANGLE=0.0)
 CALL WGRTEXTFONT(FFHELVETICA,WIDTH=X1/3.0,HEIGHT=Y1/3.0,ISTYLE=0)
 CALL WGRTEXTSTRING(XP-BND,0.0+0.5*BND,'Powered by '//TRIM(UTL_IMODVERSION())//' Copyright © 2005-2018')
 ...
 DL=0.0; IF(N.GT.0)DL =(YFRACLEGEND/REAL(N))/100.0*(LT-LB)
 LB1=LT-DL
 ...
 DO I=1,MXMPLOT
  ...
  CALL LEGPLOT_PLOT(MP(I)%LEG,1,TSIZE=TSIZE)  !## plot legend for iplot=1, nc=2 columns
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 IPLOT=1; ITRANSIENT=0; NLAYER=0; NIPFS=0 !; NRDATE=0; 
 IWEIGHT=0; ICOL=0
...
! IF(.NOT.UTL_READPOINTER(IU,NRDATE,IRDATE,'IDATE',1))RETURN
 ...
 !## read weight-class
 WC1=0.0; IF(UTL_READINITFILE('WC1',LINE,IU,1))THEN
  READ(LINE,*) WC1; WRITE(*,'(A,F10.2)') 'WC1=',WC1 
 ENDIF
 WC2=0.0; IF(UTL_READINITFILE('WC2',LINE,IU,1))THEN
  READ(LINE,*) WC2; WRITE(*,'(A,F10.2)') 'WC2=',WC2
 ENDIF
 
 IF(.NOT.UTL_READPOINTER_REAL(IU,I,HCLASSES,'HCLASSES',1,EXCLVALUE=0.0))RETURN
 IF(ASSOCIATED(HCLASSES))THEN
  ...
  ENDDO
 ELSE
  ALLOCATE(HCLASSES(23)); HCLASSES=0.0
 ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=256) :: MFFNAME,METFNAME,OUTNAME
 
 IF(.NOT.UTL_READINITFILE('MFFNAME',LINE,IU,0))RETURN
 READ(LINE,*) MFFNAME; WRITE(*,'(A)') 'MFFNAME='//TRIM(MFFNAME)
! IF(.NOT.UTL_READINITFILE('METFNAME',LINE,IU,0))RETURN
! READ(LINE,*) METFNAME; WRITE(*,'(A)') 'METFNAME='//TRIM(METFNAME)
 IF(.NOT.UTL_READINITFILE('OUTNAME',LINE,IU,0))RETURN
 READ(LINE,*) OUTNAME; WRITE(*,'(A)') 'OUTNAME='//TRIM(OUTNAME)
 
 CALL IMPORT_MF2005_MAIN(MFFNAME,METFNAME,OUTNAME)
