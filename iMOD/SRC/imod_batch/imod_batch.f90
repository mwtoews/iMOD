...
USE MOD_PLINES_TRACE, ONLY : TRACEMAIN,TRACEPOSTPROCESSING,TRACECONVERTTOGEN,TRACEDEALLOCATE,TRACE_INIT_SP,TRACE_AL_SP
USE MOD_PLINES_PAR, ONLY : SPFNAME,NSPFNAME,SP
...
! OPTIONAL KEYWORDS
 ...
 IF(.NOT.UTL_READINITFILE('SPFNAME',LINE,IU,0))RETURN
 READ(LINE,*) SPFNAME(1); WRITE(*,'(A)') 'SPFNAME='//TRIM(SPFNAME(1))
 ...
 IF(.NOT.TRACEPREPARESP(SP(1)%NPART,1,1))RETURN 
 !## read all particle in memory
 CALL TRACE_AL_SP(SP(1)%NPART,1) 
 !## set initial time for each particle to zero
 CALL TRACEREADSP(1,SP(1)%NPART)
