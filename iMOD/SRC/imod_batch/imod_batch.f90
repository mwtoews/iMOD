...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=256) :: IPFOBS,IPFMES
 
 NIPF=2; CALL IPFALLOCATE()
 
 IF(.NOT.UTL_READINITFILE('IPFOBS',LINE,IU,0))RETURN
 READ(LINE,*) IPFOBS; WRITE(*,'(A)') 'IPFOBS='//TRIM(IPFOBS)
 IF(.NOT.UTL_READINITFILE('IPFMES',LINE,IU,0))RETURN
 READ(LINE,*) IPFMES; WRITE(*,'(A)') 'IPFMES='//TRIM(IPFMES)
 DO I=1,2
  IPF(I)%XCOL=1; IPF(I)%YCOL=2; IPF(I)%QCOL=1; IPF(I)%ZCOL=1; IPF(I)%Z2COL=1
 ENDDO
 IPF(1)%FNAME=IPFOBS; IF(.NOT.IPFREAD2(1,1,1))RETURN
 IPF(2)%FNAME=IPFMES; IF(.NOT.IPFREAD2(2,1,1))RETURN
