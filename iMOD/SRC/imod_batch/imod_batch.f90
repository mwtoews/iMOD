...
! OPTIONAL KEYWORDS
 ...
 IMPLICIT NONE
 INTEGER :: JU,NLAY
 CHARACTER(LEN=256) :: MF6DIR
 REAL(KIND=DP_KIND) :: XLL,YLL
 ...
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7.MPNAM',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') 'MPBAS     '//'MP7.MPBAS'
 WRITE(JU,'(A)') 'GRBDIS    '//'GWF_1\MODELINPUT\MF6.DIS.GRB'
 WRITE(JU,'(A)') 'TDIS      '//'MFSIM.TDIS6'
 WRITE(JU,'(A)') 'HEAD      '//'GWF_1\MODELOUTPUT\HEAD\HEAD.HED'
 WRITE(JU,'(A)') 'BUDGET    '//'GWF_1\MODELPUTPUT\BUDGET\BUDGET.CBC'
 CLOSE(JU)
 
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7.MPSIM',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') 'MP7.MPNAM'
 WRITE(JU,'(A)') 'MF7\MP7.MPLIST'
 WRITE(JU,'(A)') '2 2 1 1 0 0' !## pathlines(2),backward(2),pass weaksinks(1),pass weaksources(1),no checks(0),no detailed info(0)
 WRITE(JU,'(A)') '.\MP7\MP7.ENDPOINT7'
 WRITE(JU,'(A)') '.\MP7\MP7.PATHLINE7'
 WRITE(JU,'(A)') '.\MP7\MP7.TIMESERIES7'
 ...
 WRITE(JU,'(A)') 'EXTERNAL .\MPF\MP7.SLOC'
 CLOSE(JU)
 ...
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MPF\MP7.SLOC',STATUS='UNKNOWN',ACTION='WRITE')
 ...
 DO I=1,SP(1)%NPART
  XLL=SP(1)%XLC(I)
  YLL=SP(1)%YLC(I)
 !  WRITE(JU,'(A)') TRIM(ITOS(SP(1)%KLC(I))//','//TRIM(ITOS(SP(1)%ILC(I))//','//TRIM(ITOS(SP(1)%JLC(I))       //','// &
 !                 TRIM(RTOS(XLLL,'F',2)) //','//TRIM(RTOS(YLL,'F',2))  //','//TRIM(RTOS(SP(1)%ZLL(I),'F',2))//',' !// &
       ![ID], Layer, Row, Column, LocalX, LocalY, LocalZ, TimeOffset, Drape      
 ENDDO
 CLOSE(JU)
