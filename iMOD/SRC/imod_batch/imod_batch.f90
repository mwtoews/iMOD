...
USE MOD_UTL,ONLY : UTL_CAP,UTL_GETUNIT,UTL_READINITFILE,ITOS,RTOS,UTL_IDATETOJDATE,UTL_DIRINFO,UTL_CREATEDIR, &
  ...
  IDATETOGDATE,VAR,UTL_IDFSNAPTOGRID_LLC,UTL_EQUALS_REAL,UTL_IMODFILLMENU
...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=69
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'IPF2ISG','SFRTOISG','ISGTOSFR','IDFTRACE','UZFTOCSV','GENSNAPTOGRID','DEVWELLTOIPF','ADJUSTVOXELIDF'/
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 IF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  ...
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVEFHB,'SAVEFHB',1,EXCLVALUE=0))RETURN
  ...
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVELAK,'SAVELAK',1,EXCLVALUE=0))RETURN
...
! 9014 FORMAT (62X, 2(1PE14.7, 1X))
 ...
 REAL :: HNODATA,FNODATA,MINKH,MINKD
...
! ENDIF
 ...
 ICLAY=1; IF(UTL_READINITFILE('ICLAY',LINE,IU,1))READ(LINE,*) ICLAY
 WRITE(*,'(A,I5)') 'ICLAY=',ICLAY 
! !## apply fraction for missing data
! IFRAC=1; IF(UTL_READINITFILE('IFRAC',LINE,IU,1))READ(LINE,*) IFRAC
! WRITE(*,'(A,I5)') 'IFRAC=',IFRAC 
 ...
 DO I=1,N
  ...
  IF(.NOT.ST1CREATEIPF_STEADY(IKD,ICW,MINKH,IMIDF,SDATE,EDATE,ISS,ICLAY,HNODATA,FNODATA,MINKD))WRITE(*,'(A)') 'Error IPF '//TRIM(IPF(1)%FNAME)//' failed'
 ENDDO
...
! ENDDO
 ...
 IF(UTL_READINITFILE('CSVFNAME',LINE,IU,1))THEN
  ...
  BUDGET%IACT=0; N=0; IF(.NOT.IMODBATCH_READPOINTER(N,ITMP,'BDGIACT',1))RETURN
  DO I=1,N; BUDGET(I)%IACT=ITMP(I); ENDDO
  !## read budget groups
  DO I=1,SIZE(BUDGET); BUDGET(I)%IGROUP=I; ENDDO; N=0; IF(.NOT.IMODBATCH_READPOINTER(N,ITMP,'BDGIGRP',1))RETURN
  DO I=1,N; BUDGET(I)%IGROUP=ITMP(I); ENDDO
  !## read budget colours
  DO I=1,SIZE(BUDGET); BUDGET(I)%ICLR=COLOUR_RANDOM(); ENDDO; N=0; IF(.NOT.IMODBATCH_READPOINTER(N,ITMP,'BDGICLR',1))RETURN
  ...
  LIZONE=0; N=0; IF(.NOT.IMODBATCH_READPOINTER(N,ITMP,'ZONES',1))RETURN
  DO I=1,N; 
  ...
  LILAY=0; N=0; IF(.NOT.IMODBATCH_READPOINTER(N,ITMP,'ZONES',1))RETURN
  DO I=1,N; 
  ...
  LIDATE=0; N=0; IF(.NOT.IMODBATCH_READPOINTER(N,ITMP,'ZONES',1))RETURN
  DO I=1,N; 
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 IMPLICIT NONE
 TYPE(IDFOBJ) :: IDF
 CHARACTER(LEN=528) :: LINE,DIRNAME,IDFFILE,OUTPUTDIR,IDFNAME
 CHARACTER(LEN=52) :: WC
 REAL :: VOXELOFFSET
 CHARACTER(LEN=256),DIMENSION(:),POINTER :: LISTNAME
 INTEGER :: I
