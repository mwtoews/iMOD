...
! ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ) :: IDF,SUM,PHR,FFF,FRF,FLF
 ...
 CALL IDFNULLIFY(FFF); CALL IDFNULLIFY(FRF); CALL IDFNULLIFY(FLF)
 IDF%ITYPE=8
...
!    ENDIF
 ...
 IF(UTL_READINITFILE('BDG',LINE,IU,1))THEN
  CALL IDFCOPY(IDF,FFF); CALL IDFCOPY(IDF,FRF); CALL IDFCOPY(IDF,FLF)
   ...
   SELECT CASE (IMETH)
    ...
    CASE (1)
     ...
     IF(TRIM(ADJUSTL(TEXT)).EQ.'FLOW-JA-FACE')THEN
      FLF%NODATA=1.0D30; FLF%X=0.0D0
      FFF%NODATA=1.0D30; FFF%X=0.0D0
      FRF%NODATA=1.0D30; FRF%X=0.0D0
      DO N=1,NCELLS
...
!       WRITE(*,'(A,4I10)') 'NUMBER OF CONNECTED CELLS IS CELL,ILAY,IROW,ICOL', NCON,ILAY,IROW,ICOL
       DO IPOS=IA(N)+1,IA(N+1)-1
        ...
        IF(JLAY.GT.ILAY)THEN
         FLF%X(ICOL,IROW)=Q !FLF%X(ICOL,IROW)+Q
        ELSEIF(JROW.GT.IROW)THEN
         FFF%X(ICOL,IROW)=Q !FFF%X(ICOL,IROW)+Q
        ELSEIF(JCOL.GT.ICOL)THEN
         FRF%X(ICOL,IROW)=Q !FRF%X(ICOL,IROW)+Q
        ENDIF
!        WRITE(*,'(A,2I10,F10.3)') ' N M Q: ', N,M,Q
      ...
      DO ILAY=1,ABS(NDIM3)
       IDF%NODATA=1.0D30
       IF(ILAY.LT.ABS(NDIM3))THEN
        DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL; IDF%X(ICOL,IROW)=FLF%X(ICOL,IROW); ENDDO; ENDDO; TEXT='BDGFLF'
        IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
        IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
       ENDIF
       DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL; IDF%X(ICOL,IROW)=FRF%X(ICOL,IROW); ENDDO; ENDDO; TEXT='BDGFRF'
       IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
       IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
       DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL; IDF%X(ICOL,IROW)=FFF%X(ICOL,IROW); ENDDO; ENDDO; TEXT='BDGFFF'
       IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
       IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
      ENDDO
     ELSE
...
! OPTIONAL KEYWORDS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: JU,KU,NLAY,I,IOS
 CHARACTER(LEN=256) :: MF6DIR,MODPATH7
 REAL(KIND=DP_KIND) :: XLL,YLL,X1,Y1,X2,Y2,X
 ...
 WRITE(JU,'(A)') 'GRBDIS    '//'..\GWF_1\MODELINPUT\MF6.DIS6.GRB'
...
! WRITE(JU,'(A)') 'MP7.TIMESERIES7'
 ...
 CALL IOSCOMMAND(TRIM(MF6DIR)//'\MP7\MP7.BAT')
