...
USE MOD_UTL,ONLY : UTL_CAP,UTL_GETUNIT,UTL_READINITFILE,ITOS,RTOS,UTL_IDATETOJDATE,UTL_DIRINFO,UTL_CREATEDIR, &
  ...
  IDATETOGDATE,VAR,UTL_IDFSNAPTOGRID_LLC
...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=67
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'IPF2ISG','SFRTOISG','ISGTOSFR','IDFTRACE','UZFTOCSV','GENSNAPTOGRID'/
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 PBMAN%NSTEP=1; PBMAN%NMULT=1.0; PBMAN%ISTEADY=0
 ...
 IF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  ...
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVESHD,'SAVESHD',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVEFLX,'SAVEFLX',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVEUZF,'SAVEUZF',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVESFR,'SAVESFR',1,EXCLVALUE=0))RETURN

  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVEWEL,'SAVEWEL',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVEDRN,'SAVEDRN',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVERIV,'SAVERIV',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVEGHB,'SAVEGHB',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVERCH,'SAVERCH',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVEEVT,'SAVEEVT',1,EXCLVALUE=0))RETURN
  IF(.NOT.IMODBATCH_READPOINTER(N,PBMAN%SAVEMNW,'SAVEMNW',1,EXCLVALUE=0))RETURN
...
!    IF(I.EQ.1)THEN; LQBD=.TRUE.; WRITE(*,'(/A/)') 'IQUASI3D = QUASI 3D DISCRETISATION APPLIED (USING INTERBEDS)'; ENDIF
  ...
  PBMAN%UNCONFINED=0; PBMAN%IPEST=0
...
!  TNG=TNG-0.5*PI
 ...
 !CALL ISG_ADDSTAGES(ISGFILE,IPFFILE,1,STAGETYPE)
 ...
 ICLEAN=1; IF(UTL_READINITFILE('ICLEAN',LINE,IU,0))READ(LINE,*) ICLEAN
 WRITE(*,'(A,I1)') 'ICLEAN=',ICLEAN

 CROSS_PNTR=''; CROSS_BATH=''; WIDTHFNAME=''
 ...
 CALL ISG_ADDCROSSSECTION(ISGFILE,FNAME,WIDTHFNAME,MAXDIST,CROSS_PNTR,CROSS_BATH,CROSS_ZCHK,CROSS_CVAL,CELL_SIZE,1,ICLEAN)
...
! WRITE(*,'(A,I2)') 'ITALUSCOL=',DATCOL(9)
 ...
 INTEGER :: IROW,ICOL,N,JU
 
 IDF%XMIN=0.0; IDF%YMIN=0.0; IDF%XMAX=0.0; IDF%YMAX=0.0
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  READ(LINE,*) IDF%XMIN,IDF%YMIN,IDF%XMAX,IDF%YMAX
  WRITE(*,'(A,4F10.2)') 'WINDOW=',IDF%XMIN,IDF%YMIN,IDF%XMAX,IDF%YMAX
  IF(.NOT.UTL_READINITFILE('CELL_SIZE',LINE,IU,0))RETURN
  READ(LINE,*) IDF%DX; WRITE(*,'(A,F10.2)') 'CELL_SIZE=',IDF%DX; IDF%DY=IDF%DX
  CALL UTL_IDFSNAPTOGRID_LLC(IDF%XMIN,IDF%XMAX,IDF%YMIN,IDF%YMAX,IDF%DX,IDF%NCOL,IDF%NROW)
 ELSE
  IF(.NOT.UTL_READINITFILE('IDFFILE',LINE,IU,0))RETURN
  READ(LINE,*) IDF%FNAME; WRITE(*,'(A)') 'IDFFILE='//TRIM(IDF%FNAME)
  IF(.NOT.IDFREAD(IDF,IDF%FNAME,0))RETURN
 ENDIF
 !## fill sx/sy variable in idf
 IF(.NOT.IDFFILLSXSY(IDF))RETURN

 ALLOCATE(IPC(IDF%NCOL,IDF%NROW,2)); IPC=INT(0,1)
 ...
 CALL ASC2IDF_HFB(IDF,IDF%NROW,IDF%NCOL,IPC,GENFILE,-1)
 ...
 N=0; DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL

  !## place vertical wall
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.IDF%NCOL)THEN; N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF,IDF%NROW,IDF%NCOL,IROW,ICOL,N,0.0,0.0,0.0); ENDIF
  ENDIF

  !## place horizontal wall
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   !## write line in genfile
   IF(IROW.LT.IDF%NROW)THEN; N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF,IDF%NROW,IDF%NCOL,IROW,ICOL,N,0.0,0.0,0.0); ENDIF
  ENDIF
 
 ENDDO; ENDDO

 CLOSE(JU); CALL ASC2IDF_INT_DEALLOCATE(); DEALLOCATE(IPC); CALL IDFDEALLOCATEX(IDF)
...
! 9014 FORMAT (62X, 2(1PE14.7, 1X))
 ...
 IMPLICIT NONE
 INTEGER :: I,N,IKD,ICW,SDATE,EDATE,ISS,IMIDF,ICLAY,IFRAC,IQCOL
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 IF(UTL_READINITFILE('SOURCEDIR',LINE,IU,1))THEN
  ...
  READ(LINE,'(A256)') TARGETDIR
  WRITE(*,'(A)') 'TARGETDIR='//TRIM(TARGETDIR)
 ELSE
  ALLOCATE(XYZFNAMES(2)); XYZFNAMES=''
  IF(UTL_READINITFILE('XYZFILE',LINE,IU,1))THEN
   READ(LINE,'(A256)') XYZFNAMES(1); WRITE(*,'(A)') 'XYZFILE='//TRIM(XYZFNAMES(1)); IN_TYPE=1
  ENDIF
  IF(UTL_READINITFILE('IPFFILE',LINE,IU,1))THEN
   READ(LINE,'(A256)') XYZFNAMES(1); WRITE(*,'(A)') 'IPFFILE='//TRIM(XYZFNAMES(1)); IN_TYPE=2
  ENDIF
  IF(UTL_READINITFILE('IDFFILE_IN',LINE,IU,1))THEN
   READ(LINE,'(A256)') XYZFNAMES(1); WRITE(*,'(A)') 'IDFFILE_IN='//TRIM(XYZFNAMES(1)); IN_TYPE=3
  ENDIF
  IF(UTL_READINITFILE('GENFILE',LINE,IU,1))THEN
   READ(LINE,'(A256)') XYZFNAMES(1); WRITE(*,'(A)') 'GENFILE='//TRIM(XYZFNAMES(1)); IN_TYPE=4
  ENDIF
 ENDIF
 IF(UTL_READINITFILE('IDFFILE_POINTER',LINE,IU,1))THEN
  READ(LINE,'(A256)') XYZFNAMES(2); WRITE(*,'(A)') 'IDFFILE_POINTER='//TRIM(XYZFNAMES(2))
 ENDIF
 ...
 ASSF_IDEPTH=0; ASSF_TOP=0.0; ASSF_BOT=0.0; ASSF_DZ=0.0; ASSF_NTHRESHOLD=1; ASSF_INDICATOR=0
  ...
  IF(M.GT.0.AND.M.EQ.IZCOL)THEN
   ...
   ELSE
    ...
    IF(ASSF_IDEPTH.EQ.1)THEN
     ...
     IF(.NOT.IMODBATCH_READPOINTER_REAL(I,DZ,'ASSF_DZ',0,EXCLVALUE=0.0))RETURN
    ...
    !## indicator
    ASSF_INDICATOR=0; ASSF_NTHRESHOLD=0
 ...
 IF(IGRIDFUNC.NE.7)THEN
  IF(.NOT.UTL_READINITFILE('IDFFILE',LINE,IU,0))RETURN
  READ(LINE,'(A256)') IDFFILE; WRITE(*,'(A)') 'IDFFILE='//TRIM(IDFFILE)
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  CASE (-6,6)
   ...
   READ(LINE,'(A256)') STDEVIDF; WRITE(*,'(A)') 'STDEVIDF='//TRIM(STDEVIDF)
  CASE (7)
 ...
 IF(UTL_READINITFILE('BLNFILE',LINE,IU,1))THEN
  READ(LINE,'(A256)') BLNFILE; WRITE(*,'(A)') 'BLNFILE='//TRIM(BLNFILE)
 ENDIF
...
!   1    1    1     1     1    6.3357E+04  7.9484E+03   5.5409E+04  0.000E+00  0.000E+00  0.000E+00  3.37445E+02  3.587E-01  3.931E+00  5.850E+03  1.359E+00
   ...
   DO J=1,ISG(I)%NCLC
    ...
    DATISD(IREC)%INFF =QSTR/85400.0 !## m3/day - m3/sec
   ENDDO
  ENDDO
 ENDDO
 CLOSE(JU)
 ...
 REAL :: X,MINTHICK,THICK,THICK2

 ICLEAN=0; MINTHICK=0.0; THICK=0.0; THICK2=0.0001
  ...
  DO I=2,SIZE(IDF)
   ...
   DO J=I-1,1,-1
    IF(IDF(J)%X(ICOL,IROW).NE.IDF(J)%NODATA)THEN
     IF(MINTHICK.EQ.0.0)THEN
      IDF(I)%X(ICOL,IROW)=MIN(IDF(I)%X(ICOL,IROW),IDF(J)%X(ICOL,IROW)); EXIT
     ELSE 
     !## calculate the thickness of the layer between top/bot + ignore layer between bot and top (=impermeable layer)
      DO
       THICK=IDF(J)%X(ICOL,IROW)-IDF(I)%X(ICOL,IROW)
       
       IF(I.LE.SIZE(IDF)-2)THEN
        !## calculate thickness of layers between the bot of the deeper layer and the bot of the layer above it
        !## this is needed to check if there are multiple layers without a thickness that all need to be corrected   
        THICK2=IDF(I)%X(ICOL,IROW)-IDF(I+2)%X(ICOL,IROW)
       ENDIF 
       
       !## correct idf->top in case of (multiple) permeable layers
       IF(THICK.LT.0.0.AND.MOD(I,2).EQ.0)THEN
        IDF(I)%X(ICOL,IROW)=IDF(J)%X(ICOL,IROW)-MINTHICK
       ELSEIF(THICK.LT.0.0.AND.MOD(I,2).NE.0)THEN
        IDF(I)%X(ICOL,IROW)=IDF(J)%X(ICOL,IROW)   
       ELSEIF(THICK2.EQ.0.0.AND.MOD(I,2).EQ.0)THEN
        IDF(I+1)%X(ICOL,IROW)=IDF(I+1)%X(ICOL,IROW)+MINTHICK
        IDF(I)%X(ICOL,IROW)=IDF(I+1)%X(ICOL,IROW)
        EXIT
       ELSE 
        EXIT
       ENDIF
      ...
      !## correct layer depth in case layer thickness is less then the given minimal thickness
      IF(MOD(I,2).EQ.0)THEN
       IF(THICK.LT.MINTHICK)THEN
        IDF(I)%X(ICOL,IROW)=IDF(J)%X(ICOL,IROW)-(MINTHICK-THICK)
       ENDIF  
      ENDIF        
...
! IY1=1980; IY2=2015; 
 ...
 DO I=1,NBAL
  ...
  WRITE(*,'(A)') TRIM(LINE)
  IF(.NOT.IMODBATCH_READPOINTER(TP(J)%NSYS,TP(I)%ISYS,'BAL'//TRIM(ITOS(I))//'ISYS',1))RETURN
...
! IF(.NOT.WBAL_ANALYSE_MAIN('g:\IMOD-MODELS\ALBERTA\SYLVAN_LAKE\IMOD_USER\MODELS\SLB_V9_250\WBAL.CSV'))THEN
! ENDIF
! IF(.NOT.WBAL_ANALYSE_MAIN('g:\IMOD-MODELS\ALBERTA\SYLVAN_LAKE\IMOD_USER\MODELS\SLB_V15_TRANSIENT\WBAL_WHOLE.CSV'))THEN
! ENDIF
...
! I=INDEX(FUNC,'('); I=MAX(I,1)
 ...
 !###======================================================================
 LOGICAL FUNCTION IMODBATCH_READPOINTER(NPOINTER,IPOINTER,TXT,IOPT,EXCLVALUE)
 !###======================================================================
 IMPLICIT NONE
 ...
 IF(IOS.NE.0)THEN
  ...
  STOP
 ENDIF
 WRITE(*,'(A,99(I2,A1))') TRIM(TXT)//'=',(IPOINTER(I),',',I=1,NPOINTER)
 ...
 !###======================================================================
 LOGICAL FUNCTION IMODBATCH_READPOINTER_REAL(NPOINTER,XPOINTER,TXT,IOPT,EXCLVALUE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IOPT !## ioptional
 ...
 NPOINTER=0
 IF(IOPT.EQ.0)THEN
  IMODBATCH_READPOINTER_REAL=.FALSE.
  IF(.NOT.UTL_READINITFILE(TRIM(TXT),LINE,IU,0))RETURN
 ELSEIF(IOPT.EQ.1)THEN
  IMODBATCH_READPOINTER_REAL=.TRUE.
  IF(.NOT.UTL_READINITFILE(TRIM(TXT),LINE,IU,1))RETURN
 ENDIF
 ...
 END FUNCTION IMODBATCH_READPOINTER_REAL
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 IF(.NOT.IMODBATCH_READPOINTER(NLAYER,ILAYER,'ILAYER',1))RETURN
