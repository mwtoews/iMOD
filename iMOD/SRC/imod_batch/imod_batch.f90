...
USE MOD_UTL,ONLY : UTL_CAP,UTL_GETUNIT,UTL_READINITFILE,ITOS,RTOS,IDATETOJDATE,UTL_DIRINFO,UTL_CREATEDIR, &
  ...
  UTL_GETUNIQUE,UTL_GETIDPROC,UTL_WSELECTFILE,PROCOBJ
...
USE MOD_IDF, ONLY : IDFREAD,IDFNULLIFY,IDFWRITE,IDFDEALLOCATE,IDFREADSCALE,IDFCOPY,IDFGETXYVAL,IDFIROWICOL, &
   IDFGETLOC,IDFGETVAL,IDFEQUAL,IDFCREATEIVF
...
USE MOD_PLINES_TRACE, ONLY : TRACEMAIN,TRACEPOSTPROCESSING
...
USE MOD_ISG_GRID, ONLY : ISG_SIMPLIFYMAIN,ISG_ADDCROSSSECTION,ISG2GRIDMAIN
...
USE MOD_SOLID_PCG, ONLY : SOLID_CALC
USE MOD_SOF, ONLY : SOF_MAIN,SOF_MAIN2,SOF_TRACE
USE MOD_HFB, ONLY : HFB_AL,HFB_DAL
...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=48
...
!  ProcSilent (1) Execute in a hidden window (Windows only) 
!  ProcBlocked (2) Block execution till complete 
!  ProcTimeKill (4) Kill a timed-out process 
!  ProcTimeMSec (8) Timeout delay expressed in milliseconds 
!  ProcCmdProc (16) Execute via Windows command processor 
!  ProcHideStdout (32) Hide stdout output on Linux/Mac 
!  ProcHideStderr (64) Hide stderr output on Linux/Mac 
!INTEGER, OPTIONAL ITIMEOUT Time out delay for a blocked process (seconds or milliseconds) 
!INTEGER, OPTIONAL IDPROC(2) Process identifier (returned) 
 ...
 OPEN(UNIT=6, CARRIAGECONTROL='fortran')     ! FR 20131007  
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 DO I=1,NLAY
  ...
  READ(LINE,*) IDF(I,1)%FNAME; LINE='TOP_L'//TRIM(ITOS(I))//'='; WRITE(*,'(A)') TRIM(LINE)//TRIM(IDF(1,1)%FNAME)
  IF(.NOT.UTL_READINITFILE('BOT_L'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) IDF(I,2)%FNAME; LINE='BOT_L'//TRIM(ITOS(I))//'='; WRITE(*,'(A)') TRIM(LINE)//TRIM(IDF(1,2)%FNAME)
 ENDDO
 ...
 INTEGER :: I,IFLOW,IPNTR,IWINDOW !,MINTHREAD !,DTERM
 REAL :: XMIN,YMIN,XMAX,YMAX
 
 IFLOW=0; IPNTR=0; IWINDOW=0 !; MINTHREAD=500; DTERM=0
 
! DTERM=2 !## 0=no diagonal (use 1 to apply diagonals,2=steepest)
! DTERM=3 !## 0=no diagonal (use 1 to apply diagonals,2=steepest)

 IF(UTL_READINITFILE('IFLOW',LINE,IU,1))READ(LINE,*) IFLOW
 WRITE(*,'(A,I1)') 'IFLOW=',IFLOW
! IF(UTL_READINITFILE('DTERM',LINE,IU,1))READ(LINE,*) DTERM
! IF(DTERM.EQ.1)DTERM=3; IF(DTERM.EQ.0)DTERM=2
! WRITE(*,'(A,I1)') 'DTERM=',DTERM
! IF(IFLOW.EQ.1)THEN
!  IF(UTL_READINITFILE('MINTHREAD',LINE,IU,1))READ(LINE,*) MINTHREAD
!  WRITE(*,'(A,I10)') 'MINTHREAD=',MINTHREAD
! ENDIF
 
 I=5; IF(IFLOW.EQ.1)I=I+3
 ...
 IF(UTL_READINITFILE('POINTERIDF',LINE,IU,1))THEN
  IPNTR=1; READ(LINE,*) IDF(5)%FNAME; WRITE(*,'(A)') 'POINTERIDF='//TRIM(IDF(5)%FNAME)
 ENDIF
...
!! CALL SOF_MAIN(IDF,IFLOW,IPNTR,IWINDOW,XMIN,YMIN,XMAX,YMAX,MINTHREAD,DTERM)

 !## compute sof-area with spill levels
 CALL SOF_MAIN2(IDF,IFLOW,IPNTR,IWINDOW,XMIN,YMIN,XMAX,YMAX) !,DTERM)

 IDF(1)%FNAME=IDF(3)%FNAME(:INDEX(IDF(3)%FNAME,'.',.TRUE.)-1)//'_aspect.idf'
 CALL SOF_TRACE(IDF,1)
...
! IPERIOD=0
 ...
 !INQUIRE(FILE=GENFNAME(:INDEX(GENFNAME,'.',.TRUE.)-1)//'.DAT',EXIST=LDAT)
 IF(LDAT)THEN
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 USE MOD_ASC2IDF_PAR, ONLY : IDFFILE,SOURCEDIR,TARGETDIR,IGRIDFUNC,CS,NODATA,LAGDISTANCE,SEARCHDISTANCE, &
   ...
   IEXPVARIOGRAM,ASSF_COLUMN,ASSF_STARTDATE,ASSF_ENDDATE,ASSF_MTYPE,ASSF_DDATE
 USE MOD_SOLID, ONLY : MXITER2,HCLOSE,RCLOSE
 USE MOD_UTL, ONLY : UTL_GETUNIT,JDATETOIDATE
 IMPLICIT NONE
 INTEGER :: I,J,K,N,IN_TYPE,JU,M,IDATE,SDATE,EDATE,DDATE
 ...
 IXCOL=1; IYCOL=2; IZCOL=3; ASSF_COLUMN=0; ASSF_STARTDATE=0; ASSF_ENDDATE=0; ASSF_DDATE=0
 IF(IN_TYPE.EQ.2)THEN
  ...
  IF(M.GT.0.AND.M.EQ.IZCOL)THEN
   ...
   READ(LINE,*) ASSF_DDATE; WRITE(*,'(A,I10)') 'ASSF_DDATE=',ASSF_DDATE
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  CASE (-6,6)
!   NOSEARCH=0; IF(UTL_READINITFILE('NOSEARCH',LINE,IU,1))READ(LINE,*) NOSEARCH
!   WRITE(*,'(A,I10)') 'NOSEARCH=',NOSEARCH
   NOSEARCH=1
  ...
  CASE (7)
   ...
   READ(LINE,*) LAGDISTANCE; WRITE(*,*) 'LAGDISTANCE=',LAGDISTANCE
  CASE (8)
   MXITER2=50 !20 !500 !## inner (linear system)
   HCLOSE=0.001 
   RCLOSE=10000.0
 ...
 IF(N.EQ.0)THEN
  ...
  DO IDATE=SDATE,EDATE,DDATE
   ASSF_STARTDATE= IDATE
   ASSF_ENDDATE  = IDATE+DDATE
 ...
 CALL IPFSAMPLE(FNAME(1),FNAME(2),FNAME(3),IXCOL,IYCOL)
 ...
 !## default irun=1
 IRUN=1; RUNFILE=''; IF(UTL_READINITFILE('IRUN',LINE,IU,1))READ(LINE,*) IRUN
 IF(IRUN.EQ.1)THEN
 ...
 IF(IRUN.EQ.1)THEN
  IF(TRACEMAIN(RUNFILE,1))THEN; ENDIF
 ENDIF
 IF(IPOSTP.EQ.1)CALL TRACEPOSTPROCESSING(IFFFLOW,IPFFLOW,IDFFLOW,IPFFNAME,IPFICOL) 
...
! ENDDO; ENDDO; ENDDO 
 ...
 FDISTANCES=''; IF(UTL_READINITFILE('FDISTANCES',LINE,IU,1))THEN
  READ(LINE,*) FDISTANCES; WRITE(*,'(A)') 'FDISTANCES='//TRIM(FDISTANCES)
  OUTPUTFOLDER=FDISTANCES(:INDEX(FDISTANCES,'\',.TRUE.)-1)
 ELSE
  IF(UTL_READINITFILE('NLAY',LINE,IU,1))THEN
   READ(LINE,*) CUS_NLAY; WRITE(*,'(A,I10)') 'NLAY=',CUS_NLAY
 ...
 IF(.NOT.UTL_READINITFILE('NLAY',LINE,IU,0))RETURN
 READ(LINE,*) NLAY; WRITE(*,'(A)') 'PFOLDER='//TRIM(PFOLDER)
 ...
 !###======================================================================
 USE MOD_SOLID_PAR, ONLY : IBATCH,IWINDOW,SOLFILE,OUTPUTFOLDER, &
   MDLIDF,REGISTOP,REGISBOT,REGISKHV,REGISKVV,SLD,DZ,HCLOSE,MICNVG
 USE MOD_HFB_PAR
 IMPLICIT NONE
 INTEGER :: IMASK,IHYPO,ICKDC,I
 REAL :: ZOFFSET
 
 IWINDOW=0; HCLOSE=0.001; MICNVG=5
 
 CALL SOLIDINITSLD(1)

 !## number of modellayers
 IF(.NOT.UTL_READINITFILE('NLAY',LINE,IU,0))RETURN
 READ(LINE,*) SLD(1)%NLAY; WRITE(*,'(A,I10)') 'NLAY=',SLD(1)%NLAY

 CALL SOLIDINITSLDPOINTER(1,SLD(1)%NLAY)

 ALLOCATE(DZ(SLD(1)%NLAY))
 IF(.NOT.UTL_READINITFILE('DZ',LINE,IU,0))RETURN
 READ(LINE,*) DZ; WRITE(*,'(99F8.2)') DZ
 
 SLD(1)%KDCLR=0; SLD(1)%CCLR =0
 !## try to read all idf's
 DO I=1,SLD(1)%NLAY
  IF(.NOT.UTL_READINITFILE('TOP_L'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) SLD(1)%TBNAME(I,1)
  LINE='TOP_L'//TRIM(ITOS(I))//'='//TRIM(SLD(1)%TBNAME(I,1)); WRITE(*,'(A)') TRIM(LINE)
  SLD(1)%ICLC(I,1)=1; IF(UTL_READINITFILE('ICLC_TL'//TRIM(ITOS(I)),LINE,IU,1))READ(LINE,*) SLD(1)%ICLC(I,1)
  IF(.NOT.UTL_READINITFILE('BOT_L'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) SLD(1)%TBNAME(I,2) 
  LINE='BOT_L'//TRIM(ITOS(I))//'='//TRIM(SLD(1)%TBNAME(I,2)); WRITE(*,'(A)') TRIM(LINE)
  SLD(1)%ICLC(I,2)=1; IF(UTL_READINITFILE('ICLC_BL'//TRIM(ITOS(I)),LINE,IU,1))READ(LINE,*) SLD(1)%ICLC(I,2)
 ENDDO
...
!  IF(UTL_READINITFILE('CMID',LINE,IU,1))READ(LINE,*) CMID
!  WRITE(*,'(A,I10)') 'CMID=',CMID
 ENDIF 

 IF(IHYPO.EQ.1)THEN

  !## hclosure criterium
  IF(.NOT.UTL_READINITFILE('HCLOSE',LINE,IU,0))RETURN
  READ(LINE,*) HCLOSE; WRITE(*,'(A,F15.7)') 'HCLOSE=',HCLOSE
  !## max. inner convergences
  IF(.NOT.UTL_READINITFILE('MICNVG',LINE,IU,0))RETURN
  READ(LINE,*) MICNVG; WRITE(*,'(A,I10)') 'MICNVG=',MICNVG

  !## number of hfb's
  IF(.NOT.UTL_READINITFILE('NHFB',LINE,IU,0))RETURN
  READ(LINE,*) NHFBFILES; WRITE(*,'(A,I10)') 'NHFB=',NHFBFILES
  CALL HFB_AL()
  DO I=1,NHFBFILES
   IF(.NOT.UTL_READINITFILE('HFB'//TRIM(ITOS(I)),LINE,IU,0))RETURN
   READ(LINE,*) HFBFILES(I); LINE='HFB'//TRIM(ITOS(I))//'='//TRIM(HFBFILES(I)); WRITE(*,'(A)') TRIM(LINE)
   IF(.NOT.UTL_READINITFILE('HFBC'//TRIM(ITOS(I)),LINE,IU,0))RETURN
   READ(LINE,*) HFBC(I); LINE='HFBC'//TRIM(ITOS(I))//'='//TRIM(RTOS(HFBC(I),'*',0)); WRITE(*,'(A)') TRIM(LINE)
   IF(.NOT.UTL_READINITFILE('HFBL'//TRIM(ITOS(I)),LINE,IU,0))RETURN
   READ(LINE,*) HFBILAY(I); LINE='HFBL'//TRIM(ITOS(I))//'='//TRIM(ITOS(HFBILAY(I))); WRITE(*,'(A)') TRIM(LINE)
  ENDDO
...
!    IF(TP(J)%ACRNM(1:3).EQ.'BDG')
 ...
 SELECT CASE (MVERSION)
  CASE ('1988')
   IF(.NOT.UTL_READINITFILE('BASFILE',LINE,IU,0))RETURN
