...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 TYPE(IDFOBJ) :: MASK
 INTEGER :: NLAY,ILAY,IVCW,DI,IROW,ICOL,NG,N,JU,KU,NP,IAQUIFER,IAQUITARD,IQ3D,IMASK,IACT
 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   ...
   ELSE
    ...
    CALL IDFCOPY(BND(ILAY),KHV(ILAY)); IF(.NOT.IDFREADSCALE(KHV(ILAY)%FNAME,KHV(ILAY),3,1,0.0D0,0))STOP 'Cannot read data for KHV'
    WRITE(*,'(A)') 'Reading '//TRIM(KVA(ILAY)%FNAME)//'...'
    CALL IDFCOPY(BND(ILAY),KVA(ILAY)); IF(.NOT.IDFREADSCALE(KVA(ILAY)%FNAME,KVA(ILAY),2,1,0.0D0,0))STOP 'Cannot read data for KVA'
   ENDIF
...
!    !## skip zero thickness layers
!    IF(TOP(ILAY)%X(ICOL,IROW)-BOT(ILAY)%X(ICOL,IROW).LE.0.0D0)CYCLE
    IF(ILAY.EQ.1)THEN
...
!    !## skip zero thickness layers
!    IF(BOT(ILAY)%X(ICOL,IROW)-TOP(ILAY+1)%X(ICOL,IROW).LE.0.0D0)CYCLE
  ...
  DO ILAY=1,NLAY; DO I=1,2; N=0
   DO IROW=1,BND(ILAY)%NROW,DI; DO ICOL=1,BND(ILAY)%NCOL,DI
    ...
    IF(I.EQ.2)THEN
     ...
     IACT=1; IF(MASK%X(ICOL,IROW).EQ.MASK%NODATA)IACT=0
!     !## skip zero thickness layers
!     IF(TOP(ILAY)%X(ICOL,IROW)-BOT(ILAY)%X(ICOL,IROW).LE.MINTHICKNESS)IACT=0
   ...
   DO IROW=1,BND(ILAY)%NROW,DI; DO ICOL=1,BND(ILAY)%NCOL,DI
    ...
    IF(I.EQ.2)THEN
     ...
     IF(NG.GT.0)THEN
      ...
      IACT=1; IF(MASK%X(ICOL,IROW).EQ.MASK%NODATA)IACT=0
 !     !## skip zero thickness layers
 !     IF(BOT(ILAY)%X(ICOL,IROW)-TOP(ILAY+1)%X(ICOL,IROW).LE.MINTHICKNESS)IACT=0
