...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=65
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'IPF2ISG','SFRTOISG','ISGTOSFR','IDFTRACE'/
...
!  TNG=TNG-0.5*PI
 ...
 INTEGER :: I,J,IFLOW,IPNTR,IWINDOW,IWRITE,IGRAD,ITQP,NTQP,TTQP,IFORMAT,PITTSIZE,MINFRICTION,NQHW
 REAL,DIMENSION(:),ALLOCATABLE :: PTQP
 REAL,DIMENSION(:,:),POINTER :: QHW
 ...
 ELSEIF(IFLOW.EQ.3)THEN
  ...
  NQHW=0; IF(UTL_READINITFILE('NQHW',LINE,IU,1))READ(LINE,*) NQHW
  IF(NQHW.GT.0)THEN
   WRITE(*,'(A,I10)') 'NQHW=',NQHW
   ALLOCATE(QHW(NQHW,3))
   !## q in m3/seconds
   DO I=1,NQHW
    IF(UTL_READINITFILE('QHW'//TRIM(ITOS(I)),LINE,IU,1))READ(LINE,*) QHW(I,1),QHW(I,2),QHW(I,3)
    LINE='QHW'//TRIM(ITOS(I)); WRITE(*,'(A,3(F10.2,A1))') TRIM(LINE)//'=',QHW(I,1),',',QHW(I,2),',',QHW(I,3)
   ENDDO
  ENDIF
  ...
  CALL SOF_EXPORT(IDF,SIZE(IDF),IFORMAT,RAIN,MINFRICTION,QHW)
