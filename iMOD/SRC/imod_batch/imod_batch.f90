...
USE MOD_UTL,ONLY : UTL_CAP,UTL_GETUNIT,UTL_READINITFILE,ITOS,RTOS,UTL_IDATETOJDATE,UTL_DIRINFO,UTL_CREATEDIR, &
  ...
  UTL_MINTHICKNESS,UTL_GETHELP,UTL_STDEF,UTL_GETGAMMA
...
USE MOD_COLOURS
USE MOD_SOBEK, ONLY : SOBEK1CALC,DFLOWFM1CALC
USE MOD_IPF, ONLY : IPFSAMPLE,IPFDEALLOCATE,IPFREAD2,IPFSPOTIFY,IPFASSIGNWELL,IPFEVALUATE
USE MOD_PLINES_TRACE, ONLY : TRACEMAIN,TRACEPOSTPROCESSING,TRACECONVERTTOGEN,TRACEDEALLOCATE,TRACE_INIT_SP,TRACE_AL_SP,TRACE_BRING_IN_DATA
USE MOD_PLINES_PAR, ONLY : SPFNAME,NSPFNAME,SP,ISPFNAME
...
USE MOD_PMANAGER, ONLY : PMANAGERPRJ,PMANAGERRUN,PMANAGER_UTL_INIT,PMANAGER_SAVEMF2005_HFB_GENFILES,PMANAGER_LOADPRJ,PMANAGER_GETFNAMES
USE MOD_PMANAGER_UTL, ONLY : PMANAGER_SAVEMF2005_MOD_READ
USE MOD_PMANAGER_PAR, ONLY : PBMAN,PEST,PRJILIST,FNAMES,TKHV,TKDW,TKVV,TVCW,TRIV,TDRN,TRCH
USE MOD_BATCH_UTL, ONLY : IMODBATCH_RUNFILE_READ,IMODBATCH_CREATEENSEMBLES_CHOLESKY
USE MOD_PMANAGER_MF6NETWORK, ONLY : PMANAGER_GENERATEMFNETWORKS,PMANAGER_GENERATEMFNETWORKS_WRITEXY
...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=90
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'FOSM','IPFEDIT','MSPNETRCH','MF6NETWORKS','CREATEPILOTPOINTS','IPFEDITWEIGHT','AGGREGATE', &
  'CREATEWELBORELOG','CREATEENSEMBLES','IPFEVALUATE','LAYERFROMTHICKNESS','MODPATH7','IPESTTOPARAM','IFFCROSSSECTION'/
...
! CALL WINDOWSTATUSBARPARTS(4,(/2000,2000,750,-1/),(/1,1,1,1/))
 ...
 SELECT CASE (IFN)
  ...
  CASE (13)!## idfsample
   IF(IMODBATCH_IPFSAMPLE_MAIN())THEN; ENDIF
  CASE (14)!## xyztoidf
  ...
  CASE (31)!## create ASC file from IDF
...
!   CALL IMODBATCH_CREATEVOXEL_MAIN()
  ...
  CASE (83)  !## aggregate
   CALL IMODBATCH_AGGREGATE()
  CASE (84)  !## createwelborelog
   CALL IMODBATCH_CREATEWELBORELOG()
  CASE (85)  !## ensembles
   CALL IMODBATCH_CREATEENSEMBLES()
  CASE (86)  !## evaluate timeseries if measurements are placed in correct modellayer
   CALL IMODBATCH_IPFEVALUATE()   
  CASE (87)  !## construct layers from thicknesses
   CALL IMODBATCH_LAYERFROMTHICKNESS()
  CASE (88)!## imodpath7
   CALL IMODBATCH_IMODPATH7_MAIN()
  CASE (89)!## ipesttoparam
   CALL IMODBATCH_IPESTTOPARAM()
  CASE (90)!## iffcrosssection
   CALL IMODBATCH_IFFCROSSSECTION()
 END SELECT
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER,DIMENSION(4) :: TMPINT1,TMPINT2

 CALL PMANAGER_UTL_INIT()

 IF(UTL_READINITFILE('RUNFILE_IN',LINE,IU,1))THEN

  READ(LINE,*) PBMAN%RUNFILE; WRITE(*,'(A,A)') 'RUNFILE_IN=',TRIM(PBMAN%RUNFILE)
  IF(.NOT.UTL_READINITFILE('PRJFILE_OUT',LINE,IU,0))RETURN
  READ(LINE,*) PBMAN%PRJFILE; WRITE(*,'(A,A)') 'PRJFILE_OUT=',TRIM(PBMAN%PRJFILE)
 
  IF(.NOT.PMANAGERRUN(ID_OPENRUN,PBMAN%RUNFILE,1)  )THEN; WRITE(*,'(/A/)') 'Error reading runfile '//TRIM(PBMAN%RUNFILE); STOP; ENDIF
  IF(.NOT.PMANAGERPRJ(ID_SAVE   ,PBMAN%PRJFILE,1,0))THEN; WRITE(*,'(/A/)') 'Error writing project file '//TRIM(PBMAN%PRJFILE); STOP; ENDIF

 ELSEIF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
 
  READ(LINE,*) PBMAN%PRJFILE; WRITE(*,'(2A)')  'PRJFILE_IN='//TRIM(PBMAN%PRJFILE)
  
  !## read parameters
  IF(.NOT.IMODBATCH_RUNFILE_READ(IU,1))STOP

  CALL WINDOWOPEN(FLAGS=SYSMENUON+HIDEWINDOW+STATUSBAR)
  TMPINT1=[2000,2000,750,-1]; TMPINT2=[1,1,1,1]
  CALL WINDOWSTATUSBARPARTS(4,TMPINT1,TMPINT2)

  IF(.NOT.PMANAGERPRJ(ID_OPEN   ,PBMAN%PRJFILE,1,0))THEN; WRITE(*,'(/A/)') 'Error reading project file '//TRIM(PBMAN%PRJFILE); STOP; ENDIF
  IF(.NOT.PMANAGERRUN(ID_SAVERUN,PBMAN%RUNFILE,1)  )THEN; WRITE(*,'(/A/)') 'Error writing runfile '//TRIM(PBMAN%RUNFILE); STOP; ENDIF
...
!!#####=================================================================
 ...
 REAL(KIND=DP_KIND) :: ZTOP,ZBOT,F
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: DZVAL

 I=3; ALLOCATE(IDF(I)); DO I=1,SIZE(IDF); CALL IDFNULLIFY(IDF(I)); ENDDO

 IF(.NOT.UTL_READINITFILE('TOPLEVELIDF',LINE,IU,0))RETURN
 READ(LINE,*)  IDF(1)%FNAME; WRITE(*,'(A)') 'TOPLEVELIDF='//TRIM(IDF(1)%FNAME)
 IF(UTL_READINITFILE('BOTLEVELIDF',LINE,IU,1))THEN
  READ(LINE,*) IDF(3)%FNAME; WRITE(*,'(A)') 'BOTLEVELIDF='//TRIM(IDF(3)%FNAME)
 ENDIF
 
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  READ(LINE,*) IDF(1)%XMIN,IDF(1)%YMIN,IDF(1)%XMAX,IDF(1)%YMAX
  WRITE(*,'(A,4F15.2)') 'WINDOW=',IDF(1)%XMIN,IDF(1)%YMIN,IDF(1)%XMAX,IDF(1)%YMAX
  IF(.NOT.UTL_READINITFILE('CELLSIZE',LINE,IU,0))RETURN
  READ(LINE,*) IDF(1)%DX; WRITE(*,'(A,4F10.2)') 'CELLSIZE=',IDF(1)%DX; IDF(1)%DY=IDF(1)%DX
  CALL UTL_IDFSNAPTOGRID_LLC(IDF(1)%XMIN,IDF(1)%XMAX,IDF(1)%YMIN,IDF(1)%YMAX,IDF(1)%DX,IDF(1)%DY,IDF(1)%NCOL,IDF(1)%NROW,.TRUE.)
 ENDIF
 
 IF(.NOT.IDFREADSCALE(IDF(1)%FNAME,IDF(1),2,1,0.0D0,0))THEN; RETURN; ENDIF
 IF(TRIM(IDF(3)%FNAME).NE.'')THEN
  CALL IDFCOPY(IDF(1),IDF(3))
  IF(.NOT.IDFREADSCALE(IDF(3)%FNAME,IDF(3),2,1,0.0D0,0))THEN; RETURN; ENDIF
 ENDIF
 ...
 IF(TRIM(IDF(3)%FNAME).NE.'')THEN
  DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
   ZTOP=IDF(1)%X(ICOL,IROW)
   ZBOT=IDF(3)%X(ICOL,IROW)
   !## compute F to fit the layers
   F=(ZTOP-ZBOT)/SUM(DZVAL)
   IDF(3)%X(ICOL,IROW)=F
  ENDDO; ENDDO
  IDF(3)%FNAME='D:\TMP.IDF'
  IF(.NOT.IDFWRITE(IDF(3),IDF(3)%FNAME,0))STOP
 ENDIF
 ...
 DO ILAY=1,NLAY
  DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
   ...
   IDF(2)%X(ICOL,IROW)=IDF(1)%X(ICOL,IROW)-DZVAL(ILAY)*F
  ...
  WRITE(*,'(A)') TRIM(ITOS(ILAY))//' of '//TRIM(ITOS(NLAY))//': INT_L'//TRIM(ITOS(ILAY))//'.IDF'
 ENDDO
 ...
 !###======================================================================
 USE MOD_TS_PAR, ONLY : IPFNAME1,IPFNAME2,TSILAY,SDATE,EDATE,TSDIR,LCOL,TXTCOL
 IMPLICIT NONE
 ...
 TXTCOL=2; IF(UTL_READINITFILE('TXTCOL',LINE,IU,1))READ(LINE,*) TXTCOL
 LINE=TRIM(ITOS(TXTCOL)); WRITE(*,'(A)') 'TXTCOL='//TRIM(LINE)
...
! WRITE(*,'(A,I2)') 'ITALUSCOL=',DATCOL(9)
 ...
 REAL(KIND=DP_KIND) :: T1,T2,B1,B2,T,B
 LOGICAL :: LEX
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: IDF
 ...
 IF(.NOT.UTL_READINITFILE('GENFILE_IN',LINE,IU,0))RETURN
 READ(LINE,*) GENFILE; WRITE(*,'(A)') 'GENFILE_IN='//TRIM(GENFILE)
 ...
 CALL ASC2IDF_INT_NULLIFY(); ALLOCATE(XP(100),YP(100),ZP(100),WP(100),PP(100),FP(100))
 ...
 N=0; DO IROW=1,IDF(3)%NROW; DO ICOL=1,IDF(3)%NCOL
   ...
   IF(ICOL.LT.IDF(3)%NCOL)THEN
    ...
    IF(I3D.EQ.1)THEN
     ...
     IF(T1.NE.IDF(1)%NODATA.AND.T2.NE.IDF(1)%NODATA.AND. &
      ...
      T=MAX(T1,T2); B=MIN(B1,B2); LEX=.TRUE.
     ENDIF
    ELSE
     LEX=.TRUE.; T=0.0D0; B=0.0D0
    ENDIF
    IF(LEX)THEN
     N=N+1; CALL PMANAGER_GENERATEMFNETWORKS_WRITEXY(1,JU,0,IPC,IDF(3),IROW,ICOL,N,I3D,T,B) 
    ENDIF
   ENDIF
  ENDIF
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   ...
   IF(IROW.LT.IDF(3)%NROW)THEN
    ...
    ENDIF
    IF(LEX)THEN
     N=N+1; CALL PMANAGER_GENERATEMFNETWORKS_WRITEXY(2,JU,0,IPC,IDF(3),IROW,ICOL,N,I3D,T,B) 
    ENDIF
   ENDIF
  ENDIF
...
!  WRITE(*,'(A,I5)') 'ICLAY=',ICLAY 
 ...
 !###======================================================================
 USE MOD_SOBEK_PAR, ONLY : ISGNAME,SOBEKDIR,CALCPNTHISNAME,STRUCHISNAME,IBATCH,DFLOWFMDIR,ISFINAL
 IMPLICIT NONE

 IBATCH=1

 IF(.NOT.UTL_READINITFILE('ISGNAME',LINE,IU,0))RETURN
 READ(LINE,'(A)') ISGNAME; WRITE(*,'(A)') 'ISGNAME='//TRIM(ISGNAME)
 
 IF(UTL_READINITFILE('ISFINAL',LINE,IU,1))THEN
  READ(LINE,*) ISFINAL; WRITE(*,'(A)') 'ISFINAL='//TRIM(ITOS(ISFINAL))
 ENDIF
 
 IF(UTL_READINITFILE('SOBEKDIR',LINE,IU,1))THEN
 ...
 ELSE
  IF(.NOT.UTL_READINITFILE('DFLOWFMDIR',LINE,IU,0))RETURN
  READ(LINE,'(A)') DFLOWFMDIR; WRITE(*,'(A)') 'DFLOWFMDIR='//TRIM(DFLOWFMDIR)
  IF(.NOT.DFLOWFM1CALC())THEN; ENDIF
 ENDIF
...
!
 ...
 !## correct layers
 IF(ICTB.EQ.1)THEN
  ALLOCATE(TP(NLAY)   ,BT(NLAY)   ,HK(NLAY)   ,VK(NLAY)   ,VA(NLAY)   ,IB(NLAY),TH(NLAY,2), &
           TP_BU(NLAY),BT_BU(NLAY),HK_BU(NLAY),VK_BU(NLAY),VA_BU(NLAY))
  DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
   ...
   CALL UTL_MINTHICKNESS(TP,BT,HK,VK,VA,TP_BU,BT_BU,HK_BU,VK_BU,VA_BU,IB,TH,MINTHICKNESS,NLAY,ICOL,IROW)
   DO ILAY=1,NLAY  ; IB(ILAY)=BND(ILAY)%X(ICOL,IROW); ENDDO
...
! ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ) :: IDF,SUM,PHR
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: FFF,FRF,FLF
 CHARACTER(LEN=256) :: HEDFNAME,BDGFNAME
 INTEGER :: JU,NCOL,NROW,NLAY,ILAY,KSTP,KPER,ICOL,IROW,IOS,I1,I2,I3,NDIM1,NDIM2,NDIM3, &
  IMETH,NDAT,NLIST,I,J,LENTXT,NCELLS,NTXT,NJA,N,M,IPOS,NCON,SDATE,NODE,JLAY,JROW,JCOL, & 
  NLAYERS
 INTEGER,DIMENSION(:),ALLOCATABLE :: ID1,ID2
 CHARACTER(LEN=16) :: TEXT,TXT1ID1,TXT1ID2,TXT2ID1,TXT2ID2,CDATE
 CHARACTER(LEN=50) :: HDR
 CHARACTER(LEN=16),ALLOCATABLE,DIMENSION(:) :: AUXTXT
 CHARACTER(LEN=256) :: ROOT
 REAL(KIND=DP_KIND) :: PERTIM,TOTIM,DELT,ANGROT,Q
 REAL(KIND=DP_KIND),DIMENSION(:,:,:),ALLOCATABLE :: X,Y
 CHARACTER(LEN=:),ALLOCATABLE :: STRING
 INTEGER,DIMENSION(:),ALLOCATABLE :: IA,JA
 INTEGER,DIMENSION(:),POINTER :: SAVESHD,SAVEWEL,SAVEDRN,SAVERIV,SAVEGHB,SAVERCH,SAVEEVT,SAVEFLX,SAVECHD,SAVESTO,SAVESPY
 LOGICAL :: LEX
 
 SDATE=0; IF(UTL_READINITFILE('SDATE',LINE,IU,1))THEN
  READ(LINE,*) SDATE; WRITE(*,'(A)') 'SDATE='//TRIM(ITOS(SDATE))
  SDATE=UTL_IDATETOJDATE(SDATE)
 ENDIF

 IF(.NOT.UTL_READPOINTER(IU,NLAYERS,SAVESHD,'SAVESHD',1))RETURN
 ...
 IF(.NOT.UTL_READPOINTER(IU,NLAYERS,SAVECHD,'SAVECHD',1))RETURN
 ...
 CALL IDFNULLIFY(IDF); CALL IDFNULLIFY(PHR); CALL IDFNULLIFY(SUM)
 IDF%ITYPE=8

 IF(UTL_READINITFILE('IDF',LINE,IU,1))THEN
  READ(LINE,*) IDF%FNAME; WRITE(*,'(A)') 'IDF='//TRIM(IDF%FNAME)
  IF(.NOT.IDFREAD(IDF,IDF%FNAME,1))STOP
 ELSEIF(UTL_READINITFILE('GRB',LINE,IU,1))THEN
  READ(LINE,*) IDF%FNAME; WRITE(*,'(A)') 'GRB='//TRIM(IDF%FNAME)
  JU=UTL_GETUNIT(); OPEN(JU,FILE=IDF%FNAME,ACTION='READ',ACCESS='STREAM',FORM='UNFORMATTED')
  READ(JU) HDR
  READ(JU) HDR
  READ(JU) HDR; READ(HDR(5:),*) NTXT
  READ(JU) HDR; READ(HDR(7:),*) LENTXT
  ALLOCATE(CHARACTER(LEN=LENTXT) :: STRING)
  DO I=1,NTXT; READ(JU) STRING; ENDDO
  READ(JU) NCELLS
  READ(JU) NLAY 
  READ(JU) IDF%NROW
  READ(JU) IDF%NCOL
  ...
  READ(JU) IDF%YMIN
  READ(JU) ANGROT
  ALLOCATE(IDF%SX(0:IDF%NCOL)); READ(JU) (IDF%SX(I),I=1,IDF%NCOL)
  ALLOCATE(IDF%SY(0:IDF%NROW)); READ(JU) (IDF%SY(I),I=1,IDF%NROW)
  ALLOCATE(X(IDF%NCOL*IDF%NROW,1,1)); READ(JU) (X(I,1,1),I=1,IDF%NCOL*IDF%NROW); DEALLOCATE(X)
  ALLOCATE(X(NCELLS,1,1)); READ(JU) (X(I,1,1),I=1,NCELLS); DEALLOCATE(X)
  ALLOCATE(IA(NCELLS+1)); READ(JU) (IA(I),I=1,NCELLS+1)
  ALLOCATE(JA(NJA)); READ(JU) (JA(I),I=1,NJA)
  DEALLOCATE(STRING)
  IDF%DX=IDF%SX(1); IDF%DY=IDF%SY(1)
  IDF%XMAX=IDF%XMIN+(IDF%NCOL*IDF%DX)
  IDF%YMAX=IDF%YMIN+(IDF%NROW*IDF%DY)
  IDF%NODATA=HUGE(1.0)
  IF(.NOT.IDFALLOCATEX(IDF))STOP 'CANNOT ALLOCATE MEMORY X IN IDF OBJECT'
  CLOSE(JU)
 ELSE
  STOP 'IDF OR GRB NEEDED'
 ENDIF
 
! CALL IDFCOPY(IDF,NLD); IF(.NOT.IDFREADSCALE('d:\IMOD-MODELS\LHM\NLD.IDF',NLD,7,1,0.0D0,0))STOP

 IF(UTL_READINITFILE('HED',LINE,IU,1))THEN
  CALL IDFCOPY(IDF,PHR); CALL IDFCOPY(IDF,SUM); SUM%NODATA=0.0D0
  READ(LINE,*) HEDFNAME; WRITE(*,'(A)') 'HED='//TRIM(HEDFNAME)
  ...
  DO
   ...
   IF(ILAY.EQ.1)THEN
    SUM%X=0.0; PHR%X=PHR%NODATA
   ENDIF
   
!   WRITE(*,*) KSTP,KPER,PERTIM,TOTIM,TEXT,NCOL,NROW,ILAY
   
   TEXT=ADJUSTL(TEXT)
   IF(IOS.NE.0)EXIT
   IDF%NODATA=1.0D30

   IF(KPER.EQ.1)THEN !PERTIM.EQ.0.0D0)THEN
    CDATE='STEADY-STATE'
   ELSE
    CDATE=JDATETOFDATE(TOTIM,SDATE,2)
   ENDIF

   READ(JU) ((IDF%X(ICOL,IROW),ICOL=1,NCOL),IROW=1,NROW)

   LEX=.FALSE.; IF(ASSOCIATED(SAVESHD))THEN; DO I=1,SIZE(SAVESHD); IF(SAVESHD(I).EQ.ILAY.OR.SAVESHD(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
   
   IF(LEX)THEN
    IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
    IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
   ENDIF
   
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
    IF(IDF%X(ICOL,IROW).NE.IDF%NODATA)THEN
     IF(SUM%X(ICOL,IROW).EQ.0.0D0)PHR%X(ICOL,IROW)=IDF%X(ICOL,IROW)
     SUM%X(ICOL,IROW)=SUM%X(ICOL,IROW)+1.0D0
    ENDIF
   ENDDO; ENDDO

   IF(ILAY.EQ.NLAY)THEN
    SUM%FNAME=TRIM(ROOT)//'\SUMACT_'//TRIM(CDATE)//'.IDF';   IF(.NOT.IDFWRITE(SUM,SUM%FNAME,1))STOP
    PHR%FNAME=TRIM(ROOT)//'\PHRLEVEL_'//TRIM(CDATE)//'.IDF'; IF(.NOT.IDFWRITE(PHR,PHR%FNAME,1))STOP
   ENDIF
 ...
 IF(UTL_READINITFILE('BDG',LINE,IU,1))THEN
  READ(LINE,*) BDGFNAME; WRITE(*,'(A)') 'BDG='//TRIM(BDGFNAME)
  ROOT=BDGFNAME(:INDEX(BDGFNAME,'\',.TRUE.)-1)
  JU=UTL_GETUNIT()
  OPEN(JU,FILE=BDGFNAME,STATUS='OLD',ACTION='READ',ACCESS='STREAM',FORM='UNFORMATTED') 
  DO
   READ(JU,IOSTAT=IOS) KSTP,KPER,TEXT,NDIM1,NDIM2,NDIM3; IF(IOS.NE.0)EXIT
   
!   WRITE(*,*) KSTP,KPER,TEXT,NDIM1,NDIM2,NDIM3
   
   TEXT=ADJUSTL(TEXT)

   READ(JU,IOSTAT=IOS) IMETH,DELT,PERTIM,TOTIM;          IF(IOS.NE.0)EXIT
   
!   IF(PERTIM.EQ.0.0D0)THEN
   IF(KPER.EQ.1)THEN
    CDATE='STEADY-STATE'
   ELSE
    TOTIM=TOTIM-1.0D0
   ...
   SELECT CASE (IMETH)
    !## intercell flow + storage
    CASE (1)
     ALLOCATE(X(NDIM1,NDIM2,ABS(NDIM3)))
     READ(JU) (((X(I1,I2,I3),I1=1,NDIM1),I2=1,NDIM2),I3=1,ABS(NDIM3))
     IF(TRIM(ADJUSTL(TEXT)).EQ.'FLOW-JA-FACE')THEN
      ALLOCATE(FFF(NLAY),FRF(NLAY),FLF(NLAY))
      DO ILAY=1,NLAY
       CALL IDFNULLIFY(FFF(ILAY));  CALL IDFNULLIFY(FRF(ILAY));  CALL IDFNULLIFY(FLF(ILAY))
       CALL IDFCOPY(IDF,FFF(ILAY)); CALL IDFCOPY(IDF,FRF(ILAY)); CALL IDFCOPY(IDF,FLF(ILAY))
       FLF(ILAY)%NODATA=1.0D30; FLF(ILAY)%X=0.0D0
       FFF(ILAY)%NODATA=1.0D30; FFF(ILAY)%X=0.0D0
       FRF(ILAY)%NODATA=1.0D30; FRF(ILAY)%X=0.0D0
      ENDDO
      DO N=1,NCELLS
!       WRITE(*,'(A,I10)') 'THIS IS CELL: ', N

       NODE=N; CALL UTL_GETIROWICOL(NODE,IDF%NROW,IDF%NCOL,ILAY,IROW,ICOL)

       NCON=IA(N+1)-IA(N)-1
       IF(NCON.LE.0) NCON=0
!       WRITE(*,'(A,4I10)') 'NUMBER OF CONNECTED CELLS IS CELL,ILAY,IROW,ICOL', NCON,ILAY,IROW,ICOL
       DO IPOS=IA(N)+1,IA(N+1)-1
        M=JA(IPOS)
        Q=X(IPOS,1,1) 
        NODE=M; CALL UTL_GETIROWICOL(NODE,IDF%NROW,IDF%NCOL,JLAY,JROW,JCOL)
        !## determine what face it is
        IF(JLAY.GT.ILAY)THEN
         FLF(ILAY)%X(ICOL,IROW)=Q 
        ELSEIF(JROW.GT.IROW)THEN
         FFF(ILAY)%X(ICOL,IROW)=Q 
        ELSEIF(JCOL.GT.ICOL)THEN
         FRF(ILAY)%X(ICOL,IROW)=Q 
        ENDIF
!        WRITE(*,'(A,2I10,F10.3)') ' N M Q: ', N,M,Q
      ...
      DO ILAY=1,NLAY 
       ...
       LEX=.FALSE.; IF(ASSOCIATED(SAVEFLX))THEN; DO I=1,SIZE(SAVEFLX); IF(SAVEFLX(I).EQ.ILAY.OR.SAVEFLX(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
   
       IF(LEX)THEN
        IDF%NODATA=1.0D30
        IF(ILAY.LT.NLAY)THEN 
         TEXT='BDGFLF'; IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
         IF(.NOT.IDFWRITE(FLF(ILAY),IDF%FNAME,1))STOP
        ENDIF
        TEXT='BDGFRF'; IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
        IF(.NOT.IDFWRITE(FRF(ILAY),IDF%FNAME,1))STOP
        TEXT='BDGFFF'; IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
        IF(.NOT.IDFWRITE(FFF(ILAY),IDF%FNAME,1))STOP
       ENDIF
      
      ENDDO
      CALL IDFDEALLOCATE(FFF,SIZE(FFF)); CALL IDFDEALLOCATE(FRF,SIZE(FRF)); CALL IDFDEALLOCATE(FLF,SIZE(FLF))
      DEALLOCATE(FFF,FRF,FLF)
     ELSE
      IF(TRIM(TEXT).EQ.'STO-SS')TEXT='STO'; IF(TRIM(TEXT).EQ.'STO-SY')TEXT='SPY'
      DO ILAY=1,ABS(NDIM3)
       LEX=.FALSE.
       SELECT CASE (TEXT)
        CASE ('WEL'); IF(ASSOCIATED(SAVEWEL))THEN; DO I=1,SIZE(SAVEWEL); IF(SAVEWEL(I).EQ.ILAY.OR.SAVEWEL(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
        CASE ('DRN'); IF(ASSOCIATED(SAVEDRN))THEN; DO I=1,SIZE(SAVEDRN); IF(SAVEDRN(I).EQ.ILAY.OR.SAVEDRN(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
        CASE ('RIV'); IF(ASSOCIATED(SAVERIV))THEN; DO I=1,SIZE(SAVERIV); IF(SAVERIV(I).EQ.ILAY.OR.SAVERIV(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
        CASE ('GHB'); IF(ASSOCIATED(SAVEGHB))THEN; DO I=1,SIZE(SAVEGHB); IF(SAVEGHB(I).EQ.ILAY.OR.SAVEGHB(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
        CASE ('RCH'); IF(ASSOCIATED(SAVERCH))THEN; DO I=1,SIZE(SAVERCH); IF(SAVERCH(I).EQ.ILAY.OR.SAVERCH(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
        CASE ('EVT'); IF(ASSOCIATED(SAVEEVT))THEN; DO I=1,SIZE(SAVEEVT); IF(SAVEEVT(I).EQ.ILAY.OR.SAVEEVT(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
        CASE ('CHD'); IF(ASSOCIATED(SAVECHD))THEN; DO I=1,SIZE(SAVECHD); IF(SAVECHD(I).EQ.ILAY.OR.SAVECHD(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
        CASE ('STO'); IF(ASSOCIATED(SAVESTO))THEN; DO I=1,SIZE(SAVESTO); IF(SAVESTO(I).EQ.ILAY.OR.SAVESTO(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
        CASE ('SPY'); IF(ASSOCIATED(SAVESPY))THEN; DO I=1,SIZE(SAVESPY); IF(SAVESPY(I).EQ.ILAY.OR.SAVESPY(I).EQ.-1)LEX=.TRUE.; ENDDO; ENDIF
       END SELECT
       IF(LEX)THEN
        DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL; IDF%X(ICOL,IROW)=X(ICOL,IROW,ILAY); ENDDO; ENDDO
        IDF%FNAME=TRIM(ROOT)//'\BDG'//TRIM(TEXT)//'\BDG'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
        IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
       ENDIF
      ENDDO
     ENDIF
    ...
    !## packages + connected flow to submodels
    CASE (6)

     READ(JU) TXT1ID1; READ(JU) TXT2ID1; READ(JU) TXT1ID2; READ(JU) TXT2ID2
     READ(JU) NDAT
     IF(NDAT.GT.1)THEN; ALLOCATE(AUXTXT(NDAT-1)); READ(JU) (AUXTXT(I),I=1,NDAT-1); ENDIF
     READ(JU) NLIST
     IF(NLIST.GT.0)THEN
!      write(*,*) txt1id1,txt2id1,txt1id2,txt2id2,nlist
      ALLOCATE(ID1(NLIST),ID2(NLIST),X(NDAT,NLIST,1))
      READ(JU) ((ID1(J),ID2(J),(X(I,J,1),I=1,NDAT)),J=1,NLIST)

      IDF%NODATA=1.0D30; ALLOCATE(Y(NDIM1,NDIM2,ABS(NDIM3))); Y=0.0D0
      CALL IMODBATCH_MF6TOIDF_FILLY(Y,SIZE(Y),ID1,NLIST,X(:,:,1),NDAT)
      IF(TRIM(UTL_CAP(TEXT,'U')).EQ.'FLOW-JA-FACE')THEN
       TEXT='BDG'//ADJUSTL(TXT1ID2)
      ELSE
       TEXT='BDG'//ADJUSTL(TXT2ID2) !TXT1ID2)
      ENDIF
      DO ILAY=1,ABS(NDIM3)
       LEX=.FALSE.
       SELECT CASE (TEXT)
       ...
       IF(LEX)THEN
        IDF%X=IDF%NODATA
        DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL; IF(Y(ICOL,IROW,ILAY).NE.0.0D0)IDF%X(ICOL,IROW)=Y(ICOL,IROW,ILAY); ENDDO; ENDDO
        IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
        IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
       ENDIF
      ENDDO
      DEALLOCATE(ID1,ID2,X,Y)
     ENDIF    
     IF(NDAT.GT.1)DEALLOCATE(AUXTXT)
    CASE DEFAULT
     WRITE(*,'(/A,I10/)') 'Cannot distinguish IMETH type ',IMETH; STOP
   END SELECT
  ENDDO
  CLOSE(JU)
 ENDIF
 ...
 !###======================================================================
 SUBROUTINE IMODBATCH_MF6TOIDF_FILLY(Y,NY,ID1,NLIST,X,NDAT)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NY,NLIST,NDAT
 REAL(KIND=DP_KIND),INTENT(OUT),DIMENSION(NY) :: Y
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(NDAT,NLIST) :: X
 INTEGER,INTENT(IN),DIMENSION(NLIST) :: ID1 
 INTEGER :: I

 DO I=1,NLIST
  IPOS=ID1(I); Y(IPOS)=Y(IPOS)+X(1,I)
 ENDDO

 END SUBROUTINE IMODBATCH_MF6TOIDF_FILLY
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: XMIN,YMIN,XMAX,YMAX,MINVALUE,MAXVALUE,STDV,AVERAGE
 INTEGER :: IN_TYPE,NSIM,ILOGTRANSFORM,I,IROW,ICOL
 TYPE(IDFOBJ) :: MEAN,STDEV
 ...
 ELLIPS_IDF%IU=0; ZONE_IDF%IU=0; IWFACTOR=0; ILOGTRANSFORM=0
 ...
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  ...
  READ(LINE,*) CS; WRITE(*,'(A,F10.3)') 'CS=',CS
 ENDIF

 ALLOCATE(XYZFNAMES(2)); XYZFNAMES='' 

 IF(.NOT.UTL_READINITFILE('NSIM',LINE,IU,0))RETURN
 READ(LINE,*) NSIM; WRITE(*,'(A,I10)') 'NSIM=',NSIM

 IF(UTL_READINITFILE('RANGE',LINE,IU,1))READ(LINE,*) RANGE
 WRITE(*,'(A,F10.2)') 'RANGE=',RANGE

 IF(.NOT.UTL_READINITFILE('LOGTRANSFORM',LINE,IU,0))RETURN; READ(LINE,*) ILOGTRANSFORM
 WRITE(*,'(A,I10)') 'LOGTRANSFORM=',ILOGTRANSFORM

 IF(.NOT.UTL_READINITFILE('STDEVIDF',LINE,IU,0))RETURN
 READ(LINE,*) STDEVIDF; WRITE(*,'(A)') 'STDEVIDF='//TRIM(STDEVIDF)

 !## read prior mean/stdev
 IF(UTL_READINITFILE('MEANFILE',LINE,IU,1))THEN
 
  READ(LINE,*) IDFFILE; WRITE(*,'(A)') 'MEANFILE='//TRIM(IDFFILE)  !## points from idf files

  !## read the mean/stdev again
  IF(.NOT.IDFREAD(MEAN,IDFFILE,1))STOP
  !## error standard-deviation (s=sqrt(variance)) - used as parameter variance=s^2
  CALL IDFCOPY(MEAN,STDEV)
  IF(.NOT.IDFREADSCALE(STDEVIDF,STDEV,2,1,0.0D0,0))STOP
 ...
 ELSE 
  ...
  IF(UTL_READINITFILE('IPFFILE',LINE,IU,1))THEN
   ...
   IF(.NOT.UTL_READINITFILE('IGRIDFUNC',LINE,IU,0))RETURN
   READ(LINE,*) IGRIDFUNC; WRITE(*,'(A,I2)') 'GRIDFUNC=',IGRIDFUNC
   IF(ABS(IGRIDFUNC).NE.6)STOP 'IGRIDFUNC NEED TO BE -6 (SK) OR 6 (OK)'
   ...
   IF(PNTSEARCH.EQ.1)THEN
    ...
    IF(IQUADRANT.EQ.0)THEN
     ...
     DO I=1,3
      IF(UTL_READINITFILE('ELLIPS_ANGLE'//TRIM(ITOS(I)),LINE,IU,1))READ(LINE,'(A)') ELLIPS_IDF(I,1)%FNAME
      IF(TRIM(ELLIPS_IDF(I,1)%FNAME).NE.'')WRITE(*,'(A)') 'ELLIPS_ANGLE'//TRIM(ITOS(I))//'='//TRIM(ELLIPS_IDF(I,1)%FNAME)
      IF(UTL_READINITFILE('ELLIPS_RANGE'//TRIM(ITOS(I)),LINE,IU,1))READ(LINE,'(A)') ELLIPS_IDF(I,2)%FNAME
      IF(TRIM(ELLIPS_IDF(I,2)%FNAME).NE.'')WRITE(*,'(A)') 'ELLIPS_RANGE'//TRIM(ITOS(I))//'='//TRIM(ELLIPS_IDF(I,2)%FNAME)
     ENDDO
    ENDIF
   ENDIF
   
   !## perform kriging to get stdev spatially
   IF(.NOT.ASC2IDF_INT_MAIN(1,1,XMIN,YMIN,XMAX,YMAX,IN_TYPE))STOP

   !## read the mean/stdev again
   IF(.NOT.IDFREAD(MEAN,IDFFILE,1))STOP
   !## error standard-deviation (s=sqrt(variance)) - used as parameter variance=s^2
   IF(.NOT.IDFREAD(STDEV,STDEVIDF,1))STOP
 
  !## create mean/stdev from min- and max values
  ELSE

   IF(.NOT.UTL_READINITFILE('MINVALUE',LINE,IU,0))RETURN; READ(LINE,*) MINVALUE
   WRITE(*,'(A,F10.2)') 'MINVALUE=',MINVALUE
   IF(.NOT.UTL_READINITFILE('MAXVALUE',LINE,IU,0))RETURN; READ(LINE,*) MAXVALUE
   WRITE(*,'(A,F10.2)') 'MAXVALUE=',MAXVALUE

   MEAN%XMIN=XMIN; MEAN%XMAX=XMAX; MEAN%YMIN=YMIN; MEAN%YMAX=YMAX; MEAN%DX=CS; MEAN%DY=MEAN%DX; MEAN%ITYPE=4
   CALL UTL_IDFSNAPTOGRID_LLC(MEAN%XMIN,MEAN%XMAX,MEAN%YMIN,MEAN%YMAX,MEAN%DX,MEAN%DX,MEAN%NCOL,MEAN%NROW)
   IF(.NOT.IDFALLOCATEX(MEAN))STOP
   CALL IDFCOPY(MEAN,STDEV)
   IF(.NOT.IDFALLOCATEX(STDEV))STOP
   IF(ILOGTRANSFORM.EQ.0)THEN
    AVERAGE=(MAXVALUE+MINVALUE)/2.0D0
    STDV   =(MAXVALUE-MINVALUE)/4.0D0
   ELSE
    AVERAGE=(LOG10(ABS(MAXVALUE))+LOG10(ABS(MINVALUE)))/2.0D0
    STDV   =(LOG10(ABS(MAXVALUE))-LOG10(ABS(MINVALUE)))/4.0D0
   ENDIF
   MEAN%X=AVERAGE; MEAN%NODATA =-999.99D0
   STDEV%X=STDV;   STDEV%NODATA=-999.99D0
   WRITE(*,*) 'AVERAGE,STDEV',AVERAGE,STDV
  ENDIF

 ENDIF
 
 !IF()THEN
!  DO IROW=1,STDEV%NROW; DO ICOL=1,STDEV%NCOL
!   IF(STDEV%X(ICOL,IROW).NE.STDEV%NODATA)STDEV%X(ICOL,IROW)=LOG10(STDEV%X(ICOL,IROW))
!   IF(MEAN%X(ICOL,IROW) .NE.MEAN%NODATA) MEAN%X(ICOL,IROW) =LOG10(MEAN%X(ICOL,IROW))
!  ENDDO; ENDDO
 !ENDIF
 ...
 IF(.NOT.UTL_READINITFILE('OUTPUTFOLDER',LINE,IU,0))RETURN
 READ(LINE,*) DIR; WRITE(*,'(A)') 'OUTPUTFOLDER='//TRIM(DIR) 

 CALL IMODBATCH_CREATEENSEMBLES_CHOLESKY(STDEV,MEAN,DIR,RANGE,NSIM,'ENSEMBLE',ILOGTRANSFORM)
 
 CALL IDFDEALLOCATEX(STDEV); CALL IDFDEALLOCATEX(MEAN)
 
 END SUBROUTINE IMODBATCH_CREATEENSEMBLES
 ...
 ELSE
  ...
  IF(UTL_READINITFILE('IDFFILE_IN',LINE,IU,1))THEN
   ...
   IF(UTL_READINITFILE('MASKIDF',LINE,IU,1))THEN
    READ(LINE,*) XYZFNAMES(2); WRITE(*,'(A)') 'MASKIDF='//TRIM(XYZFNAMES(2))
   ENDIF
  ENDIF
  IF(UTL_READINITFILE('GENFILE',LINE,IU,1))THEN
   READ(LINE,*) XYZFNAMES(1); WRITE(*,'(A)') 'GENFILE='//TRIM(XYZFNAMES(1)); IN_TYPE=4
  ENDIF
 ENDIF
 ...
 ILOG=0; IF(UTL_READINITFILE('ILOG',LINE,IU,1))THEN
  READ(LINE,*) ILOG; WRITE(*,'(A,I10)') 'ILOG=',ILOG
 ENDIF
 ...
 SELECT CASE (IGRIDFUNC)
  ...
  CASE (-6,6)
   ...
   !## no usage of ellipses, so specify range
   IF(TRIM(ELLIPS_IDF(1,1)%FNAME).EQ.'')THEN
    IF(.NOT.UTL_READPOINTER_REAL(IU,I,XYZRANGE,'RANGE',0))RETURN !,EXCLVALUE=0.0D0))RETURN
    DO I=1,SIZE(XYZRANGE)
     IF(XYZRANGE(I).LT.0.0D0)THEN
      LINE='RANGE('//TRIM(ITOS(I))//')='//TRIM(RTOS(XYZRANGE(I),'F',3))
      WRITE(*,'(A)') TRIM(LINE)//' which is not allowed'
     ELSEIF(XYZRANGE(I).EQ.0.0D0)THEN
      LINE='RANGE('//TRIM(ITOS(I))//')='//TRIM(RTOS(XYZRANGE(I),'F',3))
      WRITE(*,'(A)') TRIM(LINE)//' yield automatic determination of range'
     ENDIF
    ENDDO
   ELSE
    ALLOCATE(XYZRANGE(1)); XYZRANGE=0.0D0
   ENDIF
  ...
  CASE (7)
   IF(UTL_READINITFILE('LAGINTERVAL',LINE,IU,1))THEN
    READ(LINE,*) LAGINTERVAL; WRITE(*,*) 'LAGINTERVAL=',LAGINTERVAL
   ELSE
    WRITE(*,*) 'LAGINTERVAL IS COMPUTED AUTOMATICALLY BASED ON THE DATA'
   ENDIF
   IF(UTL_READINITFILE('LAGDISTANCE',LINE,IU,1))THEN
    READ(LINE,*) LAGDISTANCE; WRITE(*,*) 'LAGDISTANCE=',LAGDISTANCE
   ELSE
    WRITE(*,*) 'LAGDISTANCE IS COMPUTED AUTOMATICALLY BASED ON THE DATA'
   ENDIF
  CASE (8)
  ...
  IF(ASSF_INDICATOR.GE.0)THEN
   ...
   DO I=1,ASSF_NTHRESHOLD
    ...
    DO   
     ...
     MAXPNT=MAXPOINT
     IF(.NOT.ASC2IDF_INT_MAIN(1,1,XMIN,YMIN,XMAX,YMAX,IN_TYPE))THEN; ENDIF
  ...
  DO 
   ...
   DO I=1,N
    ...
    SELECT CASE (UTL_CAP(EXT,'U'))
     ...
     CASE ('GEN'); IN_TYPE=4
    END SELECT
    IF(.NOT.ASC2IDF_INT_MAIN(1,I,XMIN,YMIN,XMAX,YMAX,IN_TYPE))THEN
    ENDIF
   ENDDO
 ...
 !###======================================================================
 LOGICAL FUNCTION IMODBATCH_IPFSAMPLE_MAIN(OUTIPF,ICOLS,NLAY)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(OUT),OPTIONAL :: OUTIPF
 INTEGER,INTENT(OUT),DIMENSION(:),OPTIONAL :: ICOLS
 INTEGER,INTENT(OUT),OPTIONAL :: NLAY
 CHARACTER(LEN=256),DIMENSION(3) :: FNAME
 INTEGER :: IXCOL,IYCOL,IACOL,ILCOL,IMCOL,NFILES
 REAL(KIND=DP_KIND) :: NODATA
 
 IMODBATCH_IPFSAMPLE_MAIN=.FALSE.
 
 IF(.NOT.UTL_READINITFILE('IPFFILE_IN',LINE,IU,0))RETURN
 READ(LINE,*) FNAME(1); WRITE(*,'(A)') 'IPFFILE_IN='//TRIM(FNAME(1))

 IXCOL=1; IF(UTL_READINITFILE('IXCOL',LINE,IU,1))READ(LINE,*) IXCOL
 WRITE(*,'(A,I2)') 'IXCOL=',IXCOL; IF(PRESENT(ICOLS))ICOLS(1)=IXCOL
 IYCOL=2; IF(UTL_READINITFILE('IYCOL',LINE,IU,1))READ(LINE,*) IYCOL
 WRITE(*,'(A,I2)') 'IYCOL=',IYCOL; IF(PRESENT(ICOLS))ICOLS(2)=IYCOL
 !## column for the insertion
 IACOL=0; IF(UTL_READINITFILE('IACOL',LINE,IU,1))READ(LINE,*) IACOL
 WRITE(*,'(A,I2)') 'IACOL=',IACOL; IF(PRESENT(ICOLS))ICOLS(3)=IACOL
 IF(PRESENT(ICOLS))THEN
  !## measurement column
  IMCOL=0; IF(UTL_READINITFILE('IMCOL',LINE,IU,1))READ(LINE,*) IMCOL
  WRITE(*,'(A,I2)') 'IMCOL=',IMCOL; ICOLS(4)=IMCOL
  !## layer column
  ILCOL=0; IF(UTL_READINITFILE('ILCOL',LINE,IU,1))READ(LINE,*) ILCOL
  WRITE(*,'(A,I2)') 'ILCOL=',ILCOL; ICOLS(5)=ILCOL
 ENDIF
 ...
 NODATA=-999.99D0
 IF(UTL_READINITFILE('NODATA',LINE,IU,1))READ(LINE,*) NODATA
 WRITE(*,'(A)') 'NODATA='//TRIM(RTOS(NODATA,'F',7))

 CALL IPFSAMPLE(FNAME(1),FNAME(2),FNAME(3),IXCOL,IYCOL,IACOL,NODATA,NFILES)
 IF(PRESENT(NLAY))NLAY=NFILES
 IF(PRESENT(OUTIPF))OUTIPF=FNAME(2)
 
 IMODBATCH_IPFSAMPLE_MAIN=.TRUE.
 
 END FUNCTION IMODBATCH_IPFSAMPLE_MAIN
...
! OPTIONAL KEYWORDS
 ...
 IF(UTL_READINITFILE('ICDIST',LINE,IU,1))READ(LINE,*) GRIDISG%ICDIST
 WRITE(*,'(A,99I1)') 'ICDIST=',GRIDISG%ICDIST
 ...
 !###======================================================================
 SUBROUTINE IMODBATCH_IPESTTOPARAM()
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=256) :: PRJFNAME,FNAME,OUTPUTFOLDER
 INTEGER :: I,J,K,N,M,NU,IROW,ICOL,IZ,IP,JZ
 REAL(KIND=DP_KIND) :: F,P
 CHARACTER(LEN=2),DIMENSION(:),ALLOCATABLE :: UP
 INTEGER,DIMENSION(:,:),ALLOCATABLE :: ILS
 INTEGER,DIMENSION(:),ALLOCATABLE :: NLS
 TYPE(IDFOBJ),DIMENSION(:,:),ALLOCATABLE :: IDF
 TYPE(IDFOBJ) :: ZIDF
 
 PRJFNAME=''; IF(.NOT.UTL_READINITFILE('PRJFNAME',LINE,IU,0))RETURN
 READ(LINE,*) PRJFNAME; WRITE(*,'(A)') 'PRJFNAME='//TRIM(PRJFNAME)
 OUTPUTFOLDER=''; IF(.NOT.UTL_READINITFILE('OUTPUTFOLDER',LINE,IU,0))RETURN
 READ(LINE,*) OUTPUTFOLDER; WRITE(*,'(A)') 'OUTPUTFOLDER='//TRIM(OUTPUTFOLDER)
 
 CALL PMANAGER_UTL_INIT()
 IF(.NOT.PMANAGER_LOADPRJ(PRJFNAME,1))STOP 'ERROR READING PRJFILE'

 !## find unique stuff
 N=SIZE(PEST%PARAM); ALLOCATE(UP(N)); DO I=1,SIZE(PEST%PARAM); UP(I)=PEST%PARAM(I)%PPARAM; ENDDO
 CALL UTL_GETUNIQUE_CHAR(UP,N,NU)
 ALLOCATE(NLS(NU))
 DO J=1,NU; NLS(J)=0; DO I=1,N; IF(UP(J).EQ.PEST%PARAM(I)%PPARAM)NLS(J)=NLS(J)+1; ENDDO; ENDDO
 ALLOCATE(ILS(NU,MAXVAL(NLS)))
 DO J=1,NU; NLS(J)=0; DO I=1,N; IF(UP(J).EQ.PEST%PARAM(I)%PPARAM)THEN; NLS(J)=NLS(J)+1; ILS(J,NLS(J))=PEST%PARAM(I)%PILS; ENDIF; ENDDO; ENDDO
 DO J=1,NU; CALL UTL_GETUNIQUE_INT(ILS(J,:),NLS(J),M); NLS(J)=M; ENDDO
 WRITE(*,'(/1X,A)') 'Unique parameters found that can be processed:'
 DO I=1,NU; WRITE(*,'(1X,99A)') UP(I),(','//TRIM(ITOS(ILS(I,J))),J=1,NLS(I)); ENDDO
 !## find unqique layers/systems per parameter
 CALL IDFNULLIFY(ZIDF); ALLOCATE(IDF(NU,MAXVAL(NLS))); DO I=1,SIZE(IDF,1); DO J=1,SIZE(IDF,2); CALL IDFNULLIFY(IDF(I,J)); ENDDO; ENDDO
 !## use first zone to dimension idf
 DO I=1,SIZE(PEST%IDFFILES); IF(INDEX(UTL_CAP(PEST%IDFFILES(I),'U'),'.IDF').GT.0)EXIT; ENDDO
 IF(I.GT.SIZE(PEST%IDFFILES))STOP 'NEED TO FIND IDFFILES IN ZONES'
 IF(.NOT.IDFREAD(ZIDF,PEST%IDFFILES(I),0))THEN; WRITE(*,'(/1X,A/)') 'CANNOT READ '//TRIM(PEST%IDFFILES(I)); STOP; ENDIF
 IF(.NOT.IDFALLOCATEX(ZIDF))STOP
 DO I=1,SIZE(IDF,1); DO J=1,NLS(I); CALL IDFCOPY(ZIDF,IDF(I,J)); ENDDO; ENDDO
 
 DO I=1,SIZE(IDF,1); DO J=1,NLS(I); IF(.NOT.IDFALLOCATEX(IDF(I,J)))STOP 'CANNOT ALLOCATE MEMORY'; IDF(I,J)%X=0.0D0; ENDDO; ENDDO
 !## process each zone
 DO I=1,SIZE(PEST%IDFFILES)
  IF(.NOT.IDFREAD(ZIDF,PEST%IDFFILES(I),1))THEN; WRITE(*,'(/1X,A/)') 'CANNOT READ '//TRIM(PEST%IDFFILES(I)); STOP; ENDIF
  JZ=0; DO IROW=1,ZIDF%NROW; DO ICOL=1,ZIDF%NCOL
   IF(ZIDF%X(ICOL,IROW).EQ.ZIDF%NODATA)CYCLE
   IZ=INT(ZIDF%X(ICOL,IROW)); F=MOD(ZIDF%X(ICOL,IROW),1.0D0); IF(F.EQ.0.0D0)F=1.0D0
   !## find new zone
   DO IP=1,SIZE(PEST%PARAM)
    IF(PEST%PARAM(IP)%PIZONE.NE.IZ)CYCLE
    !## find idf to be add values
    DO J=1,NU; IF(UP(J).EQ.PEST%PARAM(IP)%PPARAM)EXIT; ENDDO
    !## find ils
    DO K=1,NLS(J); IF(ILS(J,K).EQ.PEST%PARAM(IP)%PILS)EXIT; ENDDO
    P=F*PEST%PARAM(IP)%PINI
    IDF(J,K)%X(ICOL,IROW)=IDF(J,K)%X(ICOL,IROW)+P
   ENDDO
  ENDDO; ENDDO
  F=DBLE(I)/SIZE(PEST%IDFFILES)*100.0D0; WRITE(6,'(A,F10.2,A)') '+Progress ',F,' ...            '
 ENDDO
 DO I=1,SIZE(IDF,1); DO J=1,NLS(I)
  DO IROW=1,IDF(I,J)%NROW; DO ICOL=1,IDF(I,J)%NCOL
   IF(IDF(I,J)%X(ICOL,IROW).EQ.0.0D0)IDF(I,J)%X(ICOL,IROW)=1.0D0
  ENDDO; ENDDO
  FNAME=TRIM(OUTPUTFOLDER)//'\'//TRIM(UP(I))//'_F_LS'//TRIM(ITOS(ILS(I,J)))//'.IDF'
  IF(.NOT.IDFWRITE(IDF(I,J),FNAME,1))THEN; WRITE(*,'(/A/)') 'ERROR WRITING '//TRIM(FNAME); CYCLE; ENDIF
  WRITE(*,'(A)') 'WRITING '//TRIM(FNAME)
 ENDDO; ENDDO
 
 END SUBROUTINE IMODBATCH_IPESTTOPARAM
 
 !###======================================================================
 SUBROUTINE IMODBATCH_IFFCROSSSECTION()
 !###======================================================================
 IMPLICIT NONE
 ...
 IF(.NOT.UTL_READINITFILE('IFFNAME',LINE,IU,0))RETURN
 READ(LINE,*) IFFNAME; WRITE(*,'(A)') 'IFFNAME='//TRIM(IFFNAME)
 IF(.NOT.UTL_READINITFILE('GENNAME',LINE,IU,0))RETURN
 ...
 !## allocate and open IFF
 IU=UTL_GETUNIT()
 CALL OSD_OPEN(IU,FILE=IFFNAME,STATUS='OLD',FORM='FORMATTED',ACTION='READ,DENYWRITE',IOSTAT=IOS)
 IF(IOS.NE.0) RETURN
 READ(IU,*) NC ; N=NC-5
 ALLOCATE(IFF(2))
 ...
 !## allocate and open GEN
 CALL POLYGON1INIT()
 CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,GENNAME,'GEN')

 JU=UTL_GETUNIT()
 CALL OSD_OPEN(JU,FILE=IPFNAME,STATUS='UNKNOWN',FORM='FORMATTED',ACTION='WRITE',IOSTAT=IOS,IQUESTION=0)
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot write file:'//CHAR(13)//TRIM(IPFNAME),'Error')
  RETURN
 ENDIF
 DO I=1,2 !## two loops: 1) get NCPOINTS and 2) write IPF
  !## read IFF header   
  REWIND(IU) ; READ(IU,*) ; DO J=1,NC; READ(IU,*); END DO
  ...
  IF(I.EQ.2)THEN   
   ...
   Write(JU,*) "YINTER"
   Write(JU,*) "ZINTER"
   Write(JU,*) "PARTICLE_NUMBER"
   Write(JU,*) "ILAY-FROM"
   Write(JU,*) "ILAY-TO"
   Write(JU,*) "TIME-FROM(YEARS)"
   Write(JU,*) "TIME-TO(YEARS)"
   Write(JU,*) "0 TXT"
  ENDIF
  ...
  DO
   ...
   IF(IFF(1)%IPART.EQ.IFF(2)%IPART)THEN
     ...
     DO IPOL=1,SIZE(SHP%POL) 
      DO ISEG=1,SIZE(SHP%POL(IPOL)%X)-1 
       !## calculate intersection    
       CALL DBL_IGRINTERSECTLINE(SHP%POL(IPOL)%X(ISEG)  ,SHP%POL(IPOL)%Y(ISEG)  , &
                                 SHP%POL(IPOL)%X(ISEG+1),SHP%POL(IPOL)%Y(ISEG+1), &
                                 IFF(1)%X,IFF(1)%Y, &
                                 IFF(2)%X,IFF(2)%Y, &
...
END SUBROUTINE IMODBATCH_IFFCROSSSECTION

 !###======================================================================
 SUBROUTINE IMODBATCH_IMODPATH7_MAIN()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: JU,KU,NLAY,I,J,K,IOS,ILAY,ID
 CHARACTER(LEN=256) :: MF6DIR,MODPATH7,LINE
 REAL(KIND=DP_KIND) :: XLL,YLL,X1,Y1,X2,Y2,X,Y,Z,T,XD
 TYPE(IDFOBJ) :: MODELIDF
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: TOP,BOT
 
 CALL IDFNULLIFY(MODELIDF)
 
 MF6DIR=''; IF(.NOT.UTL_READINITFILE('MF6DIR',LINE,IU,0))RETURN
 READ(LINE,*) MF6DIR; WRITE(*,'(A)') 'MF6DIR='//TRIM(MF6DIR)
 MODPATH7=''; IF(.NOT.UTL_READINITFILE('MODPATH7',LINE,IU,0))RETURN
 READ(LINE,*) MODPATH7; WRITE(*,'(A)') 'MODPATH7='//TRIM(MODPATH7)
 NSPFNAME=1; ALLOCATE(SPFNAME(NSPFNAME)); SPFNAME(1)=''
 IF(.NOT.UTL_READINITFILE('ISDNAME',LINE,IU,0))RETURN
 READ(LINE,*) SPFNAME(1); WRITE(*,'(A)') 'ISDNAME='//TRIM(SPFNAME(1))
 IF(.NOT.UTL_READINITFILE('NLAY',LINE,IU,0))RETURN
 READ(LINE,*) NLAY; WRITE(*,'(A,I10)') 'NLAY=',NLAY
 
 !## generate mf6 namfile
 CALL UTL_CREATEDIR(TRIM(MF6DIR)//'\MP7')
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7\MP7.MPNAM',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') 'MPBAS     '//'MP7.MPBAS'
 WRITE(JU,'(A)') 'GRBDIS    '//'..\GWF_1\MODELINPUT\MODEL_MF6.DIS6.GRB'
 WRITE(JU,'(A)') 'TDIS      '//'..\MFSIM.TDIS6'
 WRITE(JU,'(A)') 'HEAD      '//'..\GWF_1\MODELOUTPUT\HEAD\HEAD.HED'
 WRITE(JU,'(A)') 'BUDGET    '//'..\GWF_1\MODELOUTPUT\BUDGET\BUDGET.CBC'
 CLOSE(JU)
 
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7\MP7.MPSIM',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') 'MP7.MPNAM'
 WRITE(JU,'(A)') 'MP7.MPLIST'
 WRITE(JU,'(A)') '2 2 1 1 0 0' !## pathlines(2),backward(2),pass weaksinks(1),pass weaksources(1),no checks(0),no detailed info(0)
 WRITE(JU,'(A)') 'MP7.ENDPOINT7'
 WRITE(JU,'(A)') 'MP7.PATHLINE7'
! WRITE(JU,'(A)') 'MP7.TIMESERIES7'
 WRITE(JU,'(A)') '0'   !## no balance checking
 WRITE(JU,'(A)') '1'   !## reference time
 WRITE(JU,'(A)') '0.0' !## initial time
 WRITE(JU,'(A)') '2'   !## steady-state tracking simulation
 WRITE(JU,'(A)') '1'   !## no stoppping zone
 WRITE(JU,'(A)') '1'   !## no retardation
 WRITE(JU,'(A)') '1'   !## particle group count
 WRITE(JU,'(A)') 'PG1' !## particle group
 WRITE(JU,'(A)') '1'   !## single value of particle release time'
 WRITE(JU,'(A)') '0.0' !## release time'
 WRITE(JU,'(A)') 'EXTERNAL MP7.SLOC'
 CLOSE(JU)
 
 !## read the master of idf
 MODELIDF%FNAME=TRIM(MF6DIR)//'\GWF_1\MODELINPUT\DIS6\BND_L1.IDF'
 IF(.NOT.IDFREAD(MODELIDF,MODELIDF%FNAME,0))STOP

 CALL UTL_CREATEDIR(OUTDIR)
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7\MP7.MPBAS',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') '0' !## facecount
 IF(.NOT.IDFALLOCATEX(MODELIDF))STOP
 !## read and save porosity
 DO I=1,NLAY
  IF(.NOT.UTL_READINITFILE('POR_L'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) MODELIDF%FNAME; WRITE(*,'(A)') 'POR_L'//TRIM(ITOS(I))//'='//TRIM(MODELIDF%FNAME)
  READ(MODELIDF%FNAME,*,IOSTAT=IOS) X
  IF(IOS.EQ.0)THEN
   MODELIDF%X=X
  ELSE
   IF(.NOT.IDFREADSCALE(MODELIDF%FNAME,MODELIDF,2,1,0.0D0,0))RETURN !## scale mean
  ENDIF
  KU=UTL_GETUNIT(); OPEN(KU,FILE=TRIM(MF6DIR)//'\MP7\POR_L'//TRIM(ITOS(I))//'.ARR',STATUS='UNKNOWN',ACTION='WRITE')
  CALL IDFWRITEFREE(KU,MODELIDF,0,'B','*')
  CLOSE(KU)
  WRITE(JU,'(A)') 'OPEN/CLOSE POR_L'//TRIM(ITOS(I))//'.ARR 1.0D0 (FREE) -1'
 ENDDO
 ...
 !## read all top/bot
 DO I=1,NLAY
  TOP(I)%FNAME=TRIM(MF6DIR)//'\GWF_1\MODELINPUT\DIS6\TOPM_L'//TRIM(ITOS(I))//'.IDF'
  IF(.NOT.IDFREAD(TOP(I),TOP(I)%FNAME,1))STOP
  BOT(I)%FNAME=TRIM(MF6DIR)//'\GWF_1\MODELINPUT\DIS6\BOTM_L'//TRIM(ITOS(I))//'.IDF'
  IF(.NOT.IDFREAD(BOT(I),BOT(I)%FNAME,1))STOP
 ENDDO
 !## copy this into the idf of trace
 CALL TRACE_BRING_IN_DATA(MODELIDF,NLAY,TOP,BOT)
 !## initialize startpoints memory
 CALL TRACE_INIT_SP(1); ISPFNAME=1
 !## read/process particles towards readable format
 IF(.NOT.TRACEPREPARESP(SP(ISPFNAME)%NPART,1,1))RETURN 
 !## read all particle in memory
 CALL TRACE_AL_SP(SP(ISPFNAME)%NPART,1) 
 !## set initial time for each particle to zero
 CALL TRACEREADSP(1,SP(ISPFNAME)%NPART)

 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7\MP7.SLOC',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') '1' !## inputstyle
 WRITE(JU,'(A)') '1' !## locationstyle
 WRITE(JU,'(A)') TRIM(ITOS((SP(1)%NPART)))//' 1' !## particlecount,particleidoption
 !## write startpoints
 DO I=1,SP(1)%NPART
  !## compute local coordinates
 CALL IDFGETEDGE(MODELIDF,SP(1)%ILC(I),SP(1)%JLC(I),X1,Y1,X2,Y2)
  XLL=((MODELIDF%XMIN+SP(1)%XLC(I))-X1)/(X2-X1)
  YLL=((MODELIDF%YMIN+SP(1)%YLC(I))-Y1)/(Y2-Y1)
  WRITE(JU,'(A)') TRIM(ITOS(I))//','// &
                  TRIM(ITOS(SP(1)%KLC(I)))//','//TRIM(ITOS(SP(1)%ILC(I)))//','//TRIM(ITOS(SP(1)%JLC(I)))       //','// &
                  TRIM(RTOS(XLL,'F',2))   //','//TRIM(RTOS(YLL,'F',2))   //','//TRIM(RTOS(SP(1)%ZLL(I),'F',2)) //','// &
                  TRIM(RTOS(0.0D0,'F',2)) //','//TRIM(ITOS(0))
 ENDDO
 CLOSE(JU)
 
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7\MP7.BAT',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') TRIM(MODPATH7)//' < MP7_ARGUMENT.TXT'
 CLOSE(JU) 
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7\MP7_ARGUMENT.TXT',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') 'MP7.MPSIM'
 CLOSE(JU) 
 
 CALL IDFDEALLOCATE(TOP,SIZE(TOP)); CALL IDFDEALLOCATE(BOT,SIZE(BOT))

 CALL IOSDIRCHANGE(TRIM(MF6DIR)//'\MP7')
 CALL IOSCOMMAND(TRIM(MF6DIR)//'\MP7\MP7.BAT',IFLAGS=PROCBLOCKED)

 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(MF6DIR)//'\MP7\MP7.PATHLINE7',STATUS='OLD'    ,ACTION='READ' )
 JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(MF6DIR)//'\MP7\PATHLINE7.IFF',STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') '6'
 WRITE(JU,'(A)') 'PARTICLE_NUMBER'
 WRITE(JU,'(A)') 'ILAY'
 WRITE(JU,'(A)') 'XCRD.'
 WRITE(JU,'(A)') 'YCRD.'
 WRITE(JU,'(A)') 'ZCRD.'
 WRITE(JU,'(A)') 'TIME(YEARS)'
 DO I=1,3; READ(IU,*); ENDDO; ID=0
 DO
  ...
  ID=ID+1
  DO I=1,N
   READ(IU,*) J,X,Y,Z,T,XD,XD,XD,ILAY
   X=X+TOP(1)%XMIN
   Y=Y+TOP(1)%YMIN
   WRITE(JU,'(A)') TRIM(ITOS(ID     ))//','//TRIM(ITOS(ILAY   ))//','//TRIM(RTOS(X,'F',3))//','//TRIM(RTOS(Y,'F',3))//','// &
                   TRIM(RTOS(Z,'F',3))//','//TRIM(RTOS(T,'G',7))//','//TRIM(ITOS(ILAY   ))
  ENDDO
 ENDDO
 CLOSE(IU); CLOSE(JU)
 
 END SUBROUTINE IMODBATCH_IMODPATH7_MAIN
 ...
 !###======================================================================
 SUBROUTINE IMODBATCH_LAYERFROMTHICKNESS()
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: THK
 TYPE(IDFOBJ) :: MOTHER,TOP,BOT,SUM
 CHARACTER(LEN=256) :: OUTPUTFOLDER
 INTEGER :: NLAY,IOS,I,J,IROW,ICOL
 REAL(KIND=DP_KIND) :: X,MINTHICKNESS,ADDTOPLAYER
 ...
 !## minimal thickness
 MINTHICKNESS=0.0D0; IF(UTL_READINITFILE('MINTHICKNESS',LINE,IU,1))READ(LINE,*) MINTHICKNESS
 WRITE(*,'(A,F10.2)') 'MINTHICKNESS=',MINTHICKNESS

 ADDTOPLAYER=0.0D0; IF(UTL_READINITFILE('ADDTOPLAYER',LINE,IU,1))READ(LINE,*) ADDTOPLAYER
 WRITE(*,'(A,F10.2)') 'ADDTOPLAYER=',ADDTOPLAYER

 !## result folder
 IF(.NOT.UTL_READINITFILE('OUTPUTFOLDER',LINE,IU,0))RETURN
 READ(LINE,*) OUTPUTFOLDER; WRITE(*,'(A)') 'OUTPUTFOLDER='//TRIM(OUTPUTFOLDER)

 CALL IDFNULLIFY(MOTHER); CALL IDFNULLIFY(SUM)
 ALLOCATE(THK(NLAY)); DO I=1,SIZE(THK); CALL IDFNULLIFY(THK(I)); ENDDO
  
 IF(.NOT.UTL_READINITFILE('TOP',LINE,IU,0))RETURN
 READ(LINE,*) TOP%FNAME; LINE='TOP='//TRIM(TOP%FNAME); WRITE(*,'(A)') TRIM(LINE)

 DO I=1,NLAY
  IF(.NOT.UTL_READINITFILE('THK_L'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) THK(I)%FNAME
  LINE='THK_L'//TRIM(ITOS(I))//'='//TRIM(THK(I)%FNAME); WRITE(*,'(A)') TRIM(LINE)
 ENDDO
 ...
 ELSE
  ...
  THK%NROW=0; THK%NCOL=0
  DO I=1,SIZE(THK)
   READ(THK(I)%FNAME,*,IOSTAT=IOS) X
   IF(IOS.NE.0)THEN; IF(.NOT.IDFREAD(THK(I),THK(I)%FNAME,0))RETURN; ENDIF
  ENDDO 
  !## get maximal extent
  IF(.NOT.IDF_EXTENT(SIZE(THK),THK,MOTHER,1))RETURN
 ENDIF 

 CALL IDFCOPY(MOTHER,TOP)
 WRITE(*,'(A)') ' Reading '//TRIM(TOP%FNAME)
 READ(TOP%FNAME,*,IOSTAT=IOS) X
 IF(IOS.EQ.0)THEN
  IF(.NOT.IDFALLOCATEX(TOP))RETURN; TOP%X=X; TOP%NODATA=X
 ELSE
  IF(.NOT.IDFREADSCALE(TOP%FNAME,TOP,2,1,0.0D0,0))RETURN
 ENDIF
 CALL IDFCOPY(TOP,BOT); CALL IDFCOPY(TOP,SUM)
 IF(ADDTOPLAYER.GT.0.0D0)BOT%X=BOT%X-ADDTOPLAYER
 
 IF(ADDTOPLAYER.GT.0.0D0)THEN
  TOP%FNAME=TRIM(OUTPUTFOLDER)//'\TOP_L1.IDF'
  IF(.NOT.IDFWRITE(TOP,TOP%FNAME,1))RETURN
  BOT%FNAME=TRIM(OUTPUTFOLDER)//'\BOT_L1.IDF'
  IF(.NOT.IDFWRITE(BOT,BOT%FNAME,1))RETURN
  TOP%X=BOT%X
 ENDIF
 ...
 DO I=1,SIZE(THK)
  CALL IDFCOPY(MOTHER,THK(I))
  WRITE(*,'(A)') ' Reading '//TRIM(THK(I)%FNAME)
  READ(THK(I)%FNAME,*,IOSTAT=IOS) X
  IF(IOS.EQ.0)THEN
   IF(.NOT.IDFALLOCATEX(THK(I)))RETURN; THK(I)%X=X; THK(I)%NODATA=X
  ELSE
   IF(.NOT.IDFREADSCALE(THK(I)%FNAME,THK(I),2,1,0.0D0,0))RETURN
  ENDIF
 
  !## compute top and bottom
  DO IROW=1,MOTHER%NROW; DO ICOL=1,MOTHER%NCOL
   IF(TOP%X(ICOL,IROW).NE.TOP%NODATA)THEN
    TOP%X(ICOL,IROW)=BOT%X(ICOL,IROW)
    IF(THK(I)%X(ICOL,IROW).LT.MINTHICKNESS)THK(I)%X(ICOL,IROW)=0.0D0
    
    !## added enough thickness to start filling in a layer
    IF(SUM%X(ICOL,IROW).LT.ADDTOPLAYER)THEN
     SUM%X(   ICOL,IROW)=SUM%X(   ICOL,IROW)+THK(I)%X(ICOL,IROW)
!     MAJ%X(   ICOL,IROW)=REAL(I,8)
     THK(I)%X(ICOL,IROW)=THK(I)%X(ICOL,IROW)-ADDTOPLAYER
     THK(I)%X(ICOL,IROW)=MAX(0.0D0,THK(I)%X(ICOL,IROW))
    ENDIF
    
    BOT%X(ICOL,IROW)=TOP%X(ICOL,IROW)-THK(I)%X(ICOL,IROW)
   ENDIF
  ENDDO; ENDDO
 
  J=0; IF(ADDTOPLAYER.GT.0.0D0)J=1
  TOP%FNAME=TRIM(OUTPUTFOLDER)//'\TOP_L'//TRIM(ITOS(I+J))//'.IDF'
  IF(.NOT.IDFWRITE(TOP,TOP%FNAME,1))RETURN
  BOT%FNAME=TRIM(OUTPUTFOLDER)//'\BOT_L'//TRIM(ITOS(I+J))//'.IDF'
  IF(.NOT.IDFWRITE(BOT,BOT%FNAME,1))RETURN
 ...
 USE MOD_CUS_PAR, ONLY : REGISTOP,REGISBOT,OUTPUTFOLDER,MDLIDF,TOPIDF,  &
                         ...
                         MIN_THICKNESS,MINEXTENT
 IMPLICIT NONE
 INTEGER :: I,IOS
 
 FDISTANCES=''; ICPOINTERS=1; NCLIP=0; IEXPZONE=0; MIN_THICKNESS=0.0D0; CRIT_THICKNESS=0.0D0
 ...
 ELSE
  ...
  ELSE
   IF(UTL_READINITFILE('CRIT_THICKNESS',LINE,IU,1))READ(LINE,*) CRIT_THICKNESS
   WRITE(*,'(A,F10.2)') 'CRIT_THICKNESS=',CRIT_THICKNESS
   IF(UTL_READINITFILE('MIN_THICKNESS',LINE,IU,1))READ(LINE,*) MIN_THICKNESS
   WRITE(*,'(A,F10.2)') 'MIN_THICKNESS=',MIN_THICKNESS
   MINEXTENT=0; IF(UTL_READINITFILE('MINEXTENT',LINE,IU,1))READ(LINE,*) MINEXTENT
   WRITE(*,'(A,I10)') 'MINEXTENT=',MINEXTENT
   ...
   ENDIF
   IF(UTL_READINITFILE('IEXPZONE',LINE,IU,1))READ(LINE,*) IEXPZONE
   LINE='IEXPZONE='//TRIM(ITOS(IEXPZONE)); WRITE(*,'(A)') TRIM(LINE)
  ENDIF
  ZCRIT=0.0D0; IF(UTL_READINITFILE('ZCRIT',LINE,IU,1))READ(LINE,*) ZCRIT
  WRITE(*,'(A,F10.2)') 'ZCRIT=',ZCRIT
  PERCENTAGE=95; IF(UTL_READINITFILE('PERCENTAGE',LINE,IU,1))READ(LINE,*) PERCENTAGE
  LINE='PERCENTAGE'//'='//TRIM(RTOS(PERCENTAGE,'F',2)); WRITE(*,'(A)') TRIM(LINE)
 ENDIF
 ...
 CHARACTER(LEN=52),DIMENSION(3) :: STRING
 ...
 !###======================================================================
 SUBROUTINE  IMODBATCH_CREATEWELBORELOG()
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=256) :: IPFNAME,DIR
 CHARACTER(LEN=52) :: ID
 TYPE(IDFOBJ) :: SURFLEVEL
 INTEGER :: I,JU,KU
 REAL(KIND=DP_KIND) :: XC,YC,TP,BT,SL
 
 IF(.NOT.UTL_READINITFILE('IPFFILE_IN',LINE,IU,0))RETURN
 READ(LINE,*) IPFNAME; WRITE(*,'(A)') 'IPFFILE_IN='//TRIM(IPFNAME)
 IF(.NOT.UTL_READINITFILE('SURFLEVEL',LINE,IU,0))RETURN
 READ(LINE,*) SURFLEVEL%FNAME; WRITE(*,'(A)') 'SURFLEVEL='//TRIM(SURFLEVEL%FNAME)
 IF(.NOT.IDFREAD(SURFLEVEL,SURFLEVEL%FNAME,0))STOP
 
 NIPF=1; CALL IPFALLOCATE()
 !## read twice
 DO I=1,NIPF
  IPF(I)%FNAME=IPFNAME; IPF(I)%XCOL=1; IPF(I)%YCOL=2; IPF(I)%ZCOL=4; IPF(I)%Z2COL=5; IPF(I)%QCOL=1;
  IF(.NOT.IPFREAD2(I,1,1))RETURN
 ENDDO

 IF(.NOT.UTL_READINITFILE('IPFFILE_OUT',LINE,IU,0))RETURN
 READ(LINE,*) IPFNAME; WRITE(*,'(A)') 'IPFFILE_OUT='//TRIM(IPFNAME)
 DIR=IPFNAME(:INDEX(IPFNAME,'\',.TRUE.)-1)
 
 JU=UTL_GETUNIT(); OPEN(JU,FILE=IPFNAME,STATUS='UNKNOWN',ACTION='WRITE')
 WRITE(JU,'(A)') TRIM(ITOS(IPF(1)%NROW))
 WRITE(JU,'(A)') '6'
 WRITE(JU,'(A)') 'XC'
 WRITE(JU,'(A)') 'YC'
 WRITE(JU,'(A)') 'ID'
 ...
 DO I=1,IPF(1)%NROW
  ...
  TP=IPF(1)%XYZ(3,I)
  BT=IPF(1)%XYZ(4,I)
  SL=IDFGETXYVAL(SURFLEVEL,XC,YC)
  IF(SL.NE.SURFLEVEL%NODATA)THEN
   ID='WELLOC_'//TRIM(ITOS(I))
   KU=UTL_GETUNIT(); OPEN(KU,FILE=TRIM(DIR)//'\'//TRIM(ID)//'.TXT',STATUS='UNKNOWN',ACTION='WRITE')
   WRITE(KU,'(A)') '3'
   WRITE(KU,'(A)') '2,2'
   WRITE(KU,'(A)') 'Z,-999.0'
   WRITE(KU,'(A)') 'L,-999.0'
   WRITE(KU,'(A)') TRIM(RTOS(SL,'F',2))//',P'
   WRITE(KU,'(A)') TRIM(RTOS(TP,'F',2))//',F'
   WRITE(KU,'(A)') TRIM(RTOS(BT,'F',2))//',-'
   CLOSE(KU)
   WRITE(JU,'(A)') TRIM(RTOS(XC,'F',2))//','//TRIM(RTOS(YC,'F',2))//','//TRIM(ID)//','//TRIM(RTOS(TP,'F',2))//','//TRIM(RTOS(BT,'F',2))//','//TRIM(RTOS(SL,'F',2))
  ELSE
   WRITE(JU,'(A)') TRIM(RTOS(XC,'F',2))//','//TRIM(RTOS(YC,'F',2))//',NONE,'//TRIM(RTOS(TP,'F',2))//','//TRIM(RTOS(BT,'F',2))//','//TRIM(RTOS(SL,'F',2))
  ENDIF
 ENDDO
 CLOSE(JU)
 
 END SUBROUTINE  IMODBATCH_CREATEWELBORELOG
 ...
 !###======================================================================
 SUBROUTINE IMODBATCH_AGGREGATE()
 !###======================================================================
 USE MOD_AGGREGATE_PAR
 IMPLICIT NONE
 INTEGER :: OBSIROW,OBSICOL
 
 IPRJ=0; IWINDOW=0
 
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  IWINDOW=1; READ(LINE,*) XMIN,YMIN,XMAX,YMAX
 ...
 !## read ini-file
 IF(.NOT.UTL_READINITFILE('SORTFILE',LINE,IU,0))RETURN
 READ(LINE,*) SORTFILE; WRITE(*,'(A)') 'SORTFILE='//TRIM(SORTFILE)

 IF(.NOT.UTL_READINITFILE('INDIR',LINE,IU,0))RETURN
 READ(LINE,*) INDIR; WRITE(*,'(A)') 'INDIR='//TRIM(INDIR)
 
 IF(.NOT.UTL_READINITFILE('IMODE',LINE,IU,0))RETURN
 READ(LINE,*) IMODE; WRITE(*,'(A)') 'IMODE='//TRIM(ITOS(IMODE))
 ...
 SELECT CASE (IMODE)
  ...
  CASE (2)
   !## include iwhb maps
   IF(.NOT.UTL_READINITFILE('IWHBFILE',LINE,IU,0))RETURN
   READ(LINE,*) IWHBFILE; WRITE(*,'(A)') 'IWHBFILE='//TRIM(IWHBFILE)
   IF(.NOT.UTL_READINITFILE('IWHBDIR',LINE,IU,0))RETURN
   READ(LINE,*) IWHBDIR; WRITE(*,'(A)') 'IWHBDIR='//TRIM(IWHBDIR)
   OBSIROW=0; IF(UTL_READINITFILE('OBSIROW',LINE,IU,1))THEN
    READ(LINE,*) OBSIROW; WRITE(*,'(A)') 'OBSIROW='//TRIM(ITOS(OBSIROW))
   ENDIF
   OBSICOL=0; IF(UTL_READINITFILE('OBSICOL',LINE,IU,1))THEN
    READ(LINE,*) OBSICOL; WRITE(*,'(A)') 'OBSICOL='//TRIM(ITOS(OBSICOL))
   ENDIF
  CASE (3)
   !## aggregate
   IF(UTL_READINITFILE('IPRJ',LINE,IU,1))READ(LINE,*) IPRJ
   WRITE(*,'(A)') 'IPRJ='//TRIM(ITOS(IPRJ))
   IF(.NOT.UTL_READINITFILE('CMIN',LINE,IU,0))RETURN
   READ(LINE,*) CMIN; WRITE(*,'(A)') 'CMIN='//TRIM(RTOS(CMIN,'F',3))
 END SELECT
 
 !## output folder
 IF(.NOT.UTL_READINITFILE('OUTDIR',LINE,IU,0))RETURN
 READ(LINE,*) OUTDIR; WRITE(*,'(A)') 'OUTDIR='//TRIM(OUTDIR)

 CALL LHM_CONVERTREGIS_MAIN(OBSICOL,OBSIROW)

 END SUBROUTINE IMODBATCH_AGGREGATE
 ...
 IF(IHYPO.EQ.1)THEN
  ...
  IF(NGENSOL.GT.0)THEN
   ...
   DO I=1,NGENSOL
    IF(.NOT.UTL_READINITFILE('GEN_FNAME_'//TRIM(ITOS(I)),LINE,IU,0))RETURN; READ(LINE,*) GENSOL(I)%FNAME
    LINE='GEN_FNAME_'//TRIM(ITOS(I))//'='//TRIM(GENSOL(I)%FNAME)
    WRITE(*,'(A)') TRIM(LINE)
    IF(.NOT.UTL_READINITFILE('GEN_LAYER_'//TRIM(ITOS(I)),LINE,IU,0))RETURN; READ(LINE,*) GENSOL(I)%ILAY
    LINE='GEN_LAYER_'//TRIM(ITOS(I))//'='//TRIM(ITOS(GENSOL(I)%ILAY))
    WRITE(*,'(A)') TRIM(LINE)
   ENDDO
  ENDIF
 ...
 !## if layer specified, SDATE and EDATE are obliged
 ...
 !## read output file name to overrule default names (optional)
 MEAN_OUTFILE=""
 IF(UTL_READINITFILE('OUTFILE',LINE,IU,1))READ(LINE,*) MEAN_OUTFILE
 IF(LEN_TRIM(MEAN_OUTFILE).GT.0) MEAN_OUTFILE=MEAN_OUTFILE(:INDEX(MEAN_OUTFILE,'.',.TRUE.)-1)
 ...
 DO I=1,NBAL
  ...
  IF(WCTP(I)%NSYS.EQ.0)THEN
   WCTP(I)%NSYS=1
   LINE='- no system notation applied'; WRITE(*,'(A)') TRIM(LINE)
  ELSE
   LINE='- systems'
 ...
 !## make sure layers are not defined multiple times
 DO I=1,WBAL_NLAYER
  N=0; J=WBAL_ILAYER(I)
  DO K=1,WBAL_NLAYER
   IF(WBAL_ILAYER(K).EQ.J)N=N+1
  ENDDO
  IF(N.GT.1)THEN
   WRITE(*,'(/1X,A,I3,A)') 'Layer ',J,' defined more than once'; STOP
  ENDIF
 ENDDO
 ...
 N=0; IF(UTL_READINITFILE('NREPEAT',LINE,IU,1))READ(LINE,*) N
...
!
 ...
 IDFTRANSTOPOPER=0 ; IDFTRANSTOPO=0
...
!   CALL WMENUSETSTATE(ID_TOPOTRANSPARANCY,2,1)
 ...
 CALL DBL_WGRTEXTSTRING(XP-BND,0.0D0+0.5D0*BND,'Powered by '//TRIM(UTL_IMODVERSION())//' Copyright © 2005-2020')
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 IPLOT=1; ITRANSIENT=0; NLAYER=0; NIPFS=0; NRDATE=0; IWEIGHT=0; ICOL=0; IAVERAGE=0; IXY=0
 ...
 IF(UTL_READINITFILE('IXY',LINE,IU,1))THEN
  ...
  IF(IXY.EQ.1)THEN
   ...
   READ(LINE,*) XMAX; WRITE(*,'(A,F15.3)') 'XMAX=',XMAX
   IF(.NOT.UTL_READINITFILE('XMIN',LINE,IU,0))RETURN
   READ(LINE,*) XMIN; WRITE(*,'(A,F15.3)') 'XMIN=',XMIN
   IF(.NOT.UTL_READINITFILE('YMAX',LINE,IU,0))RETURN
   READ(LINE,*) YMAX; WRITE(*,'(A,F15.3)') 'YMAX=',YMAX
   IF(.NOT.UTL_READINITFILE('YMIN',LINE,IU,0))RETURN
   READ(LINE,*) YMIN; WRITE(*,'(A,F15.3)') 'YMIN=',YMIN
  ENDIF
 ENDIF
 ...
 IF(.NOT.UTL_READPOINTER_REAL(IU,I,HCLASSES,'HCLASSES',1))RETURN 
 ...
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=256) :: MFFNAME,OUTNAME,IDFNAME
 ...
 IF(.NOT.UTL_READINITFILE('OUTNAME',LINE,IU,0))RETURN
 READ(LINE,*) OUTNAME; WRITE(*,'(A)') 'OUTNAME='//TRIM(OUTNAME)
 IF(UTL_READINITFILE('IDFNAME',LINE,IU,1))THEN
  READ(LINE,*) IDFNAME; WRITE(*,'(A)') 'IDFNAME='//TRIM(IDFNAME)
 ENDIF
 
 CALL IMPORT_MF2005_MAIN(MFFNAME,OUTNAME,IDFNAME)
 
 END SUBROUTINE IMODBATCH_MFTOIMOD
 
 !###======================================================================
 SUBROUTINE IMODBATH_CREATEPILOTPOINTS_IPF()
 !###======================================================================
 IMPLICIT NONE
 ...
 !## plot triangles
 CALL UTL_CREATEDIR(IPFPPS(:INDEX(IPFPPS,'\',.TRUE.)-1))
...
! WRITE(*,*) SUM(P-C)
 ...
 !###======================================================================
 USE MOD_UTL, ONLY : UTL_MINTHICKNESS
 IMPLICIT NONE
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: BND,VCW,TOP,BOT,KVV,PPT1,PPT2,KHV,KVA
 TYPE(IDFOBJ) :: MASK
 INTEGER :: NLAY,ILAY,IVCW,DI,IROW,ICOL,NG,N,JU,KU,NP,IAQUIFER,IAQUITARD,IQ3D,IFM,IMASK,IACT,IPF
 REAL(KIND=DP_KIND) :: MINC,DX,T,K,C,X,Y,C1,C2
 CHARACTER(LEN=256) :: OUTPUTFOLDER
 CHARACTER(LEN=2) :: CPARAM1,CPARAM2
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: TP,BT,HK,VK,VA,TP_BU,BT_BU,HK_BU,VK_BU,VA_BU
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:,:) :: TH
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IB

 !## per ipf
 IPF=0; IF(UTL_READINITFILE('IPF',LINE,IU,1))READ(LINE,*) IPF
 WRITE(*,'(A)') 'IPF='//TRIM(ITOS(IPF))

 IF(IPF.EQ.1)THEN; CALL IMODBATH_CREATEPILOTPOINTS_IPF(); STOP; ENDIF
 
 !## per layer of per kh
 IFM=0; IF(UTL_READINITFILE('IFM',LINE,IU,1))READ(LINE,*) IFM
 WRITE(*,'(A)') 'IFM='//TRIM(ITOS(IFM))

 IF(IFM.EQ.0)THEN

  !## define quasi/3d simulations
  IQ3D=1; IF(UTL_READINITFILE('IQ3D',LINE,IU,1))READ(LINE,*) IQ3D
  WRITE(*,'(A)') 'IQ3D='//TRIM(ITOS(IQ3D))

  IF(IQ3D.EQ.1)THEN
   IAQUIFER=0; IF(UTL_READINITFILE('IAQUIFER',LINE,IU,1))READ(LINE,*) IAQUIFER
   WRITE(*,'(A)') 'IAQUIFER='//TRIM(ITOS(IAQUIFER))
   IAQUITARD=0; IF(UTL_READINITFILE('IAQUITARD',LINE,IU,1))READ(LINE,*) IAQUITARD
   WRITE(*,'(A)') 'IAQUITARD='//TRIM(ITOS(IAQUITARD))
  ELSE
   IAQUIFER=1; IAQUITARD=0
  ENDIF
 
  IVCW=0
  IF(IQ3D.EQ.1)THEN
   IF(UTL_READINITFILE('IVCW',LINE,IU,1))READ(LINE,*) IVCW
   WRITE(*,'(A)') 'IVCW='//TRIM(ITOS(IVCW))
  ENDIF
 ...
 ELSE
  IQ3D=0; IVCW=0; IAQUIFER=1; IAQUITARD=0; MINC=0.0D0
 ENDIF
 
 CPARAM1='KH'; IF(IVCW.EQ.1)CPARAM1='KD'; CPARAM2='KV'; IF(IVCW.EQ.1)CPARAM2='VC'
 ...
 ALLOCATE(BND(NLAY),PPT1(NLAY),PPT2(NLAY-1))
 IF(IVCW.EQ.1)THEN
  ALLOCATE(VCW(NLAY))
 ELSE
  ALLOCATE(TOP(NLAY),BOT(NLAY))
  IF(IQ3D.EQ.1)THEN
   ALLOCATE(KVV(NLAY))
  ELSE
   ALLOCATE(KHV(NLAY),KVA(NLAY))
  ENDIF
 ENDIF
 
 IMASK=0; IF(UTL_READINITFILE('MASK',LINE,IU,1))THEN
  IMASK=1; READ(LINE,*) MASK%FNAME; WRITE(*,'(A)') 'MASK='//TRIM(MASK%FNAME)
 ENDIF
 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   ...
   IF(IQ3D.EQ.1)THEN
    ...
    ENDIF
   ELSE
    CALL IDFNULLIFY(KHV(ILAY))
    IF(IFM.EQ.0)THEN
     IF(.NOT.UTL_READINITFILE('KHV_L'//TRIM(ITOS(ILAY)),LINE,IU,0))RETURN
     READ(LINE,*) KHV(ILAY)%FNAME; WRITE(*,'(A)') 'KHV_L'//TRIM(ITOS(ILAY))//'='//TRIM(KHV(ILAY)%FNAME)
    ENDIF
    CALL IDFNULLIFY(KVA(ILAY))
    IF(IFM.EQ.0)THEN
     IF(.NOT.UTL_READINITFILE('KVA_L'//TRIM(ITOS(ILAY)),LINE,IU,0))RETURN
     READ(LINE,*) KVA(ILAY)%FNAME; WRITE(*,'(A)') 'KVA_L'//TRIM(ITOS(ILAY))//'='//TRIM(KVA(ILAY)%FNAME)
    ENDIF
   ENDIF
  ENDIF
 ENDDO
 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   IF(IQ3D.EQ.1)THEN
    ...
    ENDIF
   ELSE
    CALL IDFCOPY(BND(ILAY),KHV(ILAY)); CALL IDFCOPY(BND(ILAY),KVA(ILAY))
    IF(IFM.EQ.0)THEN
     WRITE(*,'(A)') 'Reading '//TRIM(KHV(ILAY)%FNAME)//'...'
     IF(.NOT.IDFREADSCALE(KHV(ILAY)%FNAME,KHV(ILAY),3,1,0.0D0,0))STOP 'Cannot read data for KHV'
     WRITE(*,'(A)') 'Reading '//TRIM(KVA(ILAY)%FNAME)//'...'
     IF(.NOT.IDFREADSCALE(KVA(ILAY)%FNAME,KVA(ILAY),2,1,0.0D0,0))STOP 'Cannot read data for KVA'
    ELSE
     KHV(ILAY)%X=KHV(ILAY)%NODATA; KVA(ILAY)%X=KVA(ILAY)%NODATA
     DO IROW=1,BND(ILAY)%NROW; DO ICOL=1,BND(ILAY)%NCOL
      IF(BND(ILAY)%X(ICOL,IROW).GT.0)THEN
       KHV(ILAY)%X(ICOL,IROW)=ILAY
       KVA(ILAY)%X(ICOL,IROW)=1.0D0
      ENDIF
     ENDDO; ENDDO
    ENDIF
   ENDIF
 ...
 !## read mask file
 IF(IMASK.EQ.1)THEN
  WRITE(*,'(A)') 'Reading '//TRIM(MASK%FNAME)//'...'
  CALL IDFCOPY(BND(1),MASK); IF(.NOT.IDFREADSCALE(MASK%FNAME,MASK,7,1,0.0D0,0))STOP 'Cannot read data for MASK'
  DO IROW=1,MASK%NROW; DO ICOL=1,MASK%NCOL
   IF(MASK%X(ICOL,IROW).EQ.0.0D0)MASK%X(ICOL,IROW)=MASK%NODATA
  ENDDO; ENDDO
 ENDIF

 IF(IFM.EQ.1)THEN
  ALLOCATE(TP(NLAY)   ,BT(NLAY),   HK(NLAY),   VK(NLAY),   VA(NLAY), IB(NLAY))
  ALLOCATE(TP_BU(NLAY),BT_BU(NLAY),HK_BU(NLAY),VK_BU(NLAY),VA_BU(NLAY),TH(NLAY,2))
  !## is het niet de eerste laag wordt het?
  DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL; DO ILAY=1,NLAY
   IB(ILAY)=BND(ILAY)%X(ICOL,IROW)
   TP(ILAY)=TOP(ILAY)%X(ICOL,IROW)
   BT(ILAY)=BOT(ILAY)%X(ICOL,IROW)
   HK(ILAY)=ILAY 
   VK(ILAY)=1.0D0; IF(IQ3D.EQ.1)VK(ILAY)=KVV(ILAY)%X(ICOL,IROW)
   VA(ILAY)=KVA(ILAY)%X(ICOL,IROW)
  ENDDO
   CALL UTL_MINTHICKNESS(TP,BT,HK,VK,VA,TP_BU,BT_BU,HK_BU,VK_BU,VA_BU,IB,TH,1.0D0,NLAY,ICOL,IROW)
   DO ILAY=1,NLAY
    KHV(ILAY)%X(ICOL,IROW)=HK(ILAY)
   ENDDO
  ENDDO; ENDDO
  DEALLOCATE(TP,BT,HK,VK,VA,IB,TP_BU,BT_BU,HK_BU,VK_BU,VA_BU,TH)
!  DO ILAY=1,NLAY
!   KHV%FNAME='D:\TMP_'//TRIM(ITOS(ILAY))//'.IDF'
!   IF(.NOT.IDFWRITE(KHV(ILAY),KHV(ILAY)%FNAME,1))THEN; ENDIF
!  ENDDO
 ENDIF
 ...
 IF(IAQUIFER.EQ.1)THEN
  ...
  DO ILAY=1,NLAY; PPT1(ILAY)%X=PPT1(ILAY)%NODATA; ENDDO
  ...
  IF(IFM.EQ.0)THEN
   ...
   DO ILAY=1,NLAY
    ...
    DO IROW=0.5*DI,BND(ILAY)%NROW,DI; DO ICOL=0.5*DI,BND(ILAY)%NCOL,DI
     !## skip inactive cells 
     ...
     ELSE
      ...
      ELSE
       IF(IQ3D.EQ.1)THEN
        ...
        C=T/K
       ELSE
        T=TOP(ILAY-1)%X(ICOL,IROW)-BOT(ILAY-1)%X(ICOL,IROW)
        K=KHV(ILAY-1)%X(ICOL,IROW)*KVA(ILAY-1)%X(ICOL,IROW) 
        C1=0.0D0; IF(T.GT.0.0D0.AND.K.GT.0.0D0)C1=(0.5D0*T)/K
        T=TOP(ILAY  )%X(ICOL,IROW)-BOT(ILAY  )%X(ICOL,IROW)
        K=KHV(ILAY  )%X(ICOL,IROW)*KVA(ILAY  )%X(ICOL,IROW) 
        C2=0.0D0; IF(T.GT.0.0D0.AND.K.GT.0.0D0)C2=(0.5D0*T)/K
        C=C1+C2
       ENDIF
      ENDIF
  ...
  ELSE
   ...
   DO ILAY=1,NLAY
    PPT1(ILAY)%X=PPT1(ILAY)%NODATA
    DO IROW=0.5D0*DI,BND(ILAY)%NROW,DI; DO ICOL=0.5D0*DI,BND(ILAY)%NCOL,DI
     !## skip inactive cells 
     IF(BND(ILAY)%X(ICOL,IROW).EQ.0.0D0)CYCLE
     IF(ILAY.EQ.1)THEN
      NG=NG+1; NP=NP+1; PPT1(ILAY)%X(ICOL,IROW)=NG
     ELSE
      !## check c to see if similar point need to be used
      IF(KHV(ILAY)%X(ICOL,IROW).NE.KHV(ILAY-1)%X(ICOL,IROW))THEN
       NG=NG+1
       N =NG
      ELSE
       N =PPT1(ILAY-1)%X(ICOL,IROW)
...
! DO ILAY=1,NLAY
!  PPT1%FNAME='D:\TMP_'//TRIM(ITOS(ILAY))//'.IDF'
!  IF(.NOT.IDFWRITE(PPT1(ILAY),PPT1(ILAY)%FNAME,1))THEN; ENDIF
! ENDDO
 ...
 IF(IAQUITARD.EQ.1)THEN
  ...
  DO ILAY=1,NLAY-1
   ...
   DO IROW=0.5*DI,BND(ILAY)%NROW,DI; DO ICOL=0.5*DI,BND(ILAY)%NCOL,DI
    ...
    IF(BND(ILAY)%X(ICOL,IROW).EQ.0.0D0.OR.BND(ILAY+1)%X(ICOL,IROW).EQ.0.0D0)CYCLE

    IF(IVCW.EQ.1)THEN
     C=VCW(ILAY-1)%X(ICOL,IROW)
    ELSE
     T=MAX(0.0D0,BOT(ILAY)%X(ICOL,IROW)-TOP(ILAY+1)%X(ICOL,IROW))
     K=KVV(ILAY)%X(ICOL,IROW)
     C=T/K
    ENDIF
    NG=NG+1
 ...
 KU=UTL_GETUNIT()
 IF(IAQUIFER.EQ.1)THEN
  OPEN(KU,FILE=TRIM(OUTPUTFOLDER)//'\PILOTPOINT_AQUIFER.PRJ',ACTION='WRITE',FORM='FORMATTED')
 ELSE
  OPEN(KU,FILE=TRIM(OUTPUTFOLDER)//'\PILOTPOINT_AQUITARD.PRJ',ACTION='WRITE',FORM='FORMATTED')
 ENDIF
  ...
  DO ILAY=1,NLAY; DO I=1,2; N=0
   DO IROW=0.5D0*DI,BND(ILAY)%NROW,DI; DO ICOL=0.5D0*DI,BND(ILAY)%NCOL,DI
    ...
    IF(I.EQ.2)THEN
     ...
     !## skip mask cells eq. 0.0 or nodata
     IACT=1; IF(IMASK.EQ.1)THEN
      IF(MASK%X(ICOL,IROW).EQ.MASK%NODATA)IACT=0
     ENDIF

     IF(IVCW.EQ.1)THEN
      WRITE(JU,'(2(F10.2,1X),2(I10,1X),2(F10.2,1X),I5)') X,Y,NP,INT(PPT1(ILAY)%X(ICOL,IROW)),-DBLE(ILAY)+0.5D0,-DBLE(ILAY)-0.5D0,IACT
     ELSE
      WRITE(JU,'(2(F10.2,1X),2(I10,1X),2(F10.2,1X),I5)') X,Y,NP,INT(PPT1(ILAY)%X(ICOL,IROW)),TOP(ILAY)%X(ICOL,IROW),BOT(ILAY)%X(ICOL,IROW),IACT
     ENDIF
     WRITE(KU,'(A)') TRIM(ITOS(IACT))//','//TRIM(CPARAM1)//','//TRIM(ITOS(ILAY))//','//TRIM(ITOS(NP))//',1.0,1.1,0.01,100.0,10.0,'// &
   ...
   IF(I.EQ.1)THEN
    ...
    WRITE(JU,*) N; WRITE(JU,*) 7; WRITE(JU,*) 'X'; WRITE(JU,*) 'Y'; WRITE(JU,*) 'PARAMETER'
  ...
  DO ILAY=1,NLAY-1; DO I=1,2; N=0
   DO IROW=0.5D0*DI,BND(ILAY)%NROW,DI; DO ICOL=0.5D0*DI,BND(ILAY)%NCOL,DI
    ...
    IF(I.EQ.2)THEN
     NP=NP+1
     IF(IVCW.EQ.1)THEN
      WRITE(JU,'(2(F10.2,1X),2(I10,1X),2(F10.2,1X),I5)') X,Y,NP,INT(PPT2(ILAY)%X(ICOL,IROW)),-REAL(ILAY,8)+0.5D0,-REAL(ILAY,8)-0.5D0,IACT
     ELSE
      WRITE(JU,'(2(F10.2,1X),2(I10,1X),2(F10.2,1X),I5)') X,Y,NP,INT(PPT2(ILAY)%X(ICOL,IROW)),BOT(ILAY)%X(ICOL,IROW),TOP(ILAY+1)%X(ICOL,IROW),IACT
     ENDIF
     ...
     IF(NG.GT.0)THEN
      ...
      IACT=1; IF(IMASK.EQ.1)THEN
       IF(MASK%X(ICOL,IROW).EQ.MASK%NODATA)IACT=0
      ENDIF
      WRITE(KU,'(A)') TRIM(ITOS(IACT))//','//TRIM(CPARAM2)//','//TRIM(ITOS(ILAY))//','//TRIM(ITOS(NP))//',1.0,1.1,0.01,100.0,10.0,'// &
 ...
 CALL PMANAGER_GENERATEMFNETWORKS(GENFNAME,OUTFOLDER,N,1,0)
 ...
 !###======================================================================
 SUBROUTINE IMODBATCH_IPFEVALUATE()
 !###======================================================================
 IMPLICIT NONE
 ...
 !## perform ipf sample, return number of 
 IF(.NOT.IMODBATCH_IPFSAMPLE_MAIN(INIPF,ICOLS,NLAY))RETURN
  
 IF(.NOT.UTL_READINITFILE('IPFLAYERS',LINE,IU,0))RETURN
 READ(LINE,*) OUTIPF; WRITE(*,'(A)') 'IPFLAYERS='//TRIM(OUTIPF)
  
 CALL IPFEVALUATE(INIPF,OUTIPF,NLAY,ICOLS) 
 
 END SUBROUTINE IMODBATCH_IPFEVALUATE
