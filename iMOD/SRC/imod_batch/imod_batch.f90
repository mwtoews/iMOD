...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 IMPLICIT NONE
 INTEGER :: I,J,K,N,IN_TYPE,JU,M,IDATE,SDATE,EDATE,DDATE,IOS,ICOL,IROW,JF,ID,KSUM
 ...
 IF(N.EQ.0)THEN
  ...
  IF(ABS(ASSF_INDICATOR).GT.0.AND.ASSF_NTHRESHOLD.GT.1)THEN
   ...
   DO
    ...
    DO IROW=1,VOXEL(ASSF_NTHRESHOLD+1)%NROW; DO ICOL=1,VOXEL(ASSF_NTHRESHOLD+1)%NCOL
     ...
     IF(JF.GT.0)THEN
      ...
      IF(KSUM.EQ.0)THEN
       ...
       VOXEL(ASSF_NTHRESHOLD+3)%X(ICOL,IROW)=ASSF_DZ/ASSF_K_THRESHOLD(JF)
      ENDIF
     ELSE
    ...
    DO I=1,4; VOXEL(ASSF_NTHRESHOLD+I)%TOP=ASSF_TOP; VOXEL(ASSF_NTHRESHOLD+I)%BOT=ASSF_BOT; ENDDO
    ...
    IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_C.IDF'
