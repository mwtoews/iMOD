...
! WRITE(*,'(A,I2)') 'ITALUSCOL=',DATCOL(9)
 ...
 N=0; DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
  ...
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.IDF%NCOL)THEN; N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF,IDF%NROW,IDF%NCOL,IROW,ICOL,0.0,N); ENDIF
  ENDIF

  !## place horizontal wall
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   !## write line in genfile
   IF(IROW.LT.IDF%NROW)THEN; N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF,IDF%NROW,IDF%NCOL,IROW,ICOL,0.0,N); ENDIF
  ENDIF
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 REAL :: XMIN,YMIN,XMAX,YMAX,Z,Z1,Z2,MF,KH,VC
...
! ENDIF
! IF(.NOT.WBAL_GRAPHCOMPUTE('g:\IMOD-MODELS\ALBERTA\SYLVAN_LAKE\IMOD_USER\MODELS\SLB_V15_TRANSIENT\WBAL_WHOLE.CSV'))THEN
! ENDIF
