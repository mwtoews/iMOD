...
! ENDIF
 ...
 INTEGER :: JU,NCOL,NROW,NLAY,ILAY,KSTP,KPER,ICOL,IROW,IOS,I1,I2,I3,NDIM1,NDIM2,NDIM3, &
  IMETH,NDAT,NLIST,I,J,LENTXT,NCELLS,NTXT,NJA,N,M,IPOS,NCON
 INTEGER,DIMENSION(:),ALLOCATABLE :: ID1,ID2
 CHARACTER(LEN=16) :: TEXT,TXT1ID1,TXT1ID2,TXT2ID1,TXT2ID2
...
! CALL IDFCOPY(IDF,NLD); IF(.NOT.IDFREADSCALE('d:\IMOD-MODELS\LHM\NLD.IDF',NLD,7,1,0.0D0,0))STOP
 ...
 IF(UTL_READINITFILE('HED',LINE,IU,1))THEN
  ...
  SUM%X=0.0; PHR%X=PHR%NODATA; DO
   ...
   IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
