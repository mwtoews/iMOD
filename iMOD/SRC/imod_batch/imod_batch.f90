...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 IF(N.EQ.0)THEN
  ...
  IF(ASSF_INDICATOR.GT.0)THEN
   ...
   ALLOCATE(VOXEL(ASSF_NTHRESHOLD+3)); LEX=.FALSE.
   ...
   DO
    ...
    IF(.NOT.LEX)THEN
     ...
     CALL IDFCOPY(VOXEL(1),VOXEL(ASSF_NTHRESHOLD+3)); LEX=.TRUE.
    ENDIF
    ...
    DO IROW=1,VOXEL(ASSF_NTHRESHOLD+1)%NROW; DO ICOL=1,VOXEL(ASSF_NTHRESHOLD+1)%NCOL
     ...
     DO I=1,ASSF_NTHRESHOLD
      IF(VOXEL(I)%X(ICOL,IROW).NE.VOXEL(I)%NODATA)THEN
       ...
       ENDIF
       !## get weighted permeability
       VOXEL(ASSF_NTHRESHOLD+2)%X(ICOL,IROW)=VOXEL(ASSF_NTHRESHOLD+2)%X(ICOL,IROW)+VOXEL(I)%X(ICOL,IROW)*ASSF_K_THRESHOLD(I)
      ENDIF
     ENDDO
     IF(JF.GT.0)THEN
     ...
     ENDIF
    ENDDO; ENDDO
    VOXEL(ASSF_NTHRESHOLD+1)%TOP=ASSF_TOP; VOXEL(ASSF_NTHRESHOLD+1)%BOT=ASSF_BOT
    IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'.IDF'
    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+1),IDFFILE,1))STOP
    VOXEL(ASSF_NTHRESHOLD+2)%TOP=ASSF_TOP; VOXEL(ASSF_NTHRESHOLD+2)%BOT=ASSF_BOT
    IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_K.IDF'
    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+2),IDFFILE,1))STOP
    VOXEL(ASSF_NTHRESHOLD+3)%TOP=ASSF_TOP; VOXEL(ASSF_NTHRESHOLD+3)%BOT=ASSF_BOT
