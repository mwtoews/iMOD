...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=60
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'CLIPVOXELS','FAULTS3D','RUNFILE','FLUMY','GEOCONNECT','IPFSUM','VOXELVOLUME'/
...
!   CALL IMODBATCH_CREATEVOXEL_MAIN()
 ...
 IF(IFN.GT.SIZE(CFUNC))THEN
  CLOSE(IU)
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not recognize function:'//CHAR(13)//'FUNCTION='//TRIM(CF),'Error')
  RETURN
 ENDIF
 ...
 CHARACTER(LEN=52) :: WC
 INTEGER :: I,J,JU,MAXTHREAD,MAXN,NPOCKETS,ILAY,IROW,ICOL,IP
 ...
 ALLOCATE(POCKET(NPOCKETS)); POCKET%SIZE=0; POCKET%COUNT=0
 DO I=1,NPOCKETS
  IF(.NOT.UTL_READINITFILE('POCKET'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) POCKET(I)%SIZE; LINE='POCKET'//TRIM(ITOS(I))//'='//TRIM(ITOS(POCKET(I)%SIZE)); WRITE(*,'(A)') TRIM(LINE)
 ENDDO
 ...
 DO I=1,SIZE(LISTNAME)
  ...
  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL 
   IF(IDF%X(ICOL,IROW).GE.XOFFSET)THEN
    IBND(ICOL,IROW,I)=1
   ELSE
    IBND(ICOL,IROW,I)=0
   ENDIF
  ...
  DO IROW=1,IDF%NROW
   DO ICOL=1,IDF%NCOL
    IF(Y(ICOL,IROW,ILAY).EQ.0.AND.IBND(ICOL,IROW,ILAY).GT.0)THEN
     IP=1
     CALL SOLID_TRACE_3D(ILAY,IROW,ICOL,IBND,Y,SIZE(LISTNAME),IDF%NROW,IDF%NCOL,IP,MAXTHREAD,MAXN,POCKET%COUNT,POCKET%SIZE)
    ENDIF
   ENDDO
  ENDDO
 END DO
 ...
 DO I=1,SIZE(POCKET)
  WRITE(*,'(3I15)') I,POCKET(I)%SIZE,POCKET(I)%COUNT
 ENDDO
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 IF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  IMODE=1
  READ(LINE,*) PRJFILE; WRITE(*,'(A)') 'PRJFILE_IN='//TRIM(PRJFILE)

  IF(UTL_READINITFILE('RUNFILE_OUT',LINE,IU,1))THEN
   READ(LINE,*) RUNFILE; WRITE(*,'(A)') 'RUNFILE_OUT='//TRIM(RUNFILE)
   PBMAN%IFORMAT=1
  ENDIF
  IF(UTL_READINITFILE('NAMFILE_OUT',LINE,IU,1))THEN
   READ(LINE,*) RUNFILE; WRITE(*,'(A)') 'NAMFILE_OUT='//TRIM(RUNFILE)
   PBMAN%IFORMAT=2
  ENDIF
  ...
  IF(PBMAN%ISS.EQ.1)THEN
   ...
   IF(UTL_READINITFILE('TIMFNAME',LINE,IU,1))THEN
    READ(LINE,*) PBMAN%TIMFNAME; WRITE(*,'(A)') 'TIMFNAME='//TRIM(PBMAN%TIMFNAME)
   ELSE
    !## sdate
    IF(.NOT.UTL_READINITFILE('SDATE',LINE,IU,0))RETURN
...
!  READ(LINE,*) PBMAN%IFORMAT; WRITE(*,'(A,I14)') 'IFORMAT=',PBMAN%IFORMAT
  ...
  IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
   ...
   WRITE(*,'(A,4F15.7)') 'WINDOW=',PBMAN%XMIN,PBMAN%YMIN,PBMAN%XMAX,PBMAN%YMAX
   IF(.NOT.UTL_READINITFILE('CELLSIZE',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%CELLSIZE; WRITE(*,'(A,F15.7)') 'CELLSIZE=',PBMAN%CELLSIZE
   IF(.NOT.UTL_READINITFILE('BUFFER',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%BUFFER; WRITE(*,'(A,F15.7)') 'BUFFER=',PBMAN%BUFFER
  ENDIF
 ...
 IF(UTL_READINITFILE('RUNFILE_IN',LINE,IU,1))THEN
  ...
  READ(LINE,*) PRJFILE; WRITE(*,'(A)') 'PRJFILE_OUT='//TRIM(PRJFILE)
 ENDIF
...
!  TNG=TNG-0.5*PI
 ...
 CALL ISG_EXPORT(EXPORTFNAME,IEXPORT)
 ...
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),DIMENSION(:),ALLOCATABLE :: IDF,TQP
 INTEGER :: I,IFLOW,IPNTR,IWINDOW,IWRITE,IGRAD,ITQP,NTQP
 REAL,DIMENSION(:),ALLOCATABLE :: PTQP
 CHARACTER(LEN=256) :: RESULTIDF,OUTPUTFOLDER
 ...
 IF(IFLOW.EQ.0)THEN
  ...
  I=5; ALLOCATE(IDF(I)); DO I=1,SIZE(IDF); CALL IDFNULLIFY(IDF(I)); ENDDO
 ...
 ELSEIF(IFLOW.EQ.2)THEN
  ...
  I=NTQP; ALLOCATE(TQP(I),PTQP(I)); DO I=1,SIZE(TQP); CALL IDFNULLIFY(TQP(I)); ENDDO
  ...
  DO I=1,NTQP
   TQP(I)%FNAME=''; IF(.NOT.UTL_READINITFILE('TQPIDF'//TRIM(ITOS(I)),LINE,IU,0))RETURN
   READ(LINE,*) TQP(I)%FNAME; WRITE(*,'(A)') 'TQPIDF'//TRIM(ITOS(I))//'='//TRIM(TQP(I)%FNAME)
  ENDDO
  
  CALL SOF_CATCHMENTS(RESULTIDF,OUTPUTFOLDER,IDF,TQP,PTQP,ITQP)
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 USE MOD_ASC2IDF_PAR, ONLY : IDFFILE,SOURCEDIR,TARGETDIR,IGRIDFUNC,CS,NODATA,LAGDISTANCE, &
   ...
   ASSF_COLUMN,ASSF_STARTDATE,ASSF_ENDDATE,ASSF_MTYPE,ASSF_DDATE,ASSF_CDDATE,ILOG,TRIMDEPTH_IDF, &
   GENFILE,BLNFILE
 ...
 IF(IN_TYPE.EQ.2)THEN
  ...
  IF(M.GT.0.AND.M.EQ.IZCOL)THEN
   ...
   ENDIF
   IF(.NOT.UTL_READINITFILE('ASSF_MTYPE',LINE,IU,0))RETURN
 ...
 IF(.NOT.UTL_READINITFILE('ICLEAN',LINE,IU,0))RETURN
 READ(LINE,*) ICLEAN; WRITE(*,'(A,I10)') 'ICLEAN=',ICLEAN
 ...
 DO IROW=1,MOTHER%NROW; DO ICOL=1,MOTHER%NCOL
  DO I=2,SIZE(IDF)
   IF(IDF(I)%X(ICOL,IROW).GT.IDF(I-1)%X(ICOL,IROW))IDF(I)%X(ICOL,IROW)=IDF(I-1)%X(ICOL,IROW)
  ENDDO
 ENDDO; ENDDO
 ...
 I=1; DO J=1,SIZE(IDF)
  ...
  ELSE
   IF(MOD(J,2).EQ.0)THEN
    IDF(J)%FNAME=TRIM(OUTPUTFOLDER)//'\BOT_L'//TRIM(ITOS(I))//'_'//TRIM(IDF(J)%FNAME(INDEX(IDF(J)%FNAME,'\',.TRUE.)+1:))
   ELSE
    IDF(J)%FNAME=TRIM(OUTPUTFOLDER)//'\TOP_L'//TRIM(ITOS(I))//'_'//TRIM(IDF(J)%FNAME(INDEX(IDF(J)%FNAME,'\',.TRUE.)+1:))
   ENDIF
  ENDIF
 ...
 INTEGER :: I,J,K,KK,KKK,N,M,NN,JU,KU,IEXT,IDATE,JDATE,JD1,JD2,IY,IM,ID,IY1,IY2,IBAL,IROW,ICOL,IPER,NPER,DD,NYR
 REAL :: Q1,Q2,X,Y
 TYPE BALOBJ
 ...
 IY1=1980; IY2=2015; NYR=(IY2-IY1)+1
 ...
 WRITE(JU,'(14A)') 'Time',(',QSRC_GW.hist',J=1,SIZE(BAL))
 ...
 USE MOD_SOLID_PAR, ONLY : IBATCH,IWINDOW,SOLFILE,OUTPUTFOLDER,FMIDELEV,    &
   MDLIDF,REGISTOP,REGISBOT,REGISKHV,REGISKVV,SLD,DZ,HCLOSE,MICNVG,IBNDCHK,IINT_IDF,GEN
 IMPLICIT NONE
 INTEGER :: IMASK,IHYPO,ICKDC,I,J,NGEN
 REAL :: ZOFFSET
 ...
 IF(IHYPO.EQ.1)THEN
  ...
  WRITE(*,'(A,I10)') 'NGEN=',NGEN
  IF(NGEN.GT.0)THEN
   ALLOCATE(GEN(NGEN))
   DO I=1,NGEN
    IF(.NOT.UTL_READINITFILE('GEN_'//TRIM(ITOS(I)),LINE,IU,0))RETURN; READ(LINE,*) GEN(I)%ILAY,GEN(I)%FNAME
    LINE='GEN_'//TRIM(ITOS(I))//'='//TRIM(ITOS(GEN(I)%ILAY))//','//TRIM(GEN(I)%FNAME)
    WRITE(*,'(A)') TRIM(LINE)
   ENDDO
  ENDIF
...
! I=INDEX(FUNC,'('); I=MAX(I,1)
 ...
 CALL ASC2IDF_EXPORTASC_MAIN(DIRNAME,IQUICK,IBATCH)
...
!  IF(GC_INIT_POSTPROCESSING(IU))
 ENDIF
 
 IF(GC_INIT())CALL GC_COMPUTE_MAIN(1)
