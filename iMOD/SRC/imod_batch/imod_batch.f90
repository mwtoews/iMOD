...
USE MOD_ISG_PAR, ONLY : NISG,ISG,ISD,DATISD
...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=63
...
!    IF(I.EQ.1)THEN; LQBD=.TRUE.; WRITE(*,'(/A/)') 'IQUASI3D = QUASI 3D DISCRETISATION APPLIED (USING INTERBEDS)'; ENDIF
  ...
  IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
   ...
   WRITE(*,'(A12,4F15.7)') 'WINDOW=',PBMAN%XMIN,PBMAN%YMIN,PBMAN%XMAX,PBMAN%YMAX
   IF(.NOT.UTL_READINITFILE('CELLSIZE',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%CELLSIZE; WRITE(*,'(A12,F15.7)') 'CELLSIZE=',PBMAN%CELLSIZE
   IF(.NOT.UTL_READINITFILE('BUFFER',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%BUFFER; WRITE(*,'(A12,F15.7)') 'BUFFER=',PBMAN%BUFFER
  ENDIF
...
!  TNG=TNG-0.5*PI
 ...
 CALL ISG_ADDCROSSSECTION(ISGFILE,FNAME,WIDTHFNAME,MAXDIST,CROSS_PNTR,CROSS_BATH,CROSS_ZCHK,CROSS_CVAL,CELL_SIZE,1)
...
! IF(UTL_READINITFILE('IXCOLTO',LINE,IU,1))READ(LINE,*) DATCOL(3)
! WRITE(*,'(A,I2)') 'IXCOLTO=',DATCOL(3)
! IF(UTL_READINITFILE('IYCOLTO',LINE,IU,1))READ(LINE,*) DATCOL(4)
! WRITE(*,'(A,I2)') 'IYCOLTO=',DATCOL(4)
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 IF(IN_TYPE.EQ.2)THEN
  ...
  IF(M.GT.0.AND.M.EQ.IZCOL)THEN
   ...
   ELSE
    ...
    IDEPTH=1
    IF(UTL_READINITFILE('TRIMDEPTH_IDF',LINE,IU,1))READ(LINE,'(A)') TRIMDEPTH_IDF%FNAME
    IF(TRIM(TRIMDEPTH_IDF%FNAME).NE.'')WRITE(*,'(A)') 'TRIMDEPTH_IDF='//TRIM(TRIMDEPTH_IDF%FNAME)
   ENDIF
  ENDIF
 !## genfile
 ...
 DO I=1,NIPF
  ...
  WRITE(*,'(A)') TRIM(LINE)
  W_TYPE(I)=0
  IF(UTL_READINITFILE('W_TYPE'//TRIM(ITOS(I)),LINE,IU,1))READ(LINE,*) W_TYPE(I)
 ...
 SOBSDATE=0; IF(UTL_READINITFILE('SDATE',LINE,IU,1))READ(LINE,*) SOBSDATE
 LINE='SDATE='//TRIM(ITOS(SOBSDATE)); WRITE(*,'(A)') TRIM(LINE)
 EOBSDATE=0; IF(UTL_READINITFILE('EDATE',LINE,IU,1))READ(LINE,*) EOBSDATE
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 IPLOT=1; ITRANSIENT=0; NLAYER=0; NIPFS=0; NRDATE=0; IWEIGHT=0
