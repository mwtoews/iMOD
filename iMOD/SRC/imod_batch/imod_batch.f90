...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=57
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'CLIPVOXELS','FAULTS3D','RUNFILE','FLUMY'/
...
!   CALL IMODBATCH_CREATEVOXEL_MAIN()
 ...
 DO I=1,SIZE(LISTNAME)
  IF(.NOT.IDFREAD(IDF(3),LISTNAME(I),0))THEN; WRITE(*,'(A)') 'Cannot read file '//TRIM(LISTNAME(I)); STOP; ENDIF
  ZT(I)=IDF(3)%TOP; ZB(I)=IDF(3)%BOT
 ENDDO
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 USE MOD_ASC2IDF_PAR, ONLY : IDFFILE,SOURCEDIR,TARGETDIR,IGRIDFUNC,CS,NODATA,LAGDISTANCE,SEARCHDISTANCE, &
   ...
   IEXPVARIOGRAM,ASSF_COLUMN,ASSF_STARTDATE,ASSF_ENDDATE,ASSF_MTYPE,ASSF_DDATE,ASSF_CDDATE,ILOG,TRIMDEPTH_IDF, &
   GENFILE,BLNFILE
...
! ENDIF
 ...
 IF(.NOT.UTL_READINITFILE('IPFFILE',LINE,IU,0))RETURN
 READ(LINE,*) IPFFNAME; WRITE(*,'(A)') 'IPFFILE='//TRIM(IPFFNAME);write(*,*)line 
 IF(.NOT.UTL_READINITFILE('NPARAM',LINE,IU,0))RETURN
 READ(LINE,'(I)') NPAR; WRITE(*,'(A,I3)') 'NPARAM=',NPAR
 
 !## landuse code to be drainage potentially
 ALLOCATE(FLM(1)%GRN(NPAR))
 ALLOCATE(FLM(1)%FCL(NPAR))
 ALLOCATE(FLM(1)%FCN(NPAR))
 DO I=1,NPAR
  IF(.NOT.UTL_READINITFILE('GRAIN'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) FLM(1)%GRN(I)
  LINE='GRAIN'//TRIM(ITOS(I))//'='//TRIM(FLM(1)%GRN(I)); WRITE(*,*) TRIM(LINE)
  IF(.NOT.UTL_READINITFILE('FACIESL'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) FLM(1)%FCL(I)
  LINE='FACIESL'//TRIM(ITOS(I))//'='//TRIM(FLM(1)%FCL(I)); WRITE(*,*) TRIM(LINE)
  IF(.NOT.UTL_READINITFILE('FACIESN'//TRIM(ITOS(I)),LINE,IU,0))RETURN
  READ(LINE,*) FLM(1)%FCN(I)
  LINE='FACIESN'//TRIM(ITOS(I))//'='//TRIM(ITOS(FLM(1)%FCN(I))); WRITE(*,'(I2)') TRIM(LINE)    
 ENDDO

 CALL FLUMY_MAIN()
 
 DEALLOCATE(FLM(1)%FCL); DEALLOCATE(FLM(1)%FCN); DEALLOCATE(FLM(1)%GRN)
