  ...
  ELSE
   ...
   DO ISUB=1,PBMAN%NSUBMODEL
    ...
    IF(UTL_READINITFILE(TRIM(CKEYW),LINE,IU,1))THEN
     ...
     WRITE(*,'(A,4G15.7)') TRIM(CKEYW)//'=',SUBMDL(ISUB)%XMIN,SUBMDL(ISUB)%YMIN,SUBMDL(ISUB)%XMAX,SUBMDL(ISUB)%YMAX
     ...
     WRITE(*,'(A,G15.7)') TRIM(CKEYW)//'=',SUBMDL(ISUB)%BUFFER
     ...
     IF(SUBMDL(ISUB)%BUFFER.GT.0.0D0)THEN
      ...
      WRITE(*,'(A,G15.7)') TRIM(CKEYW)//'=',SUBMDL(ISUB)%BUFFERCS
     ENDIF
...
! IY1=1980; IY2=2015; 
 ...
 DO IPER=1,NYR
  ...
  WRITE(JU,'(A,13(A1,E15.8))') TRIM(LINE),(',',BAL(J)%Q(IPER),J=1,SIZE(BAL))
