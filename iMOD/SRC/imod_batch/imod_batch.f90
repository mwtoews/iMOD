...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=77
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 PBMAN%DWEL=1; PBMAN%DISG=1; PBMAN%DMMFILE=0; PBMAN%NSUBMODEL=1
 ...
 ELSEIF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  ...
  ELSE
   ...
   DO ISUB=1,PBMAN%NSUBMODEL
    ...
    CKEYW='WINDOW'; IF(PBMAN%NSUBMODEL.GT.1)CKEYW=TRIM(CKEYW)//TRIM(ITOS(PBMAN%ISUBMODEL))
    IF(UTL_READINITFILE(TRIM(CKEYW),LINE,IU,1))THEN
     ...
     WRITE(*,'(A,4G15.9)') TRIM(CKEYW)//'=',SUBMDL(ISUB)%XMIN,SUBMDL(ISUB)%YMIN,SUBMDL(ISUB)%XMAX,SUBMDL(ISUB)%YMAX
     ...
     READ(LINE,*) SUBMDL(ISUB)%CELLSIZE; WRITE(*,'(A,G15.7)') TRIM(CKEYW)//'=',SUBMDL(ISUB)%CELLSIZE
     ...
     WRITE(*,'(A,G15.9)') TRIM(CKEYW)//'=',SUBMDL(ISUB)%BUFFER
     ...
     IF(SUBMDL(ISUB)%BUFFER.GT.0.0D0)THEN
      ...
      WRITE(*,'(A,G15.9)') TRIM(CKEYW)//'=',SUBMDL(ISUB)%BUFFERCS
     ENDIF
    ...
    ELSE
     ...
     CKEYW='CELLSIZE'; IF(PBMAN%NSUBMODEL.GT.1)CKEYW=TRIM(CKEYW)//TRIM(ITOS(PBMAN%ISUBMODEL))
     IF(UTL_READINITFILE(TRIM(CKEYW),LINE,IU,1))THEN
      WRITE(*,'(/A/)') 'The usage of '//TRIM(CKEYW)//' is only valid in combination with the keyword WINDOW'; RETURN
     ENDIF
     CKEYW='BUFFER'; IF(PBMAN%NSUBMODEL.GT.1)CKEYW=TRIM(CKEYW)//TRIM(ITOS(PBMAN%ISUBMODEL))
     IF(UTL_READINITFILE(TRIM(CKEYW),LINE,IU,1))THEN
      WRITE(*,'(/A/)') 'The usage of '//TRIM(CKEYW)//' is only valid in combination with the keyword WINDOW'; RETURN
...
!!#####=================================================================
 ...
 !###======================================================================
 USE MOD_TS_PAR, ONLY : IPFNAME1,IPFNAME2,TSILAY,SDATE,EDATE,IASSF,TSDIR,LCOL
 IMPLICIT NONE
...
! ENDDO
 ...
 INTEGER :: I,J,JJ,KU,JU,LU,NVARS,MINMEASURE,M,N,NPERC,ITXTFILE
 INTEGER(KIND=SP_KIND) :: SDATE,EDATE,NGXG,DIFFDAY,IWEIGHT
 ...
 MINMEASURE=24; DIFFDAY=5; NODATA=-999.99D0; IWEIGHT=0; ITXTFILE=0; RANGE=0.0D0
 ...
 WRITE(*,'(A)') 'IWEIGHT='//TRIM(ITOS(IWEIGHT))
 ...
 DO I=1,NVARS
  ...
  END SELECT
 ENDDO
 IF(IWEIGHT.EQ.0)WRITE(JU,'(A)') 'N'
 IF(IWEIGHT.GE.1)WRITE(JU,'(A)') 'WEIGHT'
   ...
   IF(TW.GT.0.0D0)THEN
    DO J=1,IPF(1)%NROW
     IF(ILIST(J).EQ.2)THEN
      READ(IPF(1)%INFO(IPF(1)%NCOL,J),*) W
      W=W/TW
      WRITE(IPF(1)%INFO(IPF(1)%NCOL,J),*) W
      ILIST(J)=1
     ENDIF
    ENDDO
   ENDIF
