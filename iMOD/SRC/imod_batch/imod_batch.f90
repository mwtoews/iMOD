...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=54
...
!   CALL IMODBATCH_CREATEVOXEL_MAIN()
 ...
 IF(.NOT.IDFREADSCALE(IDF(1)%FNAME,IDF(1),ISCALE,1,0.0,0))STOP 'Cannot read data for IDF(1)'
 IF(.NOT.IDFREADSCALE(IDF(2)%FNAME,IDF(2),ISCALE,1,0.0,0))STOP 'Cannot read data for IDF(2)'
 ...
 DO I=1,NZ
  ...
  DO J=1,SIZE(LISTNAME)
   ...
   IF(ZT(J).LE.Z1.AND.ZB(IORDER(J)).GE.Z2)THEN
    ...
    IF(.NOT.IDFREADSCALE(LISTNAME(IORDER(J)),IDF(3),2,1,0.0,0))STOP 'Cannot read data for IDF(3)'
