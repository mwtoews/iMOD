...
USE MOD_BATCH_UTL, ONLY : IMODBATCH_RUNFILE_READ
...
!      write(*,*) txt1id1,txt2id1,txt1id2,txt2id2,nlist
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL(KIND=DP_KIND) :: XMIN,YMIN,XMAX,YMAX,SMALL,X1,Y1,X2,Y2,XCOR,GAMMA,STDV,XT,G(5),AVERAGE,VARIANCE
...
! REAL(KIND=DP_KIND),DIMENSION(3,3) :: A
 ...
 ELLIPS_IDF%IU=0; ZONE_IDF%IU=0; IWFACTOR=0; ILOG=0
 ...
 ELSE
  ...
  CALL UTL_IDFSNAPTOGRID(MEAN%XMIN,MEAN%XMAX,MEAN%YMIN,MEAN%YMAX,MEAN%DX,MEAN%NCOL,MEAN%NROW)
 ...
 I=0; DO IROW=1,STDEV%NROW; DO ICOL=1,STDEV%NCOL
  IF(STDEV%X(ICOL,IROW).EQ.STDEV%NODATA)STDEV%X(ICOL,IROW)=SMALL
  IF(MEAN%X(ICOL,IROW).EQ.MEAN%NODATA)MEAN%X(ICOL,IROW)=0.0D0
...
! DO I=1,N; V(I)=V(I)/S; ENDDO
! T=0.0D0; DO I=1,N; T=T+V(I)**2.0; ENDDO; T=SQRT(T)
! S=1.0D0
...
!    x2=x2+1.0d0
!    do k=1,5
!     g(k)=UTL_GETGAMMA(X1,Y1,X2,Y2,RANGE,SILL,NUGGET,k)
!    enddo
...
!   XCOR=SILL-GAMMA
   ...
   COV(I,J)=SQRT(COV(I,I))*SQRT(COV(J,J))*XCOR; COV(J,I)=COV(I,J)
 ...
 !## save the inverse of the parameter covariance matrix
...
!   DO I=1,NE; CALL IPEST_LUBACKSUB_DBL(M,INDX,B(1,I),NE); ENDDO
 ...
 IF(N.LT.200)THEN
  ...
  FLUSH(10)
 ENDIF
! !## perform choleski-decomposition A=LLT
 CALL CHOLESKYDECOMPOSITION(COV,N) 
! DATA A/36.0D0,30.0D0,18.0D0,30.0D0,41.0D0,23.0D0,18.0D0,23.0D0,14.0D0/
...
! !## cov is 
! ALLOCATE(L(N,N),U(N,N),X(N)); L=0.0D0; U=0.0D0
! !## perform lu-decomposition
! CALL LUDCMP_GETLU(COV,L,U,N)
! WRITE(10,*) 'L=['
! DO J=1,N; WRITE(10,'(999G17.5)') (L(J,I),I=1,N); ENDDO
! WRITE(10,*) ']'
! WRITE(10,*) 'U=['
! DO J=1,N; WRITE(10,'(999G17.5)') (U(J,I),I=1,N); ENDDO
! WRITE(10,*) ']'
...
! DEALLOCATE(COV)
 ...
 DO ISIM=1,NSIM
  ...
  I=0; DO IROW=1,STDEV%NROW; DO ICOL=1,STDEV%NCOL
   I=I+1; MEAN%X(ICOL,IROW)=V(I)
  ENDDO; ENDDO
  MEAN%FNAME=IDFFILE(:INDEX(IDFFILE,'.',.TRUE.)-1)//'_MEAN_SIM'//TRIM(ITOS(ISIM))//'.IDF'
  IF(.NOT.IDFWRITE(MEAN,MEAN%FNAME,1))STOP
 ENDDO
 IF(N.LT.200)CLOSE(10)

 DEALLOCATE(COV,X,V); CALL IDFDEALLOCATEX(STDEV); CALL IDFDEALLOCATEX(MEAN)
