...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=86
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'CREATEWELBORELOG','CREATEENSEMBLES','IPFEVALUATE'/
...
!      write(*,*) txt1id1,txt2id1,txt1id2,txt2id2,nlist
 ...
 IF(UTL_READINITFILE('IPFFILE',LINE,IU,1))THEN
  ...
  IF(.NOT.ASC2IDF_INT_MAIN(1,XMIN,YMIN,XMAX,YMAX,IN_TYPE))STOP
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 INTEGER :: I,J,JU
 REAL(KIND=DP_KIND) :: D,XP,YP
 REAL(KIND=DP_KIND),DIMENSION(2) :: SD
 ...
 ALLOCATE(TRI(IPF(1)%NROW)); TRI%P1=0; TRI%P2=0
 DO I=1,IPF(1)%NROW; TRI(I)%X=IPF(1)%XYZ(1,I); TRI(I)%Y=IPF(1)%XYZ(2,I); ENDDO
 
 DO I=1,SIZE(TRI)
  !## already part of triangle
  IF(TRI(I)%P1.NE.0)CYCLE
  SD=HUGE(1.0)
  !## find triangle
  DO J=1,SIZE(TRI)
   IF(I.EQ.J)CYCLE
   D=UTL_DIST(TRI(I)%X,TRI(I)%Y,TRI(J)%X,TRI(J)%Y)
   IF(D.LT.SD(1))THEN
    SD(2)=SD(1); SD(1)=D; TRI(I)%P2=TRI(I)%P1; TRI(I)%P1=J
   ELSEIF(D.LT.SD(2))THEN
    SD(2)=D; TRI(I)%P2=J
   ENDIF
  ENDDO
 ENDDO

 IPF(3)%NROW=SIZE(TRI); IPF(3)%NCOL=2; IPF(3)%FNAME=IPFPPS
 ALLOCATE(IPF(3)%ATTRIB(2),IPF(3)%INFO(IPF(3)%NCOL,IPF(3)%NROW))
 IPF(3)%ATTRIB(1)='X_PILOTPOINT'; IPF(3)%ATTRIB(2)='Y_PILOTPOINT'
 DO I=1,IPF(1)%NROW
  XP=TRI(I)%X+TRI(TRI(I)%P1)%X+TRI(TRI(I)%P2)%X
  YP=TRI(I)%Y+TRI(TRI(I)%P1)%Y+TRI(TRI(I)%P2)%Y
  WRITE(IPF(3)%INFO(1,I),'(F15.3)') XP/3.0D0
  WRITE(IPF(3)%INFO(2,I),'(F15.3)') YP/3.0D0
 ENDDO
 ...
 DO I=1,IPF(1)%NROW
  WRITE(JU,'(I10)') I
  WRITE(JU,'(2F15.3)') TRI(I)%X        ,TRI(I)%Y
  WRITE(JU,'(2F15.3)') TRI(TRI(I)%P1)%X,TRI(TRI(I)%P1)%Y
  WRITE(JU,'(2F15.3)') TRI(TRI(I)%P2)%X,TRI(TRI(I)%P2)%Y
  WRITE(JU,'(2F15.3)') TRI(I)%X        ,TRI(I)%Y
  WRITE(JU,'(A)') 'END'
 ENDDO
 WRITE(JU,'(A)') 'END'
