...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=89
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'CREATEWELBORELOG','CREATEENSEMBLES','IPFEVALUATE','LAYERFROMTHICKNESS','MODPATH7','IPESTTOPARAM'/
...
!   WRITE(*,*) KSTP,KPER,PERTIM,TOTIM,TEXT,NCOL,NROW,ILAY
   ...
   DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
!    IF(NLD%X(ICOL,IROW).EQ.NLD%NODATA)THEN
    SUM%X(ICOL,IROW)=SUM%NODATA
    PHR%X(ICOL,IROW)=PHR%NODATA
!    ELSE
    IF(IDF%X(ICOL,IROW).NE.IDF%NODATA)THEN
...
!    ENDIF
  ...
  ENDDO
  CLOSE(JU)
  SUM%FNAME=TRIM(ROOT)//'\SUM.IDF'; IF(.NOT.IDFWRITE(SUM,SUM%FNAME,1))STOP
  PHR%FNAME=TRIM(ROOT)//'\PHR.IDF'; IF(.NOT.IDFWRITE(PHR,PHR%FNAME,1))STOP
 ENDIF
...
! OPTIONAL KEYWORDS
 ...
 INTEGER :: I,J,K,ILAY,N,M,NU,IROW,ICOL,IZ,SCL_D,SCL_U,IINV,ITOPIC,IP,JZ
