...
!     PBMAN%IWINDOW=4
!     WRITE(*,'(A,I1)') 'IGENMF6=',PBMAN%IGENMF6
!     IF(.NOT.UTL_READINITFILE('GENFNAME',LINE,IU,0))RETURN
!     READ(LINE,*) PBMAN%GENFNAME; WRITE(*,'(A)') 'GENFNAME='//TRIM(PBMAN%GENFNAME)
!    ENDIF
!   !## seawat/mt3d
!   CASE (4,5)
!    WRITE(*,'(/A/)') 'Export to a iMOD WQ RUNFILE'
!    IF(.NOT.UTL_READINITFILE('RUNFILE_OUT',LINE,IU,0))RETURN
!    READ(LINE,*) RUNFILE; WRITE(*,'(2A)') 'RUNFILE_OUT='//TRIM(RUNFILE)
!    IF(UTL_READINITFILE('OUTPUT_FOLDER',LINE,IU,1))THEN
!     READ(LINE,*) PBMAN%OUTPUT; WRITE(*,'(2A)') 'OUTPUT_FOLDER='//TRIM(PBMAN%OUTPUT)
...
!    IF(PBMAN%INT(I).EQ.0)WRITE(*,'(A,I1)') 'INT'//TRIM(TOPICS(I)%CMOD)//'=',PBMAN%INT(I)
!   ENDDO
!  ENDIF
!  
...
!!    IF(UTL_READINITFILE('MONTECARLO',LINE,IU,1))READ(LINE,*) PBMAN%MC
!!    WRITE(*,'(A,I10)') 'MONTECARLO=',PBMAN%MC
!    IF(UTL_READINITFILE('SVD_EIGV',LINE,IU,1))READ(LINE,*) PBMAN%EIGV
!    WRITE(*,'(A,F10.4)') 'SVD_EIGV=',PBMAN%EIGV
!   ENDIF
!  ENDIF
!
!  PBMAN%PTEST=0
...
! ENDIF
 ...
 ELSEIF(UTL_READINITFILE('GRB',LINE,IU,1))THEN
  ...
  READ(JU) IDF%YMIN
  READ(JU) ANGROT
  ALLOCATE(IDF%SX(IDF%NCOL)); READ(JU) (IDF%SX(I),I=1,IDF%NCOL)
  ALLOCATE(IDF%SY(IDF%NROW)); READ(JU) (IDF%SY(I),I=1,IDF%NROW)
 ...
 IF(UTL_READINITFILE('IMODE',LINE,IU,1))READ(LINE,*) IMODE
 WRITE(*,'(A)') 'IMODE='//TRIM(ITOS(IMODE))
