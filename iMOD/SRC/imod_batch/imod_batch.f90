 ...
 IF(UTL_READINITFILE('HED',LINE,IU,1))THEN
  ...
  SUM%X=0.0; PHR%X=PHR%NODATA; DO
   ...
   IF(PERTIM.EQ.0.0D0)THEN
    CDATE='STEADY-STATE'
   ELSE
    DTIME=DBLE(SDATE)+TOTIM
    CDATE=JDATETOFDATE(DTIME,SDATE,2)
   ENDIF

   READ(JU) ((IDF%X(ICOL,IROW),ICOL=1,NCOL),IROW=1,NROW)
   IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
...
!        WRITE(*,'(A,2I10,F10.3)') ' N M Q: ', N,M,Q
     ...
     ELSE
      DO ILAY=1,ABS(NDIM3)
       IDF%NODATA=1.0D30
       DO IROW=1,NROW; DO ICOL=1,NCOL; IDF%X(ICOL,IROW)=X(ICOL,IROW,ILAY); ENDDO; ENDDO
       IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
       IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
      ENDDO
     ENDIF
    ...
    CASE (6)
     ...
     TEXT=ADJUSTL(TXT2ID2)
     DO ILAY=1,ABS(NDIM3)
      IDF%X=IDF%NODATA
      DO IROW=1,NROW; DO ICOL=1,NCOL; IF(Y(ICOL,IROW,ILAY).NE.0.0D0)IDF%X(ICOL,IROW)=Y(ICOL,IROW,ILAY); ENDDO; ENDDO
      IDF%FNAME=TRIM(ROOT)//'\'//TRIM(TEXT)//'_'//TRIM(CDATE)//'_L'//TRIM(ITOS(ILAY))//'.IDF'
      IF(.NOT.IDFWRITE(IDF,IDF%FNAME,1))STOP
     ENDDO
     
     IF(NDAT.GT.1)DEALLOCATE(AUXTXT); DEALLOCATE(ID1,ID2,X,Y)
    CASE DEFAULT
