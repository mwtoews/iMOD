...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 INTEGER :: NLAY,ILAY,IVCW,DI,IROW,ICOL,NG,N,JU
 ...
 DO ILAY=1,NLAY
  ...
  DO IROW=1,BND(ILAY)%NROW,DI; DO ICOL=1,BND(ILAY)%NCOL,DI
   ...
   IF(ILAY.EQ.1)THEN
    NG=NG+1; PPT(ILAY)%X(ICOL,IROW)=NG
   ELSE
    IF(IVCW.EQ.1)THEN
 ...
 DO ILAY=1,NLAY; DO I=1,2; N=0
  DO IROW=1,BND(ILAY)%NROW,DI; DO ICOL=1,BND(ILAY)%NCOL,DI
   ...
   IF(I.EQ.2)WRITE(JU,'(2(F10.2,1X),I10,1X,2(F10.2,1X))') X,Y,INT(PPT(ILAY)%X(ICOL,IROW)),-REAL(ILAY,8)+0.5D0,-REAL(ILAY,8)-0.5D0
  ENDDO; ENDDO
  IF(I.EQ.1)THEN
