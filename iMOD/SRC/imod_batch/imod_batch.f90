 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   ...
   IF(ILAY.LT.NLAY)THEN
    ...
    IF(.NOT.UTL_READINITFILE('KVV'//TRIM(ITOS(ILAY)),LINE,IU,0))RETURN
 ...
 NP=0; DO ILAY=1,NLAY; DO I=1,2; N=0
  DO IROW=1,BND(ILAY)%NROW,DI; DO ICOL=1,BND(ILAY)%NCOL,DI
   ...
   IF(I.EQ.2)THEN
    ...
    NP=NP+1
    WRITE(JU,'(2(F10.2,1X),I10,1X,2(F10.2,1X))') X,Y,INT(PPT(ILAY)%X(ICOL,IROW)),-REAL(ILAY,8)+0.5D0,-REAL(ILAY,8)-0.5D0
    WRITE(KU,'(A)') '1,KH,'//TRIM(ITOS(ILAY))//','//TRIM(ITOS(NP))//',1.0,1.1,0.01,100.0,2.0,'//TRIM(ITOS(NG))//',1,KH_GRP'//TRIM(ITOS(NG))//',1.0'
   ENDIF
  ENDDO; ENDDO
  IF(I.EQ.1)THEN
   JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(OUTPUTFOLDER)//'\PILOTPOINT_L'//TRIM(ITOS(ILAY))//'.IPF',ACTION='WRITE',FORM='FORMATTED')
   WRITE(JU,*) N; WRITE(JU,*) 5; WRITE(JU,*) 'X'; WRITE(JU,*) 'Y'; WRITE(JU,*) 'ZONE'; WRITE(JU,*) 'T'; WRITE(JU,*) 'B'; WRITE(JU,*) '0,TXT'
  ELSE
   CLOSE(JU)
  ENDIF
 ENDDO; ENDDO
