...
! ENDIF
 ...
 INTEGER :: JU,NCOL,NROW,NLAY,ILAY,KSTP,KPER,ICOL,IROW,IOS,I1,I2,I3,NDIM1,NDIM2,NDIM3, &
  IMETH,NDAT,NLIST,I,J,LENTXT,NCELLS,NTXT,NJA,N,M,IPOS,NCON,SDATE
...
!  WRITE(*,'(A,F10.2)') 'STDEV=',STDV
 ...
 IF(PRESENT(OUTIPF))OUTIPF=FNAME(3)
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 INTEGER :: I,II,NLAY,NIPF,TNN,LU,ITRANSIENT
 INTEGER,DIMENSION(5) :: ICOLS
 
 !## perform ipf sample, return number of 
 IF(.NOT.IMODBATCH_IPFSAMPLE_MAIN(IPFFILE,ICOLS,NLAY))RETURN
...
! IF(.NOT.UTL_READINITFILE('NLAY',LINE,IU,0))RETURN
 ...
 !## define output ipf-file and open for writing
! LU=UTL_GETUNIT(); OPEN(LU,FILE=TRIM(FNAME(2)),STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
 
! !## define when to write to output files
! TNN=0
!   FNAMEHEADS=TRIM(FNAME(3))//'_L'//TRIM(ITOS(I))//'.IDF'  
!   CALL IPFEVALUATE(FNAME(1),IPFNAMES_IN(I),FNAME(2),FNAMEHEADS,I,NLAY,II,TNN,LU,ITRANSIENT)
! 
! IF(ALLOCATED(IPFNAMES_IN))DEALLOCATE(IPFNAMES_IN)
! CLOSE(LU)
