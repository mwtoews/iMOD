...
USE MOD_STOMP_PAR
USE MOD_STOMP, ONLY : STOMP_INIT,STOMP_READ,STOMP_CLOSE
...
! ENDIF
 ...
 IMPLICIT NONE
 INTEGER :: I,NLAY
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: TOP,BOT,KHV                
 ...
 IF(GRIDISG%IEXPORT.EQ.1)THEN
  ...
  IF(NLAY.GT.0)THEN
   ...
   ALLOCATE(TOP(NLAY),BOT(NLAY),KHV(NLAY))
   DO I=1,NLAY
 ...
 IF(.NOT.ISG2GRIDMAIN(ISGFILE,1,NLAY,TOP,BOT,KHV,GRIDISG))THEN; WRITE(*,'(/A/)') 'Error occured in ISG Grid routine'; ENDIF
 CALL ISGDEAL(1) 
 DO I=1,NLAY
  CALL IDFDEALLOCATE(TOP,SIZE(TOP)); CALL IDFDEALLOCATE(BOT,SIZE(BOT)); CALL IDFDEALLOCATE(KHV,SIZE(KHV))
 ENDDO
 DEALLOCATE(TOP,BOT,KHV)
