...
USE MOD_PMANAGER, ONLY : PMANAGERPRJ,PMANAGERRUN,PMANAGERINIT,PMANAGER_SAVEMF2005_CONSISTENCY
...
! !## nothing save by default
! ALLOCATE(PBMAN%SAVESHD(99)); PBMAN%SAVESHD=0
...
!  TNG=TNG-0.5*PI
 ...
 INTEGER :: I,J,IFLOW,IPNTR,IWINDOW,IWRITE,IGRAD,ITQP,NTQP,TTQP,IFORMAT,PITTSIZE,MINFRICTION
 ...
 ELSEIF(IFLOW.EQ.3)THEN
  ...
  CALL SOF_EXPORT(IDF,SIZE(IDF),IFORMAT,RAIN,MINFRICTION)
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 IMPLICIT NONE
 INTEGER :: I,J,K,N,IN_TYPE,JU,M,IDATE,SDATE,EDATE,DDATE,IOS,ICOL,IROW,JF,ID
 CHARACTER(LEN=52) :: GRIDFUNC,WC,ROOT
 CHARACTER(LEN=256) :: FNAME,SNAME
 REAL :: XMIN,YMIN,XMAX,YMAX,Z,Z1,Z2,MF
...
!    IF(.NOT.UTL_READINITFILE('ASSF_DZ',LINE,IU,0))RETURN
!    READ(LINE,*) ASSF_DZ; WRITE(*,'(A,F10.2)') 'ASSF_DZ=',ASSF_DZ
...
!      ASSF_STO_THRESHOLD(I)=0.001; IF(UTL_READINITFILE('STO_THRESHOLD'//TRIM(ITOS(I)),LINE,IU,1))READ(LINE,*) ASSF_STO_THRESHOLD(I)
!      WRITE(*,*) 'STO_THRESHOLD'//TRIM(ITOS(I))//'=',ASSF_STO_THRESHOLD(I)
!      ASSF_SPY_THRESHOLD(I)=0.15; IF(UTL_READINITFILE('SPY_THRESHOLD'//TRIM(ITOS(I)),LINE,IU,1))READ(LINE,*) ASSF_SPY_THRESHOLD(I)
!      WRITE(*,*) 'SPY_THRESHOLD'//TRIM(ITOS(I))//'=',ASSF_SPY_THRESHOLD(I)
     ENDDO
    ENDIF
 ...
 IF(N.EQ.0)THEN
  ...
  IF(ABS(ASSF_INDICATOR).GT.0.AND.ASSF_NTHRESHOLD.GT.1)THEN
   ...
   ALLOCATE(VOXEL(ASSF_NTHRESHOLD+3)) 
...
!     VOXEL(ASSF_NTHRESHOLD+5)%X(ICOL,IROW)=0.0
     ...
     DO I=1,ASSF_NTHRESHOLD
      IF(VOXEL(I)%X(ICOL,IROW).NE.VOXEL(I)%NODATA)THEN
       ...
       !## total fraction
       VOXEL(ASSF_NTHRESHOLD+3)%X(ICOL,IROW)=VOXEL(ASSF_NTHRESHOLD+3)%X(ICOL,IROW)+VOXEL(I)%X(ICOL,IROW)
      ENDIF
     ENDDO
     MF=1.0/MF
     DO I=1,ASSF_NTHRESHOLD
...
!      VOXEL(ASSF_NTHRESHOLD+4)%X(ICOL,IROW)=ASSF_STO_THRESHOLD(JF)
!      VOXEL(ASSF_NTHRESHOLD+5)%X(ICOL,IROW)=ASSF_SPY_THRESHOLD(JF)
     ELSE
      VOXEL(ASSF_NTHRESHOLD+2)%X(ICOL,IROW)=VOXEL(ASSF_NTHRESHOLD+2)%NODATA
!      VOXEL(ASSF_NTHRESHOLD+4)%X(ICOL,IROW)=VOXEL(ASSF_NTHRESHOLD+4)%NODATA
!      VOXEL(ASSF_NTHRESHOLD+5)%X(ICOL,IROW)=VOXEL(ASSF_NTHRESHOLD+5)%NODATA
     ENDIF
    ENDDO; ENDDO

    DO I=1,3; VOXEL(ASSF_NTHRESHOLD+I)%TOP=ASSF_TOP; VOXEL(ASSF_NTHRESHOLD+I)%BOT=ASSF_BOT; ENDDO
    
    IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_L.IDF'
    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+1),IDFFILE,1))STOP

    IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_K.IDF'
    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+2),IDFFILE,1))STOP

    IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_F.IDF'
    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+3),IDFFILE,1))STOP

!    IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_STO.IDF'
!    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+4),IDFFILE,1))STOP
!    IDFFILE =TRIM(FNAME)//'_T'//TRIM(RTOS(ASSF_TOP,'F',2))//'_SPY.IDF'
!    IF(.NOT.IDFWRITE(VOXEL(ASSF_NTHRESHOLD+5),IDFFILE,1))STOP
...
! I=INDEX(FUNC,'('); I=MAX(I,1)
 ...
 ENDIF
 WRITE(*,'(A,99F10.2)') TRIM(TXT)//'=',(XPOINTER(I),I=1,NPOINTER)
