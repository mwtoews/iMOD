...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=72
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'ADJUSTVOXELIDF','MFTOIMOD','KDC','IMODTOSTOMP'/
...
!## Write GEN and RUN file  ! FR 20131016
 ...
 IF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  ...
  IF(.NOT.UTL_READINITFILE('ISS',LINE,IU,0))RETURN
  READ(LINE,*) PBMAN%ISS; WRITE(*,'(A12,I1)') 'ISS=',PBMAN%ISS
  !## transient                              
...
!    READ(LINE,*) I
!    IF(I.EQ.0)THEN; LQBD=.FALSE.; WRITE(*,'(/A/)') 'IQUASI3D = 3D DISCRETISATION APPLIED (NO INTERBEDS)'; ENDIF
!    IF(I.EQ.1)THEN; LQBD=.TRUE.; WRITE(*,'(/A/)') 'IQUASI3D = QUASI 3D DISCRETISATION APPLIED (USING INTERBEDS)'; ENDIF
  END SELECT
...
! WRITE(*,'(A,I2)') 'ITALUSCOL=',DATCOL(9)
 ...
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  ...
  CALL UTL_IDFSNAPTOGRID_LLC(IDF(3)%XMIN,IDF(3)%XMAX,IDF(3)%YMIN,IDF(3)%YMAX,IDF(3)%DX,IDF(3)%NCOL,IDF(3)%NROW)
 ELSE
 ...
 N=0; DO IROW=1,IDF(3)%NROW; DO ICOL=1,IDF(3)%NCOL
  ...
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.IDF(3)%NCOL)THEN
    N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF(3),IDF(3)%NROW,IDF(3)%NCOL,IROW,ICOL,N, &
         0.0,0.0,0.0,0,1,TOP=IDF(1),BOT=IDF(2))
   ENDIF
  ENDIF
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   ...
   IF(IROW.LT.IDF(3)%NROW)THEN
    N=N+1; CALL PMANAGER_SAVEMF2005_HFB_GENFILES(JU,0,IPC,IDF(3),IDF(3)%NROW,IDF(3)%NCOL,IROW,ICOL,N, &
         0.0,0.0,0.0,0,2,TOP=IDF(1),BOT=IDF(2))
   ENDIF
  ENDIF
...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,ILAY,IROW,ICOL,NLAY,ND,IR1,IC1,IOS
 ...
 IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
  ...
  READ(LINE,*) IDF%DX; WRITE(*,'(A,F10.2)') 'CELL_SIZE=',IDF%DX
  IDF%DY=IDF%DX
  CALL UTL_IDFSNAPTOGRID_LLC(IDF%XMIN,IDF%XMAX,IDF%YMIN,IDF%YMAX,IDF%DX,IDF%NCOL,IDF%NROW,.TRUE.)
 ENDIF
...
!  CALL WCLIPBOARDPUTBITMAP(MPW%IBITMAP)
  ...
  !## create drawing
  IBITMAP=0; IF(NFIG.EQ.4)CALL IMODBATH_PLOTFIG(TSIZE,YFRACLEGEND,(/IDFLEGTXT,IPFLEGTXT,IFFLEGTXT/),IBITMAP)
