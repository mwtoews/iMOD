...
! READ(LINE,*) MAXZ; WRITE(*,'(A,F10.2)') 'MAXZ=',MAXZ
 ...
 DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
  DO ILAY=NLAY,1,-1
   T=TOP(ILAY)%X(ICOL,IROW)
   B=BOT(ILAY)%X(ICOL,IROW)
   D=T-B
   IF(D.GT.0.0)EXIT
 ...
 DO ILAY=1,NLAY
  WRITE(*,'(A,I10)') 'Writing resulting files for layer ',ILAY
...
! ENDIF
 ...
 IF(.NOT.ASC2IDF_IMPORTXYZ_VOXEL(XYZFILE,OUTPUTDIR,IWINDOW,X1,Y1,X2,Y2,CELLSIZE,DZV,TOP,BOT))RETURN
 ...
 ASSF_IDEPTH=0; ASSF_TOP=0.0; ASSF_BOT=0.0; ASSF_DZ=0.0; ASSF_NTHRESHOLD=1; ASSF_INDICATOR=0; ELLIPS_IDF%FNAME=''
