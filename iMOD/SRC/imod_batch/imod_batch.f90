...
!## Write GEN and RUN file  ! FR 20131016
 ...
 PBMAN%ISS=0; PBMAN%IFVDL=0; PBMAN%BNDFILE=''
  ...
  IF(M.GT.0.AND.M.EQ.IZCOL)THEN
   ...
   ELSE
    ...
    IF(UTL_READINITFILE('INDICATOR',LINE,IU,1))THEN
     ...
     ASSF_NTHRESHOLD=0
     READ(LINE,*) ASSF_NTHRESHOLD; WRITE(*,*) 'NTHRESHOLD=',ASSF_NTHRESHOLD
...
! ENDDO
 ...
 DO I=1,SIZE(IDFNAMES)
  ...
  CALL INFOSTAT(IDFNAMES(I),JU,IFORMAT) !,XMIN,YMIN,XMAX,YMAX) <--- OPTIONAL
 ENDDO
 CLOSE(JU)
...
! ENDIF
 ...
 CALL WGRTEXTSTRING(XP-BND,0.0+0.5*BND,'Powered by '//TRIM(UTL_IMODVERSION())//' Copyright © 2005-2013')
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 IPLOT=1; ITRANSIENT=0; NLAYER=0; NIPFS=0; NRDATE=0; IWEIGHT=0; ICOL=0
 ...
 IF(INDEX(UTL_CAP(INPUTFILE,'U'),'IPF'))THEN
  ...
  ICOL(3)=1; IF(UTL_READINITFILE('IMCOL',LINE,IU,1))READ(LINE,*) ICOL(3)
  WRITE(*,'(A,I1)') 'IMCOL=',ICOL(3)
  ICOL(4)=1; IF(UTL_READINITFILE('IHCOL',LINE,IU,1))READ(LINE,*) ICOL(4)
  WRITE(*,'(A,I1)') 'IHCOL=',ICOL(4)
  ICOL(5)=1; IF(UTL_READINITFILE('IWCOL',LINE,IU,1))READ(LINE,*) ICOL(5)
  WRITE(*,'(A,I1)') 'IWCOL=',ICOL(5)
  ICOL(6)=1; IF(UTL_READINITFILE('ILCOL',LINE,IU,1))READ(LINE,*) ICOL(6)
