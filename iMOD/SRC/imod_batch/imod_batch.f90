...
!## Write GEN and RUN file  ! FR 20131016
 ...
 PBMAN%INTANF=1; PBMAN%PDEBUG=0; PBMAN%PRESTART=0; PBMAN%TCOMPLEX='SIMPLE'; PBMAN%MC=0;
 PBMAN%EIGV=99.0D0
 ...
 ELSEIF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  ...
  SELECT CASE (PBMAN%IFORMAT)
   ...
   CASE (3)
    ...
    IF(UTL_READINITFILE('TCOMPLEX',LINE,IU,1))THEN
     READ(LINE,*) PBMAN%TCOMPLEX
    ENDIF
    SELECT CASE (PBMAN%TCOMPLEX)
     CASE ('SIMPLE','MODERATE','COMPLEX')
      WRITE(*,'(2A)')  'TCOMPLEX='//TRIM(PBMAN%TCOMPLEX)
     CASE DEFAULT
      WRITE(*,'(/A/)') 'TCOMPLEX ('//TRIM(PBMAN%TCOMPLEX)//') NEED TO BE SIMPLE, MODERATE OR COMPLEX'; STOP
    END SELECT
   CASE DEFAULT
  ...
  IF(PBMAN%IPEST.EQ.0)THEN
   ...
   IF(PBMAN%IPESTP.EQ.1)THEN
    ...
    IF(UTL_READINITFILE('PRESTART',LINE,IU,1))READ(LINE,*) PBMAN%PRESTART
    WRITE(*,'(A,I10)') 'PRESTART=',PBMAN%PRESTART
    IF(PBMAN%PRESTART.EQ.1)THEN
     WRITE(*,'(/1X,A$)') 'If a restart is applied, all files from a previous cycle need to be present, continue ?'
     READ(*,'(A1)') YN; IF(YN.NE.'Y'.AND.YN.NE.'y')STOP
    ENDIF
...
!   jdates(2)=20110101
 ...
 ICONSISTENT=0; IPRJ=0; IAGGREGATE=0; IWHB=0; IWINDOW=0
 
 !## read ini-file
 IF(.NOT.UTL_READINITFILE('SORTFILE',LINE,IU,0))RETURN
 READ(LINE,*) SORTFILE; WRITE(*,'(A)') 'SORTFILE='//TRIM(SORTFILE)
 
 IF(UTL_READINITFILE('ICONSISTENT',LINE,IU,1))READ(LINE,*) ICONSISTENT
 WRITE(*,'(A)') 'ICONSISTENT='//TRIM(ITOS(ICONSISTENT))
 ...
 IF(ICONSISTENT.EQ.1)THEN
  ...
  IF(.NOT.UTL_READINITFILE('CONDIR',LINE,IU,0))RETURN
  READ(LINE,*) CONDIR; WRITE(*,'(A)') 'CONDIR='//TRIM(CONDIR)
 ENDIF
 
 IF(UTL_READINITFILE('IWHB',LINE,IU,1))READ(LINE,*) IWHB
 WRITE(*,'(A)') 'IWHB='//TRIM(ITOS(IWHB))

 IF(IWHB.EQ.1)THEN
  IF(.NOT.UTL_READINITFILE('NIWHB',LINE,IU,0))RETURN
  READ(LINE,*) N; WRITE(*,'(A)') 'NIWHB='//TRIM(ITOS(N))
  ALLOCATE(ALIST(N))
  DO I=1,N
   IF(.NOT.UTL_READINITFILE('IWHBFILE'//TRIM(ITOS(I)),LINE,IU,0))RETURN
   READ(LINE,*) ALIST(I); WRITE(*,'(A)') 'IWHBFILE'//TRIM(ITOS(I))//'='//TRIM(ALIST(I))
  ENDDO
  IF(.NOT.UTL_READINITFILE('CONDIR',LINE,IU,0))RETURN
  READ(LINE,*) CONDIR; WRITE(*,'(A)') 'CONDIR='//TRIM(CONDIR)
  INDIR=CONDIR
  IF(.NOT.UTL_READINITFILE('MODELDIR',LINE,IU,0))RETURN
  READ(LINE,*) MODELDIR; WRITE(*,'(A)') 'MODELDIR='//TRIM(MODELDIR)
  IF(UTL_READINITFILE('WINDOW',LINE,IU,1))THEN
 ...
 IF(UTL_READINITFILE('IAGGREGATE',LINE,IU,1))READ(LINE,*) IAGGREGATE
 WRITE(*,'(A)') 'IAGGREGATE='//TRIM(ITOS(IAGGREGATE))

 IF(IAGGREGATE.EQ.1)THEN
