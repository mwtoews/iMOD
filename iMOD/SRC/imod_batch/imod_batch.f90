...
INTEGER,PARAMETER,PRIVATE :: MAXFUNC=81
...
DATA CFUNC/'PLOT','GXG','AHNFILTER','IDFSCALE','IDFCALC','IDFMERGE','CREATEIDF','IMPORTMODFLOW',&
  ...
  'FOSM','IPFEDIT','MSPNETRCH','MF6NETWORKS','CREATEPILOTPOINTS'/
 ...
 ELSEIF(UTL_READINITFILE('PRJFILE_IN',LINE,IU,1))THEN
  ...
  IF(PBMAN%IPEST.EQ.0)THEN
   ...
   IF(PBMAN%IPESTP.EQ.1)THEN
    ...
    IF(.NOT.UTL_READINITFILE('NSWAIT',LINE,IU,0))RETURN
    READ(LINE,*) PBMAN%NSWAIT; WRITE(*,'(A,I10)') 'NSWAIT=',PBMAN%NSWAIT
   ENDIF
  ENDIF
...
!  IF(GC_INIT_IDENTIFY(IU))
 ...
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: BND,VCW,TOP,BOT,KVV,PPT
 INTEGER :: NLAY,ILAY,IVCW,DI,IROW,ICOL,NG,N,JU,KU,NP
 REAL(KIND=DP_KIND) :: MINC,DX,T,K,C,X,Y
 CHARACTER(LEN=256) :: OUTPUTFOLDER
 CHARACTER(LEN=2) :: CPARAM
 
 IF(.NOT.UTL_READINITFILE('IVCW',LINE,IU,0))RETURN
 READ(LINE,*) IVCW; WRITE(*,'(A)') 'IVCW='//TRIM(ITOS(IVCW))
 CPARAM='KH'; IF(IVCW.EQ.1)CPARAM='KD'
 ...
 ALLOCATE(BND(NLAY),PPT(NLAY)); IF(IVCW.EQ.1)THEN; ALLOCATE(VCW(NLAY)); ELSE; ALLOCATE(TOP(NLAY),BOT(NLAY),KVV(NLAY)); ENDIF
 
 DO ILAY=1,NLAY
  CALL IDFNULLIFY(PPT(ILAY)); CALL IDFNULLIFY(BND(ILAY))
 ...
 DO ILAY=1,NLAY
  ...
  CALL IDFCOPY(BND(ILAY),PPT(ILAY))
 ENDDO
 DO ILAY=1,NLAY
  IF(IVCW.EQ.1)THEN
   IF(ILAY.LT.NLAY)THEN
  ...
  ENDIF
 ENDDO
 NG=0; NP=0; PPT%NODATA=-999.99D0
 DO ILAY=1,NLAY
  DI=INT(DX/BND(1)%DX)
  PPT(ILAY)%X=PPT(ILAY)%NODATA
  DO IROW=1,BND(ILAY)%NROW,DI; DO ICOL=1,BND(ILAY)%NCOL,DI
   !## skip inactive cells 
   IF(BND(ILAY)%X(ICOL,IROW).LE.0.0D0)CYCLE
   IF(ILAY.EQ.1)THEN
    NG=NG+1; NP=NP+1; PPT(ILAY)%X(ICOL,IROW)=NG
   ELSE
    IF(IVCW.EQ.1)THEN
    ...
    IF(C.GT.MINC)THEN
     ...
     N =NG
    ELSE
     N =PPT(ILAY-1)%X(ICOL,IROW)
    ENDIF
    NP=NP+1
    PPT(ILAY)%X(ICOL,IROW)=N
   ENDIF
  ENDDO; ENDDO
 ENDDO
 ...
 WRITE(KU,*) '10,0.0,0.01,0,0,1.0,0.0,3,0.005,0.0,2,'//TRIM(RTOS(DX*4.0D0,'F',2))//',0,1.0'

 DO ILAY=1,NLAY
  PPT(ILAY)%FNAME=TRIM(OUTPUTFOLDER)//'\PILOTPOINT_L'//TRIM(ITOS(ILAY))//'.IDF'
  IF(.NOT.IDFWRITE(PPT(ILAY),PPT(ILAY)%FNAME,1))STOP 'Cannot write data for PPT'
 ENDDO
 !## write IPF and PRJ
 NP=0; DO ILAY=1,NLAY; DO I=1,2; N=0
  DO IROW=1,BND(ILAY)%NROW,DI; DO ICOL=1,BND(ILAY)%NCOL,DI
   IF(PPT(ILAY)%X(ICOL,IROW).EQ.PPT(ILAY)%NODATA)CYCLE; N=N+1
   CALL IDFGETLOC(PPT(ILAY),IROW,ICOL,X,Y)
   IF(I.EQ.2)THEN
    NG=PPT(ILAY)%X(ICOL,IROW)
    NP=NP+1
    IF(IVCW.EQ.1)THEN
     WRITE(JU,'(2(F10.2,1X),2(I10,1X),2(F10.2,1X))') X,Y,NP,INT(PPT(ILAY)%X(ICOL,IROW)),-REAL(ILAY,8)+0.5D0,-REAL(ILAY,8)-0.5D0
    ELSE
     WRITE(JU,'(2(F10.2,1X),2(I10,1X),2(F10.2,1X))') X,Y,NP,INT(PPT(ILAY)%X(ICOL,IROW)),TOP(ILAY)%X(ICOL,IROW),BOT(ILAY)%X(ICOL,IROW)
    ENDIF
    WRITE(KU,'(A)') '1,'//TRIM(CPARAM)//','//TRIM(ITOS(ILAY))//','//TRIM(ITOS(NP))//',1.0,1.1,0.01,100.0,2.0,'// &
      TRIM(ITOS(NG))//',1,'//TRIM(CPARAM)//'_GRP'//TRIM(ITOS(NG))//',1.0'
   ENDIF
  ENDDO; ENDDO
  IF(I.EQ.1)THEN
   JU=UTL_GETUNIT(); OPEN(JU,FILE=TRIM(OUTPUTFOLDER)//'\PILOTPOINT_L'//TRIM(ITOS(ILAY))//'.IPF',ACTION='WRITE',FORM='FORMATTED')
 ...
 WRITE(KU,'(I10)') NLAY
 DO ILAY=1,NLAY
  WRITE(KU,'(A)') TRIM(OUTPUTFOLDER)//'\PILOTPOINT_L'//TRIM(ITOS(ILAY))//'.IPF'
 ENDDO
