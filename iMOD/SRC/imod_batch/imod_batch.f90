...
!## Write GEN and RUN file  ! FR 20131016
 ...
 PBMAN%DWEL=1; PBMAN%DISG=1; PBMAN%DMMFILE=0; PBMAN%NSUBMODEL=1; PBMAN%IPESTP=0;
 PBMAN%NLINESEARCH=1; PBMAN%NCPU=1
...
!    IF(.NOT.UTL_READINITFILE('NLINESEARCH',LINE,IU,0))RETURN
!    READ(LINE,*) PBMAN%NLINESEARCH; WRITE(*,'(A,I10)') 'NLINESEARCH=',PBMAN%NLINESEARCH
...
! ENDDO
 ...
 INTEGER(KIND=SP_KIND) :: SDATE,EDATE,NGXG,DIFFDAY,ICLUSTER,WCOLUMN
 ...
 MINMEASURE=24; DIFFDAY=5; NODATA=-999.99D0; ICLUSTER=0; ITXTFILE=0; RANGE=0.0D0; WCOLUMN=0
 ...
 IF(ICLUSTER.EQ.1)THEN
  ...
  IF(UTL_READINITFILE('WCOLUMN',LINE,IU,1))READ(LINE,*) WCOLUMN
  WRITE(*,'(A)') 'WCOLUMN='//TRIM(ITOS(WCOLUMN))
 ENDIF
  ...
  DO
   M=0; IC=0; DO I=1,IPF(1)%NROW
    ...
    N=0; DO J=1,IPF(1)%NROW
     ...
     IF(UTL_DIST(IPF(1)%XYZ(1,I),IPF(1)%XYZ(2,I),IPF(1)%XYZ(1,J),IPF(1)%XYZ(2,J)).LE.RANGE)N=N+1
    ENDDO
