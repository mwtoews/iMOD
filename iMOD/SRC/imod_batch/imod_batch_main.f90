...
!     CALL CREATEIMODBATCHKILL()
 ...
 WRITE(IU,'(A/)') 'rem  For more info on this function or iMOD Batch (e.g. in case of an error) check the Manual chapter "iMOD Batch functions"'
 WRITE(IU,'(A)') 'rem  Step 1: write INI file for this Batch Function'
 WRITE(IU,'(A)') 'rem  ------------------'
 WRITE(IU,'(A)') 'rem  REQUIRED ARGUMENTS '
 ...
 DO I=1,BAT(J)%N
  ...
  SELECT CASE (BAT(J)%LVL(I))
   ...
   CASE ('(OPT)')
    ...
    IF(K.EQ.0)THEN 
      WRITE(IU,'(A)') 'rem  ------------------'; K=1
      WRITE(IU,'(A)') 'rem  OPTIONAL ARGUMENTS --> Not needed? Comment out each command line using "rem"'; K=1
    ENDIF  
   CASE ('(DEP)')
    II=8
   CASE ('(DEP2)')
 ...
 ENDDO 
 WRITE(IU,'(A)') 
 WRITE(IU,'(A)') 'rem  Step 2: execute INI file with iMOD executable'
