...
CONTAINS
 ...
 I=0; DO IROW=1,STDEV%NROW; DO ICOL=1,STDEV%NCOL
  IF(STDEV%X(ICOL,IROW).NE.STDEV%NODATA.AND. &
   ...
   IF(KLOG)THEN
    LV=MEAN%X(ICOL ,IROW)-2.0D0*STDEV%X(ICOL,IROW); LV=MAX(LV,MINK); LV=LOG10(ABS(LV))
    HV=MEAN%X(ICOL ,IROW)+2.0D0*STDEV%X(ICOL,IROW); HV=MAX(HV,MINK); HV=LOG10(ABS(HV))
    STDEV%X(ICOL,IROW)=((HV-LV)/4.0D0)
    MEAN%X(ICOL ,IROW)=LOG10(MEAN%X(ICOL ,IROW))
   ENDIF
  ELSE
...
!     V(I)=10.0D0**(M(I)+V(I))
   ...
   ENDIF
  ENDDO
  !## all of the ensembles go to stdv
