...
USE MOD_UTL, ONLY : UTL_READPOINTER,UTL_READINITFILE,ITOS,UTL_REALTOSTRING,UTL_GETGAMMA,UTL_STDEF,UTL_CREATEDIR
USE MOD_IDF_PAR
...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE IMODBATCH_CREATEENSEMBLES_CHOLESKY(STDEV,MEAN,DIR,RANGE,NSIM,FNAME)
 !###======================================================================
 IMPLICIT NONE
 ...
 REAL(KIND=DP_KIND) :: GAMMA,XCOR,X1,Y1,X2,Y2,STDV,XT,SMALL,NUGGET,SILL,M1,M2
 TYPE(IDFOBJ) :: IDF
 LOGICAL :: KLOG
 ...
 I=0; DO IROW=1,STDEV%NROW; DO ICOL=1,STDEV%NCOL
  IF(STDEV%X(ICOL,IROW).NE.STDEV%NODATA.AND. &
     MEAN%X(ICOL ,IROW).NE.MEAN%NODATA)THEN
   IF(KLOG)THEN
!    STDEV%X(ICOL,IROW)=LOG(STDEV%X(ICOL,IROW))
!    MEAN%X(ICOL,IROW) =MEAN%X(ICOL ,IROW)
    !## convert to log-normal values
    M1=MEAN%X(ICOL ,IROW)**2.0D0
    M2=STDEV%X(ICOL,IROW)**2.0D0
    MEAN%X(ICOL ,IROW)=LOG(M1/SQRT(M2+M1))
    STDEV%X(ICOL,IROW)=SQRT(LOG(1.0D0+M2/M1))
   ENDIF
  ELSE
...
!   IF(KLOG)THEN
!    M1=M(I); M2=M1
!    COV(I,I)=(V(I)*V(I))/(M1*M2)
!    COV(I,I)=LOG(1.0D0+COV(I,I))
!   ELSE
   COV(I,I)=V(I)*V(I)
!   ENDIF
   COR(I,I)=1.0D0
...
!   IF(KLOG)THEN
!    M1=MEAN%X(ICOL ,IROW)
!    M2=MEAN%X(ICOL2,IROW2)
!    COV(I,J)=(V(I)*V(J))/(M1*M2)
!    COV(I,J)=LOG(1.0D0+COV(I,J))*XCOR
!   ELSE
...
!   ENDIF
 ...
 !## perform choleski-decomposition A=LTL
...
!   IF(KLOG)MEAN%X(ICOL,IROW)=EXP(MEAN%X(ICOL,IROW))
  ENDDO; ENDDO
