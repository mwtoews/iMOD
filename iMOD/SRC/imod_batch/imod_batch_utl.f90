...
USE MOD_IDF, ONLY : IDFNULLIFY,IDFALLOCATEX,IDFWRITE,IDFGETLOC,IDFIROWICOL
USE MOD_ASC2IDF_PAR, ONLY : ELLIPS_IDF
USE MOD_UTL, ONLY : UTL_READPOINTER,UTL_READINITFILE,ITOS,UTL_REALTOSTRING,UTL_GETGAMMA,UTL_STDEF, &
  UTL_CREATEDIR,UTL_MATMUL,UTL_READPOINTER_REAL,UTL_DIST_3D,UTL_SETUP_ROTATIONMATRIX
USE MOD_IDF_PAR
USE MOD_LUDCMP 
...
CONTAINS
 ...
 PBMAN%IDEFLAYER=0
 PBMAN%DMMFILE=0
 PBMAN%AFWATIDF%FNAME=''
 
 PBMAN%NLAMBDASEARCH=3
 ALLOCATE(PBMAN%LAMBDA_TEST(3)); PBMAN%LAMBDA_TEST=[0.1D0,1.0D0,10.0D0]
 PBMAN%NLINESEARCH=1
 ALLOCATE(PBMAN%LINE_SEARCH(1)); PBMAN%LINE_SEARCH=[1.0D0]
 ...
 PBMAN%SCR_ISTPCS=0
 !## output control (0,1,2)
 PBMAN%SCR_ISCROC=0

 !## use specific storage
 PBMAN%SPECIFICSTORAGE=0
...
!   ENDIF
 ...
 IF(PBMAN%IFORMAT.EQ.2.OR.PBMAN%IFORMAT.EQ.3)THEN
  IF(UTL_READINITFILE('IDEFLAYER',LINE,IU,1))READ(LINE,*) PBMAN%IDEFLAYER
  IF(IBATCH.EQ.1)WRITE(*,'(A,I10)') 'IDEFLAYER=',PBMAN%IDEFLAYER
  DO I=1,SIZE(TOPICS)
 ...
 IF(PBMAN%IFORMAT.EQ.2.OR.PBMAN%IFORMAT.EQ.3)THEN
  ...
  ENDIF
  IF(UTL_READINITFILE('SPECIFICSTORAGE',LINE,IU,1))READ(LINE,*) PBMAN%SPECIFICSTORAGE
  IF(IBATCH.EQ.1)WRITE(*,'(A,I1)') 'SPECIFICSTORAGE=',PBMAN%SPECIFICSTORAGE
 ENDIF
 ...
 IF(PBMAN%IFORMAT.EQ.2)THEN
  ...
  IF(PBMAN%DMMFILE.EQ.1)THEN
   IF(UTL_READINITFILE('AFWATIDF',LINE,IU,1))READ(LINE,*) PBMAN%AFWATIDF%FNAME
   IF(IBATCH.EQ.1)WRITE(*,'(A)') 'AFWATIDF='//TRIM(PBMAN%AFWATIDF%FNAME)
  ENDIF
  IF(UTL_READINITFILE('SCR_IMETHOD',LINE,IU,1))READ(LINE,*) PBMAN%SCR_IMETHOD
  IF(IBATCH.EQ.1)WRITE(*,'(A,I1)') 'SCR_IMETHOD=',PBMAN%SCR_IMETHOD
  IF(UTL_READINITFILE('SCR_ISTPCS',LINE,IU,1))READ(LINE,*) PBMAN%SCR_ISTPCS
  IF(IBATCH.EQ.1)WRITE(*,'(A,I1)') 'SCR_ICTPCS=',PBMAN%SCR_ISTPCS
  IF(UTL_READINITFILE('SCR_ISCROC',LINE,IU,1))READ(LINE,*) PBMAN%SCR_ISCROC
  IF(IBATCH.EQ.1)WRITE(*,'(A,I1)') 'SCR_ISCROC=',PBMAN%SCR_ISCROC
 ...
 IF(PBMAN%IPEST.EQ.0)THEN
  ...
  IF(PBMAN%IPESTP.EQ.1.OR.PBMAN%IIES.EQ.1)THEN
   ...
   PBMAN%NLAMBDASEARCH=SIZE(PBMAN%LAMBDA_TEST)
   
   IF(.NOT.UTL_READPOINTER_REAL(IU,N,PBMAN%LINE_SEARCH,'LINESEARCH',1))RETURN
   !## sort from small to big
   CALL QKSORT(SIZE(PBMAN%LINE_SEARCH),PBMAN%LINE_SEARCH)
   PBMAN%NLINESEARCH=SIZE(PBMAN%LINE_SEARCH)
 ...
 INTEGER :: N,I,J,IROW,ICOL,IROW2,ICOL2,JROW,JCOL,ISIM,KTYPE,NPOP,SEED
 REAL(KIND=DP_KIND),DIMENSION(:,:),ALLOCATABLE :: COV,COR,EV,EVAL
 REAL(KIND=DP_KIND),DIMENSION(:),ALLOCATABLE :: V,M,X,E,EW
 REAL(KIND=DP_KIND) :: GAMMA,XCOR,X1,Y1,X2,Y2,STDV,XT,SMALL,NUGGET,SILL,R
 TYPE(IDFOBJ) :: IDF
 LOGICAL :: LCHOL
 ...
 !## ani-isotropic (non-symmetric matrix gaat denk ik niet werken - of toch)
 IF(ASSOCIATED(ELLIPS_IDF(1,1)%X))THEN
  DO IROW=1,STDEV%NROW; DO ICOL=1,STDEV%NCOL
   ...
   AGL=0.0D0; DO I=1,SIZE(ELLIPS_IDF,1); DO J=1,SIZE(ELLIPS_IDF,2)
    CALL IDFIROWICOL(ELLIPS_IDF(I,J),JROW,JCOL,X1,Y1)
   ...
   !## get relation to all others
   DO JROW=1,STDEV%NROW; DO JCOL=1,STDEV%NCOL
   
    CALL IDFGETLOC(STDEV,IROW2,ICOL2,X2,Y2)

    !## inside current range
    IF(UTL_DIST_3D(X1,Y2,0.0D0,X2,Y2,0.0D0).LT.R)THEN
