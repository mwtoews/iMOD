...
CONTAINS
 ...
 SELECT CASE (PBMAN%IFORMAT)
  ...
  CASE (5)
   ...
   READ(LINE,*) PBMAN%RESULT_DIR; IF(IBATCH.EQ.1)WRITE(*,'(2A)') 'RESULT_DIR='//TRIM(PBMAN%RESULT_DIR)
   IF(.NOT.UTL_READINITFILE('FLOW_RESULT_DIR',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%FLOW_RESULT_DIR; IF(IBATCH.EQ.1)WRITE(*,'(2A)') 'FLOW_RESULT_DIR='//TRIM(PBMAN%FLOW_RESULT_DIR)

   IF(.NOT.UTL_READINITFILE('NPROBS',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%BTN%NPROBS; IF(IBATCH.EQ.1)WRITE(*,'(2A)') 'NPROBS='//TRIM(ITOS(PBMAN%BTN%NPROBS))
   IF(.NOT.UTL_READINITFILE('NPRMAS',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%BTN%NPRMAS; IF(IBATCH.EQ.1)WRITE(*,'(2A)') 'NPRMAS='//TRIM(ITOS(PBMAN%BTN%NPRMAS))
   IF(.NOT.UTL_READINITFILE('MIXELM',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%ADV%MIXELM; IF(IBATCH.EQ.1)WRITE(*,'(2A)') 'MIXELM='//TRIM(ITOS(PBMAN%ADV%MIXELM))
   IF(.NOT.UTL_READINITFILE('NADVFD',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%ADV%NADVFD; IF(IBATCH.EQ.1)WRITE(*,'(2A)') 'NADVFD='//TRIM(ITOS(PBMAN%ADV%NADVFD))
   IF(.NOT.UTL_READINITFILE('PERCEL',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%ADV%PERCEL; IF(IBATCH.EQ.1)WRITE(*,'(2A)') 'PERCEL='//TRIM(UTL_REALTOSTRING(PBMAN%ADV%PERCEL))
   IF(.NOT.UTL_READINITFILE('MXSS',LINE,IU,0))RETURN
   READ(LINE,*) PBMAN%SSM%MXSS; IF(IBATCH.EQ.1)WRITE(*,'(2A)') 'MXSS='//TRIM(ITOS(PBMAN%SSM%MXSS))
