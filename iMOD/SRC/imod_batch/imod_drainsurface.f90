...
LOGICAL :: LEX
REAL(KIND=DP_KIND) :: CLB,CRITAHN,SFCT,CS
...
CONTAINS
 ...
 IF(MPW%XMAX-MPW%XMIN.EQ.0.0D0)THEN
 ...
 DO I=1,3
  ...
  IF(.NOT.IDFREADSCALE(IDF(I)%FNAME,IDF(I),SCLTYPE_UP(I),SCLTYPE_DOWN(I),0.0D0,0))RETURN
 ENDDO
 ...
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: MV
 INTEGER :: JP,MP,NP,ILB,I,J,K,NN
 REAL(KIND=DP_KIND) :: RLB
 REAL(KIND=DP_KIND),DIMENSION(1) :: DMV
...
! ENDIF
 ...
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(NCOL,NROW) :: IP
 ...
 INTEGER,INTENT(OUT) :: NP
 INTEGER,INTENT(IN) :: NCOL,NROW,MP,IROW,ICOL
 REAL(KIND=DP_KIND),INTENT(IN) :: NODATA,CRITAHN
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(NCOL,NROW) :: AHN
