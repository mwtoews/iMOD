...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: IPLOT,I,J,NIPF,NIDF,NIFF,NISG,NMDF,NGEN,NUDF
 ...
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%ISEL)THEN
   ...
   IF(MP(IPLOT)%IPLOT.EQ.7)NUDF=NUDF+1
  ENDIF
 ENDDO
 ...
 I=0; IF(NIDF+NUDF.GT.1)I=1; CALL WMENUSETSTATE(ID_SORT,1,I)

 !## at least one selected
 I=0; IF(SUM(ACTLIST).GT.0)I=1
 !## not sustained whenever nod idf,ipf,iff,mdf is selected
 IF(NIDF+NIPF+NIFF+NMDF+NUDF.EQ.0)I=0
 ...
 IF(NGEN+NIPF+NIFF+NUDF.GT.0)THEN
 ...
 IF(NIDF+NIPF+NISG+NIFF+NMDF+NGEN+NUDF.NE.1)I=0
 ...
 I=1; IF(NIDF+NMDF+NUDF.EQ.0)I=0 !## only for idf and mdf
