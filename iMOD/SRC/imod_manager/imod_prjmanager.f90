...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 CHARACTER(LEN=512) :: DIR,EXFNAME,LINE
 INTEGER :: IU,I,J,K,N,IPER,KPER,IERROR,SCL_D,SCL_U,ITOPIC,ILAY
 REAL :: HNOFLOW
 TYPE(IDFOBJ) :: IDF
 ...
 EXFNAME=FNAME(INDEX(FNAME,'\',.TRUE.)+1:INDEX(FNAME,'.',.TRUE.)-1)
 WRITE(IU,'(A)') '# Nam File Generated by iMOD '//TRIM(RVERSION)
 WRITE(IU,'(A)') 'LIST 10 '//CHAR(39)//TRIM(EXFNAME)//'.LIST'//CHAR(39)
 WRITE(IU,'(A)') 'BAS6 11 '//CHAR(39)//TRIM(EXFNAME)//'.BAS6'//CHAR(39)
 WRITE(IU,'(A)') 'DIS 11 '//CHAR(39)//TRIM(EXFNAME)//'.BAS6'//CHAR(39)
...
! WRITE(IU,'(2(I10,A1),3(F10.3,A1),I10,A1,F10.3,A1,2(I10,A1))') 500,',',20,',',0.001,',',0.1,',',0.98,',',1,',',0.1,',',25,',',1
 ...
 DO ILAY=1,NLAY
  IF(TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%ICNST.EQ.1)THEN
   WRITE(IU,'(A)') 'CONSTANT '//TRIM(ITOS(INT(TOPICS(I)%STRESS(1)%FILES(1,ILAY)%ICNST)))
  ELSEIF(TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%ICNST.EQ.2)THEN
   EXFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\bas6\ibound_l'//TRIM(ITOS(ILAY))//'.arr'
   IDF%FNAME=TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%FNAME
   IF(.NOT.PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,SCL_U,SCL_D,1,IU,HNOFLOW))IERROR=1
  ENDIF
  IF(IERROR.EQ.1)THEN; CLOSE(IU); RETURN; ENDIF
 ENDDO
 WRITE(IU,*) HNOFLOW
 !## shd settings
 ...
 DO ILAY=1,NLAY
  IF(TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%ICNST.EQ.1)THEN
   WRITE(IU,'(A)') 'CONSTANT '//TRIM(ITOS(INT(TOPICS(I)%STRESS(1)%FILES(1,ILAY)%ICNST)))
  ELSEIF(TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%ICNST.EQ.2)THEN
   EXFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\bas6\shead_l'//TRIM(ITOS(ILAY))//'.arr'
   IDF%FNAME=TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%FNAME
   IF(.NOT.PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,SCL_U,SCL_D,0,IU,HNOFLOW))IERROR=1
  ENDIF
  IF(IERROR.EQ.1)THEN; CLOSE(IU); RETURN; ENDIF
 ENDDO
 ...
 DO ILAY=1,NLAY
  ...
  IF(TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%ICNST.EQ.1)THEN
   WRITE(IU,'(A)') 'CONSTANT '//TRIM(ITOS(INT(TOPICS(I)%STRESS(1)%FILES(1,ILAY)%ICNST)))
  ELSEIF(TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%ICNST.EQ.2)THEN
   EXFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\dis\top_l'//TRIM(ITOS(ILAY))//'.arr'
   IDF%FNAME=TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%FNAME
   IF(.NOT.PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,SCL_U,SCL_D,0,IU,HNOFLOW))IERROR=1
  ENDIF
  IF(IERROR.EQ.1)THEN; CLOSE(IU); RETURN; ENDIF
  ...
  IF(TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%ICNST.EQ.1)THEN
   WRITE(IU,'(A)') 'CONSTANT '//TRIM(ITOS(INT(TOPICS(I)%STRESS(1)%FILES(1,ILAY)%ICNST)))
  ELSEIF(TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%ICNST.EQ.2)THEN
   EXFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\dis\bot_l'//TRIM(ITOS(ILAY))//'.arr'
   IDF%FNAME=TOPICS(ITOPIC)%STRESS(1)%FILES(1,ILAY)%FNAME
   IF(.NOT.PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,SCL_U,SCL_D,0,IU,HNOFLOW))IERROR=1
  ENDIF
  IF(IERROR.EQ.1)THEN; CLOSE(IU); RETURN; ENDIF
...
! CALL IDFDEALLOCATEX(IDF,SIZE(IDF)); DEALLOCATE(IDF)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,SCL_U,SCL_D,IINT,IU,HNOFLOW)
 !###======================================================================
 IMPLICIT NONE
 ...
 DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL; IF(IDF%X(ICOL,IROW).EQ.IDF%NODATA)IDF%X(ICOL,IROW)=HNOFLOW; ENDDO; ENDDO
