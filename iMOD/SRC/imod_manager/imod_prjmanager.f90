...
! CHARACTER(LEN=512) :: LINE
 ...
 INTEGER :: IU,IULAK,KPER,IERROR,SCL_D,SCL_U,IINT,ITOPIC,ILAY,ISS,IINV,NRCHOP,NEVTOP
 INTEGER :: ICB,IROW, &
   ICOL,ISTEADY,NHED,NFLW,IFHBSS,NFHBX1,NFHBX2,NBDTIM,I,J,JD0,JD1,ISEC0,ISEC1
 ...
 !## for constant flux boudary use fhb-package !!!
 ...
 IF(.NOT.PMANAGER_SAVEMF2005_DIS_READ(DIR,DIRMNAME,IBATCH,ISS,LTB))RETURN
 ...
 IF(LBCF)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(ISS.EQ.1)THEN
    ...
    IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(EXFNAME,IDF,IINT,IU))RETURN
   ENDIF   
   !## kdw
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(EXFNAME,KD(ILAY),IINT,IU))RETURN
  ...
  DO ILAY=1,NLAY
   ...
   IF(ISS.EQ.1)THEN
    ...
    IF(LAYCON(ILAY).NE.1)THEN
     ITOPIC=12; SCL_D=1; SCL_U=2; IINT=0; IINV=0
     IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ(IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINV))RETURN
     CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,IDF,0,ITOPIC)
     EXFNAME=TRIM(DIR)//'\LPF7\SF2_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG)
     IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(EXFNAME,IDF,IINT,IU))RETURN
    ENDIF
   ENDIF
   ...
   IF(LQBD.AND.ILAY.NE.NLAY)THEN
    !## kvv
    ITOPIC=10; SCL_D=1; SCL_U=3; IINT=0; IINV=0
    IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ(IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINV))RETURN
    CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,IDF,0,ITOPIC)
...
!  WRITE(IULAK,'(A)') '# LAK7 File Generated by '//TRIM(UTL_IMODVERSION())
  ...
  DO I=1,9
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ(LAK(I),ITOPIC,1,I,1,SCL_D,SCL_U,IINV))RETURN 
   IF(I.EQ.1)THEN
   ...
   ENDIF
  ENDDO
  KPER=0; IF(.NOT.PMANAGER_SAVELAKE(KPER,IULAK,ILKCB,IBATCH,DIR))RETURN
 ENDIF 
 ...
 ALLOCATE(KVV(NLAY)); DO ILAY=1,SIZE(KVV); CALL IDFNULLIFY(KVV(ILAY)); ENDDO
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_DIS_READ(DIR,DIRMNAME,IBATCH,ISS,LTB)
 !####====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
...
!save_no_buf 
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_MSP(IBATCH) !NCOL,NROW,NLAY,IDF,IBATCH)
 !####====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IBATCH  !NCOL,NROW,NLAY,
 ...
 DO IROW=1,IDF%NROW
  DO ICOL=1,IDF%NCOL
   ...
   IF(ARND.GT.0.0)THEN
    ...
    MDND2=MDND2+(LYBE-1)*IDF%NCOL*NROW
...
! IF(PMANAGER_SAVEMF2005_HFB_COMPUTE)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 IF(IPER.EQ.0)THEN
  ...
  DO ILAY=1,NLAY
   LINE=TRIM(DIR)//'\LAK6\LKARR_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG)
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(LINE,KD(ILAY),1,IULAK))RETURN
  ENDDO
...
!      C3=D/KHV(ILAY)%X(ICOL,IROW)
      !## include vertical anisotropy
!      IF(ALLOCATED(KVA))C3=C3*1.0/KVA(ILAY)%X(ICOL,IROW)
     ENDIF
  ...
  DO ILAY=1,NLAY
   LINE=TRIM(DIR)//'\LAK6\BDLKNC_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG)
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(LINE,KD(ILAY),0,IULAK))RETURN
  ENDDO
