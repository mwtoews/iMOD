...
LOGICAL,PRIVATE :: LBCF,LLPF,LPCG,LPCGN,LSIP,LRCH
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 INTEGER :: IU,I,J,K,N,IPER,KPER,IERROR,SCL_D,SCL_U,IINT,ITOPIC,ILAY,ISS,IINV,NRCHOP
 INTEGER :: IHEDUN,IBCFCB,IRCHCB
 REAL :: HNOFLOW
 TYPE(IDFOBJ) :: IDF
 ...
 !## rch
 ITOPIC=26; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
 ...
 IF(LLPF) WRITE(IU,'(A)') 'LPF  13 '//CHAR(39)//TRIM(LINE)//'.LPF'//CHAR(39)
 IF(LPCG) WRITE(IU,'(A)') 'PCG  14 '//CHAR(39)//TRIM(LINE)//'.PCG'//CHAR(39)
 ...
 IF(LRCH) WRITE(IU,'(A)') 'RCH  16 '//CHAR(39)//TRIM(LINE)//'.RCH'//CHAR(39)
 ...
 IF(LLPF)THEN
  ...
  WRITE(IU,'(A)') '# LPF File Generated by iMOD '//TRIM(RVERSION)
  ...
  DO ILAY=1,NLAY
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf\hk',HNOFLOW,IINV))RETURN
   !## vka
   ITOPIC=8; SCL_D=1; SCL_U=2; IINT=0; IINV=0
   IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf\vka',HNOFLOW,IINV))RETURN
   !## sf1
   IF(ISS.EQ.1)THEN
    ITOPIC=11; SCL_D=1; SCL_U=2; IINT=0; IINV=0
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf\ss',HNOFLOW,IINV))RETURN
   ENDIF   
   IF(ILAY.NE.NLAY)THEN
    !## kvv
    ITOPIC=10; SCL_D=1; SCL_U=3; IINT=0; IINV=1
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf\vkcb',HNOFLOW,IINV))RETURN
   ENDIF
  ENDDO  
  CLOSE(IU)
 ...
 IF(LPCG)THEN
  !## construct pcg-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.PCG',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# PCG File Generated by iMOD '//TRIM(RVERSION)
 ...
 IF(LRCH)THEN
  ITOPIC=26
  !## construct rch-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.RCH',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# RCH File Generated by iMOD '//TRIM(RVERSION)
  LINE=TRIM(ITOS(NRCHOP))//','//TRIM(ITOS(IRCHCB)); WRITE(IU,'(A)') TRIM(LINE)
  IF(.NOT.PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,'rch6\rch'))RETURN
  CLOSE(IU)
 ENDIF
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,EXT)
 !###======================================================================
 IMPLICIT NONE
   ...
   SELECT CASE (ITOPIC)
    CASE (26) !## total rch
     DO IROW=1,BND(1)%NROW; DO ICOL=1,BND(1)%NCOL
      DO ISYS=2,NSYS; PCK(1,1)%X(ICOL,IROW)=PCK(1,1)%X(ICOL,IROW)+PCK(1,ISYS)%X(ICOL,IROW); ENDDO
     ENDDO; ENDDO
     IF(IIDEBUG.EQ.0)EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_t'//TRIM(ITOS(IPER))//'.arr'
     IF(IIDEBUG.EQ.1)EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_t'//TRIM(ITOS(IPER))//'.asc'
     IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(EXFNAME,PCK(1,1),IU,HNOFLOW))RETURN
 ...
 ELSEIF(TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%ICNST.EQ.2)THEN
  IF(IIDEBUG.EQ.0)EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_l'//TRIM(ITOS(ILAY))//'.arr'
  IF(IIDEBUG.EQ.1)EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_l'//TRIM(ITOS(ILAY))//'.asc'
