...
USE MOD_MDL_PAR, ONLY : REPLACESTRING,NCORES
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_WSELECTFILE,UTL_CAP,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_FILLDATES,NEWLINE,UTL_LISTOFFILES,  &
    ...
    UTL_DIRINFO_POINTER,UTL_IDFGETLAYERS,UTL_DIRINFO,UTL_FILLDATESDIALOG,UTL_GETCURRENTDATE,UTL_IDFGETDATE,UTL_GETREAL,UTL_WRITE_FREE,&
    UTL_SYSCOREINFO
...
LOGICAL,PRIVATE :: LFREEFORMAT=.TRUE.  !## use true free-format
...
!  CALL WGRIDPUTCELLSTRING(IDF_GRID1,I,J,FILES(J,I)(INDEX(FILES(J,I),'\',.TRUE.)+1:)) 
 ...
 DO I=1,NMAXCORES
  COPTS(I)=ITOS(I)
 ENDDO 
 CALL WDIALOGPUTMENU(IDF_MENU3,COPTS,NMAXCORES,1)
 DEALLOCATE(COPTS)
 
 CALL WDIALOGPUTOPTION(IDF_MENU4,PCG%PARTOPT+1)
 ...
 ELSEIF(IMODE.EQ.2)THEN
  IF(IBATCH.EQ.0)THEN
   IF(NCORES.GT.1)THEN
    ...
    WRITE(IU,'(A)') 'set np='//ITOS(NCORES)
    WRITE(IU,'(A)') ''
    WRITE(IU,'(A)') ':: Run model'
    WRITE(IU,'(A)') '"C:\Program Files\MPICH2\bin\mpiexec.exe" -np %np% -localonly "'//TRIM(PREFVAL(8))//'" '//'IMODFLOW.RUN'
   ELSE
...
! TOPICS(33)%TNAME='(PCG) Precondition Conjugate-Gradient'
 ...
 !## read ani
 IF(.NOT.PMANAGER_SAVEMF2005_ANI_READ(ISS,IPRT))RETURN
 !## read top/bot information
 ...
 !## save bcf file
 IF(.NOT.PMANAGER_SAVEMF2005_ANI_SAVE(DIR,DIRMNAME,IBATCH,ISS))RETURN
 !## save hfb file
...
!  ENDDO
 ...
 INTEGER :: ITOPIC,SCL_D,SCL_U,IROW,ICOL,ILAY,IR1,IR2,IC1,IC2,IL1,IL2
...
! WRITE(IU,'(A)') TRIM(LINE)
 ...
 LINE=TRIM(ITOS(IBCFCB))//','//TRIM(RTOS(-9999.99,'E',7))//',0,STORAGECOEFFICIENT,THICKSTRT,CONSTANTCV'
 WRITE(IU,'(A)') TRIM(LINE)
...
!associated with dry cells. 
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_ANI_READ(ISS,IPRT)
 !####====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ISS,IPRT
 ...
 DO ISYS=1,NSYS
  ...
  DO KTOP=1,NTOP
   ...
   IF(KTOP.EQ.1)THEN
    ...
    ENDIF
    CALL PMANAGER_SAVEMF2005_FCTIMP(0,ICNST,ANF(ILAY),FCT,IMP)
   ...
   ELSEIF(KTOP.EQ.2)THEN
    ...
    ENDIF
    CALL PMANAGER_SAVEMF2005_FCTIMP(0,ICNST,ANA(ILAY),FCT,IMP)
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_ANI_SAVE(DIR,DIRMNAME,IBATCH,ISS)
 !####====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 INTEGER,INTENT(IN) :: IBATCH,ISS
...
!save_no_buf 
 ...
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IUGEN,IUDAT
 ...
 ENDDO
 ITOPIC=15; IF(.NOT.PMANAGER_SAVEMF2005_HFB_COMPUTE(IDF,ITOPIC,IU,IUGEN,IUDAT,TOP,BOT,BND,KDW,IPRT))RETURN
...
! ENDIF
 ...
 ELSE
  ...
  IF(LFREEFORMAT)THEN
   CALL UTL_WRITE_FREE(JU,IDF,IINT) !IDF%X,IDF%NCOL,IDF%NROW,IINT)
  ELSE
   IF(IINT.EQ.1)THEN
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_HFB_COMPUTE(IDF,ITOPIC,IU,IUGEN,IUDAT,TOP,BOT,BND,KD,IPRT)
 !###======================================================================
 IMPLICIT NONE
 ...
 DO IWRITE=0,1
  ...
  DO ISYS=1,SIZE(TOPICS(ITOPIC)%STRESS(1)%FILES,2)
   ...
   IF(LEN_TRIM(IDF%FNAME).GT.0)THEN
    ...
    CALL PMANAGER_SAVEMF2005_HFB_EXPORT(IPC,IDF%NROW,IDF%NCOL,FCT*IMP,NHFBNP,IWRITE,IU,IUGEN,IUDAT,IDF,TOP,BOT,BND,KD,ILAY,TIDF,BIDF)
   ENDIF
 ...
 DO ILAY=1,NLAY
  ...
  ELSE
   ...
   CLOSE(IUDAT(ILAY),STATUS='DELETE')
  ENDIF
 ENDDO
 ...
 SUBROUTINE PMANAGER_SAVEMF2005_HFB_EXPORT(IPC,NROW,NCOL,HFBFCT,NHFB,IWRITE, &
               IU,IUGEN,IUDAT,IDF,TOP,BOT,BND,KD,ITB,TIDF,BIDF) 
 !###====================================================================
 IMPLICIT NONE
    ...
    DO ILAY=IL1,IL2
     ...
     IF(ICOL+1.LE.NCOL.AND.(FCT.GT.0.0.AND.FCT.LT.1.0))THEN
      ...
      IF(IWRITE.EQ.1)THEN
       ...
       CALL PMANAGER_SAVEMF2005_HFB_GENFILES(IUGEN(ILAY),IUDAT(ILAY),IPC,IDF,NROW,NCOL,IROW,ICOL,FCT,NHFB(ILAY))
      ENDIF
     ENDIF
    ENDDO
 ...
 !###====================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_HFB_GENFILES(IU,JU,IPC,IDF,NROW,NCOL,IROW,ICOL,FCT,N) 
 !###====================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 REAL,INTENT(IN) :: FCT
 INTEGER,INTENT(IN) :: NROW,NCOL,IROW,ICOL,IU,JU,N
 INTEGER(KIND=1),INTENT(IN),DIMENSION(NCOL,NROW,2) :: IPC 

 !## place vertical wall
 IF(IPC(ICOL,IROW,1).EQ.INT(1,1).AND.ICOL.LT.NCOL)THEN
  WRITE(JU,'(I10,A1,G10.5)') N,',',FCT
  WRITE(IU,'(I10)') N !1 
