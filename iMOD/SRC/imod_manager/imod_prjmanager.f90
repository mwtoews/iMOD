...
!     I,',',FNAMES(I)%ICNST,',',FNAMES(I)%ILAY,',',FNAMES(I)%FCT  ,',', &
...
!  WRITE(IU,'(A)') TRIM(LINE)
 ...
 !## save uzf package
 IF(.NOT.PMANAGER_SAVEMF2005_PCK(DIR,DIRMNAME,IBATCH,LUZF,18,IUZFCB1,'UZF',(/1/),IPRT))RETURN
 !## save mnw package
...
!   END SELECT
 ...
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,J,IFILE,NROWIPF,NCOLIPF,IEXT,IOS,N,KPER,IPER,NP,NFILES,ICNST
...
!  NSYS=SIZE(TOPICS(ITOPIC)%STRESS(KPER)%FILES,2)
  ...
  NFILES=PMANAGER_GETFNAMES(0,0,1,0,KPER)
  IF(NFILES.LE.0)RETURN

  DO IFILE=1,NFILES !SYS=1,NSYS

   ICNST =FNAMES(IFILE)%ICNST !TOPICS(ITOPIC)%STRESS(KPER)%FILES(1,ISYS)%FCT
   CNST  =FNAMES(IFILE)%CNST  !TOPICS(ITOPIC)%STRESS(KPER)%FILES(1,ISYS)%FCT
   FCT   =FNAMES(IFILE)%FCT   !TOPICS(ITOPIC)%STRESS(KPER)%FILES(1,ISYS)%FCT
   IMP   =FNAMES(IFILE)%IMP   !TOPICS(ITOPIC)%STRESS(KPER)%FILES(1,ISYS)%IMP
   ILAY  =FNAMES(IFILE)%ILAY  !TOPICS(ITOPIC)%STRESS(KPER)%FILES(1,ISYS)%ILAY
   SFNAME=FNAMES(IFILE)%FNAME !TOPICS(ITOPIC)%STRESS(KPER)%FILES(1,ISYS)%FNAME

   IF(IPRT.EQ.0)THEN
    WRITE(*,'(1X,A,I3,A1,I1,A1,I4.3,3(A1,G15.7),A1,A)') TOPICS(ITOPIC)%TNAME(1:5)//',', &
      IFILE,',',ICNST,',',ILAY,',',FCT,',',IMP,',',CNST,',',CHAR(39)//TRIM(SFNAME)//CHAR(39)
   ELSE
    ...
    IF(Q.NE.0.0)THEN
     DO ILAY=1,NLAY
      IF(TLP(ILAY).GT.0.0)THEN
       WRITE(JU,FRM) ILAY,IROW,ICOL,Q*TLP(ILAY),IFILE
       NP=NP+1
      ENDIF
     ENDDO
    ENDIF
...
!  ENDIF
 ...
 CHARACTER(LEN=30) :: FRM
 INTEGER :: IU,JU,ILAY,I,ISYS,KPER,IPER,NTOP,NSYS,NFILES,IFILE,ICNST
...
!## IFVDL SFT RCNC
 ...
 DO IPER=1,NPER
  ...
  NSYS=NFILES/TOPICS(ITOPIC)%NSUBTOPICS; IFILE=0
...
!   SFNAME=TOPICS(ITOPIC)%STRESS(KPER)%FILES(1,ISYS)%FNAME
 ...
 CHARACTER(LEN=30) :: FRM
 INTEGER :: IU,JU,ILAY,IROW,ICOL,I,J,KTOP,KPER,IPER,NP,NTOP,SCL_D,SCL_U,ICNST,NSYS,ISYS, &
    NBDTIM,NHED,NFLW,IFBND,NRCHOP,NEVTOP,NFILES,IFILE
 REAL,ALLOCATABLE,DIMENSION(:) :: TLP,KH,TP,BT
 INTEGER(KIND=8) :: ITIME,JTIME
 INTEGER,PARAMETER :: ICLAY=1 !## shift to nearest aquifer
...
!     CNST=TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%CNST; 
    ...
    ELSEIF(TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%ICNST.EQ.2)THEN
     PCK(KTOP)%FNAME=SFNAME !TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%FNAME
