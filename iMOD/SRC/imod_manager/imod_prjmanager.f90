    ...
    IF(Q.NE.0.0)THEN
     DO ILAY=1,NLAY
      IF(TLP(ILAY).GT.0.0)THEN
       ...
       ELSE
        WRITE(JU,FRM) ILAY,IROW,ICOL,Q*TLP(ILAY),1
       ENDIF
       NP=NP+1
      ENDIF
     ENDDO
...
!   IF(PBMAN%SSYSTEM.EQ.1)THEN
   LINE='NaN1#,'//TRIM(ITOS(ICB))//',AUX RFCT AUX ISUB RFACT RFCT RSUBSYS ISUB NOPRINT'
!   ELSE
!    LINE='NaN1#,'//TRIM(ITOS(ICB))//',AUX RFCT RFACT RFCT NOPRINT'
!   ENDIF
   WRITE(IU,'(A)') TRIM(LINE)
!## IFVDL SFT RCNC
      ...
      DO ILAY=1,NLAY
       ...
       CASE DEFAULT
        IF(PCK(JTOP(2))%X(ICOL,IROW).GT.0.0)THEN
         ...
         ELSE
          WRITE(JU,FRM) ILAY,IROW,ICOL,(PCK(JTOP(I))%X(ICOL,IROW),I=1,NTOP),1
         ENDIF
         NP=NP+1
        ENDIF
      END SELECT
...
! ELSE
!  LINE=TRIM(ITOS(SUM(NO)))//','//TRIM(ITOS(CB))//','//TRIM(CAUX)//' NOPRINT'
! ENDIF
  ...
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.NCOL)THEN
    ...
    DO ILAY=IL1,IL2
     ...
     IF(FCT.NE.0.0)THEN
      IF(IWRITE.EQ.1.AND.ICOL+1.LE.NCOL)THEN
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   IF(IROW.LT.NROW)THEN
    ...
    DO ILAY=IL1,IL2
     ...
     !## enter fault if resistance <> 0.0
     IF(FCT.NE.0.0)THEN
      IF(IWRITE.EQ.1.AND.IROW+1.LT.NROW)THEN
       NHFB(ILAY)=NHFB(ILAY)+1
