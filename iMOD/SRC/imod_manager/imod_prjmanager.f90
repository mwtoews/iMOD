...
LOGICAL,PRIVATE :: LBCF,LLPF,LPCG,LPCGN,LSIP
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 INTEGER :: IU,I,J,K,N,IPER,KPER,IERROR,SCL_D,SCL_U,IINT,ITOPIC,ILAY,IBCFCB,ISS,IHEDUN,IINV
 REAL :: HNOFLOW
 TYPE(IDFOBJ) :: IDF
 ...
 WRITE(IU,'(A,I3,A)') 'DATA(BINARY) ',IBCFCB,CHAR(39)//TRIM(LINE)//'_BDGSTO'//CHAR(39) ! BDGBND BDGFRF BDGFFF BDGFLF'
   
! WRITE(IU,'(A)') 'LIST 12 '//CHAR(39)//TRIM(DIR)//'.BCF6'//CHAR(39)
! DO I=1,MAXTOPICS
!  IF(.NOT.ASSOCIATED(TOPICS(I)%STRESS))CYCLE
!  IF(.NOT.ASSOCIATED(TOPICS(I)%STRESS(1)%FILES))CYCLE
!  WRITE(IU,'(A)') '1,1,0 '//TRIM(TOPICS(I)%TNAME)
! ENDDO
 ...
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bas6\ibound',HNOFLOW,IINV))RETURN
 ENDDO
 WRITE(IU,*) HNOFLOW
 !## shd settings
 ITOPIC=5; SCL_D=1; SCL_U=2; IINT=0; IINV=0
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bas6\shead',HNOFLOW,IINV))RETURN
 ENDDO
 CLOSE(IU)
 ...
 ENDDO
 WRITE(IU,'(A)') TRIM(LINE)
 WRITE(IU,'(A)') 'CONSTANT '//TRIM(RTOS(IDF%DX,'E',7))
 WRITE(IU,'(A)') 'CONSTANT '//TRIM(RTOS(IDF%DY,'E',7))
 ...
 DO ILAY=1,NLAY
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'dis\top',HNOFLOW,IINV))RETURN
   ELSE
    WRITE(IU,'(A)') 'CONSTANT 0.0'
   ENDIF
  ELSE
   WRITE(IU,'(A)') 'CONSTANT 0.0'
  ENDIF
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'dis\bot',HNOFLOW,IINV))RETURN
   ELSE
    WRITE(IU,'(A)') 'CONSTANT 0.0'
   ENDIF
  ELSE
 ...
 IF(LBCF)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(ISS.EQ.1)THEN
    ...
    IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\sf1',HNOFLOW,IINV))RETURN
   ENDIF   
   !## kdw
   ITOPIC=6; SCL_D=1; SCL_U=3; IINT=0; IINV=0
   IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\tran',HNOFLOW,IINV))RETURN
   IF(ILAY.NE.NLAY)THEN
    !## vcont
    ITOPIC=9; SCL_D=1; SCL_U=6; IINT=0; IINV=1
    IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\vcnt',HNOFLOW,IINV))RETURN
   ENDIF
  ENDDO  
  CLOSE(IU)
 ...
 IF(LLPF)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf\hk',HNOFLOW,IINV))RETURN
   !## vka
   ITOPIC=8; SCL_D=1; SCL_U=2; IINT=0; IINV=0
   IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf\vka',HNOFLOW,IINV))RETURN
   !## sf1
   IF(ISS.EQ.1)THEN
    ITOPIC=11; SCL_D=1; SCL_U=2; IINT=0; IINV=0
    IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf\ss',HNOFLOW,IINV))RETURN
   ENDIF   
   IF(ILAY.NE.NLAY)THEN
    !## kvv
    ITOPIC=10; SCL_D=1; SCL_U=3; IINT=0; IINV=1
    IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf\vkcb',HNOFLOW,IINV))RETURN
   ENDIF
  ENDDO  
  CLOSE(IU)
...
! ENDDO
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_SAVE(IU,IDF,BND,ITOPIC,IPER,ISYS,ILAY,SCL_D,SCL_U,IINT,DIR,EXT,HNOFLOW,IINV)
 !###======================================================================
 IMPLICIT NONE
 ...
 PMANAGER_SAVEMF2005_SAVE=.FALSE.
...
!  IF(IIDEBUG.EQ.0)EXFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\'//TRIM(EXT)//'_l'//TRIM(ITOS(ILAY))//'.arr'
!  IF(IIDEBUG.EQ.1)EXFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\'//TRIM(EXT)//'_l'//TRIM(ITOS(ILAY))//'.asc'
  IDF%FNAME=TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%FNAME
  PMANAGER_SAVEMF2005_SAVE=PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,BND,ILAY,SCL_U,SCL_D,IINT,IU,HNOFLOW,FCT,IMP,IINV)
  IF(.NOT.PMANAGER_SAVEMF2005_SAVE)CLOSE(IU)
 ENDIF

 PMANAGER_SAVEMF2005_SAVE=.TRUE.
 
 END FUNCTION PMANAGER_SAVEMF2005_SAVE
 
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,BND,ILAY,SCL_U,SCL_D,IINT,IU,HNOFLOW,FCT,IMP,IINV)
 !###======================================================================
 IMPLICIT NONE
 ...
 INTEGER,INTENT(IN) :: SCL_U,SCL_D,IINT,IU,IINV,ILAY
 INTEGER :: JU,IROW,ICOL,I
 
 PMANAGER_SAVEMF2005_U2DREL=.FALSE.
 ...
 PMANAGER_SAVEMF2005_U2DREL=.TRUE.
   
 END FUNCTION PMANAGER_SAVEMF2005_U2DREL
