...
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_WSELECTFILE,UTL_CAP,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_FILLDATES,NEWLINE, &
...
!INTEGER,PRIVATE :: SIMGRO_NROW,SIMGRO_NCOL,SIMGRO_NLAY
TYPE SIMGRO_OBJ
...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,II,J,K,N,ITYPE,ID,IPER,ITOPIC,IOS,IST,IYR,IMH,IDY,ISUBTOPIC,IDATE,ISYS,IOPTION
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_EXTRAFILES()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ITYPE
 TYPE(WIN_MESSAGE) :: MESSAGE
 CHARACTER(LEN=256),POINTER,DIMENSION(:) :: FNAME
 CHARACTER(LEN=256) :: EFNAME
 INTEGER :: N
     
 CALL WDIALOGLOAD(ID_DPMANAGER_FILES,ID_DPMANAGER_FILES)
 CALL WDIALOGTITLE('Extra files')
 CALL WDIALOGPUTIMAGE(ID_OPEN,ID_ICONOPEN,1)
 CALL WDIALOGPUTIMAGE(ID_DELETE,ID_ICONDELETE,1)

 IF(ASSOCIATED(TOPICS(1)%STRESS(1)%INPFILES))THEN
  N=SIZE(TOPICS(1)%STRESS(1)%INPFILES); ALLOCATE(FNAME(N))
 ENDIF
 
 CALL PMANAGER_EXTRAFILES_FIELDS(FNAME)
 CALL WDIALOGSHOW(-1,-1,0,3)
    ...
    SELECT CASE (MESSAGE%VALUE1)
     CASE (ID_OPEN)
      IF(UTL_WSELECTFILE('All files (*.*)|*.*|',LOADDIALOG+MUSTEXIST+PROMPTON+ &
         DIRCHANGE+APPENDEXT,EFNAME,'Load All Files (*.*)'))CALL PMANAGER_EXTRAFILES_FIELDS(FNAME,1,EFNAME=EFNAME)
     CASE (ID_DELETE)
      CALL WDIALOGGETMENU(IDF_MENU1,N,EFNAME); CALL PMANAGER_EXTRAFILES_FIELDS(FNAME,-1,EFNAME=EFNAME)
     CASE (IDOK)
     CASE (IDHELP)
     CASE (IDCANCEL)
      EXIT
 ...
 END SUBROUTINE PMANAGER_EXTRAFILES

 !###======================================================================
 SUBROUTINE PMANAGER_EXTRAFILES_FIELDS(FNAME,IADD,EFNAME)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IADD
 CHARACTER(LEN=256),INTENT(IN),OPTIONAL :: EFNAME
 CHARACTER(LEN=256),POINTER,DIMENSION(:),INTENT(INOUT) :: FNAME
 CHARACTER(LEN=256),POINTER,DIMENSION(:) :: FNAME_BU

 IF(PRESENT(EFNAME))THEN
 ...
 END SUBROUTINE PMANAGER_EXTRAFILES_FIELDS
 ...
 DO I=1,MAXTOPICS  
  ...
  IF(I.EQ.1)THEN
   IF(ASSOCIATED(TOPICS(I)%STRESS(1)%INPFILES))THEN
    ...
    DO J=1,K; WRITE(IU,'(A)') TRIM(TOPICS(I)%STRESS(1)%INPFILES(J)); ENDDO
   ENDIF
  ENDIF
