...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 INTEGER :: IU,I,J,K,IPER,KPER,N,ISTEADY
 ...
 ISTEADY=0; IF(SIM(0)%DELT.EQ.0.0)ISTEADY=1
 
 WRITE(IU,'(A)') TRIM(PREFVAL(1))//'\MODELS\'//TRIM(MODELNAME)
 WRITE(IU,'(10(I10,A1))') NLAY,',',MXNLAY,',',NPER+ISTEADY,',',0,',',1,',',0,',',0,',',0,',',IUNCONF,',0'
 ...
 DO KPER=1,NPER
  ...
  DO I=1,MAXTOPICS  
   ...
   IF(KPER.EQ.1)THEN
    ...
    JTIME=ITIME
    IPER=PMANAGER_GETIPER(ITIME,JTIME,TOPICS(I)%STRESS) 
   ELSE
    ...
    JTIME=SIM(KPER  )%IYR*10000000000+SIM(KPER  )%IMT*100000000+SIM(KPER-1)%IDY*1000000+ &
          SIM(KPER  )%IHR*10000      +SIM(KPER  )%IMT*100      +SIM(KPER-1)%ISC
    IPER=PMANAGER_GETIPER(ITIME,JTIME,TOPICS(I)%STRESS)
   ENDIF
...
!    ID=UTL_IDATETOJDATE(IDATE); ID=ID-JD1+1; IF(ID.GT.0.AND.ID.LE.NPER)JLIST(ID)=1
 ...
 !###======================================================================
 INTEGER FUNCTION PMANAGER_GETIPER(ITIME,JTIME,STRESS) 
 !###======================================================================
 IMPLICIT NONE
...
! CALL WDIALOGGETCHECKBOX(IDF_CHECK2,ISTEADY)
 ...
 IF(SIM(0)%DELT.LT.0.0)THEN
  CALL WGRIDROWS(IDF_GRID1,NPER)
  DO I=1,NPER; CALL WGRIDLABELROW(IDF_GRID1,I,TRIM(ITOS(I))); ENDDO
  CALL WGRIDPUTSTRING (IDF_GRID1,1,SIM(1:)%CDATE,NPER)
  CALL WGRIDPUTREAL   (IDF_GRID1,2,SIM(1:)%DELT ,NPER)
  CALL WGRIDPUTINTEGER(IDF_GRID1,3,SIM(1:)%ISAVE,NPER)
 ELSE
  ...
  CALL WGRIDPUTINTEGER(IDF_GRID1,3,SIM%ISAVE,NPER+1)
 ENDIF
