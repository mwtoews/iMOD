...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 DO ILAY=1,NLAY
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,TOP(ILAY),BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'DIS\TOP',HNOFLOW,IINV))RETURN
 ...
 IF(LBCF)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,KD(ILAY),BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'BCF6\TRAN',HNOFLOW,IINV))RETURN
