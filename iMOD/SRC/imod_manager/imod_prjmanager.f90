...
! CHARACTER(LEN=256),POINTER,DIMENSION(:,:) :: FILES
 ...
 LOGICAL :: LEX
! CHARACTER(LEN=256),ALLOCATABLE,DIMENSION(:) :: LFILES
...
! ENDDO; ENDDO; ENDDO
 ...
 IF(ISUBMODEL.EQ.1)THEN
  ...
  SUBMODEL(1)=MAX(SUBMODEL(1),IDF%XMIN); SUBMODEL(2)=MAX(SUBMODEL(2),IDF%YMIN)
  SUBMODEL(3)=MIN(SUBMODEL(3),IDF%XMAX); SUBMODEL(4)=MIN(SUBMODEL(4),IDF%YMAX)
  ...
  DO ISYS=1,NSYS 
   ...
   IF(ISGREAD((/SFNAME/),IBATCH))THEN
    ...
    SELECT CASE (ITOPIC)
     ...
     !## open sfr file
     CASE (30)
      IF(.NOT.ISG2SFR(BND(1)%NROW,BND(1)%NCOL,NLAY,ILAY,IPER,NPER,NP,JU,GRIDISG,EXFNAME,TOP,BOT))EXIT 
    END SELECT
