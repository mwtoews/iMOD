...
!  ENDDO
 ...
 IF(LLAK)THEN
  ...
  IULAK=UTL_GETUNIT(); CALL OSD_OPEN(IULAK,FILE=TRIM(DIRMNAME)//'.LAK6',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IULAK.EQ.0)RETURN
  WRITE(IULAK,'(A)') '# LAK6 File Generated by '//TRIM(UTL_IMODVERSION())
  ...
  DO I=1,9
   ...
   ENDIF
  ENDDO
  KPER=0; IF(.NOT.PMANAGER_SAVELAKE(KPER,IULAK,ILKCB,IBATCH))RETURN
 ENDIF 
...
! IF(PMANAGER_SAVEMF2005_HFB)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVELAKE(IPER,IULAK,ILKCB,IBATCH)
 !###======================================================================
 IMPLICIT NONE
 ...
 IF(IPER.EQ.0)THEN
  ...
  DO J=2,4
   DO I=1,NLAKES
    LVL=0.0; ILVL=0.0
    DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
     IF(INT(LAK(1)%X(ICOL,IROW)).EQ.ULAKES(I))THEN
      LVL=LVL+LAK(J)%X(ICOL,IROW); ILVL=ILVL+1.0
     ENDIF
    ENDDO; ENDDO
    IF(ILVL.LE.0.0)THEN
     IF(IBATCH.EQ.0)THEN
      CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot assign a lakelevel for lake '//TRIM(RTOS(ULAKES(I),'F',1)),'Error')
      RETURN
     ELSE
      WRITE(*,'(A)') 'iMOD cannot assign a lakelevel for lake '//TRIM(RTOS(ULAKES(I),'F',1)); STOP
     ENDIF
    ENDIF
    LVL=LVL/ILVL
    IF(I.EQ.1)THEN
  ...
  WRITE(IULAK,'(A)') '1,1,0'
!  !## usage of array kd
!  DO ILAY=1,NLAY; KD(ILAY)%X=BND(ILAY)%X; ENDDO
!  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL 
!  ENDDO; ENDDO
 ENDIF
...
! ENDIF      
 ...
 TOPICS(32)%SNAME(3)='Minimal Lake Levels'
