...
! WRITE(IU,'(A)') 'STOPERROR'
 ...
 DO ILAY=1,NLAY
  ...
  CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,SHD(ILAY),0,IINV)
 ENDDO
 ...
 DO ILAY=1,NLAY
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    ...
    CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,TOP(ILAY),0,0)
    LEX=.TRUE.
   ENDIF
  ENDIF
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    ...
    CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,BOT(ILAY),0,0)
    LEX=.TRUE.
   ENDIF
  ENDIF
 ...
 IF(LBCF)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(ISS.EQ.1)THEN
    ...
    CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,IDF,0,IINV)
   ...
   CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,KD(ILAY),0,IINV)
 ...
 CHARACTER(LEN=256) :: EXFNAME
 REAL :: FCT,IMP,CNST
 INTEGER :: ILAY,IROW,ICOL,IS1,ICNST
...
!## cannot be here
     ...
     SELECT CASE (ITOPIC)
      CASE (22:28)
       ...
       CALL PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,PCK(K,ISYS),1,0)
     END SELECT   
...
!  EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG)
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_CORRECT(ILAY,BND,IDF,ITYPE,IINV)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IINV,ILAY,ITYPE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
