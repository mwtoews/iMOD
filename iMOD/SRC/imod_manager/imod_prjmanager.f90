...
!    Z=INT(GETREAL(LINE,IOS))
!    IF(IOS.EQ.0)THEN
!     IDF%X=Z
!    ELSE
!     IDF%FNAME=LINE; SCL_UP= SCL_D=
...
!   ENDIF
 ...
 DO I=1,MAXTOPICS
  ...
  IF(I.EQ.20)THEN
   ...
   CALL PMANAGER_SAVEPST(IU,1); CYCLE
  ENDIF
...
!  DO IROW=1,IDF%NROW; WRITE(JU,*) (IDF%X(ICOL,IROW),ICOL=1,IDF%NCOL); ENDDO
 ...
 !## compute block-faces
 ALLOCATE(IPC(IDF%NCOL,IDF%NROW,2),NHFBNP(NLAY))
 ...
 DO ILAY=1,NLAY
  IF(NHFBNP(ILAY).GT.0)THEN
   CLOSE(IUGEN(ILAY))
  ELSE
   CLOSE(IUGEN(ILAY),STATUS='DELETE')
  ENDIF
 ENDDO
...
! IF(PMANAGER_SAVEMF2005_HFB_COMPUTE)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 REAL :: FCT,DZ,DX,DY,C,T1,T2,CR,NODATA,K
...
!     IF(IWRITE.EQ.1)CALL PMANAGER_SAVEMF2005_HFB_GENFILES(IUGEN(ILAY),IPC,IDF,NROW,NCOL,IROW,ICOL)
     ...
     IF(ICOL+1.LE.NCOL)THEN
      ...
      IF(IWRITE.EQ.1)THEN
       ...
       !## write line in genfile
       CALL PMANAGER_SAVEMF2005_HFB_GENFILES(IUGEN(ILAY),IPC,IDF,NROW,NCOL,IROW,ICOL)
      ENDIF
     ENDIF
    ENDDO
