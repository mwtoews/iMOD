...
USE MOD_IDF, ONLY : IDFREAD,IDFDEALLOCATE,IDFNULLIFY,IDFREADSCALE
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 INTEGER :: IU,I,J,K,N,IPER,KPER,IERROR,SCL_D,SCL_U,IINT,ITOPIC,ILAY,IBCFCB,ISS
 REAL :: HNOFLOW
 TYPE(IDFOBJ) :: IDF
 ...
 IF(LBCF)THEN
  WRITE(IU,'(A)') 'BCF6 13 '//CHAR(39)//TRIM(LINE)//'.BCF6'//CHAR(39)
  WRITE(IU,'(A,I3,A)') 'DATA(BINARY) ',IBCFCB,' BDGSTO BDGBND BDGFRF BDGFFF BDGFLF'
 ENDIF
...
! WRITE(IU,'(2(I10,A1),3(F10.3,A1),I10,A1,F10.3,A1,2(I10,A1))') 500,',',20,',',0.001,',',0.1,',',0.98,',',1,',',0.1,',',25,',',1
 ...
 WRITE(IU,'(A)') 'FREE'
 !## bnd settings
 ITOPIC=4; SCL_D=0; SCL_U=1; IINT=1
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bas6\ibound',HNOFLOW))RETURN
 ENDDO
 WRITE(IU,*) HNOFLOW
 !## shd settings
 ITOPIC=5; SCL_D=1; SCL_U=2; IINT=0
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bas6\shead',HNOFLOW))RETURN
 ENDDO
 ...
 DO ILAY=1,NLAY
  ITOPIC=2
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS).AND. &
     ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
   IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'dis\top',HNOFLOW))RETURN
  ELSE
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS).AND. &
     ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
   IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'dis\bot',HNOFLOW))RETURN
  ELSE
   WRITE(IU,'(A)') 'CONSTANT 0.0'
  ENDIF
 ENDDO
 ...
 IF(LBCF)THEN
  ...
  WRITE(IU,'(A)') '# BCF6 File Generated by iMOD '//TRIM(RVERSION)
  LINE=TRIM(ITOS(IBCFCB))//','//TRIM(RTOS(HNOFLOW,'E',7))//',0,1.0,1,0'
  WRITE(IU,'(A)') TRIM(LINE)
  !## ltype code
  LINE=''; DO ILAY=1,NLAY; LINE=TRIM(LINE)//' 00'; ENDDO
  WRITE(IU,'(A)') TRIM(LINE)
  ...
  DO ILAY=1,NLAY
   !## sf1
   IF(ISS.EQ.1)THEN
    ITOPIC=11; SCL_D=1; SCL_U=2; IINT=0
    IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\sf1',HNOFLOW))RETURN
   ENDIF   
   !## kdw
   ITOPIC=6; SCL_D=1; SCL_U=3; IINT=0
   IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\tran',HNOFLOW))RETURN
   IF(ILAY.NE.NLAY)THEN
    !## vcont
    ITOPIC=9; SCL_D=1; SCL_U=6; IINT=0
    IF(.NOT.PMANAGER_SAVEMF2005_SAVE(IU,IDF,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\vcnt',HNOFLOW))RETURN
   ENDIF
...
! ENDDO
 ...
 CLOSE(IU)
! CALL IDFDEALLOCATEX(IDF,SIZE(IDF)); DEALLOCATE(IDF)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_SAVE(IU,IDF,ITOPIC,IPER,ISYS,ILAY,SCL_D,SCL_U,IINT,DIR,EXT,HNOFLOW)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IU,ITOPIC,IPER,ISYS,ILAY,SCL_D,SCL_U,IINT
 ...
 ELSEIF(TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%ICNST.EQ.2)THEN
  EXFNAME=DIR(:INDEX(DIR,'\',.TRUE.)-1)//'\'//TRIM(EXT)//'_l'//TRIM(ITOS(ILAY))//'.arr'
  IDF%FNAME=TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%FNAME
  PMANAGER_SAVEMF2005_SAVE=PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,SCL_U,SCL_D,IINT,IU,HNOFLOW,FCT,IMP)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_U2DREL(EXFNAME,IDF,SCL_U,SCL_D,IINT,IU,HNOFLOW,FCT,IMP)
 !###======================================================================
 IMPLICIT NONE
 ...
 INTEGER,INTENT(IN) :: SCL_U,SCL_D,IINT,IU
 INTEGER :: JU,IROW,ICOL,I
 
 PMANAGER_SAVEMF2005_U2DREL=.FALSE.

 CALL UTL_CREATEDIR(EXFNAME(:INDEX(EXFNAME,'\',.TRUE.)-1))
 SFNAME=EXFNAME; DO I=1,2; SFNAME=SFNAME(:INDEX(SFNAME,'\',.TRUE.)-1); ENDDO
 I=LEN_TRIM(SFNAME); SFNAME='.'//EXFNAME(I+1:)
 
 WRITE(IU,'(A)') 'OPEN/CLOSE '//TRIM(SFNAME)//' 1 (FREE) 0'

 !## read/clip/scale idf file
 IF(.NOT.IDFREADSCALE(IDF%FNAME,IDF,SCL_U,SCL_D,1.0,0))RETURN
 
 JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=EXFNAME,STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
 ...
 DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
  ...
  ENDIF
  IF()THEN
   IDF%X(ICOL,IROW)=1.0/(IDF%X(ICOL,IROW)*(IDF%DX**2.0))
  ENDIF
 ENDDO; ENDDO
 ...
 IF(ASSOCIATED(TOPICS(2)%STRESS).AND.ASSOCIATED(TOPICS(3 )%STRESS).AND. &
    ASSOCIATED(TOPICS(7)%STRESS).AND.ASSOCIATED(TOPICS(10)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(2 )%STRESS(1)%FILES).AND. & !## top
     ASSOCIATED(TOPICS(3 )%STRESS(1)%FILES).AND. & !## bot
