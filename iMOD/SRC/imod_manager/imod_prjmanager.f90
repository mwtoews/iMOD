...
!  CALL WDIALOGPUTSTRING(IDOK,'Adjust Parameters for System')
 ...
 IF(MESSAGE%VALUE1.EQ.IDOK.OR.MESSAGE%VALUE1.EQ.IDOK3)THEN
  ...
  !## fill in for all layers
!  IF(PRJ(ISUBTOPIC)%ICNST.EQ.3)THEN
!  ENDIF
  
  PRJ(ISUBTOPIC)%ICNST=MIN(PRJ(ISUBTOPIC)%ICNST,2)
    ...
    DO J=1,NSYS
     ...
     IF(IOS.EQ.0)THEN
      ...
      IF(IOS.EQ.0)THEN
       ...
       ELSEIF(TOPICS(I)%STRESS(L)%FILES(K,J)%ICNST.EQ.2)THEN !.OR. &
...
!  WRITE(IU,'(A)') TRIM(LINE)
 ...
 !## save hfb file
 IF(.NOT.PMANAGER_SAVEMF2005_HFB(IBATCH,DIRMNAME,IPRT))RETURN
 !## save pcg file
 ...
 !## save mnw package
 IF(.NOT.PMANAGER_SAVEMF2005_MNW(DIR,DIRMNAME,IBATCH,LMNW,19,IWL2CB,'MNW',IPRT))RETURN
 !## save wel package
...
!   END SELECT
 ...
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,J,NROWIPF,NCOLIPF,IEXT,IOS,N,KPER,IPER,NP,ICNST,ISYS,NSYS
 ...
 !## header
 LINE='NaN1,'//TRIM(ITOS(ICB))//',AUX ISUB DSUBSYS ISUB NOPRINT'; WRITE(IU,'(A)') TRIM(LINE)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_MNW(DIR,DIRMNAME,IBATCH,LEX,ITOPIC,ICB,CPCK,IPRT)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IBATCH,ICB,ITOPIC,IPRT
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME,CPCK
...
!save_no_buf 
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_HFB(IBATCH,DIRMNAME,IPRT)
 !####====================================================================
 IMPLICIT NONE
 ...
 ENDDO
 ITOPIC=15; IF(.NOT.PMANAGER_SAVEMF2005_HFB_COMPUTE(IDF,ITOPIC,IU,IUGEN,TOP,BOT,BND,KDW,IPRT))RETURN
 DEALLOCATE(IUGEN)
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_HFB_COMPUTE(IDF,ITOPIC,IU,IUGEN,TOP,BOT,BND,KD,IPRT)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ITOPIC,IU,IPRT
 INTEGER,INTENT(IN),DIMENSION(:) :: IUGEN
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 TYPE(IDFOBJ),DIMENSION(NLAY),INTENT(INOUT) :: TOP,BOT,BND,KD
 REAL :: FCT,IMP,CNST
 INTEGER :: ILAY,ISYS,IWRITE,MXFB,NPHFB,NSYS,ICNST
...
!     RETURN
 ...
 INTEGER :: II,I,J,K,IPER,ITOPIC,ILAY
 CHARACTER(LEN=52) :: WC
 ...
 MAXNLAY=9999; ALLOCATE(ALAY(MXNLAY)); ALAY=0
...
!  MXNLAY=MIN(MXNLAY,NLAY)
 ...
 INTEGER :: ITYPE
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: IDY,IYR,IMH,ITOPIC,IPER,I,J,K,KK,II,MINJD,MAXJD,IDATE,IHR,IMT,ISC,IHMS,MINHMS,MAXHMS,ISTEADY,ISTEP
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: IDF
 LOGICAL :: LEX
 CHARACTER(LEN=256),POINTER,DIMENSION(:) :: LISTNAME
   ...
   DO I=1,SIZE(TOPICS(K)%STRESS(J)%FILES,1)
    ...
    DO II=1,SIZE(TOPICS(K)%STRESS(J)%FILES,2)
     IF(TOPICS(K)%STRESS(J)%FILES(I,II)%ICNST.EQ.2)THEN
      ...
      ENDIF
      EXIT JLOOP
!     ELSEIF(TOPICS(K)%STRESS(J)%FILES(I,II)%ICNST.EQ.3)THEN
!      KK=INDEX(TOPICS(K)%STRESS(J)%FILES(I,II)%FNAME,'\',.TRUE.)
!      IF(KK.GT.0)THEN
!       WC=TRIM(TOPICS(K)%STRESS(J)%FILES(I,II)%FNAME(KK+1:))
!       DIR=TOPICS(K)%STRESS(J)%FILES(I,II)%FNAME(:KK-1)
!       !## get list of filenames
!       IF(UTL_DIRINFO_POINTER(DIR,WC,LISTNAME,'F'))THEN
!        IF(.NOT.IDFREAD(IDF(1),TRIM(DIR)//'\'//TRIM(LISTNAME(1)),0))THEN
!         CALL IDFDEALLOCATE(IDF,SIZE(IDF)); DEALLOCATE(IDF); RETURN
!        ENDIF
!        DEALLOCATE(LISTNAME)
!        EXIT JLOOP
