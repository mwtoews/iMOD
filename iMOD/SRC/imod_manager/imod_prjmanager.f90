...
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_WSELECTFILE,UTL_CAP,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_FILLDATES, &
    IDATETOGDATE,UTL_IDATETOJDATE,UTL_GDATE,UTL_JDATETOIDATE,JD,UTL_IDFSNAPTOGRID,UTL_CREATEDIR
USE MOD_IDF, ONLY : IDFREAD,IDFDEALLOCATE,IDFNULLIFY,IDFREADSCALE,IDFCOPY,IDFDEALLOCATEX
...
INTEGER,PRIVATE :: NLAY,NPER,MXNLAY,IUNCONF,ISTEADY,IFORMAT
REAL,PRIVATE :: MINKD,MINC
LOGICAL,PRIVATE :: LBCF,LLPF,LPCG,LPCGN,LSIP,LRCH,LEVT,LDRN,LRIV,LGHB
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 ELSEIF(ID.EQ.ID_SAVERUN)THEN
  ...
  IF(.NOT.PMANAGER_INITSIM())THEN; DEALLOCATE(SIM); RETURN; ENDIF
  ...
  IF(RUNFNAME.EQ.'')THEN
   ...
   ELSEIF(IFORMAT.EQ.2)THEN
    ...
    IF(.NOT.UTL_WSELECTFILE('Modflow 2005 Nam Files (*.nam)|*.nam|',  &
          SAVEDIALOG+PROMPTON+DIRCHANGE+APPENDEXT,FNAME,'Save Modflow 2005 Files'))RETURN
   ENDIF
  ELSE
   FNAME=RUNFNAME
  ENDIF
 ...
 LINE=TRIM(ITOS(MXITER))//','//TRIM(ITOS(ITER1))//','// & 
      ...
      TRIM(RTOS(RELAX,'E',7))//','//TRIM(ITOS(NPCOND))//',0.1,25,1'
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005(FNAME,XMIN,YMIN,XMAX,YMAX,DX)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: FNAME
 REAL,INTENT(IN),OPTIONAL :: XMIN,YMIN,XMAX,YMAX,DX
 CHARACTER(LEN=512) :: DIR,LINE
 INTEGER :: IU,I,J,K,N,IPER,KPER,IERROR,SCL_D,SCL_U,IINT,ITOPIC,ILAY,ISS,IINV,NRCHOP,NEVTOP,NP
 INTEGER :: IHEDUN,IBCFCB,IRCHCB,IEVTCB,IDRNCB,IRIVCB,IGHBCB
 REAL :: HNOFLOW
 TYPE(IDFOBJ) :: IDF
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: BND
 ...
 LDRN=.FALSE.; ITOPIC=22; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LDRN=.TRUE.
 ENDIF 
 !## riv
 LRIV=.FALSE.; ITOPIC=23; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LRIV=.TRUE.
 ENDIF 
 !## evt
 LEVT=.FALSE.; ITOPIC=24; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LEVT=.TRUE.
 ENDIF 
 !## ghb
 LGHB=.FALSE.; ITOPIC=25; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LGHB=.TRUE.
 ENDIF 
 !## rch
 LRCH=.FALSE.; ITOPIC=26; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LRCH=.TRUE.
 ENDIF 

 DIR=FNAME(:INDEX(FNAME,'.',.TRUE.)-1)

 MXNLAY=NLAY; MINKD=0.10; MINC =0.01
 ...
 WRITE(IU,'(A)') 'DIS  12 '//CHAR(39)//TRIM(LINE)//'.DIS'//CHAR(39)
 ...
 IF(LDRN) WRITE(IU,'(A)') 'DRN  18 '//CHAR(39)//TRIM(LINE)//'.DRN7'//CHAR(39)
 IF(LRIV) WRITE(IU,'(A)') 'RIV  19 '//CHAR(39)//TRIM(LINE)//'.RIV7'//CHAR(39)
 ...
 IF(LDRN)WRITE(IU,'(A,I3,A)') 'DATA(BINARY) ',IDRNCB,' '//CHAR(39)//TRIM(LINE)//'_FDRN'//CHAR(39) 
 IF(LRIV)WRITE(IU,'(A,I3,A)') 'DATA(BINARY) ',IRIVCB,' '//CHAR(39)//TRIM(LINE)//'_FRIV'//CHAR(39) 
 ...
 IF(.NOT.PRESENT(XMIN))THEN
  IF(.NOT.IDFREAD(IDF,TOPICS(4)%STRESS(1)%FILES(1,1)%FNAME,0))THEN; CLOSE(IU); RETURN; ENDIF
 ELSE
  IDF%XMIN=XMIN; IDF%YMIN=YMIN; IDF%XMAX=XMAX; IDF%YMAX=YMAX; IDF%DX=DX
 ...
 !## construct bas6-file
 IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.BAS6',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
 WRITE(IU,'(A)') '# BAS6 File Generated by iMOD '//TRIM(RVERSION)
 WRITE(IU,'(A)') 'FREE'
 !## bnd settings
 ITOPIC=4; SCL_D=0; SCL_U=1; IINT=1; IINV=0
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bas6\ibound',HNOFLOW,IINV))RETURN
 ENDDO
 WRITE(IU,*) HNOFLOW
 !## shd settings
 ITOPIC=5; SCL_D=1; SCL_U=2; IINT=0; IINV=0
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bas6\shead',HNOFLOW,IINV))RETURN
 ENDDO
 CLOSE(IU)

 CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.DIS'//'...')

 !## construct dis-file
 IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.DIS',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
 WRITE(IU,'(A)') '# DIS File Generated by iMOD '//TRIM(RVERSION)
 ...
 DO ILAY=1,NLAY
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'dis\top',HNOFLOW,IINV))RETURN
   ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'
   ENDIF
  ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'
  ENDIF
  ITOPIC=3
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'dis\bot',HNOFLOW,IINV))RETURN
   ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'
   ENDIF
  ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'
  ENDIF
 ...
 IF(LBCF)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(ISS.EQ.1)THEN
    ...
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\sf1',HNOFLOW,IINV))RETURN
   ENDIF   
   !## kdw
   ITOPIC=6; SCL_D=1; SCL_U=3; IINT=0; IINV=0
   IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\tran',HNOFLOW,IINV))RETURN
   IF(ILAY.NE.NLAY)THEN
    !## vcont
    ITOPIC=9; SCL_D=1; SCL_U=6; IINT=0; IINV=1
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\vcnt',HNOFLOW,IINV))RETURN
   ENDIF
  ENDDO  
  CLOSE(IU)
 ...
 !## use lpf7
 IF(LLPF)THEN
  CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.LPF'//'...')
  !## construct lpf7-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.LPF',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# LPF7 File Generated by iMOD '//TRIM(RVERSION)
  LINE=TRIM(ITOS(IBCFCB))//','//TRIM(RTOS(HNOFLOW,'E',7))//',0'
  WRITE(IU,'(A)') TRIM(LINE)
  !## laytyp code
  ...
  DO ILAY=1,NLAY
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf7\hk',HNOFLOW,IINV))RETURN
   !## vka
   ITOPIC=8; SCL_D=1; SCL_U=2; IINT=0; IINV=0
   IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf7\vka',HNOFLOW,IINV))RETURN
   !## sf1
   IF(ISS.EQ.1)THEN
    ITOPIC=11; SCL_D=1; SCL_U=2; IINT=0; IINV=0
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf7\ss',HNOFLOW,IINV))RETURN
   ENDIF   
   IF(ILAY.NE.NLAY)THEN
    !## kvv
    ITOPIC=10; SCL_D=1; SCL_U=3; IINT=0; IINV=1
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf7\vkcb',HNOFLOW,IINV))RETURN
   ENDIF
  ENDDO  
  CLOSE(IU)
 ...
 !## use rch
 IF(LRCH)THEN
  CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.RCH7'//'...')
  ITOPIC=26
  !## construct rch-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.RCH7',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# RCH7 File Generated by iMOD '//TRIM(RVERSION)
  LINE=TRIM(ITOS(NRCHOP))//','//TRIM(ITOS(IRCHCB)); WRITE(IU,'(A)') TRIM(LINE)
  IF(.NOT.PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,'rch7\rch',NP))RETURN
  CLOSE(IU)
 ENDIF

 !## use evt
 IF(LEVT)THEN
  CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.EVT7'//'...')
  ITOPIC=24
  !## construct evt-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.EVT7',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# EVT7 File Generated by iMOD '//TRIM(RVERSION)
  LINE=TRIM(ITOS(NEVTOP))//','//TRIM(ITOS(IEVTCB)); WRITE(IU,'(A)') TRIM(LINE)
  IF(.NOT.PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,'evt7\evt',NP))RETURN
  CLOSE(IU)
 ENDIF

 !## use drn
 IF(LDRN)THEN
  CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.DRN7'//'...')
  ITOPIC=22
  !## construct drn-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.DRN7_',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# DRN7 File Generated by iMOD '//TRIM(RVERSION)
  LINE='NaN,'//TRIM(ITOS(IDRNCB))//',AUXILIARY SYSTEM NOPRINT'; WRITE(IU,'(A)') TRIM(LINE)
  IF(.NOT.PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,'drn7\drn',NP))RETURN
  CLOSE(IU)
  CALL PMANAGER_SAVEMF2005_MAXNO(TRIM(DIR)//'.DRN7_',NP)
 ENDIF

 !## use riv
 IF(LRIV)THEN
  CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.RIV7'//'...')
  ITOPIC=23
  !## construct riv-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.RIV7_',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# RIV7 File Generated by iMOD '//TRIM(RVERSION)
  LINE='NaN,'//TRIM(ITOS(IRIVCB))//',AUXILIARY INFFCT AUXILIARY SYSTEM NOPRINT'; WRITE(IU,'(A)') TRIM(LINE)
  IF(.NOT.PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,'riv7\riv',NP))RETURN
  CLOSE(IU)
  CALL PMANAGER_SAVEMF2005_MAXNO(TRIM(DIR)//'.RIV7_',NP)
 ENDIF

 !## use ghb
 IF(LGHB)THEN
  CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.GHB7'//'...')
  ITOPIC=25
  !## construct riv-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.GHB7_',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# GHB7 File Generated by iMOD '//TRIM(RVERSION)
  LINE='NaN,'//TRIM(ITOS(IGHBCB))//',AUXILIARY SYSTEM NOPRINT'; WRITE(IU,'(A)') TRIM(LINE)
  IF(.NOT.PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,'ghb7\ghb',NP))RETURN
  CLOSE(IU)
  CALL PMANAGER_SAVEMF2005_MAXNO(TRIM(DIR)//'.GHB7_',NP)
 ENDIF
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,EXT,NP)
 !###======================================================================
 IMPLICIT NONE
 ...
 DO IPER=1,NPER
  ...
  IF(KPER.LE.0)THEN
   ...
   SELECT CASE (ITOPIC)
    CASE (24) !## evt 
     WRITE(IU,'(A)') '-1,-1,-1'
    CASE (22,23,25,26) !## drn,riv,ghb,rch 
     WRITE(IU,'(A)') '-1'
   END SELECT
  ...
  ELSE
   ...
   SELECT CASE (ITOPIC)
    CASE (24,26) !## evt,rch
     ALLOCATE(PCK(NTOP,0:NSYS)); IS1=0
    CASE DEFAULT
     ALLOCATE(PCK(NTOP,NSYS));   IS1=1
   END SELECT
   DO K=1,NTOP; DO ISYS=IS1,NSYS
    CALL IDFNULLIFY(PCK(K,ISYS)); CALL IDFCOPY(BND(1),PCK(K,ISYS)); PCK(K,ISYS)%X=0.0
   ENDDO; ENDDO
   ...
   DO K=1,NTOP 
    ...
    DO ISYS=1,NSYS 
     ...
     IF(TOPICS(ITOPIC)%STRESS(KPER)%FILES(K,ISYS)%ICNST.EQ.1)THEN
     ...
     IF(K.EQ.1)THEN
      SELECT CASE (ITOPIC)
       CASE (24,26) !## rch/evt mm/day -> m/day
        FCT=FCT*0.001
      END SELECT
     ENDIF
     
     CALL PMANAGER_SAVEMF2005_CORRECT(SCL_U,0,ILAY,BND,PCK(K,ISYS),HNOFLOW,FCT,IMP,-1)
   ...
   SELECT CASE (ITOPIC)
    CASE (22) !## drn
     ...
     IF(.NOT.PMANAGER_SAVEMF2005_PCK_ULSTRD(EXFNAME,PCK,NSYS,NTOP,IU,HNOFLOW,(/2,1/),NP))RETURN
    CASE (23) !## riv
     EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_t'//TRIM(ITOS(IPER))//FEXT(IIDEBUG)
     IF(.NOT.PMANAGER_SAVEMF2005_PCK_ULSTRD(EXFNAME,PCK,NSYS,NTOP,IU,HNOFLOW,(/2,1,3,4/),NP))RETURN
    CASE (24) !## total evt
 ...
 IF(IPER.GT.1)THEN
  ALLOCATE(X(PCK(1,0)%NROW,PCK(1,0)%NROW,NTOP))
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_PCK_ULSTRD(EXFNAME,PCK,NSYS,NTOP,IU,HNOFLOW,JTOP,NP)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IU,NSYS,NTOP
 ...
 INTEGER :: JU,IROW,ICOL,I,ITOP,ISYS
 ...
 LINE=TRIM(ITOS(I)); WRITE(IU,*) TRIM(LINE)
 ...
 ELSE
  ...
  DO ISYS=1,NSYS
   DO IROW=1,PCK(1,1)%NROW; DO ICOL=1,PCK(1,1)%NCOL
    DO ITOP=1,NTOP; IF(PCK(JTOP(ITOP),ISYS)%X(ICOL,IROW).EQ.HNOFLOW)EXIT; ENDDO
    IF(ITOP.LE.NTOP)CYCLE
    LINE=TRIM(ITOS(PCK(1,ISYS)%ILAY))//','//TRIM(ITOS(IROW))//','//TRIM(ITOS(ICOL))
    DO ITOP=1,NTOP; LINE=TRIM(LINE)//','//TRIM(RTOS(PCK(JTOP(ITOP),ISYS)%X(ICOL,IROW),'E',7)); ENDDO
    LINE=TRIM(LINE)//','//TRIM(ITOS(ISYS))
    WRITE(JU,*) TRIM(LINE)
    NP=NP+1
   ENDDO; ENDDO
  ENDDO
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_MOD(IU,IDF,BND,ITOPIC,IPER,ISYS,ILAY,SCL_D,SCL_U,IINT,DIR,EXT,HNOFLOW,IINV)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IU,ITOPIC,IPER,ISYS,ILAY,SCL_D,SCL_U,IINT,IINV
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(:) :: BND
 REAL,INTENT(IN) :: HNOFLOW
 ...
 PMANAGER_SAVEMF2005_MOD=.FALSE.

 FCT=TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%FCT
 IMP=TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%IMP
 
 IF(TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%ICNST.EQ.1)THEN
  CNST=TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%CNST
  CNST=CNST*FCT+IMP
  IF(IINT.EQ.0)WRITE(IU,'(A)') 'CONSTANT '//TRIM(RTOS(CNST,'E',7))
  IF(IINT.EQ.1)WRITE(IU,'(A)') 'CONSTANT '//TRIM(ITOS(INT(CNST)))
  IF(SCL_U.EQ.1)CALL IDFCOPY(IDF,BND(ILAY)); BND(ILAY)%X=CNST
  !## save ibound
  IF(SCL_U.EQ.1)CALL IDFCOPY(IDF,BND(ILAY))
 ELSEIF(TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%ICNST.EQ.2)THEN
  EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_l'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG)
  IDF%FNAME=TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISYS,ILAY)%FNAME
  PMANAGER_SAVEMF2005_MOD=PMANAGER_SAVEMF2005_MOD_U2DREL(EXFNAME,IDF,BND,ILAY,SCL_U,SCL_D,IINT,IU,HNOFLOW,FCT,IMP,IINV)
  IF(.NOT.PMANAGER_SAVEMF2005_MOD)CLOSE(IU)
 ENDIF
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_MOD_U2DREL(EXFNAME,IDF,BND,ILAY,SCL_U,SCL_D,IINT,IU,HNOFLOW,FCT,IMP,IINV)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: EXFNAME
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 ...
 CHARACTER(LEN=256) :: SFNAME
 INTEGER,INTENT(IN) :: IINT,IU,IINV,ILAY,SCL_U,SCL_D
 INTEGER :: JU,IROW,ICOL,I
 REAL :: MINV,MAXV
 ...
 !## read/clip/scale idf file
 IF(.NOT.IDFREADSCALE(IDF%FNAME,IDF,SCL_U,SCL_D,1.0,0))RETURN
 !## save ibound
 IF(SCL_U.EQ.1)CALL IDFCOPY(IDF,BND(ILAY))

 CALL PMANAGER_SAVEMF2005_CORRECT(SCL_U,IINV,ILAY,BND,IDF,HNOFLOW,FCT,IMP,0)
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_CORRECT(SCL_U,IINV,ILAY,BND,IDF,HNOFLOW,FCT,IMP,ITYPE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: SCL_U,IINV,ILAY,ITYPE
 REAL,INTENT(IN) :: HNOFLOW,FCT,IMP
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(:) :: BND
 INTEGER :: IROW,ICOL
 
 !## replace nodata for hnoflow-value - except for ibound scaling
 IF(SCL_U.NE.1)THEN
 ...
 I=0
 IF(ASSOCIATED(TOPICS(6)%STRESS).AND.ASSOCIATED(TOPICS(9)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(6)%STRESS(1)%FILES).AND. & !## kdw
     ASSOCIATED(TOPICS(9)%STRESS(1)%FILES))I=1    !## vcw
 ENDIF
 ...
 IF(ASSOCIATED(TOPICS(2)%STRESS).AND.ASSOCIATED(TOPICS(3 )%STRESS).AND. &
    ASSOCIATED(TOPICS(7)%STRESS).AND.ASSOCIATED(TOPICS(10)%STRESS).AND. &
    ASSOCIATED(TOPICS(8)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(2 )%STRESS(1)%FILES).AND. & !## top
     ASSOCIATED(TOPICS(3 )%STRESS(1)%FILES).AND. & !## bot
     ASSOCIATED(TOPICS(7 )%STRESS(1)%FILES).AND. & !## khv
     ASSOCIATED(TOPICS(8 )%STRESS(1)%FILES).AND. & !## kva
     ASSOCIATED(TOPICS(10)%STRESS(1)%FILES))J=1    !## kvv
 ENDIF
 ...
 IF(ISS.EQ.2)THEN
  ...
  ENDIF
  DO I=1,NPER-ISTEADY
   SIM(I+1)%CDATE=TRIM(ITOS(UTL_JDATETOIDATE(IDT(I))))
