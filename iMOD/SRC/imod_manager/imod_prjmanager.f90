...
USE MOD_IDF, ONLY : IDFREAD,IDFDEALLOCATE,IDFNULLIFY,IDFREADSCALE,IDFCOPY,IDFDEALLOCATEX,IDFIROWICOL,IDFALLOCATEX,IDFGETAREA,IDFFILLSXSY
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 IF(LHFB) WRITE(IU,'(A)') 'FHB  23 '//CHAR(39)//TRIM(LINE)//'.FHB7'//CHAR(39)
...
!  ENDDO
 ...
 IF(LHFB)THEN
  ...
  IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.FHB7'//'...')
  IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'.FHB7'//'...'

  !## construct hfb-file
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIR)//'.FHB7',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# FHB7 File Generated by '//TRIM(UTL_IMODVERSION())
  ALLOCATE(IUGEN(NLAY)); IUGEN=0
  DO ILAY=1,NLAY
   IUGEN(ILAY)=UTL_GETUNIT(); CALL OSD_OPEN(IUGEN(ILAY),FILE=TRIM(DIR)//'_FHB_L'//TRIM(ITOS(ILAY))//'.GEN',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
...
!     IF(.NOT.LEX)CYCLE
      ...
      ELSE
       DO ILAY=1,NLAY
        ...
        IF(ITOPIC.EQ.28)THEN
         IF(BND(ILAY)%X(ICOL,IROW).LT.0)THEN
          WRITE(JU,FRM) ILAY,IROW,ICOL,PCK(JTOP(1),ISYS)%X(ICOL,IROW),PCK(JTOP(1),ISYS)%X(ICOL,IROW),ISYS  !PCK(1,ISYS)%ILAY
          MP(1)=MP(1)+1
         ENDIF
        !## olf
        ELSEIF(ITOPIC.EQ.27)THEN
         OLFCOND=(IDFGETAREA(PCK(JTOP(1),ISYS),ICOL,IROW)/COLF)   !## drainage conductance
         WRITE(JU,FRM) ILAY,IROW,ICOL,PCK(JTOP(1),ISYS)%X(ICOL,IROW),OLFCOND,ISYS  !PCK(JTOP(1),ISYS)%ILAY
         MP(1)=MP(1)+1
        ELSE
         WRITE(JU,FRM) ILAY,IROW,ICOL,(PCK(JTOP(ITOP),ISYS)%X(ICOL,IROW),ITOP=1,NTOP),ISYS !PCK(JTOP(1),ISYS)%ILAY
         MP(1)=MP(1)+1
        ENDIF
       ENDDO 
      ENDIF
...
!      write(*,*)
!      endif
      IF(IWRITE.EQ.1)WRITE(IU,'(5(I10,1X),G10.4)') ILAY,IROW,ICOL,IROW,ICOL+1,FCT !## x-direction
     ENDIF
    ENDDO
...
!      write(*,*)
!      endif
      IF(IWRITE.EQ.1)WRITE(IU,'(5(I10,1X),G10.4)') ILAY,IROW,ICOL,IROW,ICOL+1,FCT !## y-direction
     ENDIF
    ENDDO
   ENDIF
  ENDIF
  ...
  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
   ...
   ELSE
    ...
    IF(IDF%X(ICOL,IROW).EQ.IDF%NODATA)THEN
     ...
     WRITE(*,'(A3,I4,F15.1,2E15.7)') CMOD(ITOPIC),BND(ILAY)%X(ICOL,IROW),IDF%X(ICOL,IROW),IDF%NODATA
     PAUSE; STOP
    ENDIF  
   ENDIF
