...
!  ENDDO
 ...
 ELSE
  ...
  IF(LUZF)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',IUZFCB1,' '//CHAR(39)//'UZFINF BDGGRC BDGGET UZFRUN UZFET'
...
!           'FHB','LAK','PCG'/                                             !31-40
  ...
  DO ISYS=1,NSYS
   ...
   SELECT CASE (ITOPIC)
    ...
    CASE (18)
      IF(IPER.EQ.1)THEN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_UZBND_T'//TRIM(ITOS(IPER))//'.ARR',PCK(1),IU,IFBND,1))RETURN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_EPS_T'//TRIM(ITOS(IPER))//  '.ARR',PCK(2),IU,IFBND,0))RETURN

!## thr residual is computed as difference of thts and sy - dus kan niet nul zijn
...
!save_no_buf 
 ...
 DO IPER=1,NPER
  ...
  ENDIF
  IF(LMNW)THEN
   LINE='SAVE BUDGET '//TRIM(ITOS(IWL2CB)); DO ILAY=1,NLAY; LINE=TRIM(LINE)//' '//TRIM(ITOS(ILAY)); ENDDO; WRITE(IU,'(A)') TRIM(LINE)
  ENDIF
  IF(LSFR)THEN
  ...
  ENDIF
  IF(LUZF)THEN
   LINE='SAVE BUDGET '//TRIM(ITOS(IUZFCB1)); DO ILAY=1,1; LINE=TRIM(LINE)//' '//TRIM(ITOS(ILAY)); ENDDO; WRITE(IU,'(A)') TRIM(LINE)
  ENDIF
 ENDDO
