...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 INTEGER :: IHEDUN,IBCFCB,IRCHCB,IEVTCB,IDRNCB,IRIVCB,IGHBCB,ICHDCB,IWELCB,ISFRCB,IFHBCB,ICB,IROW, &
   ICOL,ISTEADY,NHED,NFLOW,IFHBUN,IFHBSS,NFHBX1,NFHBX2,NBDTIM
 INTEGER,DIMENSION(2) :: NP
 REAL :: HNOFLOW,STOPER,CONST,DLEAK
! TYPE(IDFOBJ) :: IDF
...
!  ENDDO
 ...
 DO ILAY=1,NLAY
  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
   IF(BND(ILAY)%X(ICOL,IROW).EQ.-1)NHED =NHED+1
   IF(BND(ILAY)%X(ICOL,IROW).EQ.-2)NFLOW=NFLOW+1
  ENDDO; ENDDO
 ENDDO
 ...
 DO ITOPIC=21,SIZE(TOPICS)
  ...
  IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# '//TRIM(CPCK)//'7 File Generated by '//TRIM(UTL_IMODVERSION())
  !## sfr-package
...
!
  ...
  ELSEIF(ITOPIC.EQ.31)THEN
   ...
   ALLOCATE(FHBNBDTIM(NPER))

   NBDTIM=1
   IFHBSS=0; NFHBX1=0; NFHBX2=0
   LINE=TRIM(ITOS(NBDTIM))//','//TRIM(ITOS(NFLOW)) //','//TRIM(ITOS(NHED))//','//TRIM(ITOS(IFHBSS))//','// &
        TRIM(ITOS(IFHBCB))//','//TRIM(ITOS(NFHBX1))//','//TRIM(ITOS(NFHBX2))
   WRITE(IU,'(A)') TRIM(LINE)
   LINE=TRIM(ITOS(IFHBUN))//',1.0,1'
   !## allocate for fhb paclage
   ALLOCATE(FHBHED(NHED,NPER),FHBFLOW(NFLOW,NPER)) 
  !## all the rest
  ELSE
   LINE=TRIM(CMAXNO1)//','//TRIM(ITOS(ICB))//','//TRIM(CAUX)
  ENDIF
  WRITE(IU,'(A)') TRIM(LINE)
  IF(.NOT.PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,TRIM(CPCK)//'7\'//TRIM(CPCK),NP,LTB,TOP,BOT,KD,IBATCH))RETURN
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,EXT,NP,LTB,TOP,BOT,KD,IBATCH)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IU,ITOPIC,IBATCH
  ...
  ELSE
   ...
   IF(INEW.EQ.1)THEN
    DO K=1,NTOP; DO ISYS=IS1,NSYS
     ...
     SELECT CASE (ITOPIC)
      CASE (22:28); CALL IDFCOPY(BND(1),PCK(K,ISYS)) 
     END SELECT
    ENDDO; ENDDO
   ENDIF
