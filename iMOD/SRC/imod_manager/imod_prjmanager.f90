...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,II,J,K,N,ITYPE,ID,IPER,ITOPIC,IOS,IST,IYR,IMH,IDY,ISUBTOPIC,IDATE,ISYS,IOPTION,IHR,IMT,ISC
 ...
 IF(IPER.GT.0)THEN
  IF(TOPICS(ITOPIC)%TIMDEP)THEN
   ...
   IF(IOS.EQ.0)THEN
    IHR=TOPICS(ITOPIC)%STRESS(IPER)%IHR
    IMT=TOPICS(ITOPIC)%STRESS(IPER)%IMT
    ISC=TOPICS(ITOPIC)%STRESS(IPER)%ISC
     ...
     CASE (IDOK)
      ...
      IF(TOPICS(ITOPIC)%TIMDEP)THEN
       ...
       IF(I.EQ.1)THEN     !## steady-state
        CD='STEADY-STATE'; IHR=0; IMT=0; ISC=0
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: FNAME
 INTEGER :: IU,I,J,K,IOS,NC,L,NSYS
 ...
 DO
  ...
  DO L=1,NPER 
   IF(TOPICS(I)%TIMDEP)THEN
    ...
    READ(LINE,'(3(I2,1X))') TOPICS(I)%STRESS(L)%IHR,TOPICS(I)%STRESS(L)%IMT,TOPICS(I)%STRESS(L)%ISC
   ENDIF
   READ(IU,*) NC,NSYS
...
!## cannot be here
 ...
 NPER=JD2-JD1+1; ALLOCATE(SIM(NPER))
...
!  ELSE
 ...
 INTEGER :: ITYPE
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: IDY,IYR,IMH,ITOPIC,IPER,I,J,MINJD,MAXJD,IOS,IDATE,IHR,IMT,ISC,IHMS,MINHMS,MAXHMS
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: IDF
 LOGICAL :: LEX
  ...
  DO IPER=1,SIZE(TOPICS(ITOPIC)%STRESS)
   ...
   ELSE
    ...
    IF(IOS.EQ.0)THEN
     ...
     !## get sum of hours/minute/seconds
     IHMS=HMSTOITIME(TOPICS(ITOPIC)%STRESS(IPER)%IHR,TOPICS(ITOPIC)%STRESS(IPER)%IMT,TOPICS(ITOPIC)%STRESS(IPER)%ISC)
     IF(IDATE.LE.MINJD)THEN
      MINJD=MIN(MINJD,IDATE)
      IF(IHMS.LT.MINHMS)MINHMS=IHMS
     ELSEIF(IDATE.GT.MAXJD)THEN
      MAXJD=MAX(MAXJD,IDATE)
      IF(IHMS.GT.MAXHMS)MAXHMS=IHMS
     ENDIF
    ELSE
 ...
 IF(MINJD.GT.MAXJD.AND.MINHMS.GT.MAXHMS)THEN
  ...
  CALL WDIALOGPUTINTEGER(IDF_INTEGER8,0)
  CALL WDIALOGPUTMENU(IDF_MENU3,CDATE,12,IMH)
  CALL WDIALOGPUTINTEGER(IDF_INTEGER4,IDY)
  CALL WDIALOGPUTINTEGER(IDF_INTEGER5,IYR)
  CALL WDIALOGPUTINTEGER(IDF_INTEGER9,0)
  CALL WDIALOGPUTINTEGER(IDF_INTEGER10,0)
  CALL WDIALOGPUTINTEGER(IDF_INTEGER11,0)
...
! CALL WGRIDPUTINTEGER(IDF_GRID1,1,SIM%ISUM,NPER)
 ...
 INTEGER :: JD1,JD2,IPERIOD,I,IYR,IMH,IDY,ISS,IHR,IMT,ISC,IHMS1,IHMS2,ISC1,ISC2
 ...
 IF(ISS.EQ.2)THEN
  ...
  SELECT CASE (IPERIOD)
   CASE (1) !## hourly
    NPER=JD2-JD1; NPER=NPER+1; NPER=NPER*24     
   CASE (2) !## daily
    NPER=JD2-JD1; NPER=NPER+1
   CASE (3) !## weekly
    NPER=JD2-JD1; NPER=NPER+1; NPER=NPER/7
   CASE (4) !## monthly
    NPER=JD2-JD1; NPER=NPER+1; NPER=NPER/28
   CASE (5) !## yearly
    NPER=JD2-JD1; NPER=NPER+1; NPER=NPER/365
   CASE (6) !## packages
  ...
  SELECT CASE (IPERIOD)
   CASE (1) !## hourly
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0,1,4))EXIT; ENDDO     
   CASE (2) !## daily
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0,1,3))EXIT; ENDDO
   CASE (3) !## weekly
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0,7,3))EXIT; ENDDO
   CASE (4) !## monthly
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0,1,2))EXIT; ENDDO
   CASE (5) !## yearly
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0,1,1))EXIT; ENDDO
   CASE (6) !## packages
 ...
 INTEGER :: IDAY,IHMS1,IHMS2,JD1,JD2
  ...
  IF(N.GT.0)THEN
   ...
   DO IPER=1,SIZE(TOPICS(I)%STRESS)
    ...
    IF(TOPICS(I)%TIMDEP.AND.NF.GT.0)THEN
     ...
     IF(TOPICS(I)%STRESS(IPER)%IHR+TOPICS(I)%STRESS(IPER)%IMT+TOPICS(I)%STRESS(IPER)%ISC.GT.0)THEN
      WRITE(STRING,'(A,3(A1,I2.2))') TRIM(TOPICS(I)%STRESS(IPER)%CDATE),' ', &
            TOPICS(I)%STRESS(IPER)%IHR,':',TOPICS(I)%STRESS(IPER)%IMT,':',TOPICS(I)%STRESS(IPER)%ISC
     ELSE
