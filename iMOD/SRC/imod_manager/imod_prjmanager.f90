...
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_WSELECTFILE,UTL_CAP,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_FILLDATES,NEWLINE,UTL_LISTOFFILES, &
    ...
    HMSTOITIME,UTL_IDATETOJDATE,UTL_PCK_READTXT,UTL_PCK_GETTLP,ITIMETOGDATE
...
CONTAINS
 ...
 CALL WDIALOGPUTINTEGER(IDF_INTEGER8 ,PEST%PE_NPERIOD)
 CALL WDIALOGFIELDSTATE(ID_PERIODS,MIN(1,PEST%PE_NPERIOD))
 CALL WDIALOGPUTINTEGER(IDF_INTEGER9 ,PEST%PE_NBATCH)
 CALL WDIALOGFIELDSTATE(ID_BATCHFILES,MIN(1,PEST%PE_NBATCH))
 N=0; IF(ASSOCIATED(PEST%PARAM))N=SIZE(PEST%PARAM)
 CALL WDIALOGPUTINTEGER(IDF_INTEGER10,N)
 CALL WDIALOGFIELDSTATE(ID_PARAMETERS,MIN(1,N))
 N=0; IF(ASSOCIATED(PEST%IDFFILES))N=SIZE(PEST%IDFFILES)
 CALL WDIALOGPUTINTEGER(IDF_INTEGER11,N)
 CALL WDIALOGFIELDSTATE(ID_ZONES,MIN(1,N))
 N=0; IF(ASSOCIATED(PEST%MEASURES))N=SIZE(PEST%MEASURES)
 CALL WDIALOGPUTINTEGER(IDF_INTEGER12,N)
 CALL WDIALOGFIELDSTATE(ID_MEASURES,MIN(1,N))
   ...
   CASE(FIELDCHANGED)
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_INTEGER8)
      ...
      CALL WDIALOGFIELDSTATE(ID_PERIODS,MIN(1,N))
     CASE (IDF_INTEGER9)
      CALL WDIALOGGETINTEGER(IDF_INTEGER9,N)
      CALL WDIALOGFIELDSTATE(ID_BATCHFILES,MIN(1,N))
     CASE (IDF_INTEGER10)
 ...
 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  ...
  CALL WDIALOGGETINTEGER(IDF_INTEGER8 ,PEST%PE_NPERIOD)
  CALL WDIALOGGETINTEGER(IDF_INTEGER9 ,PEST%PE_NBATCH)
 ENDIF
 ...
 SELECT CASE (ID)
  ...
  CASE (ID_MEASURES)
   ...
   IF(ASSOCIATED(PEST%MEASURES))THEN
    ...
    IF(N.NE.M)THEN
     ALLOCATE(PEST%MEASURES_BU(N)); PEST%MEASURES%IPFNAME=''; PEST%MEASURES%IXCOL=1; PEST%MEASURES%IYCOL=2; PEST%MEASURES%ILCOL=3;
 ...
 SELECT CASE (ID)
  ...
  CASE (ID_ZONES)
   ...
   CALL WDIALOGSELECT(ID_DPMANAGER_PESTFILES_TAB4)
   CALL WGRIDGETSTRING(IDF_GRID1,1,PEST%IDFFILES,N)
...
!      CALL UTL_FILLDATES(IDF_INTEGER5,IDF_MENU2,IDF_INTEGER2)
 ...
 IF(ICODE.EQ.0)THEN
  ...
  PERIOD(IOPTION)%NAME=''
  PERIOD(IOPTION)%IMH=4; PERIOD(IOPTION)%IDY=1;  PERIOD(IOPTION)%IYR(1)=2014
  PERIOD(IOPTION)%IHR=0; PERIOD(IOPTION)%IMT=0;  PERIOD(IOPTION)%ISC=0
  PERIOD(IOPTION)%IYR(2)=2014
 !## use existing one
 ELSE
  CALL WDIALOGGETMENU(IDF_MENU3,IOPTION)
 ENDIF
...
!  ENDDO
 ...
 DO I=1,NPERIOD
  ...
  WRITE(IU,'(2(I2.2,A1),I4.4,3(A1,I2.2),A1,I4.4)') PERIOD(I)%IDY,'-',PERIOD(I)%IMH,'-',PERIOD(I)%IYR(1),' ', &
                                                   PERIOD(I)%IHR,':',PERIOD(I)%IMT,':',PERIOD(I)%ISC,' ',PERIOD(I)%IYR(2)
 ENDDO
 ...
 I=0; DO
  ...
  READ(IU,'(2(I2.2,1X),I4.4,3(1X,I2.2),1X,I4.4)',IOSTAT=IOS) PERIOD(I)%IDY,PERIOD(I)%IMH,PERIOD(I)%IYR(1), &
 ...
 LINE=TRIM(ITOS(PEST%PE_MXITER))         //','//TRIM(RTOS(PEST%PE_STOP,'F',2))     //','// &
      TRIM(RTOS(PEST%PE_SENS,'F',2))     //','//TRIM(ITOS(PEST%PE_NPERIOD))        //','// &
      TRIM(ITOS(PEST%PE_NBATCH))         //','//TRIM(RTOS(PEST%PE_TARGET(1),'F',2))//','// &
 ...
 WRITE(IU,'(A)') TRIM(LINE)
 IF(PEST%PE_NPERIOD.GT.0)THEN
  DO I=1,PEST%PE_NPERIOD
   LINE=TRIM(ITOS(PEST%S_PERIOD(I)))//','//TRIM(ITOS(PEST%E_PERIOD(I)))
   WRITE(IU,'(A)') TRIM(LINE)   
  ENDDO
 ENDIF
 IF(PEST%PE_NBATCH.GT.0)THEN
  DO I=1,PEST%PE_NBATCH
   LINE=TRIM(RTOS(PEST%B_FRACTION(I),'F',2))//','//TRIM(PEST%B_BATCHFILE(I))//','//TRIM(PEST%B_OUTFILE(I))
   WRITE(IU,'(A)') TRIM(LINE)   
  ENDDO
 ENDIF
 ...
 READ(IU,'(A)') LINE
 READ(LINE,*) PEST%PE_MXITER,PEST%PE_STOP,PEST%PE_SENS,PEST%PE_NPERIOD,PEST%PE_NBATCH,PEST%PE_TARGET(1),PEST%PE_TARGET(2),PEST%PE_SCALING, &
              PEST%PE_PADJ,PEST%PE_DRES,PEST%PE_KTYPE
 PEST%PE_SCALING=PEST%PE_SCALING+1
 IF(PEST%PE_NPERIOD.GT.0)THEN
  ALLOCATE(PEST%S_PERIOD(PEST%PE_NPERIOD),PEST%E_PERIOD(PEST%PE_NPERIOD))
 ...
 ENDIF
 IF(PEST%PE_NBATCH.GT.0)THEN
  ALLOCATE(PEST%B_FRACTION(PEST%PE_NBATCH),PEST%B_BATCHFILE(PEST%PE_NBATCH),PEST%B_OUTFILE(PEST%PE_NBATCH))
 ...
 ENDIF
 WRITE(IU,'(10(I10,A1))') NLAY,',',MXNLAY,',',NPER,',',0,',',1,',',0,',',N,',',0,',',IUNCONF,',0'
 ...
 DO KPER=1,NPER
  ...
  DO I=1,MAXTOPICS  
   ...
   IF(IPER.LE.0)THEN
    ...
    WRITE(IU,'(A)') ' -1,'//TRIM(TOPICS(I)%TNAME)
 ...
 WRITE(IU,'(A)') '# Nam File Generated by iMOD '//TRIM(BVERSION)
 ...
 WRITE(IUBAS,'(A)') '# BAS6 File Generated by iMOD '//TRIM(BVERSION)
 ...
 WRITE(IU,'(A)') '# DIS6 File Generated by iMOD '//TRIM(BVERSION)
...
!  ENDDO
 ...
 IF(LLPF)THEN
  ...
  WRITE(IU,'(A)') '# LPF7 File Generated by iMOD '//TRIM(BVERSION)
 ...
 IF(LHFB)THEN
  ...
  WRITE(IU,'(A)') '# HFB7 File Generated by iMOD '//TRIM(BVERSION)
 ...
 IF(LPCG)THEN
  ...
  WRITE(IU,'(A)') '# PCG7 File Generated by iMOD '//TRIM(BVERSION)
 ...
 IF(LSIP)THEN
  ...
  WRITE(IU,'(A)') '# SIP File Generated by iMOD '//TRIM(BVERSION)
 ...
 WRITE(IU,'(A)') '# OC File Generated by iMOD '//TRIM(BVERSION)
 ...
 DO ITOPIC=21,29
  ...
  IF(IU.EQ.0)RETURN
  WRITE(IU,'(A)') '# '//TRIM(CPCK)//'7 File Generated by iMOD '//TRIM(BVERSION)
   ...
   ELSEIF(LIPF)THEN
    ...
    DO I=1,NROWIPF
     ...
     IF(ILAY.EQ.0)THEN
      ...
      CALL UTL_PCK_GETTLP(NLAY,TLP,KH,TP,BT,Z1,Z2,MINKH) 
...
!     IF(.NOT.LEX)CYCLE
 ...
 !## two cycles, first to determin number of boundaries, second to write them
 DO IWRITE=0,1
  ...
  DO ILAY=1,NLAY
   ...
   DO ISYS=1,SIZE(TOPICS(ITOPIC)%STRESS(1)%FILES,1)
    ...
    IF(LEN_TRIM(IDF%FNAME).GT.0)CALL ASC2IDF_HFB(IDF,IDF%NROW,IDF%NCOL,IPC,IDF%FNAME)
   ENDDO
 
   CALL PMANAGER_SAVEMF2005_HFBEXPORT(IPC,IDF%NROW,IDF%NCOL,FCT,NHFBNP(ILAY),ILAY,IWRITE,IU,IDF,TOP,BOT,BND,KD)
...
! IF(PMANAGER_SAVEMF2005_HFB)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 !###====================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_HFBEXPORT(IPC,NROW,NCOL,HFBFCT,NHFB,ILAY,IWRITE,IU,IDF,TOP,BOT,BND,KD) 
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NROW,NCOL,IWRITE,IU,ILAY 
 INTEGER,INTENT(OUT) :: NHFB
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 TYPE(IDFOBJ),DIMENSION(NLAY),INTENT(INOUT) :: TOP,BOT,BND,KD
  ...
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.NCOL)THEN
    ...
    ELSE
     FCT=HFBFCT
    ENDIF
    WRITE(IU,'(5I10,G10.4)') ILAY,IROW,ICOL,IROW,ICOL+1,FCT !## x-direction
   ENDIF
  ENDIF
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   IF(IROW.LT.NROW)THEN
    ...
    ELSE
     FCT=HFBFCT
    ENDIF
    WRITE(IU,'(5I10,G10.4)') ILAY,IROW,ICOL,IROW,ICOL+1,FCT !## Y-direction
   ENDIF
  ENDIF
...
!!
  ...
  DO J=1,SIZE(TOPICS(I)%STRESS)
   ...
   ELSE
    ...
    IF(K.LE.NPERIOD)THEN
     DO IYR=PERIOD(K)%IYR(1),PERIOD(K)%IYR(2)

      IMH=PERIOD(K)%IMH; IDY=PERIOD(K)%IDY; IHR=PERIOD(K)%IHR; IMT=PERIOD(K)%IMT; ISC=PERIOD(K)%ISC
      !## true date specified
      IF(IYR+IMH+IDY+IHR+IMT+ISC.GT.0)THEN
       IPER=IPER+1; CALL PMANAGER_GETNPER_ITIME(ITIME,IPER)
       ITIME(IPER)=IYR*10000000000+IMH*100000000+IDY*1000000+IHR*10000+IMT*100+ISC
 ...
 ELSE
  ...
  DO I=1,SIZE(STRESS)
   ...
   IF(J.LE.NPERIOD)THEN
    ...
    DO IYR=PERIOD(J)%IYR(1),PERIOD(J)%IYR(2)
 ...
 !## default packages
 CALL WDIALOGPUTMENU(IDF_MENU4,TMENU,SIZE(TMENU),8)
  ...
  DO IPER=1,SIZE(TOPICS(ITOPIC)%STRESS)
   ...
   ELSE
    ...
    ELSE
     ...
     IF(J.LE.NPERIOD)THEN
      DO K=1,2
       IYR=PERIOD(J)%IYR(K); IMH=PERIOD(J)%IMH; IDY=PERIOD(J)%IDY
...
!  CALL WDIALOGPUTSTRING(IDF_STRING1,MODELNAME)
 ...
 CALL WDIALOGPUTMENU(IDF_MENU1,TMENU,SIZE(TMENU),2)
    ...
    SELECT CASE (MESSAGE%VALUE2)
     CASE (IDF_RADIO1,IDF_RADIO2)
      ...
      IF(I.EQ.2)THEN
       ...
       CALL WDIALOGFIELDSTATE(IDF_RADIO3,1); CALL WDIALOGFIELDSTATE(IDF_RADIO4,1)
      ENDIF
     CASE (IDF_MENU1)
      CALL WDIALOGGETMENU(IDF_MENU1,I)
      CALL PMANAGER_TIMESTEPS_GETISTEP(I,J,ISTEP)    
 ...
 ALLOCATE(SIM(100)); CALL PMANAGER_ASSIGNTIMESTEPS(1,2,JD1,JD2,IHMS1,IHMS2,IPERIOD,ISTEP)
 
 !## adjust time-steps
 IF(ITG.EQ.1)THEN
  J=NPER+SIZE(SIM_C2)-(IR2-IR1); ALLOCATE(ITIME(J))
    ...
    DO J=1,NF   !## number of periods (types)
     DO K=1,MF  !## number of files (systems)
      ...
      IF(I.EQ.20)THEN
       ...
       ENDIF
       STRING=TRIM(STRING)//';nperiods='//TRIM(ITOS(PEST%PE_NPERIOD))
       STRING=TRIM(STRING)//';nbatchfiles='//TRIM(ITOS(PEST%PE_NBATCH))
       IF(ASSOCIATED(PEST%IDFFILES))THEN
