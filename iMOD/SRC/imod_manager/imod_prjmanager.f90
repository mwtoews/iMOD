...
CONTAINS
 ...
 !## pest goes to another dialog
 IF(ITOPIC.EQ.20)THEN
  CALL PMANAGEROPEN_PEST()
...
! CHARACTER(LEN=512) :: LINE
 ...
 DO
  ...
  !## pst module is exception
  IF(I.EQ.20)THEN
   CALL PMANAGER_LOADPST(IU,NPER,1)
...
! CHARACTER(LEN=512) :: LINE
 ...
 INTEGER :: IU,IULAK,KPER,SCL_D,SCL_U,ITOPIC,ILAY,ISS,IINV,NRCHOP,NEVTOP
 INTEGER :: ICB,IROW,ICOL,ISTEADY,NHED,NFLW,IFHBSS,NFHBX1,NFHBX2,NBDTIM,I,J,JD0,JD1,ISEC0,ISEC1
 INTEGER,DIMENSION(2) :: NP
 REAL :: STOPERROR,CONST,DLEAK,DDAY,DSEC
 ...
 STOPERROR=0.1      !## stop error of total waterbalance
 ...
 !## save bas file
 IF(.NOT.PMANAGER_SAVEMF2005_BAS_SAVE(DIR,DIRMNAME,IBATCH,STOPERROR))RETURN
 !## save dis file
 ...
 !## save lak file
 IF(.NOT.PMANAGER_SAVEMF2005_LAK_SAVE(IBATCH,DIR,DIRMNAME))RETURN
 !## save hfb file
 ...
 DO ITOPIC=21,SIZE(TOPICS)
  ...
  SELECT CASE (ITOPIC)
   ...
   CASE (27); LEX=LOLF
    ...
    ICB=IDRNCB; CMAXNO1='NaN1'; CAUX='AUXILIARY SYSTEM NOPRINT'; TEXT='_'
   CASE (28); LEX=LCHD; CPCK='CHD'; ICB=ICHDCB; CMAXNO1='NaN1'; CAUX='AUXILIARY SYSTEM NOPRINT'; TEXT='_'
   CASE (29); LEX=LISG
   ...
   CASE (32); LEX=.FALSE. !LLAK
  END SELECT
 
  !## not available
  IF(.NOT.LEX)CYCLE

  IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIRMNAME)//'.'//TRIM(CPCK)//'7...')
  IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIRMNAME)//'.'//TRIM(CPCK)//'7...'

  IU=UTL_GETUNIT()
  CALL OSD_OPEN(IU,FILE=TRIM(DIRMNAME)//'.'//TRIM(CPCK)//'7'//TRIM(TEXT),STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
  IF(IU.EQ.0)RETURN
  !## fhb cannot handle comment-lines
  IF(ITOPIC.NE.31)WRITE(IU,'(A)') '# '//TRIM(CPCK)//'7 File Generated by '//TRIM(UTL_IMODVERSION())
  !## sfr-package
...
!  ENDDO
 ...
 ENDIF
 IF(LLAK)THEN; ALLOCATE(LAK(10)); DO ILAY=1,SIZE(LAK); CALL IDFNULLIFY(LAK(ILAY)); ENDDO; ENDIF
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_GETPACKAGES()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ITOPIC
 ...
 END SUBROUTINE PMANAGER_GETPACKAGES
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_BAS_SAVE(DIR,DIRMNAME,IBATCH,STOPERROR)
 !####====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 REAL,INTENT(IN) :: STOPERROR
 ...
 WRITE(IU,'(A,F15.7)') 'FREE STOPERROR ',STOPERROR
 DO ILAY=1,NLAY
 ...
 !####====================================================================
 IMPLICIT NONE
 INTEGER :: I,ITOPIC,SCL_D,SCL_U,ILAY,IROW,ICOL
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_LAK_SAVE(IBATCH,DIR,DIRMNAME)
 !####====================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 INTEGER,INTENT(IN) :: IBATCH
 ...
 IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(DIRMNAME)//'.LAK7',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN
 !## lak settings - use most frequent 
 IF(.NOT.PMANAGER_SAVELAKE(0,IU,ILKCB,IBATCH,DIR))RETURN
  ...
  ELSE
   ...
   SELECT CASE (ITOPIC)
    CASE (24,26) !## evt,rch
     ...
     IS1=0
    CASE (21,22,23,25,27,28,29,30,31) !## wel,drn,riv,ghb,chd,olf,isg,sfr,fhb
   ...
   IF(INEW.EQ.1)THEN
    DO K=1,NTOP; DO ISYS=IS1,NSYS
     ...
     SELECT CASE (ITOPIC)
      CASE (22:28,31); CALL IDFCOPY(BND(1),PCK(K,ISYS)) 
     END SELECT
    ENDDO; ENDDO
   ENDIF
   ...
   SELECT CASE (ITOPIC)
    ...
    CASE (31) !## fhb
     ...
     DO K=1,NTOP; DO ISYS=1,NSYS
      ILAY=TOPICS(ITOPIC)%STRESS(KPER)%FILES(K,ISYS)%ILAY
      DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
       IF(BND(ILAY)%X(ICOL,IROW).EQ.-2)THEN; NHED=NHED+1; FHBHED(NHED,NBDTIM)=PCK(K,ISYS)%X(ICOL,IROW); ENDIF
       IF(BND(ILAY)%X(ICOL,IROW).EQ. 2)THEN; NFLW=NFLW+1; FHBFLW(NFLW,NBDTIM)=PCK(K,ISYS)%X(ICOL,IROW); ENDIF
      ENDDO; ENDDO
     ENDDO; ENDDO
   END SELECT
...
!     IF(.NOT.LEX)CYCLE
 ...
 ENDDO
! IF(kh(nlay).GT.HUGE(1.0))then
! !IF(SUM(KH).NE.SUM(KH))THEN
! WRITE(*,*)
! ENDIF
 DO ILAY=1,NLAY
...
! IF(PMANAGER_SAVEMF2005_HFB_COMPUTE)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVELAKE(IPER,IULAK,ILKCB,IBATCH,DIR)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPER,IULAK,ILKCB,IBATCH
 CHARACTER(LEN=*),INTENT(IN) :: DIR
 INTEGER :: IROW,ICOL,ILAY,NSSITR,I,J,IOP
 REAL :: THETA,SSCNCR,LVL,C1,C2,C3,D,A
 
 THETA=0.0; SSCNCR=0.001; NSSITR=100
  
 PMANAGER_SAVELAKE=.FALSE.
 
 !## initialisation - adjust ibound adjust all above 
 IF(IPER.EQ.0)THEN
  ...
  DO I=1,NLAKES
   DO J=2,4
    ...
    END SELECT
    IF(.NOT.PMANAGER_SAVELAKE_GETMEANVALUE(LAK(1)%X,LAK(J)%X,ULAKES(I),LVL,IBATCH,IOP))RETURN
    IF(J.EQ.2)THEN
  ...
  WRITE(IULAK,'(A)') '1,1,0'
  !## reset array
  DO ILAY=1,NLAY; DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL; KD(ILAY)%X(ICOL,IROW)=0.0; ENDDO; ENDDO; ENDDO
  !## get lake identification
  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
   !## find layer underneath bathymetry of lake
   DO ILAY=1,NLAY
    IF(LAK(5)%X(ICOL,IROW).LE.TOP(ILAY)%X(ICOL,IROW))THEN
     KD(ILAY)%X(ICOL,IROW)=LAK(1)%X(ICOL,IROW)     
  ...
  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
   !## find layer underneath bathymetry of lake
   DO ILAY=1,NLAY
    IF(LAK(5)%X(ICOL,IROW).LE.BOT(ILAY)%X(ICOL,IROW))THEN

     !## lake bed resistance
     C1=LAK(6)%X(ICOL,IROW)

     !## aquitard resistance of remainder aquitard
     D= LAK(5)%X(ICOL,IROW)-TOP(ILAY)%X(ICOL,IROW)
     C2=0.0; IF(D.GT.0.0)C2=KVV(ILAY)%X(ICOL,IROW)/D

     !## aquifer resistance
     D=LAK(5)%X(ICOL,IROW)-BOT(ILAY)%X(ICOL,IROW)
     C3=0.0
     IF(D.GT.0.0)THEN
      C3=D/KHV(ILAY)%X(ICOL,IROW)
      !## include vertical anisotropy (inverse in modflow2005)
      C3=C3*KVA(ILAY)%X(ICOL,IROW)
     ENDIF
     
     !## compute conductance
     A =IDFGETAREA(IDF,ICOL,IROW)
     KD(ILAY)%X(ICOL,IROW)=A/(C1+C2+C3)
 ...
 PMANAGER_SAVELAKE=.TRUE.
  
 END FUNCTION PMANAGER_SAVELAKE

 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVELAKE_GETMEANVALUE(X,Y,ULAKE,LVL,IBATCH,IOP)
 !###======================================================================
 IMPLICIT NONE
 ...
 PMANAGER_SAVELAKE_GETMEANVALUE=.FALSE.
 ...
 PMANAGER_SAVELAKE_GETMEANVALUE=.TRUE.

 END FUNCTION PMANAGER_SAVELAKE_GETMEANVALUE
...
!    LAYCON(I)=0
 ...
 IF(I.EQ.0.AND.J.EQ.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Missing data, e.g. TOP, BOT, KHV packages to (a) convert to BCF6 or (b) convert to LPF package','Warning')
 ...
 IF(ASSOCIATED(TOPICS(32)%STRESS))THEN
  ...
  IF(J.EQ.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'For usage of the LAK paclage you need to specify TOP and BOT packages as well','Warning')
   CALL WDIALOGUNLOAD(); RETURN
  ENDIF
 ENDIF
 ...
 CALL WDIALOGFIELDSTATE(IDF_RADIO7,2)
 CALL WDIALOGFIELDSTATE(IDF_RADIO8,2)
...
! IF(I.EQ.1)THEN; CALL WDIALOGPUTSTRING(IDF_LABEL12,'Transmissivity as a function of head')
  ...
  IF(ITOPIC.GT.0)THEN
   IF(TOPICS(ITOPIC)%IACT.EQ.1)THEN
    ...
    IF(ITOPIC.EQ.20)THEN
     ...
     CALL PMANAGER_LOADPST(IU,NSYS,0); TOPICS(ITOPIC)%IACT_MODEL=1; CYCLE
    ENDIF
