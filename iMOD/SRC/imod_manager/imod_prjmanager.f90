...
LOGICAL,PRIVATE :: LBCF,LLPF,LPCG,LPCGN,LSIP,LRCH,LEVT,LDRN,LRIV,LGHB,LSOF,LCHD,LOLF,LWEL,LISG
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 !## sof
 LSOF=.FALSE.; ITOPIC=27; IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))LSOF=.TRUE.; ENDIF 
 !## chd
 ...
 IF(LDRN.OR.LSOF) WRITE(IU,'(A)') 'DRN  18 '//CHAR(39)//TRIM(LINE)//'.DRN7'//CHAR(39)
 ...
 IF(LDRN.OR.LSOF)WRITE(IU,'(A,I3,A)') 'DATA(BINARY) ',IDRNCB,' '//CHAR(39)//TRIM(LINE)//'_FDRN'//CHAR(39) 
 ...
 DO ILAY=1,NLAY
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   ...
   ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'; LTB=.FALSE.
   ENDIF
  ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'; LTB=.FALSE.
  ENDIF
  ITOPIC=3
 ...
 DO ITOPIC=21,29
  ...
  IF(.NOT.PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,TRIM(CPCK)//'7\'//TRIM(CPCK),NP,TOP,BOT,KD))RETURN
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_PCK(IU,ITOPIC,BND,HNOFLOW,DIR,EXT,NP,TOP,BOT,KD)
 !###======================================================================
 IMPLICIT NONE
  ...
  ELSE
   ...
   SELECT CASE (ITOPIC)
    CASE (21) !## wel
     ...
     IF(.NOT.PMANAGER_SAVEMF2005_PCK_ULSTRD(EXFNAME,PCK,NSYS,NTOP,IU,HNOFLOW,(/2,1/),NP,BND,TOP,BOT,KD))RETURN
    ...
    CASE (23) !## riv
     EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_t'//TRIM(ITOS(IPER))//FEXT(IIDEBUG)
     IF(.NOT.PMANAGER_SAVEMF2005_PCK_ULSTRD(EXFNAME,PCK,NSYS,NTOP,IU,HNOFLOW,(/2,1,3,4/),NP,BND,TOP,BOT,KD))RETURN
    CASE (24) !## evt
    ...
    CASE (27) !## olf
     EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_t'//TRIM(ITOS(IPER))//FEXT(IIDEBUG)
     IF(.NOT.PMANAGER_SAVEMF2005_PCK_ULSTRD(EXFNAME,PCK,NSYS,NTOP,IU,HNOFLOW,(/1/),NP,BND,TOP,BOT,KD))RETURN
    CASE (28) !## chd
     EXFNAME=TRIM(DIR)//'\'//TRIM(EXT)//'_t'//TRIM(ITOS(IPER))//FEXT(IIDEBUG)
     IF(.NOT.PMANAGER_SAVEMF2005_PCK_ULSTRD(EXFNAME,PCK,NSYS,NTOP,IU,HNOFLOW,(/1/),NP,BND,TOP,BOT,KD))RETURN
    CASE (29) !## isg
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_PCK_ULSTRD(EXFNAME,PCK,NSYS,NTOP,IU,HNOFLOW,JTOP,NP,BND,TOP,BOT,KD)
 !###======================================================================
 IMPLICIT NONE
 ...
 CHARACTER(LEN=5) :: EXT
 INTEGER :: JU,KU,ILAY,IROW,ICOL,I,ITOP,ISYS,NROWIPF,NCOLIPF,IEXT,MP,ID
 LOGICAL :: LIPF,LISG
 ...
 LWEL=INDEX(EXFNAME,'\RIV7\').GT.0; LISG=INDEX(EXFNAME,'\ISG7\').GT.0
...
!!## eerst indelen dan winning toekennen ...
!         N=SIZE(XWEL,2)
!         IF(.NOT.PCK1GETILAY_TOPBOT(IWEL(IP),XWEL(IP,2),XWEL(IP,3),XWEL(IP,1:N),NTMP,IBOUND,TOP,BOT,NODES))THEN
!          XWEL(IP,1)=0.0  !## if not; no active cell found!
!          NB        =NB+1
!         ENDIF
