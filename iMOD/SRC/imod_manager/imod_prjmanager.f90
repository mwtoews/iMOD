...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 IF(ISUBMODEL.EQ.1)THEN
  ...
  IF(SUBMODEL(1).GT.IDF%XMIN)IFULL(1)=1
  IF(SUBMODEL(2).GT.IDF%YMIN)IFULL(2)=1
  IF(SUBMODEL(3).LT.IDF%XMAX)IFULL(3)=1
  IF(SUBMODEL(4).LT.IDF%YMAX)IFULL(4)=1
 ...
 IF(SCL_U.EQ.1)THEN
  ...
  DO IROW=1,IDF%NROW
   IF(IFULL(1).EQ.0)THEN
    ICOL=1; IF(BND(ILAY)%X(ICOL,IROW).GT.0)BND(ILAY)%X(ICOL,IROW)=-1
   ENDIF
   IF(IFULL(3).EQ.0)THEN
    ICOL=IDF%NCOL; IF(BND(ILAY)%X(ICOL,IROW).GT.0)BND(ILAY)%X(ICOL,IROW)=-1
   ENDIF
  ENDDO
  DO ICOL=1,IDF%NCOL
   IF(IFULL(4).EQ.0)THEN
    IROW=1; IF(BND(ILAY)%X(ICOL,IROW).GT.0)BND(ILAY)%X(ICOL,IROW)=-1
   ENDIF
   IF(IFULL(2).EQ.0)THEN
    IROW=IDF%NROW; IF(BND(ILAY)%X(ICOL,IROW).GT.0)BND(ILAY)%X(ICOL,IROW)=-1
   ENDIF
  ENDDO
...
! ENDIF
 ...
 IF(I.EQ.1)CALL WDIALOGGETCHECKBOX(IDF_CHECK3,I)
