...
!## IFVDL SFT RCNC
 ...
 INTEGER :: IU,JU,ILAY,IROW,ICOL,I,J,KTOP,KPER,IPER,NP,NTOP,SCL_D,SCL_U,ICNST,NSYS,ISYS,JSYS,MP,N,IIPER,KKPER, &
    ...
    DO ISYS=1,NSYS
     ...
     DO KTOP=1,NTOP
      ...
      DO IIPER=1,IPER-1
       ...
       IF(NSYS.EQ.SIZE(TOPICS(ITOPIC)%STRESS(KPER)%FILES,2))THEN
        IF(ICNST.EQ. TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%ICNST.AND. &
           CNST .EQ. TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%CNST.AND.  &
           FCT.EQ.   TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%FCT.AND.   &
           IMP .EQ.  TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%IMP.AND.   &
           ILAY.EQ.  TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%ILAY.AND.  &
           SFNAME.EQ.TOPICS(ITOPIC)%STRESS(KPER)%FILES(KTOP,ISYS)%FNAME)JEQUAL(ISYS,KTOP)=IIPER
       ENDIF
      ENDDO
