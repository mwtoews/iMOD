...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 DO IPER=1,NPER
  ...
  !## overrule wel/isg/sfr packages per stress-period - make sure to include them always
  SELECT CASE (ITOPIC); CASE (21,29,30); KPER=ABS(KPER); END SELECT
  ...
  ELSE
   ...
   SELECT CASE (ITOPIC)
    CASE (24,26) !## evt,rch
     ...
     IS1=0
    CASE (21,22,23,25,27,28,29,30,31) !## wel,drn,riv,ghb,chd,olf,isg,sfr
  ...
  DO ISYS=1,NSYS
   ...
   SELECT CASE (ITOPIC)
    ...
    CASE (30) 
     ...
     GRIDISG%SDATE=UTL_JDATETOIDATE(GRIDISG%SDATE)
     GRIDISG%EDATE=UTL_JDATETOIDATE(GRIDISG%EDATE)-1  !<- edate is equal to sdate if one day is meant
     IF(.NOT.ISG2SFR(BND(1)%NROW,BND(1)%NCOL,NLAY,ILAY,TOP,BOT,IPER,MP,JU,GRIDISG))EXIT 
