...
!associated with dry cells. 
 ...
 WRITE(FRM,'(A9,I2.2,A11)') '(3(I5,1X),',1,'(G15.7,1X))'
    ...
    IF(Q.NE.0.0)THEN
     DO ILAY=1,NLAY
      IF(TLP(ILAY).GT.0.0)THEN
       WRITE(JU,FRM) ILAY,IROW,ICOL,Q*TLP(ILAY)
...
!save_no_buf 
 ...
 ITOPIC=15; IF(.NOT.PMANAGER_SAVEMF2005_HFB_COMPUTE(IDF,ITOPIC,JU,TOP,BOT,BND,IPRT))RETURN
 ...
 DO ILAY=1,NLAY
  ...
  WRITE(IUDAT(ILAY),'(A)') 'NO,NET_RESIS,TOT_RESIS,FRACTION'
 ENDDO

 !## collect all faults
 JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=TRIM(DIRMNAME)//'_HFB.TXT',STATUS='OLD',ACTION='READ',FORM='FORMATTED')
 CALL PMANAGER_SAVEMF2005_HFB_EXPORT(NHFBNP,IU,JU,IUGEN,IUDAT,IDF) 
...
! ENDIF
 ...
 ELSE
  ...
  IF(LFREEFORMAT)THEN
   CALL UTL_WRITE_FREE(JU,IDF,IINT) 
  ELSE
   IF(IINT.EQ.1)THEN
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_HFB_COMPUTE(IDF,ITOPIC,IU,TOP,BOT,BND,IPRT)
 !###======================================================================
 IMPLICIT NONE
 ...
 WRITE(IU,'(5A10,2A12)') 'ILAY','IROW1','ICOL1','IROW2','ICOL2','HFBFCT','FDZ'
 ...
 DO ISYS=1,SIZE(TOPICS(ITOPIC)%STRESS(1)%FILES,2)
  ...
  IF(LEN_TRIM(IDF%FNAME).GT.0)THEN
   ...
   CALL PMANAGER_SAVEMF2005_HFB_COLLECT(IPC,IDF%NROW,IDF%NCOL,FCT+IMP,IU,TOP,BOT,BND,ILAY,TIDF,BIDF)
  ENDIF
 ...
 SUBROUTINE PMANAGER_SAVEMF2005_HFB_COLLECT(IPC,NROW,NCOL,HFBRESIS, &
               IU,TOP,BOT,BND,ITB,TIDF,BIDF) 
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NROW,NCOL,IU,ITB
 ...
 DO IROW=1,NROW; DO ICOL=1,NCOL; DO ILAY=IL1,IL2
  ...
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.NCOL)THEN
    ...
    IF(ICOL+1.LE.NCOL.AND.FDZ.GT.0.0)THEN
     IF(BND(ILAY)%X(ICOL,IROW).NE.0.AND.BND(ILAY)%X(ICOL+1,IROW).NE.0)THEN
      WRITE(IU,'(5I10,2G12.7)') ILAY,IROW,ICOL,IROW,ICOL+1,HFBRESIS,FDZ !## x-direction
     ENDIF
    ENDIF
 ...
 !###====================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_HFB_EXPORT(NHFBNP,IU,JU,IUGEN,IUDAT,IDF)
 !###====================================================================
 IMPLICIT NONE
 ...
 DO ILAY=1,NLAY
  ...
  READ(JU,*)
  DO
   READ(JU,'(5I10,2G12.7)',IOSTAT=IOS) JLAY,IR1,IC1,IR2,IC2,C,Z
   IF(IOS.NE.0)EXIT
 ...
 !###====================================================================
 REAL FUNCTION PMANAGER_SAVEMF2005_HFB_GETFDZ(TOP,BOT,TF,BF,IC1,IR1,IC2,IR2,NODATA,ILAY)
 !###====================================================================
 IMPLICIT NONE
 ...
 INTEGER(KIND=1),INTENT(IN),DIMENSION(NCOL,NROW,2) :: IPC 
 TYPE(IDFOBJ),INTENT(IN),OPTIONAL :: TOP,BOT
 LOGICAL :: LTB
 REAL :: T1,T2,B1,B2
  
 LTB=.FALSE.
 IF(PRESENT(TOP).AND.PRESENT(BOT))THEN
  IF(ASSOCIATED(TOP%X).AND.ASSOCIATED(BOT%X))LTB=.TRUE.
 ENDIF
 ...
 IF(IPC(ICOL,IROW,1).EQ.INT(1,1).AND.ICOL.LT.NCOL)THEN
  ...
  IF(LTB)THEN
   ...
   T1=TOP%X(ICOL,IROW-1); T2=TOP%X(ICOL,IROW)
   B1=BOT%X(ICOL,IROW-1); B2=BOT%X(ICOL,IROW)
   IF(T1.NE.TOP%NODATA.AND.T2.NE.TOP%NODATA.AND. &
      B1.NE.TOP%NODATA.AND.B2.NE.TOP%NODATA)THEN
    T1=MAX(T1,T2); B1=MIN(B1,B2)
    WRITE(IU,'(I10)') N 
    WRITE(IU,'(3(F10.2,A1))') IDF%SX(ICOL),',',IDF%SY(IROW-1),',',T1
    WRITE(IU,'(3(F10.2,A1))') IDF%SX(ICOL),',',IDF%SY(IROW)  ,',',T1
    WRITE(IU,'(3(F10.2,A1))') IDF%SX(ICOL),',',IDF%SY(IROW)  ,',',B1
    WRITE(IU,'(3(F10.2,A1))') IDF%SX(ICOL),',',IDF%SY(IROW-1),',',B1
    WRITE(IU,'(A)') 'END'
   ENDIF
  ELSE
   WRITE(IU,'(I10)') N 
 ...
 IF(IPC(ICOL,IROW,2).EQ.INT(1,1).AND.IROW.LT.NROW)THEN
  ...
  IF(LTB)THEN
   !# find appropriate top/bot
   T1=TOP%X(ICOL-1,IROW); T2=TOP%X(ICOL-1,IROW)
   B1=BOT%X(ICOL  ,IROW); B2=BOT%X(ICOL  ,IROW)
   IF(T1.NE.TOP%NODATA.AND.T2.NE.TOP%NODATA.AND. &
      B1.NE.TOP%NODATA.AND.B2.NE.TOP%NODATA)THEN
    T1=MAX(T1,T2); B1=MIN(B1,B2)
    WRITE(IU,'(I10)') N  
    WRITE(IU,'(3(F10.2,A1))') IDF%SX(ICOL-1),',',IDF%SY(IROW),',',T1
    WRITE(IU,'(3(F10.2,A1))') IDF%SX(ICOL  ),',',IDF%SY(IROW),',',T1
    WRITE(IU,'(3(F10.2,A1))') IDF%SX(ICOL  ),',',IDF%SY(IROW),',',B1
    WRITE(IU,'(3(F10.2,A1))') IDF%SX(ICOL-1),',',IDF%SY(IROW),',',B1
    WRITE(IU,'(A)') 'END'
   ENDIF
  ELSE
   WRITE(IU,'(I10)') N  
