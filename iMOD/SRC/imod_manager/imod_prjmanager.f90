...
!## IFVDL SFT RCNC
 ...
 INTEGER :: IU,JU,ILAY,IROW,ICOL,I,J,KTOP,KPER,IPER,NP,NTOP,SCL_D,SCL_U,ICNST,NSYS,ISYS,JSYS,MP,N, &
...
!           'FHB','LAK','PCG'/                                             !31-40
  ...
  SELECT CASE (ITOPIC)
   ...
   CASE (22,27)
    IF(PBMAN%ICONCHK.EQ.0)WRITE(FRM,'(A10,I2.2,A14)') '(3(I5,1X),',N,'(G15.7,1X),I5)'
    IF(PBMAN%ICONCHK.EQ.1)WRITE(FRM,'(A10,I2.2,A20)') '(3(I5,1X),',N,'(G15.7,1X),I5,F10.1)'
   CASE DEFAULT
...
IF(TLP(ILAY).NE.1.0)THEN
WRITE(*,*) 
ENDIF
         JSYS=1; IF(PBMAN%SSYSTEM.EQ.1)JSYS=ISYS
         IF(ITOPIC.EQ.22.OR.ITOPIC.EQ.27)THEN
          IF(PBMAN%ICONCHK.EQ.0)WRITE(JU,FRM) ILAY,IROW,ICOL,(XTMP(JTOP(I)),I=1,NTOP),JSYS
          IF(PBMAN%ICONCHK.EQ.1)WRITE(JU,FRM) ILAY,IROW,ICOL,(XTMP(JTOP(I)),I=1,NTOP),JSYS,1.0
         ELSE
 ...
 CLOSE(IU); DEALLOCATE(TLP,TP,BT,KH,XTMP); CALL PMANAGER_SAVEMF2005_DEALLOCATEPCK()
