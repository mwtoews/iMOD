...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 CHARACTER(LEN=512) :: DIRMNAME,LINE,DIR
 ...
 INTEGER :: IHEDUN,IBCFCB,IRCHCB,IEVTCB,IDRNCB,IRIVCB,IGHBCB,ICHDCB,IWELCB,ISFRCB,IFHBCB,ICB,IROW, &
 ...
 IF(LLAK)THEN; ALLOCATE(LAK(9   )); DO ILAY=1,SIZE(LAK); CALL IDFNULLIFY(LAK(ILAY)); ENDDO; ENDIF
...
!  WRITE(IULAK,'(A)') '# LAK7 File Generated by '//TRIM(UTL_IMODVERSION())
 ...
 IF(LMSP)THEN
  ...
  CALL PMANAGER_SAMEMF2005_METASWAP(IDF%NCOL,IDF%NROW,NLAY,HNOFLOW,BND,IDF)
 ENDIF
 ...
 IF(LHFB)THEN
  ...
  ENDDO
  ITOPIC=15; IF(.NOT.PMANAGER_SAVEMF2005_HFB(IDF,ITOPIC,IU,IUGEN,TOP,BOT,BND,KD))RETURN
  DEALLOCATE(IUGEN)
 ENDIF
 ...
 CALL PMANAGER_SAMEMF2005_METASWAP_CHECK(IDF,NCOL,NROW,NLAY,NODATA)
...
!   IF(M.LT.5)CALL PRINTTEXT('IPF for artificial recharge should be at least 5 column, x,y,ilay,id,capacity',2)
 ...
 !###====================================================================
 SUBROUTINE PMANAGER_SAMEMF2005_METASWAP_CHECK(IDF,NCOL,NROW,NLAY,NODATA)
 !###====================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(IN) :: IDF
 INTEGER,INTENT(IN) :: NCOL,NROW,NLAY
...
! ENDIF
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_HFB(IDF,ITOPIC,IU,IUGEN,TOP,BOT,BND,KD) 
 !###======================================================================
 IMPLICIT NONE
...
!      end if
  ...
  DO ISYS=1,SIZE(TOPICS(ITOPIC)%STRESS(1)%FILES,2)
   ...
   IF(LEN_TRIM(IDF%FNAME).GT.0)THEN
    ...
    CALL PMANAGER_SAVEMF2005_HFBEXPORT(IPC,IDF%NROW,IDF%NCOL,FCT+IMP,NHFBNP,IWRITE,IU,IUGEN,IDF,TOP,BOT,BND,KD,ILAY,TIDF,BIDF)
   ENDIF
...
! IF(PMANAGER_SAVEMF2005_HFB)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 !###====================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_HFBEXPORT(IPC,NROW,NCOL,HFBFCT,NHFB,IWRITE, &
  ...
  IF(IPC(ICOL,IROW,1).EQ.INT(1,1))THEN
   IF(ICOL.LT.NCOL)THEN
    ...
    DO ILAY=IL1,IL2
     ...
     IF(ITB.EQ.0)THEN
      FCT=PMANAGER_SAVEMF2005_HFBGETFACTOR(TOP,BOT,HFBFCT,TIDF%X,BIDF%X,ICOL,IROW,ICOL+1,IROW,NODATA,ILAY) 
      !## take the next no fault on this modellayer
      IF(FCT.EQ.0.0)CYCLE
     ENDIF
 
     IF(IWRITE.EQ.1)CALL PMANAGER_SAVEMF2005_HFBGENFILES(IUGEN(ILAY),IPC,IDF,NROW,NCOL,IROW,ICOL,ILAY)
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   IF(IROW.LT.NROW)THEN
    ...
    DO ILAY=IL1,IL2
     ...
     IF(ITB.EQ.0)THEN
      FCT=PMANAGER_SAVEMF2005_HFBGETFACTOR(TOP,BOT,HFBFCT,TIDF%X,BIDF%X,ICOL,IROW,ICOL,IROW+1,NODATA,ILAY) 
      !## take the next no fault on this modellayer
      IF(FCT.EQ.0.0)CYCLE
     ENDIF
 ...
 !###====================================================================
 REAL FUNCTION PMANAGER_SAVEMF2005_HFBGETFACTOR(TOP,BOT,FCT,TF,BF,IC1,IR1,IC2,IR2,NODATA,ILAY)
 !###====================================================================
 IMPLICIT NONE
 ...
 PMANAGER_SAVEMF2005_HFBGETFACTOR=0.0
 ...
 PMANAGER_SAVEMF2005_HFBGETFACTOR=FFCT*DZ**4.0

 END FUNCTION PMANAGER_SAVEMF2005_HFBGETFACTOR

 !###====================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_HFBGENFILES(IU,IPC,IDF,NROW,NCOL,IROW,ICOL,ILAY) 
 !###====================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 ...
 END SUBROUTINE PMANAGER_SAVEMF2005_HFBGENFILES
 ...
 INTEGER :: IROW,ICOL,ILAY,NSSITR,I,J
 REAL :: THETA,SSCNCR,LVL,ILVL
 CHARACTER(LEN=256) :: LINE
 ...
 IF(IPER.EQ.0)THEN
  ...
  DO I=1,NLAKES
   DO J=2,4
    IF(.NOT.PMANAGER_SAVELAKE_GETMEANVALUE(LAK(1)%X,LAK(J)%X,ULAKES(I),LVL,IBATCH))RETURN
    IF(J.EQ.2)THEN
  ...
  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
   !## find layer underneath bottom of lake (minimal level)
   DO ILAY=1,NLAY
    IF(LAK(3)%X(ICOL,IROW).LE.TOP(ILAY)%X(ICOL,IROW))THEN
  ...
  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
   ...
   DO ILAY=1,NLAY
    IF(LAK(3)%X(ICOL,IROW).LE.TOP(ILAY)%X(ICOL,IROW))THEN
     KD(ILAY)%X(ICOL,IROW)=LAK(5)%X(ICOL,IROW)
    ENDIF
   ENDDO
  ENDDO; ENDDO
  !## get lakebed leakance
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVELAKE_GETMEANVALUE(X,Y,ULAKE,LVL,IBATCH)
 !###======================================================================
 IMPLICIT NONE
 ...
 DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
  IF(INT(X(ICOL,IROW)).EQ.INT(ULAKE))THEN
   LVL=LVL+Y(ICOL,IROW); ILVL=ILVL+1.0
  ENDIF
 ENDDO; ENDDO
 IF(ILVL.LE.0.0)THEN
  IF(IBATCH.EQ.0)THEN
...
! ENDIF      
 ...
 TOPICS(32)%SNAME(5)='Lakebed Leakance'
 TOPICS(32)%SNAME(6)='Precipitation at surface Lake'
 TOPICS(32)%SNAME(7)='Evaporation at surface Lake'
 TOPICS(32)%SNAME(8)='Overland runoff'
 TOPICS(32)%SNAME(9)='Lake Withdrawall'
