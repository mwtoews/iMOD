...
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_WSELECTFILE,UTL_CAP,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_FILLDATES,NEWLINE,UTL_LISTOFFILES, &
    ...
    HMSTOITIME,UTL_IDATETOJDATE,UTL_PCK_READTXT,UTL_PCK_GETTLP,ITIMETOGDATE,UTL_IMODVERSION
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 ELSEIF(ID.EQ.ID_SAVE)THEN
  ...
  IF(PMANAGER_SAVEPRJ(FNAME))THEN
   IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Successfully written project file:'//CHAR(13)//TRIM(FNAME),'Information')
 ...
 ELSEIF(ID.EQ.ID_SAVERUN)THEN
  ...
  IF(IFORMAT.EQ.1)THEN
   IF(PMANAGER_SAVERUN(FNAME))THEN
    IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Successfully written runfile:'//CHAR(13)//TRIM(FNAME),'Error')
  ...
  ELSEIF(IFORMAT.EQ.2)THEN
   IF(PMANAGER_SAVEMF2005(FNAME,IBATCH))THEN
    IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Successfully written MF2005 files:'//CHAR(13)//TRIM(FNAME),'Error')
 ...
 ENDIF
 IBNDCHK=0; IFVDL=0
 WRITE(IU,'(12(I10,1X))') NLAY,MXNLAY,NPER,0,1,0,N,0,IUNCONF,IFVDL,IARMWP,IBNDCHK
...
!  ENDDO
 ...
 IF(LHFB)THEN
  ...
  ITOPIC=15; IF(.NOT.PMANAGER_SAVEMF2005_HFB(IDF,ITOPIC,IU,TOP,BOT,BND,KD))RETURN
 ENDIF
...
!     IF(.NOT.LEX)CYCLE
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_HFB(IDF,ITOPIC,IU,TOP,BOT,BND,KD) 
 !###======================================================================
 IMPLICIT NONE
 ...
 DO IWRITE=0,1
  ...
  DO ISYS=1,SIZE(TOPICS(ITOPIC)%STRESS(1)%FILES,1)
   ...
   ILAY     =TOPICS(ITOPIC)%STRESS(1)%FILES(ISYS,1)%ILAY
   FCT      =TOPICS(ITOPIC)%STRESS(1)%FILES(ISYS,1)%FCT
   IMP      =TOPICS(ITOPIC)%STRESS(1)%FILES(ISYS,1)%IMP
   IDF%FNAME=TOPICS(ITOPIC)%STRESS(1)%FILES(ISYS,1)%FNAME

   IF(LEN_TRIM(IDF%FNAME).GT.0)THEN
    CALL ASC2IDF_HFB(IDF,IDF%NROW,IDF%NCOL,IPC,IDF%FNAME,ILAY,TIDF,BIDF)
    CALL PMANAGER_SAVEMF2005_HFBEXPORT(IPC,IDF%NROW,IDF%NCOL,FCT+IMP,NHFBNP,IWRITE,IU,IDF,TOP,BOT,BND,KD,ILAY,TIDF,BIDF)
   ENDIF
   
  ENDDO
 ENDDO
  
! OPEN/CLOSE .\wcab\RIV7\RIV_t1.ARR 1.0 (FREE) -1
...
! IF(PMANAGER_SAVEMF2005_HFB)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 !###====================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_HFBEXPORT(IPC,NROW,NCOL,HFBFCT,NHFB,IWRITE,IU,IDF,TOP,BOT,BND,KD,ITB,TIDF,BIDF) 
 !###====================================================================
 IMPLICIT NONE
 ...
 !    CALL PMANAGER_SAVEMF2005_HFBGENFILES(IPC,IDF,NROW,NCOL,IROW,ICOL,ILAY,I,JLINE)
     ...
     IF(FCT.NE.0.0)THEN
      ...
      IF(IWRITE.EQ.1)WRITE(IU,'(5I10,G10.4)') ILAY,IROW,ICOL,IROW,ICOL+1,FCT !## x-direction
     ENDIF
    ENDDO
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   IF(IROW.LT.NROW)THEN
    ...
    DO ILAY=IL1,IL2
     ...
     IF(ITB.EQ.0)THEN
      FCT=PMANAGER_SAVEMF2005_HFBGETFACTOR(TOP,BOT,HFBFCT,TIDF%X,BIDF%X,ICOL,IROW,ICOL,IROW+1,NODATA,ILAY) !,TF(ICOL,IROW),BF(ICOL,IROW),TOP(ICOL,IROW,ILAY),BOT(ICOL,IROW,ILAY))
      !## take the next no fault on this modellayer
      IF(FCT.EQ.0.0)CYCLE
     ENDIF
   
!    CALL PMANAGER_SAVEMF2005_HFBGENFILES(IPC,NROW,NCOL,IROW,ICOL,ILAY,I,JLINE)
     ...
     IF(FCT.NE.0.0)THEN
      NHFB(ILAY)=NHFB(ILAY)+1
      IF(IWRITE.EQ.1)WRITE(IU,'(5I10,G10.4)') ILAY,IROW,ICOL,IROW,ICOL+1,FCT !## Y-direction
     ENDIF
    ENDDO
   ENDIF
  ENDIF
 ...
 !## no in current modellayer
 IF(DZ.LE.0.0)RETURN

 DZ=(TPV-BTV)/DZ

 !# factor declines quadratically with layer occupation
 PMANAGER_SAVEMF2005_HFBGETFACTOR=FCT*(1.0/DZ**2.0)

 END FUNCTION PMANAGER_SAVEMF2005_HFBGETFACTOR

 !###====================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_HFBGENFILES(IU,IPC,IDF,NROW,NCOL,IROW,ICOL,ILAY,I,JLINE)
 !###====================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 INTEGER,INTENT(IN) :: NROW,NCOL,IROW,ICOL,ILAY,JLINE,IU
...
! ENDDO
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (ID_APPLY)
      ...
      CALL WDIALOGGETINTEGER(IDF_INTEGER1,IROW1)
      CALL WDIALOGGETINTEGER(IDF_INTEGER2,IROW2)
      CALL WDIALOGGETINTEGER(IDF_INTEGER3,ISTEP) 
 ...
 !## adjust time-steps
 IF(ITG.EQ.1)THEN
  J=NPER+IR1+(SIZE(SIM_C2)-IR2+1) !(IROW2-IROW1+1); 
  ALLOCATE(ITIME(J))
