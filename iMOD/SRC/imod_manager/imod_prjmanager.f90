...
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_WSELECTFILE,UTL_CAP,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_FILLDATES, &
    IDATETOGDATE,UTL_IDATETOJDATE,UTL_GDATE,UTL_JDATETOIDATE,JD,UTL_IDFSNAPTOGRID,UTL_CREATEDIR,UTL_GETMED
USE MOD_IDF, ONLY : IDFREAD,IDFDEALLOCATE,IDFNULLIFY,IDFREADSCALE,IDFCOPY,IDFDEALLOCATEX,IDFIROWICOL
...
LOGICAL,PRIVATE :: LBCF,LLPF,LPCG,LPCGN,LSIP,LRCH,LEVT,LDRN,LRIV,LGHB,LOLF,LCHD,LWEL,LISG
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 INTEGER :: IHEDUN,IBCFCB,IRCHCB,IEVTCB,IDRNCB,IRIVCB,IGHBCB,ICHDCB,IWELCB,ICB
 REAL :: HNOFLOW
 TYPE(IDFOBJ) :: IDF
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: BND,TOP,BOT,KD
 ...
 DO ILAY=1,NLAY
  ...
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,TOP(ILAY),BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'dis\top',HNOFLOW,IINV))RETURN
   ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'; TOP(ILAY)%X=0.0; LTB=.FALSE.
   ENDIF
  ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'; TOP(ILAY)%X=0.0; LTB=.FALSE.
  ENDIF
  ITOPIC=3
  IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS))THEN
   IF(ASSOCIATED(TOPICS(ITOPIC)%STRESS(1)%FILES))THEN
    IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,BOT(ILAY),BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'dis\bot',HNOFLOW,IINV))RETURN
   ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'; BOT(ILAY)%X=0.0; LTB=.FALSE.
   ENDIF
  ELSE; WRITE(IU,'(A)') 'CONSTANT 0.0'; BOT(ILAY)%X=0.0; LTB=.FALSE.
  ENDIF
 ENDDO
 ...
 IF(LBCF)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,KD(ILAY),BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'bcf6\tran',HNOFLOW,IINV))RETURN
 ...
 IF(LLPF)THEN
  ...
  DO ILAY=1,NLAY
   ...
   IF(.NOT.PMANAGER_SAVEMF2005_MOD(IU,KD(ILAY),BND,ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINT,DIR,'lpf7\hk',HNOFLOW,IINV))RETURN
   !## vka
 ...
 DO ITOPIC=21,29
  ...
  SELECT CASE (ITOPIC)
   ...
   CASE (27); LEX=LOLF; CPCK='OLF'; ICB=IDRNCB; CMAXNO='NaN'; CAUX=', AUXILIARY SYSTEM NOPRINT'; TEXT='_'
   CASE (28); LEX=LCHD; CPCK='CHD'; ICB=ICHDCB; CMAXNO='NaN'; CAUX=', AUXILIARY SYSTEM NOPRINT'; TEXT='_'
   CASE (29); LEX=LISG; CPCK='ISG'; ICB=IRIVCB; CMAXNO='NaN'; CAUX=', AUXILIARY SYSTEM NOPRINT'; TEXT='_'
  END SELECT
 ...
 IF(LOLF)CALL PMANAGER_SAMEMF2005_COMBINE(DIR,(/'OLF','DRN','DRN_'/),(/LOLF,LDRN,.TRUE./),IDRNCB,'')
 IF(LISG)CALL PMANAGER_SAMEMF2005_COMBINE(DIR,(/'ISG','RIV','RIV_'/),(/LISG,LRIV,.TRUE./),IRIVCB,'AUXILIARY INFFCT')
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_SAMEMF2005_COMBINE(DIR,PCK,LPCK,CB,CAUX)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: DIR,CAUX
 INTEGER,INTENT(IN) :: CB
 LOGICAL,INTENT(IN),DIMENSION(3) :: LPCK
 CHARACTER(LEN=*),INTENT(IN),DIMENSION(3) :: PCK
 INTEGER,DIMENSION(3) :: IU,JU,NO
 CHARACTER(LEN=256) :: LINE
 INTEGER :: I,IPER
 
 !## nothing to do
 IF(.NOT.LPCK(1))RETURN
 
 !## read from files
 IU=0
 DO I=1,SIZE(PCK)
  IF(.NOT.LPCK(I))CYCLE
  LINE=TRIM(DIR)//'\'//TRIM(PCK(I))//'7\'//TRIM(PCK(I))
  IF(I.LE.2)THEN
 ...
 DO I=1,SIZE(PCK); IF(LPCK(I))READ(IU(I),'(A256)') LINE; ENDDO; WRITE(IU(I),'(A)') TRIM(LINE)

 NO=0; DO I=1,2; IF(LPCK(I))READ(IU(I),*) NO(I); ENDDO
 LINE=TRIM(ITOS(SUM(NO)))//','//TRIM(ITOS(CB))//','//TRIM(CAUX)//' AUXILIARY SYSTEM NOPRINT'
 WRITE(IU(3),'(A)') TRIM(LINE)
 
 IF(.NOT.LPCK(2))CALL UTL_CREATEDIR(TRIM(DIR)//'\'//TRIM(PCK(2))//'7\')
 ...
 DO IPER=1,NPER
  ...
  !## onthouden welke als laatste ingelezen ...
  
  JU=0
  DO I=1,2
   READ(IU(I),*) TXT,LINE
   IF(I.EQ.1)THEN; JU(I)=UTL_GETUNIT(); CALL OSD_OPEN(JU(I),FILE=LINE,STATUS='OLD',ACTION='READ'); ENDIF
   IF(I.EQ.2)THEN; JU(I)=UTL_GETUNIT(); CALL OSD_OPEN(JU(I),FILE=LINE,STATUS='OLD',ACTION='APPEND'); ENDIF
  ENDDO
  
  WRITE(JU(3),'(A)') 'OPEN/CLOSE .\'//TRIM(DIR(INDEX(DIR,'\',.TRUE.)+1:))//'\'//PCK(2)//'7'//'\'//PCK(2)//'_t'//TRIM(ITOS(IPER)
  
  CLOSE(JU(1),STATUS='DELETE') 

!OPEN/CLOSE .\AD\RIV7\RIV_t1.arr 1.0 (FREE) -1
 ...
 DO IPER=1,NPER
  ...
  ELSE
   ...
   SELECT CASE (ITOPIC)
    ...
    CASE (21,22,23,25,27,28) !## wel,drn,riv,ghb,chd,olf
...
!## cannot 
     ...
     SELECT CASE (ITOPIC)
      CASE (22:28); CALL PMANAGER_SAVEMF2005_CORRECT(SCL_U,0,ILAY,BND,PCK(K,ISYS),HNOFLOW,FCT,IMP,-1)
     END SELECT
 ...
 CHARACTER(LEN=256) :: SFNAME,LINE,ID
 CHARACTER(LEN=5) :: EXT
 INTEGER :: JU,KU,ILAY,IROW,ICOL,I,ITOP,ISYS,NROWIPF,NCOLIPF,IEXT,MP,SDATE,EDATE,IOS,MTYPE
 LOGICAL :: LIPF,LISG
 ...
 ELSE
  ...
  DO ISYS=1,NSYS
   ...
   ELSEIF(LIPF)THEN
    ...
    LINE=PCK(1,ISYS)%FNAME(:INDEX(PCK(1,ISYS)%FNAME,'\',.TRUE.)-1)
    ...
    DO I=1,NROWIPF
     ...
     IF(IEXT.GT.0)THEN
      IF(.NOT.PMANAGER_SAVEMF2005_PCK_READTXT(2,SDATE,EDATE,MTYPE,Q,TRIM(LINE)//'\'//TRIM(ID)//'.'//TRIM(EXT)))THEN
       CLOSE(JU); CLOSE(KU); RETURN
      ENDIF
     ENDIF
     ...
     DO ILAY=1,NLAY
      IF(TLP(ILAY).GT.0.0)THEN
       LINE=TRIM(ITOS(ILAY))//','//TRIM(ITOS(IROW))//','//TRIM(ITOS(ICOL))//','// &
            TRIM(RTOS(Q*TLP(ILAY),'F',7))//','//TRIM(ITOS(ISYS))
       WRITE(JU,*) TRIM(LINE)
       NP=NP+1; MP=MP+1
      ENDIF
     ENDDO
   ...
   ELSE 
    ...
    DO IROW=1,PCK(1,1)%NROW; DO ICOL=1,PCK(1,1)%NCOL
     ...
     DO ILAY=1,NLAY
      ...
      LINE=TRIM(LINE)//','//TRIM(ITOS(ISYS)); WRITE(JU,*) TRIM(LINE)
      NP=NP+1; MP=MP+1
     ENDDO
