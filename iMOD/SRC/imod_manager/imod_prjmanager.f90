 ...
 DO ILAY=1,NLAY; DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
  ...
  IF(ILAY.LT.NLAY)THEN
   IF(BND(ILAY  )%X(ICOL,IROW).NE.0.AND. &
    ...
    T1=0.0; IF(KHV(ILAY)%X(ICOL,IROW).GT.0.0)T1=T/KHV(ILAY)%X(ICOL,IROW)
    !## bottom aquifer
    T =0.5*(TOP(ILAY+1)%X(ICOL,IROW)-BOT(ILAY+1)%X(ICOL,IROW))
    T2=0.0; IF(KHV(ILAY+1)%X(ICOL,IROW).GT.0.0)T2=T/KHV(ILAY+1)%X(ICOL,IROW)
    !## intermediate aquitard
    T =     BOT(ILAY)%X(ICOL,IROW)-TOP(ILAY+1)%X(ICOL,IROW)
    T3=0.0; IF(KVV(ILAY)%X(ICOL,IROW).GT.0.0)T3=T/KVV(ILAY)%X(ICOL,IROW)
    !## total resistance
...
!associated with dry cells. 
 ...
 DO IPER=1,NPER
  ...
  JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=EXFNAME,STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(JU.EQ.0)THEN; IOS=-1; EXIT; ENDIF
  ...
  DO ISYS=1,NSYS
   ...
   DEALLOCATE(STRING)
   CLOSE(KU); IF(NP.LE.0)CLOSE(JU,STATUS='DELETE')
 ...
 !## store maximum number of well in simulation
 ALLOCATE(NP(0:NPER)); NP=0; LPER=0
...
!       IMP .EQ.  TOPICS(ITOPIC)%STRESS(LPER)%FILES(1,ISYS)%IMP.AND.   &
   ...
   DO I=1,NROWIPF
    ...
    IF(IPER.EQ.0)THEN
     ...
     LINE='WELLID_'//TRIM(ITOS(NP(IPER)))//','//TRIM(ITOS(NNODES))
     !## identification
     WRITE(IU,'(A)') TRIM(LINE)
...
!         'Select one of the following:'//CHAR(13)//'NONE, THIEM, SKIN, GENERAL, SPECIFYCWC','Error'); IOS=-1; EXIT
    ...
    ELSE
     ...
     LINE='WELLID_'//TRIM(ITOS(NP(IPER)))//','//TRIM(RTOS(Q,'G',7))
     WRITE(IU,'(A)') TRIM(LINE)
