...
CONTAINS
 ...
 IF(ITOPIC.EQ.20)THEN; CALL PMANAGEROPEN_PEST(); RETURN; ENDIF
...
! ENDIF
 ...
 SELECT CASE (ID)
  ...
  CASE (ID_PARAMETERS)
   ...
   CALL WGRIDPUTOPTION(IDF_GRID1  ,2 ,PEST%PARAM%IPARAM,N)
...
!  ENDDO
 ...
 DO
  ...
  !## pst module is exception
  IF(I.EQ.20)THEN; CALL PMANAGER_LOADPST(IU,NPER); TOPICS(I)%IACT_MODEL=IACT; CYCLE; ENDIF
 ...
 IF(PEST%PE_NBATCH.GT.0)THEN
  DO I=1,PEST%PE_NBATCH
   READ(IU,'(A256)') LINE
   READ(LINE,*) PEST%B_FRACTION(I),PEST%B_BATCHFILE(I),PEST%B_OUTFILE(I) 
  ENDDO
 ENDIF
 DO I=1,SIZE(PEST%PARAM)
  READ(IU,'(A256)') LINE
 ...
 DO I=1,MAXTOPICS
  ...
  !## pst module is exception
  IF(I.EQ.20)THEN
   LINE='(PST),'//TRIM(ITOS(SIZE(PEST%PARAM)))
   WRITE(IU,'(A)') TRIM(LINE) 
   LINE=TRIM(ITOS(PEST%PE_MXITER))         //','//TRIM(RTOS(PEST%PE_STOP,'F',2))     //','// &
        TRIM(RTOS(PEST%PE_SENS,'F',2))     //','//TRIM(ITOS(PEST%PE_NPERIOD))        //','// &
        TRIM(ITOS(PEST%PE_NBATCH))         //','//TRIM(RTOS(PEST%PE_TARGET(1),'F',2))//','// &
   ...
   WRITE(IU,'(A)') TRIM(LINE)   
   IF(PEST%PE_NPERIOD.GT.0)THEN
    DO J=1,PEST%PE_NPERIOD
     LINE=TRIM(ITOS(PEST%S_PERIOD(J)))//','//TRIM(ITOS(PEST%E_PERIOD(J)))
     WRITE(IU,'(A)') TRIM(LINE)   
    ENDDO
   ENDIF
   IF(PEST%PE_NBATCH.GT.0)THEN
    DO J=1,PEST%PE_NBATCH
     LINE=TRIM(RTOS(PEST%B_FRACTION(J),'F',2))//','//TRIM(PEST%B_BATCHFILE(J))//','//TRIM(PEST%B_OUTFILE(J))
     WRITE(IU,'(A)') TRIM(LINE)   
    ENDDO
   ENDIF
   DO J=1,SIZE(PEST%PARAM)
    LINE=TRIM(ITOS(PEST%PARAM(J)%PACT))           //','// &
         TRIM(PEST%PARAM(J)%PPARAM)               //','// &
         TRIM(ITOS(PEST%PARAM(J)%PILS))           //','// &
         TRIM(ITOS(PEST%PARAM(J)%PIZONE))         //','// &    
         TRIM(RTOS(PEST%PARAM(J)%PINI,'F',2))     //','// &
         TRIM(RTOS(PEST%PARAM(J)%PDELTA,'F',2))   //','// &
         TRIM(RTOS(PEST%PARAM(J)%PMIN,'F',2))     //','// &
         TRIM(RTOS(PEST%PARAM(J)%PMAX,'F',2))     //','// &
         TRIM(RTOS(PEST%PARAM(J)%PINCREASE,'F',2))//','// &
         TRIM(ITOS(PEST%PARAM(J)%PIGROUP))        //','// &
         TRIM(ITOS(PEST%PARAM(J)%PLOG))
    WRITE(IU,'(A)') TRIM(LINE)   
   ENDDO
   LINE=TRIM(ITOS(SIZE(PEST%IDFFILES)))
   WRITE(IU,'(A)') TRIM(LINE) 
   DO J=1,SIZE(PEST%IDFFILES)
    LINE=TRIM(PEST%IDFFILES(I))
    WRITE(IU,'(A)') TRIM(LINE)    
   ENDDO
   CYCLE
 ...
 DO
  ...
  IF(ITOPIC.GT.0)THEN
   IF(TOPICS(ITOPIC)%IACT.EQ.1)THEN
    ...
    !## pst module
    IF(ITOPIC.EQ.20)THEN; CALL PMANAGER_LOADPST(IU,NSYS); TOPICS(ITOPIC)%IACT_MODEL=1; CYCLE; ENDIF
