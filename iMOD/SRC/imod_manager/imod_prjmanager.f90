...
!  ENDIF
   DO J=3,5
    ...
    ENDIF
   ENDDO
   WRITE(IULAK,'(A)') TRIM(LINE)//'      ORIGINAL LAKE IDENTIFICATION: '//TRIM(RTOS(ULAKES(I),'F',2))
  ENDDO
...
!     RETURN
    ...
    DO I=1,NLAKES
     IF(LAK(1)%X(ICOL,IROW).EQ.ULAKES(I))THEN; LBD(ILAY)%X(ICOL,IROW)=LAK(1)%X(ICOL,IROW); EXIT; ENDIF
    ENDDO
