...
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_WSELECTFILE,UTL_CAP,UTL_MESSAGEHANDLE,UTL_SUBST,UTL_FILLDATES,NEWLINE,UTL_LISTOFFILES, &
    ...
    HMSTOITIME,UTL_IDATETOJDATE,UTL_PCK_READTXT,UTL_PCK_GETTLP,ITIMETOGDATE,UTL_IMODVERSION,UTL_DEBUGLEVEL
...
USE MOD_ISG_UTL, ONLY : ISGDEAL,ISGREAD,ISGOPENFILES,ISGCLOSEFILES
...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 ELSEIF(ID.EQ.ID_SAVERUN)THEN
  ...
  FNAME=RUNFNAME
  IF(.NOT.PMANAGER_INITSIM(FNAME,IBATCH))THEN; IF(ASSOCIATED(SIM))DEALLOCATE(SIM); RETURN; ENDIF

  IF(IBATCH.EQ.0)CALL UTL_MESSAGEHANDLE(0)

  IF(IFORMAT.EQ.1)THEN
   IF(PMANAGER_SAVERUN(FNAME))THEN
    IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully written runfile:'//CHAR(13)//TRIM(FNAME),'Error')
    IF(IBATCH.EQ.1)WRITE(*,'(/A/)') 'Successfully written runfile:'//TRIM(FNAME)
    PMANAGERRUN=.TRUE.
   ENDIF
  ELSEIF(IFORMAT.EQ.2)THEN
   IF(PMANAGER_SAVEMF2005(FNAME,IBATCH,IDEBUG))THEN
    IF(IBATCH.EQ.0)CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'Successfully written MF2005 files:'//CHAR(13)//TRIM(FNAME),'Error')
    IF(IBATCH.EQ.1)WRITE(*,'(/A/)') 'Successfully written MF2005 files:'//TRIM(FNAME)
    PMANAGERRUN=.TRUE.
   ENDIF
 ...
 INTEGER :: IU,IUBAS,IPER,KPER,IERROR,SCL_D,SCL_U,IINT,ITOPIC,ILAY,ISS,IINV,NRCHOP,NEVTOP,ISFRCB2
 INTEGER :: IHEDUN,IBCFCB,IRCHCB,IEVTCB,IDRNCB,IRIVCB,IGHBCB,ICHDCB,IWELCB,ISFRCB,IFHBCB,ICB,IROW, &
   ICOL,ISTEADY,NHED,NFLW,IFHBUN,IFHBSS,NFHBX1,NFHBX2,NBDTIM,I,J,JD0,JD1,ISEC0,ISEC1
 ...
 CALL UTL_CREATEDIR(DIR(:INDEX(DIR,'\',.TRUE.)-1))
 ...
 IF(LHFB) WRITE(IU,'(A)') 'HFB6 23 '//CHAR(39)//TRIM(LINE)//'.HFB7'//CHAR(39)
 IF(LSFR) WRITE(IU,'(A)') 'SFR  24 '//CHAR(39)//TRIM(LINE)//'.SFR7'//CHAR(39)
 IF(LFHB)THEN; WRITE(IU,'(A)') 'FHB  25 '//CHAR(39)//TRIM(LINE)//'.FHB7'//CHAR(39); IFHBUN=25; ENDIF
 ...
 IF(LSFR)THEN
  WRITE(IU,'(A,I3,A)') 'DATA(BINARY) ',ISFRCB,' '//CHAR(39)//TRIM(LINE)//'_FSFR.DAT'//CHAR(39) 
  IF(ISFRCB2.GT.0)WRITE(IU,'(A)') 'DATA         ',ISFRCB2,' '//CHAR(39)//TRIM(LINE)//'_FSFR.TXT'//CHAR(39) 
 ENDIF
 IF(LFHB)WRITE(IU,'(A,I3,A)') 'DATA(BINARY) ',IFHBCB,' '//CHAR(39)//TRIM(LINE)//'_FFHB.DAT'//CHAR(39) 
 ...
 IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.BAS6'//'...')
 IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'.BAS6'//'...'
 ...
 IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.DIS6'//'...')
 IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'.DIS6'//'...'
...
!  ENDDO
 ...
 !## write metaswap
 IF(LMSP)THEN
  IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.MSP7 ...')
  IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'.MSP7 ...'
 ...
 IF(LBCF)THEN
  ...
  IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.BCF6'//'...')
  IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'.BCF6'//'...'
 ...
 IF(LLPF)THEN
  ...
  IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.LPF7'//'...')
  IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'.LPF7'//'...'
 ...
 IF(LHFB)THEN
  ...
  IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.HFB7'//'...')
  IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'.HFB7'//'...'
  ...
  DO ILAY=1,NLAY
   IUGEN(ILAY)=UTL_GETUNIT(); CALL OSD_OPEN(IUGEN(ILAY),FILE=TRIM(DIR)//'_HFB_L'//TRIM(ITOS(ILAY))//'.GEN',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
 ...
 DO ITOPIC=21,SIZE(TOPICS)
  ...
  IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIR)//'.'//TRIM(CPCK)//'7...')
  IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIR)//'.'//TRIM(CPCK)//'7...'
...
! COMMENT ALS ISFRCB2>0
! STREAM LISTING     PERIOD      1   STEP        1
!
...
!
!   1    1    1     1     1    6.3357E+04  8.0273E+03   5.5330E+04  0.000E+00  0.000E+00  0.000E+00  3.37488E+02  3.586E-01  3.930E+00  5.908E+03  1.359E+00
!   1    1    1     1     2    5.5330E+04  1.8588E+03   5.3471E+04  0.000E+00  0.000E+00  0.000E+00  3.36397E+02  3.482E-01  3.801E+00  1.379E+03  1.348E+00
!   1    2    1     1     3    5.3471E+04  8.3889E+02   5.2632E+04  0.000E+00  0.000E+00  0.000E+00  3.36089E+02  3.453E-01  3.765E+00  6.236E+02  1.345E+00
!   1    2    2     1     4    5.2632E+04  4.8102E+03   4.7822E+04  0.000E+00  0.000E+00  0.000E+00  3.35422E+02  3.391E-01  3.683E+00  3.592E+03  1.339E+00
!   1    2    2     1     5    4.7822E+04  2.6537E+03   4.5168E+04  0.000E+00  0.000E+00  0.000E+00  3.34525E+02  3.319E-01  3.544E+00  1.992E+03  1.332E+00
!   1    2    2     1     6    4.5168E+04  4.8467E+02   4.4684E+04  0.000E+00  0.000E+00  0.000E+00  3.34135E+02  3.287E-01  3.484E+00  3.648E+02  1.329E+00
!   1    3    2     1     7    4.4684E+04  3.2190E+01   4.4651E+04  0.000E+00  0.000E+00  0.000E+00  3.34070E+02  3.282E-01  3.474E+00  2.424E+01  1.328E+00
!   1    3    3     1     8    4.4651E+04  2.0511E+03   4.2600E+04  0.000E+00  0.000E+00  0.000E+00  3.33803E+02  3.260E-01  3.434E+00  1.547E+03  1.326E+00
!   1    3    3     1     9    4.2600E+04  3.8740E+03   3.8726E+04  0.000E+00  0.000E+00  0.000E+00  3.33023E+02  3.197E-01  3.316E+00  2.936E+03  1.320E+00
!   1    3    4     1    10    3.8726E+04  3.0442E+02   3.8422E+04  0.000E+00  0.000E+00  0.000E+00  3.32465E+02  3.150E-01  3.230E+00  2.315E+02  1.315E+00
!   1    3    4     1    11    3.8422E+04  3.9377E+03   3.4484E+04  0.000E+00  0.000E+00  0.000E+00  3.31864E+02  3.100E-01  3.141E+00  3.006E+03  1.310E+00
!   1    3    4     2     1    9.1421E+03  7.9353E+02   8.3486E+03  0.000E+00  0.000E+00  0.000E+00  3.31015E+02  1.114E-01  4.000E+00  7.140E+02  1.111E+00
!   1    3    5     2     2    8.3486E+03  6.4520E+02   7.7034E+03  0.000E+00  0.000E+00  0.000E+00  3.30834E+02  1.058E-01  4.000E+00  5.835E+02  1.106E+00
!
  !## fhb package
  ...
  DO ISYS=1,NSYS
   ...
   SELECT CASE (ITOPIC)
    ...
    CASE (30) 
     ...
     GRIDISG%SDATE=UTL_JDATETOIDATE(GRIDISG%SDATE)
     GRIDISG%EDATE=UTL_JDATETOIDATE(GRIDISG%EDATE)-1  !<- edate is equal to sdate if one day is meant
     IF(.NOT.ISG2SFR(BND(1)%NROW,BND(1)%NCOL,NLAY,ILAY,TOP,BOT,IPER,NPER,MP,JU,GRIDISG))EXIT 
     CALL ISGCLOSEFILES()
...
! IF(PMANAGER_SAVEMF2005_HFB)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
   ...
   IF(ICOL.LT.NCOL)THEN
    ...
    DO ILAY=IL1,IL2
     ...
     IF(FCT.NE.0.0)THEN
      ...
      IF(IWRITE.EQ.1.AND.ICOL+1.LE.NCOL)WRITE(IU,'(5(I10,1X),G10.4)') ILAY,IROW,ICOL,IROW,ICOL+1,FCT !## x-direction
     ENDIF
    ENDDO
  ...
  IF(IPC(ICOL,IROW,2).EQ.INT(1,1))THEN
   IF(IROW.LT.NROW)THEN
    ...
    DO ILAY=IL1,IL2
     ...
     IF(FCT.NE.0.0)THEN
      NHFB(ILAY)=NHFB(ILAY)+1
      IF(IWRITE.EQ.1.AND.IROW+1.LT.NROW)WRITE(IU,'(5(I10,1X),G10.4)') ILAY,IROW,ICOL,IROW+1,ICOL,FCT !## y-direction
     ENDIF
    ENDDO
   ENDIF
  ENDIF
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_INITSIM(FNAME,IBATCH)
 !###======================================================================
 IMPLICIT NONE
...
JLOOP: DO K=1,SIZE(TOPICS)
  ...
  DO J=1,SIZE(TOPICS(K)%STRESS)
   ...
   DO I=1,SIZE(TOPICS(K)%STRESS(J)%FILES,1)
    ...
    DO II=1,SIZE(TOPICS(K)%STRESS(J)%FILES,2)
     IF(TOPICS(K)%STRESS(J)%FILES(I,II)%ICNST.EQ.0)THEN
 ...
 IF(IDF(1)%DX.GT.0.0)THEN
  CALL WDIALOGPUTREAL(IDF_REAL5,IDF(1)%DX,'(F10.2)') 
 ELSE
  CALL WDIALOGPUTREAL(IDF_REAL5,25.0,'(F10.2)') 
 ENDIF
 CALL IDFDEALLOCATE(IDF,SIZE(IDF)); DEALLOCATE(IDF)
...
! TOPICS(31)%SNAME(1)='Indication for Flow(1)/Head(2)'
