...
!  ENDDO
 ...
 DO
  ...
  !## pst module is exception
  IF(I.EQ.20)THEN
   ALLOCATE(PEST%PARAM(NPER)); TOPICS(I)%IACT_MODEL=IACT
   READ(IU,'(A)') LINE
   ...
   IF(PEST%PE_NPERIOD.GT.0)THEN
    ...
    DO J=1,PEST%PE_NPERIOD
     READ(IU,'(A256)') LINE
     READ(LINE,*) PEST%S_PERIOD(J),PEST%E_PERIOD(J) 
    ENDDO
   ENDIF
   IF(PEST%PE_NBATCH.GT.0)THEN
    DO J=1,PEST%PE_NBATCH
     READ(IU,'(A256)') LINE
     READ(LINE,*) PEST%B_FRACTION(J),PEST%B_BATCHFILE(J),PEST%B_OUTFILE(J) 
    ENDDO
   ENDIF
   DO J=1,SIZE(PEST%PARAM)
    READ(IU,'(A256)') LINE
    READ(LINE,*) PEST%PARAM(J)%PACT,PEST%PARAM(J)%PPARAM,PEST%PARAM(J)%PILS,PEST%PARAM(J)%PIZONE,PEST%PARAM(J)%PINI,PEST%PARAM(J)%PDELTA, &
                 PEST%PARAM(J)%PMIN,PEST%PARAM(J)%PMAX,PEST%PARAM(J)%PINCREASE,PEST%PARAM(J)%PIGROUP,PEST%PARAM(J)%PLOG
   ENDDO
   READ(IU,*) J
   ALLOCATE(PEST%IDFFILES(J))
   DO J=1,SIZE(PEST%IDFFILES)
    READ(IU,'(A256)') LINE
