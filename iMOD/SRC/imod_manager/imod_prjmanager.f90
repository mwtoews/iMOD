...
USE MOD_PMANAGER_UTL, ONLY : PMANAGER_SAVEMF2005_ALLOCATEPCK,PMANAGER_SAVEMF2005_DEALLOCATEPCK
...
REAL,PARAMETER,PRIVATE :: HNOFLOW=HUGE(1.0)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_OPENRUN,ID_SAVERUN)
     IF(PMANAGERRUN(MESSAGE%VALUE1,'',0,0))THEN; ENDIF
...
!  WRITE(IU,'(A)') TRIM(LINE)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGERRUN(ID,RUNFNAME,IBATCH,IDEBUG)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ID,IBATCH,IDEBUG
 CHARACTER(LEN=*),INTENT(IN) :: RUNFNAME
 ...
 ELSEIF(ID.EQ.ID_SAVERUN)THEN
  ...
  ELSEIF(IFORMAT.EQ.2)THEN
   IF(PMANAGER_SAVEMF2005(FNAME,IBATCH,IDEBUG))THEN
    IF(IBATCH.EQ.0)THEN
 ...
 IF(IMODE.EQ.1)THEN
  ...
  IF(LSFR)THEN
   ...
   WRITE(IU,'( A)')  'ECHO ISGFILE_IN= "'//TRIM(DIR)//'\MODELINPUT\SFR7\SFR_T1.ISG" >> SFRTOISG.INI'
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005(FNAME,IBATCH,IDEBUG) 
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: FNAME
 INTEGER,INTENT(IN) :: IBATCH,IDEBUG
 CHARACTER(LEN=512) :: DIRMNAME,DIR 
 INTEGER :: ILAY,ISS
 INTEGER :: IROW,ICOL,ISTEADY,NHED,NFLW,IFHBSS,NFHBX1,NFHBX2,NBDTIM,JD0,JD1,ISEC0,ISEC1
 INTEGER,DIMENSION(2) :: NP
 REAL :: CONST,DLEAK,DDAY,DSEC
 CHARACTER(LEN=52) :: TEXT,CAUX,CMAXNO1,CMAXNO2,CPCK    
 LOGICAL :: LTB
 ...
 IIDEBUG=IDEBUG     !## if 1 write idf files instead of arr
 ...
 IF(.NOT.PMANAGER_SAVEMF2005_SIM())RETURN
 ...
 IF(.NOT.PMANAGER_SAVEMF2005_DIS_READ(ISS,LTB))RETURN
 !## read bcf
 ...
 !## save lak file - adjust ibound
! IF(.NOT.PMANAGER_SAVEMF2005_LAK_SAVE(LAK,IULAK,1,IBATCH,DIR,DIRMNAME=DIRMNAME))RETURN
 !## save hfb file
...
!  !## lak unit already opened
!  IF(ITOPIC.EQ.32)THEN
!   IU=IULAK
!  ENDIF

 !## save wel package
 IF(.NOT.PMANAGER_SAVEMF2005_WEL(DIR,DIRMNAME,IBATCH))RETURN
 !## save drn package
...
! !## save evt package
! IF(.NOT.PMANAGER_SAVEMF2005_PCK(DIR,DIRMNAME,IBATCH,LRIV,24,IEVTCB,'EVT',(/2,1,3/)))RETURN
 !## save ghb package
...
! IF(.NOT.PMANAGER_SAVEMF2005_PCK(DIR,DIRMNAME,IBATCH,LRCH,26,IRCHCB,'RCH',(/1/)))RETURN
 ...
 !## save hfb package
 IF(.NOT.PMANAGER_SAVEMF2005_PCK(DIR,DIRMNAME,IBATCH,LSFR,31,IFHBCB,'FHB',(/0/)))RETURN
! !## save lak package
! IF(.NOT.PMANAGER_SAVEMF2005_PCK(DIR,DIRMNAME,IBATCH,LSFR,32,ISFRCB,'SFR'))RETURN
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_EXPORT(EXFNAME,ISYS)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: EXFNAME
 INTEGER,INTENT(IN) :: ISYS
 CHARACTER(LEN=256) :: SFNAME
 INTEGER :: ITOP,IU,IROW,ICOL,NTOP
 ...
 IF(IIDEBUG.EQ.1)THEN
  ...
  DO ITOP=1,NTOP
   ...
   IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=SFNAME,STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN

   WRITE(IU,'(A14,I10)')   'NCOLS'       ,PCK(ITOP)%NCOL
   WRITE(IU,'(A14,I10)')   'NROWS'       ,PCK(ITOP)%NROW
   WRITE(IU,'(A14,F15.7)') 'XLLCORNER'   ,PCK(ITOP)%XMIN
   WRITE(IU,'(A14,F15.7)') 'YLLCORNER'   ,PCK(ITOP)%YMIN
   WRITE(IU,'(A14,F15.7)') 'CELLSIZE'    ,PCK(ITOP)%DX
   WRITE(IU,'(A14,F15.7)') 'NODATA_VALUE',PCK(ITOP)%NODATA
   DO IROW=1,PCK(ITOP)%NROW; WRITE(IU,*) (PCK(ITOP)%X(ICOL,IROW),ICOL=1,PCK(ITOP)%NCOL); ENDDO
   CLOSE(IU); IU=0
  ENDDO

 ELSEIF(IIDEBUG.EQ.2)THEN 

  DO ITOP=1,NTOP
   SFNAME=UTL_SUBST(EXFNAME,'.IDF',TRIM(ITOS(ISYS))//'_'//TRIM(ITOS(ITOP))//'.IDF')
   IF(.NOT.IDFWRITE(PCK(ITOP),SFNAME,1))RETURN
  ENDDO

 ENDIF
   
 END SUBROUTINE PMANAGER_SAVEMF2005_EXPORT
...
!  ENDDO
 ...
 IF(LMODFLOW2005)THEN
  ...
  IF(LLAK)WRITE(IU,'(A,I3,A)') 'DATA(BINARY) ',ILKCB ,' '//CHAR(39)//TRIM(DIRMNAME)//'_FLAK.DAT'//CHAR(39) 
 ELSE
  ...
  IF(LLAK)WRITE(IU,'(A,I3,A)') 'DATA(BINARYIDF) ',ILKCB ,' '//CHAR(39)//'BDGLAK'//CHAR(39) 
 ENDIF
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_SIM()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: ILAY
 LOGICAL :: LEX
 ...
 ALLOCATE(KD (NLAY)); DO ILAY=1,SIZE(KD ); CALL IDFNULLIFY(KD (ILAY)); ENDDO
 IF(LLPF)THEN
 ...
 DO ILAY=1,SIZE(KD);  CALL IDFCOPY(IDF,KD(ILAY) ); ENDDO
 IF(LLPF)THEN
 ...
 ENDIF
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\BAS6\IBOUND_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
      BND(ILAY),1,IU))RETURN
 ENDDO
 WRITE(IU,'(A)') TRIM(RTOS(HNOFLOW,'E',7))
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\BAS6\STRT_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
      SHD(ILAY),0,IU))RETURN
 ENDDO
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_DIS_READ(ISS,LTB)
 !####====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(OUT) :: ISS
 LOGICAL,INTENT(OUT) :: LTB
 INTEGER :: ILAY,KPER,IINV,SCL_D,SCL_U,ITOPIC
 LOGICAL :: LEX
 ...
 DO ILAY=1,NLAY
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\DIS6\TOP_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
      TOP(ILAY),0,IU))RETURN
  !## quasi-3d scheme add bot aquifer modellayer
  IF(LQBD.OR.ILAY.EQ.NLAY)THEN
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\DIS6\BOTM_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
       BOT(ILAY),0,IU))RETURN
  ENDIF
 ENDDO
 ...
 DO ILAY=1,NLAY
  ...
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ(KD(ILAY),ITOPIC,1,1,ILAY,SCL_D,SCL_U,IINV))RETURN
 ...
 DO ILAY=1,NLAY
  ...
  IF(ISS.EQ.1)THEN
   ...
   !## sf1
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\BCF6\SF1_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
       IDF,0,IU))RETURN

  ENDIF   

  !## kdw
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\BCF6\TRAN_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
      KD(ILAY),0,IU))RETURN

  IF(ILAY.NE.NLAY)THEN

   !## vcont
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\BCF6\VCONT_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
       IDF,0,IU))RETURN
 ...
 DO ILAY=1,NLAY
  ...
  !## hk
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\LPF7\HK_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
      KHV(ILAY),0,IU))RETURN

  !## vka
  IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\LPF7\VKA_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
      KVA(ILAY),0,IU))RETURN

  !## transient simulation
  IF(ISS.EQ.1)THEN

   !## sf1 - specific storage
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\LPF7\SF1_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
       IDF,0,IU))RETURN

   !## sf2 - specific yield in case not confined
   IF(LAYCON(ILAY).NE.1)THEN
    IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\LPF7\SF2_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
        IDF,0,IU))RETURN
   ENDIF
  ...
  IF(LQBD.AND.ILAY.NE.NLAY)THEN
   ...
   !## kvv
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\LPF7\VKCB_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
       KVV(ILAY),0,IU))RETURN
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_WEL(DIR,DIRMNAME,IBATCH)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IBATCH
 CHARACTER(LEN=*),INTENT(IN) :: DIR,DIRMNAME
 ...
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,J,ISYS,NROWIPF,NCOLIPF,IEXT,IOS,N,KPER,IPER,NP,ITOPIC,NTOP,NSYS
 ...
 IF(IBATCH.EQ.0)CALL WINDOWOUTSTATUSBAR(4,'Writing '//TRIM(DIRMNAME)//'.WEL7...')
 IF(IBATCH.EQ.1)WRITE(*,'(/1X,A)') 'Writing '//TRIM(DIRMNAME)//'.WEL7...'

 IU=UTL_GETUNIT()
 CALL OSD_OPEN(IU,FILE=TRIM(DIRMNAME)//'.WEL7_',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
 IF(IU.EQ.0)RETURN

 LINE='NaN1,'//TRIM(ITOS(IWELCB))//',AUXILIARY SYSTEM NOPRINT'
 WRITE(IU,'(A)') TRIM(LINE)
 ...
 !## create subfolders
 CALL UTL_CREATEDIR(TRIM(DIR)//'\WEL7')
 ...
 DO IPER=1,NPER
  ...
  EXFNAME=TRIM(DIR)//'\WEL7\WEL_T'//TRIM(ITOS(IPER))//FEXT(IIDEBUG)
    ...
    IF(Q.NE.0.0)THEN
     DO ILAY=1,NLAY
      IF(TLP(ILAY).GT.0.0)THEN
       ...
       !## store for export purposes
!       PCK()%X(ICOL,IROW)=Q*TLP(ILAY)
       NP=NP+1
      ENDIF
     ENDDO
    ENDIF
   ...
   !## export to asc-files - if needed
   CALL PMANAGER_SAVEMF2005_EXPORT(EXFNAME,ISYS)
 ...
 CALL PMANAGER_SAVEMF2005_MAXNO(TRIM(DIRMNAME)//'.WEL7_',(/NP/))
 ...
 CHARACTER(LEN=5) :: EXT
 CHARACTER(LEN=30) :: FRM
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,J,ISYS,IOS,N,KPER,IPER,NTOP,NSYS
 INTEGER,DIMENSION(2) :: NP
 INTEGER(KIND=8) :: ITIME,JTIME
 ...
 SELECT CASE (ITOPIC)
  ...
  CASE (31)
   LINE='NaN1,NaN2,0,0,'//TRIM(RTOS(CONST,'G',7))//','//TRIM(RTOS(DLEAK,'E',4))//','// &
 ...
 DO IPER=1,NPER
  ...
  EXFNAME=TRIM(DIR)//'\'//CPCK//'\ISG_T'//TRIM(ITOS(IPER))//FEXT(IIDEBUG)
  JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=EXFNAME,STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(JU.EQ.0)RETURN
...
!   CALL PMANAGER_SAVEMF2005_EXPORT(EXFNAME,ISYS)
 ...
 REAL :: X,Y,Q,Z1,Z2,FCT,IMP,CNST,OLFCOND
 CHARACTER(LEN=256) :: SFNAME,EXFNAME,CDIR,CAUX
 CHARACTER(LEN=1) :: TEXT
 CHARACTER(LEN=3) :: CPCK
 CHARACTER(LEN=30) :: FRM,CMAXNO1
 CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:) :: STRING
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,J,KTOP,ISYS,NROWIPF,NCOLIPF,IEXT,IOS,KPER,IPER,NP,NTOP,NSYS,SCL_D,SCL_U,ICNST, &
    NBDTIM,NHED,NFLW
 REAL,ALLOCATABLE,DIMENSION(:) :: TLP,KH,TP,BT
 INTEGER(KIND=8) :: ITIME,JTIME
 INTEGER,PARAMETER :: ICLAY=1 !## shift to nearest aquifer
  ...
  CASE(31)
   ...
   DO ILAY=1,NLAY
    DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
     IF(BND(ILAY)%X(ICOL,IROW).EQ.-2)NHED=NHED+1
     IF(BND(ILAY)%X(ICOL,IROW).EQ. 2)NFLW=NFLW+1
    ENDDO; ENDDO
   ENDDO
...
!     NHED=0; NFLW=0
  ...
  EXFNAME=TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_T'//TRIM(ITOS(IPER))//FEXT(IIDEBUG)
  JU=UTL_GETUNIT(); CALL OSD_OPEN(JU,FILE=EXFNAME,STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(JU.EQ.0)RETURN
...
!     IS1=0
   ...
   CASE (21:23,25,27,28) !:32) 
    CALL PMANAGER_SAVEMF2005_ALLOCATEPCK(NTOP) !; IS1=1; INEW=1
  END SELECT
   ...
   DO IROW=1,PCK(1)%NROW; DO ICOL=1,PCK(1)%NCOL
    ...
    DO ILAY=1,NLAY
     ...
     SELECT CASE (ITOPIC)
      ...
      !## fhb
      CASE (31)
       IF(BND(ILAY)%X(ICOL,IROW).EQ.-2)THEN; NHED=NHED+1; FHBHED(NHED,NBDTIM)=PCK(KTOP)%X(ICOL,IROW); ENDIF
       IF(BND(ILAY)%X(ICOL,IROW).EQ. 2)THEN; NFLW=NFLW+1; FHBFLW(NFLW,NBDTIM)=PCK(KTOP)%X(ICOL,IROW); ENDIF      
      CASE DEFAULT
...
!   DEALLOCATE(IEQUAL)
 ...
 CALL PMANAGER_SAVEMF2005_MAXNO(TRIM(DIRMNAME)//'.'//CPCK//'7',(/NP/))
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_LAK_READ()
 !####====================================================================
 IMPLICIT NONE
 INTEGER :: I,ITOPIC,SCL_D,SCL_U,IROW,ICOL,KPER
 ...
 !KPER=PMANAGER_GETCURRENTIPER(1,ITOPIC,ITIME,JTIME)
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_LAK_SAVE(IULAK,IINI,IBATCH,DIR,DIRMNAME)
 !####====================================================================
 IMPLICIT NONE
 ...
 INTEGER :: IU,NSSITR,I,J,IOP,ILAY
 ...
 IF(IINI.EQ.1)THEN
  ...
  THETA=0.0; SSCNCR=0.001; NSSITR=100

  !## read lake package (also adjust ibound for lakes)
  IULAK=UTL_GETUNIT(); CALL OSD_OPEN(IULAK,FILE=TRIM(DIRMNAME)//'.LAK7',STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED'); IF(IU.EQ.0)RETURN

  !## set number of lakes
  LINE=TRIM(ITOS(NLAKES))//','//TRIM(ITOS(ILKCB))
  ...
  DO ILAY=1,NLAY
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\LAK6\LKARR_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
       LBD(ILAY),1,IULAK))RETURN 
  ENDDO
  !## get lakebed leakance
  DO ILAY=1,NLAY
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_READ_U2DREL(TRIM(DIR)//'\LAK6\BDLKNC_L'//TRIM(ITOS(ILAY))//FEXT(IIDEBUG), &
       LCD(ILAY),0,IULAK))RETURN
  ENDDO
  !## no connected lakes
  LINE=TRIM(ITOS(0))
  WRITE(IULAK,'(A)') TRIM(LINE) 
...
!save_no_buf 
 ...
 ENDDO
 ITOPIC=15; IF(.NOT.PMANAGER_SAVEMF2005_HFB_COMPUTE(IDF,ITOPIC,IU,IUGEN,TOP,BOT,BND,KD))RETURN
 DEALLOCATE(IUGEN)
...
! IF(PMANAGER_SAVEMF2005_HFB_COMPUTE)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_MOD_READ_U2DREL(EXFNAME,IDF,IINT,IU)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: EXFNAME
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 ...
 PMANAGER_SAVEMF2005_MOD_READ_U2DREL=.FALSE.
 ...
 PMANAGER_SAVEMF2005_MOD_READ_U2DREL=.TRUE.
   
 END FUNCTION PMANAGER_SAVEMF2005_MOD_READ_U2DREL
...
! ENDDO
 ...
 ELSEIF(ID.EQ.ID_SAVE)THEN
  ...
  DO I=1,SIZE(SIM)
   WRITE(LINE,'(I4.4,5I2.2,A1,I2)') SIM(I)%IYR,SIM(I)%IMH,SIM(I)%IDY,SIM(I)%IHR,SIM(I)%IMT,SIM(I)%ISC,',',SIM(I)%ISAVE
   WRITE(IU,'(A)') TRIM(LINE)
  ENDDO   
  CLOSE(IU)
