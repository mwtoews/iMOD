...
!  WRITE(IU,'(A)') TRIM(LINE)
 ...
 INTEGER :: ISS,IULAK,ISTEADY,IPER
 ...
 !## read top/bot information
 IF(.NOT.PMANAGER_SAVEMF2005_LAK_READ(1))RETURN
 ...
 IF(.NOT.PMANAGER_SAVEMF2005_LAK_SAVE(IULAK,1,IBATCH,DIR,DIRMNAME=DIRMNAME))RETURN
 !## save hfb file
 ...
 DO IPER=2,NPER
  IF(.NOT.PMANAGER_SAVEMF2005_LAK_READ(IPER))RETURN
  IF(.NOT.PMANAGER_SAVEMF2005_LAK_SAVE(IULAK,0,IBATCH,DIR,DIRMNAME=DIRMNAME))RETURN
 ENDDO
...
!  ENDDO
 ...
 CHARACTER(LEN=30) :: FRM
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,ISYS,IOS,KPER,IPER,NTOP,NSYS
 INTEGER,DIMENSION(2) :: NP
 INTEGER(KIND=8) :: ITIME,JTIME
 ...
 CHARACTER(LEN=30) :: FRM
 CHARACTER(LEN=52),ALLOCATABLE,DIMENSION(:) :: STRING
 INTEGER :: IU,JU,KU,ILAY,IROW,ICOL,I,J,KTOP,ISYS,NROWIPF,NCOLIPF,IEXT,KPER,IPER,NP,NTOP,NSYS,SCL_D,SCL_U,ICNST, &
    NBDTIM,NHED,NFLW
 REAL,ALLOCATABLE,DIMENSION(:) :: TLP,KH,TP,BT
 INTEGER(KIND=8) :: ITIME,JTIME
 INTEGER,PARAMETER :: ICLAY=1 !## shift to nearest aquifer
 ...
 DO IPER=1,NPER
  ...
  DO ISYS=1,NSYS 
   ...
   DO KTOP=1,NTOP 
    ...
    SELECT CASE (ITOPIC); CASE (24,26); FCT=FCT*0.001; END SELECT
 ...
 !####====================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_LAK_READ(IPER)
 !####====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IPER
 ...
 IF(IINI.EQ.1)THEN
  ...
  DO ILAY=1,NLAY
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\LAK6\LKARR_L'//TRIM(ITOS(ILAY))//'.ARR', &
       LBD(ILAY),1,IULAK,ILAY))RETURN 
  ENDDO
  !## get lakebed leakance
  DO ILAY=1,NLAY
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\LAK6\BDLKNC_L'//TRIM(ITOS(ILAY))//'.ARR', &
       LCD(ILAY),0,IULAK,ILAY))RETURN
  ENDDO
  !## no connected lakes
  LINE=TRIM(ITOS(0))
  WRITE(IULAK,'(A)') TRIM(LINE) 
...
! END FUNCTION PMANAGER_SAVEMF2005_PCK_ULSTRD
 ...
 IF(.NOT.PMANAGER_SAVEMF2005_PCK_GETMINMAX(IDF%X,IDF%NCOL,IDF%NROW,BND(1)%X,MINV,MAXV))RETURN
...
! IF(PMANAGER_SAVEMF2005_HFB_COMPUTE)CALL PMANAGER_SAVEMF2005_FCTIMP(IINV,ICNST,IDF,HNOFLOW,FCT,IMP)
 ...
 !###======================================================================
 LOGICAL FUNCTION PMANAGER_SAVEMF2005_MOD_U2DREL(EXFNAME,IDF,IINT,IU,ILAY)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: EXFNAME
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 CHARACTER(LEN=256) :: SFNAME
 INTEGER,INTENT(IN) :: IINT,IU,ILAY
 INTEGER :: JU,IROW,ICOL,I
 REAL :: MINV,MAXV
 
 PMANAGER_SAVEMF2005_MOD_U2DREL=.FALSE.

 IF(.NOT.PMANAGER_SAVEMF2005_PCK_GETMINMAX(IDF%X,IDF%NCOL,IDF%NROW,BND(ILAY)%X,MINV,MAXV))RETURN
