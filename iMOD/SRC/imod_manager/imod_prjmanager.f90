...
!  WRITE(IU,'(A)') TRIM(LINE)
 ...
 IF(PEST%PE_KTYPE.LT.0)THEN
  ...
  ELSEIF(IOPTION.EQ.2)THEN
   ...
   IF(.NOT.IDFREADSCALE(PEST%PPBNDIDF,IDF,SCL_UP,SCL_D,1.0,0))RETURN
   !## save array, do not correct for boundary condition as we not yet know for what layer the zone will apply
   IF(.NOT.PMANAGER_SAVEMF2005_MOD_U2DREL(TRIM(DIR)//'\PST1\PPBNDIDF.ARR',IDF,0,IU,1,0))RETURN
  ENDIF
 ENDIF
...
! !####====================================================================
! LOGICAL FUNCTION PMANAGER_SAVEMF2005_BAS_BOUNDARY()
! !####====================================================================
! IMPLICIT NONE
! LOGICAL :: LW,LE,LN,LS
! INTEGER :: ILAY,IROW,ICOL,NW,NE,NN,NS
! 
! IF(TRIM(PBMAN%SUBMDL%FNAME).EQ.'')RETURN
! 
! !## adjust boundary as a submodel is concerned 
! LW=.FALSE.; IF(PBMAN%SUBMDL%XMIN.LT.IDF%XMIN)LW=.TRUE.
! LE=.FALSE.; IF(PBMAN%SUBMDL%XMAX.GT.IDF%XMAX)LE=.TRUE.
! LN=.FALSE.; IF(PBMAN%SUBMDL%YMAX.GT.IDF%YMAX)LN=.TRUE.
! LS=.FALSE.; IF(PBMAN%SUBMDL%YMIN.LT.IDF%YMIN)LS=.TRUE.
!
! NW=0; IF(LW)THEN 
!  ICOL=1
!  DO ILAY=1,NLAY; DO IROW=1,IDF%NROW
!   IF(BND(ILAY)%X(ICOL,IROW).GT.0)THEN
!    BND(ILAY)%X(ICOL,IROW)=-1.0; NW=NW+1
!   ENDIF
!  ENDDO; ENDDO
! ENDIF
!
...
!  DO ILAY=1,NLAY; DO IROW=1,IDF%NROW
!   IF(BND(ILAY)%X(ICOL,IROW).GT.0)THEN
!    BND(ILAY)%X(ICOL,IROW)=-1.0; NE=NE+1
!   ENDIF
!  ENDDO; ENDDO
! ENDIF
!
! NN=0; IF(LN)THEN 
!  IROW=1
!  DO ILAY=1,NLAY; DO ICOL=1,IDF%NCOL
!   IF(BND(ILAY)%X(ICOL,IROW).GT.0)THEN
!    BND(ILAY)%X(ICOL,IROW)=-1.0; NN=NN+1
!   ENDIF
!  ENDDO; ENDDO
! ENDIF
!
...
!  DO ILAY=1,NLAY; DO ICOL=1,IDF%NCOL
!   IF(BND(ILAY)%X(ICOL,IROW).GT.0)THEN
!    BND(ILAY)%X(ICOL,IROW)=-1.0; NS=NS+1
!   ENDIF
!  ENDDO; ENDDO
! ENDIF
!
! WRITE(*,'(/A)') 'Modified boundary due to submodelling:'
...
! 
! END FUNCTION PMANAGER_SAVEMF2005_BAS_BOUNDARY
! 
 !####====================================================================
  ...
  DO ISYS=1,NSYS
   ...
   DO I=1,NROWIPF
    ...
    IF(ILAY.EQ.0)THEN
     ...
     CALL PMANAGER_SAVEMF2005_PCK_ULSTRD_PARAM(NLAY,ICOL,IROW,BND,TOP,BOT,KDW,TP,BT,KH,.TRUE.)
...
!         'Select one of the following:'//CHAR(13)//'NONE, THIEM, SKIN, GENERAL, SPECIFYCWC','Error'); IOS=-1; EXIT
 ...
 CHARACTER(LEN=30) :: FRM
 INTEGER :: IU,JU,ILAY,I,ISYS,KPER,IPER,NTOP,NSYS,ICNST,ICOL,IROW
    ...
    CASE DEFAULT
     ...
     DO IROW=1,PCK(1)%NROW; DO ICOL=1,PCK(1)%NCOL
      ...
      IF(PCK(1)%ILAY.EQ.0)THEN
       ...
       !## get filter fractions
       CALL PMANAGER_SAVEMF2005_PCK_ULSTRD_PARAM(NLAY,ICOL,IROW,BND,TOP,BOT,KDW,TP,BT,KH,.FALSE.)
       SELECT CASE (ITOPIC)
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 !###======================================================================
 SUBROUTINE PMANAGER_SAVEMF2005_PCK_ULSTRD_PARAM(NLAY,ICOL,IROW,BND,TOP,BOT,KD,TP,BT,KH,LKHV)
 !###======================================================================
 IMPLICIT NONE
...
!tib = 2
 ...
 DO IROW=1,BND%NROW
  IF(IFULL(1).EQ.1)THEN; ICOL=1;        IF(BND%X(ICOL,IROW).GT.0)THEN; NW=NW+1; BND%X(ICOL,IROW)=-1; ENDIF; ENDIF
  IF(IFULL(3).EQ.1)THEN; ICOL=BND%NCOL; IF(BND%X(ICOL,IROW).GT.0)THEN; NE=NE+1; BND%X(ICOL,IROW)=-1; ENDIF; ENDIF
 ENDDO
 DO ICOL=1,BND%NCOL
  IF(IFULL(4).EQ.1)THEN; IROW=1;        IF(BND%X(ICOL,IROW).GT.0)THEN; NN=NN+1; BND%X(ICOL,IROW)=-1; ENDIF; ENDIF
  IF(IFULL(2).EQ.1)THEN; IROW=BND%NROW; IF(BND%X(ICOL,IROW).GT.0)THEN; NS=NS+1; BND%X(ICOL,IROW)=-1; ENDIF; ENDIF
 ENDDO
