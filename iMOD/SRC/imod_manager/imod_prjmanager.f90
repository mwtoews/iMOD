...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,ITYPE,ITOPIC,IPER,ISYS,ISUBTOPIC,ID,NF,ICNST,ILAY,IOS,IYR,IMH,IDY,IHR,IMT,ISC,ISEL
 REAL :: CNST
 TYPE(WIN_MESSAGE) :: MESSAGE
 CHARACTER(LEN=256) :: CNAME
...
!## IFVDL SFT RCNC
 ...
 INTEGER(KIND=8) :: ITIME,JTIME
 INTEGER,PARAMETER :: ICLAY=1 !## shift to nearest aquifer
 INTEGER :: JD0,JD1,ISEC0,ISEC1,NUZGAG,IRUNFLG,IEQUAL
 REAL :: DDAY,DSEC
  ...
  IF(KPER.LE.0)THEN
   SELECT CASE (ITOPIC)
    ...
    CASE (18)
     IF(IPER.EQ.1)THEN
      !cannot
!      DO I=1,4; WRITE(IU,'(A)') '-1,-1,-1';  ENDDO
     ELSE
...
!           'FHB','LAK','PCG'/                                             !31-40
  ...
  DO ISYS=1,NSYS
   ...
   SELECT CASE (ITOPIC)
    ...
    CASE (18)
      IF(IPER.EQ.1)THEN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_UZBND_T'//TRIM(ITOS(IPER))//'.ARR',PCK(1),IU,IFBND))RETURN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_EPS_T'//TRIM(ITOS(IPER))//  '.ARR',PCK(2),IU,IFBND))RETURN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_THTS_T'//TRIM(ITOS(IPER))// '.ARR',PCK(3),IU,IFBND))RETURN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_THTI_T'//TRIM(ITOS(IPER))// '.ARR',PCK(4),IU,IFBND))RETURN
      ENDIF
      LINE=TRIM(ITOS(NUZF1)); WRITE(IU,'(A)') TRIM(LINE)
      IF(NUZF1.EQ.1)THEN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_FINF_T'//TRIM(ITOS(IPER))// '.ARR',PCK(5),IU,IFBND))RETURN
      ENDIF
      LINE=TRIM(ITOS(NUZF2)); WRITE(IU,'(A)') TRIM(LINE)
      IF(NUZF2.EQ.1)THEN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_PET_T'//TRIM(ITOS(IPER))//  '.ARR',PCK(6),IU,IFBND))RETURN
      ENDIF
      LINE=TRIM(ITOS(NUZF3)); WRITE(IU,'(A)') TRIM(LINE)
      IF(NUZF3.EQ.1)THEN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_EXDP_T'//TRIM(ITOS(IPER))// '.ARR',PCK(7),IU,IFBND))RETURN
      ENDIF
      LINE=TRIM(ITOS(NUZF4)); WRITE(IU,'(A)') TRIM(LINE)
      IF(NUZF4.EQ.1)THEN
       IF(.NOT.PMANAGER_SAVEMF2005_PCK_U2DREL(TRIM(DIR)//'\'//CPCK//'7\'//CPCK//'_EXTWC_T'//TRIM(ITOS(IPER))//'.ARR',PCK(8),IU,IFBND))RETURN
      ENDIF
    !## rch
    CASE (26)
...
!  MXNLAY=MIN(MXNLAY,NLAY)
 ...
 INTEGER :: ITYPE
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: IDY,IYR,IMH,ITOPIC,IPER,I,J,K,II,MINJD,MAXJD,IDATE,IHR,IMT,ISC,IHMS,MINHMS,MAXHMS,ISTEADY,ISTEP,ITRANSIENT
 TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: IDF
 LOGICAL :: LEX
