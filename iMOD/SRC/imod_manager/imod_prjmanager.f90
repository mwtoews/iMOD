...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 DO KPER=1,NPER
  ...
  DO I=1,MAXTOPICS  
   ...
   IF(SIM(KPER)%DELT.EQ.0.0)THEN !## steady-state
    IPER=PMANAGER_GETIPER(INT(0,8),INT(0,8),TOPICS(I)%STRESS)   
   ELSE
    ...
    ITIME=JTIME
    IF(KPER.GT.1)THEN
     IF(SIM(KPER-1)%DELT.NE.0.0)THEN !## previous timestep steady-state
...
! CALL WDIALOGPUTREAL(IDF_REAL4,5808000.0,'(F15.7)') 
 ...
 IF(ID.EQ.ID_OPEN)THEN
  ...
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=FNAME,STATUS='OLD',ACTION='READ,DENYWRITE',FORM='FORMATTED')
  IF(IU.EQ.0)RETURN
  IF(ASSOCIATED(SIM))DEALLOCATE(SIM); ALLOCATE(SIM(100))
  NPER=1; N=SIZE(SIM); DO
   READ(IU,*,IOSTAT=IOS) SIM(NPER)%CDATE,SIM(NPER)%DELT,SIM(NPER)%ISAVE
   IF(IOS.NE.0)EXIT
   IF(NPER.GE.N)THEN
    ALLOCATE(SIM_C(N+100))
    DO I=1,N; SIM_C(I)=SIM(I); ENDDO
 ...
 IF(ISS.EQ.2)THEN
  ...
  !## get periods 1=Hourly,2=Daily,3=Weekly,4=Decade,5=14/28,6=Monthly,7=Yearly,8=Package
  SELECT CASE (IPERIOD)
   CASE (1) !## hourly
  ...
  CALL UTL_GDATE(JD2,SIM(1)%IYR,SIM(1)%IMH,SIM(1)%IDY)
  CALL ITIMETOHMS(IHMS2,SIM(1)%IHR,SIM(1)%IMT,SIM(1)%ISC)

  !## fill in start date on position(1)
  CALL UTL_GDATE(JD1,SIM(2)%IYR,SIM(2)%IMH,SIM(2)%IDY)
  CALL ITIMETOHMS(IHMS1,SIM(2)%IHR,SIM(2)%IMT,SIM(2)%ISC)

  !## fill in intermediate timesteps
  SELECT CASE (IPERIOD)
   CASE (1) !## hourly
    I=2; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,1, 1,4))EXIT; ENDDO; NPER=I
   CASE (2) !## daily
    I=2; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,1, 1,3))EXIT; ENDDO; NPER=I
   CASE (3) !## weekly
    I=2; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,1, 7,3))EXIT; ENDDO; NPER=I
   CASE (4) !## decade
    I=2; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,1,10,3))EXIT; ENDDO; NPER=I
   CASE (5) !## 14/28
    I=2; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,1,10,7))EXIT; ENDDO; NPER=I
   CASE (6) !## monthly
    I=2; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,1, 1,2))EXIT; ENDDO; NPER=I
   CASE (7) !## yearly
    I=2; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,1, 1,1))EXIT; ENDDO; NPER=I
   CASE (8) !## packages
  ...
  IF(ISTEADY.EQ.0)THEN; DO I=1,NPER; SIM(I)=SIM(I+1); ENDDO; NPER=NPER-1; ENDIF
