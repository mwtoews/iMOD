...
USE MOD_IDF, ONLY : IDFREAD,IDFDEALLOCATE,IDFNULLIFY,IDFREADSCALE,IDFCOPY,IDFDEALLOCATEX,IDFIROWICOL,IDFALLOCATEX,IDFGETAREA, &
    IDFFILLSXSY,IDFWRITE,IDFGETEDGE,IDFGETILAY
...
!  PRJ%INHERENT=0
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,ITYPE,ITOPIC,IPER,ISYS,ISUBTOPIC,ID,NF,ICNST,ILAY,IOS,IYR,IMH,IDY,IHR,IMT,ISC
 REAL :: CNST
 TYPE(WIN_MESSAGE) :: MESSAGE
 CHARACTER(LEN=256) :: CNAME
 ...
 IF(TOPICS(ITOPIC)%TIMDEP)THEN
  CALL WDIALOGPUTSTRING(IDF_RADIO1,'iMOD will look for unique TIME STEPS (>0) at the wildcard and add those files to your Project Manager')
 ...
 IF(MESSAGE%VALUE1.EQ.IDOK)THEN
  ...
  DO I=1,SIZE(FILES,1)
   ...
   DO ISUBTOPIC=1,TOPICS(ITOPIC)%NSUBTOPICS
    ...
    ICNST=2; IF(IOS.EQ.0)THEN; ICNST=1; FILES(I,ISUBTOPIC)=''; ENDIF   

!    ILAY=I
    
    TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%IACT =1     !PRJ(ISUBTOPIC)%IACT
    TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%FNAME=FILES(I,ISUBTOPIC) !PRJ(ISUBTOPIC)%FNAME
    TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%FCT  =1.0   !PRJ(ISUBTOPIC)%FCT
    TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%IMP  =0.0   !PRJ(ISUBTOPIC)%IMP
    TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%ICNST=ICNST !PRJ(ISUBTOPIC)%ICNST
    TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%CNST =CNST  !PRJ(ISUBTOPIC)%CNST
    TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%ILAY =ILAY  !PRJ(ISUBTOPIC)%ILAY
    IF(ICNST.EQ.2)THEN
     TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%ALIAS= &
        UTL_CAP(TRIM(TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%FNAME &
       (INDEX(TOPICS(ITOPIC)%STRESS(IPER)%FILES(ISUBTOPIC,ISYS)%FNAME,'\',.TRUE.)+1:)),'L')
 ...
 DO I=1,N; CALL WGRIDLABELROW(IDF_GRID1,I,'Modellayer '//TRIM(ITOS(I))); ENDDO
 DO I=1,M
  DO J=1,N
   ...
   ELSE
    ...
    IF(INDEX(PNAME(I),'*').GT.0)THEN
     ...
     DO K=1,M
      ...
      ELSE
       IF(UTL_IDFGETDATE(LISTNAME(K),IYR=IYR,IMH=IMH,IDY=IDY,IHR=IHR,IMT=IMT,ISC=ISC))THEN
 ...
 IF(TOPICS(ITOPIC)%TIMDEP)THEN
  DO I=1,SUM(NF)
   ...
   !## find min-value
   K=0; DO J=1,N
    IF(UTL_IDFGETDATE(FILES(PF(J),J),IYR=IYR,IMH=IMH,IDY=IDY,IHR=IHR,IMT=IMT,ISC=ISC))THEN
     IT=IYR*10000000000+IMH*100000000+IDY*1000000+IHR*10000+IMT*100+ISC
 ...
 DO
  ...
  DO
   ...
   IF(TRIM(LINE).NE.'')THEN
    ...
    IF(IOS.NE.0)THEN
     IACT=1; READ(LINE,*,IOSTAT=IOS) NPER,CTOPIC
     IF(IOS.EQ.0)THEN
      IF(NPER.LE.0)CYCLE
...
!     SIMGRO(ICOL,IROW)%BODEM=22
 ...
 REAL :: FCT,IMP,CNST
 INTEGER :: ILAY,ISYS,IWRITE,MXFB,NPHFB,ICNST
