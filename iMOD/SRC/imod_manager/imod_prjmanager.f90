...
! CALL WDIALOGPUTREAL(IDF_REAL4,5808000.0,'(F15.7)') 
 ...
 INTEGER :: ITYPE
 TYPE(WIN_MESSAGE) :: MESSAGE
 INTEGER :: DID,I,ICOL,IROW,IROW1,IROW2
...
!     IF(MESSAGE%VALUE3.EQ.0)THEN
!      CALL WGRIDPOS(MESSAGE%X,ICOL1,IROW1) !## from
      CALL WGRIDPOS(MESSAGE%Y,ICOL,IROW2) !## to
      IF(IROW1.NE.IROW2)THEN
       IF(IROW1.EQ.0)IROW1=IROW2
       CALL WGRIDCOLOURCELL(IDF_GRID1,1,IROW1,-1,WRGB(0,155,0))
       DO IROW=IROW1+1,IROW2
        CALL WGRIDCOLOURCELL(IDF_GRID1,1,IROW,-1,WRGB(155,0,0))
       ENDDO
      ELSEIF(IROW1.EQ.IROW2)THEN
       IROW1=0; IROW2=0
      ENDIF
      CALL WDIALOGPUTSTRING(ID_APPLY,'Change times for row '//TRIM(ITOS(MIN(IROW1,IROW2)))//' up to row '//TRIM(ITOS(MAX(IROW1,IROW2))))

!      CALL WGRIDSETSELECTION(IDF_GRID1,ICOL1,ICOL2,IROW1,IROW2)
!     IF(MESSAGE%VALUE3.EQ.2)THEN
!      CALL WGRIDPOS(MESSAGE%X,ICOL,IROW1)
!      CALL WGRIDPOS(MESSAGE%Y,ICOL,IROW2)
!      CALL WDIALOGPUTSTRING(ID_APPLY,'Change times for row '//TRIM(ITOS(IROW1))//' up to row '//TRIM(ITOS(IROW2)))
!      CALL WDIALOGFIELDSTATE(ID_APPLY,1)
!     ELSE
!      CALL WDIALOGPUTSTRING(ID_APPLY,'No selection')
!      CALL WDIALOGFIELDSTATE(ID_APPLY,0)
!     ENDIF
    ENDIF
   
!IROW1 = WInfoGrid(IDF_GRID1,GridSelRow1)
!IROW2 = WInfoGrid(IDF_GRID1,GridSelRow2)
 ...
 IR1=MAX(1,IROW1-1); IR2=MIN(SIZE(SIM),IROW2+1)
