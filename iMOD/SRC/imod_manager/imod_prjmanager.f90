...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 DO KPER=1,NPER
  ...
  DO I=1,MAXTOPICS  
   ...
   IF(KPER.EQ.1)THEN
    ITIME=SIM(KPER)%IYR*10000000000+SIM(KPER)%IMT*100000000+SIM(KPER)%IDY*1000000+ &
          SIM(KPER)%IHR*10000      +SIM(KPER)%IMT*100      +SIM(KPER)%ISC
    JTIME=ITIME
    IPER=PMANAGER_GETIPER(ITIME,JTIME,TOPICS(I)%STRESS) !,SIM(KPER)%CDATE  ,SIM(KPER)%CDATE,TOPICS(I)%STRESS)
   ELSE
    ITIME=SIM(KPER-1)%IYR*10000000000+SIM(KPER-1)%IMT*100000000+SIM(KPER-1)%IDY*1000000+ &
          SIM(KPER-1)%IHR*10000      +SIM(KPER-1)%IMT*100      +SIM(KPER-1)%ISC
    JTIME=SIM(KPER)%IYR*10000000000  +SIM(KPER)%IMT*100000000  +SIM(KPER)%IDY*1000000+ &
          SIM(KPER)%IHR*10000        +SIM(KPER)%IMT*100        +SIM(KPER)%ISC
    IPER=PMANAGER_GETIPER(ITIME,JTIME,TOPICS(I)%STRESS) !SIM(KPER-1)%CDATE,SIM(KPER)%CDATE,TOPICS(I)%STRESS)
   ENDIF
...
!## cannot be here
 ...
 INTEGER :: I,II,J,K,IOS,IDATE,IYR,IMH,IDY,IHR,IMT,ISC,JDP1,JDP2,IPER
 INTEGER(KIND=8),POINTER,DIMENSION(:) :: ITIME,JTIME
...
!    ID=UTL_IDATETOJDATE(IDATE); ID=ID-JD1+1; IF(ID.GT.0.AND.ID.LE.NPER)JLIST(ID)=1
 ...
 !###======================================================================
 INTEGER FUNCTION PMANAGER_GETIPER(ITIME,JTIME,STRESS) !CDATE1,CDATE2,STRESS)
 !###======================================================================
 IMPLICIT NONE
 INTEGER(KIND=8),INTENT(IN) :: ITIME,JTIME
! CHARACTER(LEN=*),INTENT(IN) :: CDATE1,CDATE2
 TYPE(STRESSOBJ),INTENT(IN),DIMENSION(:) :: STRESS
 INTEGER :: I,J,K,IDATE,JDI,ID,IOS,JDP1,JDP2
 INTEGER(KIND=8) :: KTIME,MD
...
!  ITIME=INT(0,8); READ(CDATE2,'(I14)',IOSTAT=IOS) ITIME
!  JTIME=ITIME   ; READ(CDATE1,'(I14)',IOSTAT=IOS) JTIME
...
!    ENDDO
   ELSE
    KTIME=STRESS(I)%IYR*10000000000+STRESS(I)%IMT*100000000+STRESS(I)%IDY*1000000+STRESS(I)%IHR*10000+STRESS(I)%IMT*100+STRESS(I)%ISC
...
!    JDI=UTL_IDATETOJDATE(IDATE)
   ...
   IF(ITIME-KTIME.LE.MD.AND.ITIME-KTIME.GE.0)THEN
    MD=ITIME-KTIME
    IF(KTIME.GT.JTIME.OR.JTIME.EQ.ITIME)THEN
     ID= I
    ELSE
     ID=-I
    ENDIF
 ...
 CALL WDIALOGPUTMENU(IDF_MENU4,(/'Hourly  ','Daily   ','Weekly  ','Decade  ', &
                                 '14/28   ','Monthly ','Yearly  ','Packages','Custom  '/),9,2)
 CALL WDIALOGPUTMENU(IDF_MENU5,(/'Hourly  ','Daily   ','Weekly  ','Decade  ', &
                                 '14/28   ','Monthly ','Yearly  ','Packages','Inherit','Custom  '/),10,9)
...
! CALL WDIALOGPUTREAL(IDF_REAL1,497300.0,'(F15.7)') 
! CALL WDIALOGPUTREAL(IDF_REAL2,5734800.0,'(F15.7)') 
! CALL WDIALOGPUTREAL(IDF_REAL3,607700.0,'(F15.7)') 
! CALL WDIALOGPUTREAL(IDF_REAL4,5872800.0,'(F15.7)') 
...
! CALL WDIALOGGETCHECKBOX(IDF_CHECK2,ISTEADY)
 ...
 IF(NPER.GT.WINFOGRID(IDF_GRID1,GRIDROWSMAX))THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'There is a maximum of '// &
     TRIM(ITOS(WINFOGRID(IDF_GRID1,GRIDROWSMAX)))//' timesteps in this iMOD version','Error')
  CALL WDIALOGUNLOAD(); CALL WDIALOGSELECT(DID); RETURN
 ENDIF
 
 CALL WGRIDROWS(IDF_GRID1,NPER+1)
 DO I=1,NPER+1; CALL WGRIDLABELROW(IDF_GRID1,I,TRIM(ITOS(I))); ENDDO
 CALL WGRIDPUTSTRING(IDF_GRID1,1,SIM%CDATE,NPER+1)
 CALL WGRIDPUTREAL  (IDF_GRID1,2,SIM%DELT ,NPER+1)

! CALL WGRIDPUTINTEGER(IDF_GRID1,1,SIM%ISAVE,NPER)
! CALL WGRIDPUTINTEGER(IDF_GRID1,1,SIM%ISUM,NPER)
 ...
 INTEGER :: JD1,JD2,IPERIOD,I,ISS,IHR,IMT,ISC,IHMS1,IHMS2,ISC1,ISC2,ISTEADY

 PMANAGER_FILLTIMESTEPS=.FALSE.
 
 !## get timestep configurations
 CALL WDIALOGGETMENU(IDF_MENU4,IPERIOD)
 IF(IPERIOD.EQ.7.OR.IPERIOD.EQ.8)THEN
 ...
 IF(ISS.EQ.2)THEN
  ...
  !## get periods 1=Hourly,2=Daily,3=Weekly,4=Monthly,5=Yearly
  SELECT CASE (IPERIOD)
   CASE (1) !## hourly
    NPER=JD2-JD1; NPER=NPER+2; NPER=NPER*24     
   CASE (2) !## daily
    NPER=JD2-JD1; NPER=NPER+2
   CASE (3) !## weekly
    NPER=JD2-JD1; NPER=NPER+2; NPER=NPER/7
   CASE (4) !## decade
    NPER=JD2-JD1; NPER=NPER+2; NPER=NPER/10
   CASE (5) !## 14/28
    NPER=JD2-JD1; NPER=NPER+2; NPER=NPER/14
   CASE (6) !## monthly
    NPER=JD2-JD1; NPER=NPER+2; NPER=NPER/28
   CASE (7) !## yearly
    NPER=JD2-JD1; NPER=NPER+2; NPER=NPER/365
   CASE (8) !## packages
    NPER=1
  END SELECT
  ...
  !## fill in final date on position(0)
  CALL UTL_GDATE(JD2,SIM(0)%IYR,SIM(0)%IMH,SIM(0)%IDY)
  CALL ITIMETOHMS(IHMS2,SIM(0)%IHR,SIM(0)%IMT,SIM(0)%ISC)
  !## fill in start date on position(1)
  CALL UTL_GDATE(JD1,SIM(1)%IYR,SIM(1)%IMH,SIM(1)%IDY)
  CALL ITIMETOHMS(IHMS1,SIM(1)%IHR,SIM(1)%IMT,SIM(1)%ISC)

  !## fill in intermediate timesteps
  SELECT CASE (IPERIOD)
   CASE (1) !## hourly
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0, 1,4))EXIT; ENDDO; NPER=I
   CASE (2) !## daily
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0, 1,3))EXIT; ENDDO; NPER=I
   CASE (3) !## weekly
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0, 7,3))EXIT; ENDDO; NPER=I
   CASE (4) !## decade
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0,10,3))EXIT; ENDDO; NPER=I
   CASE (5) !## 14/28
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0,10,7))EXIT; ENDDO; NPER=I
   CASE (6) !## monthly
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0, 1,2))EXIT; ENDDO; NPER=I
   CASE (7) !## yearly
    I=1; DO; I=I+1; IF(.NOT.PMANAGER_ADDTIMESTEP(I,0, 1,1))EXIT; ENDDO; NPER=I
   CASE (8) !## packages
    CALL PMANAGER_GETNPER(JD1,IHMS1,JD2,IHMS2)
  END SELECT

  !## first is always a steady-state potentially
  SIM(0)%CDATE='STEADY-STATE'; SIM(0)%DELT=0.0; SIM(0)%ISAVE=1; SIM(0)%ISUM=0
 ...
 IF(SIZE(SIM).LE.ISIM)THEN
  N=SIZE(SIM)+99; ALLOCATE(SIM_C(0:N))
  DO I=1,SIZE(SIM); SIM_C(I-1)=SIM(I-1); ENDDO
 ...
 SELECT CASE (TSTEP)
  ...
  CASE (7) !## 14/28
   IF(SIM(ISIM-1)%IDY.LE.14)THEN
    SIM(ISIM)%IDY=28
   ELSE
    SIM(ISIM)%IDY=14
    SIM(ISIM)%IMH=SIM(ISIM-1)%IMH+1
   ENDIF
 END SELECT
 ...
 !## evaluate whether the new date is greate or equal esim - trim on it alternatively
