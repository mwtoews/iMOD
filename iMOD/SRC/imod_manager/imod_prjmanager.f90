...
KLOOP: DO K=1,SIZE(TOPICS(JJ)%STRESS(1)%FILES,1)
 ...
 DO IPER=1,NPER
  ...
  DO I=1,2
   ...
   IF(I.EQ.2)WRITE(JU(3),'(A)') 'OPEN/CLOSE '//TRIM(FNAME(I))//' 1.0 (FREE) -1'
   IF(NO(I).GT.0)THEN
    FNAME(I)=UTL_CAP(FNAME(I),'L')
    J=INDEX(FNAME(I),'.arr',.TRUE.)-1
    FNAME(I)=DIR(:INDEX(DIR,'\',.TRUE.)-1)//TRIM(FNAME(I)(2:J))//'.arr'
   ENDIF
   IF(I.EQ.1)THEN; JU(I)=UTL_GETUNIT(); CALL OSD_OPEN(JU(I),FILE=FNAME(I),STATUS='OLD',ACTION='READ'); ENDIF
   IF(I.EQ.2)THEN
 ...
 ELSE
  ...
  DO ISYS=1,NSYS
   ...
   ELSEIF(LIPF)THEN
    ...
    WRITE(FRM,'(A6,I2.2,A9)') '(3I5,',1,'F15.7,I5)'
...
!            TRIM(RTOS(Q*TLP(ILAY),'F',7))//','//TRIM(ITOS(ISYS))
       WRITE(LINE,FRM) ILAY,IROW,ICOL,Q*TLP(ILAY),ISYS
       WRITE(JU,*) TRIM(LINE)
       NP=NP+1; MP=MP+1
      ENDIF
     ENDDO
   ...
   ELSE 
    ...
    DO IROW=1,PCK(1,1)%NROW; DO ICOL=1,PCK(1,1)%NCOL
     ...
     WRITE(FRM,'(A6,I2.2,A9)') '(3I5,',NTOP,'F15.7,I5)'
     DO ILAY=1,NLAY
...
!      LINE=TRIM(LINE)//','//TRIM(ITOS(ISYS))
      WRITE(LINE,FRM) ILAY,IROW,ICOL,(PCK(JTOP(ITOP),ISYS)%X(ICOL,IROW),ITOP=1,NTOP),ISYS
      WRITE(JU,*) TRIM(LINE)
      NP=NP+1; MP=MP+1
     ENDDO
