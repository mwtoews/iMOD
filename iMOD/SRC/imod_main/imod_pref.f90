...
USE MOD_COLOURS
USE MODPLOT
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,UTL_CREATEDIR,UTL_IMODFILLMENU,UTL_WSELECTFILE,UTL_RELPATHNAME,UTL_GETHELP,UTL_SUBST,UTL_CAP,UTL_DIALOGSHOW
USE MOD_PREF_PAR
...
CONTAINS
 ...
 DO
  ...
  SELECT CASE (I)
   ...
   CASE(3,5)
    CALL UTL_CREATEDIR(TRIM(PREFVAL(I)))
   !## top25[2],help[4],modflow[8],imod-wq[9],modflow6[12],irdbase[6],tags[7],pumpingtool[10]
   CASE(2,4,6:10,12)
   !## northarrow
   CASE(11)
    INQUIRE(FILE=PREFVAL(I),EXIST=LEX); IF(LEX)CALL WMENUSETSTATE(ID_SHOWNARROW,1,1)
   !## ro-tool
   CASE(14:24)
    IOR=IOR+1
   !## plugin tool
