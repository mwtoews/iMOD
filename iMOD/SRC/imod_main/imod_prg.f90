...
CALL WINITIALISE(' ') !IFLAGS=XServerFonts ) !' ')

!CALL UTM_MAIN()
!CALL GOOGLE_MAIN()

!!## puzzle (if neccessary), to become one single line, or at least one
!GENFNAME='d:\iMOD-MODELS\WTI\test\MULTLINE.gen'; CALL PUZZLEMAIN(); STOP
...
!MPW%DIY_BAR=1.0-(190.0/1080.0) !190
!MPW%DIX_BAR=1.0-(15.0 /1920.0) !15
...
!OPEN(99,FILE='OUTPUT.TXT',STATUS='UNKNOWN')
!CALL WPLAYSOUND('RASPBERRY.WAV')
!CALL WPLAYMOVIE('RASPBERRY.AVI')
...
! !## inform what files are still opened, should be none probably ...
! CALL INFOUNITS()
