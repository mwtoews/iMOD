...
USE MOD_PMANAGER, ONLY : PMANAGER_UTL_INIT,PMANAGERMAIN,PMANAGERRUN,PMANAGER_UTL_SHOW,PMANAGER_UTL_UPDATE_TREEVIEW,PMANAGERFIELDS
...
USE MOD_3D_PAR, ONLY : IRENDER_3D,D4ITIMEDIR
use MOD_3D_SETTINGS, ONLY : IMOD3D_SETTINGS_4DVAR_PROGRESS
...
!USE MOD_IPEST_GLM, ONLY : IPEST_ECHELON_DBL
!USE MOD_LUDCMP, ONLY : UTL_JACOBI_MAIN !UTL_FIT_PARABOLA_MAIN !CHOLESKYDECOMPOSITION
...
!REAL(KIND=DP_KIND),DIMENSION(2,7) :: A
!INTEGER,DIMENSION(7) :: B
!DATA A/0.0D0,0.0D0, &
!0.0D0,0.0D0, &
!1.0D0,2.0D0, &
!1.0D0,2.0D0, &
...
!DATA A/1.0D0,4.0D0,5.0D0,-9.0D0,-7.0D0, &
!       -1.0D0,-2.0D0,-1.0D0,3.0D0,1.0D0, &
!        0.0D0,-3.0D0,-6.0D0,4.0D0,9.0D0, &
!       -2.0D0,-3.0D0,0.0D0,3.0D0,-1.0D0/
!CALL IPEST_ECHELON_DBL(A,B,SIZE(A,2),SIZE(A,1))
!data a/36.0d0,30.0d0,18.0d0, &
!       30.0d0,41.0d0,23.0d0, &
!       18.0d0,23.0d0,14.0d0/
!CALL CHOLESKYDECOMPOSITION(a,3) - gives upper-triangle
!DO I=1,3
! WRITE(*,'(99F15.7)') (A(J,I),J=1,3)
!ENDDO

!CALL UTL_EQUALNAMES_UNITTEST()

!call UTL_FIT_PARABOLA_main()
...
!CALL IEEE_SET_HALTING_MODE (IEEE_DIVIDE_BY_ZERO, .TRUE.)
!## underflow is set to zero - as compiler flag
!CALL IEEE_SET_HALTING_MODE (IEEE_UNDERFLOW, .TRUE.)
...
! !## report to commandtool
! CALL WMENUSETSTATE(ID_IDEBUGLEVEL1,2,1)
!#endif
!## store debug level
...
CALL PMANAGER_UTL_INIT(); CALL PMANAGER_UTL_UPDATE_TREEVIEW(0,0,0); CALL PMANAGERFIELDS()
!## initialize iMOD
CALL IMODINIT()
!## initialize preferences
CALL PREFINIT()
...
DO
 ...
 IF(D4ITIMEDIR.NE.0)THEN
  CALL IMOD3D_SETTINGS_4DVAR_PROGRESS(D4ITIMEDIR); CALL IMOD3D_DISPLAY(1)
 ENDIF
