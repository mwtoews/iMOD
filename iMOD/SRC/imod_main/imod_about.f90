...
USE MOD_UTL, ONLY : NEWLINE,UTL_GETUNIT,ITOS,UTL_IMODVERSION,UTL_DIALOGSHOW
...
! CALL WDIALOGPUTIMAGE(IDF_PICTURE4,ID_ICONOPENGL,1)
 ...
 INTEGER :: ITYPE,I,IU,IY,IM,ID,IOS
 TYPE(WIN_MESSAGE) :: MESSAGE
 LOGICAL :: LEX
 CHARACTER(LEN=256) :: TEXT
 CHARACTER(LEN=10) :: CDATE

 IF(LBETA_QUESTION)THEN
 ...
 INQUIRE(FILE=TRIM(EXEPATH)//'\'//TRIM(LICFILE),EXIST=LEX)
 IF(LEX)THEN
  IU=UTL_GETUNIT(); CALL OSD_OPEN(IU,FILE=TRIM(EXEPATH)//'\'//TRIM(LICFILE),STATUS='OLD',ACTION='READ,DENYWRITE')
  IOS=-1; IF(IU.GT.0)READ(IU,'(A256)',IOSTAT=IOS) TEXT
  !## error occured reading license-text file
  IF(IOS.NE.0)THEN
 ...
 IF(I.EQ.1)THEN
  IU=UTL_GETUNIT()
  CALL OSD_OPEN(IU,FILE=TRIM(EXEPATH)//'\'//TRIM(LICFILE),STATUS='UNKNOWN',ACTION='WRITE,DENYREAD')
  IF(IU.EQ.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot save license file:'//CHAR(13)// &
      TRIM(EXEPATH)//'\'//TRIM(LICFILE),'Error')
  ELSE
 ...
 STR='Copyright (C) Stichting Deltares, 2005-2020. '//NEWLINE// &
NEWLINE// &
...
' Delft,The Netherlands. Email: imod.support@deltares.nl. '
 ...
 IF(LEX)THEN
  IU=UTL_GETUNIT()
  CALL OSD_OPEN(IU,FILE=TRIM(EXEPATH)//'\'//TRIM(LICFILE),STATUS='OLD',ACTION='READ,DENYWRITE')
  READ(IU,'(79X,A)') TEXT
  CLOSE(IU)
