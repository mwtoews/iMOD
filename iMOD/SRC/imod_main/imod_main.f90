...
!##TEST READ PLUGIN -> ALS ZE BESTAAN ALLOCATE PI1 OF PI2
!## LIJSTJE MET PLUGIN-NAMEN (DIRNAME -> MET ERONDER DE NAMEN)
...
DO
 ...
 DO 
   ...
   IF(I.LE.MAXPLKEYS)THEN
    ...
    IF(I.EQ.1)THEN
     CALL IMODSAVEPLUGIN(PI1,27,LINE,IU)
    ELSE
     CALL IMODSAVEPLUGIN(PI2,28,LINE,IU)
    ENDIF
    READ(IU,'(A256)',IOSTAT=IOS) LINE !reading ******-en
    IF(IOS.NE.0)EXIT
    IF(LINE(1:10).EQ.'**********')EXIT
