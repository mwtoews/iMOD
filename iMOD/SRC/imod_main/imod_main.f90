...
SELECT CASE (ITYPE)
 ...
 CASE(TIMEREXPIRED)
  SELECT CASE (MESSAGE%VALUE1)
   CASE(1)
    CALL IMODSAVEIMF(TRIM(PREFVAL(1))//'\IMFILES\AUTOSAVE-IMOD_'//TRIM(OSD_GETENV('USERNAME'))//'.IMF',0)
   CASE(2)
...
! WRITE(*,*) MESSAGE%GX,MESSAGE%GY,MPW%XMIN,MPW%YMIN,MPW%XMAX,MPW%YMAX
...
SELECT CASE (IDOWN)
 ...
 CASE (0)
  ICRD=0
  CRDITYPE=0
  !## get nearby location of shape-definition; in case of QS drawing or scenario drawing!!!
...
USE MODPLOT
USE MOD_UTL, ONLY : IMESSAGE,UTL_WSELECTFILE,ICDEBUGLEVEL,UTL_DEBUGLEVEL,UTL_MEASUREMAIN,IDFPLOT2BITMAP
...
USE MOD_ISG, ONLY : ISGEDITINIT,ISGISPRESET,ISGISPSAVE,ISGISPSTOP,ISGFIELDS,ISGATTRIBUTES,ISGDEL, &
                    ISGPOSITIONCRSCLC,ISGDELCLOSE,ISGISPSTART
...
! CALL IGRAREA(0.0,0.0,1.0,1.0)
! CALL IGRUNITS(MPW%XMIN,MPW%YMIN,MPW%XMAX,MPW%YMAX)
!WRITE(*,*) WInfoGrReal(GraphicsAreaMinX),WInfoGrReal(GraphicsAreaMinY),WInfoGrReal(GraphicsAreaMaxX ),WInfoGrReal(GraphicsAreaMaxY )
!WRITE(*,*) WInfoGrReal(GraphicsUnitMinX ),WInfoGrReal(GraphicsUnitMinY ),WInfoGrReal(GraphicsUnitMaxX  ),WInfoGrReal(GraphicsUnitMaxY  )
...
USE MOD_TOOLS, ONLY : TOOLS_CLOSE
...
!###======================================================================
SUBROUTINE IMODSAVEIMF(FNAME,IERROR)
!###======================================================================
USE IMODVAR
USE MODPLOT
...
USE MOD_DEMO_PAR
IMPLICIT NONE
INTEGER,PARAMETER :: MAXMPWKEYS=7
INTEGER,PARAMETER :: MAXMPKEYS =34
...
DATA MPWKEYS/'NACT=','XMIN=','XMAX=','YMIN=','YMAX=','ITRANSP=','IASAVE='/
DATA MPKEYS/'IDFNAME=','ALIAS=','ISEL=','IDFI=','IEQ=','IDFKIND=','SYMBOL=',      &
            ...
            'TSIZE=','ISCREEN=','ASSCOL1=','ASSCOL2=','ILEG=','ILEGDLF=','GLSCALE=',&
            'SCALED=','BORECK1=','BORECK2=','BORECLS=','HCOL_M='/
...
!   CALL LEG_CONSTUCTICLRGRAD(IPLOT)
  ...
  IF(I.LE.MAXMPKEYS)THEN
   SELECT CASE (I)
    !## character
    CASE (1:2,18,30,33,34)
     READ(LINE,'(8X,A256)')  CVALUE
    !## integer
    CASE (4:17,19:29,31:32)
     READ(LINE,'(8X,I10)')   IVALUE
    !## real
    CASE (0)
   ...
   SELECT CASE (I)
    ...
    CASE (30)
     ...
     SCALED=1.0; READ(CVALUE,FRM) SCALED(1:N)
    CASE (31)
    ...
    CASE (33)
     ...
     MP(IPLOT)%ICPERC=1.0; READ(CVALUE,FRM) MP(IPLOT)%ICPERC  
    CASE (34)
...
USE MOD_SCENTOOL, ONLY : ST1CLOSE
USE MOD_SOLID, ONLY : SOLID_CLOSE
