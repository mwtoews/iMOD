...
USE PMANAGER_MOD, ONLY : PMANAGERSHOW
...
USE MOD_ABOUT, ONLY : IMODSTARTSCREEN,IMODAGREEMENT,IMODABOUT,IMODDISCLAIMER
...
INTEGER,PARAMETER :: MAXMPKEYS =26
...
READ(IU,*) !'IMOD META-FILE VERSIE 1.0, DEFINITION: APRIL 2007'
READ(IU,*)
 ...
 DO
  ...
  IF(I.LE.MAXMPKEYS)THEN
   ...
   SELECT CASE (I)
    ...
    CASE (18)
     READ(CVALUE,'(10I1)') MP(IPLOT)%IAXES!=CVALUE
...
SELECT CASE (ID)
 CASE (ID_SAVEMASK)
  ...
  IF(.NOT.UTL_WSELECTFILE('iMOD Mask File (*.msk)|*.msk|',SAVEDIALOG+PROMPTON+DIRCHANGE+APPENDEXT,&
       FNAME,'Select iMOD Mask File'))RETURN
  IU=UTL_GETUNIT()
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (IDHELP)
     CALL IMODGETHELP('3.3.2','Goto XY')
    CASE (IDCANCEL)
     EXIT
   END SELECT
...
DO JPLOT=1,MXMPLOT
 IF(MP(JPLOT)%IACT)THEN
  ...
  IF(LEX)THEN
   ...
   MP(IPLOT)%ISCREEN=MAX(MP(IPLOT)%ISCREEN,1)
...
!## mouse-move
  CASE (MOUSEMOVE)
   ...
   CALL WINDOWSELECT(0)
   CALL WINDOWOUTSTATUSBAR(1,'X:'//TRIM(RTOS(MESSAGE%GX/1000.,'F',2))//' km, Y:'//TRIM(RTOS(MESSAGE%GY/1000.,'F',2))//' km')
...
STRING='Total distance= '//TRIM(RTOS(TDIST/1000.0,'F',3))//' km, Distance last segment= '// &
                           TRIM(RTOS(DIST/1000.0,'F',3))//' km'
...
!## acrobat reader
IF(LACROBAT)THEN
 LINE='"'//TRIM(PREFVAL(13))//'" /n /s /A "nameddest=sec:'//TRIM(TOPIC)//'" "'//TRIM(PREFVAL(4))//'"'
!## sumatra pdf
ELSE
 LINE='"'//TRIM(PREFVAL(13))//'" -reuse-instance -named-dest sec:'//TRIM(TOPIC)//' "'//TRIM(PREFVAL(4))//'"'
ENDIF
