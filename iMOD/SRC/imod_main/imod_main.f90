...
SELECT CASE (ITYPE)
 ...
 CASE (BITMAPSCROLLED)
  CALL IMOD1BITMAPSCROLLED(MESSAGE)
 !## exposed/resized
 CASE (EXPOSE,RESIZE)
...
USE IMOD
USE MOD_LEGPLOT, ONLY : LEGADJUSTSHAPE,LEGPLOTMOUSEMOVE
...
SELECT CASE (IDOWN)
 ...
 CASE (4)
  ...
  ELSEIF(CRDITYPE.LT.0)THEN
   CALL LEGADJUSTSHAPE(CRDITYPE,ICRD,MESSAGE%GX,MESSAGE%GY,DOWNX,DOWNY)
  ENDIF
 ...
 CASE (0)
  ...
  IF(CRDITYPE.EQ.0)CALL LEGPLOTMOUSEMOVE(MESSAGE%GX,MESSAGE%GY,ICRD,CRDITYPE)
END SELECT
...
!###======================================================================
SUBROUTINE IMOD1BITMAPSCROLLED(MESSAGE)
!###======================================================================
USE WINTERACTER
USE RESOURCE
USE MODPLOT
...
USE MOD_LEGPLOT, ONLY : LEGPLOTINIT,LEGPLOTUPDATE
...
USE MOD_PLUGIN, ONLY : PLUGIN_MAIN,PLUGIN_EXE
...
!## stop iMOD
 CASE(ID_QUIT)
  CALL IMODCLOSE(0)
!## save drawing
 CASE (ID_SAVE,ID_SAVEAS,ID_OPEN)
...
!## autosave
 CASE (ID_AUTOSAVE)
  ...
  IF(WMENUGETSTATE(MESSAGE%VALUE1,2).EQ.1)THEN
   ...
   CALL WMESSAGETIMER(0)
   IMESSAGE(TIMEREXPIRED)=0
  ELSE
   CALL WMENUSETSTATE(MESSAGE%VALUE1,2,1)
   CALL WMENUSETSTRING(MESSAGE%VALUE1,'AutoSave On (1 minute)')
   CALL WMESSAGETIMER(60*1000,IREPEAT=1)  !minutes
...
USE WINTERACTER
USE RESOURCE
USE IMODVAR, ONLY : IDIAGERROR,OPENDIR,SAVEDIR
USE MODPLOT
...
WRITE(IU,'(A/)') 'IMOD META-FILE VERSIE '//TRIM(RVERSION)
...
!  ENDIF
  IF(MP(I)%IPLOT.NE.4)THEN !## ne isg
   ...
   END DO
   DO J=1,MXCGRAD
    WRITE(IU,'(A8,I10)') 'CGRAD=  ',MP(I)%LEG%CGRAD(J)
   END DO
  ENDIF
  WRITE(IU,'(50A1)') ('+',K=1,50)
 ENDIF
...
IF(IDEMO.EQ.1)THEN
 WRITE(IU,'(A,A)') 'FUNCTION=',DEMO%TDNAME
 WRITE(IU,'(A,I)') 'NPOINTS=',DEMO%NXY
 DO I=1,DEMO%NXY
  WRITE(IU,'(I,I)') DEMO%X(I),DEMO%Y(I)
 ENDDO
ENDIF
WRITE(IU,'(50A1)') ('D',K=1,50)
...
INTEGER :: IU,I,J,IOS,IVALUE,IPLOT
REAL :: RVALUE
LOGICAL :: LEX,LVALUE
...
DATA MPWKEYS/'NACT=','XMIN=','XMAX=','YMIN=','YMAX='/
...
DO IPLOT=1,MXMPLOT
 ...
 DO
  ...
  IF(LINE(1:10).EQ.'----------')THEN
   ...
   END DO
   DO J=1,MXCGRAD
    READ(IU,'(8X,I10)',IOSTAT=IOS) MP(IPLOT)%LEG%CGRAD(J)
   END DO
...
DO
 ...
 DO 
   ...
   IF(I.LE.MAXPLKEYS)THEN
    ...
    IF(I.EQ.1)THEN
     CALL PLUGIN_SAVEPLUGIN(PI1,27,LINE,IU)
    ELSE
     CALL PLUGIN_SAVEPLUGIN(PI2,28,LINE,IU)
    ENDIF
    READ(IU,'(A256)',IOSTAT=IOS) LINE !reading ******-en
    IF(IOS.NE.0)EXIT
    IF(LINE(1:10).EQ.'**********')EXIT
...
!DO
! READ(IU,'(A256)',IOSTAT=IOS) LINE !reading DEMOKEYS
! IF(IOS.NE.0)EXIT
!
! DO 
!   READ(LINE,'(A)') CKEY
...
!   
!   DO I=1,MAXDEMOKEYS
!    IF(TRIM(CKEY).EQ.DEMOKEYS(I))EXIT
!   END DO
!   
!   IF(I.EQ.1)THEN
!    !#reads function name (e.g. cross, 3D)
!    IF(.NOT.UTL_READINITFILE('FUNCTION=',LINE,IU,0))THEN; CLOSE(IU); RETURN; ENDIF
!    READ(LINE,*) DEMO%TDNAME                
!    IF(.NOT.UTL_READINITFILE('NPOINTS=',LINE,IU,0))THEN; CLOSE(IU); RETURN; ENDIF
!    READ(LINE,*) DEMO%NXY
!    
!    IF(.NOT.ASSOCIATED(DEMO%Y))THEN
!     ALLOCATE(DEMO%Y(DEMO%NXY))       
!    ELSE
!     DEALLOCATE(DEMO%Y); ALLOCATE(DEMO%Y(DEMO%NXY))
!    ENDIF 
!    IF(.NOT.ASSOCIATED(DEMO%X))THEN
!     ALLOCATE(DEMO%X(DEMO%NXY))       
!    ELSE
!     DEALLOCATE(DEMO%X); ALLOCATE(DEMO%X(DEMO%NXY))
!    ENDIF  
!        
!    IF(.NOT.UTL_READINITFILE('PROFILE=',LINE,IU,0))THEN; CLOSE(IU); RETURN; ENDIF
!    READ(LINE,*) DEMO%X(1),DEMO%Y(1),DEMO%X(2),DEMO%Y(2)
!    
!   ENDIF
...
!ENDDO
 ...
 DO 
   ...
   IF(I.EQ.1)THEN
    !#reads function name(s) (e.g. cross, 3D)
    DEMO%TDNAME=''; IF(UTL_READINITFILE('FUNCTION=',LINE,IU,1))READ(LINE,*) DEMO%TDNAME
    ...
    DEMO%NXY=0; IF(UTL_READINITFILE('NPOINTS=',LINE,IU,1))READ(LINE,*) DEMO%NXY
    IF(DEMO%NXY.EQ.0)THEN
     CALL MESSAGEBOX(OKCANCEL,EXCLAMATIONICON,)
    ENDIF
    ...
    IF(UTL_READINITFILE('PROFILE=',LINE,IU,1))READ(LINE,*) DEMO%X(1),DEMO%Y(1),DEMO%X(2),DEMO%Y(2)
   ENDIF
   READ(IU,'(A256)',IOSTAT=IOS) LINE !reading DDDDDDDDDDD-en
   IF(IOS.NE.0)EXIT
   IF(LINE(1:10).EQ.'DDDDDDDDDD')EXIT
