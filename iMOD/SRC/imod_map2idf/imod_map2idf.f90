...
CONTAINS
 ...
 INTEGER :: IU,ICOL,IROW,I,I4MINVAL,I4MAXVAL,IOS
 REAL(KIND=DP_KIND) :: R4MINVAL,R4MAXVAL,NODATA
 ...
 REAL(KIND=DP_KIND) :: XUL,YUL
 INTEGER(KIND=4) :: NRROWS,NRCOLS
 REAL(KIND=DP_KIND) :: CELLSIZEX,CELLSIZEY
 ...
 IDF(1)%YMIN=YUL-NRROWS*CELLSIZEY; IDF(1)%YMAX=YUL
 ...
 SELECT CASE(CELLREPR)
  ...
  CASE(CR_INT4)
   ...
   READ(IU,POS=256+1) ((I4A(IROW,ICOL),IROW=1,NRROWS),ICOL=1,NRCOLS)
   !## copy in IDF object
   IDF(1)%X=REAL(I4A)
   DEALLOCATE(I4A)
  !## real map file   
  CASE(CR_REAL4)
   READ(IU,POS=256+1)((IDF(1)%X(IROW,ICOL),IROW=1,NRROWS),ICOL=1,NRCOLS)  
 END SELECT
 ...
 !## write idf file
 IF(.NOT.IDFWRITE(IDF(1),IDFNAME(:I)//'.IDF',1))THEN; ENDIF
