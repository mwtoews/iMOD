...
REAL,ALLOCATABLE,DIMENSION(:),PRIVATE :: BU_LLIMP,BU_ULIMP,BU_IMP
...
ILOOP: DO I=1,NOPT
       ...
       IF(BU_IFIXED(I).EQ.1)THEN
        CALL WGRIDGETCELLREAL(IDF_GRID1,5,I,X1)
        X2=X1
       ELSE
        CALL WGRIDGETCELLREAL(IDF_GRID1,3,I,X1)
        CALL WGRIDGETCELLREAL(IDF_GRID1,4,I,X2)
        IF(X1.GE.X2)THEN
       ...
       IF(X1.GT.IR(IOPT)%MAXIR.OR.X2.GT.IR(IOPT)%MAXIR)THEN
        ...
        IF(BU_IFIXED(I).EQ.0)CALL WGRIDPUTCELLREAL(IDF_GRID1,4,I,BU_ULIMP(I))
        IF(BU_IFIXED(I).EQ.1)CALL WGRIDPUTCELLREAL(IDF_GRID1,5,I,BU_ULIMP(I))
        IERROR=1
        EXIT ILOOP
       ENDIF
       IF(X1.LT.IR(IOPT)%MINIR.OR.X2.LT.IR(IOPT)%MINIR)THEN
        ...
        IF(BU_IFIXED(I).EQ.0)CALL WGRIDPUTCELLREAL(IDF_GRID1,3,I,BU_LLIMP(I))
        IF(BU_IFIXED(I).EQ.1)CALL WGRIDPUTCELLREAL(IDF_GRID1,5,I,BU_LLIMP(I))
        IERROR=1
        EXIT ILOOP
       ENDIF
...
!WRITE(*,*) 'nopt=',nopt
 ...
 ELSE
  ...
  DO I=1,NOPT
   ...
   CALL WGRIDPUTCELLREAL(IDF_GRID1,3,I,BU_LLIMP(I),'(F8.2)')
   CALL WGRIDPUTCELLREAL(IDF_GRID1,4,I,BU_ULIMP(I),'(F8.2)')
   CALL WGRIDPUTCELLREAL(IDF_GRID1,5,I,BU_IMP(I),'(F8.2)')
  END DO
 ENDIF
 ...
 CALL WGRIDGETREAL(IDF_GRID1,3,BU_LLIMP,NOPT)
 CALL WGRIDGETREAL(IDF_GRID1,4,BU_ULIMP,NOPT)
 CALL WGRIDGETREAL(IDF_GRID1,5,BU_IMP,NOPT)
