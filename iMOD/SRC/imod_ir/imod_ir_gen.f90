...
USE MOD_UTL, ONLY : UTL_INVERSECOLOUR,ITOS,UTL_WAITMESSAGE,UTL_GETUNIT,UTL_MESSAGEHANDLE,UTL_CAP,UTL_PLOT1BITMAP, &
    UTL_PLOT2BITMAP,DBL_IGRINSIDEPOLYGON,UTL_PLOTPOINT
USE MOD_POLYGON_PAR
...
CONTAINS
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open/read the optional file: '//CHAR(13)// &
    TRIM(FNAME),'Warning')
  RETURN
 ENDIF
 ...
 IF(ID.EQ.ID_DIR_PMTAB1TAB2)CALL IGRCOLOURN(UTL_INVERSECOLOUR(ICLRTARGET))   !## target
 IF(ID.EQ.ID_DIR_PMTAB2TAB2)CALL IGRCOLOURN(UTL_INVERSECOLOUR(ICLRMEASURE))  !## measure
 ...
 DO
  ...
  SELECT CASE (ITYPE)
   ...
   CASE (MOUSEMOVE)
    !## highlight polygon
    CALL IR1GENHIGHLIGHT(REAL(MESSAGE%GX,8),REAL(MESSAGE%GY,8),IBC)
 ...
 DO I=1,BC(IBC)%NGEN
  IF(ISELGEN(I).EQ.INT(1,1))THEN
   ...
   I2=BC(IBC)%IXY(I+1)
   N =I2-I1
   CALL DBL_IGRPOLYGONCOMPLEX(BC(IBC)%X(I1:N),BC(IBC)%Y(I1:N),N)
  ENDIF
 ENDDO
 ...
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ID,IBC
 REAL(KIND=DP_KIND),PARAMETER :: DCRIT=15.0  !## points closer than 5 meter will be removed!
 INTEGER :: I,J,I1,I2,N,ISEL
 REAL(KIND=DP_KIND) :: D,DX,DY
 ...
 SHP%NPOL           =SHP%NPOL+1
 SHP%POL(SHP%NPOL)%ITYPE  =ID_POLYGON
 SHP%POL(SHP%NPOL)%IACT  =1
 IF(ID.EQ.ID_DIR_PMTAB1TAB2)SHP%POL(SHP%NPOL)%ICOLOR=ICLRTARGET  !## target
 IF(ID.EQ.ID_DIR_PMTAB2TAB2)SHP%POL(SHP%NPOL)%ICOLOR=ICLRMEASURE !## measure
 SHP%POL(SHP%NPOL)%PNAME  ='Peilvak_'//TRIM(ITOS(ISEL))
 SHP%POL(SHP%NPOL)%IWIDTH =2

 !## analyse polygon, remove double/close points
 J=I1
 !## set first point
 N=1
 SHP%POL(SHP%NPOL)%X(N)=BC(IBC)%X(I1)
 SHP%POL(SHP%NPOL)%Y(N)=BC(IBC)%Y(I1)
 DO I=I1+1,I2-1
  ...
  IF(D.GT.DCRIT)THEN
   N=N+1
   SHP%POL(SHP%NPOL)%X(N)=BC(IBC)%X(I)
   SHP%POL(SHP%NPOL)%Y(N)=BC(IBC)%Y(I)
   J=I
  ENDIF
 END DO
 !## remove double point on begin/end
 IF(SHP%POL(SHP%NPOL)%X(1).EQ.SHP%POL(SHP%NPOL)%X(N).AND. &
    SHP%POL(SHP%NPOL)%Y(1).EQ.SHP%POL(SHP%NPOL)%Y(N))N=N-1

 SHP%POL(SHP%NPOL)%N=N
 ...
 IF(ABS(XC-XCMOUSE).LE.0.01D0.AND.ABS(YC-YCMOUSE).LE.0.01D0)RETURN
 ...
 DO I=1,BC(IBC)%NGEN
  ...
  IF(BC(IBC)%XMIN(I).LT.XC.AND.BC(IBC)%XMAX(I).GT.XC.AND. &
   ...
   IF(DBL_IGRINSIDEPOLYGON(XC,YC,BC(IBC)%X(I1:N),BC(IBC)%Y(I1:N),N).EQ.1)ISELGEN(I)=INT(1,1)
 ...
 REAL(KIND=DP_KIND) ::  D,MIND
 INTEGER :: I,J

 MIND=10.0D10
 DO J=J1,J2
  DO I=I1,I2
   D=((BC(IBC)%X(J)-BC(IBC)%X(I))**2.0D0+(BC(IBC)%Y(J)-BC(IBC)%Y(I))**2.0D0)
   IF(D.NE.0.0D0)D=SQRT(D)
   IF(D.LT.MIND)THEN
    D   =MIND
    IPOS=I
 ...
 DO IBC=1,NBC
  ...
  IF(WMENUGETSTATE(IDV(IBC),2).EQ.1)THEN
   ...
   ELSEIF(BC(IBC)%ITYPE.EQ.1)THEN
    ...
    DO I=1,BC(IBC)%NGEN
     CALL UTL_PLOTPOINT(BC(IBC)%X(I),BC(IBC)%Y(I),IBC,1.0D0)!MP(IPLOT)%SYMBOL,MP(IPLOT)%THICKNESS)
    END DO
