...
! WRITE(*,*) 'itree,ifield',ITREE,IFIELD,linvir,lquick

!WRITE(*,*) 'before',itree,ifield
...
!WRITE(*,*) i,mtree(ifield)%idpos,mtree(ifield)%measure_id
  CALL IR1GETTREEID(ITREE,IFIELD,I)
!  WRITE(*,*) 'itree,ifield',ITREE,IFIELD,I
...
!  WRITE(*,*) 'JTREE,JFIELD',JTREE,JFIELD
 ENDIF

! WRITE(*,*) 'after',itree,ifield
! WRITE(*,*) MTREE(IFIELD)%NPOL,SHPNO
! WRITE(*,*) MTREE(IFIELD)%NPOL
! DO I=1,MTREE(IFIELD)%NPOL
! WRITE(*,*) I,MTREE(IFIELD)%POL(I)%IACT
! END DO

 !## generate results - activate all polygons!
! IF(.NOT.LINVIR.AND..NOT.LQUICK)THEN
!  DO I=1,MTREE(IFIELD)%NPOL; MTREE(IFIELD)%POL(I)%IACT=1; END DO
! ENDIF
...
!  CALL WDIALOGGETMENU(IDF_MENU2,IRES)
...
!WRITE(*,*) IR%IMP
!WRITE(*,*) IR%IFIXED
!WRITE(*,*) IR%TYPEIR  !0=real,1=binair
...
!WRITE(*,*) JIR,NPOL1,NPOL2,SUM(IPR),SUM(IRS)
!WRITE(*,*) NIR*NPOL1*NPOL2*NRES*NPER

  !## allocate matrix to store coefficients
  ALLOCATE(COEF(JIR,NPOL1,NPOL2,SUM(IRS),SUM(IPR),2),STAT=IOS)
!  WRITE(*,*) JIR,NPOL1,NPOL2,SUM(IRS),SUM(IPR)
!  WRITE(*,*) SIZE(COEF,1),SIZE(COEF,2),SIZE(COEF,3),SIZE(COEF,4),SIZE(COEF,5)
  IF(IOS.NE.0)THEN
   CALL IR2DEALLOCATE()
...
!WRITE(*,*) IC1,IC2,IR1,IR2

! IR1=INT((IDFP(IIR)%IDF%YMAX-YMAX)/IDFP(IIR)%IDF%DY)
! IR2=INT((IDFP(IIR)%IDF%YMAX-YMIN)/IDFP(IIR)%IDF%DY)
! IC1=INT((XMIN-IDFP(IIR)%IDF%XMIN)/IDFP(IIR)%IDF%DX)
! IC2=INT((XMAX-IDFP(IIR)%IDF%XMIN)/IDFP(IIR)%IDF%DX)

!WRITE(*,*) IC1,IC2,IR1,IR2

 DO IROW=MAX(1,IR1),MIN(IR2,IDFP(IIR)%IDF%NROW)
!  YC=IDFP(IIR)%IDF%YMAX-(IROW-1)*IDFP(IIR)%IDF%DY-IDFP(IIR)%IDF%DY/2.0
  DO ICOL=MAX(1,IC1),MIN(IC2,IDFP(IIR)%IDF%NCOL)
!   XC=IDFP(IIR)%IDF%XMIN+(ICOL-1)*IDFP(IIR)%IDF%DX+IDFP(IIR)%IDF%DX/2.0
...
!WRITE(*,*) XC,YC
!PAUSE
   IF(UTL_INSIDEPOLYGON(XC,YC,MTREE(IFIELD)%POL(IPOL)%X,MTREE(IFIELD)%POL(IPOL)%Y,XPOL,YPOL,MTREE(IFIELD)%POL(IPOL)%NCRD).EQ.1)THEN
...
!      IDF(NIDF)%IMP=MTREE(IFIELD)%POL(IPOL)%MES(IMES)%IMP/IR(IIR)%MAXIR
      IDF(NIDF)%IMP=IR1IMPULSEFACTOR(MTREE(IFIELD)%POL(IPOL)%MES(IMES)%IMP,IIR)
!WRITE(*,*) nidf,ipol,imes,IDF(NIDF)%IMP,MTREE(IFIELD)%POL(IPOL)%MES(IMES)%IMP
     ELSE
      !## not available
      NIDF=NIDF-1
     ENDIF
