...
USE MOD_UTL, ONLY : ITOS,UTL_IDFSNAPTOGRID,UTL_WAITMESSAGE,UTL_GETUNIQUE,UTL_CREATEDIR,UTL_MESSAGEHANDLE,&
    UTL_GETUNIT,DBL_IGRINSIDEPOLYGON
USE MOD_PREF_PAR, ONLY : PREFVAL
USE MOD_IDF, ONLY : IDFREAD,IDFOPEN,IDFREADDIM,IDFGETVAL,IDFDEALLOCATEX,IDFNULLIFY, &
...
USE MOD_POLYGON_PAR !, ONLY : SHP%NPOL
  ...
  DO IPOL=1,MTREE(IFIELD)%NPOL
   ...
   DO IIR=1,NIR
    ...
    IF(IR(IIR)%ISEL.NE.0)THEN
     ...
     MTREE(IFIELD)%POL(IPOL)%MES(J)%IMP =0.0D0!IIR
    ENDIF
   ENDDO
  ENDDO
...
IIRLOOP: DO IIR=1,NIR
   ...
   IF(IR(IIR)%ISEL.NE.0)THEN
    ...
    DO IPOL=1,MTREE(IFIELD)%NPOL
     ...
     MTREE(IFIELD)%POL(IPOL)%MES(1:NIR)%IMP=0.0D0
  ...
  DO IFT=1,2
   ...
   DO
    ...
    CALL IR1LINEARPROGRAMMING(IFIELD,JFIELD,IFT,INEXT) !## restricted
    !## finished
    IF(INEXT.EQ.0)EXIT
   ENDDO
  ...
  IF(IPR(IPER).EQ.1)THEN
   ...
   DO IRES=1,NRES
    IF(IRS(IRES).EQ.1)THEN
     ...
     IF(.NOT.IR2READIDF(IFIELD,XMIN,YMIN,XMAX,YMAX,LQUICK,NIDF,IDFNAME))RETURN
     ...
     IF(LQUICK.AND..NOT.LINVIR)THEN
      !## plot idf in imod
      CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=IDFNAME,LEGNAME=TRIM(RES(IRES)%LEGRES))
      !call idfplotfast()       
      !## draw legend
     ...
     IF(LINVIR)THEN
      IF(.NOT.IR2LINEAR_PERCENTILES(JFIELD,IIR,JPOL,JPER,JRES,IRES))RETURN
     ENDIF
 ...
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: SX,SY
 ...
 DO IIDF=1,NIDF
  IF(IDF(IIDF)%IDF%IU.GT.0)THEN
   NCOL=NCOL+IDF(IIDF)%IDF%NCOL+1 
   NROW=NROW+IDF(IIDF)%IDF%NROW+1 
  ENDIF
 END DO
 ...
 IF(ALLOCATED(EFFECT))THEN
  ...
  DEALLOCATE(EFFECT)
 ENDIF
 ALLOCATE(EFFECT(1)); CALL IDFNULLIFY(EFFECT(1))
 ...
 INTEGER :: IIR,NC,NR,MXIDF,IOS,TIR,I
 REAL(KIND=DP_KIND) :: DX,DY

 IR2READIDFP=.FALSE.

 ALLOCATE(IDFP(NIR)); DO I=1,SIZE(IDFP); CALL IDFNULLIFY(IDFP(I)%IDF); ENDDO
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot store current number of IDF"s ['//TRIM(ITOS(MXIDF))//']','Error')
  RETURN
 ENDIF
 DO I=1,SIZE(IDF); CALL IDFNULLIFY(IDF(I)%IDF); ENDDO
 ...
 DO IPOL=1,MTREE(IFIELD)%NPOL
  ...
  IF(LEX)THEN
   DO IIR=1,MTREE(IFIELD)%POL(IPOL)%NMES
    IF(MTREE(IFIELD)%POL(IPOL)%MES(IIR)%IMP.NE.0.0D0)TIR=TIR+1
   ENDDO
  ENDIF
 ENDDO
 ...
 !###======================================================================
 FUNCTION IR2READIDF(IFIELD,XMIN,YMIN,XMAX,YMAX,LQUICK,NIDF,IDFNAME)
 !###======================================================================
 IMPLICIT NONE
 LOGICAL :: IR2READIDF
 ...
 IF(MTREE(IFIELD)%POL(IPOL)%MES(IMES)%IMP.EQ.0.0D0)THEN
  IR2OPENIRIDF=.TRUE.
  RETURN
 ENDIF
 ...
 DO IROW=MAX(1,IR1),MIN(IR2,IDFP(IIR)%IDF%NROW)
  DO ICOL=MAX(1,IC1),MIN(IC2,IDFP(IIR)%IDF%NCOL)
   ...
   IF(DBL_IGRINSIDEPOLYGON(XC,YC,MTREE(IFIELD)%POL(IPOL)%X,MTREE(IFIELD)%POL(IPOL)%Y,MTREE(IFIELD)%POL(IPOL)%NCRD).EQ.1)THEN
...
!WRITE(*,*) 'IMP=',IIDF,IDF(IIDF)%IMP!,ITYPERES,IDF(IIDF)%IDF%IEQ
  ...
  IF(EFFECT(1)%IEQ.EQ.0)THEN
   DO IROW=1,EFFECT(1)%NROW
    ...
    DO ICOL=1,EFFECT(1)%NCOL
     ...
     EFFECT(1)%X(ICOL,IROW)=1000.0D0*EFFECT(1)%X(ICOL,IROW)/(DX*DY)
    ENDDO
   ENDDO
  ENDIF
 ENDIF
 ...
 REAL(KIND=DP_KIND),INTENT(IN) :: XY
 INTEGER :: IR,IC
