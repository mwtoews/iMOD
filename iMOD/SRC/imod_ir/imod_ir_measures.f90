...
USE MOD_IR_PAR
USE MOD_IR_UTL, ONLY : IR1GETTREEVIEWID,IR1SHAPE2POL,IR1POL2SHAPE,UTL_DIALOGSHOW
...
REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:),PRIVATE :: BU_IMP
     ...
     CASE (IDF_GRID1)
      ...
      IF(ICOL.EQ.1)THEN
       ...
       IF(IR(I)%TYPEIR.EQ.0)THEN
        ...
        CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,IROW,BU_IMP(ICOL))
       !## binair
       ELSEIF(IR(I)%TYPEIR.EQ.1)THEN
        CALL WGRIDSTATECELL(IDF_GRID1,2,IROW,0)
        CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,IROW,1.0D0)
       ENDIF
      ENDIF
    END SELECT
...
ILOOP: DO I=1,NMES
 ...
 ELSE
  ...
  DO I=1,NMES
   ...
   IF(IR(BU_IMES(I))%TYPEIR.EQ.0)THEN
    ...
    CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,I,BU_IMP(I))
   !## binair
   ELSEIF(IR(BU_IMES(I))%TYPEIR.EQ.1)THEN
    CALL WGRIDSTATECELL(IDF_GRID1,2,I,0)
    CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,I,1.0D0)
   ENDIF
 ...
 DO I=1,NMES
  ...
  CALL WGRIDGETCELLDOUBLE(IDF_GRID1,2,I,BU_IMP(I))
...
! CALL WGRIDRANGEREAL(IDF_GRID1,2,0.0D0,1.0D0)
! DO I=1,NIR
!  CALL WGRIDPUTCELLDOUBLE(IDF_GRID1,2,I,1.0D0,'(F7.2)')
! END DO
