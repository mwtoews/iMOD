...
USE MOD_POLYGON_PAR
USE MOD_IR_PAR
USE MOD_IDF, ONLY : IDFREAD,IDFDEALLOCATEX,IDFCOPY,IDFGETLOC,IDFIROWICOL
...
! CALL IGRPLOTMODE(MODECOPY)
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,IOS,MXCRD,IROW,ICOL,ITREE,IFIELD,IR1,IR2,IC1,IC2
  ...
  IF(MTREE(IFIELD)%POL(I)%IACT.EQ.1)THEN
   ...
   DO IROW=IR1,IR2
    DO ICOL=IC1,IC2
     ...
     IF(UTL_INSIDEPOLYGON(XC,YC,MTREE(IFIELD)%POL(I)%X,MTREE(IFIELD)%POL(I)%Y,XPOL,YPOL,MTREE(IFIELD)%POL(I)%NCRD).EQ.1)THEN
...
! DO I=1,NTHREAD
!  WRITE(*,*) I,YSEL(1,I),YSEL(2,I)
! END DO

!   IF(NTHREAD.GE.MAXTHREAD)THEN
!    CALL WINDOWOUTSTATUSBAR(4,TRIM(ITOS(NTHREAD))//' Selected (maximum)')
!   ELSE
!    CALL WINDOWOUTSTATUSBAR(4,TRIM(ITOS(NTHREAD))//' Selected')
!   ENDIF
