...
USE MOD_UTL, ONLY : DBL_IGRINSIDEPOLYGON
...
! CALL IGRPLOTMODE(MODECOPY)
 ...
 REAL(KIND=DP_KIND) :: XMIN,YMIN,XMAX,YMAX,XC,YC
  ...
  IF(MTREE(IFIELD)%POL(I)%IACT.EQ.1)THEN
   ...
   DO IROW=IR1,IR2
    DO ICOL=IC1,IC2
     ...
     IF(DBL_IGRINSIDEPOLYGON(XC,YC,MTREE(IFIELD)%POL(I)%X,MTREE(IFIELD)%POL(I)%Y,MTREE(IFIELD)%POL(I)%NCRD).EQ.1)THEN
