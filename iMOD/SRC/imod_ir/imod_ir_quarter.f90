...
USE WINTERACTER
USE RESOURCE
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_SUBST,UTL_CLOSEUNITS,ITOS,RTOS,UTL_CAP,UTL_MESSAGEHANDLE,UTL_CREATEDIR,UTL_IDFSNAPTOGRID
USE MOD_IR_PAR
USE MOD_IR_UTL, ONLY : IR1GETTREEVIEWID,IR1FIELDS_STRING,IR1GETTREEIDS
USE MOD_IR_CLC, ONLY : IR2GETEXTENSION
USE MOD_PREF_PAR, ONLY : PREFVAL
USE MOD_GXG_CLC, ONLY : GXG1_COMPUTEGXG
...
CONTAINS
 ...
 IF(IR1QUARTER_MAIN)THEN
  ...
  IF(IR1QUARTER_SCNFILE(TRIM(OUTPUT)//'\simulation',IMEASURE,IMDLTYPE))THEN
   !## start simulation
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot create run.bat','Error')
  RETURN
 ENDIF
 ...
 !###======================================================================
 LOGICAL FUNCTION IR1QUARTER_SCNFILE(OUTPUT,IMEASURE,IMDLTYPE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IMEASURE,IMDLTYPE
 CHARACTER(LEN=*),INTENT(IN) :: OUTPUT
 LOGICAL :: LEX
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot create scenariofile: '//TRIM(OUTPUT)//'\imodfow.scn','Error')
  RETURN
 ENDIF
...
!   LINE=TRIM(OUTPUT)//'\sdf_measure'//TRIM(ITOS(I))//'.sdf'
 ...
 IF(.NOT.LEX)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot find scenario definition file: '//TRIM(SDF_FROM),'Error')
  RETURN
 ENDIF

 CALL OSD_OPEN(IU,FILE=SDF_FROM,ACTION='READ,DENYWRITE',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot open scenario definition file: '//TRIM(SDF_FROM),'Error')
  RETURN
 ENDIF
 JU=UTL_GETUNIT()
 CALL OSD_OPEN(JU,FILE=SDF_TO,ACTION='WRITE',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot create scenario definition file: '//TRIM(SDF_TO),'Error')
  RETURN
 ENDIF
 ...
 REAL(KIND=DP_KIND) :: XMIN,YMIN,XMAX,YMAX,CS,MAXCS,BUFFER
 LOGICAL :: LEX
 CHARACTER(LEN=256) :: LINE
 ...
 IF(.NOT.LEX)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot find runfile: '//TRIM(RUNFILE),'Error')
  RETURN
 ENDIF

 CALL OSD_OPEN(IU,FILE=RUNFILE,ACTION='READ,DENYWRITE',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot open runfile: '//TRIM(RUNFILE),'Error')
  RETURN
 ENDIF
 JU=UTL_GETUNIT()
 CALL OSD_OPEN(JU,FILE=TRIM(OUTPUT)//'\imodflow.run',ACTION='WRITE',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot create runfile: '//TRIM(OUTPUT)//'\imodflow.run','Error')
  RETURN
 ENDIF
...
!  WRITE(*,*) 'factor ...',FCT,TRIM(LINE),IOS
  !## error occured
  IF(IOS.NE.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot read impulse in key: '//CHAR(13)//TRIM(LINE),'Error')
   RETURN
  ENDIF
  !## get multiplyer
