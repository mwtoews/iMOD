...
USE MOD_IDF, ONLY : IDFDEALLOCATE,IDFWRITE,IDFREAD,IDFGETVAL,IDFCOPY,IDFGETLOC,IDFIROWICOL
...
! CALL IR1FIELDS_GETIPERIRES(PERRES(I),IPER,IRES)
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,ILIST,IPOL,ITARGET,IDEF,IROW,ICOL,MXCRD,IC1,IC2,IR1,IR2,IPER,IRES,ITREE,IFIELD
 ...
 ALLOCATE(XPOL(MXCRD),YPOL(MXCRD))
...
!   TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_LO=0
!   TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_UP=0
...
!WRITE(*,*) idef,ipol,TRIM(IDFRESLIST(ILIST))//'.IDF'
!WRITE(*,*) TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%LOWER,TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%UPPER
       ...
       DO IROW=IR1,IR2
        DO ICOL=IC1,IC2
         ...
         !## inside polygon?
         IF(UTL_INSIDEPOLYGON(X,Y,TTREE(ITARGET)%POL(IPOL)%X,TTREE(ITARGET)%POL(IPOL)%Y,XPOL,YPOL, &
...
!           IF(XRESULT.GE.TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%LOWER)THEN
!            TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_LO=TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_LO+1
!           ENDIF
!           IF(XRESULT.LE.TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%UPPER)THEN
!            TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_UP=TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_UP+1
!           ENDIF
...
!    IF(TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%POLSIZE.GT.0)THEN
!     XX=REAL(TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_UP)
!     XX=(XX*100.0)/YY
!     TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_UP=INT(XX)
!     XX=REAL(TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_LO)
!     XX=(XX*100.0)/YY
!     TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_LO=INT(XX)
!    ELSE
!     TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_UP=0
!     TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_LO=0
!    ENDIF
 ...
 DO IROW=1,IDFDIFF(2)%NROW
  DO ICOL=1,IDFDIFF(2)%NCOL
   ...
   DO IPOL=1,TTREE(ITARGET)%NPOL
    !## inside polygon?
    IF(UTL_INSIDEPOLYGON(X,Y,TTREE(ITARGET)%POL(IPOL)%X,TTREE(ITARGET)%POL(IPOL)%Y,XPOL,YPOL,TTREE(ITARGET)%POL(IPOL)%NCRD).EQ.1)THEN
     IP(ICOL,IROW)=INT(1,1)
     DO IDEF=1,TTREE(ITARGET)%POL(IPOL)%NDEF
!      !## aimed percentage
!      YY=TTREE(ITARGET)%POL(IPOL)%EFFECT
...
!       !## put value inside entire polygon - lower target
!       XX=TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_LO
!       !## target achieved?
!       IF(XX.GE.YY)THEN
!        XX=100.0/(2.0*REAL(TTREE(ITARGET)%POL(IPOL)%NDEF)) !## to make it 100%
!       ELSE
!        XX=0.0
!       ENDIF
!       IDFDIFF(2)%X(ICOL,IROW)=IDFDIFF(2)%X(ICOL,IROW)+XX

!       !## put value inside entire polygon - upper target
!       XX=TTREE(ITARGET)%POL(IPOL)%DEF(IDEF)%REFFECT_UP
!       !## target achieved?
!       IF(XX.GE.YY)THEN
!        XX=100.0/(2.0*REAL(TTREE(ITARGET)%POL(IPOL)%NDEF)) !## to make it 100%
!       ELSE
