...
END FUNCTION MSPINSPECTOR_OPENFILES_AREASVAT
 ...
 CALL WDIALOGSELECT(ID_DMSPANALYSER_TAB4) ; CALL WGridClear(IDF_GRID1); CALL WGridClear(IDF_GRID2); CALL WGridClear(IDF_GRID3)
 ...
 INTEGER :: ICOL,IROW,JROW,JCOL,SVAT,SVATID,T,I

 CALL IGRCOLOURN(UTL_INVERSECOLOUR(WRGB(0,0,255)))

 CALL IDFIROWICOL(MSPIDF,IROW,ICOL,MOUSEX,MOUSEY)
 SVATID=0    
 !## start put SVAT DATA
 DO I=1,IDFSVAT%MXID
  IF(IROW.EQ.IDFSVAT%INFO(I)%IROW.AND.ICOL.EQ.IDFSVAT%INFO(I)%ICOL)THEN
     SVATID=IDFSVAT%INFO(I)%NUND 
     CALL WDIALOGSELECT(ID_DMSPANALYSER_TAB4)
     CALL WGridPutCellInteger(IDF_GRID1,1,1,IDFSVAT%INFO(I)%IROW)
     CALL WGridPutCellInteger(IDF_GRID1,2,1,IDFSVAT%INFO(I)%ICOL)
 ...
 CALL WDIALOGSELECT(ID_DMSPANALYSER_TAB4)
 CALL UTL_PLOT1BITMAP()
 !## remove drawn rectangles
 T=1
 DO 
  IF(WInfoGridCell(IDF_GRID3,1,T,1).EQ.0)EXIT
  CALL WGridGetCellInteger(IDF_GRID3,1,T,JROW)
  CALL WGridGetCellInteger(IDF_GRID3,2,T,JCOL)
  CALL IDFGETEDGE(MSPIDF,JROW,JCOL,X1,Y1,X2,Y2)
  CALL DBL_IGRRECTANGLE(X1,Y1,X2,Y2,IOFFSET=1)
  T=T+1
 ENDDO
 ...
 !## fill GRID drawn new rectangles
 T=0
 DO I=1,SCAPSVAT%MXID
  IF(SVATID.EQ.SCAPSVAT%INFO(I)%SVATAB)then
     T=T+1
     JROW=IDFSVAT%INFO(i)%IROW
     JCOL=IDFSVAT%INFO(i)%ICOL
