...
END FUNCTION MSPINSPECTOR_OPENFILES_AREASVAT
 ...
 ALLOCATE(IDFSVAT%LABEL(2),IDFSVAT%IACT(2))
 
 IDFSVAT%LABEL(1)='SVAT-ID'
 IDFSVAT%LABEL(2)='Iniltration-capacity'
 IDFSVAT%IACT=1
 ...
 DO I=1,2
  N=1
  DO
    READ(IU,'(I6,1X,A19,I6,F6.0,11F8.0,3F6.0)',IOSTAT=IOS) LUSESVAT%INFO(N)%LU,LUSESVAT%INFO(N)%LUNA,LUSESVAT%INFO(N)%VGLU,&
 ...
 INTEGER :: ICOL,IROW,JROW,JCOL,SVAT,IDFSVAT_I,SVATID,T,I
 ...
 !## start put SVAT DATA
 DO IDFSVAT_I=1,IDFSVAT%MXID
 IF(IROW.EQ.IDFSVAT%INFO(IDFSVAT_I)%IROW.AND.ICOL.EQ.IDFSVAT%INFO(IDFSVAT_I)%ICOL)then
     SVATID=IDFSVAT%INFO(IDFSVAT_I)%NUND 
     CALL WDIALOGSELECT(ID_DMSPANALYSER_TAB4)
     CALL WGridPutCellInteger(IDF_GRID1,1,1,IDFSVAT%INFO(IDFSVAT_I)%IROW)
     CALL WGridPutCellInteger(IDF_GRID1,2,1,IDFSVAT%INFO(IDFSVAT_I)%ICOL)
     CALL WGridPutCellInteger(IDF_GRID1,3,1,SVATID)  

     CALL WDIALOGSELECT(ID_DMSPANALYSER_TAB2)
     CALL WGridPutCellInteger(IDF_GRID1,2,1,SVATID)
     CALL WGridPutCellInteger(IDF_GRID1,2,2,SVATID)
     CALL WGridPutCellInteger(IDF_GRID4,1,1,SVATID)
     CALL WGridPutCellInteger(IDF_GRID5,1,1,SVATID)
           
     CALL WGridPutCellInteger(IDF_GRID2,1,1,SVATID)
     CALL WGridPutCellInteger(IDF_GRID2,2,1,IDFSVAT%INFO(IDFSVAT_I)%IROW)
     CALL WGridPutCellInteger(IDF_GRID2,3,1,IDFSVAT%INFO(IDFSVAT_I)%ICOL)
     EXIT 
 ENDIF
 ENDDO
 ...
 DO I=1,SCAPSVAT%MXID
  IF(SVATID.EQ.SCAPSVAT%INFO(I)%SVATAB)then
     T=T+1
     CALL WGridPutCellInteger(IDF_GRID3,1,T,IDFSVAT%INFO(i)%IROW)
     CALL WGridPutCellInteger(IDF_GRID3,2,T,IDFSVAT%INFO(i)%ICOL)
     CALL WGridPutCellInteger(IDF_GRID3,3,T,IDFSVAT%INFO(I)%NUND)
  ENDIF
 ENDDO
