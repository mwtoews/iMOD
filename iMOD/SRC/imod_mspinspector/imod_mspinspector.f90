...
! --> roept aan        ASC2IDF_IMPORTASC(IDFNAMES(I),TOP,BOT,IERROR,IBATCH)
 ...
 IF(CODE.EQ.ID_SAVE)THEN
  ...
  DO I=1,SIZE(IDFSVAT%IACT)  ; WRITE(IU,'(A)') TRIM(MSFILES(2))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(IDFSVAT%IACT(I))) //'    !## '//TRIM(IDFSVAT%LABEL(I))   ; END DO
  DO I=1,SIZE(MODSVAT%IACT)  ; WRITE(IU,'(A)') TRIM(MSFILES(3))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(MODSVAT%IACT(I))) //'    !## '//TRIM(MODSVAT%LABEL(I))   ; END DO
