...
!###======================================================================
 ...
 DO K=1,2
  ...
  DO I=1,SIZE(DXC%LABEL);      IF(DXC%IACT(I).EQ.0)CYCLE      ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,DXC%LABEL(I))    ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,DXC%INSPVAL(I))   
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,DXC%UNIT(I))     ; CALL WGRIDPUTCELLOPTION(IDF_GRID1,4,J,0); ENDIF ; END DO
  DO I=1,SIZE(MODSVAT%LABEL);  IF(MODSVAT%IACT(I).EQ.0)CYCLE  ; N=N+1; J=J+1;  
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,MODSVAT%LABEL(I)) ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,MODSVAT%INSPVAL(I)) 
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,MODSVAT%UNIT(I))  ; CALL WGRIDPUTCELLOPTION(IDF_GRID1,4,J,0); ENDIF ; END DO
  DO I=1,SIZE(IDFSVAT%LABEL);  IF(IDFSVAT%IACT(I).EQ.0)CYCLE  ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,IDFSVAT%LABEL(I)) ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,IDFSVAT%INSPVAL(I))
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,IDFSVAT%UNIT(I))  ; CALL WGRIDPUTCELLOPTION(IDF_GRID1,4,J,0); ENDIF ; END DO
  DO I=1,SIZE(AREASVAT%LABEL); IF(AREASVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,AREASVAT%LABEL(I)); CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,AREASVAT%INSPVAL(I)) 
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,AREASVAT%UNIT(I)) ; CALL WGRIDPUTCELLOPTION(IDF_GRID1,4,J,0); ENDIF ; END DO
  DO I=1,SIZE(LUSESVAT%LABEL); IF(LUSESVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,LUSESVAT%LABEL(I)); CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,LUSESVAT%INSPVAL(I)); 
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,LUSESVAT%UNIT(I)) ; CALL WGRIDPUTCELLOPTION(IDF_GRID1,4,J,0); ENDIF ; END DO
  DO I=1,SIZE(FACTSVAT%LABEL); IF(FACTSVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,FACTSVAT%LABEL(I)); CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,FACTSVAT%INSPVAL(I)); 
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,FACTSVAT%UNIT(I)) 
  ...
  DO I=1,SIZE(INFISVAT%LABEL); IF(INFISVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,INFISVAT%LABEL(I)); CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,INFISVAT%INSPVAL(I)); 
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,INFISVAT%UNIT(I)) ; CALL WGRIDPUTCELLOPTION(IDF_GRID1,4,J,0); ENDIF ; END DO
  DO I=1,SIZE(SCAPSVAT%LABEL); IF(SCAPSVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,SCAPSVAT%LABEL(I)); CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,SCAPSVAT%INSPVAL(I)); 
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,SCAPSVAT%UNIT(I)) ; CALL WGRIDPUTCELLOPTION(IDF_GRID1,4,J,0); ENDIF ; END DO
  DO I=1,SIZE(METEGRID%LABEL); IF(METEGRID%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,METEGRID%LABEL(I)); CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,METEGRID%INSPVAL(I)); 
                      CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,METEGRID%UNIT(I)) ; 
...
END FUNCTION MSPINSPECTOR_OPENFILES_AREASVAT
 ...
 IF(LEN(LUSESVAT%INSPVAL).EQ.0)THEN
         DXC%INSPVAL=' ' ; MODSVAT%INSPVAL=' '  ; IDFSVAT%INSPVAL=' '  ; AREASVAT%INSPVAL=' ' ; LUSESVAT%INSPVAL=' '
    FACTSVAT%INSPVAL=' ' ; INFISVAT%INSPVAL=' ' ; SCAPSVAT%INSPVAL=' ' ; METEGRID%INSPVAL=' '   
 !ELSE
! (/'DXC', 'MOD2SVAT','IDF_SVAT','AREA_SVAT','LUSE_SVAT','FACT_SVAT','INFI_SVAT','SCAP_SVAT','METE_GRID'/)   !## INP files not needed: PARASIM, TIOPSIM
    
    !         DXC%INSPVAL=' ' ;   READ(IU,*,IOSTAT=IOS) DXC%INFO(I)%ILAY,DXC%INFO(I)%IROW,DXC%INFO(I)%ICOL,DXC%INFO(I)%MFID
    !     MODSVAT%INSPVAL=' '  ; READ(IU,'(I10,2X,I10,I5)',IOSTAT=IOS) MODSVAT%INFO(N)%MFID,MODSVAT%INFO(N)%SVATID,MODSVAT%INFO(N)%LYBE

    !     IDFSVAT%INSPVAL=' '  ;    READ(IU,'(3I10)',IOSTAT=IOS) IDFSVAT%INFO(N)%NUND,IDFSVAT%INFO(N)%IROW,IDFSVAT%INFO(N)%ICOL 
    !
    !     AREASVAT%INSPVAL=' ' ;    READ(IU,'(I10,F10.1,2F8.3,I6,16X,I6,F8.3,I10,2F8.3)',IOSTAT=IOS) AREASVAT%INFO(N)%NUND,AREASVAT%INFO(N)%ARK, &
    ! AREASVAT%INFO(N)%SURF,AREASVAT%INFO(N)%TEMP,AREASVAT%INFO(N)%SOIL,AREASVAT%INFO(N)%LUSE,AREASVAT%INFO(N)%RZ, &
    ! AREASVAT%INFO(N)%METE,AREASVAT%INFO(N)%LCFPREP,AREASVAT%INFO(N)%LCFPOT
    !
    !     LUSESVAT%INSPVAL=' '    READ(IU,'(I6,1X,A19,I6,F6.0,10F8.0,1F8.2,3F6.0)',IOSTAT=IOS) LUSESVAT%INFO(N)%LU,LUSESVAT%INFO(N)%LUNA,LUSESVAT%INFO(N)%VGLU,&
    !   LUSESVAT%INFO(N)%ALPHACRIT,LUSESVAT%INFO(N)%P1FD,LUSESVAT%INFO(N)%P2FD,LUSESVAT%INFO(N)%P3HFD,LUSESVAT%INFO(N)%P3LFD,&
    !   LUSESVAT%INFO(N)%P4FD,LUSESVAT%INFO(N)%T3HFD,LUSESVAT%INFO(N)%T3LFD,LUSESVAT%INFO(N)%PBGSPLU,LUSESVAT%INFO(N)%FREVSPLU,&
    !   LUSESVAT%INFO(N)%GISPLU,LUSESVAT%INFO(N)%TIGISPLU,LUSESVAT%INFO(N)%RPSPLU,LUSESVAT%INFO(N)%TDBGSPLU,LUSESVAT%INFO(N)%TDEDSPLU
    !
    !FACTSVAT%INSPVAL=' ' ;     READ(IU,'(2I6,9F8.0)',IOSTAT=IOS) FACTSVAT%INFO(N)%VG,FACTSVAT%INFO(N)%DY,FACTSVAT%INFO(N)%CSVG,FACTSVAT%INFO(N)%LAIVG, &
    !                      FACTSVAT%INFO(N)%VXICVG,FACTSVAT%INFO(N)%FAEVVG,FACTSVAT%INFO(N)%FAEIVG,FACTSVAT%INFO(N)%FAEBSVG,    &
    !                      FACTSVAT%INFO(N)%FAEPDVG,FACTSVAT%INFO(N)%CHVG,FACTSVAT%INFO(N)%DRPZVG
    !
    !INFISVAT%INSPVAL=' ' ;    READ(IU,'(I10,F8.0)',IOSTAT=IOS) INFISVAT%INFO(N)%NUND,INFISVAT%INFO(N)%QINBASIC,INFISVAT%INFO(N)%CTOP_DOWN, & 
    !                                INFISVAT%INFO(N)%CTOP_UP,INFISVAT%INFO(N)%CBOT,INFISVAT%INFO(N)%SC2
    !
    !SCAPSVAT%INSPVAL=' ' ;    READ(IU,'(I10,4F8.0,I10,I6,I10)',IOSTAT=IOS) SCAPSVAT%INFO(N)%NUND,SCAPSVAT%INFO(N)%FMMXABGW,SCAPSVAT%INFO(N)%FMMXABSW,&
    ! SCAPSVAT%INFO(N)%FXABGW,SCAPSVAT%INFO(N)%FXABSW,SCAPSVAT%INFO(N)%SVATAB,SCAPSVAT%INFO(N)%LYAB,SCAPSVAT%INFO(N)%SWNRAB
    !
    !METEGRID%INSPVAL=' '       READ(IU,*,IOSTAT=IOS) METEGRID%INFO(N)%TD,METEGRID%INFO(N)%IY,METEGRID%INFO(N)%PRECGRID,METEGRID%INFO(N)%ETREFGRID
