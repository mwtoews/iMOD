...
USE MOD_IDF, ONLY : IDFNULLIFY,IDFALLOCATEX,IDFGETEDGE,IDFIROWICOL,IDFGETDXDY,IDFGETLOC,IDFCOPY,IDFWRITE,IDFGETVAL,IDFREAD,IDFCHECKRC
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_CAP,UTL_IMODFILLMENU,ITIMETOGDATE,RTOS,UTL_PLOT1BITMAP,UTL_PLOT2BITMAP,UTL_FILLDATES,LISTNAME,UTL_INVERSECOLOUR,UTL_GDATE,UTL_GETDAYANDMONTHFROMDAYNUMBER, &
   UTL_FILLDATESDIALOG,UTL_DEBUGLEVEL,UTL_GETHELP
...
CONTAINS
   ...
   SELECT CASE (ITYPE)
    ...
    CASE (PUSHBUTTON)
     SELECT CASE (MESSAGE%VALUE1)
      ...
      CASE (IDHELP)
       CALL UTL_GETHELP('MetaSWAP Analyser','TMO.MspAna')
      CASE (IDCANCEL)
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (FIELDCHANGED)
   ...
   IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
    SELECT CASE (MESSAGE%VALUE1)
     !## switch between Rural  / Urban info in grid
     CASE (IDF_RADIO1,IDF_RADIO2)
      !## Check if Inspector has selected a cell, which cell 
      ...
      IF(WInfoGridCell(IDF_GRID2,2,1,1).EQ.1)THEN
        ...
        !## if cell is selected put parameter values for selected radio button
        IF(MOUSECOL*MOUSEROW.NE.0) CAll MSPINSPECTOR_GETXY_PUTVALUES(MOUSEROW,MOUSECOL)  
      ENDIF 
    END SELECT
   ENDIF
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: II,I,J,M,N,WID,FTYPE,STYPE,ICOL,IROW,IERROR,NG,IVEGTYPE,I1,I2,IOS,ITYPE,JDATE
 ...
 DO I=3,SIZE(METEGRID%IACT)
  ...
  GRAPHDIM(NG)%GRAPHNAMES=TRIM(METEGRID%LABEL(I))
  ...
  N=0; DO J=1,METEGRID%MXID
   ...
   IF(METEGRID%INFO(J)%JD.GE.UTL_IDATETOJDATE(MSPSDATE))THEN
    ...
    SELECT CASE (FTYPE)
     !## asc->idf, idf
     CASE (-2,3)
      IF(I.EQ.3)METEO%FNAME=METEGRID%INFO(J)%PRECGRID( :INDEX(METEGRID%INFO(J)%PRECGRID,'.',.TRUE. )-1)//'.IDF'
      IF(I.EQ.4)METEO%FNAME=METEGRID%INFO(J)%ETREFGRID(:INDEX(METEGRID%INFO(J)%ETREFGRID,'.',.TRUE.)-1)//'.IDF'
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You need to apply the Inspector first','Error'); RETURN
 ENDIF
 ...
 DO I=1,SIZE(FACTSVAT%IACT)
  ...
  IF(LDATE)THEN
   JDATE=INT(GRAPH(M,1)%RX(1))
   II=I1+UTL_GETDAYNUMBERFROMJD(JDATE)-1
  ELSE
   II=I1-1
  ENDIF
  ...
  DO J=1,N
   ...
   IF(II.GT.I2)THEN
    IF(LDATE)THEN
     JDATE=INT(GRAPH(M,1)%RX(1))
     II=I1+UTL_GETDAYNUMBERFROMJD(JDATE)
    ELSE
     II=I1
    ENDIF
   ENDIF
   SELECT CASE (I)
    CASE (1); XV=FACTSVAT%INFO(II)%VG
    CASE (2); XV=FACTSVAT%INFO(II)%DY
    CASE (3); XV=FACTSVAT%INFO(II)%CSVG
    CASE (4); XV=FACTSVAT%INFO(II)%LAIVG
    CASE (5); XV=FACTSVAT%INFO(II)%VXICVG
    CASE (6); XV=FACTSVAT%INFO(II)%FAEVVG
    CASE (7); XV=FACTSVAT%INFO(II)%FAEIVG
    CASE (8); XV=FACTSVAT%INFO(II)%FAEBSVG
    CASE (9); XV=FACTSVAT%INFO(II)%FAEPDVG
    CASE (10); XV=FACTSVAT%INFO(II)%CHVG
    CASE (11); XV=FACTSVAT%INFO(II)%DRPZVG
   END SELECT
   IF(LDATE)     GRAPH(M,NG)%RX(J)=GRAPH(M,1)%RX(J) !DBLE(METEGRID%INFO(J)%JD)
 ...
 !## start graph
 CALL GRAPH_INIT(2,LMULT=.TRUE.,SUBTITLE='svat-nr: '//TRIM(AREASVAT%INSPVAL(1))//', landuse-nr: '//TRIM(AREASVAT%INSPVAL(6)))  
 !CALL WBitmapSave(IHANDLE,'image.png')
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,JJ,JCOL,JROW,ILAY,NLAY,JSVAT,NPRGS,IPRGS,MFID
 INTEGER :: DROW,DCOL  ! offset of Modflow window compared to MetaSWAP winoow
 TYPE(IDFOBJ) :: MSPLUSE,MSPMFID
 LOGICAL :: LEX
 ...
 CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'FILE SVAT_RURAL.IDF...') ; IF(.NOT.IDFWRITE(SVATRU,TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT_RURAL.IDF',1))THEN ; ENDIF 
 CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'FILE SVAT_URBAN.IDF...') ; IF(.NOT.IDFWRITE(SVATUR,TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT_URBAN.IDF',1))THEN ; ENDIF 
 CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'FILE SVAT_IRRIGATION.IDF...') ; IF(.NOT.IDFWRITE(SVATIR,TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT_IRRIGATION.IDF',1))THEN ; ENDIF 
 !CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'FILE SVAT_IRRIGATION-SOURCE.IDF...') ; IF(.NOT.IDFWRITE(SVATIRS,TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT_IRRIGATION-SOURCE.IDF',1))THEN ; ENDIF 
 !CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'FILE SCAP_SVAT_LAY.IDF...') ; IF(.NOT.IDFWRITE(SCAPSVAT_LYAB,TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SCAP_SVAT_LAY.IDF',1))THEN ; ENDIF 
 CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT_RURAL.idf')
 CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT_URBAN.idf')
 CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT_IRRIGATION.idf')
 !CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT_IRRIGATION-SOURCE.idf')

 !## landuse
 CALL IDFCOPY(MSPIDF,MSPLUSE)
 CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'create MSPLUSE.idf...')
 JJ=1 ; IPRGS=0
 DO I=1,AREASVAT%MXID
   ...
   ENDIF 
 ENDDO    
 FNAME=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\MSPLUSE.idf' ; IF(.NOT.IDFWRITE(MSPLUSE,FNAME,1))THEN ; ENDIF ;  CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=FNAME)
 CALL IDFDEALLOCATEX(MSPLUSE)
 ...
 !## MODFLOW ID per layer
 CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'create IDF Modflow ID per layer...') ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,3,ABSOLUTE)
 CALL IDFCOPY(MSPIDF,MSPMFID)
 NLAY=MAXVAL(MODSVAT%INFO%LY)
 !## get offset ModflowWINDOW
 JSVAT=IDFSVAT%INFO(1)%SVAT
 CALL MSPINSPECTOR_SVAT2ROWCOL(JSVAT,JROW,JCOL,MFID)
 CALL MSPINSPECTOR_MFID2LAYROWCOL(MFID,ILAY,JROW,JCOL)
 DROW=JROW-1 ; DCOL=JCOL-1
 
 DO ILAY=1,NLAY
   FNAME=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\ModflowID_L'//TRIM(ITOS(ILAY))//'.idf'  
   CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'create '//TRIM(FNAME))
   IPRGS=0
   LEX=.FALSE. 
   MSPMFID%X=MSPMFID%NODATA
   DO I=1,MODSVAT%MXID
     IF(INT(I*NPRGS/MODSVAT%MXID).GT.IPRGS) THEN ; IPRGS = INT(I*NPRGS/MODSVAT%MXID) ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,IPRGS,ABSOLUTE) ; ENDIF
     IF(ILAY.EQ.MODSVAT%INFO(I)%LY)THEN 
       MFID=MODSVAT%INFO(I)%MFID 
       !# assumption: DXC is complete and sorted. MFID is also record number 
       !CALL MSPINSPECTOR_MFID2LAYROWCOL(MFID,J,JROW,JCOL)
       JROW=DXC%INFO(MFID)%IROW 
       JCOL=DXC%INFO(MFID)%ICOL 
       IF(IDFCHECKRC(MSPMFID,JROW-DROW,JCOL-DCOL))THEN
          MSPMFID%X(JCOL-DCOL,JROW-DROW)=MFID
          LEX=.TRUE.  
       ENDIF
     ENDIF  
   ENDDO    
   IF(LEX)THEN
      IF(.NOT.IDFWRITE(MSPMFID,FNAME,1))THEN ; ENDIF !; CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=FNAME)
   ENDIF       
 ENDDO    
 CALL IDFDEALLOCATEX(MSPMFID)
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (PUSHBUTTON)
   IF(MESSAGE%VALUE1.EQ.MESSAGE%VALUE2)THEN
    SELECT CASE (MESSAGE%VALUE1)
     !## switch Irrigation source/target on/off
     CASE (IDF_CHECK1)
      !## Check if Inspector has selected a cell, which cell 
      CALL WDIALOGSELECT(ID_DMSPANALYSER_TAB2)
      IF(WInfoGridCell(IDF_GRID2,2,1,1).EQ.1)THEN
        ...
        CALL WGRIDGETCELLINTEGER(IDF_GRID2,2,1,MOUSEROW)
        CALL WGRIDGETCELLINTEGER(IDF_GRID2,3,1,MOUSECOL)
        !## if cell is selected put parameter values for all grids
        IF(MOUSECOL*MOUSEROW.NE.0) CAll MSPINSPECTOR_GETXY_PUTVALUES(MOUSEROW,MOUSECOL)  
      ENDIF 
    END SELECT
   ENDIF
 ...
 INTEGER,INTENT(IN) :: ITYPE
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    !## load and save of settings file
 ...
 IF(CODE.EQ.ID_SAVE)THEN
  ...
  DO I=1,SIZE(SVAT2SWNRROFF%IACT); WRITE(IU,'(A)') TRIM(MSFILES(12))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(SVAT2SWNRROFF%IACT(I)))//'    !## '//TRIM(SVAT2SWNRROFF%LABEL(I))  ; END DO
 ...
 ELSEIF(CODE.EQ.ID_OPEN)THEN
  ...
  DO I=1,SIZE(SVAT2SWNRROFF%IACT); IF(UTL_READINITFILE(TRIM(MSFILES(12))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) SVAT2SWNRROFF%IACT(I) ; END DO
 ...
 DO K=1,2
  ...
  DO I=1,SIZE(SVAT2SWNRROFF%LABEL);      IF(SVAT2SWNRROFF%IACT(I).EQ.0)CYCLE      ; N=N+1; J=J+1; 
     IF(K.EQ.2)THEN ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,SVAT2SWNRROFF%LABEL(I))     ; CALL WGRIDPUTCELLSTRING(IDF_GRID1,3,J,SVAT2SWNRROFF%UNIT(I))     ; CALL WGRIDPUTCELLOPTION(IDF_GRID1,4,J,0); ENDIF ; END DO
  TAB3GRIDSIZE=J
 ...
 DO K=1,2
  ...
  DO I=1,SIZE(SVAT2SWNRROFF%LABEL); IF(SVAT2SWNRROFF%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1;  IF(K.EQ.2) CALL WGRIDPUTCELLSTRING(IDF_GRID1,2,J,SVAT2SWNRROFF%INSPVAL(I)); END DO
  TAB3GRIDSIZE=J
!  IF(K.EQ.1) THEN ; CALL WGRIDROWS(IDF_GRID1,MAX(1,N)) ; CALL WGRIDSTATE(IDF_GRID1,4,DialogReadOnly) ; ENDIF
  IF(K.EQ.1) THEN ; CALL WGRIDROWS(IDF_GRID1,MAX(1,N))  ; ENDIF
 ...
 DO I=1,SIZE(SVAT2SWNRROFF%LABEL); IF(SVAT2SWNRROFF%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; END DO
 ...
 SELECT CASE (I)
  ...
  CASE (12)
   CALL WDIALOGPUTMENU(IDF_MENU2,SVAT2SWNRROFF%LABEL,SIZE(SVAT2SWNRROFF%LABEL),SVAT2SWNRROFF%IACT)
  END SELECT
 ...
 SELECT CASE (I)
  ...
  CASE (12)
   CALL WDIALOGGETMENU(IDF_MENU2,SVAT2SWNRROFF%IACT)
  END SELECT
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\AREA_SVAT.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\PARA_SIM.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\IDF_SVAT.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\INFI_SVAT.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\SCAP_SVAT.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\LUSE_SVAT.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\METE_GRID.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\FACT_SVAT.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\TIOP_SIM.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\MOD2SVAT.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\SVAT2PRECGRID.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\SVAT2ETREFGRID.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 ...
 FNAME='\'//TRIM(MNAME)//'\METASWAP\SVAT2SWNR_ROFF.INP'
 I=I+1 ; CALL WDIALOGSELECT(ID_DIRPROGRESS) ; CALL WDIALOGPUTPROGRESSBAR(IDF_PROGRESS1,I,0) ; CALL WDIALOGPUTSTRING(IDF_LABEL1,TRIM(FNAME))
 IF(.NOT.MSPINSPECTOR_OPENFILES_SVAT2SWNRROFF(IU))THEN
  INQUIRE(UNIT=IU,OPENED=LEX); IF(LEX)CLOSE(IU)
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open file '//CHAR(13)//TRIM(ROOT)//TRIM(FNAME),'Error'); RETURN
 ENDIF
 ...
 DXC%LABEL(2)='Row number (Modflow area)' ;  DXC%UNIT(2)='-'
 DXC%LABEL(3)='Column number (Modflow area)' ;  DXC%UNIT(3)='-'
 ...
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\PARA_SIM.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 !## allocate memory - mainly number of svats
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\MOD2SVAT.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 !## allocate memory - mainly number of svats
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\IDF_SVAT.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 IDFSVAT%LABEL(2)='Row number (MetaSWAP area)' ;  IDFSVAT%UNIT(2)='-'
 IDFSVAT%LABEL(3)='Column number (MetaSWAP area)' ;  IDFSVAT%UNIT(3)='-'
 IDFSVAT%LABEL(4)='X location of MetaSWAP cell' ;  IDFSVAT%UNIT(4)='-'
 IDFSVAT%LABEL(5)='Y location of MetaSWAP cell' ;  IDFSVAT%UNIT(5)='-'
 ...
 !## allocate memory - mainly number of svats
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\AREA_SVAT.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 AREASVAT%LABEL(5)='SLK-soil physical unit number' ;  AREASVAT%UNIT(5)='-'
...
END FUNCTION MSPINSPECTOR_OPENFILES_AREASVAT
 ...
 !## allocate memory - .......
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\INFI_SVAT.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 !## allocate memory - .......
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SCAP_SVAT.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 !## allocate memory - .......
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\LUSE_SVAT.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 !## allocate memory - .......
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\METE_GRID.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
...
! DN=INT(METEGRID%INFO(1)%TD); IY=METEGRID%INFO(1)%IY
! CALL UTL_GETDAYANDMONTHFROMDAYNUMBER(DN,IY,ID,IM); METEGRID%INFO(1)%JD=JD(IY,IM,ID)
 ...
 DO I=1,METEGRID%MXID
  METEGRID%INFO(I)%JD=0
  !## does not support timesteps < 1 day
  IF(INT(METEGRID%INFO(I)%TD).EQ.0)CYCLE
  DN=INT(METEGRID%INFO(I)%TD); IY=METEGRID%INFO(I)%IY
 ...
 !## allocate memory - .......
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT2PRECGRID.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 !## allocate memory - .......
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\FACT_SVAT.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
...
! INTEGER :: YEAR,MONTH,DAY
 ...
 !## allocate memory - .......
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\TIOP_SIM.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
 ...
 !###======================================================================
 LOGICAL FUNCTION MSPINSPECTOR_OPENFILES_SVAT2SWNRROFF(IU)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(OUT) :: IU
 INTEGER :: I,IOS,N

 MSPINSPECTOR_OPENFILES_SVAT2SWNRROFF=.FALSE.

 !## allocate memory - .......
 IU=UTL_GETUNIT(); OPEN(IU,FILE=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SVAT2SWNR_ROFF.INP',STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN

 ALLOCATE(SVAT2SWNRROFF%INFO(1),STAT=IOS); IF(IOS.NE.0)RETURN
 SVAT2SWNRROFF%MXID=0
 DO I=1,2
  N=1
  DO
!    READ(IU,'(2I10,3F8.0,8X,F8.0)',IOSTAT=IOS) SVAT2SWNRROFF%INFO(N)%SVAT,SVAT2SWNRROFF%INFO(N)%SWNR,SVAT2SWNRROFF%INFO(N)%VXMU, &
!                                               SVAT2SWNRROFF%INFO(N)%CRUNOFF,SVAT2SWNRROFF%INFO(N)%CRUNON,SVAT2SWNRROFF%INFO(N)%SLPCN
    READ(IU,'(2I10,3F8.0)',IOSTAT=IOS) SVAT2SWNRROFF%INFO(N)%SVAT,SVAT2SWNRROFF%INFO(N)%SWNR,SVAT2SWNRROFF%INFO(N)%VXMU, &
                                       SVAT2SWNRROFF%INFO(N)%CRUNOFF,SVAT2SWNRROFF%INFO(N)%CRUNON
   IF(IOS.NE.0)EXIT
   IF(I.EQ.2)THEN ; N=N+1 ; IF(N.GT.SVAT2SWNRROFF%MXID)EXIT ; ELSE ; SVAT2SWNRROFF%MXID=SVAT2SWNRROFF%MXID+1 ; ENDIF
  ENDDO
  IF(I.EQ.1)THEN; DEALLOCATE(SVAT2SWNRROFF%INFO); ALLOCATE(SVAT2SWNRROFF%INFO(SVAT2SWNRROFF%MXID),STAT=IOS); IF(IOS.NE.0)RETURN; ENDIF
  REWIND(IU)
 ENDDO
 CLOSE(IU)

 ALLOCATE(SVAT2SWNRROFF%LABEL(5),SVAT2SWNRROFF%UNIT(5),SVAT2SWNRROFF%IACT(5),SVAT2SWNRROFF%INSPVAL(5))

 SVAT2SWNRROFF%LABEL(1)='SVAT-ID' ;  SVAT2SWNRROFF%UNIT(1)='-'
 SVAT2SWNRROFF%LABEL(2)='SWNR-surface water location identifier' ;  SVAT2SWNRROFF%UNIT(2)='-'
 SVAT2SWNRROFF%LABEL(3)='VXMU-micro-storage capacity' ;  SVAT2SWNRROFF%UNIT(3)='m'
 SVAT2SWNRROFF%LABEL(4)='CRUNOFF-runoff resistance' ;  SVAT2SWNRROFF%UNIT(4)='d'
 SVAT2SWNRROFF%LABEL(5)='CRUNON-runon resistance' ;  SVAT2SWNRROFF%UNIT(5)='d'
 !SVAT2SWNRROFF%LABEL(6)='SLPCN-slope fraction of soil surface (CNM)' ;  SVAT2SWNRROFF%UNIT(6)='m/m'

 SVAT2SWNRROFF%IACT=1 ; SVAT2SWNRROFF%INSPVAL=''
 
 MSPINSPECTOR_OPENFILES_SVAT2SWNRROFF=.TRUE.

 END FUNCTION MSPINSPECTOR_OPENFILES_SVAT2SWNRROFF
  
 !###======================================================================
 LOGICAL FUNCTION MSPINSPECTOR_PROCESSFILES()
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,JJ,N,NPRGS,IPRGS,IROW,ICOL,IERROR,ISVAT
 ...
 !## set starting date
 MSPSDATE=UTL_JDATETOIDATE(METEGRID%INFO(2)%JD); CALL UTL_FILLDATESDIALOG(ID_DMSPANALYSER_TAB5,IDF_INTEGER1,IDF_MENU5,IDF_INTEGER2,MSPSDATE)
 !## to date
 ...
 CALL IDFCOPY(MSPIDF,SVATRU) ;  CALL IDFCOPY(MSPIDF,SVATUR) ;  CALL IDFCOPY(MSPIDF,SVATIR) ; CALL IDFCOPY(MSPIDF,SVATIRS) 
 CALL IDFCOPY(MSPIDF,SCAPSVAT_LYAB)
 ...
 DO N=1,IDFSVAT%MXID
   ...
   !## find and fill irrigation IDF (both target and source SVAT and layer)     
   IF(AREASVAT%INFO(ISVAT)%REC_SCAPSVAT.GT.0)        SVATIR%X(ICOL,IROW)=DBLE(SCAPSVAT%INFO(AREASVAT%INFO(ISVAT)%REC_SCAPSVAT)%SVAT)
   IF(AREASVAT%INFO(ISVAT)%REC_SCAPSVAT.GT.0)       SVATIRS%X(ICOL,IROW)=DBLE(SCAPSVAT%INFO(AREASVAT%INFO(ISVAT)%REC_SCAPSVAT)%SVATAB)
   IF(AREASVAT%INFO(ISVAT)%REC_SCAPSVAT.GT.0) SCAPSVAT_LYAB%X(ICOL,IROW)=DBLE(SCAPSVAT%INFO(AREASVAT%INFO(ISVAT)%REC_SCAPSVAT)%LYAB)   
 ENDDO
...
! CALL MSPINSPECTOR_CLEANGRIDS()
 ...
 !## disable tab and bottons while hoovering
 CALL MSPINSPECTOR_HANDLEFIELD(0)
 ...
 DO
  ...
  SELECT CASE (ITYPE)
   ...
   CASE (MOUSEBUTDOWN)
    SELECT CASE (MESSAGE%VALUE1)
    CASE (RIGHTBUTTON)
      !## Exit hoover modus WITH removing drawn rectangle 
      IF(LEX)THEN
       CALL IGRCOLOURN(UTL_INVERSECOLOUR(WRGB(255,0,0)))
       CALL UTL_PLOT1BITMAP()
       CALL IDFGETEDGE(MSPIDF,IROW,ICOL,X1,Y1,X2,Y2)
      ...
      EXIT
     CASE (LEFTBUTTON)
      !## Exit hoover modus WITHOUT removing drawn rectangle 
      EXIT
    END SELECT
 ...
 CALL MSPINSPECTOR_HANDLEFIELD(1)
 ...
 INTEGER :: JROW,JCOL,T,I,J,N,IREXIST,SVATID,IMF,RADIO1,ISVAT,IREC
 INTEGER,DIMENSION(3+1) :: SVATINDEX_AREA ! Position of actual rural/irrigation/urban svat in AreaSVAT + selected SVAT on tab3
 INTEGER,DIMENSION(3+1) :: SVATINDEX_MOD  ! Position of actual rural/irrigation source/urban svat in MOD2SVAT + selected SVAT on tab3
 ...
 SVATINDEX_AREA(1)=INT(SVATRU%X(ICOL,IROW))  ! rural
 SVATINDEX_AREA(2)=INT(SVATIR%X(ICOL,IROW))  ! irrigation
 SVATINDEX_AREA(3)=INT(SVATUR%X(ICOL,IROW))  ! urban
 SVATINDEX_AREA(4)=0                         ! default

 !!## Find location/record of all SVAT ID's (max 2) from MOD2SVAT file
 SVATINDEX_MOD=0 
 DO I=1,MODSVAT%MXID
   IF(SVATINDEX_AREA(1).EQ.MODSVAT%INFO(I)%SVATID.AND.SVATINDEX_MOD(1).EQ.0)THEN
       SVATINDEX_MOD(1)=I    ! rural
       IF(SVATINDEX_AREA(2).EQ.MODSVAT%INFO(I)%SVATID.AND.SCAPSVAT_LYAB%X(ICOL,IROW).EQ.MODSVAT%INFO(I)%LY) SVATINDEX_MOD(2)=I    ! irrigation, might be overruled
       IF(I.EQ.MODSVAT%MXID) EXIT
       IF(SVATINDEX_AREA(2).EQ.MODSVAT%INFO(I+1)%SVATID.AND.SCAPSVAT_LYAB%X(ICOL,IROW).EQ.MODSVAT%INFO(I+1)%LY) SVATINDEX_MOD(2)=I+1  ! irrigation (source!)
   ENDIF    
   DO J=I,MODSVAT%MXID
      IF(SVATINDEX_AREA(3).EQ.MODSVAT%INFO(J)%SVATID) SVATINDEX_MOD(3)=J  ! urban
      EXIT
   ENDDO
 ENDDO
  
 !## start filling grids, first clean
 CALL MSPINSPECTOR_CLEANGRIDS()
 ...
 IF(SVATRU%X(ICOL,IROW).GT.0) THEN
     ...
     IF(SVATPREC%INFO(I)%SVAT.NE.I)THEN
      CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Expected SVAT='//TRIM(ITOS(SVATPREC%INFO(I)%SVAT))//' on line '//TRIM(ITOS(I))//' of file SVAT2PRECGRID.INP','Error'); RETURN
     ENDIF
 ...
 !## start put data (TAB3)
 !## for items 1. DXC / 2 MOD2SVAT / 3. IDF_SVAT / 4. AREA_SVAT / 5. LUSE_SVAT / 6. FACT_SVAT / 
 !##.          7. INFI_SVAT / 8. SCAP_SVAT / 9. METE_GRID / 10. SVATPREC / 11. SVATETREF
 ...
 IF((RADIO1.EQ.1.AND.SVATRU%X(ICOL,IROW).GT.0).OR.(RADIO1.EQ.2.AND.SVATUR%X(ICOL,IROW).GT.0))THEN 
    ...
    IF(RADIO1.EQ.1) THEN ; ISVAT=INT(SVATRU%X(ICOL,IROW)) ; SVATINDEX_MOD(4)=SVATINDEX_MOD(1) ; SVATINDEX_AREA(4)=SVATINDEX_AREA(1); ENDIF 
    IF(RADIO1.EQ.2) THEN ; ISVAT=INT(SVATUR%X(ICOL,IROW)) ; SVATINDEX_MOD(4)=SVATINDEX_MOD(3) ; SVATINDEX_AREA(4)=SVATINDEX_AREA(3); ENDIF
     ...
     !## find SVATPREC parameters (10)
     I=ISVAT  ! assumption: recordnumber is line number
     IF(ISVAT.NE.SVATPREC%INFO(I)%SVAT)THEN   
        CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Expected SVAT='//TRIM(ITOS(I))//' on line '//TRIM(ITOS(I))//' of file SVAT2PRECGRID.INP','Error'); RETURN ; ENDIF
     SVATPREC%INSPVAL(1)=ITOS(SVATPREC%INFO(I)%SVAT)
     SVATPREC%INSPVAL(2)=ITOS(SVATPREC%INFO(I)%ROW)
     SVATPREC%INSPVAL(3)=ITOS(SVATPREC%INFO(I)%COLUMN)
     
     !## find SVATETREF parameters (11)
     I=ISVAT  ! assumption: recordnumber is line number
     IF(ISVAT.NE.SVATETREF%INFO(I)%SVAT)THEN   
        CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Expected SVAT='//TRIM(ITOS(I))//' on line '//TRIM(ITOS(I))//' of file SVAT2ETREFGRID.INP','Error'); RETURN ; ENDIF
     SVATETREF%INSPVAL(1)=ITOS(SVATETREF%INFO(I)%SVAT)
     SVATETREF%INSPVAL(2)=ITOS(SVATETREF%INFO(I)%ROW)
     SVATETREF%INSPVAL(3)=ITOS(SVATETREF%INFO(I)%COLUMN)
     
     !## find SVAT2SWNRROFF parameters (12)
     I=ISVAT  ! assumption: recordnumber is line number
     IF(ISVAT.NE.SVAT2SWNRROFF%INFO(I)%SVAT)THEN   
        CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Expected SVAT='//TRIM(ITOS(I))//' on line '//TRIM(ITOS(I))//' of file SVAT2SWNR_ROFF.INP','Error'); RETURN ; ENDIF
     SVAT2SWNRROFF%INSPVAL(1)=ITOS(SVAT2SWNRROFF%INFO(I)%SVAT)
     SVAT2SWNRROFF%INSPVAL(2)=ITOS(SVAT2SWNRROFF%INFO(I)%SWNR)
     SVAT2SWNRROFF%INSPVAL(3)=UTL_REALTOSTRING(SVAT2SWNRROFF%INFO(I)%VXMU)
     SVAT2SWNRROFF%INSPVAL(4)=UTL_REALTOSTRING(SVAT2SWNRROFF%INFO(I)%CRUNOFF)
     SVAT2SWNRROFF%INSPVAL(5)=UTL_REALTOSTRING(SVAT2SWNRROFF%INFO(I)%CRUNON)
     !SVAT2SWNRROFF%INSPVAL(6)=UTL_REALTOSTRING(SVAT2SWNRROFF%INFO(I)%SLPCN)
ELSE
    DXC%INSPVAL=''
    MODSVAT%INSPVAL=''
    IDFSVAT%INSPVAL=''
...
IF(SVATIR%X(ICOL,IROW).NE.0)THEN  

  !## fill irrigation cell info   
  SVATID=SVATIR%X(ICOL,IROW) 
  ...
  CALL WGridPutCellINTEGER(IDF_GRID1,3,1,SVATID)
  CALL WGridPutCellINTEGER(IDF_GRID1,4,1,MODSVAT%INFO(SVATINDEX_MOD(2))%MFID) 
  
  !## fill irrigation cell info: source   
  IREC=AREASVAT%INFO(SVATID)%REC_SCAPSVAT
  IF(SCAPSVAT%INFO(IREC)%FMMXABSW.GT.0) CALL WGridPutCellSTRING(IDF_GRID1,5,1,'SW')
  IF(SCAPSVAT%INFO(IREC)%FMMXABGW.GT.0) THEN 
    CALL WGridPutCellSTRING(IDF_GRID1,5,1,'GW')
    CALL WGridPutCellINTEGER(IDF_GRID1,6,1,INT(SCAPSVAT_LYAB%X(ICOL,IROW)))  
    CALL MSPINSPECTOR_SVAT2ROWCOL(INT(SVATIRS%X(ICOL,IROW)),JROW,JCOL,IMF)
    CALL WGridPutCellINTEGER(IDF_GRID1,7,1,IMF)
  ENDIF  
    
  IF(SVATINDEX_AREA(4).GT.0)THEN ; LUSEINDEX=AREASVAT%INFO(SVATINDEX_AREA(4))%LUSE ; ELSE ; LUSEINDEX=0 ; ENDIF
  ...
  !## Find and Fill section 2 op Tab 4 - related IRR locations
  CALL WDIALOGGETCHECKBOX(IDF_CHECK1,IREXIST) !## check if section should be filled
  IREC=AREASVAT%INFO(SVATID)%REC_SCAPSVAT  ! Line in file SCAPSVAT
  IF(IREXIST.EQ.1.AND.SCAPSVAT%INFO(IREC)%FMMXABGW.GT.0)THEN  !## checkbox is on and irrigation source is Groundwater
    ...
    !## A. fill and draw irrigation source 
    SVATID=SCAPSVAT%INFO(IREC)%SVATAB   ! svat irr source  
    CALL MSPINSPECTOR_SVAT2ROWCOL(SVATID,JROW,JCOL,IMF)
    !K=0 
    DO J=SVATID,MODSVAT%MXID  !## search for record in MOD2SVAT
       !## change if content of MOD2SVAT is complete 
       IF(SCAPSVAT%INFO(IREC)%SVATAB.EQ.MODSVAT%INFO(J)%SVATID.AND.&
          SCAPSVAT%INFO(IREC)%LYAB.EQ.MODSVAT%INFO(J)%LY)THEN ; IREC=J; EXIT ; ENDIF
       !IF(SCAPSVAT%INFO(IREC)%SVATAB.EQ.MODSVAT%INFO(J)%SVATID)THEN
       !   K=J 
       !   IF(SCAPSVAT%INFO(IREC)%LYAB.EQ.MODSVAT%INFO(J)%LY)THEN ; K=J; EXIT ; ENDIF
       !ENDIF      
    ENDDO   
    !IREC=K   
    
    !## source cell: Display row/col
    IF(IDFCHECKRC(MSPIDF,JROW,JCOL))THEN
    ...
    !## sourve cell: Display rowIDs
    CALL WGRIDGETCELLINTEGER(IDF_GRID1,6,1,I) ; CALL WGridPutCellInteger(IDF_GRID3,3,1,I)
    CALL WGridPutCellInteger(IDF_GRID3,4,1,INT(SVATIRS%X(ICOL,IROW)))
    CALL WGRIDGETCELLINTEGER(IDF_GRID1,7,1,I) ; CALL WGridPutCellInteger(IDF_GRID3,5,1,I)

    
    !## B. fill and draw irrigation targets
    DO I=1,SCAPSVAT%MXID  
     IF(SVATID.EQ.SCAPSVAT%INFO(I)%SVATAB)THEN
       CALL MSPINSPECTOR_SVAT2ROWCOL(SCAPSVAT%INFO(I)%SVAT,JROW,JCOL,IMF)
       IF(IDFCHECKRC(MSPIDF,JROW,JCOL))THEN
         T=T+1
         IREC=0
         DO J=SCAPSVAT%INFO(I)%SVAT,MODSVAT%MXID
           IF(SCAPSVAT%INFO(I)%SVAT.EQ.MODSVAT%INFO(J)%SVATID)THEN ; IREC=J; EXIT ; ENDIF  ! 1st SVAT is irrigation SVAT, 2nd SVAT can be deeper irigation source
         ENDDO
         CALL WGridPutCellInteger(IDF_GRID4,1,T,JROW)
         CALL WGridPutCellInteger(IDF_GRID4,2,T,JCOL)
         CALL WGridPutCellInteger(IDF_GRID4,3,T,MODSVAT%INFO(IREC)%LY)
         CALL WGridPutCellInteger(IDF_GRID4,4,T,MODSVAT%INFO(IREC)%SVATID)
         CALL WGridPutCellInteger(IDF_GRID4,5,T,MODSVAT%INFO(IREC)%MFID)
...
! DO IROW=1,4 ; DO ICOL=2,2 ;  CALL WGRIDSTATECELL(IDF_GRID1,ICOL,IROW,DIALOGREADONLY) ; ENDDO ; ENDDO
 ...
 CALL WDIALOGSELECT(ID_DMSPANALYSER); CALL UTL_DIALOGSHOW(-1,-1,0,2)
