...
CONTAINS
 ...
 !CALL WDIALOGSELECT(ID_DIRPROGRESS); CALL WDIALOGPUTSTRING(IDF_LABEL1,'FILE SCAP_SVAT_LAY.IDF...') ; IF(.NOT.IDFWRITE(MSPSCAPSVAT,TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\SCAP_SVAT_LAY.IDF',1))THEN ; ENDIF 
...
! CALL MSPINSPECTOR_CLEANGRIDS()
 ...
 INTEGER,DIMENSION(3+1) :: SVATINDEX_AREA ! Position of rural/irrigation/urban svat in AreaSVAT + selected SVAT on tab3
 INTEGER,DIMENSION(3+1) :: SVATINDEX_MOD  ! Position of rural/irrigation source/urban svat in MOD2SVAT + selected SVAT on tab3
 ...
 DO I=1,MODSVAT%MXID
   IF(SVATINDEX_AREA(1).EQ.MODSVAT%INFO(I)%SVATID.AND.SVATINDEX_MOD(1).EQ.0)THEN
       ...
       IF(SVATINDEX_AREA(2).EQ.MODSVAT%INFO(I)%SVATID.AND.MSPSCAPSVAT%X(ICOL,IROW).EQ.MODSVAT%INFO(I)%LY) SVATINDEX_MOD(2)=I    ! irrigation, might be overruled
       IF(I.EQ.MODSVAT%MXID) EXIT
       IF(SVATINDEX_AREA(2).EQ.MODSVAT%INFO(I+1)%SVATID.AND.MSPSCAPSVAT%X(ICOL,IROW).EQ.MODSVAT%INFO(I+1)%LY) SVATINDEX_MOD(2)=I+1  ! irrigation (source!)
   ENDIF    
   DO J=I,MODSVAT%MXID
...
IF(SVATIR%X(ICOL,IROW).NE.0)THEN  
  ...
  CALL WGridPutCellINTEGER(IDF_GRID1,4,1,MODSVAT%INFO(SVATINDEX_MOD(1))%MFID) ! if cell is indicated IRRIGATION than the SVAT = SVAT RURAL
  ...
  IF(SCAPSVAT%INFO(IREC)%FMMXABGW.GT.0) THEN 
    ...
    CALL WGridPutCellINTEGER(IDF_GRID1,6,1,INT(MSPSCAPSVAT%X(ICOL,IROW)))  
    CALL WGridPutCellINTEGER(IDF_GRID1,7,1,MODSVAT%INFO(SVATINDEX_MOD(2))%MFID)! index 2 because Irrigation
  ENDIF  
  ...
  IF(IREXIST.EQ.1.AND.SCAPSVAT%INFO(IREC)%FMMXABGW.GT.0)THEN  !## checkbox is on and irrigation source is Groundwater
    ...
    CALL WGridPutCellInteger(IDF_GRID3,3,1,MODSVAT%INFO(IREC)%LY)
    CALL WGridPutCellInteger(IDF_GRID3,4,1,MODSVAT%INFO(IREC)%SVATID)
    CALL WGridPutCellInteger(IDF_GRID3,5,1,MODSVAT%INFO(IREC)%MFID)
    !## not correct so overuled
    IF(MODSVAT%INFO(IREC)%LY.EQ.1)THEN
      CALL WGridPutCellInteger(IDF_GRID3,3,1,MODSVAT%INFO(SVATINDEX_MOD(2))%LY)
      CALL WGridPutCellInteger(IDF_GRID3,4,1,MODSVAT%INFO(SVATINDEX_MOD(2))%SVATID)
      CALL WGridPutCellInteger(IDF_GRID3,5,1,MODSVAT%INFO(SVATINDEX_MOD(2))%MFID)
    ENDIF  
    
    !## fill and draw irrigation targets
