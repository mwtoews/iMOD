...
! GRAPH(1,1)%NP=1
   ...
   SELECT CASE (MESSAGE%VALUE1)
    !## load en save van settings file
     CASE (ID_OPEN,ID_SAVE)
       CALL MSP1SETTING(MESSAGE%VALUE1)       
   END SELECT
 END SELECT
 ...
 END SUBROUTINE MSPINSPECTOR_TAB5
  ...
  !###======================================================================
 SUBROUTINE MSP1SETTING(CODE,FNAME)
 !###======================================================================
 IMPLICIT NONE
...
! nog nodig??  CALL PLINESALLOCATE()
  ...
  IF(TOS.NE.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Errors occured in reading the ims file','Error')
  ENDIF
...
!###======================================================================
 ...
 IF(.NOT.MSPINSPECTOR_OPENFILES_METEGRID(IU))THEN
  INQUIRE(UNIT=IU,OPENED=LEX); IF(LEX)CLOSE(IU)
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open file '//CHAR(13)//TRIM(ROOT)//'\'//TRIM(MNAME)//'\METE_GRID.INP','Error'); RETURN
 ENDIF
 IF(.NOT.MSPINSPECTOR_OPENFILES_FACTSVAT(IU))THEN
  INQUIRE(UNIT=IU,OPENED=LEX); IF(LEX)CLOSE(IU)
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open file '//CHAR(13)//TRIM(ROOT)//'\'//TRIM(MNAME)//'\FACT_SVAT.INP','Error'); RETURN
 ENDIF
 IF(.NOT.MSPINSPECTOR_OPENFILES_TIOPSIM(IU))THEN
  INQUIRE(UNIT=IU,OPENED=LEX); IF(LEX)CLOSE(IU)
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open file '//CHAR(13)//TRIM(ROOT)//'\'//TRIM(MNAME)//'\TIOP_SIM.INP','Error'); RETURN
 ENDIF
...
END FUNCTION MSPINSPECTOR_OPENFILES_AREASVAT
 ...
 DO I=1,2
  ...
  DO
   ...
   IF(IOS.NE.0)EXIT
   IF(I.EQ.2)THEN  ! fill irigation pointer IDF
...
!     IF(SCAPSVAT%INFO(N)%SVATAB.GT.0)THEN ; CALL MSPINSPECTOR_SVAT2ROWCOL(SCAPSVAT%INFO(N)%SVATAB,JROW,JCOL,J) ; MSPSCAPSVAT%X(JCOL,JROW)=MSPSCAPSVAT%X(JCOL,JROW) + 100. ; ENDIF
 ...
 !## write scapsvat IDF
 IF(.NOT.IDFWRITE(MSPSCAPSVAT,'D:\roelofs_fs\_projecten\imod\version-develop\IMOD_USER\scapvat.idf',1))THEN
 ENDIF
 ...
 DO I=1,DXC%MXID
  IF(IROW.EQ.DXC%INFO(I)%IROW.AND.ICOL.EQ.DXC%INFO(I)%ICOL)THEN
    LEX=.TRUE.
    IMF=IMF+1  ;  DXC%DXCIREC(IMF)=I ; MFID(IMF)=DXC%INFO(I)%ID
 ...
 DO I=1,MODSVAT%MXID
   IF(MFID(1).EQ.MODSVAT%INFO(I)%UNID)THEN !## search urban and rural svat from DXC
    ...
    IF(I+1.LE.MODSVAT%MXID)THEN
     IF(MFID(2).EQ.MODSVAT%INFO(I+1)%UNID) SVATINDEX_MOD(2)=I+1   !## rural + irrigation
     IF(MFID(1).EQ.MODSVAT%INFO(I+1)%UNID) SVATINDEX_MOD(3)=I+1   !## rural + urban
    ENDIF
    !## rural + irrigation + urban ?
    IF(I+2.LE.MODSVAT%MXID)THEN
     IF(MFID(1).EQ.MODSVAT%INFO(I+2)%UNID) SVATINDEX_MOD(3)=I+2
    ENDIF
    EXIT
   ENDIF
 ENDDO
   ...
   DO I=1,AREASVAT%MXID
     IF(MODSVAT%INFO(SVATINDEX_MOD(1))%NUND.EQ.AREASVAT%INFO(I)%NUND)THEN
       ...
       IF(SVATINDEX_MOD(3).NE.0.AND.I+1.LE.AREASVAT%MXID)THEN   ! assumption : if present, the Urban SVAT always follows the rural svat directely
            IF(MODSVAT%INFO(SVATINDEX_MOD(3))%NUND.EQ.AREASVAT%INFO(I+1)%NUND)THEN
