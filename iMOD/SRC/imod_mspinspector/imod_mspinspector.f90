...
CONTAINS
 ...
 DO I=1,AREASVAT%MXID
   ...
   ENDIF 
 ENDDO    
 IF(.NOT.IDFWRITE(MSPLUSE,TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\MSPLUSE.idf',1))THEN ; ENDIF
 CALL MANAGER_UTL_ADDFILE(IDFNAMEGIVEN=TRIM(ROOT)//'\'//TRIM(MNAME)//'\METASWAP\MSPLUSE.idf')
  ...
  IF(IREXIST.EQ.1.AND.SCAPSVAT%INFO(IREC)%FMMXABGW.GT.0)THEN  !## checkbox is on and irrigation source is Groundwater
    ...
    DO I=1,SCAPSVAT%MXID  !## search for irrigated cells from same source
     IF(SVATID.EQ.SCAPSVAT%INFO(I)%SVATAB)THEN
       ...
       CALL WGridPutCellInteger(IDF_GRID4,3,1,MODSVAT%INFO(IREC)%LY)
       CALL WGridPutCellInteger(IDF_GRID4,4,1,MODSVAT%INFO(IREC)%SVATID)
       CALL WGridPutCellInteger(IDF_GRID4,5,1,MODSVAT%INFO(IREC)%MFID)
