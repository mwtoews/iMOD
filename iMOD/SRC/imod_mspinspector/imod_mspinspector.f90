...
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_CAP,UTL_IMODFILLMENU,ITIMETOGDATE,RTOS,UTL_PLOT1BITMAP,UTL_PLOT2BITMAP,UTL_FILLDATES,LISTNAME,UTL_INVERSECOLOUR,UTL_GDATE,UTL_GETMONTHFROMDAYNUMBER, &
   UTL_FILLDATESDIALOG
...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: II,I,J,K,M,N,NXG,WID,FTYPE,STYPE,ICOL,IROW,IERROR,NG
 ...
 NG=0
 DO I=1,SIZE(FACTSVAT%IACT); IF(FACTSVAT%IACT(I).EQ.1)NG=NG+1; ENDDO
 DO I=1,SIZE(METEGRID%IACT); IF(METEGRID%IACT(I).EQ.1)NG=NG+1; ENDDO
 ...
 DO I=1,SIZE(METEGRID%IACT)
  IF(METEGRID%IACT(I).EQ.0)CYCLE
  
  NG=NG+1
  
  N=0; DO J=1,METEGRID%MXID
   !## stop
   IF(METEGRID%INFO(J)%JD.GT.MSPEDATE)EXIT
   IF(METEGRID%INFO(J)%JD.GE.MSPSDATE)N=N+1
  ENDDO
  ...
  GRAPHDIM(NG)%GRAPHNAMES='Graph '//TRIM(ITOS(NG))
  ...
  N=0; DO J=1,METEGRID%MXID
   !## stop
   IF(METEGRID%INFO(J)%JD.GT.MSPEDATE)EXIT
   IF(METEGRID%INFO(J)%JD.GE.MSPSDATE)THEN
   
    SELECT CASE (FTYPE)
     !## ascii 
     CASE (1)
      IF(I.EQ.1)CALL ASC2IDF_IMPORTASC(METEGRID%INFO(J)%PRECGRID ,0.0D0,0.0D0,IERROR,0,IDFIMPORT=METEO)
      IF(I.EQ.2)CALL ASC2IDF_IMPORTASC(METEGRID%INFO(J)%ETREFGRID,0.0D0,0.0D0,IERROR,0,IDFIMPORT=METEO)
      IF(IERROR.EQ.1)RETURN; XV=METEO%X(ICOL,IROW)
     !## ascii->idf
     CASE (2)
      IF(I.EQ.1)CALL ASC2IDF_IMPORTASC(METEGRID%INFO(J)%PRECGRID ,0.0D0,0.0D0,IERROR,0) 
      IF(I.EQ.2)CALL ASC2IDF_IMPORTASC(METEGRID%INFO(J)%ETREFGRID,0.0D0,0.0D0,IERROR,0)
    ...
    SELECT CASE (FTYPE)
     !## asc->idf, idf
     CASE (-2,3)
      IF(I.EQ.1)METEO%FNAME=METEGRID%INFO(I)%PRECGRID( :INDEX(METEGRID%INFO(J)%PRECGRID,'.',.TRUE. )-1)//'.IDF'
      IF(I.EQ.2)METEO%FNAME=METEGRID%INFO(I)%ETREFGRID(:INDEX(METEGRID%INFO(J)%ETREFGRID,'.',.TRUE.)-1)//'.IDF'
 ...
 DO I=1,SIZE(FACTSVAT%IACT)
  ...
  DO
   N=N+1
   SELECT CASE (I)
    CASE (1); XV=FACTSVAT%INFO(N)%CSVG
    CASE (2); XV=FACTSVAT%INFO(N)%LAIVG
    CASE (3); XV=FACTSVAT%INFO(N)%VXICVG
    CASE (4); XV=FACTSVAT%INFO(N)%FAEVVG
    CASE (5); XV=FACTSVAT%INFO(N)%FAEIVG
    CASE (6); XV=FACTSVAT%INFO(N)%FAEBSVG
    CASE (7); XV=FACTSVAT%INFO(N)%FAEPDVG
    CASE (8); XV=FACTSVAT%INFO(N)%CHVG
    CASE (9); XV=FACTSVAT%INFO(N)%DRPZVG
   END SELECT
   IF(LDATE)     GRAPH(M,NG)%RX(N)=DBLE(METEGRID%INFO(J)%JD)
   IF(.NOT.LDATE)GRAPH(M,NG)%RX(N)=DBLE(N)
   GRAPH(M,NG)%RY(N)=XV
  ENDDO
  
  !CSVG,LAIVG,VXICVG,FAEVVG,FAEIVG,FAEBSVG,FAEPDVG,CHVG,DRPZVG
...
!###======================================================================
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: I,J,N,IOS,NPRGS,IPRGS,IROW,ICOL,IERROR
 ...
 !## set starting date
 MSPSDATE=METEGRID%INFO(1)%JD; CALL UTL_FILLDATESDIALOG(ID_DMSPANALYSER_TAB5,IDF_INTEGER1,IDF_MENU5,IDF_INTEGER2,MSPSDATE)
 
 !## to date
 MSPEDATE=METEGRID%INFO(METEGRID%MXID)%JD; CALL UTL_FILLDATESDIALOG(ID_DMSPANALYSER_TAB5,IDF_INTEGER11,IDF_MENU6,IDF_INTEGER12,MSPEDATE)
...
END FUNCTION MSPINSPECTOR_OPENFILES_AREASVAT
 ...
 ID=INT(METEGRID%INFO(1)%TD)+1; IY=METEGRID%INFO(1)%IY
 CALL UTL_GETMONTHFROMDAYNUMBER(ID,IM,IY); METEGRID%INFO(1)%JD=JD(IY,IM,ID)
  
 !## configure julian date to all entries
 DO I=1,METEGRID%MXID
  !## does not support timesteps < 1 day
  ID=INT(METEGRID%INFO(I)%TD)+1; IY=METEGRID%INFO(I)%IY
  CALL UTL_GETMONTHFROMDAYNUMBER(ID,IM,IY); METEGRID%INFO(I)%JD=JD(IY,IM,ID)
 ENDDO
