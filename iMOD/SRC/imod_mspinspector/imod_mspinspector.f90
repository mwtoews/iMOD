...
CONTAINS
 ...
 IF(.TRUE.)THEN
  ...
  N=0; DO I=1,METEGRID%MXID
   !## stop
   IF(METEGRID%INFO(I)%JD.GT.MSPEDATE)EXIT
   IF(METEGRID%INFO(I)%JD.GE.MSPSDATE)N=N+1
  ENDDO

  ALLOCATE(GRAPH(M,NG-1)%RX(N),GRAPH(M,NG-1)%RY(N),GRAPH(M,NG)%RX(N),GRAPH(M,NG)%RY(N))
 
  GRAPH(M,NG-1)%NP=N
  GRAPH(M,NG  )%NP=N
  GRAPH(M,NG-1)%CTYPE=''
  GRAPH(M,NG  )%CTYPE=''
  GRAPH(M,NG-1)%ICLR=WRGB(0,0,255)
  GRAPH(M,NG  )%ICLR=WRGB(200,0,0)
  GRAPH(M,NG-1)%GTYPE=4
  GRAPH(M,NG  )%GTYPE=4
  GRAPH(M,NG-1)%LEGTXT='Precipitation'
  GRAPH(M,NG  )%LEGTXT='Evaporation'
  GRAPHDIM(NG-1)%GRAPHNAMES='Graph '//TRIM(ITOS(NG-1))
  GRAPHDIM(NG  )%GRAPHNAMES='Graph '//TRIM(ITOS(NG))
  GRAPHDIM(NG-1)%XTITLE='Date'
  GRAPHDIM(NG  )%XTITLE='Date'
  GRAPHDIM(NG-1)%YTITLE='Precipitation (mm/d)'
  GRAPHDIM(NG  )%YTITLE='Evaporation (mm/d)'
  GRAPHDIM(NG-1)%LDATE=.TRUE.
  GRAPHDIM(NG  )%LDATE=.TRUE.

  N=0; DO I=1,METEGRID%MXID
   !## stop
   IF(METEGRID%INFO(I)%JD.GT.MSPEDATE)EXIT
   IF(METEGRID%INFO(I)%JD.GE.MSPSDATE)THEN
   
    SELECT CASE (FTYPE)
     !## ascii 
     CASE (1)
      CALL ASC2IDF_IMPORTASC(METEGRID%INFO(I)%PRECGRID ,0.0D0,0.0D0,IERROR,0,IDFIMPORT=METEO); IF(IERROR.EQ.1)RETURN; XV1=METEO%X(ICOL,IROW)
      CALL ASC2IDF_IMPORTASC(METEGRID%INFO(I)%ETREFGRID,0.0D0,0.0D0,IERROR,0,IDFIMPORT=METEO); IF(IERROR.EQ.1)RETURN; XV2=METEO%X(ICOL,IROW)
     !## ascii->idf
     CASE (2)
      CALL ASC2IDF_IMPORTASC(METEGRID%INFO(I)%PRECGRID ,0.0D0,0.0D0,IERROR,0 ); IF(IERROR.EQ.1)RETURN
      CALL ASC2IDF_IMPORTASC(METEGRID%INFO(I)%ETREFGRID,0.0D0,0.0D0,IERROR,0 ); IF(IERROR.EQ.1)RETURN
      FTYPE=-1*FTYPE
    END SELECT
    ...
    SELECT CASE (FTYPE)
     !## asc->idf, idf
     CASE (-2,3)
      METEO%FNAME=METEGRID%INFO(I)%PRECGRID(:INDEX(METEGRID%INFO(I)%PRECGRID,'.',.TRUE.)-1)//'.IDF'; IF(.NOT.IDFREAD(METEO,METEO%FNAME,0))RETURN
      XV1=IDFGETVAL(METEO,IROW,ICOL); CLOSE(METEO%IU)
      METEO%FNAME=METEGRID%INFO(I)%PRECGRID(:INDEX(METEGRID%INFO(I)%PRECGRID,'.',.TRUE.)-1)//'.IDF'; IF(.NOT.IDFREAD(METEO,METEO%FNAME,0))RETURN
      XV2=IDFGETVAL(METEO,IROW,ICOL); CLOSE(METEO%IU)
    END SELECT

    !## fill in values
    N=N+1
    !## precipitation
    GRAPH(M,NG-1)%RX(N)=DBLE(METEGRID%INFO(I)%JD)
    GRAPH(M,NG-1)%RY(N)=XV1
    !## evporation
    GRAPH(M,NG  )%RX(N)=DBLE(METEGRID%INFO(I)%JD)
    GRAPH(M,NG  )%RY(N)=XV2
...
END FUNCTION MSPINSPECTOR_OPENFILES_AREASVAT
 ...
 DO I=1,METEGRID%MXID
  ...
  ID=INT(METEGRID%INFO(N)%TD)+1; IY=METEGRID%INFO(N)%IY
