...
! GRAPH(1,1)%NP=1
 ...
 DO I=1,AREASVAT%MXID
   DO J=AREASVAT%INFO(I)%NUND,MODSVAT%MXID           !NB diferent start point to speed up the search 
     IF(AREASVAT%INFO(I)%NUND.EQ.MODSVAT%INFO(J)%SVATID)THEN
         ...
         MSPLUSE%X(JCOL,JROW)=AREASVAT%INFO(I)%LUSE 
         EXIT
     ENDIF    
   ENDDO      
 ENDDO        
 ...
 IF(CODE.EQ.ID_SAVE)THEN
  ...
  DO I=1,SIZE(IDFSVAT%IACT)  ; WRITE(IU,'(A)') TRIM(MSFILES(1))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(IDFSVAT%IACT(I)))  ; END DO
  DO I=1,SIZE(MODSVAT%IACT)  ; WRITE(IU,'(A)') TRIM(MSFILES(2))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(MODSVAT%IACT(I)))  ; END DO
  DO I=1,SIZE(AREASVAT%IACT) ; WRITE(IU,'(A)') TRIM(MSFILES(3))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(AREASVAT%IACT(I)))  ; END DO
  DO I=1,SIZE(INFISVAT%IACT) ; WRITE(IU,'(A)') TRIM(MSFILES(4))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(INFISVAT%IACT(I)))  ; END DO
  DO I=1,SIZE(SCAPSVAT%IACT) ; WRITE(IU,'(A)') TRIM(MSFILES(5))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(SCAPSVAT%IACT(I)))  ; END DO
  DO I=1,SIZE(LUSESVAT%IACT) ; WRITE(IU,'(A)') TRIM(MSFILES(6))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(LUSESVAT%IACT(I)))  ; END DO
  DO I=1,SIZE(METEGRID%IACT) ; WRITE(IU,'(A)') TRIM(MSFILES(7))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(METEGRID%IACT(I)))  ; END DO
  DO I=1,SIZE(FACTSVAT%IACT) ; WRITE(IU,'(A)') TRIM(MSFILES(8))//'_'//TRIM(ITOS(I))//' = '//TRIM(ITOS(FACTSVAT%IACT(I)))  ; END DO
 ...
 ELSEIF(CODE.EQ.ID_OPEN)THEN
  ...
  DO I=1,SIZE(IDFSVAT%IACT)  ; IF(UTL_READINITFILE(TRIM(MSFILES(1))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) IDFSVAT%IACT(I) ; END DO
  DO I=1,SIZE(MODSVAT%IACT)  ; IF(UTL_READINITFILE(TRIM(MSFILES(2))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) MODSVAT%IACT(I) ; END DO
  DO I=1,SIZE(AREASVAT%IACT) ; IF(UTL_READINITFILE(TRIM(MSFILES(3))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) AREASVAT%IACT(I) ; END DO
  DO I=1,SIZE(INFISVAT%IACT) ; IF(UTL_READINITFILE(TRIM(MSFILES(4))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) INFISVAT%IACT(I) ; END DO
  DO I=1,SIZE(SCAPSVAT%IACT) ; IF(UTL_READINITFILE(TRIM(MSFILES(5))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) SCAPSVAT%IACT(I) ; END DO
  DO I=1,SIZE(LUSESVAT%IACT) ; IF(UTL_READINITFILE(TRIM(MSFILES(6))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) LUSESVAT%IACT(I) ; END DO
  DO I=1,SIZE(METEGRID%IACT) ; IF(UTL_READINITFILE(TRIM(MSFILES(7))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) METEGRID%IACT(I) ; END DO
  DO I=1,SIZE(FACTSVAT%IACT) ; IF(UTL_READINITFILE(TRIM(MSFILES(8))//'_'//TRIM(ITOS(I)),LINE,IU,1)) READ(LINE,*) FACTSVAT%IACT(I) ; END DO
...
!###======================================================================
 ...
 DO K=1,2
  ...
  DO I=1,SIZE(DXC%LABEL);  IF(DXC%IACT(I).EQ.0)CYCLE  ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,DXC%LABEL(I)) ; END DO
  DO I=1,SIZE(MODSVAT%LABEL);  IF(MODSVAT%IACT(I).EQ.0)CYCLE  ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,MODSVAT%LABEL(I)) ; END DO
  DO I=1,SIZE(IDFSVAT%LABEL);  IF(IDFSVAT%IACT(I).EQ.0)CYCLE  ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,IDFSVAT%LABEL(I)) ; END DO
  DO I=1,SIZE(AREASVAT%LABEL); IF(AREASVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,AREASVAT%LABEL(I)) ; END DO
  DO I=1,SIZE(LUSESVAT%LABEL); IF(LUSESVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,LUSESVAT%LABEL(I)) ; END DO
  DO I=1,SIZE(FACTSVAT%LABEL); IF(FACTSVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,FACTSVAT%LABEL(I)) ; END DO
  DO I=1,SIZE(INFISVAT%LABEL); IF(INFISVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,INFISVAT%LABEL(I)) ; END DO
  DO I=1,SIZE(SCAPSVAT%LABEL); IF(SCAPSVAT%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,SCAPSVAT%LABEL(I)) ; END DO
  DO I=1,SIZE(METEGRID%LABEL); IF(METEGRID%IACT(I).EQ.0)CYCLE ; N=N+1; J=J+1; IF(K.EQ.2)CALL WGRIDPUTCELLSTRING(IDF_GRID1,1,J,METEGRID%LABEL(I)) ; END DO
 ...
 ALLOCATE(DXC%LABEL(4),DXC%IACT(4),DXC%DXCIREC(2))
 ...
 DO N=1,MODSVAT%MXID
   ...
   I=MODSVAT%INFO(N)%MFID  
   MODSVAT%INFO(N)%IROW=DXC%INFO(I)%IROW
   MODSVAT%INFO(N)%ICOL=DXC%INFO(I)%ICOL
 ENDDO

 !## spatial sort svat typse (rural,urban,irrigation) 
 ! read from modsvat, check in areasvat and scapsvat
 CALL IDFCOPY(MSPIDF,SVATRU)
 CALL IDFCOPY(MSPIDF,SVATUR)
 CALL IDFCOPY(MSPIDF,SVATIR)
 ...
 DO N=1,MODSVAT%MXID
   ...
   IROW=MODSVAT%INFO(N)%IROW
   ICOL=MODSVAT%INFO(N)%ICOL  
   I=MODSVAT%INFO(N)%SVATID
   ...
   IF(AREASVAT%INFO(I)%LUSE.EQ.18)THEN
     SVATUR%X(ICOL,IROW)=FLOAT(MODSVAT%INFO(N)%SVATID)
   ELSE
     SVATRU%X(ICOL,IROW)=FLOAT(MODSVAT%INFO(N)%SVATID)
   ENDIF 
   CALL POL1LOCATEINT(SCAPSVAT%INFO%NUND,SCAPSVAT%MXID,MODSVAT%INFO(N)%SVATID,I)
   IF(I.GT.0) SVATIR%X(ICOL,IROW)=FLOAT(MODSVAT%INFO(I)%SVATID)
...
END FUNCTION MSPINSPECTOR_OPENFILES_AREASVAT
 ...
 ALLOCATE(LUSESVAT%LABEL(18),LUSESVAT%IACT(18))

 LUSESVAT%LABEL(1)='LU-index of land use type'
 LUSESVAT%LABEL(2)='LUNA-name of land use type'
 LUSESVAT%LABEL(3)='VGLU-index of vegetation type'
 LUSESVAT%LABEL(4)='ALPHACRIT-parameter of Jarvis(1989)'
 LUSESVAT%LABEL(5)='P1FD-p1 Feddes function'
 LUSESVAT%LABEL(6)='P2FD-p2 Feddes function'
 LUSESVAT%LABEL(7)='P3HFD-p3h Feddes function'
 LUSESVAT%LABEL(8)='P3LFD-p3l Feddes function'
 LUSESVAT%LABEL(9)='P4FD-p4 Feddes function'
 LUSESVAT%LABEL(10)='T3HFD-t3 Feddes function'
 LUSESVAT%LABEL(11)='T3LFD-t3 Feddes function'
 LUSESVAT%LABEL(12)='PBGSPLU-pressure head begin sprinkling'
 LUSESVAT%LABEL(13)='FREVSPLU-fraction evaporated sprinkling water'
 LUSESVAT%LABEL(14)='GISPLU-gift in rotational period'
 LUSESVAT%LABEL(15)='TIGISPLU-duration gift'
 LUSESVAT%LABEL(16)='RPSPLU-rotational period'
 LUSESVAT%LABEL(17)='TDBGSPLU-beginning of sprinkling period'
 LUSESVAT%LABEL(18)='TDEDSPLU-end of sprinkling period'
 LUSESVAT%IACT=1
 ...
 ALLOCATE(METEGRID%LABEL(4),METEGRID%IACT(4))

 METEGRID%LABEL(1)='day from 00:00:00'
 METEGRID%LABEL(2)='year number'
 METEGRID%LABEL(3)='precipitation'
 METEGRID%LABEL(4)='evapotranspiration'
 ...
 ALLOCATE(FACTSVAT%LABEL(11),FACTSVAT%IACT(11))

 FACTSVAT%LABEL(1)='VG-vegetation type'
 FACTSVAT%LABEL(2)='DY-day number'
 FACTSVAT%LABEL(3)='CSVG-soil cover'
 FACTSVAT%LABEL(4)='LAIVG-leaf area index'
 FACTSVAT%LABEL(5)='VXICVG-interception capacity'
 FACTSVAT%LABEL(6)='FAEVVG-vegetation factor'
 FACTSVAT%LABEL(7)='FAEIVG-factor for interception evaporation'
 FACTSVAT%LABEL(8)='FAEBSVG-factor for bare soil evaporation'
 FACTSVAT%LABEL(9)='FAEPDVG-factor for ponding'
 FACTSVAT%LABEL(10)='CHVG-crop height'
 FACTSVAT%LABEL(11)='DRPZVG-dynamic root zone depth'
 FACTSVAT%IACT=1
 ...
 ALLOCATE(TIOPSIM%LABEL(2),TIOPSIM%IACT(2))
 ...
 !## fill dates in TAB5
 CALL WDIALOGSELECT(ID_DMSPANALYSER_TAB5)
 CALL WDIALOGPUTSTRING(IDF_RADIO1,'Use ALL available export dates ('//TRIM(ITOS(TIOPSIM%MXID))//')-dates)')
 ...
 LEX=.FALSE. ; LUSEINDEX=0  ; SCAPINDEX=0

 !## Get unique Modflow ID's for this cell (from DXC file)
 DXC%DXCIREC=0   ;  IMF=0 ; MFID=0
 DO I=1,DXC%MXID
  IF(IROW.EQ.DXC%INFO(I)%IROW.AND.ICOL.EQ.DXC%INFO(I)%ICOL)THEN
    LEX=.TRUE.
    IMF=IMF+1  ;  DXC%DXCIREC(IMF)=I ; MFID(IMF)=DXC%INFO(I)%MFID
 ...
 DO I=1,MODSVAT%MXID
   IF(MFID(1).EQ.MODSVAT%INFO(I)%MFID)THEN !## search urban and rural svat from DXC
    !## search for LUSE type
    DO J=1,AREASVAT%MXID
      IF(MODSVAT%INFO(I)%SVATID.EQ.AREASVAT%INFO(J)%NUND)THEN
    ...
    ELSE  ! Rural + ?
     SVATINDEX_MOD(1)=I
    ENDIF
    !## if rural svat, search for irrigation/urban svat (next line in file)
    IF(I+1.LE.MODSVAT%MXID)THEN
     IF(MFID(2).EQ.MODSVAT%INFO(I+1)%MFID) SVATINDEX_MOD(2)=I+1   !## rural + irrigation
     IF(MFID(1).EQ.MODSVAT%INFO(I+1)%MFID) SVATINDEX_MOD(3)=I+1   !## rural + urban
    ENDIF
    !## rural + irrigation + urban ?
    IF(I+2.LE.MODSVAT%MXID)THEN
     IF(MFID(1).EQ.MODSVAT%INFO(I+2)%MFID) SVATINDEX_MOD(3)=I+2
    ENDIF
    EXIT
   ENDIF
 ENDDO
 IF(SUM(SVATINDEX_MOD).EQ.0)RETURN
 ...
 IF(SVATINDEX_MOD(1).NE.0)THEN
  ...
  CALL WDIALOGSELECT(ID_DMSPANALYSER_TAB3)
  CALL WGridPutCellINTEGER(IDF_GRID1,2,3,LUSEINDEX)
     ...
     IF(MODSVAT%INFO(SVATINDEX_MOD(1))%SVATID.EQ.AREASVAT%INFO(I)%NUND)THEN
       ...
       IF(SVATINDEX_MOD(3).NE.0.AND.I+1.LE.AREASVAT%MXID)THEN   ! assumption : if present, the Urban SVAT always follows the rural svat directely
            IF(MODSVAT%INFO(SVATINDEX_MOD(3))%SVATID.EQ.AREASVAT%INFO(I+1)%NUND)THEN
               CALL WGridPutCellDOUBLE(IDF_GRID1,7,3,AREASVAT%INFO(I+1)%ARND,'(F8.1)')
               NOPP=NOPP-AREASVAT%INFO(I+1)%ARND
