...
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,UTL_DIRINFO,UTL_CREATEDIR,RTOS,LISTNAME,ILIST,UTL_IMODFILLMENU, &
    UTL_PLOT1BITMAP,UTL_PLOT2BITMAP,UTL_SETTEXTSIZE
USE MOD_POLYGON_UTL, ONLY : POLYGON_UTL_GETSHAPE
!USE MOD_IPF, ONLY : UTL_PLOTPOINT,UTL_PLOTLABEL
USE MOD_TAGS_PAR
...
!  IF(ALLOCATED(LISTNAME))DEALLOCATE(LISTNAME)
 ...
 REAL(KIND=DP_KIND) :: TWIDTH,THEIGHT
...
! CALL IGRFILLPATTERN(HATCHED,DENSE1,DIAGDOWN)
 ...
 CALL UTL_SETTEXTSIZE(TWIDTH,THEIGHT,FCT=0.0D02)
 ...
 DO I=1,NTAGS
  IF(TAGLIST(I).EQ.1)THEN
   ...
   SELECT CASE (ISHAPE)
    CASE (ID_POINT)
!     CALL DBL_IGRMARKER(XTAG(1),YTAG(1),2)
    CASE (ID_RECTANGLE)
     CALL IGRFILLPATTERN(HATCHED,DENSE1,DIAGDOWN)
     CALL DBL_IGRRECTANGLE(XTAG(1),YTAG(1),XTAG(2),YTAG(2))
    CASE (ID_CIRCLE)
     CALL IGRFILLPATTERN(HATCHED,DENSE1,DIAGDOWN)
     RADIUS = SQRT((XTAG(1)-XTAG(2))**2.+(YTAG(1)-YTAG(2))**2.)
     CALL DBL_IGRCIRCLE(XTAG(1),YTAG(1),RADIUS)
    CASE (ID_LINE)
!     CALL IGRLINEWIDTH(3)
     DO J=2,TAGPOINTS
      CALL DBL_IGRJOIN(XTAG(J),YTAG(J),XTAG(J-1),YTAG(J-1))
     END DO
!     CALL IGRLINEWIDTH(1)
    CASE (ID_POLYGON)
     CALL IGRFILLPATTERN(HATCHED,DENSE1,DIAGDOWN)
     CALL DBL_IGRPOLYGONCOMPLEX(XTAG,YTAG,TAGPOINTS)
   END SELECT
!   CALL UTL_PLOTPOINT(XTAGMID,YTAGMID,2,1.0D0)
!   CALL UTL_PLOTLABEL(XTAGMID,YTAGMID,(/TRIM(RESTAG(I))/),(/1/),1,TWIDTH,THEIGHT,(/'File:'/),.FALSE.,0)
 ...
 !###======================================================================
 SUBROUTINE TAGZOOM(XMIN,YMIN,XMAX,YMAX)
 !###======================================================================
 IMPLICIT NONE
 ...
 DO I=1,NTAGS
  IF(TAGLIST(I).EQ.1)THEN
   ...
   K=K+1
   IF(K.EQ.1)THEN
    XMIN=MINVAL(XTAG(1:TAGPOINTS))
    YMIN=MINVAL(YTAG(1:TAGPOINTS))
    XMAX=MAXVAL(XTAG(1:TAGPOINTS))
    YMAX=MAXVAL(YTAG(1:TAGPOINTS))
   ELSE
    XMIN=MIN(XMIN,MINVAL(XTAG(1:TAGPOINTS)))
    YMIN=MIN(YMIN,MINVAL(YTAG(1:TAGPOINTS)))
...
!                   IFLAGS=NOTOOLBAR+WORDWRAP+NOFILENEWOPEN+NOFILESAVEAS+NOFILEPRINT)
 ...
 CALL POLYGON_UTL_GETSHAPE(ISHAPE,(/3,1,1,1,1,3/))
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot open file '//TRIM(TRIM(DIRNAME)//'\'//TRIM(TAGFILE))//CHAR(13)// &
...
! CALL TAGUPDATEFIELD()
 ...
 INTEGER :: ITYPE,IDOWN
 REAL(KIND=DP_KIND) :: XC1,YC1,XC2,YC2,XC3,YC3,XTAGTEMP,YTAGTEMP
 LOGICAL :: LEX
...
!   CALL IGRLINEWIDTH(3)
 ...
 DO WHILE(.TRUE.)
  ...
  SELECT CASE(ITYPE)
   ...
   CASE(MOUSEMOVE)
    ...
    CALL WINDOWSELECT(0)
    CALL WINDOWOUTSTATUSBAR(1,'X:'//TRIM(RTOS(REAL(MESSAGE%GX,8),'F',2))//'M, Y:'//TRIM(RTOS(REAL(MESSAGE%GY,8),'F',2))//'M')

 ! FIRST POINT SET!
    IF(IDOWN.EQ.1)CALL TOOLSUPDATESHAPE(ISHAPE,XC1,YC1,XC2,YC2,XC3,YC3,LEX)
 ...
 !#######REMOVE PREVIOUS RECTANGLE/POLYGON-SEGMENT
         CALL TOOLSUPDATESHAPE(ISHAPE,XC1,YC1,XC2,YC2,XC3,YC3,LEX)
 ...
 !####REMOVE PREVIOUS RECTANGLE/POLYGON-SEGMENT
      ...
      IF(ISHAPE.EQ.ID_POLYGON.OR.ISHAPE.EQ.ID_LINE)THEN
       ...
       IF(ISHAPE.EQ.ID_LINE)THEN
        CALL DBL_IGRJOIN(XTAG(TAGPOINTS),YTAG(TAGPOINTS),XC2,YC2)
       ENDIF
       ...
       IF(ISHAPE.EQ.ID_POLYGON) THEN
        ...
        IF(TAGPOINTS.GT.1) THEN
         CALL DBL_IGRJOIN(XTAG(1),YTAG(1),XC2,YC2)
         CALL DBL_IGRJOIN(XTAG(TAGPOINTS),YTAG(TAGPOINTS),XTAG(1),YTAG(1))
        ENDIF
 ...
 !###======================================================================
 SUBROUTINE TOOLSUPDATESHAPE(ISHAPE,XC1,YC1,XC2,YC2,XC3,YC3,LEX)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ISHAPE
 REAL(KIND=DP_KIND),INTENT(IN) :: XC1,YC1,XC2,YC2,XC3,YC3
 LOGICAL,INTENT(IN OUT) :: LEX
 REAL(KIND=DP_KIND) :: RADIUS
 ...
 !!!  IF (LEX) CALL DBL_IGRRECTANGLE(XTAG(1),YTAG(1),XTAG(2),YTAG(2))
   LEX=.FALSE.
 ...
 !!!  IF(LEX) CALL DBL_IGRRECTANGLE(XTAG(1),YTAG(1),XTAGTEMP,YTAGTEMP)

  CASE (ID_CIRCLE)
   IF(LEX)THEN
    RADIUS=SQRT((XC1-XC3)**2.+(YC1-YC3)**2.)
    CALL DBL_IGRCIRCLE(XC1,YC1,RADIUS)
 !!!   RADIUS=SQRT((XTAG(1)-XTAG(2))**2.+(YTAG(1)-YTAG(2))**2.)
 !!!   CALL DBL_IGRCIRCLE(XTAG(1),YTAG(1),RADIUS)
   ENDIF
   LEX=.FALSE.
 ...
 !!!  RADIUS=SQRT((XTAG(1)-XTAGTEMP)**2.+(YTAG(1)-YTAGTEMP)**2.)
   ...
   IF(RADIUS.NE.0.0D0)LEX=.TRUE.
 ! NEW CIRCLE
   IF(LEX)CALL DBL_IGRCIRCLE(XC1,YC1,RADIUS)
 !!!  IF(LEX) CALL DBL_IGRCIRCLE(XTAG(1),YTAG(1),RADIUS)

  CASE (ID_POLYGON)
 ! REMOVE PREVIOUS POLYGON-SEGMENT
   IF(LEX)THEN
    CALL DBL_IGRJOIN(XTAG(TAGPOINTS),YTAG(TAGPOINTS),XC3,YC3)
    IF (TAGPOINTS.GT.1) CALL DBL_IGRJOIN(XTAG(1),YTAG(1),XC3,YC3)
 !!!   CALL DBL_IGRJOIN(XTAG(TAGPOINTS),YTAG(TAGPOINTS),XTAG(TAGPOINTS+),YTAG(2))
 !!!   IF (TAGPOINTS.GT.1) CALL DBL_IGRJOIN(XTAG(1),YTAG(1),XTAG(2),YTAG(2))
   ENDIF
   LEX=.TRUE.
 ! NEW POLYGON-SEGMENT
    CALL DBL_IGRJOIN(XTAG(TAGPOINTS),YTAG(TAGPOINTS),XC2,YC2)
    IF (TAGPOINTS.GT.1) CALL DBL_IGRJOIN(XTAG(1),YTAG(1),XC2,YC2)

 !!!   CALL DBL_IGRJOIN(XTAG(TAGPOINTS),YTAG(TAGPOINTS),XTAGTEMP,YTAGTEMP)
 !!!   IF (TAGPOINTS.GT.1) CALL DBL_IGRJOIN(XTAG(1),YTAG(1),XTAGTEMP,YTAGTEMP)
