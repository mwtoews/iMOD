...
CONTAINS
 ...
 ELSEIF(MEAN_ISEL.EQ.2)THEN
  CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,0,MEAN_GENFNAME,VAR)
  SHPIACT(1:SHPNO)=1
 !## usage of idf
  ...
  IF(LEX)THEN
   DO IROW=1,IDF(0)%NROW
    ...
    DO ICOL=1,IDF(0)%NCOL
     IF(IDFCP%X(ICOL,IROW).NE.0.0)THEN !IDFCP%NODATA)THEN
      ...
      IF((TRIM(CFUNC)).EQ.'PERC')THEN
       CALL UTL_GETMED(XPERC,INT(IDF(-1)%X(ICOL,IROW)),-999999.9,(/PERCVALUE/),1,I,XMED)
 ...
 ALLOCATE(NP(IDF(0)%NCOL,IDF(0)%NROW)); NP=0.0
