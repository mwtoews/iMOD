...
CONTAINS
 ...
 ELSEIF(MEAN_ISEL.EQ.2)THEN
  CALL POLYGON1SAVELOADSHAPE(ID_LOADSHAPE,MEAN_GENFNAME,'GEN')
 ...
 DO II=1,MAX(1,MEAN_NLAYER)
  ...
  ELSE
   DO IROW=1,IDF(0)%NROW
    ...
    ELSE
     WRITE(6,'(A,F10.2,A)') '+Progress NON-Equal IDFs: ',REAL(IROW*100)/REAL(IDF(0)%NROW),'%'
    ENDIF
   ENDDO
  ENDIF
   ...
   IF(IDF(-1)%X(ICOL,IROW).NE.IDF(I)%NODATA)THEN
    SELECT CASE (IOPTION)
     ...
     CASE (4)   !## max
      ...
      NP(ICOL,IROW)=1.0D0
     CASE (5)   !## min
      IDF(0)%X(ICOL,IROW)=MIN(IDF(0)%X(ICOL,IROW),IDF(-1)%X(ICOL,IROW))
      NP(ICOL,IROW)=1.0D0
    END SELECT
   ENDIF
  ENDDO; ENDDO
