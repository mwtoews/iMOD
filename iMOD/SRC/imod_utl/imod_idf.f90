...
!     DYC=DYC/REAL(IRC2-IRC1+1,8)

     DXM=IDFM%SX(ICM) -IDFM%SX(ICM-1)
     DXC=IDFC%SX(ICC2)-IDFC%SX(ICC1-1)
!     DXC=DXC/REAL(ICC2-ICC1+1,8)

!if(SCLTYPE_UP.EQ.5.AND.icm.eq.46.and.irm.eq.13)then
!write(*,*)
!endif
...
!        !## get net overlapping area
!        XD1=MAX(IDFM%SX(ICM-1),IDFC%SX(ICC1-1))
!        XD2=MIN(IDFM%SX(ICM  ),IDFC%SX(ICC2  ))
!        YD2=MAX(IDFM%SY(IRM)  ,IDFC%SY(IRC2))
!        YD1=MIN(IDFM%SY(IRM-1),IDFC%SY(IRC1))
!        RAT=((XD2-XD1)*(YD2-YD1))/(DXC*DXC)
!        SVALUE=SVALUE*RAT
...
!###======================================================================
 ...
 !## open idf
 IF(IDFOPEN(IDF%IU,IDFNAME,TXT,IDF%ITYPE,IDATA,IQUESTION=IQ))THEN
