...
USE WINTERACTER ! SORTDESCENT
USE IMODVAR, ONLY : DP_KIND,SP_KIND

CONTAINS

 !###====================================================
 SUBROUTINE UTL_WSORT(X,I1,I2,IFLAGS,IORDER)
 !###====================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: I1,I2
 INTEGER,INTENT(IN),OPTIONAL :: IFLAGS
 INTEGER,INTENT(INOUT),DIMENSION(:),OPTIONAL :: IORDER
 REAL(KIND=DP_KIND),INTENT(INOUT),DIMENSION(:) :: X
 REAL(KIND=DP_KIND),ALLOCATABLE,DIMENSION(:) :: XORDER
 INTEGER :: I,N
 
 N=(I2-I1)+1

 IF(PRESENT(IFLAGS))THEN; IF(IFLAGS.EQ.SORTDESCEND)X=-1.0D0*X; ENDIF
 
 IF(PRESENT(IORDER))THEN
  ALLOCATE(XORDER(N)); DO I=1,N; XORDER(I)=REAL(IORDER(I),8); ENDDO
  CALL QKSORT(N,X,V2=XORDER)
  DO I=1,N; IORDER(I)=INT(XORDER(I)); ENDDO
 ELSE
  CALL QKSORT(N,X)
 ENDIF
