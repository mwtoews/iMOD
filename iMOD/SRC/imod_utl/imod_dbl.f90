...
! INTEGER,INTENT(IN),OPTIONAL :: IOFFSET
 ...
 SELECT CASE (POL%ITYPE)
  CASE (ID_POLYGON)
!   IF(PRESENT(IOFFSET))THEN
!    IF(DBL_IGRINSIDEPOLYGON(X,Y,POL%X,POL%Y,POL%N,IOFFSET).EQ.1)DBL_IGRINSIDESHAPE=1
!   ELSE
...
! ENDIF
 ...
 REAL(KIND=SP_KIND),DIMENSION(:),ALLOCATABLE :: XS,YS
 INTEGER,INTENT(IN),OPTIONAL :: IOFFSET
 
 ALLOCATE(XS(N),YS(N))
 IF(PRESENT(IOFFSET))THEN
  IF(IOFFSET.EQ.0)THEN
   XS=REAL(X,4); YS=REAL(Y,4)
  ELSE
   XS=REAL(X-OFFSETX,4); YS=REAL(Y-OFFSETY,4)  
  ENDIF
 ELSE
  XS=REAL(X,4); YS=REAL(Y,4)
 ENDIF
 CALL IGRPOLYGONSIMPLE(XS,YS,N)
 DEALLOCATE(XS,YS)
 ...
 ELSE
  XS=REAL(X,4); YS=REAL(Y,4)
 ENDIF
 CALL IGRPOLYGONCOMPLEX(XS,YS,N)
 DEALLOCATE(XS,YS)
