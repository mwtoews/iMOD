...
INTEGER,PARAMETER :: MAXLENVAR=52
CHARACTER(LEN=MAXLENVAR),POINTER,DIMENSION(:,:) :: VAR_TMP,DVAR
...
!    CALL IGRUNITS(MPW%XMIN,MPW%YMIN,MPW%XMAX,MPW%YMAX)
 ...
 IF(PRESENT(S1).AND.PRESENT(S2))THEN
  UTL_IMODVERSION=TRIM(UTL_IMODVERSION)//' ['//TRIM(UTL_SUBST(RVERSION,S1,S2))//' '//TRIM(CCONFIG)//']'
 ELSE
  UTL_IMODVERSION=TRIM(UTL_IMODVERSION)//' ['//TRIM(RVERSION)//' '//TRIM(CCONFIG)//']'
 ENDIF
 IF(LEXPDATE)THEN
 ...
 !###====================================================================
 LOGICAL FUNCTION UTL_PCK_READTXT(ICOL,STIME,ETIME,QT,FNAME,INDICATOR,THRESHOLD) 
 !###====================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ICOL,INDICATOR
 INTEGER(KIND=8),INTENT(IN) :: STIME,ETIME
 CHARACTER(LEN=*),INTENT(IN) :: FNAME,THRESHOLD
 REAL,INTENT(OUT) :: QT
 INTEGER :: IR,I,I1,I2,IU,NR,NC,IDATE,JDATE,NDATE,N,IOS,TTIME,ITYPE,IZ,IZMIN,IZMAX,DIZ,SDATE,EDATE
 REAL :: Q1,QQ,Z,TZ,BZ,DZ,F,Z1,NQ
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (1)
   ...
   TTIME=EDATE-SDATE
   IF(TTIME.LE.0)THEN
    CLOSE(IU); CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Timestep size to extract data is '//TRIM(ITOS(TTIME)),'Error')
    RETURN
   ENDIF
  !## boreholes/seismic
  CASE (2,3)
...
!###=========================================================================
 ...
 !###======================================================================
 LOGICAL FUNCTION UTL_DATA_CSV(TXT,VAR,IVAR,ICOL_VAR,IACT_VAR,CCNST)
 !###======================================================================
 IMPLICIT NONE
 ...
 INTEGER,ALLOCATABLE,DIMENSION(:),INTENT(INOUT) :: IVAR,ICOL_VAR,IACT_VAR
 ...
 CALL UTL_GENLABELSREAD(FNAME,VAR) !,IVAR,ICOL_VAR,IACT_VAR,CCNST)
 IF(NV.LE.0)THEN
 ...
 CHARACTER(LEN=256),POINTER,DIMENSION(:,:) :: VAR
 ...
 !###======================================================================
 SUBROUTINE UTL_GENLABELSREAD(FNAME,VAR,SKIPLINES,ILABELS)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: FNAME
 ...
 CHARACTER(LEN=*),POINTER,DIMENSION(:,:),INTENT(OUT) :: VAR
  ...
  DO I=1,NV
   ...
   IF(BPV(I).LE.LEN(STRING).AND. &
      BPV(I).LE.EPV(I))THEN
       ...
       J=(EPV(I)-BPV(I))+1; EPV(I)=BPV(I)+MIN(MAXLENVAR,J)-1
 ...
 !###======================================================================
 SUBROUTINE UTL_GENLABELSDEALLOCATE(VAR,IVAR,ICOL_VAR,IACT_VAR,CCNST)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),POINTER,DIMENSION(:,:),INTENT(INOUT) :: VAR
 CHARACTER(LEN=*),POINTER,DIMENSION(:),INTENT(INOUT) :: CCNST
...
! CALL IGRJOIN(X,Y2,X,MPW%YMIN); CALL IGRJOIN(X,Y1,X,MPW%YMAX)
 ...
 IF(LEX)THEN 
  CALL WMESSAGEBOX(OKONLY,COMMONOK,EXCLAMATIONICON,'iMOD cannot open more than 1000000 files simultaneously!','ERROR')
  UTL_GETUNIT=0
 ENDIF
