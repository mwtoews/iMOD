...
CONTAINS
 ...
 IF(PRESENT(STRING))THEN
  IF(LEN_TRIM(STRING(1)).NE.0)CALL WDIALOGTITLE(TRIM(STRING(1))) !#changes title of dialog window
  IF(LEN_TRIM(STRING(2)).NE.0)CALL WDIALOGPUTSTRING(IDCANCEL,TRIM(STRING(2))) !#changes text on close-button
  IF(LEN_TRIM(STRING(3)).NE.0)CALL WDIALOGPUTSTRING(IDHELP,TRIM(STRING(3))) !#changes text on help-button
  IF(LEN_TRIM(STRING(4)).NE.0)CALL WDIALOGPUTSTRING(IDOK,TRIM(STRING(4))) !#changes text on apply-button
 ENDIF
    ...
    SELECT CASE (MESSAGE%VALUE1)  
     CASE (ID_OPEN)
      IF(UTL_WSELECTFILE('All files (*.*)|*.*|',LOADDIALOG+MUSTEXIST+PROMPTON+ &
         DIRCHANGE+APPENDEXT,EFNAME,'Load All Files (*.*)'))CALL UTL_LISTOFFILES_MANIPULATE(FNAME,STRLEN,1,EFNAME)
     CASE (ID_DELETE)
      CALL WDIALOGGETMENU(IDF_MENU1,N,EFNAME); CALL UTL_LISTOFFILES_MANIPULATE(FNAME,STRLEN,-1,EFNAME)
     CASE (IDOK)
