...
CONTAINS
 ...
 DO IROW=1,IDF%NROW
  N=0; XC=IDF%X(1,IROW)
  DO ICOL=1,IDF%NCOL
   IF(IDF%X(ICOL,IROW).NE.XC.OR.ICOL.EQ.IDF%NCOL)THEN
    IF(ICOL.EQ.IDF%NCOL)N=N+1
    IF(N.GT.1)THEN
...
!    CALL IGRUNITS(MPW%XMIN,MPW%YMIN,MPW%XMAX,MPW%YMAX)
 ...
 REAL,INTENT(OUT) :: QT
 INTEGER :: IR,I,I1,I2,IU,NR,NC,IDATE,JDATE,NDATE,N,IOS,TTIME,ITYPE,IZMAX,SDATE,EDATE
