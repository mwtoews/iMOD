...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE UTL_LISTOFFILES(FNAME_IN,STRING)
 !###======================================================================
 IMPLICIT NONE
 ...
 CHARACTER(LEN=*),OPTIONAL,INTENT(IN),DIMENSION(5) :: STRING
 CHARACTER(LEN=STRLEN),POINTER,DIMENSION(:) :: FNAME => NULL()
 CHARACTER(LEN=STRLEN) :: EFNAME,WNAME,B1NAME,B2NAME,B3NAME
 INTEGER :: N,DID,I
 ...
 !#"String" contains 5 variables: title, 3 buttons, list-argument
 CALL WDIALOGLOAD(ID_DLISTOFFILES,ID_DLISTOFFILES)
 CALL WDIALOGTITLE('Extra files')
 IF(PRESENT(STRING))THEN
  IF(LEN_TRIM(STRING(1)).NE.0)CALL WDIALOGTITLE(STRING(1)) !#changes title of dialog window
  IF(LEN_TRIM(STRING(2)).NE.0)CALL WDIALOGPUTSTRING(IDCANCEL,STRING(2)) !#changes text on close-button
  IF(LEN_TRIM(STRING(3)).NE.0)CALL WDIALOGPUTSTRING(IDHELP,STRING(3)) !#changes text on help-button
  IF(LEN_TRIM(STRING(4)).NE.0)CALL WDIALOGPUTSTRING(IDOK,STRING(4)) !#changes text on apply-button
 ENDIF

 CALL WDIALOGPUTIMAGE(ID_OPEN,ID_ICONOPEN,1)
 CALL WDIALOGPUTIMAGE(ID_DELETE,ID_ICONDELETE,1)

 CALL UTL_MANIPULATEDLISTOFFILES(FNAME,STRLEN,0,EFNAME)
 ...
 DO
  ...
  SELECT CASE (ITYPE)
   ...
   CASE(PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)  
     CASE (ID_OPEN)
      IF(UTL_WSELECTFILE('All files (*.*)|*.*|',LOADDIALOG+MUSTEXIST+PROMPTON+ &
         DIRCHANGE+APPENDEXT,EFNAME,'Load All Files (*.*)'))CALL UTL_MANIPULATEDLISTOFFILES(FNAME,STRLEN,1,EFNAME)
     CASE (ID_DELETE)
      CALL WDIALOGGETMENU(IDF_MENU1,N,EFNAME); CALL UTL_MANIPULATEDLISTOFFILES(FNAME,STRLEN,-1,EFNAME)
     CASE (IDOK)
      !## copy adjusted filename
      IF(ASSOCIATED(FNAME_IN))DEALLOCATE(FNAME_IN)
      ALLOCATE(FNAME_IN(SIZE(FNAME)))
      DO I=1,SIZE(FNAME); FNAME_IN(I)=FNAME(I); ENDDO; EXIT
     CASE (IDHELP)
 ...
 !###======================================================================
 SUBROUTINE UTL_MANIPULATEDLISTOFFILES(FNAME,STRLEN,IADD,EFNAME)
 !###======================================================================
 IMPLICIT NONE
 ...
 CHARACTER(LEN=STRLEN),POINTER,DIMENSION(:) :: FNAME_BU !=> NULL()
 INTEGER :: I,J,ISEL
...
!## check double files !!!
 CALL WMESSAGEBOX(OKONLY,INFORMATIONICON,COMMONOK,'check double files','info')
 
  ALLOCATE(FNAME_BU(SIZE(FNAME)+1))
  DO I=1,SIZE(FNAME); FNAME_BU(I)=FNAME(I); ENDDO
  FNAME_BU(I)=EFNAME; ISEL=I
 ...
 IF(ASSOCIATED(FNAME))THEN
  CALL WDIALOGPUTMENU(IDF_MENU1,FNAME,SIZE(FNAME),ISEL)
 ...
 END SUBROUTINE UTL_MANIPULATEDLISTOFFILES
