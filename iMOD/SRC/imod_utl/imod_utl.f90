...
CONTAINS
 ...
 CHARACTER(LEN=*),OPTIONAL,INTENT(IN),DIMENSION(6) :: STRING
 ...
 IF(STRING(1).EQ.'IMODMANAGER')THEN
  ...
  !## all on left menu
  DO I=1,SIZE(FNAME); LRLIST(I)=I; ENDDO
 ELSE
 ...
 IF(PRESENT(STRING))THEN
  ...
  IF(LEN_TRIM(STRING(6)).NE.0)CALL WDIALOGPUTSTRING(IDF_STRING1,TRIM(STRING(6)))!## changes text on text field
 ENDIF
    ...
    SELECT CASE (MESSAGE%VALUE1)  
     CASE (ID_OPEN)
      IF(UTL_WSELECTFILE('Files ('//TRIM(STRING(1))//')|'//TRIM(STRING(1))//'|',LOADDIALOG+MUSTEXIST+PROMPTON+ &
         DIRCHANGE+APPENDEXT,EFNAME,'Load Files ('//TRIM(STRING(1))//')'))CALL UTL_LISTOFFILES_MANIPULATE(FNAME,STRLEN,1,EFNAME)
     CASE (ID_DELETE)
 ...
 !## check double files - to be added later !!!
