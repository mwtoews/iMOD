...
CONTAINS
 ...
 DO
  ...
  IERROR=WINFOERROR(LASTERROR) 
  IF(IERROR.NE.0)THEN
   IF(IERROR.EQ.ERRBUFFERSIZE)CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Too many files selected, please select less.','Error')
  ELSE 
   IF(WINFODIALOG(4).NE.1)THEN; FILEDIR=''; RETURN; ENDIF
   !## check extent ...
   I=INDEX(FILEDIR,'.',.TRUE.); IF(I.EQ.0)EXIT
