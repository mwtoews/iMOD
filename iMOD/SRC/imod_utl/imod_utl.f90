...
! CHARACTER(LEN=*),POINTER,DIMENSION(:,:),INTENT(OUT) :: VAR
 ...
 ALLOCATE(VARIABLE(NVV,INL+1:INCL))
 ML=INCL
 NVL=INL
 DO
  NVL=NVL+1
...
1 GAMMCF=EXP(-X+A*LOG(X)-GLN)*H
 ...
 CHARACTER(LEN=512) :: LINE,BATFILE,TXTFILE
 CHARACTER(LEN=2) :: NOC
 CHARACTER(LEN=52) :: LINE2
 INTEGER :: IU,I,INOC,IOS
 LOGICAL :: LEX
 ...
 LINE="wmic cpu get NumberOfCores, NumberOfLogicalProcessors/Format:List"//' > "'//TRIM(TXTFILE)//'"'
 ...
 INOC=0; NOC=1
 DO 
  READ(IU,'(A)',IOSTAT=IOS) LINE2
  INOC=INDEX(LINE2,'=')
  IF(IOS.NE.0)EXIT; IF(INOC.GT.0)EXIT
 ENDDO
 IF(NOC.GT.1)THEN
  NOC=TRIM(LINE2(INOC+1:))
  READ(NOC,*) NOCINT
 ENDIF
 !## delete result txt file
 CLOSE(IU,STATUS='DELETE')
