...
! X3=X1+X2
 ...
 INTEGER,INTENT(IN),OPTIONAL :: EXCLVALUE !## wrong value
 ...
 IF(IOS.NE.0)THEN
  ...
  STOP
 ENDIF
 WRITE(*,'(A,99(I8,A1))') TRIM(TXT)//'=',(IPOINTER(I),',',I=1,NPOINTER)
 ...
 !###======================================================================
 LOGICAL FUNCTION UTL_READPOINTER_REAL(IU,NPOINTER,XPOINTER,TXT,IOPT,EXCLVALUE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IOPT,IU !## ioptional
 REAL,INTENT(IN),OPTIONAL :: EXCLVALUE !## wrong value
 INTEGER,INTENT(OUT) :: NPOINTER
 ...
 READ(LINE,*,IOSTAT=IOS) (XPOINTER(I),I=1,NPOINTER)
 IF(IOS.NE.0)THEN
  WRITE(*,'(/1A/)') 'ERROR READING '//TRIM(TXT); STOP
 ENDIF
...
! write(*,*) RX*(360.0/(2.0*pi)),RY*(360.0/(2.0*pi)),RZ*(360.0/(2.0*pi))
 ...
 ALLOCATE(VARIABLE(NVV,INL+1:NVL)) !INCL))
 ML=NVL 
! ML=INCL
 NVL=INL
 DO
...
!    READ(STRING(BPV(I):EPV(I)),*) VARIABLE(I,NVL)
