...
USE WINTERACTER
USE MOD_UTL, ONLY : UTL_GETUNIT,JD,UTL_CAP,UTL_IDFGETDATE,UTL_IDATETOJDATE,ITOS,UTL_FILLARRAY,NEWLINE,UTL_CREATEDIR, &
   ...
   DO ICM=1,IDFM%NCOL
    ...
    IF(ICC2.GE.ICC1.AND.ICC2.LE.IDFC%NCOL.AND.ICC1.NE.0.AND.ICC2.NE.0)THEN
     ...
     IF(IDFM%IXV.EQ.0)THEN
      ...
      !## downscaling
      IF(IDOWN.EQ.1)THEN 
       IF(SCLTYPE_UP.EQ.5.AND.SVALUE.NE.IDFC%NODATA)SVALUE=SVALUE/((DXM*DYM)/(DXC*DYC))
      ENDIF
...
!      ENDIF
 ...
 REAL :: XC,XX,YY,YC,XCIDF,YCIDF,XMID,YMID,Y
...
!  IREC=IREC+1
 ...
 IF(IDF%IEQ.EQ.0)THEN
  ...
  DX=X-IDF%XMIN; IF(X.GT.IDF%XMIN.AND.X.LT.IDF%XMAX)ICOL=INT(DX/IDF%DX)+1
  DY=IDF%YMAX-Y; IF(Y.GT.IDF%YMIN.AND.Y.LT.IDF%YMAX)IROW=INT(DY/IDF%DY)+1
!  IF(MOD(DX,IDF%DX).EQ.0.0)ICOL=ICOL-1
!  IF(MOD(DY,IDF%DY).EQ.0.0)IROW=IROW-1
