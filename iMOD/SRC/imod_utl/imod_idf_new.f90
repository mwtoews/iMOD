...
!   SVALUE=SVALUE+(1.0-NFRAC)
 ...
 DO IR=IR1,IR2  !## loop over row in coarser child
  ...
  DO IC=IC1,IC2 !## loop over col in coarser child idf
   ...
   IF(IROW.EQ.0)THEN
    !## if file to be inteporlated
    IF(YMID.LT.IDFM%YMIN)IROW=IDFM%NROW
    IF(YMID.GT.IDFM%YMAX)IROW=1
   ENDIF
   IF(ICOL.EQ.0)THEN
    IF(XMID.LT.IDFM%XMIN)ICOL=1
    IF(XMID.GT.IDFM%XMAX)ICOL=IDFM%NCOL
   ENDIF
