...
!  IF(IDATA.NE.1)IDF%IXV=0  !## initialize %ixv in case no data is read from idf
  ...
  ENDIF
! ELSE
!  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Error occured OPENING IDF'//CHAR(13)//TRIM(FNAME),'Error')
 ENDIF
...
! IF(.NOT.IDFREAD(IDF(1),FNAME,0,IQ=IOPTIONAL))THEN
!  RETURN
! ENDIF
 !## try to allocate idf(1) and read the entire idf
! IF(IDFALLOCATEX(IDF(1)))THEN
!  CLOSE(IDF(1)%IU); IDF(1)%IU=0
! IF(IDFREAD(IDF(1),FNAME,1,IQ=IOPTIONAL))THEN
