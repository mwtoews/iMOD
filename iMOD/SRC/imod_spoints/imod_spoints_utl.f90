...
! ISHAPE=SHPI
 ...
 SELECT CASE (SHP%POL(SHPI)%ITYPE)
  ...
  CASE (ID_POLYGON,ID_RECTANGLE)
   CALL WDIALOGPUTSTRING(IDF_LABEL11,'Top-Level (enter IDF/numeric value)')
   ...
   CALL WDIALOGPUTDOUBLE(IDF_REAL1,SPNT(SHPI)%IDX)  !DISTX
   CALL WDIALOGPUTDOUBLE(IDF_REAL3,SPNT(SHPI)%IDY)  !DISTY
  ...
  CASE (ID_CIRCLE,ID_POINT)
   ...
   CALL WDIALOGFIELDSTATE(IDF_REAL1,1)
   CALL WDIALOGFIELDSTATE(IDF_REAL3,1)
   CALL WDIALOGPUTDOUBLE(IDF_REAL1,SPNT(SHPI)%IRADIUS)  !RADIUS
   CALL WDIALOGPUTDOUBLE(IDF_REAL3,SPNT(SHPI)%ISX)      !SAMPL.
  ...
  CASE (ID_LINE)
   ...
   CALL WDIALOGPUTDOUBLE(IDF_REAL1,SPNT(SHPI)%ISX)  !SAMPL.

  CASE (ID_GRID)
   CALL WDIALOGPUTSTRING(IDF_LABEL11,'Reference IDF')
 ...
 CALL WDIALOGPUTINTEGER(IDF_INTEGER5,SPNT(SHPI)%ISZ)  !Z.SAMPL.
 ...
 DO SHPI=1,SHP%NPOL
  ...
  SELECT CASE (SHP%POL(SHPI)%ITYPE)
   ...
   CASE (ID_POLYGON,ID_RECTANGLE)
    CALL WDIALOGGETDOUBLE(IDF_REAL1,SPNT(SHPI)%IDX)  !DISTX
    CALL WDIALOGGETDOUBLE(IDF_REAL3,SPNT(SHPI)%IDY)  !DISTY
   ...
   CASE (ID_POINT)
    CALL WDIALOGGETDOUBLE(IDF_REAL1,SPNT(SHPI)%IRADIUS)  !RADIUS
    CALL WDIALOGGETDOUBLE(IDF_REAL3,SPNT(SHPI)%ISX)      !SAMPL.
   ...
   CASE (ID_LINE)
    CALL WDIALOGGETDOUBLE(IDF_REAL1,SPNT(SHPI)%ISX)  !SAMPL.
    IF(SPNT(SHPI)%ISX.LE.0.0D0)THEN
   ...
   CASE (ID_GRID)
    CALL WDIALOGGETDOUBLE(IDF_REAL1,SPNT(SHPI)%IDZ)  !DISTZ
    IF(SPNT(SHPI)%IDZ.LT.0.0D0)THEN
     CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'You need to specify a zero or positive value for Offset.','Error')
     RETURN
    ENDIF
