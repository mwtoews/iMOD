...
DO WHILE(.TRUE.)
 ...
 ELSE
  ...
  SELECT CASE(ITYPE)
   ...
   CASE(MOUSEMOVE)
    IF(IDOWN.EQ.1)THEN
     IMOVED=1; CALL IDFMOVEIT(MESSAGE%GX,MESSAGE%GY,IWIN_ID)
    ENDIF
   CASE (MOUSEBUTUP)
...
!!## store previous drwable setting
!DDRTYPE=WInfoDrawable(DRAWABLETYPE)
!DRWID=WInfoDrawable(DRAWABLEID)
!DRWIDD=WInfoDrawable(DRAWABLEDIALOG)
! getekentn in ibitmap niet i nmpw%ibitmap --- moet ook init() routine aanroepen voro move?

!## reset drawable of 2D-canvas
!CALL WINDOWSELECT(0)
...
REAL :: FZ,XC1,YC1,XC2,YC2,XC3,YC3,XMIN,XMAX,YMIN,YMAX,Y,DX,DY
LOGICAL :: LEX
  ...
  DO IPLOT=1,MXMPLOT
   IF(ACTLIST(IPLOT).EQ.1)THEN
    SELECT CASE (MP(IPLOT)%IPLOT)
     !## idf,mdf,udf
     CASE (1,5,7) !## IF(MP(IPLOT)%IPLOT.EQ.1.OR.5)THEN
      !## get idf or mdf file
      LEX=.TRUE.
     ...
     CASE (4) !## ELSEIF(MP(IPLOT)%IPLOT.EQ.4)THEN
      I=I+1; CALL ISGPLOTMINMAX(MP(IPLOT)%IDFNAME,XMIN,XMAX,YMIN,YMAX)
    END SELECT
   ENDIF
  ENDDO
  IF(I.GT.0)THEN
   DX=(XMAX-XMIN)/25.0; DY=(YMAX-YMIN)/25.0
   MPW%XMIN=XMIN-DX
   MPW%YMIN=YMIN-DY
   MPW%XMAX=XMAX+DX
   MPW%YMAX=YMAX+DY
