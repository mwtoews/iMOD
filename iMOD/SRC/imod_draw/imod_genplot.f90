...
USE MOD_UTL, ONLY : UTL_WAITMESSAGE,UTL_MESSAGEHANDLE,UTL_GETUNIT,ITOS,UTL_GETRELEVANTDIR,UTL_FILLARRAY,UTL_IDFGETCLASS,UTL_SETTEXTSIZE,&
  UTL_READARRAY,UTL_WSELECTFILE,RTOS,UTL_INSIDEPOLYGON,MAXLEN,UTL_GENLABELSREAD,UTL_GENLABELSGET,UTL_GENLABELSDEALLOCATE, &
  UTL_GENLABELSWRITE
...
CONTAINS
 ...
 !###======================================================================
 SUBROUTINE TOPOGENINIT(GENNAME,LPLOT,LDEACTIVATE,GENCOLOUR,GENTHICKNESS)
 !###======================================================================
 IMPLICIT NONE
 ...
 DO IGEN=1,NIGEN
  ...
  SELECT CASE (GENFNAME(I:I+2))
   ...
   CASE ('SHP')
    ...
    IF(.NOT.TOPOSHPTOGEN(TRIM(GENFNAME),LIPF))CYCLE
    IF(LIPF)THEN
     GEN(IPLOT)%ITYPE=2; GENFNAME=GENFNAME(:INDEX(GENFNAME,'.',.TRUE.)-1)//'.IPF'
    ELSE
     GEN(IPLOT)%ITYPE=1; GENFNAME=GENFNAME(:INDEX(GENFNAME,'.',.TRUE.)-1)//'.GEN'
    ENDIF
   CASE DEFAULT
...
! IF(WMENUGETSTATE(ID_GENOPTIONS,1).NE.I)CALL WMENUSETSTATE(ID_GENOPTIONS,1,I)
  ...
  IF(GEN(IGEN)%IACT.AND.GEN(IGEN)%ISEL)THEN
   ...
   IF(LEX)THEN
    ...
    IF(GEN(IGEN)%ITYPE.EQ.1)THEN
     ...
     IF(IOS.NE.0)THEN
      CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open/read '//CHAR(13)//TRIM(GEN(IGEN)%GENFNAME),'Error')
      EXIT
     ENDIF
 ...
 DO
  ...
  CALL IGRMOVETO(FROMX,FROMY)
  DO
   READ(IU,*,IOSTAT=IOS) TOX,TOY; IF(IOS.NE.0)EXIT
!   IF(TOX  .GE.MPW%XMIN.OR.TOX  .LE.MPW%XMAX.OR.TOY  .GE.MPW%YMIN.OR.TOY  .LE.MPW%YMAX.OR. &
!      FROMX.GE.MPW%XMIN.OR.FROMX.LE.MPW%XMAX.OR.FROMY.GE.MPW%YMIN.OR.FROMY.LE.MPW%YMAX)THEN
!    CALL IGRJOIN(FROMX,FROMY,TOX,TOY)
    CALL IGRLINETO(TOX,TOY)
!   ENDIF
...
! USE gdal 
 ...
 !###===============================================================================
 LOGICAL FUNCTION TOPOSHPTOGEN(FNAME,LIPF)
 !###===============================================================================
 IMPLICIT NONE
 INTEGER,PARAMETER :: IMAXWIDTH=512 !## maximal widht of columns in dbf file
 CHARACTER(LEN=*),INTENT(IN) :: FNAME
 LOGICAL,INTENT(OUT) :: LIPF
 INTEGER :: I,J,K,IOS,IREC
 INTEGER,DIMENSION(3) :: IU,JU
 DOUBLE PRECISION :: X,Y,Z,M
 DOUBLE PRECISION,ALLOCATABLE,DIMENSION(:) :: MP,ZP 
 ...
 CHARACTER(LEN=1),ALLOCATABLE,DIMENSION(:,:) :: DBFSTRING
 TYPE DBFTYPE
 ...
 !## open shapefile
 IU(1)=UTL_GETUNIT()
 CALL OSD_OPEN(IU(1),FILE=FNAME,STATUS='OLD',FORM='UNFORMATTED',ACTION='READ,DENYWRITE',ACCESS='TRANSPARENT',IOSTAT=IOS) 
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot read '//CHAR(13)//TRIM(FNAME),'Error')
  GOTO 10
 ENDIF
 ...
 IF(IU(2).GT.0)THEN
  ...
  IF(IOS.NE.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open '//CHAR(13)//FNAME(:INDEX(FNAME,'.',.TRUE.)-1)//'.dat','Error')
   GOTO 10
  ENDIF
 ENDIF
 !## open ipf file - in case of points
 JU(3)=UTL_GETUNIT()
 CALL OSD_OPEN(JU(3),FILE=FNAME(:INDEX(FNAME,'.',.TRUE.)-1)//'.ipf',ACTION='WRITE',STATUS='UNKNOWN',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot open '//CHAR(13)//FNAME(:INDEX(FNAME,'.',.TRUE.)-1)//'.ipf','Error')
  GOTO 10
 ENDIF
 ...
 IF(IU(2).GT.0)THEN
  ...
  I=SUM(DBF%COLWIDTH); ALLOCATE(DBFSTRING(I,NRECS),STAT=IOS); DBFSTRING=''
  IF(IOS.NE.0)THEN
   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot allocate memory for reading dbf file.','Error')
   GOTO 10
  ENDIF
  WRITE(FORM,'(A1,I5.5,A3)') '(',SIZE(DBFSTRING,1),'A1)'
!  I=MAXVAL(DBF%COLWIDTH) !; I=SIZE(DBFSTRING,1)
!  ALLOCATE(STR(I),STAT=IOS) !; STR='' !,STAT=IOS); STRING=''
!  IF(IOS.NE.0)THEN
!   CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot allocate memory for reading column width from file.','Error')
!   GOTO 10
!  ENDIF
  ...
  !## write header in ipf-file
  IF(JU(3).GT.0)THEN
   LINE=TRIM(ITOS(NRECS))
   WRITE(JU(3),'(A)') TRIM(LINE)
   LINE=TRIM(ITOS(MAXCOL+2))
   WRITE(JU(3),'(A)') TRIM(LINE)
   WRITE(JU(3),'(A)') 'x'
   WRITE(JU(3),'(A)') 'y'
   DO I=1,MAXCOL
    LINE=CHAR(39)//TRIM(ADJUSTL(DBF(I)%COLNAME))//CHAR(39)
    WRITE(JU(3),'(A)') TRIM(LINE)
   ENDDO
   WRITE(JU(3),'(A)') '0,txt'
  ENDIF
  ...
  DO IREC=1,NRECS
   ...
   DO I=1,LEN_TRIM(LINE); DBFSTRING(I,IREC)=LINE(I:I); ENDDO
 ...
 DO
  ...
  SELECT CASE (ISHP)
   ...
   CASE (3,5,13,15,23)
    ...
    IF(ISHP.EQ.13.OR.ISHP.EQ.23)THEN
...
!     IF(ISHP.EQ.13.OR.ISHP.EQ.15)WRITE(JU(1),'(A3)') 'END'
   ...
   CASE (1) 
    ...
    READ(IU(1)) X,Y
    K=K+1
    WRITE(JU(1),*) TRIM(CID(K)),X,Y,K
    IF(JU(3).GT.0)THEN 
     WRITE(LINE,FORM) (DBFSTRING(I,K),I=1,SIZE(DBFSTRING,1))
     WRITE(JU(3),'(2(F10.2,A1),A)') X,',',Y,',',TRIM(LINE) 
    ENDIF
   ...
   CASE (8)
    ...
    DO I=1,NUMPOINTS
     READ(IU(1)) X,Y
     K=K+1
     WRITE(JU(1),*) TRIM(CID(K)),X,Y
     IF(JU(3).GT.0)THEN 
      WRITE(LINE,FORM) (DBFSTRING(J,K),J=1,SIZE(DBFSTRING,1))
      WRITE(JU(3),'(2(F10.2,A1),A)') X,',',Y,',',TRIM(LINE) 
     ENDIF
    ENDDO
 ...
 IF(ALLOCATED(DBFSTRING))DEALLOCATE(DBFSTRING)

 TOPOSHPTOGEN=.TRUE.

10 DO I=1,SIZE(IU); IF(IU(I).GT.0)CLOSE(IU(I)); ENDDO
   DO I=1,SIZE(JU); IF(JU(I).GT.0)CLOSE(JU(I)); ENDDO
...
!    CALL IGRUNITS(MPW%XMIN,MPW%YMIN,MPW%XMAX,MPW%YMAX)
 ...
 CHARACTER(LEN=MAXLEN),POINTER,DIMENSION(:,:) :: VAR
 ...
 IF(ABS(MP(IPLOT)%IEQ).GE.1.OR.MP(IPLOT)%ILEG.EQ.1)THEN
  !## reading labels
  CALL UTL_GENLABELSREAD(MP(IPLOT)%IDFNAME (:INDEX(MP(IPLOT)%IDFNAME,'.',.TRUE.)-1)//'.dat',VAR,NL,NV)
 ENDIF
 IF(NV.GT.0)THEN
  ALLOCATE(IVAR(NV))
...
!    CALL GENLABELSPLOT(JL,N,IPLOT,TWIDTH,THEIGHT) 
  ...
  ELSE
   ...
   IF(LEX)THEN
    ...
    !## put labels if jl.ne.0
    CALL IGRCOLOURN(WRGB(0,0,0)); CALL IGRFILLPATTERN(OUTLINE); CALL IGRLINETYPE(SOLIDLINE); CALL IGRLINEWIDTH(1)
    CALL GENLABELSPLOT(JL,N,IPLOT,TWIDTH,THEIGHT,VAR,IVAR) 
   ENDIF
 ...
 !###======================================================================
 INTEGER FUNCTION GENLABELGETCOLOR(JL,IPLOT,VAR)
 !###======================================================================
 IMPLICIT NONE
 ...
 ELSE
  I=INFOERROR(1)
  IF(JL.NE.0)CALL ISTRINGTOREAL(VAR(MP(IPLOT)%IATTRIB,JL),X)
 ...
 !###======================================================================
 SUBROUTINE GENLABELSPLOT(JL,N,IPLOT,TWIDTH,THEIGHT,VAR,IVAR)
 !###======================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN),POINTER,DIMENSION(:,:) :: VAR
...
! !###======================================================================
! SUBROUTINE GENDATAGRID(FNAME,VARS) 
! !###======================================================================
! IMPLICIT NONE
! CHARACTER(LEN=*),INTENT(IN) :: FNAME
...
! CHARACTER(LEN=*),POINTER,DIMENSION(:,:),INTENT(IN),OPTIONAL :: VARS
! CHARACTER(LEN=MAXLEN),POINTER,DIMENSION(:,:) :: DVAR,VAR_TMP
! 
! IF(ASSOCIATED(VARS))VAR=VARS
! 
