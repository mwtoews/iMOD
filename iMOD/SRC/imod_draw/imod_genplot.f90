...
! USE gdal 
 ...
 CHARACTER(LEN=2512) :: LINE,STRING
...
! CHARACTER(LEN=:),ALLOCATABLE :: STRING
 ...
 IF(IU(2).GT.0)THEN
  ...
  I=SUM(DBF%COLWIDTH); ALLOCATE(DBFSTRING(I,NRECS)); DBFSTRING=''
  WRITE(FORM,'(A1,I5.5,A3)') '(',SIZE(DBFSTRING,1),'A1)'
!  I=SIZE(DBFSTRING,1); ALLOCATE(CHARACTER(LEN=I) :: STRING)
!  WRITE(*,*) LEN(STRING)
 ...
 DO
  ...
  SELECT CASE (ISHP)
   ...
   CASE (1) 
    ...
    IF(JU(3).GT.0)THEN 
     WRITE(STRING,FORM) (DBFSTRING(I,K),I=1,SIZE(DBFSTRING,1))
     WRITE(JU(3),'(2(F10.2,A1),A)') X,',',Y,',',TRIM(STRING) 
    ENDIF
   ...
   CASE (8)
    ...
    DO I=1,NUMPOINTS
     ...
     IF(JU(3).GT.0)THEN 
      WRITE(STRING,FORM) (DBFSTRING(J,K),J=1,SIZE(DBFSTRING,1))
      WRITE(JU(3),'(2(F10.2,A1),A)') X,',',Y,',',TRIM(STRING) 
     ENDIF
    ENDDO
