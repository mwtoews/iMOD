 ...
 IF(IU(2).GT.0)THEN
  ...
  DO IREC=1,NRECS
   ...
   DO I=1,MAXCOL
    ...
    ENDIF
   ENDDO
   READ(IU(2),IOSTAT=IOS) DAY !,J=1,1)
   IF(IOS.NE.0)EXIT
   WRITE(JU(2),'(A)') TRIM(LINE)
  ENDDO
 ENDIF
...
!    CALL GENLABELSPLOT(JL,N,IPLOT,TWIDTH,THEIGHT) 
 ...
 IF(UTL_INSIDEPOLYGON(XC,YC,X,Y,XPOL,YPOL,N).NE.1)THEN
  INT=10
DOLOOP: DO
   X1=MINVAL(X(1:N))
   Y1=MINVAL(Y(1:N))
   ...
   DO I=1,INT
    ...
    DO J=1,INT
     XC=X1+DX     
     IF(UTL_INSIDEPOLYGON(XC,YC,X,Y,XPOL,YPOL,N).NE.1)EXIT DOLOOP
    ENDDO  
   ENDDO
   INT=INT+10
  ENDDO DOLOOP
  ...
  SELECT CASE (ITYPE)
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     CASE (IDHELP)
       CALL IMODGETHELP('4.2.1','IPF Configure') 
     CASE (IDOK)
   ...
   CASE (PUSHBUTTON)
    SELECT CASE (MESSAGE%VALUE1)
     ...
     CASE (ID_RENAME)
      ...
      ENDIF
     CASE (IDHELP)
      CALL IMODGETHELP('4.1.4','IDF Edit1')
     !## get data
     CASE (IDOK)
