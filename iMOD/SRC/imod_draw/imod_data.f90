...
!   WMENUGETSTATE(ID_TRANSPARANTNODATAIDF,2).EQ.0)THEN
 N=0
 DO IPLOT=1,MXMPLOT
  IF(MP(IPLOT)%IPLOT.EQ.1.OR.MP(IPLOT)%IPLOT.EQ.4)N=N+1
 ENDDO
 IF(N.GT.10)THEN
  CALL WMESSAGEBOX(OKONLY,QUESTIONICON,COMMONYES,'Are you sure to display '//TRIM(ITOS(N))//' IDF files in transparancy?'//CHAR(13)// &
