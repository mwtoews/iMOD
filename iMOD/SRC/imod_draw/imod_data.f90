...
!                                       ID_HIGHACCURACY,ID_EXCELLENTACCURACY/
 ...
 IF(WMENUGETSTATE(ID_TOPOGRAPHY,2).EQ.1.AND. &
    WMENUGETSTATE(ID_TOPOTRANSPARANCY0,2).EQ.1)CALL TOPO1DRAW()
 ...
 CALL WINDOWSELECT(0)
 IF(WMENUGETSTATE(ID_TOPOGRAPHY,2).EQ.1.AND. &
    WMENUGETSTATE(ID_TOPOTRANSPARANCY0,2).EQ.0)CALL TOPO1DRAW()
...
INTEGER :: NC1,NC2,NR1,NR2,ANROW,ANCOL,IRAT,IRAT1,IDY,IDX,I,IOS,IR,IG,IB, &
           J,IWID,IHGT,JBITMAP,KBITMAP,ITYPE,IROW,ICOL,ICLR,ITNODATA,ITRANS
INTEGER :: IDTYPE,IDHANDLE,SDX,SDY,IDX1,IDX2,IDY1,IDY2,IWHITE
...
SDX     =WINFOSCREEN(SCREENWIDTH)     ! ()   SCREENWIDTH
SDY     =WINFOSCREEN(SCREENHEIGHT)    ! ()   SCREENHEIGHT
...
IF(IP(1).EQ.1.AND.LPLOT)THEN
 ...
 X=REAL(ANCOL,8)/REAL(IDX,8); IWID=CEILING(X)
 X=REAL(ANROW,8)/REAL(IDY,8); IHGT=CEILING(X)
...
IF(ITNODATA.EQ.1)CALL IDFDEALLOCATESX(IDF)
...
INTEGER,INTENT(INOUT),DIMENSION(NDIM) :: IBMPDATA
...
DO I=1,NDIM
 IF(KBMPDATA(I).NE.IWHITE)THEN
  IBMPDATA(I)=KBMPDATA(I)
! ELSE
!  IBMPDATA(I)=IWHITE
 ENDIF
END DO
