...
USE WINTERACTER
USE MODPLOT
USE MOD_UTL, ONLY : UTL_GETAXESCALES,UTL_GETFORMAT
IMPLICIT NONE
REAL :: R1,R2,R3
INTEGER,PARAMETER :: N=10
INTEGER :: I,J,M,NSX,NSY
...
CALL UTL_GETAXESCALES(MPW%XMIN+DX,MPW%YMIN+DX,MPW%XMAX-DX,MPW%YMAX-DX,SXVALUE,NSX,SYVALUE,NSY)
...
DO
 ...
 IF(V1.GT.MPW%XMIN+DXX*(R1/100.0))THEN
  ...
  J=INDEX(FRM,'.0)',.TRUE.)
  IF(J.EQ.0)THEN
   CALL WGRTEXTREAL(V1,Y1-3*DY,V1/1000.0,FRM)
   CALL WGRTEXTREAL(V1,Y2+3*DY,V1/1000.0,FRM)
  ELSE
   CALL WGRTEXTINTEGER(V1,Y1-3*DY,INT(V1/1000.0))
   CALL WGRTEXTINTEGER(V1,Y2+3*DY,INT(V1/1000.0))
  ENDIF
 ENDIF
END DO
...
DO
 ...
 IF(V1.GT.MPW%YMIN+DXX*(R1/100.0))THEN
  ...
  J=INDEX(FRM,'.0)',.TRUE.)
  IF(J.EQ.0)THEN
   CALL WGRTEXTREAL(X1-3*DX,V1,V1/1000.0,FRM)
   CALL WGRTEXTREAL(X2+3*DX,V1,V1/1000.0,FRM)
  ELSE
   CALL WGRTEXTINTEGER(X1-3*DX,V1,INT(V1/1000.0))
   CALL WGRTEXTINTEGER(X2+3*DX,V1,INT(V1/1000.0))
  ENDIF
 ENDIF
END DO
...
X1=0.0; X2=(WX2-WX1)/1.25 !/5.0
CALL UTL_GETAXESCALES(X1,0.0,X2,1.0,SXVALUE,NSX,SYVALUE,NSY)

XI=SXVALUE(2)-SXVALUE(1); N=NSX
...
IF(XFCT.EQ.1000.0)CALL WGRTEXTSTRING((WX1+WX2)/2.0,Y,'kilometers')
!IF(XFCT.EQ.1000.0)CALL WGRTEXTSTRING((WX1+WX2)/2.0,Y,'meters')
