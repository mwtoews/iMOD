...
USE MOD_LEGEND, ONLY : LEGCREATE,LEGINITCLR,LEGDEFAULT
USE MOD_LEGEND_UTL, ONLY : LEGREAD
...
DO IDF=1,NIDF
 ...
 IF(PRESENT(LEGNAME))THEN
  IF(LEGNAME.NE.'')THEN
   CALL LEGREAD(MP(IPLOT)%LEG,LEGNAME,IOS)
...
!  MP(IPLOT)%UNITS  =0    !nog vrij te gebruiken
  ...
  CASE (4) !## isg ELSEIF(MP(IPLOT)%IPLOT.EQ.4)THEN
...
!  MP(IPLOT)%FADEOUT=0    !nog vrij te gebruiken
 ...
 IF(LLEG)THEN
  SELECT CASE (MP(IPLOT)%IPLOT)
   CASE (1:3,5,6)
    CALL LEGCREATE(IPLOT,'LIN','ALE')
  END SELECT
  CALL LEGINITCLR(IPLOT)
 ENDIF
...
!  IF(J.NE.I.AND.ITOPO.EQ.1)CALL WMENUSETSTATE(ID_TOPOGRAPHY,2,0)
!  IF(J.EQ.I.AND.ITOPO.EQ.1)CALL WMENUSETSTATE(ID_TOPOGRAPHY,2,1)
  IF(J.EQ.I)THEN
...
!  CALL IR1DRAWSHAPES(2)
 ...
 IF(WMENUGETSTATE(ID_SOLIDS,2).EQ.1)THEN !.AND.WMENUGETSTATE(ID_PROFILE,2).EQ.1)THEN
  CALL IGRSELECT(DRAWBITMAP,MPW%IBITMAP)
  CALL IGRPLOTMODE(MODECOPY)
...
!## was nodig om in variabele te stoppen??? --- door negatieve character lengte!!
CALL IGRAREA(0.0,0.0,1.0,1.0)
CALL IGRUNITS(MPW%XMIN,MPW%YMIN,MPW%XMAX,MPW%YMAX)
...
!###======================================================================
SUBROUTINE IDFPLOT_CONTOUR(IDF,LEG,DMIN,DMAX,XMAX,XMIN,YMAX,YMIN,IP,THICKNESS)
!###======================================================================
USE WINTERACTER
USE MOD_IDF_PAR, ONLY : IDFOBJ
USE MOD_IDF, ONLY : IDFREAD,IDFDEALLOCATEX
USE MODPLOT, ONLY : LEGENDOBJ,CONT
USE MOD_IDF, ONLY : IDFREADPART,UTL_IDFGETCLASS,IDFALLOCATEX !,IDFGETVAL
...
CALL UTL_SETTEXTSIZE(TWIDTH,THEIGHT,REAL(THICKNESS)*0.01) !20) !REAL(MP(IPLOT)%TSIZE)*0.01)
...
DO I=1,NLAB
 ...
 CALL IDFPLOT_CLEANBOX(CONT(I)%XLAB,CONT(I)%YLAB,CONT(I)%ALAB,DX,DYS*0.8,WRGB(255,255,200))
 CALL WGRTEXTORIENTATION(ALIGNCENTRE,ANGLE=CONT(I)%ALAB) !,0)
! CALL IGRCOLOURN(WRGB(225,255,255))
! CALL WGRTEXTFONT(FFHELVETICA,WIDTH=TWIDTH,HEIGHT=THEIGHT,ISTYLE=FSBOLD)
...
IF(IP(1).EQ.1.AND.LPLOT)THEN
 ...
 IF(ITNODATA.EQ.1)THEN
  ...
  !## create array
  ALLOCATE(IBMPDATA(IWID*IHGT))!,KBMPDATA(IWID*IHGT))
...
IF(IP(2).NE.0.OR.IP(3).NE.0.OR.IP(4).NE.0)THEN
 ...
 IF(IP(2).EQ.1)THEN
  ...
  CALL IDFPLOT_CONTOUR(IDF,LEG,DMIN,DMAX,XMAX,XMIN,YMAX,YMIN,IP,THICKNESS)
 ENDIF
 ...
 IF(IP(3).EQ.1)THEN
  ...
  CALL WINDOWSELECT(0); CALL WINDOWOUTSTATUSBAR(4,TRIM(WAITTXT)//'0%')
  CALL IDFPLOT_VECTOR(IDF,LEG,UNITS,DMIN,DMAX,XMAX,XMIN,YMAX,YMIN,IP)
 ENDIF
 ...
 IF(IP(4).EQ.1)THEN
  ...
  CALL WINDOWSELECT(0); CALL WINDOWOUTSTATUSBAR(4,TRIM(WAITTXT)//'0%')
  CALL IDFPLOT_TEXTING(IDF,DMIN,DMAX,XMAX,XMIN,YMAX,YMIN,THICKNESS)
 ENDIF
...
!###======================================================================
SUBROUTINE IDFPLOT_TEXTING(IDF,DMIN,DMAX,XMAX,XMIN,YMAX,YMIN,THICKNESS)
!###======================================================================
USE WINTERACTER
USE MOD_IDF_PAR, ONLY : IDFOBJ
USE MOD_UTL, ONLY : UTL_SETTEXTSIZE
...
!###======================================================================
SUBROUTINE IDFPLOT_VECTOR(IDF,LEG,UNITS,DMIN,DMAX,XMAX,XMIN,YMAX,YMIN,IP)
!###======================================================================
USE WINTERACTER
USE MOD_IDF_PAR, ONLY : IDFOBJ
...
USE MOD_SOF, ONLY : SOF_COMPUTE_GRAD,SOF_COMPUTE_GRAD_3D,SOF_COMPUTE_GRAD_STEEPEST
IMPLICIT NONE
TYPE(IDFOBJ),INTENT(INOUT) :: IDF
TYPE(LEGENDOBJ),INTENT(INOUT) :: LEG
REAL,INTENT(IN) :: XMAX,XMIN,YMAX,YMIN
...
!  IF(IDF%IVF.EQ.0)CALL SOF_COMPUTE_GRAD_STEEPEST(IDF,ICOL,IROW,DZDX,DZDY)
  ...
  IF(IP(1).EQ.0.AND.IP(2).EQ.0)THEN
   ...
   IF(F.NE.IDF%NODATA)THEN
    ...
    IF(F.NE.IDF%NODATA)THEN; DMIN=MIN(DMIN,F); DMAX=MAX(DMAX,F); ENDIF
   ELSE
