...
ELSE
 ...
 ITOPO=WMENUGETSTATE(ID_TOPOGRAPHY,2)
  ...
  IF(IDFREAD(MP(IPLOT)%IDF,MP(IPLOT)%IDFNAME,0))THEN
   ...
   IF(MP(IPLOT)%IDF%XMIN.LT.MPW%XMAX.AND.MP(IPLOT)%IDF%XMAX.GE.MPW%XMIN.AND. &
    ...
    IF(WMENUGETSTATE(ID_TRANSPARANTIDF,2).EQ.0.AND.   &
     ...
     I=I+1  
     CALL IDFPLOTAREA(MXMPLOT,XMIN,YMIN,XMAX,YMAX,PLOTAREA,I,LPLOT)
    ENDIF
    !## plot anyhow
...
USE MOD_UTL, ONLY : RTOS,UTL_SETTEXTSIZE
IMPLICIT NONE
...
INTEGER :: I,IROW,ICOL,ICLR
REAL :: F,TWIDTH,THEIGHT,DXS,DYS,DX
...
DO I=0,LEG%NCLR
 ...
 CALL IDFPLOT_COMPCONTOUR2(IDF%X,IDF%NCOL,IDF%NROW,XC,YC,DELR,DELC,LEG%CLASS(I),IDF%NODATA)
! CALL IDFPLOT_COMPCONTOUR(IDF%X,IDF%NCOL,IDF%NROW,XC,YC,DELR,DELC,LEG%CLASS(I),IDF%NODATA)
ENDDO
...
DO I=1,NLAB
 STR=RTOS(CONT(I)%VLAB,'F',2)
 DX=WGRTEXTLENGTH(' '//TRIM(STR)//' ',0)*DXS
 CALL IDFPLOT_CLEANBOX(CONT(I)%XLAB,CONT(I)%YLAB,CONT(I)%ALAB,DX,DYS*0.8,WRGB(255,255,255)) !WRGB(255,255,200))
 CALL WGRTEXTORIENTATION(ALIGNCENTRE,ANGLE=CONT(I)%ALAB) !,0)
...
!###======================================================================
SUBROUTINE IDFPLOT_COMPCONTOUR2(A,NCOL,NROW,XC,YC,DELR,DELC,V,XNODATA)
!###======================================================================
USE MODPLOT, ONLY : MPW
IMPLICIT NONE
INTEGER,INTENT(IN) :: NCOL,NROW
REAL,INTENT(IN) :: XNODATA,V
REAL,INTENT(IN),DIMENSION(NCOL,NROW) :: A
REAL,INTENT(IN),DIMENSION(NCOL) :: DELR,XC
REAL,INTENT(IN),DIMENSION(NROW) :: DELC,YC
REAL,DIMENSION(4) :: XS,YS
INTEGER :: I,II,INILAB,IROW,ICOL
REAL :: A1,A2,A3,A4,X1,X2,X3,X4,Y1,Y2,Y3,Y4,XMIN,YMIN,XMAX,YMAX,DX,DY
...
DO ICOL=1,NCOL-1
 DO IROW=2,NROW
  ...
  A1=A(ICOL,IROW); A2=A(ICOL+1,IROW); A3=A(ICOL+1,IROW-1); A4=A(ICOL,IROW-1)

  !## skip flat areas
  IF(A1.EQ.V.AND.A2.EQ.V.AND.A3.EQ.V.AND.A4.EQ.V)CYCLE
  !## skip nodata areas  
  IF(A1.EQ.XNODATA.AND.A2.EQ.XNODATA.AND.A3.EQ.XNODATA.AND.A4.EQ.XNODATA)CYCLE

  !## midpoints
  X1=XC(ICOL);  X2=XC(ICOL+1);  X3=X2;  X4=X1
  Y1=YC(IROW);  Y2=Y1;  Y3=YC(IROW-1);  Y4=Y3

  !## cellsizes
  DX =DELR(ICOL)/2.0+DELR(ICOL+1)/2.0
  DY =DELC(IROW)/2.0+DELC(IROW-1)/2.0
  
  !## set contour-line
  II=0

  !## equal to current contour interval
  IF(A1.EQ.V)THEN
   II=1; XS(II)=X1; YS(II)=Y1
  ELSE
   !## top
   IF(V.GT.A1.AND.V.LT.A4)THEN
    II=II+1; XS(II)=X1; YS(II)=Y1+(V-A1)/(A4-A1)*DY
   ENDIF 
   !## top-east
   IF(V.GT.A1.AND.V.LT.A3)THEN
    II=II+1; XS(II)=X1+(V-A1)/(A3-A1)*DX; YS(II)=Y1+(V-A1)/(A3-A1)*DY
   ENDIF
   !## bottom-east
   IF(V.GT.A1.AND.V.LT.A2)THEN
    II=II+1; XS(II)=X1+(V-A1)/(A2-A1)*DX; YS(II)=Y2
   ENDIF
  ENDIF
  
  DO I=1,II-1
   CALL IGRJOIN(XS(I),YS(I),XS(I+1),YS(I+1))
  ENDDO
!     CALL IDFPLOT_PUTLABEL(XS(1),YS(1),XS(2),YS(2),V,INILAB,XMIN,YMIN,XMAX,YMAX)
!     II=0

 ENDDO
ENDDO

END SUBROUTINE IDFPLOT_COMPCONTOUR2

!###======================================================================
SUBROUTINE IDFPLOT_COMPCONTOUR(A,NX,NY,XC,YC,DELR,DELC,V,XNODATA)
!###======================================================================
USE MODPLOT, ONLY : MPW
IMPLICIT NONE
...
REAL,INTENT(IN),DIMENSION(NX) :: DELR,XC
REAL,INTENT(IN),DIMENSION(NY) :: DELC,YC
REAL,DIMENSION(2) :: XS,YS
INTEGER :: I,J,II,INILAB
REAL :: A1,A2,A3,A4,X1,X2,X3,X4,Y1,Y2,Y3,Y4,DXY,XMIN,YMIN,XMAX,YMAX

INILAB=0
XMIN=MAX(MPW%XMIN,XC(1));  XMAX=MIN(MPW%XMAX,XC(NX))
YMIN=MAX(MPW%YMIN,YC(NY)); YMAX=MIN(MPW%YMAX,YC(1))
...
DO I=1,NX-1
 DO J=NY,2,-1
  ...
  IF(A1.NE.XNODATA.AND.A2.NE.XNODATA.AND.A3.NE.XNODATA.AND.A4.NE.XNODATA)THEN

   IF(V.EQ.A1)A1=A1+0.0001
   IF(V.EQ.A2)A2=A2+0.0001
   IF(V.EQ.A3)A3=A3+0.0001
   IF(V.EQ.A4)A4=A4+0.0001
   ...
   II=0
!  IF(A1.NE.XNODATA.AND.A2.NE.XNODATA.AND.A3.NE.XNODATA.AND.A4.NE.XNODATA)THEN
   IF((V.GE.A1.AND.V.LE.A2).OR.(V.GE.A2.AND.V.LE.A1))THEN
    DXY=DELR(I)/2+DELR(I+1)/2
    II=II+1
    XS(II)=X1+(V-A1)/(A2-A1)*DXY
    YS(II)=Y1
   ENDIF
   IF((V.GE.A2.AND.V.LE.A3).OR.(V.GE.A3.AND.V.LE.A2))THEN
    DXY=DELC(J)/2+DELC(J-1)/2
    II=II+1
    XS(II)=X2
    YS(II)=Y2+(V-A2)/(A3-A2)*DXY
    IF(II.EQ.2)THEN  
     CALL IGRJOIN(XS(1),YS(1),XS(2),YS(2))
     CALL IDFPLOT_PUTLABEL(XS(1),YS(1),XS(2),YS(2),V,INILAB,XMIN,YMIN,XMAX,YMAX)
     II=0
    ENDIF
   ENDIF
   IF((V.GE.A3.AND.V.LE.A4).OR.(V.GE.A4.AND.V.LE.A3))THEN
    DXY=DELR(I)/2+DELR(I+1)/2
    II=II+1
    XS(II)=X3-(V-A3)/(A4-A3)*DXY
    YS(II)=Y3
    IF(II.EQ.2)THEN  
     CALL IGRJOIN(XS(1),YS(1),XS(2),YS(2))
     CALL IDFPLOT_PUTLABEL(XS(1),YS(1),XS(2),YS(2),V,INILAB,XMIN,YMIN,XMAX,YMAX)
    ...
    ENDIF
   ENDIF
   IF((V.GE.A4.AND.V.LE.A1).OR.(V.GE.A1.AND.V.LE.A4))THEN
    DXY=DELC(J)/2+DELC(J-1)/2
    II=II+1
    XS(II)=X4
    YS(II)=Y4-(V-A4)/(A1-A4)*DXY
    IF(II.EQ.2)THEN  
     CALL IGRJOIN(XS(1),YS(1),XS(2),YS(2))
     CALL IDFPLOT_PUTLABEL(XS(1),YS(1),XS(2),YS(2),V,INILAB,XMIN,YMIN,XMAX,YMAX)
     II=0
...
USE WINTERACTER
USE MODPLOT
USE MOD_PROFILE_UTL, ONLY : PROFILE_AXES,PROFILE_GETFORMAT
IMPLICIT NONE
REAL :: R1,R2,R3
...
!R1=3.5  !% percentage of total border around plot (white)
!R2=3.0  !% part occupied by axes (blue)
!R3=0.5  !% of area to be used for axes (other area occupied by text)
...
DY=DX
!## wit
CALL IGRCOLOURN(WRGB(255,255,255)) !WRGB(123,152,168))
CALL IGRRECTANGLE(MPW%XMIN,MPW%YMIN,MPW%XMIN+DX,MPW%YMAX)
CALL IGRRECTANGLE(MPW%XMIN,MPW%YMIN,MPW%XMAX,MPW%YMIN+DY)
CALL IGRRECTANGLE(MPW%XMIN,MPW%YMAX-DY,MPW%XMAX,MPW%YMAX)
CALL IGRRECTANGLE(MPW%XMAX-DX,MPW%YMIN,MPW%XMAX,MPW%YMAX)
...
!## x-axes
M =N    !## first quess interval
V1=MPW%XMIN+DXX*(R1/100.0)
V2=MPW%XMAX-DXX*(R1/100.0)
CALL PROFILE_AXES(V1,V2,0.0,M)
!## check i
DO WHILE(M.LT.N); M=MAX(1,M)*2; ENDDO
DO WHILE(M.GT.N); M=MIN(N*2,M)/2; ENDDO
VI=(V2-V1)/REAL(M)
...
CALL IGRLINEWIDTH(1)
DO I=1,M*6
 DX=V1+(VI/6.0)*REAL(I-1)
 IF(DX.GT.MPW%XMIN+DXX*(R1/100.0).AND.DX.LT.MPW%XMAX-DXX*(R1/100.0))THEN
  CALL IGRJOIN(DX,Y1-DY/2.0,DX,Y1+DY/2.0)
  CALL IGRJOIN(DX,Y2-DY/2.0,DX,Y2+DY/2.0)
 ENDIF
END DO
CALL IGRLINEWIDTH(2)
DO I=1,M
 DX=V1+VI*REAL(I-1)
 IF(DX.GT.MPW%XMIN+DXX*(R1/100.0).AND.DX.LT.MPW%XMAX-DXX*(R1/100.0))THEN
  CALL IGRJOIN(DX,Y1-DY,DX,Y1+DY)
  CALL IGRJOIN(DX,Y2-DY,DX,Y2+DY)
  FRM=TRIM(PROFILE_GETFORMAT(DX/1000.0))
  J=INDEX(FRM,'.0)',.TRUE.)
  IF(J.EQ.0)THEN
   CALL WGRTEXTREAL(DX,Y1-3*DY,DX/1000.0,FRM)
   CALL WGRTEXTREAL(DX,Y2+3*DY,DX/1000.0,FRM)
  ELSE
   CALL WGRTEXTINTEGER(DX,Y1-3*DY,INT(DX/1000.0))
   CALL WGRTEXTINTEGER(DX,Y2+3*DY,INT(DX/1000.0))
  ENDIF
 ENDIF
END DO
...
M =N    !## first quess interval
V1=MPW%YMIN+DXX*(R1/100.0)
V2=MPW%YMAX-DXX*(R1/100.0)
CALL PROFILE_AXES(V1,V2,0.0,M)
!## check i
DO WHILE(M.LT.N); M=MAX(1,M)*2; ENDDO
DO WHILE(M.GT.N); M=MIN(N*2,M)/2; ENDDO
...
CALL IGRLINEWIDTH(1)
DO I=1,M*6
 DY=V1+(VI/6.0)*REAL(I-1)
 IF(DY.GT.MPW%YMIN+DXX*(R1/100.0).AND.DY.LT.MPW%YMAX-DXX*(R1/100.0))THEN
  CALL IGRJOIN(X1-DX/2.0,DY,X1+DX/2.0,DY)
  CALL IGRJOIN(X2-DX/2.0,DY,X2+DX/2.0,DY)
 ENDIF
END DO
CALL IGRLINEWIDTH(2)
DO I=1,M
 DY=V1+VI*REAL(I-1)
 IF(DY.GT.MPW%YMIN+DXX*(R1/100.0).AND.DY.LT.MPW%YMAX-DXX*(R1/100.0))THEN
  CALL IGRJOIN(X1-DX,DY,X1+DX,DY)
  CALL IGRJOIN(X2-DX,DY,X2+DX,DY)
  FRM=TRIM(PROFILE_GETFORMAT(DY/1000.0))
  J=INDEX(FRM,'.0)',.TRUE.)
  IF(J.EQ.0)THEN
   CALL WGRTEXTREAL(X1-3*DX,DY,DY/1000.0,FRM)
   CALL WGRTEXTREAL(X2+3*DX,DY,DY/1000.0,FRM)
  ELSE
   CALL WGRTEXTINTEGER(X1-3*DX,DY,INT(DY/1000.0))
   CALL WGRTEXTINTEGER(X2+3*DX,DY,INT(DY/1000.0))
  ENDIF
 ENDIF
END DO
...
RAT=(WX2-WX1)/(WY2-WY1)
XOFFSET=((WX2-WX1)/5.0)*2.0

M=2 

X2=(WX2-WX1)/5.0
N = M    !## first quess interval (five)
X1= 0.0

CALL PROFILE_AXES(X1,X2,0.0,N)
!## check i
DO WHILE(N.LT.M); N=MAX(1,N)*2; ENDDO
DO WHILE(N.GT.M); N=MIN(M*2,N)/2; ENDDO
XI=(X2-X1)/REAL(N)

!## fill in entire space
DO 
 IF(XI*REAL(N).GE.(WX2-WX1-XOFFSET))EXIT
 N=N+1 
ENDDO
...
ICLR(1,1)=WRGB(123,152,168) !WRGB(0,0,0)
ICLR(2,1)=WRGB(255,255,255) !WRGB(225,225,225)
ICLR(1,2)=WRGB(255,255,255) !WRGB(123,152,168)
ICLR(2,2)=WRGB(0,102,161)   !WRGB(0,0,0)
CALL IGRFILLPATTERN(SOLID)
...
DO I=1,N+1
 ...
 CALL IGRCOLOURN(WRGB(0,0,0))
 FRM=TRIM(PROFILE_GETFORMAT(XT/1000.0))
 J=INDEX(FRM,'.0)',.TRUE.)
 IF(J.EQ.0)THEN
  CALL IREALTOSTRING(XT/1000.0,LINE,FRM)
  LINE=ADJUSTL(LINE)
  CALL WGRTEXTSTRING(X,WY1+1.75*DY,TRIM(LINE))
 ELSE
  CALL INTEGERTOSTRING(INT(XT/1000.0),LINE,'(I10)')
  LINE=ADJUSTL(LINE)
  CALL WGRTEXTSTRING(X,WY1+1.75*DY,TRIM(LINE))
 ENDIF
END DO
Y =WY1+DY*5.0
CALL WGRTEXTFONT(FFHELVETICA,WIDTH=TWIDTH,HEIGHT=THEIGHT,ISTYLE=0)!FSITALIC)
CALL WGRTEXTSTRING((WX1+WX2)/2.0,Y,'kilometers')
...
!###======================================================================
SUBROUTINE IDFPLOTAREA(MXMPLOT,XMIN,YMIN,XMAX,YMAX,PLOTAREA,I,LPLOT)
!###======================================================================
IMPLICIT NONE
INTEGER,INTENT(IN)          :: MXMPLOT,I
...
INTEGER :: NC1,NC2,NR1,NR2,ANROW,ANCOL,IRAT,IRAT1,IDY,IDX,I,IOS,&!IX1,IY1,IX2,IY2,iu,&
           J,IWID,IHGT,JBITMAP,KBITMAP,ITYPE,IROW,ICOL,ICLR,ITNODATA,ITRANS!, &!,iwindow,iwhite
!           IXPIXL,IYPIXL,IXPIXR,IYPIXR,IWINDOW
...
IF(IP(2).NE.0.OR.IP(3).NE.0.OR.IP(4).NE.0)THEN
 ...
 IF(IP(4).EQ.1)THEN
  ...
  CALL WINDOWSELECT(0); CALL WINDOWOUTSTATUSBAR(4,TRIM(WAITTXT)//'0%')
  CALL IDFPLOT_TEXTING(IDF,XMAX,XMIN,YMAX,YMIN,THICKNESS)
 ENDIF
...
USE WINTERACTER
USE MOD_IDF_PAR, ONLY : IDFOBJ
USE MOD_UTL, ONLY : UTL_SETTEXTSIZE,ITOS
USE MOD_IDF, ONLY : IDFGETLOC
IMPLICIT NONE
INTEGER,INTENT(IN) :: THICKNESS
REAL,INTENT(IN) :: XMAX,XMIN,YMAX,YMIN
TYPE(IDFOBJ) :: IDF
INTEGER :: IROW,ICOL,IDX,IDY,I,J
REAL :: TWIDTH,THEIGHT,X,Y,F
CHARACTER(LEN=15) :: STRING,FSTRING
...
DO IROW=1,IDF%NROW,IDY; DO ICOL=1,IDF%NCOL,IDX
 ...
 I=INT(IDF%X(ICOL,IROW)); F=IDF%X(ICOL,IROW)-I
 STRING=TRIM(ITOS(I))
 IF(F.NE.0.0)THEN
  WRITE(FSTRING,*) F; FSTRING=ADJUSTL(FSTRING)
  IF(F.GT.0.0)STRING=TRIM(STRING)//'.'//FSTRING(3:)
  IF(F.LT.0.0)STRING=TRIM(STRING)//'.'//FSTRING(4:)
 ENDIF
...
INTEGER :: IROW,ICOL,NR1,NR2,NC1,NC2,IDX,IDY,ICLR
REAL :: X,DX,Y,DY,DZ,F,FF,VL,DX1,DX2,DY1,DY2,GAMMA
