...
USE BMPVAR
USE MOD_DBL
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_WSELECTFILE,ITOS,RTOS,UTL_MESSAGEHANDLE,UTL_CREATEDIR,UTL_LOADIMAGE,UTL_DIST,UTL_STOREZOOMEXTENT,UTL_GETHELP,UTL_CAP,UTL_DIALOGSHOW
...
CONTAINS
 ...
 IDOWN=0; CALL UTL_DIALOGSHOW(-1,-1,0,2)
 CALL WINDOWSELECT(0)
...
!       5543948.000000000000000
 ...
 INTEGER :: NROW,NCOL,IU,IOS,I,IBMP,ITR,ICT
 ...
 CALL WINDOWSELECT(0); IF(WMENUGETSTATE(ID_TOPOTRANSPARANCY0,2).EQ.0)CALL WBITMAPPLOTMODE(MODEAND)

 !## get level of bitmap transparancy
 ITR=MPW%ITRBITMAP; MPW%ITRBITMAP=0
 ...
 ICT=MPW%ICTONE; MPW%ICTONE=0
 IF(WMENUGETSTATE(ID_COLOURTONE,2).EQ.1)THEN
  IF(WMENUGETSTATE(ID_GREYTONES1,2).EQ.1)MPW%ICTONE=1
  IF(WMENUGETSTATE(ID_GREYTONES2,2).EQ.1)MPW%ICTONE=2
  IF(WMENUGETSTATE(ID_GREYTONES3,2).EQ.1)MPW%ICTONE=3
 ENDIF
 ...
 DO
  ...
  ELSE
   ...
   !## see whether something changed - in that case read image
   IF(ITR.NE.MPW%ITRBITMAP.OR.ICT.NE.MPW%ICTONE)THEN
    IHANDLE=BMP(IBMP)%IBITMAP; IF(IHANDLE.NE.0)CALL WBITMAPDESTROY(IHANDLE)
    BMP(IBMP)%IBITMAP=0
   ENDIF
   ...
   IF(IHANDLE.EQ.0)THEN
    CALL TOPO1LOADBMP(IHANDLE,BMP(IBMP)%BMPFNAME,BMP(IBMP)%NCOL,BMP(IBMP)%NROW)
    LREAD=.TRUE.
   ENDIF
   IF(IHANDLE.EQ.0)BMP(IBMP)%IACT=0
   BMP(IBMP)%IBITMAP=IHANDLE
  ...
  IF(XMIN.LT.MPW%XMAX.AND.XMAX.GE.MPW%XMIN.AND.YMIN.LT.MPW%YMAX.AND.YMAX.GE.MPW%YMIN)THEN
   ...
   IF(IHANDLE.NE.0)THEN
    ...
    CALL WBITMAPSTRETCHMODE(STRETCHHALFTONE)
    !## modify colour if read again
    IF(LREAD)CALL TOPO1BMPTONE(IHANDLE,F(MPW%ITRBITMAP),MPW%ICTONE)
!    CALL WBITMAPPUT(IHANDLE,1,0,IX1,IY1,IX2,IY2)
    CALL WBITMAPPUT(IHANDLE,1,1,IX1,IY1,IX2,IY2)
   ENDIF
   IF(.NOT.LBMP.AND.IHANDLE.NE.0)CALL WBITMAPDESTROY(IHANDLE)
 ...
 !###======================================================================
 SUBROUTINE TOPO1BMPTONE(IHANDLE,F,ITONE)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: IHANDLE,ITONE
 ...
 DO I=1,SIZE(IBMPDATA)
  ...
  SELECT CASE (ITONE)
   !## lichtness
   CASE(1); ICLR=(MAX(IR,IG,IB)+MIN(IR,IG,IB))/2
   !## averaging
   CASE(2); ICLR=(IR+IG+IB)/3
   !## luminosity
   CASE(3); ICLR=0.21*IR+0.72*IG+0.07*IB
  END SELECT
  IF(ITONE.NE.0)THEN; IR=ICLR; IG=ICLR; IB=ICLR; ENDIF
