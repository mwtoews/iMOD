...
USE RESOURCE
USE BMPVAR
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_WSELECTFILE,ITOS,RTOS,UTL_MESSAGEHANDLE,UTL_CREATEDIR,UTL_LOADIMAGE,UTL_DIST
...
CONTAINS
 ...
 DO
  ...
  SELECT CASE (ITYPE)
   ...
   CASE (MOUSEMOVE)
    IF(IDOWN.EQ.0)THEN
     CALL TOPO1_BITMAP_MOUSEMOVE(MESSAGE%GX,MESSAGE%GY,CRDITYPE)
    ELSE
     CALL TOPO1_BACKGROUND_BITMAP_ADJUST(LMBXPIX,LMBYPIX,MESSAGE%GX,MESSAGE%GY,CRDITYPE) 
     CALL IDFPLOTFAST(0)
    ENDIF
   CASE (FIELDCHANGED)
    !## moved from
 ...
 !###======================================================================
 IMPLICIT NONE
 REAL,INTENT(INOUT) :: LMBXPIX,LMBYPIX
 REAL,INTENT(IN) :: IPX,IPY
...
! CALL IGRUNITSFROMPIXELS(BMP(IBMP)%IX1,BMP(IBMP)%IY1,BMP(IBMP)%XMIN,BMP(IBMP)%YMIN,IORIGIN=1)
! CALL IGRUNITSFROMPIXELS(BMP(IBMP)%IX2,BMP(IBMP)%IY2,BMP(IBMP)%XMAX,BMP(IBMP)%YMAX,IORIGIN=1)
