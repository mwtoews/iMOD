...
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_WSELECTFILE,ITOS,RTOS,UTL_MESSAGEHANDLE,UTL_CREATEDIR
...
! BMP%IACT      =0
 ...
 IW=MIN(WINFOBITMAP(MPW%IBITMAP,BITMAPWIDTH) ,NCOL)
 IH=MIN(WINFOBITMAP(MPW%IBITMAP,BITMAPHEIGHT),NROW)
 ...
 IF(I.EQ.0)THEN
  ...
  CALL WDIALOGFIELDSTATE(IDF_BUTTON5,2)
 ELSE
  ...
  DO I=1,NBMP
   IF(BMP(I)%IACT.EQ.1)THEN
    ...
    CALL WDIALOGPUTREAL(IDF_REAL1,BMP(I)%XMIN/1000.0)
    CALL WDIALOGPUTREAL(IDF_REAL2,BMP(I)%YMIN/1000.0)
    CALL WDIALOGPUTREAL(IDF_REAL4,BMP(I)%XMAX/1000.0)
    CALL WDIALOGPUTREAL(IDF_REAL3,BMP(I)%YMAX/1000.0)
    CALL WDIALOGPUTREAL(IDF_REAL5,BMP(I)%DX/1000.0)
    CALL WDIALOGPUTREAL(IDF_REAL6,BMP(I)%DY/1000.0)
    CALL WDIALOGPUTINTEGER(IDF_INTEGER3,I)
    EXIT
   ENDIF
  ENDDO
 ENDIF
 ...
 CALL WDIALOGSELECT(ID_DADDTOPO)
 CALL WDIALOGGETINTEGER(IDF_INTEGER3,I)
 IF(I.EQ.0)RETURN
 CALL WDIALOGGETREAL(IDF_REAL1,BMP(I)%XMIN)
 CALL WDIALOGGETREAL(IDF_REAL2,BMP(I)%YMIN)
 CALL WDIALOGGETREAL(IDF_REAL4,BMP(I)%XMAX)

 IF(BMP(I)%XMIN.GT.BMP(I)%XMAX)THEN
  X1=MIN(BMP(I)%XMIN,BMP(I)%XMAX)
  X2=MAX(BMP(I)%XMIN,BMP(I)%XMAX)
  BMP(I)%XMIN=X1
  BMP(I)%XMAX=X2
  CALL WDIALOGPUTREAL(IDF_REAL3,BMP(I)%XMIN)
  CALL WDIALOGPUTREAL(IDF_REAL5,BMP(I)%XMAX)
 ENDIF

 BMP(I)%XMIN=BMP(I)%XMIN*1000.0
 BMP(I)%XMAX=BMP(I)%XMAX*1000.0
 BMP(I)%YMIN=BMP(I)%YMIN*1000.0
 BMP(I)%DX =(BMP(I)%XMAX-BMP(I)%XMIN)/BMP(I)%NCOL
 BMP(I)%DY =(BMP(I)%YMAX-BMP(I)%YMIN)/BMP(I)%NROW
 ...
 CALL TOPO1UPDATEMANAGER() !UPDATETOPO()
