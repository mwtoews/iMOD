...
CONTAINS
 ...
 !###======================================================================
 IMPLICIT NONE
 INTEGER :: N,IPLOT,I,J,K,NPER,NF,NP,NS,I1,I2,ISEC,IEXT
 LOGICAL :: L1,L2
 ...
 IF(IOSDIREXISTS(TRIM(PREFVAL(1))//'\MOVIES\'//TRIM(DIR)))THEN
  CALL WMESSAGEBOX(YESNO,QUESTIONICON,COMMONNO,'Output folder ['//TRIM(DIR)//'] allready exist, do you want to overwrite the entire content ?','Question')
  IF(WINFODIALOG(4).NE.1)THEN; DEALLOCATE(NFILES); RETURN; ENDIF
  IF(.NOT.UTL_DEL1TREE(TRIM(PREFVAL(1))//'\MOVIES\'//TRIM(DIR)))THEN; DEALLOCATE(NFILES); RETURN; ENDIF
 ENDIF
 ...
 DO K=1,NPER
  ...
  DO I=1,NF
   DO  
    ...
    IF(FDATES(IFILES(I),I).LE.LDATES(K))EXIT
  ...
  DO I=1,NF
   ...
   CALL IDFINIT(PLOTNAMES(I),LEGNAME=TRIM(PREFVAL(1))//'\tmp\movie_legend_file'//TRIM(ITOS(N))//'.leg', & 
        ISTYLE=IDFSTYLE(I),LPLOT=L1,LDEACTIVATE=L2) !,BGTEXT=BGTEXT)

  ENDDO

  !## save bitmaps
  WRITE(BMPOUTNAME(K),'(A,I3.3,A)') TRIM(PREFVAL(1))//'\MOVIES\'//TRIM(DIR)//'\IMAGE',K,'.'//TRIM(EXT(IEXT)) !BMP'
  CALL WDIALOGPUTSTRING(IDF_LABEL3,'Saving: '//TRIM(BMPOUTNAME(K)))
...
! CALL MOVIE_CREATE_MPEG(BMPOUTNAME,DIR,ISEC)
 ...
 CALL WMESSAGEBOX(YESNO,INFORMATIONICON,COMMONOK,'Succesfully saved '//TRIM(ITOS(SIZE(NFILES)))//' images.'//CHAR(13)// &
