...
CONTAINS
 ...
 IF(ICPOINTERS.EQ.1)THEN
  ...
  IF(TRIM(FDISTANCES).EQ.'')THEN
   ...
   IF(IOS.NE.0)THEN; WRITE(*,'(A)') 'Cannot open '//TRIM(OUTPUTFOLDER)//'\distances.txt'; RETURN; ENDIF
   ...
   IF(NCLIP.EQ.1)THEN
    ...
    IF(.NOT.IDFREADSCALE(CLIPIDF%FNAME,CLIPIDF,10,0,0.0D0,0))RETURN
   ENDIF
   
   WRITE(*,'(/A)') 'Reading data from:'
   WRITE(*,'(A)') '[TOPSYSTEM] - '//TRIM(TOPSYSTEM)
   IF(.NOT.IDFREADSCALE(TOPSYSTEM,MDLIDF(4),10,0,0.0D0,0))RETURN
   WRITE(*,'(A)') '[BOTSYSTEM] - '//TRIM(BOTSYSTEM)
   IF(.NOT.IDFREADSCALE(BOTSYSTEM,MDLIDF(5),10,0,0.0D0,0))RETURN
 ...
 ELSE
  DO I=1,NFORM
   ...
   IF(ZIDF(I)%DMAX.LT.ZIDF(I)%DMIN)ZIDF(I)%DMAX=0.0D0
 ...
 WRITE(*,'(/A)') 'Reading data from:'
 WRITE(*,'(A)') '[TOPSYSTEM] - '//TRIM(TOPSYSTEM)
 IF(.NOT.IDFREADSCALE(TOPSYSTEM,MDLTOP(1),10,0,0.0D0,0))RETURN
 WRITE(*,'(A)') '[BOTSYSTEM] - '//TRIM(BOTSYSTEM)
 IF(.NOT.IDFREADSCALE(BOTSYSTEM,MDLBOT(NLAY),10,0,0.0D0,0))RETURN
 ...
 !## check consistency (thickness<0.0D0), create base model
 DO IROW=1,MDLTOP(1)%NROW; DO ICOL=1,MDLTOP(1)%NCOL
  T=MDLTOP(1)%X(ICOL,IROW); B=MDLBOT(NLAY)%X(ICOL,IROW); D=0.0D0
 ...
 DO IIDF=1,NFORM
  ...
  IF(.NOT.IDFREADSCALE(ZIDF(IIDF)%FNAME,ZIDF(IIDF),7,0,0.0D0,0))RETURN
  
  WRITE(*,'(2(I3.3,A))') IIDF,'-',NFORM,' Reading Top Files for '//TRIM(TOPIDF(IIDF)%FNAME)//' ...'
  CALL IDFDEALLOCATEX(TOPIDF(IIDF)); CALL IDFCOPY(MDLIDF(1),TOPIDF(IIDF))
  IF(.NOT.IDFREADSCALE(TOPIDF(IIDF)%FNAME,TOPIDF(IIDF),10,0,0.0D0,0))RETURN

  WRITE(*,'(2(I3.3,A))') IIDF,'-',NFORM,' Reading Bot Files for '//TRIM(BOTIDF(IIDF)%FNAME)//' ...'
  CALL IDFDEALLOCATEX(BOTIDF(IIDF)); CALL IDFCOPY(MDLIDF(1),BOTIDF(IIDF))
  IF(.NOT.IDFREADSCALE(BOTIDF(IIDF)%FNAME,BOTIDF(IIDF),10,0,0.0D0,0))RETURN
...
!     LEX=.TRUE.
    ...
    IF(ILAY.GE.1.AND.ILAY.LT.NLAY)THEN
     ...
     T=TOPIDF(IIDF)%X(ICOL,IROW); B=BOTIDF(IIDF)%X(ICOL,IROW)
     !## found thickness
     IF(T-B.GT.0.0D0)THEN
!     IF(T.NE.TOPIDF(IIDF)%NODATA.AND.B.NE.BOTIDF(IIDF)%NODATA)THEN
  ...
  CALL IDFDEALLOCATEX(ZIDF(IIDF))
  CALL IDFDEALLOCATEX(TOPIDF(IIDF))
  CALL IDFDEALLOCATEX(BOTIDF(IIDF))
 ...
 DO I=1,NLAY; MDLTOP(I)%X=0.0D0; ENDDO !MDLTOP(I)%NODATA; ENDDO
 DO IIDF=1,NFORM
  ...
  DO IROW=1,MDLTOP(1)%NROW; DO ICOL=1,MDLTOP(1)%NCOL
   ...
   !## assign not assignable distinctive layer in modellayer 1
   IF(ILAY.EQ.-1)ILAY=1
   IF(ILAY.GE.1.AND.ILAY.LE.NLAY)MDLTOP(ILAY)%X(ICOL,IROW)=MDLTOP(ILAY)%X(ICOL,IROW)+1.0D0 !DBLE(IIDF)
  ENDDO; ENDDO
...
!  IF(SMPLX(I)%IAREA.LE.0)CYCLE
 ...
 !## check xvar() 
 DO I=1,NVAR; IF(SUM(ABS(A(:,I))).EQ.0.0D0)XVAR(I)=-1.0D0; ENDDO
 ...
 IF(IOS.NE.0)THEN; WRITE(*,'(A)') 'Cannot open '//TRIM(FDISTANCES); RETURN; ENDIF
 READ(IU,*) NFORM; ALLOCATE(ZIDF(NFORM),TOPIDF(NFORM),BOTIDF(NFORM),ZINFO(NFORM))
 DO I=1,NFORM
...
!   IF(SMPLX(I)%PERC(1).EQ.0.0D0)SMPLX(I)%IAREA=0
...
!   IF(SMPLX(J)%IAREA.LE.SMPLX(I)%IAREA)SMPLX(J)%IAREA=0
 ...
 NVAR=0; DO I=1,NFORM; NVAR=NVAR+ZINFO(I)%NZ; ENDDO
...
!  IF(IEXPZONE.GT.0)CALL CUS_EXTENT_TOPBOT(I)
  ...
  !## initialize arrays
  DZTOP=10.0D10; DZBOT=10.0D10; IZTOP=0; IZBOT=0; ILTOP=0; ILBOT=0; ILTB=INT(0,1); IZTB=INT(0,1)
...
!   IF(IEXPZONE.GT.0)CALL CUS_EXTENT_TOPBOT(J)
   ...
   DO IROW=1,MDLIDF(1)%NROW; DO ICOL=1,MDLIDF(1)%NCOL
    ...
    !## skip nodata zone
    Z=INT(ZIDF(I)%X(ICOL,IROW)); IF(Z.LE.0)CYCLE 
    ...
    IF(T-B.GT.0.0D0)THEN
     ...
     IF(TT-BB.GT.0.0D0)THEN
      ...
      IF(BB.GE.T)THEN     !## j'th file above (positive)
       ...
       IF(ZZ.LT.0.AND.D.NE.0.0D0)D=10.E10
...
!   DO IROW=1,MDLIDF(1)%NROW; DO ICOL=1,MDLIDF(1)%NCOL
!    J=ILTOP(ICOL,IROW); K=IZTOP(ICOL,IROW)
!    IF(J.GT.0.AND.K.LT.0)THEN; DZTOP(ICOL,IROW)=0.0D0; ENDIF
!    J=ILBOT(ICOL,IROW); K=IZBOT(ICOL,IROW)
!    IF(J.GT.0.AND.K.LT.0)THEN; DZTOP(ICOL,IROW)=0.0D0; ENDIF
!   ENDDO; ENDDO
...
!  IF(.NOT.IDFWRITE(  TOPIDF(I),'d:\iMOD-TEST\IMODBATCH_CUS\tmp\izbot_l'//TRIM(ITOS(i))//'.IDF',1))STOP
   ...
   DO Z=1,ZINFO(I)%NZ
    ...
    KK=1; DO K=2,DZ(Z)%NZ
     IF(DZ(Z)%IZ(K).NE.DZ(Z)%IZ(K-1))THEN
      CALL UTL_GETMED(DZ(Z)%D(KK:),K-KK,-999.99D0,PERC,NPERC,MX,XMED)
      KK=K
 ...
 THICKNESS=0.0D0 !## trace thicknesses greater than zero

 WRITE(*,'(A)') 'Writing thickness ...'
 IF(.NOT.IDFWRITE(MDLIDF(3),MDLIDF(3)%FNAME,1))STOP 'ERROR WRITING THICKNESSES'

 WRITE(*,'(A,F10.2)') 'Tracing thickness > 0.0D0, max. vertical stepsize =',CRIT_THICKNESS
 ...
 IF(.NOT.IDFREADSCALE(TOPIDF(IIDF)%FNAME,MDLIDF(1),10,0,0.0D0,0))RETURN
 WRITE(*,'(A)') '[BOT] - '//TRIM(BOTIDF(IIDF)%FNAME)
 !## no interpolation, take data as it is
 IF(.NOT.IDFREADSCALE(BOTIDF(IIDF)%FNAME,MDLIDF(2),10,0,0.0D0,0))RETURN
 
 !## check consistency (thickness<0.0D0)
 NCOR=0
 DO IROW=1,MDLIDF(1)%NROW; DO ICOL=1,MDLIDF(1)%NCOL  
  T=MDLIDF(1)%X(ICOL,IROW); B=MDLIDF(2)%X(ICOL,IROW) 
  IF(T.NE.MDLIDF(1)%NODATA.AND.B.NE.MDLIDF(2)%NODATA)THEN
   !## clip for top- and bottom-system
   MDLIDF(1)%X(ICOL,IROW)=MIN(MDLIDF(1)%X(ICOL,IROW),MDLIDF(4)%X(ICOL,IROW))
   MDLIDF(2)%X(ICOL,IROW)=MAX(MDLIDF(2)%X(ICOL,IROW),MDLIDF(5)%X(ICOL,IROW))
   IF(T-MIN_THICKNESS.LE.B)THEN
  ...
  IF(NCLIP.EQ.1)THEN
   !## do not clip whenever iclip values is zero
   IF(ICLIP(IIDF,1).EQ.0.AND.ICLIP(IIDF,2).EQ.0)CYCLE
   !## check clip
 ...
 IF(.NOT.IDFALLOCATEX(MDLIDF(3)))STOP 'Cannot allocate mdlfidf(3)'
 
 !## compute thickness, reset mdlidf(2) and use mdlidf(1)=top
 DO IROW=1,MDLIDF(1)%NROW; DO ICOL=1,MDLIDF(1)%NCOL
  T=MDLIDF(1)%X(ICOL,IROW); B=MDLIDF(2)%X(ICOL,IROW); D=0.0D0
  IF(T.NE.MDLIDF(1)%NODATA.AND.B.NE.MDLIDF(2)%NODATA) D=MAX(0.0D0,T-B)
  MDLIDF(3)%X(ICOL,IROW)=D
  IF(D.EQ.0.0D0)MDLIDF(1)%X(ICOL,IROW)=MDLIDF(1)%NODATA
 ENDDO; ENDDO
 MDLIDF(3)%NODATA=0.0D0; MDLIDF(2)%X=MDLIDF(2)%NODATA
 ...
 ENDIF
 DO I=2,5; CALL IDFCOPY(MDLIDF(1),MDLIDF(I)); ENDDO
