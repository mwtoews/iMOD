...
!              00/ !## 24 qmodf
 ...
 CALL WDIALOGSELECT(ID_DWBAL_ANALYSE_TAB4)
 GTITLE='Balance $'; IF(IBATCH.EQ.0)CALL WDIALOGGETSTRING(IDF_STRING2,GTITLE)
 ...
 DO I=1,MGROUP
  GRAPHDIM(I)%IFIXX=1 !GRAPHDIM(I)%IFIXX=0; 
  GRAPHDIM(I)%IFIXY=0; GRAPHDIM(I)%LDATE=.FALSE.
  ALLOCATE(GRAPHDIM(I)%XTXT(NXG),GRAPHDIM(I)%XPOS(NXG)); GRAPHDIM(I)%XTXT=''; GRAPHDIM(I)%XPOS=0.0D0 !; IF(ITTYPE.EQ.1)GRAPHDIM(I)%IFIXX=1
  GRAPHDIM(I)%IGROUP=1
!  IF(ITTYPE.EQ.0)THEN; SELECT CASE (IAG); CASE(8); GRAPHDIM(I)%LDATE=.TRUE.; END SELECT; ENDIF
 ENDDO
...
!  IF(ZSUM.EQ.1)IZ=1
 ...
 IY=IY1; DO I=1,NY
  DT(I)=365.0D0; IF(WDATELEAPYEAR(IY))DT(I)=DT(I)+1.0D0
...
!!     Q(12,1)=QEVT_IN   Q(12,1)=QEVT_OUT    Q(24,1)=QMODF_IN  Q(24,1)=QMODF_OUT   
 ...
 IF(LEN_TRIM(FNAME).GT.0)THEN
  IF(WBAL_ANALYSE_READCSV(FNAME))THEN
   CALL WBAL_ANALYSE_TAB3_FIELD()
   CALL WDIALOGSELECT(ID_DWBAL_ANALYSE)
   IF(IBATCH.EQ.0)CALL UTL_DIALOGSHOW(-1,-1,0,2)
  ELSE
   CALL WDIALOGUNLOAD()
  ENDIF
 ELSE
