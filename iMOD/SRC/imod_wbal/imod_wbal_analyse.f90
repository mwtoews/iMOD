...
! IF(ALLOCATED(GRAPH))CALL GRAPH_DEALLOCATE()
 ...
 INTEGER :: I,J,K,IL,IZ,ID,IG,IQIN,IQOU,JQIN,JQOU,IQ,IIQ,JQ,IOS,IDATE,ITTYPE,IHIT, &
     IAG,NXG,IY1,IM1,ID1,IY2,IM2,ID2,IPOS,SDATE,IB,LSUM,ZSUM,N,INET,IUNIT
 REAL,POINTER,DIMENSION(:) :: DT
 CHARACTER(LEN=16),POINTER,DIMENSION(:) :: XTXT=>NULL()
 CHARACTER(LEN=16) :: TXTL,TXTZ
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IDATES,SQ,CLILAY,CLIZONE
 REAL :: QIN,QOU
 ...
 !## sum layers
 CALL WDIALOGGETRADIOBUTTON(IDF_RADIO3,LSUM)
 !## get selected zones
 CALL WDIALOGGETMENU(IDF_MENU2,LIZONE)
 !## sum zones
 CALL WDIALOGGETRADIOBUTTON(IDF_RADIO1,ZSUM)
...
!  IF(LIZONE(J).EQ.0)CYCLE
  ...
  GRAPHNAMES(K)=TRIM(TXTL)//'; '//TRIM(TXTZ) !'Layer '//TRIM(CILAY(I))//'; Zone Number '//TRIM(CIZONE(J))
 ENDDO; ENDDO
...
! DO I=1,NBUDGET; SQ(I)=I; ENDDO
 ...
 JQ=0; DO IIQ=1,NBUDGET
  ...
  IQ=SQ(IIQ); IF(BUDGET(IQ)%IACT.EQ.0)CYCLE
...
!   GRAPH(JQOU,IG)%RX(IPOS)=REAL(IPOS)
 ...
 DEALLOCATE(CMDATE,CMLAY,CMZONE,IDATES,SQ,CLILAY,CLIZONE)
