...
!     CALL WBAL_ANALYSE_MAIN_INI(MESSAGE%VALUE1)
    ...
    !## open csv
    CASE (ID_OPEN2)
     IF(.NOT.WBAL_ANALYSE_READCSV('g:\IMOD-MODELS\IBRAHYM_V2.0\DBASE_DLD\IMOD_USER\MODELS\IBRAHYM_V2.0.5_23\test2.csv'))CALL WBAL_ANALYSE_DEALLOCATE()
...
! IF(ALLOCATED(GRAPH))CALL GRAPH_DEALLOCATE()
 ...
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IDATES
 REAL :: QIN,QOU
 ...
 IF(ITTYPE.EQ.0)THEN
  ...
  SELECT CASE (IAG)
   ...
   !## year
   CASE (2); NXG=WBAL_ANAYSE_PREPARE_GETYEAR(IY1,IY2,SDATE,DT,XTXT)
   !## months
 ...
 CALL WBAL_ANALYSE_PREPARERDATES(SDATE,DT,XTXT,IDATES,IAG)
...
!    IF(IAG.EQ.7)GRAPHDIM%XTXT(ID)=GWBAL(1)%CDATE(I)
 ...
 DEALLOCATE(CMDATE,CMLAY,CMZONE,IDATES)
 ...
 !###======================================================================
 SUBROUTINE WBAL_ANALYSE_PREPARERDATES(SDATE,DT,XTXT,IDATES,IAG)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: SDATE,IAG
 ...
 ND=0; DO I=1,SIZE(CMDATE)
  ND=ND+1
  READ(CMDATE(I),*)   IDATE
  READ(CMDATE(I+1),*) JDATE
  DT(1)=(UTL_IDATETOJDATE(JDATE)-UTL_IDATETOJDATE(IDATE))+1.0
 ENDDO
 ...
 ALLOCATE(DT(1),XTXT(1)); DT=0.0; XTXT=''
 ...
 !###======================================================================
 INTEGER FUNCTION WBAL_ANAYSE_PREPARE_GETYEAR(IY1,IY2,SDATE,DT,XTXT)
 !###======================================================================
 IMPLICIT NONE
