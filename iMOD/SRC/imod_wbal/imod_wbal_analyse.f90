...
CONTAINS
 ...
 WRITE(IU,'(99(I1  ,A1))') 'BDGIACT=',(BUDGET(I)%IACT,','  ,I=1,SIZE(BUDGET))
 WRITE(IU,'(99(I2.2,A1))') 'BDGIGRP=',(BUDGET(I)%IGROUP,',',I=1,SIZE(BUDGET))
 WRITE(IU,'(99(I6.6,A1))') 'BDGICLR=',(BUDGET(I)%ICLR  ,',',I=1,SIZE(BUDGET))
 ...
 WRITE(IU,'(A)') 'LAYERS=',LINE(I:LEN_TRIM(LINE)-1)
 LINE=''; DO I=1,NLAY; IF(LILAY(I).EQ.1)LINE=TRIM(LINE)//','; ENDDO
 WRITE(IU,'(A)') 'ZONES=',LINE(I:LEN_TRIM(LINE)-1)
 LINE=''; DO I=1,NDATE; IF(LIDATE(I).EQ.1)LINE=TRIM(LINE)//','; ENDDO
 WRITE(IU,'(A)') 'DATES=',LINE(I:LEN_TRIM(LINE)-1)
...
!     IF(.NOT.WBAL_ANALYSE_READCSV(''))CALL WBAL_ANALYSE_DEALLOCATE()
 ...
 SELECT CASE (ITYPE)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_PLUS1,ID_MIN1)
     CALL WDIALOGGETSTRING(IDF_STRING1,SEARCH)
     CALL WDIALOGGETMENU(IDF_MENU2,LILAY)
     IADD=1; IF(MESSAGE%VALUE1.EQ.ID_MIN1)IADD=-1; CALL WBAL_FINDIT(CILAY,LILAY,NLAY,SEARCH,IADD)
     CALL WDIALOGPUTOPTION(IDF_MENU2,LILAY)
    !## zone wildcard selection
    CASE (ID_PLUS2,ID_MIN2)
     CALL WDIALOGGETSTRING(IDF_STRING2,SEARCH)
     CALL WDIALOGGETMENU(IDF_MENU1,LIZONE)
     IADD=1; IF(MESSAGE%VALUE1.EQ.ID_MIN2)IADD=-1; CALL WBAL_FINDIT(CIZONE,LIZONE,NZONE,SEARCH,IADD)
     CALL WDIALOGPUTOPTION(IDF_MENU1,LIZONE)
   END SELECT
 ...
 DO I=1,NLIST
  IF(CHF_LK(SEARCH,LEN(SEARCH),LIST(I),LEN(LIST(I))))THEN
   !## remove from list
