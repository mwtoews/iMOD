  ...
  CASE (FIELDCHANGED)
   ...
   SELECT CASE (MESSAGE%VALUE2)
    CASE (IDF_RADIO1,IDF_RADIO2,IDF_RADIO3,IDF_RADIO4,IDF_RADIO5)
     ...
     SELECT CASE (I)
      !## timeseries
      CASE (1); J=1; K=1; L=0; TEXT='Generate Graphics'
      !## graphics
      CASE (2); J=1; K=1; L=1; TEXT='Generate Graphics'
      !## table
      CASE (3); J=1; K=0; L=0; TEXT='Generate Graphics'
      !## export csv
      CASE (4); J=0; K=1; L=0; TEXT='Export CSV-File'
      !## idf
      CASE (5); J=0; K=1; L=0; TEXT='Export IDF-File'
     END SELECT
     ...
     CALL WDIALOGPUTSTRING(ID_GRAPHICS,TRIM(TEXT))
   END SELECT
 END SELECT
...
! IF(ALLOCATED(GRAPH))CALL GRAPH_DEALLOCATE()
 ...
 JQ=0; DO IIQ=1,SIZE(OSQ) 
  ...
  IQ=SQ(OSQ(IIQ)) !; IF(BUDGET(IQ)%IACT.EQ.0)CYCLE
 ...
 !## set row labels
 DO I=1,SIZE(GRAPHDIM%XTXT) !MDATE
...
!   ENDIF
 ...
 !## zone numbers
 ALLOCATE(IPLG(NZONE)); IPLG=0 !SIZE(CIZONE))); IPLG=0
 DO I=1,NZONE !SIZE(CIZONE)
!  READ(CMZONE(I),*,IOSTAT=IOS) IPLG(I)
