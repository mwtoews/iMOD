 ...
 SELECT CASE (ITYPE)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    !## open csv
    CASE (ID_OPEN2)
!     IF(.NOT.WBAL_ANALYSE_READCSV('d:\iMOD-TEST\IMODBATCH_WBALANCE\MODELTEST\wbal.csv'))CALL WBAL_ANALYSE_DEALLOCATE()    
!     IF(.NOT.WBAL_ANALYSE_READCSV('d:\iMOD-TEST\IMODBATCH_WBALANCE\deltares1994.csv'))CALL WBAL_ANALYSE_DEALLOCATE()
 ...
 SELECT CASE (IOPT)
  ...
  CASE (2)
   ...
   IF(ID.EQ.ID_PREVIEW)THEN
    IF(N.GT.25)THEN
     CALL WMESSAGEBOX(YESNOCANCEL,QUESTIONICON,COMMONNO,'Are you sure to plot '//TRIM(ITOS(N))//' images ?','Question')
     IF(WINFODIALOG(4).NE.1)RETURN
    ENDIF
   ENDIF
   CALL WDIALOGSELECT(ID_DWBAL_ANALYSE_TAB4)
