...
!     IF(.NOT.WBAL_ANALYSE_READCSV(''))CALL WBAL_ANALYSE_DEALLOCATE()
 ...
 INTEGER,INTENT(IN) :: ITYPE
 TYPE(WIN_MESSAGE),INTENT(IN) :: MESSAGE
 INTEGER :: IROW,ICOL,IRGB,I,J,IOS,IDATE,IADD
 ...
 SELECT CASE (IOPT)
  ...
  CASE (2)
   ...
   IF(ID.EQ.ID_PREVIEW.AND.N.GT.25)THEN
    CALL WMESSAGEBOX(OKONLY,QUESTIONICON,COMMONNO,'Are you sure to plot '//TRIM(ITOS(N))//' images ?','Question')
...
! ENDDO
 ...
 DO IT=1,SIZE(GRAPH(1,1)%RY)
  ...
  DO IB=1,MBUDGET,2
   ...
   J=WBAL_ANALYSE_GETQCATEGORY(GRAPH(I1,IG)%CTYPE) !BUDGET(I)%FLUXTERM)
