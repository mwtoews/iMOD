...
!              00/ !## 24 qmodf
 ...
 SELECT CASE (IOPT)
  ...
  CASE (1)
   ...
   GRAPHDIM(1)%IFIXX=0; GRAPHDIM(1)%IFIXY=0; GRAPHDIM(1)%XTITLE='Time'
   IF(IUNIT.EQ.1)THEN
    GRAPHDIM(1)%YTITLE='Volumes (m3/d)'; CALL GRAPH_PLOT(LEXPORT=LEXPORT,DIR=DIR)
   ELSEIF(IUNIT.EQ.2)THEN
    GRAPHDIM(1)%YTITLE='Quantity (mm/d)'; CALL GRAPH_PLOT(LEXPORT=LEXPORT,DIR=DIR)
   ENDIF
  !## graph
  CASE (2)
 ...
 DO I=1,MGROUP
  ...
  ALLOCATE(GRAPHDIM(I)%XTXT(NXG),GRAPHDIM(I)%XPOS(NXG)); GRAPHDIM(I)%XTXT=''; GRAPHDIM(I)%XPOS=0.0D0; GRAPHDIM(I)%IFIXX=1
 ENDDO
...
!  IF(ZSUM.EQ.1)IZ=1
 ...
 IF(ITTYPE.EQ.0)THEN
  ...
  DO I=1,SIZE(DT)
   ...
   GRAPHDIM(1)%XTXT(I)=XTXT(I)
   GRAPHDIM(1)%XPOS(I)=REAL(I) 
  ENDDO
 ELSE
  DO I=1,SIZE(DT)
   GRAPHDIM(1)%XTXT(I)=XTXT(I)
