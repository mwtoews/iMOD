...
USE IMODVAR, ONLY : DP_KIND,SP_KIND,IDIAGERROR,TP,ICPL
USE MOD_IDF, ONLY : IDFALLOCATEX,IDFALLOCATESXY,IDFNULLIFY,IDFDEALLOCATEX,IDFFILLSXSY,IDFREADFREE,IDFWRITE,IDFCOPY
USE MOD_WBAL_PAR
USE MOD_WBAL_GRAPHICS, ONLY : DRAWBAL
USE MOD_WBAL_UTL, ONLY : WBAL_ANALYSE_SAVECONFIG,WBAL_ANALYSE_READCONFIG
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_CAP,UTL_IDATETOJDATE,UTL_JDATETOIDATE,ITOS,IDATETOGDATE,UTL_GDATE,UTL_GETUNIQUE_CHAR,UTL_EQUALNAMES, &
            UTL_WSELECTFILE,RTOS,UTL_DEBUGLEVEL,UTL_GETUNIQUE_INT,UTL_MESSAGEHANDLE,UTL_READINITFILE,UTL_SUBST,UTL_COUNT_COLUMNS
USE MOD_OSD, ONLY : OSD_OPEN
USE MOD_GRAPH, ONLY : GRAPH,GRAPH_DEALLOCATE,GRAPH_ALLOCATE,GRAPH_MAIN,GRAPH_INIT,GRAPHDIM
USE MOD_MAIN_UTL
...
CONTAINS
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_OPEN1)
     ...
     IF(UTL_READINITFILE('CSVFNAME',LINE,IU,1))THEN
      ...
      IF(WBAL_ANALYSE_READCSV(FNAME))THEN 
       LEX=WBAL_ANALYSE_READCONFIG(IU,0,IOPT)
      ENDIF
     ENDIF
     IF(.NOT.LEX)THEN
     ...
     ENDIF
     CLOSE(IU)
     CALL WBAL_ANALYSE_TAB3_FIELD()
     
    !## open csv
    CASE (ID_OPEN2)
     IF(WBAL_ANALYSE_READCSV(''))THEN
      CALL WBAL_ANALYSE_TAB3_FIELD()
     ELSE
    ...
    CASE (IDF_BUTTON3)
     ...
     IF(IDFWRITE(IDFP,IDFP%FNAME,0))CALL MANAGER_UTL_ADDFILE(IDFP%FNAME)
     !call idfplotfast()       
   END SELECT
 ...
 INTEGER,INTENT(IN) :: ITYPE
 TYPE(WIN_MESSAGE),INTENT(IN) :: MESSAGE
 INTEGER :: IROW,ICOL,IRGB,I,J,IADD,IBAL
 ...
 END SUBROUTINE WBAL_ANALYSE_TAB3_FIELD    
...
!     CALL WDIALOGFIELDSTATE(IDF_RADIO2,I)
  ...
  CASE (PUSHBUTTON)
   SELECT CASE (MESSAGE%VALUE1)
    ...
    CASE (ID_GRAPHICS,ID_PREVIEW)
     CALL WBAL_ANALYSE_PLOT(MESSAGE%VALUE1,0,'')
 ...
 DO I=1,NLIST
  IF(UTL_EQUALNAMES(SEARCH,LIST(I),ICAP=1))THEN
   !## remove from list
 ...
 INTEGER :: I,J,K,II,IU,IOS,SKIPLINES,IBAL
 INTEGER :: NV,NL
 ...
 NV=UTL_COUNT_COLUMNS(LINE,',;'); NV=NV-4
...
!              00/ !## 24 qmodf
 ...
 !###======================================================================
 SUBROUTINE WBAL_ANALYSE_PLOT(ID,IBATCH,OUTPUTFNAME)
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ID,IBATCH
 CHARACTER(LEN=*) :: OUTPUTFNAME
 INTEGER :: IOPT,IG,I1,I2,N,IUNIT,IGG,ITYPE,IEXIT
 CHARACTER(LEN=256) :: DIR,GTITLE,FNAME
 ...
 CALL WDIALOGSELECT(ID_DWBAL_ANALYSE_TAB4)
 GTITLE='Balans $'; IF(IBATCH.EQ.0)CALL WDIALOGGETSTRING(IDF_STRING2,GTITLE)

 IF(ID.EQ.ID_GRAPHICS)THEN
  !## will be an existing folder
  CALL WDIALOGGETSTRING(IDF_STRING1,DIR)
  IF(LEN_TRIM(DIR).EQ.0)THEN
   CALL WMESSAGEBOX(YESNOCANCEL,QUESTIONICON,COMMONNO,'There is no output folder specified.'//CHAR(13)// &
 ...
 SELECT CASE (IOPT)
  !## timeseries
  CASE (1)
   !## in graph plot optie maken om de bmp te saven en terug te keren - loopn of GRAPHDIM%GRAPHNAMES() met name
   LEXPORT=.FALSE.; IF(ID.EQ.ID_GRAPHICS)LEXPORT=.TRUE.
   !GRAPHDIM(:)%IFIXX=0; GRAPHDIM(:)%IFIXY=0; 
!   GRAPHDIM(:)%XTITLE='Time'
   CALL GRAPH_INIT(LEXPORT=LEXPORT,DIR=DIR)
   DO
    CALL WMESSAGE(ITYPE,MESSAGE)
    CALL GRAPH_MAIN(ITYPE,MESSAGE,IEXIT=IEXIT)
    IF(IEXIT.EQ.1)EXIT
   ENDDO
!   CALL GRAPH_PLOT(LEXPORT=LEXPORT,DIR=DIR)
!   IF(IUNIT.EQ.1)THEN
!    GRAPHDIM(:)%YTITLE='Volumes (m3/d)'; 
!    CALL GRAPH_PLOT(LEXPORT=LEXPORT,DIR=DIR)
!   ELSEIF(IUNIT.EQ.2)THEN
!    GRAPHDIM(:)%YTITLE='Quantity (mm/d)'; 
!    CALL GRAPH_PLOT(LEXPORT=LEXPORT,DIR=DIR)
!   ENDIF
  !## graph
  CASE (2)
   ...
   IF(N.GT.25)THEN
    IF(ID.EQ.ID_GRAPHICS)THEN
     CALL WMESSAGEBOX(YESNOCANCEL,QUESTIONICON,COMMONNO,'Are you sure to EXPORT '//TRIM(ITOS(N))//' images ?','Question')
    ELSE
     CALL WMESSAGEBOX(YESNOCANCEL,QUESTIONICON,COMMONNO,'Are you sure to DISPLAY '//TRIM(ITOS(N))//' images sequentially ?','Question')
    ENDIF
    IF(WINFODIALOG(4).NE.1)RETURN
   ENDIF
   CALL WDIALOGSELECT(ID_DWBAL_ANALYSE_TAB4)
   ...
   IF(ID.EQ.ID_GRAPHICS)CALL UTL_MESSAGEHANDLE(0)
   IGG=0; DO IG=1,SIZE(GRAPHDIM%GRAPHNAMES)
  ...
  CASE (4)
   ...
   ELSE
    FNAME=OUTPUTFNAME
   ENDIF
   IF(WBAL_ANALYSE_EXPORTCSV(FNAME))THEN; ENDIF
 ...
 REAL(KIND=DP_KIND),POINTER,DIMENSION(:) :: DT
 CHARACTER(LEN=16),POINTER,DIMENSION(:) :: XTXT=>NULL()
 CHARACTER(LEN=16) :: TXTL,TXTZ,TUNIT,LUNIT
 INTEGER,ALLOCATABLE,DIMENSION(:) :: IDATES,SQ,UQ,OSQ,OQ
 REAL(KIND=DP_KIND) :: QIN,QOU,Q
 ...
 ALLOCATE(SQ(MBUDGET),UQ(MBUDGET),OSQ(MBUDGET),OQ(MBUDGET))
 IQ=0; DO I=1,NBUDGET
 ...
 IF(ITTYPE.EQ.0)THEN
  ...
  SELECT CASE (IAG)
   !## all
   CASE (1); NXG=WBAL_ANAYSE_PREPARE_GETALL(IY1,IM1,ID1,IY2,IM2,ID2,SDATE,DT,XTXT);     TUNIT='d'
   !## year
   CASE (2); NXG=WBAL_ANAYSE_PREPARE_GETYEAR(IM1,ID1,IY1,IY2,SDATE,DT,XTXT);            TUNIT='year'
   !## months
   CASE (3); NXG=WBAL_ANAYSE_PREPARE_GETMONTHS(IY1,IM1,IY2,IM2,SDATE,DT,XTXT);          TUNIT='month'
   !## hydrologic seasons
   CASE (4); NXG=WBAL_ANAYSE_PREPARE_GETHSEASON(IY1,IM1,IY2,IM2,SDATE,DT,XTXT);         TUNIT='hseason'
   !## decade
   CASE (5); NXG=WBAL_ANAYSE_PREPARE_GETDECADES(IY1,IM1,ID1,IY2,IM2,ID2,SDATE,DT,XTXT); TUNIT='decade'
   !## hydrological year
   CASE (6); NXG=WBAL_ANAYSE_PREPARE_GETHYEAR(IY1,IM1,IY2,IM2,SDATE,DT,XTXT);           TUNIT='hyear'
   !## quarters
   CASE (7); NXG=WBAL_ANAYSE_PREPARE_GETQUARTERS(IY1,IM1,IY2,IM2,SDATE,DT,XTXT);        TUNIT='quarteryear'
   !## none - true dates in the result array
   CASE (8); NXG=WBAL_ANAYSE_PREPARE_GETSELDATES(CMDATE,SDATE,DT,XTXT);                 TUNIT='d'
  END SELECT
 ...
 ELSE
  ...
  DO I=1,NXG; DT(I)=1.0D0; XTXT(I)=CMDATE(I); ENDDO
  TUNIT='-'
  
 ENDIF
 
 LUNIT='m3'; IF(IUNIT.EQ.2)LUNIT='mm'
 ...
 DO I=1,MBUDGET; DO J=1,MGROUP
  ...
  GRAPH(I,J)%RX=0.0D0; GRAPH(I,J)%RY=0.0D0; GRAPH(I,J)%NP=NXG; GRAPH(I,J)%CTYPE=''; GRAPH(I,J)%ICLR=0
 ENDDO; ENDDO

 !## add custom predefined axes titles
 DO I=1,MGROUP
  GRAPHDIM(I)%IFIXX=0; GRAPHDIM(I)%IFIXY=0
  ALLOCATE(GRAPHDIM(I)%XTXT(NXG),GRAPHDIM(I)%XPOS(NXG)); GRAPHDIM(I)%XTXT=''; GRAPHDIM(I)%XPOS=0.0D0; IF(ITTYPE.EQ.1)GRAPHDIM(I)%IFIXX=1
 ENDDO
 ...
 !## set axes dimensions
 DO I=1,MGROUP
  GRAPHDIM(I)%XINT=1.0D0; GRAPHDIM(I)%XMIN=GRAPHDIM(I)%XPOS(1)-1.0D0; GRAPHDIM(I)%XMAX=GRAPHDIM(I)%XPOS(NXG)+1.0D0
  !## all on a seperate graph-plot
  GRAPH(:,I)%IGRAPH=I
  !## all stacked histograms
  GRAPH(:,I)%GTYPE =3
  !## set graph title
  IF(IUNIT.EQ.1)THEN
   GRAPHDIM(I)%YTITLE='Volumes ('//TRIM(LUNIT)//'/'//TRIM(TUNIT)//')'
  ELSEIF(IUNIT.EQ.2)THEN
 ...
 K=0; DO I=1,NLAY; DO J=1,NZONE
  ...
  GRAPHDIM(K)%GRAPHNAMES=TRIM(TXTL)//'; '//TRIM(TXTZ) 
 ENDDO; ENDDO
 ...
 JQ=0; DO IIQ=1,SIZE(OSQ) 
  ...
  IF(IIQ.GT.1)THEN
   PQ=OQ(OSQ(IIQ-1)); IF(BUDGET(PQ)%IGROUP.NE.BUDGET(IQ)%IGROUP)JQ=JQ+1
  ELSE
   !## increase only whenever different group starts
   JQ=JQ+1
  ENDIF
  
  IG=0; DO J=1,NZONE; DO I=1,NLAY
   IF(CLILAY(I).EQ.0)CYCLE
   IF(CLIZONE(J).EQ.0)CYCLE
...
!  IF(ZSUM.EQ.1)IZ=1
  ...
  !## sum for layers
  N=MZONE; IF(ZSUM.EQ.1)THEN; IZ=1; N=1; ENDIF
  !## sum for zones
  IF(LSUM.EQ.1)IL=1
  ...
  DO IIQ=1,SIZE(OSQ) 
   ...
   IF(IUNIT.EQ.2)THEN
    QIN=QIN/(1000.0D0*GWBAL(1)%AREA(I))
 ...
 DEALLOCATE(CMDATE,CMLAY,CMZONE,IDATES,SQ,UQ,OSQ,OQ) 
  ...
  DO IG=1,MGROUP
   ...
   IF(INET.EQ.1)THEN
    DO IB=1,MBUDGET,2
     ...
     GRAPH(IB  ,IG)%RY(ID)=0.0D0
     GRAPH(IB+1,IG)%RY(ID)=0.0D0
     IF(Q.GT.0.0D0)GRAPH(IB  ,IG)%RY(ID)=Q
     IF(Q.LT.0.0D0)GRAPH(IB+1,IG)%RY(ID)=Q
    ENDDO
   ENDIF
   !## aggregate
   IF(IOPT.EQ.1)THEN
 ...
 REAL(KIND=DP_KIND),POINTER,INTENT(IN),DIMENSION(:) :: DT
 CHARACTER(LEN=*),INTENT(INOUT),DIMENSION(:),POINTER :: XTXT
 INTEGER :: I,J
 ...
 REAL(KIND=DP_KIND),INTENT(OUT),DIMENSION(:),POINTER :: DT
 ...
 ND=SIZE(CMDATE); ALLOCATE(DT(ND),XTXT(ND)); DT=0.0D0; XTXT=''
 
 ND=0; DO I=1,SIZE(CMDATE)
  ND=ND+1
  READ(CMDATE(I),*) IDATE
  JDATE=IDATE; IF(I.LT.SIZE(CMDATE))READ(CMDATE(I+1),*) JDATE
  DT(I)  =(UTL_IDATETOJDATE(JDATE)-UTL_IDATETOJDATE(IDATE))+1.0D0
 ...
 ALLOCATE(DT(1),XTXT(1)); DT=0.0D0; XTXT=TRIM(ITOS(SDATE))//'-'//TRIM(ITOS(EDATE))
 DT(1)=(UTL_IDATETOJDATE(EDATE)-UTL_IDATETOJDATE(SDATE))+1.0D0
 ...
 !## define dt pointer
 ALLOCATE(DT(NY),XTXT(NY)); XTXT=''; DT=0.0D0
 IY=IY1; DO I=1,NY
  DT(I)=365.0; IF(WDATELEAPYEAR(IY))DT(I)=DT(I)+1.0D0
 ...
 DO I=1,2
  ...
  !## define dt pointer
  IF(I.EQ.1)THEN; ALLOCATE(DT(ND),XTXT(ND)); XTXT=''; DT=0.0D0; ENDIF
 ...
 DO I=1,2
  ...
  !## define dt pointer
  IF(I.EQ.1)THEN; ALLOCATE(DT(ND),XTXT(ND)); DT=0.0D0; XTXT=''; ENDIF
 ...
 !## define dt pointer
 ALLOCATE(DT(NM),XTXT(NM)); XTXT=''; DT=0.0D0
 IM=IM1; IY=IY1
 DO I=1,NM
 ...
 ENDIF
 CALL WDIALOGPUTMENU(IDF_MENU1,GRAPHDIM%GRAPHNAMES,MGROUP,1)
 ...
 DO I=1,MBUDGET
  ...
  CALL WGRIDPUTDOUBLE(IDF_GRID1,I,GRAPH(I,IG)%RY,N,'(F15.3)')
 ENDDO
 ...
 DO IG=1,MGROUP 
  WRITE(IU,'(/A)') GRAPHDIM(IG)%GRAPHNAMES
  ...
  ENDIF
  DO I=1,SIZE(GRAPHDIM(1)%XTXT),INET+1
   CDATE=GRAPHDIM(1)%XTXT(I)
   WRITE(IU,'(999A)') TRIM(CDATE),(','//TRIM(RTOS(GRAPH(J,IG)%RY(I),'G',7)),J=1,MBUDGET)
  ENDDO
 ENDDO
 ...
 REAL(KIND=DP_KIND),DIMENSION(NFLX,2) :: Q
 CHARACTER(LEN=10),DIMENSION(NFLX) :: QTXT
 REAL(KIND=DP_KIND),PARAMETER :: CS=0.0075D0 !## charactersize
...
!!     Q(12,1)=QEVT_IN   Q(12,1)=QEVT_OUT    Q(24,1)=QMODF_IN  Q(24,1)=QMODF_OUT   
  ...
  TITLE=GTITLE
  IF(INDEX(GTITLE,'$').GT.0)TITLE=UTL_SUBST(GTITLE,'$','('//TRIM(GRAPHDIM(1)%XTXT(IT))//';'//TRIM(GRAPHDIM(IG)%GRAPHNAMES)//')')

  IF(.NOT.DRAWBAL(Q,QTXT,NXPIX,NYPIX,CS,IPOL,SIZE(LIZONE),QSUBREGIO,CLRIZONE,IPLG,IDFP,LOCAL,TITLE,IBITMAP,LSUM,STRUNIT,SUMNR))EXIT ITLOOP
  ...
  ELSE
   ...
   PNGNAME=TRIM(GRAPHDIM(1)%XTXT(IT))//'_'//TRIM(GRAPHDIM(IG)%GRAPHNAMES)
   DO
 ...
 IF(IBATCH.EQ.0)THEN
  ...
  CALL MAIN_UTL_INACTMODULE(ID_WBAL_ANALYSE)
