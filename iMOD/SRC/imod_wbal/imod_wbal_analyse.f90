...
USE MOD_UTL, ONLY : UTL_GETUNIT,UTL_CAP,UTL_IDATETOJDATE,UTL_JDATETOIDATE,ITOS,IDATETOGDATE,UTL_GDATE,UTL_GETUNIQUE_CHAR, &
            UTL_WSELECTFILE,RTOS,UTL_DEBUGLEVEL,UTL_GETUNIQUE_INT,UTL_MESSAGEHANDLE,UTL_READINITFILE
 ...
 SELECT CASE (IOPT)
  ...
  CASE (2)
   ...
   IGG=0; DO IG=1,SIZE(GRAPHNAMES)
    ...
    CALL WBAL_ANALYSE_PLOTIMAGE(ID,IG,IGG,I1,I2,DIR,GTITLE,IUNIT,LSUM)
   ENDDO
  !## table
  CASE (3)
...
! IF(IOPT.EQ.1)THEN
 ...
 !###======================================================================
 SUBROUTINE WBAL_ANALYSE_PLOTIMAGE(ID,IG,IGG,IP1,IP2,DIR,GTITLE,IUNIT,LSUM)
 !###======================================================================
 IMPLICIT NONE
 ...
 DO IT=1,SIZE(GRAPH(1,1)%RY)
  ...
  DO IB=1,MBUDGET,2
   ...
   !## skip this one
   IF(I.GT.NZONE)THEN
    CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Cannot find the polygon number from '//TRIM(GRAPH(I1,IG)%CTYPE),'Error')
    CALL WBAL_ANALYSE_PLOTIMAGE_CLOSE(); RETURN
   ENDIF
...
!!     Q(12,1)=QEVT_IN   Q(12,1)=QEVT_OUT    Q(24,1)=QMODF_IN  Q(24,1)=QMODF_OUT   
  ...
  IF(.NOT.DRAWBAL(Q,QTXT,NXPIX,NYPIX,CS,IPOL,SIZE(LIZONE),QSUBREGIO,LPERC,CLRIZONE,IPLG,IDFP,LOCAL,GTITLE,IBITMAP,LSUM,STRUNIT,SUMNR))THEN
   CALL WBAL_ANALYSE_PLOTIMAGE_CLOSE(); RETURN
  ENDIF
  ...
  ELSE
   ...
   PNGNAME=TRIM(DIR)//'\FIGURE_'//TRIM(ITOS(IG))//'.PNG'
   ...
   IF(I.NE.0)THEN 
    CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'iMOD cannot SAVE the requested bitmap file called:'//CHAR(13)//&
     TRIM(PNGNAME),'Error')
   ELSE
    IF(IT.EQ.SIZE(GRAPH(1,1)%RY))THEN
