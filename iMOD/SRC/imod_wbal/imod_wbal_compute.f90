...
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,RTOS,UTL_CAP,JD,UTL_IDATETOJDATE,UTL_SUBST,UTL_GDATE,UTL_JDATETOIDATE,UTL_CLOSEUNITS, &
     UTL_INSIDEPOLYGON,UTL_CREATEDIR,UTL_DIRINFO_POINTER,JDATETOFDATE,IDATETOGDATE, &  !GDATETOJDATE
...
CONTAINS
 ...
 INTEGER :: I,J,IU,NLAY,NZONE,IOS,IY1,IY2,ITIME,SKIPLINES,CFN_N_ELEM,IPERIOD,ITYPE
 ...
 IF(WBAL_ISTEADY.EQ.0)THEN     !## transient
  ...
  DO I=1,NIP
   ...
   DO IBAL=1,MXTP
    ...
    DO JSYS=1,TP(IBAL)%NSYS
     IF(TP(IBAL)%ISYS(JSYS).EQ.0)THEN
      WRITE(IUTXT(I),'(A,F10.1)') '"'//TRIM(TP(IBAL)%ALIAS)//'_IN(M3/D)"',-9999.0
      WRITE(IUTXT(I),'(A,F10.1)') '"'//TRIM(TP(IBAL)%ALIAS)//'_OUT(M3/D)"',-9999.0
     ELSE
