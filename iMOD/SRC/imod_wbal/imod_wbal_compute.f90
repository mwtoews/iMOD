...
USE MOD_IDF, ONLY : IDFREAD,IDFALLOCATEX,IDFCOPY,IDFWRITE,IDFGETVAL,IDFDEALLOCATEX,IDFIROWICOL,IDFGETLOC,IDFFILLCOMMENT,IDFNULLIFY
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,RTOS,UTL_CAP,JD,IDATETOJDATE,UTL_SUBST,GDATE,JDATETOIDATE,CLOSEUNITS,UTL_INSIDEPOLYGON
...
CONTAINS
 ...
 REAL :: QMIN,QMAX
 INTEGER :: I,J,K,ILAY,JLAY,IY,IM,ID,JDATE,IU,ICSV,IBAL,ITYPE, &
   IWIN,JSYS,NBDG,IBDG,JBAL,IG,JG,JD1,JD2
 LOGICAL :: LEX,LIP
 ...
 DO JDATE=JD1,JD2
  ...
  IF(.NOT.WBALCORRECTPERIOD(JDATE,CDATE))CYCLE; NPER=NPER+1 
 ENDDO
 ...
 DO JDATE=JD1,JD2
  ...
  !## is current date within specified time window?
  IF(.NOT.WBALCORRECTPERIOD(JDATE,CDATE))CYCLE
  ...
  DO JLAY=1,WBAL_NLAYER
   ...
   DO IBAL=1,MXTP
    ...
    DO JSYS=1,TP(IBAL)%NSYS
     ...
     IF(IBAL.EQ.3.AND.ILAY.GT.1)NBDG=1 !THEN
     ...
     DO IBDG=0,NBDG
      ...
      ELSE
       FNAME=TRIM(FNAME)//'_'//TRIM(ITOS(JDATETOIDATE(JDATE)))//'_L'//TRIM(ITOS(ILAY-IBDG))//'.IDF'
      ENDIF
      ...
      IF(.NOT.LIP)THEN
       ...
       IF(SUM(IPIDF%X).EQ.0.0)THEN
        ...
        RETURN
       ENDIF
       IF(ALLOCATED(WBAL))DEALLOCATE(WBAL)  ! FR 20131025
...
!10 CONTINUE
 ...
 !###======================================================================
 LOGICAL FUNCTION WBALCORRECTPERIOD(JDATE,CDATE)
 !###======================================================================
 IMPLICIT NONE
...
!  IF(.NOT.LEX)CYCLE
 ...
 IF(WBAL_ISTEADY.EQ.0)THEN     !## transient
  ...
  NN=0; DO IBAL=1,MXTP; IF(TP(IBAL)%IACT.NE.1)NN=NN+1; ENDDO
 
  DO I=1,NIP
   CALL WBALGETXYZONE(X,Y,INT(IPLIST(I)))
   WRITE(IU,*) X,Y,'wbaltimeseries\zone_'//TRIM(ITOS(INT(IPLIST(I))))

   FNAME=WBAL_OUTFNAME(:INDEX(WBAL_OUTFNAME,'.',.TRUE.))//'\wbaltimeseries\zone_'//TRIM(ITOS(INT(IPLIST(I))))
   CALL OSD_OPEN(IUTXT(I),FILE=FNAME,STATUS='UNKNOWN')
   WRITE(IUTXT(I),*) NPER
   WRITE(IUTXT(I),*) NN*2 !## in and out fluxes
   DO IBAL=1,MXTP
    IF(TP(IBAL)%IACT.NE.1)CYCLE
    DO JSYS=1,TP(IBAL)%NSYS
     IF(TP(IBAL)%ISYS(JSYS).EQ.0)THEN
      WRITE(IUTXT(I),'(A,F10.1)') '"'//TRIM(TP(IBAL)%ALIAS)//'_IN"',-99999999.0
      WRITE(IUTXT(I),'(A,F10.1)') '"'//TRIM(TP(IBAL)%ALIAS)//'_OUT"',-99999999.0
     ELSE
