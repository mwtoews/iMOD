...
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,RTOS,UTL_CAP,JD,IDATETOJDATE,UTL_SUBST,GDATE,JDATETOIDATE,CLOSEUNITS, &
     UTL_INSIDEPOLYGON,UTL_CREATEDIR,GDATETOJDATE,UTL_DIRINFO_POINTER,JDATETOFDATE,IDATETOGDATE,UTL_GENLABELSREAD, &
...
CONTAINS
 ...
 INTEGER :: I,J,K,JJ,IU,NLAY,NZONE,IOS,ILABELS,IY1,IY2,ITIME,SKIPLINES,CFN_N_ELEM,NLL,JD1,JD2,JDP,IPERIOD,IM1,IM2,ID1,ID2
 TYPE(WBALOBJ),ALLOCATABLE,DIMENSION(:) :: WBAL
 INTEGER,ALLOCATABLE,DIMENSION(:) :: CILAY,CIZONE
  
 WBAL_GRAPHCOMPUTE=.FALSE.
 
 ALLOCATE(WBAL(2)); DO I=1,2; NULLIFY(WBAL(I)%IDATE,WBAL(I)%ILAY,WBAL(I)%IZONE,WBAL(I)%TXT,WBAL(I)%Q); ENDDO

 IU=UTL_GETUNIT()
 CALL OSD_OPEN(IU,FILE=CSVFNAME,STATUS='OLD',ACTION='READ',IOSTAT=IOS)
 IF(IOS.NE.0)THEN
 ...
 NV=CFN_N_ELEM(' ,;',3,LINE)
 NV=NV-3
 
 ALLOCATE(WBAL(1)%TXT(NV),WBAL(2)%TXT(NV))
 READ(LINE,*) TXT,TXT,TXT,(WBAL(1)%TXT(I),I=1,NV)
 WBAL(2)%TXT=WBAL(1)%TXT
 ...
 ALLOCATE(WBAL(1)%IDATE(NL),WBAL(1)%Q(NV,NL),WBAL(1)%ILAY(NL),WBAL(1)%IZONE(NL))
 ...
 !## read data
 DO I=1,NL
  READ(IU,*) WBAL(1)%IDATE(I),WBAL(1)%ILAY(I),WBAL(1)%IZONE(I),(WBAL(1)%Q(J,I),J=1,NV)
 ENDDO
...
! ENDDO
 ...
 DO 
  SELECT CASE (ITIME)
   !## as is
   CASE (0)
    ALLOCATE(WBAL(2)%IDATE(NL),WBAL(2)%Q(NV,NL),WBAL(2)%ILAY(NL),WBAL(2)%IZONE(NL))
    WBAL(2)=WBAL(1); NLL=NL
   !## total per period (m3/day)
   CASE (1)

    JD1=IDATETOJDATE(IY1)
    JD2=IDATETOJDATE(IY2)
    JDP=0

    !## number of periods
    SELECT CASE (IPERIOD)
     CASE (1)  !## year
      CALL GDATE(JD1,IY1,IM1,ID1)
      CALL GDATE(JD2,IY2,IM2,ID2)
      NLL=IY2-IY1+1
     CASE (2)  !## month
      CALL GDATE(JD1,IY1,IM1,ID1)
      CALL GDATE(JD2,IY2,IM2,ID2)
    ...
    NLL=NLL*NZONE*NLAY    
    ALLOCATE(WBAL(2)%IDATE(NLL),WBAL(2)%Q(NV,NLL),WBAL(2)%ILAY(NLL),WBAL(2)%IZONE(NLL))
    WBAL(2)%IDATE=0; WBAL(2)%Q=0.0; WBAL(2)%ILAY=0; WBAL(2)%IZONE=0
                        
    I=0
    K=0
    DO 
     I=I+1
     IF(I.GT.SIZE(WBAL(1)%IDATE))EXIT
     
     !## get year - julian dates !!!
     J=IDATETOJDATE(WBAL(1)%IDATE(I))
     ...
     IF(J.GT.JDP)THEN     !## goto the next period
      ...
      CALL IDATETOGDATE(WBAL(1)%IDATE(I),IY1,IM1,ID1)
      SELECT CASE (IPERIOD)
       CASE (1)  !## year
        JDP=JD1+365 !-1
     ...
     DO J=1,NLAY*NZONE
      ...
      WBAL(2)%IDATE(K+J)=JDATETOIDATE(JD1)
      WBAL(2)%ILAY(K+J) =WBAL(1)%ILAY(I)
      WBAL(2)%IZONE(K+J)=WBAL(1)%IZONE(I)

      DO JJ=1,NV
       WBAL(2)%Q(JJ,K+J)=WBAL(2)%Q(JJ,K+J)+WBAL(1)%Q(JJ,I)
      ENDDO
     ENDDO
  ...
  IF(WBAL_GRAPHPLOT(WBAL(2),NV,ILABELS,NLL,NLAY,NZONE))EXIT
 ...
 !###======================================================================
 LOGICAL FUNCTION WBAL_GRAPHPLOT(WBAL,NV,ILABELS,NL,NLAY,NZONE) 
 !###======================================================================
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: NV,NL,ILABELS,NLAY,NZONE
 TYPE(WBALOBJ),INTENT(INOUT) :: WBAL
 INTEGER,PARAMETER :: MXFLUX=11
 INTEGER :: NG,NP,NB,I,J,K,II,JJ,III,KK,KKK,IDATE,IX,IPOS,ITYPE
...
! ICOLOR(14)=WRGB(128,0  ,255) 
 ...
 DO I=1,NLAY
  DO J=1,NZONE
   K=K+1
   GRAPHNAMES(K)='Layer '//TRIM(ITOS(WBAL%ILAY(I)))//'; Zone Number '//TRIM(ITOS(WBAL%IZONE(J)))
  ENDDO
 ENDDO
 ...
 DO J=1,NG
  ...
  DO JJ=1,2
   ...
   DO I=1,SIZE(IBAR,1)
    ...
    GRAPH(III,J)%NP=NP
    GRAPH(III,J)%GTYPE=1
    GRAPH(III,J)%LEGTXT=UTL_CAP(WBAL%TXT(II),'U')
 ...
 IF(WBAL_ISTEADY.EQ.0)THEN
  ...
  !## short date
  IF(LDATE)THEN
   WRITE(CDATE,'(I8)') JDATETOIDATE(JDATE)
  !## long date
  ELSE
