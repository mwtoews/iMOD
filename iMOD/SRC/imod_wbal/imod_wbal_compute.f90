...
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,RTOS,UTL_CAP,JD,IDATETOJDATE,UTL_SUBST,GDATE,JDATETOIDATE,CLOSEUNITS, &
     ...
     NV,NL,VAR,MAXLEN,UTL_MESSAGEHANDLE,UTL_GETUNIQUE_CHAR
...
CONTAINS
 ...
 INTEGER :: NG,NP,NB,IU,I,J,K,II,JJ,III,KK,KKK,IDATE,NLAY,NZONE,IX,IPOS,ITYPE,SKIPLINES,IOS
 REAL :: X,TX
 ...
 ICOLOR(4) =WRGB(0  ,128,64 ) 
 ICOLOR(5) =WRGB(0  ,255,0  ) 
 ICOLOR(6) =WRGB(0  ,255,255) 
 ICOLOR(7) =WRGB(128,255,128) 
 ICOLOR(8) =WRGB(255,0  ,0  ) 
 ICOLOR(9) =WRGB(255,128,64 ) 
 ICOLOR(10)=WRGB(128,0  ,0  ) 
 ICOLOR(11)=WRGB(255,0,0)  !## wells
...
! ICOLOR(14)=WRGB(128,0  ,255) 
 ...
 DO 
  ...
  IF(UTL_CAP(CDATE,'U').EQ.'DATE')EXIT
 ENDDO
 CLOSE(IU)
 SKIPLINES=SKIPLINES-1
 
 !## read entire file in var()
 CALL UTL_GENLABELSREAD(CSVFNAME,SKIPLINES=SKIPLINES,ILABELS=2)
 
 IF(ALLOCATED(GRAPH))CALL PROFILE_DEALLGRAPH()
 ALLOCATE(CLAY(NL),CZONE(NL))
 DO I=1,NL; CLAY(I)=VAR(2,I); CZONE(I)=VAR(3,I); ENDDO
 
 !## find how many layers
 CALL UTL_GETUNIQUE_CHAR(CLAY,NL-2,NLAY)
 !## find how many zones
 CALL UTL_GETUNIQUE_CHAR(CZONE,NL-2,NZONE)
 ...
 NB=(NV-3)/(2*ITYPE) !## number of bars
 ...
 DO I=1,NLAY
  DO J=1,NZONE
   K=K+1
   GRAPHNAMES(K)='Layer '//TRIM(CLAY(I))//'; Zone Number '//TRIM(CZONE(J))
  ENDDO
 ENDDO
