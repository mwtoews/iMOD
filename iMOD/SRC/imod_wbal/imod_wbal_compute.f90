...
USE MOD_IDF, ONLY : IDFREAD,IDFALLOCATEX,IDFCOPY,IDFWRITE,IDFGETVAL,IDFDEALLOCATEX,IDFIROWICOL,IDFGETLOC, &
     IDFFILLCOMMENT,IDFNULLIFY,UTL_IDFGETDATE,IDFGETAREA,UTL_WRITE_FREE
USE MOD_UTL, ONLY : ITOS,UTL_GETUNIT,RTOS,UTL_CAP,JD,UTL_IDATETOJDATE,UTL_SUBST,UTL_GDATE,UTL_JDATETOIDATE,UTL_CLOSEUNITS, &
     ...
     MAXLEN,UTL_MESSAGEHANDLE,UTL_GETUNIQUE_INT  !NV,NL,
...
CONTAINS
 ...
 INTEGER :: I,ILAY,JLAY,IY,IM,ID,IU,ICSV,IBAL,ITYPE, &
   JSYS,NBDG,IBDG,IG,JG,NFILES,IFILES,J1,J2,NU,IOS,IROW,ICOL
...
!maximum is dit .... 65535
 ...
 IF(ICSV.EQ.2)THEN
  ...
  DO ICOL=1,IPIDF%NCOL; DO IROW=1,IPIDF%NROW
   IF(IPIDF%X(ICOL,IROW).GT.0.0)THEN
    IPIDF%X(ICOL,IROW)=IPLIST(INT(IPIDF%X(ICOL,IROW)))
   ELSE
    IPIDF%X(ICOL,IROW)=0.0
   ENDIF
  ENDDO; ENDDO
