...
USE MOD_PROFILE_UTL, ONLY : GRAPH,PROFILE_DEALLGRAPH,PROFILE_ALLGRAPH,PROFILE_PLOTGRAPH
USE MOD_WBAL_PAR
...
! ICOLOR(14)=WRGB(128,0  ,255) 
 ...
 CALL UTL_GENLABELSREAD(CSVFNAME,SKIPLINES=7,ILABELS=2)
 
 IF(ALLOCATED(GRAPH))CALL PROFILE_DEALLGRAPH()
 ALLOCATE(CLAY(NL),CZONE(NL))
 DO I=1,NL; CLAY(I)=VAR(2,I); CZONE=VAR(3,I); ENDDO
 ...
 IX=0; DO I=1,NB/2; DO J=1,2; IF(IBAR(I,J).GT.0)IX=IX+1; ENDDO; ENDDO; NB=IX
