...
!     PLOT separating layers
      ...
      CALL DBL_WGRTEXTFONT(IFAMILY=FFCOURIER,ISTYLE=1,TWIDTH=DW,THEIGHT=DH)
...
!     PLOT DASHED RECTANGLE AROUND SUBREGIOFLOW
      IF(II.GT.0) CALL DBL_IGRRECTANGLE(0.13D0*B,0.65D0*H,XPOS1+0.01D0*B,0.65D0*H-(II+0.5D0)*DH)
      IF(JJ.GT.0) CALL DBL_IGRRECTANGLE(0.76D0*B,0.65D0*H,XPOS2+0.01D0*B,0.65D0*H-(JJ+0.5D0)*DH)
...
!     *********************UNSATURATED*********************
      ...
      CALL CSIZE(1.5D0*CS,DW,DH,IPLOT)
      CALL DBL_WGRTEXTFONT(IFAMILY=FFHELVETICA,ISTYLE=1,TWIDTH=DW,THEIGHT=DH)
...
!###====================================================================
      ...
      IF(QIN.NE.0.OR.QUIT.NE.0) THEN
         ...
         CALL DBL_IGRLINETO(X+0.1D0*B,Y)
         CALL IGRCOLOURN(WRGB(0,0,0))
         CALL DBL_WGRTEXTORIENTATION(IALIGN=0,ANGLE=0.0D0,IDIR=0)
...
!        PLOT RIVER                                                                                         
         ...
         CALL IGRTRAPEZIUM(REAL(XTRAP1,4),REAL(YTRAP1,4),REAL(XTRAP2,4),REAL(YTRAP2,4),REAL(XL1,4),REAL(XL2,4),2)                                           
         IF(QUIT.NE.0) THEN                                                                                 
            CALL ARROWTEXT(XTRAP1,(YTRAP2+YTRAP1)/2,XTRAP1,(YTRAP2+YTRAP1)/2+0.1D0*H,&
                           XTRAP1,(YTRAP2+YTRAP1)/2+0.1D0*H,&
                           0,90.0D0,WRGB(255,0,0),ABS(QUIT),QTXT(1:LEN_TRIM(QTXT)))
         ENDIF                                                                                              
         IF(QIN.NE.0) THEN                                                                                  
            CALL ARROWTEXT(XTRAP1+XL1,(YTRAP2+YTRAP1)/2+0.1D0*H,XTRAP1+XL1,(YTRAP2+YTRAP1)/2,&
                           XTRAP1+XL1,(YTRAP2+YTRAP1)/2+0.1D0*H,&
...
!     CALCULATE PART OF GRAPHICS AREA IN XAND Y IRECTION
      ...
      CH=2.5D0*CS*REAL(WINFOBITMAP(IPLOT,1))/(DY*REAL(WINFOBITMAP(IPLOT,2)))
      CW=CS/DX
...
!###====================================================================
      ...
      IF(QIN.GT.0) THEN
         ...
         CALL ARROWTEXT(0.15D0*B,0.65D0*H-II*DH,0.2D0*B,0.65D0*H-II*DH,&
                       0.15D0*B,0.65D0*H+DH/2-II*DH,&
                       0,0.0D0,WRGB(0,0,255),QIN,STR(1:ILEN))
         IF(IFIT.EQ.0) THEN
            CALL PASTEL(ICOLPLG(JPOL),0.6D0); CALL DBL_IGRCIRCLE(0.14D0*B,0.65D0*H-II*DH,0.006D0*B)
         ELSE
            CALL IGRCOLOURN(WRGB(255,125,0)); CALL DBL_IGRCIRCLE(0.14D0*B,0.65D0*H-II*DH,0.006D0*B)
         ENDIF
         CALL IGRCOLOURN(WRGB(255,125,0)); CALL DBL_IGRCIRCLE(0.21D0*B,0.65D0*H-II*DH,0.006D0*B)
         
         CALL IGRFILLPATTERN(0,0,0)
         CALL IGRLINETYPE(0)
         CALL IGRCOLOURN(WRGB(0,0,0))
         CALL DBL_IGRCIRCLE(0.14D0*B,0.65D0*H-II*DH,0.006D0*B)
         CALL DBL_IGRCIRCLE(0.21D0*B,0.65D0*H-II*DH,0.006D0*B)
      ENDIF
      ...
      IF(QUIT.LT.0) THEN
         ...
         CALL ARROWTEXT(0.78D0*B,0.65D0*H-JJ*DH,0.83D0*B,0.65D0*H-JJ*DH,&
                        0.78D0*B,0.65D0*H+DH/2-JJ*DH,&
                        0,0.0D0,WRGB(255,0,0),ABS(QUIT),STR(1:ILEN))

         CALL IGRCOLOURN(WRGB(255,125,0)); CALL DBL_IGRCIRCLE(0.77D0*B,0.65D0*H-JJ*DH,0.006D0*B)
         IF(IFIT.EQ.0) THEN
            CALL PASTEL(ICOLPLG(JPOL),0.6D0); CALL DBL_IGRCIRCLE(0.84D0*B,0.65D0*H-JJ*DH,0.006D0*B)
         ELSE
            CALL IGRCOLOURN(WRGb(255,125,0)); CALL DBL_IGRCIRCLE(0.84D0*B,0.65D0*H-JJ*DH,0.006D0*B)
         ENDIF

         CALL IGRFILLPATTERN(0,0,0)
         CALL IGRCOLOURN(WRGB(0,0,0))
         CALL IGRLINETYPE(0)
         CALL DBL_IGRCIRCLE(0.84D0*B,0.65D0*H-JJ*DH,0.006D0*B)
         CALL DBL_IGRCIRCLE(0.77D0*B,0.65D0*H-JJ*DH,0.006D0*B)
      ENDIF
