...
CONTAINS
 ...
 DO I=1,MEASURE%NPER
  ...
  IF(IDF(I)%IU.NE.0) THEN
   ...
   IF(IROW.NE.0.AND.ICOL.NE.0)MEASURE%OBS(I)=IDFGETVAL(IDF(I),IROW,ICOL)
  ENDIF
 ENDDO
 ...
 DO I=1,NC
  READ(IU(3),*,IOSTAT=IOS) CDUM,XVAL(I)
  IF(IOS.NE.0) THEN; CALL TS_ERROR_MESSAGE(IBATCH,'ERROR READING FILE, line 3 in  '//TRIM(FNAME)//'. (2A)'); RETURN; ENDIF
 ENDDO
 OBSERVATION%NODATA=XVAL(TXTCOL)

 ALLOCATE(OBSERVATION%OBS(NDATES),OBSERVATION%IDATE(NDATES)); OBSERVATION%OBS=OBSERVATION%NODATA
 
 !## Read rest of the file within the range of dates
 J=0; DO I=1,NDATES
  READ(IU(3),*,IOSTAT=IOS) IDATE,(XVAL(K),K=1,NC-1)
  ...
  IF(IDATE.GE.SDATE.AND.IDATE.LE.EDATE) THEN
   ...
   OBSERVATION%OBS(J)=XVAL(TXTCOL-1)
   OBSERVATION%IDATE(J)=IDATE
  ENDIF
  OBSERVATION%NPER=J
 ENDDO

 CLOSE(IU(3)); DEALLOCATE(XVAL)
 ...
 ELSE
  ...
  ALLOCATE(IDATES(MEASURE%NPER+OBSERVATION%NPER))
  DO I=1,MEASURE%NPER; IDATES(I)=MEASURE%IDATE(I); ENDDO
  I=I-1; DO J=1,OBSERVATION%NPER; I=I+1; IDATES(I)=OBSERVATION%IDATE(J); ENDDO
...
!  CF(2)='Flux_mm/d'
  SELECT CASE (I)
   ...
   CASE (2); LINE=TRIM(CF(I))//','//TRIM(RTOS(OBSERVATION%NODATA,'G',8))
   CASE (3); LINE=TRIM(CF(I))//','//TRIM(RTOS(MEASURE%NODATA,'G',8))
  END SELECT
 ...
 IF(IASSF.EQ.0)THEN
  DO I=1,MEASURE%NPER
   LINE=TRIM(ITOS_DBL(MEASURE%IDATE(I)))//','//TRIM(RTOS(MEASURE%OBS(I),'G',8))
   WRITE(IU(3),*) TRIM(LINE)
   N1=N1+1
  ENDDO
 ELSE
  DO I=1,NRECS
   ...
   DO J=1,OBSERVATION%NPER
    IF(OBSERVATION%IDATE(J).EQ.IDATES(I))THEN
     IF(OBSERVATION%OBS(J).NE.OBSERVATION%NODATA)THEN
      LINE=TRIM(LINE)//','//TRIM(RTOS(OBSERVATION%OBS(J),'G',8)); N2=N2+1; EXIT
     ENDIF
    ENDIF
   ENDDO
   !## nothing found
   IF(J.GT.OBSERVATION%NPER)LINE=TRIM(LINE)//','//TRIM(RTOS(OBSERVATION%NODATA,'G',8))
   DO J=1,MEASURE%NPER
    IF(MEASURE%IDATE(J).EQ.IDATES(I))THEN
     IF(MEASURE%OBS(J).NE.MEASURE%NODATA)THEN 
      LINE=TRIM(LINE)//','//TRIM(RTOS(MEASURE%OBS(J),'G',8)); N1=N1+1; EXIT
     ENDIF
    ENDIF
   ENDDO
   !## nothing found
   IF(J.GT.MEASURE%NPER)LINE=TRIM(LINE)//','//TRIM(RTOS(MEASURE%NODATA,'G',8))
   WRITE(IU(3),*) TRIM(LINE)
  ENDDO
 ENDIF
 ...
 J=0; DO I=1,SIZE(IDFNAMES)
  ...
  IF(LEX)THEN
   ...
   IF(IDATE.GE.SDATE.AND.IDATE.LE.EDATE)THEN
    J=J+1; MEASURE%IDATE(J)=IDATE; MEASURE%NODATA=IDF(I)%NODATA; IDF(J)%IU=IDF(I)%IU
   ENDIF
 ...
 IF(IBATCH.EQ.0)THEN
  CALL WINDOWSELECT(0)
  CALL WINDOWOUTSTATUSBAR(4,'Found '//TRIM(ITOS(MEASURE%NPER))//' IDF"s within time constraint')
 ELSE
  WRITE(*,*) 'Found ',MEASURE%NPER,' IDF"s within time constraint'
 ENDIF
 ...
 IF(OBSERVATION%NPER.EQ.0)RETURN

 MEASURE%RES=MEASURE%NODATA
 DO I=1,MEASURE%NPER
  
  DO J=1,OBSERVATION%NPER
   IF(MEASURE%IDATE(I).EQ.OBSERVATION%IDATE(J))THEN
    MEASURE%RES(I)=MEASURE%OBS(I)-OBSERVATION%OBS(J); EXIT
   ENDIF
  ENDDO
 ...
 ALLOCATE(MEASURE%OBS(SIZE(IDFNAMES)))
 ALLOCATE(MEASURE%RES(SIZE(IDFNAMES)))
 ALLOCATE(MEASURE%IDATE(SIZE(IDFNAMES)))
 ...
 IF(ASSOCIATED(MEASURE%OBS))  DEALLOCATE(MEASURE%OBS)
 IF(ASSOCIATED(OBSERVATION%OBS))  DEALLOCATE(OBSERVATION%OBS)
 IF(ASSOCIATED(MEASURE%RES))  DEALLOCATE(MEASURE%RES)
 IF(ASSOCIATED(OBSERVATION%RES))  DEALLOCATE(OBSERVATION%RES)
 IF(ASSOCIATED(MEASURE%IDATE))DEALLOCATE(MEASURE%IDATE)
 IF(ASSOCIATED(OBSERVATION%IDATE))DEALLOCATE(OBSERVATION%IDATE)
