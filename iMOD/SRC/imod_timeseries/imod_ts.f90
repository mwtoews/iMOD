...
!   WRITE(*,*) OBS%IDATE(j),OBS%OBS(J)
 ...
 ELSE
  DO I=1,NRECS
   ...
   DO J=1,MSR%NPER
    IF(MSR%IDATE(J).EQ.IDATES(I))THEN; LINE=TRIM(LINE)//','//TRIM(RTOS(MSR%OBS(J),'G',8)); N1=N1+1; EXIT; ENDIF
   ENDDO
   !## nothing found
   IF(J.GT.MSR%NPER)LINE=TRIM(LINE)//','//TRIM(RTOS(MSR%NODATA,'G',8))
   DO J=1,OBS%NPER
    IF(OBS%IDATE(J).EQ.IDATES(I))THEN; LINE=TRIM(LINE)//','//TRIM(RTOS(OBS%OBS(J),'G',8)); N2=N2+1; EXIT; ENDIF
   ENDDO
   !## nothing found
