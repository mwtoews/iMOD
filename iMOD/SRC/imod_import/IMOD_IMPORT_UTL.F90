...
TYPE PACKAGE_TYPE
 ...
 REAL(KIND=DP_KIND),DIMENSION(10) :: X
...
CONTAINS
 ...
 CHARACTER(LEN=*),INTENT(IN) :: TXT
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(:) :: F
...
!  IUF=IU
 ...
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(NROW) :: DELC
 REAL(KIND=DP_KIND),INTENT(IN),DIMENSION(NCOL) :: DELR
 ...
 DO ILAY=1,NLAY
  ...
  NP=0
  DO I=1,N
   IF(XP(I)%ILAY.EQ.ILAY.AND.XP(I)%X(1).NE.0.0D0)NP=NP+1
  END DO
  IF(NP.GT.0)THEN
   TLAY(ILAY)=1
   WRITE(IU,*) NP
   ...
   WRITE(IU,*) '0,TXT'
   DO I=1,N
    IF(XP(I)%ILAY.EQ.ILAY.AND.XP(I)%X(1).NE.0.0D0)THEN
 ...
 DO ILAY=1,NLAY
  IF(TLAY(ILAY).EQ.1)THEN
   LINE='  '//TRIM(ITOS(ILAY))//',1.0D0,0.0D0,"'//TRIM(FNAME)//TRIM(ITOS(ILAY))//'.IPF"'
   WRITE(IURUN,'(A)') TRIM(LINE)
  ENDIF
 END DO
 ...
 !###======================================================================
 LOGICAL FUNCTION IMPORT_WRT1PACKAGE(FNAME,IDF,N,M,NLAY,CPCK,CPACKAGE,NODATA,IURUN,IORDER)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ) :: IDF
 INTEGER,INTENT(IN) :: N,M,NLAY,IURUN
 ...
 INTEGER,ALLOCATABLE,DIMENSION(:,:) :: TLAY
 REAL(KIND=DP_KIND),DIMENSION(:,:),ALLOCATABLE :: IX
   ...
   IF(NP.GT.0)THEN
    ...
    DO IS=1,TLAY(J,ILAY)
     ...
     ELSE
      ...
      IF(NODATA(IPCK).EQ.0.0D0)IX=MIN(IX,1.0D0)
     ENDIF

     DO IROW=1,IDF%NROW
      DO ICOL=1,IDF%NCOL
       IF(IX(ICOL,IROW).GT.0.0D0)THEN
 ...
 IF(TRIM(CPCK).EQ.'riv'.AND.IRIV5.EQ.0)THEN
  DO ILAY=1,NLAY
   IF(TLAY(1,ILAY).GT.0)THEN
    DO IS=1,TLAY(1,ILAY)
     LINE=' '//TRIM(ITOS(ILAY))//',1.0D0,0.0D0,1.0D0'
