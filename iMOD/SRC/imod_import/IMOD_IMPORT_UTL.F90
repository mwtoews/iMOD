...
USE WINTERACTER
USE MOD_UTL, ONLY : UTL_CAP,ITOS,UTL_GETUNIT,UTL_CREATEDIR,JDATETOIDATE,UTL_MESSAGEHANDLE
...
! DO I=1,SIZE(XP); DEALLOCATE(XP(I)%X); ENDDO
 DEALLOCATE(XP)
 ...
 INTEGER :: IROW,ICOL,IOS,I,LOCAT,IUF,JU,IFILE
 ...
 ELSE
  ...
  READ(IU,'(I10,F10.0,A20)') LOCAT,CNSTNT,FMTIN
  IFILE=0
 ...
 INTEGER,INTENT(IN) :: NCOL,NROW,IU
 REAL,INTENT(OUT),DIMENSION(NCOL,NROW) :: X
 INTEGER :: IROW,ICOL,IOS,I,LOCAT,CNSTNT,IUF,JU,IFILE
 ...
 ELSE
  IF(IFILE.EQ.0)THEN
   IF(FMTIN.EQ.'*')THEN
    READ(IU,*,IOSTAT=IOS) ((IX(ICOL,IROW),ICOL=1,NCOL),IROW=1,NROW)
   ELSE
    DO IROW=1,NROW
     READ(IU,FMTIN,IOSTAT=IOS) (IX(ICOL,IROW),ICOL=1,NCOL)
    ENDDO
   ENDIF
   IF(IOS.NE.0)THEN
    CALL IMPORT_ERROR('Error reading data matrix '//TRIM(TXT),IOS,IU)
 ...
 !###======================================================================
 LOGICAL FUNCTION IMPORT_WRT1PACKAGE(FNAME,IDF,N,M,NLAY,CPCK,CPACKAGE,NODATA,IPER,IURUN,IORDER,SDATE)
 !###======================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ) :: IDF
 INTEGER,INTENT(IN) :: N,M,NLAY,IPER,IURUN,SDATE
 ...
 INTEGER :: ILAY,I,J,NP,IS,IPCK,IROW,ICOL,NPFILES
 INTEGER,ALLOCATABLE,DIMENSION(:,:) :: TLAY
 ...
 DO J=1,M
  ...
  DO ILAY=1,NLAY
   ...
   IF(NP.GT.0)THEN
    ...
    DO IS=1,TLAY(J,ILAY)
     LINE=TRIM(FNAME)//'_'//TRIM(CPACKAGE(IPCK))//'_'//TRIM(ITOS(JDATETOIDATE(SDATE)))//'_L'//TRIM(ITOS(ILAY))
