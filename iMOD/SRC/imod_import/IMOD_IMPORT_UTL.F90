...
CONTAINS
 ...
 IF(IOS.NE.0)THEN
  CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not allocate memory for XP()','Error')
  RETURN
 ENDIF
 ...
 ELSE
  ...
  ELSEIF(IFILE.EQ.1.OR.IFILE.EQ.2)THEN
   ...
   IF(IUF.LE.0)THEN
    CALL WMESSAGEBOX(OKONLY,EXCLAMATIONICON,COMMONOK,'Can not find file'//CHAR(13)// &
     TRIM(FNAME),'Error')
    RETURN
   ENDIF
   ...
   IF(ALLOCATED(DATAI))THEN
    DO I=1,SIZE(DATAI)
     IF(LOCAT.EQ.DATAI(I))THEN
      ...
      IF(IOS.NE.0)THEN
       CALL IMPORT_ERROR('Can not open file: '//TRIM(DATAF(I)),IOS,IU)
       RETURN      
      ENDIF
      EXIT
     ENDIF
