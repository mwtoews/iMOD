...
!#modules
USE WINTERACTER
USE MOD_UTL, ONLY : UTL_GETUNIT,ITOS,RTOS,UTL_CAP,UTL_CREATEDIR,UTL_JDATETOIDATE,UTL_CLOSEUNITS
USE MOD_IMPORT_PAR
USE MOD_IMPORT_UTL
  ...
  DO I=1,NSTP(IPER)
   ...
   IF(NPER.GT.1)THEN
    ...
    IF(NSTP(IPER).EQ.1)THEN
     SDATE=TPER(IPER-1)
     LINE=TRIM(ITOS(IPER))//','//TRIM(RTOS(DT,'F',3))//','//TRIM(ITOS(UTL_JDATETOIDATE(TPER(IPER-1))))//','//TRIM(ITOS(1))
    ELSE
...
!     CALL ITIMETOGTIME(ITIME,IH,IM,IS)
     WRITE(LINE,'(I8,3I2.2)') UTL_JDATETOIDATE(INT(TT)),IH,IM,IS
   ...
   IF(IBATCH.EQ.0)THEN
    ...
    IF(NPER.GT.1)CALL WINDOWOUTSTATUSBAR(1,'Busy importing transient period '//TRIM(ITOS(IPER))//' date: '// &
               TRIM(ITOS(UTL_JDATETOIDATE(TPER(IPER-1)))))
   ELSE
    IF(NPER.EQ.1)WRITE(*,'(A)') 'Busy importing steady-state period'
    IF(NPER.GT.1)WRITE(*,'(A,I5,A,I10)') 'Busy importing transient period ',IPER,' date: ',UTL_JDATETOIDATE(TPER(IPER-1))
   ENDIF
   WRITE(IURUN,'(A)') TRIM(LINE)
   IF(I.EQ.1)THEN
...
!  IDF%X=FL*IDF%X
 ...
 DO ILAY=1,NLAY
  LINE=TRIM(DIR_DBS)//'\CONSTANTHEAD\VERSION_1\CHD_STAGE_'//TRIM(ITOS(UTL_JDATETOIDATE(TPER(0))))//'_L'//TRIM(ITOS(ILAY))//'.IDF'
...
! DEALLOCATE(BAS)
 ...
 LINE=TRIM(DIR_DBS)//'\WELLS\VERSION_1\WELLS_'//TRIM(ITOS(UTL_JDATETOIDATE(SDATE)))//'_L'
...
!  WRITE(IURUN,'(A)') TRIM(LINE)
 ...
 LINE=TRIM(DIR_DBS)//'\RECHARGE\VERSION_1\RECHARGE_'//TRIM(ITOS(UTL_JDATETOIDATE(SDATE)))//'_L1.IDF'
! IDF%X=IDF%X*1000.0
 DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
...
!  LINE=TRIM(DIR_DBS)//'\RECHARGE\VERSION_1\RECHARGE_ILAY_IPER'//TRIM(ITOS(IPER))//'.IDF'
  LINE=TRIM(DIR_DBS)//'\RECHARGE\VERSION_1\RECHARGE_ILAY_'//TRIM(ITOS(UTL_JDATETOIDATE(SDATE)))//'.IDF'
  IF(.NOT.IMPORT_WRT1IDF(LINE,IDF))RETURN
 ENDIF
...
!  LINE=TRIM(DIR_DBS)//'\EVAPOTRANSPIRATION\VERSION_1\EVT_SURFACE_IPER'//TRIM(ITOS(IPER))//'.IDF'
  LINE=TRIM(DIR_DBS)//'\EVAPOTRANSPIRATION\VERSION_1\EVT_SURFACE_'//TRIM(ITOS(UTL_JDATETOIDATE(SDATE)))//'.IDF'
  IF(.NOT.IMPORT_WRT1IDF(LINE,IDF))RETURN
  IEVTPER(1)=IEVTPER(1)+1
 ENDIF
 IF(INEVTR.GE.0)THEN
  IF(.NOT.IMPORT_READU2DREL(IU(IEVT),IDF%X,IDF%NCOL,IDF%NROW,'EVAPOTRANSPIRATION'))RETURN
  LINE=TRIM(DIR_DBS)//'\EVAPOTRANSPIRATION\VERSION_1\EVT_RATE_'//TRIM(ITOS(UTL_JDATETOIDATE(SDATE)))//'.IDF'
!  IDF%X=IDF%X*1000.0
  DO IROW=1,IDF%NROW; DO ICOL=1,IDF%NCOL
   IF(IDF%X(ICOL,IROW).NE.IDF%NODATA)IDF%X(ICOL,IROW)=FL/FT*IDF%X(ICOL,IROW)*1000.0
  ENDDO; ENDDO
 ...
 IF(INEXDP.GE.0)THEN
  IF(.NOT.IMPORT_READU2DREL(IU(IEVT),IDF%X,IDF%NCOL,IDF%NROW,'EVAPOTRANSPIRATION'))RETURN
  LINE=TRIM(DIR_DBS)//'\EVAPOTRANSPIRATION\VERSION_1\EVT_DEPTH_'//TRIM(ITOS(UTL_JDATETOIDATE(SDATE)))//'.IDF'
  IF(.NOT.IMPORT_WRT1IDF(LINE,IDF))RETURN
  IEVTPER(3)=IEVTPER(3)+1
 ENDIF
 IF(INIEVT.GE.0.AND.NEVTOP.EQ.2)THEN
  IF(.NOT.IMPORT_READU2DREL(IU(IEVT),IDF%X,IDF%NCOL,IDF%NROW,'EVAPOTRANSPIRATION'))RETURN
  LINE=TRIM(DIR_DBS)//'\EVAPOTRANSPIRATION\VERSION_1\EVT_IEVT_'//TRIM(ITOS(UTL_JDATETOIDATE(SDATE)))//'.IDF'
  IF(.NOT.IMPORT_WRT1IDF(LINE,IDF))RETURN
