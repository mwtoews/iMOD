...
CONTAINS
 ...
 INTEGER :: ITYPE,I,J,ITIME,IH,IM,IS,KPER
  ...
  DO I=1,NSTP(IPER)
   ...
   IF(NPER.GT.1)THEN
    ...
    IF(NSTP(IPER).EQ.1)THEN
     SDATE=TPER(IPER-1)
     LINE=TRIM(ITOS(KPER))//','//TRIM(RTOS(DT,'F',3))//','//TRIM(ITOS(UTL_JDATETOIDATE(TPER(IPER-1))))//','//TRIM(ITOS(1))
    ELSE
     !## get number of hours,minutes,seconds
     FTIME=TDT-INT(TDT); CALL FTIMETOITIME(FTIME,IH,IM,IS)
     TT=TPER(IPER-1)+INT(TDT)
!     WRITE(*,*) DT,TDT,FTIME,IH,IM,IS
     WRITE(LINE,'(I8,3I2.2)') UTL_JDATETOIDATE(INT(TT)),IH,IM,IS
     LINE=TRIM(ITOS(KPER))//','//TRIM(RTOS(DT,'G',7))//','//TRIM(LINE)//','//TRIM(ITOS(1))
    ENDIF
   ELSE
    LINE=TRIM(ITOS(KPER))//',0.0,STEADY-STATE,1'
   ENDIF
   IF(IBATCH.EQ.0)THEN
