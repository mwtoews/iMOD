...
function mf_init_no_sub_pest_moz(config_file_name, endOfSimulation, startTime,stsOverRule) result(ret_val)
!DEC$ ATTRIBUTES DLLEXPORT :: mf_init_no_sub_pest_moz
 use mf_module
 implicit none
 integer              :: ret_val
 character(len=*)     :: config_file_name
 logical, intent(out) :: endOfSimulation
 logical, intent(in) :: stsOverRule
 double precision, intent(in) :: startTime
 logical              :: dummyConvergedMozart
 integer :: dummynnsub
 
 call mf_init(config_file_name,dummynnsub,stsOverRule)
 if (exitcode == 0) then
     endOfSimulation   = .false.
     call mf_init_simulation(endOfSimulation,dummyConvergedMozart,startTime)
 endif
 ret_val = exitcode

end function  mf_init_no_sub_pest_moz

subroutine mf_update_no_sub_pest_moz(endOfSimulation, doTimeLoop, stssaveOverRule)
!DEC$ ATTRIBUTES DLLEXPORT :: mf_update_no_sub_pest_moz
 implicit none
 logical, intent(inout) :: endOfSimulation
 logical, intent(inout) :: doTimeLoop
 logical, intent(in) :: stssaveOverRule
 
 logical :: convergedIter
 call mf_init_iter(stssaveOverRule)
 convergedIter = .false.
 do while(.not.convergedIter) ! Picard iter loop
    call mf_iter(convergedIter)    
 end do
 call mf_finish_timestep(convergedIter,endOfSimulation,doTimeLoop)
...
!DEC$ ATTRIBUTES DLLEXPORT :: mf_finish_no_sub_pest_moz
 ...
 call mf_finish_pest(dummyConvergedPest)
...
!DEC$ ATTRIBUTES DLLEXPORT :: mf_get_heads
  ...
  ok = mf2005_PutHeadsForLayer(head,ncol,nrow,ilay,igrid)
