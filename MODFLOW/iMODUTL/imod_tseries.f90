...
! CALL IMOD_UTL_OPENASC(IUIPFTXT,LINE,'W')
 ...
 DO JJ=1,ABS(IIPF)
  ...
  OPEN(TS(JJ)%IUIPF,FILE=LINE,STATUS='UNKNOWN',ACTION='WRITE',FORM='FORMATTED')
!  CALL IMOD_UTL_OPENASC(TS(JJ)%IUIPF,LINE,'W')
...
ELSE
 ...
 ELSE
  ...
  CALL IMOD_UTL_SWAPSLASH(LINE)
  IU=IMOD_UTL_GETUNIT()
  OPEN(IU,FILE=LINE,ACTION='READ',STATUS='OLD',FORM='FORMATTED')
  ...
  DO J=1,ABS(IIPF)
   DO I=1,TS(J)%NROWIPF
    ...
    IF(TS(J)%IEXT.GT.0)THEN
     ...
     ENDIF
     OPEN(JUTXT(JJ),FILE=LINE,FORM='FORMATTED',ACTION='READ',STATUS='OLD',IOSTAT=IOS) 
