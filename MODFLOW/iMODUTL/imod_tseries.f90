...
!####============================================================
 ...
 TYPE STOBJ
  LOGICAL :: VALID = .TRUE.
  REAL(KIND=8) :: X,Y,W,M,C,D ! X,Y,W(EIGHT),M(EASURED),C(COMPUTED),D(DIFFERENCE - ACCEPTABLE)
  INTEGER :: ILAY
  INTEGER :: ICOL = 0
  INTEGER :: IROW = 0
  CHARACTER(LEN=52) :: ID
...
DO JJ=1,ABS(IIPF)
 ...
 IF(TS(JJ)%IDCOL.GT.TS(JJ)%NCOLIPF)TS(JJ)%IDCOL=0
 ...
 IF(TS(JJ)%IDCOL.NE.0)CALL IMOD_UTL_PRINTTEXT('Acceptable Diff.: '//TRIM(ATTRIB(ABS(TS(JJ)%IDCOL))),0)
 DEALLOCATE(ATTRIB)
 ...
 DO I=1,TS(JJ)%NROWIPF
  ...
  IF(TS(JJ)%IVCOL.NE.0)THEN
   ...
   IF(IOS.NE.0)CALL IMOD_UTL_PRINTTEXT('Missing proper column expressing the WEIGHING in timeserie IPF (record:'// &
       TRIM(IMOD_UTL_ITOS(I))//')',2)
  ENDIF
  TS(JJ)%STVALUE(III)%D=0.0
  IF(TS(JJ)%IDCOL.GT.0)READ(IPFSTRING(TS(JJ)%IDCOL),*,IOSTAT=IOS) TS(JJ)%STVALUE(III)%D
  IF(IOS.NE.0)CALL IMOD_UTL_PRINTTEXT('Missing proper column expressing the LAYER identification in timeserie IPF (record:'// &
     TRIM(IMOD_UTL_ITOS(I))//')',2)
...
!  CALL IMOD_UTL_PRINTTEXT('          >>> NO locations will be used that are WITHIN the buffer-zone <<<',0)
  ...
  WRITE(TS(JJ)%IUIPF,'(A)') 'VARIANCE'
  IF(TS(JJ)%IDCOL.GT.0)WRITE(TS(JJ)%IUIPF,'(A)') 'ACCEPTABLE_DIFFERENCE'
  ...
  DO I=1,TS(JJ)%NROWIPF
   ...
   IF(TS(JJ)%IDCOL.GT.0)BIGLINE=TRIM(BIGLINE)//','//TRIM(IMOD_UTL_DTOS(TS(JJ)%STVALUE(I)%D,'G',7))
...
REAL(KIND=8) :: DH1,DH2,H,X,Y,W,Z,M,HH,WW,XXX
   ...
   DO JJ=1,ABS(IIPF)
    DO I=1,TS(JJ)%NROWIPF
     ...
     IF(TS(JJ)%IEXT.GT.0)THEN
      IF(TSDATE(II).EQ.DBLDATE)THEN 
       M=TSM(II)
      !## try to read next date
      ELSEIF(TSDATE(II).LT.DBLDATE)THEN 
       ...
       ENDDO
       M=TSM(II)
       IF(TSDATE(II).NE.DBLDATE)M=TSNODATA(II) 
      ELSE
       M=TSNODATA(II)
      ENDIF
     ENDIF
