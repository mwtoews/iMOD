...
CONTAINS
 ...
 !###===============================================================================
 INTEGER FUNCTION UTL_GETRECORDLENGTH(FNAME,TSTATUS,TFORM,TACT)
 !###===============================================================================
 IMPLICIT NONE
 CHARACTER(LEN=*),INTENT(IN) :: FNAME,TFORM,TACT,TSTATUS
 ...
 !## open file
 IU=IMOD_UTL_GETUNIT()
 OPEN(IU,FILE=FNAME,STATUS=TSTATUS,FORM=TFORM,ACTION=TACT,ACCESS='STREAM',IOSTAT=IOS)
 IF(IOS.NE.0)RETURN
...
!###======================================================================
 ...
 REAL(KIND=8),INTENT(IN) :: X,Y,RANGE,SILL,NUGGET,MZ,NODATA
 REAL(KIND=8),INTENT(IN),DIMENSION(ND) :: XD,YD,ZD 
 TYPE(IDFOBJ),INTENT(IN) :: BLNKOUT
 INTEGER :: I,J,IC,IR,NP,N,ID,JD,IROW,ICOL
 REAL(KIND=8) :: DX,DY,DXY,GAMMA,H,MZZ,F
...
!###====================================================
 ...
 IF(XMIN.GT.DELR(0).OR.XMAX.LT.DELR(NCOL).OR.YMIN.GT.DELC(NROW).OR.YMAX.LT.DELC(0))THEN
  ...
  IF(XMIN.GT.DELR(0))THEN
   CALL IMOD_UTL_PRINTTEXT('XMIN IDF '//TRIM(IMOD_UTL_DTOS(XMIN,'F',2))//' > XMIN MODEL '//TRIM(IMOD_UTL_DTOS(DELR(0),'F',2)),0)
  ENDIF
  IF(XMAX.LT.DELR(NCOL))THEN
   CALL IMOD_UTL_PRINTTEXT('XMAX IDF '//TRIM(IMOD_UTL_DTOS(XMAX,'F',2))//' < XMAX MODEL '//TRIM(IMOD_UTL_DTOS(DELR(NCOL),'F',2)),0)
  ENDIF
  IF(YMIN.GT.DELC(NROW))THEN
   CALL IMOD_UTL_PRINTTEXT('YMIN IDF '//TRIM(IMOD_UTL_DTOS(YMIN,'F',2))//' > YMIN MODEL '//TRIM(IMOD_UTL_DTOS(DELC(NROW),'F',2)),0)
  ENDIF
  IF(YMAX.LT.DELC(0))THEN
   CALL IMOD_UTL_PRINTTEXT('YMAX IDF '//TRIM(IMOD_UTL_DTOS(YMAX,'F',2))//' < YMAX MODEL '//TRIM(IMOD_UTL_DTOS(DELC(0),'F',2)),0)
  ENDIF
 ! CALL PRINTTEXT('---------------------------------------',0)
