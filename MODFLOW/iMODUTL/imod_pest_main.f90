...
!   DH(PEST_IGRAD,:)=DH(0,:)
 ...
 REAL(KIND=8),ALLOCATABLE,DIMENSION(:) :: EIGW
...
!  IF(ABS(PARAM(IP1)%MAX)-PARAM(IP1)%ALPHA(1).LT.XPBND)PARAM(IP1)%IBND= 1 !## max
  ...
  IF(ABS(PARAM(IP1)%IACT).EQ.1.AND.PARAM(IP1)%IGROUP.GT.0)THEN
   WRITE(IUPESTOUT,'(3X,A2,2I5.5,A1,I3.3,A)') PARAM(IP1)%PTYPE,PARAM(IP1)%ILS,PARAM(IP1)%IZONE,'-',ABS(PARAM(IP1)%IGROUP),TRIM(LINE)
  ENDIF
