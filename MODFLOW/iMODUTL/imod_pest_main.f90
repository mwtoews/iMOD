...
! endif
 ...
 IF(IOPTION.EQ.0)THEN
  NCOL=IDF%NCOL; NROW=IDF%NROW
 ELSE
  NCOL=IDFM%NCOL; NROW=IDFM%NROW
 ENDIF
 
 LREUSEDAT=PEST1INIT_READ_ZONES_OPENFILE(PDIR,IUDUMP,NCOL,NROW)

 !## create zones and dump-file
 ...
 IF(.NOT.LREUSEDAT)THEN
  ...
  !## write header
  WRITE(IUDUMP,'(/A)')       'PARAMETER-DUMP-FILE'
  WRITE(IUDUMP,'(A22,I10)')  'NUMBER-OF-COLUMNS:    ',NCOL
  WRITE(IUDUMP,'(A22,I10)')  'NUMBER-OF-ROWS:       ',NROW
  WRITE(IUDUMP,'(A22,I10)')  'NUMBER-OF-PARAMETERS: ',SIZE(PARAM)
  
  DO I=1,SIZE(PARAM)
   WRITE(IUDUMP,'(/3A10)') 'NODES','PAR.-TYPE','PARAMETER'  
   WRITE(IUDUMP,*) PARAM(I)%NODES,PARAM(I)%ZTYPE
   IF(PARAM(I)%ZTYPE.EQ.0)THEN
    WRITE(IUDUMP,'(3A10)') 'IROW','ICOL','FACTOR'
    DO J=1,PARAM(I)%NODES
     WRITE(IUDUMP,*) PARAM(I)%IROW(J),PARAM(I)%ICOL(J),PARAM(I)%F(J)
    ENDDO
   ELSE
    WRITE(IUDUMP,'(3A10)') 'X-CORD','Y-CRD','FACTOR'   
    DO J=1,PARAM(I)%NODES
     WRITE(IUDUMP,*) PARAM(I)%XY(J,1),PARAM(I)%XY(J,2)
    ENDDO
   ENDIF
  ENDDO
  CLOSE(IUDUMP)  
 ...
 ELSE
  ...
  DO I=1,SIZE(PARAM)
   ...
   ELSE
    ALLOCATE(PARAM(I)%XY(PARAM(I)%NODES,2))
    DO J=1,PARAM(I)%NODES
     READ(IUDUMP,*) PARAM(I)%XY(J,1),PARAM(I)%XY(J,2)
    ENDDO
   ENDIF
   IF(PARAM(I)%PTYPE.EQ.'HF')THEN
    PARAM(I)%NODES=0 !## one single cell used as zone for horizontal barrier module
 ...
 !###====================================================================
 LOGICAL FUNCTION PEST1INIT_READ_ZONES_OPENFILE(DIR,JU,NCOL,NROW)
 !###====================================================================
 IMPLICIT NONE
 ...
 READ(JU,*)
 READ(JU,*)
 READ(JU,'(22X,I10)',IOSTAT=IOS) NC
 READ(JU,'(22X,I10)',IOSTAT=IOS) NR
 READ(JU,'(22X,I10)',IOSTAT=IOS) N
 IF(IOS.NE.0)THEN
  CLOSE(JU); WRITE(*,'(/A)') 'OOPS, AN OLD VERSION OF A PARAM_DUMP_IPEST FOUND; FILE IS RECREATED'; RETURN
 ENDIF
 IF(N.NE.SIZE(PARAM))THEN
  CLOSE(JU); WRITE(*,'(/A)') 'NUMBER OF ZONES IN THE PARAM_DUMP_IPEST DOES NOT COINCIDE WITH THE ACTUAL NUMBER OF PARAMETERS; FILE IS RECREATED'; RETURN
 ENDIF
 IF(NC.NE.NCOL)THEN
  CLOSE(JU); WRITE(*,'(/A)') 'NUMBER OF COLUMNS IN THE PARAM_DUMP_IPEST DOES NOT COINCIDE WITH THE ACTUAL NUMBER OF PARAMETERS; FILE IS RECREATED'; RETURN
 ENDIF
 IF(NR.NE.NROW)THEN
  CLOSE(JU); WRITE(*,'(/A)') 'NUMBER OF ROWS IN THE PARAM_DUMP_IPEST DOES NOT COINCIDE WITH THE ACTUAL NUMBER OF PARAMETERS; FILE IS RECREATED'; RETURN
 ENDIF
 
 PEST1INIT_READ_ZONES_OPENFILE=.TRUE.
 
 END FUNCTION  PEST1INIT_READ_ZONES_OPENFILE
