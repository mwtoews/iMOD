  ...
  DO IP1=1,SIZE(PARAM)
   ...
   IF(F.LT.1.0D0/PARAM(IP1)%FADJ.OR.F.GT.PARAM(IP1)%FADJ)THEN
    IF(TRIM(PARAM(IP1)%ACRONYM).EQ.'')THEN
     WRITE(IUPESTOUT,'(A,G15.9)') 'Increase Lambda - adjustment for parameter '//TRIM(IMOD_UTL_ITOS(IP1))//' to small/large ',F
    ELSE
     WRITE(IUPESTOUT,'(A,G15.9)') 'Increase Lambda - adjustment for parameter '//TRIM(PARAM(IP1)%ACRONYM)//' to small/large ',F
    ENDIF
    FLUSH(IUPESTOUT)
    RETURN
   ENDIF
