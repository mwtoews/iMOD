...
!   CALL IDFCOPY(IDF,IDFM)
  ...
  ELSEIF(IOPTION.EQ.1)THEN
   CALL U2DREL(BLNKOUT%X, 'blnkout', NROW, NCOL, 0, IURUN, IOUT)
  ENDIF
 ELSE
  !## default value 
  BLNKOUT%X=1.0
...
!   NDAMPING=ININDAMPING !## do it onces only
   ...
   IF(IMPROVEMENT.LE.PEST_JSTOP)THEN
    ...
    CALL IMOD_UTL_PRINTTEXT('',-1,IUPESTOUT); CALL IMOD_UTL_PRINTTEXT('Pest iteration terminated decrease objective function ('// &
        TRIM(IMOD_UTL_DTOS(100.0*IMPROVEMENT,'G',7))//'%) > PEST_JSTOP ('//TRIM(IMOD_UTL_DTOS(100.0*PEST_JSTOP,'G',7))//'%)',-1,IUPESTOUT)
   ENDIF
...
!PAUSE
 ...
 WRITE(BLINE,'(19X,99F10.5)') (GRADUPDATE(I),I=PEST_ITER,1,-1)
 WRITE(IUPESTOUT,'(A)') TRIM(BLINE)
...
! ENDDO
 ...
 REAL(KIND=8),ALLOCATABLE,DIMENSION(:) :: TSNODATA,M,C,GF_H,GF_O
  ...
  ELSE
   ...
   DO J=1,TS(I)%NROWIPF
    ...
    IF(KK.GT.0)THEN
     ...
     !## percentiles
     CALL IMOD_UTL_GETMED(M,KK,-999.99D0,(/10.0D0,90.0D0/),2,NAJ,PM)
     CALL IMOD_UTL_GETMED(C,KK,-999.99D0,(/10.0D0,90.0D0/),2,NAJ,PC)
...
! IF(PEST_IREGULARISATION.EQ.1)CALL PEST_GETQPP(NP,.TRUE.)
 ...
 !## average observation
 YA=0.0; DO I=1,N; YA=YA+Y(I)          ; ENDDO; YA=YA/REAL(N)
 XN=0.0; DO I=1,N; XN=XN+ABS(Y(I)-X(I)); ENDDO; XN=XN**2.0
 YN=0.0; DO I=1,N; YN=YN+ABS(Y(I)-YA)  ; ENDDO; YN=YN**2.0
 
 PEST_GOODNESS_OF_FIT=1.0-XN/YN
