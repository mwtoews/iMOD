...
CONTAINS
 ...
 IF(SCLTYPE.EQ.7.OR.SCLTYPE.EQ.9)THEN
  MXN=0; DO I=1,IDFC%NCOL; N=(IDFC%SX(I)-IDFC%SX(I-1))/IDFC%DX; MXN=MAX(MXN,N); END DO
  MXN=MXN+2
  MXM=0; DO I=1,IDFC%NROW; M=(IDFC%SY(I-1)-IDFC%SY(I))/IDFC%DY; MXM=MAX(MXM,M); END DO
  MXM=MXM+2
 ENDIF
...
!  CALL IMOD_UTL_POL1LOCATE(IDFC%SY,IDFC%NROW+1,IDFM%SY(IRM-1)-TINY,IRC1)
!  CALL IMOD_UTL_POL1LOCATE(IDFC%SY,IDFC%NROW+1,IDFM%SY(IRM  )+TINY,IRC2)
    DYM=IDFM%SY(IRM-1) -IDFM%SY(IRM)
    DYC=IDFC%SY(IRC1-1)-IDFC%SY(IRC2)

!  IF(IRC2.GE.IRC1)THEN
!   DO ICM=1,IDFM%NCOL
    !## get window to scale/cut data from IDFC
!    CALL IMOD_UTL_POL1LOCATE(IDFC%SX,IDFC%NCOL+1,IDFM%SX(ICM-1)+TINY,ICC1)
!    CALL IMOD_UTL_POL1LOCATE(IDFC%SX,IDFC%NCOL+1,IDFM%SX(ICM  )-TINY,ICC2)
!    IF(ICC2.GE.ICC1)THEN
