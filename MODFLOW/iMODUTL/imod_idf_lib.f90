...
CONTAINS
 ...
 !CALL IDFSMOOTH(IDFM%X,IDFM%XMIN,IDFM%YMAX,IDFM%DX,4,IDFM%NODATA)
 ...
 !###====================================================================
 SUBROUTINE IDFGETBLOCKVALUE(IDF,SCLTYPE,IR1,IR2,IC1,IC2,FREQ,SFCT,SVALUE,IDOWN)
 !###====================================================================
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 ...
 IF (IDOWN.EQ.1) THEN
  IF (IR1.NE.IR2) CALL IMOD_UTL_PRINTTEXT('Program error subroutine IDFGETBLOCKVALUE: row',2)
  IF (IC1.NE.IC2) CALL IMOD_UTL_PRINTTEXT('Program error subroutine IDFGETBLOCKVALUE: column',2)
...
!!###====================================================================
!SUBROUTINE IDFSMOOTH(X,XMINIDF,YMAXIDF,DXIDF,IINT,NODATA)
!!###====================================================================
!IMPLICIT NONE
!USE MOD_UTL, ONLY : PRINTTEXT
!USE BASVAR, ONLY : NCOL,NROW,DELR,DELC
!IMPLICIT NONE
!REAL,INTENT(IN) :: XMINIDF,YMAXIDF,DXIDF,NODATA
!REAL,DIMENSION(NCOL,NROW),INTENT(INOUT) :: X
!INTEGER :: IC,IR,I,J,IROW,ICOL,NPC,NPR,IY,II,JJ
!REAL :: XC,XX,YY,Y,YC,XCIDF,YCIDF,XMIN,XMAX,YMIN,YMAX
!REAL,ALLOCATABLE,DIMENSION(:) :: X1A,X2A
!REAL,ALLOCATABLE,DIMENSION(:,:) :: Y2A,XCOPY
!
!CALL PRINTTEXT('Applying Four-point Smoothing ',0)
!
!IF(ALLOCATED(X1A)) DEALLOCATE(X1A); IF(ALLOCATED(X2A))DEALLOCATE(X2A)
!IF(ALLOCATED(Y2A))DEALLOCATE(Y2A);  IF(ALLOCATED(XCOPY))DEALLOCATE(XCOPY)
! 
!XMIN=DELR(0); XMAX=DELR(NCOL); YMIN=DELC(NROW); YMAX=DELC(0)
!
...
!
!!## assign one extra row/column for boundary
!ALLOCATE(X1A(NPC),X2A(NPR),Y2A(NPC,NPR),XCOPY(NCOL,NROW))
!
...
! NPR=NPR+1
!
! CALL POL1LOCATE(DELC,NROW+1,YC,IR)
!
! IR=MIN(NROW,IR)
! IR=MAX(1,IR)
!
...
!  NPC=NPC+1
!
!  CALL POL1LOCATE(DELR,NCOL+1,XC,IC)
!
!  IC=MIN(NCOL,IC)
!  IC=MAX(1,IC)
!
...
!DO I=1,NPC
! DO J=1,NPR
!  IF(Y2A(I,J).EQ.NODATA)THEN
!   IY=0
!   Y =0.0
!   DO II=MAX(1,I-1),MIN(NPC,I+1)
!    DO JJ=MAX(1,J-1),MIN(NPR,J+1)
!     IF(Y2A(II,JJ).NE.NODATA)THEN
!      Y =Y+Y2A(II,JJ)
!      IY=IY+1
!     ENDIF
!    END DO
...
!DO I=1,NCOL
! DO J=1,NROW
!  IF(XCOPY(I,J).EQ.NODATA)X(I,J)=NODATA
! ENDDO
!ENDDO
!
!IF(ALLOCATED(X1A))  DEALLOCATE(X1A)
