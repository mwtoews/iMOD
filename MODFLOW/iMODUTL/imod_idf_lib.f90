...
!  INQUIRE(UNIT=IDFC%IU,NAME=IDFNAME)
  ...
  IF(IDFC%XMIN.GT.IDFM%XMIN)THEN
   CALL IMOD_UTL_PRINTTEXT('XMIN IDF '//TRIM(IMOD_UTL_DTOS(IDFC%XMIN,'F',2))//' > XMIN MODEL '//TRIM(IMOD_UTL_DTOS(IDFM%XMIN,'F',2)),0)
  ENDIF
  IF(IDFC%XMAX.LT.IDFM%XMAX)THEN
   CALL IMOD_UTL_PRINTTEXT('XMAX IDF '//TRIM(IMOD_UTL_DTOS(IDFC%XMAX,'F',2))//' < XMAX MODEL '//TRIM(IMOD_UTL_DTOS(IDFM%XMAX,'F',2)),0)
  ENDIF
  IF(IDFC%YMIN.GT.IDFM%YMIN)THEN
   CALL IMOD_UTL_PRINTTEXT('YMIN IDF '//TRIM(IMOD_UTL_DTOS(IDFC%YMIN,'F',2))//' > YMIN MODEL '//TRIM(IMOD_UTL_DTOS(IDFM%YMIN,'F',2)),0)
  ENDIF
  IF(IDFC%YMAX.LT.IDFM%YMAX)THEN
   CALL IMOD_UTL_PRINTTEXT('YMAX IDF '//TRIM(IMOD_UTL_DTOS(IDFC%YMAX,'F',2))//' < YMAX MODEL '//TRIM(IMOD_UTL_DTOS(IDFM%YMAX,'F',2)),0)
  ENDIF
  CALL IMOD_UTL_PRINTTEXT('=======================================',0)
...
! store file name of output IDFs
 ...
 !## open idf
 IF(.NOT.IDFOPEN(IDF%IU,IDFNAME,IDF%ITYPE,'WO',1,0))RETURN
...
!  IREC=IREC+1
 ...
 IF(IDF%IXV.EQ.0.AND.ASSOCIATED(IDF%X))THEN
  IDF%DMIN= REAL(10.0E10,8); IDF%DMAX=-REAL(10.0E10,8)
...
! IF(ICF.EQ.1)RECLEN=1 !## words
 ...
 IF(TSTAT(1:1).EQ.'R')THEN
  ...
  !## get idftype 0/1
  IDFTYPE=UTL_GETRECORDLENGTH(IDFNAME,'OLD','UNFORMATTED','READ,DENYWRITE')
  RECLEN=IDFTYPE/4
