...
CONTAINS
 ...
 REAL,ALLOCATABLE,DIMENSION(:) :: FREQ,FFREQ
...
!     WRITE(*,*) FLAG2,FLAG1,IRM,ICM,idfm%x(icm,irm),idfc%nodata
...
!      write(*,*) 'icm,irm,value=',icm,irm,svalue
 ...
 IMPLICIT NONE
 TYPE(IDFOBJ),INTENT(INOUT) :: IDF
 REAL,INTENT(OUT),DIMENSION(:) :: FREQ,FFREQ
 REAL,INTENT(IN) :: SFCT
...
!   IC=IC+1
 ...
 REAL :: XC,XX,Y,YY,YC,XCIDF,YCIDF,XMID,YMID,TINY
...
! IF(ALLOCATED(XCOPY))DEALLOCATE(XCOPY)
 ...
 ENDIF
 CALL UTL_IDFIROWICOL(IDF2,IR1,IC1,X,Y)!IDF1%XMIN,IDF1%YMAX)
 IF(IDF1%IEQ.EQ.0)THEN
 ...
 ENDIF
 CALL UTL_IDFIROWICOL(IDF2,IR2,IC2,X,Y)!IDF1%XMAX,IDF1%YMIN)
