...
USE MOD_RF2MF, ONLY : IIDEBUG,IURUN,IACT,RESULTDIR,SIMBOX,SIMCSIZE,PCAP,MDIM,IDFTINY,IARMWP, &
                   ISAVE,CDATE,MMOD,PCAP,PPWT,LINE,NLINES,ISCEN,SIMCSIZE,USEBOX
...
! WRITE(JU,'(A)') '      sw_dtgw_unf            =      0    ! File  with dtgw-info Surfw for water quality'
 ...
 READ(IU,*) TXT,A_NCOL
 READ(IU,*) TXT,A_NROW
 READ(IU,*) TXT,A_XLLC; IX=0.0; IF(TXT.EQ.'XLLCENTER')IX=1.0
 READ(IU,*) TXT,A_YLLC; IY=0.0; IF(TXT.EQ.'YLLCENTER')IY=1.0
