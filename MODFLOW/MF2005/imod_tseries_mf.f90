...
INTEGER :: I,J,IR,IC
REAL,INTENT(IN) :: X,Y
REAL,DIMENSION(3) :: XCRD,YCRD
REAL,DIMENSION(3,3) :: ZCRD
REAL,DIMENSION(1,1) :: XINT

XCRD=0.0; YCRD=0.0; ZCRD=HNOFLO

J=0
DO IR=IROW-1,IROW+1
 J=J+1; I=0
 IF(IR.LT.1)THEN
  YCRD(J)=CDELR(IR)+(CDELC(IR)-CDELC(IR+1))/2.0
 ELSEIF(IR.GT.NROW)THEN
  YCRD(J)=CDELC(IR-1)-(CDELC(IR-2)-CDELC(IR-1))/2.0
 ELSE
 ...
 DO IC=ICOL-1,ICOL+1
  I=I+1
  IF(IC.LT.1)THEN
   XCRD(I)=CDELR(IC)-(CDELR(IC+1)-CDELR(IC))/2.0
  ELSEIF(IC.GT.NCOL)THEN
   XCRD(I)=CDELR(IC-1)+(CDELR(IC-2)-CDELR(IC-1))/2.0
  ELSE
   XCRD(I)=(CDELR(IC)+CDELR(IC-1))/2.0
  ENDIF
