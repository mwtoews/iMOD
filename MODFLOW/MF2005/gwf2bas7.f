...
C  this file to use the IMPLICIT NONE statement.
      ...
      MODULE GWFBASMODULE
        ...
        REAL,    SAVE, POINTER  ::DELT,PERTIM,TOTIM,STOPER
        REAL,SAVE, POINTER :: HNOFLO,HDRY
      ...
      TYPE GWFBASTYPE
        ...
        REAL,    POINTER  ::DELT,PERTIM,TOTIM,STOPER
        REAL,POINTER  :: HNOFLO,HDRY
  ...
  162    FORMAT(' SSFLAG MUST BE EITHER "SS" OR "TR"',
     1      ' -- STOP EXECUTION (SGWF2BAS7ARDIS)')
      ...
      IF(ISSFLG(N).EQ.0)THEN
       WRITE (IOUT,163) N,PERLEN(N),NSTP(N),TSMULT(N),'TR',
     1LINE(ISTART:ISTOP)
      ELSE
       WRITE (IOUT,163) N,PERLEN(N),NSTP(N),TSMULT(N),'SS',
     1LINE(ISTART:ISTOP)
      ENDIF
...
C       DEALLOCATE(GLOBALDAT(IGRID)%ichloride)
C       DEALLOCATE(GLOBALDAT(IGRID)%con)
C
...
C
        ...
        NPERTXT=>GLOBALDAT(IGRID)%NPERTXT
...
C
        ...
        GLOBALDAT(IGRID)%NPERTXT=>NPERTXT
