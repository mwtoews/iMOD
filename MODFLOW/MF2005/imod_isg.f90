...
IF(iact.gt.0)THEN
 ...
 DO I=1,NISG
  ...
  IDF(9) %X(ICOL,IROW)=IDF(9) %X(ICOL,IROW)+(ISGLIST(I,8)*ISGLIST(I,5))  !## length
 ...
 DO IROW=1,NROW; DO ICOL=1,NCOL
  IF(IDF(1)%X(ICOL,IROW).GT.0.0)THEN
   ...
   IDF(9) %X(ICOL,IROW)=IDF(9) %X(ICOL,IROW)/IDF(1)%X(ICOL,IROW)
...
! END DO
 ...
 DO I=2,ABS(DWP(J)%NCROS)
  ...
  IF(ICHK.EQ.1)PIDF%X(ICOL,IROW)=CROS(J,I)%ZP
 ENDDO 
  ...
  DO IROW=IR1,IR2; DO ICOL=IC1,IC2
   ...
   IF(JROW.NE.0.AND.JCOL.NE.0)THEN
    ...
    IF(IDF(6)%X(ICOL,IROW).EQ.0.0)THEN
     IDF(1)%X(ICOL,IROW)=IDFGETAREA(IDF(1),ICOL,IROW)/C  
    ELSE
     IDF(1)%X(ICOL,IROW)=IDF(1)%X(ICOL,IROW)+IDFGETAREA(IDF(1),ICOL,IROW)/C  
    ENDIF
