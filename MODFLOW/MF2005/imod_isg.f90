...
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(4),LINE,'R',44)
!## open *.ISC1 file
LINE=FNAME(:I)//'.ISC1'
CALL IMOD_UTL_FILENAME(LINE)
...
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(8),LINE,'R',28)
  ...
  DO I=1,NISGH
   ...
   DO J=1,NCRS
    ...
    DO ICROS=1,ABS(DWP(J)%NCROS)
     ...
     ELSE
      IF(ISGTYPE.EQ.4)THEN
       READ(ISGIU(6),REC=JREC+ICF) DS,BS,ZM,ZC,ZP
       CROS(J,ICROS)%DIST=  REAL(DS,8)
       CROS(J,ICROS)%BOTTOM=REAL(BS,8)
      ELSE
...
IF(NR.GT.0)THEN
 ...
 DO 
  ...
  IF(ITYPE.EQ. 1)THEN
   ...
   ELSE
    READ(LU,REC=IREC+ICF) IDATE,CTIME,(RVAL(I),I=1,MAXITEMS)
   ENDIF
  ENDIF
  IF(ITYPE.EQ.-1)THEN
   IF(ISGTYPE.EQ.4)THEN
   ...
   ELSE
    READ(LU,REC=IREC+ICF) IDATE,CTIME,(RVAL(I),I=1,2)
   ENDIF
  ENDIF
...
REAL(KIND=4),DIMENSION(NISG,11),INTENT(INOUT) :: ISGLIST
REAL(KIND=4) ::  LEKCONDUCTANCE_Y
INTEGER :: I, ILAY, IROW, ICOL
REAL(KIND=4) ::  DXY,LI,BIN,C0,C1,KH,KV,COND_IN,FCT,D
REAL(KIND=4),PARAMETER :: TINY=1.0D-20
LOGICAL :: LUSED ! PKS
...
DO I = 1, NISG
 ...
 IF(C1.GT.0.0)THEN
  ...
  IF(KH.LE.0.0D0)CALL IMOD_UTL_PRINTTEXT('Error KH='//TRIM(IMOD_UTL_RTOS(KH,'F',7)),2)
  IF(D .LE.0.0D0)CALL IMOD_UTL_PRINTTEXT('Error D ='//TRIM(IMOD_UTL_RTOS(D ,'F',7)),2)
