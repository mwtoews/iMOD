...
! end of program
      ...
      use gwfmetmodule, only: coord_xll, coord_yll, coord_xur, coord_yur,&
                              ...
                              time_cstring, iss, ieq, cdelr, cdelc
...
! arguments
      real, intent(inout) :: im_delt
      real(kind=8), intent(inout) :: im_simcsize
      integer, intent(inout) :: im_iss
      character(len=20), intent(inout) :: im_cdate
      real(KIND=8), dimension(4), intent(inout) :: im_simbox
      real(KIND=8), dimension(0:im_ncol), intent(inout) :: im_delr
      real(KIND=8), dimension(0:im_nrow), intent(inout) :: im_delc
...
! check if grid is uniform
      ...
      if (lqd) then
       ...
       if(nrproc.gt.1)then
         write(*,*) 'Error: non-uniform grids not yet supported icm PKS.'
         call ustop(' ')
       endif
      end if
...
!###====================================================================
SUBROUTINE PCK1RPISG(NISG,FNAME,ILAY)
!###====================================================================
USE IMOD_UTL, ONLY : IMOD_UTL_IDATETOJDATE,IMOD_UTL_FILENAME,IMOD_UTL_PRINTTEXT,IMOD_UTL_ITOS, &
   ...
   XA,YA,ZA,FA,LN,CA,RA,INTERSECT_DEALLOCATE,UTL_GETRECORDLENGTH
USE MOD_GLBVAR, ONLY : LINE,ISS,CDATE,DELT,SIMCSIZE,SIMBOX,LQD
USE IMOD_UTL, ONLY : ICF,UTL_IDFIROWICOL
USE MOD_BASVAR, ONLY : DELR,DELC,NROW,NCOL
USE MOD_ISGVAR
USE IMOD_IDF_PAR
USE IMOD_IDF, ONLY : IDFNULLIFY,IDFALLOCATEX,IDFDEALLOCATEX,IDFDEALLOCATE
IMPLICIT NONE
REAL(KIND=8),PARAMETER :: MAXWIDTH=1000.0D0
INTEGER, INTENT(OUT) :: NISG
CHARACTER(LEN=*),INTENT(IN) :: FNAME
INTEGER,INTENT(IN) :: ILAY 
REAL(KIND=8),POINTER,DIMENSION(:,:) :: ISGLIST_BU
CHARACTER(LEN=32) :: CNAME
INTEGER,DIMENSION(8) :: ISGIU
INTEGER :: I,II,III,J,JJ,K,SDATE,EDATE,TTIME,MX,N,IROW,ICOL,ICRS,NCRS,IREC,ICROS,N2DIM,IC,IR,ISGTYPE
INTEGER :: MAXCROS,JCRS,NCLC,IREF,JREC,ICLC,NR,ISTW,NSTW,NS1,NS2,NN,NODE,ICSTYPE
INTEGER(KIND=2) :: ZM
INTEGER(KIND=1) :: ZC,ZP
REAL(KIND=8) ::  DXY,C,WP,BH,RWIDTH,WETPER,ISGLEN,MIND,D,FCT,H1,H2,Z,WL,INFF,ZCHK
REAL(KIND=8),ALLOCATABLE,DIMENSION(:,:) :: QSORT,RVAL
REAL(KIND=8),ALLOCATABLE,DIMENSION(:) :: DIST
REAL(KIND=8),ALLOCATABLE,DIMENSION(:) :: XNR
INTEGER,ALLOCATABLE,DIMENSION(:) :: IPOS
REAL(KIND=8) ::  NODATA=-9999.0D0
REAL(KIND=4) :: XS,YS,DS,BS,KS
LOGICAL :: LNODAT
REAL(KIND=8) ::  WATERDEPTH=0.10D0 !## minimal waterdepth
TYPE(IDFOBJ),ALLOCATABLE,DIMENSION(:) :: IDF
TYPE(IDFOBJ) :: ICROSS,PCROSS
LOGICAL :: ISGATTRIBUTES_2DCROSS_READ
INTEGER :: IOS,NA,MA
REAL(KIND=8),PARAMETER :: NODATAIDF=0.0D0 !## do not change !!!
...
LINE=FNAME(:I)//'.ISP'
CALL IMOD_UTL_FILENAME(LINE)
J=UTL_GETRECORDLENGTH(LINE) 
ISGTYPE=4; IF(J.EQ.16)ISGTYPE=8
IF(ISGTYPE.EQ.4)CALL IMOD_UTL_OPENIDF(ISGIU(2),LINE,'R',8)
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(2),LINE,'R',16)
!## open *.ISD1 file
LINE=FNAME(:I)//'.ISD1'
CALL IMOD_UTL_FILENAME(LINE)
IF(ISGTYPE.EQ.4)CALL IMOD_UTL_OPENIDF(ISGIU(3),LINE,'R',44)
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(3),LINE,'R',48)
!## open *.ISD2 file
LINE=FNAME(:I)//'.ISD2'
CALL IMOD_UTL_FILENAME(LINE)
IF(ISGTYPE.EQ.4)CALL IMOD_UTL_OPENIDF(ISGIU(4),LINE,'R',20)
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(4),LINE,'R',36)
!## open *.ISC1 file
LINE=FNAME(:I)//'.ISC1'
CALL IMOD_UTL_FILENAME(LINE)
IF(ISGTYPE.EQ.4)CALL IMOD_UTL_OPENIDF(ISGIU(5),LINE,'R',44)
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(5),LINE,'R',48)
!## open *.ISC2 file
LINE=FNAME(:I)//'.ISC2'
CALL IMOD_UTL_FILENAME(LINE)
IF(ISGTYPE.EQ.4)CALL IMOD_UTL_OPENIDF(ISGIU(6),LINE,'R',12)
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(6),LINE,'R',24)
!## open *.IST1 file
LINE=FNAME(:I)//'.IST1'
CALL IMOD_UTL_FILENAME(LINE)
IF(ISGTYPE.EQ.4)CALL IMOD_UTL_OPENIDF(ISGIU(7),LINE,'R',44)
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(7),LINE,'R',48)
!## open *.IST2 file
LINE=FNAME(:I)//'.IST2'
CALL IMOD_UTL_FILENAME(LINE)
IF(ISGTYPE.EQ.4)CALL IMOD_UTL_OPENIDF(ISGIU(8),LINE,'R',12)
IF(ISGTYPE.EQ.8)CALL IMOD_UTL_OPENIDF(ISGIU(8),LINE,'R',20)
...
DO I=1,NISGH
 ...
 IREC=ISEG-1
 DO J=1,NSEG
  IF(ISGTYPE.EQ.4)THEN
   READ(ISGIU(2),REC=IREC+J+ICF) XS,YS
   X(J)=XS; Y(J)=YS
  ELSE
   READ(ISGIU(2),REC=IREC+J+ICF) X(J),Y(J)
  ENDIF
 END DO
 ...
 IF(MAXVAL(X(1:NSEG)).GT.SIMBOX(1).AND.MAXVAL(Y(1:NSEG)).GT.SIMBOX(2).AND. &
  ...
  CALL PCK7RPISG(ICLC,ISGIU(3),NCLC,NXY,NSEG,X,Y,DIST,IPOS,+1,ISGTYPE)
  !## include structure nodes in between segments
  CALL PCK7RPISG(ISTW,ISGIU(7),NSTW,NXY,NSEG,X,Y,DIST,IPOS,-1,ISGTYPE)
  ...
  DO
   ...
   IF(IPOS(J).GT.0)THEN
    ...
    CALL PCK2RPISG(SDATE,EDATE,TTIME,QSORT,XNR,ISGIU(4),MAXITEMS,RVAL(1,J),NR,IREF,I,1,NODATA,ISGTYPE)
   ENDIF
   IF(IPOS(J).LT.0)THEN
    READ(ISGIU(7),REC=ABS(IPOS(J))+ICF) NR,IREF
    CALL PCK2RPISG(SDATE,EDATE,TTIME,QSORT,XNR,ISGIU(8),MAXITEMS,RVAL(1,J),NR,IREF,I,-1,NODATA,ISGTYPE)
    J=J+1
  ...
  IF(LNODAT)THEN
   ...
   DO J=1,NCRS
    IF(ISGTYPE.EQ.4)THEN
     READ(ISGIU(5),REC=IREC+J+ICF) DWP(J)%NCROS,IREF,DS
     DWP(J)%DIST=REAL(DS,8)
    ELSE
    ...
    DO ICROS=1,DWP(J)%NCROS
     JREC=JREC+1
     IF(ISGTYPE.EQ.4)THEN
      READ(ISGIU(6),REC=JREC+ICF) DS,BS,KS
      CROS(J,ICROS)%DIST=  REAL(DS,8)
      CROS(J,ICROS)%BOTTOM=REAL(BS,8)
      CROS(J,ICROS)%KM=    REAL(KS,8)
     ELSE
       ...
       IF(ICOL.GE.1.AND.IROW.GE.1.AND. &
        ...
        IF(WETPER.GT.0.0)THEN
         ...
         ENDIF
          ...
          ISGDATA(NISG,5) = LN(J)*WETPER/C                !CONDUCTANCES
          ISGDATA(NISG,6) = BH
          ISGDATA(NISG,7) = RVAL(4,ISEG-1)+DXY*RVAL(4,0)  !INF.FACTORS

          ! NECESSARY FOR IFVDL=1
          ISGDATA(NISG,8)  = LN(J)
          ISGDATA(NISG,9)  = WETPER
          ISGDATA(NISG,10) = C
          ISGDATA(NISG,11) = RWIDTH
...
ENDDO
 ...
 IDF%XMIN=SIMBOX(1); IDF%YMIN=SIMBOX(2); IDF%XMAX=SIMBOX(3); IDF%YMAX=SIMBOX(4); IDF%NODATA=NODATAIDF
 IDF%IXV=0; IDF%ITB=0; IDF%NCOL=NCOL; IDF%NROW=NROW
 IF(LQD)THEN
  IDF%DX=SIMCSIZE; IDF%IEQ=0; IDF%DY=IDF%DX
 ELSE
  IDF%DX=HUGE(1.0); DO I=1,NCOL; IDF%DX=MIN(DELR(I)-DELR(I-1),IDF%DX); ENDDO
  IDF%DY=HUGE(1.0); DO I=1,NROW; IDF%DY=MIN(DELC(I-1)-DELC(I),IDF%DY); ENDDO
  IDF%IEQ=1
  DO I=1,SIZE(IDF)
   ALLOCATE(IDF(I)%SX(0:NCOL),IDF(I)%SY(0:NROW))
   DO J=0,NCOL; IDF(I)%SX(J)=DELR(J); ENDDO
   DO J=0,NROW; IDF(I)%SY(J)=DELC(J); ENDDO
  ENDDO
 ENDIF
 ...
 DO I=1,NISG
  IROW=isgdata(I,2); ICOL=isgdata(I,3)
  !## skip nodata
  IF(isgdata(I,5).EQ.NODATA)CYCLE
  IDF(1) %X(ICOL,IROW)=IDF(1) %X(ICOL,IROW)+ isgdata(I,5)                !## conductance
  IDF(2) %X(ICOL,IROW)=IDF(2) %X(ICOL,IROW)+(isgdata(I,4)*isgdata(I,5))  !## wl
  IDF(3) %X(ICOL,IROW)=IDF(3) %X(ICOL,IROW)+(isgdata(I,6)*isgdata(I,5))  !## bh
  IDF(4) %X(ICOL,IROW)=IDF(4) %X(ICOL,IROW)+(isgdata(I,7)*isgdata(I,5))  !## inf
  IDF(5) %X(ICOL,IROW)=IDF(5) %X(ICOL,IROW)+(isgdata(I,10)*isgdata(I,5)) !## c
  IDF(7) %X(ICOL,IROW)=IDF(7) %X(ICOL,IROW)+(isgdata(I,11)*isgdata(I,5)) !## rwidth
  IDF(9) %X(ICOL,IROW)=IDF(9) %X(ICOL,IROW)+ isgdata(I,8)                !## total length
  IDF(10)%X(ICOL,IROW)=IDF(10)%X(ICOL,IROW)+(isgdata(I,9)*isgdata(I,5))  !## wetper
 ENDDO
 ...
 IF(N2DIM.GT.0)THEN 
  ...
  DO I=1,NISGH
   ...
   DO J=1,NCRS
    IF(ISGTYPE.EQ.4)THEN
     READ(ISGIU(5),REC=IREC+J+ICF) DWP(J)%NCROS,IREF,DS,CNAME
     DWP(J)%DIST=REAL(DS,8)
    ELSE
    ...
    CALL UTL_IDFIROWICOL(IDF(1),IROW,ICOL,ISGX,ISGY)               !## get location in raster
...
IRLOOP: DO IR=MAX(1,IROW-1),MIN(NROW,IROW+1)
 ...
 DO IROW=1,NROW; DO ICOL=1,NCOL
  IF(IDF(1)%X(ICOL,IROW).GT.0.0)THEN
   ...
   isgdata(NISG,2) = IROW
   isgdata(NISG,3) = ICOL
   isgdata(NISG,5)= IDF(1)%X(ICOL,IROW)  ! cond
   isgdata(NISG,4)= IDF(2)%X(ICOL,IROW)  ! stage
   isgdata(NISG,6)= IDF(3)%X(ICOL,IROW)  ! rbot
   isgdata(NISG,7)= IDF(4)%X(ICOL,IROW)  ! inf.
   isgdata(NISG,8)= IDF(9)%X(ICOL,IROW)  ! length
   isgdata(NISG,9)= IDF(10)%X(ICOL,IROW) ! wettedperimeter
   isgdata(NISG,10)=IDF(5)%X(ICOL,IROW)  ! resis
...
SUBROUTINE PCK2RPISG(SDATE,EDATE,TTIME,QSORT,XNR,LU,MAXITEMS,RVAL, &
                     NR,IREF,IISGH,ITYPE,NODATA,ISGTYPE)
!###====================================================================
...
INTEGER,INTENT(IN) :: SDATE,EDATE,TTIME,MAXITEMS,LU,IISGH,NR,IREF,ITYPE,ISGTYPE
REAL(KIND=8),INTENT(INOUT),DIMENSION(TTIME,MAXITEMS) :: QSORT
REAL(KIND=8),DIMENSION(MAXITEMS),INTENT(INOUT) :: RVAL,XNR
REAL(KIND=4),DIMENSION(MAXITEMS) :: SVAL
...
IF(NR.GT.0)THEN
 ...
 DO 
  ...
  IF(ITYPE.EQ. 1)THEN
   IF(ISGTYPE.EQ.4)THEN
    READ(LU,REC=IREC+ICF) IDATE,(SVAL(I),I=1,MAXITEMS)
    DO I=1,MAXITEMS; RVAL(I)=REAL(SVAL(I),8); ENDDO
   ELSE
    READ(LU,REC=IREC+ICF) IDATE,(RVAL(I),I=1,MAXITEMS)
   ENDIF
  ENDIF
  IF(ITYPE.EQ.-1)THEN
   IF(ISGTYPE.EQ.4)THEN
    READ(LU,REC=IREC+ICF) IDATE,(SVAL(I),I=1,2)
    DO I=1,2; RVAL(I)=REAL(SVAL(I),8); ENDDO
   ELSE
    READ(LU,REC=IREC+ICF) IDATE,(RVAL(I),I=1,2)
   ENDIF
  ENDIF
...
REAL(KIND=8),INTENT(IN) :: BH,WP
REAL(KIND=8),INTENT(OUT) :: RWIDTH,WETPER
INTEGER :: ICROS
REAL(KIND=8) ::  DISTB,DISTW,FACTOR,X1,X2,WPCOR
...
REAL(KIND=8),INTENT(IN),DIMENSION(NXY) :: X,Y
REAL(KIND=8),INTENT(OUT),DIMENSION(NXY) :: DIST
INTEGER :: ISEG
REAL(KIND=8) ::  DXY
...
!###====================================================================
SUBROUTINE PCK7RPISG(ITYP,LU,NTYP,NXY,NSEG,X,Y,DIST,IPOS,ITYPE,ISGTYPE)
!###====================================================================
USE IMOD_UTL, ONLY: ICF
IMPLICIT NONE
INTEGER,INTENT(IN) :: NXY,ITYP,LU,NTYP,ITYPE,ISGTYPE
INTEGER,INTENT(INOUT) :: NSEG
REAL(KIND=8),INTENT(INOUT),DIMENSION(NXY) :: X,Y
REAL(KIND=8),INTENT(INOUT),DIMENSION(NXY) :: DIST
INTEGER,INTENT(OUT),DIMENSION(NXY) :: IPOS
INTEGER :: IREC,ISEG,IREF,I,J,II
REAL(KIND=8) ::  DXY,D1,D2,F,XC,YC,SDIST
REAL(KIND=4) :: DS
...
DO I=1,NTYP
 ...
 IREC=IREC+1
 IF(ISGTYPE.EQ.4)THEN
  READ(LU,REC=IREC+ICF) J,IREF,DS
  SDIST=REAL(DS,8)
 ELSE
  READ(LU,REC=IREC+ICF) J,IREF,SDIST
 ENDIF
 DXY=SDIST  !k=number,dxy=distance from beginning
...
!###====================================================================
USE IMOD_UTL, ONLY : IMOD_UTL_PRINTTEXT,IMOD_UTL_DTOS
USE MOD_BASVAR, ONLY : DELR,DELC
USE GLOBAL, ONLY: CC, CV, BOTM, LBOTM, KDSV
USE GWFRIVMODULE, ONLY: IFVDL,ISFT,SFT
IMPLICIT NONE
INTEGER, INTENT(IN) :: NISG
REAL(KIND=8),DIMENSION(NISG,11),INTENT(INOUT) :: ISGLIST
REAL(KIND=8) ::  LEKCONDUCTANCE_Y
INTEGER :: I, ILAY, IROW, ICOL
REAL(KIND=8) ::  DXY,LI,BIN,C0,C1,KH,KV,COND_IN,FCT,D
...
DO I = 1, NISG
 ...
 IF(C1.GT.0.0)THEN
  ...
  IF(KH.LE.0.0D0)CALL IMOD_UTL_PRINTTEXT('Error KH='//TRIM(IMOD_UTL_DTOS(KH,'F',7)),2)
  IF(D .LE.0.0D0)CALL IMOD_UTL_PRINTTEXT('Error D ='//TRIM(IMOD_UTL_DTOS(D ,'F',7)),2)
  ...
  IF(ISGLIST(I,7).GT.1.0.OR.ISGLIST(I,7).LT.0.0)THEN
   CALL IMOD_UTL_PRINTTEXT('Error C0/FCT    ='//TRIM(IMOD_UTL_DTOS(C0/FCT,'F',7)),0)
   CALL IMOD_UTL_PRINTTEXT('Error COND_INF  ='//TRIM(IMOD_UTL_DTOS(COND_IN,'F',7)),0)
   CALL IMOD_UTL_PRINTTEXT('Error C0        ='//TRIM(IMOD_UTL_DTOS(C0,'F',7)),0)
   CALL IMOD_UTL_PRINTTEXT('Error COND_DRN  ='//TRIM(IMOD_UTL_DTOS(ISGLIST(NISG,5),'F',7)),0)
   CALL IMOD_UTL_PRINTTEXT('Error Inf.Factor='//TRIM(IMOD_UTL_DTOS(ISGLIST(NISG,7),'F',7)),0)
  ENDIF
 ELSE
  ISGLIST(I,5)=0.0
 ENDIF
...
!###===============================================================================
 ...
 DO I=2,ABS(DWP(J)%NCROS)
  CALL UTL_IDFIROWICOL(IDF,IROW,ICOL,CROS(J,I)%DIST,CROS(J,I)%BOTTOM) 
 ...
 DO IROW=1,ICROSS%NROW; DO ICOL=1,ICROSS%NCOL
  ...
  !## all okay - continue
  CALL UTL_IDFGETLOC(ICROSS,IROW,ICOL,XC,YC)
  CALL UTL_IDFIROWICOL(IDF(1),JROW,JCOL,XC,YC)
 ...
 !###====================================================================
 SUBROUTINE ISG2GRID_EXTENT_WITH_WIDTH(NIDF,IDF,MAXWIDTH)
 !###====================================================================
 USE IMOD_IDF_PAR
 USE IMOD_IDF, ONLY : IDFGETAREA
 IMPLICIT NONE
 REAL(KIND=8),PARAMETER :: FDX=1.5D0 !## apply extent whenever width is 1.5*dx cell
 INTEGER,INTENT(IN) :: NIDF
 REAL(KIND=8),INTENT(IN) :: MAXWIDTH
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(NIDF) :: IDF
 INTEGER :: IROW,ICOL,N,NN,IR,IC,IRR,ICC,I,IRAT,IRAT1,NRE,NCE
 REAL(KIND=8) ::  W,L,X,Y,X1,Y1,F,A
 ...
 DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
  ...
  IF(IDF(6)%X(ICOL,IROW).NE.0.0)THEN
   IDF(6)%X(ICOL,IROW)=-1.0*IDF(6)%X(ICOL,IROW); CYCLE
  ENDIF

  !## get aggregation (erosion-matrix) pointer to one for all initially
  IF(IDF(1)%X(ICOL,IROW).NE.IDF(1)%NODATA)IDF(6)%X(ICOL,IROW)=1.0
 ...
 DO IROW=1,IDF(1)%NROW; DO ICOL=1,IDF(1)%NCOL
  ...
  IF(IDF(6)%X(ICOL,IROW).LT.0.0)CYCLE
  !## river available
 ...
 !###====================================================================
 USE IMOD_IDF_PAR
 USE IMOD_IDF, ONLY : IDFGETAREA,IDFGETEDGE
 USE IMOD_UTL, ONLY : UTL_IDFIROWICOL,UTL_IDFGETLOC
 IMPLICIT NONE
 INTEGER,INTENT(IN) :: ICOL,IROW,NIDF
 TYPE(IDFOBJ),INTENT(INOUT),DIMENSION(NIDF) :: IDF
 REAL(KIND=8),INTENT(IN) :: W,A
 INTEGER :: I,J,K,IC,IR,IC1,IC2,IR1,IR2
 REAL(KIND=8) ::  X,Y,X1,X2,Y1,Y2,TRAD,C,F,DX,DY
 REAL(KIND=8),DIMENSION(9) :: XP,YP
  
 !## half of the river width
 TRAD=W/2.0

 !## get current location
 CALL UTL_IDFGETLOC(IDF(1),IROW,ICOL,X,Y)
 !## get window for brush-function
 CALL UTL_IDFIROWICOL(IDF(1),IR2,IC1,X-TRAD,Y-TRAD)
 CALL UTL_IDFIROWICOL(IDF(1),IR1,IC2,X+TRAD,Y+TRAD)
 ...
 DO IR=IR1,IR2
  DO IC=IC1,IC2
   ...
   IF(IDF(6)%X(IC,IR).LT.0.0.OR.IDF(6)%X(IC,IR).GE.1.0)CYCLE
   ...
   IF(K.GT.0)THEN
    ...
    IF(F.EQ.1.0.OR.F.GT.IDF(6)%X(IC,IR))THEN
     IDF(6)%X(IC,IR)=F
