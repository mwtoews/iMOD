...
MX=1000; ALLOCATE(XA(MX),YA(MX),LN(MX))
    ...
    IF(DXY.GT.0.0)THEN
     ...
     DO
      IF(LQD)THEN
       ...
       IF(IMOD_UTL_INTERSECT_EQUI(DELR(0),DELR(NCOL),DELC(NROW),DELC(0),SIMCSIZE,X1,X2,Y1,Y2,MX,N,XA,YA,LN))EXIT
      ELSE
       IF(IMOD_UTL_INTERSECT_NONEQUI(DELR,DELC,NROW,NCOL,X1,X2,Y1,Y2,MX,N,XA,YA,LN))EXIT
      ENDIF
      DEALLOCATE(XA,YA,LN); MX=2*MX
...
IF(N2DIM.GT.0 .and. iact.gt.0)THEN
 ...
 ALLOCATE(IDF(6)); DO I=1,SIZE(IDF); CALL IDFNULLIFY(IDF(I)); ENDDO
